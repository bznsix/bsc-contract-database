...
contract PulsinatorBondsFarm is ReentrancyGuard {
    ...
    uint public referralPercentage = 250; // 2.5% Referral Percentage
    ...
    function fundTokenBonds(uint _amount) external onlyOwner {
        ...
        tokensForBondsSupply += _amount * 10 ** 18;
    ...
    function defundTokenBonds(uint _amount) external onlyOwner {
        require(_amount > 0, "Invalid Amount");
        require(
            _amount * 10 ** 18 <= tokensForBondsSupply,
            "Not Enough Tokens Supply"
        );
        tokensForBondsSupply -= _amount * 10 ** 18;
        IERC20(TOKEN).transfer(msg.sender, _amount * 10 ** 18);
    }
    ...
    function setPoolRewards(
        ...
        if (block.timestamp >= poolEndTime) {
            rewardRate = (_amount * 10 ** 18) / poolDuration;
        } else {
            uint remainingRewards = (poolEndTime - block.timestamp) *
                rewardRate;
            rewardRate = (_amount * 10 ** 18 + remainingRewards) / poolDuration;
        }
