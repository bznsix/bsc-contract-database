...
contract KinderBondsFarm is ReentrancyGuard {
    ...
    uint public poolDuration = 90 days;
    ...
    uint public referralPercentage = 250; // referral percentage is 2.5%
    ...
    function purchaseTokenBond(
        ...
        totalTokenOwed += tokenHalfOfBond;
    ...
    function redeemTokenBond() external nonReentrant {
    ...
    function _beanToToken(uint _amount) private returns (uint) {
        ...
        (uint bnbReserves, uint tokenReserves, ) = IPancakePair(lpContract)
            .getReserves();
    ...
    function fundTokenBonds(uint _amount) external onlyOwner {
        ...
        tokensForBondsSupply += _amount * 1e18;
        IERC20(TOKEN).transferFrom(msg.sender, address(this), _amount * 1e18);
    }

    function defundTokenBonds(uint _amount) external onlyOwner {
        require(_amount > 0, "Invalid Amount");
        require(
            _amount * 1e18 <= tokensForBondsSupply,
            "Not Enough Tokens Supply"
        );
        tokensForBondsSupply -= _amount * 1e18;
        IERC20(TOKEN).transfer(msg.sender, _amount * 1e18);
    }
    ...
    function setPoolRewards(
        ...
        if (block.timestamp >= poolEndTime) {
            rewardRate = (_amount * 1e18) / poolDuration;
        } else {
            uint remainingRewards = (poolEndTime - block.timestamp) *
                rewardRate;
            rewardRate = (_amount * 1e18 + remainingRewards) / poolDuration;
        }
    ...
    function emergencyRecoverTokens(
        ...
        require(_amount > 0, "Invalid Amount");
        IERC20(tokenAddress).transfer(msg.sender, _amount * 1e18);
    }
