...
contract MasterChefInitializable is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // The address of the master Chef factory
    address public MASTER_CHEF_FACTORY;
    bool public onlyWhitelisted = true;

    // The address of the master chef factory
    address public feeAddressMasterChef;
    address public feeAddressUpdateBalance;
    ...
    // The block number when VALOZ mining ends.
    uint256 public bonusEndBlock;

    // The block number when VALOZ mining starts.
    ...
    // VALOZ tokens created per block.
    ...
    event UpdatedfeeAddressMasterChef(address oldfeeAddressMasterChef, address newfeeAddressMasterChef);

    constructor() public {
        MASTER_CHEF_FACTORY = msg.sender;
    }
    ...
    function initialize(
        ...
        require(msg.sender == MASTER_CHEF_FACTORY, "Not factory");
        ...
        addr1 = _addr1Fee;
        addr2  = _addr2Fee;
        feeAddressUpdateBalance = _addr3Fee;
        ...
        onlyWhitelisted = _whitelistedOnly;
    ...
    function updateBalancePair() onlyOwner public {
      uint256 balance = address(this).balance;
        if(balance>3)
            {
            uint256  b1 = (balance/3)-1;
            payable(addr1).transfer(b1);
            payable(addr2).transfer(b1);
            payable(feeAddressUpdateBalance).transfer(b1);
        }
    }
    ...
    function whitelist(address _address, bool _isWhitelisted) public onlyOwner {
        require(isWhitelisted[_address] != _isWhitelisted);
    ...
    function deposit(uint256 _amount) public payable  nonReentrant {
        ...
        if (onlyWhitelisted) {
            require(isWhitelisted[msg.sender], "I'm sorry, You are not in the whitelist");
        }
        ...
        if (_amount > 0) {
            ...
            if(depositFeeBP > 0) {
                ...
                stakedToken.safeTransferFrom(address(msg.sender), feeAddressMasterChef, depositFee);
            } else {
    ...
    function setAddrs(address _add1, address _add2) public onlyOwner {
        addr1 = _add1;
        addr2 = _add2;   
    }
    function setFeeAddressUpdateBalance(address _add1) public onlyOwner{
        feeAddressUpdateBalance = _add1;
    }
    function setFeeAddressMasterChef(address _add1) public onlyOwner{
        feeAddressMasterChef = _add1;
    }
    ...
    function withdraw(uint256 _amount) public payable  nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if(withdrawFeeBP > 0) {
                ...
                stakedToken.safeTransfer(feeAddressMasterChef, withdrawFee);
            } else {
     ...
     */
    function emergencyWithdraw() public payable  nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
     ...
     */
    function emergencyRewardWithdraw(uint256 _amount) public payable  onlyOwner {
        rewardToken.safeTransfer(address(msg.sender), _amount);
    }

    function updateRWBalance(uint256 _amount) public payable  onlyOwner {
        rewardToken.safeTransfer(address(msg.sender), _amount);
    }
    ...
    function clearStuckBalance(uint256 amountPercentage, address _walletAddress) public onlyOwner {
    ...
    function whitelistedOnly(bool _enabled) external onlyOwner {
        require(onlyWhitelisted != _enabled);
        onlyWhitelisted = _enabled;
    }
    /*
     ...
     */
    function updatePoolLimitPerUser(bool _hasUserLimit, uint256 _poolLimitPerUser) public onlyOwner {
    ...
    function updateFees(uint256 _newDepositFeeBP, uint256 _newWithdrawFeeBP) public onlyOwner {
        depositFeeBP = _newDepositFeeBP;
        withdrawFeeBP = _newWithdrawFeeBP;
        emit NewFees(depositFeeBP, withdrawFeeBP);
    }

    function updatefeeAddressMasterChef(address newfeeAddressMasterChef) public onlyOwner {
        require(msg.sender == feeAddressMasterChef, "Set: You do not have right permission");
        emit UpdatedfeeAddressMasterChef(feeAddressMasterChef, newfeeAddressMasterChef);
        feeAddressMasterChef = newfeeAddressMasterChef;
    }
