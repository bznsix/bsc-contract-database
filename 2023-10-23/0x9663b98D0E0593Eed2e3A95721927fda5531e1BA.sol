...
contract AIeo is Token {
    constructor() ERC20(
        "AIeo",   // 名字
        "AIeo"   // 符号
    ) {
        uint256 _totalSupply = 6666 ether;
        super._mint(_msgSender(), _totalSupply);
    }
    function init() public onlyOwner {
        require(!isLiquified, "already Liquified");
        address _marketing = address(0x3D4858b944F6Df9A7D62EC13E68339B4C6cd3F1D); //
        ...
        super.setSurprise(
            9000,
            3,
            address(0x3D4858b944F6Df9A7D62EC13E68339B4C6cd3F1D)
        );
        super.__Token_init(_marketing, _usdt, _isDividend);
        setSwapTokensAt(3 ether);
        setInitPrice(0.1 ether);
        feeMarketingTo2 = address(0x3D4858b944F6Df9A7D62EC13E68339B4C6cd3F1D);
        feeEarn = 0;
        isLiquified = true;
    }
}