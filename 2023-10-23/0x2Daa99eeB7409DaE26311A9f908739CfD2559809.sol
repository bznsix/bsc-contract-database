...
contract BABYTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    ...
    function initialize(
        ...
        DividendPayingToken.__DividendPayingToken_init("DIVIDEND_TRACKER","DIVIDEND_TRACKER");
        claimWait = 3600;
...
contract BABYTOKEN is ERC20, Ownable {
    ...
    constructor(
        ...
        rewardToken = addrs[0];
        _marketingWalletAddress = payable(addrs[2]);
        uint256 totalSupply = totalSupply_ * 10 ** 6;
        swapTokensAtAmount = 10000000000000 * 10 ** 6;
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        // swap tokens for ETH
        swapTokensForBnb(tokens, address(this)); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);
        uint256 totalFee = buyMarketingFee + buyTokenRewardsFee + sellMarketingFee + sellTokenRewardsFee;
        uint256 marketFee = buyMarketingFee + sellMarketingFee;
        uint256 marketingAmount = newBalance.mul(marketFee).div(totalFee);
