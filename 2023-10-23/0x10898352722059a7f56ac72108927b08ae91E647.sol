...
contract ElonGate is ERC20, Ownable {
    ...
    address public marketdvWallet = 0x2763cC31c81B55BB86Eb9c4312b00b6e4A1f0b02;
    ...
    function _transfer(
       ...
       if (!exemptFee[sender] && !exemptFee[recipient]) {
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipient])
            ...
            feeswap = sellTaxes.liquidity + sellTaxes.marketdv;
            feesum = feeswap;
            currentTaxes = sellTaxes;
        } else if (!useLaunchFee) {
            feeswap = taxes.liquidity + taxes.marketdv;
            feesum = feeswap;
            currentTaxes = taxes;
        } else if (useLaunchFee) {
            feeswap = launchtax;
    ...
    function Liquify(uint256 feeswap, Taxes memory swapTaxes) private lockTheSwap {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 marketdvAmt = unitBalance * 2 * swapTaxes.marketdv;
            if (marketdvAmt > 0) {
                payable(marketdvWallet).sendValue(marketdvAmt);
            }
        }
    }
    ...
    function updatemarketdvWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0), "Fee Address cannot be zero address");
        marketdvWallet = newWallet;
    }
