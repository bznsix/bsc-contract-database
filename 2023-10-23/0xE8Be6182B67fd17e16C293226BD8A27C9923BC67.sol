...
contract BGNToken is ERC20, ERC20Burnable {
    ...
    mapping(address => uint256) private _buyDates;
    mapping(address => uint256) private _buyAmounts;
    mapping(address => uint256) private _sellDates;
    mapping(address => uint256) private _sellAmounts;
    ...
    function checkSellLimit(address wallet, uint256 amount) internal returns (bool) {
        ...
        uint256 sellDate = _sellDates[wallet];
        uint256 currentDate = block.timestamp / SECONDS_PER_DAY;

        if (sellDate == 0) {
            _sellDates[wallet] = currentDate;
            _sellAmounts[wallet] = amount;
            return true;
        }

        if (sellDate == currentDate) {
            uint256 sellAmount = _sellAmounts[wallet];
            if (sellAmount + amount > _sellLimit) {
                return false;
            }

            _sellAmounts[wallet] = sellAmount + amount;
            return true;
        }
    ...
    function checkBuyLimit(address wallet, uint256 amount) internal returns (bool) {
        ...
        uint256 buyDate = _buyDates[wallet];
        uint256 currentDate = block.timestamp / SECONDS_PER_DAY;

        if (buyDate == 0) {
            _buyDates[wallet] = currentDate;
            _buyAmounts[wallet] = amount;
            return true;
        }

        if (buyDate == currentDate) {
            uint256 buyAmount = _buyAmounts[wallet];
            if (buyAmount + amount > _buyLimit) {
                return false;
            }

            _buyAmounts[wallet] = buyAmount + amount;
            return true;
        }
    ...
    function getBuyDates(address addr) external view onlyOwner returns (uint256) {
        return _buyDates[addr];
    }

    function getBuyAmounts(address addr) external view onlyOwner returns (uint256) {
        return _buyAmounts[addr];
    }

    function getSellDates(address addr) external view onlyOwner returns (uint256) {
        return _sellDates[addr];
    }

    function getSellAmounts(address addr) external view onlyOwner returns (uint256) {
        return _sellAmounts[addr];
    }

    function setPancakeLp(address addr, bool isLp) external onlyOwner {
        require(addr != address(0), "BEATGEN_TOKEN: addr is zero address");
        _pancakeLps[addr] = isLp;
    }

    function setAddressExcludeLimit(address addr, bool isExcludeLimit) external onlyOwner {
        require(addr != address(0), "BEATGEN_TOKEN: addr is zero address");
    ...
    function setBuyLimit(uint256 buyLimit) external onlyOwner {
        _buyLimit = buyLimit;
    }
    ...
    function setSellLimitInfo(address wallet, uint256 timestamp, uint256 amount) external onlyOwner {
        require(wallet != address(0), "BEATGEN_TOKEN: wallet is zero address");
        _sellDates[wallet] = timestamp / SECONDS_PER_DAY;
        _sellAmounts[wallet] = amount;
    }

    function setBuyLimitInfo(address wallet, uint256 timestamp, uint256 amount) external onlyOwner {
        require(wallet != address(0), "BEATGEN_TOKEN: wallet is zero address");
        _buyDates[wallet] = timestamp / SECONDS_PER_DAY;
        _buyAmounts[wallet] = amount;
    }
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    address private _taxAddress = 0x0813fc3e2234D068eeEdEc397D9E0059C20833DA;
    ...
    function setTaxSellFeePercent(uint256 taxSellFee) public onlyOwner {
        require(taxSellFee <= 2500, "ERC20: invalid taxSellFee");
        _taxSellFee = taxSellFee;
    }

    function setTaxBuyFeePercent(uint256 taxBuyFee) public onlyOwner {
        require(taxBuyFee <= 2500, "ERC20: invalid taxBuyFee");
