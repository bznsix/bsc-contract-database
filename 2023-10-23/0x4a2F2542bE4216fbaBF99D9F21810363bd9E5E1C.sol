...
contract GmCats is Owned(msg.sender), ERC721A {
  uint256 public constant maxSupply = 500;
  uint256 public constant maxPerWallet = 5;
  uint256 public constant price = 0.001 ether;
  uint8 public saleState = 1;
  uint8 public baseURILocked = 1;
  string private baseURI;

  constructor() ERC721A("GM Cats", "GMCat") {}

  function mint(uint256 quantity) external payable {
    if (saleState == 1) revert SaleClosed();
    if (msg.value != price * quantity) revert InsufficientETH();
  ...
  function setBaseURI(string calldata newURI) external onlyOwner {
    if (baseURILocked == 2) revert BaseURILocked();
    baseURI = newURI;
  }
