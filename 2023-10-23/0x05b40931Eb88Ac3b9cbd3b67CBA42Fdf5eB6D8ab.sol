...
contract Micky is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MickyDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Micky Rat", "Micky") {
        marketingWallet = payable(0x2A43E7D1526895059283903235d9EDa303CEE427);
        devWallet = payable(0x1A9268973C8bD6A743234B51821E430ca8F62568);
        ...
        dividendTracker = new MickyDividendTracker(
            payable(this),
            router,
        ...
        swapTokensAtAmount = totalTokenSupply / 100; // 0.10%;
    ...
    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 0, "Protect 2");
    ...
    function updateFees(
        ...
        require(totalSellFees <= 10 && totalBuyFees <= 10, "total");
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (!isBuying && !isSelling) {
                ...
                if (limitsInEffect) {
                require(block.number > launchblock + 1,"So sorry. Please try again.");
...
contract MickyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        MickyContract = Micky(mainContract);
        minimumTokenBalanceForAutoDividends = 20000_000000000000000000; // 20.000 tokens
    ...
    function _transfer(
        ...
        require(false, "Micky_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Micky_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Micky contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(MickyContract);

            uint256 prevBalance = MickyContract.balanceOf(address(this));
            ...
            {
                uint256 received = MickyContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MickyContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
