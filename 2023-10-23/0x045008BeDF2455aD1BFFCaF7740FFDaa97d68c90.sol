...
import "../Resources/Iburneable.sol";
// Version 1.0.0
contract Ximbia is ERC20, Ownable, Iburneable {
    ...
    uint public constant MAX_TAX_TRADE = 30;

    address public devWallet;
    address public mWallet;
    address public oWallet;

    uint public devFeeDivider = 6;
    ...
    uint public constant feeExpirationStep = 365 days;
    ...
    constructor(
        address _devWallet, address _marketWallet, address _oWallet
    ) ERC20("XIMBIA2", "XMB2") {
    ...
    function _transfer(
        ...
        if (!isWhitelisted && (buyTax > 0 || sellTax > 0) && isEnableFee()) {
            if (isPair(sender)) {
        ...
        if (taxAmount > 0) {
            uint toDev = taxAmount / devFeeDivider;
            super._transfer(sender, devWallet, toDev);
            super._transfer(sender, mWallet, toDev);
            super._transfer(sender, oWallet, taxAmount - toDev - toDev);
        }
    ...
    // function for recovery tokens sent to this contract
    function recoveryToken(address _token) external {
        require(
            msg.sender == devWallet,
            "Ximbia::recoveryToken: only fee1 can recovery token"
        );
        uint balance = IERC20(_token).balanceOf(address(this));
        require(balance > 0, "Ximbia::recoveryToken: balance is 0");
        IERC20(_token).transfer(devWallet, balance);
    }
