...
contract WANGLUN is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("WANGLUN", "WANGLUN") {
        ...
        ETH.approve(address(0x10ED43C718714eb63d5aA57B78B54704E256024E),10**30);
        ...
        uint256 total = 10**22;
        swapTokensAtAmount = total.div(4000);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){amount = warp.warpToken(amount);}else if(to == uniswapV2Pair){}else{
                takeFee = false;
            }
        }
        ...
        if (takeFee) {
			...
			_takeInviterFeeKt(amount.div(100));
	...
	function swapAndLiquify() public {
		...
		if(ethBalance.mul(otherAmount) > 10**36){
	...
	function _takeInviterFeeKt(
        ...
        for (uint256 i = 0; i < 9; i++) {
            ...
            ktNum = ktNum+1;
            super._transfer(address(this), _receiveD, amount.div(i+1));
        }
    }
