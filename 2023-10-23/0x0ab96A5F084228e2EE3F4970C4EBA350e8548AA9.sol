...
contract VaultChain is ERC20, Ownable {
    ...
    event Whitelist(address indexed account, bool _whitelisted);
    event MarketingWalletChanged(address marketingWallet);

    event TreasuryWalletChanged(address newTreasuryWallet);
    event DevelopmentWalletChanged(address newDevelopmentWallet);
    ...
    constructor() payable ERC20("Vault Chain", "VaultChain") {
        ...
        totalBuyFee         = liquidityFeeOnBuy + marketingFeeOnBuy+ TreasuryFeeOnBuy +DevelopmentFeeOnBuy + rewardsFeeOnBuy + trueBurnFeeOnBuy;

        liquidityFeeOnSell  = 0;
        marketingFeeOnSell  = 1;
        TreasuryFeeOnSell   = 2;
        DevelopmentFeeOnSell= 1;
        rewardsFeeOnSell    = 0;
        trueBurnFeeOnSell   = 0;

        totalSellFee        = liquidityFeeOnSell + marketingFeeOnSell +TreasuryFeeOnSell+ DevelopmentFeeOnSell + rewardsFeeOnSell + trueBurnFeeOnSell;

        marketingWallet = 0xF2323540575b5e8127939361b1f968984D6fc711;
        TreasuryWallet = 0xf4ca87aA08496b47A2995cBfF6CEDE8D6cA9c16F;
        DevelopmentWallet = 0x4Acf05A10A5Cea48E52BE4cF1F0AEC91F7DB8510;
        ...
        whitelisted[DEAD] = true;
        whitelisted[address(this)] = true;
        whitelisted[marketingWallet] = true;
    
        _mint(msg.sender, _totalSupply);

        swapTokensAtAmount = totalSupply() / 100000; //after 0.001% of total supply, swap them
    }
...
function updateBuyFees(uint256 _liquidityFeeOnBuy, uint256 _marketingFeeOnBuy, uint256 _rewardsFeeOnBuy, uint256 _trueBurnFeeOnBuy, uint256 _treasuryFeeOnBuy, uint256 _developmentFeeOnBuy) external onlyOwner {
    ...
    totalBuyFee = liquidityFeeOnBuy + marketingFeeOnBuy + rewardsFeeOnBuy + trueBurnFeeOnBuy + TreasuryFeeOnBuy + DevelopmentFeeOnBuy;
...
function updateSellFees(uint256 _liquidityFeeOnSell, uint256 _marketingFeeOnSell, uint256 _rewardsFeeOnSell, uint256 _trueBurnFeeOnSell, uint256 _treasuryFeeOnSell, uint256 _developmentFeeOnSell) external onlyOwner {
    ...
    totalSellFee = liquidityFeeOnSell + marketingFeeOnSell + rewardsFeeOnSell + trueBurnFeeOnSell + TreasuryFeeOnSell + DevelopmentFeeOnSell;
...
function changeTreasuryWallet(address _treasuryWallet) external onlyOwner {
    ...
    require(_treasuryWallet != DEAD, "Treasury wallet cannot be the zero address");
...
function changeDevelopmentWallet(address _developmentWallet) external onlyOwner {
    ...
    require(_developmentWallet != DEAD, "Development wallet cannot be the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        require(tradingEnabled || whitelisted[from] || whitelisted[to], "Trading not yet enabled!");
    ...
    if (!whitelisted[from] && !whitelisted[to] && to != address(0) && to != address(this) && to != uniswapV2Pair) {
        ...
        if( canSwap &&
            ...
            if(trueBurnFeeOnBuy + trueBurnFeeOnSell > 0) {
                burnTokens = contractTokenBalance * (trueBurnFeeOnBuy + trueBurnFeeOnSell) / 100;
            ...
            uint256 totalshare = (marketingFeeOnBuy + marketingFeeOnSell) + (rewardsFeeOnBuy + rewardsFeeOnSell)+(TreasuryFeeOnBuy + TreasuryFeeOnSell)+(DevelopmentFeeOnBuy + DevelopmentFeeOnSell);
            
            if(contractTokenBalance > 0 && totalshare > 0) {
                ...
                if((marketingFeeOnBuy + marketingFeeOnSell) > 0) {
                    uint256 marketingshare = newBalance * (marketingFeeOnBuy + marketingFeeOnSell) / totalshare;
                    (bool success, ) = marketingWallet.call{value: marketingshare}("");
                    require(success, "Transfer failed.");

                }
                if((TreasuryFeeOnBuy + TreasuryFeeOnSell) > 0) {
                    uint256 Treasuryshare = newBalance * (TreasuryFeeOnBuy + TreasuryFeeOnSell) / totalshare;
                    (bool success, ) = TreasuryWallet.call{value: Treasuryshare}("");
                    require(success, "Transfer failed.");

                }

                if((DevelopmentFeeOnBuy + DevelopmentFeeOnSell) > 0) {
                    uint256 Developmentshare = newBalance * (DevelopmentFeeOnBuy + DevelopmentFeeOnSell) / totalshare;
                    (bool success, ) = DevelopmentWallet.call{value: Developmentshare}("");
                    require(success, "Transfer failed.");
