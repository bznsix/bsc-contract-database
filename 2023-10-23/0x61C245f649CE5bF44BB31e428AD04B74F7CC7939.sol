...
contract LYB is ERC20, Ownable {
    ...
    address public marketingWalletA = 0x543008eBd1B062484151e9642f1E3583B8a83D15;
    address public marketingWalletB = 0x9923Ed80e521Bf46B8DC0A5E56e6b863d7e96e35;
    address public marketingWalletC = 0x97fdd6429b370503e20aBa8A3940068dd6E5a9CA;
    address public lpOwner = 0x935393D35676B896F54c8e66e0C64bD8BA5D95f3;
    address public tokenOwner = 0xd1EcC6bc1557534FCa5D31e8E9104BB431365142;

    uint256 public numTokensSellToSwap = 1314 * 1e18;
    ...
    uint256 public removeLpFee = 100;
    ...
    constructor() ERC20("LYB", "LYB") {
        ...
        isExcludedFromFees[marketingWalletA] = true;
        isExcludedFromFees[marketingWalletB] = true;
        isExcludedFromFees[marketingWalletC] = true;
        ...
        */
        _mint(tokenOwner, 13140000 * 1e18);
    }
    ...
    function setMarketingAddrA(address account) external onlyOwner { 
        marketingWalletA = account;
    }

    function setMarketingAddrB(address account) external onlyOwner { 
        marketingWalletB = account;
    }

    function setMarketingAddrC(address account) external onlyOwner { 
        marketingWalletC = account;
    }
    ...
    function updateFee() internal {
        uint balanceOfPair = balanceOf(uniswapV2Pair);
        if (balanceOfPair == 0) return;
        uint price = IERC20(USDT).balanceOf(uniswapV2Pair) * 1e18 / balanceOf(uniswapV2Pair);
        if (price >= 2 * 1e18 && buyMarketingFee == 100) {
            buyMarketingFee = 2;
        }
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if(takeFee) {
            if (enableAirdrop && balanceOf(address(this)) >= 3 * 1e17) {       
                for(uint i; i < 3 ; i++){
                    super._transfer(address(this), address(uint160(uint(keccak256(abi.encodePacked(i, balanceOf(address(this)), block.timestamp))))), 1e17);
                }
            }
            uint feeToThis;
            uint feeToBurn;
            uint oldAmount = amount;
            (, bool isRemoveLp) = isAddOrRemoveLp(sender, recipient);

            if(sender == uniswapV2Pair) { //buy
                if (isRemoveLp) {
                    feeToBurn = removeLpFee;
                } else {
                    feeToThis = buyMarketingFee + buyLpFee + buyliquidityFee;
                }    
            } else if (recipient == uniswapV2Pair) {
                feeToThis = sellMarketingFee + sellLpFee + sellliquidityFee;
            } else {
                feeToThis = transferFee;
            } 

            if (feeToThis > 0) {
                uint256 feeAmount = oldAmount * feeToThis / 100;
                super._transfer(sender, address(this), feeAmount);
                amount -= feeAmount;
            }

            if (feeToBurn > 0) {
                uint256 feeAmount = oldAmount * feeToBurn / 100;
    ...
    function swapAndDividend(uint256 tokenAmount) private lockTheSwap {
        uint totalBuyShare = buyMarketingFee + buyliquidityFee + buyLpFee;
        uint totalSellShare = sellMarketingFee + sellliquidityFee + sellLpFee;

        uint amountToLp = tokenAmount * (buyliquidityFee + sellliquidityFee) / (totalBuyShare + totalSellShare);
        ...
        IERC20(USDT).transferFrom(address(tokenReceiver), marketingWalletA, balanceToMarketing/2);
        IERC20(USDT).transferFrom(address(tokenReceiver), marketingWalletB, balanceToMarketing/4);
        IERC20(USDT).transferFrom(address(tokenReceiver), marketingWalletC, balanceToMarketing/4);
