pragma solidity ^0.5.11; //0.5.17

// Heads or tails game contract
contract DegenApeBoyFlip {
    address payable public owner; // Changed to public for read access
    string public name;
    ...
    // Modifier to restrict contracts
    modifier onlyEOA {
    ...
    // Play the game!
    function degenapeboy(uint8 guess) public payable onlyEOA returns (bool) {
        require(guess == 0 || guess == 1, "Variable 'guess' should be either 0 ('heads') or 1 ('tails')");
        require(msg.value > 1000000000000000, "Bet more than 0.001");
        require(msg.value <= address(this).balance * 10 / 100 - msg.value, "You cannot bet more than what is available in the jackpot");
