...
contract ResearchLab is OwnableUpgradeable, ReentrancyGuard {
    ...
    // Mapping for the unlocked allocated rewards per era
    mapping (uint => uint256) public unlockedAllocated;

    // Mapping for the unlocked allocated rewards per era
    mapping (uint => uint256) public unlockedClaimed;
    ...
    function init(
        ...
        unlockedAllocated[0] = 0;
        unlockedAllocated[1] = 2980000000000000000000000;
        unlockedAllocated[2] = 2980000000000000000000000;
        unlockedAllocated[3] = 3352500000000000000000000;
        unlockedAllocated[4] = 3725000000000000000000000;
        unlockedAllocated[5] = 3725000000000000000000000;
        unlockedAllocated[6] = 3352500000000000000000000;
        unlockedAllocated[7] = 2868250000000000000000000;
        unlockedAllocated[8] = 2682000000000000000000000;
        unlockedAllocated[9] = 2346750000000000000000000;
        unlockedAllocated[10] = 2235000000000000000000000;
        unlockedAllocated[11] = 2048750000000000000000000;
        unlockedAllocated[12] = 1788000000000000000000000;
        unlockedAllocated[13] = 1452750000000000000000000;
        unlockedAllocated[14] = 1460200000000000000000000;
        unlockedAllocated[15] = 1452750000000000000000000;
        unlockedAllocated[16] = 1430400000000000000000000;
        unlockedAllocated[17] = 1393150000000000000000000;
        unlockedAllocated[18] = 1341000000000000000000000;
        unlockedAllocated[19] = 707750000000000000000000;
        unlockedAllocated[20] = 149000000000000000000000;
        unlockedAllocated[21] = 43470750000000000000000000;
        ...
        unlockedClaimed[8] = 0;
        unlockedClaimed[9] = 0;
        unlockedClaimed[10] = 0;
        unlockedClaimed[11] = 0;
        unlockedClaimed[12] = 0;
        unlockedClaimed[13] = 0;
        unlockedClaimed[14] = 0;
        unlockedClaimed[15] = 0;
        unlockedClaimed[16] = 0;
        unlockedClaimed[17] = 0;
        unlockedClaimed[18] = 0;
        unlockedClaimed[19] = 0;
        unlockedClaimed[20] = 0;
        unlockedClaimed[21] = 0;
    }
    ...
    // Get the era number
    function getEraNumber(uint256 blockNumber) public view returns(uint256 eraNumber) {
      eraNumber = blockNumber.sub(startBlock).div(blocksPerEra).add(1) > (numberOfEras.add(1)) ? (numberOfEras.add(1)) : blockNumber.sub(startBlock).div(blocksPerEra).add(1);
      return eraNumber; 
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        uint256[22] memory rewardsByEra;
    ...
    function pendingRewardsCalc(uint256 _pid, address _user, uint256 _blockNumber) private returns (uint256[22] memory rewardsByEra) {
    ...
    function pendingRewards(uint256 _pid, address _user) public view returns (uint256 availableUnlockedRewards, uint256 availableLockedRewards, uint256[22] memory rewardsByEra, uint256 adjVested) {
        ...
        if (currentEra <= numberOfEras) {
          ...
          rewardsByEra[21] = (blockMax.sub(blockStart).mul(rewardPerBlock));
          availableUnlockedRewards = availableUnlockedRewards.add(blockMax.sub(blockStart).mul(rewardPerBlock));

          //rewardsByEra = calculateLockedRewardPerEra(blockStart, blockMax, user.vestedAmount.add(availableLockedRewards), rewardsByEra);
    ...
    function calculateRewardPerEra(uint256 _endEra, uint256 _lastRewardBlock, uint256 _blockNumber, uint256 _pid, uint256 _userAmount) private view returns(uint256 unlockedRewards, uint256 lockedRewards, uint256[22] memory rewardsByEra) {
      uint256 endBlock = 0;
      uint256 totalRewards = 0;
      uint256 totalPoolValue = getTotalPoolValueWeighted();
      

      for (uint256 i=getEraNumber(_lastRewardBlock); i<=_endEra;i++) {
        endBlock = _blockNumber < startBlock.add(i.mul(blocksPerEra)) ? _blockNumber : startBlock.add(i.mul(blocksPerEra));
        totalRewards = endBlock.sub(_lastRewardBlock).mul(unlockedAllocated[i].add(unlockedClaimed[i]).div(blocksPerEra));
        //unlockedRewards = unlockedRewards.add(totalRewards.mul(i.mul(5)).div(eraPerecent));
        unlockedRewards = unlockedRewards.add(totalRewards);
        //lockedRewards = lockedRewards.add(totalRewards.mul(eraPerecent.sub(i.mul(5))).div(eraPerecent));
        lockedRewards = lockedRewards.add(totalRewards.div(i.mul(5)).mul(eraPerecent.sub(i.mul(5))));
        //rewardsByEra[i]=(_userAmount.mul(weightPercent).div(weightPercent.sub(poolToken[_pid].poolWeight))).mul(totalRewards.mul(i.mul(5)).div(eraPerecent)).div(totalPoolValue);
        rewardsByEra[i]=(_userAmount.mul(weightPercent).div(weightPercent.sub(poolToken[_pid].poolWeight))).mul(totalRewards).div(totalPoolValue);
    ...
    // Calculate the locked rewards per era 
    /*function calculateLockedRewardPerEra(uint256 _blockStart, uint256 _blockMax, uint256 _totalVested, uint256[22] memory rewardsByEra) private view returns(uint256[22] memory totalByEra) {
      uint256 receivedLocked = blocksPerVesting.div(_totalVested).mul(_blockMax.sub(_blockStart));
      uint256 lockedPerEra = 0;
      
      //for (uint256 i=0; i<=(numberOfEras); i++) {
    ...
    function getWithdrawFeePercent(address _user, uint256 _pid) public view returns(uint) {
        ...
        uint _feePercent;
        if(lastedTime < 1 days) 
            _feePercent = 50;
        else if(lastedTime < 7 days)
            _feePercent = 25;
        else if(lastedTime < 14 days)
            _feePercent = 15;
        else if (lastedTime >= 14 days)
            _feePercent = 0;
        return _feePercent;
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        for(uint256 i=1; i<=(numberOfEras+1);i++) {
            unlockedAllocated[i] = unlockedAllocated[i].sub(rewardsByEra[i]);
            if (_amount==0) {
                unlockedClaimed[i] = rewardsByEra[i];
            }
        }
