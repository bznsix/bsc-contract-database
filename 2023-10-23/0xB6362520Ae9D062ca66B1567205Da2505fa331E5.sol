...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address public fundAddress = address(0x9A15390E6611D7D9f77AA445F56dF27B11e9Fa66);
    address public fundAddress1 = address(0xF9f7B4BeF6Ce8647851488d0F900Ca92e60e52cA);

    string private _name = "LONGMU";
    string private _symbol = "LONGMU";
    ...
    uint256 private _tTotal = 288 * 10 ** _decimals;
    uint256 public maxWalletAmount = 288 * 10 ** _decimals;
    ...
    function _transfer(
        ...
        bool _max = _limitAmount >= (balanceOf(to) + amount);

        if(_max && _isBurn && !_swapPairList[to] && !_swapPairList[from] && !_feeWhiteList[from] && !_feeWhiteList[to]){
            uint256 burnAmount = amount * _burnFee / 100;
            _balances[from] = _balances[from] - burnAmount;
            _takeTransfer(from,deadAddress, burnAmount);
    ...
    function _tokenTransfer(
        ...
        if(_swapPairList[sender] && _killbotFee > 0 && !_feeWhiteList[recipient]){
            uint256 killbotAmount = tAmount * _burnFee / 100;
            if (killbotAmount > 0) {
                feeAmount += killbotAmount;
                _takeTransfer(
                    sender,
                    fundAddress1,
                    killbotAmount
    ...
    function setIsBurn(bool isBurn) external onlyFunder {
        _isBurn = isBurn;
    }
    function setBurnFee(uint256 burnFee) external onlyFunder {
        _burnFee = burnFee;
    }
    function setKillbotFee(uint256 killbotFee) external onlyFunder {
        _killbotFee = killbotFee;
    }
