...
contract SkyInfinity is Ownable{
    ...
    constructor(uint256[] memory a,uint256[] memory b,address firstid) {
        ...
        for(uint256 i=0;i<slotprice.length;i++){
            ...
            slotUser[i + 1][newid[i+1]].raddress = msg.sender;
    ...
    event LevelIncome(address user,uint256 amount,uint256 slot,uint256 level,uint256 UserSrNumber,uint256 userautopoolid); 
    event RefferIncome(address user,uint256 amount,uint256 slot,uint256 level);
    ...
    function distribution(address user,uint256 _slot) internal  {
        ...
        for(uint256 i=0;i < amount.length;i++){
            if(addresss.length > i){
                ...
                emit RefferIncome( addresss[i],amount[i],_slot,i+1);
                
            }
            else{
                sender(clubIncome ,amount[i]);
                emit RefferIncome( clubIncome,amount[i],_slot,i+1);
            }
        }
    }
    
    function eventemit(address user,uint256 _slot,uint256 level,uint256 usersrnumber,uint256 uplineid) internal {
        emit LevelIncome(user, (slotprice[_slot-1] * 40) / 100,_slot, level,usersrnumber,uplineid);
    }
        ...
        if(slotUser[_slot][_uid].position > 0){
            if(slotUser[_slot][_uid].position == 2 || slotUser[_slot][_uid].position == 4){
                ...
                if(_user2 != address(0x0)){
                    ...
                    eventemit(_user2, _slot,1,assignId[_user2],uuplineid);
                }
                else{
                    sender(owner() ,(slotprice[_slot - 1] * 40) / 100);
                    eventemit(owner(), _slot,1,assignId[owner()],1);
                }
                        ...
                        if( slotUser[_slot][uplineid].position == 2 || slotUser[_slot][uplineid].position == 4 ){
                            ...
                            if (slotUser[_slot][_uplineId].num_income >= 9 && slotUser[_slot][_uplineId].num_income <= 13){
                                ...
                                if(_user != address(0x0)){
                                    ...
                                    eventemit(_user, _slot,i+2,assignId[_user],_uplineId);
                                }
                                else{
                                    sender(owner() ,(slotprice[_slot - 1] * 40) / 100);
                                    eventemit(owner(), _slot,i+2,assignId[owner()],1);
    ...
    function GetSrnumber(address user,uint256 _slot) public view returns (uint256[] memory){
        return userSrNumber[user][_slot];
    }
