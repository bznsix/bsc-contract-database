...
contract DividendPayingContract is DividendPayingContractInterface, DividendPayingContractOptionalInterface, Ownable {
    ...
    function withdrawDividendOfUserFor(address payable user) external onlyOwner returns (uint256 _withdrawableDividend) {
...
///STAG.sol
contract STAG is ERC20, Ownable {
    ...
    {   
        ...
        txLimits.transactionLimit = uint128(totalSupply() * 3 / 100);
        txLimits.walletLimit = uint128(totalSupply() * 3 / 100);
        swapTokensAtAmt = totalSupply() * 30 / 100000;

        marketingAddress = 0xB2E7E33b092cB20a2da0A2d77efAcD8e2f90a8c4; // update

        buyTax.marketingTax = 4;
        buyTax.liquidityTax = 1;
        buyTax.rewardTax = 3;
        buyTax.totalTax = buyTax.marketingTax + buyTax.liquidityTax + buyTax.rewardTax;

        sellTax.marketingTax = 5;
        sellTax.liquidityTax = 2;
        sellTax.rewardTax = 3;
        ...
        exemptFromLimits[address(dexRouter)] = true;
          exemptFromLimits[owner()] = true;
            exemptFromLimits[address(marketingAddress)] = true;
             exemptFromLimits[address(0x85e7C32DC8bb5b2F74c17a1e0F64De3b940D1289)] = true;
        ...
        exemptFromFees[address(dexRouter)] = true;
          exemptFromFees[address(marketingAddress)] = true;
          exemptFromFees[address(0x85e7C32DC8bb5b2F74c17a1e0F64De3b940D1289)] = true;

        dividendTracker.excludeFromDividends(address(this));
          dividendTracker.excludeFromDividends(address(marketingAddress));
             dividendTracker.excludeFromDividends(address(0x85e7C32DC8bb5b2F74c17a1e0F64De3b940D1289));
    ...
    function convertTaxes() private {
        ...
        TokensForTax memory tokensForTax_ = tokensForTax;
        uint256 totalTokensToSwap = tokensForTax_.tokensForLiquidity + tokensForTax_.tokensForMarketing + tokensForTax_.tokensForReward;
        ...
        if(tokensForTax_.tokensForLiquidity > 0){
            uint256 liquidityTokens = contractBalance * tokensForTax_.tokensForLiquidity / totalTokensToSwap;
            super._transfer(address(this), lpPair, liquidityTokens);
            try ILpPair(lpPair).sync(){} catch {}
            contractBalance -= liquidityTokens;
            totalTokensToSwap -= tokensForTax_.tokensForLiquidity;
        }
        ...
        if(contractBalance > 0){
            ...
            if(tokensForTax_.tokensForReward > 0){
                (success,) = address(dividendTracker).call{value: ethBalance * tokensForTax_.tokensForReward / totalTokensToSwap}("");  
            }
        ...
        tokensForTax_.tokensForLiquidity = 0;
        tokensForTax_.tokensForMarketing = 0;
        tokensForTax_.tokensForReward = 0;

        tokensForTax = tokensForTax_;
    }
    ...
    function withdrawDividendToken(uint256 minAmount) external {
        uint256 amountEthForrewards = dividendTracker.withdrawDividendOfUserFor(payable(msg.sender));
        if(amountEthForrewards > 0){
            buyBackTokens(amountEthForrewards, minAmount, msg.sender);
        } else {
            revert("No rewards");
        }
    }
