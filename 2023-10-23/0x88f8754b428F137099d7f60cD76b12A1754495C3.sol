...
import {ERC721Enumerable, IERC721} from "@openzeppelin/contracts/token/ERC721/extensions/ERC721Enumerable.sol";
import {Ownable} from "@openzeppelin/contracts/access/Ownable.sol";
import {SafeERC20, IERC20} from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import {Strings} from "@openzeppelin/contracts/utils/Strings.sol";


abstract contract ERC721Base is Ownable, ERC721Enumerable {
...
abstract contract ERC721BaseEnforcedRoyalties is DefaultOperatorFilterer, ERC721Base {
...
abstract contract ERC721Reservable is Ownable {
     ...
     */
    function mintReservedTo(address[] calldata to) external {
...
abstract contract ERC721Royalty is Ownable, ERC2981 {

    /**
     * @notice Set royalties for the collection.
     * @param _royalty royalty with base 10_000. 5% => 500  .
     */
    function setRoyalty(address _receiver, uint96 _royalty) external onlyOwner {
        _setDefaultRoyalty(_receiver, _royalty);
    }

    /** @notice Returns the royalty with base 10_000. 5% => 500. */
    function royaltyFraction() external view returns (uint256) {
        (, uint256 royaltyFraction_) = royaltyInfo(0, uint256(_feeDenominator()));
        return royaltyFraction_;
    }

    /** @notice Returns the royalty receiver. */
    function royaltyReceiver() external view returns (address) {
        (address royaltyReceiver_, ) = royaltyInfo(0, uint256(_feeDenominator()));
        return royaltyReceiver_;
    }
}
// SPDX-License-Identifier: MIT
...
import {ERC721BaseEnforcedRoyalties, ERC721} from "./erc721/ERC721BaseEnforcedRoyalties.sol";
import {ERC721Reservable} from "./erc721/ERC721Reservable.sol";
import {ERC721Royalty, ERC2981} from "./erc721/ERC721Royalty.sol";
    ...
    ReentrancyGuard
{
    ...
    uint256 public priceWOM = 3000 ether;
    ...
    // forward any BNB sent to this contract to paymentReceiver
    ...
    /**
     * @notice Mint `_amount` nfts to `_to`. Fee is payed in WOM.
     * @param _to: wallet to mint nfts to
     * @param _amount: amount of nfts to mint
     */
    function mintWithWOM(address _to, uint256 _amount) external {
        IERC20(WOM).transferFrom(msg.sender, paymentReceiver, _amount * priceWOM);
        _mintTo(_to, _amount);
    }

    /**
     * @notice Mint `_amount` nfts to `_to`. Fee is payed in BNB.
     * @param _to: wallet to mint nfts to
     * @param _amount: amount of nfts to mint
     * @param _route: pancakeswap route to swap BNB to WOM
     */
    function mintWithBNB(address _to, uint256 _amount, address[] memory _route) external payable {
        require(_route[_route.length - 1] == WOM, "Invalid route");
        uint256[] memory amounts = IPancakeswapRouter(ROUTER).swapETHForExactTokens{value: msg.value}(
            _amount * priceWOM, 
            _route, 
            paymentReceiver, 
            block.timestamp + 500
    ...
    function setPaymentReceiver(
        ...
        require(
            ...
            "Invalid payment receiver not set"
        );
        paymentReceiver = _paymentReceiver;
    }
    ...
    function priceBNB(uint256 _amount, address[] memory _route) public view returns (uint256) {
        uint256[] memory amountsIn = IPancakeswapRouter(ROUTER).getAmountsIn(_amount * priceWOM, _route);
        return amountsIn[0];
    }
}
// SPDX-License-Identifier: MIT
