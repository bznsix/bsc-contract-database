...
/// @title MetaRacing LootBox NFT Contract
/// @notice ERC721 (ERC721A Quaryable / Burnable) Standart Tokens Contract
/// @author metaraces.io
contract MTRLootBox is ERC721AQueryable, ERC721ABurnable, Ownable, EIP712 {
    ...
    constructor(
        string memory _name, // MetaRacing: LootBox NFT
        string memory _symbol, // MLB
        string memory _baseUri, // https://nft.metaraces.io/lootboxes/metadata/
        address signer
    ) ERC721A(_name, _symbol) EIP712("Metaracing", "1") {
        uriBase = _baseUri;
        signerRole = signer;
    }

    /// @notice Executes mint tokens
    /// @param receiver Address - receiver of new minted tokens
    ...
    function mint(
        ...
        require(receiver != address(0), "Metaracing: Receiver address zero");
        require(!isNonceUsed[nonce], "Metaracing: Nonce already used");
        require(block.timestamp <= deadline, "Metaracing: Transaction overdue");
        ...
        bytes32 typedHash = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(
                        "MintData(address receiver,uint256 quantity,address executor,uint256 nonce,uint256 deadline)"
                    ),
                    receiver,
                    quantity,
                    msg.sender,
