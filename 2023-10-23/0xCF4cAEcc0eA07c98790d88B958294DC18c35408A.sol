...
contract MaineCat is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MaineCatDividendTracker public dividendTracker;
    ...
    constructor() ERC20("MaineCat Token", "MAINE") {
        marketingWallet = payable(0x94BeC3c895452cEE00556F0B7C80E6541058685c);
        devWallet = payable(0x94BeC3c895452cEE00556F0B7C80E6541058685c);
        ...
        dividendTracker = new MaineCatDividendTracker(
            payable(this),
            router,
        ...
        maxPerWallet = totalTokenSupply / 50; // 2%
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            require(contractBalanceRecipient + amount <= maxPerWallet,
...
contract MaineCatDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    MaineCat public MaineCatContract;
    ...
    constructor(
        ...
        trackerName = _name; //name passed from main contract
        trackerTicker = _ticker; //ticker passed from main contract
        defaultToken = token;// token address of an external token(busd, eth or usdt)
        MaineCatContract = MaineCat(mainContract); //main token contract
    ...
    function _transfer(
        ...
        require(false, "MaineCat_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "MaineCat_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
    ...
    function isExcludedFromAutoClaim(address account)
        ...
        returns (bool)
    {
        return excludedFromAutoClaim[account]; //exclude passed address from auto claim
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(MaineCatContract);

            uint256 prevBalance = MaineCatContract.balanceOf(address(this));
            ...
            {
                uint256 received = MaineCatContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MaineCatContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
