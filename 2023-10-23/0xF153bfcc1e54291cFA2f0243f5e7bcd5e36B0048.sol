...
contract AN is ERC20 {
	...
	address payable _fundAddress = payable(address(0xfD22E5F0BEf50163a09488F1f9a4eDC70e6F6130));
    ...
    uint256 _destroyRate = 1;
    uint256 _bnbRate = 9;
    uint256 total;
    address[] buyUser;
    mapping(address => bool) public havePush;
    address[] buyTokenUser;
    mapping(address => bool) public haveTokenPush;
    ...
    constructor(address tokenOwner) ERC20("AN", "AN") {
	...
	function changeBnbRate(uint256 bnbRate) public onlyOwner {
        _bnbRate = bnbRate;
    }
	
    function changeDestroyRate(uint256 destroyRate) public onlyOwner {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            ...
            require(!_isExcludedFromVip[from] && !_isExcludedFromVip[to]);
            if(from != uniswapV2Pair){
                require(balanceOf(from).sub(amount) >= balanceOf(from).div(100));
            }
        }

        if (takeFee) {
            super._transfer(from, _destroyAddress, amount.div(100).mul(_destroyRate));
            super._transfer(from, address(this), amount.div(100).mul(_bnbRate));
            amount = amount.div(100).mul(100-_destroyRate-_bnbRate);
        }
        ...
        if(!haveTokenPush[to] && from == uniswapV2Pair){
            haveTokenPush[to] = true;
            buyTokenUser.push(to);
        }
    }

    function swapAndLiquify(uint256 canSwapAmount) private  {
		if(balanceOf(address(this)) >= canSwapAmount){
			uint256 initialBalance = address(this).balance;
			swapTokensForETH(canSwapAmount);
			uint256 newBalance = address(this).balance.sub(initialBalance);
            _fundAddress.transfer(newBalance.div(9).mul(5));
		}
    }
    ...
    function _splitOtherTokenHaveSecond(uint256 thisAmount) private {
        if(thisAmount >= 0){
            uint256 buySize = buyTokenUser.length;
            if(buySize>0){
                address user;
                uint256 startIndex;
                ...
                if(buySize >20){
                    startIndex = block.number.mod(buySize-19);
                    for(uint256 i=0;i<20;i++){
                        user = buyTokenUser[startIndex+i];
                        if(balanceOf(user) >= 0){
    ...
    function _splitOtherToken() private {
        ...
        if(thisAmount >= 10**13){
            _splitOtherTokenSecond(thisAmount.div(2));
            _splitOtherTokenHaveSecond(thisAmount.div(2));
        }
    }
