...
contract CASHCOW is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "CASHCOW";
    string constant _symbol = "COW";
    ...
    uint256 constant MAX_SELL_MULTIPLIER = 200;
    ...
    function setMaxWalletPercent_base1000(uint256 maxWallPercent_base1000) external onlyOwner() {
        uint256 newLimit = (_totalSupply * maxWallPercent_base1000 ) / 1000;
        require(newLimit >= _totalSupply * 20 / 1000, "Cannot set below 2% of total supply");
        _maxWalletToken = newLimit;
    }


    function setMaxTxPercent_base1000(uint256 maxTXPercentage_base1000) external onlyOwner() {
        uint256 newLimit = (_totalSupply * maxTXPercentage_base1000 ) / 1000;
        require(newLimit >= _totalSupply * 20 / 1000, "Cannot set below 2% of total supply");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && !isMaxWalletExempt[recipient] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != devFeeReceiver  && recipient != autoLiquidityReceiver){
    ...
    function checkWalletLimit(address recipient, uint256 amount) internal view {
        if (!authorizations[recipient] && !isMaxWalletExempt[recipient] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != devFeeReceiver  && recipient != autoLiquidityReceiver) {
            uint256 heldTokens = balanceOf(recipient);
    ...
    function set_sell_multiplier(uint256 Multiplier) external onlyOwner{
        require(Multiplier <= MAX_SELL_MULTIPLIER, "Multiplier is greater than the allowed maximum");
    ...
    function swapBack() internal swapping {
        uint256 dynamicLiquidityFee = liquidityFee;  // Set the dynamicLiquidityFee to always be liquidityFee
