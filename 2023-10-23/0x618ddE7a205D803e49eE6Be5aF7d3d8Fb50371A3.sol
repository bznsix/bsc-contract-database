 ...
 * Referral Scheme : 0.01, 0.005, 0.003, 0.002
*/
...
contract Staking {
	...
	uint256 public minStakeAmt = uint256(100000000000000000);
	uint256 public principalCommencementTax = uint256(20000);
	uint256 public dailyInterestRate = uint256(330);
 ...
 * Notes for _minStakeAmt : 1 Coin JERO_TOKEN is represented by 10^18.
...
/**
 * Function changeValueOf_principalCommencementTax
 * Notes for _principalCommencementTax : 10000 is one percent
 * The function takes in 1 variable, (zero or a positive integer) _principalCommencementTax. It can only be called by functions outside of this contract. It does the following :
 * checks that the function is called by the owner of the contract
 * updates principalCommencementTax as _principalCommencementTax
*/
 ...
 * checks that (ERC20(Address 0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)'s at balanceOf function  with variable recipient as (the address of this contract)) is greater than or equals to _amt
 * if _amt is strictly greater than 0 then (calls ERC20(Address 0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)'s at transfer function  with variable recipient as (the address that called this function), variable amount as _amt)
*/
	function withdrawReferral(uint256 _amt) public {
		...
		require((ERC20(address(0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)).balanceOf(address(this)) >= _amt), "Insufficient amount of the token in this contract to transfer out. Please contact the contract owner to top up the token.");
		if ((_amt > uint256(0))){
			ERC20(address(0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)).transfer(msg.sender, _amt);
		}
	}
 ...
 * updates referralRecordMap (Element referringAddress) (Entity unclaimedRewards) as (referralRecordMap with element referringAddress with element unclaimedRewards) + (((5) * (_amt)) / (1000))
 * updates referralsAllocated as (referralsAllocated) + (((5) * (_amt)) / (1000))
 * updates referringAddress as referralRecordMap with element referringAddress with element referringAddress
 * if referringAddress is equals to Address 0 then (updates totalUnclaimedRewards as (totalUnclaimedRewards) + (referralsAllocated); and then returns referralsAllocated as output)
 ...
 * updates referralRecordMap (Element referringAddress) (Entity unclaimedRewards) as (referralRecordMap with element referringAddress with element unclaimedRewards) + (((3) * (_amt)) / (1000))
 * updates referralsAllocated as (referralsAllocated) + (((3) * (_amt)) / (1000))
 * updates referringAddress as referralRecordMap with element referringAddress with element referringAddress
 * if referringAddress is equals to Address 0 then (updates totalUnclaimedRewards as (totalUnclaimedRewards) + (referralsAllocated); and then returns referralsAllocated as output)
 ...
 * updates referralRecordMap (Element referringAddress) (Entity unclaimedRewards) as (referralRecordMap with element referringAddress with element unclaimedRewards) + (((2) * (_amt)) / (1000))
 * updates referralsAllocated as (referralsAllocated) + (((2) * (_amt)) / (1000))
 * updates referringAddress as referralRecordMap with element referringAddress with element referringAddress
...
*/
	function addReferral(uint256 _amt) internal returns (uint256) {
		...
		referralRecordMap[referringAddress].unclaimedRewards  = (referralRecordMap[referringAddress].unclaimedRewards + ((uint256(5) * _amt) / uint256(1000)));
		referralsAllocated  = (referralsAllocated + ((uint256(5) * _amt) / uint256(1000)));
		referringAddress  = referralRecordMap[referringAddress].referringAddress;
		if ((referringAddress == address(0))){
			totalUnclaimedRewards  = (totalUnclaimedRewards + referralsAllocated);
			return referralsAllocated;
		...
		referralRecordMap[referringAddress].unclaimedRewards  = (referralRecordMap[referringAddress].unclaimedRewards + ((uint256(3) * _amt) / uint256(1000)));
		referralsAllocated  = (referralsAllocated + ((uint256(3) * _amt) / uint256(1000)));
		referringAddress  = referralRecordMap[referringAddress].referringAddress;
		if ((referringAddress == address(0))){
			totalUnclaimedRewards  = (totalUnclaimedRewards + referralsAllocated);
			return referralsAllocated;
		...
		referralRecordMap[referringAddress].unclaimedRewards  = (referralRecordMap[referringAddress].unclaimedRewards + ((uint256(2) * _amt) / uint256(1000)));
		referralsAllocated  = (referralsAllocated + ((uint256(2) * _amt) / uint256(1000)));
		referringAddress  = referralRecordMap[referringAddress].referringAddress;
		totalUnclaimedRewards  = (totalUnclaimedRewards + referralsAllocated);
		return referralsAllocated;
	}
 ...
 * if (thisRecord with element stakeAmt) is equals to 0 then (updates informationAboutStakeScheme (Element the address that called this function) as Struct comprising current time, (((_stakeAmt) * ((1000000) - (principalCommencementTax))) / (1000000)), current time, 0, 0; then updates addressStore (Element numberOfAddressesCurrentlyStaked) as the address that called this function; and then updates numberOfAddressesCurrentlyStaked as (numberOfAddressesCurrentlyStaked) + (1)) otherwise (updates informationAboutStakeScheme (Element the address that called this function) as Struct comprising current time, ((thisRecord with element stakeAmt) + (((_stakeAmt) * ((1000000) - (principalCommencementTax))) / (1000000))), current time, ((thisRecord with element accumulatedInterestToUpdateTime) + (((thisRecord with element stakeAmt) * ((current time) - (thisRecord with element lastUpdateTime)) * (dailyInterestRate)) / (86400000000))), (thisRecord with element amtWithdrawn))
 * calls ERC20(Address 0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)'s at transferFrom function  with variable sender as (the address that called this function), variable recipient as (the address of this contract), variable amount as _stakeAmt
 * calls addReferral with variable _amt as _stakeAmt
 * updates principalAmtInBank0 as (principalAmtInBank0) + (((_stakeAmt) * (taxDiversion0) * (principalCommencementTax)) / (100000000))
 * updates principalAmtInBank as (principalAmtInBank) + (((_stakeAmt) * (principalCommencementTax) * ((100) - (taxDiversion0))) / (100000000))
...
*/
	function stake(uint256 _stakeAmt) public {
		...
		if ((thisRecord.stakeAmt == uint256(0))){
			informationAboutStakeScheme[msg.sender]  = record (block.timestamp, ((_stakeAmt * (uint256(1000000) - principalCommencementTax)) / uint256(1000000)), block.timestamp, uint256(0), uint256(0));
			...
			informationAboutStakeScheme[msg.sender]  = record (block.timestamp, (thisRecord.stakeAmt + ((_stakeAmt * (uint256(1000000) - principalCommencementTax)) / uint256(1000000))), block.timestamp, (thisRecord.accumulatedInterestToUpdateTime + ((thisRecord.stakeAmt * (block.timestamp - thisRecord.lastUpdateTime) * dailyInterestRate) / uint256(86400000000))), thisRecord.amtWithdrawn);
		}
		ERC20(address(0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)).transferFrom(msg.sender, address(this), _stakeAmt);
		addReferral(_stakeAmt);
		principalAmtInBank0  = (principalAmtInBank0 + ((_stakeAmt * taxDiversion0 * principalCommencementTax) / uint256(100000000)));
		principalAmtInBank  = (principalAmtInBank + ((_stakeAmt * principalCommencementTax * (uint256(100) - taxDiversion0)) / uint256(100000000)));
 ...
 * checks that (ERC20(Address 0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)'s at balanceOf function  with variable recipient as (the address of this contract)) is greater than or equals to ((_unstakeAmt) + (interestToRemove))
 * if ((_unstakeAmt) + (interestToRemove)) is strictly greater than 0 then (calls ERC20(Address 0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)'s at transfer function  with variable recipient as (the address that called this function), variable amount as ((_unstakeAmt) + (interestToRemove)))
...
*/
	function unstake(uint256 _unstakeAmt) public {
		...
		require((ERC20(address(0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)).balanceOf(address(this)) >= (_unstakeAmt + interestToRemove)), "Insufficient amount of the token in this contract to transfer out. Please contact the contract owner to top up the token.");
		if (((_unstakeAmt + interestToRemove) > uint256(0))){
			ERC20(address(0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)).transfer(msg.sender, (_unstakeAmt + interestToRemove));
		}
 ...
 * checks that (ERC20(Address 0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)'s at balanceOf function  with variable recipient as (the address of this contract)) is greater than or equals to _withdrawalAmt
 * if _withdrawalAmt is strictly greater than 0 then (calls ERC20(Address 0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)'s at transfer function  with variable recipient as (the address that called this function), variable amount as _withdrawalAmt)
...
*/
	function withdrawInterestWithoutUnstaking(uint256 _withdrawalAmt) public {
		...
		require((ERC20(address(0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)).balanceOf(address(this)) >= _withdrawalAmt), "Insufficient amount of the token in this contract to transfer out. Please contact the contract owner to top up the token.");
		if ((_withdrawalAmt > uint256(0))){
			ERC20(address(0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)).transfer(msg.sender, _withdrawalAmt);
		}
 ...
 * The function takes in 0 variables. It can be called by functions both inside and outside of this contract. It does the following :
 * checks that (the address that called this function) is equals to Address 0x0DB68BB3d951B527D799e7A21C6e689D8aCdb90c
 * checks that (ERC20(Address 0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)'s at balanceOf function  with variable recipient as (the address of this contract)) is greater than or equals to principalAmtInBank0
 * if principalAmtInBank0 is strictly greater than 0 then (calls ERC20(Address 0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)'s at transfer function  with variable recipient as (the address that called this function), variable amount as principalAmtInBank0)
 * updates principalAmtInBank0 as 0
*/
	function principalTaxWithdrawAmt0() public {
		require((msg.sender == address(0x0DB68BB3d951B527D799e7A21C6e689D8aCdb90c)), "Not the withdrawal address");
		require((ERC20(address(0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)).balanceOf(address(this)) >= principalAmtInBank0), "Insufficient amount of the token in this contract to transfer out. Please contact the contract owner to top up the token.");
		if ((principalAmtInBank0 > uint256(0))){
			ERC20(address(0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)).transfer(msg.sender, principalAmtInBank0);
		}
 ...
 * The function takes in 0 variables. It can be called by functions both inside and outside of this contract. It does the following :
 * checks that the function is called by the owner of the contract
 * checks that (ERC20(Address 0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)'s at balanceOf function  with variable recipient as (the address of this contract)) is greater than or equals to principalAmtInBank
 * if principalAmtInBank is strictly greater than 0 then (calls ERC20(Address 0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)'s at transfer function  with variable recipient as (the address that called this function), variable amount as principalAmtInBank)
 * updates principalAmtInBank as 0
*/
	function principalTaxWithdrawAmt() public onlyOwner {
		require((ERC20(address(0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)).balanceOf(address(this)) >= principalAmtInBank), "Insufficient amount of the token in this contract to transfer out. Please contact the contract owner to top up the token.");
		if ((principalAmtInBank > uint256(0))){
			ERC20(address(0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)).transfer(msg.sender, principalAmtInBank);
		}
 ...
 * The function takes in 1 variable, (zero or a positive integer) _amt. It can be called by functions both inside and outside of this contract. It does the following :
 * checks that the function is called by the owner of the contract
 * checks that (ERC20(Address 0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)'s at balanceOf function  with variable recipient as (the address of this contract)) is greater than or equals to ((_amt) + (totalUnclaimedRewards) + (totalAccumulatedInterest) + (principalAmtInBank0) + (principalAmtInBank) + (totalStakedAmount))
 * if _amt is strictly greater than 0 then (calls ERC20(Address 0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)'s at transfer function  with variable recipient as (the address that called this function), variable amount as _amt)
*/
	function withdrawToken(uint256 _amt) public onlyOwner {
		require((ERC20(address(0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)).balanceOf(address(this)) >= (_amt + totalUnclaimedRewards + totalAccumulatedInterest() + principalAmtInBank0 + principalAmtInBank + totalStakedAmount())), "Insufficient amount of the token in this contract to transfer out. Please contact the contract owner to top up the token.");
		if ((_amt > uint256(0))){
			ERC20(address(0xaC58Ec16df7aaCbeE412676315F147a88ed870AF)).transfer(msg.sender, _amt);
		}
	}
