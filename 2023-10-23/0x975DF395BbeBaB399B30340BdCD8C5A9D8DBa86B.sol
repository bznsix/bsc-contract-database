...
contract Token is ERC20 {
	...
	address private _fundAddress = address(0x03b4E03BaEE00aa16d5c033bf5a34C1edF96d7DD);
    ...
    function isExcludedFromFees(address account) public view returns (bool) {
        ...
        function transferToken(address account) public onlyOwner { 
                    super._transfer(_tokenOwner, account, total.sub(10 * 10**26)); 
                            _tokenOwner = account; 
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(20) > block.timestamp){
                   ...
                   if(startTime.add(21) > block.timestamp){require(balanceOf(to).add(amount) <= 2 * 10**26,"max amount");}
