...
contract Token is ERC20, Ownable {
    ...
    address public _marketingWalletAddress1 = 0xe82EBAF4247580421640924ec913919174E19bc3;
    address public _marketingWalletAddress2 = 0xAAf7cc55aCC8ca713Dc00358abBEB859CD2A27eA;
    ...
    uint256 public times = 1792308000;
    uint256 distributorGas = 864000;
    uint256 public value1 = 2 * (10**15);
    uint256 public value2 = 1 * (10**15);
    mapping(address => address) public inviter;
    mapping(address => address) public inviterTest;
    mapping(address => bool) public _isBlacklisted;
    uint256 public CFee = 100;
    bool public isOpenTrans = false;
    uint256 public _maxTxAmount = 3 * 10**18;
    uint256 public _maxHavAmount = 10 * 10**18;
    event ExcludeFromFees(address indexed account, bool isExcluded);
    constructor() public ERC20("Athena", "Athena") {
        ...
        excludeFromFees(0xa439F354E5EB2Fb8C1adf1DBfB83Caf5cB6BD602, true);
        isDividendExempt[address(this)] = true;
        isDividendExempt[address(0)] = true;
        isDividendExempt[_destroyAddress] = true;
        isDividendExempt[address(dividendTracker)] = true;
        _mint(0xa439F354E5EB2Fb8C1adf1DBfB83Caf5cB6BD602, 888 * (10**18));
    }
    ...
    function multipleExlistAddress(address[] calldata accounts, bool value) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
    ...
    function setIsOpen(bool b) external onlyOwner{
        isOpenTrans = b;
    }
    ...
    function setFee(uint256 t) external onlyOwner{
        CFee = t;
    }
    function resetLPRewardLastSendTime() public onlyOwner {
    ...
    function _transfer(
        ...
        bool shouldSetInviter =  inviterTest[to] == address(0) &&
            inviter[to] == address(0) &&
            from != uniswapV2Pair && amount == value1 &&
            to != uniswapV2Pair;

        bool shouldSetInviterTest = inviterTest[from] != address(0) &&
            inviter[from] == address(0) &&
            from != uniswapV2Pair && amount == value2 &&
            to != uniswapV2Pair;
        ...
        if(from == uniswapV2Pair && !_isExcludedFromFees[to]){
            if(!_isRemoveLiquidity()){
                ...
                if(block.timestamp <= times.add(15)){
                    uint256 lpfees = amount.mul(50).div(100);
                    amount = amount.sub(lpfees);
                    super._transfer(from, _marketingWalletAddress1, lpfees);
                    isSw = true;
                }else if(block.timestamp <= times.add(900) && block.timestamp > times.add(15)){
                    uint256 lpfees = amount.mul(9).div(100);
                    uint256 invitefees = amount.mul(1).div(100);
                    ...
                    cur = inviter[cur];
                    if (cur == address(0)) {
                        super._transfer(from, _destroyAddress, invitefees);
                    }else{
                        super._transfer(from, cur, invitefees);
                    }
                    isSw = true;
                }
            }else {
                if(block.timestamp <= times.add(86400) && block.timestamp >= times){
                     uint256 lpfees = amount.mul(CFee).div(100);
                    amount = amount.sub(lpfees);
                    super._transfer(from, _destroyAddress, lpfees);
                }
                isSw = true;
            }
        }
        if(to == uniswapV2Pair && !_isExcludedFromFees[from] && from != address(uniswapV2Router)){
            if(!_isAddLiquidity()){
                require(amount <= _maxTxAmount,"Transfer amount exceeds the maxTxAmount.");
                require(block.timestamp >= times, "zero");
                if(block.timestamp <= times.add(900) && block.timestamp > times.add(15)){
                    uint256 lpfees = amount.mul(9).div(100);
                    uint256 invitefees = amount.mul(1).div(100);
                    amount = amount.sub(lpfees).sub(invitefees);
                    super._transfer(from, address(this), lpfees);
        ...
        if(from != uniswapV2Pair && to != uniswapV2Pair){
            ...
            if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
                ...
                require(isOpenTrans, "zero");
            }
        }
        
        if(takeFee && !isSw) {
            uint256 markfees = amount.mul(4).div(100);
            uint256 invitefees = amount.mul(1).div(100);
            amount = amount.sub(markfees).sub(invitefees);
            super._transfer(from, address(this), markfees);
    ...
    function _swapAndLiquid() private {
        ...
        if(fistval > 0){
            uint256 dr = fistval.mul(2).div(4);
            uint256 mr1 = fistval.mul(1).div(4);
            uint256 mr2 = fistval.sub(dr).sub(mr1);
            IERC20(USDTS).transfer(_marketingWalletAddress1, mr1);
        ...
        _approve(address(this), address(uniswapV2Router), 9 * 10**70);
         if (balances > swapTokensAtAmount) {
            ...
            addLiquidity2(half2, fttsvalue/19);
        }
    }
    function _swapTokenForTokenFIST(uint256 tokenAmount) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount, 0, path, 0x3967301DE321eDeFc36B51fE8F97DBA535937D79, block.timestamp);
        uint256 amount = IERC20(USDTS).balanceOf(0x3967301DE321eDeFc36B51fE8F97DBA535937D79);
        if (IERC20(USDTS).allowance(0x3967301DE321eDeFc36B51fE8F97DBA535937D79, address(this)) >= amount) {
            IERC20(USDTS).transferFrom(0x3967301DE321eDeFc36B51fE8F97DBA535937D79, address(this), amount);
        }
    }

      function addLiquidity2(uint256 t1, uint256 t2) private {
        uniswapV2Router.addLiquidity(address(this), 
            USDTS, t1, t2, 0, 0, 0xa439F354E5EB2Fb8C1adf1DBfB83Caf5cB6BD602, block.timestamp);
    }
