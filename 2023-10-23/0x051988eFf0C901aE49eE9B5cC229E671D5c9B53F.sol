...
contract WinniePooh is Context, IERC20, Ownable {

    using SafeMath for uint256;
    using Address for address;

    string private _name = "WinniePooh";
    string private _symbol = "WinniePooh";
    ...
    uint256 private _totalSupply = 18_776_325_400 * 10**_decimals;
    uint256 public _maxTxAmount = 18_776_325_400 * 10**_decimals;
    uint256 public _walletMax = 18_776_325_400 * 10**_decimals;
    ...
    constructor (
        ...
        teamWalletAddress = payable(_team);
        ...
        _totalTaxIfBuying = _buyMarketingFee + _buyLpFee + _buyTeamFee;
        _totalTaxIfSelling = _sellMarketingFee + _sellLpFee + _sellTeamFee;
        _totalDistributionShares = _marketingShare + _teamShare + _lpShare;
    ...
    function setBuyTaxes(
        ...
        uint256 newTeamPercent
    ) external onlyOwner {
        _buyMarketingFee = newMarketingTax;
        _buyLpFee = newLpPercent;
        _buyTeamFee = newTeamPercent;

        _totalTaxIfBuying = _buyTeamFee + _buyLpFee + _buyMarketingFee;
    ...
    function setSellTaxes(
        ...
        _sellTeamFee = newTeamPercent;
        _sellLpFee = newLpPercent;

        _totalTaxIfSelling = _sellLpFee + _sellTeamFee + _sellMarketingFee;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if (isMarketPair[sender] && recipient != owner()) {
            require(startTrade, 'Error: trades dont start');
        }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountETHTeam > 0)
            transferToAddressETH(payable(address(teamWalletAddress)), amountETHTeam);
