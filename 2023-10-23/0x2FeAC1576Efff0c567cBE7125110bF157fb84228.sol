...
// Contract for $Raini to $RST conversion bridge
// $Raini sent through the bridge will be sent to the burn address and $RST will be minted on Beam

contract RainiToRstReceiver is BaseOFTV2, Ownable2Step {
    ...
    function burnRainiSupply() external onlyOwner {
        uint _amount = innerToken.balanceOf(address(this));
        innerToken.transfer(address(0xdead), _amount);
    }
