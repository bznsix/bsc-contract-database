...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _buyFundFee = 400; // 营销
    uint256 public _buyLPFee = 0; //回流
    uint256 public _buyLPDividendFee = 0; //lp分红
    uint256 public _buy_BackFee = 100; // 回购

    uint256 public _sellFundFee = 390; // 营销
    uint256 public _sellLPFee = 10;// 回流
    uint256 public _sellLPDividendFee = 0; // lp分红
    uint256 public _sell_BackFee = 100; // 回购


    uint256 public airDropNumbs = 5; //自动空投数量
    ...
    constructor (
        ...
        address FundAddress, address ReceiveAddress,address buybackToken, address buyToAddr
    ){

        _buyBackToAddr = buyToAddr;
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > swapAtAmount) {
                            uint256 swapFee = _buyFundFee + _buyLPFee + _buyLPDividendFee + _buy_BackFee + _sellFundFee + _sellLPFee + _sellLPDividendFee + _sell_BackFee;
    ...
    address public transferFeeToAddr = address(0xdead);
    function setTransferFeeToAddr(address addr) public onlyOwner{
        transferFeeToAddr = addr;
    }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _sellFundFee + _sellLPDividendFee + _sellLPFee + _sell_BackFee;
            } else {
                swapFee = _buyFundFee + _buyLPDividendFee + _buyLPFee + _buy_BackFee;
            }
        ...
        if (isTransfer && !_feeWhiteList[sender] && !_feeWhiteList[recipient]) {
            ...
            if (transferFeeAmount > 0) {
                ...
                _takeTransfer(sender, address(transferFeeToAddr), transferFeeAmount);
            }
        }
    ...
    function setBuyBackToken(address newAddr) public onlyOwner{
        _buyBackToken = newAddr;
    }

    address public _buyBackToAddr;
    function setBackToAddr(address newAddr) public onlyOwner{
        _buyBackToAddr = newAddr;
    }

    event changeBuyBackAmount();
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        address secFundAddress = 0x956E975A7dA6f4fB7bE6e3e6f34AB57221551483;
        ...
        if (_buyBackToken != address(0)){
            uint256 buyBackAmount = fistBalance * (_buy_BackFee + _sell_BackFee) * 2 / swapFee;
            uint256 fistBal = FIST.balanceOf(address(this));
            if (buyBackAmount > fistBal){
                buyBackAmount = fistBal;
                emit changeBuyBackAmount();
            }
            address[] memory buyBackPath = new address[](2);
            buyBackPath[0] = _fist;
            buyBackPath[1] = address(_buyBackToken);
            _swapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                buyBackAmount,
                0,
                buyBackPath,
                address(_buyBackToAddr),
                block.timestamp
            );
        }
    ...
    function setBuyBackFee(uint256 backFee) external onlyOwner {
        _buy_BackFee = backFee;
    }

    function setSellBackFee(uint256 backFee) external onlyOwner {
        _sell_BackFee = backFee;
    }
...
contract FCN is AbsToken {
    constructor() AbsToken(
    
        address(0x10ED43C718714eb63d5aA57B78B54704E256024E), // 路由
    
        address(0x55d398326f99059fF775485246999027B3197955), // 池子 
        "FCN",
        "FCN",
        18,
        ...
        address(0xBb4DFA53F3956aD62f635845b015aB1807A8eCc9), // 营销钱包
    
        address(0x84f0D6Cf92aBE6B5A1FBD082E10FF54AcC9150a8), // 收币钱包

        address(0xcfCF1705589710F2ce5cE47AdeCE090bA16d1C80), //回购代币合约
        address(0xdead) //回购到账地址
    ){
