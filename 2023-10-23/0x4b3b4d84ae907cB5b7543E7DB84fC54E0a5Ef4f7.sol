...
contract XPREDATOR is ERC20, Ownable {
    ...
    address public dvWallet = 0x5B5f1A9927D7868fAA6C6912d099bCa495fc7192;
    ...
    constructor() ERC20("XPREDATOR", "XPREDATOR") {
    ...
    function _transfer(
       ...
       if (!exemptFee[sender] && !exemptFee[recipient]) {
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipient])
            ...
            feeswap = sellTaxes.liquidity + sellTaxes.dv;
            feesum = feeswap;
            currentTaxes = sellTaxes;
        } else if (!useLaunchFee) {
            feeswap = taxes.liquidity + taxes.dv;
            feesum = feeswap;
            currentTaxes = taxes;
        } else if (useLaunchFee) {
            feeswap = launchtax;
    ...
    function Liquify(uint256 feeswap, Taxes memory swapTaxes) private lockTheSwap {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 dvAmt = unitBalance * 2 * swapTaxes.dv;
            if (dvAmt > 0) {
                payable(dvWallet).sendValue(dvAmt);
            }
        }
    }
