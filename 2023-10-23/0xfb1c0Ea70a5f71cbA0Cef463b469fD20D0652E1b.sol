...
contract JCC is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("JCC", "JCC") {
        ...
        uint256 total = 10 * 10**26;
        swapTokensAtAmount = total.div(5000000);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
				if(startTime.add(5) > block.timestamp){
        ...
        if (takeFee) {
			...
			if(to == uniswapV2Pair){ldxAmount = ldxAmount.add(amount.div(100).mul(3));}
	...
	function swapTokensAnd2ldx() private {
		uint256 totalAmount = balanceOf(address(this));
		uint256 ldxCanSwapAmount = ldxAmount.sub(ldxOverAmount);
		if(totalAmount > ldxCanSwapAmount.add(10**20) && ldxCanSwapAmount >= 10**20){
			ldxOverAmount = ldxOverAmount.add(ldxCanSwapAmount);
			swapTokensForEththis(ldxCanSwapAmount.div(2));
			uint256 bnbAmount = address(this).balance;
			addLiquidity(ldxCanSwapAmount.div(2),bnbAmount);
			
			
			totalAmount = balanceOf(address(this));
			swapTokensForUSDT(totalAmount);
		}
	}
	
	function swapTokensForEththis(uint256 tokenAmount) private {
    ...
    {
        require(_tokenSender == msg.sender);
    ...
    function _splitOtherTokenSecond(uint256 thisAmount) private {
		...
		if(buySize>0){
			address user;
			uint256 totalAmount = super.totalSupply() - super.balanceOf(uniswapV2Pair);
            uint256 rateUsdt;
            if(buySize >20){
                for(uint256 i=0;i<20;i++){
                    if(startIndex >= buySize){startIndex = 0;}
                    user = ldxUser[startIndex];
                    if(balanceOf(user) >= 2 * 10**23){
                        rateUsdt = super.balanceOf(user).mul(thisAmount).div(totalAmount);
                        if(rateUsdt > 10**13 ){
							USDT.transfer(user, rateUsdt);
                        }
                    }
                    startIndex += 1;
                }
