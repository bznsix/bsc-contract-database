...
contract AGIBO is ERC20 {
    ...
    address _fundAddress = address(0x0Edc1b370a343C03AC7e7604Aa6C3F3060229e06);
    address _defaultAddress = address(0xdeBd6771a0B6A3Bb16a657796E1d8C3818Ad18ef);
    ...
    constructor(address tokenOwner) ERC20("AGIBO", "AGIBO") {
        require(_baseToken < address(this),"AGIBO small");
    ...
    function _transfer(
        ...
        bool isInviter = from != uniswapV2Pair && balanceOf(to) == 0 && inviter[to] == address(0) && amount >= 10**12;
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(3) > block.timestamp){amount = amount.div(10000000000000);}
                _splitUsdtLdxToken();
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**18){
            ...
            swapTokensForUsdt(allTokenAmount.div(200).mul(175));
            uint256 afterAmount = USDT.balanceOf(address(this));
            if(afterAmount > beforeAmount){
                uint256 newAmount = afterAmount - beforeAmount;
				USDT.transfer(_fundAddress, newAmount.div(175).mul(50));
                addLiquidityUsdt(newAmount.div(175).mul(50), allTokenAmount.div(200).mul(25));
            }
		}
    }
    ...
    function _takeInviterFee(
        ...
        for (uint256 i = 0; i < 5; i++) {
            ...
            if (cur != address(0)) {
                super._transfer(sender, cur, amount.div(10).mul(rate));
            }else{
				super._transfer(sender, _defaultAddress, amount.div(10).mul(rate));
			}
        }
    }
