...
/*                                                        
 ...
 PLAYWORLD NFT COLLECTION 1 - In game assets                                                                                                   
*/
...
contract NFTCollection is ERC721Enumerable, Ownable {
  ...
  function getPrice(address user) public view returns (uint256 pricePer) {
    uint256 howManyNfts = NFT(discountNFT).balanceOf(user, discountId);
    uint256 yourDiscount = 0;
    if(howManyNfts > 0)  {
        yourDiscount =  (costBNB * discount) / 100;
    }
    pricePer = costBNB - yourDiscount;
  }
 

  function setNFT(address newNFT, uint256 _discountId) external onlyOwner {
    discountNFT = newNFT;
  ...
  function mint(address _to, uint256 _mintAmount) public payable returns (uint256[] memory ids) {
    ...
    if(costBNB > 0) {
        uint256 userPrice = getPrice(msg.sender);
        uint256 fullCost = userPrice * _mintAmount;
