...
contract DefiEdgeTwapStrategy is UniswapV3TwapLiquidityManager {
    ...
    function mint(
        ...
        uint256 _amount1Min,
        uint256 _minShare
    ) external onlyValidStrategy nonReentrant returns (uint256 amount0, uint256 amount1, uint256 share) {
...
contract UniswapV3TwapLiquidityManager is TwapStrategyBase, ReentrancyGuard, IAlgebraMintCallback {
    ...
    function _transferPerformanceFees(uint256 _fee0, uint256 _fee1) internal {
        ...
        ) = TwapShareHelper.calculateFeeTokenShares(factory, manager, address(this), _fee0, _fee1);
     ...
     */
    function swap(bool zeroToOne, uint256 amountIn, bool isOneInchSwap, bytes calldata data) public onlyOperator onlyValidStrategy nonReentrant {
    ...
    function skim(address to) external onlyOperator {
    ...
    function sync() external onlyOperator {
        reserve0 = token0.balanceOf(address(this));
        reserve1 = token1.balanceOf(address(this));
...
library TwapShareHelper {
        ...
        )
    {
        ...
        // if sum of both performance fees is greater than 100% then reduce protocol performance fees and adjust both fees sum to 100% (100% -  performanceFeeRate)
        if(performanceFeeRate.add(protocolPerformanceFeeRate) > 100e6){
            protocolPerformanceFeeRate = uint256(100e6).sub(performanceFeeRate);
        }
