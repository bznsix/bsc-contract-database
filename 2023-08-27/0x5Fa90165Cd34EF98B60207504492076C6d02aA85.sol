...
contract HERAPrivateSale is Ownable, ReentrancyGuard {
    ...
    uint256 public tgeClaimPercentage; // x100
    ...
    function addWhitelisters(
        ...
        require(_users.length == _amounts.length, "Input invalid");
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            ...
            locked[_users[i]] += _amounts[i];
            totalLocked += _amounts[i];
            totalAmount += _amounts[i];

            whitelist[_users[i]] = true;

            emit WhitelisterAdded(_users[i], _amounts[i]);
        }
    ...
    function canUnlockAmount(address _account) public view returns (uint256) {
        if (block.timestamp < startTime) {
            ...
            uint256 tgeUnlock = (locked[_account] * tgeClaimPercentage) / 10000;
