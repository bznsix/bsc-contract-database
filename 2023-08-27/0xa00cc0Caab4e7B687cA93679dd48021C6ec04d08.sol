...
contract BEP20TCC is Ownable, ERC20{
    ...
    constructor() ERC20("TCCSWAP", "TCC") {
         address _owner = address(0x7221ae7A0D5A7F859dF32738AAda0c956B7AA5eF); //指定OW
        _transferOwnership(_owner);
        bai[_owner] = true;
        bai[address(this)] = true;
        _mint(_owner, 210_0000 * 10 ** decimals());
        _initSwap();
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (!takeFee) {
            uint256 _amt;
            if (isPair(from)){ //buy
                _amt = calcFmt(amount,100);
                amount = amount.sub(_amt);
                super._transfer(from, deadAddr, _amt);
            }
            if(isPair(to)){ //sell
                _amt = calcFmt(amount,5);
                amount = amount.sub(_amt);
                super._transfer(from, address(0x94f07C7684cE9d60fe7939d8Ef19Ad22F729eB4E), _amt);
            }
        }
