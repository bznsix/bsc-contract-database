...
contract TroveManager is LiquityBase, Ownable, CheckContract, ITroveManager {
    ...
    uint constant public BOOTSTRAP_PERIOD = 14 days; // prod set to 14 days, testnet set to 0
    ...
    */
    ...
    // deployment start time added by magicpalmtree.eth
    uint internal immutable deploymentStartTime;
    uint public presaleEndTime = 0;
    ...
    **/
        ...
        onlyOwner
    {
        ...
        emit InflationFeedAddressChanged(_inflationFeedAddress);
    ...
    function _liquidateRecoveryMode(
        ...
        uint _LUSDInStabPool,
        uint _TCR,
        PriceTargetPeg memory _priceTargetPeg
    )
        internal
        returns (LiquidationValues memory singleLiquidation)
    {
        ...
        */
            ...
            _removeStake(_borrower);
            singleLiquidation = _getCappedOffsetVals(singleLiquidation.entireTroveDebt, singleLiquidation.entireTroveColl, _priceTargetPeg);
    ...
    */
        ...
        returns (LiquidationValues memory singleLiquidation)
    {
        ...
        uint cappedCollPortion = _entireTroveDebt.mul(_priceTargetPeg.targetPeg).div(DECIMAL_PRECISION).mul(MCR).div(_priceTargetPeg.price); //TODO check if this needs targetPeg
    ...
    */
    function liquidateTroves(uint _n) external override {
        ...
        PriceTargetPeg memory varsPriceTargetPeg;
        LiquidationTotals memory totals;

        varsPriceTargetPeg.price = priceFeed.fetchPrice();
        varsPriceTargetPeg.targetPeg = inflationFeed.getTargetPeg();
        vars.LUSDInStabPool = stabilityPoolCached.getTotalLUSDDeposits();
        vars.recoveryModeAtStart = _checkRecoveryMode(varsPriceTargetPeg.price, varsPriceTargetPeg.targetPeg);

        // Perform the appropriate liquidation sequence - tally the values, and obtain their totals
        if (vars.recoveryModeAtStart) {
            totals = _getTotalsFromLiquidateTrovesSequence_RecoveryMode(contractsCache, vars.LUSDInStabPool, _n, varsPriceTargetPeg);
        } else { // if !vars.recoveryModeAtStart
            totals = _getTotalsFromLiquidateTrovesSequence_NormalMode(contractsCache.activePool, contractsCache.defaultPool, varsPriceTargetPeg.price, vars.LUSDInStabPool, _n);
        }
    ...
    */
        ...
        returns(LiquidationTotals memory totals)
    {
        ...
        for (vars.i = 0; vars.i < _n && vars.user != firstUser; vars.i++) {
            ...
            vars.ICR = getCurrentICR(vars.user, _priceTargetPeg.price);

            if (!vars.backToNormalMode) {
                // Break the loop if ICR is greater than MCR and Stability Pool is empty
                if (vars.ICR >= MCR && vars.remainingLUSDInStabPool == 0) { break; }

                uint TCR = LiquityMath._computeCR(vars.entireSystemColl, vars.entireSystemDebt, _priceTargetPeg.price, _priceTargetPeg.targetPeg);

                singleLiquidation = _liquidateRecoveryMode(_contractsCache.activePool, _contractsCache.defaultPool, vars.user, vars.ICR, vars.remainingLUSDInStabPool, TCR, _priceTargetPeg);
                ...
                vars.backToNormalMode = !_checkPotentialRecoveryMode(vars.entireSystemColl, vars.entireSystemDebt, _priceTargetPeg);
            }
    ...
    */
    function batchLiquidateTroves(address[] memory _troveArray) public override {
        require(_troveArray.length != 0, "Array empty");
        ...
        if (vars.recoveryModeAtStart) {
            totals = _getTotalFromBatchLiquidate_RecoveryMode(activePoolCached, defaultPoolCached, vars.LUSDInStabPool, _troveArray, varsPriceTargetPeg);
        } else {  //  if !vars.recoveryModeAtStart
            totals = _getTotalsFromBatchLiquidate_NormalMode(activePoolCached, defaultPoolCached, varsPriceTargetPeg.price, vars.LUSDInStabPool, _troveArray);
        }
    ...
    */
        ...
        returns(LiquidationTotals memory totals)
    {
        ...
        for (vars.i = 0; vars.i < _troveArray.length; vars.i++) {
            ...
            if (!vars.backToNormalMode) {
                ...
                singleLiquidation = _liquidateRecoveryMode(_activePool, _defaultPool, vars.user, vars.ICR, vars.remainingLUSDInStabPool, TCR, _priceTargetPeg);
        ...
        internal returns (SingleRedemptionValues memory singleRedemption)
    {
        ...
        singleRedemption.ETHLot = singleRedemption.LUSDLot.mul(_priceTargetPeg.targetPeg).div(_priceTargetPeg.price); //now includes moving target peg price of USDL. 
    ...
    */
        ...
        override
    {
        ...
        PriceTargetPeg memory priceTargetPeg;
        _requireValidMaxFeePercentage(_maxFeePercentage);
        _requireAfterBootstrapPeriod();
        priceTargetPeg.price = priceFeed.fetchPrice();
        priceTargetPeg.targetPeg = inflationFeed.getTargetPeg();
        _requireTCRoverMCR(priceTargetPeg.price, priceTargetPeg.targetPeg);
        ...
        if (_isValidFirstRedemptionHint(contractsCache.sortedTroves, _firstRedemptionHint, priceTargetPeg.price)) {
            currentBorrower = _firstRedemptionHint;
        } else {
            currentBorrower = contractsCache.sortedTroves.getLast();
            // Find the first trove with ICR >= MCR
            while (currentBorrower != address(0) && getCurrentICR(currentBorrower, priceTargetPeg.price) < MCR) {
        ...
        require(totals.totalETHDrawn > 0, "Unable to redeem");

        // Decay the baseRate due to time passed, and then increase it according to the size of this redemption.
        // Use the saved total LUSD supply value, from before it was reduced by the redemption.
        _updateBaseRateFromRedemption(totals.totalETHDrawn, totals.totalLUSDSupplyAtStart, priceTargetPeg);
    ...
    function getCurrentICR(address _borrower, uint _price) public view override returns (uint) {
        ...
        uint targetPeg = inflationFeed.getTargetPeg();
        uint ICR = LiquityMath._computeCR(currentETH, currentLUSDDebt, _price, targetPeg);
        return ICR;
    }
    ...
    */
    ...
    function getTCR(uint _price) external view override returns (uint) {
        uint targetPeg = inflationFeed.getTargetPeg();
        return _getTCR(_price, targetPeg);
    }

    function checkRecoveryMode(uint _price) external view override returns (bool) {
        uint targetPeg = inflationFeed.getTargetPeg();
        return _checkRecoveryMode(_price, targetPeg);
    }
    ...
    returns (bool)
    {
        uint TCR = LiquityMath._computeCR(_entireSystemColl, _entireSystemDebt, _priceTargetPeg.price, _priceTargetPeg.targetPeg);
    ...
    */
    function _updateBaseRateFromRedemption(uint _ETHDrawn, uint _totalLUSDSupply, PriceTargetPeg memory _priceTargetPeg) internal returns (uint) {
        uint decayedBaseRate = _calcDecayedBaseRate();

        /* Convert the drawn ETH back to LUSD at face value rate (1 LUSD:1 USD), in order to get
        * the fraction of total supply that was redeemed at face value. */
        // USDL Change: added targetPeg to calculations to convert USDL to USD as well.
        uint redeemedLUSDFraction = _ETHDrawn.mul(_priceTargetPeg.price).div(_totalLUSDSupply.mul(_priceTargetPeg.targetPeg).div(DECIMAL_PRECISION)); 
    ...
    function _calcRedemptionFee(uint _redemptionRate, uint _ETHDrawn) internal pure returns (uint) {
        ...
        require(redemptionFee < _ETHDrawn, "Fee would eat up all returned collateral");
    ...
    function _requireTroveIsActive(address _borrower) internal view {
        require(Troves[_borrower].status == Status.active, "Trove not exist or closed");
    }

    function _requireLUSDBalanceCoversRedemption(ILUSDToken _lusdToken, address _redeemer, uint _amount) internal view {
        require(_lusdToken.balanceOf(_redeemer) >= _amount, "Redemption amount must be <= user's LUSD token balance");
    }

    function _requireMoreThanOneTroveInSystem(uint TroveOwnersArrayLength) internal view {
        require (TroveOwnersArrayLength > 1 && sortedTroves.getSize() > 1, "Only one trove in the system");
    }
    ...
    function _requireTCRoverMCR(uint _price, uint _targetPeg) internal view {
        require(_getTCR(_price, _targetPeg) >= MCR, "Cannot redeem when TCR < MCR");
    }

    function _requireAfterBootstrapPeriod() internal view {
        require(presaleEndTime > 0, "Presale Not Ended");
        require(block.timestamp >= presaleEndTime.add(BOOTSTRAP_PERIOD), "Redemptions Not allowed during bootstrap");
    }

    function _requireValidMaxFeePercentage(uint _maxFeePercentage) internal pure {
        require(_maxFeePercentage >= REDEMPTION_FEE_FLOOR && _maxFeePercentage <= DECIMAL_PRECISION,
            "Max fee must be 0.5~100%");
    }
    ...
    function setPresaleEndTime() external override {
        _requireCallerIsBorrowerOperations();
...
contract LiquityBase is BaseMath, ILiquityBase {
    ...
    // uint constant public LUSD_GAS_COMPENSATION = 200e18; // 200 USDL 
    uint constant public LUSD_GAS_COMPENSATION = 10e18; // 10 USDL

    // Minimum amount of net LUSD debt a trove must have
    // uint constant public MIN_NET_DEBT = 1800e18; // 1800 USDL
    uint constant public MIN_NET_DEBT = 40e18; // 40 USDL
