...
contract USDRMigrationV2 is Ownable, Pausable {
    ...
    constructor(
        ...
        address _wusdr,
        bool _isMainChain,
        bytes32[] memory _alreadyRescued
    ) {
        oldUSDR = _oldUSDR;
        oldWUSDR = _oldWUSDR;
        usdr = _usdr;
        ...
        for (uint256 i = _alreadyRescued.length; i != 0; ) {
            unchecked {
                --i;
            }
            _rescueHashes[_alreadyRescued[i]] = true;
        }
    }
    ...
    function rescueWrappedUSDR(
        ...
        _rescueHashes[_hash] = true;
        IMintable(wusdr).mint(_account, _amount);
