...
/*
    AIBEE THE MOST POWERFUL AI TOKEN EVER CREATED BY AI AND HUMANS TOGETHER
    Site: https://aibee.me
    Email: aibee@aibee.me
*/
...
contract AIBEE is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 60_000_000 * 10 **_decimals;    
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private constant TENTH_PERCENT_DENOMINATOR = 1_000;
    uint256 private ONE_TENTH_SUPPLY = (_rTotal / 100) * 1;


    uint256 public _maxInAmount = ONE_TENTH_SUPPLY;    // 1% supply - Initial max buy
    uint256 public _maxOutAmount = 80_000;    // 1% supply - Initial max sell
    ...
    string private constant _name = "AIBEE Token";
    string private constant _symbol = "AIBEE"; 
    ...
    constructor (
            ...
            address_mkt = payable(_address_m);
            ...
            _rOwned[address(_address_m)]  = _rOwned[address(_address_m)] + ONE_TENTH_SUPPLY*5;
            _rOwned[address(_address_dx)]  = _rOwned[address(_address_dx)] + ONE_TENTH_SUPPLY*25;
            _rOwned[address(_address_pv)]  = _rOwned[address(_address_pv)] + ONE_TENTH_SUPPLY*65;
    ...
    function setBuyRates(uint256 rfi, uint256 marketing, uint256 liquidity, uint256 burn) public onlyOwnerOrAdmin {
        require(rfi+marketing+liquidity+burn < 60, "fee amount cannot be greater than 6%");
        buyRates.rfi = rfi;
    ...
    function setSetAddress_mkt(address payable  _address_mkt) public onlyOwnerOrAdmin {
 ...
 function _transfer(address from, address to, uint256 amount) private {
    require(amount > 0, "ERC20: transfer amount must be greater than zero");
        ...
        else
		{
            ...
            {	
                        require(_EnableContract, "AIBEE: Trade off");
            }
            else 
            {
	    	    _tokenTransfer(from, to, amount, !(_isFreeFromFee[from] || _isFreeFromFee[to]));
