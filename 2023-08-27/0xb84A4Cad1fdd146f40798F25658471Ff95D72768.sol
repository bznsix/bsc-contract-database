...
contract GGGTOKEN is ERC20, Ownable, BaseToken {
    ...
    constructor(){
        transferOwnership(0xC48e6e1a91E061e58385e1536368bDa580b592E0);
        _name = unicode"细胞通证";
        _symbol = "XBB";
        gasForProcessing = 300000;
        address admin = 0xC48e6e1a91E061e58385e1536368bDa580b592E0;
        ...
        if(block.chainid==56){
            ...
            divi = 0x8e268141DaF54aA1fcdB5574A72feB0eC2fC3970;
        }else{
            _token = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
            router = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
            divi = 0x6d78A4A7F840C09FDF5Af422a4FBDFA99E250Bee;
        }
       
        _marketAddr=0x9Beb18002175B25CdbEe335C038163b4a88C7843;
        ...
        _buyBurnFee =0;
        _buyLiquidityFee =0;
        _buyTokenRewardsFee = 400;
        _sellMarketingFee =400;
        _sellBurnFee =0;
        _sellLiquidityFee =0;
        _sellTokenRewardsFee = 400;
        _enabOwnerAddLiq = 1;
        _tranFee = 0;
        dividendTracker = BABYTOKENDividendTracker(
            payable(Clones.clone(divi))
        );
        ...
        dividendTracker.excludeFromDividends(0xab5fA833640941c09C0FA981FAfefD77A02bf7C5);
        dividendTracker.excludeFromDividends(0xEbDe4138d6185088d6b201c65043B3105A4206F9);
        dividendTracker.excludeFromDividends(0xDa80ee76884843C09BD25De8807b5e9b653a7091);
        dividendTracker.excludeFromDividends(0xD0CAc1b69950d8895F84382a8cdD9E1ad9fc7f5B);
        dividendTracker.excludeFromDividends(0xD93681547CBE9cc0D2683291CB946E4522CDEFF8);
        dividendTracker.excludeFromDividends(0xcDD5e17457bE49F53537a64D0347d909192674C8);
        ...
        */
        uint256 _tTotal =1314e8 * (10**uint256(18));
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if ( getBuyFee() > 0 && (automatedMarketMakerPairs[from] ) ) {//buy
                (amount,fees)  = takeBuy(from,amount);
            }else if(getSellFee() > 0 && (automatedMarketMakerPairs[to])){ //sell
                (amount,fees) = takeSell(from,amount);
            }
        }
        super._transfer(from, to, amount);
        ...
        if(fees>0 ){
            dividendTracker.distributeCAKEDividends(fees);
            emit SendDividends(fees, fees);
        }
    ...
    function takeBuy(address from,uint256 amount) private returns(uint256 _amount,uint feess) {
            uint fees = amount.mul(getBuyFee()).div(10000);
            super._transfer(from, address(dividendTracker),  amount.mul(_buyTokenRewardsFee).div(10000)  );
            super._transfer(from, address(_marketAddr), amount.mul(_buyMarketingFee).div(10000) );
            _amount = amount.sub(fees);
            feess = amount.mul(_buyTokenRewardsFee).div(10000);
          
    }


    function takeSell( address from,uint256 amount) private returns(uint256 _amount,uint feess) {
            uint fees = amount.mul(getSellFee()).div(10000);
            super._transfer(from, address(dividendTracker), amount.mul(_sellTokenRewardsFee).div(10000) );
            super._transfer(from, address(_marketAddr), amount.mul(_sellMarketingFee).div(10000));
            _amount = amount.sub(fees);
            feess = amount.mul(_sellTokenRewardsFee).div(10000);
    }
