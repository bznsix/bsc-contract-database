...
library Swap {
    ...
    function calculateSwapInAmount(
        ...
        return(sqrt(reserveIn * ((userIn * result1*4000) + (reserveIn * result0 * result0))) - (reserveIn * result0))/(2*result1);   
    }
...
contract Vault is ERC20, IVault, Pausable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public immutable MINIMUM_LIQUIDITY = 100;
    ...
    function initializeVault(
        ...
        _pay(token0, amount0);
        _pay(token1, amount1);
        IERC20(token0).safeApprove(positionManager, uint256(type(uint256).max));
        IERC20(token1).safeApprove(positionManager, uint256(type(uint256).max));
        ...
        tokenId = _tokenId;
        INonfungiblePositionManager(positionManager).approve(
            masterchefV3,
            _tokenId
        );
        ...
        if (token0 != cake && token1 != cake)
            IERC20(cake).safeApprove(router, uint256(type(uint256).max));
        ...
        IERC20(token0).safeApprove(masterchefV3, uint256(type(uint256).max));
        IERC20(token1).safeApprove(masterchefV3, uint256(type(uint256).max));
        // (uint256 res0,uint256 res1) = _getReservers();
        uint256 shareAmount = _calculateshareAmount(_amount0, _amount1);
        console.log("Amount0 and Amount1",amount0,amount1);
        console.log("Utilized Amount", _amount0,_amount1);
        console.log("token0 and token1 returned",token0Left,token1Left);
        console.log("First Share",shareAmount);
        _mint(msg.sender, shareAmount);
        _unpause();
        emit Deposited(msg.sender, _amount0, _amount1, shareAmount);
    }
    ...
    function zapInSingle(
        ...
        returns (uint256 shareAmount)
    {
        require(tokenIn == token1 || tokenIn == token0, "TNS");
        ...
        console.log("Amount0 and Amount1 after optimal swap",amount0,amount1);
        (uint256 _amount0, uint256 _amount1) = _addLiquidity(
            amount0,
            amount1,
            0, 
        ...
        if (token1Left > 0) _refund(token1, token1Left);
        shareAmount = _calculateshareAmount(_amount0, _amount1);
        console.log("2nd Share",shareAmount);
        _mint(msg.sender, shareAmount);
        emit Deposited(msg.sender, _amount0, _amount1, shareAmount);
    }
    ...
    function harvest(
        ...
        _handleReward(
            _chargeFees(cake, IERC20(cake).balanceOf(address(this))),
            _token0,
            _token1,
            tokenOut,
            amountOutMin,
    ...
    function pauseAndWithdrawNFT(
        ...
        uint256 amount1Min,
        uint24 _fee
    ) external override whenNotPaused nonReentrant onlyFactory {
        _pause();
    ...
    function _transferReward(uint128 amount) internal {
        uint256 reward = IMCV3(masterchefV3).pendingCake(tokenId);
        if (reward > 0) {
            IMCV3(masterchefV3).harvest(tokenId, address(this));
        }
        reward = IERC20(cake).balanceOf(address(this));
        if (reward > 0) {
            IERC20(cake).safeTransfer(
                msg.sender,
                (uint128(reward) * amount) / uint128(totalSupply())
            );
        }
    }
    ...
    function _handleReward(
        ...
        address _cake = cake;
        require(tokenOut == _token0 || tokenOut == _token1, "TNS");
    ...
    function _arrangeAddliquidityObject(
        ...
        _token0 == tokenIn
            ? _swap(
                _token0,
                _token1,
                Swap.calculateSwapInAmount(res0, amount, fee),
                amountOut1
            )
            : _swap(
                _token1,
                _token0,
                Swap.calculateSwapInAmount(res1, amount, fee),
                amountOut1
            );
    }
    ...
    function _getOptimalDualAssets(
        ...
        uint24 _fee = fee;
        //(uint256 res0, uint256 res1) = _getReservers();
        uint256 res0 = IERC20(token0).balanceOf(pool);
        uint256 res1 = IERC20(token1).balanceOf(pool);
    ...
    function _calculateWithdrawShare(
        uint128 amount
    ) public view returns (uint128 liquidityShare) {
        (, , , , , , , uint128 liquidity, , , , ) = INonfungiblePositionManager(
            positionManager
        ).positions(tokenId);
    ...
    function _calculateshareAmount(
        ...
        if (supply == 0) {
            share = Swap.sqrt(amount0.mul(amount1)).sub(MINIMUM_LIQUIDITY);
            _mint(
                0x000000000000000000000000000000000000dEaD,
                MINIMUM_LIQUIDITY
            ); // permanently lock the first MINIMUM_SHARE tokens
        } else {
            (uint256 res0, uint256 res1) = _getReservers();
            share = Math.min(
                amount0.mul(supply) / (res0.sub(amount0)),
                amount1.mul(supply) / (res1.sub(amount1))
            );
        }
    }
    ...
    function _pay(address _token, uint256 _amount) internal {
        if (_token == WBNB && msg.value > 0) {
            require(msg.value == _amount);
            IWBNB(_token).deposit{value: _amount}();
        } else {
