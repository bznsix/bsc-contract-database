...
contract QJKToken is Context, IERC20, IERC20Metadata, Ownable {
    ...
    address public receiverTe = 0x1fe0EC5fb690537A4320129e6b0D1C2f721125D7;

    //商学院44
    address public receiverT = 0x45A9655F50f76f863FbFc56041FFfeDc6009FB72;
//

    address public receiverTk = 0x4c846697D2ee3d866aE286bd2432C3DaB4F5504a;
//
//    address public receiverTw = 0xEae47522d5D32ccd6A6D34DFda10E440C9e26C77;


    address public resonancePoolAddress = 0xcA6AbdE906F28A29ebB69C45fE41680CBAcbAa4f;
    Automatic2 public resonancePool = Automatic2(resonancePoolAddress);


    address public lpQuantityDividendsAddress = 0xF62e2705dEe786047585c2070d5d690a9860f912;
    LPQuantityDividends2 public lpQuantityDividends = LPQuantityDividends2(lpQuantityDividendsAddress);

    uint256 public clickTime = 1800;
...
//        _mint(receiverTe, 66000000000 * (10 ** 18) * 38 / 100);
//        _mint(receiverT, 66000000000 * (10 ** 18) * 30 / 100);
//        _mint(receiverTk, 66000000000 * (10 ** 18) * 30 / 100);
//        _mint(receiverTw, 66000000000 * (10 ** 18) * 2 / 100);

        //共振池
        _mint(resonancePoolAddress, 5000000000 * (10 ** 18));

        //用户加流动性池
        _mint(receiverTe, 20000000000 * (10 ** 18));

        //商学院
        _mint(receiverT, 4400000000 * (10 ** 18));

        //私募团队
        _mint(receiverTe, 33800000000 * (10 ** 18));

        //空投

        _mint(receiverTk, 2800000000 * (10 ** 18));
...
//            _transfer(from, to, amount);
    ...
    function getReserveAmount(uint amountIn) public view returns (uint amountOut) {
        (uint112 reserve0, uint112 reserve1,) = V2Pair.getReserves();
        //        amountOut = ( * amountIn) / reserve0;
