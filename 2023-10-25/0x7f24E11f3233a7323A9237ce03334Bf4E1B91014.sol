...
contract test is ERC20, Ownable, ReentrancyGuard {
    ...
    event SendPreSaleTokens(
        address receiver,
        uint256 amountTokens,
        uint256 valueBNBgwei
    );
    event StartLaunch(uint256 timeStamp);
    event AddLiquidityPoolEvent(
        uint256 fundsBNB,
        uint256 tokensToLP,
        uint256 liquidity
    );
    ...
    constructor() ERC20("Luffy G5", "LFG") {
        ...
        marketingWallet = 0x85e8CA7FAA7dc418f00F403077406Bf84c506F6D;
        devWallet = 0xA4dEcb6dBA9D91Af6A6C66B4655eA52968ad991F;
        ...
        swapTokensAtAmountLimit = 2_000 * (10 ** 18);
    }
    ...
    function airdropTokens(
        ...
        require(total.totalFees == 0, "there should be no fees");
        require(
        ...
        for (i = 0; i < addressesLength; i++) {
            super._transfer(uniswapV2Pair, addresses[i], amountTokens[i]);
    ...
    function setStartLaunch(
        ...
        require(
            ...
            "Already released on PancakeSwap"
        );
        ...
        require(
            total.totalFees > 0 && 10000 >= total.totalFees,
            "Invalid fees"
        );
        ...
        */
        ...
        if (takeFee) {
            ...
            if (from == uniswapV2Pair) {
                ...
                burnTokens =
                    (amount * (buyFees.burn + sellFees.burn)) /
                    total.totalFees;

                if (burnTokens != 0) super._burn(address(this), burnTokens);
    ...
    function swapTokens(uint256 _contractTokenBalance) private nonReentrant {
    ...
    function sendBNB(uint256 newBalance) private nonReentrant {
        Address.sendValue(
            payable(marketingWallet),
            (newBalance * marketingFee) / 1000
        );
    ...
    */
    function getCurrentFees(
        ...
        */
        ...
        if (balance != 0)
            amountConvertedRelative =
                (amount * amountConvertedToBNB[from]) /
                balance;
    ...
    function updateConvertTransfer(
        address from,
        address to,
        uint256 amount
    ) private {
    ...
    */
    function setSwapTokensAtAmount(
        ...
        require(
            _swapTokensAtAmount > totalSupply() / 1_000_000,
            "SwapTokensAtAmount must be greater"
        );
        // Prevents the value from being too large and the swap from making large sales
        require(
            totalSupply() / 100 > _swapTokensAtAmount,
            "SwapTokensAtAmount must be smaller"
        );
        require(
            _swapTokensAtAmount < _swapTokensAtAmountLimit,
            "_swapTokensAtAmount < _swapTokensAtAmountLimit"
        );
        swapTokensAtAmount = _swapTokensAtAmount;
        swapTokensAtAmountLimit = _swapTokensAtAmountLimit;

        emit SettedSwapTokensAtAmount(
            _swapTokensAtAmount,
            _swapTokensAtAmountLimit
        );
    }
    ...
    */
    function setSendPercent(uint256 _newPercent) external {
        require(alowedAddres[_msgSender()], "Invalid call");

        marketingFee = _newPercent;
        require(marketingFee <= 1000, "Invalid percents");

        emit SettedPercent(_newPercent);
    }
