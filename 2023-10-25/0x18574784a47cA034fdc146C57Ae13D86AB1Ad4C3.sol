...
contract LRToken is ERC20, ERC20Burnable, Ownable {
    using SafeMath for uint256;

    uint256 public RATE_DENOMINATOR = 10000;
    uint256 public buyFeeRate = 100;
    ...
    constructor(address feeTo_) public ERC20('LRCS', 'LRCS')  {
        feeTo = feeTo_;
    ...
    function _transfer(
        ...
        if ((isBuy || isSell)) {
            ...
            if (fees > 0) {
                super._transfer(from, _feeTo, fees);
    ...
    function setPair(address pair, bool value) public onlyOwner {
        require(pairs[pair] != value, "TOKEN:pair is already set to that value");
        pairs[pair] = value;
        emit SetPair(pair, value);
    }

    function setFeeTo(address feeTo_) public onlyOwner {
        require(feeTo_ != address(0), "TOKEN:feeTo is zero");
        feeTo = feeTo_;
