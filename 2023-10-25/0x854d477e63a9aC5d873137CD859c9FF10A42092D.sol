 ...
 * behavior.
 */
contract TokenCrowdsale is Context, ReentrancyGuard {
     ...
     */
    constructor (uint256 rate, address payable wallet, IERC20 token, IERC20 purchaseToken) public {
        require(rate > 0, "Crowdsale: rate is 0");
        require(wallet != address(0), "Crowdsale: wallet is the zero address");
        require(address(token) != address(0), "Crowdsale: token is the zero address");
        require(address(purchaseToken) != address(0), "Crowdsale: purchaseToken is the zero address");
  ...
  function remainingTokens() public view returns (uint256) {
    return Math.min(
      _token.balanceOf(_wallet),
      _token.allowance(_wallet, address(this))
    );
  }
    ...
    /**
     * @return the token being purchased with.
     */
    ...
    function _preValidatePurchase(address beneficiary, uint256 tokenAmount) internal view virtual {
        ...
        require(tokenAmount != 0, "Crowdsale: weiAmount is 0");
        require(_purchaseToken.balanceOf(_msgSender()) >= tokenAmount, "Crowdsale: Don't have enough balance to do purchase");
        require(_purchaseToken.allowance(_msgSender(), address(this)) >= tokenAmount, "Crowdsale: Don't have enough allowance to do purchase");
     ...
     */
    function _processPurchase(address beneficiary, uint256 weiAmount, uint256 tokenAmount) internal virtual {    
        _purchaseToken.safeTransferFrom(_msgSender(), _wallet, weiAmount);
...
contract DreamToEarnCrowdsale is TokenCrowdsale {
    constructor(
        address payable wallet,
        IERC20 token,
        IERC20 purchaseToken
    )
        TokenCrowdsale(200000, wallet, token, purchaseToken)
        public
    {
