...
/_/  |_/____/\__/_/   \____/\____/
 ...
 AstroG is an improvement on the "Q" contract (shout out to DiamondQ).
 Each transaction is timed, early sells are punished with a higher tax
 reducing the overall supply. This is a mechanism intended to prevent
 measures of buy the rumour and sell the news.

 Buy tax:
     Tax is the same no matter the time:
         5% to the house wallet
 Sell tax:
     If selling within 1 day:
         5% sent to the house wallet
         15% sent to the burn wallet
     If selling within 1-2 days:
         5% sent to the house wallet
         10% sent to the burn wallet
     If selling within 2-3 days:
         5% sent to the house wallet
         5% sent the burn wallet
     If selling after 3 days:
         5% sent to the house wallet
...
contract AstroG is Context, IERC20, Ownable {
    ...
    uint256 constant DEFAULT_PAPER_HAND_FEE = 15;
    uint256 private _currentPaperHandFee = 15;
    uint256 private _paperHandTime = 1 days;

    uint256 constant DEFAULT_GATE1_FEE = 10;
    uint256 private _currentGate1Fee = 10;
    uint256 private _gate1Time = 2 days;

    uint256 constant DEFAULT_GATE2_FEE = 5;
    uint256 private _currentGate2Fee = 5;
    uint256 private _gate2Time = 3 days;

    string private _name = "AstroG";
    string private _symbol = "AstroG";
    ...
    uint256 private _totalTokens = 500 * 10**6 * 10**9;
    uint256 private _minTokensBeforeSwap = 500 * 10**3 * 10**9;

    address payable private _houseContract = payable(0xBB954F2fd06c8c95A4aD71f25A6b2eD47f5965dc);
    ...
    function balanceLessThan1Day(address account) external view returns (uint256) {
        ...
        for (uint i = 0; i < _timedTransactionsMap[account].txBlockTimes.length; i++) {
            ...
            // Only add up balance in the last day
    ...
    function balanceBetween1And2Days(address account) external view returns (uint256) {
        uint256 totalTokens = 0;
        ...
        for (uint i = 0; i < _timedTransactionsMap[account].txBlockTimes.length; i++) {
            ...
            // Only add up balance in the last 1-2 days
    ...
    function balanceBetween2And3Days(address account) external view returns (uint256) {
        uint256 totalTokens = 0;
        ...
        for (uint i = 0; i < _timedTransactionsMap[account].txBlockTimes.length; i++) {
            ...
            // Only add up balance in the last 2-3 days
    ...
    function balanceOver3Days(address account) public view returns (uint256) {
        uint256 totalTokens = 0;
        ...
        for (uint i = 0; i < _timedTransactionsMap[account].txBlockTimes.length; i++) {
            ...
            // Only add up balance over the last 3 days
    ...
    function _transfer(
        ...
        if (isOnlyDiamondHandTx) {
            require(balanceOver3Days(from) >= amount, "Insufficient diamond hand token balance");
        }
    ...
    function _reduceSenderBalance(address sender, address recipient, uint256 initialTransferAmount) private returns (uint256) {
        ...
        // Keep track of the amount of tokens to be burned
    ...
    function _increaseRecipientBalance(address recipient, uint256 transferAmount, bool isDiamondHandOnlyTx) private {
        ...
        // If transaction was within the past 6 hours then keep as part of the same bucket for efficiency
        if (lastTxTime > block.timestamp - 6 hours) {
    ...
    function removeGateFees() external onlyOwner {
        _currentPaperHandFee = 0;
        _currentGate1Fee = 0;
        _currentGate2Fee = 0;
    }

    function reinstateGateFees() external onlyOwner {
    ...
    function _aggregateOldTransactions(address sender) private {
        ...
        // If the first transaction is not at the max age then do not aggregate
