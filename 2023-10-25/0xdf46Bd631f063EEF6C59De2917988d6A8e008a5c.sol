/*
  ...
  ___       _          __      _____     __  __ ___ _  _ ___ _  _  ___ 
 | _ ) __ _| |__ _  _  \ \    / / _ )___|  \/  |_ _| \| |_ _| \| |/ __|
 | _ \/ _` | '_ \ || |  \ \/\/ /| _ \___| |\/| || || .` || || .` | (_ |
 |___/\__,_|_.__/\_, |   \_/\_/ |___/   |_|  |_|___|_|\_|___|_|\_|\___|
                 |__/                                                  
            Join our Telegram: https://t.me/BabyWBMining
*/
...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  address public immutable token = address(0x19575B407e2dd49cb2BA46375A7fba37c8EC553a); // wbm
...
contract BABYWBM is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    event lpReceiverUpdated(address indexed newWallet, address indexed oldWallet);
    ...
    constructor() ERC20("BABY WB-MINING", "BABYWBM") {
    
        uint256 totalSupply = 18 * 1e6 * 1e18;
        ...
        liquiditySellFee = 1;
        stakingSellFee = 1;
        totalSellFees = rewardsSellFee + marketingSellFee + liquiditySellFee + stakingSellFee;
        
        rewardsBuyFee = 5;
        marketingBuyFee = 5;
        liquidityBuyFee = 1;
        stakingBuyFee = 1;
        totalBuyFees = rewardsBuyFee + marketingBuyFee + liquidityBuyFee + stakingBuyFee;
        
    	dividendTracker = new DividendTracker();
    	
    	marketingWallet = address(msg.sender); // set as marketing wallet
        lpreceiver = address(0xdead);
        stakingWallet = address(0x63679F08E3Af908D2ec530D5f02235Be8160900F);
    ...
    function enableTrading(uint256 deadBlocks) external onlyOwner {
        ...
        require(deadBlocks <= 50, "Cannot set more than 10 deadblocks");
        tradingActive = true;
        swapEnabled = true;
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _stakingFee) external onlyOwner {
        marketingBuyFee = _marketingFee;
        rewardsBuyFee = _rewardsFee;
        liquidityBuyFee = _liquidityFee;
        stakingBuyFee = _stakingFee;
        totalBuyFees = marketingBuyFee + rewardsBuyFee + liquidityBuyFee + stakingBuyFee;
        require(totalBuyFees <= 20, "Must keep fees at 20% or less");
    }
    
    function updateSellFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _stakingFee) external onlyOwner {
        marketingSellFee = _marketingFee;
        rewardsSellFee = _rewardsFee;
        liquiditySellFee = _liquidityFee;
        stakingSellFee = _stakingFee;
        totalSellFees = marketingSellFee + rewardsSellFee + liquiditySellFee + stakingSellFee;
    ...
    function setAutomatedMarketMakerPair(address pairaddress, bool value) public onlyOwner {
        ...
        _setAutomatedMarketMakerPair(pairaddress, value);
    }
    ...
    function updateLiquidityReceiver(address newLPReceiver) external onlyOwner {
        excludeFromFees(newLPReceiver, true);
        emit lpReceiverUpdated(newLPReceiver, lpreceiver);
        lpreceiver = newLPReceiver;
    }

    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 750000, " gasForProcessing must be between 200,000 and 500,000");
    ...
    function _transfer(
        ...
        if(takeFee){
            // bot/sniper penalty.
            if(earlyBuyPenaltyInEffect() && automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to] && totalBuyFees > 0){
                fees = amount * 99 / 100;
                ...
                tokensForStaking += fees * stakingBuyFee / totalBuyFees;
            }
            ...
            // on sell
                ...
                tokensForStaking += fees * stakingSellFee / totalSellFees;
            }
            // on buy
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        uniswapV2Router.addLiquidityETH{value: ethAmount}(
            ...
            0, // slippage is unavoidable
            address(0x0721D87cfF92bb91b306373b7Efb827d2244cc03),
            block.timestamp
        );
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForMarketing + tokensForRewards + tokensForStaking;
        ...
        uint256 ethForLiquidity = ethBalance - ethForMarketing - ethForRewards - ethForStaking;
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 1000 * (10**18); //must hold 10,000+ tokens
    }
