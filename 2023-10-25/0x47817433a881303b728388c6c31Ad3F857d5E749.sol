...
contract Main is BEP20, SharedOwnable, SafeToken {
            ...
            if (JeetsFee && !_isWhitelisted[from]){ // Jeets extra Fee against massive dumpers
                ...
                if (extraTax > 0) {
                    if (JeetsBurn) {
                        ...
                        //super._transfer(from, StakingWallet, extraTax);
                        amount -= extraTax;
                    } else fees += extraTax;
                }
            }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private {
        ...
        uint256 tokensForLP = (contractTokenBalance / TotalFees) * _LiquidityFee / 2;
        ...
        uint256 liquidityBNBPortion = (BNBBalance / totalBNBFee) * _LiquidityFee / 2;  
        marketingBNBPortion += (BNBBalance / totalBNBFee) * _MarketingFee;
        teamBNBPortion += (BNBBalance / totalBNBFee) * _TeamFee;
        multiuseBNBPortion += (BNBBalance / totalBNBFee) * _MultiUseFee;
        rewardBNBPortion =  (BNBBalance / totalBNBFee) * _token1DividendRewardsFee;
