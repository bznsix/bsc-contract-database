...
contract OctopusesPool is Ownable {
    ...
    uint256 constant STAKING_DURATION = 28703 * 30;
    uint256 constant COOL_OFF_DURATION = 28703 * 3;
    ...
    function recalculateRewardPerBlockPerNFT() internal {
        if (totalStakedNFTs > 0) {
            rewardPerBlockPerNFT = rewardToken
                .balanceOf(address(this))
                .div(STAKING_DURATION)
                .div(totalStakedNFTs);
        } else {
    ...
    function stakeNFT(uint256 _tokenId) external {
        ...
        stakers[msg.sender].eligibleToUnstake = false;
    ...
    function unstakeNFT(uint256 _tokenId) external {
        require(
            (!stakingActive) ||
                (block.number > endBlock && block.number < coolOffEndBlock),
        ...
        require(
            stakers[msg.sender].eligibleToUnstake,
            "Pending rewards must be claimed before unstaking"
        );
    ...
    function claimRewards() external {
        ...
        if (isInCoolOffPeriod()) {
            stakers[msg.sender].eligibleToUnstake = true;
        }
    ...
    function restartStaking(uint256 _additionalRewardAmount)
        external
        onlyOwner
    {
    ...
    function getTokenIdByWallet(address walletAddress)
        public
        view
...
function emergencyUnstakeByID(address stakerAddress, uint256 _tokenId) external onlyOwner {
    require(nftToken.ownerOf(_tokenId) == address(this), "Token not staked");
    uint256[] storage stakedTokens = stakers[stakerAddress].stakedTokens;
    for (uint256 i = 0; i < stakedTokens.length; i++) {
        if (stakedTokens[i] == _tokenId) {
            stakedTokens[i] = stakedTokens[stakedTokens.length - 1];
            stakedTokens.pop();
    ...
    nftToken.transferFrom(address(this), stakerAddress, _tokenId);
    totalStakedNFTs--;
    stakers[stakerAddress].nftCount--;
    emit Unstaked(stakerAddress, _tokenId);
    recalculateRewardPerBlockPerNFT();
}

function emergencyFeed(uint256 _amount) external onlyOwner {
    require(
        rewardToken.transferFrom(msg.sender, address(this), _amount),
        "Token transfer failed"
    );
    emit EmergencyFeed(msg.sender, _amount);
}

event EmergencyFeed(address indexed feeder, uint256 amount);
}