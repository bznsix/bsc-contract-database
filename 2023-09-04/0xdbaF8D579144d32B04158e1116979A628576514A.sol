...
/** #$#
#$# After deployment:
#$# 1. Set this contract as MINTER role for reward NFT.
#$# */
...
contract StakingMapV2MilestoneForCardNFT is Ownable, Pausable {
    ...
    function receiveRewards(uint256[] calldata _rewardIds) external whenNotPaused {
        uint256 _totalCost = 0;
        uint256 _len = _rewardIds.length;
        uint256 i;
        for (i = 0; i < _len; i++) {
            _totalCost += uint256(REWARDS[_rewardIds[i]].pointsCostWithFraction);
        }
        ...
        for (i = 0; i < _len; i++) {
            REWARDS_CONTRACT.mint(_msgSender(), uint256(REWARDS[_rewardIds[i]].rewardId));
        }

        emit RewardReceived(_msgSender(), _rewardIds);
    }
