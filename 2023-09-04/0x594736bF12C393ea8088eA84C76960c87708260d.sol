...
// TODO:
// clear other toods
// technically, we need to track balance of all registred pools
...
contract Scale is OFTCoreUpgradeable, IERC20Upgradeable {
    ...
    uint256 constant LAUNCH_FEE = 5_000;
    ...
    uint256 public totalSubLPBalance;
    ...
    // TODO could pack those more tightly & read cheaper
    ...
    function initialize(
        ...
        totalReflected = 0;
        totalSubLPBalance = _totalSupply;
    ...
    function baseToReflectionAmount(
        ...
        return
            isRegistredPool[account] != 0
                ? baseAmount 
                // TODO this is a problem since the diff can underflow
    ...
    function _transferFrom(
        ...
        if (
            ...
            !senderIsPool &&
            isInSwap == 0 &&
            block.timestamp - launchTime < launchFeeDuration
        ) {
            isInSwap = 1;
            ...
            // TODO use amount instead of base amount? since we convert it back anyway
            uint reflectedLaunchFeeAmount = (amount *
                LAUNCH_FEE *
                (launchFeeDuration - (block.timestamp - launchTime))) /
                launchFeeDuration /
                MAX_BP;

            baseLaunchFeeAmount = senderIsPool // if sender is registred pool
                ? reflectedLaunchFeeAmount // TODO can undeflow
                : (reflectedLaunchFeeAmount *
                    (local_totalSubLPBalance - totalReflected)) /
                    local_totalSubLPBalance;
        ...
        // the balance in the liquidity pool, thus altering the result
        {
            ...
            if (
                isInSwap == 0 &&
                // this only swaps if it's not a buy, amplifying impacts of sells and leaving buys untouched but also shifting gas costs of this to sellers only
                isRegistredPool[msg.sender] == 0 && 
                isClaimingFees != 0 &&
                // no checks for pool needed since first condition covers this already
                _baseBalance[address(this)] * totalSubLPBalance / (local_totalSubLPBalance - totalReflected) /* = balanceOf(address(this)) */ >= swapThreshold 
            ) {
                isInSwap = 1;
        ...
        uint256 baseAmount = 
            senderIsPool // if sender is registred pool
            ? amount
            // TODO can undeflow
            : amount * (local_totalSubLPBalance - totalReflected) / local_totalSubLPBalance;
        ...
        if (isInSwap != 0) {
            ...
            if(senderIsPool) totalSubLPBalance = local_totalSubLPBalance + baseAmount;
            if(recipientIsPool) totalSubLPBalance = local_totalSubLPBalance - baseAmount;

            emit Transfer(sender, recipient, amount);
            return true;
        }

        uint256 baseAmountReceived = isClaimingFees != 0 // TODO this modifies LP balance and thus also reflection amount that we previously calculated
        ...
        if(senderIsPool) totalSubLPBalance = local_totalSubLPBalance + baseAmount;
        if(recipientIsPool)  totalSubLPBalance = local_totalSubLPBalance - baseAmountReceived;
    ...
    // TODO
    function nonblockingLzReceivePublic(
        uint16 _srcChainId,
    ...
    function _receiveReflectionUpdate(
        ...
        totalReflected = totalReflected + delta;
        emit Reflect(delta, totalReflected);
