...
import { IVelodromeRouter } from "../../common/interfaces/IVelodromeRouter.sol";
...
contract PaymentModuleV1 is IDepositHandler, IPaymentModule, AccessControlEnumerable, IERC721Receiver {
    ...
    uint256 private _feeCollectedLastBlock;
    uint256 private _flokiBurnedLastBlock;
    uint256 private _referrerShareLastBlock;
    ...
    function _weth() private view returns (address) {
        try routerForFloki.WETH() returns (address weth) {
            return weth;
        } catch {
            // Some routers use lower case "weth()"
            bytes memory data;
            bool success;
            (success, data) = address(routerForFloki).staticcall(abi.encodePacked("function weth()"));
            if (!success) {
                return address(0);
            }
            address tokenAddress;
    ...
    function _swapETHForExactTokens(
        ...
        try routerForFloki.swapETHForExactTokens{ value: msg.value }(amountOut, path, to, deadline) returns (
            uint256[] memory amounts_
        ) {
            return amounts_;
        } catch {}
        IVelodromeRouter.route[] memory routes = new IVelodromeRouter.route[](path.length - 1);
        for (uint256 i = 0; i < path.length - 1; i++) {
            routes[i] = IVelodromeRouter.route(path[i], path[i + 1], false);
        }
        // This type of router doesn't support swapping for exact output amount.
        return
            IVelodromeRouter(address(routerForFloki)).swapExactETHForTokens{ value: msg.value }(
                amountOut,
                routes,
                to,
                deadline
    ...
    function _processFee(
        ...
        if (usdtAmount > 0) {
            ...
            if (referrer != address(0)) {
                ...
                emit ReferrerSharePaid(_referrerShareLastBlock, vault, referrer, referrerUSDTShare);
                _referrerShareLastBlock = block.number;
            }
            ...
            if (FLOKI != address(0)) {
                ...
                uint256 flokiBalance = IERC20(FLOKI).balanceOf(address(this));
                lpTokenProcessor.swapTokens(address(USDT), burnShare, FLOKI, burnAddress, address(routerForFloki));
                uint256 flokiBurned = IERC20(FLOKI).balanceOf(address(this)) - flokiBalance;
                emit FlokiBurned(_flokiBurnedLastBlock, vault, burnShare, flokiBurned);
                _flokiBurnedLastBlock = block.number;
            }
            uint256 treasuryUSDTShare = usdtAmount - referrerUSDTShare - burnShare;
            USDT.safeTransfer(treasury, treasuryUSDTShare);
            emit FeeCollected(_feeCollectedLastBlock, vault, treasuryUSDTShare);
            _feeCollectedLastBlock = block.number;
        }
    }
    ...
    function notifyFeeCollected(address _vault, uint256 _amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        emit FeeCollected(_feeCollectedLastBlock, _vault, _amount);
        _feeCollectedLastBlock = block.number;
    }
    ...
    function notifyFlokiBurned(
        ...
        emit FlokiBurned(_flokiBurnedLastBlock, _vault, _usdAmount, _flokiAmount);
        _flokiBurnedLastBlock = block.number;
    }
    ...
    function notifyReferrerSharePaid(
        ...
        emit ReferrerSharePaid(_referrerShareLastBlock, _vault, _referrer, _amount);
        _referrerShareLastBlock = block.number;
    }
