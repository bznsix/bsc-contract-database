...
contract GlobalBotUtility is AttributeMap, ERC20 {
  string public constant URL = "https://gpbots.com/";

  address constant DEAD = 0x000000000000000000000000000000000000dEaD;
  address constant ZERO = 0x0000000000000000000000000000000000000000;

  string constant NAME = "GLOBAL BOT UTILITY";
  string constant SYMBOL = "GBUX";

  uint constant MAX_SUPPLY = 30_000_000_000e18;

  uint constant MAX_TOTAL_FEE = 1000; // Max allowed fee is 10%
  ...
  receive() external payable {
    revert("Not allowed to receive gas");
  }
  ...
  // $ ----------------- Fee Settings -----------------
  function setFees(uint burnRate) external onlyOwner {
  ...
  function setMaxTxAmount(uint maxTxAmount_) public onlyOwner {
    require(maxTxAmount_ >= 1_000, "The new max must be bigger then 1000");
  ...
  function _transfer(
    ...
    if (!_isExemptFeeSender(senderAttributes) && !_isExemptFeeReceiver(receiverAttributes)) {
      if (feeBurnRate > 0) {
        feeAmount = (feeBurnRate * amount) / 10_000;
...
contract AttributeMap is Authorized {
  ...
  function setExemptFeeReceiver(address target, bool operation) public onlyOwner {
    _attributeMap[target] = _setExemptFeeReceiver(_attributeMap[target], operation);
  }

  function setExemptTxLimit(address target, bool operation) public onlyOwner{
