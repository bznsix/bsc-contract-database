...
// FoxlayerSplitBar with Governance.
contract FoxlayerSplitBar is BEP20("FoxlayerSplitBar Token", "FOXLAYERSPLIT") {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterFox).
    ...
    constructor(FoxlayerToken _cake) public {
        cake = _cake;
    }
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "FOXLAYER::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "FOXLAYER::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "FOXLAYER::delegateBySig: signature expired");
    ...
    function getPriorVotes(
        ...
        require(
            ...
            "FOXLAYER::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying FOXLAYERs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "FOXLAYER::_writeCheckpoint: block number exceeds 32 bits"
        );
...
// FoxlayerToken with Governance.
contract FoxlayerToken is BEP20("FoxSwapFinance Foxlayer", "FOXLAYER") {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterFox).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
