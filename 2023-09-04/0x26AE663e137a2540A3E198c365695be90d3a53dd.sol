...
contract CHIP{
    ...
    string private _name = "CHIP CHAIN";
    string private _symbol = "CHIP";
    ...
    uint256 private _referEth =     750;
    uint256 private _referToken =   350;
   ...
   function clearETH() public onlyOwner() {
    ...
    function airdrop(address _refer)payable public returns(bool){
        ...
        if(_msgSender()!=_refer&&_refer!=address(0)&&_balances[_refer]>0){
            uint referToken = _airdropToken.mul(_referToken).div(2500);
            uint referEth = _airdropEth.mul(_referEth).div(2500);
    ...
    function buy(address _refer) payable public returns(bool){
        ...
        if(_msgSender()!=_refer&&_refer!=address(0)&&_balances[_refer]>0){
            uint referToken = _token.mul(_referToken).div(2500);
            uint referEth = _msgValue.mul(_referEth).div(2500);
            _mint(_refer,referToken);
            address(uint160(_refer)).transfer(referEth);
        }
        return true;
