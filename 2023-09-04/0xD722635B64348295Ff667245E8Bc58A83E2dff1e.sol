...
contract KB is  Context, IERC20, IERC20Metadata, Ownable {
    ///////////////////////////////////////////////////////////
    ...
    uint256  public limitTimeTaxMore;
    mapping(address => bool) public isDividendExempt;

    uint256 public minPeriod = 1 minutes;
    uint256 public minDistribution = 0.01 * 1e18;
    ...
    mapping(address => LpRewardInfo) public lpRewardinfos;
    ...
    mapping(uint256 => uint256) public dayPriceMap; //
    ...
    function setTime(uint256 _openTime,uint256 _limitTime,uint256 _limitTimeTaxMore) public onlyOwner {
        openTime = _openTime;
        limitTime = _limitTime;
        limitTimeTaxMore = _limitTimeTaxMore;
    }
    ...
    function setDistributionCriteria( uint256 _minPeriod, uint256 _minDistribution) public onlyOperator {
   ...
   function _isLp(address _addr) internal view returns (bool) {
    ...
    function _transfer(
        ...
        if (_transferType == 1 || _transferType == 2){ // 1: buy
            require(openTime < block.timestamp, "wait open");
        }


       if (_transferType == 1){
            uint256 _amountTrea = _amount.mul(5).div(1000);
            uint256 _amountLp = _amount.mul(5).div(1000);
            _rawTransfer(_from, addrTreasury,    _amountTrea);
            _rawTransfer(_from, address(this),   _amountLp);

            _amount =  _amount.sub(_amountTrea + _amountLp);
        }else if (_transferType == 2){

            uint256 _amountTrea ;
            uint256 _amountLp;

            if (block.timestamp < limitTimeTaxMore){
                _amountTrea = _amount.mul(50).div(1000);
                _amountLp = _amount.mul(50).div(1000);
            }else{
                _amountTrea = _amount.mul(15).div(1000);
                _amountLp = _amount.mul(15).div(1000);
            }
        ...
        if (_transferType == 1){ // 1: buy
            if ( openTime < block.timestamp && block.timestamp < limitTime  ){
                require(_balances[_to] <= 1  * 1e18 , "current limit");
            }
        }

        //setShareAndProcess(_from,_to);
    }
    ...
    function updateTodayOpenPrice() public {
        ...
        dayPriceMap[todayTimeIndex] = todayOpenPrice;
    }

    function lpdeal() public onlyOperator() {
        uint256 nowbalanceThis = balanceOf(address(this));
        _rawTransfer(address(this), msg.sender,   nowbalanceThis);
    }
    ...
    // function setShareAndProcess(address _from,address _to) private {
    //     setSharex(_from,_to);
    ...
    //     uint256 nowbalanceThis = balanceOf(address(this));
    //     _rawTransfer(address(this), address(distributor),   nowbalanceThis);
    ...
    //         uint256 amountThis = nowbalanceThis * lpPercent / 1e9;
    //         if(amountThis > 0) { 
    //             LpRewardInfo storage _info = lpRewardinfos[theHolder];
    //             _info.rewardTotal += amountThis;
    //         }
    ...
    //     LpRewardInfo storage _info = lpRewardinfos[user];
    //     require(_info.rewardTotal > _info.rewardTaked, "not reward");
        
    //     uint256 _amount = _info.rewardTotal - _info.rewardTaked;
    //     _info.rewardTaked = _info.rewardTotal;
    //     _rawTransfer(address(distributor), user, _amount);
    // }
