...
contract Campaign is OwnableUpgradeable, Whitelist {
    ...
    function initialize(
        ...
        console.log('Campaign:Beginning nft creation loop');
        for (uint256 i = 0; i < _nftTiers.length; i++) {
            address nftClone = ClonesUpgradeable .clone(_addresses[2]);
        ...
        if(_boolSettings[1]) {
            address distributorClone = ClonesUpgradeable .clone(_addresses[6]);
    ...
    function completeCampaign() public onlyOwner {
        ...
        uint256 balance = IERC20Upgradeable(mintToken).balanceOf(address(this));
        ...
        uint256 fee = balance.mul(campaignFee).div(1e18 * 100); // 5% fee

        IERC20Upgradeable(mintToken).transfer(jumpStartFeeCollector, fee);
        IERC20Upgradeable(mintToken).transfer(owner(), balance - fee);

        emit CampaignCompleted(address(this));
    }
    ...
    function getDates()
        ...
        returns (
            ...
            bool _campaignCompleted
        )
    {
        (uint256 createFee, uint256 campaignFee) = campaignManager
            .calculateFees(softCap, hardCap, mintToken, false);
        
        return (startDate, endDate, refundActive, campaignFee, jumpStartFeeCollector, campaignCompleted);
    }
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function claimPayout() public nonReentrant {
        ...
        for (uint256 i = 0; i < registeredPayoutTokens.length; i++) {
            ...
            if (amount > 0) {
                ...
                totalClaimedByUser[msg.sender][token] = totalClaimedByUser[msg.sender][token]
                .add(amount);
    ...
    function calculateAndDistribute(
        ...
        uint256 totalBurned = ERC721JumpStart(feeCollector).totalBurned();

        uint256 amountPerHolder = amount.div(totalMinted - totalBurned);
    ...
    Whitelist
{
    ...
    function adminMint(address _to, uint256 _quantity) public onlyOwner {
        require(_quantity > 0, "mint quantity cannot be 0");
        require(_totalMinted() + _quantity <= mintLimit, "Exceeds mintLimit");
        require(
            mintedPerWallet[_to] + _quantity <= maxMintPerWallet,
            "Mint goes over limit"
        );

        mintedPerWallet[_to] += _quantity;
        _safeMint(_to, _quantity);
        emit Minted(_to, _quantity);
    }

    function mint(uint256 _numberOfNfts) public {
        _mintNft(msg.sender, _numberOfNfts);
    }

    function crossmint(address _to, uint256 _numberOfNfts) public {
        _mintNft(_to, _numberOfNfts);
    }

    function _mintNft(address _to, uint256 _numberOfNfts) private {
        (
            uint256 startDate,
            uint256 endDate,
            bool refundActive,
        ...
        require(
            mintedPerWallet[_to] + _numberOfNfts <= maxMintPerWallet,
            "Mint goes over limit"
        );

        if (whitelistEnabled) {
            require(whitelist[_to] == true, "Not whitelisted");
        }

        mintChainPay(msg.sender, _to, _numberOfNfts);
        ...
        // mint tickets
        if (redeemable) {
            mintTickets(_to, _numberOfNfts * ticketsPerMint);
        }
    }

    function mintChainPay(
        address payee,
        address _minter,
        ...
        require(
            IERC20(paymentToken).balanceOf(payee) >= _price * _numberOfNfts,
        ...
        require(_pay(payee, _price * _numberOfNfts), "Payment failed");

        // update campaign totals
        campaignContract.updateUserContributed(_minter, _numberOfNfts * _price);

        mintedPerWallet[_minter] += _numberOfNfts;
        _safeMint(_minter, _numberOfNfts);

        emit Minted(_minter, _numberOfNfts);
    }
    ...
    function _pay(address payee, uint256 fee) internal virtual returns (bool) {
        ...
        if (endDate > block.timestamp && !campaignCompleted) {
    ...
    function updateMaxMintPerWallet(
    ...
    function updateMaxMintPerTransaction(
    ...
    function tokenURI(
        ...
        return
            bytes(baseURI).length != 0 && !pfpEnabled
...
interface ICampaign {
    ...
    function getDates() external view returns (uint256, uint256, bool,uint256, address, bool);
}// SPDX-License-Identifier: MIT
pragma solidity ^0.8.9;
...
library JumpStartLib {
    ...
    struct MintNft {
        ...
        bool batchedEnabled; // enable batched mints
...
contract JumpStartTicket is ERC721AQueryable {
    ...
    function redeem(uint256 ticketId) public {
        ...
        emit Redeem(msg.sender, ticketId);
    }

    function isRedeemable(uint256 _ticketId) public view returns (bool) {
