...
contract PaymentModuleV1 is IDepositHandler, IPaymentModule, AccessControlEnumerable, IERC721Receiver {
    ...
    constructor(
        ...
        require(uniswapV2RouterAddress != address(0), "PaymentModuleV1::constructor::ZERO: Router cannot be zero address.");
    ...
    function _processErc20(ProcessPaymentParams memory params, IPricingModule.PriceInfo memory price) private {
        for (uint256 i = 0; i < params.fungibleTokenDeposits.length; i++) {
            ...
            IERC20(params.fungibleTokenDeposits[i].tokenAddress).safeTransferFrom(params.user, address(this), params.fungibleTokenDeposits[i].amount);
            ...
            IERC20(params.fungibleTokenDeposits[i].tokenAddress).safeTransfer(params.vault, receivedAmount - price.v2LpAmounts[i]);
            if (params.fungibleTokenDeposits[i].tokenAddress == price.v2LpTokens[i]) {
                ...
                uint256 initialLpBalance = IERC20(params.fungibleTokenDeposits[i].tokenAddress).balanceOf(address(lpTokenProcessor));
                IERC20(params.fungibleTokenDeposits[i].tokenAddress).safeTransfer(address(lpTokenProcessor), price.v2LpAmounts[i]);
                uint256 amountReceived = IERC20(params.fungibleTokenDeposits[i].tokenAddress).balanceOf(address(lpTokenProcessor)) - initialLpBalance;
    ...
    function _processNfts(ProcessPaymentParams memory params, IPricingModule.PriceInfo memory price) private {
        for (uint256 i = 0; i < params.nonFungibleTokenDeposits.length; i++) {
            ...
            if (params.nonFungibleTokenDeposits[i].tokenAddress == price.v3LpTokens[i].tokenAddress) {
                ...
                uint256 amount0Received = initialToken0Balance - IERC20(price.v3LpTokens[i].token0).balanceOf(address(this));
                uint256 amount1Received = initialToken1Balance - IERC20(price.v3LpTokens[i].token1).balanceOf(address(this));
    ...
    function _processFee(
        ...
        if (usdtAmount > 0) {
            ...
            if (msg.value > 0) {
                ...
                routerForFloki.swapETHForExactTokens{ value: msg.value }(usdtAmount, path, address(this), block.timestamp);
            ...
            if (FLOKI != address(0)) {
                ...
                bool success = lpTokenProcessor.swapTokens(address(USDT), burnShare, FLOKI, burnAddress, address(routerForFloki));
