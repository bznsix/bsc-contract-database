...
contract DGF is ERC20, Ownable {
    ...
    event UpdateBuyFees(uint256 liquidityFeeOnBuy);
    event UpdateSellFees(uint256 liquidityFeeOnSell);
    event SwapAndLiquify(uint256 tokensSwapped,uint256 bnbReceived,uint256 tokensIntoLiqudity);
    event SwapTokensAtAmountUpdated(uint256 swapTokensAtAmount);

    constructor () ERC20("Dogfire", "DGF") 
    {   
        address router;
        if (block.chainid == 56) {
    ...
    function updateBuyFees(uint256 _liquidityFeeOnBuy) external onlyOwner {
        ...
        emit UpdateBuyFees(liquidityFeeOnBuy);
    }

    function updateSellFees(uint256 _liquidityFeeOnSell) external onlyOwner {
        ...
        emit UpdateSellFees(liquidityFeeOnSell);
    }
