...
interface IStrategyInfo {
    ...
    function isFirstRescale() external view returns (bool);

    function tickBoundaryOffset() external view returns (int24);

    function rescaleTickBoundaryOffset() external view returns (int24);
...
library LiquidityNftHelper {
    ...
    */
    ...
    function verifyCurrentPriceInLiquidityNftValidOffsetRange(
        int24 tickSpacing,
        int24 tickBoundaryOffset,
        uint256 liquidityNftId,
        address pancakeswapV3FactoryAddress,
        address nonfungiblePositionManagerAddress
    ) internal view returns (bool isInValidOffsetRange) {
        ...
        // Current tick is within tickLower-(tickBoundaryOffset*tickSpacing) ~ tickUpper+(tickBoundaryOffset*tickSpacing)
        if (
            tick <= tickLower - (tickBoundaryOffset * tickSpacing) ||
            tick >= tickUpper + (tickBoundaryOffset * tickSpacing)
        ) {
            return false;
        } else {
            return true;
    ...
    // for general condition (except tickSpacing 1 condition)
    function calculateRescaleTickBoundary(
        uint256 liquidityNftId,
        int24 tickSpacing,
        int24 tickSpreadUpper,
        int24 tickSpreadLower,
...
contract Controller is AccessControl, IControllerEvent {
    ...
    // for general condition (except tickSpacing 1 condition)
    function isNftWithinTickOffsetRange(
        address _strategyContract
    ) public view returns (bool isWithinTickOffsetRange) {
        uint256 liquidityNftId = IStrategyInfo(_strategyContract)
            .liquidityNftId();
        ...
        int24 tickBoundaryOffset = IStrategyInfo(_strategyContract)
            .tickBoundaryOffset();
        bool isFirstRescale = IStrategyInfo(_strategyContract).isFirstRescale();

        isWithinTickOffsetRange = LiquidityNftHelper
            .verifyCurrentPriceInLiquidityNftValidOffsetRange(
                tickSpacing,
                isFirstRescale ? int24(0) : tickBoundaryOffset,
                liquidityNftId,
                PANCAKE_V3_FACTORY_ADDRESS,
                NONFUNGIBLE_POSITION_MANAGER_ADDRESS
            );
    ...
    // for general condition (except tickSpacing 1 condition)
    function isNewTickBoundaryOverlap(
        address _strategyContract
    ) public view returns (bool isOverlap) {
        uint256 liquidityNftId = IStrategyInfo(_strategyContract)
            .liquidityNftId();
        ...
        int24 tickSpacing = IStrategyInfo(_strategyContract).tickSpacing();
        int24 tickSpreadUpper = IStrategyInfo(_strategyContract)
            .tickSpreadUpper();
        int24 tickSpreadLower = IStrategyInfo(_strategyContract)
            .tickSpreadLower();
        int24 rescaleTickBoundaryOffset = IStrategyInfo(_strategyContract)
            .rescaleTickBoundaryOffset();

        (, int24 tickLower, int24 tickUpper) = LiquidityNftHelper.getTickInfo(
            liquidityNftId,
            PANCAKE_V3_FACTORY_ADDRESS,
            NONFUNGIBLE_POSITION_MANAGER_ADDRESS
        );

        (int24 newTickLower, int24 newTickUpper) = LiquidityNftHelper
            .calculateRescaleTickBoundary(
                liquidityNftId,
                tickSpacing,
                tickSpreadUpper,
                tickSpreadLower,
        ...
        isOverlap = (tickUpper == newTickUpper && tickLower == newTickLower);
    }
    ...
    function setTickBoundaryOffset(
        ...
        IStrategySetter(_strategyContract).setTickBoundaryOffset(
            _tickBoundaryOffset
        );
    ...
    function setRescaleTickBoundaryOffset(
        ...
        IStrategySetter(_strategyContract).setRescaleTickBoundaryOffset(
