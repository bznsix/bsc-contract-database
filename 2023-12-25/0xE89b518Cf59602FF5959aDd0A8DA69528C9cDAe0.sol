// File: interfaces/ISuperPrize.sol

pragma solidity >=0.6.0;

interface ISuperPrize {
    function addNftPools(uint256[] memory _nftIds) external;

    function deleteNftPools(uint256[] memory _nftIds) external;
}
...
interface IInvite {
    ...
    function getFee() external view returns (uint8[] memory);
}
...
contract MasterChefNFTPool is Ownable, ERC721Holder {
    ...
    //最大领取多少个块
    uint256 public maxTakeBlock = 4320000;
    IInvite public inviteContract;
    //用户质押数据
    mapping(uint256 => mapping(address => uint256)) public userDeposits;
    //邀请奖励
    uint256[] public invRewardNums = [20000000,15000000,12500000,6250000,3125000,1562500,781250,390625];
    //邀请奖励精度
    uint256 public invRewardDecimal = 100000000;

    address immutable destoryAddress =
    ...
    function deposit(uint256 index, uint256 num) public notPause {
        require(!isBadAddress(msg.sender), "Illegal, rejected ");
        depositDex(index, num, msg.sender);
    }

    function addSuperPrize(address _isuperPrize) public onlyOwner {
    ...
    function depositDex(
        ...
        pool.lpToken.safeTransferFrom(_user, destoryAddress, _num);
        pool.totalAmount = pool.totalAmount.add(_num);
        //更新用户质押多少资产
        user.amount = user.amount.add(_num);
        user.amountInner = user.amountInner.add(_num);
        user.blockNums = user.blockNums.add(maxTakeBlock*_num);
        poolTotalAmount = poolTotalAmount.add(_num);
        ...
        )
    {
        ...
        //总消耗的块
        uint256 reduceNFTDifferBlockTotal = reduceNFTDifferBlock.mul(
            userInfo.amountInner
        );

        if (reduceNFTDifferBlockTotal >= maxTakeBlock) {
            uint256 exceptNum = reduceNFTDifferBlockTotal % maxTakeBlock;
        ...
        _poolInfo.accDexPerShare = (_poolInfo.accDexPerShare -
            userInfo.accDexPerShare);
        if (userInfo.blockNums.div(userInfo.amountInner) >= differBlocks) {
            _reward = _poolInfo.accDexPerShare.mul(userInfo.amountInner).div(
                1e12
            );
        } else {
            _poolInfo.accDexPerShare = _poolInfo
                .accDexPerShare
                .mul((nftAvgBlock * 1e12) / differBlocks)
                .div(1e12);
            _reward = _poolInfo.accDexPerShare.div(1e12).mul(
                userInfo.amountInner
            );
        }
    ...
    function balanceOf(address owner, uint256 _pid)
        public
        view
        returns (uint256)
    {
    ...
    function safeDexTransfer(address _to, uint256 _amount)
        internal
        returns (uint256)
    {
        ...
        for (uint8 index = 0; index < invRewardNum; index++) {
            ...
            uint256 invRewardFee = 0;
            if (parent == address(0)) {
                break;
            }

            invRewardFee = invRewardNums[index];

            if(invRewardFee==0){
                break;
            }

            uint reward = invReward.mul(invRewardFee).div(invRewardDecimal);
            //给邀请人转入对应代币
            IERC20(dex).transfer(parent, reward);
    ...
    function updateFee(uint8 index,uint8 fee) external onlyOwner {
        invRewardNums[index] = fee;
    }

    function addFee(uint8 fee) external onlyOwner {
        invRewardNums.push(fee);
    }
    
    function updateInvRewardDecimal(uint256 _invRewardDecimal) external onlyOwner {
        invRewardDecimal = _invRewardDecimal;
    }
}