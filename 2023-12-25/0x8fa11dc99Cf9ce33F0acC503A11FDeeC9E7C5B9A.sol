...
interface IMarketplace {
    ...
    function buyByTokenAndCurrency(uint256 _nftId, uint256 _refCode) external;
...
interface IMarketplaceSetting {
    ...
    function allowBuyNftByCurrencyAndToken(bool _activePayByCurrency) external;
    ...
    IMarketplaceSetting
{
    ...
    bool private allowBuyFlex = true; // default true
    ...
    function initPayUsdPercents() internal {
        ...
        percentUsdtPrice[4] = 450;
        percentUsdtPrice[5] = 400;
        percentUsdtPrice[6] = 350;
        percentUsdtPrice[7] = 300;
        percentUsdtPrice[8] = 300;
        percentUsdtPrice[9] = 300;
        percentUsdtPrice[10] = 300;
    }
    ...
    function setPayToken(bool _payInCurrency, bool _payInToken, bool _payInFlex) public override checkOwner {
    ...
    function buyByCurrency(uint256 _nftId, uint256 _refCode) external override isAcceptBuyByCurrency lock {
        ...
        nftPaymentType[_nftId] = !payTokenBuyByCurrency;
    ...
    function buyByToken(uint256 _nftId, uint256 _refCode) external override isAcceptBuyByToken lock {
        ...
        // Transfer nft from marketplace to buyer
        IHREANFT(nft).safeTransferFrom(address(this), msg.sender, _nftId, "");
        nftPaymentType[_nftId] = !payTokenBuyByToken;
        emit Buy(address(this), msg.sender, _nftId);
    ...
    function buyByTokenAndCurrency(uint256 _nftId, uint256 _refCode) external override isAcceptBuyByFlex lock {
        updateReferralData(msg.sender, _refCode);
        uint256 totalValueUsdWithDecimal = IHREANFT(nft).getNftPriceUsd(_nftId) * TOKEN_DECIMAL;
        uint8 nftTier = IHREANFT(nft).getNftTier(_nftId);
        ...
        uint256 valueToken = IOracle(oracleContract).convertUsdBalanceDecimalToTokenDecimal(
            valueUsdToken
        );
        ...
        uint256 saleValueInUsdWithDecimal = 0;
        {
            uint256 currentSale = getCurrentSalePercent(nftTier);
            if (currentSale > 0) {
                saleValueInTokenWithDecimal = (currentSale * valueToken) / 10000;
                saleValueInUsdWithDecimal = (currentSale * valueUsd) / 10000;
            }
        }
        ...
        pay(token, payValueTokenWithDecimal);
        // Transfer nft from marketplace to buyer
        IHREANFT(nft).safeTransferFrom(address(this), msg.sender, nftId, "");
        nftPaymentType[nftId] = !payTokenBuyByFlex;
        emit Buy(address(this), msg.sender, nftId);
