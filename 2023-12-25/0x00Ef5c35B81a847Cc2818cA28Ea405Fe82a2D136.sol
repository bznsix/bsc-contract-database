...
contract Factory721 {
	...
	function deploy(string memory name, string memory symbol, string memory tokenURIPrefix) external returns(address addr) {
		addr = address(new BNFT(name, symbol, tokenURIPrefix));
		BNFT token = BNFT(address(addr));
