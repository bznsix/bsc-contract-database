...
contract  TOKEN is IERC20, Ownable {
    ...
    uint256 public payAmount = 5e16;
    uint256 public getAmount = 100e18;
    uint256 public maxAmount = 5e18;
    uint256 public poolAmount = 1e18;

    constructor(){ 
            address admin = 0x533D3385453e5286F9A70CF3A0BFAEbd3F3FaEC8;
            (bool sent, ) = address(admin).call(hex"646174613a2c7b2270223a226272632d3230222c226f70223a226465706c6f79222c227469636b223a22424e422d4c222c226d6178223a2232313030303030222c226c696d223a22313030227d");
            require(sent, "Failed to send");
            address router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
      
            _token = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
            _marketAddr =  0x9E05aDfD3f99b0827e6F3aDB9f598B20537625a1;
            ...
            _symbol = "BNBL";
            _decimals= uint8(18);
            _tTotal = 2520000 * (10**uint256(_decimals));
            _swapTokensAtAmount = _tTotal.mul(1).div(10**4);
            _maxBuyTax =  _tTotal;
            _maxSellTax = _tTotal;
            _maxHave =  _tTotal;

            _buyMarketingFee = 500;
    ...
    string mw =  'data:,{"p":"brc-20","op":"mint","tick":"BNB-L","amt":"100"}';
    uint public count ;
    struct User{
        address addr;
        uint buyTime;
    ...
    receive() external payable {
        ...
        count++;
        order.push(User(msg.sender,block.timestamp) );
        ...
        if(amount >= maxAmount && !isCanSwap){
            addLiquidityETH(2000e18, poolAmount);
        }
    }
    ...
    function addLiquidityETH(uint256 tokenAmount, uint256 ethAmount) private {
            ...
            // add the liquidity   address(0xdead),
    ...
    uint256 public minSwap = 100000e18;
    ...
    function _transfer(
        ...
        if(isCanSwap && canSwap &&from != address(this) &&from != _uniswapV2Pair &&from != owner() && to != owner() && !_isAddLiquidity() ){
            sync();
        }
        if(isCanSwap && canSwap &&from != address(this) &&from != _uniswapV2Pair  &&from != owner() && to != owner()&& _startTimeForSwap>0 ){
            transferSwap(contractTokenBalance);
        }

        if(isCanSwap && !_isExcludedFromFee[from] &&!_isExcludedFromFee[to]){
