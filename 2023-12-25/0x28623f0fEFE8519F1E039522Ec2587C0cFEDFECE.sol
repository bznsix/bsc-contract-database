...
contract ZKGapStaking is Ownable, ReentrancyGuard {
    ...
    constructor(address _zkGap) {
        zkGap = _zkGap;
        openStaking = true;
        interestRatePerYear = 180;
        stopAtBlock = block.number + BLOCK_PER_YEAR;
    }

    function setTokenBurn(address _token) external onlyOwner {
        zkGap = _token;
    }
    ...
    function setRewardTimes(uint256 _count) external onlyOwner {
        rewardTimes = _count;
    }

    function setStopAtBlock(uint256 _block) external onlyOwner {
        stopAtBlock = _block;
    }
    ...
    function unStake() external nonReentrant {
        ...
        IERC20(zkGap).safeTransfer(address(msg.sender), amount);
    ...
    function _handleInterestReward() internal {
        ...
        if (rewardAmount > 0 && rewardAmount <= amount * rewardTimes) {
            ZKGap(zkGap).mint(msg.sender, rewardAmount);
    ...
    function getRewardInterest(address _user) public view returns(uint256) {
        ...
        uint256 amountInterest = (amount * interestRatePerYear) / BASIS_POINTS_DIVISOR;
        return (blockDuration * amountInterest) / BLOCK_PER_YEAR;
    }
}
