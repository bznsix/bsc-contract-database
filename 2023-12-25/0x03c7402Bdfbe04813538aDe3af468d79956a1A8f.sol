...
contract InstantGain is ReentrancyGuard {
    ...
    uint256 constant private MAX_COMMISSION_LEVEL = 5;
    uint256 constant private MAX_DIRECT_REFERRAL = 2;
    uint256 constant private TOTAL_ROI = 1040;
	...
	constructor() {
        ...
        oldInstantGain = InstantGain(0x36d43B77bE04aBB7579918c25b79aa540618ff1E);
    ...
    function withdrawProfit() public noReentrant {
        require(initial, "Project not launch yet");
        require(users[msg.sender].joinDate > 0, "Activate first");
        require(users[msg.sender].checkpoint + TIME_STEP <= block.timestamp, "User is active");
        require(!userReactiveStatus[msg.sender], "Only once");

        users[msg.sender].available += ACTIVATE_PRICE * TOTAL_ROI / PERCENTS_DIVIDER;
    ...
    function _payCommission(address userAddress) internal {
    ...
    function getUserDividends(address userAddress) public view returns (uint256) {
        ...
        uint256 share = ACTIVATE_PRICE * TOTAL_ROI / PERCENTS_DIVIDER;
    ...
    function isUserActive(address userAddress) public view returns (bool) {
		if((users[userAddress].checkpoint + TIME_STEP > block.timestamp && userAddress != address(0)) || userAddress == ownerWallet){
            return true;
        } else{
            return false;
        }
	...
	function getUserExtraInfo(address userAddress) public view returns(uint256 totalReferralCommission, uint256 totalReferralCommissionMissed, uint256 availableROI, uint256 userDeactivateTime, bool canUserReactive) {
		return (
            ...
            userReactiveStatus[msg.sender] ? true : false
        );
	}
    ...
    function treeView(address _user) public view returns (address[] memory,bool[] memory, uint256[] memory ) {
        ...
        bool[] memory activeStatus = new bool[](62);
    ...
    function MigrateOldUser(uint256 limit) public{
        ...
        for (uint i = 0; i < limit; i++) {
            ...
            if(olduser1.joinDate > 0 && users[oldusers].joinDate == 0){
                ...
                users[oldusers].reactivateCount = olduser2.reactivateCount;
                users[oldusers].paidCommission = olduser2.paidCommission;
                users[oldusers].referral = oldInstantGain.getUserReferral(oldusers);
                users[oldusers].totalReferralCommission = oldInstantGain.getUserReferralCommission(oldusers);
                users[oldusers].missedCommission = oldInstantGain.getUserMissedCommission(oldusers);
