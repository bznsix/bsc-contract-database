...
contract SanSanHuaLuo13 is Ownable, ReentrancyGuard {
    ...
    function join(address sponsor) public nonReentrant {
        ...
        if(members[sponsor].referrals.length < 3) {
            ...
            members[sponsor].queue.push(msg.sender);
        } else {
    ...
    function checkSliding(address _referrer) public onlyOwner{
        for (uint i ; i < members[_referrer].referrals.length; i++) {
            address referral = members[_referrer].referrals[i];
    ...
    function findNextAvailableSponsor(address sponsor) private  returns(address) {
        ...
        for(uint i = 1; i <= members[sponsor].queue.length; i++) {
            address queue = members[sponsor].queue[i];
            if(members[queue].referrals.length <= 3 && members[queue].level >=1 ) {
                return queue;
            }
        }

        // 递归查找下一个可用的推荐人
        for(uint i = 1; i <= members[sponsor].referrals.length; i++) {
    ...
    function getQueue(address _member) public view returns(address[] memory) {
        return members[_member].queue;
    }
