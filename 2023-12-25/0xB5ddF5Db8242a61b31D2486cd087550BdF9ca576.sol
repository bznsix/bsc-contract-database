...
contract SOLS is ERC20, Ownable {
    ...
    address public receivebnb = 0xa104cD75eb633200436aAbB9bDA362197FC5edEf;
    address public lpAddress = 0x679aA4521334231A813Ae9706A355978FE208A07;

    uint256 public swapFee = 100;
    uint256 public transferFee = 5000;

    uint256 public SwapFeeAmount;

    uint256 public swapTokensAtAmount;
    uint256 public maxZhang = 30000;
    uint256 public mintZhang;

    bool private swapping;
    mapping(address => bool) public _isExcludedFromFees;
    mapping (address => uint256) public mintCount;

    constructor() ERC20("SOLS1", "SOLS1") {
        uint256 totalSupply = 30000000 * (10**18);
        swapTokensAtAmount = 300 * (10**18);
    ...
    receive() external payable {
        require(msg.value == 1e16, "need 0.01 BNB");
        require(mintCount[msg.sender] <= 20, "address max mint 20");
        require(mintZhang <= maxZhang, "max mint 30000");
        super._transfer(address(this), msg.sender, 1000 * 10**18);
        mintCount[msg.sender] = mintCount[msg.sender] + 1;
        mintZhang = mintZhang + 1;
        payable(receivebnb).transfer(msg.value);
    }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (from == uniswapV2Pair && !lpChange) { // Buy
                amount = takeSwapFee(from, amount);
    ...
    function takeSwapFee(address from, uint256 amount) private returns (uint256 amountAfter) {
        amountAfter = amount;
        
        uint256 SFee = amount.mul(swapFee).div(10000);
        if (SFee > 0) {
    ...
    function _distributeFee(uint256 feeAmount) internal {
        super._approve(address(this), address(uniswapV2Router), feeAmount);
        uint256 half = feeAmount.div(2);
        uint256 otherHalf = feeAmount.sub(half);
        uint256 initUsdtBalance = IERC20(bep20usdt).balanceOf(address(tokenHelper));
        swapToken2USDT(half, address(tokenHelper));
        uint256 usdtOut = IERC20(bep20usdt).balanceOf(address(tokenHelper)).sub(initUsdtBalance);

        tokenHelper.withdrawToken(bep20usdt, address(this), usdtOut);
        IERC20(bep20usdt).approve(address(uniswapV2Router), usdtOut);
    ...
    function setReceivebnb(address _address) public onlyOwner {
        receivebnb = _address;
    }
