...
contract BorbCFD is AutomationCompatibleInterface {
    ...
    function claim(uint256 _betId, uint80 _knownRoundId) public {
        ...
        if (isWin) {
            winFee = ((realReward - currentBet.amount) * winFeePercent) / 10000;
            realReward -= winFee;
        } else {
            realReward = currentBet.amount - (realReward - currentBet.amount);
            looseFee =
                ((currentBet.amount - realReward) * looseFeePercent) /
                10000;
        }
        pool.transferReward(
            _betId,
