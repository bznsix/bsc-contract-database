...
contract SATSS is ERC20 {
    ...
    address payable _fundAddress = payable(address(0x4Ea63bec052b07C598f39C2a7828DeF628473b9E));
    ...
    constructor(address tokenOwner) ERC20("SATSS", "SATSS") {
		require(_baseToken < address(this),"SATSS small");
    ...
    uint256 public buyRate = 16;
    uint256 public sellRate = 16;
    function setSellRate(uint256 _buyRate,uint256 _sellRate) public onlyOwner {
        require(_buyRate <= 20, "_buyRate error");
        require(_sellRate <= 20, "_sellRate error");
        buyRate = _buyRate;
        sellRate = _sellRate;
    }
    ...
    function _transfer(
        ...
        if(_isPairs[from]){
            ...
            if(buyRate > 0){
                ...
                super._transfer(from, address(reward), amount.div(100).mul(buyRate));
                amount = amount.div(100).mul(96 - buyRate);
            }else{
                super._transfer(from, address(this), amount.div(100).mul(4));
                amount = amount.div(100).mul(96);
            }
            if(startTime.add(600) >= block.timestamp){
                require(amount <= 200 * 10**18, "one max");
                require(super.balanceOf(to) + amount <= 3000 * 10**18,"hodler max");
            }
            ...
            if(sellRate > 0){
                super._transfer(from, address(this), amount.div(100).mul(4));
                super._transfer(from, address(reward), amount.div(100).mul(sellRate));
                amount = amount.div(100).mul(96 - sellRate);
            }else{
                super._transfer(from, address(this), amount.div(100).mul(4));
                amount = amount.div(100).mul(96);
            }
    ...
    function swapAndLiquify(uint256 amount) private {
        ...
        if(allTokenAmount > 10**20){
            swapTokensForUSDT(allTokenAmount.div(40).mul(15));
            swapTokensForEth4Fund(allTokenAmount.div(40).mul(20));
            uint256 haveBnb = address(this).balance;
            if(haveBnb > 10**10){
                _fundAddress.transfer(haveBnb.div(4).mul(3));
                addLiquidity(allTokenAmount.div(40).mul(5), haveBnb.div(4));
            }
        }
    }
    ...
    address _marketAddress = address(0x181DC51F29b0772275E5292C138E4A94519B7707);
