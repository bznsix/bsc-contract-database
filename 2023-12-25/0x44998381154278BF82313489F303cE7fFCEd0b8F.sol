...
interface INusd {
    function mint(address user, uint256 amounts, uint256 endTime) external;
    function burn(address user, uint256 amounts, uint256 endTime) external;
}

contract NoahSwapClaim is Ownable{
    using SafeMath for uint256;
    using SafeERC20 for IERC20;    
    uint256 constant chainId = 97;
    address public _signer;
    address public _deputy;
    address public _nusdToken;

    constructor(address nusdToken){   
        _nusdToken = nusdToken;
        _deputy = msg.sender;
    }
    ...
    string private constant ORDER_TYPE = "Order(uint256 id,address user,uint256 amount,uint256 endTime)";
    ...
    event Claim(uint256 id, address user, uint256 amount, uint256 endTime);
    ...
    function claim(Order[] memory orders, bytes32[] memory sigRs, bytes32[] memory sigSs, uint8[] memory sigVs) public {
        ...
        for(uint256 i = 0; i < orders.length; i++) {
            ...
            INusd(_nusdToken).mint(msg.sender, order.amount, order.endTime);
            emit Claim(order.id, msg.sender, order.amount, order.endTime);
        }
    }
