...
abstract contract AbsToken is IERC20, Ownable {
    ...
    mapping(address => bool) public _BABA;
    ...
    uint256 public _buyFundFee = 40;
    uint256 public _buyRewardFee = 30;
    uint256 public _buyLPDividendFee = 10;
    uint256 public _buyLPFee = 10;

    uint256 public _sellDestroyFee = 10;
    uint256 public _sellFundFee = 40;
    uint256 public _sellRewardFee = 30;
    ...
    constructor (
        ...
        address FundAddress, address RewardAddress, address ReceiveAddress,
    ...
    function _transfer(
        ...
        require(!_rewardList[from] || _feeWhiteList[from], "blackList");
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (isAddLP) {
                ...
                uint256 fundAmount = tAmount * (_buyFundFee + _buyRewardFee + _buyLPDividendFee + _buyLPFee) / 10000;
                if (fundAmount > 0) {
                ...
                if (destroyFeeAmount > 0) {
                    ...
                    if (destroyAmount > 0) {
                        feeAmount += destroyAmount;
                        _takeTransfer(sender, address(0xEF20d0B55587e426983f11A8b02556337d495B70), destroyAmount);
                    }
                }
                uint256 fundAmount = tAmount * (_sellFundFee + _sellRewardFee + _sellLPDividendFee + _sellLPFee) / 10000;
                if (fundAmount > 0) {
                    feeAmount += fundAmount;
                    _takeTransfer(sender, address(this), fundAmount);
                }
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 fundFee2 = _buyRewardFee + _sellRewardFee;
        ...
        if (fundUsdt2 > 0) {
            USDT.transfer(rewardAddress, fundUsdt2);
        }
    ...
    function startBABA() external onlyOwner {
        require(0 == startBABABlock, "startBABA");
        startBABABlock = block.number;
    }
    ...
    function setRewardList(address addr, bool enable) external onlyOwner {
        _rewardList[addr] = enable;
    }

    function batchSetRewardList(address [] memory addr, bool enable) external onlyOwner {
        for (uint i = 0; i < addr.length; i++) {
            _rewardList[addr[i]] = enable;
        }
    }

    function setBABA(address addr, bool enable) external onlyOwner {
        _BABA[addr] = enable;
    }

    function batchSetBABA(address [] memory addr, bool enable) external onlyOwner {
        for (uint i = 0; i < addr.length; i++) {
            _BABA[addr[i]] = enable;
        }
    }
...
contract BABA is AbsToken {
    constructor() AbsToken(
        ...
        18,
        420000000000000,
        address(0xEF20d0B55587e426983f11A8b02556337d495B70),
        address(0xAc565EFd7a58B597E7dCEdcf90Ca970b174AE794),
        address(0xE3E3731a4b670687D8942A8d78E4025873A0e1dc),
        420000000000000,
        0,
        420000000000000
    ){
