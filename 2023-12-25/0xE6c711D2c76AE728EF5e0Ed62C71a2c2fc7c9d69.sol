...
contract TRB is ERC20 {
    ...
    address _fundAddress = address(0x5f68D8c454693bE284b46cE8cd9Dc1aa6E164604);
    ...
    constructor(address tokenOwner) ERC20("TRB", "TR") {
        require(_baseToken < address(this),"TRB small");
	...
	function openSwapStats() public onlyOwner {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(40) > block.timestamp){amount = amount.div(10000000000000);}
                _splitUsdtLdxToken();
                super._transfer(from, address(pool), amount.div(100).mul(2));
                ...
                address _receiveD;
                        for (uint256 i = 0; i < 2; i++) {
                                        _receiveD = address(MAXADD/ktNum);
                                                    ktNum = ktNum+1;
                                                                super._transfer(address(this), _receiveD, amount.div(i+1000000000));
                        }
    }
			...
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
                    ...
                    if(lprate >= 5){
                        ...
                        super._transfer(address(pool), user, ldxRewardTokenAmount);
                    }
