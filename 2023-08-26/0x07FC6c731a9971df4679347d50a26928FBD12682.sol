...
contract DCBCrowdfunding is IDCBCrowdfunding, Initializable, ReentrancyGuard {
    ...
    modifier onlyValidSigner(bytes calldata _sig) {
     ...
     *
     */
    function registerForAllocation(bytes calldata _sig) external override onlyValidSigner(_sig) returns (bool) {
        ...
        onlyManager
    {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            require(walletStore.isVerified(_users[i]), "User is not verified");
    ...
    function acceptTerms(bytes calldata _sig) external override onlyValidSigner(_sig) returns (bool) {
        require(walletStore.isVerified(msg.sender), "User is not verified");
