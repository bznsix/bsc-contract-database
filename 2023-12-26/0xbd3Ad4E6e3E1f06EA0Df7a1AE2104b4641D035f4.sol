...
/// @title TraderSZ: A standard bep20 token
contract TraderSZ is BEP20, Ownable {

    error ArrayLengthMismatch();
    error TokenClaimFailed();
    
    constructor()
        BEP20("TraderSZ", "TSZ") /// token name and ticker
        Ownable(msg.sender) /// deployer is the owner
    {
        /// mint the max supply to the deployer wallet
        _mint(msg.sender, 1000000000 * 10 ** decimals());
    }
    
    /// @dev claim stucked bep20 from contract, if accidently sent by someone
    /// @param token: token address 
    /// @param amount: amount to rescue
    function claimStuckedBep20 (address token, uint256 amount) external onlyOwner {
        IBEP20 tkn = IBEP20(token);
        bool sent = tkn.transfer(owner(), amount);
        if(!sent){
            revert TokenClaimFailed();
        }
    }
    
    /// @dev airdrop users with custom amounts
    ...
    function airdrop (address[] calldata users, uint256 [] calldata amounts) external {
        if(users.length != amounts.length){
         ...
         for (uint256 i = 0; i < users.length; ++i){
            _update(msg.sender, users[i], amounts[i]);
         }
    }
