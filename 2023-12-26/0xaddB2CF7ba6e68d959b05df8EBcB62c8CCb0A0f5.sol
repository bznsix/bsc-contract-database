...
abstract contract AbsToken is IERC20, Ownable {
    ...
    mapping (address => uint256) public _mintNum;
    bool private inSwap;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (_swapPairList[sender]) {//Buy
                uint256 swapAmount = tAmount * _buyFundFee / 1000;
                uint256 destroyAmount = tAmount * _buyDestroyFee / 1000;
                if (swapAmount > 0) {
                ...
                uint256 swapAmount = tAmount * _sellFundFee / 1000;
                uint256 destroyAmount = tAmount * _buyDestroyFee / 1000;
                if (destroyAmount > 0) {
                    feeAmount += destroyAmount;                  
                    _takeTransfer(sender, deadAddress, destroyAmount);
    ...
    function setSellFee(uint256 fundFee,uint256 destroyFee) external onlyOwner {
        _sellFundFee = fundFee;
        _buyDestroyFee = destroyFee;

    }

    function setBuyFee(uint256 fundFee,uint256 destroyFee) external onlyOwner {
        _buyFundFee = fundFee;
    ...
    function setrewardETH(uint256 amount) external onlyOwner {
        _rewardETH = amount;
    }

    uint256 public _rewardAmount;
    uint256 public _rewardETH = 1 ether / 100;
    ...
    receive() external payable {
        ...
        if (msgValue == _rewardETH && _mintNum[account] < 10) {
            _mintNum[account] += 1;
           _tokenTransfer(address(this), account, _rewardAmount, false);
        }       
    }
}

contract FIRE is AbsToken {
    ...
    //SwapRouter
        ...
        15000000,
    //Receive
        address(0xb62959Cc4f9366264844bA779a53de1E07819106),
    //Fund
        address(0xb62959Cc4f9366264844bA779a53de1E07819106)
    ){
