...
contract Tr8bit is ERC20, Ownable {
    //buy taxes
    ...
    constructor() payable ERC20("Tr8bit", "TR8") {

        rewardToken = 0x55d398326f99059fF775485246999027B3197955; // USDT
    operationsFeeOnBuy = 1;    
    marketingFeeOnBuy = 1;
    rewardsFeeOnBuy = 3;


    totalBuyFee = marketingFeeOnBuy + rewardsFeeOnBuy + operationsFeeOnBuy;

    marketingFeeOnSell = 1;
    rewardsFeeOnSell = 3;
    operationsFeeOnSell = 1;

    totalSellFee = marketingFeeOnSell + rewardsFeeOnSell + operationsFeeOnSell;

        marketingWallet = 0xDd34835d30771a04fa0efCe8CEd34cF96116Bb8c;
        operationsWallet = 0xE4e53eFF79B52EdC89BdF02a13615a1a71e2BAA2;

        dividendTracker = new DividendTracker(12_500_000, rewardToken);
        ...
        whitelisted[operationsWallet] = true;
    
        _mint(owner(), 25_000_000_000 * (10 ** 18));
    ...
    function setwhitelist(address account, bool excluded) external onlyOwner {
        require(whitelisted[account] != excluded, "Account is already set to that state");
        whitelisted[account] = excluded;
...
function updateBuyFees(uint256 _marketingFeeOnBuy, uint256 _rewardsFeeOnBuy, uint256 _operationsFeeOnBuy) external onlyOwner {
    ...
    require(totalBuyFee <= 6, "Buy fee cannot be more than 6%");

    emit BuyFeesUpdated(totalBuyFee);
}

function updateSellFees(uint256 _marketingFeeOnSell, uint256 _rewardsFeeOnSell, uint256 _operationsFeeOnSell) external onlyOwner {
    ...
    require(totalSellFee <= 6, "Sell fee cannot be more than 6%");
    ...
    function changeMarketingWallet(address _marketingWallet) external onlyOwner {
        ...
        require(_operationsWallet != operationsWallet, "Marketing wallet is already that address");
        require(_operationsWallet != DEAD, "Marketing wallet cannot be the zero address");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 totalshare = (marketingFeeOnBuy + marketingFeeOnSell) + (rewardsFeeOnBuy + rewardsFeeOnSell);
            ...
            if(contractTokenBalance > 0 && totalshare > 0) {
                ...
                //operations tax
                    if((operationsFeeOnBuy + operationsFeeOnSell) > 0) {
                    uint256 operationsshare = newBalance * (operationsFeeOnBuy + operationsFeeOnSell) / totalshare;
