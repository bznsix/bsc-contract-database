...
contract YFIs is ERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    bool public isOpenSwap;
    uint public feeAmount;
    uint public mintSurplus = 2_0000e18;
    ...
    constructor() ERC20("YFIs", "YFIs") {
        address recieve = msg.sender;
        if (block.chainid == 56) {
            recieve = 0x18B0357412c079D1337A797b0D204fE10546B8a5;
            manager = 0x18B0357412c079D1337A797b0D204fE10546B8a5;
            market = 0x4C0F28f02236A2b0E2c833337D2DD7BE0b3Dfa6F;
            idoAdd = 0x149C940271B9cb50dAC2d1268B22475DA69f7d61;
        ...
        _mint(address(this), 2_0000 * 10 ** decimals());
        _mint(recieve, 1_0000 * 10 ** decimals());
        renounceOwnership();
    }
    receive() external payable {
        address account = msg.sender;
        require(msg.value == 2e16, "BNB Error");
        require(mintSurplus >= 1e18, "Has End");
        require(!account.isContract(), "Not Wallet");
        require(userMints[account] < 10, "Over Max");
        userMints[account]++;
        super._transfer(address(this), account, 1e18);
        mintSurplus -= 1e18;
        payable(idoAdd).transfer(msg.value);
    }
    ...
    function setIdoAdd(address data) public {
        if (manager == _msgSender()) {
            idoAdd = data;
        }
    }
    ...
    function _transfer(
        ...
        if (_inSwapAndLiquify || isFeeExempt[from] || isFeeExempt[to]) {
            ...
            if (!isOpenSwap && to == swapPair && isFeeExempt[from]) {
                isOpenSwap = true;
                if (mintSurplus > 0)
                    super._transfer(address(this), _dead, mintSurplus);
            }
        } else if (from == swapPair || to == swapPair) {
            if (!isOpenSwap) {
                require(isFeeExempt[from], "Not open swap");
            }
            if (
                to == swapPair &&
                !_inSwapAndLiquify &&
                feeAmount >= getAutoSwapMin()
            ) {
                _swapAndLiquify();
            }
