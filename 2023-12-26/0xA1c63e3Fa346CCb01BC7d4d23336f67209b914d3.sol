...
interface IAccessControl {
     ...
     *
     * `DEFAULT_MINTER_ROLE` is the starting admin for all roles, despite
...
// OpenZeppelin Contracts v4.3.2 (utils/introspection/IBEP165.sol)
 ...
 *
 * By default, the admin role for all roles is `DEFAULT_MINTER_ROLE`, which means
 * that only accounts with this role will be able to grant or revoke other
 * roles. More complex role relationships can be created by using
 * {_setRoleAdmin}.
 *
 * WARNING: The `DEFAULT_MINTER_ROLE` is also its own admin: it has permission to
...
abstract contract AccessControl is Context, IAccessControl, BEP165 {
    ...
    bytes32 public constant DEFAULT_MINTER_ROLE = 0x00;
...
contract LEGACYLIFE is BEP20, BEP20Burnable, Pausable, Ownable,AccessControl {
    
    uint256 public MaxSupply = 50000000000e18;
    constructor() BEP20("LEGACYLIFE", "LLIFE") {
        _mint(msg.sender,1500000000e18);
    ...
    function mint(address to, uint256 amount) external onlyRole(getRoleAdmin(DEFAULT_MINTER_ROLE)) {
