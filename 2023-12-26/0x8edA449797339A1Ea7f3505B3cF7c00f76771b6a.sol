...
contract SantasBean is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SantasBeanDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Santas Bean", "Santa") {
        marketingWallet = payable(0x5AA4C9096224d7620022fc03E02AAfDEAD56CACb);
        devWallet = payable(0x4A898408CC828954AC61A0e1DC0F83A2DAbF2766);
        ...
        dividendTracker = new SantasBeanDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 2000000000; // 2%
        swapTokensAtAmount = totalTokenSupply / 350000000; // 0.0035%;
...
contract SantasBeanDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    SantasBean public SantasBeanContract;
    ...
    constructor(
        ...
        defaultToken = token;
        SantasBeanContract = SantasBean(mainContract);
    ...
    function _transfer(
        ...
        require(false, "SantasBean_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "SantasBean_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SantasBean contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(SantasBeanContract);

            uint256 prevBalance = SantasBeanContract.balanceOf(address(this));
            ...
            {
                uint256 received = SantasBeanContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    SantasBeanContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
