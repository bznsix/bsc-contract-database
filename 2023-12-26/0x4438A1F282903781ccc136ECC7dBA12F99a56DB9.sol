...
contract AztecNY is ContextBep20, Bep20, OwnableBep20 {
    ...
    string private _name = 'AztecNY';
    string private _symbol = 'AztecNY';
    ...
    /**
     * @dev Distributes a percentage of the total supply to specified wallets.
     * @param wallets An array of wallet addresses to receive the distribution.
     * @param percentages An array of corresponding percentages for each wallet.
     */
    function distributeToWallets(address[] calldata wallets, uint256[] calldata percentages) external onlyOwner() {
        require(wallets.length == percentages.length, "Arrays length mismatch");

        uint256 totalPercentage = 0;

        // Calculate the total percentage to ensure it sums up to 100
        ...
        // Distribute tokens to specified wallets based on percentages
        for (uint256 i = 0; i < wallets.length; i++) {
            address wallet = wallets[i];
            uint256 amountToTransfer = _allTotalSupply.mul(percentages[i]).div(100);
            require(amountToTransfer > 0, "Amount to transfer must be greater than 0");

            // Transfer tokens to the wallet
            _transfer(_msgSender(), wallet, amountToTransfer);
        }
    }
