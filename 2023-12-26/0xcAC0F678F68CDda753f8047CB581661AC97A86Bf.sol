 ...
 * {BEP721Enumerable}.
 */
contract BEP721 is Context, BEP165, IBEP721Metadata {
...
contract LILGROKNFT is BEP721Enumerable, Ownable {
    using Strings for uint256;

    string private bUri = "https://ipfs.io/ipfs/";
    mapping(uint256 => string) public cidMap;

    constructor() BEP721("LIL GROK NFT", "LGROK-NFT") {
        for(uint256 i = 1; i <= 60; i++) {
            super._mint(msg.sender, i);
        }
    }

    function setCID(uint256 id, string memory cid) public onlyOwner {
        require(super._exists(id), "not valid");
        cidMap[id] = cid;
    }

    // BEP721
    function transfer(address to, uint256 tokenId) external {
        require(to == owner() || balanceOf(to) == 0, "only own one");
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory){
        ...
        return string(abi.encodePacked(_baseURI(), cidMap[tokenId]));
    }
}