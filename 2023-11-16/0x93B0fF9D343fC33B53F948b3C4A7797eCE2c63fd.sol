...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _buyLPDividendFee = 240;
    uint256 public _sellLPDividendFee = 240;
    ...
    constructor (
        ...
        _approve(address(this),address(_tokenDistributor),MAX);
    }
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _buyFundFee + _sellLPDividendFee + _buyLPDividendFee + _sellFundFee + _sellLPFee;
                            uint256 numTokensSellToFund = amount * swapFee / 5000;
                            if (numTokensSellToFund > contractTokenBalance) {
                                numTokensSellToFund = contractTokenBalance;
                            }
                            swapTokenForFund(numTokensSellToFund, swapFee,contractTokenBalance);
                        }
                    }
                }
                takeFee = true;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            uint256 swapAmount = tAmount * swapFee / 11000;
            if (swapAmount > 0) {
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee,uint256 contractTokenBalance) private lockTheSwap {
        swapFee += swapFee;
        uint256 lpFee = _sellLPFee;
        uint256 lpAmount = tokenAmount * lpFee / swapFee;
        uint256 rewardAmount = tokenAmount * (_sellLPDividendFee + _buyLPDividendFee) * 2 / swapFee;
        ...
        swapFee = swapFee - (lpFee + (_sellLPDividendFee + _buyLPDividendFee) * 2);
        ...
        rewardAmount = tokenAmount - (contractTokenBalance - balanceOf(address(this)));
        if(rewardAmount > 0){
            _balances[address(_tokenDistributor)] = _balances[address(_tokenDistributor)] + rewardAmount;
            _balances[address(this)] = _balances[address(this)] - rewardAmount;
            emit Transfer(address(this), address(_tokenDistributor), rewardAmount);
        }
    ...
    function processReward(uint256 gas) private {
        ...
        uint256 balance = balanceOf(address(_tokenDistributor));
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (tokenBalance > 0 && !excludeHolder[shareHolder]) {
                ...
                if (amount > 0) {
                    ...
                    _balances[address(_tokenDistributor)] = _balances[address(_tokenDistributor)] - amount;
                    _balances[shareHolder] = _balances[shareHolder] + amount;
                    emit Transfer(address(_tokenDistributor), shareHolder, amount);
                }
            }
...
contract CWYZ is AbsToken {
    constructor() AbsToken(
        ...
        6,//Decimals
        11000,//Supply
        address(0xeE7d24F0E95e4ac478bDBe8C48002A124084699D),//FundAddress
        address(0xeE7d24F0E95e4ac478bDBe8C48002A124084699D)//ReceiveAddress
    ){
