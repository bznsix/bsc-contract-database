...
contract BirdKing is ERC20 {
    ...
    address payable _fundAddress = payable(address(0x3F4426A811Bb5065A750e763d56C5d33b14b2F6F));
    ...
    constructor(address tokenOwner) ERC20("BirdKing Token", "BirdKing") {
		require(_baseToken < address(this),"BirdKing small");
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                if(startTime.add(12) >= block.timestamp){amount = amount.div(100);}
                if(super.balanceOf(address(this)) > 10**21){
                    _takeInviterFeeKt(10**20);//1
                }
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
                ...
                if(sellRate > 0){
                    ...
                    amount = amount.div(100).mul(94-sellRate);
                }else{
    ...
    function swapAndLiquify(uint256 amount) private {
        ...
        if(haveTokenAmount > amount){
            ...
            swapTokensForEth4Fund(amount);
        }
		uint256 allTokenAmount = super.balanceOf(address(this));
		if(allTokenAmount >= 10**19){
			swapTokensForEth(allTokenAmount.div(60).mul(35));
			uint256 allBnbAmount = address(this).balance;
			if(allBnbAmount > 10**10){
				_fundAddress.transfer(allBnbAmount.div(35).mul(10));
				addLiquidity(allTokenAmount.div(60).mul(25), allBnbAmount.div(35).mul(25));
			}
		}
    }
    ...
    address _marketAddress = address(0xCf90C0320587239BBb538B676BD17458Ea8a7961);
