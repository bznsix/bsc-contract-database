...
contract Lootboxes is ERC1155, TokenAccessControl, ERC2981 {
    struct Lootbox {
        bool allowedMint;
        uint256 mintPrice;
    ...
    mapping(address => mapping(uint256 => uint256)) public walletLastMint;
    ...
    function setLootbox(
        ...
        lootboxes[tokenId] = Lootbox(
            allowedMint,
            mintPrice,
            mintCooldown,
            limit,
            lootboxes[tokenId].supply
        );
    }
    ...
    function mintTo(
        ...
        require(
            lootboxes[tokenId].supply + amount <= lootboxes[tokenId].limit,
            "ERC1155 Hotdrop: Lootbox limit reached!"
        );
        lootboxes[tokenId].supply += amount;
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            require(
                lootboxes[ids[i]].supply + amounts[i] <=
                    lootboxes[ids[i]].limit,
                "ERC1155 Hotdrop: Lootbox limit reached!"
            );
            lootboxes[ids[i]].supply += amounts[i];
    ...
    function mint(address to, uint256 tokenId) public payable {
        require(
            ...
            "ERC1155 Hotdrop: Minting is temporarily paused!"
        );
        require(
            lootboxes[tokenId].allowedMint == true,
            "ERC1155 Hotdrop: Invalid token id"
        );
        require(
            lootboxes[tokenId].supply < lootboxes[tokenId].limit,
            "ERC1155 Hotdrop: Lootbox limit reached!"
        );
        require(
            msg.value >= lootboxes[tokenId].mintPrice,
            "ERC1155 Hotdrop: Insufficient payment!"
        );
        require(
            walletLastMint[msg.sender][tokenId] +
                lootboxes[tokenId].mintCooldown <=
                block.timestamp,
            "ERC1155 Hotdrop: You can't mint yet, the mint cooldown hasn't expired yet "
        );
        walletLastMint[msg.sender][tokenId] = block.timestamp;
        lootboxes[tokenId].supply += 1;
        _mint(to, tokenId, 1, "");
    }
}