...
contract PresaleContract is ReentrancyGuard, Ownable, Pausable {
    ...
    mapping(uint256 => uint256) public saleRoundTime;
    mapping(uint256 => uint256) public roundRate;
    Round[] public round_list;

    uint256 public current_round_index;
    uint256 public presaleRoundEndTime;
    bool public presale_ended;
    address private feeWallet;

    event Deposit(
        address indexed _user_wallet,
        uint256 indexed _pay_method,
        uint256 _user_usdt_trans,
        uint256 _user_tokens_trans
    );
    ...
    constructor(
        ...
        usdt_interface = IERC20Metadata(0x55d398326f99059fF775485246999027B3197955);
        busd_interface = IERC20Metadata(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
        token_interface = IERC20Metadata(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
        price_feed = AggregatorV3Interface(0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE);
        feeWallet = 0x8377464940B0a0305b8A4f8231F3F396F2662bEa; 
        roundRate[0] = 5;
        roundRate[1] = 3;
        roundRate[2] = 1;

        saleRoundTime[0] = 60 days;
        saleRoundTime[1] = 45 days;
        saleRoundTime[2] = 30 days;
        presaleRoundEndTime = block.timestamp + 60 days;

        current_round_index = 0;
        presale_ended = false;

        round_list.push(
            Round(payable(feeWallet), 5, 0, 1e7 * (10**18))
        );
    }
    ...
    function get_bnb_in_usd() private view returns (uint256) {
        (, int256 price, , , ) = price_feed.latestRoundData();
        price = price * 1e10;
        return uint256(price);
    }

    function get_USD_VALUE(uint256 bnb, uint256 decimals) private view returns (uint256){
         return ((get_bnb_in_usd() * bnb) / 10** decimals);
    }
        ...
        returns (bool)
    {
        ...
        if (block.timestamp > presaleRoundEndTime) {
            current_round_index = current_round_index + 1;
            presaleRoundEndTime =
                block.timestamp +
                saleRoundTime[current_round_index];
            round_list.push(
                Round(
                    payable(feeWallet),
                    roundRate[current_round_index],
                    0,
                    2e6 * 1e18
                )
            );
        ...
        uint256 amount_in_tokens = ((amount_in_usdt *
            ...
            (10**token_interface.decimals())) / (10**usdt_interface.decimals());
    ...
    function buy_with_busd(uint256 amount_)
        external
        nonReentrant
        whenNotPaused
        canPurchase(_msgSender(), amount_)
        returns (bool)
    {
        ...
        uint256 amount_in_tokens = ((amount_in_usdt *
            round_list[current_round_index].usdt_to_token_rate) *
            (10**token_interface.decimals())) / (10**busd_interface.decimals());
    ...
    function buy_with_bnb()
        ...
        returns (bool)
    {
        uint256 amount_in_usdt = get_USD_VALUE(msg.value, 18);
        if (block.timestamp > presaleRoundEndTime) {
            current_round_index = current_round_index + 1;
            presaleRoundEndTime =
                block.timestamp +
        ...
        (bool sent, ) = round_list[current_round_index].wallet.call{
            value: msg.value
        }("");
    ...
    function buy_with_bnb_wert(address user)
        external
        payable
        nonReentrant
        whenNotPaused
    ...
    function start_next_round(
        ...
        round_list.push(
            Round(wallet_, usdt_to_token_rate_, 0, usdt_round_cap_ * (10**18))
        );
    }
    ...
    function set_Current_Round_Time(uint256 _time) external onlyOwner {
        require(_time > block.timestamp, "can't set time in past");
        presaleRoundEndTime = _time;
    }
