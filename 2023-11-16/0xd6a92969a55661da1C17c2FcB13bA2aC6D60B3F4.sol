...
contract DragonInfinity is ERC20, ERC20Burnable, Ownable {
    ...
    constructor() ERC20("Dragon Infinity Token", "DRIN") {
        totalTokens = 1_000_000_000 ether;
    ...
    function _transfer(
        ...
        if (
            ...
            if (recipient == swapPair) {
                uint256 fee = caculateFee(amount, sellTax);
                super._transfer(sender, buyBackWallet, fee);
                amount = amount.sub(fee);
            } else if (sender == swapPair) {
                uint256 fee = caculateFee(amount, buyTax);
                super._transfer(sender, buyBackWallet, fee);
                amount = amount.sub(fee);
            }
        }
    ...
    /**
     * @dev Set tax
     * @param _sellTax: The sell tax.
     * @param _buyTax: The buy tax.
     */
    function setupTax(uint16 _sellTax, uint16 _buyTax) external onlyOwner {
        require(_sellTax <= 1000, "Sell fee must be less than 10%");
        require(_buyTax <= 1000, "Buy fee must be less than 10%");
        sellTax = _sellTax;
        buyTax = _buyTax;
    }
    ...
    /**
     * @dev Set swap pair
     * @param _swapPair: The swap pair address.
     */
    ...
    /**
     * @dev Set excluded from fee
     * @param _address: The address.
     * @param _excluded: The excluded.
     */
    ...
    /**
     * @dev Set anti bot value
     * @param _valueAntiBot: The value anti bot.
     */
    function setValueAntiBot(uint256 _valueAntiBot) external onlyOwner {
        valueAntiBot = _valueAntiBot;
    }
