...
import "./variables/DCAconfig.sol";
...
contract GetData is DCAconfig, ERC2771Context {
    ...
    function getUserSwapHistory(
        address _user,
        uint256 _id
    ) public view returns (uint256[] memory swapHistory) {
        swapHistory = userSwapHistory[_user][_id];
    }
}
// SPDX-License-Identifier: MIT
pragma solidity 0.8.19;
...
error more_than_deposited();
error low_balance();
error low_gas();
error Gas_refund_failed();
error time_not_match();
error not_active();
error Schedule_complete();
error SWAP_CALL_FAILED();
error ETH_not_transfer();
error not_equal();
