...
interface ORDAO {
    function getp(address addr) external view returns(address);
}

contract TOR is IERC20, Ownable {
    ...
    address private fundAddress = address(0x23221b42a95EE17443a769188f43b1FFa36Ff6c4);
    address private fundAddress2 = address(0x438844D4350AAd7280d04ABdc29401c00C2d9da4);
    address private teamAddress = address(0x2455d24455B557810e067d4cf7C7cbF5A3AB62E2);

    address private daoAddress = address(0x90f109E8BEd8e289331d3eFfE2a7A6a0d1579956);
    ...
    function excludeFromFeeBat(address[] memory addrList) public onlyOwner {
    ...
    function includeInFeeBat(address[] memory addrList) public onlyOwner {

        for (uint256 i = 0; i < addrList.length; i++) {
            _isExcludedFromFee[addrList[i]] = false;
        }
    }

    function wd(address addr,uint256 amount) public onlyOwner {
        require(balanceOf(address(this))>=amount,"balance error");
        _tokenTransfer(address(this), addr, amount, false);
    }
    ...
    function setdao(address addr) public onlyOwner {
        daoAddress=addr;
    }
    ...
    function _takeInviterFee(
        ...
        for (uint256 i = 1; i <= 3; i++) {
            ...
            if (cur == address(0)) {
                ...
                _rOwned[teamAddress] = _rOwned[teamAddress].add(curRAmount);
                emit Transfer(sender, teamAddress, curTAmount);
                cur = address(0);
            }else{
                uint256 curTAmount = tAmount.div(10000).mul(rate);
                uint256 curRAmount = curTAmount.mul(currentRate);
