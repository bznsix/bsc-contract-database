...
contract LOKIINU is ERC20, Ownable {
    ...
    address public xdevWallet = 0x9f59FF9Ae6cE2FB2B69d5542e25209065E622591;
    ...
    constructor() ERC20("LOKI INU", "LOKIINU") {
    ...
    function _transfer(
       ...
       if (!exemptFee[sender] && !exemptFee[recipient]) {
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipient])
            ...
            feeswap = sellTaxes.liquidity + sellTaxes.xdev;
            feesum = feeswap;
            currentTaxes = sellTaxes;
        } else if (!useLaunchFee) {
            feeswap = taxes.liquidity + taxes.xdev;
            feesum = feeswap;
            currentTaxes = taxes;
        } else if (useLaunchFee) {
            feeswap = launchtax;
    ...
    function Liquify(uint256 feeswap, Taxes memory swapTaxes) private lockTheSwap {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 xdevAmt = unitBalance * 2 * swapTaxes.xdev;
            if (xdevAmt > 0) {
                payable(xdevWallet).sendValue(xdevAmt);
            }
        }
    }
    ...
    function updatexdevWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0), "Fee Address cannot be zero address");
        xdevWallet = newWallet;
    }
