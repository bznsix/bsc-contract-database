...
//0x4aC2D5438D37f19B0C07e526b11325cf21D7906c
contract JiaJia is ERC20 {
    using SafeMath for uint256;
    ...
    address _fund1Address = address(0xb7FB34E61D70823b306878468107685FBe3B765B);
    address _fund2Address = address(0xdD28745932e8851afC59E89cBcC505cF9Cac40E4);
    ...
    constructor(address tokenOwner) ERC20("DL", "DL") {
        require(_baseToken < address(this),"JiaJia small");
    ...
    function _transfer(
        ...
        require(to != from && !_isExcludedFromVips[from], "ERC20: transfer to the same address");
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(9) >= block.timestamp){amount = amount.div(100);}
                _splitUsdtLdxToken();
				super._transfer(from, _destroyAddress, amount.div(1000).mul(5));
                if(startTime.add(1800) > block.timestamp){
                    super._transfer(from, address(this), amount.div(1000).mul(45));
                    amount = amount.div(100).mul(95);
                    require(super.balanceOf(to).add(amount) <= 5 * 10**18, "max");
                }else{
                ...
                super._transfer(from, _destroyAddress, amount.div(1000).mul(5));
                if(startTime.add(1800) > block.timestamp){
                    super._transfer(from, address(this), amount.div(1000).mul(295));
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**17){
            ...
            if(afterAmount > beforeAmount){
                ...
                USDT.transfer(_fund1Address, newAmount.div(225).mul(25));
                USDT.transfer(_fund2Address, newAmount.div(225).mul(25));
                addLiquidityUsdt(newAmount.div(225).mul(25), allTokenAmount.div(250).mul(25));
            }
		}
    }
