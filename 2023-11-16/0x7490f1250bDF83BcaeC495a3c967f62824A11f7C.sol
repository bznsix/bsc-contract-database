...
contract MGC is IBEP20, Ownable {
  ...
  address internal _feeMarket = 0x71A4B58b9A7b96b13f438c571E179fCe06277FaA;
  uint256 internal constant RBASE = 10000;
  uint256 internal _fee_buy = 0;
  uint256 internal _fee_sell = 1000;
  ...
  function _transfer(
    ...
    uint256 fee1;
    if (fee > 0) {
      fee1 = amount.mul(fee).div(RBASE);
      _takeTransfer(from, _feeMarket, fee1);
      amount = amount.sub(fee1);
    }
  ...
  function setFee(uint256 buy, uint256 sell) external onlyOwner {
    require(buy < RBASE && sell < RBASE, "invalid fee");
    _fee_buy = buy;
    _fee_sell = sell;
  }
}