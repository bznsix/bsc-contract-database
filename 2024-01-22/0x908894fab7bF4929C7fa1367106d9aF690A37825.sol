...
contract DollarFortUpdated {
    ...
    uint256 public immutable CONST_MAX_WITHDRAWAL = toWei(600);
    ...
    struct UserDetails {
        ...
        uint256 latestInvestmentTimestamp;
        UserInvestment[] investments;
        address[] referrals;
        address sponsor;
        UserIncome userIncome;
        uint256 balanceFromLastWithdrawal;
    }
        ...
        if (_user.details.userIncome == UserIncome.NN) {
            ...
            for (uint256 i = 0; i < _user.details.investments.length; i++) {
                ...
                if (
                    ...
                    _investment.incomeType != UserIncome.NN
                ) {
                    continue;
                }

                uint256 startingPoint = _user.details.lastWithdrawalTimestamp ==
                    0
                    ? _investment.timestamp
                    : _user.details.lastWithdrawalTimestamp;

                if (_user.details.lastWithdrawalTimestamp == 0) {
                    startingPoint = _investment.timestamp;
                } else {
                    if (
                        _investment.timestamp >
                        _user.details.lastWithdrawalTimestamp
                        ...
                        startingPoint = _user.details.lastWithdrawalTimestamp;
                    }
                }

                uint256 endingPoint = timestamp;

                if (startingPoint > endingPoint) {
                    // this will likely never happen, but just a precaution for number overflow
                    startingPoint = endingPoint;
                }

                uint256 _seconds = (endingPoint - startingPoint);
    ...
    function getUserAvailableGenerationIncome(
        ...
        uint256 startingPoint = _user.details.lastWithdrawalTimestamp == 0
    ...
    function invest(
        ...
        if (userIncome == UserIncome.NN) {
            if (msg.sender != CONST_SYSTEM_ADDRESS) {
                require(
        ...
        if (isNewUser) {
            ...
            _user.details.latestInvestmentTimestamp = block.timestamp;
            ...
            for (
                ...
                if (!isCurrentlyInactive && userIncome == UserIncome.NN) {
        ...
        if (isNewUser) {
            ...
            if (_sponsorUser.details.referrals.length == 1) {
                ...
                if (
                    ...
                    uint256 earnedLimit = 0;
                    if (_sponsorUser.details.pendingFromMaxLimit > maxLimit) {
                        earnedLimit = 0;
                    } else {
                        earnedLimit =
                            maxLimit -
        ...
        if (userIncome == UserIncome.NN && getContractBalance() >= _fees) {
            _usdtToken.transfer(CONST_PROJECT_ADDRESS, _fees / 2);
            _usdtToken.transfer(CONST_FEES_ADDRESS, _fees / 2);
        }
    ...
    function publicLimit(address _addr) public view returns (uint256) {
        if (
            users[_addr].details.userIncome == UserIncome.NN ||
            users[_addr].details.userIncome == UserIncome.AP
        ) {
            return users[_addr].details.pendingFromMaxLimit;
        }

        if (users[_addr].details.userIncome == UserIncome.RR) {
            if (_getUserReferralCount(_addr) == 0) {
                return 0;
            } else {
                return users[_addr].details.pendingFromMaxLimit;
            }
        ...
        if (_user.details.userIncome == UserIncome.NN) {
            ...
            for (uint256 i = 0; i < _user.details.investments.length; i++) {
                ...
                uint256 startingPoint = _user.details.lastWithdrawalTimestamp ==
                    ...
                    ? _investment.timestamp
                    : _user.details.lastWithdrawalTimestamp;
                uint256 endingPoint = block.timestamp;
                ...
                if (
                    (endingPoint - _investment.timestamp > 365 * 86400 ||
                        _seconds > 365 * 86400 ||
                        _user.details.pendingFromMaxLimit == 0) &&
                    _investment.endedTimestamp == 0
                ) {
...
contract BEP20USDT is Context, IBEP20, Ownable {
    ...
    constructor() {
        ...
        _symbol = "USDT";
        _decimals = 18;
        _totalSupply = 30000000000000000000000000000000000000000;
