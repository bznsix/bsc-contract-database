...
// Golden
// Whitepaper: https://goldengator.gitbook.io/golden/
// Telegram: https://t.me/golgator
...
contract Goldengator is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    GoldengatorDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Golden gator", "GGTR") {
        marketingWallet = payable(0xc14580730A0A860a3CBa19D52e82F2b0466740e2);
        devWallet = payable(0x7d63FFd3C38222059016c6B4A993e36e481DaaB1);
        ...
        dividendTracker = new GoldengatorDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 100000; // 0.005%
        swapTokensAtAmount = totalTokenSupply / 10000; // 0.05%;
...
contract GoldengatorDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Goldengator public GoldengatorContract;
    ...
    constructor(
        ...
        defaultToken = token;
        GoldengatorContract = Goldengator(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Goldengator_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Goldengator_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Goldengator contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(GoldengatorContract);

            uint256 prevBalance = GoldengatorContract.balanceOf(address(this));
            ...
            {
                uint256 received = GoldengatorContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    GoldengatorContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
