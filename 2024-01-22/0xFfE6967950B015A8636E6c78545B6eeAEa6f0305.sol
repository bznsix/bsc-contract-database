...
import {SignatureVerification} from "../interface/SignatureVerification.sol";

contract WhitelistController is SignatureVerification{

    event ApplyWhitelist(
        uint256 indexed timestamp,
        address indexed user,
        string projectName
    );

    constructor() SignatureVerification("WhitelistController","1.0.0"){}

    function applyWhitelist(string memory projectName, bytes calldata signature) external isValidSigner(signature){
        emit ApplyWhitelist(block.timestamp, msg.sender, projectName);
    }
}
...
contract SignatureVerification is EIP712 {
    ...
    address constant _signerAddress = 0x69d2581E43F3E8357ec7ca2770b2D976645BE91A;
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                          "Hightable(address contractAddress,address account,uint256 nonce)"
                        ),
                        contractAddress,
                        account,
                        nonce
