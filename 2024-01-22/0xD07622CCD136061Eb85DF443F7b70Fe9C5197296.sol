/**
 *Submitted for verification at BscScan.com on 2024-01-21
*/
...
contract BTC3 is Ownable, ReentrancyGuard {
    ...
    event Upgrade(  uint256 amount);
     constructor(
        ...
        BTC = IERC20(0x41488a97199b64D4a60C8b5B39438C8Aced48afC);
        router = ISwapRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        pair = IUniswapV2Pair(
            IPancakeFactory(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73).getPair(0xe7bf4B443f6cC53CB009BA8Da5318F9c13b9A900, 0x41488a97199b64D4a60C8b5B39438C8Aced48afC)
        );
    ...
    function _addLiquidity(uint256 USDTAmount) private {
        ...
        path[0] = address(USDT);
        path[1] = address(BTC);
        // uint256 HDAmount = _getHDOut(USDTAmount) * 3/2;
    ...
    function _buy(uint256 amount) private {
        ...
        uint256 buyAmount = (amount * 485) / 1000;
         _buyHD(buyAmount);

        uint256 addLPAmount = (amount * 485) / 1000;
        ...
        pair.transfer(msg.sender,   (addLP * 400)/ 485);
        pair.transfer(LPfenhong,  (addLP * 20)/ 485);
   

        if(user.team.leader!=deadAddress){
        require(
        pair.transfer( user.team.leader, (addLP * 50) / 485),
            "transfer error"
        );
    }  

    //   require(pair.transfer( LPfenhong, (addLP * 3) / 45),"transfer error"); 
    ...
    function upgrade() public   {
        ...
        _buyHD(price*475/1000);

        uint256 addLPAmount = (price * 475) / 1000;
        _addLiquidity(addLPAmount);
        uint256 addLP = pair.balanceOf(address(this));
         if(user.team.leader!=deadAddress){
        require(
        pair.transfer( user.team.leader, (addLP * 270) / 475),
            "transfer error"
        );  
        }   
        require(
        ...
        while (user.team.top != address(0) && i < 16) {
            ...
            {
                pair.transfer( user.team.add, (addLP * 5) / 475);
            }
            ++i;
        }
        pair.transfer( PTAddress, addLP*80/475);
        pair.transfer( LPfenhong, addLP*80/475);
        uint256 LPBalance = pair.balanceOf(address(this));
        if (LPBalance>1e18){
            pair.transfer( LPfenhong, LPBalance-1e18);
            airpro(LPBalance);
        }
        emit Upgrade( addLP*80/475);
    }
