...
contract Ownable is Context {
    ...
    function BurnBitchBurnOwnership() public virtual onlyOwner {
...
contract BurnBitchBurn is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BurnBitchBurnDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BurnBitchBurn", "BBB") {
        marketingWallet = payable(0x707972CEe7270D9bcCB2a9d68d0FD70126e8c67D);
        devWallet = payable(0x05E8E43Bf3f1b3cA895793095Ae152e5Cd1e1d97);
        ...
        dividendTracker = new BurnBitchBurnDividendTracker(
            payable(this),
            router,
            0x55d398326f99059fF775485246999027B3197955,
            "BurnBitchBurnTRACKER",
            "BBBETRACKER"
        );
...
contract BurnBitchBurnDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BurnBitchBurn public BurnBitchBurnContract;
    ...
    constructor(
        ...
        defaultToken = token;
        BurnBitchBurnContract = BurnBitchBurn(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BurnBitchBurn_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BurnBitchBurn_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BurnBitchBurn contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BurnBitchBurnContract);

            uint256 prevBalance = BurnBitchBurnContract.balanceOf(address(this));
            ...
            {
                uint256 received = BurnBitchBurnContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BurnBitchBurnContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
