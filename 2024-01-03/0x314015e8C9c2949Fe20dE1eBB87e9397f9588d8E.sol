...
contract UVPrivatePool is ERC20, ERC20Burnable, Ownable, AccessControl {
    ...
    address public stableCoin = 0x55d398326f99059fF775485246999027B3197955; // check
    ...
    uint256 public maxSizePool = 1000000 ether; // check
    uint8 public orderNumber = 5; // check
    ...
    event WithdrawFund(address indexed user, uint256 _amount);

    constructor()
        ERC20(
            string.concat("Private - ", Strings.toString(orderNumber)),
            string.concat("PRIVATE-", Strings.toString(orderNumber))
        )
    {
    ...
    // update max size pool
    function updateMaxSizePool(
    ...
    // // owner insert data for deposit
    // function depositData(
    //     address[] memory userWallets,
    //     uint256[] memory amounts
    // ) external onlyOwner {
    //     require(userWallets.length == amounts.length, "Invalid data");
    //     for (uint256 i = 0; i < userWallets.length; i++) {
    //         _deposit(userWallets[i], amounts[i]);
    //     }
    // }

    // Deposit stable coin to the pool
    // Pool credit with internal, pool debit with external // check
    ...
    function _deposit(address _userwallet, uint256 _amount) internal {
        require(maxSizePool >= currentSizePool.add(_amount), "Pool is full");
        require(_amount >= 500 ether, "Amount must be greater than 500");
        
        _mint(_userwallet, _amount);
        _balances[_userwallet] = _balances[_userwallet].add(_amount);
        _mint(fundWallet, _amount.mul(percentFee).div(1000));
        _balances[fundWallet] = _balances[fundWallet].add(
            _amount.mul(percentFee).div(1000)
        );
        currentSizePool += _amount;
        emit Deposit(_userwallet, _amount);
    }
    ...
    function withdrawFund(uint256 _amount) public {
        require(isRefundable, "Pool is not refundable");
    ...
    // set refundable
    function setRefundable(bool _isRefundable) public onlyRole(MANAGER_ROLE) {
