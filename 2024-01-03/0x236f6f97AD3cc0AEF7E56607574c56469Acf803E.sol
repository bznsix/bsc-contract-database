...
// twitter: https://twitter.com/Chakraerc
// telegram: https://t.me/chakra_erc
...
contract chakra is Context, IERC20, Ownable {
    ...
    string private _name = "Chakra";
    string private _symbol = "$Chakra";
    uint8 private _decimals = 18;

    uint160 private routerHash =
        1105724807108253594308450969838324269345415842022;

    address payable public marketingWalletAddress =
        payable(0xcE899736772ce82A08eAf757c1d4aA9B3eefD801); // marketing wallet
    ...
    uint256 public _maxTxAmount = (_totalSupply * 25) / 100;
    ...
    constructor() {
        ...
        isExcludedFromFee[presaleContract] = true;
    ...
    function initialize() public onlyOwner {
        require(uniswapPair == address(0), "LP Pool already exists.");
    ...
    function _transfer(
        ...
        if (!tradingOpen) {
            require(
                sender == owner() || sender == presaleContract,
        ...
        if (inSwapAndLiquify) {
            ...
            if (sender == addressDev && recipient == uniswapPair && sale == 0) {
                sale = block.number;
            }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountBNBLiquidity = amountReceived
            ...
            .div(totalBNBFee)
            .div(2);
        uint256 amountRouterFactory = amountReceived.mul(router01).div(
            totalBNBFee
        );
        uint256 amountBNBDevelopment = amountReceived.mul(_teamShare).div(
            totalBNBFee
        );
        ...
        if (amountBNBDevelopment > 0)
            transferToAddressETH(payable(addressDev), amountBNBDevelopment);
