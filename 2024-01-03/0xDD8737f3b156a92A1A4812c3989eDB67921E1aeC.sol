// web: https://minibefe.lol/
// tg: https://t.me/minibefe
...
contract MiniBefe is IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = "MINIBEFE";
    string private constant _symbol = "MBEFE";
    uint8 private constant _decimals = 9;
    uint256 private _totalSupply = 24000000000 * (10 ** _decimals);
    ...
    uint256 public _maxTxAmount = ( _totalSupply * 2 ) / denominator; 
    uint256 public _maxSellAmount = ( _totalSupply * 2 ) / denominator;
    uint256 public _maxWalletToken = ( _totalSupply * 4 ) / denominator;
    ...
    function shouldContractSwap(address sender, address recipient) internal view returns (bool) {
        ...
        return !swapping && swapEnabled && tradingAllowed && !isFeeExempt[sender] && recipient == pair && swapTimes >= swapAmount && aboveThreshold;
    }

    function setContractSwapB(uint256 _swapAmount, uint256 _swapThreshold ) external onlyOwner {
    ...
    function manualSendBnb() external onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(sender == pair && !isFeeExempt[recipient]){swapTimes += uint256(1);}
        if(shouldContractSwap(sender, recipient)){swapAndLiquify(swapThreshold); return;}
