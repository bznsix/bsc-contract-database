...
contract BEP20 is IBEP20, Ownable {
  ...
  string private constant NAME = "Bitcoin2";
  string private constant SYMBOL = "BTC2";
...
contract BTC2 is BEP20 {
  ...
  bool dumpProtectionEnabled = false;
  bool sniperTax = false;
  bool tradingEnabled;
  bool inSwap;
  ...
  uint256 public liquidityShare = 1000;
  uint256 public marketingShare = 0;
  ...
  event SetTaxes(uint256 dev, uint256 liquidity, uint256 marketing);
  event SetShares(uint256 devShare, uint256 liquidityShare, uint256 marketingShare);
  ...
  event SetdevWallet(address newAddress, address oldAddress);
  event AutoLiquidity(uint256 pair, uint256 tokens);
  event DepositMarketing(address indexed wallet, uint256 amount);
  event Depositdevs(address indexed wallet, uint256 amount);
  ...
  constructor(address owner, address marketing, address devs) BEP20(owner, marketing) {
  ...
  function _swapBack() private swapping {
    ...
    uint256 amountBNBdevs = amountBNB * devShare / totalBNBShares;
    ...
    (bool devSuccess,) = payable(devWallet).call{value: amountBNBdevs, gas: transferGas}("");
    if (devSuccess) { emit Depositdevs(devWallet, amountBNBdevs); }
  ...
  function setIsMarketMaker(address account, bool value) external onlyOwner {
    ...
    function setBlacklist(address blacklist, bool value) external onlyOwner{
        require(_isBlacklisted[blacklist] != value);
        _isBlacklisted[blacklist] = value;
    }
  ...
  function setShares(uint256 newdevShare, uint256 newLiquidityShare, uint256 newMarketingShare) external onlyOwner {
    devShare = newdevShare;
    liquidityShare = newLiquidityShare;
    marketingShare = newMarketingShare;
    totalShares = devShare + liquidityShare + marketingShare;
    emit SetShares(devShare, liquidityShare, marketingShare);
  }
  ...
  function setdevWallet(address newAddress) external onlyOwner {
    require(newAddress != address(0), "New dev wallet is the zero address");
    emit SetdevWallet(newAddress, devWallet);
