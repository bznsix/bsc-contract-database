...
//TG:https://t.me/DegenUSDC
//Website:https://www.degenusdc.com

//Tax layout
//BUY:   5% Rewards  2% Liquidity  2% Marketing  1% Dev Team  -  Total: 10%
//SELL: 10% Rewards  2% Liquidity  2% Marketing  1% Dev Team  -  Total: 15%

//Rewards in USDC based on volume, then distributed through buys/sells
...
contract DegenUSDC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DEGENDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Degen USDC", "DUSD") {
        marketingWallet = payable(0x7383760B83d3dF4e5bc12F9659Ba1156fB0D9641);
        devWallet = payable(0xE401BEa4e090797757eAc649D74Ff0B2BdA90dde);
        ...
        dividendTracker = new DEGENDividendTracker(
            payable(this),
            router,
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 30000 && newValue <= 30000);
...
// If blockage clear and send Dividends 
...
contract DEGENDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    DegenUSDC public DegenUSDCContract;
    ...
    constructor(
        ...
        defaultToken = token;
        DegenUSDCContract = DegenUSDC(mainContract);
        minimumTokenBalanceForAutoDividends = 1000000_000000000000000000; // 1M tokens
    ...
    function _transfer(
        ...
        require(false, "DEGEN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DEGEN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(DegenUSDCContract);

            uint256 prevBalance = DegenUSDCContract.balanceOf(address(this));
            ...
            {
                uint256 received = DegenUSDCContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    DegenUSDCContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
