...
// TELEGRAM : https://t.me/b_defi 
// WEBSITE : https://b-defi.co
...
contract bdefilocker is ITokenLock, Ownable {
  ...
  modifier feePaid(address token) {
    if (token == wBNB) {
      require(msg.value == feeNative, "Fee");
    } else {
      IERC20(token).safeTransferFrom(msg.sender, address(this), feeToken);
    }
    _;
  }
  ...
  constructor(uint256 _feeNative, uint256 _feeToken) {
    feeNative = _feeNative;
    feeToken = _feeToken;
  }

  function setFeeNative(uint256 newFee) external onlyOwner {
    feeNative = newFee;
  }

  function setFeeToken(uint256 newFee) external onlyOwner {
    feeToken = newFee;
  }
  ...
  function lock(
    ...
    uint256 amount,
    uint256 unlockDate
  ) external payable override feePaid(_feeToken) returns (uint256 id) {
    require(
    ...
    if (isLpToken) {
      ...
      try IPancakePair(token).factory() returns (address factory) {
  ...
  function editLock(
    ...
    uint256 newAmount,
    uint256 newUnlockDate
  ) external payable override feePaid(_feeToken) validLock(lockId) {
    Lock storage userLock = _locks[lockId];
    require(userLock.owner == msg.sender, "You are not the owner of this lock");
