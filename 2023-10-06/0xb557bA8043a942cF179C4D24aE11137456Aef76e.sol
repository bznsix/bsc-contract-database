 ...
 *                               MUST READ
 * 
 * This is a pure stealth launch, Token will go live once some people join the Telegram from the contract
 * 
 * Anti-Whales/Bots  => Max Buy is 2% of the supply. Max hold is 2.5% of the total Supply
 * 
 * 
 * There's also an anti-dump mechanic so holders get rewarded.
 * 
 * Sell within 1 hour   : 32% Tax (24% Back to the Liquidity Pool, 8% Redistributed to Holders)
 * Sell within 2 hours  : 24% Tax (18% Back to the Liquidity Pool, 6%  Redistributed to Holders)
 * Sell within 3 hours  : 16% Tax (12% Back to the Liquidity Pool, 4%  Redistributed to Holders)
 * Standard Tax         : 8%  Tax  (6% Back to the Liquidity Pool, 2%  Redistributed to Holders)
 * 
 * 
 * Ownership will be renounced
 * Liquidity will be locked
 * Initial burn is 50% of the suuply
 * 
 * 
 * 
 * Telegram : https://t.me/AstroPandaOfficial
 * 
 * Website : https://astropanda.finance/
 * 
 *                               _,add8ba,
 *                            ,d888888888b,
...
contract AstroPanda is Context, IBEP20, Ownable {
    ...
    uint256 private constant _tTotal = 10 * 10**9 * 10**8; 
    ...
    uint256 public _maxTxAmount = _tTotal.div(10**2).mul(2); // 
    uint256 public _numTokensSellToAddToLiquidity = _tTotal.div(10**3).mul(5);
    uint256 public _maxWalletToken = _tTotal.div(10**3).mul(25); //

    IPancakeRouter02 public immutable pcsV2Router;
    address public immutable pcsV2Pair;

    string private _name = "AstroPanda Finance";
    string private _symbol = "AstroPanda";
    ...
    function _transfer(
        ...
        uint256 hour = 60 * 60;
        bool timeToSwapTaxes = time_since_start < 2 * hour;

        bool overMinTokenBalance =
            contractTokenBalance >= _numTokensSellToAddToLiquidity;
        if (overMinTokenBalance && !inSwapAndLiquify && sender != pcsV2Pair && timeToSwapTaxes) {
            contractTokenBalance = _numTokensSellToAddToLiquidity;
    ...
    function _getAntiDumpMultiplier() private view returns (uint256) {
        ...
        if (time_since_start < 3 * hour) {
            ...
            return (3);
        } else if (time_since_start < 5 * hour) {
            return (2);
        } else {
            return (1);
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        address developmentWalletAddress = 0xe5051093b7CDCB0C122A80Ec9773bd4Bf4B058c8;
        ...
        swapTokensForBNB(devPortion, payable(developmentWalletAddress));
    }
