...
interface IMintable is IERC721Enumerable {
    // Getters
...
contract Card is IMintable, ERC721Enumerable, AccessControl {
    bytes32 private constant MINTER_ROLE = keccak256("MINTER");

    address public minter;

    constructor() ERC721("Alien Wars Card", "AWC") {
    ...
    {
        return IUriProvider(minter).tokenURI(tokenId);
    }

    // Modifiers
    function setCardMinter(address a) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (uint160(minter) != 0) {
