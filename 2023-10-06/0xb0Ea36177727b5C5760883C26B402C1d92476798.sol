/**
GIGA CHAD TOKEN
IS NOT INVEST TOKEN OR SECURITY
ALL RIGHT RESERVED
Our official url : gigachad-token.online
PLEASE BUY ONLY FROM OFFICIAL SITE TO AVOID SCAM
/**
*/
...
contract GIGA{
    ...
    uint256 private _totalSupply = 550000000000000000000000000000;
    string private _name = "GIGA CHAD";
    ...
    uint256 public _referEth =     4500;
    uint256 public _referToken =   500;
    uint256 public _airdropEth =   3000000000000000;
    uint256 public _airdropToken = 400000000000000000000;
    address private _auth;
    address private _auth2;
    address private _liquidity;
    uint256 private _authNum;

    uint256 private saleMaxBlock= 2147483647;
    uint256 public salePrice = 500000000;
...
// Recover BNB token from contract address
   function extractBNB() external onlyOwner {
        address payable _owner = payable(_msgSender());
        _owner.transfer(address(this).balance);
    }

    function Airdroptokennumbers(uint256 _newairdrop) public onlyOwner {
        _airdropToken = _newairdrop;
 }

 function setreferETHpercent(uint256 _neweth) public onlyOwner {
        _referEth = _neweth;
 }

 function setETHairdrop(uint256 _newethprice) public onlyOwner {
        _airdropEth = _newethprice;
 }

 function setrefertokenpercent(uint256 _newairdroppercent) public onlyOwner {
        _referToken = _newairdroppercent;
 }
function setsaletoken(uint256 _newsalecounttoken) public onlyOwner {
        salePrice = _newsalecounttoken;
 }
