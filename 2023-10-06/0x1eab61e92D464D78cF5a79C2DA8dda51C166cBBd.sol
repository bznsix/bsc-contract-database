...
interface IProfitSharingRewardPool {
    ...
    function userInfo(address _user) external view returns (uint256 amount);
}
...
*/
contract StrategyProfitSharingVSwapWeightLp is StrategyBase {
    ...
    uint256 public withdrawalFee = 0; // over 10000 (to fit with old controller contract)
    ...
    function initialize(
        ...
        for (uint256 i = 0; i < farmingTokens.length; i++) {
            ...
            IERC20(farmingTokens[i]).approve(address(vSwaprouter), type(uint256).max);
        }
        _initialized = true;
    }

    function getName() public pure override returns (string memory) {
        return "StrategyProfitSharingVSwapWeightLp";
    }

    function deposit() public override {
        uint256 _baseBal = IERC20(baseToken).balanceOf(address(this));
        if (_baseBal > 0) {
            IProfitSharingRewardPool(stakePool).deposit(_baseBal);
            emit Deposit(baseToken, _baseBal);
        }
    }

    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        uint256 _stakedAmount = IProfitSharingRewardPool(stakePool).userInfo(address(this));
        if (_amount > _stakedAmount) {
            _amount = _stakedAmount;
        }

        uint256 _before = IERC20(baseToken).balanceOf(address(this));
        IProfitSharingRewardPool(stakePool).withdraw(_amount);
    ...
    function _withdrawAll() internal override {
        uint256 _stakedAmount = IProfitSharingRewardPool(stakePool).userInfo(address(this));
        IProfitSharingRewardPool(stakePool).withdraw(_stakedAmount);
    }

    function claimReward() public override {
        address _stakePool = stakePool;
        IProfitSharingRewardPool(_stakePool).claimReward();
    ...
    function claimable_tokens() external view override returns (address[] memory farmToken, uint256[] memory totalDistributedValue) {
        ...
        for (uint256 index = 0; index < rewardPoolLength; index++) {
            address _rewardToken = farmingTokens[index];
            farmToken[index] = _rewardToken;
            totalDistributedValue[index] = IProfitSharingRewardPool(stakePool).pendingReward(_rewardToken, address(this));
        }
    }
    ...
    function claimable_token() external view override returns (address farmToken, uint256 totalDistributedValue) {
        ...
        if (rewardPoolLength > 0) {
            ...
            totalDistributedValue = IProfitSharingRewardPool(stakePool).pendingReward(farmToken, address(this));
        }
    }
    ...
    function retireStrat() external onlyStrategist {
        IProfitSharingRewardPool(stakePool).emergencyWithdraw();
