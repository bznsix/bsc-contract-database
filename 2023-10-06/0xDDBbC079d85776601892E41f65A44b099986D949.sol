...
contract SVE is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 100_000_000 * 10 **_decimals;     // Supply do Token = 100m
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

	uint256 public _maxWallet = 100_000_000 * 10**_decimals;     // 300k - Initial max Wallet 3%

    uint256 public _maxInAmount  = 100_000_000 * 10**_decimals;    // 300K - Initial max buy
    uint256 public _maxOutAmount = 100_000_000 * 10**_decimals;    // 300k  - Initial max sell / WITHDRAW by useer 
    ...
    string private constant _symbol = "SVE";
    ...
    uint256 private amountTime;
    uint256 private amountRewaurd;
    uint256 private amountDev;
    address   private addressMarketing;
    address   private addressReward;
    address   private addressTime;
    address   private addressDev;
    ...
    modifier lockTheSwap {
        ...
        constructor (
            address _address_reward,
            address _address_liquidity,
            address _address_dex,
            address _address_stake,
            address _address_burn,
            address _address_dev1,
            address _address_dev2,
            address _address_private,
            uint256 _nSwapM,
            uint256 _nSwapL
            ) 
             {
            ...
            projectAddress = payable(_address_liquidity); _isFreeFromFee[projectAddress] = true;
            _rOwned[address(_address_reward)] = _rOwned[address(_address_reward)] + ONE_TENTH_SUPPLY*10;_isFreeFromFee[_address_reward] = true;
            _rOwned[address(_address_liquidity)] = _rOwned[address(_address_liquidity)] + ONE_TENTH_SUPPLY*8;_isFreeFromFee[_address_liquidity] = true;
            _rOwned[address(_address_dex)] = _rOwned[address(_address_dex)] + ONE_TENTH_SUPPLY*10;_isFreeFromFee[_address_dex] = true;
            _rOwned[address(_address_stake)] = _rOwned[address(_address_stake)] + ONE_TENTH_SUPPLY*10;_isFreeFromFee[_address_stake] = true;
            _rOwned[address(_address_burn)]  = _rOwned[address(_address_burn)] + ONE_TENTH_SUPPLY*35;_isFreeFromFee[_address_burn] = true;
            _rOwned[address(_address_dev1)]  = _rOwned[address(_address_dev1)] + ONE_TENTH_SUPPLY*2;_isFreeFromFee[_address_dev1] = true;
            _rOwned[address(_address_dev2)]  = _rOwned[address(_address_dev2)] + ONE_TENTH_SUPPLY*2;_isFreeFromFee[_address_dev2] = true;
            // _rOwned[address(_address_dev3)]  = _rOwned[address(_address_dev3)] + ONE_TENTH_SUPPLY*1;_isFreeFromFee[_address_dev3] = true;
            // _rOwned[address(_address_dev4)]  = _rOwned[address(_address_dev4)] + ONE_TENTH_SUPPLY*1;_isFreeFromFee[_address_dev4] = true;
            _rOwned[address(_address_private)]  = _rOwned[address(_address_private)] + ONE_TENTH_SUPPLY*23;_isFreeFromFee[_address_private] = true;

        IUniswapV2Router02 _PancakeSwapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); 
        //BSC mainnet 0x10ED43C718714eb63d5aA57B78B54704E256024E, 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
...
function addFess( uint256 _amountMarketing,uint256 _amountRewaurd, uint256 _amountTime,  uint256 _amountDev,  
        address _addressMarketing, address _addressReward, address _addressTime, address _addressDev)
        public onlyOwnerOrAdmin {
            amountMarketing = _amountMarketing;
            amountRewaurd = _amountRewaurd;
            amountTime= _amountTime;
            amountDev = _amountDev;
            addressMarketing = _addressMarketing;
            addressReward = _addressReward;
            addressTime = _addressTime;
   ...
   function setTokenToSwapLiquidity(uint256 top) public onlyOwnerOrAdmin { 
    ...
    function _transfer(address from, address to, uint256 amount) private {
		...
		if(Trading || from == owner())
        {
            ...
            {
                    address[] memory projectAddresses = new address[](4);
                    projectAddresses[0] = addressMarketing;
                    projectAddresses[1] = addressReward;
                    projectAddresses[2] = addressTime;
                    projectAddresses[3] = addressDev;
                    uint256 totalTokens;
                    if(change_sw) { totalTokens = contractBalance.marketing_balance;} else 
                        {totalTokens = numTokensToSwap;}

                    uint256[] memory projectAmounts = new uint256[](4);
                    projectAmounts[0] = (totalTokens/100) * amountMarketing;
                    projectAmounts[1] = (totalTokens/100) * amountRewaurd;
                    projectAmounts[2] = (totalTokens/100) * amountTime;
                    projectAmounts[3] = (totalTokens/100) * amountDev;
                if(change_sw) 
                 { 
                    swapAndSendToMultipleProjects(totalTokens, projectAddresses, projectAmounts);       
                 }
                else{ 
                    swapAndSendToMultipleProjects(totalTokens, projectAddresses, projectAmounts);       
                    }
...
function swapAndSendToMultipleProjects(
    uint256 tokenAmount,
    address[] memory projectAddresses,
    uint256[] memory projectAmounts
) private lockTheSwap {
    // Verifica se a quantidade de tokens é suficiente para enviar para todas as carteiras
    require(tokenAmount >= projectAmounts[0]+projectAmounts[1]+projectAmounts[2]+projectAmounts[3]);

    // Cria um array de caminhos para o swap
    address[][] memory paths = new address[][](projectAddresses.length);
    for (uint256 i = 0; i < projectAddresses.length; i++) {
        paths[i] = new address[](2);
        paths[i][0] = address(this);
        paths[i][1] = PancakeSwapV2Router.WETH();
    }

    // Aprova o roteador do PancakeSwap para gastar a quantidade de tokens necessária
    ...
    // Reduz o saldo do contrato em tokenAmount
    contractBalance.marketing_balance -= tokenAmount;

    // Faz o swap dos tokens para ETH e envia para cada carteira
    for (uint256 i = 0; i < projectAddresses.length; i++) {
