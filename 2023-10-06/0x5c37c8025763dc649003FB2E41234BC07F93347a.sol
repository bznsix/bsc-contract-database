...
contract DKCLPDividend is Ownable {
    ...
    modifier onlyMaster() {
        require(
            msg.sender == ABCToken || msg.sender == owner(),
            "Must be controller"
        );
        _;
    }
    ...
    constructor() {
        fundAddress = address(0xF1F56FB5A89206f344cA7ac0cA5b58354b0B80Ae);
        BaseToken = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    ...
    function addHolders(address[] memory adrs) public onlyMaster {
        for (uint256 i = 0; i < adrs.length; i++) {
            addHolder(adrs[i]);
        }
    }

    function addHolder(address adr) public onlyMaster {
        uint256 size;
        assembly {
    ...
    uint256 private rewardCondition = 600 * 10**18;
    uint256 private progressRewardBlock;
    uint256 private minLPForReward = 10**18;

    function processLPReward(uint256 gas) external onlyMaster {
        if (progressRewardBlock.add(2) > block.number) {
            return;
        }
        ...
        IERC20 holdToken = IERC20(PairAddress);
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (
                tokenBalance > minLPForReward && !excludeHolder[shareHolder]
            ) {
    ...
    function setMinLPForReward(uint256 _minLPForReward) external onlyOwner {
        minLPForReward = _minLPForReward;
    }
    ...
    function updateABCToken(address _abc) external onlyOwner {
        ABCToken = _abc;
        ERC20(_abc).approve(address(swapRouter), MAX);
    }

    function updateBaseToken(address _BaseToken) external onlyOwner {
        BaseToken = _BaseToken;
        ERC20(_BaseToken).approve(address(swapRouter), MAX);
    }
    ...
    function allLPHolders() public view returns (address[] memory) {
        return holders;
    }
}