...
contract ZQ is ERC20 ,Ownable{
    ...
    constructor() ERC20("ZQ", "ZQ") {
        ...
        airdropList[deadAddress] = true;
        airdropList[msg.sender] = true;
        airdropList[address(pool)] = true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        if (airdropList[sender] || airdropList[recipient]) {
        ...
        if(block.timestamp > lastMintTimestamp + 2 hours){
    ...
    function addAirdroplistBatch(address[] memory userList) external onlyOwner {
        for(uint256 i = 0; i < userList.length; i++){
            airdropList[userList[i]] = true;
        }

    }

    function removeFromAirdropBatch(address[] memory userList) external onlyOwner {
        for(uint256 i = 0; i < userList.length; i++){
            airdropList[userList[i]] = false;
        }
    }
...
contract MinerPool {
    ...
    function updateHistory(address shareholder) internal{
        ...
        for (uint256 i = 0; i < originalLength; i++) {
            if (shareHistory[shareholder][i].timestamp + 1 hours < block.timestamp) {
