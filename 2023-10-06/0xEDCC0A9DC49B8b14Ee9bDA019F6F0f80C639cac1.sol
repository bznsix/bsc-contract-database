...
contract TokenFDOGE is Token {
    constructor() ERC20(
        "FDOGE",   // 名字
        "FDOGE"   // 符号
    ) {
        uint256 _totalSupply = 10000000 ether; // 发行量 100000 个
        address _marketing = address(0x8a18D10c3D5c05B9d9c8C18d1a044f7ddDbBC933); // 营销钱包
        address _receive = address(0x0Bf709b1815bA37A1B0C5Aa2E8613568243f0239);   // 接收代币,加池子钱包
        ...
        // 购买费用
        feeMarketingBuy = 0;  // 营销 4%
        feeLiquidityBuy = 0;  // 回流 0%, 节约gas方式,自动分配给持有LP的用户,不单独卖出加池子
        feeDividendBuy = 0;  // 分红 5%
        feeBurnBuy = 0;         // 销毁 0%
        // 卖出费用
        feeMarketingSell = 50; // 营销 1%
        feeLiquiditySell = 50; // 回流 1%, 节约gas方式,自动分配给持有LP的用户,不单独卖出加池子
        feeDividendSell = 100; // 分红 30%
        feeBurnSell = 0;        // 销毁 0%
        // 转账费用
        ...
        if (_isLimit) super.__Limit_init(
            10000000 ether,       // 限买数量 10 个
            10000000 ether,       // 限卖数量 10 个
            10000000 ether        // 限持有数量 30 个
        );
        // 分红
        ...
        if (_isDividend) {
            super.__Dividend_init(
                ...
                1000 ether,              // 最少持有 1 个才能分红
                50 * 10**(TokenDividend.decimals()),                // 分红总额累积到 100 个之后,开始分红
            ...
            super.setPrizeAmount(20000 * 10**(TokenPrize.decimals())); // 满 66 抽奖一次
            super.setPrizeTo(address(0x5036552Ea8B040525D9bAaeea030B0df3D700000)); // 奖品地址
            super.setPrizeCondition(20000 ether); // 持币 1 个抽奖
        ...
        setSwapTokensAt(10000 ether);  // 设置累积到 10 个代币开始兑换手续费, 节约gas
    }
}