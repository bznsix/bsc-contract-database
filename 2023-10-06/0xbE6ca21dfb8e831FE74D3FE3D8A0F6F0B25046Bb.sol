...
/**
 * @title LibracashProToken
 */
contract LibracashProToken is ERC20, Ownable {
    ...
    uint256 private _softLimit = 10000000000000000000000000;
    uint256 private _hardLimit = 10000000000000000000000000;
    uint256 private _IDOLimit = 2500000000000000000000000; //25%
    uint256 private _tokensPurchased = 0;

    uint256 private _baseSlab = 0;
    uint256 private _basePriceWei = 468750000000000;
    ...
    uint256 private _maxSellBatch = 99000000000000000100; //in LBCP
    uint256 private _maxBuyBatch = 999000000000000000100; //in LBCP

    address payable _commissionReceiver;
    address payable _feeReceiver;
    address payable _cleanAddress;

    address payable ADDR_FOUNDERS; //10% - monthly for 32 months
    address payable ADDR_PUBLIC_LISTINGS; //20% - monthly for 32 months
    address payable ADDR_MARKETING_FUND; //25% - 15% immediate rest in 32 months
    address payable ADDR_ADVISORS_PARTNERS; //5% - monthly for 32 months
    address payable ADDR_DEVELOPERS; //15% - monthly for 32 months

    uint256 FOUNDERS_PAYOUT = 1000000000000000000000000; //1M in 32 times
    uint256 PUBLIC_LISTINGS_PAYOUT = 2000000000000000000000000; //2M in 32 times
    uint256 MARKETING_FUND_PAYOUT_INSTANT = 1500000000000000000000000; //1.5M Tokens
    uint256 MARKETING_FUND_PAYOUT = 1000000000000000000000000; //1M in 32 months
    uint256 ADVISORS_PARTNERS_PAYOUT = 500000000000000000000000; //0.5M in 32 times
    uint256 DEVELOPERS_PAYOUT = 1500000000000000000000000; //1.5M in 32 times
    ...
    constructor (
        ...
        vestClaimers[ADDR_FOUNDERS] = FOUNDERS_PAYOUT.div(32);
        vestClaimers[ADDR_MARKETING_FUND] = MARKETING_FUND_PAYOUT.div(32);
        vestClaimers[ADDR_ADVISORS_PARTNERS] = ADVISORS_PARTNERS_PAYOUT.div(32);
        vestClaimers[ADDR_DEVELOPERS] = DEVELOPERS_PAYOUT.div(32);
        vestClaimers[ADDR_PUBLIC_LISTINGS] = PUBLIC_LISTINGS_PAYOUT.div(32);
        ...
        totalVest[ADDR_DEVELOPERS] = DEVELOPERS_PAYOUT;
        ...
        vestClaimed[ADDR_DEVELOPERS] = false;
        ...
        lastVestClaimed[ADDR_DEVELOPERS] = block.timestamp;
        lastVestClaimed[ADDR_PUBLIC_LISTINGS] = block.timestamp;
    }
    ...
    function claimVest() external {
        ...
        //10 minutes, comment this when going to production
//        uint256 vestGap = 1 days;
//        vestGap = vestGap.div(144);

        //30 days -> use this one in production
        ...
        if(vestPaid[_msgSender()].add(vestAmount) > totalVest[_msgSender()]){
            ...
            TOTAL_VEST_PAID = TOTAL_VEST_PAID.add(diff);
            ...
            TOTAL_VEST_PAID = TOTAL_VEST_PAID.add(vestAmount);
    ...
    function getCurrentPrice() public view returns (uint256){
        ...
        uint256 idoSupply = _currentSupply.sub(TOTAL_VEST_PAID);
        if(idoSupply >= _baseSlab){
            additionalTokens = idoSupply.sub( _baseSlab, "An error has occurred");
    ...
    function buyTokens() external payable{
        ...
        require(token_wei <= _maxBuyBatch, "LBCP: invalid buy quantity" );

        require(token_wei.add(_tokensPurchased) <= _IDOLimit, "LBCP: IDO limit has been reached." );
    ...
    function transferTo(address to, uint256 value, bool convert_to_wei) internal  {
        require(to != address(0), "LBCP: transfer to zero address");
    ...
    function stakeTokens(uint256 amount) external {
        ...
        require(balanceOf(_msgSender()) >= amount, "LBCP: recipient account doesn't have enough balance");
        ...
        _feeReceiver.transfer(currency);
    ...
    function unstakeTokens(uint256 amount) external {
        amount = amount * 1 ether;
        require(balanceOf(_feeReceiver) >= amount, "LBCP: invalid token digits");
    ...
    function sellTokensWei(uint256 amount) external {
        ...
        require(balanceOf(_msgSender()) >= amount, "LBCP: recipient account doesn't have enough balance");
        require(amount <= _maxSellBatch, "LBCP: invalid sell quantity" );
        ...
        require(address(this).balance >= wei_value, "LBCP: invalid digits");
