...
contract CoreFeesRountingContract is Context,Ownable,ReentrancyGuard{
    ...
    function setUserForNFT(uint256 tokenId, uint256 userId) public onlyOwner {
        nftUser[tokenId] = userId;
    }

    // Function to retrieve the user associated with an NFT token ID
    function getUserForNFT(uint256 tokenId) public view returns (uint256) {
    ...
    function withoutWalletMint(uint256 user_id,string memory order_id,OrderCarbonDetails memory order) external onlyOwner{
        uint256[] memory nftlist = CarbonCredit.withoutWalletMint(order_id,address(this),order.cpo_address, order.project_id, order.tonnes, order.no_of_nft, order.price);
        userWallet[user_id];
    ...
    mapping(uint256 => address) private userIdWallet;

    mapping(uint256 => uint256[]) private userNftList;

    
      // Public read function to get user's wallet address
    function getUserWalletAddress(uint256 userId) public view returns (address) {
        return userIdWallet[userId];
    }

    // Public read function to get user's NFT list
    function getUserNftList(uint256 userId) public view returns (uint256[] memory) {
        return userNftList[userId];
    }
    
    // Admin-only function to set user's wallet address
    function setUserWalletAddress(uint256 userId, address walletAddress) public onlyOwner {
        userIdWallet[userId] = walletAddress;
    }

    // Admin-only function to add NFT to user's list
    function addUserNft(uint256 userId, uint256 nftId) public onlyOwner {
        userNftList[userId].push(nftId);
    }
    


    function WalletMint(bool is_wallet,string memory order_id,OrderCarbonDetails memory order) private{
     
        if(is_wallet){
            CarbonCredit.ContractMint(order_id,order.buyer_address,order.cpo_address, order.project_id, order.tonnes, order.no_of_nft, order.price);
        }else{
            CarbonCredit.ContractMint(order_id,address(this),order.cpo_address, order.project_id, order.tonnes, order.no_of_nft, order.price);
        }
    }
...
contract BCCarbonCredit is ERC721, ERC721Burnable, Ownable {
   ...
   function ContractMint(string memory order_id,address to,address _cpo,uint project_id,uint tonnes,uint no_of_nft,uint price) public returns(uint) {
    ...
    function withoutWalletMint(string memory order_id,address to,address _cpo,uint project_id,uint tonnes,uint no_of_nft,uint price) public returns(uint256[] memory) {
    require(isWhitelisted(msg.sender), "Only whitelisted addresses can mint.");
    uint startTokenId = tokenIdCounter + 1;
