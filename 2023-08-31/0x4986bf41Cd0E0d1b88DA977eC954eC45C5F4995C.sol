...
contract SGBPoints is Ownable {
    ...
    function getUserDataByAddr(address _userAddr) external view returns (TokenBalance memory) {
    ...
    function setCredit(address _userAddr, uint256 _value) external {
        require(admin[msg.sender] || msg.sender == owner(), "Only admin or owner can execute.");
    ...
    function setReinvest(address _userAddr, uint256 _value) external {
        require(admin[msg.sender] || msg.sender == owner(), "Only admin or owner can execute.");
    ...
    function setTransfer(address _userAddr, uint256 _value) external {
        require(admin[msg.sender] || msg.sender == owner(), "Only admin or owner can execute.");
        tokenBalance[_userAddr].transfer = _value;
    }

    function transferToCredit(uint256 _exchangeAmount) external {
