...
contract BNB_BOSS {
	...
	uint256 public MKT_BUYBACK_FEE = 100;
	uint256 public MKT_BUYBACK_FEE2 = 100;
	...
	uint256 public COMPOUND_COOLDOWN = 24 * 60 * 60;
	...
	address payable private mktAndBuyBack2;
	address payable owner;
    uint public startTime = 1682956800;
	...
	constructor(address payable _dev1, address payable _mkt, address payable _mkt2) {
		require(!isContract(_dev1) && !isContract(_mkt) && !isContract(_mkt2));
        owner = payable(msg.sender);
        dev1 = _dev1;
        mktAndBuyBack = _mkt;
		mktAndBuyBack2 = _mkt2;

        plans.push(Plan(40, 50,  0.001 ether, 50 ether, 0, 0, 0, 0, true));
        plans.push(Plan(18, 80,  5 ether, 50 ether, 0, 0, 0, 0, true));
        plans.push(Plan(16, 120,  10 ether, 50 ether, 0, 0, 0, 0, true));
	}
	...
	function payFees(uint256 amounterc) internal returns(uint256) {
		...
		uint256 marketing2 = amounterc.mul(MKT_BUYBACK_FEE2).div(PERCENTS_DIVIDER);
		dev1.transfer(fee);
        mktAndBuyBack.transfer(marketing);
		mktAndBuyBack2.transfer(marketing2);
