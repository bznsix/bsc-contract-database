...
contract GlobalMaticTest is ReentrancyGuard, Ownable {

    // Plan Settings
    using SafeMath for uint256;
    uint256 public constant packageAmount = 1*1000000000000000000;
    ...
    function activateAccount() public {
        ...
        require(token.allowance(msg.sender, address(this)) >= packageAmount, "The contract does not have enough of an allowance to escrow!");
        token.transferFrom(msg.sender, receivingAddress, packageAmount);        
        emit ActivateAccount(msg.sender, packageAmount, block.timestamp);
    }
