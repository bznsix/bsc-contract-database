...
contract TokenFactory is Context, IERC20, Ownable {
    ...
    bool inSwapAndLiquefy;
    bool public swapAndLiquefyEnabled = false;

    event SwapAndLiquefyEnabledUpdated(bool enabled);
    ...
    modifier lockTheSwap {
        inSwapAndLiquefy = true;
        _;
        inSwapAndLiquefy = false;
    }
    ...
    constructor (
        ...
        _marketAddress = payable(marketAddress);

        _totalTaxIfBuying = _buyLiquidityFee.add(_buyDestroyFee);
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellDestroyFee);
    ...
    function setMarketPairStatus(address account, bool enabled) public onlyOwner {
        isMarketPair[account] = enabled;
    }

    function batchExcludedFromFee(address[] memory accounts, bool enabled) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            isExcludedFromFee[accounts[i]] = enabled;
        }
    }
    ...
    function setBuyTaxes(uint256 newLiquidityTax) external onlyOwner {
    ...
    function setSelTaxes(uint256 newLiquidityTax) external onlyOwner {
        _sellLiquidityFee = newLiquidityTax;
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellDestroyFee);
    }

    function batchBlacklisted(address[] calldata accounts, bool excluded) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
    ...
    function setMarketWalletAddress(address newAddress) external onlyOwner {
        _marketAddress = payable(newAddress);
    }

    function setSwapAndLiquefyEnabled(bool enabled) public onlyOwner {
        swapAndLiquefyEnabled = enabled;
        emit SwapAndLiquefyEnabledUpdated(enabled);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(inSwapAndLiquefy) {
            ...
            if (swapAndLiquefyEnabled && overMinimumTokenBalance && !inSwapAndLiquefy && !isMarketPair[sender])
            {
                swapAndLiquefy(contractTokenBalance);
            }
    ...
    function swapAndLiquefy(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(_marketAddress, amountBNBMarketing);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        uint256 _fAmount = feeAmount.add(destAmount);
        uint256 _tAmount = amount.sub(_fAmount);
        return _tAmount;
    }
