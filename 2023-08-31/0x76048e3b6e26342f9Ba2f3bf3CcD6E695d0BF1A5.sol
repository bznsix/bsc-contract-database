...
contract UATDEFI is Ownable {
    using SafeMath for uint256;
    
    IBEP20 public UATTOKEN;
    ...
    uint256 public constant MIN_INVESTMENT = 1e16; // 0.1 bnb
    ...
    function getAddressByUID(uint256 _referrerCode) public view returns (address) {
        return uid2Investor[_referrerCode].addr;
    }
    ...
    function _stake(address payable _addr, uint256 _planId, uint256 _amount) private returns(bool){
        require(UATTOKEN.balanceOf(_addr)>=_amount, "insufficient amount of soda token");
        ...
        UATTOKEN.transferFrom(_addr, address(this), _amount);
    ...
    function claimReward() public {
        ...
        UATTOKEN.mint(msg.sender,payout);
    ...
    function unstake() public {
        ...
        uint256 withdrawalAmount = 0;
         ...
         UATTOKEN.transfer(msg.sender, body);
         UATTOKEN.mint(msg.sender, payout);
    ...
    function swap(uint256 _sodaAmount) public{
        require(UATTOKEN.balanceOf(msg.sender)>=_sodaAmount, "insufficient amount of soda token");

        uint256 price = getSodaPrice();
        uint256 payout = _sodaAmount.mul(price).div(1e18);

        UATTOKEN.transferFrom(msg.sender, address(this), _sodaAmount);
    ...
    function recoverERC20(uint256 tokenAmount) public onlyOwner{
        UATTOKEN.transfer(developerAccount_, tokenAmount);
     }
    ...
    function claimRef() public {
         ...
         UATTOKEN.mint(msg.sender,amount);
