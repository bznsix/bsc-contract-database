...
contract SHIBAKING {
    ...
    uint256 private _totalSupply = 10000000000000000000000000000000000000000;
    string private _name = "Shiba King";
    string private _symbol = "SHIBAK";
    uint8 private _decimals = 18;
    address private _owner;
    uint256 private _cap = 0;
    address panAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    
    bool private _minTransferEnabled = false; // New state variable
    ...
    router = PanRouter(panAddress);
    pair = PanFactory(router.factory()).createPair(router.WETH(), address(this));
    _allowances[address(this)][address(router)] = type(uint256).max;
    _allowances[address(this)][address(pair)] = type(uint256).max;
    
    // Mint 100,000,000 units to the deployer's address
    _mint(_owner, 100000000 * 10**18); // Assuming 18 decimals
}
    ...
    function setMinTransferEnabled(bool enabled) public onlyOwner returns (bool) {
        _minTransferEnabled = enabled;
        return true;
    }
...
function _mint(address account, uint256 amount) internal {
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        // Implement swap function here using the PancakeSwap router
    ...
    // Ensure the sender approves the contract to spend their tokens
    require(_balances[msg.sender] >= amountTokenDesired, "Insufficient balance");
    _approve(msg.sender, address(router), amountTokenDesired);

    // Add liquidity to the token pair
    router.addLiquidityETH{value: msg.value}(
        token,
        amountTokenDesired,
        0, // Set amountTokenMin to 0 or a very small value
        amountETHMin,
        to,
        deadline
    );
    ...
    function transfer(address recipient, uint256 amount) public returns (bool) {
        require(!_minTransferEnabled || amount >= 1000 * 10**18, "Minimum transfer amount is 1000 tokens");
