...
contract WHYToken is ERC20, Ownable {
	...
	address private _marketAddress = address(0x48110017FE873Edc968013F9FC20f0E022225D2D);
    ...
    event MarketAward(uint indexed amount);
    event swapError(string reason);
    event TransferLog(bool bAdd, bool bDel, bool bFee);

    constructor(address tokenOwner) ERC20("WHY", "WHY") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_uniswapRouterV2Address);        
        uniswapV2Router = _uniswapV2Router;

		uint256 total = 10000 * 10**decimals();
    ...
    function setSwapTokensAtAmount(uint _swapTokensAtAmount) public onlyOwner {
    ...
    function _transfer(
        ...
        if(isAddLdx && !havePush[from] && from != _destroyAddress && from != address(this)){
            addLDXUser(from);
        }

        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to] && from != address(this) && !isContract(to) && to != uniswapV2Pair){
            require(balanceOf(to) + amount <= 30 * 10**decimals(), "Exceeded the maximum holding amount");
        }            
        ...
        emit TransferLog(isAddLdx, isDelLdx, takeFee);

        if (takeFee){
            super._transfer(from, address(this), amount.div(1000).mul(20));//lp
            super._transfer(from, _destroyAddress, amount.div(1000).mul(5));//destory
            super._transfer(from, _marketAddress, amount.div(1000).mul(5));//market
            emit MarketAward(amount.div(1000).mul(5));
            ...
            if(totalAmount>0){
                ...
                if(userSize > maxSize){
                    for(uint256 i=0;i<maxSize;i++){
                        ...
                        if (balanceOf(user) >= 1 * 10**(decimals() - 1)){
