...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x63aaC96f50bd0215FF210383B8167cfF891Fabe4;
...
contract BurnaTest is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress = 0x3c83AD59EE5aa82a03B358cBf8D9E889cb1693d2;
    address private _teamFeeAddress = 0x0C8f8e03CaB51CbAcb5F0F156650A98757f61A28;
    address private _buyBurnParentsAddress = 0x82f8253f86D20ad7c4ece21e5869aFe2CA319263;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1000000000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "Burnatest";
    string private _symbol = "BTEST";
    ...
    uint256 public _buyBurnParentsFee = 20;
    uint256 private _previousBuyBurnParentsFee = _buyBurnParentsFee;
    uint256 public _burnFee = 20;
    uint256 private _previousBurnFee = _burnFee;
    uint256 public _maxWalletBalance = 1000000000000 * 10**18;
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        return tokenFromReflection(_rOwned[account]) < _maxWalletBalance ? tokenFromReflection(_rOwned[account]) : _maxWalletBalance;
    }
    ...
    function setMaxWalletBalance(uint256 maxWalletBalance) external onlyOwner() {
        _maxWalletBalance = maxWalletBalance;
    }
    ...
    function setBuyBurnParentsFeePercent(uint256 buyBurnParentsFee) external onlyOwner() {
        _buyBurnParentsFee = buyBurnParentsFee;
    }
    ...
    function calculateBuyBurnParentsFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_buyBurnParentsFee).div(10**3);
    }
    ...
    function _transfer(
        ...
        // Check if addresses are excluded from max wallet balance constraint
        if (to != address(uniswapV2Pair) && !_isExcludedFromFee[to] && !_isExcludedFromMaxWallet[to]) {
            require(balanceOf(to).add(amount) <= _maxWalletBalance, "Recipient's wallet balance would exceed the maximum limit");
        }

        // Check if addresses are excluded from max transaction amount constraint
        ...
        // Calculate fees and perform the actual token transfer
        uint256 appliedFees = 0; // Renamed totalFees to appliedFees
        if (takeFee) {
            appliedFees = calculateTaxFee(amount).add(calculateDevelopmentFee(amount)).add(calculateLiquidityFee(amount));
        }

        // Calculate the burn amount (2% of the transferred amount)
        uint256 burnAmount = amount.mul(2).div(100);

        // Subtract the burn amount from the transfer amount
        uint256 transferAmount = amount.sub(appliedFees).sub(burnAmount);

        _tokenTransfer(from, to, transferAmount, takeFee);
    ...
    function _burnTokens(uint256 amount) private {
        ...
        require(amount <= _tTotal, "Amount to burn exceeds total supply");
...
// added function setMaxWalletBalance
// function excludeFromMaxWallet
// function includeInMaxWallet