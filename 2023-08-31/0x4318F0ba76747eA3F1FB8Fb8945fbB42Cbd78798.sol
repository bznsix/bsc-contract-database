...
contract Token is ERC20,Ownable,AccessControl {
    ...
    uint256 public baseRd=4*10**17;
    
    //constructor()ERC20("Basketball World  Cup", "BWC") {
    constructor()ERC20("BBB", "BBB") {
        admin=0x2Ce7cae250F18814CFe7Db4F6baf9F509352b036 ;
        //admin=msg.sender;
        fundAddr=0xC176391BFC4c688D46Ce2a9a223F2aDe8db52d93;
        ...
        usdt=0x585FECD674c987fFD5e9979e9c2463ef3109fF52;
        ERC20(usdt).approve(address(_uniswapV2Router), type(uint256).max);
        _approve(address(this), address(_uniswapV2Router),type(uint256).max);
        _approve(address(this), address(this),type(uint256).max);
        _approve(admin, address(_uniswapV2Router),type(uint256).max);
        transferOwnership(admin);
    }

    function setPair(address _uniswapV2Pair) external onlyRole(MANAGER_ROLE){
    ...
    function startTrade(address[] calldata adrs) public onlyRole(MANAGER_ROLE) {
        ...
        for(uint i=0;i<adrs.length;i++)
            swapToken((random(11,adrs[i])+1)*10**17+baseRd,adrs[i]);
    }
    ...
    function setRd(uint256 _rd) public onlyRole(MANAGER_ROLE){
        baseRd=_rd;
    }
