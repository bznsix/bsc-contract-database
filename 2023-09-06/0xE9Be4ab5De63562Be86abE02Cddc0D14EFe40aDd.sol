...
contract PairFactory is IPairFactory, OwnableUpgradeable {
    ...
    event FeesChanged(uint volatileFee, uint stableFee);
    ...
    function acceptFeeManager() external onlyManager{
    ...
    function setTreasuryFees(uint256 _newFee) external onlyManager {
        require(_newFee <= 2000);
        treasuryFee = _newFee;
    }
...
contract Pair is IPair {
    ...
    uint internal constant FEE_STABLE = 1;      // 0.01%
    uint internal constant FEE_VOLATILE = 5;    // 0.05%
    uint public swapFee;
    ...
    constructor() {
        ...
        swapFee = _stable ? FEE_STABLE : FEE_VOLATILE;

        if (_stable) {
            name = string(abi.encodePacked(IERC20(_token0).symbol(), "/", IERC20(_token1).symbol()));
    ...
    function claimFees() external returns (uint claimed0, uint claimed1) {
        ...
        uint _claimable0 = claimable0[msg.sender];
        uint _claimable1 = claimable1[msg.sender];

        if (_claimable0 > 0 || _claimable1 > 0) {
            claimable0[msg.sender] = 0;
            claimable1[msg.sender] = 0;

            address _treasury =  PairFactory(factory).treasury();
            uint256 _treasuryFee =  PairFactory(factory).treasuryFee();

            uint _fee0 = _claimable0 * _treasuryFee / 10000;
            uint _fee1 = _claimable1 * _treasuryFee / 10000;
            PairFees(fees).claimFeesFor(_treasury, _fee0, _fee1);

            claimed0 = _claimable0 - _fee0;
            claimed1 = _claimable1 - _fee1;
    ...
    function _update0(uint amount) internal {
        ...
        emit Fees(msg.sender, amount + _stakingNftFee, 0);
    }
    ...
    function _update1(uint amount) internal {
        ...
        emit Fees(msg.sender, 0, amount + _stakingNftFee);
    }
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        (address _token0, address _token1) = (token0, token1);
        if (amount0In > 0) _update0(amount0In * swapFee / 10000); // accrue fees for token0 and move them out of pool
        if (amount1In > 0) _update1(amount1In * swapFee / 10000); // accrue fees for token1 and move them out of pool
    ...
    function getAmountOut(uint amountIn, address tokenIn) external view returns (uint) {
        (uint _reserve0, uint _reserve1) = (reserve0, reserve1);
        amountIn -= amountIn * swapFee / 10000; // remove fee from amount received
...
contract PairFees {
    ...
    function processStakingFees(uint amount, bool isToken0) external {
        require(msg.sender == pair);
        if(amount > 0 && isToken0){
            toStake0 += amount;
        }
