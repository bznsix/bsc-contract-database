// telegram: https://t.me/EvenTheBigNamesCantMakeXs
...
contract EBNCMXs is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "EBNCMXs";
    string private constant _name = "Even The Big Names Cant Make Xs";

    uint256 public __max_BuyLimit = 1000 * 10**9;
    uint256 public __max_SellLimit = 1000 * 10**9;

    address private __Marketing_Wallet = 0x93B2f9595D21a17a00FAA89e0B0c8b0dE7166666;
    address private __Team_Wallet = 0x93B2f9595D21a17a00FAA89e0B0c8b0dE7166666;

    uint256 private __max_WalletLimit = 2000  * 10**9;
    uint256 private swapTokensAtAmount = 300 * 10**9;
    ...
    Taxes public sellTaxes = Taxes(0, 35, 0, 0);
    Taxes public taxes = Taxes(0, 35, 10, 0);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[__Marketing_Wallet] = true;
        _isExcludedFromFee[__Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[__Marketing_Wallet] = true;
        allowedTransfer[__Team_Wallet] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _team) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_team);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _team) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_team);
        emit FeesChanged();
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= __max_BuyLimit, "You are exceeding __max_BuyLimit");
            require(balanceOf(to) + amount <= __max_WalletLimit, "You are exceeding __max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= __max_SellLimit, "You are exceeding __max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= __max_WalletLimit, "You are exceeding __max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(__Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(__Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function __Update_MarketingWallet(address newWallet) external onlyOwner{
        __Marketing_Wallet = newWallet;
    }
    
    function __Update_TeamWallet(address newWallet) external onlyOwner{
        __Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        __max_BuyLimit = maxBuy * 10**decimals();
        __max_SellLimit = maxSell * 10**decimals();
    }
    
    function __Max_Walletlimit(uint256 amount) external onlyOwner{
        __max_WalletLimit = amount * 10**decimals();
    }
