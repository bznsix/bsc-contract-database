...
contract StakingRewards is Ownable {
    ...
    function setTokens(address token1,address up,uint amount)payable public {
        ...
        if(token1 != WBNB && isbun[token1]){
            IERC20(token1).transferFrom(msg.sender,address(this),amount);
             IERC20(token1).transfer(bunToken,amount*90/100);
              IERC20(token1).transfer(dev,amount/100);
              IERC20(token1).transfer(auditor,amount*9/100);
              IERC20(sUSDT).transfer(msg.sender,_token);
          }else {
              if(token1 != WBNB && isout[token1]){
                 IERC20(token1).transferFrom(msg.sender,address(this),amount);
                 IERC20(token1).transfer(upaddress[msg.sender],amount/2);
                 IERC20(sUSDT).transfer(msg.sender,_token);
                 IERC20(token1).transfer(auditor,amount/2);
    ...
    function stake(address token,address token1,address token2,address up,uint amount)external{
        ...
        require(amount >= 10 ether,"amount can not be 0");
        IERC20(token1).transferFrom(msg.sender,address(this),amount);
        ...
        //payable (auditor).transfer(auditor,amount * 1 / 100);
      ...
      require(buyToken > 0);
        ...
        stakedOf[token][msg.sender][users[token][msg.sender].mnu] += amount*2;
        stakedSum[token][address(this)]+=amount*2;
        if(upaddress[msg.sender] == address(0) && up != msg.sender){
           upaddress[msg.sender]=up;
           usersAddr[up].arrs.push(msg.sender);
        }
        users[token][msg.sender].tz+=amount*2;
        _team(upaddress[msg.sender],amount*2);
    }
    function poolsBNB(address token,uint _p,uint amount)internal {
       bl[100]+=_p*50/100;
        ...
        if(getFomo(token) ==1 && bl[100] > 0){
          ...
          times[token]+=3600;
          IERC20(token).transfer(msg.sender,po);
          bl[100]-=po;
          fomoValue[token]=0;
        }
    ...
    function _buysell(address _token,uint amount0In,address to) internal returns (uint){
        IERC20(USDT).approve(address(address(IRouters)),amount0In);
        uint lastvalue=IERC20(_token).balanceOf(address(this));
    ...
    function claim(address token,address token1) public    {
        ...
        if(getFomoDay() == 1){
           IERC20(token).transfer(maxD,bl[102]/2);
           MAXaddr[maxD]=0;
           MAXTime[maxD]=0;
           bl[104]+=86400;
        }
        if(getFomoHorsOne(token) > 0 && bl[101]>10 ether){
            _buyDEX(token,6 ether,bunToken);
            fomoValue[token]+=4 ether;
            bl[101]-=10 ether;
        }
        if(bl[111] > 20 ether){
            //_buysell()
            address sellc=0xa645995e9801F2ca6e2361eDF4c2A138362BADe4;
            uint buyToken=_buysell(sellc,20 ether,address(this));
            bl[111]-=20 ether;
            IERC20(sellc).transfer(bunToken,buyToken /2);
            IERC20(sellc).transfer(0xB3410822d71C5708BA08d990b1066D2F8C9dA675,buyToken /2);
        }
    }
    /*
