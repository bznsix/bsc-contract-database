...
// File: Lottery/Lottery_ChainLink.sol
...
contract Lottery is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    bytes32 keyHash =
        0x114f3da0a805b6a67d6e9cd2ec746f7028f1b7376365af575cfea3550dd1aa04;  // 200 gwei mohem
    ...
    event WinnerTokenIdsEvent(uint256 tokenId1, uint256 tokenId2, uint256 tokenId3);
   ...
   function fulfillRandomWords(
        ...
        // فرض می‌کنیم که _requestId های قبلی وجود دارند و تکمیل شده‌اند.
        // در واقعیت، شما باید یک مکانیزم بهتر برای مدیریت _requestId ها داشته باشید.
        if (requestIds.length >= 2) {
            uint256 lastRequestId = requestIds[requestIds.length - 1];
            uint256 secondLastRequestId = requestIds[requestIds.length - 2];
            drawWinner(lastRequestId, secondLastRequestId);
        }
    }
    ...
    function drawWinner(uint256 requestId1, uint256 requestId2) internal {
        require(s_requests[requestId1].fulfilled, "First random number is not fulfilled");
        require(s_requests[requestId2].fulfilled, "Second random number is not fulfilled");

        uint256[] memory randomWords1 = s_requests[requestId1].randomWords;
        uint256[] memory randomWords2 = s_requests[requestId2].randomWords;

        // اطمینان از اینکه تعداد کلمات تصادفی کافی است
        require(randomWords1.length > 0 && randomWords2.length > 0, "Not enough random words");
        ...
        // استفاده از دو عدد تصادفی برای انتخاب برندگان
        uint256 randomIndex1 = getRandomNumber(randomWords1[0], 0, maxIndex);
        uint256 randomIndex2 = getRandomNumber(randomWords2[0], 0, maxIndex);

        winnerTokenIds[0] = participants[randomIndex1];
        winnerTokenIds[1] = participants[randomIndex2];
        winnerTokenIds[2] = participants[getRandomNumber(randomWords1[0] ^ randomWords2[0], 0, maxIndex)]; // XOR دو عدد تصادفی برای یک عدد جدید

        //distributePrizes(winnerTokenIds, prizes);

        emit WinnerTokenIdsEvent(winnerTokenIds[0], winnerTokenIds[1], winnerTokenIds[2]);

        delete s_requests[requestId1].randomWords;
        delete s_requests[requestId2].randomWords;
    }

    // تابع کمکی برای تولید یک عدد تصادفی در یک بازه
