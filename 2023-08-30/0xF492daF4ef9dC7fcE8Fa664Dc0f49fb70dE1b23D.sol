...
import '../interfaces/callback/IOwlswapMintCallback.sol';
import '../interfaces/callback/IOwlswapSwapCallback.sol';
import '../interfaces/callback/IOwlswapFlashCallback.sol';

import '../interfaces/IOwlswapPool.sol';

contract TestOwlswapCallee is IOwlswapMintCallback, IOwlswapSwapCallback, IOwlswapFlashCallback {
    ...
    function swapExact0For1(
        ...
        IOwlswapPool(pool).swap(recipient, true, amount0In.toInt256(), sqrtPriceLimitX96, abi.encode(msg.sender));
    }
    ...
    function swap0ForExact1(
        ...
        IOwlswapPool(pool).swap(recipient, true, -amount1Out.toInt256(), sqrtPriceLimitX96, abi.encode(msg.sender));
    }
    ...
    function swapExact1For0(
        ...
        IOwlswapPool(pool).swap(recipient, false, amount1In.toInt256(), sqrtPriceLimitX96, abi.encode(msg.sender));
    }
    ...
    function swap1ForExact0(
        ...
        IOwlswapPool(pool).swap(recipient, false, -amount0Out.toInt256(), sqrtPriceLimitX96, abi.encode(msg.sender));
    }
    ...
    function swapToLowerSqrtPrice(
        ...
        IOwlswapPool(pool).swap(recipient, true, type(int256).max, sqrtPriceX96, abi.encode(msg.sender));
    }
    ...
    function swapToHigherSqrtPrice(
        ...
        IOwlswapPool(pool).swap(recipient, false, type(int256).max, sqrtPriceX96, abi.encode(msg.sender));
    }
    ...
    function uniswapV3SwapCallback(
        ...
        if (amount0Delta > 0) {
            IERC20Minimal(IOwlswapPool(msg.sender).token0()).transferFrom(sender, msg.sender, uint256(amount0Delta));
        } else if (amount1Delta > 0) {
            IERC20Minimal(IOwlswapPool(msg.sender).token1()).transferFrom(sender, msg.sender, uint256(amount1Delta));
        } else {
    ...
    function mint(
        ...
        IOwlswapPool(pool).mint(recipient, tickLower, tickUpper, amount, abi.encode(msg.sender));
    }
    ...
    function uniswapV3MintCallback(
        ...
        if (amount0Owed > 0)
            IERC20Minimal(IOwlswapPool(msg.sender).token0()).transferFrom(sender, msg.sender, amount0Owed);
        if (amount1Owed > 0)
            IERC20Minimal(IOwlswapPool(msg.sender).token1()).transferFrom(sender, msg.sender, amount1Owed);
    }
    ...
    function flash(
        ...
        IOwlswapPool(pool).flash(recipient, amount0, amount1, abi.encode(msg.sender, pay0, pay1));
    }
    ...
    function uniswapV3FlashCallback(
        ...
        if (pay0 > 0) IERC20Minimal(IOwlswapPool(msg.sender).token0()).transferFrom(sender, msg.sender, pay0);
        if (pay1 > 0) IERC20Minimal(IOwlswapPool(msg.sender).token1()).transferFrom(sender, msg.sender, pay1);
    }
}
// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;
...
/// @notice Contains a subset of the full ERC20 interface that is used in Owlswap V1
...
/// @title Callback for IOwlswapPoolActions#mint
/// @notice Any contract that calls IOwlswapPoolActions#mint must implement this interface
interface IOwlswapMintCallback {
    /// @notice Called to `msg.sender` after minting liquidity to a position from IOwlswapPool#mint.
    /// @dev In the implementation you must pay the pool tokens owed for the minted liquidity.
    /// The caller of this method must be checked to be a OwlswapPool deployed by the canonical OwlswapFactory.
    /// @param amount0Owed The amount of token0 due to the pool for the minted liquidity
    /// @param amount1Owed The amount of token1 due to the pool for the minted liquidity
    /// @param data Any data passed through by the caller via the IOwlswapPoolActions#mint call
    function uniswapV3MintCallback(
        uint256 amount0Owed,
        uint256 amount1Owed,
        bytes calldata data
...
/// @title Callback for IOwlswapPoolActions#swap
/// @notice Any contract that calls IOwlswapPoolActions#swap must implement this interface
interface IOwlswapSwapCallback {
    /// @notice Called to `msg.sender` after executing a swap via IOwlswapPool#swap.
    ...
    /// @param data Any data passed through by the caller via the IOwlswapPoolActions#swap call
    function uniswapV3SwapCallback(
        int256 amount0Delta,
        int256 amount1Delta,
        bytes calldata data
...
interface IOwlswapPoolImmutables {
    /// @notice The contract that deployed the pool, which must adhere to the IOwlswapFactory interface
...
interface IOwlswapPoolDerivedState {
...
interface IOwlswapPoolActions {
    ...
    /// @dev The caller of this method receives a callback in the form of IOwlswapMintCallback#uniswapV3MintCallback
    ...
    /// @dev The caller of this method receives a callback in the form of IOwlswapSwapCallback#uniswapV3SwapCallback
    ...
    /// @dev The caller of this method receives a callback in the form of IOwlswapFlashCallback#uniswapV3FlashCallback
...
interface IOwlswapPoolOwnerActions {
...
contract TestOwlswapSwapPay is IOwlswapSwapCallback {
    function swap(
        address pool,
        address recipient,
        bool zeroForOne,
    ...
    function uniswapV3SwapCallback(
        ...
        if (pay0 > 0) {
            IERC20Minimal(IOwlswapPool(msg.sender).token0()).transferFrom(sender, msg.sender, uint256(pay0));
        } else if (pay1 > 0) {
            IERC20Minimal(IOwlswapPool(msg.sender).token1()).transferFrom(sender, msg.sender, uint256(pay1));
        }
    }
}
// SPDX-License-Identifier: UNLICENSED
...
contract TestOwlswapRouter is IOwlswapSwapCallback {
    using SafeCast for uint256;
    ...
    function uniswapV3SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? IOwlswapPool(msg.sender).token0() : IOwlswapPool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IOwlswapPool(pools[0]).token1();
            IOwlswapPool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IOwlswapPool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IOwlswapPool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
...
contract OwlswapPoolSwapTest is IOwlswapSwapCallback {
    ...
    {
        (amount0Delta, amount1Delta) = IOwlswapPool(pool).swap(
            address(0),
            zeroForOne,
            amountSpecified,
            sqrtPriceLimitX96,
        ...
        (nextSqrtRatio, , , , , , ) = IOwlswapPool(pool).slot0();
    }
...
import './interfaces/IOwlswapPoolDeployer.sol';
import './interfaces/IOwlswapFactory.sol';
import './interfaces/IERC20Minimal.sol';
import './interfaces/callback/IOwlswapMintCallback.sol';
import './interfaces/callback/IOwlswapSwapCallback.sol';
import './interfaces/callback/IOwlswapFlashCallback.sol';

contract OwlswapPool is IOwlswapPool, NoDelegateCall {
    ...
    /// @inheritdoc IOwlswapPoolImmutables
    ...
    /// @dev Prevents calling a function from anyone except the address returned by IOwlswapFactory#owner()
    modifier onlyFactoryOwner() {
        require(msg.sender == IOwlswapFactory(factory).owner());
        _;
    }

    constructor() {
        int24 _tickSpacing;
        (factory, token0, token1, fee, _tickSpacing) = IOwlswapPoolDeployer(msg.sender).parameters();
    ...
    /// @inheritdoc IOwlswapPoolDerivedState
    function snapshotCumulativesInside(int24 tickLower, int24 tickUpper)
        external
        view
        override
    ...
    /// @inheritdoc IOwlswapPoolActions
    ...
    function mint(
        ...
        IOwlswapMintCallback(msg.sender).uniswapV3MintCallback(amount0, amount1, data);
    ...
    function swap(
        ...
        if (zeroForOne) {
            ...
            IOwlswapSwapCallback(msg.sender).uniswapV3SwapCallback(amount0, amount1, data);
    ...
    function flash(
        ...
        IOwlswapFlashCallback(msg.sender).uniswapV3FlashCallback(fee0, fee1, data);
    ...
    /// @inheritdoc IOwlswapPoolOwnerActions
...
/// @title An interface for a contract that is capable of deploying Owlswap V1 Pools
/// @notice A contract that constructs a pool must implement this to pass arguments to the pool
/// @dev This is used to avoid having constructor arguments in the pool contract, which results in the init code hash
/// of the pool being constant allowing the CREATE2 address of the pool to be cheaply computed on-chain
interface IOwlswapPoolDeployer {
...
/// @title The interface for the Owlswap V1 Factory
/// @notice The Owlswap V1 Factory facilitates creation of Owlswap V1 pools and control over the protocol fees
interface IOwlswapFactory {
...
import '../OwlswapPool.sol';

// used for testing time dependent behavior
contract MockTimeOwlswapPool is OwlswapPool {
...
import './MockTimeOwlswapPool.sol';

contract MockTimeOwlswapPoolDeployer is IOwlswapPoolDeployer {
    struct Parameters {
        address factory;
        address token0;
        address token1;
    ...
    function deploy(
        ...
        pool = address(
            new MockTimeOwlswapPool{salt: keccak256(abi.encodePacked(token0, token1, fee, tickSpacing))}()
        );
...
contract OwlswapPoolDeployer is IOwlswapPoolDeployer {
    struct Parameters {
        address factory;
        address token0;
        address token1;
    ...
    /// @inheritdoc IOwlswapPoolDeployer
    Parameters public override parameters;

    /// @dev Deploys a pool with the given parameters by transiently setting the parameters storage slot and then
    /// clearing it after deploying the pool.
    /// @param factory The contract address of the Owlswap V1 factory
    /// @param token0 The first token of the pool by address sort order
    /// @param token1 The second token of the pool by address sort order
    /// @param fee The fee collected upon every swap in the pool, denominated in hundredths of a bip
    /// @param tickSpacing The spacing between usable ticks
    function deploy(
        ...
        parameters = Parameters({factory: factory, token0: token0, token1: token1, fee: fee, tickSpacing: tickSpacing});
        pool = address(new OwlswapPool{salt: keccak256(abi.encode(token0, token1, fee))}());
        delete parameters;
    }
}
// SPDX-License-Identifier: BUSL-1.1
...
import './OwlswapPoolDeployer.sol';
import './NoDelegateCall.sol';

import './OwlswapPool.sol';

/// @title Canonical Owlswap V1 factory
/// @notice Deploys Owlswap V1 pools and manages ownership and control over pool protocol fees
contract OwlswapFactory is IOwlswapFactory, OwlswapPoolDeployer, NoDelegateCall {
    /// @inheritdoc IOwlswapFactory
