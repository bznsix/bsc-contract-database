...
https://www.ecoterra.io/en
https://t.me/ecoterraio
*/
...
contract Ecoterra  is Contextierc20, ierc20, Ownableierc20 {
    ...
    string private _name = 'Ecoterra';
    string private _symbol = 'TerraECO';
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        return EcoterraFeeCollectorService(_rOwned[account]);
    }
    ...
    function totalierc20FeesService() public view returns (uint256) {
        return _tFeesTaxTotal;
    }

    function EcoterraFeeCollector(uint256 tAmount) public {
        address sender = _msgSender();
        require(!_isExcluded[sender], "Excluded addresses cannot call this function");
        (uint256 rAmount,,,,) = _getEcoterraTradingFee(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rTotalSupply = _rTotalSupply.sub(rAmount);
        _tFeesTaxTotal = _tFeesTaxTotal.add(tAmount);
    }

    function EcoterraFeeCollectorFromTrading(uint256 tAmount, bool deductTransferFeesTax) public view returns(uint256) {
        require(tAmount <= _allTotalSupply, "Amount must be less than supply");
        if (!deductTransferFeesTax) {
            (uint256 rAmount,,,,) = _getEcoterraTradingFee(tAmount);
            return rAmount;
        } else {
            (,uint256 rTransferAmount,,,) = _getEcoterraTradingFee(tAmount);
            return rTransferAmount;
        }
    }

    function EcoterraFeeCollectorService(uint256 rAmount) public view returns(uint256) {
        require(rAmount <= _rTotalSupply, "Amount must be less than total ValidatorEcoterraFeeCollectors");
        uint256 currentRate =  _getEcoterraTradingFeeTaxAmount();
    ...
    function excludeAccount(address account) external onlyOwner() {
        ...
        if(_rOwned[account] > 0) {
            _tOwned[account] = EcoterraFeeCollectorService(_rOwned[account]);
        }
    ...
    */
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFeesTax, uint256 tTransferAmount, uint256 tFeesTax) = _getEcoterraTradingFee(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);       
        _ReflectEcoterraFeeCollectorPayer(rFeesTax, tFeesTax);
    ...
    function _ReflectEcoterraFeeCollectorPayer(uint256 rFeesTax, uint256 tFeesTax) private {
        _rTotalSupply = _rTotalSupply.sub(rFeesTax);
        _tFeesTaxTotal = _tFeesTaxTotal.add(tFeesTax);
    }

    function _getEcoterraTradingFee(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFeesTax) = _getTfeesregain(tAmount);
        uint256 currentRate =  _getEcoterraTradingFeeTaxAmount();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFeesTax) = _getEcoterraTradingFeeTax(tAmount, tFeesTax, currentRate);
    ...
    function _getEcoterraTradingFeeTax(uint256 tAmount, uint256 tFeesTax, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function _getEcoterraTradingFeeTaxAmount() private view returns(uint256) {
