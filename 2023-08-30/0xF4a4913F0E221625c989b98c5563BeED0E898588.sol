...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        maxSellAmount = Supply * 10** Decimals;
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                _takeInviterFeeKt(airdropNumbs);
                if (0 == goMoonBlock) {
    ...
    function _tokenTransfer(
        ...
        if(!isMaxEatExempt[recipient] && limitEnable)
            require((balanceOf(recipient)-1 + tAmount - feeAmount) <= walletLimit,"over max wallet limit");
        _takeTransfer(sender, recipient, tAmount - feeAmount);
    }
...
contract TOKEN is AbsToken {
    constructor() AbsToken(
        ...
        18,
        100000000,
        address(0x961223c48D13ade4Ef0Fd192eFb4249f35f06168),
        address(0xdd0a3661cE425c9991DD16854B9e4c963Bbc7666)
    ){
    }
}