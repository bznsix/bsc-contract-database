...
interface IBandRandomNumberGenerator {
    function getRandomNumber(uint256 userProvidedSeed) external returns (bytes32 requestId);
    function randomnessResults(bytes32 requestId) external view returns (uint256);
}
...
contract Lottery is IVRFConsumer {
    ...
    function drawWinner() external onlyOwner afterDrawDate {
