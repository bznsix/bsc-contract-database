...
contract Blood is Context, IERC20, Ownable {
    ...
    string private _name     = "Blood";

    string private _symbol   = "BLO0D";  
    ...
    uint256 public bloodTeamFee = 0;

    uint256 private _previousbloodTeamFee = bloodTeamFee;
    ...
    uint256 public growthFee = 5;

    uint256 private _previousgrowthFee = growthFee;



    uint256 public totalFees = taxFee + burnFee + marketingFee + growthFee + bloodTeamFee;



    address public marketingWallet = 0x000000000000000000000000000000000000dEaD;

    address public growthWallet = 0x000000000000000000000000000000000000dEaD;

    address public burnWallet = 0x000000000000000000000000000000000000dEaD;

    address public bloodTeamWallet = 0x000000000000000000000000000000000000dEaD;    
    ...
    event growthWalletChanged(address growthWallet);

    event burnWalletChanged(address burnWallet);

    event bloodTeamWalletChanged(address bloodTeamWallet);

    event SwapEnabledUpdated(bool enabled);

    event SendMarketing(uint256 bnbSend);

    event Sendgrowth(uint256 bnbSend);
    ...
    { 
        ...
        _isExcludedFromFees[growthWallet] = true;

        _isExcludedFromFees[burnWallet] = true;

        _isExcludedFromFees[bloodTeamWallet] = true;
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        (uint256 tTransferAmount, uint256 tFee, uint256 tMarketing, uint256 tgrowth) = _getTValues(tAmount);

        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tMarketing, tgrowth,  _getRate());

        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tMarketing, tgrowth);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount - tFee - tMarketing - tgrowth;

        return (tTransferAmount, tFee, tMarketing, tgrowth);

    }



    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tMarketing, uint256 tgrowth, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount - rFee - rMarketing - rgrowth;
    ...
    function _takebloodTeam(address sender, uint256 tTransferAmount, uint256 rTransferAmount, uint256 tAmount) private returns (uint256, uint256) {

        if(bloodTeamFee==0) {

            return(tTransferAmount, rTransferAmount); }

        uint256 tCharity = calculatebloodTeamFee(tAmount);
    ...
    function calculatebloodTeamFee(uint256 _amount) private view returns (uint256) {

        return _amount * bloodTeamFee / 100;
    ...
    function removeAllFee() private {
        ...
        if(taxFee == 0 && burnFee == 0 && bloodTeamFee == 0 && marketingFee == 0 && growthFee == 0) return;
        ...
        _previousbloodTeamFee = bloodTeamFee;

        _previousMarketingFee = marketingFee;

        _previousgrowthFee = growthFee;
    ...
    function restoreAllFee() private {
        ...
        bloodTeamFee = _previousbloodTeamFee;

        marketingFee = _previousMarketingFee;

        growthFee = _previousgrowthFee;
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 taxForSwap = growthFee + marketingFee + burnFee + bloodTeamFee;
            ...
            if(taxForSwap > 0) {
                ...
                if(growthFee > 0) {
                    ...
                    uint256 growthBNB = newBalance * growthFee / taxForSwap;

                    sendBNB(payable(growthWallet), growthBNB);

                    emit Sendgrowth(growthBNB);
                ...
                if(bloodTeamFee > 0) {
                    ...
                    uint256 charityBNB = newBalance * bloodTeamFee / taxForSwap;

                    sendBNB(payable(bloodTeamWallet), charityBNB);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tMarketing, uint256 tgrowth) = _getValues(tAmount);

        (tTransferAmount, rTransferAmount) = _takeburn(sender, tTransferAmount, rTransferAmount, tAmount);

        (tTransferAmount, rTransferAmount) = _takebloodTeam(sender, tTransferAmount, rTransferAmount, tAmount);
    ...
    function changegrowthWallet(address _growthWallet) external onlyOwner {

        require(_growthWallet != growthWallet, "growth wallet is already that address");

        require(!isContract(_growthWallet), "growth wallet cannot be a contract");

        growthWallet = _growthWallet;

        emit growthWalletChanged(growthWallet);
    ...
    function changebloodTeamWallet(address _bloodTeamWallet) external onlyOwner {

        require(_bloodTeamWallet != bloodTeamWallet, "wallet is already that address");

        require(!isContract(_bloodTeamWallet), "wallet cannot be a contract");

        bloodTeamWallet = _bloodTeamWallet;

        emit bloodTeamWalletChanged(bloodTeamWallet);
    ...
    function setTaxFeePercent(uint256 _taxFee) external onlyOwner() {
        ...
        totalFees = taxFee + marketingFee + burnFee + growthFee + bloodTeamFee;
    ...
    function setbloodTeamPercent(uint256 _bloodTeam) external onlyOwner() {

        bloodTeamFee = _bloodTeam;
    ...
    function setgrowthFeePercent(uint256 _growthFee) external onlyOwner() {

        growthFee = _growthFee;
