...
contract TarotPiToken is ERC20Votes, Ownable {
    ...
    modifier onlyMinter() {
        require(isMinter(_msgSender()), "CALLER_IS_NOT_THE_MINTER");
        _;
    }

    constructor() ERC20Permit("") ERC20("TarotPi Token", "TART") {
        _mint(_msgSender(), initTotalSupply);
    }
    ...
    function getMinter(uint256 index) external view returns (address) {
        require(index <= EnumerableSet.length(minters) - 1, "OUT_OF_BOUNDS");
