...
LIX FACTORY WITH 2% FEE

Contract will take 2% fee on every transaction until 2 BNB is paid
...
contract LIX_FACTORY_PAY_LATER is Context {
    ...
    function CreateToken(string memory Token_Name, 
                         ...
                         string memory factory_Website) public {
...
contract TOKEN is Context, IERC20 { 
    ...
    address payable public constant fee_Collector = payable(0xDc9Db0cB2293468989ea8d50D3279E2798366A07); 
    ...
    uint256 public contractAmountPaid; 
    ...
    function Project_Information() external view returns(address Owner_Wallet,
                                                       ...
                                                       string memory Contract_Created_At,
                                                       string memory Contract_Code_By) {
                                                           
        string memory _creator = "https://t.me/GenTokens_GEN"; 
    ...
    function F_02_Set_Fees(
        ...
        // Buyer Protection: Max Fee 20% 
        require (Marketing_on_BUY + Liquidity_on_BUY + Burn_on_BUY + _fee__Buy_Contract <= 20, "SF1");  // Max fee 20% (including contract fee if applicable)

        // Buyer Protection: Max Fee 20% 
        require (Marketing_on_SELL + Liquidity_on_SELL + Burn_on_SELL + _fee__Sell_Contract <= 20, "SF2");  // Max fee 20% (including contract fee if applicable)
    ...
    */
    ...
    function F_05_Update_Links_Website(
    ...
    function F_06_Update_Links_Telegram(
    ...
    function F_07_Update_Links_LP_Lock(
    ...
    */
    ...
    Will be removed automatically after 2 BNB is paid or can be removed manually with this function

    */

    function _Remove_Contract_Fee() external payable { 

        require(msg.value >= (contractFee - contractAmountPaid), "REQ_2BNB"); // Insufficient amount

        send_BNB(fee_Collector, msg.value);
    ...
    */
    function processFees(uint256 Tokens) private {
        ...
        if (!contractFeePaid){
            ...
            // Cap fee at 2 BNB!
            uint256 BNB_To_Pay = contractFee - contractAmountPaid;

            if (BNB_ContractFee > BNB_To_Pay) {
                BNB_ContractFee = BNB_To_Pay;
            }

            // Send contract fee to fee collection contract 
            send_BNB(fee_Collector, BNB_ContractFee);

            contractAmountPaid += BNB_ContractFee;

            // Auto end contract fee once 2 BNB has been paid  
            if(contractAmountPaid >= contractFee){ 
...
/*
    ...
    Code by GEN
        ...
        Telegram: @GenTokens_GEN
        Website: TokensByGen.com
