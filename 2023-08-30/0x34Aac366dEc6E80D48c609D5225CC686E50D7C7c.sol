...
contract BABYPEPE is ERC20, Ownable {
    ...
    constructor(
        ...
        address owner,
        address mintToAddr
    ) ERC20('BABYPEPE2.0', 'BEPE2.0') {
        require(
            feeAccount != address(0),
            'BABYPEPE2.0: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'BABYPEPE2.0: incorrect fee percent');
        require(owner != address(0), 'BABYPEPE2.0: incorrect owner address');
        require(mintToAddr != address(0), 'BABYPEPE2.0: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'BABYPEPE2.0: transfer from the zero address');
        require(recipient != address(0), 'BABYPEPE2.0: transfer to the zero address');
