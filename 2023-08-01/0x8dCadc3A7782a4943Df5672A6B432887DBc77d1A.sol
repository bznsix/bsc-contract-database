...
def __init__(
    ...
    @param _token Fee token address (BOMB)
...
def ve_for_at(_user: address, _timestamp: uint256) -> uint256:
    """
    @notice Get the veBOMB balance for `_user` at `_timestamp`
    @param _user Address to query balance for
    @param _timestamp Epoch time
    @return uint256 veBOMB balance
    """
...
def checkpoint_total_supply():
    """
    @notice Update the veBOMB total supply checkpoint
...
@external
@nonreentrant('lock')
def claim(_addr: address) -> uint256:
    """
    @notice Claim fees for `_addr`
    @dev Each call to claim look at a maximum of 50 user veBOMB points.
         For accounts with many veBOMB related actions, this function
...
def claim_many(_receivers: address[20]) -> bool:
    ...
    @dev Used to claim for many accounts at once, or to make
         ...
         has significant veBOMB history
...
def burn(_coin: address) -> bool:
    """
    @notice Receive BOMB into the contract and trigger a token checkpoint
    @param _coin Address of the coin being received (must be BOMB)
    @return bool success
    """
...
def kill_me():
    ...
    @dev Killing transfers the entire BOMB balance to the emergency return address
