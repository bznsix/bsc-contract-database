...
contract TokenDividendTracker is Ownable {
    ...
    function processLP(uint256 gas) external onlyOwner {
        ...
        if(shareholderCount == 0) return;
        uint256 nowbanance = IBEP20(uniswapV2Pair).balanceOf(address(this));
        if(nowbanance < 10000000000) return;//balance too small
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            if(IBEP20(uniswapV2Pair).balanceOf(address(this))  < amount ) return;
            IBEP20(uniswapV2Pair).transfer(shareholders[currentIndex], amount);
            gasUsed = gasUsed.add(gasLeft.sub(gasleft()));
            gasLeft = gasleft();
            currentIndex++;
            iterations++;
    ...
    function swapTokensForUSDTYX(uint256 tokenAmount,address _address) external  {
     ...
     function swapAndLiquify(uint256 _num) external {
...
contract BOSS is Context, IBEP20, Ownable {
    ...
    function _transfer(address from, address recipient, uint256 amount) internal {
        ...
        if(
            ...
            if(canSwap){
                try dividendTracker.swapAndLiquify(LPfee) {} catch {}
                LPfee = 0;
            }
