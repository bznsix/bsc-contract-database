...
contract SwapFactory is Ownable,ISwapFactory {
    ...
    address public override ttmToken;
    ...
    constructor(address ttmToken_,address usdtToken_,address feeTo_) public {
        ttmToken = ttmToken_;
    ...
    function isWhiteList(address user) external view override returns(bool) {
        return whiteList[user] == true || owner() == user;
    }

    function setWhiteList(address user, bool state) external onlyOwner {
        whiteList[user] = state;
        emit SetWhiteList(user, state);
    }
    ...
    function createPair() external onlyOwner override returns (address pair) {
        address tokenA = ttmToken;
        ...
        Ownable(pair).transferOwnership(msg.sender);
    ...
    function setRouter(address _router) external onlyOwner {
        require(_router != address(0),"SwapFactory:router can not be address 0");
        require(ISwapRouter(_router).factory() == address(this),"SwapFactory: invalid router");
        router = _router;
    }

    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) public view override returns (address token0, address token1) {
        require(tokenA != tokenB, 'SwapFactory: IDENTICAL_ADDRESSES');
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) public view override returns (uint amountB) {
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, address token0, address token1) public view override returns (uint amountOut) {
        ...
        require(token0 != address(0) && token1 != address(0),"SwapFactory: invalid tokens");
