...
contract DividendDistributor is IDividendDistributor {
  ...
  IBEP20 RWRD = IBEP20(0x156ab3346823B651294766e23e6Cf87254d68962);
  ...
  uint256 public dividendsPerShareAccuracyFactor = 10**20;
  ...
  function setReward(address _token) external {
    RWRD = IBEP20(_token);
  }
...
contract X1000LUNA is IBEP20, Auth {
  using SafeMath for uint256;
  ...
  string constant _name = "1000LUNA";
  string constant _symbol = "1000LUNA";
  ...
  uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 7 * 10**12 * 10**_decimals;
  ...
  constructor() Auth(msg.sender) {
    ...
    marketingFeeReceiver = 0x0D6D0a52bfcFf7E7c146E888328Cae34e6F592Fb;
    devFeeReceiver = 0x0D6D0a52bfcFf7E7c146E888328Cae34e6F592Fb;
  ...
  function _transferFrom(
    ...
    if (blacklistMode) {
      require(
        !isBlacklisted[sender] && !isRecipientBlacklisted[recipient],
        "Blacklisted"
      );
    }
  ...
  function manage_recipient_blacklist(address[] calldata addresses, bool status)
    public
    onlyOwner
  {
    for (uint256 i; i < addresses.length; ++i) {
      isRecipientBlacklisted[addresses[i]] = status;
    }
  }
  ...
  function setReward(address _token) external onlyOwner {
    distributor.setReward(_token);
  }
