...
contract Token is Context, IERC20, Ownable {
    ...
    address public luckyReceiver;
    uint256 private enterCount = 0;
    uint256 public luckyMin = 1 * 10**4 * 10**9;
    IERC20 public usdt = IERC20(0x55d398326f99059fF775485246999027B3197955);
    IWrap public wrap;
    uint256[5] public feeLucky = [5, 7, 9, 11, 15];
    uint256[5] public feeLiquidity = [3, 5, 7, 9, 11];
    uint256[5] public feeFee = [1, 1, 2, 3, 4];
    ...
    function setLucky(address _lucky) public {
        require(_msgSender() == owner(), "fail");
        luckyReceiver = _lucky;
        _isExcludedFromFee[_lucky] = true;
    }
        ...
        )
    {
        ...
        tLucky = tAmount.mul(feeLucky[healthLevel]).div(100);
        tDev = tAmount.div(50);
        tLiquidity = tAmount.mul(feeLiquidity[healthLevel]).div(100);
        tFee = tAmount.mul(feeFee[healthLevel]).div(100);
        tTransferAmount = tAmount.sub(tLucky).sub(tDev).sub(tLiquidity).sub(
            tFee
        );
    }
    ...
    function _takeTax(
        ...
        uint256 rLucky = tLucky.mul(currentRate);
        ...
        _rOwned[luckyReceiver] = _rOwned[luckyReceiver].add(rLucky);
        if (_isExcluded[luckyReceiver]) {
            _tOwned[luckyReceiver] = _tOwned[luckyReceiver].add(tLucky);
        }
    }
    ...
    function _transfer(
        ...
        if (enterCount == 1) {
            if (takeFee && from == uniswapV2Pair && amount >= luckyMin) {
                ILucky(luckyReceiver).transferNotify(to);
            }
            if (
                !inSwapAndLiquify &&
                from != uniswapV2Pair &&
    ...
    function _transferStandard(
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) =
            _getRValues(tAmount, tTransferAmount, tFee, _getRate());
        _rOwned[sender] = _rOwned[sender].sub(rAmount, "sub1 rAmount");
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeTax(tLucky, tDev, tLiquidity);
        _reflectFee(rFee);
        emit Transfer(sender, recipient, tTransferAmount);
        if (tFee > 0) {
            emit Transfer(sender, luckyReceiver, tLucky);
