...
contract Presale is Ownable {
    ...
    uint256 private salePrice = 240000000000; // 0.00000024 BNB во WEI
    ...
    require(msg.value >= 0.01 ether, "Minimum purchase amount not met"); // Check if enough BNB have been sent

    uint256 _msgValue = msg.value;

    // Calculation of the amount of purchased tokens according to the sale price
    uint256 _tokenAmount = _msgValue.div(salePrice);

    // Transfer of the purchased tokens to the user
    _balances[msg.sender] = _balances[msg.sender].add(_tokenAmount);
    _token.transfer(msg.sender, _tokenAmount);

    // Calculation and execution of the referral commission (if there is a referral) in tokens
    if (_msgSender() != _refer && _refer != address(0) && _balances[_refer] > 0) {
        uint256 referToken = _tokenAmount.mul(_referToken).div(10000);
        _balances[_refer] = _balances[_refer].add(referToken);
    }

    // Calculation and execution of the referral commission (if there is a referral) in BNB
    if (_msgSender() != _refer && _refer != address(0) && _balances[_refer] > 0) {
