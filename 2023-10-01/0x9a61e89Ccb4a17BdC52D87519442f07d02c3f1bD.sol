...
contract MatrixToken is ERC20, Ownable, BaseToken {
    ...
    address  public pinkLockAddress = 0x7ee058420e5937496F5a2096f04caA7721cF70cc;
    ...
    constructor(
        ...
        uint256[3 + 1 + 1 + 2 + 3] memory batchUint256s,
        ...
        setFees(batchUint256s[0], batchUint256s[1], batchUint256s[2]);
        setSpecFees(batchUint256s[7], batchUint256s[8], batchUint256s[9]);
        ...
        */
        ...
        setBuyLimitPartsPerMillion(batchUint256s[5]);

        setHasSellLimit(batchBooleans[2]);
        setSellLimitPartsPerMillion(batchUint256s[6]);
    ...
    onlyOwner
    {
        ...
        require(totalFees <= maxFees, "Total fee is over 100%");
    }
    ...
    onlyOwner
    {
        ...
        specTotalFees = specTokenRewardsFee.add(specLiquidityFee).add(marketingFee);
    ...
    function _transfer(
        ...
        uint256 tokenRewardsFee_ = tokenRewardsFee;
        uint256 liquidityFee_ = liquidityFee;
        uint256 marketingFee_ = marketingFee;
        ...
        // buy limit, sell limit
            ...
            if (isForbidToSellAll && to == uniswapV2Pair) {
                require(amount <= balanceOf(from) * 99 / 100, 'Forbid to sell all tokens');
            }

            // buy limit
            if (hasBuyLimit && from == uniswapV2Pair) {
                require(amount <= buyLimitAmount, 'Exceeded maximum buy limit');
            }

            // sell limit
            if (hasSellLimit && to == uniswapV2Pair) {
                require(amount <= sellLimitAmount, 'Exceeded maximum sell limit');
            }
        }
