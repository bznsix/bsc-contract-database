...
contract ANNE is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public charityAddress = 0x6207F5A93a03e5c5Fe77fF2B3cA4Cb9cd0a590FB;
    ...
    string private _name = "ANNE";
    string private _symbol = "ANNE";
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tFomo, uint256 tDev, uint256 tLiquidity, uint256 tFee) {
        ...
        tFomo = tAmount.mul(feeFomo).div(100);
    ...
    function _transfer(
        ...
        if (enterCount == 1 && fomoEnable) {
            if (takeFee && from == pancakePair && amount >= fomoMin) {
                IFomo(fomoReceiver).transferNotify(to);
            }
            if (!inSwapAndLiquify && from != pancakePair && from != fomoReceiver) {
    ...
    function setFomoMin(uint256 _amount) public onlyOwner {
        fomoMin = _amount;
    }
    ...
    function setFomoEnable(bool _fomoEnable) public onlyOwner {
        fomoEnable = _fomoEnable;
    }
    
    function setAllFee(uint256 tax, uint256 liquidity, uint256 fomo, uint256 dev) public onlyOwner {
        _taxFee = tax;
    ...
    // Innovation for protocol by CARROT Team
