...
contract panpan is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
   
    address payable  addressD; 
    ...
    string private _name = "panpan";
    string private _symbol = "panpan";
   ...
   _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        uint256 currentRate = _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        _rOwned[addressD] = _rOwned[addressD].add(rLiquidity);
        if(_isExcluded[address(this)])
            _tOwned[addressD] = _tOwned[addressD].add(tLiquidity);
    }
