...
import "./IBurn.sol";
import "../token/TurboDexToken.sol";
...
contract Burn is IBurn, Ownable, ERC721Holder {
    ...
    constructor(address _turboToken, address _oracleContract) {
    ...
    function burnToken(uint256 _totalAmountUsdDecimal) public override onlyStakingContract() {
        uint256 _totalAmountTokenDecimal = Oracle(oracleContract).convertUsdBalanceDecimalToTokenDecimal(
            _totalAmountUsdDecimal
        );
        uint256 balanceOfContract = ERC20(turboToken).balanceOf(address(this));
        ...
        if (burnableTokenDecimal > 0) {
            TurboDexToken(turboToken).burn(burnableTokenDecimal);
    ...
    function withdrawTokenEmergency(address _token, uint256 _amount) public onlyOwner {
        require(_amount > 0, "BURN: INVALID AMOUNT");
        require(ERC20(_token).transfer(msg.sender, _amount), "BURN: CANNOT WITHDRAW TOKEN");
    }
}
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.8;
