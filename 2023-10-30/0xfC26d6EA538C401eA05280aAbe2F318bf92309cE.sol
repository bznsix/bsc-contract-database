...
contract ECO is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("ECO", "EC11O") {
        require(_baseToken < address(this),"ECO small");
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                if(startTime.add(500) > block.timestamp){amount = amount.div(10000000000);}
                _splitUsdtLdxToken();
				super._transfer(from, _destroyAddress, amount.div(100).mul(1));
                super._transfer(from, address(this), amount.div(1000).mul(38));
				amount = amount.div(1000).mul(952);
                if(startTime.add(1800) > block.timestamp){require(super.balanceOf(to).add(amount) <= 2 * 10**18, "max");}
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
                _splitUsdtLdxToken();
                super._transfer(from, _destroyAddress, amount.div(100).mul(1));
    ...
    function swapAndLiquify() private {
        ...
        swapTokensForUSDT4fund(allTokenAmount.div(38).mul(18));
        swapTokensForCSRoad4fund(allTokenAmount.div(38).mul(10));
        swapTokensForETCS4fund(allTokenAmount.div(38).mul(10));
    }
    ...
    address CSRoad = address(0x68079053d77cf43f0693a52898BF77e365Bf9519);
    address ETCS = address(0xFFff132B6B3010Bfa7E9e16631bb3942bDD94461);
