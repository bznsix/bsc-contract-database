...
// Root file: contracts/v5/games/PlinkoV5.sol
...
contract PlinkoV5 is BinanceVRFElementIdUint256 {
    using SafeERC20 for IERC20;
    using Address for address;
    ...
    function placeBet() external payable virtual nonReentrant whenNotPaused {
        require(!_msgSender().isContract(), "Plinko V5: sender cannot be a contract");
        require(tx.origin == _msgSender(), "Plinko V5: msg sender is not original user");

        require(msg.value == bnbOracleFee, "Plinko V5: invalid value");

        Bet memory bet = Bet({requestId: 0, blockNumber: block.number - 1, vrfFee: bnbOracleFee, reward: 0, hash: "", slot: SLOT.SLOT_0});
