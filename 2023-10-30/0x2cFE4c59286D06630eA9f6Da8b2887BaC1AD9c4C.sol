...
import "../interfaces/IICHIVaultFactory.sol";

contract ICHIAdapterInitializable is IAdapter, Initializable, Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    string public constant PROTOCOL = "ICHI";
    ...
    function initialize(
        ...
        wrapper = _wrapper;
        vault = _vault;
        token0 = IICHIVault(vault).token0();
        token1 = IICHIVault(vault).token1();
        lpToken = vault;
    ...
    function totalSupply(
        ...
        return IICHIVault(vault).totalSupply();
    }
    ...
    function tokenPerShare(
        ...
        if (totalSupply_ > 0) {
            (uint256 amount0, uint256 amount1) = IICHIVault(vault).getTotalAmounts();
    ...
    function pool(
        ...
        return IICHIVault(vault).pool();
    }

    function manager(
    ) public view returns(address) {
        return IICHIVault(vault).owner();
    }

    function managerFee(
    ) public view returns(uint256) {
        address vaultFactory = IICHIVault(vault).ichiVaultFactory();
        return IICHIVaultFactory(vaultFactory).baseFee() * PRECISION / IICHIVault(vault).PRECISION();
    }
    ...
    function deposit(
        ...
        _share = IICHIVault(vault).deposit(_amount0, _amount1, wrapper);
    ...
    function withdraw(
        ...
        (_amount0, _amount1) = IICHIVault(vault).withdraw(_share, _user);
