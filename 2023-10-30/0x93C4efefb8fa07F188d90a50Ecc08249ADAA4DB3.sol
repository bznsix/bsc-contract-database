...
contract MinpitCSun is Ownable,AccessControl{
    ...
    //打卡有效周期
    uint256 public _sign_cycle;

    //打卡是否叠加，1：是，2：否
    uint256 public _sign_overlay;
    ...
    constructor() Ownable(){
        _token = 0x84b221ac6C1a21A2Ad8D477ecF313d4dC5b4cC56;
        ...
        _mayorAddr = 0x73885331f181ff3197D13E3C3a646761A8353E16;
    ...
    **/
    ...
    //购买矿产。userAddr：用户地址；signCycle：打卡有效周期；price：价格；time：时间戳
    event Minerals(uint256 type_,address userAddr,uint256 signCycle,uint256 price,uint256 signOverlay, uint256 time);
    ...
    function distributionToken(address userAddr,uint256 tokenAmount) internal{
        if(_mayorDivide > 0){
            IERC20(_token).transferFrom(userAddr,_mayorAddr,tryDiv(tokenAmount * _mayorDivide,1000));
        }
        if(_governorDivide > 0){
            IERC20(_token).transferFrom(userAddr,_governorAddr,tryDiv(tokenAmount * _governorDivide,1000));
        }
        if(_operateDivide > 0){
            IERC20(_token).transferFrom(userAddr,_operateAddr,tryDiv(tokenAmount * _operateDivide,1000));
        }
        if(_terminalDivide > 0){
            IERC20(_token).transferFrom(userAddr,_terminalAddr,tryDiv(tokenAmount * _terminalDivide,1000));
        }
    }
    ...
    function purchaseMinerals(
        ...
        require(block.timestamp <= expired_time + 60 * 5,"2");

        uint256 price = getTokenPrice();

        uint256 tokenSum = _price_usdt * (10 ** 18) / price;
        
        distributionToken(msg.sender,tokenSum);
        emit Minerals(2,msg.sender,_sign_cycle,_price_usdt,_sign_overlay,block.timestamp);

        emit Order(order_nu, block.timestamp, _price_usdt);
    }
