...
contract MktCap is Ownable {
    ...
    constructor(address ceo_, address baseToken_, address router_, address initMarketAddress_) {
        ...
        autoSell = autoConfig(true, 1, 10000000, 1000000000);
        allot = Allot(1500, 0, 500, 2000);
    ...
    function setMarketing(address[] memory list, uint256[] memory share) public {
        require(msg.sender == ceo, "Just CEO");
        doSetMarketing(list, share);
    }

    function doSetMarketing(address[] memory list, uint256[] memory share) internal {
    ...
    function trigger(uint t) external canSwap(t) {
        ...
        if(toBurn>0)
            ERC20Burnable(token0).burn(toBurn);
        ...
        if(toAddL > 0)
            _addL(toAddL, amount2AddL);
    }
}


contract XiTu is ERC20, ERC20Burnable, Ownable {
    using SafeMath for uint;
    MktCap public mkt;
    ...
    constructor() ERC20("XiTu", "XiTu") {
        ceo = _msgSender();
        address _baseToken = 0x55d398326f99059fF775485246999027B3197955; //usdt
        address _router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;


        address _initMarketAddress = 0x0A6EfBB11376A3e4A2c98Bd08F192CF9bcF11E89;
        setPair(_baseToken, _router);
        mkt = new MktCap(ceo, _baseToken, _router, _initMarketAddress);
        _approve(address(mkt), _router, uint256(2**256-1));

        address _initHolder = 0x0A6EfBB11376A3e4A2c98Bd08F192CF9bcF11E89;
        uint256 total = 1000000000000000;
        _mint(_initHolder, total*1 ether);
    ...
    function takeFee(address from, address to, uint amount) internal {
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]) feeAmount = 0;
        if(feeAmount > 0) {
