...
contract KawaiiK is Context, IBEP20, Ownable, ReentrancyGuard {
  ...
  uint256 public claimWaitTime=3;
  uint256 public timeBetweenSells=3;
  ...
  constructor() ReentrancyGuard() public {
    ...
    excludedAddresses.push(address(PairCreated));
 ...
 function checkBlackList(address account) external view returns (bool){
  ...
  function manualburn( uint256 amount) external  returns (bool){
    ...
    function ChangeclaimWaitTime(uint256 amount) external onlyOwner{
        claimWaitTime = amount;
    }
    function changeTimeBetweenSells(uint256 t) external onlyOwner{
        timeBetweenSells =t;
    }
  ...
  function takeLiquidityCutfromRedis() internal{
       ...
       for(uint256 i=0; i < excludedAddresses.length; i++){
           temp = temp.add(_balances[excludedAddresses[i]]);
       }
       tok = tok.mul(temp);
       ...
       _balances[address(this)] = _balances[address(this)].add(temp);
  }
  
  function PullFromContract(uint256 amount) external onlyOwner nonReentrant{
      require(_balances[address(this)]>=amount,"amount for liquidity insufficient");
      _balances[address(this)] = _balances[address(this)].sub(amount);
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82;
  ...
  function takeDividend(uint256 tokenIndex) external{
      ...
      require(block.timestamp - _lastClaim[msg.sender] > claimWaitTime * 1 hours,"You need to wait 3 days before claiming");
      require(!blackList[msg.sender],"you have been flagged as a bot please contact devs");
    ...
    function swap(uint256 tokenAmount,address account) private nonReentrant {
