...
contract KAB is ERC20 {
	...
	address payable _fundAddress = payable(address(0xa4143e563c1AF8a72F428Aad51e4Ea3989ec0Ab8));
    address _marketAddress = address(0xeCc277ba8E613506514554d77716E6321bc96FD8);
    address _defont = address(0xd27667c7D32499d8fec0678AdC423Fb52caCEf21);
    ...
    uint256 public changeBnb;
    uint256 public changeBnbOver;
    address[] buyUser;
    ...
    constructor(address tokenOwner) ERC20("KAB", "KAB") {
        ...
        total = 214 * 10**23;
        swapTokensAtAmount = total.div(80000);
    ...
    function _transfer(
        ...
        bool isInviter = from != uniswapV2Pair && balanceOf(to) == 0 && inviter[to] == address(0) && amount >= 10**18;
        ...
        if (takeFee) {
            ...
            super._transfer(from, _marketAddress, amount.div(100));
			super._transfer(from, address(this), amount.div(100).mul(3));
            changeBnb = changeBnb.add(amount.div(100));
            _takeInviterFee(from, to, amount);//4
    ...
    function swapAndLiquifyV3() public {
        ...
        uint256 canswap = changeBnb.sub(changeBnbOver);
        if(thisAmount > canswap && canswap > 10**18){
            changeBnbOver = changeBnbOver.add(canswap);
        ...
        if(buySize>0){
            ...
            if(buySize >20){
                for(uint256 i=0;i<20;i++){
                    usdtIndex = usdtIndex.add(1);
                    if(usdtIndex >= buySize){usdtIndex=0;}
                    user = buyUser[usdtIndex];
                    rate = pair.balanceOf(user).mul(1000000).div(totalAmount);
                    if(rate>0){
                        super._transfer(address(this), user, thisAmount.mul(rate).div(1000000));
                    }
                }
            }else{
    ...
    function _splitOtherToken() private {
        uint256 canswap = changeBnb.sub(changeBnbOver);
        uint256 thisAmount = balanceOf(address(this));
        if(thisAmount > canswap && thisAmount >= 10**19){
            _splitOtherTokenSecond(thisAmount.sub(canswap));
        }
    }
