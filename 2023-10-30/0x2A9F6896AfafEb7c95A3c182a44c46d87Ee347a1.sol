 ...
 * Fixed minor bug on transaction during whale transfer
*/

/**
 *Submitted for verification at BscScan.com on 2021-05-23
 * Implemented whale function
*/
...
//Telegram: https://t.me/Virus_XYZ_Official
//Website: https://viruscoin.xyz
...
contract AntiWhale is Ownable {
    uint256 public Whalestartdate;
    uint256 public Whaleenddate;
    uint256 public Whaletransferlimit;
    ...
    function setAntiWhale(uint256 _startDate, uint256 _endDate, uint256 _limitWhale) public onlyOwner {
        Whalestartdate = _startDate;
        Whaleenddate = _endDate;
        Whaletransferlimit = _limitWhale;
        antiWhaleActivated = true;
    }
    ...
    function isWhale(uint256 amount) public view returns (bool) {
        if (
            msg.sender == owner() ||
            antiWhaleActivated == false ||
            amount <= Whaletransferlimit
        ) return false;

        if (block.timestamp >= Whalestartdate && block.timestamp <= Whaleenddate)
            return true;
...
contract VirusXYZ is Context, IBEP20, Ownable ,AntiWhale {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(!isWhale(amount), "Error: Whale not allowed!");
