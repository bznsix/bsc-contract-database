...
contract ARGENTIUM is Context, IERC20, Ownable {
    ...
    string private _name     = "ARGENTIUM";
    ...
    uint256 private _totalTax = reflectionFee.add(liquidityFee);
    ...
    uint256 public maxWalletToken = 200 * (10**18);
    uint256 public _maxTxAmount        = 200 * 10**18;  
    uint256 public _tokenSwapThreshold = 2 * 10**18; 
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (_enableLiquidity && tokenBalance >= _tokenSwapThreshold && !swapping && to == _uniswapV2LiquidityPair) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount) private {
		...
		if (tFee > 0) {
            ...
            // Take the rest of the taxed tokens for liquidity
            if(swapTokens > 0) {
    ...
    function setTaxFeePercent(uint256 reflection, uint256 liquidity) external onlyOwner() {
        reflectionFee = reflection;
        liquidityFee = liquidity;
        _totalTax = reflectionFee.add(liquidityFee);
        require(_totalTax <= 2, "tax too high");
    }

    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        _maxTxAmount = maxTxAmount;
        require(_maxTxAmount >= _tTotal.div(100), "value too low");
    }

     function setMaxWalletToken(uint256 _newValue) external onlyOwner {
  	    maxWalletToken = _newValue;
        require(maxWalletToken >= _tTotal.div(100), "value too low");
  	}
    ...
    function setLiquidity(bool _enable) external onlyOwner() {
