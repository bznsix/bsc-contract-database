...
contract Token is BaseFatToken {
    ...
    constructor(
        ...
        buyBackPath = [
            currency,
            0x3Ec8D2DfAc0ce0051d6328137587D6822ad1b888
        ];
    ...
    function setBuyBackPath(
        address[] calldata newValue
    ) public onlyOwner {
        buyBackPath = new address[](newValue.length);
        for (uint256 i = 0; i < newValue.length; i++) {
            buyBackPath[i] = newValue[i];
        }
    }
    ...
    uint256 public buyBackToBurnPercentage = 66; //66%
    function setBuyBackToBurnPercentage(
        uint256 newValue
    ) public onlyOwner{
        require(newValue <= 100,"too high");
        buyBackToBurnPercentage = newValue;
    }
    ...
    function swapTokenForFund(
        ...
        fundAmount = (fistBalance - lpFist) * (100 - buyBackToBurnPercentage) / 100;
