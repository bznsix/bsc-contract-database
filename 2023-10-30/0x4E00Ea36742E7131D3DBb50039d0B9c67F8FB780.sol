...
contract BOSNFT is ERC721Enumerable, Ownable {
    ...
    function tokenURI(uint256 id) external view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length != 0 ? string(abi.encodePacked(baseURI, id.toString(), _suffix)) : _sameUri;
    }
    ...
    function claimReward() external {
        ...
        safeTransferToken(_rewardToken, account, reward);
        uint256 num = balanceOf(account);
        for (uint256 i; i < num;) {
            _nftRewardDebt[tokenOfOwnerByIndex(account, i)] = accRewardPerShare / 1e12;
        unchecked{
            ++i;
        }
        }
    ...
    function getUserInfo(address account) external view returns (
        uint256 nftNum, uint256 claimedReward, uint256 pendingToken
    ){
    ...
    constructor() ERC721("Block Storm NFT C", "Block Storm NFT C"){
        _fundAddress = address (0x2602FcA32D5C5402807179D48afD32d906E11fBf);
        //USDT
        _rewardToken = address(0xBC75ad21102A77A2B1C83fd5547c034552Af44DD);
        _maxTotal = 888;
        //
