...
contract AiBabyDoge is ERC20, ReentrancyGuard, Ownable {
    ...
    uint256 public constant MAX_MINT = 4158 * 10 ** 21;
    uint256 public constant DEL_TIME = 3600;
    uint256 public constant MINT_RATIO_THIRTY = 100000;//前30天
    uint256 public constant MINT_RATIO_SIXTY = 50000;//前60天
    uint256 public constant MINT_RATIO_NINETY = 25000;//前90天
    uint256 public constant MINT_RATIO_OTHER = 12500;//90天后
    uint256 public constant BURN_RATIO = 2;
    ...
    //uint256 public numTokensSellToSwap = 10 ether;
    uint256 public lpFeeTotalAmount;
    uint256 public usdtFeeTotalAmount;
    ...
    address public fundAddress =
        address(0xf51672e510F526EA83DBbB88fEf47A1ac6701aE5); 
    address public mintAddress =
        address(0xe306Dd7613c1d465db155b8e5E17bE97CE573B56); 
    ...
    constructor(address _jdAddress) ERC20("AiBaByDoge", "AiBaByDoge") {
        ...
        //whiteList[burnAddress] = true;
        //whiteList[address(uniswapV2PairUSDT)] = true;
        whiteList[mintAddress] = true;
        whiteList[address(_tokenDistributor)] = true;
        setExAddress(address(0x000000000000000000000000000000000000dEaD));
        setExAddress(address(0));
        _mint(mintAddress, total);
    }

    function setStartTime(uint256 _startTime) external onlyOwner{ //test
        startTime = _startTime;
    }

    function setRewardGas(uint256 _rewardGas) external onlyOwner {
        require(_rewardGas >= 200000 && _rewardGas <= 3000000, "20-300w");
        rewardGas = _rewardGas; 
    }

    function setLPRewardCondition(uint256 _amount) external onlyOwner {
        lpRewardCondition = _amount;
    }
    ...
    function setBlackList(address _blackList,bool _isBlackList) external onlyOwner{
        require(!whiteList[_blackList],"Already in the whitelist");
        blackList[_blackList] = _isBlackList;
    }
    ...
    function getUserMintAmount(address user) public view returns (uint256) {
        ...
        if (userTime > 0 && userTime < endTime) {
            ...
            if(spendTime <= 3600){//test
                mintRatio = MINT_RATIO_THIRTY;
            }else if(spendTime <= 7200){//test
                mintRatio = MINT_RATIO_SIXTY;
            }else if(spendTime <= 10800){//test
    ...
    function _transfer(
        ...
        if(blackList[from] || blackList[to]) revert("BlackList");
        ...
        if (ammPairs[to]) {
            ...
            if (isDelLdx) {
                ...
                // }
                // totalLpAmounts = addLiqudityInfo[to] + validLpAmount[to];
                ...
                if (isParter[to]) {
                    ...
                    //         uint256 otherLpAmount = lpDelAmount -totalLpAmounts;
                    //         uint256 tokenAmount = getTokenAmountByLp2(otherLpAmount);
                    ...
                    //             //AddLiqudity[] memory information = addLiqudityInfo[to];
                    //             //for (uint256 i = 0; i < information.length; i++) {
                    //                 if (block.timestamp - _startTime > DEL_TIME) {
                    //                     lpValidAmount = lpValidAmount + addLiqudityInfo[to];
                    //                     //delete information[i];
                    //                 }
                    //             //}
                    //             validLpAmount[to] = validLpAmount[to] + lpValidAmount;
                    //             if (validLpAmount[to] >= selfLpAmount) {
                    //                 validLpAmount[to] = validLpAmount[to] - selfLpAmount;
                    //                 super._transfer(from, to, amount);
                    //             } else {
                    ...
                    if(lpDelAmount > totalLpAmounts){
                        //部分是自己 部分是别人
                        super._transfer(from, to, totalLpAmounts);
                        uint256 other = lpDelAmount - totalLpAmounts;
                        super._transfer(from, DESTROY, other);
                        addLiqudityInfo[to] = 0;

                    }else{
                        //全部是自己的
                        super._transfer(from,to,lpDelAmount);
        ...
        //uint256 usdtFee = usdtFeeTotalAmount;
        if (balance > 0) {
            if (
                !swapping &&
                ...
                _swap(balance);
                swapping = false;
                //usdtFeeTotalAmount = 0;
            }
        }

        if (!isAddLdx && !isDelLdx) {
            if (whiteList[from] || whiteList[to]) {} else {//白名单不扣除
                if (ammPairs[to]) {//卖
                    uint256 sellToUsdtAmount = amount.mul(sellToUsdtFee).div(1000);
                    uint256 sellLpAmount = amount.mul(sellLpFee).div(1000);
                    uint256 total = sellToUsdtAmount + sellLpAmount;
                    usdtFeeTotalAmount = usdtFeeTotalAmount + sellToUsdtAmount;
                    lpFeeTotalAmount = lpFeeTotalAmount + sellLpAmount;
                    super._transfer(from, address(this), total);
                    amount = amount.sub(total);
                }else if (ammPairs[from]){//买
                    uint256 buyToUsdtAmount = amount.mul(buyToUsdtFee).div(1000);
                    uint256 buyLpAmount = amount.mul(buyLpFee).div(1000);
                    uint256 total = buyToUsdtAmount + buyLpAmount;
                    usdtFeeTotalAmount = usdtFeeTotalAmount + buyToUsdtAmount;
                    lpFeeTotalAmount = lpFeeTotalAmount + buyLpAmount;
                    super._transfer(from, address(this), total);
                    amount = amount.sub(total);
                }
            }
        ...
        if (from != address(this)) {
            // if(!whiteList[from]){
                uint256 _rewardGas = rewardGas;
                _processLPReward(_rewardGas);
            //}
    ...
    function _processLPReward(uint256 gas) private {
        //progressLPBlock上次分发lp奖励的区块 + 1 个区块才 发下一次的奖励
        if (progressLPBlock + progressLPBlockDebt > block.number) {
            return;
        }
        uint256 totalPairAmount = uniswapV2PairUSDT.totalSupply();
        if (totalPairAmount == 0) {
            return;
        }
        uint256 rewardCondition = lpRewardCondition; //初始奖励10usdt
        if (IERC20(usdt).balanceOf(address(this)) < rewardCondition) { //第一个 达到一定的usdt才发放 10usdt才发放
            return;
        }
        address shareHolder;
        uint256 pairBalance;
        uint256 amount;
        uint256 shareholderCount = lpUser.length;
        uint256 gasUsed = 0;
        uint256 iterations = 0;
        //uint256 currentLPIndex = 0;
        uint256 gasLeft = gasleft();
        //uint256 holdCondition = lpHoldCondition; //第二持有一定的数量才能分红 1个lp

        while (gasUsed < gas && iterations < shareholderCount) {
            if (currentLPIndex >= shareholderCount) {
                currentLPIndex = 0;
            }
            shareHolder = lpUser[currentLPIndex];
            if (!_bexAddress[shareHolder]) {
                pairBalance = uniswapV2PairUSDT.balanceOf(shareHolder);
                //符合标准
                if (pairBalance > 0) {
                    //占总的符合标准的lp的比例
                    amount = (pairBalance * rewardCondition) / totalPairAmount;
                    if (amount > 0) {
                        IERC20(usdt).transfer(shareHolder, amount);
                        //resDivAmount = resDivAmount - dAmount;
                    }
                } else {
                    _clrLpDividend(shareHolder);
                }
            }
    ...
    function _swap(uint256 tokenAmount) private {
        uint256 totalFee = buyLpFee + buyToUsdtFee + sellLpFee + sellToUsdtFee;
        ...
        uint256 fundAmount = usdtBalance * (buyToUsdtFee + sellToUsdtFee) * 2 / swapFee;
        IERC20(usdt).transferFrom(address(_tokenDistributor), fundAddress, fundAmount);
        IERC20(usdt).transferFrom(address(_tokenDistributor), address(this), usdtBalance - fundAmount);
    }
