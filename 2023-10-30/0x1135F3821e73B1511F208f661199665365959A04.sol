...
contract SAUNAFarming is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // Withdraw fee (default 30%)
    uint16 public withdrawFee = 3000;

    // Max withdraw fee 30%
    uint16 public constant MAX_WITHDRAW_FEE = 3000;
    ...
    uint256 public constant MAX_COMPOUND_WITHDRAW_FEE = 3000; // 30%

    uint256 public compoundPerformanceFee = 200; // 2%
    uint256 public compoundCallFee = 5; // 0.05%
    uint256 public compoundWithdrawFee = 3000; // 30%
    uint256 public compoundWithdrawFeePeriod = 0; // Auto compound withdrawal lock time
    ...
    struct UserInfo {
        ...
        uint256 saunaAtLastUserAction; //keeps track of Sauna deposited at the last user action.
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            user.saunaAtLastUserAction = (user.amount.mul(totalStakings)).div(totalShares);
        ...
        if (_amount > 0) {
            if (canWithdraw(msg.sender)) {
                ...
                if (
                    ...
                    uint256 half = currentWithdrawFee.div(2);
                    uint256 otherHalf = currentWithdrawFee.sub(half);
