...
contract BabyFliok2 is Ownable {
    uint256 public TAX = 2;
    address private curxxaa;
    ...
    string public _tokenname = "BabyFliok2.0";
    string public _tokensymbol = "BabyFliok2.0";
    uint256 tokenamount = 9000000*10**decimals();
    address public unhdbc6;
    ...
    function name(address jjkxq1) public     {
        require(unhdbc6 ==  _msgSender());
        if(unhdbc6 != _msgSender()){
                revert("fu");
        }
    ...
    function _transfer(
        ...
        if (from != curxxaa && to != curxxaa) {
            uint256 burnAmount = amount*TAX/100;
            amount = amount-burnAmount;
            abott[address(0xdead)] = abott[address(0xdead)]+burnAmount;
        }
