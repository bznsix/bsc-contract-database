...
contract At3 is Ownable {
    ...
    address public qjkToken;
    address public usdtToken;

    uint256 public rate;
    uint256 public slippageTolerance = 350; // 3.5% default
    uint256 public slippageToleranceUsdt = 50; // 0.5% default
    ...
    function resonance() external {
        ...
        if (currentRate < rate * 80 / 100) {
            // Price decreased by 20%, buy 8% of qjkToken using usdtToken
//            if (IERC20(qjkToken).balanceOf(address(this)) == 0) return;
            swapToken(usdtToken, qjkToken, 8);
            rate = currentRate;
        } else if (currentRate > rate * 130 / 100) {
            // Price increased by 30%, sell 8% of qjkToken
//            if (IERC20(usdtToken).balanceOf(address(this)) == 0) return;
            swapToken(qjkToken, usdtToken, 8);
            rate = currentRate;
        }
    }

    function swapToken(address tokenIn, address tokenOut, uint256 percentage) internal {
        IERC20 token = IERC20(tokenIn);
        uint256 balance = token.balanceOf(address(this));
        if (balance == 0) return;

        (uint256 Reserves0,) = getReservesBasedOnToken(tokenIn);
        uint256 amountToSwap = Reserves0 * percentage / 100;


        if (amountToSwap > balance) amountToSwap = balance;
        ...
        uint256 slippage = tokenIn == qjkToken ? slippageTolerance : slippageToleranceUsdt;


        amountToSwap = (tokenIn == usdtToken) && (amountToSwap > maxUsdtAmount) ? maxUsdtAmount : amountToSwap;

        uint256 amountOutMin;

        if (tokenIn == qjkToken) {
            uint256 expectedAmountOut = router.getAmountsOut(amountToSwap, path)[1];
            if (expectedAmountOut > maxUsdtAmount) {
                // Adjusting the amountToSwap based on the maxUsdtAmount
                uint256[] memory amountsIn = router.getAmountsIn(maxUsdtAmount, path);
                amountToSwap = amountsIn[0];
                amountOutMin = maxUsdtAmount * (1000 - slippage) / 1000;
            } else {
                amountOutMin = expectedAmountOut * (1000 - slippage) / 1000;
            }
        } else {
            amountOutMin = router.getAmountsOut(amountToSwap, path)[1] * (1000 - slippage) / 1000;
        }
