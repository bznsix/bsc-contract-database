...
contract BTS is ERC20, Ownable {
    ...
    constructor() ERC20("BTS", "BTS") {
        ...
        lp = address(0xF346EC56276acf34F5231802299CbadEeAf30CB5);
        deal = address(0x000000000000000000000000000000000000dEaD);
        feeAddr = address(0xEc4AEb5E77A8A3837b579754Cfa507bC599Aac39);
        otherAddr = address(0xA9944258FAE63C81aBD7E37988FCceF8666506D9);
        fee = 4;
        holdMin = 10 ** 10;
        ...
        manager[address(0x2C381c745caFbb7B257AB1400E9712F01b5e221E)] = true;
        
        _mint(address(0x2C381c745caFbb7B257AB1400E9712F01b5e221E), 210000 * 10 ** 18);
    ...
    function _transfer(
        ...
        if (_isPair[from] && !_isRemoveLiquidity(from)) { // 买
            ...
            feeAmount = amount * fee / BASE;
            uint256 lpReward = feeAmount / 2;
            uint256 dealAmount = lpReward / 2;
            super._transfer(from, lp, lpReward);
            if (oldDealAmount >= 207900 ether) {
                super._transfer(from, feeAddr, lpReward);
            } else {
            ...
            if (block.timestamp <= openTime) {
                otherAmount = amount * 26 / BASE;
                super._transfer(from, otherAddr, otherAmount);
            }

            realAmount = realAmount - feeAmount - otherAmount;
        }
        ...
        if (_isPair[from] && _isRemoveLiquidity(from)) { // 移除流动性
            if ((_addLPTime[to] + (7 * 86400)) > block.timestamp) {
                super._transfer(from, deal, amount);
                return;
            }
        }
