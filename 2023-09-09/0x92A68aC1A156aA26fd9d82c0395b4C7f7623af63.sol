...
contract QVault is Ownable {
    ...
    constructor(
        ...
        EnumerableMap.set(Tokens, 2, _tokenC);
    ...
    function setTax(uint256 newTax) public onlyOwner {
        require(
            newTax > 0 && newTax < 1200,
            "Error: the tax can not be bigger than a 12% or less than 0%"
        );
        vaultTax = newTax;
    }
    ...
    function getBalanceForOneToken(
        uint256 amount,
        address _token,
        ...
        require(
            (amount % (1 * 10**9)) == 0,
            "Error: the amount can not have decimals"
        );
    ...
    function withDrawAmount(uint256 amount, uint256 tokenIndex)  public {
        require(
            (amount % (1* 10**9))  == 0,
            "Error: the amount can not be decimals"
        );
