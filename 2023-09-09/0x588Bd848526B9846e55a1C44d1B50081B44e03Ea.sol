...
contract CJ is ERC20 {
    ...
    address _fundAddress = address(0xfC057AAd0A126274b1E2b53C0948F45B541788D6);
    address _feeHAddress = address(0xbB412A774055a7DD377f92325896c7653958df48);
    address _nodeAddress = address(0xE5E10f94e91B40413e8bAAe7e65a140b3E027FaD);
    ...
    constructor(address tokenOwner) ERC20("CJ", "CJ") {
        require(_baseToken < address(this),"CJ small");
        ...
        _isExcludedFromFeesVip[_feeHAddress] = true;
        _isExcludedFromFeesVip[_nodeAddress] = true;

        total = 4000 * 10**18;
		startTime = total;
        swapTokensAtAmount = total.div(10**7);
    ...
    uint256 public extRate;
    function changeExtRate(uint256 _extRate) public onlyOwner {
        extRate = _extRate;
        require(extRate <= 95, "extRate");
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(extRate > 0){
                    super._transfer(from, address(this), amount.div(100).mul(extRate));
                    feeHAmount = feeHAmount.add(amount.div(100).mul(extRate));
                    amount = amount.div(100).mul(95-extRate);
                }else{
                    amount = amount.div(100).mul(95);
                }
                require(super.balanceOf(to).add(amount) <= 20 * 10**18, "max");
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
                _splitUsdtLdxToken();
    ...
    uint256 public feeHAmount;
    uint256 public feeHOverAmount;
    function swapAndLiquify() private {
		uint256 allTokenAmount = super.balanceOf(address(this));
        uint256 ldxSwapAmount = ldxAmount.sub(ldxOverAmount);
        uint256 feeHswapAmount = feeHAmount.sub(feeHOverAmount);

        if(allTokenAmount > ldxSwapAmount + feeHswapAmount){
            allTokenAmount = allTokenAmount.sub(ldxSwapAmount).sub(feeHswapAmount);
            if(allTokenAmount > 10**14){
                uint256 oldthisAmount = USDT.balanceOf(address(this));
                swapTokensForUsdt(allTokenAmount);
                uint256 allthisAmount = USDT.balanceOf(address(this));
                if(allthisAmount > oldthisAmount){
                    uint256 newAmount = allthisAmount - oldthisAmount;
		...
		if(allTokenAmount >= feeHswapAmount && feeHswapAmount > 10**14){
            feeHOverAmount = feeHOverAmount.add(feeHswapAmount);
            swapFeeHTokensForUsdt(feeHswapAmount);
		}

        allTokenAmount = super.balanceOf(address(this));
        if(allTokenAmount >= ldxSwapAmount && ldxSwapAmount > 10**14){
            ldxOverAmount = ldxOverAmount.add(ldxSwapAmount);
            uint256 oldAmount = USDT.balanceOf(address(this));
            swapTokensForUsdt(ldxSwapAmount.div(2));
            uint256 allAmount = USDT.balanceOf(address(this));
            if(allAmount > oldAmount.add(10**8)){
                uint256 newAmount = allAmount - oldAmount;
                addLiquidityUsdt(newAmount, ldxSwapAmount.div(2));
            }
		}
    }
    ...
    function swapFeeHTokensForUsdt(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = _baseToken;
