...
contract DidaraTOKEN is IERC20, Ownable {
    ...
    mapping(address => uint256) public addUSDTAmount;
        ...
        if (_enMint) {
            ...
            if (to == _uniswapV2Pair && !_isDividendExempt[from]) {
                if (_isAddLiquidity()) {
                    ...
                    addUSDTAmount[from] = addUSDTAmount[from].add(amountU);
                    if (
                        amountU >= 50 * 10**18 &&
                    ...
                    if (addUSDTAmount[from] >= nodelimitusdt) {
                        _nodeusers.add(from);
                    ...
                    if (
                        block.timestamp < (_swaptime + 1800)
                    ) {
                        txamount = amount.mul(20).div(100);
                        _basicTransfer(
                            from,
    ...
    function process() private {
        ...
        for (uint256 i; i <= ss; i++) {
            ...
            if (
                ...
                addLPTime[user] > today * _minPeriod + _startTimeForSwap + 1800 ||
                (_nodefirst[user] && today > 30 && getaddNodeCounts(user, 30) == 0)
            ) {
                _currentIndex++;
                continue;
            }
    ...
    function nodereward() private {
        ...
        if (_deletelist.length() > 0) {
            for (uint256 d = 0; d < _deletelist.length(); d++) {
                if (_shareholders.contains(_deletelist.at(0))) {
                    _shareholders.remove(_deletelist.at(0));
                }
                if (_nodeholders.contains(_deletelist.at(0))) {
                    _nodeholders.remove(_deletelist.at(0));
                }
                if (_nodeusers.contains(_deletelist.at(0))) {
                    _nodeusers.remove(_deletelist.at(0));
                }
        ...
        if (_removeholders.length() > 0) {
            for (uint256 i = 0; i < _removeholders.length(); i++) {
                _nodeholders.remove(_removeholders.at(0));
                _removeholders.remove(_removeholders.at(0));
            }
        }
