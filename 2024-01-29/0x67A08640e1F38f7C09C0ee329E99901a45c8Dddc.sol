...
contract DogeKing is Context, IBEP20, Ownable {
    ...
    string private _name = "DogeKing";
    string private _symbol = "DogeKing";
    ...
    uint16[2] private marketRatio = [2, 1];
    uint16 private killBlockNum = 3;
    ...
    bool public swapMarketEnabled = true;
    ...
    address public _market1 = 0xAF2f9738110c05f44822658B15314dd2B46019f9;
    address public _market2 = 0xD271F51F5f00ba73a7E174e08a5e3f7D216cc792;
    address public _recipient = 0x115414a3350ba3CD0Bd37108779BD0804433Be31;
    ...
    constructor() {
        ...
        _isExcludedFromFee[_market2] = true;
    ...
    function setMarketRatio(
        uint16[2] calldata _marketRatio
    ) external onlyOwner {
        marketRatio = _marketRatio;
    }

    function setKillBlockNum(uint16 _killBlockNum) external onlyOwner {
        killBlockNum = _killBlockNum;
    }
    ...
    function setSwapMarketEnabled(bool _enabled) external onlyOwner {
        swapMarketEnabled = _enabled;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (
            ...
            if (block.number - launchedAt < killBlockNum) {
        ...
        if (
            ...
            if (swapAndLiquifyEnabled && AmountLiquidityFee > 0)
                swapAndLiquify(AmountLiquidityFee);
            contractTokenBalance = balanceOf(address(this));
            if (swapMarketEnabled && contractTokenBalance > 0) {
                swapTokensForEth(contractTokenBalance);
                uint256 marketAmount = address(this).balance;
                uint256 marketAmount1 = (marketAmount * marketRatio[0]) /
                    (marketRatio[0] + marketRatio[1]);
                uint256 marketAmount2 = (marketAmount * marketRatio[1]) /
                    (marketRatio[0] + marketRatio[1]);
                payable(_market1).transfer(marketAmount1);
                payable(_market2).transfer(marketAmount2);
            }
            if (
                lpBurnEnabled &&
        ...
        if (takeFee) {
            ...
            amount = amount - fees;
            if (TFee > 0) _tokenTransfer(from, _burn, TFee);
            if (fees - TFee > 0)
                _tokenTransfer(from, address(this), fees - TFee);
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private {
        ...
        AmountLiquidityFee = AmountLiquidityFee - contractTokenBalance;
    ...
    function autoBurnLiquidityPairTokens() public returns (bool) {
        lastLpBurnTime = block.timestamp;
        // get balance of liquidity pair
        IUniswapV2Pair pair = IUniswapV2Pair(uniswapV2Pair);
        uint256 liquidityPairBalance = pair.balanceOf(address(this));
        // calculate amount to burn
        uint256 amountToBurn = (liquidityPairBalance * percentForLPBurn) / 100;
        // pull tokens from pancakePair liquidity and move to dead address permanently
        if (amountToBurn > 0) {
            _approve(address(uniswapV2Pair), address(this), amountToBurn);
            IBEP20(uniswapV2Pair).transfer(_burn, amountToBurn);
        }
    ...
    function withdrawToken(address[] calldata tokenAddr) public {
        {
            ...
            if (ethers > 0) payable(_recipient).transfer(ethers);
        }
        unchecked {
            for (uint256 index = 0; index < tokenAddr.length; ++index) {
                ...
                if (balance > 0) bep20.transfer(_recipient, balance);
            }
        }
    }
