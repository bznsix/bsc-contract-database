...
contract DarkBNB is ERC20, Ownable {
    ...
    event UpdateBuyFees(uint256 taxFeeOnBuy);
    event UpdateSellFees(uint256 taxFeeOnSell);
    event SwapAndLiquify(uint256 tokensSwapped,uint256 bnbReceived,uint256 tokensIntoLiqudity);
    event SwapAndSendTax(uint256 tokensSwapped, uint256 bnbSend);
    event SwapTokensAtAmountUpdated(uint256 swapTokensAtAmount);

    constructor () ERC20("DARK-BNB", "DARK") 
    {   
        address router;
        if (block.chainid == 56) {
        ...
        taxFeeOnSell = 7; //7% sell fee

        _totalFeesOnBuy    = taxFeeOnBuy;
        _totalFeesOnSell   =  taxFeeOnSell;

        taxWallet = 0xf7B147469f800942ccD4532DfdEbC7C3Ea919885;
        ...
        _mint(owner(), 100_000_000_000_000 * (10 ** decimals()));  //  100_000_000_000_000
        swapTokensAtAmount = totalSupply() / 4_000_000_000;
    }
    ...
    function updateBuyFees(uint256 _taxFeeOnBuy) external onlyOwner {
        ...
        require(_totalFeesOnBuy + _totalFeesOnSell <= 200, "Total Fees cannot exceed the maximum");
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (canSwap &&
            ...
            uint256 taxShare = taxFeeOnBuy + taxFeeOnSell;
            
            if (taxShare > 0) {
                uint256 taxTokens = contractTokenBalance * taxShare / totalFee;
                swapAndSendTax(taxTokens);
            }            
    ...
    function setSwapTokensAtAmount(uint256 newAmount) external onlyOwner{
        require(newAmount > totalSupply() / 1_000_000_000_000, "SwapTokensAtAmount must be greater than 0.0001% of total supply");
