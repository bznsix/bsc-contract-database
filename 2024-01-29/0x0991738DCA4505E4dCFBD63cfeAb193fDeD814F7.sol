...
contract HangingTrailsToken is ERC20, Ownable, Withdrawable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address payable public communityWallet =
        payable(0x6FA6DA462CBA635b0193809332387cDC25Df3e8D);

    uint256 public communityFeeOnBuy;
    uint256 public communityFeeOnSell;
    ...
    constructor() ERC20("Hanging Trails", "TRAILS") {
        communityFeeOnBuy = 5;
        communityFeeOnSell = 7;

        communityShare = communityFeeOnBuy + communityFeeOnSell;

        totalFeesOnBuy = communityFeeOnBuy;
        totalFeesOnSell = communityFeeOnSell;
    ...
    function updateCommunityWallet(
        ...
        require(
            newCommunityWallet != communityWallet,
            "The community wallet is already this address"
        );
        excludeFromFees(newCommunityWallet, true);
        excludeFromFees(communityWallet, false);
        emit CommunityWalletUpdated(newCommunityWallet, communityWallet);
        communityWallet = payable(newCommunityWallet);
    }
    ...
    function collectFees() external onlyOwner {
        ...
        uint256 tokensForCommunity = tokenAmount.mul(communityShare).div(
            totalShares
        );
        if (tokensForCommunity > 0) {
            transferNativeToCommunityWallet(tokensForCommunity);
        }
    }

    function transferNativeToCommunityWallet(uint256 amount) private {
