...
contract tokenGold is Context, IERC20, Ownable {
    ...
    string private _name = "tesGOLD";
    string private _symbol = "tesGOLD";
    ...
    constructor() {
        ...
        _questToken = questToken(0xbe20bc3b18Ca2591Ad487f153dc612f848065348); // Main token address
    ...
    function burnToHolder(uint256 amount,address _invitation) external {
        ...
        path[0] = address(_questToken);
        path[1] = uniswapRouter.WETH();
        uint256 deserved = 0;
        deserved = uniswapRouter.getAmountsOut(amount, path)[path.length - 1];
        require(payable(address(_questToken)).balance>=deserved,'not enough balance');
        _questToken.burnToholder(sender, amount, deserved);
        _QuestTokenToDead(sender,amount);
    ...
    function _QuestTokenToDead(address to,uint256 amountIn) private {
        address _addr = address(_questToken);
