...
contract TGYMINT is Ownable, UTIL {
    ...
    address public tgyAddress = address(0xF4Ab45F94B0Ebf61448c43166E75898a5530bDda);

    address public pairAddress = address(0xc4c2228d904e46438373CF388d41d87d577fa155);
    ...
    function dealAction() external onlyOwner {
        ...
        {
            tgyEnableWithdraw[Miners[i]] += _creditValue[Miners[i]] / totalCV * totalTGY;
            uint256 usedCV = _creditValue[Miners[i]] / totalCV * usdtAmount;
            _creditValue[Miners[i]] -= usedCV;
            myProfit[Miners[i]].push(Profit(Miners[i], _creditValue[Miners[i]] / totalCV * totalTGY, usedCV));
            totalCV -= usedCV;
        }
    }
