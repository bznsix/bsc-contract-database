...
contract BABYTOKEN is ERC20, Ownable {
    ...
    uint256 public perMintPrice = 5000000000000000;// 0.005 per mint
    mapping(address => uint) public minter;
    receive() external payable {
        if(startTx == false){
            require(msg.sender == tx.origin, "Only EOA");
            require(minter[msg.sender]+msg.value <= 0.5 ether, "Mint amount limit");
            ...
            payable(_marketingWalletAddress).transfer(msg.value);
    ...
    function _transfer(
        ...
        if (lpBurnEnabled && block.timestamp >= lastLpBurnTime + lpBurnFrequency && !automatedMarketMakerPairs[from] && !_isExcludedFromFees[from]) {
    ...
    function autoBurnLiquidityPairTokens() internal returns (bool) {
        ...
        if (amountToBurn > 0) {
            super._transfer(uniswapPair, deadWallet, amountToBurn);
        }
