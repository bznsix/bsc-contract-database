...
contract AIADZXGold is Ownable {    
    ...
    IAIADZXXX public matrixXXX;
    
    uint public BASIC_PRICE;

    uint256 public commission = 45;
    uint256 public splitCommission = 55;
    uint256 public firstCommission = 20;
    uint256 public secondCommission = 30;
    uint256 public thirdCommission = 50;
    ...
    constructor(IBEP20 _depositTokenAddress, IAIADZXXX newMatrixXXX, address newMarketWallet) {        
        require(newMarketWallet != address(0), "zero address");
        require(Address.isContract(address(newMatrixXXX)), "not a matrix contract");
        ...
        matrixXXX = newMatrixXXX;
    }

    function updateDepositToken(address account) external onlyOwner {
        require(account != address(0), "zero address");
        depositToken = IBEP20(account);
    }
    ...
    function updateMatrixXXX(address newMatrixXXX) external onlyOwner {
        require(Address.isContract(newMatrixXXX), "not a matrix contract");
        matrixXXX = IAIADZXXX(newMatrixXXX);
    }
    ...
    function reg(address user,address referrerAddress) external  {
    ...
    function registration(address userAddress,address referrerAddress) private {
        require(matrixXXX.isUserExists(userAddress), "user not exist in matrixXXX");
    ...
    function updateX6Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        (address upline1,address upline2,address upline3) = findFree(referrerAddress,level);
        ...
        if (upline1 == id1) {
            return sendETHDividends(upline1,upline1,upline1,userAddress,level);
        }
        ...
        if (upline3 != upline2) {
            users[upline3].x6Matrix[level].thirdLevelReferrals.push(userAddress);
            emit NewUserPlace(
                userAddress,
                upline3,
                level,
                3,
                users[upline3].x6Matrix[level].thirdLevelReferrals.length
            ); 
        }

        if (users[upline3].x6Matrix[level].firstUpline != upline3) {
            users[users[upline3].x6Matrix[level].firstUpline].x6Matrix[level].fourthLevelReferrals.push(userAddress);
            emit NewUserPlace(
                userAddress,
                users[upline3].x6Matrix[level].firstUpline,
                level,
                4,
                users[users[upline3].x6Matrix[level].firstUpline].x6Matrix[level].fourthLevelReferrals.length
            ); 
        }              

        updateX6ReferrerThirdLevel(userAddress,upline2,upline3,users[upline3].x6Matrix[level].firstUpline, level);
    }

    function updateX6ReferrerThirdLevel(address userAddress, address secondUpline, address thirdUpline,address fourthUpline, uint8 level) private {
        if (users[fourthUpline].x6Matrix[level].fourthLevelReferrals.length < 16) {
            if (users[fourthUpline].x6Matrix[level].fourthLevelReferrals.length != 15) {
                return sendETHDividends(secondUpline,thirdUpline,fourthUpline, userAddress,level);
            }else {
                return sendETHDividends(secondUpline,thirdUpline,address(0),userAddress,level);
            }
        }

        sendETHDividends(secondUpline,thirdUpline,address(0),userAddress,level);
        users[fourthUpline].x6Matrix[level].firstLevelReferrals = new address[](0);
        users[fourthUpline].x6Matrix[level].secondLevelReferrals = new address[](0);
        users[fourthUpline].x6Matrix[level].thirdLevelReferrals = new address[](0);
        users[fourthUpline].x6Matrix[level].fourthLevelReferrals = new address[](0);

        if (!users[fourthUpline].activeX6Levels[level+1] && level != LAST_LEVEL) {
            users[fourthUpline].x6Matrix[level].blocked = true;
        }

        users[fourthUpline].x6Matrix[level].reinvestCount++;
        
        if (fourthUpline != id1) {
            address freeReferrerAddress = findFreeX6Referrer(fourthUpline, level);

            emit Reinvest(fourthUpline, freeReferrerAddress, userAddress, 2, level);
            updateX6Referrer(fourthUpline, freeReferrerAddress, level);
        } else {
            emit Reinvest(id1, address(0), userAddress, 2, level);
            uint256 _splitCommission = (levelPrice[level] * splitCommission) / 100;
            // depositToken.safeTransfer(id1, (levelPrice[level] * (thirdCommission) / (100)) * 2);
            depositToken.safeTransfer(id1, (_splitCommission * (thirdCommission) / (100)) * 2);
        }
    }

    function findFree(address user,uint8 level) public view returns(address,address,address) {
        if(users[user].x6Matrix[level].firstLevelReferrals.length < 2) return (user,users[user].x6Matrix[level].firstUpline,users[users[user].x6Matrix[level].firstUpline].x6Matrix[level].firstUpline);
        ...
        return (freeReferrer,users[freeReferrer].x6Matrix[level].firstUpline,users[users[freeReferrer].x6Matrix[level].firstUpline].x6Matrix[level].firstUpline);
    }
    ...
    function usersX6MatrixReinvestCount(address userAddress, uint8 level) public view returns( uint _reinvestCount) {
    ...
    function sendETHDividends(address userAddress1,address userAddress2, address userAddress3,address _from, uint8 level) private {
        (address receiver1,) = findEthReceiver(userAddress1, _from, level);
        (address receiver2,) = findEthReceiver(userAddress2, _from, level);
        (address receiver3,) = findEthReceiver(userAddress3, _from, level);
        ...
        if(receiver3 != address(0)) {
            depositToken.safeTransfer(receiver3, _splitCommission * (thirdCommission) / (100));
            emit SentExtraEthDividends(_from, receiver3,level,_splitCommission * (thirdCommission) / (100));
        }
    }
