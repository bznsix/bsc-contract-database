...
contract PantheraiStaking is Ownable, ReentrancyGuard, Initializable {
    ...
    require(_users[user_].stakeAmount == 0, "TokenStaking: user has already staked");
    require(_amount <= 20000000000000000000000000, "TokenStaking: maximum stake amount per user is 20 Million");
    ...
    require(getCurrentTime() >= _users[user].lastStakeTime + 365 days, "TokenStaking: cannot unstake before 1 year");
    ...
    function updateAPYRate(uint256 newApyRate) external onlyOwner {
    require(newApyRate <= 10000, "TokenStaking: APY rate should be less than or equal to 10000");
    _apyRate = newApyRate;
    }
}