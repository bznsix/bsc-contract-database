...
contract StakePool is Ownable, ReentrancyGuard {
    ...
    constructor( IERC20 _stakedToken,uint256 _lockPeriod) public {
        stakedToken = _stakedToken;
        lockPeriod = _lockPeriod;
    }

    function getStakeInfo(address userAddress) public view returns (uint256, uint256) {
        UserInfo storage user = userInfo[userAddress];
        return (user.amount, user.timestamp);
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        emit Deposit(msg.sender, _amount, user.amount);
    }
    ...
    function withdraw() external nonReentrant {
        ...
        require(block.timestamp >= user.timestamp + lockPeriod, "Lock period not yet over");
