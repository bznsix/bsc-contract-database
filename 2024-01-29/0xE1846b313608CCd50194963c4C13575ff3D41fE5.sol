...
contract BabyGEC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BabyGECDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BabyGEC", "BABYGEC") {
        marketingWallet = payable(0xf4E3DE5A2a08B8c23975d0FE41f0317f39BdC3a0);
        devWallet = payable(0xf4E3DE5A2a08B8c23975d0FE41f0317f39BdC3a0);
        ...
        dividendTracker = new BabyGECDividendTracker(
            payable(this),
            router,
            0x93891A3328Cc16ebD59474ced882b1D91Dec63E1, // reward token GreenEnvCoalition (GEC)
            "BabyGECTRACKER",
            "BabyGECTRACKER"
        );
        ...
        uint256 totalTokenSupply = (210000000) * (10**18);
...
contract BabyGECDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BabyGEC public BabyGECContract;
    ...
    constructor(
        ...
        defaultToken = token;
        BabyGECContract = BabyGEC(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BabyGEC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BabyGEC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyGEC contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BabyGECContract);

            uint256 prevBalance = BabyGECContract.balanceOf(address(this));
            ...
            {
                uint256 received = BabyGECContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BabyGECContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
