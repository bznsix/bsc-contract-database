...
contract BOSNFT is ERC721Enumerable, Ownable {
    ...
    function mint() external {
        ...
        require(
            tx.origin == account,
            "Only external user can call this function"
        );

        require(!hasMinted[account], "You have already minted the NFT");

        uint256 userLevel = MintPool(_mintPoolAddress).getUserNFTLevel(account);
        ...
        returns (string memory)
    {
        ...
        return
            bytes(baseURI).length != 0
                ? string(abi.encodePacked(baseURI, id.toString(), _suffix))
                : _sameUri;
    }
    ...
    function setBaseUri(string memory uri, string memory suffix)
        external
        onlyWhiteList
    {
        _baseUri = uri;
    ...
    function claimToken(
        ...
        require(tokenContract.balanceOf(address(this)) >= amount, "PTNE");
    ...
    function setRewardToken(address rewardToken) external onlyWhiteList {
    ...
    function addTokenReward(uint256 rewardAmount) external {
        ...
        require(_rewardAdmin[caller], "rqa");
    ...
    function claimReward() external {
        ...
        require(token.balanceOf(address(this)) >= reward, "PTNE");
        token.transfer(account, reward);

        uint256 num = balanceOf(account);
        for (uint256 i; i < num; ) {
            _nftRewardDebt[tokenOfOwnerByIndex(account, i)] =
                accRewardPerShare /
                1e12;
            unchecked {
                ++i;
        ...
        _claimedReward[account] += reward;
    }

    function pendingNFTReward(uint256 nftId)
        public
        view
    ...
    constructor() ERC721("Block Storm NFT", "BOS NFT") {
        _fundAddress = address(0x3226750136a5D53c93dBFd5d652042f14B82b1f4);
        //CUSD
        _rewardToken = address(0x4145553a65276093CffB8522FBC5d0002a605b5F);
        _maxTotal = 888;
        //
