...
import {IBrewlabsRouter} from "./interfaces/IBrewlabsRouter.sol";
...
contract BrewlabsRouter is IBrewlabsRouter {
    ...
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        ...
        _safeTransferFrom(token, address(this), to, IERC20(token).balanceOf(address(this)));
        IWETH(WETH).withdraw(amountETH);
        _safeTransferETH(to, amountETH);
    }
    ...
    function swapExactTokensForTokens(
        ...
        uint256 discount = getNFTDiscount(to);
        ...
        _safeTransferFrom(path[0], msg.sender, BrewlabsLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        uint256 discount = getNFTDiscount(_to);
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = BrewlabsLibrary.sortTokens(input, output);
            ...
            uint256 amountOutput;
            {
                ...
                uint256 amountInput = IERC20(input).balanceOf(address(pair)) - reserve0;
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(IBrewlabsFactory(factory).isAllowedToSwap(msg.sender), "BrewlabsRouter: NOT_ALLOWED_TO_SWAP");
        _safeTransferFrom(path[0], msg.sender, BrewlabsLibrary.pairFor(factory, path[0], path[1]), amountIn);
    ...
    function getNFTDiscount(address addr) internal view returns (uint256 discount) {
        if (IBrewlabsFactory(factory).isWhitelisted(addr)) return 10000;
...
import {IBrewlabsRouter01} from "./IBrewlabsRouter01.sol";
