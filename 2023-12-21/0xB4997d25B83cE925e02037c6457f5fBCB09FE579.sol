...
contract WonderForexToken is ERC20, ERC20Detailed{
    IERC20 public USDT;
    constructor()
    ERC20Detailed(" Wonder Forex Token ", "WFT",18)
    { _mint(msg.sender, 20000000 * (10**18)); }
    event TokensMultisent(address indexed contributor, uint256 amount);
    ...
    external
    {
        ...
        for (i; i < _contributors.length; i++) {
            ...
            emit TokensMultisent(_contributors[i], __balances[i]);
        }
    }

    function setTokenAddress(address _usdt)
    external 
    onlyOwner
    {
    ...
    function BuyviaUSDT(uint256 amount)
    external
    {
        require(amount > 0, "Select amount first");
    ...
    function rescueEth(uint256 _amount)
    external 
    onlyOwner
    { 
    ...
    function rescueTokens(address _token)
    external
    onlyOwner
    {
