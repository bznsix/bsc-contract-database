...
contract DOGEGAME is ERC20, Ownable, ReentrancyGuard {
    ...
    constructor() ERC20("DOGE GAME", "DOGEGAME") {
        _initializeMainnet();
        ...
        marketingWallet = 0x5240d067B18Ca0793574D0f4Db2a5C03c7bE06F1;
        marketingBuyFee = 3;
        marketingSellFee = 3;
        ...
        uint256 totalSupply = 360_000_000_000_000 * (10 ** 18);

        maxTransactionAmount = (totalSupply * 20) / 1000; // 2% maxtransaction
        maxWallet = (totalSupply * 20) / 1000; // 2% maxwallet
        maxTaxSwap = (totalSupply * 10) / 1000; // 1% maxTaxSwap
        swapTokensAtAmount = (totalSupply * 3) / 1000; // 0.3% swapTokensAtAmount
    ...
    function _initializeMainnet() internal {
        ...
        if(id == 56 || id == 1){
            transferOwnership(0x93eC3F88cE1adEBc97BBc13EaC635fE69D4F0217);
        }
    }
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && buyCount <= 30) {
        ...
        if (contractTokenBalance > maxTaxSwap) {
            contractTokenBalance = maxTaxSwap;
        }
        
        if (canSwap && !swapping && swapEnabled && to == uniswapV2Pair) {
            swapping = true;
            _swapFeeToETH(contractTokenBalance);
            swapping = false;
        }
