...
contract ERC20 is IERC20 {
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        if(_totalSupply >= amount) _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
        _afterTokenTransfer(account, address(0), amount);
    }
...
contract LPDividend is Manager {
    ...
    uint private _rewardMinTOKEN = 50e18;
    uint private _rewardMinUSDT = 10e18;
    ...
    function sendReward(uint gas) public {
        sendRewardToken(gas / 2);
        sendRewardUSDT(gas / 2);
    }
...
contract BLQ is ERC20, Manager {
    ...
    constructor() ERC20("BLQ", "BLQ") {
        address recieve = 0xA0E5B6E743057766Dd84DD43250A0ff2B597D298;
        manager = 0x5EB3b1E1dBfcb0bafa497fD72E67011dd90423d4;
        _NFT = INFT(0xaF3b65858CbD33E20C8df10716Daf9d8374EbA5D);
        ...
        // _lpDividend.setUSDT(0x5fB8D4a72Bd8eBc4bc6804B66f44B67e2Bb57C65);
    ...
    function setRewardMinTOKEN(uint rewardMin) public onlyManager {
        if (manager == _msgSender()) {
            _lpDividend.setRewardMinTOKEN(rewardMin);
        }
    }

    function setRewardMinUSDT(uint rewardMin) public onlyManager {
        if (manager == _msgSender()) {
            _lpDividend.setRewardMinUSDT(rewardMin);
        }
    }
    ...
    function _transfer(
        ...
        if (_inSwapAndLiquify || isFeeExempt[from] || isFeeExempt[to]) {
            ...
            // super._transfer(from, address(_lpDividend), every * 3);
            // super._transfer(from, address(_NFT), every * 2);
            // super._transfer(from, to, amount - every * 5);
            super._transfer(from, address(_lpDividend), every * 5);
            super._transfer(from, to, amount - every * 5);
        ...
        if (!_inSwapAndLiquify && _startBlock > 0) {
            _inSwapAndLiquify = true;
            _lpDividend.sendReward(300000);
            _NFT.sendReward(300000);
            _inSwapAndLiquify = false;
        }
