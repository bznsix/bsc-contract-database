...
contract Marketplace is IMarketplace, Ownable, ERC721Holder {
    ...
    * @dev allow buy NFT by flex
    ...
    function pay(address payToken, uint256 payValueTokenWithDecimal) internal {
        require(
            NovaXERC20(payToken).balanceOf(msg.sender) >= payValueTokenWithDecimal,
            "MARKETPLACE: Not enough balance to buy NFTs"
        );
        require(
            NovaXERC20(payToken).allowance(msg.sender, address(this)) >= payValueTokenWithDecimal,
            "MARKETPLACE: Must approve first"
        );
        require(
            NovaXERC20(payToken).transferFrom(msg.sender, saleWallet, payValueTokenWithDecimal),
    ...
    function buyByTokenAndCurrency(
        ...
        uint index;
        for (index = 0; index < _nftIds.length; index++) {
            try NovaXNFT(nft).safeTransferFrom(address(this), msg.sender, nftIds[index]) {
