...
contract BriseStakingPool is ReentrancyGuard, IStakingPool {
    ...
    function setExcludeFromLimit (address _address) external onlyOwner {
        _isExcludeFromLimit[_address] = true;
    }

    function setMinMaxValue (uint256 _newMinLimit, uint256 _newMaxLimit) external onlyOwner {
        require(_newMaxLimit > _newMaxLimit, "error");
    ...
    function stake(
        ...
        if (!_isExcludeFromLimit[msg.sender]){
            require(_amount < stakeMAXLimitAmount && _amount > stakeMINLimitAmount, "bigger than limit amount");
        }
    ...
    function unstake(uint256 _amount, uint256 _stakeNumber) external nonReentrant {
        ...
        require(_amount <= userStakingInfo[msg.sender][_stakeNumber].stakedAmount, "BriseStakingPool::unstake: not enough balance");
    ...
    function initializeStaking(
        ...
        require(_startsAt > block.timestamp, "BriseStakingPool::initializeStaking: _startsAt must be in the future");
        require(_rewardsDuration > 0, "BriseStakingPool::initializeStaking: _rewardsDuration = 0");
        require(_amount > 0, "BriseStakingPool::initializeStaking: _amount = 0");
        require(startsAt == 0, "BriseStakingPool::initializeStaking: staking already started");
    ...
    function resumeStaking() external onlyOwner {
        require(rewardRatePerSec > 0, "BriseStakingPool::startStaking: reward rate = 0");
    ...
    modifier inProgress() {
        ...
        require(endsAt > block.timestamp, "BriseStakingPool::notFinished: staking has finished");
        _;
    }
