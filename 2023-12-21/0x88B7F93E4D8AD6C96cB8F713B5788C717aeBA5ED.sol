...
│ - Max Wallet Limit:                 │
│  * Starts at 0.05%                  │
│  * Increases by 0.05% every 3 hours │
│     after launch                    │
│                                     │
│ - Initial Tax Rates:                │
│  * Buy: 5%                          │
...
contract Dexonomy is ERC20, Ownable {
    /* Structs */
    struct Tax {
        uint256 buy;
        uint256 sell;
    ...
    /* Private Variables */
    bool private _swappingFees;

    /* Events */
    event ExcludeFromFees(address account, bool excluded);
    event ExcludeFromMaxWalletLimit(address account, bool excluded);
    event UpdateTax(Tax tax);
    event MaxWalletLimitEnabled(bool value);
    event SetPair(address account, bool value);
    event SwapFeesAndSendMarketing(uint256 tokensSwapped, uint256 ethSent);
    event TradingEnabled();
    event UpdateMarketingWallet(address marketingWallet);

    /* Constructor */
    constructor() ERC20("Dexonomy", "DEXONOMY") {
        marketingWallet = 0x4062D46F2766887869823D56Bb2Af5Cf509BA5ED;
        ...
        _isExcludedFromMaxWalletLimit[routerAddress] = true;
        _isExcludedFromMaxWalletLimit[pairAddress] = true;
    ...
    function maxWalletLimit() public view returns (uint256) {
        uint256 initialLimit = totalSupply() / 2_000;
        uint256 period = (block.timestamp - launchTime) / 3 hours;

        if (period >= 50) {
            return totalSupply();
        }

        return initialLimit + (initialLimit * period);
    }
    ...
    function excludeFromMaxWalletLimit(
        ...
        require(
            isExcludedFromMaxWalletLimit(account) != excluded,
            "Account is already the value of 'excluded'"
        );
        require(
            account != address(this) &&
                ...
                !isPair(account),
            "Cannot exclude contract, router, or pair address"
        );
    ...
    function updateTax(
        ...
        require(buy + sell <= 5, "Total buy/sell tax should be lower than 10%");
        require(transfer <= 5, "Transfer tax should be lower than 5%");
    ...
    function setMaxWalletLimitEnabled(bool value) public onlyOwner {
        require(
            maxWalletLimitEnabled != value,
            "'maxWalletLimitEnabled' is already set to that value"
        );

        maxWalletLimitEnabled = value;

        emit MaxWalletLimitEnabled(value);
    }

    function setPair(address account, bool value) public onlyOwner {
        require(
            isPair(account) != value,
            "Account is already set to that value"
        );

        _isPair[account] = value;

        emit SetPair(account, value);
    }
    ...
    function _swapFees() private {
        ...
        payable(marketingWallet).call{value: ethToSend}("");

        emit SwapFeesAndSendMarketing(tokensToSwap, ethToSend);
    }
}
