...
contract CBAI is Context,ERC20Burnable, Ownable {
    using Address for address payable;

    struct FeesConfig {
        uint256 sellFee; //6%
        uint256 burnSellFee; //2%
        uint256 transferFee; //2% kept incase if we are planning
    }
    ...
    uint256 public constant initialSupply = 1000_000_000e18;
    ...
    constructor(address _router) ERC20("CBAI", "CBAI") {
        ...
        marketingWallet = 0xF6975B005822A78f7aaf75C21c9172b480934F5B;
        feesConfig = FeesConfig({sellFee: 6, burnSellFee: 2, transferFee: 2});
    ...
    function setSwapTokensAtAmount(uint256 amount) external onlyOwner {
        require(
            ...
            "Value cannot be less than 0.0001%"
        );
    ...
    function setMaxTransactionAmount(uint256 amount) external onlyOwner {
        require(
            amount >= (totalSupply() / 1_000),
            "Value cannot be less than 0.1%"
        );
    ...
    function setFeesConfig(FeesConfig memory config) external onlyOwner {
        uint256 totalFees = config.sellFee +
            config.burnSellFee +
            config.transferFee;
    ...
    function _transfer(
        ...
        if (listedAt + 3 minutes >= block.timestamp) {
            // don't allow to sell for 3 minutes after launch
            if (listedAt + 180 seconds >= block.timestamp)
                // don't allow sell for 180 seconds after launch
                require(to != uniswapV2Pair, "AntiBot: Sell Banned!");
        }
    ...
    function claimStuckTokens(address token) external onlyOwner {
        require(
            token != address(this),
            "Owner cannot claim contract's balance"
        );
