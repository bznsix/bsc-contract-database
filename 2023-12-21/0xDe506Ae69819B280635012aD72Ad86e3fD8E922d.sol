...
contract SBDStaking is ReentrancyGuard,Ownable,Pausable{
    ...
    EnumerableSet.UintSet private allProfit;
    ...
    function claim(uint _amount) public whenNotPaused nonReentrant{
    ...
    function changeRate(uint[] memory month,uint[] memory rate) external onlyOwner {
        ...
        for (uint i = 0 ; i < month.length ;i++){
            if(!allProfit.contains(month[i])){
                allProfit.add(month[i]);
            }
    ...
    function getAllProfit() external view returns(uint[] memory) {
        return allProfit.values();
    }
