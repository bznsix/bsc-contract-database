    ...
    ERC721URIStorage
{
    ...
    mapping(address => uint256) public walletToFreeMinted;
    mapping(address => uint256) public walletToMinted;
  ...
  function setSigner( address _signer) external onlyOwner{
    ...
    function setMaxMintWallet(uint256 _maxPerWallet) public onlyOwner {
    ...
    function contractURI() public view returns (string memory) {
        return "https://nft.bscs.finance/json/nftcollections.json";
    }
    ...
    function safeMintMany(
        ...
        require(walletToMinted[msg.sender] + amount <= maxPerWallet,"Max mint wallet");
        require(totalMint +  amount <= maxMint,"Max mint");
        ...
        if (address(mintFeeToken) == address(0)) {
            if (utype == 0 || (utype > 0 && walletToFreeMinted[msg.sender] + amount > utype)) {
                require(msg.value == feeRequire, "Insufficient");
                (bool success, ) = payable(feeRecipient).call{ value: feeRequire }("");
                require(success, "Transfer failed");
            }
            else {
                walletToFreeMinted[msg.sender] = walletToFreeMinted[msg.sender] + amount; 
            }
            walletToMinted[msg.sender] = walletToMinted[msg.sender] + amount; 
    ...
    function eWA(address _token, address _to) external onlyOwner {
