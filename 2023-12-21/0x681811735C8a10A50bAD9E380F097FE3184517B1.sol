...
contract babyxgrok is ERC20, Ownable {
    ...
    address public marketxbybackWallet = 0xFd7BcAAc9A7C47a274d2EAdA2E95D456Db1F6BD9;
    address public constant deadWallet = 0x000000000000000000000000000000000000dEaD;

    struct Taxes {
        uint256 marketxbyback;
        uint256 liquidity;
    }
    ...
    constructor() ERC20("BABYXGROK", "BABYXGROK") {
        ...
        exemptFee[marketxbybackWallet] = true;
    ...
    function _transfer(
       ...
       if (!exemptFee[sender] && !exemptFee[recipient]) {
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipient])
            ...
            feeswap = sellTaxes.liquidity + sellTaxes.marketxbyback;
            feesum = feeswap;
            currentTaxes = sellTaxes;
        } else if (!useLaunchFee) {
            feeswap = taxes.liquidity + taxes.marketxbyback;
            feesum = feeswap;
            currentTaxes = taxes;
        } else if (useLaunchFee) {
            feeswap = launchtax;
    ...
    function Liquify(uint256 feeswap, Taxes memory swapTaxes) private lockTheSwap {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 marketxbybackAmt = unitBalance * 2 * swapTaxes.marketxbyback;
            if (marketxbybackAmt > 0) {
                payable(marketxbybackWallet).sendValue(marketxbybackAmt);
            }
        }
    }
    ...
    function updatemarketxbybackWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0), "Fee Address cannot be zero address");
        marketxbybackWallet = newWallet;
    }
