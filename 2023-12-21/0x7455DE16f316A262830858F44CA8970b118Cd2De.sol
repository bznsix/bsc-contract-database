...
contract MythofDagda is Context, IERC20, Ownable {
    ...
    constructor (
        ...
        uint256 __refPer,
        uint256[4] memory __buy_data,    // [buy_tax, tax_auto_liq, tax_burn, tax_marketing]
        uint256[4] memory __sell_data,   // [sell_tax, tax_auto_liq, tax_burn, tax_marketing]
        uint256[3] memory __data,        // [run_tokenomics, max_tx, max_hold]
        address __router,
        address __sender
    ) public {
        ...
        b_burnPer = __buy_data[2];
        b_marketingPer = __buy_data[3];

        s_autoLiqPer = __sell_data[1];
        s_burnPer = __sell_data[2];
        s_marketingPer = __sell_data[3];
    ...
    function _setBuyTokenomics(uint256 autoLiqPer, uint256 burnPer, uint256 marketingPer) external onlyOwner() {

        uint256 total = autoLiqPer.add(burnPer).add(marketingPer);
        ...
        b_marketingPer = marketingPer.mul(100);
         ...
         emit setTokenomics(autoLiqPer, burnPer, marketingPer);
    }

    function _setSellTokenomics(uint256 autoLiqPer, uint256 burnPer, uint256 marketingPer) external onlyOwner() {
    ...
    function _doTokenomics(bool is_buy) public lockLiquidities {
        ...
        if(!is_buy && amount >= _minTokensForLiquidity && _lockLiquiditiesEnabled == true) {
            ...
            _doLiquidity(buyLiqAmount.add(sellLiqAmount), buy_amount.add(sell_amount).sub(buyBurnAmount).sub(sellBurnAmount), buyBurnAmount.add(sellBurnAmount), false);

            buyTaxBalance = 0;
        }
    }

    function _doLiquidity(uint256 liqTokensAmount, uint256 fullBalance, uint256 burnAmount, bool is_buy) private {
        ...
        if(bnbNewBalance > 0) {
            ...
            if(is_buy) {
                ...
                liqBnbAmount = bnbNewBalance.sub(marketingBnbAmount);
            }
    ...
    event setTokenomics(uint256 autoLiqPer, uint256 burnPer, uint256 marketingPer);
