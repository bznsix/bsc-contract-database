...
contract INVESTO is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    INVESTODividendTracker public dividendTracker;
    ...
    constructor() ERC20("FUCK INVESTOMANIAC", "FUCK INVESTOMANIAC") {
        marketingWallet = payable(0x9dfde9904b5a8b4F0a0a384fc6e3f688128AFDeb);
        ...
        dividendTracker = new INVESTODividendTracker(payable(this), router, address(this),
                                                   "INVESTOTRACKER", "INVESTOTRACKER");
...
contract INVESTODividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    INVESTO public INVESTOContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        INVESTOContract = INVESTO(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main INVESTO contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(INVESTOContract);

            uint256 prevBalance = INVESTOContract.balanceOf(address(this));
            ...
            {
                uint256 received = INVESTOContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    INVESTOContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
