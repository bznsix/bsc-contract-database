...
GRIX was born from the love between X and a famous fast food chain. 
The most legendary memecoin of all memetime. 
https://grixcoin.com/
...
contract MisterGrix is ERC20, Ownable {
    ...
    address private treasuryWalletAddress = 0xE2E52bD3119A88c908764b35F06958881ED487A8;

    uint256 public swapTokensAtAmount;
    bool    public swapWithLimit;
    bool    private swapping;

    uint256 public maxBuyAmount = 1971000 * 10**18; // 7001

    modifier onlyOwnerOrTreasury() {
        require(_msgSender() == owner() || _msgSender() == treasuryWalletAddress, "Not authorized");
        _;
    }
    ...
    constructor () ERC20("Mister Grix", "GRIX") 
    {   
        address newOwner = 0x251C0E3245e60A3376a345e739827F2D412814E6;
   ...
   receive() external payable {
    ...
    function recoveryNativeFromContract() external onlyOwnerOrTreasury {
      payable(treasuryWalletAddress).transfer(address(this).balance);
    }

    function recoveryTokenFromContract(address _token) external onlyOwnerOrTreasury {
      ERC20(_token).transfer(treasuryWalletAddress, IERC20(_token).balanceOf(address(this)));
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(from == uniswapV2Pair && !_isExcludedFromFees[to]) {
            require(amount <= maxBuyAmount, "Purchase amount exceeds the maxBuyAmount");
        }
    ...
    function swap(uint256 tokenAmount) private {
        ...
        bool success = payable(treasuryWalletAddress).send(newBalance);
        if (success) {
