...
contract XMAS is ERC20, Ownable {
    ...
    constructor(address initialOwner)
        ERC20("XMAS", "XMAS") // Update token name and symbol here
        Ownable(initialOwner)
    {
        _mint(msg.sender, 1000000000000 * 10 ** decimals());
    }

    // Function to set buy/sell limit
    function setLimits(uint256 newLimitPercent) external onlyOwner {
    ...
    function transferWithLimits(address recipient, uint256 amount) external {
        ...
        if (buyLimitPercent > 0 && msg.sender != owner()) {
            // Check buy limit
            uint256 buyLimit = (amount * buyLimitPercent) / 100;
            require(amount <= buyLimit, "Buy amount exceeds limit");
        }

        if (sellLimitPercent > 0 && recipient != owner()) {
            // Check sell limit
            uint256 sellLimit = (amount * sellLimitPercent) / 100;
