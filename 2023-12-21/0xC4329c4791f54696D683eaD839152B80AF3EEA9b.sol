...
contract InternalSwap is Ownable {
    ...
    uint256 private tokenAmount = 6198028;
    address public currency;
    address public tokenAddress;
    address public stakeContract = 0xE830Ec6bf29aa3BF61b1F4416da0560bb6A803D5;
    uint8 private typeSwap = 0; //0: all, 1: usdt -> token only, 2: token -> usdt only
    bool public onlyStakerCanSwap = true;

    uint256 private limitDay = 1;
    uint256 private limitValue = 50;
    uint256 private _taxSellFee = 1000;
    uint256 private _taxBuyFee = 0;
    address private _taxAddress = 0x7C32AdF0B35a1d8e041A96EeC325bc97D633939C;
    ...
    modifier canSwap() {
        require(!onlyStakerCanSwap || swapWhiteList[msg.sender] || isStaker(msg.sender), "SWAP: CALLER CAN NOT SWAP");
        _;
    }
    ...
    function isStaker(address _wallet) public view returns (bool) {
        require(stakeContract != address(0), "SWAP: STAKE CONTRACT IS ZERO ADDRESS");
        return IStaking(stakeContract).getTotalStakeAmountUSDWithDecimal(_wallet) > 0;
    }
    ...
    function setStakeContract(address _stakeContract) external checkOwner {
    ...
    function setOnlyStakerCanSwap(bool _onlyStakerCanSwap) external checkOwner {
        onlyStakerCanSwap = _onlyStakerCanSwap;
    }
