...
contract PresaleFactory is AccessControl, IStructs, IPresaleFactory {
    bytes32 public constant APPROVER_ROLE = keccak256("APPROVER_ROLE");

    uint256 private constant DENOMINATOR = 100;
    uint256 private constant LINK_FEE = 0.2 ether;
    ...
    event CloneTemplatesSetted(
        address indexed pub,
        address indexed inq,
        address indexed priv
    );
    event FeeParamsSetted(address indexed receiver, uint256 percent);
    event BackendSetted(address indexed back);

    constructor(address link, address staking, address approver, address back) {
        require(
            link != address(0) &&
                staking != address(0) &&
                approver != address(0) &&
    ...
    function setMasterAddresses(
        ...
        emit CloneTemplatesSetted(_public, _inqubator, _private);
    }
    ...
    function setFeeParams(
        ...
        feePercent = perc;
        emit FeeParamsSetted(receiver, perc);
    }

    function changeBackendAddress(
        address back
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(back != address(0), "Address 0x00...");
        backend = back;
        emit BackendSetted(back);
    }
    ...
    function createPresalePublic(
        ...
        require(
            amountForLiquidity > 0 && amountForSale > 0,
            "ZERO AMOUNT FOR SALE OR LIQUIDITY"
        );
    ...
    function createPresaleInqubator(
        ...
        require(
            _info.openTime >= block.timestamp + 48 hours &&
                _info.openTime + 9 hours <= _info.closeTime &&
                _info.closeTime < _dexInfo.liquidityAllocationTime,
            "TIME"
        );
