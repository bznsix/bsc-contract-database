...
/**
 * @dev Implementation of a strategy to get yields from farming LP Pools in the platform.
 *
 * This strat is currently compatible with all LP pools.
 */
contract StrategyPalmBnbLP is Ownable, Pausable {
    ...
    address public constant masterChef =
        address(0xf1530E2826920847Ba3Eaf9f60101Bf557332E67);
    address public unirouter;
    uint8 public poolId;

    /**
     * @dev palm Contracts:
     * {burnAddress} - Address for funds allocated for burning Palm.
    ...
    address public constant treasury =
        address(0xBc26982D38ee4964501bc1e2A05C658cC7487bD8);
    ...
    uint256 public constant CALL_FEE = 333;
    uint256 public constant TREASURY_FEE = 667;
     ...
     * {palmToWbnbRoute} - Route we take to get from {palm} into {wbnb}.
     * {wbnbToPalmRoute} - Route we take to get from {wbnb} into {palm}.
     * {palmToLp0Route} - Route we take to get from {palm} into {lpToken0}.
     * {palmToLp1Route} - Route we take to get from {palm} into {lpToken1}.
     */
    address[] public palmToWbnbRoute = [palm, wbnb];
    address[] public wbnbToPalmRoute = [wbnb, palm];
    address[] public palmToLp0Route;
    address[] public palmToLp1Route;
    ...
    constructor(
        ...
        if (lpToken0 == wbnb) {
            palmToLp0Route = [palm, wbnb];
        } else if (lpToken0 != palm) {
            palmToLp0Route = [palm, wbnb, lpToken0];
        }

        if (lpToken1 == wbnb) {
            palmToLp1Route = [palm, wbnb];
        } else if (lpToken1 != palm) {
            palmToLp1Route = [palm, wbnb, lpToken1];
        }

        IERC20(lpPair).safeApprove(masterChef, uint256(-1));
        IERC20(palm).safeApprove(unirouter, uint256(-1));
     ...
     * It deposits {lpPair} in the MasterChef to farm {palm}
     */
     ...
     * 3. It swaps the {palm} token for {lpToken0} & {lpToken1}
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(palm).balanceOf(address(this)).mul(15).div(1000);
        uint256 burnsFee = IERC20(palm).balanceOf(address(this)).mul(30).div(1000);
        ...
        IERC20(palm).safeTransfer(burnAddress, burnsFee);

     
    }

    /**
     * @dev Swaps {palm} for {lpToken0}, {lpToken1} & {wbnb} using the configure unirouter.
     */
    function addLiquidity() internal {
        uint256 palmHalf = IERC20(palm).balanceOf(address(this)).div(2);
    ...
    function pause() public onlyOwner {
        ...
        IERC20(palm).safeApprove(unirouter, 0);
