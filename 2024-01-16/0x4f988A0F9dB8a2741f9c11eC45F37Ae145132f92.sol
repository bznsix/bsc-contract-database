...
contract MstDeposit is Ownable {
    ...
    function buyLand() external {
        ...
        if (recommerLevelMap[0][recommer] >= 3) { // 60%直推奖
            userInfo[recommer].hashrate += landPrice * 60 / 100;
            ...
            userInfo[recommer].hashrate += landPrice * 50 / 100;
            ...
            userInfo[recommer].hashrate += landPrice * 40 / 100;
        ...
        for (uint256 i=0; i<fathers.length; i++) {
            ...
            if (father.level >= 1 && father.level <= 6) { // 可领取流水奖 3% + 星数加成2%
                uint256 rolloverBonus = landPrice * (3 + father.level - 1) / 100;
                father.hashrate += rolloverBonus;
                emit RolloverProfit(fathers[i], msg.sender, landPrice, rolloverBonus, father.level);
                // 上级获得伯乐奖
                userInfo[father.recommer].hashrate += rolloverBonus;
        ...
        // 分红奖
        // 总裁分红2%
        userInfo[president].hashrate += landPrice * 2 / 100;
        emit PresidentProfit(president, msg.sender, landPrice, landPrice * presidentRadio / 10000);
        // 董事分红3%
        userInfo[director].hashrate += landPrice * 3 / 100;
