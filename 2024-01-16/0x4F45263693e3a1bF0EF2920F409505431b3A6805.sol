...
//Baby Martik, from the same team as Martik, 
//a token with 500 units that uses the same supply as Martik, 
//bringing usability to the eco system. Its 10% rates will be full
// for burning, making the 2 that ends up being 1 only more scarce...
...
contract BabyMartik {
    ...
    string constant _name = "Baby Martik";
    string constant _symbol = "BMTK";
    uint8 constant _decimals = 18;
    uint256 _totalSupply = 500 * (10**_decimals);
    ...
    function toBabyMartik(uint256 amount) external {
        require(migrate);
    ...
    function manualSend() external onlyOwner {
     ...
     function manualSendToken(address token) external onlyOwner {
        BEP20(token).transfer(
            marketingFeeReceiver,
            BEP20(token).balanceOf(address(this))
        );
    }
    function setDistributionCriteria(
        uint256 _minPeriod,
