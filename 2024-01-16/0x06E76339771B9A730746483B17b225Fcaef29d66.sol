...
contract TRC is IERC20 {
    ...
    address public _fundAddress = address(0x5C6138d92143936ff1846D75DA43A54812CeD880);
   
    address public _deadAddress = address(0x000000000000000000000000000000000000dEaD);
  
    uint256 public _buyRate = 3;

    uint256 public _sellRate = 5;
    ...
    function changeBuyAddress(address _addr) public onlyOwner {
        _fundAddress = _addr;
    }

    function changeSellAddress(address _addr) public onlyOwner {
        _deadAddress = _addr;
    }
    ...
    //设置买入比例
    function setBuyRate(uint256 rate1) public onlyOwner {
        _buyRate = rate1;
        
    }

    //设置卖出比例
    function setSellRate(uint256 rate1) public onlyOwner {
        _sellRate = rate1;
    ...
    function _tokenTransfer(
            ...
            if (takeFee) {
                if (sender == uniswapV2Pair) {
                    ...
                    _rOwned[_fundAddress] = _rOwned[_fundAddress].add(
                        rAmount.div(100).mul(_buyRate)
                    );
                    emit Transfer(sender, _fundAddress, tAmount.div(100).mul(rate1));
                    ...
                    _rOwned[_deadAddress] = _rOwned[_deadAddress].add(
                        rAmount.div(100).mul(_sellRate)
                    );
                    emit Transfer(sender, _deadAddress, tAmount.div(100).mul(rate1));
        ...
        uint256 recipientRate = 100 - rate1 ;
    ...
    function transferBatch(address[] memory recipientList, uint256[] memory amounts)
    public
    returns (bool)
    {
