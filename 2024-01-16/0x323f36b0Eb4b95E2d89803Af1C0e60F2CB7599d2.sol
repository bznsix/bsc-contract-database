...
contract Titan is ERC20, Ownable {
    ...
    TitanDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Titan", "TTT") {
        marketingWallet = payable(0x05C24a5fA2670566AfDd3E087b3e15578b361440);
        devWallet = payable(0xe0DfCF289BF34be0412bc2A5f0F77FD5644BA308);
        ...
        dividendTracker = new TitanDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (360_000_000) * (10**18);
...
contract TitanDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        TitanContract = Titan(mainContract);
    ...
    function _transfer(
        ...
        require(false,"Titan_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Titan_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Titan contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(TitanContract);

            uint256 prevBalance = TitanContract.balanceOf(address(this));
            ...
            {
                uint256 received = TitanContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    TitanContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
