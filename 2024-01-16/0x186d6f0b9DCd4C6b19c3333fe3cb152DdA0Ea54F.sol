...
contract RITZ is Ownable, ReentrancyGuard {
    ...
    function betCustomT1(IBEP20 _token, uint256 id, uint256 amount) public {
    ...
    function betCustomT2(IBEP20 _token, uint256 id, uint256 amount) public {
        require(supportedTokens[address(_token)] == true, 'no token support');
        (,uint256 thetime, uint256 result, uint256 currentId) = getOracleData(id);
        require(result == 0 && id <= currentId, 'has already played or is inexistent');
        require(block.timestamp <= thetime, 'match already live');
    ...
    function forceMajore(address receiver, uint256 _id) public onlyOwner {
        // if funds get ever stuck, allow for a refund 
        require(contributionsT1[receiver][_id] > 0, 'no contributions in this round');
        TransferHelper.safeTransfer(address(USDT), receiver, contributionsT1[receiver][_id]);
