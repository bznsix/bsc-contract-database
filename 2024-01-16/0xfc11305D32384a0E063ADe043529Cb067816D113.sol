...
contract Mojo is ERC20, Ownable {
    ...
    constructor () ERC20("Mojo Jojo", "Mojo") 
    {   
        address router;
        address pinkLock;
        ...
        feeReceiver = 0x91e77389ca8A4868cFB8E4C3Bfe330744ca6fCeE;
        stakingWallet = 0x4624D5A324701efd4eF9F1E8e55d67629D7bCA69;
    ...
    function changeStakingWallet(address _stakingWallet) external onlyOwner{
        require(_stakingWallet != address(0), "CSLT: Staking receiver cannot be the zero address");
        stakingWallet = _stakingWallet;

        emit StakingWalletChanged(_stakingWallet);
    }
    ...
    function swapAndSendFee(uint256 tokenAmount) private {
        ...
        uint256 staking = newBalance / 4;

        payable(stakingWallet).sendValue(staking);
        payable(feeReceiver).sendValue(address(this).balance);
