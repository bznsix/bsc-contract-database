 ...
 * updates referralsAllocated as (referralsAllocated) + (_amt)
 ...
 * updates referralsAllocated as (referralsAllocated) + (((5) * (_amt)) / (10))
 * updates referringAddress as referralRecordMap with element referringAddress with element referringAddress
 * if referringAddress is equals to Address 0 then (updates totalUnclaimedRewards as (totalUnclaimedRewards) + (referralsAllocated); and then returns referralsAllocated as output)
 ...
 * updates referralRecordMap (Element referringAddress) (Entity unclaimedRewards) as (referralRecordMap with element referringAddress with element unclaimedRewards) + (((3) * (_amt)) / (10))
 * updates referralsAllocated as (referralsAllocated) + (((3) * (_amt)) / (10))
 * updates referringAddress as referralRecordMap with element referringAddress with element referringAddress
 * if referringAddress is equals to Address 0 then (updates totalUnclaimedRewards as (totalUnclaimedRewards) + (referralsAllocated); and then returns referralsAllocated as output)
...
*/
	function addReferral(uint256 _amt) internal returns (uint256) {
		...
		referralsAllocated  = (referralsAllocated + _amt);
		...
		referralsAllocated  = (referralsAllocated + ((uint256(5) * _amt) / uint256(10)));
		referringAddress  = referralRecordMap[referringAddress].referringAddress;
		if ((referringAddress == address(0))){
			totalUnclaimedRewards  = (totalUnclaimedRewards + referralsAllocated);
			return referralsAllocated;
		...
		referralRecordMap[referringAddress].unclaimedRewards  = (referralRecordMap[referringAddress].unclaimedRewards + ((uint256(3) * _amt) / uint256(10)));
		referralsAllocated  = (referralsAllocated + ((uint256(3) * _amt) / uint256(10)));
		referringAddress  = referralRecordMap[referringAddress].referringAddress;
		if ((referringAddress == address(0))){
			totalUnclaimedRewards  = (totalUnclaimedRewards + referralsAllocated);
			return referralsAllocated;
