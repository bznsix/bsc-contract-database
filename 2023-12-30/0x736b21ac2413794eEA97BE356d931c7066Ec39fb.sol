...
contract maxxchainbscDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("maxxchainbsc_Dividend_Tracker", "maxxchainbsc_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18);
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "maxxchainbsc_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "maxxchainbsc_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main maxxchainbsc contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "maxxchainbsc_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"); 
        require(newClaimWait != claimWait, "maxxchainbsc_Dividend_Tracker: Cannot update claimWait to same value");
...
contract maxxchainbsc is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    maxxchainbscDividendTracker public dividendTracker;
    ...
    constructor() ERC20("maxxchainbsc", "mcb") 
    {
        BNBRewardsFee = 1;
        liquidityFee = 1;
        marketingFee = 4;
        ...
        marketingWallet = payable(0xB9B81e1D7744D3b19c2452c9682719eD46b882B9);
        devWallet = payable(0x34D711C3B998fDc336c276445D83aE1EEc947197);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(devFee); // total fee transfer and buy

        dividendTracker = new maxxchainbscDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "maxxchainbsc: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "maxxchainbsc: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "maxxchainbsc: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "maxxchainbsc: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "maxxchainbsc: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "maxxchainbsc: Cannot update gasForProcessing to same value");
