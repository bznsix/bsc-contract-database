...
contract AW20 is IERC20, ERC721 {
  ...
  constructor(
    ...
    uint64 mintLimit_,
    address proxy_
  ) ERC721("aw-20-new", tick) {
    _tick = tick;
    hash = keccak256(
      string.concat(
        '{"p":"aw-20-new","op":"mint","tick":"',
        bytes(tick),
        '","amt":"1000"}'
      )
    );
...
function inscribewl(bytes calldata data,bytes32[] calldata proof,uint256 amount) public {
    ...
    require(verify(proof,leaf),"not in merkle tree");
    require(!mintedAddress[msg.sender],'already mint');
  ...
  function inscribe(bytes calldata data) public {
    ...
    if (block.number > lastBlock) {
      ...
      require(
        mintedPer < 1,
        "Only 1 ticks per block. Using Flashbots can prevent failed txes."
      );
      unchecked {
        mintedPer++;
      }
  ...
  function _transfer20(address from, address to, uint256 amount) internal {
    ...
    string memory t = string(
      string.concat(
        '{"p":"aw-20-new","op":"transfer","tick":"eSports","amt":"',
  ...
  function tokenURI(
    ...
    data = abi.encodePacked(
      ...
      '",</text><text x="130" y="190" class="base">"tick":"eSports",</text><text x="130" y="220" class="base">"amt":'
    );
    data = abi.encodePacked(
      data,
    ...
    string memory json = Base64.encode(
      bytes(
        string(
          abi.encodePacked(
            '{"description": "aw20 is a social experiment, a first attempt to practice inscription within the EVM.", "image": "data:image/svg+xml;base64,',
            Base64.encode(data),
            '"}'
          )
        )
  ...
  function toString(uint256 value) internal pure returns (string memory) {
   ...
   function getLeaf(address account,uint256 amount) internal pure returns (bytes32) {
