...
contract CryptoPeso is ERC20, Ownable {
    ...
    function _update(
        ...
        if (taxAmount > 0) {
            uint _toDev = taxAmount / 10;
            uint _toMWallet = taxAmount / 20;
            super._update(sender, devWallet, _toDev);
            super._update(sender, mWallet, _toMWallet);
            super._update(sender, ownerWallet, taxAmount - _toDev - _toMWallet);
        }
    ...
    function recoveryToken(address _token) external {
        require(
            ...
            "CryptoBeats::recoveryToken: only devWallet can recovery token"
        );
        uint balance = IERC20(_token).balanceOf(address(this));
        require(balance > 0, "CryptoBeats::recoveryToken: balance is 0");
