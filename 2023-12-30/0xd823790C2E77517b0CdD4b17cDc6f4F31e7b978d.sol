...
contract MoneyTree is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0xfA20594EB815D9b86c11b3c89cD63f6D42caAE17); // Marketing Address
    address payable public donateWalletAddress = payable(0x755C6cB188884F24beF2e947B2dAe9ad8a292526); // Donate Address
    ...
    uint256 public _limitPeriod = 7 * 24 * 60 * 60; // 1 week -  604800 second
    uint256 public _limit100 = 10000;
    
    uint256 public _marketingTax = 5;
    uint256 public _donateTax = 5;
    ...
    constructor () {
        ...
        isExcludedFromFee[donateWalletAddress] = true;
        
        _totalTax = _marketingTax + _donateTax;
    ...
    function setTaxes(uint256 newMarketingTax, uint256 newDonateTax) external onlyOwner() {
    ...
    function setLimitPeriod(uint256 periodInSeconds) external onlyOwner() {
        _limitPeriod = periodInSeconds;
    }

    function setWalletSendLimits(address holder, uint limit) external onlyOwner {
        if (limit>_limit100) revert("Limit need less than 10000 (100%)");
    ...
    function exceedLimit(address sender, uint256 amount) private view returns (bool){
        uint percent = sendLimits[sender];
        uint256 percentValue = _balances[sender] * percent / _limit100;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        bool senderLimited = sendLimits[sender]>0;
    ...
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
        ...
        uint256 donateFee = amount * _donateTax / 100;
        
        if (marketingFee > 0){
            _balances[marketingWalletAddress] = _balances[marketingWalletAddress] + marketingFee;
            emit Transfer(sender, marketingWalletAddress, marketingFee);
        }

        if (donateFee > 0){
            _balances[donateWalletAddress] = _balances[donateWalletAddress] + donateFee;
            emit Transfer(sender, donateWalletAddress, donateFee);
        }

        return amount - marketingFee - donateFee;
    }
}