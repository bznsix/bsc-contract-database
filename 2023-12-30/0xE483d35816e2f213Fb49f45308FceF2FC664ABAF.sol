...
abstract contract AbsMiner is Ownable, ReentrancyGuard {
    ...
    uint256 public maxLimits = 500e18;
    ...
    constructor(address sponsor) {
        ...
        inviter[sponsor] = address(this);
        members[sponsor].account = sponsor;
        members[sponsor].sponsor = address(this);
        members[sponsor].joinTimestamp = block.timestamp;
    }
    ...
    function deposit(uint256 uAmount) external nonReentrant returns (bool) {
        ...
        // sort
        uint256 currentIndex = dayBuyAddressList.length;
        uint256 parentIndex = currentIndex / 2;
        while (
            parentIndex > 0 &&
            dayBuyAmountMap[dayBuyAddressList[currentIndex - 1]] >
            dayBuyAmountMap[dayBuyAddressList[parentIndex - 1]]
        ) {
            (
                dayBuyAddressList[currentIndex - 1],
                dayBuyAddressList[parentIndex - 1]
            ) = (
                dayBuyAddressList[parentIndex - 1],
                dayBuyAddressList[currentIndex - 1]
            );
