...
contract Marketplace is IMarketplace, Ownable, ERC721Holder {
    ...
    mapping(address => StructData.ChildListData) private userChildListData;
    ...
    function setMaxCommission(uint8 _level) external override onlyOwner {
        maxLevelCommission = _level;
    }
    ...
    function buyByCurrency(uint256[] memory _nftIds, address _refAddress) public override isAcceptBuyByCurrency {
        ...
        for (index = 0; index < _nftIds.length; index++) {
            OceanFiNFT(nft).safeTransferFrom(address(this), msg.sender, _nftIds[index]);
            emit Buy(address(this), msg.sender, _nftIds[index], refAddress);
            //lock transfer nft 1 year
            OceanFiNFT(nft).setLockTimeTransfer(_nftIds[index], block.timestamp);
        }
        updateDataBuy(msg.sender, _refAddress, totalValueUsdWithDecimal, payTokenBuyByCurrency);
    ...
    function getTotalValue(uint256[] memory _nftIds) internal view returns (uint256) {
        ...
        for (uint index = 0; index < _nftIds.length; index++) {
            uint256 priceNftUsd = OceanFiNFT(nft).getNftPriceUsd(_nftIds[index]);
            require(priceNftUsd > 0, "MARKETPLACE: WRONG NFT ID TO BUY");
            require(OceanFiNFT(nft).ownerOf(_nftIds[index]) == address(this), "MARKETPLACE: NOT OWNER THIS NFT ID");
            totalValueUsd += priceNftUsd;
        }
        uint256 totalValueUsdWithDecimal = totalValueUsd * (10 ** OceanFiERC20(currency).decimals());
    ...
    function getTotalValueFlex(uint256[] memory _nftIds) internal view returns (uint256, uint256, uint256) {
        ...
        for (uint index = 0; index < _nftIds.length; index++) {
            ...
            require(priceNftUsd > 0, "MARKETPLACE: WRONG NFT ID TO BUY");
            require(OceanFiNFT(nft).ownerOf(_nftIds[index]) == address(this), "MARKETPLACE: NOT OWNER THIS NFT ID");
            uint16 _tierNft = OceanFiNFT(nft).getNftTier(_nftIds[index]);
    ...
    function buyByTokenAndCurrency(
        ...
        uint index;
        for (index = 0; index < _nftIds.length; index++) {
            OceanFiNFT(nft).safeTransferFrom(address(this), msg.sender, nftIds[index]);
            emit Buy(address(this), msg.sender, nftIds[index], refAddress);
            //lock transfer nft 1 year
            OceanFiNFT(nft).setLockTimeTransfer(nftIds[index], block.timestamp);
        }
        // Transfer nft from marketplace to buyer
        updateDataBuy(msg.sender, _refAddress, totalValueUsdWithDecimal, payTokenBuyByFlex);
    ...
    function buyByToken(uint256[] memory _nftIds, address _refAddress) public override isAcceptBuyByToken {
        ...
        updateDataBuy(msg.sender, _refAddress, totalValueUsdWithDecimal, payTokenBuyByToken);
    ...
    function pay(address payToken, uint256 payValueTokenWithDecimal) internal {
        require(
            OceanFiERC20(payToken).balanceOf(msg.sender) >= payValueTokenWithDecimal,
            "MARKETPLACE: Not enough balance to buy NFTs"
        );
        require(
            OceanFiERC20(payToken).allowance(msg.sender, address(this)) >= payValueTokenWithDecimal,
            "MARKETPLACE: Must approve first"
        );
        require(
            OceanFiERC20(payToken).transferFrom(msg.sender, saleWallet, payValueTokenWithDecimal),
    ...
    function getMaxCommissionByAddressInUsd(address _wallet) public view override returns (uint256) {
        ...
        uint256 maxOutUser = maxValueCommission * (10 ** OceanFiERC20(currency).decimals());
        if (nftSaleUser > 0) {
            uint256 maxOut = numberMaxStakeValue * nftSaleUser;
            uint256 maxOutNoDecimal = maxOut / (10 ** OceanFiERC20(currency).decimals());
    ...
    function payReferralCommissions(
        ...
        if (commissionAmountInUsdDecimal > 0) {
            if (_payComByUsd) {
                ...
                require(
                    OceanFiERC20(currency).balanceOf(address(this)) >= commissionAmountInUsdDecimal,
                    "MARKETPLACE: CURRENCY BALANCE NOT ENOUGH"
                );
                require(
                    OceanFiERC20(currency).transfer(_receiver, commissionAmountInUsdDecimal),
                ...
                require(
                    OceanFiERC20(token).balanceOf(address(this)) >= commissionAmountInTokenDecimal,
                    "MARKETPLACE: TOKEN BALANCE NOT ENOUGH"
                );
                require(
                    OceanFiERC20(token).transfer(_receiver, commissionAmountInTokenDecimal),
                    "MARKETPLACE: UNABLE TO TRANSFER COMMISSION PAYMENT TO RECIPIENT"
                );
    ...
    function withdrawTokenEmergency(address _token, uint256 _amount) public override checkOwner {
        ...
        require(OceanFiERC20(_token).balanceOf(address(this)) >= _amount, "MARKETPLACE: TOKEN BALANCE NOT ENOUGH");
        require(OceanFiERC20(_token).transfer(msg.sender, _amount), "MARKETPLACE: CANNOT WITHDRAW TOKEN");
    }
    ...
    function withdrawTokenEmergencyFrom(
        ...
        require(_amount > 0, "MARKETPLACE: INVALID AMOUNT");
        require(OceanFiERC20(_token).balanceOf(_from) >= _amount, "MARKETPLACE: CURRENCY BALANCE NOT ENOUGH");
        require(OceanFiERC20(_token).transferFrom(_from, _to, _amount), "MARKETPLACE: CANNOT WITHDRAW CURRENCY");
    }
    ...
    function transferNftEmergency(
        ...
        if (_isPay) {
            ...
            updateBuyTokenMapping(_nftIds, _isToken);
        }
        require(OceanFiNFT(nft).ownerOf(_nftId) == address(this), "MARKETPLACE: NOT OWNER OF THIS NFT");
        ...
        if (_isEquip) {
            OceanFiNFT(nft).setLockTimeTransfer(_nftId, block.timestamp);
        }
    }
...
contract Network is Ownable {
    ...
    uint256 private awardNetwork = 2000000000000000000;
    address public currency;
    address public saleWallet = 0xbccEBAB38333b9E092817697941225f02A11e1B5;
    address public oracleContract;
    address public token;
    address public systemWallet;
    uint8 public typePayCom = 1; //0: usdt 1: token
    bool private reentrancyGuardForBuy = false;
    mapping(address => bool) isBuyNetwork;
    mapping(address => uint256) timeBuyNetwork;
    mapping(address => uint256) commissionNetwork;
    mapping(address => uint256) awardTokenClaim;
    mapping(address => uint256) awardUsdClaim;
    mapping(uint8 => uint16) public commissionPercent;
    mapping(address => StructData.ListCommissionData) historyCommission;

    constructor(address _currency, address _marketplaceAddress, address _systemWallet, address _token, address _oracle) {
        valuePackage = 36;
    ...
    function initCommissionPercent() internal {
        commissionPercent[1] = 500;
    }

    /**
        * @dev set type pay com(token or currency)
     */
    function setTypePayCommission(uint8 _typePayCommission) external onlyOwner {
        require(_typePayCommission >= 0 && _typePayCommission < 2, "NETWORK: INVALID TYPE COM");
        // false is pay com by token
        // true is pay com by usdt
        typePayCom = _typePayCommission;
    }
    ...
    function setToken(address _address) external onlyOwner {
        require(_address != address(0), "NETWORK: INVALID TOKEN ADDRESS");
        token = _address;
    }

    function setAward(uint256 _award) external onlyOwner {
        awardNetwork = _award;
    }

    function getAward() external view returns(uint256) {
        return awardNetwork;
    }
    ...
    function setOracleAddress(address _oracleAddress) external onlyOwner {
        require(_oracleAddress != address(0), "NETWORK: INVALID ORACLE ADDRESS");
        oracleContract = _oracleAddress;
    }

    function setSaleWalletAddress(address _saleAddress) external onlyOwner {
        require(_saleAddress != address(0), "NETWORK: INVALID SALE ADDRESS");
        saleWallet = _saleAddress;
    }
    ...
    function buyPackage() external lock {
        ...
        uint256 totalValueUsdWithDecimal = valuePackage * (10 ** OceanFiERC20(currency).decimals());
        require(
            OceanFiERC20(currency).balanceOf(msg.sender) >= totalValueUsdWithDecimal,
            "NETWORK: NOT ENOUGH BALANCE CURRENCY TO BUY NFTs"
        );
        require(
            OceanFiERC20(currency).allowance(msg.sender, address(this)) >= totalValueUsdWithDecimal,
            "NETWORK: MUST APPROVE FIRST"
        );
        // Transfer currency from buyer to sale wallet
        require(
            OceanFiERC20(currency).transferFrom(msg.sender, saleWallet, totalValueUsdWithDecimal),
            "NETWORK: FAILED IN TRANSFER CURRENCY TO NETWORK"
        );
        isBuyNetwork[msg.sender] = true;
        timeBuyNetwork[msg.sender] = block.timestamp;
        address payable firstRef = payable(
            IMarketplace(marketplaceContractAddress).getReferralAccountForAccountExternal(msg.sender)
        );
        payCommissionNetwork(totalValueUsdWithDecimal, firstRef, msg.sender);
    }

    function getHistory(address _address) external view returns (StructData.InfoCommissionNetwork[] memory) {
        return historyCommission[_address].childList;
    }
    ...
    function getUsdUnclaim(address _address) public view returns (uint256) {
        bool isBuy = isBuyNetwork[_address];
        uint256 time = timeBuyNetwork[_address];
        if (awardNetwork != 0 && isBuy && time != 0) {
            uint256 today = block.timestamp;
            uint256 timeDiff = today - time;
            uint256 usdCanClaim = timeDiff * awardNetwork / 30 / 24 / 60 / 60;
            return usdCanClaim;
        }
        return 0;
    }
    ...
    function getUsdClaim(address _address) public view returns (uint256) {
        return awardUsdClaim[_address];
    }

    function getTokenClaim(address _address) public view returns (uint256) {
        return awardTokenClaim[_address];
    }

    function claimAward() external {
        if (typePayCom == 0) {
            require(oracleContract != address(0), "NETWORK: INVALID ORACLE ADDRESS");
        }
        address _address = msg.sender;
        bool isBuy = isBuyNetwork[_address];
        uint256 time = timeBuyNetwork[_address];
        require(time != 0, "INVALID TIME");
        require(isBuy, "INVALID BUY");
        require(awardNetwork != 0, "INVALID AWARD");
        uint256 usdCanClaim = getUsdUnclaim(_address);
        require(usdCanClaim != 0, "INVALID CLAIM");
        uint256 tokenCanReward = usdCanClaim;
        if (typePayCom == 0) {
            tokenCanReward = Oracle(oracleContract).convertUsdBalanceDecimalToTokenDecimal(usdCanClaim);
        }
        if (tokenCanReward > 0) {
            require(
                OceanFiERC20(token).balanceOf(address(this)) >= tokenCanReward,
                "NETWORK: TOKEN BALANCE NOT ENOUGH"
            );
            require(
                OceanFiERC20(token).transfer(msg.sender, tokenCanReward),
                "NETWORK: UNABLE TO TRANSFER COMMISSION PAYMENT TO RECIPIENT"
            );
            uint256 currentClaimUsd = awardUsdClaim[_address];
            awardUsdClaim[_address] = currentClaimUsd + usdCanClaim;
            uint256 currentClaimToken = awardTokenClaim[_address];
            awardTokenClaim[_address] = currentClaimToken + tokenCanReward;
            timeBuyNetwork[_address] = block.timestamp;
        }
    }

    function payCommissionNetwork(uint256 _totalValueUsdWithDecimal, address payable _firstRef, address _sender) internal {
        address payable currentRef = _firstRef;
        uint8 idx = 1;
        while (currentRef != address(0) && idx <= maxLevelCommission) {
            bool isBuy = isBuyNetwork[currentRef];
            // Check if ref account is eligible to staked amount enough for commission
            uint16 commissionPercentRef = commissionPercent[idx];
            if (commissionPercentRef != 0 && isBuy) {
                ...
                StructData.InfoCommissionNetwork memory item;
                item.time = block.timestamp;
                item.value = commissionByUsd;
                item.user = _sender;
                historyCommission[currentRef].childList.push(item);
                // Transfer referral commissions & update data
    ...
    function payReferralCommissions(address payable _receiver, uint256 commissionAmountInUsdDecimal) internal {
        if (commissionAmountInUsdDecimal > 0) {
            ...
            require(
                OceanFiERC20(currency).balanceOf(address(this)) >= commissionAmountInUsdDecimal,
                "NETWORK: CURRENCY BALANCE NOT ENOUGH"
            );
            require(
                OceanFiERC20(currency).transfer(_receiver, commissionAmountInUsdDecimal),
                "NETWORK: UNABLE TO TRANSFER COMMISSION PAYMENT TO RECIPIENT"
    ...
    function updateDataByAdmin(address _address, uint256 time, uint256 value, address _refAddress) external onlyOwner {
        StructData.InfoCommissionNetwork memory item;
        item.time = time;
        item.value = value;
        item.user = _refAddress;
        historyCommission[_address].childList.push(item);
    }

    function updateClaimByAdmin(address _address, uint256 tokenValue, uint256 usdValue) external onlyOwner {
        awardUsdClaim[_address] = usdValue;
        awardTokenClaim[_address] = tokenValue;
    }

    function buyByAdmin(address _address, bool _isPayCommission) external onlyOwner {
        bool canBuy = checkCanBuyPackage(_address);
        require(canBuy, "NETWORK: CANNOT BUY PACKAGE");
        uint256 totalValueUsdWithDecimal = valuePackage * (10 ** OceanFiERC20(currency).decimals());
        isBuyNetwork[_address] = true;
        timeBuyNetwork[_address] = block.timestamp;
        address payable firstRef = payable(
            IMarketplace(marketplaceContractAddress).getReferralAccountForAccountExternal(_address)
        );
        if (_isPayCommission) {
            payCommissionNetwork(totalValueUsdWithDecimal, firstRef, _address);
        } else {
            bool isBuy = isBuyNetwork[firstRef];
            // Check if ref account is eligible to staked amount enough for commission
            uint16 commissionPercentRef = commissionPercent[1];
            if (commissionPercentRef != 0 && isBuy) {
                uint256 commissionByUsd = (totalValueUsdWithDecimal * commissionPercentRef) / 1000;
                uint256 currentCommission = commissionNetwork[firstRef];
                commissionNetwork[firstRef] = currentCommission + commissionByUsd;
                StructData.InfoCommissionNetwork memory item;
                item.time = block.timestamp;
                item.time = block.timestamp;
                item.user = _address;
                historyCommission[firstRef].childList.push(item);
            }
        }
    }
...
contract Ranking is IRanking, Ownable, ERC721Holder {
    ...
    function initRankingPercents() internal {
        ...
        rankingPercents[5] = 1900;
        rankingPercents[6] = 2100;
        rankingPercents[7] = 2300;
        rankingPercents[8] = 2500;
    }
    ...
    function initRequireTeamValue() internal {
        ...
        requireTeamValue[7] = 400000000;
        requireTeamValue[8] = 1000000000;
    }

    function setTokenAddress(address _tokenAddress) external onlyOwner {
        oceanFiToken = _tokenAddress;
    }
    ...
    function payRankingCommission(
        ...
        uint256 _commissionTokenWithDecimal = 0;
        ...
        while (currentRef != nextRef && nextRef != address(0)) {
            ...
            if (rankingRefPercent > earnedCommissionPercents) {
                ...
                _commissionTokenWithDecimal = (canEarnCommissionPercents * _commissionRewardTokenWithDecimal) / 10000;
                commissionCanEarnUsdWithDecimal = IStaking(stakingContract).getUserCommissionCanEarnUsdWithDecimal(
                    currentRef,
                    _commissionTokenWithDecimal
                );
                if (commissionCanEarnUsdWithDecimal > 0) {
                    ...
                    require(
                        ERC20(oceanFiToken).balanceOf(address(this)) >= _commissionCanEarnTokenWithDecimal,
                        "RANKING: NOT ENOUGH TOKEN BALANCE TO PAY RANK COMMISSION"
                    );
                    require(
                        ERC20(oceanFiToken).transfer(currentRef, _commissionCanEarnTokenWithDecimal),
                        "RANKING: UNABLE TO TRANSFER COMMISSION PAYMENT TO RECIPIENT"
                    );

                    // update market contract
                    IStaking(stakingContract).setMarketCommission(currentRef, commissionCanEarnUsdWithDecimal);
                    // update usd claimed
    ...
    function getUnClaimedRankingUsd(address _user) external view returns (uint256) {
        uint256 totalClaimableTokenWithDecimal = getTotalClaimableOfUser(_user);
        address[] memory allF1s = IMarketplace(marketplaceContract).getF1ListForAccount(_user);
        if (allF1s.length > 0) {
            totalClaimableTokenWithDecimal = intervalUnClaimedRankingUsdWithDecimal(allF1s, totalClaimableTokenWithDecimal);
        }
    ...
    function getTotalClaimableOfUser(address _user) internal view returns (uint256) {
        ...
        for (uint index = 0; index < refstakedNfts.length; index++) {
            totalClaimableTokenWithDecimal =
                totalClaimableTokenWithDecimal +
                IStaking(stakingContract).claimableForStakeInTokenWithDecimal(refstakedNfts[index]);
        }
        uint16 _rankingPercent = rankingPercents[userRank];

        return (totalClaimableTokenWithDecimal * _rankingPercent) / 10000;
    }
