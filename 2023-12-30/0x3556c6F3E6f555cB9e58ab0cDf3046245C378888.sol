...
contract PM is ERC20, Ownable {
    ...
    address private constant PIM = 0x9E76225322234C6c82DCCE1A3dddeE99917B9999;

    address public marketingAddr = 0x03aab269dCDf42E484723dBc22A88dA8A86c3922;
    address public lpOwner = 0x57b356CD8Adc58ef6374e2BaD23e4c32DD58EBEe;
    address public tokenOwner = 0x57b356CD8Adc58ef6374e2BaD23e4c32DD58EBEe;

    uint256 public numTokensSellToSwap = 300 * 1e18;

    uint256 public buyMarketingFee = 0;//
    uint256 public buyLiquifyFee = 0;//
    uint256 public buyLpFee0 = 3;//
    uint256 public buyLpFee1 = 1;//

    uint256 public sellMarketingFee = 0;
    uint256 public sellLiquifyFee = 0;
    uint256 public sellLpFee0 = 3;
    uint256 public sellLpFee1 = 1;
    uint256 public maxToken = 3000000 * 1e18;//
    ...
    function setBuyLpFee(uint256 _buyLpFee0, uint256 _buyLpFee1) external onlyOwner { 
    ...
    function setSellLpFee(uint256 _sellLpFee0, uint256 _sellLpFee1) external onlyOwner { 
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if(takeFee) {
            require(amount <= maxToken, "max token limit");
            uint256 feeToThis;
            if(sender == uniswapV2Pair) { //buy
                feeToThis = buyMarketingFee + buyLiquifyFee + buyLpFee0 + buyLpFee1;
            } else if (recipient == uniswapV2Pair) {
                feeToThis = sellMarketingFee + sellLiquifyFee + sellLpFee0 + sellLpFee1;
            }
    ...
    function swapAndDividend(uint256 tokenAmount) private lockTheSwap {
        uint totalBuyShare = buyMarketingFee + buyLiquifyFee + buyLpFee0 + buyLpFee1;
        uint totalSellShare = sellMarketingFee + sellLiquifyFee + sellLpFee0 + sellLpFee1;
        ...
        uint amountToSwapUSDt = tokenAmount * (buyMarketingFee + sellMarketingFee + buyLpFee0 + sellLpFee0) / (totalBuyShare + totalSellShare);
        if (amountToSwapUSDt > 0) {
            ...
            uint256 totalLeftShare = buyMarketingFee + sellMarketingFee + buyLpFee0 + sellLpFee0;
            uint256 bToM = newBalance * (buyMarketingFee + sellMarketingFee) / totalLeftShare;
        ...
        uint amountToSwapPIM = tokenAmount - amountToLp - amountToSwapUSDt;
        if (amountToSwapPIM > 0) {
    ...
    function dividendToLPHolders(uint256 gas) private {
        ...
        uint256 totalRewardsUSDT = IERC20(USDT).balanceOf(address(this));
        uint256 totalRewardsPIM = IERC20(PIM).balanceOf(address(this));
        while (gasUsed < gas && iterations < numberOfTokenHolders) {
            ...
            if (LPAmount >= minAmountForLPDividend) {
                uint256 dividendAmountUSDT = totalRewardsUSDT * LPAmount / totalLPAmount;
                if (dividendAmountUSDT > 0) {
                    uint256 balanceOfThis = IERC20(USDT).balanceOf(address(this));
                    if (balanceOfThis > dividendAmountUSDT) {
                        IERC20(USDT).transfer(cur, dividendAmountUSDT);
                    }     
                }   

                uint256 dividendAmountPIM = totalRewardsPIM * LPAmount / totalLPAmount;
                if (dividendAmountPIM > 0) {
                    uint256 balanceOfThis = IERC20(PIM).balanceOf(address(this));
                    if (balanceOfThis > dividendAmountPIM) {
                        IERC20(PIM).transfer(cur, dividendAmountPIM);
                    }     
                }  
            }
