...
NFT-Mint    : https://flokix.h0st.org/mint/
Twitter     : https://twitter.com/flokixbnb
...
contract FlokiXnft is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  // Lists 
  uint256[966] public remainingIds;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0x25c4Ce39Fc037f8e0dbe8f6D1cb54426A8D34E2d;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "FlokiXnft: minting has not started yet");
    require(_mintAmount > 0, "FlokiXnft: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "FlokiXnft: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "FlokiXnft: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "FlokiXnft: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "FlokiXnft: zero address");
    require(_amount > 0, "FlokiXnft: you have to mint at least one");
    require(lastSupply != 0,  "FlokiXnft: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"FlokiXnft: URI query for nonexistent token");
