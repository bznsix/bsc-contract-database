...
// @author Movr Network
*/
contract OOSwapImpl is MiddlewareImplBase {
    using SafeERC20 for IERC20;
    address payable public ooSwap;

    event UpdateOOSwapAddress(address indexed ooSwap);

    address private constant NATIVE_TOKEN_ADDRESS =
    address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE);

    /// one inch aggregator contract is payable to allow ethereum swaps
    constructor(address registry, address _ooSwap)
    MiddlewareImplBase(registry)
    {
        ooSwap = payable(_ooSwap);
    }

    /// @notice Sets _ooSwap address
    /// @param _ooSwap is the address for oneInchAggreagtor
    function setOOSwap(address _ooSwap)
    external
    onlyOwner
    {
        ooSwap = payable(_ooSwap);
        emit UpdateOOSwapAddress(_ooSwap);
    }
    ...
    */
    function performAction(
        ...
        if (fromToken != NATIVE_TOKEN_ADDRESS) {
            ...
            IERC20(fromToken).safeIncreaseAllowance(ooSwap, amount);
            {
                // solhint-disable-next-line
                (bool success, bytes memory result) =
                ooSwap.call(swapExtraData);
                IERC20(fromToken).safeApprove(ooSwap, 0);
            ...
            (bool success, bytes memory result) =
            ooSwap.call{value : amount}(swapExtraData);
            require(success, MovrErrors.MIDDLEWARE_ACTION_FAILED);
            uint256 returnAmount = abi.decode(result, (uint256));
            return returnAmount;
        }
