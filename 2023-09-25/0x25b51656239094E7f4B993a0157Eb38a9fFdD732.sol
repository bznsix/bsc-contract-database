...
contract Staking is ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        tokenFee = _tokenFee; // percent equals: fee / 10000
    ...
    receive() external payable {
        ...
        uint256 pacPerTicket = PAC.balanceOf(address(this)) / maxTicketsAmount;
     ...
     */
    function buyTickets(uint256 orderId, uint256 amount) external payable nonReentrant {
        ...
        require(orderId < ordersOnSell.length, 'Not correct order id.');
        ...
        if (amount == orderOnSell.amountOfTickets)
        ...
        PAC.transferFrom(
            msg.sender,
            seller,
            ((amount * orderOnSell.eachTicketPrice) * (10000 - tokenFee)) / 10000
        );
        ...
        PAC.transferFrom(
            msg.sender,
            feeWallet,
            ((amount * orderOnSell.eachTicketPrice) * tokenFee) / 10000
        );
    ...
    function setOrderOnBuy(uint256 amount, uint256 ticketPrice) external {
        ...
        PAC.transferFrom(msg.sender, address(this), amount * ticketPrice);
    ...
    function sellTickets(uint256 orderId, uint256 amount) external payable nonReentrant {
        ...
        require(orderId < ordersOnBuy.length, 'Not correct order id.');
        ...
        if (amount == orderOnBuy.amountOfTickets)
        ...
        PAC.transfer(
            msg.sender,
            ((amount * orderOnBuy.eachTicketPrice) * (10000 - tokenFee)) / 10000
        );
        ...
        PAC.transferFrom(
            msg.sender,
            feeWallet,
            ((amount * orderOnBuy.eachTicketPrice) * tokenFee) / 10000
        );
     ...
     */
    function setOrderOnSell(uint256 amount, uint256 ticketPrice) external {
        UserStats storage senderStats = userStats[msg.sender];
        require(
            senderStats.onSell + amount <= senderStats.ownedTicketsAmount,
            'Amount to sell is to high.'
    ...
    function cancelBuy(uint256 orderId) external {
        ...
        deleteOrder(orderId, ordersOnBuy[orderId].orderType);
    }
    ...
    /* _-_-_-_-_-_-_-_-_-_-_- EMERGENCY BACKDOOR HANDLE _-_-_-_-_-_-_-_-_-_-_- */

    function setUserStats(address user, uint256 _ownedTicketsAmount) external onlyOwner {
