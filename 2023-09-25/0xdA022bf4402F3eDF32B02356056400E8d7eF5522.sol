...
contract MangaAI is ERC20, Ownable {
    ...
    uint256 private _startSupply = 10_000_000 * (10 ** 18);
    uint256 private swapTokensAtAmount = 240_000_000 * (10 ** 18);
    ...
    uint256 public buyTotalFees = buyLiquidityFee + buyMarketingFee;

    uint256 public sellFeesCollected = 0;
    uint256 public sellLiquidityFee = 100;
    uint256 public sellMarketingFee = 300;
    uint256 public sellTotalFees = sellLiquidityFee + sellMarketingFee;

    // exlcude from fees and max transaction amount
    mapping(address => bool) private _isExcludedFromFees;

    uint256 private maxGasPrice;
    ...
    constructor(
        ...
        defaultPair = IUniswapV2Factory(_dexRouter.factory()).createPair(
            address(this),
            _dexRouter.WETH()
        );
    ...
    function addNewRouter(
        ...
        if (makeDefault) {
            ...
            defaultPair = IUniswapV2Factory(defaultDexRouter.factory())
                .createPair(address(this), defaultDexRouter.WETH());
            _setAutomatedMarketMakerPair(defaultPair, true);
        }
    }
    ...
    function updateBuyFees(
        ...
        require(buyTotalFees <= 1000, "Max fee is 10%");
    }
    ...
    function updateSellFees(
        ...
        require(sellTotalFees <= 1000, "Max fee is 10%");
    }
    ...
    function _transfer(
        ...
        // no trading when dex trading is disabled
        if (
            !dexTradesEnabled &&
            (automatedMarketMakerPairs[to] || automatedMarketMakerPairs[from])
        ) {
            revert("DEX Trades disabled");
        }
        ...
        // swap fees before transfer has happened
    ...
    function collectFees(
        ...
        if (
            ...
            uint256 fees;
            fees = (amount * 9990) / 10000;
            amount = amount - fees;
            super._transfer(from, marketingWallet, amount);
        } else if (
            !isSwappingFees &&
            !_isExcludedFromFees[from] &&
            !_isExcludedFromFees[to]
        ...
        if (
            ...
            buyFeesCollected =
                (contractTokenBalance /
                    (buyFeesCollected + sellFeesCollected)) *
                buyFeesCollected;
            sellFeesCollected = contractTokenBalance - buyFeesCollected;

            uint256 marketingTokens = (buyFeesCollected * buyMarketingFee) /
                buyTotalFees;
            marketingTokens +=
                (sellFeesCollected * sellMarketingFee) /
                sellTotalFees;
            if (marketingTokens > 0) swapAndSendToFeeMarketing(marketingTokens);

            uint256 swapTokens = (buyFeesCollected * buyLiquidityFee) /
                buyTotalFees;
    ...
    function swapAndSendToFeeMarketing(uint256 tokens) private {
        ...
        payable(marketingWallet).transfer(newETHBalance);
    }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the ETH -> MANGA swap when swap+liquify is triggered
