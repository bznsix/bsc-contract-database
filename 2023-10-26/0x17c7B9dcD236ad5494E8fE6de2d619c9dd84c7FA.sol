// Telegram: https://t.me/babykabosu_bsc
// X: https://x.com/babykabosu_2023
...
contract TokenDividendTracker is Ownable, DividendPayingToken {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 60 && newClaimWait <= 86400, "Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
...
contract BabyToken is ERC20, Ownable {
    ...
    uint256 public buyInspireFee;
    uint256 public sellInspireFee;
    uint256 public AmountLiquidityFee;
    uint256 public AmountTokenRewardsFee;
    uint256 public AmountMarketingFee;
    uint256 public AmountInspireFee;

    address public marketingWalletAddress;
    address public inspireWalletAddress;
    ...
    event MultiTransfer(address[] addresses);
    ...
    constructor(
    ...
    ) payable ERC20("BabyKabosu", "BabyKabosu")  {
        rewardToken = addr_[0];
        marketingWalletAddress = addr_[1];
        inspireWalletAddress = addr_[2];
        address router = addr_[3];
        liquidityWalletAddress = owner();

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(router);
        require(IERC20(rewardToken).totalSupply() > 0 && rewardToken != _uniswapV2Router.WETH(), "Invalid Reward Token ");
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH());

        uint256 decimals = decimals();
        uint256 totalSupply = 10000000 * 100000000 * 10 ** decimals;
        swapTokensAtAmount = totalSupply.mul(2).div(100000);
        uint256 tokenBalanceForReward_ = 10000 * 100000000 * 10 ** decimals;
        ...
        excludeFromFees(inspireWalletAddress, true);
    ...
    function updateMinimumTokenBalanceForDividendsByPercent(uint256 percent) public onlyOwner {
        uint256 peerCount = percent.mul(1).div(100).mul(totalSupply());
        dividendTracker.setMinimumTokenBalanceForDividends(peerCount);
    }
    ...
    function setInspireWallet(address addr) public onlyOwner {
        inspireWalletAddress = payable(addr);
        excludeFromFees(addr, true);
    }

    function setLiquidityWallet(address addr) public onlyOwner {
        liquidityWalletAddress = addr;
        excludeFromFees(addr, true);
    }

    function setBuyTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 deadFee, uint256 inspireFee) external onlyOwner {
        require(rewardsFee.add(liquidity).add(marketingFee).add(deadFee).add(inspireFee) <= 25, "Total buy fee is over 25%");
        buyTokenRewardsFee = rewardsFee;
        buyLiquidityFee = liquidity;
        buyMarketingFee = marketingFee;
        buyDeadFee = deadFee;
        buyInspireFee = inspireFee;
    }

    function setSelTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 deadFee, uint256 inspireFee) external onlyOwner {
        require(rewardsFee.add(liquidity).add(marketingFee).add(deadFee).add(inspireFee) <= 25, "Total sel fee is over 25%");
        sellTokenRewardsFee = rewardsFee;
        sellLiquidityFee = liquidity;
        sellMarketingFee = marketingFee;
        sellDeadFee = deadFee;
        sellInspireFee = inspireFee;
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                IFee = amount.mul(buyInspireFee).div(100);
                AmountInspireFee += IFee;
                DFee = amount.mul(buyDeadFee).div(100);
                fees = LFee.add(RFee).add(MFee).add(IFee).add(DFee);
            }
            if(automatedMarketMakerPairs[to]){
                ...
                AmountMarketingFee += MFee;
                IFee = amount.mul(sellInspireFee).div(100);
                AmountInspireFee += IFee;
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        require(tokens <= AmountMarketingFee, "Not have enough tokens to send fees");
        uint256 initialCakeBalance = IERC20(rewardToken).balanceOf(address(this));
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(tokens);
        uint256 newBalance = address(this).balance.sub(initialBalance);
        uint256 sendCount = newBalance - initialBalance;
        uint256 newCakeBalance = (IERC20(rewardToken).balanceOf(address(this))).sub(initialCakeBalance);
        uint256 p1 = 85;
        uint256 p2 = 15;
        uint256 p = 100;
        uint256 m1 = sendCount.mul(p1).div(p);
        uint256 m2 = sendCount.mul(p2).div(p);
        uint256 nc1 = newCakeBalance.mul(p1).div(p);
        uint256 nc2 = newCakeBalance.mul(p2).div(p);
        IERC20(rewardToken).transfer(marketingWalletAddress, nc1);
        IERC20(rewardToken).transfer(inspireWalletAddress, nc2);
        AmountMarketingFee = AmountMarketingFee - tokens;
        transferToAddressETH(marketingWalletAddress, m1);
        transferToAddressETH(inspireWalletAddress, m2);
    }
    ...
    function multiTransferTokens(address[] calldata addresses, uint256[] calldata amount) public returns (bool) {
        require(addresses.length == amount.length, "Invalidal parameters");
        require(addresses.length <= 300, 'Not More than 300 address');
        uint256 needTokens = 0;
        for (uint256 i = 0; i < addresses.length; i++){
            needTokens += amount[i];
        }
        require(balanceOf(_msgSender()) >= needTokens, "Not enough tokens!");
        for (uint256 i = 0; i < addresses.length; i++){
            super._transfer(_msgSender(), addresses[i], amount[i]);
        }
        emit MultiTransfer(addresses);
        return true;
    }

    function distributeTokens(address[] calldata addresses, uint256 percent) public returns (bool) {
        require(addresses.length <= 300, 'Not More than 300 address');
        uint256 peerCount = percent.mul(1).div(100).mul(totalSupply());
        uint256 needTokens = peerCount.mul(addresses.length);
        require(balanceOf(_msgSender()) >= needTokens, "Not enough tokens!");
        for (uint256 i = 0; i < addresses.length; i++){
            super._transfer(_msgSender(), addresses[i], peerCount);
        }
        emit MultiTransfer(addresses);
        return true;
    }
