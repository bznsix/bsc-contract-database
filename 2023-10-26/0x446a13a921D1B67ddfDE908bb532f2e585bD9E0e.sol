...
contract PrivateSale_Claim is Ownable{
    LPP_PrivateSale public privateSaleContract;  // Declare an instance of LPP_PrivateSale
    PublicSale_Claim_1 public privateSaleClaimContract;
    uint256 public claimStartDate = block.timestamp; // Timestamp when the claim period starts
    uint256 public claimInterval = 30 days; // Interval between each claim
    uint256 public totalClaimRounds = 13; // Total number of claim rounds (including the initial release)
    mapping (address => uint256) public claimed;
    ...
    constructor() {
        // Initialize the LPP_PrivateSale instance with an existing contract address
        privateSaleContract = LPP_PrivateSale(0xA53226F1B93Ea3700f83c793C72763aC547C935F);
        privateSaleClaimContract = PublicSale_Claim_1(0x764267e84f68B981d822c5Fc02749991aec6bE80);
    }
    ...
    function calculateClaimableAmount(address account) public view returns (uint256) {
        ...
        uint256 userDate;
        if(currentClaimCount[account]==0 && oldClaimCount(account)==0){
            userDate = claimStartDate;
        }else{
            userDate = claimTime[account];
        }
        ...
        if(claimed[account] + (claimableAmount*currentRound) + oldClaimed(msg.sender)  > privateSaleContract.checkContributionUsdt(account)){
            return privateSaleContract.checkContributionUsdt(account) - claimed[account] - oldClaimed(msg.sender);
        }else{    
            return claimableAmount*currentRound;
        }
    ...
    function oldClaimtime(address account) public view returns(uint256){
        return privateSaleClaimContract.claimTime(account);
    }

    function oldClaimCount(address account) public view returns(uint256){
        return privateSaleClaimContract.currentClaimCount(account);
    }

    function oldClaimed(address account) public view returns(uint256){
        return privateSaleClaimContract.totalClaimed(account);
    }
    ...
    function claim() external {
        ...
        if(privateSaleClaimContract.currentClaimCount(msg.sender)>0){
            currentClaimCount[msg.sender] = privateSaleClaimContract.currentClaimCount(msg.sender);
        }
...
contract PrivateSale_Claim_FirstIssue is Ownable{
    LPP_PrivateSale public privateSaleContract;  // Declare an instance of LPP_PrivateSale
    PublicSale_Claim_1 public privateSaleClaimContract;
    PrivateSale_Claim public privateSaleClaimContractOld;
    ...
    constructor() {
        ...
        privateSaleContract = LPP_PrivateSale(0xA53226F1B93Ea3700f83c793C72763aC547C935F);
        privateSaleClaimContract = PublicSale_Claim_1(0x764267e84f68B981d822c5Fc02749991aec6bE80);
        privateSaleClaimContractOld = PrivateSale_Claim(0xa3D8710783C40c9b4f2791Bd30361908d0EC5442);
    }
    ...
    function calculateClaimableAmount(address account) public view returns (uint256) {
        ...
        uint256 userDate;
        if(currentClaimCount[account]==0 && oldClaimed(account)==0 && oldClaimed1(account)==0 && originalClaimed(account)==0){
            userDate = claimStartDate;
        }else{
            userDate = claimTime[account];
        }

        if(currentClaimCount[account]==0 && originalClaimed(account)>0){
            userDate = 1692184675;
        }
        if(currentClaimCount[account]==0 && oldClaimtime(account)>0){
            userDate = oldClaimtime(account);
        }
        if(currentClaimCount[account]==0 && oldClaimtime1(account)>0){
            userDate = oldClaimtime1(account);
        }
        ...
        if(claimed[account] + (claimableAmount*currentRound) + oldClaimed(msg.sender) + oldClaimed1(msg.sender) + originalClaimed(msg.sender)  > privateSaleContract.checkContributionUsdt(account)){
            return privateSaleContract.checkContributionUsdt(account) - (claimed[account] + oldClaimed(msg.sender) + oldClaimed1(msg.sender) + originalClaimed(msg.sender));
        }else{    
            return claimableAmount*currentRound;
        }
    ...
    function oldClaimtime1(address account) public view returns(uint256){
        return privateSaleClaimContractOld.claimTime(account);
    }



    function oldClaimed(address account) public view returns(uint256){
        return privateSaleClaimContract.claimed(account);
    }
    function oldClaimed1(address account) public view returns(uint256){
        return privateSaleClaimContractOld.claimed(account);
    }
    function originalClaimed(address account) public view returns(uint256){
        return privateSaleContract.claimed(account);
    }


    function claimTimeEstimate(address account) public view returns(uint256){
        uint256 userDate;
        if(currentClaimCount[account]==0 && oldClaimed(account)==0 && oldClaimed1(account)==0 && originalClaimed(account)==0){
            userDate = claimStartDate;
        }else{
    ...
    function claim() external {
        ...
        if(privateSaleClaimContract.currentClaimCount(msg.sender)>0 && claimed[msg.sender]==0){
            currentClaimCount[msg.sender] = privateSaleClaimContract.currentClaimCount(msg.sender);
        }
        if(privateSaleClaimContract.currentClaimCount(msg.sender)>0 && privateSaleClaimContractOld.currentClaimCount(msg.sender)>0 && claimed[msg.sender]==0){
            currentClaimCount[msg.sender] = privateSaleClaimContractOld.currentClaimCount(msg.sender);
        }
    ...
    function addClaimed(address _user, uint256 _amount) external onlyOwner {
