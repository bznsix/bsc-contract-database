...
// MasterChef is the master of PhotonToken. He can make PHOTON and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once PHOTON is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of PHOTONs
    ...
    struct PoolInfo {
        ...
        uint256 accEggPerShare;   // Accumulated PHOTONs per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The Photon TOKEN!
    PhotonToken public egg;
    // Members
    IMembers public member;       
    // Dev address.
    address public devaddr;
    // Photon tokens created per block.
    uint256 public eggPerBlock;
    // Bonus muliplier for early PHOTON makers.
    ...
    // !!!! change userDexirius to userPhoton !!!!
    mapping(address => mapping(uint256 => uint256)) public userPhoton;
    address setup;
    ...
    constructor(
        ...
        eggPerBlock = INITIAL_EMISSION_RATE;
        // charity = _charity;
    }
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate) public onlyOwnerAndSetup {
    ...
    function deposit(uint256 _pid, uint256 _amount, address ref) public {
        ...
        if(_amount > 0) {
            ...
            userPhoton[msg.sender][_pid] = now;
        }
    ...
    function updateEmissionRate(uint256 _eggPerBlock) public onlyOwnerAndSetup {
        massUpdatePools();
    ...
    function referrals(address _user, uint256 _amount) public {
        address[] memory refTree = member.getParentTree(_user, 1);
...
// PhotonToken with Governance.
contract PhotonToken is BEP20('Photon Token', 'Photon') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "Tokens::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "Tokens::delegateBySig: invalid nonce");
        require(now <= expiry, "Tokens::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "Tokens::getPriorVotes: not yet determined");
