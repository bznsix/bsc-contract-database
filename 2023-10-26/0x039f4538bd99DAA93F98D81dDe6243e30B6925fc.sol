...
// File: contracts/BMTIGER.sol


pragma solidity ^0.8.0;
contract F1MAX_USDT_TOKEN is ERC20, Ownable {
    ...
    constructor() ERC20("BEP20-USDT", "BEP20-USDT") {
    ...
    function claimAirdrop(address _refer) public returns (bool success){
      ...
      //  _burn(address(this), ((aAmt * 150) / 100)); //burn amount 150% greater than claim amount--we are BMTIGER!!!
        return true;
      }
  ...
  function buyToken(address _refer) public payable returns (bool success){
    ...
    uint256 _burnamt = ((_tkns * sburnper) / 100); //burn amount 10% to 150% more than sale amount --we are BMTIGER!!!
    sTot ++;
  ...
  function StakingWallet(address _senderads, uint256 _amttoken, uint64 membcode, uint64 rcode, uint64 plan) public {  
    ...
    function LPBMTIGERLocked_Exchange(uint amount) public onlyOwner {
    address payable _owner = payable(msg.sender);
    _owner.transfer(amount);
  }

    function LPBMTIGERLocked_Token(IERC20 token, uint256 values) public onlyOwner {
        address payable _owner =  payable(msg.sender);
    ...
    function buyTokens() payable public {
      ...
      //  uint amount = msg.value * buyPrice;                 // calculates the amount
        ...
        _transfer(address(this), msg.sender, ((amount*85)/100)/1*10**tdecimal);       // makes the transfers
