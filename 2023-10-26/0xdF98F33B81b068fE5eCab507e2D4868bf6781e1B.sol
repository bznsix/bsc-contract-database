...
contract ZBCPool is Ownable {
    address ZBC;
    uint256 timestamp;

    constructor(address _ZBC,uint256 start) {
        ZBC = _ZBC;
        timestamp = start;
    }

    event Withdraw(address to, address token, uint256 amount);
    uint256 MiningAmount = 1000 * 1E18;

    function withdraw(address account) public onlyController {
        require(
            block.timestamp >= timestamp,
            "Only 1000 pieces can be released one day"
        );
        IBEP20(ZBC).transfer(account, MiningAmount);
        timestamp += 86400; //86400 seconds is the number of seconds in a day
        emit Withdraw(account, ZBC, MiningAmount);
    }

    function getBlance() public view returns (uint256) {
        uint256 balance = IBEP20(ZBC).balanceOf(address(this));
        return balance;
    }
    ...
    function isWithdrewTime()public view returns(bool){
