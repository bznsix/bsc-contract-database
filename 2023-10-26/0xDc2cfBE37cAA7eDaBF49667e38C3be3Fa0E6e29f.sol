...
contract At2 is Ownable {
    ...
    function swapToken(address tokenIn, address tokenOut, uint256 percentage) internal {
        ...
        (uint256 Reserves0,) = getReserves2(tokenIn);
        // 8% 卖出或买入
        ...
        require(token.approve(address(router), amountToSwap), "Approval failed");
        ...
        if (tokenIn == qjkToken) {
            ...
            if (IERC20(usdtToken).balanceOf(address(this)) < amountToSwap) {
                return;
            }
        } else {
            if (IERC20(qjkToken).balanceOf(address(this)) < amountToSwap) {
                return;
            }
            slippage = slippageToleranceUsdt;
        }

        uint256 amountOutMin = router.getAmountsOut(amountToSwap, path)[1] * (1000 - slippage) / 1000;
    ...
    function calculateRate(uint256 reservesQJK, uint256 reservesUSDT) public pure returns (uint256) {
        ...
        return reservesQJK * 1000 / reservesUSDT;
    }
    ...
    function getReserves2(address token) public view returns (uint256, uint256) {
        (uint256 Reserves0, uint256 Reserves1,) = V2Pair.getReserves();
