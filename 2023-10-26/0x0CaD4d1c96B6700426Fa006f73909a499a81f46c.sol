...
contract SPC is ERC20, Ownable {
    ...
    uint256 public sellmarketingFee = 280;
    uint256 public lpRewardFee = 10; 
    ...
    address public marketingWalletAddress=0x2E083a3072e3b161E7Db08b9413B0Db9f923cbB8; 
   ...
   TokenDividendTracker public dividendTracker;
    ...
    ) payable ERC20("SPC", "SPC")  {
        uint256 totalSupply = 130000000 * (10**18);
    ...
    function setMarketingFee(uint256 val) public onlyOwner {
       ...
       function setsellMarketingFee(uint256 val) public onlyOwner {
        sellmarketingFee = val;
    }
    ...
    function setkill(uint256 _kill)public onlyOwner {
        kill  = _kill;
    }
    function setblack(address _address,bool val)public onlyOwner{
        black[_address] = val;
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            if(AmountLpRewardFee > 0){
               ...
               if(AmountMarketFee > 0){
                swapmerketToken(AmountMarketFee);
                AmountMarketFee = 0;
            }
            swapping = false;
        }
        ...
        if(takeFee) {
            	...
            	if(startTime.add(kill)>= block.timestamp){
            ...
            amount =  takeAllFee(from, amount,_type); 
    ...
    function takeAllFee(address from,uint256 amount,uint256 _type) private returns(uint256 amountAfter) {
        amountAfter = amount;
        uint256 MFee = 0;
        if(_type == 1){
         MFee = amount.mul(marketingFee).div(1000);
        }else if(_type == 2){
         MFee = amount.mul(sellmarketingFee).div(1000);
        }
        ...
        super._transfer(from, address(this), MFee.add(LPFee));
    }
    ...
    function swapmerketToken(uint256 tokenAmount) private {
