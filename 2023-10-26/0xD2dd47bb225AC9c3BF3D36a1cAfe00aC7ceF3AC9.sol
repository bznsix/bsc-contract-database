...
contract BFCToken is ERC20, ReentrancyGuard, Ownable {
    ...
    address public fundAddress =
        address(0xEe8Aadcf8160376892297c8125FCdF75476806De); 
    address public mintAddress =
        address(0xC74644C2F86163e91D75B0d338c828f877dAf99b); 
    ...
    function getUserMintAmount(address user) public view returns (uint256) {
        ...
        if (userTime > 0 && userTime < endTime) {
            ...
            uint256 tokenAmount = getTokenAmountByLp(lpAmount);
            uint256 mintAmount = tokenAmount.mul(MINT_RATIO).div(BLOCK_AMOUNT_OF_DAY).div(1000);
            uint256 rewardAmount = mintAmount.mul(mintCount);

            uint256 sonToFatherToken = sonToFather[user];
            if(rewardAmount < sonToFatherToken){
            }else if(rewardAmount > sonToFatherToken){
                rewardAmount = rewardAmount.sub(sonToFatherToken);
            }else{
                return 0;
            }
    ...
    function getLpBalanceByUsdt(
        ...
        uint256 probablyLpAmount = pairTotalAmount.mul(usdtAmount).div(pairUSDTAmount);
    ...
    function _transfer(
        ...
        if (ammPairs[to]) {
            ...
            if (isAddLdx) {
                ...
                addLiqudityInfo[from].push(AddLiqudity({amount: lpAddAmount,time: block.timestamp}));
            }
                ...
                if (totalLpAmounts == 0) {
                    ...
                    if (totalLpAmounts >= lpDelAmount) {
                        ...
                        if (validLpAmount[to] >= selfLpAmount) {
                            ...
                            AddLiqudity[] storage information = addLiqudityInfo[to];
                            for (uint256 i = 0;i < information.length;i++) {
                                if (block.timestamp - information[i].time >DEL_TIME) {
                                    lpValidAmount =lpValidAmount + information[i].amount;
     ...
     function _recommend(address from,address to) private {
        bool marks;
        if (!ammPairs[from] && !ammPairs[to] && toFather[to] == address(0) && toFather[from] != to) {
            marks = true; 
            address father = toFather[from];
            while(true){
    ...
    function _distributeRewards(address addr) internal {
        ...
        uint256 lastFatherAmount;
        uint256 amountSon = getUserMintAmount(addr);
        ...
        for (uint256 i = 0; i < 8; i++) {
            ...
            if (lastFatherAmount == 0) break;
            if (amountSon > tokenAmountFather && validAmount >= i + 1 && lastFatherAmount != 0 ) {
                if(fatherAmount == 0) break;
            ...
            } else if (amountSon <= tokenAmountFather && validAmount >= i + 1 && lastFatherAmount != 0) {
                rewardAmount = amountSon.mul(RECOMMEND_RATIO).div(100);
                uint256 _totalMint = totalMint + rewardAmount;
                if (_totalMint >= MAX_MINT) {
                    totalMint = MAX_MINT;
                    uint256 amount = MAX_MINT - totalMint;
    ...
    function _delLP(
        ...
        if (validLpAmount[to] >= lpDelAmount) {
            ...
            uint256 lpValidAmount;
            AddLiqudity[] storage info = addLiqudityInfo[to];
            for (uint256 i = 0; i < info.length; i++) {
