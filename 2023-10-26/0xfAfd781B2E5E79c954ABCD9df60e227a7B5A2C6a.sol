...
// main contract
contract SPX6900Token is IERC20Extended, Auth {
    using SafeMath for uint256;

    string private constant _name = "SPX6900";
    string private constant _symbol = "SPX ";
    uint8 private constant _decimals = 9;
    uint256 private constant _totalSupply =
        420_000_000_000 * 10 ** _decimals;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountReflection = contractBalance.mul(_reflectionFeeCount).div(totalFee);
        if (amountReflection > 0) {
            _basicTransfer(address(this), address(distributor), amountReflection);
            try distributor.deposit(amountReflection) {} catch {}
        }
        uint256 amountToSwap = contractBalance.sub(amountReflection);
    ...
    function setSellFees(
        ...
        totalSellFee = _reflectionFee.add(_marketingFee);
        feeDenominator = _feeDenominator;
        require(
