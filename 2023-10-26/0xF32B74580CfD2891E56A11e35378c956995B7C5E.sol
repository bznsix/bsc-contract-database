   ...
   function checkteamconditions(address _user) private view returns(uint256, uint256, uint256 ) {
    ...
    function _deposit(address _user, uint256 _amount) private {
        ...
        {  
           ...
           if(user.totalDepositbeforeclaimed<=500e18){
             user.totalDeposit = user.totalDeposit.add(user.totalDepositbeforeclaimed);
             user.totalDepositbeforeclaimed = 0;
           } 

           if(user.totalDepositbeforeclaimed>500e18){
             user.totalDeposit = user.totalDeposit.add(500e18);
             user.totalDepositbeforeclaimed =user.totalDepositbeforeclaimed-500e18;
           }
        ...
        {  
             if(user.maxDeposit < _amount){
                ...
                uint256 amountallow= 0;
                userclaim.acheived = block.timestamp;
              
                if(user.directnum<5){
                  amountallow = 500e18;
                }
                if(user.directnum>=5 && user.directnum<=9){
                  amountallow = 1000e18;
                }
                 if(user.directnum>=10 ){
                  amountallow = 2000e18;
                }
                   if(user.totalDepositbeforeclaimed>=amountallow){
                   user.totalDeposit = user.totalDeposit.add(amountallow);
                   user.totalDepositbeforeclaimed = user.totalDepositbeforeclaimed-amountallow;
                   }
                   else 
                   {
                       user.totalDeposit = user.totalDeposit.add(user.totalDepositbeforeclaimed);
