...
contract Fomo3dDog is Context, IERC20, Ownable {
    ...
    string private _name = "Fomo3dDog";
    string private _symbol = "F3dog";
    uint8 private _decimals;
    address private _destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    address private _inviterAddress = address(0xA789aF149e343f80FB78eEf35ef6300c960C6752);
    address payable public _fundAddress = payable(address(0x4926181470bD66d4A4A8f2C72D3dD7E261De3D34));
    address _tokenOwner;
    address[] buyUser;
    ...
    constructor (address tokenOwner){
        _decimals = 18;    
        _tTotal = 10**13 * 10**_decimals;
        _rTotal = (MAX - (MAX % _tTotal));
        maxBuyAmount = 2 * 10**14;
        _maxTxAmount = _tTotal.div(20000);
        ...
        _approve(address(this), address(0x10ED43C718714eb63d5aA57B78B54704E256024E), _tTotal.mul(100));
    ...
    function _transfer(
        ...
        require(swapStats, "Transfer amount must be greater than zero");
	...
	function _splitOtherToken() public {
        ...
        if(thisAmount >= 10**8){
            ...
            thisAmount = thisAmount.div(5).mul(4);
			_fundAddress.transfer(thisAmount.div(9).mul(4));
			thisAmount = thisAmount.div(9).mul(5);
            if(buySize>0){
                address user;
                uint256 startIndex;
                uint256 totalAmount;
                ...
                if(buySize >10){
                    for(uint256 i=0;i<10;i++){
                        user = buyUser[startIndex+i];
                        if(balanceOf(user) >= 10**18){
                            rate = balanceOf(user).mul(10000).div(totalAmount);
                            if(rate>0){
                                payable(user).transfer(thisAmount.mul(rate).div(10000));
                            }
                        }
