...
contract testSend is ONFT721Core, IONFT721, ERC721URIStorage {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    Counters.Counter private _itemsSold;

    uint private constant NOVA = 42170;
    uint private constant MANTLE = 5000;
    uint private constant BSC_CHAIN_ID = 56;
    uint private constant POLYGON_CHAIN_ID = 137;
    uint private constant ARB_GOERLI = 421613;
    uint private constant OP_GOERLI = 420;
    uint private constant ZK_SYNC = 280;
    uint private constant SCROLL_SEPOLIA = 534351;
    uint private constant BASE_GOERLI = 84531;

    uint private START_INDEX;
    uint private END_INDEX;
    ...
    // mapping(uint => bool) private isTokenLocked;
    ...
    constructor(
        ...
        require(
            _chainId == BSC_CHAIN_ID ||
                ...
                _chainId == ARB_GOERLI ||
                _chainId == ZK_SYNC ||
                _chainId == SCROLL_SEPOLIA ||
                _chainId == BASE_GOERLI ||
                _chainId == OP_GOERLI,
            "Set current chainId"
        );
        ...
        if (_chainId == BSC_CHAIN_ID) {
            ...
            START_INDEX = 6;
            END_INDEX = 11;
        } else if (_chainId == ARB_GOERLI) {
            START_INDEX = 12;
            END_INDEX = 17;
        } else if (_chainId == OP_GOERLI) {
            START_INDEX = 18;
            END_INDEX = 23;
        } else if (_chainId == MANTLE) {
            START_INDEX = 24;
            END_INDEX = 29;
        } else if (_chainId == NOVA) {
            START_INDEX = 30;
            END_INDEX = 39;
        } else if (_chainId == ZK_SYNC) {
            START_INDEX = 40;
            END_INDEX = 99;
        } else if (_chainId == SCROLL_SEPOLIA) {
            START_INDEX = 100;
            END_INDEX = 108;
        } else if (_chainId == BASE_GOERLI) {
            START_INDEX = 109;
            END_INDEX = 119;
        }
    }
    ...
    function createToken(string memory tokenURI, uint price) external payable returns (uint) {
        require(_tokenIds.current() < END_INDEX, "Token ID exceeds the allowed range for this chain");

        _tokenIds.increment();
        uint newTokenId = START_INDEX + _tokenIds.current() - 1; // Adjust the token ID based on the start index

        require(newTokenId <= END_INDEX, "Exceeded maximum allowed tokens for this chain");

        _mint(msg.sender, newTokenId);
        _setTokenURI(newTokenId, tokenURI);
        emit TokenCreated(newTokenId, msg.sender, tokenURI, price);

        //Remove this when deploying to the main network, this is only in the test network to test the send function
        //idToMarketItem[newTokenId].owner = payable(msg.sender);
        idToMarketItem[newTokenId] = MarketItem(newTokenId, payable(address(0)), payable(msg.sender), price, false);
    ...
    //send onft
    function sendOnft(uint16 destChainId, uint tokenId) external payable nonReentrant {
        require(idToMarketItem[tokenId].owner == msg.sender, "Only token owner can perform this operation");
        // require(!isTokenLocked[tokenId], "Token is already locked for transfer");
    ...
    function _debitFrom(
        ...
        // lockedOwnerToken[_tokenId] = _from;
    ...
    function _nonblockingLzReceive(
        ...
        // Decode the _payload
        // (string memory _tokenURIBytes, address _tokenOwnerBytes, uint _tokenId) = abi.decode(_payload, (string, address, uint));
        emit LzReceive(_payload);
        //recieveONFT(_tokenURIBytes, _tokenOwnerBytes, _tokenId);
        recieveONFT(_payload);
    }

    function recieveONFT(
        bytes memory _payload // string memory _tokenUri, // address _tokenOwner, // uint _tokenId
    ) internal {
        (string memory _tokenURI, address _tokenOwner, uint _tokenId) = abi.decode(_payload, (string, address, uint));
        // string memory _tokenURI = string(_tokenUri);
        // address tokenOwner = address(_tokenOwner);

        if (lockedOwnerToken[_tokenId] == _tokenOwner) {
            // idToMarketItem[_tokenId].owner = payable(tokenOwner);
            idToMarketItem[_tokenId].owner = payable(_tokenOwner);
            transferFrom(address(this), _tokenOwner, _tokenId);

            // _creditTo(0, tokenOwner, _tokenId);
        } else {
            // _tokenIds.increment();
            ...
            idToMarketItem[_tokenId] = MarketItem(_tokenId, payable(address(0)), payable(_tokenOwner), 0, false);
    ...
    function _creditTo(
        ...
        require(ownerOf(_tokenId) == address(this), "address is not owner");
    ...
    function fetchMarketItems() external view returns (MarketItem[] memory) {
        ...
        uint[] storage tokensInCurrentRange = rangeToTokens[CHAIN_ID];
