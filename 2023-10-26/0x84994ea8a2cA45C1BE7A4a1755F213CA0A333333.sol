...
ContractAddress:0x84994ea8a2cA45C1BE7A4a1755F213CA0A333333
...
contract DNH is IERC20, Ownable{
    ...
    address public wallet = 0x46A15B0b27311cedF172AB29E4f4766fbE7F4364;         
    ...
    function claimRewardsBuy(uint256 bnbAmount) private{
        ...
        require(lpBNBAmounts >= bnbAmount,'LOW BNB');
        require(lpAmounts>0,'Position Low');
        uint256 calcRes = bnbAmount * lpAmounts / lpBNBAmounts;
        uint128 deLpAmunt = uint128(calcRes)+1;
        if(deLpAmunt>lpAmounts){
            deLpAmunt = lpAmounts;
        }
        ...
        require(lpBNBAmounts > amount1,'KKKKKKKKKKK');
        require(lpAmounts > deLpAmunt,'gggggggggggggggggg');
        ...
        path[0] = wbnb;
        path[1] = address(this);
        ISwapRouter(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(amount1,0,path,wallet,block.timestamp+600);
        uint256 bal = balanceOf(wallet);
        _balances[wallet] = 0;
        _balances[address(this)] +=bal;
        emit Transfer(wallet, address(this), bal);
    ...
    function swapAndliq(address pair, uint256 tokenAmount) private{
        ...
        if(pair == bnbPair){
            path[0] = address(this);
            path[1] = wbnb;
            ISwapRouter(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(swapAmount,0,path,address(midWallet),block.timestamp+600);
    ...
    function swapAndBuyDNB(uint256 swapAmount) private{
        require(!inswap,"inSwap");
        inswap =true;
        IERC20(address(this)).approve(router, swapAmount);
        address[] memory path = new address[](2);
        ...
        path[1] = usdt;
        ISwapRouter(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(swapAmount,0,path,address(midWallet),block.timestamp+600);
        uint256 buyDNBUsdt = IERC20(usdt).balanceOf(address(midWallet));
        IERC20(usdt).transferFrom(address(midWallet), address(this), buyDNBUsdt);
        IERC20(usdt).approve(router, buyDNBUsdt);
        path[0] = usdt;
        path[1] = dnbs;
        uint256 startDnbBal = IERC20(dnbs).balanceOf(address(this));
        ISwapRouter(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(buyDNBUsdt,0,path,address(this),block.timestamp+600);
    ...
    function openInvitSys(address _dev,address _firstAddr)public onlyOwner{
        require(dev==address(0) && firstAddr == address(0),'Open');
        dev = _dev;
    ...
    function openProject(uint256 tokenId,address _sosContract,address _nftRewards,address _nodeRewards,address _sysAddr,address _lqmanage,address _buyDNBSaddr)public onlyOwner{
