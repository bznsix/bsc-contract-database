...
contract NFTMarket1155 is AccessControl, MarketEventsV2, verification1155, VerifySignature, MarketStruct {
 ...
 function createSale(localSell memory sell) public
    ...
    ///@notice Additionally, a buyer can pay the asking price to conclude a sale of an NFT.
     function makeBid(Bid memory bid) external payable {
        ...
        _updateOngoingAuction(bid._nftContractAddress, bid._tokenId, bid._marketid, _sign, bid._value,bid.referals, bid.amounts, bid.totalReferals);
    }
    ...
    function _updateOngoingAuction(
        ...
        if (_sign) {
            if (_value >= buyNowPrice) {
                ...
                _transferNftAndPaySeller(_nftContractAddress, _tokenId, _marketId, _value,referals, amounts, totalReferals);
                return;
            }
        } else {
            if (_isBuyNowPriceMet(_nftContractAddress, _tokenId, _marketId)) {
                _transferNftAndPaySeller(_nftContractAddress, _tokenId, _marketId, 0,referals, amounts, totalReferals);
                return;
            }
        }
    }
   ...
   function _reversePreviousBidAndUpdateHighestBid(
        ...
        (address auctionERC20Token, address prevBidder,uint256 prevBid) = marketInfo.bidAndToken(_nftContractAddress, _tokenId, _marketId);
            ...
            if (prevBidder != address(0)) {
                _payout(prevBidder,prevBid,auctionERC20Token);
            }
    }
    ...
    function _transferNftAndPaySeller(
        ...
        _payFeesAndSeller( _nftContractAddress,_tokenId, _marketId,_nftSeller,_highestBid, _value, referals,amounts, totalReferals);
    ...
    function _payFeesAndSeller(
        ...
        feesPaid = _payoutroyalties(_nftContractAddress, _tokenId, _marketId, subtotal,auctionERC20Token);

        _payout(_nftSeller,(subtotal - feesPaid),auctionERC20Token);
    ...
    function _payoutroyalties(
        ...
        _payout(_feeRecipients,fee,token);
        }
        return fee; 
    }
    ...
    function takeHighestBid(address _nftContractAddress, uint256 _tokenId, uint256 _marketid,address[] calldata referals,uint256[] calldata amounts,uint256 totalReferals) external {
...
abstract contract verification1155 {
