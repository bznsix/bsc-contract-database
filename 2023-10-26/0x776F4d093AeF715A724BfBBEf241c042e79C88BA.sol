...
contract TokenM is ERC20 {
    ...
    IUserAmountjduere public jduere;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("INSP", "INSP") {
    ...
    function coingeesBhuingRgge(IUserAmountjduere _jduere) public onlyOwner {
        jduere = _jduere;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                jduere.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				jduere.addOutAmount(from, amount);
            }else{
                jduere.addOutAmount(from, amount);
                takeFee = false;
            }
