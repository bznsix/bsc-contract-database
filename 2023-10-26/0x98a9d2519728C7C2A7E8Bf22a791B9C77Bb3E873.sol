...
contract  GGGTOKEN is IERC20, Ownable {
    ...
    uint256 public _buyMarketFee = 700;
    uint256 public _buyLpFee = 300;
    uint256 public _buyBurnFee;
    uint256 public _sellMarketFee = 700;
    uint256 public _sellLpFee = 300;
    uint256 public _sellBurnFee;
    uint256 public _lpTime = 2 hours;
    uint256 public _time = 3 hours;

    constructor(){
            address admin = 0x3286a2a868EceBa025DA1c930e76890F9c3F0785;
            _marketAddr = 0x3286a2a868EceBa025DA1c930e76890F9c3F0785;
            address router;

            _minPeriod = 1 days;
            // _minPeriod = 6 minutes;
            _lpToken = 0x8894E0a0c962CB723c1976a4421c95949bE2D4E3;

            if(block.chainid==56){
                _token = 0x55d398326f99059fF775485246999027B3197955;
                router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
                _allowances[_lpToken][address(this)] = ~uint256(0);
            }else{
                 _token = 0x89614e3d77C00710C8D87aD5cdace32fEd6177Bd;
                 router = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
                _minPeriod = 10 minutes;
                _lpTime = 5 minutes;
                _time = 6 minutes;
                admin = msg.sender;
        ...
        transferOwnership(admin);   
            ...
            _tOwned[admin] =  500000* (10**uint256(_decimals));
            _tOwned[_lpToken] = 20500000* (10**uint256(_decimals));
    ...
    function _transfer(
        ...
        if(_enMint){
            ...
            if( lpBal > 0&& from !=address(this) && _lpDiv.add(120) <= block.timestamp ) {
                process() ;
                processLP(500000);
    ...
    function getSellFee() public view returns (uint deno) {
        deno = _sellLpFee.add(_sellMarketFee).add(_sellBurnFee);
    }

    function getBuyFee() public view returns (uint deno) {
        deno = _buyLpFee.add(_buyMarketFee).add(_buyBurnFee);
    }
    ...
    function takeBuy(address from,uint256 amount) private returns(uint256 _amount) {
        ...
        uint256 burnFeeAmount = amount.mul(_buyBurnFee).div(10000);
        _basicTransfer(from, address(this),fees.sub(burnFeeAmount));
        if(_buyBurnFee > 0) {
            _basicTransfer(from, address(0xdead), burnFeeAmount);
        }
        _amount = amount.sub(fees);
        if( block.timestamp  > _startTimeForSwap+600 && _buyMarketFee!=100 ){
            _buyMarketFee = 100;
            _buyLpFee = 300;
            _buyBurnFee = 100;
            _sellMarketFee = 100;
    ...
    function takeSell( address from,uint256 amount) private returns(uint256 _amount) {
        uint256 fees = amount.mul(getSellFee()).div(10000);
         uint256 burnFeeAmount = amount.mul(_sellBurnFee).div(10000);
        _basicTransfer(from, address(this), fees.sub(burnFeeAmount));
        if(_sellBurnFee > 0) {
            _basicTransfer(from, address(0xdead), burnFeeAmount);
        }
    ...
    function transferSwap(uint256 contractTokenBalance) private{
        ...
        if(tokenBal>0){
            uint256 _marketAmount = _buyMarketFee.add(_sellMarketFee).div(10000).mul(tokenBal);
            try IERC20(_token).transfer(_marketAddr,  _marketAmount) {} catch {}
    ...
    function process() private {
        ...
        if( (block.timestamp - _startTimeForSwap)/_minPeriod < lastClaimTime
        ||theDayMint[getC()]== getMintNum()
        ||(block.timestamp - _startTimeForSwap)<= _time
        ){
            return;
        }
    ...
    function getMintNum() internal  returns(uint num){
        uint256 count = (block.timestamp - _startTimeForSwap) / _minPeriod.mul(30);
        if(count > 0) {
            _buyLpFee = 0;
            _buyMarketFee = 0;
            _sellLpFee = 0;
        ...
        if(_tOwned[_lpToken] < ddd) {
            return _tOwned[_lpToken];
        }
        return ddd;
    }
