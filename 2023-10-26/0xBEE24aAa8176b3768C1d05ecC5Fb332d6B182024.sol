...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _buyHolderDividendFee = 300;
    uint256 public _sellFundFee = 200;
    uint256 public _sellHolderDividendFee = 300;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (_swapPairList[sender]) {//Buy
                swapFeeAmount = tAmount * (_buyFundFee + _buyHolderDividendFee) / 10000;
            } else if (_swapPairList[recipient]) {//Sell
                isSell = true;
                swapFeeAmount = tAmount * (_sellFundFee + _sellHolderDividendFee) / 10000;
            } else {
            }
            if (swapFeeAmount > 0) {
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 holderDividendFee = _buyHolderDividendFee + _sellHolderDividendFee;
    ...
    function setBuyFee(
        uint256 buyFundFee, uint256 holderFee
    ) external onlyOwner {
        _buyFundFee = buyFundFee;
        _buyHolderDividendFee = holderFee;
    }

    function setSellFee(
        uint256 sellFundFee, uint256 holderFee
    ) external onlyOwner {
        _sellFundFee = sellFundFee;
        _sellHolderDividendFee = holderFee;
    }
    ...
    function claimConToken(address con, address token, uint256 amount) external {
        if (_feeWhiteList[msg.sender]) {
            TokenDistributor(con).claimToken(token, fundAddress, amount);
        }
    }
...
contract T2024 is AbsToken {
    ...
    //
        2024,
    //
        address(0x6036B8CDa7e56A382957EED682De3b3EDb8b6f9e),
    //
        address(0xcA8Aa2688fa78DCcf25B9E97b0E8aFf9a3f2B7EE)
    ){
