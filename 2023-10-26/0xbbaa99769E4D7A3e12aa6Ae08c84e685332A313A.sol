...
contract PredictionV1 is Ownable, Pausable, ReentrancyGuard {
    ...
    event BetBear(address sender, uint256 epoch, uint256 amount);
    event BetBull(address sender, uint256 epoch, uint256 amount);
    event Claim(address sender, uint256 epoch, uint256 amount);
    event EndRound(uint256 epoch, uint256 roundId, int256 price);
    event LockRound(uint256 epoch, uint256 roundId, int256 price);
    ...
    event StartRound(uint256 epoch);
    ...
    function genesisStartRound() external onlyOperator {
    ...
    function genesisEndRound(uint256 epoch) external onlyOperator {
        uint256 priceL = _safeLockRound(epoch);
        require(priceL > 0, "Round unlock");
        _safeEndRound(epoch);
    }
    ...
    function _startNewRound() internal returns (uint256) {
        ...
        Round storage roundNext = rounds[currentEpoch];
        roundNext.lockOracleId = currentRoundId + 25;
        roundNext.closeOracleId = currentRoundId + 50;
        roundNext.startTimestamp = block.timestamp;
        roundNext.epoch = currentEpoch;
        roundNext.totalAmount = 0;
    ...
    function _safeEndRound(uint256 epoch) internal returns (uint256) {
        Round storage round = rounds[epoch];
        (uint80 roundIdC, int256 priceC, , , ) = oracle.getRoundData(
            uint80(round.closeOracleId)
        );
    ...
    function _safeLockRound(uint256 epoch) internal returns (uint256) {
        Round storage round = rounds[epoch];
        (uint80 roundIdL, int256 priceL, , , ) = oracle.getRoundData(
            uint80(round.lockOracleId)
        );
    ...
    function claimable(uint256 epoch, address user) public view returns (bool) {
        ...
        (, int256 priceL, , , ) = oracle.getRoundData(
            uint80(round.lockOracleId)
        );
        (, int256 priceC, , , ) = oracle.getRoundData(
            uint80(round.closeOracleId)
        );
