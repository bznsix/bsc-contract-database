...
contract ShillMuun is Context, ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public DividendsToken = address(0x2900e6b68658128784B9a1de242F811d938d8bA7);
   ...
   DividendTracker public dividendTracker;
    ...
    address private _devWallet3;
    address private _devWallet4;
    ...
    uint256 private sellTimes;
    uint256 private lastSell;
    ...
    uint256 private constant DefaultTime = 4 weeks;
    ...
    constructor() ERC20 ("ShillMuun", "SHLM") {
        _buyRewardsFee = 2;
        _buyLiquidityFee = 1;
		_buyMarketingFee = 3;
        _buyDevFee = 1;
        ...
        _maxWallet = 2500000 * (10**18);
        _maxBuy = 2500000 * (10**18);
        _maxSell = 2500000 * (10**18);

        totalBuyFees = _buyRewardsFee.add(_buyLiquidityFee).add(_buyMarketingFee).add(_buyDevFee).add(_buyBuyBackFee);
        totalSellFees = _sellRewardsFee.add(_sellLiquidityFee).add(_sellMarketingFee).add(_sellDevFee).add(_sellBuyBackFee);

    	dividendTracker = new DividendTracker();

    	liquidityWallet = owner();
		marketingWallet = payable(0x0cb2c1F7Ac772c92477397723A6ec11775476177);
        _devWallet1 = payable(0x9aA6f2C7f457DFA3a48c2FF3F93Ca1aF88EDf465);
        _devWallet2 = payable(0x29Ce6Bf8B3Fe80d745eABe696Aa7C0610362Ed3a);
        _devWallet3 = payable(0xA728095fBd436513a2253dbE2B4264d773f4CB0d);
        _devWallet4 = payable(0x66a913fEfa687c742C888a100A8b1ab4F35d9bdB);
    ...
    function updatedividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ShillMuun The dividend tracker already has that address");

        DividendTracker newdividendTracker = DividendTracker(payable(newAddress));

        require(newdividendTracker.owner() == address(this), "ShillMuun: The new dividend tracker must be owned by the ShillMuun token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "ShillMuun: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ShillMuun: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ShillMuun: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ShillMuun: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ShillMuun: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "ShillMuun: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "ShillMuun: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ShillMuun: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ShillMuun: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
		...
		if(canSwap && !swapping && !automatedMarketMakerPairs[from] && from != liquidityWallet && to != liquidityWallet && from != marketingWallet && to != marketingWallet && !_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
            ...
            uint256 devSplit = devAmount.div(4);
            devAmount = devAmount.sub(devSplit).sub(devSplit).sub(devSplit);
            transferToWallet(payable(_devWallet1), devSplit);
            transferToWallet(payable(_devWallet2), devSplit);
            transferToWallet(payable(_devWallet3), devSplit);
            transferToWallet(payable(_devWallet4), devAmount);

            emit Transfer(address(this), _devWallet1, devSplit);
            emit Transfer(address(this), _devWallet2, devSplit);
            emit Transfer(address(this), _devWallet3, devSplit);
            emit Transfer(address(this), _devWallet4, devAmount);
            }
			...
			uint256 sellTokens = address(this).balance.mul(_sellBuyBackFee).div(swapAmount);
            sellTimes = (block.timestamp.sub(lastSell).div(60)).add(4);
            if (sellTimes > 23) {
                sellTimes = 23;
            }
            sellTokens = sellTokens.div(sellTimes);
            uint256 i = 0;

            while(i <= sellTimes) {

                buyBackAndBurn(sellTokens);
                i++;
            }
	...
	function letsGoLive() external onlyOwner() {
        updateBuyFees(4,4,2,2,3);
        updateSellFees(4,4,2,2,2);
    ...
    function updateBuyFees(uint8 newBuyLiquidityFee, uint8 newBuyMarketingFee, uint8 newBuyRewardsFee, uint8 newBuyDevFee, uint8 newBuyBuyBackFee) public onlyOwner {
    ...
    function updateSellFees(uint8 newSellLiquidityFee, uint8 newSellMarketingFee, uint8 newSellRewardsFee, uint8 newSellDevFee, uint8 newSellBuyBackFee) public onlyOwner {
...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    address private DividendToken = address(0x2900e6b68658128784B9a1de242F811d938d8bA7);
    ...
    constructor() DividendPayingToken("Dividend_Tracker", "Dividend_Tracker", DividendToken) {
        ...
        isAuth[owner()] = true;
        isAuth[address(0xA0C46A65AC842d8C18FA0a6a8AF3CbDe3215f82b)];
    }
