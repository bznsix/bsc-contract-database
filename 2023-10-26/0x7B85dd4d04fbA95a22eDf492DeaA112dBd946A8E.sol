...
3% Marketing 3% Rewards

**/

//Telegram: t.me/bitcoinetf_bsc
...
contract BTCETF is ERC20, Ownable {
    ...
    BTCETFDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Bitcoin ETF", "BTCETF") {
        marketingWallet = payable(0x4457d66F3d9A703678aE5Dd0a8433979A5D56Df1);
        devWallet = payable(0x4457d66F3d9A703678aE5Dd0a8433979A5D56Df1);
        ...
        dividendTracker = new BTCETFDividendTracker(
            payable(this),
            router,
...
contract BTCETFDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        BTCETFContract = BTCETF(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BTCETF_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BTCETF_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BTCETFContract);

            uint256 prevBalance = BTCETFContract.balanceOf(address(this));
            ...
            {
                uint256 received = BTCETFContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BTCETFContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
