...
contract ERC721MintContract is ReentrancyGuard, ERC721Enumerable, Ownable {
    ...
    function addCurrency(
        IERC20 _payToken,
        uint256 _ERC20publicCost,
        uint256 _ERC20whitelistCost,
        uint256 _ERC20royaltyPercentage,
        address _ERC20royaltyWallet
    ) external onlyOwner {
    ...
    function setETHInfo(
        uint256 _ETHpublicCost,
        uint256 _ETHwhitelistCost,
        uint256 _ETHroyaltyPercentage,
        address payable _ETHroyaltyWallet
    ) external onlyOwner {
        ethInfo = ETHInfo({
    ...
    function purchaseWithERC20(
        ...
        require(
            _numberOfTokens > 0 && _numberOfTokens <= maxMintAmount,
        ...
        if (isWhitelisted(msg.sender)) {
            ...
            require(
                payTokens[_token].ERC20paytoken.allowance(
                    msg.sender,
                    address(this)
                ) >= payTokens[_token].ERC20publicCost.mul(_numberOfTokens),
                "Allowance not sufficient"
            );
            cost = payTokens[_token].ERC20publicCost.mul(_numberOfTokens);
        }

        uint256 royaltyAmount = cost
            .mul(payTokens[_token].ERC20royaltyPercentage)
            .div(100);
        uint256 ownerAmount = cost.sub(royaltyAmount);

        payTokens[_token].ERC20paytoken.safeTransferFrom(
            msg.sender,
            payTokens[_token].ERC20royaltyWallet,
            royaltyAmount
        );
        payTokens[_token].ERC20paytoken.safeTransferFrom(
            msg.sender,
