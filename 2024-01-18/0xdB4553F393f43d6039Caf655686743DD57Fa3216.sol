...
contract TheBullionGameShapDailyV2 is
    VRFConsumerBaseV2,
    ReentrancyGuard,
    Ownable,
    ...
    AccessControl
{
    ...
    mapping(address => bool) selectedAddresses;

    struct PaidPlayers {
        address[] addresses;
    }

    PaidPlayers private paidPlayers;
    ...
    function updateDepositDistributorAddress(
        address _depositDistributorAddress
    ) external onlyRole(SETTINGS_ROLE) {
        depositDistributorAddress = payable(_depositDistributorAddress);
    }
    ...
    function declareWinner(
        ...
        selectedAddresses[winner] = true;
        paidPlayers.addresses.push(winner);
        emit WinnerDeclared(winner, winnerShares);
        declareRunnerUps(winner, _randomWords);
    }

    function isPaid(address _address) private view returns (bool) {
        for (uint256 i = 0; i < paidPlayers.addresses.length; i++) {
            if (paidPlayers.addresses[i] == _address) {
                return true;
            }
        }
        return false;
    ...
    function declareRunnerUps(
        ...
        uint256 successfulSelections = 0;
        uint256[] memory runnerIndices = new uint256[](players.length); // Adjusted size to players.length

        if (numRunnersUp <= 20) {
            for (uint256 j = 0; j < players.length; j++) {
                if (!selectedAddresses[players[j]] && !isPaid(players[j])) {
                    selectedAddresses[players[j]] = true;
                    runnerIndices[successfulSelections] = j;
            ...
            for (uint256 j = 0; j < iterations; j++) {
                uint256 randomIndex = _randomWords[j + 1] % numRunnersUp;
                uint256 attempts = 0;
                uint256 maxAttempts = numRunnersUp * 2;

                while (
                    selectedAddresses[players[randomIndex]] ||
                    isPaid(players[randomIndex])
                ) {
                    randomIndex = (randomIndex + 1) % numRunnersUp;
                    attempts++;
                ...
                if (
                    ...
                    selectedAddresses[players[randomIndex]] = true;
                    runnerIndices[successfulSelections] = randomIndex;
                    successfulSelections++;
                }
            }
        }

        uint256 runnersUpSharePerPlayer = successfulSelections > 0
            ? runnersUpShare / successfulSelections
            : 0;

        for (uint256 j = 0; j < successfulSelections; j++) {
            address payable runner = payable(players[runnerIndices[j]]);
            require(
                runner != address(0) && runner != address(this),
                "Invalid runner-up address"
            );
            runner.sendValue(runnersUpSharePerPlayer);
            paidPlayers.addresses.push(runner);
            emit RunnerUpDeclared(
                players[runnerIndices[j]],
                runnersUpSharePerPlayer
            );
        }
    ...
    function transferBalanceToDAO() private {
        ...
        for (uint256 i = 0; i < paidPlayers.addresses.length; i++) {
            selectedAddresses[paidPlayers.addresses[i]] = false;
        }

        delete paidPlayers.addresses;
    ...
    function enterRef(
        ...
        require(
            ...
            "Referral address cannot be the sender"
        );

        uint256 totalAmount = msg.value;
        uint256 commissionInWei = (totalAmount * commissionPercentage) /
            PERCENT_DIVISOR;
        uint256 referralShare = (commissionInWei * referralPercentage) /
            PERCENT_DIVISOR;

        players.push(payable(msg.sender));

        require(
            referralShare <= commissionInWei,
            "Referral share cannot exceed commission"
        );
