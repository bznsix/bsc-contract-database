/*
#####################################
由龙之国PiDKDAO技术社区开发 Pilong.top
The Year of the Dragon is back with a shock, creating a hundredfold myth
#####################################
*/
...
contract Pilong is ERC20, ERC20Burnable, Ownable2Step, DividendTrackerFunctions, Initializable {
    
    uint16 public swapThresholdRatio;
    
    uint256 private _pidkdaoPending;
    uint256 private _rewardsPending;

    address public pidkdaoAddress;
    uint16[3] public pidkdaoFees;
    ...
    event pidkdaoAddressUpdated(address pidkdaoAddress);
    event pidkdaoFeesUpdated(uint16 buyFee, uint16 sellFee, uint16 transferFee);
    event pidkdaoFeeSent(address recipient, uint256 amount);
    ...
    constructor()
        ERC20(unicode"Pilong", unicode"Pilong") 
    {
        address supplyRecipient = 0x38a8bB40E71c81b06BCe6D13Dd44FEc7D602F9a8;
        
        updateSwapThreshold(50);

        pidkdaoAddressSetup(0x38a8bB40E71c81b06BCe6D13Dd44FEc7D602F9a8);
        pidkdaoFeesSetup(0, 200, 0);

        autoBurnFeesSetup(100, 100, 100);

        _deployDividendTracker(7200, 10000 * (10 ** decimals()) / 10);
        ...
        _transferOwnership(0x38a8bB40E71c81b06BCe6D13Dd44FEc7D602F9a8);
    }
    ...
    */
    ...
    function getAllPending() public view returns (uint256) {
        return 0 + _pidkdaoPending + _rewardsPending;
    }

    function pidkdaoAddressSetup(address _newAddress) public onlyOwner {
        require(_newAddress != address(0), "TaxesDefaultRouterWallet: Wallet tax recipient cannot be a 0x0 address");

        pidkdaoAddress = _newAddress;
        excludeFromFees(_newAddress, true);

        emit pidkdaoAddressUpdated(_newAddress);
    }

    function pidkdaoFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        totalFees[0] = totalFees[0] - pidkdaoFees[0] + _buyFee;
        totalFees[1] = totalFees[1] - pidkdaoFees[1] + _sellFee;
        totalFees[2] = totalFees[2] - pidkdaoFees[2] + _transferFee;
        require(totalFees[0] <= 2500 && totalFees[1] <= 2500 && totalFees[2] <= 2500, "TaxesDefaultRouter: Cannot exceed max total fee of 25%");

        pidkdaoFees = [_buyFee, _sellFee, _transferFee];

        emit pidkdaoFeesUpdated(_buyFee, _sellFee, _transferFee);
    }
    ...
    function _transfer(
        ...
        if (!_swapping && amount > 0 && to != address(routerV2) && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            ...
            if (txType < 3) {
                ...
                _pidkdaoPending += fees * pidkdaoFees[txType] / totalFees[txType];
        ...
        if (!_swapping && !AMMPairs[from] && from != address(routerV2) && canSwap) {
            ...
            if (false || _pidkdaoPending > 0) {
                uint256 token2Swap = 0 + _pidkdaoPending;
                bool success = false;

                _swapTokensForCoin(token2Swap);
                uint256 coinsReceived = address(this).balance;
                
                uint256 pidkdaoPortion = coinsReceived * _pidkdaoPending / token2Swap;
                if (pidkdaoPortion > 0) {
                    success = payable(pidkdaoAddress).send(pidkdaoPortion);
                    if (success) {
                        emit pidkdaoFeeSent(pidkdaoAddress, pidkdaoPortion);
                    }
                }
                _pidkdaoPending = 0;
