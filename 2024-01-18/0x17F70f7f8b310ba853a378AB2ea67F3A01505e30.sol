/**
$WEBPAL Provides quick and painless web development in a cost effective manner.

Https://t.me/thewebpal
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional, Ownable {
  ...
  address public DividendsToken = address(0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE);
...
contract Webpal is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   WEBPALDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("Webpal", "WEBPAL") {
        _buyRewardsFee = 2;
        _buyLiquidityFee = 4;
		_buyMarketingFee = 4;
        _buyDevFee = 0;
    	...
    	dividendTracker = new WEBPALDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0xB60bf876a614e7337dDAb0b551ac78258C777E9f);
        _devWallet = payable(0x6640d7a7e03B994E91C7eEDf61565D16d70f250D);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "WEBPAL: The dividend tracker already has that address");

        WEBPALDividendTracker newDividendTracker = WEBPALDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "WEBPAL: The new dividend tracker must be owned by the WEBPAL token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "WEBPAL: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "WEBPAL: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "WEBPAL: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "WEBPAL: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "WEBPAL: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "WEBPAL: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "WEBPAL: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "WEBPAL: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "WEBPAL: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "WEBPAL: Cannot update gasForProcessing to same value");
	...
	function letsGoLive() external onlyOwner() {
        updateBuyFees(2,9,9,0);
        updateSellFees(2,8,20,0);
...
contract WEBPALDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("WEBPAL_Dividend_Tracker", "WEBPAL_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "WEBPAL_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WEBPAL contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "WEBPAL_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "WEBPAL_Dividend_Tracker: Cannot update claimWait to same value");
