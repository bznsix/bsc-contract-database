...
contract YOLO0001 is IBEP20, ReentrancyGuard {
    using SafeMath for uint256;
    address _owner;

    string constant _name = 'YOLO0001 Token';
    string constant _symbol = 'YOLO0001';
    uint8 immutable _decimals = 9;

    uint256 _totalsupply = 100000000000 * 10 ** 9;

    uint256 private _buyFeeRate = 0;
    uint256 private _sellFeeRate = 0;
    uint256 private _transFeeRate = 0;
    address private _pancakeAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address private immutable _buyFeeowner = 0x0000000000000000000000000000000000000000; // Marketing Address
    address private immutable _sellsFeeowner = 0x0000000000000000000000000000000000000000; // Marketing Address
    address private immutable _transFeeowner = 0x0000000000000000000000000000000000000000; // Marketing Address
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (!isExcluded(sender) && !isExcluded(recipient)) {
            ...
            if (isPancakeAddress(recipient)) {
                ...
                // fee
                if (_fee > 0) {
                    _balances[_sellsFeeowner] = _balances[_sellsFeeowner].add(_fee);
                ...
                // fee
                if (_fee > 0) {
                    _balances[_buyFeeowner] = _balances[_buyFeeowner].add(_fee);
                    toamount = toamount.sub(_fee, 'ERC20: Transfer amount exceeds allowance');
                }
            } else {
                // Tran
                ...
                // fee
                if (_fee > 0) {
                    _balances[_transFeeowner] = _balances[_transFeeowner].add(_fee);
                    toamount = toamount.sub(_fee, 'ERC20: Transfer amount exceeds allowance');
                }
            }
        }
