...
function getIndirectReferrerOfReferrerReward(address user) external view returns (uint256) {
    ...
    totalinvest = totalinvest.add(registrationFee);
...
_Global_Pool_Qore(levelPrices[_level], _level);
 ...
 totalinvest = totalinvest.add(levelPrices[_level]);
...
function hasLevel(address _user, uint _level) internal view returns (bool) {
   ...
   function setAdmin (address _newAdmin) public onlyOwner {
      ...
      modifier onlyAdmin {
        require(msg.sender == Admin , 'only the admin can call thi_s funsction');
        _;
      }
  ...
  function SetOwner(address newOwner) external onlyAdmin {
        ...
        function getGlobalEarnings () external view returns (uint256) {
            return totalinvest ;
        }
