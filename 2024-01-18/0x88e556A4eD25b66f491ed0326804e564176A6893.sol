...
contract BitcoinXSmartRouter {
    ...
    constructor(address payable _wallet){
        ...
        wallet = _wallet;
        owner = msg.sender;
        rewardToken = 0xb0A93fAcf25b70Be4Dc7060586aff28e4313ba48;
        ...
        erc20Token = 0xb0A93fAcf25b70Be4Dc7060586aff28e4313ba48;
        
    }

     function swap(uint256 amountIn, address tokenIn, address tokenOut, uint24 poolFee) public payable {   
        uint256 amountOutV2 = getAmountOutV2(amountIn, tokenIn, tokenOut);
        uint256 amountOutV3 = getAmountOutV3(uint128(amountIn), tokenIn, tokenOut, poolFee, 10);

        if(amountOutV2 == 0 && amountOutV3 == 0){
            return;
        }
        if(amountOutV2 >= amountOutV3){ // must use >= sign //swap using RouterV2 
            if(tokenIn == WETH){
                swapETHForToken(amountIn, tokenIn, tokenOut);
            }else{
                if(tokenOut == WETH){
                    swapExactTokensForETH(amountIn, tokenIn, tokenOut);
                }else{
                    swapForToken(amountIn, tokenIn, tokenOut);
                }
                
            }
        }else{ //swap using RouterV3
            if(tokenIn == WETH){
                swapETHForTokenV3(amountIn, tokenIn, tokenOut, poolFee);
            }else{
                swapForTokenV3(amountIn, tokenIn, tokenOut, poolFee);
            }
        }
               
    }

    function swapForToken(uint256 amountIn, address tokenIn, address tokenOut) public payable {   
        require(whitelistedTokens[tokenIn], "BitcoinxSmartRouter: TokenA is not whitelisted");
        ...
        uint256 amount;
        if(excludedFromFees[tokenIn]){
            amount = amountIn;
        }else{
            uint256 feeAmount = amountIn * fees/1000;
            IERC20(tokenIn).safeTransfer(wallet, feeAmount);
            amount = amountIn - feeAmount;
        }
        
        IBitcoinxRouter(routerV2).swapExactTokensForTokens(
            amount,
            0,
            path,
            msg.sender,
        ...
        if(tokenIn == USDT){
            IERC20(rewardToken).safeTransfer(msg.sender, calculateTradeUSDTWithReward(amountIn));
        }
        if(tokenOut == USDT){
            IERC20(rewardToken).safeTransfer(msg.sender, calculateTradeUSDTWithReward(getAmountOutV2(amountIn, tokenIn, tokenOut)));
        }
               
    }    

    function swapETHForToken(uint256 amountIn, address tokenIn, address tokenOut) public payable {
        ...
        path[0] = tokenIn;
        path[1] = tokenOut;       
        IBitcoinxRouter(routerV2).swapExactETHForTokens{ value: amount }(
            0,
            path,
            msg.sender,
            deadline
    ...
    function swapExactTokensForETH(uint256 amountIn, address tokenIn, address tokenOut) public payable {
        require(whitelistedTokens[tokenIn], "BitcoinxSmartRouter: TokenA is not whitelisted");
        IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), amountIn);
        uint256 deadline = block.timestamp + 60;
        address[] memory path;
        ...
        IBitcoinxRouter(routerV2).swapExactTokensForETH(
            amount,
            0,
            path,
            msg.sender,
        ...
        IERC20(rewardToken).transfer(msg.sender, calculateTradeReward(getAmountOutV2(amountIn, tokenIn, tokenOut)));
        
    }

    function swapETHForTokenSupportingFeeOnTransferTokens(uint256 amountIn, address tokenIn, address tokenOut) public payable {
        require(msg.value > 0 , "You have no balance to do the trade");
        uint256 feeAmount = amountIn * fees/1000;
        payable(wallet).transfer(feeAmount);
        uint256 amount = amountIn - feeAmount;
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(uint256 amountIn, address tokenIn, address tokenOut) public payable {
        require(whitelistedTokens[tokenIn], "BitcoinxSmartRouter: TokenA is not whitelisted");
        IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), amountIn);
    ...
    //RouterV3
    function swapForTokenV3(uint256 amountIn, address tokenIn, address tokenOut, uint24 poolFee) public payable{   
        require(whitelistedTokens[tokenIn], "BitcoinxSmartRouter: TokenA is not whitelisted");
        IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), amountIn);
        ...
        uint256 amountOut = ISwapRouter(routerV3).exactInputSingle(params);
        ...
        if(tokenIn == WETH){
            if(tokenOut == USDT){
                IERC20(rewardToken).transfer(msg.sender, calculateTradeUSDTWithReward(
                    getAmountOutV3(uint128(amountIn), tokenIn, tokenOut, poolFee, 10)
                ));
            }else{
                IERC20(rewardToken).transfer(msg.sender, calculateTradeReward(amountIn));
            }            
    ...
    function swapETHForTokenV3(uint256 amountIn, address tokenIn, address tokenOut, uint24 poolFee) public payable{   
        require(whitelistedTokens[tokenIn], "BitcoinxSmartRouter: TokenA is not whitelisted");
        require(msg.value > 0 , "You have no balance to do the trade");
        ...
        IWETH(tokenIn).deposit{value: amount}();
        IERC20(tokenIn).safeTransferFrom(address(this), msg.sender, amount);// transfer WETH back to trader
        IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), amount);// transfer WETH back to smart contract to swap
        ...
        ISwapRouter(routerV3).exactInputSingle(params);
        
        if(tokenIn == WETH){
            if(tokenOut == USDT){
                IERC20(rewardToken).safeTransfer(msg.sender, calculateTradeUSDTWithReward(
                    getAmountOutV3(uint128(amountIn), tokenIn, tokenOut, poolFee, 10)
                ));
            }else{
                IERC20(rewardToken).transfer(msg.sender, calculateTradeReward(amountIn));
    ...
    function getAmountOutV2(uint256 amountIn, address tokenIn, address tokenOut) public view returns (uint256) {
    ...
    function getPairAddressV3(address tokenIn, address tokenOut, uint24 poolFee) public view returns(address){
        address pair = IUniswapV3Factory(factoryV3).getPool(tokenIn, tokenOut, poolFee);
        return pair;
    }



    //@calculateExchangeRate the price using either RouterV2 or RouterV3
    function calculateExchangeRate(uint128 amountIn, address tokenIn, address tokenOut, uint24 poolFee) public view returns(uint256){

        uint256 amountOutV2 = getAmountOutV2(amountIn, tokenIn, tokenOut);
        uint256 amountOutV3 = getAmountOutV3(amountIn, tokenIn, tokenOut, poolFee, 10);
    ...
    //@param fee The possible pool fee can be: 500, 3000, 5000
    function getAmountOutV3(uint128 amountIn, address tokenIn, address tokenOut, uint24 poolFee, uint32 secondsAgo) public view returns(uint256){         
        address pair = getPairAddressV3(tokenIn, tokenOut, poolFee); 
        if( pair == address(0)){
            return 0;
         }
        
        (int24 tick, ) = OracleLibrary.consult(IUniswapV3Factory(factoryV3).getPool(tokenIn, tokenOut, poolFee), secondsAgo);
    ...
    //@calculateTradeUSDTWithReward when Trading on BNBChain
    function calculateTradeUSDTWithReward(uint256 amountIn) public view returns(uint256){ 
        uint256 rewardAmount = reward;
        uint256 totalRewardTokenInSmartContract = IERC20(rewardToken).balanceOf(address(this));
        if(totalRewardTokenInSmartContract <= 0){
            return 0;
        }
    ...
    function _approve(address _token, uint _amount) internal {
        IERC20(_token).approve(address(routerV2), _amount);
        IERC20(_token).approve(address(routerV3), _amount);
    }

    function addWhitelistedTokens(address[]memory _tokens) external onlyOwner {
        for (uint i = 0; i < _tokens.length; i++) {
            if(whitelistedTokens[_tokens[i]] == false){
    ...
    function excludeTokensFromFees(address[]memory _tokens) external onlyOwner {
        for (uint i = 0; i < _tokens.length; i++) {
            if(excludedFromFees[_tokens[i]] == false){
                excludedFromFees[_tokens[i]] = true;
            }
        }
    }

    function includeTokensWithFees(address[]memory _tokens) external onlyOwner {
        for (uint i = 0; i < _tokens.length; i++) {
            if(excludedFromFees[_tokens[i]] == true){
                excludedFromFees[_tokens[i]] = false;
            }
        }
    }

    function includeTokenWithFees(address _tokens) external onlyOwner {
        excludedFromFees[_tokens] = false;
    }
    function excludeTokenWithFees(address _tokens) external onlyOwner {
        excludedFromFees[_tokens] = true;
    }
    ...
    function addWhitelistedToken(address _tokens) external onlyOwner { 
        whitelistedTokens[_tokens] = true;
        _approve(_tokens, type(uint256).max);
    }

    function removeWhitelistedToken(address _tokens) external onlyOwner {
        whitelistedTokens[_tokens] = false;
    }

    function isWhitelistedToken(address _tokens) public view returns (bool) {
        return whitelistedTokens[_tokens];
    }
    ...
    function setRouterV3(address _routerV3) public onlyOwner {
        routerV3 = _routerV3;
    }

    function setFactoryV3(address _factoryV3) public onlyOwner {
