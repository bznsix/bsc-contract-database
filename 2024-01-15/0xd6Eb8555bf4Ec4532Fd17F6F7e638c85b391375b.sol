...
contract PuppyShiba is Context, IERC20, Ownable {
    ...
    event SetMarketingFees(uint256 amount);
    event BurnToken(address indexed from, uint256 value);
    event SetBurnFees(uint256 amount);
    event SetLiquidityFees(uint256 amount);
    ...
    address public marketingWalletAddress = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address payable public marketingFeesAddress = payable(0x691f13B70a3540F218300f11543E77a212054666);
    ...
    uint256 private _totalFeeToContract;
    ...
    constructor () {
        ...
        _totalFeeToContract = _liquidityFee.add(_marketingFee);
    ...
    function setMarketingFeesAddress(address payable newMarketingFeesAddress) external onlyOwner() {
        marketingFeesAddress = newMarketingFeesAddress;
    }
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        _totalFee = _liquidityFee.add(_marketingFee).add(_burnFee);
        _totalFeeToContract = _liquidityFee.add(_marketingFee);
        emit SetLiquidityFees(_liquidityFee);
    }
    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        ...
        _totalFee = _liquidityFee.add(_marketingFee).add(_burnFee);
        _totalFeeToContract = _liquidityFee.add(_marketingFee);
        emit SetMarketingFees(_marketingFee);
    }
    ...
    function setMaxBalance(uint256 newMaxBalance) external onlyOwner(){
        require(newMaxBalance >= _totalSupply.mul(10).div(1000));
    ...
    function _transfer(
        ...
        if(
            ...
            _totalFeeToContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        if(
        	...
        	uint256 feesToContract = amount.mul(_totalFeeToContract).div(100);
            ...
            transferToken(from, deadWalletAddress, toBurnAmount);
            emit BurnToken(from, toBurnAmount);
        }
        transferToken(from, to, amount);
    }
    function collectFees() private lockTheSwap {
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_liquidityFee).div(_totalFeeToContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_marketingFee).div(_totalFeeToContract);
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        IERC20(marketingWalletAddress).transfer(marketingFeesAddress, IERC20(marketingWalletAddress).balanceOf(address(this)));
    }
