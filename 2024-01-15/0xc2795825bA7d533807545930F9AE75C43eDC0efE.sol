// TeslaAugust,Earn TeslaSafe just by Holding  TG https://t.me/TeslaAugust
...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        ...
        // benefit is lost if 'b' is also TeslaAugusted.
...
// File: contracts/TeslaAugustDividendPayingToken.sol
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public immutable TeslaSafe = address(0x3504de9e61FDFf2Fc70f5cC8a6D1Ee493434C1Aa);
  ...
  function distributeTeslaSafeDividends(uint256 amount) public {
    require(totalSupply() > 0);
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      bool success = IERC20(TeslaSafe).transfer(user, _withdrawableDividend);
...
// File: contracts/TeslaAugust.sol
...
contract TeslaAugust is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public immutable TeslaSafe = address(0x3504de9e61FDFf2Fc70f5cC8a6D1Ee493434C1Aa);
    address private _marketingWallet = address(0x2A52eaBd243bfD367f11094677c01F9c41105Fbf);
    address private _devWallet = address(0x1078f7c4825Fa7D171c55A1eC89A2A8B528bBde4);
    ...
    TeslaAugustDividendTracker public dividendTracker;
    ...
    uint256 public maxSellTransactionAmount = 1000000000 * (10**18); // 1% of total supply
    ...
    constructor() public ERC20("TeslaAugust", "TeslaAugust") {
    	...
    	dividendTracker = new TeslaAugustDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "TeslaAugust: The dividend tracker already has that address");

        TeslaAugustDividendTracker newDividendTracker = TeslaAugustDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "TeslaAugust: The new dividend tracker must be owned by the TeslaAugust token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TeslaAugust: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TeslaAugust: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TeslaAugust: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "TeslaAugust: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TeslaAugust: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TeslaAugust: Cannot update gasForProcessing to same value");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TeslaAugustBSC: Account is already the value of 'excluded'");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "TeslaAugust: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapTokensForTeslaSafe(uint256 tokenAmount, address recipient) private {

        // generate the uniswap pair path of weth -> TeslaSafe
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of TeslaSafe
            path,
            recipient,
            block.timestamp
        );
    ...
    function swapAndSendDividends(uint256 tokens) private {
        swapTokensForTeslaSafe(tokens, address(this));
        uint256 dividends = IERC20(TeslaSafe).balanceOf(address(this));
        bool success = IERC20(TeslaSafe).transfer(address(dividendTracker), dividends);

        if (success) {
            dividendTracker.distributeTeslaSafeDividends(dividends);
...
contract TeslaAugustDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("TeslaAugust_Dividend_Tracker", "TeslaAugust_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "TeslaAugust_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "TeslaAugust_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TeslaAugust contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TeslaAugust_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TeslaAugust_Dividend_Tracker: Cannot update claimWait to same value");
