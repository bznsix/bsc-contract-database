...
contract BullToken is Context, IBEP20, Ownable {
    ...
    uint256 constant private _tTotal = 1_000_000_000_000_000 * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public swapTokensAtAmount = _tTotal / 5_000;

    address constant private DEAD = 0x000000000000000000000000000000000000dEaD;
    address public marketingWallet = address(0xec50E715762D7F294735Da6B6b286daE1F3919b9);

    string private constant _name = "Bull Token";
    ...
    uint256 private reflectionSellFee = 2;
    uint256 private liquiditySellFee = 2;

    uint256 private feeToCollectInTokensInBuy = 3;
    uint256 private feeToCollectInTokensInSell = 5;
    uint256 private feeToCollectReflectionInBuy = 0;
    uint256 private feeToCollectReflectionInSell = 2;

    uint256 private feeInTokens;
    uint256 private feeInReflections;
    ...
    constructor() {
        ...
        if (block.chainid == 56) {
            ...
            AX = AntiBot(0x8A04E760287a22dF6CBD721661A7ba6a5814DeCd);
    ...
    function _getTValues(
        ...
        s.tRfi = (tAmount * feeInReflections) / 100;
        s.tMarketing = (tAmount * feeInTokens) / 100;
        s.tTransferAmount =
            tAmount -
            s.tRfi -
            s.tMarketing;
    ...
    function _transfer(
        ...
        if(from == pair) {
            feeInTokens = feeToCollectInTokensInBuy;
            feeInReflections = feeToCollectReflectionInBuy;
        }

        if(to == pair) {
            feeInTokens = feeToCollectInTokensInSell;
            feeInReflections = feeToCollectReflectionInSell;
        }
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        uint256 swapForMarketing = contractBalance * 6 / 8;
        ...
        if(contractBalance > 0 && contractBalance >= balanceOf(address(this))) {
