...
contract TRockitDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("TRockit_Dividend_Tracker", "TRockit_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "TRockit_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "TRockit_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TRockit contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TRockit_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TRockit_Dividend_Tracker: Cannot update claimWait to same value");
...
contract TRockit is BEP20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    TRockitDividendTracker public dividendTracker;

    uint256 public _totalSupply = 100_000_000 * (10**18);
    uint256 public maxSellTransactionAmount = _totalSupply.div(100); //1% of total supply.
    uint256 public swapTokensAtAmount = _totalSupply.div(10_000);
    ...
    address payable public  marketingWallet = payable(0x80f82ea5033905bffDF7cf6b10A4b78cE9F02bAe);
    address payable public  developerWallet = payable(0x760C03Daf2e8B6D7ad48Fa24eb401fa1e85387Cf);
    address payable public  teamWallet 		= payable(0xeC3553985BeECC30dE97F511E1d6C4218b8C3Fa5);
    ...
    function updateAllFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _developerFee, uint256 _teamFee, uint256 _burnFee)
    public onlyOwner
    {
        ...
        teamFee = _teamFee;
        burnFee = _burnFee;
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(developerFee).add(teamFee); // total fee transfer and buy
    ...
    constructor() BEP20("TROCKIT SOCIAL NETWORK", "TROCKIT")
    {
        BNBRewardsFee = 5;
        liquidityFee = 2;
        marketingFee = 0;
        ...
        extraFeeOnSell = 0; // extra fee only on sell

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(developerFee).add(teamFee); // total fee transfer and buy

    	dividendTracker = new TRockitDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TRockit: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TRockit: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TRockit: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TRockit: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TRockit: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TRockit: Cannot update gasForProcessing to same value");
    ...
    function _transfer(address from, address to, uint256 amount) open(from, to) internal override
    {
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to])
        {
            ...
            if(burnTokens>0) { super._transfer(from, deadWallet, burnTokens); }
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        swapTokensForBnb(swapableTokens, address(this)); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        ...
        uint256 developerAmount = deltaBalance.mul(developerFee).div(totalFees);
        developerWallet.transfer(developerAmount);

        uint256 teamAmount = deltaBalance.mul(teamFee).div(totalFees);
        ...
        if(BNBRewardsFee>0)
        {
            ...
            uint256 dividenTokens = contractTokenBalance.mul(BNBRewardsFee).div(100);
            (bool success,) = address(dividendTracker).call{value: dividends}("");
            if(success)
            {
                emit SendDividends(dividenTokens, dividends);
            }
        }
