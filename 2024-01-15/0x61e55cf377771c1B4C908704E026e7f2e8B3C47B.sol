...
interface IXSwapFactory {
    ...
    function setFees(uint, uint) external;
...
library XSwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                IXSwapFactory(factory).INIT_CODE_PAIR_HASH()
            ))));
    }
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, uint tradingFee) internal pure returns (uint amountOut) {
        ...
        uint amountInWithFee = amountIn.mul(uint(10000).sub(tradingFee));
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, uint tradingFee) internal pure returns (uint amountIn) {
        require(amountOut > 0, 'XSwapLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'XSwapLibrary: INSUFFICIENT_LIQUIDITY');
        uint numerator = reserveIn.mul(amountOut).mul(10000);
        uint denominator = reserveOut.sub(amountOut).mul(uint(10000).sub(tradingFee));
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        amounts[0] = amountIn;
        uint tradingFee = IXSwapFactory(factory).tradingFee();
        for (uint i; i < path.length - 1; i++) {
            (uint reserveIn, uint reserveOut) = getReserves(factory, path[i], path[i + 1]);
            amounts[i + 1] = getAmountOut(amounts[i], reserveIn, reserveOut, tradingFee);
        }
    }
    ...
    function getAmountsIn(address factory, uint amountOut, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        for (uint i = path.length - 1; i > 0; i--) {
            ...
            amounts[i - 1] = getAmountIn(amounts[i], reserveIn, reserveOut, tradingFee);
        }
    }
}
pragma solidity ^0.6.6;
...
contract XSwapRouter is IXSwapRouter, Ownable {
    ...
    function _createLiquidityPair(address tokenA, address tokenB) internal onlyOwner {
    ...
    function _addLiquidity(
        ...
        if (IXSwapFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            _createLiquidityPair(tokenA, tokenB);
        }
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IPancakePair(XSwapLibrary.pairFor(factory, input, output)).swap(
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'XSwapRouter: INVALID_PATH');
        amounts = XSwapLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'XSwapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(XSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'XSwapRouter: INVALID_PATH');
        amounts = XSwapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'XSwapRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, XSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        ...
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = XSwapLibrary.sortTokens(input, output);
            IPancakePair pair = IPancakePair(XSwapLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            amountOutput = XSwapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, tradingFee);
            }
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(XSwapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    {
        return XSwapLibrary.getAmountOut(amountIn, reserveIn, reserveOut, IXSwapFactory(factory).tradingFee());
    }
    ...
    {
        return XSwapLibrary.getAmountIn(amountOut, reserveIn, reserveOut, IXSwapFactory(factory).tradingFee());
    }
