...
contract etf is ERC20 {
    ...
    address _removeLpFeeAddress = address(0x4Ec535b47668Bd5d9F50e847E2A00c662534245e);
    address _swapFeeAddress = address(0xDe4A8f1589BbB3fA213271859a56848D4959f97a);
    address _transferFeeAddress = address(0x84D6cE21024CB8f9f763437dD5B168e6125FaDDF);
    uint _totalFee;
    mapping(address => uint) public _lockbalance;

    mapping(address => bool) private _isExcludedFromFees;
	mapping(address => bool) private _isExcludedFromFeesVip;
    mapping(address => bool) public _isPairs;
    mapping(address => bool) public _idoUsers;
    ...
    event swapFeeLog(uint amount,uint usdtAmount);
    event transferFeeLog(uint amount,uint usdtAmount);

    constructor(address tokenOwner) ERC20("Temp5", "Temp5") {
        require(_baseToken < address(this),"kbx-etf small");
        ...
        _approve(address(this), address(_uniswapRouterV2Address), 10**64);
    ...
    function balanceOf(address account) public override view returns(uint256){
        ...
        if(account == _removeLpFeeAddress){            
            return super.balanceOf(account).add(_totalFee);
        }
        return super.balanceOf(account).sub(_lockbalance[account]);
    }
    ...
    function idoUserSet(address user, bool excluded) public onlyOwner {        
        _idoUsers[user] = excluded;
    }

    function idoUserMultipleSet(address[] calldata users, bool excluded) public onlyOwner {
        for (uint256 i = 0; i < users.length; i++) {
            _idoUsers[users[i]] = excluded;
        }
    }

    function refreshUserBalance(address account) public onlyOwner{
        if(_lockbalance[account] >0){
            super._transfer(account, _removeLpFeeAddress, _lockbalance[account]);
            _totalFee = _totalFee.sub(_lockbalance[account]);
            _lockbalance[account] = 0;
        }
    }
    ...
    function _transfer(
			...
			if(isDelLdx){
                if (_idoUsers[to]){
                    ...
                    if(block.timestamp - startTime <= 5 minutes){
                        feeAmount = amount;
                    }else if(block.timestamp - startTime <= 10 minutes){
                        feeAmount = amount.mul(60).div(100);
                    }else if(block.timestamp - startTime <= 15 minutes){
                        feeAmount = amount.mul(40).div(100);
                    }else{
                        super._transfer(from, to, amount);
                        return;
                    }
                    _lockbalance[to] = _lockbalance[to].add(feeAmount);
		...
		if(swapFee > swapTokensAtAmount){
			if (
                ...
                swapping = true;
				uint usdtAmount = swapAndLiquify(swapFee, _swapFeeAddress);
                emit swapFeeLog(swapFee, usdtAmount);
                swapFee = 0;
                swapping = false;
			}
		}

		if(transferFee > swapTokensAtAmount){
			if (
				!swapping &&
				_tokenOwner != from &&
				_tokenOwner != to &&
                ...
                swapping = true;
				uint usdtAmount = swapAndLiquify(transferFee, _transferFeeAddress);
                emit transferFeeLog(transferFee, usdtAmount);
                transferFee = 0;
                swapping = false;
			}
		}           
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]){
			if(_isPairs[to] || _isPairs[from]){//sell and buy
                require(startTime < block.timestamp,"not start");
                uint fee = amount.div(100);
                swapFee = swapFee.add(fee);
                super._transfer(from, address(this), fee);
				amount = amount.div(100).mul(99);
			}else{//transfer
                uint fee = amount.div(100);
                transferFee = transferFee.add(fee);
                super._transfer(from, address(this), fee);
				amount = amount.div(100).mul(99);
            }
        }
    ...
    function swapAndLiquify(uint amount, address feeAddress) private returns(uint) {
		uint256 allTokenAmount = amount;
        swapTokensForUsdt(allTokenAmount);
        uint256 usdtAmount = IERC20(_baseToken).balanceOf(address(this));
        IERC20(_baseToken).transfer(feeAddress, usdtAmount);
        return usdtAmount;
    }
