...
contract WEB30 is ERC20 {
    ...
    address _btcToken = address(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c);
    address _dogToken = address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43);
    ...
    address _fund1Address = address(0x8874F7618644308c8bCa736E48852a049cddaa4D);
    address _fund2Address = address(0xFF2F759d2c1F5E3E7bA05F06642d330A3B8DAAf8);
	...
	uint256 total;
    uint256 mintTotal;
    uint256 daySecond = 86400;

    constructor(address tokenOwner) ERC20("WEB3.0", "WEB3.0") {
        require(_baseToken < address(this),"WEB3.0 small");
        ...
        BTC = IERC20(_btcToken);
        DOGE = IERC20(_dogToken);
    ...
    function updatePrice() private {
        uint256 nowDay = block.timestamp.div(daySecond);
        if(startDownDay.add(1) >= nowDay || startDownDay == 0){
    ...
    uint256 public startDownDay;
    function getDownRate() internal view returns(uint256){
        if(downRate > 0){
            uint256 nowDay = block.timestamp.div(daySecond);
            if(nowDay < startDownDay.add(10)){
                return downRate;
            }
        }
        uint256 nowPrice = getPrice();
        if(nowPrice <= lastPrice.div(100).mul(90)){
            return 16;
        }
        return 0;
    } 
    ...
    mapping(address => address[]) public sunList;
    mapping(address => uint256) public _userAddDay;
    mapping(address => uint256) public _userDelDay;
    mapping(address => uint256) public _userRelease;
    mapping(address => uint256) public _userCanSell;
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair){
			...
			if(isDelLdx){
                ...
                super._transfer(from, to, amount);
                uint256 nowDay = block.timestamp.div(daySecond);
                _userDelDay[to] = nowDay;
                _userRelease[to] = _userRelease[to].add(amount);
                if(nowDay >= _userAddDay[to].add(56)){
                    _userCanSell[to] = _userCanSell[to].add(amount);
                }
                return ;
			}else if(bot){
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(130) > block.timestamp){amount = amount.div(10000000);}
                _splitBTCandDOGELdxToken();
                reward2node(to, amount);
				super._transfer(from, address(this), amount.div(10000).mul(10));
                super._transfer(from, address(this), amount.div(10000).mul(255));
                _takeInviterFee(from, to, amount.div(10000).mul(35));//0.35
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
                _splitBTCandDOGELdxToken();
                super._transfer(from, address(this), amount.div(10000).mul(10));
                ...
                _takeInviterFee(from, to, amount.div(10000).mul(35));//0.35
                if(downRate > 0){
                    startDownDay = block.timestamp.div(daySecond);
                    super._transfer(from, address(this), amount.div(100).mul(downRate));
                    swapAndLiquifySell(amount.div(100).mul(downRate));
                    amount = amount.div(100).mul(97 - downRate);
                }else{
                    amount = amount.div(100).mul(97);
                }
                uint256 canSell = getUserCanSell(from);
                require(canSell >= amount,"max Sell");
                _userOverSell[from] = _userOverSell[from].add(amount);
			}
        }
        super._transfer(from, to, amount);
        ...
        if(isInviter){
            inviter[to] = from;
            sunList[from].push(to);
        }
    }

    mapping(address => uint256) public _buyRelease;
    function reward2node(address buyUser, uint256 amount) private {
        address parent = inviter[buyUser];
        if(parent != address(0)){
            _buyRelease[parent] = _buyRelease[parent].add(amount.div(100).mul(3));
        }
    }

    mapping(address => uint256) public _userOverSell;
    function getUserCanSell(address user) public view returns(uint256){
        return getUserRelease(user) + _userCanSell[user] - _userOverSell[user];
    }

    function getUserRelease(address user)  public view returns(uint256){
        uint256 waitReleaseAmount = _userRelease[user] - _userCanSell[user];
        uint256 overDay = block.timestamp.div(daySecond) - _userDelDay[user];
        uint256 releaseAmount = waitReleaseAmount.div(100).mul(overDay);
        if(releaseAmount < waitReleaseAmount){
            return releaseAmount;
        }
        return waitReleaseAmount;
    }
        ...
        if(to == uniswapV2Pair){
			...
			if(isAddLdx){
				...
				if(lpTokenAmount >= amount){
                    ...
                    super._transfer(from, to, amount);
                    _userAddDay[from] = block.timestamp.div(daySecond);
    ...
    function swapTokensForBtc(uint256 tokenAmount) private {
        ...
        path[1] = _baseToken;
        path[2] = _btcToken;
        uint256 beforeAmount = BTC.balanceOf(address(this));
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
        ...
        uint256 afterAmount = BTC.balanceOf(address(this));
        if(afterAmount > beforeAmount){
            uint256 newAmount = afterAmount - beforeAmount;
            BTC.transfer(_fund1Address, newAmount.div(7).mul(2));
            BTC.transfer(_fund2Address, newAmount.div(7).mul(3));
        }
    }
    ...
    function swapTokensForDoge(uint256 tokenAmount) private {
        ...
        path[1] = _baseToken;
        path[2] = _dogToken;
        uint256 beforeAmount = DOGE.balanceOf(address(this));
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
        ...
        uint256 afterAmount = DOGE.balanceOf(address(this));
        if(afterAmount > beforeAmount){
            uint256 newAmount = afterAmount - beforeAmount;
            DOGE.transfer(_fund1Address, newAmount.div(90).mul(20));
            DOGE.transfer(_fund2Address, newAmount.div(90).mul(25));
        }
    }

    function swapAndLiquifySell(uint256 amount) private {
		uint256 allTokenAmount = super.balanceOf(address(this));
		if(allTokenAmount >= amount){
            swapTokensForBtc2Fund(amount.div(2));
            swapTokensForDoge2Fund(amount.div(2));
		}
    }

    function swapTokensForBtc2Fund(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
		address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = _baseToken;
        ...
        if(afterAmount > beforeAmount){
            uint256 newAmount = afterAmount - beforeAmount;
            BTC.transfer(_fund1Address, newAmount.div(2));
            BTC.transfer(_fund2Address, newAmount.div(2));
        }
    }

    function swapTokensForDoge2Fund(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
		address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = _baseToken;
        ...
        if(afterAmount > beforeAmount){
            uint256 newAmount = afterAmount - beforeAmount;
            DOGE.transfer(_fund1Address, newAmount.div(2));
            DOGE.transfer(_fund2Address, newAmount.div(2));
        }
    }
    ...
    function swapTokensForAi2Dead(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
		address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = _baseToken;
        path[2] = address(0x9181f6674A6a21a443F264295524805ff1578888);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
    ...
    function _splitBTCandDOGELdxToken() private {
        uint256 allBTCAmount = BTC.balanceOf(address(this));
        uint256 allDOGEAmount = DOGE.balanceOf(address(this));
		if(allBTCAmount >= 10**10 && allDOGEAmount > 10**10){
			_splitOtherUsdtLdx(allBTCAmount.div(5).mul(4), allDOGEAmount.div(5).mul(4));
		}
    }
    ...
    function _splitOtherUsdtLdx(uint256 btcAmount, uint256 dogeAmount) private {
		uint256 buySize = ldxUser.length;
		uint256 totalLpAmount = pair.totalSupply();
		if(buySize>0 && totalLpAmount > 0){
			address user;
			uint256 ldxRewardBtcAmount;
            uint256 ldxRewardDogeAmount;
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
					if(startLdxIndex >= buySize){startLdxIndex = 0;}
					user = ldxUser[startLdxIndex];
                    ldxRewardBtcAmount = pair.balanceOf(user).mul(btcAmount).div(totalLpAmount);
                    ldxRewardDogeAmount = pair.balanceOf(user).mul(dogeAmount).div(totalLpAmount);
                    if(ldxRewardBtcAmount > 10**15){
                        BTC.transfer(user, ldxRewardBtcAmount);
                    }
                    if(ldxRewardDogeAmount > 10**15){
                        DOGE.transfer(user, ldxRewardDogeAmount);
                    }
    ...
    address defaultAddress = address(0xFF2F759d2c1F5E3E7bA05F06642d330A3B8DAAf8);
    uint256[] shareFee = [20,10,5];
    function _takeInviterFee(
        ...
        for (uint256 i = 0; i < 3; i++) {
            cur = inviter[cur];
            if (cur != address(0)) {
                super._transfer(sender, cur, amount.div(10000).mul(shareFee[i]));
            }else{
				super._transfer(sender, defaultAddress, amount.div(10000).mul(shareFee[i]));
			}
        }
    }
}