...
contract AdvancedShadowDepositTokens is Ownable {
    ...
    // token => amount => on/off
    mapping(address => mapping(uint => bool)) public prices;

    address internal tokenKeeper;
    struct RewardInfo {
        address rewardToken;
    ...
    function editPrices(address _token, uint _amount, bool _on) external onlyOwner {
        prices[_token][_amount] = _on;
    }

    function tokenDeposit(address _token, uint _amount, uint _pid) external {
        require(prices[_token][_amount], "Price did not set");
    ...
    function setTokenKeeper(address _tokenKeeper) external  onlyOwner {
