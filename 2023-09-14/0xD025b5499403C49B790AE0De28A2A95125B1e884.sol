...
// ChimpToken with Governance.
contract ChimpToken is BEP20('chimp.money', 'CHIMP') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChimp).
...
import "./ChimpToken.sol";

// MasterChimp is the master of CHIMP. He can make CHIMP and he is a fair guy.
//
...
contract MasterChimp is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. CHIMP to distribute per block.
        uint256 lastRewardBlock;  // Last block number that CHIMP distribution occurs.
        uint256 accChimpPerShare;   // Accumulated CHIMP per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The CHIMP TOKEN!
    ChimpToken public chimp;
    // Dev address.
    ...
    // Deposit Fee address
    address public feeAddBb = 0xf59AAb71E52251585b6de12E55719Df126de38E5;
    address public feeAddSt = 0x2737CC5cA61f927d599b21f85e7D836EE24737Ec;
    ...
    event UpdateEmissionRate(address indexed user, uint256 chimpPerBlock);
    ...
    function pendingChimp(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accChimpPerShare = accChimpPerShare.add(chimpReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accChimpPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 chimpReward = multiplier.mul(chimpPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        chimp.mint(devaddr, chimpReward.mul(888).div(10000));
        chimp.mint(address(this), chimpReward);
        pool.accChimpPerShare = pool.accChimpPerShare.add(chimpReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChimp for CHIMP allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant  poolExists(_pid) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accChimpPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accChimpPerShare).div(1e12);
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Withdraw LP tokens from MasterChimp.
    ...
    function safeChimpTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > chimpBal) {
            transferSuccess = chimp.transfer(_to, chimpBal);
        } else {
            transferSuccess = chimp.transfer(_to, _amount);
        }
        require(transferSuccess, "safeChimpTransfer: transfer failed");
    }
