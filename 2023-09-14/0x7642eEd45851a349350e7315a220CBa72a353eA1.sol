...
contract Nmslpreward is Ownable, ReentrancyGuard, Pausable {
    using SafeERC20 for IERC20;
	
	address public NMSLP;
	address public suppContract;
    ...
    constructor () {
	   ...
	   rewardPoolBalance = 25 * 10**18;
	   rewardPercent = 200;
	   decayPercent = 100;
	   nextRewardTime = 1694822400;
	   NMSLP = address(0x2CBceE506304b579149749758BBF877B4b40E6d2);
	   suppContract = address(0x70CD37103A713fEEd527Cb91dA82D35eC3bFdBC3);
    }
	
	function clcDay() public view returns (uint256) {
       return (block.timestamp - (nextRewardTime - 1 days)) / 1 days;
    }
	...
	function _deposit(address user, uint256 amount) internal {
		...
		if (pending > 0) {
		   ISupp(suppContract).safeTokenTransfer(address(user), pending);
		}
		
		IERC20(NMSLP).safeTransferFrom(address(user), 0x000000000000000000000000000000000000dEaD, amount);
	...
	function withdrawReward() external nonReentrant whenNotPaused{
		...
		if(mapUserInfo[msg.sender].amount > 0) 
		{
			...
			if (pending > 0) {
			   ISupp(suppContract).safeTokenTransfer(address(msg.sender), pending);
			}
    ...
    function changeTokenRewardAmount(uint amount) external onlyOwner {
	   require(amount > 0, "Amount can't be zero");
