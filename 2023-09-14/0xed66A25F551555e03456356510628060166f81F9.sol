...
import "./INonFungibleApes.sol"; 

contract NFAStaking is ReentrancyGuard, Ownable, Initializable {
    ...
    // The nft contract
    INonFungibleApes public NFT_CONTRACT;
    // The nfa tier helper contract
    uint256 public TIER;
    // The reward token
	...
	// The maximum amount of staked NFTs.
    ...
    function  validNFt(uint256 _id) public view returns (bool) {
        return NFT_CONTRACT.isNfaOfRarityTier(_id, TIER);
    }
    ...
    function deposit(uint256[] calldata _ids) external nonReentrant {
        ...
        if (user.amount > 0) {
         ...
         for (uint256 i=0; i < _ids.length; i++) {
            require(validNFt(_ids[i]), "Invalid nft");
            NFT_CONTRACT.transferFrom(msg.sender, address(this), _ids[i]);
    ...
    function withdraw(uint256[] calldata _ids) external nonReentrant {
        ...
        for (uint256 i=0; i<_ids.length; i++) {
            ...
            NFT_CONTRACT.transferFrom(address(this), msg.sender, _ids[i]);
        }
    ...
    /// @param _maxStakedAmount The maximum NFTs to stake
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            NFT_CONTRACT.transferFrom(address(this), msg.sender, currentId);
