...
contract BlackBull is ERC20, Ownable {
    ...
    constructor(
        ...
        address _devWallet,
        address _burnWallet
    ) ERC20("BLACK BULL", "BULL") {
        uniswapV2Router = IUniswapV2Router02(_uniSwapRouterAddress);
        excludedFromMaxTransactionAmount(address(uniswapV2Router), true);
        ...
        excludedFromMaxTransactionAmount(address(uniswapV2Pair), true);
        excludedFromWalletAmount(address(uniswapV2Pair), true);
        ...
        buyMarketingFee = 3;
        buyDevFee = 3;
        buyFee = buyDevFee + buyMarketingFee + buyBurnFee;

        sellBurnFee = 8;
        sellMarketingFee = 6;
        sellDevFee = 6;
        sellFee = sellBurnFee + sellMarketingFee + sellDevFee;

        uint256 totalSupply = 1_000_000 ether;
        maxTransaction = 10000 ether;
        maxWallet = 10000 ether;
    ...
    function setBuyFees(string memory name, uint256 newAm) public onlyOwner {
        if (
            ...
            require(
                sellFee + newAm + buyDevFee + buyMarketingFee < 40,
                "Total fee should be less then %40"
            );
            buyBurnFee = newAm;
            buyFee = buyDevFee + buyMarketingFee + buyBurnFee;
        } else if (
            ...
            require(
                sellFee + newAm + buyBurnFee + buyMarketingFee < 40,
                "Total fee should be less then %40"
            );
            buyDevFee = newAm;
            buyFee = buyDevFee + buyMarketingFee + buyBurnFee;
            ...
            require(
                sellFee + newAm + buyBurnFee + buyDevFee < 40,
                "Total fee should be less then %40"
            );
            buyMarketingFee = newAm;
            buyFee = buyDevFee + buyMarketingFee + buyBurnFee;
        } else {
            revert("");
        }
    ...
    function setSellFees(string memory name, uint256 newAm) public onlyOwner {
        if (
            ...
            require(
                buyFee + newAm + sellDevFee + sellMarketingFee < 40,
                "Total fee should be less then %40"
            );
            sellBurnFee = newAm;
            sellFee = sellBurnFee + sellMarketingFee + sellDevFee;
            ...
            require(
                buyFee + newAm + sellBurnFee + sellMarketingFee < 40,
                "Total fee should be less then %40"
            );
            sellDevFee = newAm;
            sellFee = sellBurnFee + sellMarketingFee + sellDevFee;
            ...
            require(
                buyFee + newAm + sellBurnFee + sellDevFee < 40,
                "Total fee should be less then %40"
            );
            sellMarketingFee = newAm;
            sellFee = sellBurnFee + sellMarketingFee + sellDevFee;
        } else {
            revert();
        }
    ...
    function setFeeWallet(string memory name, address newAdd) public onlyOwner {
        if (
            ...
            burnWallet = newAdd;
            emit BurnWalletUpdated(oldWallet, newAdd);
        } else if (
            keccak256(abi.encodePacked(name)) ==
            keccak256(abi.encodePacked("dev"))
        ) {
            ...
            devWallet = newAdd;
            emit DevWalletUpdated(oldWallet, newAdd);
        } else if (
            keccak256(abi.encodePacked(name)) ==
            keccak256(abi.encodePacked("market"))
        ) {
            ...
            emit MarketingWalletUpdated(oldWallet, newAdd);
        } else {
            revert();
        }
    }
    ...
    function _swapBack(address from, address to) internal {
        ...
        uint256 tokensToSwap = burnFeeAmount +
            devFeeAmount +
            marketingFeeAmount;
        ...
        if (
            ...
            uint256 ethMarket = ethBalance.mul(marketingFeeAmount).div(
                tokensToSwap
            );
            uint256 ethDev = ethBalance.mul(devFeeAmount).div(tokensToSwap);
            ...
            (success, ) = address(marketingWallet).call{value: ethMarket}("");
            (success, ) = address(burnWallet).call{
    ...
    function _safeFee(
        ...
        if (
            ...
            burnFeeAmount += (amount * sellBurnFee) / 100;
            devFeeAmount += (amount * sellDevFee) / 100;
            marketingFeeAmount += (amount * sellMarketingFee) / 100;
        } else if (
            automatedMarketMakerPairs[from] &&
            buyFee > 0 &&
            !_isExcludedFromFees[to]
            ...
            burnFeeAmount += (amount * buyBurnFee) / 100;
            devFeeAmount += (amount * buyDevFee) / 100;
            marketingFeeAmount += (amount * buyMarketingFee) / 100;
        }
    ...
    function syncTotalFees() public onlyOwner {
        buyFee = buyDevFee + buyMarketingFee + buyBurnFee;
        sellFee = sellDevFee + sellMarketingFee + sellBurnFee;
    }
