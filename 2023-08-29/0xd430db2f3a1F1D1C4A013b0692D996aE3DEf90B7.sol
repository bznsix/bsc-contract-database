...
contract DGWTest is Ownable {
  ...
  address private Funds_Wallet = 0xC6b227007B5208724C7dBE5c3B0b15F0D60d9718;
  address private Default_Refer_Wallet = 	0xb8bb0b55ae0Bdc4c146C24E24F3b0BA7f444d99A;
  IERC20 private USDT = IERC20(0xc9585f4e69a2826D075B886D190Fa255Fc42E9B6);
  IERC20 private SaleToken = IERC20(0xcAb5575D9F3D978191F6bCC86E18DF03d515aAdB);
...
uint256[] referLevels = [13,8,4,3,2];
  ...
  constructor() {
    referralAddress[Default_Refer_Wallet] = msg.sender;
  ...
  function buyTokens(uint256 amount, address _referralAddress) external {
    ...
    if (tokenSold >= phaseLimit[currentPhase]) {
      currentPhase += 1;
    }
    if(tokenSold >= phaseLimit[2]){
      _vestingStatus=true;
      initialVestingDate = block.timestamp+ 35 days; //Change time here to change first vesting date
    }
  }
...
if(uplineAddress == address(0)){
  ...
  function claimTokensS1() public {
    ...
    require(
      sale.claimedDate <= block.timestamp,
      "Tokens are only claimable once in 30 days"
    );
  ...
  function vestingStatus() public view returns (bool){
   ...
   function getLevel1Reffers(address userAddresss)
    ...
    // Calculate the token amount required for the given USD amount at the current price
        tokenAmountRequired = (_usdAmount * 1e18) /getCurrentPrice();
        ...
        uint256 remUSD = (remainingTokens /1e18)* getCurrentPrice()  ;
        ...
        uint256 tokensNeededFromNextPhase = (remUSD * 1e18)/ phasePrice[currentPhase + 1];
