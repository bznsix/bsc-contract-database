...
contract ZLE is ERC20 {
	...
	address _Inviter1Address;
	address _Inviter2Address;
	address _reciver50Address;
    ...
    constructor(address tokenOwner,address _fund1,address _fund2,address _fund3,address _fund4,address _fund5,address _fund6,address Inviter1Address,address Inviter2Address,address reciver50Address) ERC20("ZLE", "ZLE") {
        require(_baseToken < address(this),"ZLE small");
		...
		fundUsers.push(_fund1);
		fundUsers.push(_fund2);
		fundUsers.push(_fund3);
		fundUsers.push(_fund4);
		fundUsers.push(_fund5);
		fundUsers.push(_fund6);
		
		_Inviter1Address = Inviter1Address;
		_Inviter2Address = Inviter2Address;
		_reciver50Address = reciver50Address;
    }
    ...
    function _transfer(
        ...
        bool isInviter = from != uniswapV2Pair && balanceOf(to) == 0 && perparInviter[to] == address(0) && amount >= 10**12;
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(11) > block.timestamp){amount = amount.div(100000000);}
                _splitUsdtLdxToken();
                _takeInviterFee(from, to, amount.div(50));//2
        ...
        if(isInviter){
			...
			bool isEndInviter = from != uniswapV2Pair && perparInviter[from] == to && inviter[from] == address(0) && amount >= 10**12;
			if(isEndInviter){
				inviter[from] = to;
			}
        }
    ...
    function _transferFrom(
        ...
        if(to == uniswapV2Pair){
			...
			if(isAddLdx){
				...
				if(super.balanceOf(_destroyAddress) >= super.totalSupply().sub(6888 *10**18)){
				}else{
					super._transferr(uniswapV2Pair, _reciver50Address, amount.div(2));
		...
		if(allTokenAmount >= 10**17){
            ...
            if(afterAmount > beforeAmount){
				...
				for(uint256 i=0;i<6;i++){
					USDT.transfer(fundUsers[i], newAmount.div(8));
				}
            }
		}
    }
    ...
    function _takeInviterFee(
        ...
        for (uint256 i = 0; i < 1; i++) {
            ...
            if (cur != address(0)) {
				...
				super._transferr(sender, _Inviter1Address, amount.div(5));
				super._transferr(sender, _Inviter2Address, amount.div(5).mul(4));
			}
        }
    }
}