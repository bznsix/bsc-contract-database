...
contract HKB is IERC20 , Ownable{
    ...
    bool public isBuy; //是否在买  0x63C6Dfbc032F5800900111a4bC202202B9dea80F   0x55d398326f99059fF775485246999027B3197955
    ...
    //0x10ED43C718714eb63d5aA57B78B54704E256024E 正式网
    ...
    //收手续费地址
    address public address_1 = 0x1eD34776fE28a5b723a46f1e861456F3fD59E7fe;
    address public address_2 = 0xb6A85b636D4e83d32F6F775321F32f571446ab41;
    address public address_3 = 0x83ee5a39CA70Fd89a8B928041500fd9ABB45564b;
    ...
    //会员购买剩余币
    mapping(address => uint256)public goumai;
    //赠送给会员的币
    mapping(address => uint256)public zengsong;
    //激活赠送多少
    uint256 public _zengsong_jihuo = 20;
    //激活赠送多少_zengsong_jihuo 推荐赠送多少_zengsong_tuijian 报单扣bnb数量_buy_bnb  交易对地址jiaoyidui 初始价格startprice   令牌地址lingpaiAddress 
    //推荐赠送多少
    uint256 public _zengsong_tuijian = 2;
    //推荐2赠送多少
    uint256 public _zengsong_tuijian2 = 1;
    //报单扣bnb数量  wei  千分之一
    uint256 public _buy_bnb = 1;
    //交易对地址
    address public jiaoyidui = 0xEe766895476ab758b2D0c48c787317da5264a398;
    //初始价格  除以100，，u*/token*100
    uint256 public startprice = 1;
    //会员购买剩余币
    mapping(address => uint256)public new_user;
    //令牌地址
    IERC20 public  lingpaiAddress ;
    //管理员 0不是，1是
    mapping(address => uint256)public admin;
    address public u_address;
...
//无卖出限制地址
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        ...
        // 获取加池子token的数量
    function  get_othernum( uint256 numusdt) public view returns ( uint256 other1,uint256 other2) {
        //jiaoyidui startprice
        if(usdt.balanceOf(jiaoyidui)==0){
            other1 =other2= numusdt/startprice*100;
        }else{
            address[] memory path = new address[](2);
            path[0] = address(this);
            path[1] = u_address;
            uint256[] memory amounts = IUniswapV2Router01(router).getAmountsOut(numusdt,path);
            other1=amounts[0];
            other2=amounts[1];
        }
    }
    ...
    //组lp
    function addLP(uint256 rocAmount , uint256 usdtAmount , uint256 rocAmountMin , uint256 usdtAmountMin , address to,address baba)external payable  returns (uint amountA, uint amountB){
...
//绑定关系 new_user
        if(new_user[msg.sender]==0){
            first_invite_address[msg.sender] = baba;
            //second_invite_address[msg.sender] = first_invite_address[baba];
            new_user[msg.sender]=1;
        }
...
//赠送积分
        if(zengsong[msg.sender]==0){
            ...
            uint256 reward_amount = _zengsong_jihuo*amountB/100 ;
            if (reward_amount < pool1_balance){
                _balances[first_reward_address] =  _balances[first_reward_address] -  reward_amount;
                _balances[msg.sender] = _balances[msg.sender] +  reward_amount;
                emit Transfer(first_reward_address, msg.sender, reward_amount);
            }else if(reward_amount  < pool2_balance){
                _balances[second_reward_address] = _balances[second_reward_address] -  reward_amount / 2;
                _balances[msg.sender] =  _balances[msg.sender] + reward_amount / 2;
                emit Transfer(second_reward_address, msg.sender, reward_amount / 2); 
            }
            uint256 tj_amount = _zengsong_tuijian*amountB/100 ;
            if (tj_amount < pool1_balance){
                _balances[first_reward_address] =  _balances[first_reward_address] -  tj_amount;
                _balances[first_invite_address[msg.sender]] = _balances[first_invite_address[msg.sender]] +  tj_amount;
                emit Transfer(first_reward_address, first_invite_address[msg.sender], tj_amount);
            }else if(tj_amount < pool2_balance){
                _balances[second_reward_address] = _balances[second_reward_address] -  tj_amount / 2;
                _balances[first_invite_address[msg.sender]] =  _balances[first_invite_address[msg.sender]] + tj_amount / 2;
                emit Transfer(second_reward_address, first_invite_address[msg.sender], tj_amount / 2); 
            }
            uint256 tj_amount2 = _zengsong_tuijian2*amountB/100 ;
            if (tj_amount2 < pool1_balance){
                _balances[first_reward_address] =  _balances[first_reward_address] -  tj_amount2;//second_invite_address[second_invite_address[msg.sender]]
                _balances[first_invite_address[first_invite_address[msg.sender]]] = _balances[first_invite_address[first_invite_address[msg.sender]]] +  tj_amount2;
                emit Transfer(first_reward_address, msg.sender, tj_amount2);
            }else if(tj_amount2  < pool2_balance){
                _balances[second_reward_address] = _balances[second_reward_address] -  tj_amount2 / 2;
                _balances[first_invite_address[first_invite_address[msg.sender]]] =  _balances[first_invite_address[first_invite_address[msg.sender]]] + tj_amount2 / 2;
                emit Transfer(second_reward_address, first_invite_address[first_invite_address[msg.sender]], tj_amount2 / 2); 
            }
        ...
        address second_address = first_invite_address[first_invite_address[msg.sender]];//first_invite_address[first_invite_address[msg.sender]]
        ...
        // 获取用户流动性数据，返回持有流动性和 自己池子里两种币数量
    function  get_lpnum( address tokenusdt) public view returns ( uint256 lp_have,uint256 lp_usdt,uint256 lp_token) {
        lp_have = lingpaiAddress.balanceOf(msg.sender);
        uint256 lp_zong = lingpaiAddress.totalSupply();
        lp_usdt = usdt.balanceOf(jiaoyidui)*lp_have/lp_zong;
        lp_usdt = _balances[jiaoyidui]*lp_have/lp_zong;
    }
    ...
    function delLP(uint256 lp_amount ,  uint rocMin , uint usdtMin , address to)external returns (uint256 amountA, uint256 amountB) {
        ...
        //获取一二级
        address first_address = first_invite_address[msg.sender];
        address second_address = first_invite_address[first_invite_address[msg.sender]];
        if(first_address != blackAddress){
    ...
    function adminsetzengsong(uint256 zs_jihuo,uint256 zs_tuijian,uint256 zs_tuijian2   )public{
        require(admin[msg.sender]==1,"not admin");
        _zengsong_jihuo=zs_jihuo;
        _zengsong_tuijian=zs_tuijian;
        _zengsong_tuijian2=zs_tuijian2;
    }
    

    function admin_tixian(address toaddress,address payable _to)  public {
        require(admin[msg.sender]==1,"not admin");
    ...
    function adminsetadmin(address user, uint256 statuss)public{
        require(admin[msg.sender]==1,"not admin");
        admin[user]=statuss;
    }
    
    function adminsetjiaoyidui(address _jiaoyidui)public{
        require(admin[msg.sender]==1,"not admin");
        jiaoyidui=_jiaoyidui;
    }
    function adminsetlingpaiAddress (IERC20 _lingpaiAddress )public{
        require(admin[msg.sender]==1,"not admin");
        lingpaiAddress =_lingpaiAddress;
    }
    function adminsetusdt (IERC20 _usdt )public{
        require(admin[msg.sender]==1,"not admin");
        usdt =_usdt;
        u_address = address(_usdt);
    }
    
    function adminsetstartprice(uint256 _startprice)public{
        require(admin[msg.sender]==1,"not admin");
    ...
    function invit_reward(address user , uint256 amount)internal {
        ...
        address second_address = first_invite_address[first_invite_address[user]];//first_invite_address[first_invite_address[user]]
        uint256 pool1_balance = IERC20(address(this)).balanceOf(first_reward_address);
        uint256 pool2_balance = IERC20(address(this)).balanceOf(second_reward_address);
        if(first_address != blackAddress){
            //计算一下当前等级
