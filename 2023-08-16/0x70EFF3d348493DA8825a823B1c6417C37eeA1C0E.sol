...
contract FunFundSale_v2 is Ownable {
    IERC20 public USDT;
    IERC20 public USDC;
    IERC20 public BUSD;
    IERC20 public FUND;
    ...
    function buy(uint _round, uint _amount, address _referrer, uint _currency) public {
        ...
        if(_round == 1) {
            ...
            if(rounds[_round].bought[msg.sender] == false) {
        ...
        rounds[_round].bought[msg.sender] = true;
    ...
    function buyAdm(address _sender, uint _round, uint _amount, address _referrer) public onlyOwner {
        ...
        if(_myReferrer != 0x0000000000000000000000000000000000000000 && _lvl != 0) {
            ...
            rounds[_round].purchased[_myReferrer] += calculate(levels[_lvl].bonusReferrer, _amount);
            //Adds FUND to refereal
            rounds[_round].purchased[_sender] += calculate(levels[_lvl].bonusReferral, _amount);
        }
    }
