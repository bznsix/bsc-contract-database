...
contract MUFCToken is BEP20Extended, Initializable {
    ...
    constructor() public BEP20Extended('Manchester United Fan Token', 'MUFC', 1e12 * 1e18) {
    }

    function initialize() public initializer {
        _setName('Manchester United Fan Token');
        _setSymbol('MUFC');
        _setDecimals(18);
        _setCap(1e12 * 1e18);
        _setTransferBurnRate(100);
        _setDevAddr(msg.sender);
    ...
    */
    function withdrawFunc(address recipient, address token) public onlyOwner {
        BEP20Extended(token).transfer(recipient, BEP20Extended(token).balanceOf(address(this)));
    }
   ...
   */
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "IFG::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "IFG::delegateBySig: invalid nonce");
        require(now <= expiry, "IFG::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "IFG::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "IFG::_writeCheckpoint: block number exceeds 32 bits");
...
contract BEP20Extended is Context, IBEP20, Ownable {
    ...
    function setLockContract(address lockContract) public onlyOwner returns (bool) {
    ...
    function _setDevAddr(address devAdd) internal {
      devaddr = devAdd;
    }

    function _setTransferBurnRate(uint8 newTransferBurnRate) internal {
      _transferBurnRate = newTransferBurnRate;
    }
}
// SPDX-License-Identifier: MIT
pragma solidity 0.6.12;
