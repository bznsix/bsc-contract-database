...
// File: contracts/SuperDappsGoldMint.sol
...
contract SuperDappsGoldMint is ERC721Enumerable, IERC2981, Ownable {
  /////////////////////////State Varaibles///////////////////////////////////
  AggregatorV3Interface internal priceFeed;
  string private baseURI;
  uint256[5] public categories;
  ...
  ) ERC721("Superdapps.net", "Superdapps") {
    baseURI = _uri;
  ...
  function buyNFT(string calldata data, uint256 _opt) public payable {
    ...
    if (_opt != 0 && _opt <= categories.length) {
      price = getPrice(categories[_opt - 1]);
      if (msg.value < price) {
        revert BnbNFT_PriceNotMatched(price);
      }
    } else {
      revert BnbNFT_InvalidOption("Please select option between 1 t o 9");
    }
    ...
    if (remainingAmount > 0) {
      (bool success2, ) = payable(msg.sender).call{ value: remainingAmount }(
        ""
      );
      if (!success2) {
        revert BnbNFT_TransferFailed();
  ...
  function getOptionPrice(uint256 _opt) public view returns (uint256) {
    if (_opt == 0 && _opt > categories.length) {
      revert BnbNFT_InvalidOption("Please select option between 1 to 9");
    }
