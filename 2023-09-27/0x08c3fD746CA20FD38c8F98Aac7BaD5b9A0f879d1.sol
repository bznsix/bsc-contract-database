...
abstract contract TwoPhaseOwnable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "ERROR 6!");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "ERROR 5!");
        _pendingOwner = newOwner;
    }

    function acceptOwnership() public virtual {
        require(_msgSender() == pendingOwner(), "ERROR 4!");
...
contract MOON is ERC20, ERC20Burnable, Pausable, TwoPhaseOwnable {

    uint256 private maxSupply = 10000000000 * (10 ** uint256(decimals()));

    mapping (address => bool) blacklisted;

    constructor() ERC20("MOON", "MOON", maxSupply ) {
    }
    ...
    function batchBlackList(address[] memory _evils, bool[] memory _blacks) external onlyOwner {
        if (_blacks.length == 1) {
            ...
            require(_evils.length == _blacks.length, "ERROR 1");
            for (uint256 i = 0; i < _evils.length; i++) {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount)
        ...
        override
    {
        require(!blacklisted[from], "ERROR 2");
        require(!blacklisted[to], "ERROR 3");
