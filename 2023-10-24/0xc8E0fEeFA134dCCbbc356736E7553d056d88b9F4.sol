...
contract MEGAETH is ERC20, Ownable {
    ...
    address _buybackWalletAddress = 0x7D68AFdcEd7CF7213942BaA785aA0f9096b08732;
    ...
    constructor() ERC20("MEGAETH", "METH") {
        ...
        totalFees = _TokenRewardsFee.add(_liquidityFee).add(_marketingFee).add(_buybackFee);
    ...
    function updateRewardsFee(uint256 RewardFeePercent) external onlyOwner{
        require(RewardFeePercent <= 15, "MEGAETH: updateRewardsFee must be between 0 and 30.");
    ...
    function updateMarketingFee(uint256 marketingFeePercent) external onlyOwner{
        require(marketingFeePercent <= 7, "MEGAETH: updateRewardsFee must be between 0 and 10.");
        marketingFee = marketingFeePercent;
        totalFees = TokenRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee);
 
    }
     function updatebuybackFee(uint256 buybackFeePercent) external onlyOwner{
        require(buybackFeePercent <= 3, "MEGAETH: updateRewardsFee must be between 0 and 10.");
        buybackFee = buybackFeePercent;
        totalFees = TokenRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee);
...
contract MEGAETHDividendTracker is Ownable, DividendPayingToken{
    ...
    uint256 public minimunForRewards = 5000;
    ...
    constructor() DividendPayingToken("MEGAETH_Dividend_Tracker", "MEGAETH_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = minimunForRewards * (10**18); //must hold 5000+ tokens
    }
    ...
    function updateMinimunForRewards(uint256 newValueMinimunForRewards) external onlyOwner {
        require(newValueMinimunForRewards >= 5000 && newValueMinimunForRewards <= 5000, "MEGAETH_Dividend_Tracker: newValueMinimunForRewards must be updated to between 10000 and 500000");
