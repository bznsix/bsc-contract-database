...
contract Branaverse_Public_Sale_Bsc  is Ownable, NonblockingLzApp, ReentrancyGuard {
    ...
    bool public _saleEnd;
    uint256 public maxAmount;
    uint256 public minAmount;
    uint8 public mainDecimals;
    uint256 public paymentDecimals;

    event requestsend(address indexed buyer, uint256 CurrencyAmount);
    ...
    modifier FullfillRequirement(uint256 _amount) {
        ...
        require(_saleEnd, "sale not active!");
        _;
    }
    ...
    function setPaymentDecimals(uint256 _newDecimals) external onlyOwner {
        paymentDecimals = _newDecimals;
    }

    function minBuyMax(uint256 minAmt, uint256 maxAmt) external onlyOwner {
        uint256 min = minAmt * 10**paymentDecimals;
        uint256 max = maxAmt * 10**paymentDecimals;
        minAmount = min;
        maxAmount = max;
    }
    ...
    {
        uint256 amount = (_amount / 10**paymentDecimals) * 10**mainDecimals;
        transferCurrency(USDTaddress, msg.sender, treasuryWallet, _amount);
        //send the transation
