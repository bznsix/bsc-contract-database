...
contract SpookyRabbit is ERC20, Ownable {
    ...
    address public mrktWallet = 0x065D3030b221aEB677b25e70385967DBDBaCbBd9;
    ...
    constructor() ERC20("SPOOKY RABBIT", "SPOOKY") {
    ...
    function _transfer(
       ...
       if (!exemptFee[sender] && !exemptFee[recipient]) {
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipient])
            ...
            feeswap = sellTaxes.liquidity + sellTaxes.mrkt;
            feesum = feeswap;
            currentTaxes = sellTaxes;
        } else if (!useLaunchFee) {
            feeswap = taxes.liquidity + taxes.mrkt;
            feesum = feeswap;
            currentTaxes = taxes;
        } else if (useLaunchFee) {
            feeswap = launchtax;
    ...
    function Liquify(uint256 feeswap, Taxes memory swapTaxes) private lockTheSwap {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 mrktAmt = unitBalance * 2 * swapTaxes.mrkt;
            if (mrktAmt > 0) {
                payable(mrktWallet).sendValue(mrktAmt);
            }
        }
    }
