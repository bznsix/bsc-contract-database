...
abstract contract AntKingSlippage {
    function buySlippage(uint256 amountToWei, address [] calldata superiors) external virtual returns (address [] memory slippageAddress, uint256 [] memory slippageAmount);
...
contract AntKingToken is IERC20, Ownable {
    ...
    uint private slippageBuyRatio;
    uint private slippageSellRatio;
    ...
    function _transfer(
        ...
        if(sender == swapRouter && !_isExcludedFromFee[recipient]) {
            ...
            uint256 slidingPoint = amount.mul(slippageBuyRatio).div(100);
            ...
            (address [] memory slippageAddresses, uint256 [] memory slippageAmounts) = AntKingSlippage(slippageAddress).buySlippage(amount, superiors);
            ...
            uint256 slidingPoint = amount.mul(slippageSellRatio).div(100);
            uint256 surplus = amount.sub(slidingPoint);
        
		    _balances[sender] = _balances[sender].sub(amount);
            _balances[recipient] = _balances[recipient].add(surplus);

            (address [] memory slippageAddresses, uint256 [] memory slippageAmounts) = AntKingSlippage(slippageAddress).sellSlippage(amount);
            for(uint8 i=0; i<slippageAddresses.length; i++) {
                if(slippageAddresses[i] != address(0)) {
                    _balances[slippageAddresses[i]] = _balances[slippageAddresses[i]].add(slippageAmounts[i]);
                    emit Transfer(sender, slippageAddresses[i], slippageAmounts[i]);
    ...
    function setSlippageBuyRatio(uint ratio) public onlyOwner {
        slippageBuyRatio = ratio;
    }

    function setSlippageSellRatio(uint ratio) public onlyOwner {
        slippageSellRatio = ratio;
    }
