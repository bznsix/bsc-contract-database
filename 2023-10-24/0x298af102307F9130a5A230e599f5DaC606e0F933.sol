...
contract BaseNFT is INFTCore, ERC721, Ownable {
     ...
     * @dev Mint NFT Batch
     */
    function mintNFTBatch(uint256 quantity, uint256 _rare, string memory _name, string memory _description, address userAddress)
        external
        onlySafeNFT
    {
        require(quantity <= 100, "box too much");
        _mintNFTBatch(quantity, _rare, _name, _description, userAddress);
    }

    /// @notice Mint nft with info
    function _mintNFTBatch(uint256 quantity, uint256 _rare, string memory _name, string memory _description, address userAddress) internal {
        uint256 tokenId;
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    */
    ...
    function isApprovedForAll(address owner, address operator) public view virtual override returns (bool) {
        if ((manager.safeNFT(operator) && manager.safeNFT(_msgSender())) || (manager.safeNFT(owner) && manager.safeNFT(_msgSender()))) return true;
