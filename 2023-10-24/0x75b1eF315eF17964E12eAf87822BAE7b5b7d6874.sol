...
contract ERC20PledgeToken is Ownable{
    using SafeMath for uint256;

    address public receiver;

    event WithDrawalToken(address indexed token, address indexed receiver, uint indexed amount);
    event AddLiquidity(uint256 tokensSwapped, uint256 ethReceived, uint256 tokensIntoLiqudity);
    event Pledge(uint indexed _type, address indexed _address, uint indexed amount, uint cycle, uint state);

    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair = address(0x21bF04BC64d5F0F17a45Fe6AC75a7327F3e28bc5);
    ...
    address public token = address(0x5ECCD741761E2C7642DbBFC86Dd6D524d9e840BE);
    ...
    mapping(address=>PledgeData[]) private pledgeData;
    ...
    function setTokenAddress(address _address) public onlyOwner returns(bool){
        token = _address;

        return true;
    }

    function setUniswapV2Pair(address _address) public onlyOwner returns(bool){
    ...
    function addLiquidity(uint256 usdtAmount, uint256 tokenAmount) private {
        _approveToken(token, tokenAmount);
        _approveToken(_baseToken, usdtAmount);
        // add the liquidity
    ...
    function getPledgeData(address _address) public view returns (PledgeData[] memory) {
        return pledgeData[_address];
    }
}