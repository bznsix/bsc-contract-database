...
contract Maintenance is Ownable, ERC721Holder {
    ...
    modifier validId(uint256 _nftId) {
        require(FitZenNFT(nft).ownerOf(_nftId) != address(0), "INVALID NFT ID");
        _;
    }
    ...
    function getFeeMaintainNft(uint256 _nftId) public view validId(_nftId) returns (uint256) {
        ...
        uint256 nftValueInUsd = FitZenNFT(nft).getNftPriceUsd(_nftId);
        uint256 nftValueInToken = Oracle(oracleContract).convertUsdBalanceDecimalToTokenDecimal(nftValueInUsd);
        uint256 feeToMaintain = (nftValueInToken * (10 ** FitZenERC20(token).decimals()) * maintenanceFee) / 100;
    ...
    function getTimeRepair(uint256 _nftId) public view returns (uint256) {
        ...
        if (lastTimeRepair == 0) {
            lastTimeRepair = FitZenNFT(nft).getLockTime(_nftId); //buy time
        }
        if (lastTimeRepair == 0) {
            return 0;
        }
    ...
    function repairNft(uint256 _nftId) external validId(_nftId) {
        ...
        if (feeNft != 0 && token != address(0)) {
            require(
                FitZenERC20(token).balanceOf(msg.sender) >= feeNft,
                "MAINTENANCE: NOT ENOUGH BALANCE CURRENCY TO REPAIR NFTs"
            );
            require(
                FitZenERC20(token).allowance(msg.sender, address(this)) >= feeNft,
                "MAINTENANCE: MUST APPROVE FIRST"
            );
            require(
                FitZenERC20(token).transferFrom(msg.sender, saleWallet, feeNft),
...
contract Staking is IStaking, Ownable, ERC721Holder {
    ...
    // mapping to store reward APR per staking period
    mapping(uint32 => uint16) public nftTierAprs;
    ...
    // mapping to require user buy min value nft to get reward
    mapping(uint8 => uint32) public requireQuantityUserConditions;
    ...
    constructor(
        ...
        initRequireQuantityUserCondition();
    }
    ...
    modifier isOwnerNft(uint256 _nftId) {
        require(FitZenNFT(nft).ownerOf(_nftId) == msg.sender, "STAKING: NOT OWNER THIS NFT ID");
        _;
    }
    ...
    function initCommissionPercents() internal {
        ...
        commissionPercents[2] = 700;
        commissionPercents[3] = 500;
        commissionPercents[4] = 300;
        commissionPercents[5] = 200;
        commissionPercents[6] = 200;
        commissionPercents[7] = 200;
        commissionPercents[8] = 200;
        commissionPercents[9] = 200;
        commissionPercents[10] = 100;
        commissionPercents[11] = 100;
        commissionPercents[12] = 50;
        commissionPercents[13] = 30;
        commissionPercents[14] = 20;
    }
    ...
    function initCommissionConditionUsd() internal {
        ...
        amountConditions[3] = 500;
        amountConditions[4] = 500;
        amountConditions[5] = 1000;
        amountConditions[6] = 1000;
        amountConditions[7] = 1000;
        amountConditions[8] = 1000;
        amountConditions[9] = 1000;
        amountConditions[10] = 5000;
        amountConditions[11] = 5000;
        amountConditions[12] = 5000;
        amountConditions[13] = 5000;
        amountConditions[14] = 5000;
    }

    /**
     * @dev init require F1 buy 150$ NFT for commission profit
     */
    function initRequireQuantityUserCondition() internal {
        requireQuantityUserConditions[0] = 0;
        requireQuantityUserConditions[1] = 0;
        requireQuantityUserConditions[2] = 2;
        requireQuantityUserConditions[3] = 3;
        requireQuantityUserConditions[4] = 4;
        requireQuantityUserConditions[5] = 5;
        requireQuantityUserConditions[6] = 6;
        requireQuantityUserConditions[7] = 7;
        requireQuantityUserConditions[8] = 8;
        requireQuantityUserConditions[9] = 9;
        requireQuantityUserConditions[10] = 10;
        requireQuantityUserConditions[11] = 10;
        requireQuantityUserConditions[12] = 10;
        requireQuantityUserConditions[13] = 10;
        requireQuantityUserConditions[14] = 10;
    }

    /**
     * @dev init stake apr for each NFT ID
     */
    function initStakeApr() internal {
        nftTierAprs[10] = 3800;
        nftTierAprs[9] = 4300;
        nftTierAprs[8] = 4800;
        nftTierAprs[7] = 5300;
    ...
    /**
     * @dev function to get stake apr for NFT ID
     * @param _nftTier NFT ID
     */
    function getStakeAprForTier(uint32 _nftTier) external view override returns (uint16) {
        return nftTierAprs[_nftTier];
    }

    /**
     * @dev function to set stake apr for NFT ID
     * @param _nftTier NFT ID
     * @param _apr apr value want to set
     */
    function setStakeAprForTier(uint32 _nftTier, uint16 _apr) external override onlyOwner {
        nftTierAprs[_nftTier] = _apr;
    }
    ...
    /**
     * @dev function to get require user condition
     * @param _level commission level
     */
    function getRequireQuantityUserCondition(uint8 _level) public view override returns (uint32) {
        return requireQuantityUserConditions[_level];
    }

    /**
     * @dev function to set commission percent
     * @param _level commission level
     * @param _quantity user require
     */
    function setRequireQuantityUserCondition(uint8 _level, uint32 _quantity) external override onlyOwner {
        requireQuantityUserConditions[_level] = _quantity;
    }
    ...
    function stakeExecute(uint256 _nftId, bytes memory _data) internal {
        // Get total balance in usd staked for user
        require(FitZenNFT(nft).isApprovedForAll(msg.sender, address(this)), "STAKING: MUST APPROVE FIRST");
        ...
        uint16 _apr = nftTierAprs[FitZenNFT(nft).getNftTier(_nftId)];
        uint256 totalAmountStakeUsdDecimal = estimateValueUsdForStakeNft(_nftId) * tokenDecimal;
        uint256 rewardUsdDecimal = calculateRewardInUsd(totalAmountStakeUsdDecimal, _apr);
        // Update struct data
        ...
        require(FitZenNFT(nft).ownerOf(_nftId) == msg.sender, "STAKING: NOT OWNER THIS NFT ID");
        try FitZenNFT(nft).safeTransferFrom(msg.sender, address(this), _nftId, _data) {
            // Emit event
            emit Staked(_nftId, msg.sender, _nftId, unlockTimeEstimate, _apr);
    ...
    function claim(uint256 _nftId) public override lock {
        ...
        if (claimableRewardInUsdWithDecimal > 0) {
            ...
            require(
                IERC20(fitzenToken).balanceOf(address(this)) >= claimableRewardInTokenWithDecimal,
                "STAKING: NOT ENOUGH TOKEN BALANCE TO PAY REWARD"
            );
            require(
                IERC20(fitzenToken).transfer(msg.sender, claimableRewardInTokenWithDecimal),
            ...
            if (earnableCommissionRewardUsdDecimal > 0) {
                ...
                if (isEnablePayRankingCommission) {
                    IRanking(rankingContract).payRankingCommission(currentRef, earnableCommissionRewardUsdDecimal);
                }
            }
    ...
    function payCommissionMultiLevels(
        ...
        uint8 index = 0;
        while (currentRef != address(0) && index < 15) {
            ...
            if (totalStakeAmount) {
                ...
                if (_commissionCanEarnUsdWithDecimal > 0) {
                    ...
                    require(
                        IERC20(fitzenToken).balanceOf(address(this)) >= totalCommissionInTokenDecimal,
                        "STAKING: NOT ENOUGH TOKEN BALANCE TO PAY COMMISSION"
                    );
                    require(
                        IERC20(fitzenToken).transfer(currentRef, totalCommissionInTokenDecimal),
                        "STAKING: UNABLE TO TRANSFER COMMISSION PAYMENT TO RECIPIENT"
                    );
    ...
    function earnableForStakeInUsdWithDecimal(uint256 _nftId) public view override returns (uint256) {
        ...
        uint256 claimableStartTime = FitZenNFT(nft).getLockTime(_nftId);
        uint256 claimedTimeByRun = claimableStartTime > 0 ? (stakeInfo.startTime - claimableStartTime) : 0;
        uint256 timeLimit = stakingLimit * 30 * 24 * 3600;
        if (claimedTimeByRun >= timeLimit) {
            // 6 * 30 * 24 * 3600
            return 0;
        }
        uint256 totalDurationCanEarn = timeLimit - claimedTimeByRun;
    ...
    function possibleForCommission(address _staker, uint8 _level) public view returns (bool) {
        ...
        address[] memory f1s = IMarketplace(marketplaceContract).getF1ListForAccount(_staker);
        uint8 totalF1BuyMinPriceNFT = 0;
        for (uint index = 0; index < f1s.length; index++) {
            bool isBuyMinNFT = IMarketplace(marketplaceContract).checkIsBuyMinValuePackage(f1s[index]);
            if (isBuyMinNFT) {
                totalF1BuyMinPriceNFT += 1;
            }
            if (totalF1BuyMinPriceNFT >= requireQuantityUserConditions[_level]) {
                return true;
            }
        }
    ...
    function estimateValueUsdForStakeNft(uint256 _nftId) public view returns (uint256) {
        return FitZenNFT(nft).getNftPriceUsd(_nftId);
    }
     ...
     * @param _apr apr
     */
    function calculateRewardInUsd(uint256 _totalValueStakeUsd, uint16 _apr) public view override returns (uint256) {
        // Get years
        uint256 yearAmount = stakingPeriod / 12;
        // Calculate to reward in token
        uint256 rewardInUsd = (_totalValueStakeUsd * _apr * yearAmount) / 10000;
        return rewardInUsd;
    }
    ...
    function claimableForStakeInUsdWithDecimal(uint256 _nftId) public view override returns (uint256) {
        ...
        uint256 timeBroken = Maintenance(maintenanceContract).getTotalTimeStakeBroken(_nftId, stakeInfo.startTime);
        uint256 totalDurationStaked = stakeInfo.unlockTime - stakeInfo.startTime;
        if (accumulateTimeStaked > timeBroken) {
            accumulateTimeStaked = accumulateTimeStaked - timeBroken;
        } else {
            accumulateTimeStaked = 0;
        }
        ...
        uint256 accumulateRewardInUsdWithDecimal = (stakeInfo.totalRewardAmountUsdWithDecimal * accumulateTimeStaked) /
            totalDurationStaked;
        if (accumulateRewardInUsdWithDecimal <= stakeInfo.totalClaimedAmountUsdWithDecimal) {
            return 0;
        }
    ...
    function unstake(uint256 _nftId, bytes memory _data) external override lock {
        ...
        try FitZenNFT(nft).safeTransferFrom(address(this), staker, _nftId, _data) {
            // Emit event
        ...
        require(
            IERC20(fitzenToken).balanceOf(address(this)) >= tokenAmountWithDecimal,
            "STAKING: NOT ENOUGH TOKEN BALANCE TO PAY UNSTAKE REWARD"
        );
        require(
            IERC20(fitzenToken).transfer(staker, tokenAmountWithDecimal),
            "STAKING: UNABLE TO TRANSFER COMMISSION PAYMENT TO RECIPIENT"
        );
        // get total commission can earned of ref 6 months
        uint256 claimableRewardInUsdWithDecimal = stakedNFTs[_nftId].totalRewardAmountUsdWithDecimal -
    ...
    function transferNftEmergency(address _receiver, uint256 _nftId) public override onlyOwner {
        require(FitZenNFT(nft).ownerOf(_nftId) == address(this), "STAKING: NOT OWNER OF THIS NFT");
    ...
    function setMarketCommission(address _currentRef, uint256 _commissionUsdWithDecimal) external override {
        require(
            msg.sender == rankingContract || msg.sender == earnContract,
    ...
    function getCommissionProfitUnclaim(address _user) external view override returns (uint256) {
        ...
        totalClaimableUsdWithDecimal = getClaimableProfitF1(_user, 0, maxFloor, totalClaimableUsdWithDecimal);
        uint256 _commissionCanEarnUsdWithDecimal = getUserCommissionCanEarnUsdWithDecimal(
            _user,
    ...
    function getMaxFloorProfit(address _user) public view override returns (uint8) {
        ...
        for (uint8 i = 0; i < 14; i++) {
            if (totalF1BuyMinPriceNFT >= requireQuantityUserConditions[i] || totalBuyAmount >= amountConditions[i]) {
                maxFloor = i;
            }
        }
    ...
    function getClaimableProfitF1(
        ...
        for (uint index = 0; index < f1s.length; index++) {
            ...
            for (uint i = 0; i < nfts.length; i++) {
                ...
                uint256 commissionNftCanEarnUsd = (earnableCommissionRewardUsdDecimal * _commissionPercent) / 10000;
                totalClaimableUsdWithDecimal = totalClaimableUsdWithDecimal + commissionNftCanEarnUsd;
            }
            if (floor <= maxFloor) {
                totalClaimableUsdWithDecimal = getClaimableProfitF1(
                    f1s[index],
                    floor,
                    maxFloor,
                    totalClaimableUsdWithDecimal
    ...
    function setUserStakingNfts(address _stakerAddress, uint256[] memory _nftIds) external onlyOwner {
        userStakingNfts[_stakerAddress] = _nftIds;
    }
