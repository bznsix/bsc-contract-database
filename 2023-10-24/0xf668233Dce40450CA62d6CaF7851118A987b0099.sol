...
contract TokenDividendTracker is Ownable {
    ...
    function setShares(address[] calldata shareholder) external onlyOwner {
        for(uint8 i = 0; i < shareholder.length; i++){
            addShareholder(shareholder[i]);
        }
    }  
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _cast(address account, uint256 amount) internal virtual {
        ...
        require(_totalSupply.add(amount) <= 121000000000*10**18, "ERC20: Exceeded maximum limit");
...
contract FOMO3D is ERC20, Ownable {
    using SafeMath for uint256;
    using SafeMath for uint112;
    ...
    uint256 public beginBNB = 0.01 ether;
    uint256 public repurchase = 0.1 ether;
    ...
    constructor(
        ...
        dividendTracker = new TokenDividendTracker(address(this),lpRewardReceiveAddress_);
        pair = _uniswapV2Pair;
    ...
    function setRepurchase(uint256 val) external onlyOwner {
        repurchase = val;
    }
    ...
    function setBuyFee(uint256 val) public onlyOwner {
        buyFee = val;
    }

    function setSellFee(uint256 val) public onlyOwner {
        sellFee = val;
    }
    ...
    function setMultiplexerShares(address[] calldata shares) public onlyOwner{
        dividendTracker.setShares(shares);
    }
    ...
    function takeAllFee(address from,address to,uint256 amount) private returns(uint256 amountAfter) {
        ...
        if(uniswapV2Pair[to]){
            ...
            if(bnbBlance > repurchase){
                swapBNBForTokens(bnbBlance.div(4),marketWalletAddress);
            }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        payable(address(marketWalletAddress)).transfer(newBalance);
    }
    ...
    receive () payable external{
        if (msg.value >= bnbFee) {
            if(!isContract(msg.sender)){    
                ...
                if(bnbToken > beginBNB){
                    try dividendTracker.setShare(msg.sender) {} catch {}
                    uint256  bnbAmount = msg.value;
                    uint256 addLiquidityBNB = bnbAmount.mul(20).div(100).div(2);
                    uint256 shareBNB = bnbAmount.mul(50).div(100);
                    ...
                    payable(address(dividendTracker)).transfer(shareBNB);
                    liquidityBackHelper.addLiquidity(tokenSub, addLiquidityBNB);
                    //mint 
                    uint256 mitTokenAmount = addLiquidityBNB.mul(2).mul(token).div(bnbToken);
