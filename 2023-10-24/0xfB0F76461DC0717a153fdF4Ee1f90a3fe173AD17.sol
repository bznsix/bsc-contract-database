...
contract MPSFarm is Ownable, ReentrancyGuard, IAutoFarm {
    using SafeMath for uint256;
    ...
    uint256 public AFIMaxSupply = 25000000e18;
    ...
    function updatePool(uint256 _pid) public {
        ...
        MintableToken(AFIRAddress).farmMint(
            owner(),
            AFIReward.mul(ownerAFIReward).div(1000)
        );
        MintableToken(AFIRAddress).farmMint(address(this), AFIReward);
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        uint256 pending = user.shares.mul(pool.accAFIPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeAFITransfer(msg.sender, pending);
