...
contract FlokiGangster is Context, IERC20, Ownable {
    ...
    string private _name = "Floki Gangster";
    string private _symbol = "FGN";
    uint8 private _decimals = 18;
    
    uint256 public _liquidityFee = 3;
    uint256 private _previousLiquidityFee = _liquidityFee;
    uint256 public _holdersFee = 3;
    uint256 private _previousTaxFee = _holdersFee;
    uint256 public _burnFee = 1; // Burn Fee
    uint256 private _previousBurnFee = _burnFee;
    uint256 public _devFee = 3; 
    address payable public devWallet = 0x89F846424A920db515b261255fE01A5Cba4629DF; 
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "zero"));
        return true;
    }
    ...
    function deliver(uint256 tAmount) public {
        ...
        require(!_isExcluded[sender], "cannot call");
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        require(tAmount <= _tTotal, "less than supply");
    ...
    function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
        require(rAmount <= _rTotal, "less reflections");
    ...
    function setfeeRatios(uint256 liquidityFee, uint256 devFee, uint256 burnFee, uint256 holdersFee) external onlyOwner {
        require (liquidityFee + devFee + burnFee + holdersFee >= 0, "Must add up to 100% or 99%.");
        _liquidityFee = liquidityFee;
        _devFee = devFee;
        _burnFee = burnFee;
        _holdersFee = holdersFee;
        _tFeeTotal = liquidityFee + devFee + burnFee + holdersFee;
    }
    ...
    function setDevWallet(address payable devAddress) external onlyOwner() { 
    ...
    function swapAndLiquify(uint256 TokenBalance) private lockTheSwap {
        ...
        uint256 halfOfOtherHalf = TokenBalance.sub(half).sub(otherHalf);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private { 
        ...
        else if(recipient == uniswapV2Pair){ require(amount <= _maxTxAmount, "exceeds the maxTxAmount."); } 
        else{ 
        } 
