...
contract HPC_Staking {
    ...
    constructor(address _stakingContractAddress, address  _rewardTokenAddress, address _manAddress, uint256 _claimInterval) {
    ...
    function stake(uint256 _amount) external {
        ...
        // re-implement rewardToken transfer fee mechanism.
        uint256 deductions = (_amount * 18) / 1000; // fixed 1.8% transfer fee.

        // Determine the net amount to stake (deduct the fee)
        uint256 netAmount = _amount - deductions;
    ...
    function unstake(uint256 amountToUnstake) external {
        ...
        //require(rewards == 0, "You still have unclaimed reward");
        require(amountToUnstake > 0 && amountToUnstake <= stakes, "Invalid unstake amount");

        // Update stakedAmount.
        stakedAmount[msg.sender] -= amountToUnstake;
    ...
    function claimRewards() external {
        ...
        totalRewardsReceived[msg.sender] += reward; 
    ...
    function compoundRewards() external {
        ...
        stakedAmount[msg.sender] += _reward;
        ...
        // Update total rewards received by the staker
        totalRewardsReceived[msg.sender] += _reward; 
    ...
    function calculateRewardByAddress(address _staker) external view returns (uint256) {
        ...
        if (userStake == 0) {
            ...
            uint256 rewardPerTokenStakedLocal = (rewardAmount[_staker] * 1e8) / userStake;
            return userStake * rewardPerTokenStakedLocal / 1e8;
        }
    }
    ...
    function calculateMyReward() external view returns (uint256) {
        ...
        if (userStake == 0) {
            ...
            // Calculate the potential rewards based on staked amount and current reward per staked token
            uint256 rewardPerTokenStakedLocal = (rewardAmount[msg.sender] * 1e8) / userStake;
            return userStake * rewardPerTokenStakedLocal / 1e8;
        }
    }
    ...
    function rewardCalculator(uint256 _stakedAmount) external view returns (uint256) {
        ...
        if (totalS == 0 || totalR == 0) {
            ...
            uint256 rewardPerTokenStakedLocal = (totalR * 1e8) / totalS;
            return (_stakedAmount * rewardPerTokenStakedLocal) / 1e8;
        }
    }
    ...
    function autoDistributeRewards() internal {
        ...
        uint256 totalBalance = calculateAllBalances();
        uint256 availableBalance;

        if (newBalance <= totalBalance) {
            emit DistributeRewardsError("No new reward yet");
            return; // do nothing.
        }

        availableBalance = newBalance - totalBalance;

        if (availableBalance > 0 && getStakedAmount() > 0){
        // Calculate management and holders shares
        uint256 managementShare = (availableBalance * Man_Percentage) / 100;
        uint256 holdersShare = (availableBalance * Stakers_Percentage) / 100;

        // Calculate the reward per staked token locally
        uint256 rewardPerTokenStakedLocal = (holdersShare * 1e8) / getStakedAmount();
        ...
        for (uint256 i = 0; i < stakerAddresses.length; i++) {
            ...
            if (userStake > 0) {
                uint256 stakerReward = (userStake * rewardPerTokenStakedLocal) / 1e8;
                rewardAmount[stakerAddress] += stakerReward;
        ...
        managementVaultBalance += managementShare;
    ...
    function manualDistributeRewards() external {
        ...
        if (newBalance <= totalBalance) {
            emit DistributeRewardsError("Contract has no new reward");
            return; // do nothing.
        }
    ...
    function calculateAllBalances() internal view returns (uint256) {
        uint256 totalBalances = getStakedAmount() + getRewardAmount() + manVaultBalance();
    ...
    function updateCurrentAPY() internal {
        ...
        for (uint256 i = 0; i < stakerAddresses.length; i++) {
            ...
            // Access the staked and reward amounts from the mappings
            uint256 userStake = stakedAmount[stakerAddress];
            uint256 userReward = rewardAmount[stakerAddress];
            
            rewards += userReward;
            stakes += userStake;
        }

        // Calculate the current annual interest rate (APY)
        uint256 currentRate;

        // Check if there are stakers and total staked tokens are not zero
        if (stakes > 0 && rewards > 0) {
            currentRate = (rewards * 10000) / stakes; // result with 2 decimal places.

            // Set the APY directly as the annual interest rate
            currentAPY = currentRate;
        } else {
