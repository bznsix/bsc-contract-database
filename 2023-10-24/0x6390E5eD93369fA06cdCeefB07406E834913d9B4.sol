...
contract Marketplace is IERC721Receiver,Ownable {
    ...
    function getListedNft(address _address) view public returns (ListDetail [] memory)  {
        
        uint256 balance = nft.balanceOf(address(this));

        if (_address == address(this)) {
            ListDetail[] memory _listDetail = new ListDetail[](balance);
            for (uint256 i = 0; i < balance; i++) {
                _listDetail[i] = listDetail[nft.tokenOfOwnerByIndex(address(this), i)];
            }
            return _listDetail;
        }
        ...
        {
            if (listDetail[nft.tokenOfOwnerByIndex(address(this), i)].author == _address)
            {
                count ++;
            }
        }

        ListDetail[] memory result = new ListDetail[](count);
        uint256 length = 0;
       
        for( uint i = 0; i < balance; i++)
        {
            ListDetail memory _listDetail = listDetail[nft.tokenOfOwnerByIndex(address(this), i)];
            if (_listDetail.author == _address)
            {
                result[length++] = _listDetail;
            }
        }
    ...
    function buyNft(uint256 _tokenId) public payable {
        ...
        require(nft.ownerOf(_tokenId) == address(this), "This NFT doesn't exist on marketplace");
        require(listDetail[_tokenId].price <= msg.value, "Minimum price has not been reached");
        
        payable(listDetail[_tokenId].author).transfer(msg.value * (100 - tax) / 100);
