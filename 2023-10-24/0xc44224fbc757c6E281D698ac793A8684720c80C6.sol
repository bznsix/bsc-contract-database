...
// File: contracts/supra/interfaces/egg/IShareRewardPool.sol
...
// File: contracts/supra/strategies/egg/StrategyeggLP.sol
 ...
 * egg rewards from providing liquidity are farmed every few minutes, sold and split 50/50.
 ...
 */
contract StrategyeggLP is Ownable, Pausable {
    ...
    address[] public eggToWbnbRoute = [egg, wbnb];
    address[] public eggToLp0Route;
    address[] public eggToLp1Route;
    ...
    constructor(
        ...
        if (lpToken0 == wbnb) {
            eggToLp0Route = [egg, busd, wbnb];
        } else if (lpToken0 != egg) {
            eggToLp0Route = [egg, busd, lpToken0];
        }

        if (lpToken1 == wbnb) {
            eggToLp1Route = [egg, busd, wbnb];
        } else if (lpToken1 != egg) {
            eggToLp1Route = [egg, busd, lpToken1];
        }

        IERC20(lpPair).safeApprove(shareRewardPool, uint256(-1));
        IERC20(egg).safeApprove(unirouter, uint256(-1));
     ...
     * It deposits {lpPair} in the shareRewardPool to farm {egg}
     */
     ...
     * 3. It swaps the {egg} token for {lpToken0} & {lpToken1}
    ...
    function harvest() external whenNotPaused {
        if (degen == true)
            require(
                flp.balanceOf(msg.sender) > 0,
                "You need falopa in your life"
            );
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(egg).balanceOf(address(this)).mul(40).div(1000);
    ...
    /**
     * @dev Swaps {egg} for {lpToken0}, {lpToken1} & {wbnb} using PancakeSwap.
     */
    function addLiquidity() internal {
        uint256 eggHalf = IERC20(egg).balanceOf(address(this)).div(2);
    ...
    function pause() public onlyOwner {
        ...
        IERC20(egg).safeApprove(unirouter, 0);
