...
contract DigitizedFiat  is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DigitizedFiatDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Digitized Fiat", "Digitized") {
        marketingWallet = payable(0xea43a1aCf4A623022465A6853D210a39976efBE2);
        DappWallet = payable(0x1BfAA08af6AB1Ae8dc5aEC7Ea91A9053F94dcd12);
        ...
        dividendTracker = new DigitizedFiatDividendTracker(
            payable(this),
            router,
            0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c,
            "DigitizedFiatTRACKER",
            "DigitizedFiatTRACKER"
        );
        ...
        maxWallet = totalTokenSupply / 100; //
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (!isBuying && !isSelling) {
                ...
                if (limitsInEffect) {
                require(block.number > launchblock + 6,"you shall not pass");
...
contract DigitizedFiatDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    DigitizedFiat public DigitizedFiatContract;
    ...
    constructor(
        ...
        defaultToken = token;
        DigitizedFiatContract = DigitizedFiat(mainContract);
    ...
    function _transfer(
        ...
        require(false, "DigitizedFiat_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DigitizedFiat_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DigitizedFiat contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(DigitizedFiatContract);

            uint256 prevBalance = DigitizedFiatContract.balanceOf(address(this));
            ...
            {
                uint256 received = DigitizedFiatContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    DigitizedFiatContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
