...
interface IERC20 {
  ...
  event Approval(address indexed owner, address indexed spender, uint256 value);
    ...
    uint256 private constant maxAddFreeze = 20 minutes;
    ...
    uint256 private constant directPercents = 3000;
    ...
    constructor(address _usdtaddr)   {
        USDT = IERC20(_usdtaddr);
           ...
           feeReceivers[0] = address(0x88b2bEba0A9Cff19A11f13ba677de622D19F1112);   //// Creater Fund 2
           feeReceivers[1] = address(0x88b2bEba0A9Cff19A11f13ba677de622D19F1112);   //// Developer Fund  1.5
           feeReceivers[2] = address(0x88b2bEba0A9Cff19A11f13ba677de622D19F1112);   //// Reserve fund 1.5
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 500e18){
            ...
            if(total >= 10000e18 && totaldirectnum>=9 && totaldirectdepositnum>=5000e18   && user.teamNum >= 250 &&   otherTeam  + othermaxTeam + maxTeam >=210000e18       ){
                levelNow = 5;
            }else if(total >= 3000e18 && totaldirectnum>=8 && totaldirectdepositnum>=3000e18 && user.teamNum >= 100 &&  otherTeam  + othermaxTeam + maxTeam >=60000e18    ){
                levelNow = 4;
            }else if(total >= 1000e18  && totaldirectnum>=7 && totaldirectdepositnum>=1000e18 && user.teamNum >= 50 &&  otherTeam  + othermaxTeam + maxTeam >=15000e18  ){
                levelNow = 3;
            }
            else if(total >= 500e18 && totaldirectnum>=5  && totaldirectdepositnum>=500e18)
            {
               levelNow = 2;
            }
    ...
    function _deposit(address _user, uint256 _amount) private {
       ...
       function _calCurRewardingMultiple(address _user) public view returns(uint256) {
        ...
        return rewarding;
         ...
         function _caldailyrewardperc(uint256 _amount) public pure returns(uint256) {
        uint256 rewardingperc = dayRewardPercents;
        if( _amount >= 50e18 && _amount<=1000e18) {
          rewardingperc = 200;    // now 2%
        }
         if( _amount > 1000e18 && _amount<=5000e18) {
          rewardingperc = 250;    // now 2.5%
        }
          if( _amount > 5000e18 ) {
          rewardingperc = 300;    // now 3%
        }
       
        return rewardingperc;
       }
     ...
     if(user.totalDeposit>0){
        ...
        if(block.timestamp > userclaim.currentdays)
        {
           ...
           for(uint256 i = 0; i < orderInfos[msg.sender].length; i++){
                 ...
                 uint256 _dailyrewardingper = _caldailyrewardperc(order.amount);
                  uint256 interest = order.amount.mul(_dailyrewardingper).mul(dayPerCycle).div(timeStep).div(baseDivider);    
 ...
 function _releaseReward(address _user, uint256 _amount, uint256 _initialamt) private {
               ...
               uint256 newinitialamt = _initialamt;
                if(upline != defaultRefer){       
                    ...
                    if(maxFreezing < newinitialamt){
                          uint256 _dailyrewardingper = _caldailyrewardperc(maxFreezing);
                         newAmount = maxFreezing.mul(_dailyrewardingper).mul(dayPerCycle).div(timeStep).div(baseDivider);  
                    }
                   }
    ...
    function withdraw() external {
        ...
        USDT.transfer(feeReceivers[0], withdrawable.mul(250).div(baseDivider));
        USDT.transfer(feeReceivers[1], withdrawable.mul(250).div(baseDivider));
 ...
 function _distributeDeposit(uint256 _amount) private {
         ...
         USDT.transfer(feeReceivers[0], _amount.mul(200).div(baseDivider));
         USDT.transfer(feeReceivers[1], _amount.mul(150).div(baseDivider));
         USDT.transfer(feeReceivers[2], _amount.mul(150).div(baseDivider));
