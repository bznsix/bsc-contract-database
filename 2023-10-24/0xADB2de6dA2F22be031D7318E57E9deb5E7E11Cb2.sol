...
contract LuncBurnMachineReflect is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LuncBurnMachineDividendTracker public dividendTracker;
    ...
    constructor() ERC20("LuncBurnMachine", "LBM") {
        marketingWallet = payable(0x04Ca40f8fFF2da720f8087E684ED31919E75E6c3);
        devWallet = payable(0x06ad731A5E734811679ACfD4F37DF422debbC80E);
        ...
        dividendTracker = new LuncBurnMachineDividendTracker(
            payable(this),
            router,
            0xECCF35F941Ab67FfcAA9A1265C2fF88865caA005,
            "LuncBurnMachineTRACKER",
            "LBMTRACKER"
        );
...
contract LuncBurnMachineDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    LuncBurnMachineReflect public LuncBurnMachineReflectContract;
    ...
    constructor(
        ...
        defaultToken = token;
        LuncBurnMachineReflectContract = LuncBurnMachineReflect(mainContract);
    ...
    function _transfer(
        ...
        require(false, "LuncBurnMachineReflect_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "LuncBurnMachineReflect_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LuncBurnMachineReflect contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(LuncBurnMachineReflectContract);

            uint256 prevBalance = LuncBurnMachineReflectContract.balanceOf(address(this));
            ...
            {
                uint256 received = LuncBurnMachineReflectContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    LuncBurnMachineReflectContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
