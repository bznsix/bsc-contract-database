...
abstract contract MintableToken is ERC20 {
    function farmMint(address _to, uint256 _amount) public virtual;
}
// SPDX-License-Identifier: MIT
...
contract MPS is ERC20("MPS", "MPS"), Ownable {
    ...
    constructor(uint256 _startReleaseBlock, uint256 _endReleaseBlock) public {
        require(
            _endReleaseBlock > _startReleaseBlock,
            "endReleaseBlock < startReleaseBlock"
        );
        _setupDecimals(18);
    ...
    function farmMint(address _to, uint256 _amount) public onlyMinter {
        require(
            farmMinted.add(_amount) <= farmMintLimit,
            "farm mint limit exceeded"
        );
        farmMinted = farmMinted.add(_amount);
        mint(_to, _amount);
    }
    ...
    function unlock() public {
        require(_locks[msg.sender] > 0, "no locked MPSs");
