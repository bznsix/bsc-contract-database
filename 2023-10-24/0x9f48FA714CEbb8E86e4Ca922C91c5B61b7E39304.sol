...
contract GEN_REWARDS is Context, IERC20 { 
    ...
    event updated_Buy_fees(uint8 Marketing, uint8 Liquidity, uint8 Rewards, uint8 Burn);
    event updated_Sell_fees(uint8 Marketing, uint8 Liquidity, uint8 Rewards, uint8 Burn);
    ...
    mapping (address => bool) public _bypassDistribution;                       // Tokens sent from this wallet do not triger distribution (used for airdrops)
    ...
    function Set_Fees(
        ...
        require(Rewards_on_SELL + Rewards_on_BUY >= 4, "Rewards on buy and sell combined must be 4 or more"); 

        // Buyer Protection: Max Fee 15% 
        require (Marketing_on_BUY + Liquidity_on_BUY + Rewards_on_BUY + Burn_on_BUY <= 15, "F1");  // Max fee 15%

        // Buyer Protection: Max Fee 15% 
        require (Marketing_on_SELL + Liquidity_on_SELL + Rewards_on_SELL + Burn_on_SELL <= 15, "F2");  // Max fee 15%
        ...
        emit updated_Buy_fees(_fee__Buy_Marketing, _fee__Buy_Liquidity, _fee__Buy_Rewards, _fee__Buy_Burn);
        emit updated_Sell_fees(_fee__Sell_Marketing, _fee__Sell_Liquidity, _fee__Sell_Rewards, _fee__Sell_Burn);
    ...
    */
    ...
    function Rewards_Exclude_Wallet(address Wallet_Address, bool true_or_false) external onlyOwner {
    ...
    */
    ...
    function setDistributionTriggers(uint256 Minutes_Between_Payments, uint256 Required_Reward_Balance) external onlyOwner {
    ...
    */
    ...
    function setDistributionGas(uint256 Gas_Amount) external onlyOwner {
    ...
    */
    ...
    // Used for airdrop to allow transfer to bypass distribution attempt
    function Wallet_ByPass_Distribution(
    ...
    */
    ...
    function _transfer(
        ...
        require(balanceOf(from) >= amount, "NB"); // Sender does not have enough tokens!
...
contract RewardDistributor is IDividendDistributor {
    ...
    IERC20 RWDTKN = IERC20(0x5a457fb51bD0F50df9eC9F3d0aeC04CD4B9Ce0BF); // GEN
