...
contract VibeZoneOnline is ERC20, Ownable {
    ...
    event UpdateBuyTax(uint256 taxOnBuy);
    event UpdateSellTax(uint256 taxOnSell);
    event UpdateWalletToWalletTransferTax(uint256 walletToWalletTransferTax);
    event SwapAndSendTax(uint256 tokensSwapped, uint256 busdSend);
    event SwapTokensAtAmountUpdated(uint256 swapTokensAtAmount);

    constructor () ERC20("VibeZoneOnline", "VZO") 
    {   
        transferOwnership(0x98fB052c12052c42fF0d43437860511D83957Eb9);
        ...
        taxOnBuy  = 5;
        taxOnSell = 5;
        walletToWalletTransferTax = 5;

        taxReceiver = 0x1509D902897f56E936BC99a9fC354e3E12Ffd3c8;

        _isExcludedFromFees[owner()] = true;
        _isExcludedFromFees[address(0xdead)] = true;
        _isExcludedFromFees[address(this)] = true;
        _isExcludedFromFees[taxReceiver] = true;
    ...
    function changeTaxReceiver(address _taxReceiver) external onlyOwner {
        require(_taxReceiver != address(0), "Tax Receiver cannot be zero address");
        taxReceiver = _taxReceiver;
        emit TaxWalletChanged(taxReceiver);
    }
    ...
    function swapAndSendTax(uint256 tokenAmount) private {
        ...
        IERC20(BUSD).transfer(taxReceiver, balance);

        emit SwapAndSendTax(tokenAmount, balance);
    }
}