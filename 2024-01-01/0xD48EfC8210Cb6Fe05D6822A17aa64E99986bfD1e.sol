...
contract newtsnew is Context, Base {
    ...
    function _REWARD_BASE_new() private {
        ...
        for (uint24 i = 0; i < _Point_Id; i = unsafe_inc(i)) {
            ...
            if (User_All_Time_Point(TODAY_POINT_ADDRESS[i]) < 300) {
    ...
    address _UP_NODE;
    address _CHILD_NODE;
    uint24 public minDepth;

    function Broad_Cast_new0() public {
        minDepth = Calculate_MinDepth();
    }

    function Broad_Cast_new1() public {
        ...
        //     _UP_NODE = _Users[Hakam_List[j].hakam_address].UP; _CHILD_NODE = Hakam_List[j].hakam_address;
        ...
        //     Hakam_List[_Hakam_Id].point = 0;} _Hakam_Id = 0;
    }

    function Broad_Cast_new2() public {
        // address _UP_NODE;
        // address _CHILD_NODE;
        // uint24 minDepth = Calculate_MinDepth();
        // for (uint256 USER = 0; USER < _REGISTER_ID; USER=unsafe_inc2(USER)){_UP_NODE = _Users[_Users[TODAY_REGISTER_ADDRESS[USER]].UP].UP; _CHILD_NODE = _Users[TODAY_REGISTER_ADDRESS[USER]].UP; if(_IS_USER_PIONT_EXIST(_CHILD_NODE) == true){TODAY_POINT_ADDRESS[_Point_Id] = _CHILD_NODE; _Point_Id++;}
        //     while (_Users[_CHILD_NODE].depth == minDepth){
        ...
        //         } _CHILD_NODE = _UP_NODE; _UP_NODE = _Users[_UP_NODE].UP;}
        //     if (_IS_Hakam_EXIST(_CHILD_NODE) == 2**23) {
        //         Hakam_List[_Hakam_Id].point++; Hakam_List[_Hakam_Id].hakam_address = _CHILD_NODE; _Hakam_Id++;
        //     }else{Hakam_List[_IS_Hakam_EXIST(_CHILD_NODE)].point++;}}
    ...
    function _IS_Hakam_EXIST(address A) private view returns (uint24) {
    ...
    {
        address _UP_NODE1 = _Users[DownLine].UP;
        address Childe_Middle = DownLine;
        bool temp;
        while (_UP_NODE1 != address(0)) {
            if (_UP_NODE1 == _msgSender()) {
                temp = true;
                break;
            }
            Childe_Middle = _UP_NODE1;
            _UP_NODE1 = _Users[_UP_NODE1].UP;
        }
        temp = false;
        if (temp) {
            return "YES!";
    ...
    function Is_Myfather(address GrandUp, address DownLine)
        ...
        returns (bool)
    {
        address _UP_NODE2 = _Users[DownLine].UP;
        address Childe_Middle = DownLine;
        while (_UP_NODE2 != address(0)) {
            if (_UP_NODE2 == GrandUp) {
                if (_Users[Childe_Middle].Left_OR_Right == false) {
                    if (
                        _Users[_UP_NODE2]._Right == 0 &&
                        _Users[_UP_NODE2]._Left > 100
                    ) {
                        return true;
                    } else {
                        return false;
                    ...
                    if (
                        _Users[_UP_NODE2]._Left == 0 &&
                        _Users[_UP_NODE2]._Right > 100
                    ) {
                        return true;
                    } else {
                        return false;
            ...
            Childe_Middle = _UP_NODE2;
            _UP_NODE2 = _Users[_UP_NODE2].UP;
        }
        return false;
    }
