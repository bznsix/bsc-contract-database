...
contract Matscoin is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MatscoinDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Matscoin", "Mats") {
        marketingWallet = payable(0xF2dD0E8dFD4De3eA34C67cf50408766b5673a7CB);
        devWallet = payable(0x6E0eF49A72d99153Fa1f746dd92B3a1DB783D407);
        ...
        dividendTracker = new MatscoinDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 1000; // 0.1%
        swapTokensAtAmount = totalTokenSupply / 100; // 1%;
...
contract MatscoinDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Matscoin public MatscoinContract;
    ...
    constructor(
        ...
        defaultToken = token;
        MatscoinContract = Matscoin(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Matscoin_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Matscoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Matscoin contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(MatscoinContract);

            uint256 prevBalance = MatscoinContract.balanceOf(address(this));
            ...
            {
                uint256 received = MatscoinContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MatscoinContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
