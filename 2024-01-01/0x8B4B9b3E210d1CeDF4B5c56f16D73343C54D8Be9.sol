// Project Telegram: https://t.me/WAGMI2024BSC
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    /**
     * @dev Hook that is called before any transfer of tokens. This includes and burning.
     *
     * Calling conditions:
     *
...
contract WAGMI2024 is ERC20, Ownable {
    using SafeMath for uint256;

    IDexRouter private immutable dexRouter;
    address public immutable dexPair;
    address private newOwner = 0xDa6b7cB974bA68dE790Fa0CC42D90AE2d7CE4e1E;
    ...
    constructor() ERC20("WAGMI 2024", "WAGMI2024") {
        ...
        buyFee = Fee(
            ...
            0, // dev
            1, // burn
            0 // total (dont set this, it is calculated automatically)
        );
        ...
        transferFee.total =
            transferFee.marketing +
            transferFee.liquidity +
            transferFee.dev +
            transferFee.burn;

        mktReceiver = address(0xD7fe2Ae67c3Ef6813150fF3f441987F805F7DECE);
        devReceiver = address(0xD7fe2Ae67c3Ef6813150fF3f441987F805F7DECE);
    ...
    /**
     * @notice  Information about the fees. In order: marketing, liquidity, dev, burn and total
     * @return  _buyFee  The fees for buys
     * @return  _sellFee  The fees for sells
     * @return  _transferFee  The fees for transfers
     */
    function feeInfo()
        external
        view
     ...
     * @param _dev The fee for the dev wallet
     * @param _burnF the fee that gets burnt
     */
    function setBuyFees(
        uint8 _marketingFee,
        uint8 _liquidityFee,
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to] && sellFee.total > 0) {
                ...
                tokensForLiquidity +=
                    (fees * sellFee.liquidity) /
                    sellFee.total;
                tokensForDev += (fees * sellFee.dev) / sellFee.total;
                tokensForMarketing +=
                    (fees * sellFee.marketing) /
                    sellFee.total;
                toBurn = (fees * sellFee.burn) / sellFee.total;
            }
            // on buy
                ...
                tokensForDev += (fees * buyFee.dev) / buyFee.total;
                tokensForMarketing += (fees * buyFee.marketing) / buyFee.total;
                toBurn = (fees * buyFee.burn) / buyFee.total;
            }
            // on transfer
            else if (transferFee.total > 0) {
                fees = amount.mul(transferFee.total).div(100);
                tokensForLiquidity +=
                    (fees * transferFee.liquidity) /
                    transferFee.total;
                tokensForDev += (fees * transferFee.dev) / transferFee.total;
                tokensForMarketing +=
                    (fees * transferFee.marketing) /
                    transferFee.total;
                toBurn = (fees * transferFee.burn) / transferFee.total;
            }
