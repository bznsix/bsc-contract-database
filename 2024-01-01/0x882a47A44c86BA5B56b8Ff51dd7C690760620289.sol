...
contract SanSanHuaLuo13 is Ownable, ReentrancyGuard {
    ...
    constructor() {
        nftToken = address(0x0fD708c726c21039142d48051FbCC43010bd113c);   //主网
    ...
    function buyLevel(uint8 _level) public nonReentrant {
        ...
        uint256 tokenAmount = usdt2Token(1e18);
        uint256 LevelPrices = levelPrice[_level];
        uint256 commission = tokenAmount * LevelPrices ;
        ...
        // 支付或待领取的佣金
        payOrAccrueCommission(msg.sender, tokenAmount, _level, LevelPrices ,commission);
    }
    ...
    function claimPendingRewards(uint8 level) public nonReentrant {
        ...
        require(nftt.transfer(msg.sender, reward * tokenAmount ), "Transfer failed");
        emit CommissionClaimed(msg.sender, level, reward * tokenAmount);
    }
    ...
    function _getUsdt2Token(uint usdtAmount)internal view returns(uint){
        address[] memory path = new address[](2);
        path[0] = USDT_ADDRESS;
        path[1] = nftToken;
        uint[] memory amountOuts = uniswapV2Router.getAmountsOut(usdtAmount, path);
        return amountOuts[1];
    }
    ...
    function _getToken2Usdt(uint tokenAmount)internal view returns(uint){
        ...
        path[0] = nftToken;
        path[1] = USDT_ADDRESS;
        uint[] memory amountOuts = uniswapV2Router.getAmountsOut(tokenAmount, path);
        return amountOuts[1];
    }

    function token2Usdt(uint tokenAmount) public view returns(uint){
        return _getToken2Usdt(tokenAmount);
    }

    function usdt2Token(uint usdtAmount) public view returns(uint){
