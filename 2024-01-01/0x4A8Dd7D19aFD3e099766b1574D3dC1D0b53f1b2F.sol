...
contract ICO is Context, ReentrancyGuard, Ownable {
    ...
    function copyData(
        ...
        for (uint256 index = 0; index < _buyers.length; index++) {
            totalTokens[_buyers[index]] = ICOOld(oldICO).totalTokens(_buyers[index]);
            totalWithdrawnTokens[_buyers[index]] = ICOOld(oldICO).totalWithdrawnTokens(_buyers[index]);
            referralCodes[_buyers[index]] = ICOOld(oldICO).referralCodes(_buyers[index]);
    ...
    function buyToken(
        ...
        require(codes[_refferalCode] != address(0), "Error: use valid referral code");
    ...
    function sendReferralCodes(address _buyer, uint256 _amount) internal {
        ...
        while (iterator < 5) {
            uint256 tokenToSend = (_amount *
                (referalPercentage[iterator].div(10))).div(100);
    ...
    function sendPendingReferral(address[] memory _buyer, address[] memory _referral, uint256[] memory tokenToSend, uint256[] memory _referalPercentage) public onlyOwner() {
