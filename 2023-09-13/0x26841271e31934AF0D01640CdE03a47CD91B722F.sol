...
contract PayrollInstanceV1 is IPayrollInstanceV1, Ownable, Pausable, Initializable, Multicall {
    ...
    modifier onlyOperator {
        require (msg.sender == owner() || operators.contains(msg.sender), 'PayrollInstanceV1(onlyOperator): forbidden');
        _;
    }
    ...
    function setPause(bool pause) external onlyOperator {
        if (pause) {
            _pause();
        } else {
            _unpause();
    ...
    function setEmployee(address employee, Employee calldata data, bool skipPayment) external whenNotPaused onlyOperator {
    ...
    function removeEmployee(address employee, bool skipPayment) external whenNotPaused onlyOperator {
    ...
    function changeEmployeeAddress(address employee, address newAddress, bool skipPayment) external whenNotPaused onlyOperator {
    ...
    function increaseFine(address employee, uint fine) external whenNotPaused onlyOperator {
    ...
    function decreaseFine(address employee, uint fine) external whenNotPaused onlyOperator {
    ...
    function payBonus(address employee, uint bonus) external whenNotPaused onlyOperator {
    ...
    function getOperators() external view returns (address[] memory) {
        address[] memory list = new address[](operators.length());
        for (uint i = 0; i < operators.length(); i++) {
            list[i] = operators.at(i);
        }
        return list;
    }
    ...
    function pendingTill(address employee, uint tillTime) public view returns (uint) {
        ...
        if (tillTime <= since.end) {
            return empl.salary * (tillTime - sinceTime + 1) / since.len / deno;
        }

        uint forThisMonth = empl.salary * (since.end - sinceTime + 1) / since.len;
        ...
        if (tillTime <= since.end + 31 days) {
            ...
            if (tillTime <= next.end) {
                return (forThisMonth + empl.salary * (tillTime + next.len - next.end) / next.len) / deno;
            }
        }
        ...
        return (
            ...
            empl.salary * (tillTime + till.len - till.end) / till.len
        ) / deno;
    }
