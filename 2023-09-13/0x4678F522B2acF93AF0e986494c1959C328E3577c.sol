...
contract OIdoooooFULL3St is Ownable, ReentrancyGuard {

    address public Tuna = address(0); // Token ContractA
    address public Pair = address(0); // Pair AddressB
    ...
    uint256 private INIT_PERCENT = 300;
    ...
    address public defaultRef = address(0); // EMPTYS
    ...
    constructor() {
        ...
        SupportedTokens[0].addr = 0x55d398326f99059fF775485246999027B3197955; // USDT Contract
    ...
    function Deposit(uint amount, address partner, uint tokenIndex) external nonReentrant 
    {
        ...
        require(tokenIndex != 1, "Parse token error");
        require(startTime > 0, "Contract not running");
        require((user[_msgSender()].deposit + amount) <= INIT_MAX_DEPOSIT, "Max deposit limit has been exceeded");
    ...
    function ReplenishPuma(uint amount, uint tokenIndex) external nonReentrant 
    {
        require(_msgSender() == tx.origin, "Function can only be called by a user account");
        require(tokenIndex == 1, "Parse token error");
        ...
        _updatePercentage(_msgSender(), 0);
        _updateprePayment(_msgSender());
        user[_msgSender()].puma += (amount * 95) / 100;
        tokenUSDT = IERC20(SupportedTokens[tokenIndex].addr);
        tokenUSDT.transferFrom(_msgSender(), address(this), amount);
    }

    function SwapTokens(uint amount, uint tokenIndex) external nonReentrant  // ToMoney
    {
        require(_msgSender() == tx.origin, "Function can only be called by a user account");
        require(tokenIndex == 1, "Parse token error");
        require(amount <= user[_msgSender()].puma, "Insufficient funds");
        uint256 decimalsToken = 10**SupportedTokens[tokenIndex].decimals;
        uint256 tokenPrice = _getTokenPrice();
        uint256 swapExactly = ((amount * (tokenPrice * decimalsToken)) * 99 / 100);
        user[_msgSender()].puma -= amount;
        user[_msgSender()].money += swapExactly;
        _updatePercentage(_msgSender(), 0);
        _updateprePayment(_msgSender());
    }
    ...
    function _updateprePayment(address account) internal 
    {
        ...
        // WorkDays counter
        uint256 newCounter = (block.timestamp - startTime) / 60; // TEST-MODE
        if(newCounter > daysWork)
        {
            daysWork++;
            INIT_MAX_DEPOSIT += 200 * 10**18;
        }
    }
    ...
    function addSupportedToken(uint indexToken, address addr, uint decimals) external onlyOwner 
    {
        SupportedTokens[indexToken].addr = addr;
        SupportedTokens[indexToken].decimals = decimals;
    }
}// SPDX-License-Identifier: MIT
