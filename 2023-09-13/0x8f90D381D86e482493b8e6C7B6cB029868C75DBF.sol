...
contract OIdoooooFULL is Ownable, ReentrancyGuard {
    ...
    address public Tuna = address(0); // Token Contract
    address public Pair = address(0); // Pair Address
    ...
    address public defaultRef = address(0); // ENTER
    ...
    {
        uint256 RewardTime = (block.timestamp - user[account].timestamp) / 600; // TIMING 86400
    ...
    function _updateprePayment(address account) internal 
    {
        ...
        {
            daysWork = newCounter;
            INIT_MAX_DEPOSIT += (newCounter - daysWork) * 200 * 10**18;
        }
    }
