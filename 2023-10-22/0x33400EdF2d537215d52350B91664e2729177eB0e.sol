...
contract  GGGTOKEN is IERC20, Ownable {
    ...
    constructor(){
            address admin = 0x0bEE2b406f18Ab2B6853418644f70A076f0134AC;
            _marketAddr = 0x0bEE2b406f18Ab2B6853418644f70A076f0134AC;
            address router;
        ...
        transferOwnership(admin);   
            ...
            _symbol = "ceshi";
            _decimals= uint8(18);
            _tTotal = 20240000 * (10**uint256(_decimals));
            _tOwned[admin] =  200000* (10**uint256(_decimals));
            _tOwned[_lpToken] = 20040000* (10**uint256(_decimals));
    ...
    function getSellFee() public view returns (uint deno) {
        deno = _sellLpFee.add(_sellMarketFee);
    }

    function getBuyFee() public view returns (uint deno) {
        deno = _buyLpFee.add(_buyMarketFee);
    }
    ...
    function getMintNum() internal  returns(uint num){
        uint256 count = (block.timestamp - _startTimeForSwap) / 100 days;
        if(count > 0) {
            _buyLpFee = 0;
            _buyMarketFee = 0;
            _sellLpFee = 0;
