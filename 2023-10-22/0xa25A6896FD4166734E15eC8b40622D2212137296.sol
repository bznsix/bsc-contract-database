// File: contracts/Compound/IRuleProxy.sol
...
interface IRuleProxy {
    ...
    function burn(MintParams calldata param) external returns(uint256 burnAmount);
...
contract GeneralCompound is  ReentrancyGuardUpgradeable, GovernanceUpgradeable {
    ...
    function earned(address account) public view returns(uint256) {
        uint256 beforeEarned = _balance[account].mul(rewardPerToken().sub(_userRewardPerTokenPaid[account])).div(1e18).add(_rewards[account]);
        uint256 afterEarned = beforeEarned.add(_balance[account]).mul(rewardPerToken().sub(_userRewardPerTokenPaid[account])).div(1e18).add(_rewards[account]);
        return afterEarned;     
    }
    ...
    function stake(MintData calldata mintData) public updateReward(msg.sender) checkStart nonReentrant {
        uint256 tAmount;
        ...
        (tAmount) = _ruleProxys[0].mint(params);

        uint256 stakedAmount = _balance[msg.sender];
        uint256 stakingAmount = stakedAmount.add(tAmount);

        totalSupply = totalSupply.add(tAmount);

        if(stakingAmount > 0) {
            _balance[msg.sender] = _balance[msg.sender].add(stakingAmount);
        }
        ...
        emit Staked(msg.sender,mintData.amount);
    }
    ...
    function unstake() public updateReward(msg.sender) checkStart nonReentrant {
        ...
        params.user = msg.sender;
        uint256 bAmount;
        (bAmount) = _ruleProxys[0].burn(params);
    ...
    function notifyReward(uint256 reward) external onlyGovernance updateReward(address(0)) {
        ...
        IERC20Upgradeable(_rewardERC20).transferFrom(msg.sender, address(this), reward);
