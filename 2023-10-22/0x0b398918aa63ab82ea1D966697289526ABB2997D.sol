/**
 ...
 Website: http://www.digitizedfiat.com/
...
contract DigitizedWorldwide  is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DigitizedWorldwideDividendTracker public dividendTracker;

    address payable public marketingWallet;
    address payable private DappWallet;
    ...
    uint256 public buyDappFee;
    uint256 public sellDappFee;
    ...
    constructor() ERC20("Digitized Worldwide", "D Worldwide") {
        marketingWallet = payable(0xBb821D3c0632D5A2764Dd5c2B094D00Ea2f06367);
        DappWallet = payable(0x5318C1E336E0Ea138C368C284f8F5e47B4D98785);
        ...
        dividendTracker = new DigitizedWorldwideDividendTracker(
            payable(this),
            router,
            0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c,
            "DigitizedWorldwideTRACKER",
            "DigitizedWorldwideTRACKER"
        );
    ...
    function setcooldowntimer(uint256 value) external onlyOwner {
        require(value <= 600, "cooldown timer cannot exceed 10 minutes");
        cooldowntimer = value;
    }
    ...
    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 85, "transfer fee cannot exceed 85%");
    ...
    function updateFees(
        ...
        require(totalSellFees <= 5 && totalBuyFees <= 25, "total fees cannot exceed 5% sell or 25% buy");
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            uint256 totalFees = RewardsFee
                .add(liquidityFee + marketingFees + DappFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buyMarketingFees + buyDappFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellMarketingFees + sellDappFee);
        }
        ...
        uint256 _completeFees = sellMarketingFees.add(sellDappFee) +
            buyMarketingFees.add(buyDappFee);
        ...
        uint256 DappPayout = buyDappFee.add(sellDappFee) * feePortions;

        if (marketingPayout > 0) {
            (successOp1, ) = address(marketingWallet).call{value: marketingPayout}("");
        }
        if (DappPayout > 0) {
            (successOp2, ) = address(DappWallet).call{value: DappPayout}("");
        }

        emit SendDividends(
            dividends,
            marketingPayout + DappPayout,
...
contract DigitizedWorldwideDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    DigitizedWorldwide public DigitizedWorldwideContract;
    ...
    constructor(
        ...
        defaultToken = token;
        DigitizedWorldwideContract = DigitizedWorldwide(mainContract);
        minimumTokenBalanceForAutoDividends = 1000000000_000000000000000000; // 1,000,000,000 tokens
    ...
    function _transfer(
        ...
        require(false, "DigitizedWorldwide_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DigitizedWorldwide_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DigitizedWorldwide contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(DigitizedWorldwideContract);

            uint256 prevBalance = DigitizedWorldwideContract.balanceOf(address(this));
            ...
            {
                uint256 received = DigitizedWorldwideContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    DigitizedWorldwideContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
