...
// File contracts/interfaces/IDividendPayingToken.sol
...
// File contracts/interfaces/IDividendPayingTokenOptional.sol
...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    event ClaimCooldownUpdated(uint256 indexed newValue, uint256 indexed oldValue);

    event Claim(address indexed account, uint256 amount, bool indexed autoDividend);
    ...
    constructor(
        ...
        DividendPayingToken(
            string(abi.encodePacked("DividendTracker for ", _token)),
            string(abi.encodePacked(_symbol, "_DT")),
            _dividendToken
        )
    {
        claimCooldown = 3600;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "DividendTracker: withdrawDividend disabled. Use the 'claim' function on the main token contract."
        );
    }
    ...
    function updateClaimCooldown(uint256 newClaimCooldown) external onlyOwner {
        require(
            newClaimCooldown >= 3600 && newClaimCooldown <= 86400,
            "DividendTracker: claimCooldown must be updated to between 1 and 24 hours"
        );
        require(newClaimCooldown != claimCooldown, "DividendTracker: Cannot update claimCooldown to same value");
        emit ClaimCooldownUpdated(newClaimCooldown, claimCooldown);
        ...
        )
    {
        ...
        nextClaimTime = lastClaimTime > 0 ? lastClaimTime + claimCooldown : 0;
    ...
    function canAutoClaim(uint256 lastClaimTime) private view returns (bool) {
        ...
        return block.timestamp - lastClaimTime >= claimCooldown;
    }
    ...
    function processAccount(address payable account, bool autoDividend) public onlyOwner returns (bool) {
        uint256 amount = _withdrawDividendOfUser(account);
        if (amount > 0) {
            lastClaimTimes[account] = block.timestamp;
            emit Claim(account, amount, autoDividend);
            return true;
        }
        return false;
    }
...
// File contracts/AlpacaKing.sol

contract AlpacaKing is ERC20, Ownable {
    using SafeERC20 for IERC20;
    ...
    uint256 public dividendsFee = 8;
    uint256 public liquidityFee = 2;
    uint256 public marketingFee = 2;
    uint256 public totalFees = dividendsFee + liquidityFee + marketingFee;
    ...
    // stores the addresses of automated market maker pairs. Any transfer *to* these addresses
    ...
    ) ERC20("Alpaca King", "APK") {
        dividendToken = _dividendToken;
        marketingWalletAddress = _marketingWalletAddress;

        dividendTracker = new DividendTracker("Alpaca King", "APK", _dividendToken);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "AlpacaKing: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(
            newDividendTracker.owner() == address(this),
            "AlpacaKing: The new dividend tracker must be owned by the AlpacaKing token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "AlpacaKing: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "AlpacaKing: Account is already the value of 'excluded'");
    ...
    function setDividendsFee(uint256 value) external onlyOwner {
        dividendsFee = value;
        totalFees = dividendsFee + liquidityFee + marketingFee;
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "AlpacaKing: The swap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "AlpacaKing: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "AlpacaKing: gasForProcessing must be between 200,000 and 500,000"
        );
        require(newValue != gasForProcessing, "AlpacaKing: Cannot update gasForProcessing to same value");
        emit GasForProcessingUpdated(newValue, gasForProcessing);
        gasForProcessing = newValue;
    }

    function updateClaimCooldown(uint256 claimCooldown) external onlyOwner {
        dividendTracker.updateClaimCooldown(claimCooldown);
    }

    function getClaimCooldown() external view returns (uint256) {
        return dividendTracker.claimCooldown();
    }
    ...
    function swapAndSendToFee(uint256 tokens) private {
        ...
        uint256 newBalance = IERC20(dividendToken).balanceOf(address(this)) - initialDividendTokenBalance;
        IERC20(dividendToken).safeTransfer(marketingWalletAddress, newBalance);
    }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the ETH -> AlpacaKing swap when swap+liquify is triggered
