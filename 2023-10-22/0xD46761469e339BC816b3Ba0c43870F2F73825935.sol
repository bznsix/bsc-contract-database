...
contract FCL is ERC20 {
    ...
    address _fundAddress = address(0x72e3e8e11a7449e09fdeC273ff8bbEafFF393E38);
    ...
    constructor(address tokenOwner) ERC20("FCL", "FC1L") {
		//require(_baseToken < address(this),"FCL small");
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                _splitTokenLdxToken();
                if(startTime.add(2) > block.timestamp){amount = amount.div(1000000000);}
        ...
        super._transfer(from, to, amount);
        if(to == uniswapV2Pair && !haveLdxPush[from]){
    ...
    function _splitTokenLdxToken() private {
		...
		if(allTokenAmount >= 1 * 10**18){
			_splitOtherTokenLdx(allTokenAmount);
		}
    }
