...
/// @title The Forgotten Kings
contract TheForgottenKings is ERC20, Ownable2Step {
    /// custom errors
    ...
    error BNBClaimFailed();
    error OnlyFeeWalletCanCallThis();
    
    /// @notice Max limit on Buy / Sell fees
    uint256 public constant MAX_FEE_LIMIT = 5; 
    /// @notice max total supply 777,000,000 tokens (18 decimals)
    uint256 private maxSupply = 777000000 * 1e18; 
    /// @notice swap threshold at which collected fees tokens are swapped for bnb, autoLP
    uint256 public swapTokensAtAmount = 7e3 * 1e18;
    ...
    //// EVENTS //// 
    event BuyFeesUpdated(uint16 indexed developmentFee, uint16 indexed liquidityFee);
    event SellFeesUpdated(uint16 indexed developmentFee, uint16 indexed liquidityFee);
    event FeesSwapped(uint256 indexed ethForLiquidity, uint256 indexed tokensForLiquidity,  uint256 indexed ethFordevelopment);

    
    constructor () ERC20("The Forgotten Kings", "TFK"){
      uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
      uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());
      isAutomatedMarketMaker[uniswapV2Pair] = true;

      buyFee.development = 2;
      buyFee.autoLP = 2;
      totalBuyFee = 4;

      sellFee.development = 2;
      sellFee.autoLP = 2;
      totalSellFee = 4;
      
      developmentWallet = 0x0402f83d180D5311592B4023A030850507cAF72a;
    ...
    /// @dev owner can claim other bep20 tokens, if accidently sent by someone
    /// @param _token: token address to be rescued
    /// @param _amount: amount to rescued
    /// Requirements --
    /// Cannot claim native token
    function claimOtherBep20 (address _token, uint256 _amount) external onlyOwner {
    ...
    /// @dev claim bnb from contract
    function claimBNB () external onlyOwner{
        (bool sent,) = payable(developmentWallet).call{value: address(this).balance}("");
        if(!sent){
            revert BNBClaimFailed();
        }
    }
    ...
    function addOrRemoveLiquidityPairs (address _newPair, bool value) external onlyOwner {
    ...
    /// @dev update development fee wallet
    /// @param _newdevelopmentWallet: new development wallet address
    /// Requirements -
    /// Address should not be zero
    function setDevelopmentWallet(address _newdevelopmentWallet) external onlyOwner {
    ...
    /// @dev update buy fees
    /// @param _development: development fees
    /// @param _autoLP: liquidity fees
    /// Requirements --
    /// total Buy fees must be less than equals to MAX_FEE_LIMIT (10%);
    function updateBuyFees (uint16 _development, uint16 _autoLP) external onlyOwner {
        if(_development + _autoLP > MAX_FEE_LIMIT){
            revert FeesLimitExceeds();
        }
        buyFee.development = _development;
        buyFee.autoLP = _autoLP;
        totalBuyFee = _development + _autoLP;
        emit BuyFeesUpdated(_development, _autoLP);
    ...
    function updateSellFees (uint16 _development, uint16 _autoLP) external onlyOwner {
        if(_development + _autoLP > MAX_FEE_LIMIT){
            revert FeesLimitExceeds();
        }
        sellFee.development = _development;
        sellFee.autoLP = _autoLP;
        totalSellFee = _development + _autoLP;
        emit SellFeesUpdated(_development, _autoLP);
    ...
    /// after conversion, it sends eth to development wallet, add auto liquidity
    ...
    function swapAndLiquify (uint256 tokenAmount) private lockTheSwap {
        if(totalBuyFee + totalSellFee == 0){
            ...
            bool m;
           (m,) = payable(developmentWallet).call{value: address(this).balance}("");

        } else {

        uint256 developmentTokens = ((buyFee.development + sellFee.development) * tokenAmount) / (totalBuyFee + totalSellFee);
        uint256 liquidityTokens = tokenAmount - developmentTokens;
        ...
        uint256 bnbBalanceAfterSwap = address(this).balance - bnbBalanceBeforeSwap;
        uint256 bnbForLiquidity = (liquidityTokensHalf * bnbBalanceAfterSwap) /swapTokens;

        if(bnbForLiquidity > 0 && liquidityTokensHalf > 0){
            addLiquidity(liquidityTokensHalf, bnbForLiquidity);
        }
        bool success;
        uint256 developmentBnb = address(this).balance;

        if(developmentBnb > 0){
          (success,) = payable(developmentWallet).call{value: developmentBnb}("");
        }
        emit FeesSwapped(bnbForLiquidity, liquidityTokensHalf, developmentBnb);
        }

    }

    /// @notice manages tokens conversion to BNB
    /// @param tokenAmount: tokens to be converted to BNB
    ...
    /// @param bnbAmount: bnb to be added to liquidity
