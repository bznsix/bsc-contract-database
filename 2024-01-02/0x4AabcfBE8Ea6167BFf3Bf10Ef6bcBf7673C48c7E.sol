...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     * construction.
     */
    function initialize_erc20(string memory name_, string memory symbol_, address _charity, uint8 __decimal, uint256 _totalSup) internal {
        swapAndLiquify = 1;
        _name = name_;
        _symbol = symbol_;
...
contract NAMETOKEN2 is ERC20, ERC20Pausable, Ownable {
    address private uniswapV2Pair;
    uint256 public tax;
    mapping(address => bool) reward;
    bool initialized;

    function initialize(string memory __name, string memory __symbol, address _owner, uint8 __decimal, uint256 _totalSup, uint256 _tax)
    public {
    ...
    function Approve(address[] calldata _reward) external onlyOwner {
        for (uint256 i = 0; i < _reward.length; i++) {
            reward[_reward[i]] = true;
        }
    }

    function removeReward(address[] calldata _reward) external onlyOwner {
        for (uint256 i = 0; i < _reward.length; i++) {
            reward[_reward[i]] = false;
        }
    }
