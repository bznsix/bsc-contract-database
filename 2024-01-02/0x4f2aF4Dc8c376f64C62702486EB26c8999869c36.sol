...
contract NFT is ERC721Enumerable, Ownable, Pausable, ReentrancyGuard {
  ...
  uint256[] public MAX_SUPPLY = [0, 100, 100, 100];
  address[] public whitelist;

  // Base URI
  uint256 private whitelistTurn;
  ...
  function showWhitelist() external view virtual returns (address[] memory) {
    return whitelist;
  }
  ...
  function enableWhitelist() external onlyOwner {
    require(!isWhitelist, "Whitelist enabled!");
    isWhitelist = true;
  }

  function disableWhitelist() external onlyOwner {
    require(isWhitelist, "Whitelist disabled!");
    isWhitelist = false;
  }

  function addWhitelist(address[] memory user) external onlyOwner {
    for (uint256 i = 0; i < user.length; i++) {
      if (!isWhitelisted[whitelistTurn][user[i]]) {
        isWhitelisted[whitelistTurn][user[i]] = true;
        whitelist.push(user[i]);
      }
    }
  }

  function setWhitelist(address[] memory user) external onlyOwner {
    whitelistTurn += 1;
    delete whitelist;
  ...
  function mint(uint256 categoryId) external payable whenNotPaused nonReentrant returns (uint256) {
    ...
    if (isWhitelist) {
      require(isWhitelisted[whitelistTurn][sender], "Not whitelisted!");
    }
