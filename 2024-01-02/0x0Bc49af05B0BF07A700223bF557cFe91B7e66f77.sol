...
contract EventManager {
    ...
    enum EventOutcome { Home, Draw, Away }
    ...
    function declareResult(string memory _eventId) external onlyOwner returns (EventOutcome) {
        require(events[_eventId].outcomeCount > 0, "Invalid event");
    ...
    function getRandomOutcome(uint outcomeCount) internal view returns (EventOutcome) {
        ...
        if (outcomeCount == 2) {
            return randomValue == 0 ? EventOutcome.Home : EventOutcome.Away;
        } else if (outcomeCount == 3) {
            if (randomValue == 0) {
                return EventOutcome.Home;
            } else if (randomValue == 1) {
                return EventOutcome.Draw;
            } else {
                return EventOutcome.Away;
            }
        } else {
