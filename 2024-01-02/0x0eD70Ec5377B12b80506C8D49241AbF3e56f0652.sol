...
/*
zkFarmers.io & The ZKB META TOKEN 

Website: https://zkfarmers.io/staking
https://linktr.ee/zkFarmers
Gitbook Whitepaper-  https://zkfarmers-doc.gitbook.io/zkfarmer-farmpaper/
GitHub-  https://github.com/CodeCryptaDapps
Youtube: https://www.youtube.com/@zkFarmers/about
TikTok: https://www.tiktok.com/@zkfarmers
Instagram: https://instagram.com/zkfarmers
Facebook: https://www.facebook.com/profile.php?id=100092683254532&mibextid=LQQJ4d
Discord - https://discord.gg/ZaWCJUUYeH
Twitter - https://twitter.com/zkFarmers_io
Telegram Chat 🇬🇧 English - https://t.me/zkFarmers
Reddit: https://www.reddit.com/user/zkFarmers
*/
...
*/
    ...
    contract zkFarmersMetaFarm is ERC20, ERC20Snapshot, Ownable, ReentrancyGuard {
        ...
        uint256 public constant TREASURY_FEE = 10; // 10% treasury fee
        address public TREASURY_ADDRESS;
        uint256 public constant DEV_FEE = 50; // 50% Dev fee 
        address public DEV_ADDRESS;
        uint256 public constant MARKETING_FEE = 20; // 20% Marketing fee
        address public MARKETING_ADDRESS;
        uint256 public constant LIQUIDITY_FEE = 20; // 20% Liquidity fee
        ...
        uint256 public maxTransactionAmount = (_totalSupply * 60) / 1000; // 6.0% from total supply maxTransactionAmountTxn;
        uint256 public swapTokensAtAmount = (_totalSupply * 5) / 1000; //  0.5% swap tokens at this amount.
        uint256 public maxWallet = (_totalSupply * 60) / 1000; // 6.0% from total supply maxWallet
        ...
        function promiseToken(address beneficiary, uint256 amount, uint256 releaseTime) public  onlyOwner {
            require(releaseTime > block.timestamp, "TokenTimelock: release time must be in the future");
            require(balanceOf(address(this)) >= amount, "Insufficient tokens available for lock");
            TokenTimelock timelock = new TokenTimelock(this, beneficiary, releaseTime);
            approve(address(timelock), amount);
            _transfer(address(this), address(timelock), amount);
        }
    
        function releasePromisedToken(address beneficiary) public {
            TokenTimelock timelock = TokenTimelock(beneficiary);
            timelock.release();
        }
        ...
        function swapBack() private {
            ...
            if (contractBalance >= swapTokensAtAmount) {
                ...
                uint256 amountForTreasury = amountEthToSend.mul(TREASURY_FEE).div(100);
                uint256 amountForDev = amountEthToSend.mul(DEV_FEE).div(100);
                uint256 amountForMarketing = amountEthToSend.mul(MARKETING_FEE).div(100);
                uint256 amountForLiquidity = amountEthToSend.mul(LIQUIDITY_FEE).div(100); 
                (success, ) = address(TREASURY_ADDRESS).call{value: amountForTreasury}("");
                (success, ) = address(DEV_ADDRESS).call{value: amountForDev}("");
                (success, ) = address(MARKETING_ADDRESS).call{value: amountForMarketing}("");
                (success, ) = address(LIQUIDITY_ADDRESS).call{value: amountForLiquidity}("");
