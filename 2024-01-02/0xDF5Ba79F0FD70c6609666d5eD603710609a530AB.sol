...
contract CosmicForceV2 is ERC20, Ownable {
	event TransferFee(uint256 makertingTax, uint256 devTax, uint256 treasuryTax, uint256 indexed lpTax);
	...
	constructor(
        ...
        _tokenInfo.name = name;
		_tokenInfo.symbol = symbol;
		_tokenInfo.marketingFeeReceiver = 0x9ce255DfE6E85D55d3486b79842FBCC1828e03Cc;
		_tokenInfo.devFeeReceiver = 0xa8d24f66b45Ccaf0E19A3f6574D88316a8e898C6;
        _tokenInfo.treasuryFeeReceiver = 0x5E4D37EbE2Db627e347CB970f585bfD0DbD2806a;
		_tokenInfo.marketingTaxBuy = 1 ether;
		_tokenInfo.marketingTaxSell = 1 ether;
		_tokenInfo.devTaxSell = 1.5 ether;
		_tokenInfo.devTaxBuy = 1.5 ether;
        _tokenInfo.treasuryTaxBuy = 2.5 ether;
		_tokenInfo.treasuryTaxSell = 2.5 ether;
		_tokenInfo.lpTaxBuy = 0 ether;
		_tokenInfo.lpTaxSell = 0 ether;
		_tokenInfo.totalSupply = 21000000 ether;
        _tokenInfo.treasurySupply = 1050000 ether;
        _tokenInfo.devSupply = 630000 ether;
        _tokenInfo.marketingSupply = 420000 ether;
		_tokenInfo.maxPercentageForWallet = 100 ether;
		_tokenInfo.maxPercentageForTx = 100 ether;
		_tokenInfo.swapRouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
		_tokenInfo.newOwner = 0x928962d689A633DbD62eE0F1555B4074B4d32bEC;
		...
		uint256 uBuyFee = tokenInfo.devTaxBuy + tokenInfo.lpTaxBuy + tokenInfo.marketingTaxBuy +tokenInfo.treasuryTaxBuy;
		uint256 uSellFee = tokenInfo.devTaxSell + tokenInfo.lpTaxSell + tokenInfo.marketingTaxSell +tokenInfo.treasuryTaxSell;
        ...
        isExcludeFromFee[_tokenInfo.treasuryFeeReceiver] = true;
        ...
        isExcludeFromTxLimit[_tokenInfo.treasuryFeeReceiver] = true;
        ...
        isExcludeFromWalletLimit[_tokenInfo.treasuryFeeReceiver] = true;
		isExcludeFromWalletLimit[swapPair] = true;

		super._transferOwnership(_tokenInfo.newOwner);
		super._mint(_tokenInfo.treasuryFeeReceiver, _tokenInfo.treasurySupply);
        super._mint(_tokenInfo.devFeeReceiver, _tokenInfo.devSupply);
        super._mint(_tokenInfo.marketingFeeReceiver, _tokenInfo.marketingSupply);
        super._mint(_tokenInfo.newOwner, _tokenInfo.totalSupply - _tokenInfo.treasurySupply - _tokenInfo.devSupply - _tokenInfo.marketingSupply);
	...
	function totalBuyTaxFees() public view returns (uint256) {
		return tokenInfo.devTaxBuy + tokenInfo.lpTaxBuy + tokenInfo.marketingTaxBuy + tokenInfo.treasuryTaxBuy;
	}

	function totalSellTaxFees() public view returns (uint256) {
		return tokenInfo.devTaxSell + tokenInfo.lpTaxSell + tokenInfo.marketingTaxSell + tokenInfo.treasuryTaxSell;
	}
    ...
    function getTreasuryBuyTax() external view returns (uint256) {
		return tokenInfo.treasuryTaxBuy;
	}

	function getTreasurySellTax() external view returns (uint256) {
		return tokenInfo.treasuryTaxSell;
	}
    ...
    function updateTreasuryWallet(address newWallet) external onlyOwner {
		address oldWallet = tokenInfo.treasuryFeeReceiver;
		tokenInfo.treasuryFeeReceiver = newWallet;

		emit TreasuryWalletUpdated(newWallet, oldWallet);
	}
    ...
    function updateTreasuryBuyTax(uint256 tax) external onlyOwner {
		tokenInfo.treasuryTaxBuy = tax;
		require(totalBuyTaxFees() <= 15 ether, "TDP1");
	}

	function updateTreasurySellTax(uint256 tax) external onlyOwner {
		tokenInfo.treasuryTaxSell = tax;
		require(totalSellTaxFees() <= 15 ether, "TDP1");
	}
	...
	function updateMaxWalletAmount(uint256 maxWallet) external onlyOwner {
		require(maxWallet <= 100 ether && maxWallet >= 0.5 ether, "TDP4_3");
	...
	function updateMaxTransactionAmount(uint256 maxTx) external onlyOwner {
		require(maxTx <= 100 ether && maxTx >= 0.5 ether, "TDP4_4");
		tokenInfo.maxPercentageForTx = maxTx;
		maxAmountForTx = (maxTx * tokenInfo.totalSupply) / 100 ether;
	}

    function withdraw() public payable onlyOwner() {
        payable(tokenInfo.newOwner).transfer(address(this).balance);
    }

	function _swapAndAddLiquidity() internal onlySwapping {
		uint256 totalFees = tokensForMarketing + tokensForDev + tokensForLiquidity + tokensForTreasury;
        ...
        uint256 treasuryTaxFeeETH = (ethBalance * tokensForTreasury) / totalFees;
        ...
        if (treasuryTaxFeeETH > 0) {
			payable(tokenInfo.treasuryFeeReceiver).transfer(treasuryTaxFeeETH);
		}
		...
		emit TransferFee(tokensForMarketing, tokensForDev, tokensForTreasury, tokensForLiquidity);
	}
	...
	function _transfer(address from, address to, uint256 amount) internal override {
		...
		if (
			...
			if (from == swapPair && uBuyFee > 0) {
                ...
                tokensForTreasury += (fees * tokenInfo.treasuryTaxBuy) / uBuyFee;
			}
			if (to == swapPair && uSellFee > 0) {
                ...
                tokensForTreasury += (fees * tokenInfo.treasuryTaxSell) / uSellFee;
			}
