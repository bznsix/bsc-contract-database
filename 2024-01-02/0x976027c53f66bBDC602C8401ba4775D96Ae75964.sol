...
contract Crowdsale is Context, Ownable, ReentrancyGuard {
    ...
    event TokenPurchased(address indexed purchaser, address indexed beneficiary, uint256 value, uint256 amount, uint256 date);
	
	event TokenPurchasedUsdt(address indexed purchaser, address indexed beneficiary, uint256 value, uint256 amount, uint256 date);

    event TokenReleased(address indexed beneficiary, uint256 value, uint256 date);
	...
	event referralBonus(address indexed beneficiary, address indexed sponsor, uint256 reward, uint256 usdt, uint256 package, uint256 level, uint256 date);
    ...
    modifier minAmountUsdt(uint256 usdAmount) {
        require(usdAmount >= 10,"Minimal amount is $10");
        _;
    }
    ...
    constructor(IERC20 token, IERC20 usdt, address payable wallet, uint256 rate) {
        _token = token;
		_usdt = usdt;
        _wallet = wallet;
        _admin = _msgSender();
        ...
        _cliffVesting = 7890000;
		
		m_refPercent[0] = Percent.percent(3, 100); // 3/100*100% = 3%
	    m_refPercent[1] = Percent.percent(2, 100); // 2/100*100% = 2%
    ...
    fallback() external payable {
		...
		buyTokens(_msgSender(), a);
    }

    function buyTokens(address beneficiary, address sponsor) public payable minAmount nonReentrant {
	...
	function buyTokensByUSDT(uint256 usdAmount, address sponsor) public payable minAmountUsdt(usdAmount) nonReentrant {
      ...
      if (!success) {
        revert tokensBuy__Failed();
      } else {
        uint256 tokenAmount = (usdAmount*(_rate))/10**6;
        ...
        if (sponsor.notZero()) {
            ...
            doMarketing(usdAmount, true);       
        } 

        _usdt.transfer(_wallet, _usdt.balanceOf(address(this)));

        emit TokenPurchasedUsdt(_msgSender(), beneficiary, usdAmount, tokenAmount, block.timestamp);
      }
    }
	
	function doMarketing(uint256 weiAmount, bool is_usdt) internal {
      // level 1
      address payable sponsorOne = payable(referral_tree[_msgSender()]);
      if (notZeroNotSender(sponsorOne)) {
          addReferralBonus(sponsorOne, weiAmount, 1, is_usdt);
          // level 2
          address payable sponsorTwo = payable(referral_tree[sponsorOne]);
          if (notZeroNotSender(sponsorTwo)) { 
              addReferralBonus(sponsorTwo, weiAmount, 2, is_usdt);
          }
      }
    }

    function addReferralBonus(address payable sponsor, uint256 weiAmount, uint level, bool is_usdt) internal {
        uint index = level-1;
        uint reward = m_refPercent[index].mul(weiAmount);
        if (is_usdt) {
		    emit referralBonus(_msgSender(), sponsor, reward, reward, weiAmount, level, block.timestamp);
            _usdt.transfer(sponsor, reward);            
        } else {
		    emit referralBonus(_msgSender(), sponsor, reward, _getUsdAmount(reward), _getUsdAmount(weiAmount), level, block.timestamp);
    ...
    function release(
        ...
        emit TokenReleased(beneficiaryPayable, amount, block.timestamp);
    }
    ...
    function addVestingScheduleByManual(address payable beneficiary, uint256 tokenAmount, uint256 durationVesting) public onlyOwner {
        uint256 currentTime = getCurrentTime();
        createVestingSchedule(beneficiary, currentTime, _cliffVesting, durationVesting, _periodVesting, true, tokenAmount);
    }
	
	//unfreeze transfer another token to contract by accident
