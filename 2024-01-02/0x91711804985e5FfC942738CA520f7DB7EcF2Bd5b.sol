...
contract SmartMoneyy is Ownable {
    ...
    { token = IERC20(_token); miniDeposit = 50*(10**18); }

 
    function deposit(uint256 _amount) external  {
        require(_amount >= miniDeposit, "Deposit amount must be greater than miniDeposit!");
        require(token.transferFrom(msg.sender, address(this), _amount), "Transfer to contract failed!"); 
        emit Deposited(msg.sender, _amount);
    }

    function updateMiniDeposit(uint256 _amount) 
    external 
    onlyOwner
    { miniDeposit = _amount ; }
