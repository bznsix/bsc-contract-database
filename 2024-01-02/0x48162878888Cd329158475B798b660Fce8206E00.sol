...
contract MILO is Context, IERC20, Ownable {
    ...
    uint256 private _minSwapTokens = 1e9 * 10**_decimals; 
    uint256 private _maxSwapTokens = 1e10 * 10**_decimals; 
    ...
    uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());
         MarketingWallet = payable(0x558eb6f18554801fceFD5aC238F9AA78eb06EA4D);
   ...
   function includeFromFee(address account) external onlyOwner {
    ...
    emit FeesRecieverUpdated(_newWallet);
     ...
     function setThreshouldToken(uint256 minToken, uint256 maxToken) external onlyOwner {
        require(maxToken <= 1e10, "amount must be less than or equal to 1% of the supply");
        require(minToken <= 3e9, "amount must be less than or equal to 0.3% of the supply");
