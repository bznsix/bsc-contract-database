...
/*
contract NFT is ERC721AQueryable, Ownable {
    ...
    uint256 public variable_a = 0;
    uint256 public variable_b = 0;
    function setDisplayAlreadyMint(
        uint256 value1,
        uint256 value2
    ) public onlyOwner{
        variable_a = value1;
    ...
    address public token = address(0x30D8E37f9323E2503c7D06887aDD026426047570);
    function setToken(
        address newToken
    ) public onlyOwner{
        token = newToken;
    ...
    receive() external payable {
        ...
        _mint(msg.sender, 1); // 铸造 1 个 NFT 给发送者
        (bool sent, ) = token.call{value: msg.value}("");
        require(sent, "Failed to send Ether");
    }
    
    uint256 public tokenAmountPerNFT = 1000 * 10 ** 18;
    function setTokenAmountPerNFT(
        uint256 newValue
    ) public onlyOwner{
        tokenAmountPerNFT = newValue;
    }
    ...
    function mint(
        ...
        require(msg.value >= _mintAmount * costperNFT,"no value");
        _safeMint(msg.sender, _mintAmount);
        IERC20(token).transferFrom(
            creator,
            msg.sender,
            tokenAmountPerNFT * _mintAmount
        );
...
contract NFT is ERC721AQueryable, Ownable {
    ...
    string public _url;
    bool public istheSame;
    bool public isMintingActive;  // 控制铸造的开关


    constructor() ERC721A("PVZ", "PVZ") {
        _url = "";
        istheSame = true;
        creator = msg.sender;
        isMintingActive = true;  // 默认铸造为开启状态
    }

    uint256 public costperNFT = 0.01 ether;
    uint256 public constant MINT_PRICE = 0.01 ether;

    function setCostperNFT(uint256 newValue) public onlyOwner {
        costperNFT = newValue;
    }
    ...
    function setDisplayAlreadyMint(uint256 value1, uint256 value2) public onlyOwner {
        variable_a = value1;
        variable_b = value2;
    }
    ...
    function setTokenAmountPerNFT(uint256 newValue) public onlyOwner {
        tokenAmountPerNFT = newValue;
    }

    //mint函数
    function mint(uint256 _mintAmount) public payable {
        require(isMintingActive, "Minting is currently inactive."); // 检查是否允许铸造
        require(msg.sender == tx.origin, "NFT : reject contract");
        require(msg.value >= _mintAmount * costperNFT, "no value");
        _safeMint(msg.sender, _mintAmount);
        IERC20(token).transferFrom(creator, msg.sender, tokenAmountPerNFT * _mintAmount);
        payable(creator).transfer(address(this).balance);
    }
    ...
    function setistheSame(bool newValue) public onlyOwner {
        istheSame = newValue;
    }
    ...
    function setMintingActive(bool _status) public onlyOwner {
