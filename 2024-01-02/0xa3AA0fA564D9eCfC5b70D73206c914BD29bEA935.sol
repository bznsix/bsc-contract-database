...
contract SphereProtocol is Context, IERC20, Ownable {
    ...
    uint256 private minimumTokensBeforeSwap = 210000 * 10**_decimals;
    ...
    constructor () {
        ...
        _totalTaxIfBuying = _buyLiquidityFee.add(_buyMarketingFee).add(_buyTeamFee).add(_buyDevFee);
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellTeamFee).add(_sellDevFee);
        _totalDistributionShares = _LiquidityShare.add(_MarketingShare).add(_TeamShare).add(_DevShare);
    ...
    function setBuyTax(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTeamTax, uint256 newDevTax) external onlyOwner() {
    ...
    function setSellTax(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTeamTax, uint256 newDevTax) external onlyOwner() {
