...
contract Bridge {
    ...
    function deposit(
        ...
        if (token == address(0)) {
            ...
            if (isPeggingToken[token]) {
                ...
                revert("bridge: token not allowed");
            }
        }
    ...
    function transfer(uint[][] memory args) external payable onlyAdmin {
        for (uint i = 0; i < args.length; i++) {
            ...
            uint _fee = args[i][3];
            bytes32 _extra = bytes32(args[i][4]);

            if (!exists[_extra]) {
                if (_token == address(0)) {
                    TransferHelper.safeTransferETH(_to, _amount);
                } else if (isPeggingToken[_token]) {
    ...
    function claimFee(address token) external onlyAdmin {
        require(fee[token] > 0, "bridge: no fee");

        if (token == address(0)) {
            TransferHelper.safeTransferETH(admin, fee[token]);
        } else if (isPeggingToken[token]) {
            IERC20(token).mintTo(admin, fee[token]);
        } else if (isToken[token]) {
            TransferHelper.safeTransfer(token, admin, fee[token]);
        }
