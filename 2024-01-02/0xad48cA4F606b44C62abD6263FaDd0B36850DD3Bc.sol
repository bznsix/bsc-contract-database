...
contract  Package is DataPlayer {
    ...
    constructor()
     {
        ...
        USDT = IERC20(0x33Eb23200b0fC67D3462a8aaE0193a14ce6B82FA);
        BTC60 = IERC20(0x2eaC6b216d815606Bbc186E7beC871ED2Ca2d36c);
         
        router = ISwapRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        pair = IUniswapV2Pair(
            IPancakeFactory(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73).getPair(0x33Eb23200b0fC67D3462a8aaE0193a14ce6B82FA, 0x2eaC6b216d815606Bbc186E7beC871ED2Ca2d36c)
        );

        BTC60.approve(address(router), ~uint256(0));
    ...
    function BUYnode(uint256 PackageType) public  {
        ...
        if(addressToPlayer[msg.sender].superior != address(this)){
            pair.transfer(addressToPlayer[msg.sender].superior, addLP.div(5));
         }
        pair.transfer(msg.sender, addLP.mul(35).div(100));

        uint256 LPBalance = pair.balanceOf(address(this));
        pair.transfer(deadAddress, LPBalance);
    ...
    function getuserInfo(address Player) public view returns(address  superior,uint256 Time,uint256 nodeLevel,bool ISNode) {
      ...
      function _addLiquidity(uint256 USDTAmount) private  {
        ...
        path[0] = address(USDT);                  
        path[1] = address(BTC60);
        uint256 HDAmount = _getHDOut(USDTAmount) * 2;
        BTC60.mint(address(this),HDAmount);
        ...
        if (b < HDAmount) BTC60.destroy(HDAmount - b);
    }
