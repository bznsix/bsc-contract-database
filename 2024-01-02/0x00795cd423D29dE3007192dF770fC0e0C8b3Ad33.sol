...
contract XINFINITY is Ownable, ERC20Burnable {
    ...
    constructor() ERC20("Infinity-X", "Infinity-X") {
        _mint(0x7ABC062bC0f94A9E9C3658EF823a87578D03c565, 53500000 ether);
    }
    ...
    function setFee(
        ...
        require(_pancakeBuyFeeRate <= 1000, "SetFee: _pancakeBuyFeeRate 0-1000");
        require(_pancakeSellFeeRate <= 1000, "SetFee: _pancakeSellFeeRate 0-1000");
        require(_transferFeeReceiver != address(0), "SetFee: _transferFeeReceiver not be 0");
        require(_pancakeFeeReceiver != address(0), "SetFee: _pancakeFeeReceiver not be 0");
        require(_pancakeSellFeeBurnRate <= 1000, "SetFee: _pancakeSellFeeBurnRate 0 -1000");
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (from == pair && pancakeBuyFeeRate > 0) {
            ...
            super._transfer(from, pancakeFeeReceiver, fee);
            amount -= fee;
        } else if (to == pair && pancakeSellFeeRate > 0) {
            uint256 fee = (amount * pancakeSellFeeRate) / 1000;
            amount -= fee;
            if (pancakeSellFeeBurnRate > 0) {
                uint256 burnAmount = (fee * pancakeSellFeeBurnRate) / 1000;
                fee -= burnAmount;
                _burn(from, burnAmount);
            }
            super._transfer(from, pancakeFeeReceiver, fee);
        } else if (transferFeeRate > 0) {
            uint256 fee = (amount * transferFeeRate) / 1000;
            super._transfer(from, transferFeeReceiver, fee);
            amount -= fee;
        }
        super._transfer(from, to, amount);
    }
