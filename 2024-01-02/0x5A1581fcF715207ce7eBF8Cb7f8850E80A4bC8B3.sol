...
contract LPTokenWrapper {
    ...
    constructor(IERC721 _nftCollection) public {
...
// File: contracts/BIFI/infra/ZeFiRewardPool.sol

pragma solidity ^0.5.0;

// @param _nftCollection - The address of the ERC721 Collection. -> 0xCc10aDE57c9C9DB73f2365EA2d8B6418Ed5673A0
// @param _rewardsToken - The address of the ERC20 Reward Token. -> 0x55d398326f99059fF775485246999027B3197955


contract NFTRewardPool is LPTokenWrapper, Ownable, ReentrancyGuard {
    ...
    event Withdrawn(address indexed user, uint256[] _tokenIds);
    event RewardPaid(address indexed user, uint256 reward);

    constructor(IERC721 _nftCollection, address _rewardToken)
        public
        LPTokenWrapper(_nftCollection)
    {
    ...
    function stake(uint256[] memory _tokenIds) public updateReward(msg.sender) {
        require(_tokenIds.length > 0, "Cannot stake 0");
        super.stake(_tokenIds);
        emit Staked(msg.sender, _tokenIds);
    }


    function withdraw(uint256[] memory _tokenIds) public updateReward(msg.sender) nonReentrant {
        require(_tokenIds.length > 0, "Cannot withdraw 0");
        super.withdraw(_tokenIds);
        emit Withdrawn(msg.sender, _tokenIds);
    }


    function exit() external {
        uint256[] memory _tokenIds = stakers[msg.sender].stakedTokenIds;
        withdraw(_tokenIds);
        getReward();
    }
     ...
     */
    function userStakeInfo(address _user) public view returns (uint256[] memory _stakedTokenIds) {
