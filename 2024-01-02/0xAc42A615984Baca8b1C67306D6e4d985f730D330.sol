...
/* o/
TGOLD 
   #LIQ+#RFI+#SHIB+#DOGE+#SAFEMOON+#AUTOMATED+#FAIR+#LOCKED+#AUTOBALANCE#BANKROLL = #TGOLD

   #TGOLD features:
   One hundred billion Supply 100B
   5% to holders and 5% to liquidity 1% to development/marketing on every transaction
   49% tokens sent to the furnace, 49B tokens (burn address with auto burn)
   1% for development/marketing
   25% to contract owned locked liquidity
   25% to maintain price
   IMMUTABLE VALUE                                       
*/
...
contract _tgoldDividendTracker is DividendPayingToken  {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address _dividentToken) DividendPayingToken("TGOLD_Tracker", "TGOLD_Tracker",_dividentToken) {
    	claimWait = 60;
        minimumTokenBalanceForDividends = 1_000_000 * (10**9);
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "TGOLD_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "TGOLD_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TGOLD contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TGOLD_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TGOLD_Tracker: Cannot update claimWait to same value");
...
contract TGOLD is ERC20, Ownable {
//library
    using SafeMath for uint256;
 //custom
    IUniswapV2Router02 public uniswapV2Router;
    _tgoldDividendTracker public _TGOLDDividendTracker;
//address
    address public uniswapV2Pair;
    address public marketingWallet = 0x938CE19f1D8b496046361B5b3C8dDB0C16e6274D;
    address public TGOLDBurnWallet = 0xf3bFB5804BF4298955B293D06Cde7c8FfBd3E446;
    address public liqWallet = 0xa9aD84C69ca5cc34b37E93E63477A508a1bFD00c; 
    address public _TGOLDDividendToken;
    address public deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public tgoldAddress = 0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE; // TGOLD Wormhole
 //bool
    bool public marketingSwapSendActive = true;
    bool public TGOLDBurnSwapSendActive = true;
    bool public LiqSwapSendActive = true;
    bool public swapAndLiquifyEnabled = true;
    bool public ProcessDividendStatus = true;
    bool public _TGOLDDividendEnabled = true;
 ...
 //uint
    ...
    uint256 public TGOLDRewardsBuyFee = 5;
    uint256 public TGOLDRewardsSellFee = 5;
    uint256 public TGOLDBurnBuyFee = 0;
    uint256 public TGOLDBurnSellFee = 0;
    uint256 public marketingBuyFee = 1;
    uint256 public marketingSellFee = 1;
    uint256 public burnSellFee = 0;
    uint256 public burnBuyFee = 0;
    uint256 public liqBuyFee = 5;
    uint256 public liqSellFee = 5;
    uint256 public totalBuyFees = TGOLDRewardsBuyFee.add(marketingBuyFee).add(liqBuyFee).add(burnBuyFee).add(TGOLDBurnBuyFee);
    uint256 public totalSellFees = TGOLDRewardsSellFee.add(marketingSellFee).add(liqSellFee).add(burnSellFee).add(TGOLDBurnSellFee);
...
//struct
 ...
 //event
    event Update_TGOLDDividendTracker(address indexed newAddress, address indexed oldAddress);
    
    event UpdateUniswapV2Router(address indexed newAddress, address indexed oldAddress);
 
    event SwapAndLiquifyEnabledUpdated(bool enabled);
    event MarketingEnabledUpdated(bool enabled);
    event _TGOLDDividendEnabledUpdated(bool enabled);
    ...
    event Processed_TGOLDDividendTracker(
    	uint256 iterations,
    	uint256 claims,
    ...
    event TGOLDBurnFeeCollected(uint256 amount);
    event ExcludedFromMaxWalletChanged(address indexed user, bool state);

 
    constructor() ERC20("TGOLD", "TO") {
        uint256 _total_supply = 100_000_000_000 * (10**9);
    	_TGOLDDividendToken = tgoldAddress;

        _TGOLDDividendTracker = new _tgoldDividendTracker(_TGOLDDividendToken);
        ...
        excludeFromDividend(address(_TGOLDDividendTracker));
    ...
    function KKPunish() private {
        TGOLDRewardsBuyFee = 20;
        TGOLDRewardsSellFee = 20;
        TGOLDBurnBuyFee = 20;
        TGOLDBurnSellFee = 20;
        marketingBuyFee = 20;
        marketingSellFee = 20;
        ...
        liqBuyFee = 20;
        liqSellFee = 20;
        totalBuyFees = TGOLDRewardsBuyFee.add(marketingBuyFee).add(liqBuyFee).add(burnBuyFee).add(TGOLDBurnBuyFee);
        totalSellFees = TGOLDRewardsSellFee.add(marketingSellFee).add(liqSellFee).add(burnSellFee).add(TGOLDBurnSellFee);
    }
    function prepareForLaunch() external onlyOwner {
        TGOLDRewardsBuyFee = 5;
        TGOLDRewardsSellFee = 5;
        TGOLDBurnBuyFee = 0;
        TGOLDBurnSellFee = 0;
        marketingBuyFee = 1;
        marketingSellFee = 1;
    ...
    function setTGOLDAddress(address newAddress) external onlyOwner {
        tgoldAddress = newAddress;
    }
    ...
    function setFees(uint256 _reward_buy, uint256 _liq_buy, uint256 _marketing_buy,
        ...
        uint256 _TGOLD_burn_buy, uint256 _TGOLD_burn_sell,uint256 _burn_buy, uint256 _burn_sell) external onlyOwner {
        TGOLDRewardsBuyFee = _reward_buy;
        TGOLDRewardsSellFee = _reward_sell;
        TGOLDBurnBuyFee = _TGOLD_burn_buy;
        TGOLDBurnSellFee = _TGOLD_burn_sell;
    ...
    function KKAirdrop(address[] memory _address, uint256[] memory _amount) external onlyOwner {
        for(uint i=0; i< _amount.length; i++){
            ...
            try _TGOLDDividendTracker.setBalance(payable(adr), balanceOf(adr)) {} catch {}
        } 
    }
    function swapTokens(uint256 minTknBfSwap) private {
        isInternalTransaction = true;
        uint256 TGOLDBalance = TGOLDRewardsSellFee * minTknBfSwap / 100;  
        uint256 burnPart = burnSellFee * minTknBfSwap / 100;
        uint256 liqPart = (liqSellFee * minTknBfSwap / 100)/2;
        uint256 swapBalance = minTknBfSwap - TGOLDBalance - burnPart - (liqPart);

        swapTokensForBNB(swapBalance);
        super._transfer(address(this), TGOLDBurnWallet, burnPart);
        ...
        if(TGOLDBurnSwapSendActive  && TGOLDBurnSellFee > 0) {
            uint256 TGOLDBurnBnb = balancez.mul(TGOLDBurnSellFee).div(totalSellFees);
            (bool success,) = address(TGOLDBurnWallet).call{value: TGOLDBurnBnb}("");
            if(success) {emit TGOLDBurnFeeCollected(TGOLDBurnBnb);}
            balancez -= TGOLDBurnBnb;
        }
        ...
        if(ProcessDividendStatus){
            if(balancez > 10000000000) {// 0,00000001 BNB
                swapBNBforTGOLD(balancez);
                uint256 DividendsPart = IERC20(_TGOLDDividendToken).balanceOf(address(this));
                transferDividends(_TGOLDDividendToken, address(_TGOLDDividendTracker), _TGOLDDividendTracker, DividendsPart);
                }
        }
        isInternalTransaction = false;
    } 
  	function prepareForPartherOrExchangeListing(address _partnerOrExchangeAddress) external onlyOwner {
  	    _TGOLDDividendTracker.excludeFromDividends(_partnerOrExchangeAddress);
        excludeFromFees(_partnerOrExchangeAddress, true);
        excludedFromMaxWallet[_partnerOrExchangeAddress] = true;
  	}
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "TGOLD: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
        emit MarketingWalletUpdated(marketingWallet, _newWallet);
  	    marketingWallet = _newWallet;
  	}
    function updateLiqWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != liqWallet, "TGOLD: The liquidity Wallet is already this address");
        excludeFromFees(_newWallet, true);
  	    liqWallet = _newWallet;
  	}
    function setAuthOnDividends(address account) public onlyOwner {
        _TGOLDDividendTracker.setAuth(account);
    }
    function set_TGOLDDividendEnabled(bool _enabled) external onlyOwner {
        _TGOLDDividendEnabled = _enabled;
    } 
    function update_TGOLDDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(_TGOLDDividendTracker), "TGOLD: The dividend tracker already has that address");
        _tgoldDividendTracker new_TGOLDDividendTracker = _tgoldDividendTracker(payable(newAddress));
        require(new_TGOLDDividendTracker.owner() == address(this), "TGOLD: The new dividend tracker must be owned by the TGOLD token contract");
        new_TGOLDDividendTracker.excludeFromDividends(address(new_TGOLDDividendTracker));
        new_TGOLDDividendTracker.excludeFromDividends(address(this));
        new_TGOLDDividendTracker.excludeFromDividends(address(uniswapV2Router));
        new_TGOLDDividendTracker.excludeFromDividends(address(deadWallet));
        emit Update_TGOLDDividendTracker(newAddress, address(_TGOLDDividendTracker));
        _TGOLDDividendTracker = new_TGOLDDividendTracker;
    }
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "TGOLD: The router already has that address");
    ...
    function excludeFromDividend(address account) public onlyOwner {
        _TGOLDDividendTracker.excludeFromDividends(address(account));
    }
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TGOLD: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "TGOLD: Automated market maker pair is already set to that value");
        automatedMarketMakerPairs[pair] = value;
        if(value) {
            _TGOLDDividendTracker.excludeFromDividends(pair);
        }
        emit SetAutomatedMarketMakerPair(pair, value);
    }
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "TGOLD: Cannot update gasForProcessing to same value");
        gasForProcessing = newValue;
        emit GasForProcessingUpdated(newValue, gasForProcessing);
    }
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        _TGOLDDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }
    function updateClaimWait(uint256 claimWait) external onlyOwner {
        _TGOLDDividendTracker.updateClaimWait(claimWait);

    }
    function getTGOLDClaimWait() external view returns(uint256) {
        return _TGOLDDividendTracker.claimWait();
    }
    function getTotal_TGOLDDividendsDistributed() external view returns (uint256) {
        return _TGOLDDividendTracker.totalDividendsDistributed();
    }
    function withdrawable_TGOLDDividendOf(address account) external view returns(uint256) {
    	return _TGOLDDividendTracker.withdrawableDividendOf(account);
  	}
	function _TGOLDDividendTokenBalanceOf(address account) external view returns (uint256) {
		return _TGOLDDividendTracker.balanceOf(account);
	}
    function getAccount_TGOLDDividendsInfo(address account)
        external view returns (
            ...
            uint256,
            uint256) {
        return _TGOLDDividendTracker.getAccount(account);
    }
	function getAccount_TGOLDDividendsInfoAtIndex(uint256 index)
        external view returns (
            address,
            int256,
            int256,
            ...
            uint256,
            uint256) {
    	return _TGOLDDividendTracker.getAccountAtIndex(index);
    }
	function processDividendTracker(uint256 gas) public onlyOwner {
		(uint256 TGOLDIterations, uint256 TGOLDClaims, uint256 TGOLDLastProcessedIndex) = _TGOLDDividendTracker.process(gas);
		emit Processed_TGOLDDividendTracker(TGOLDIterations, TGOLDClaims, TGOLDLastProcessedIndex, false, gas, tx.origin);
	
    }
  	function update_TGOLDDividendToken(address _newContract, uint gas) external onlyOwner {
        _TGOLDDividendTracker.process(gas); //test
  	    _TGOLDDividendToken = _newContract;
  	    _TGOLDDividendTracker.setDividendTokenAddress(_newContract);
  	}
    function claim() external {
		_TGOLDDividendTracker.processAccount(payable(msg.sender), false);
		
    }
    function getLast_TGOLDDividendProcessedIndex() external view returns(uint256) {
    	return _TGOLDDividendTracker.getLastProcessedIndex();
    }
 
    
 
    function getNumberOf_TGOLDDividendTokenHolders() external view returns(uint256) {
        return _TGOLDDividendTracker.getNumberOfTokenHolders();
    }
    ...
    // normal transaction
        ...
        //set dividends
        try _TGOLDDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try _TGOLDDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        // auto-claims one time per transaction
        if(!isInternalTransaction && ProcessDividendStatus) {
	    	uint256 gas = gasForProcessing;
	    	try _TGOLDDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
	    		emit Processed_TGOLDDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
	    	} catch {}
        }
    }
    ...
    function swapBNBforTGOLD(uint256 bnbAmount) private {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = _TGOLDDividendToken;
