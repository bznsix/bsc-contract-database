...
contract TokenSale is Initializable, ITokenSale {
    ...
    function initialize(Params calldata _params, address _stakingContract, address _admin, address _priceFeed) override external initializer {
        params = _params;
    ...
    modifier isBlacklisted() {
        require(!blacklist[msg.sender], 'adr in the blacklist');
        _;
    }
    
    modifier onlyInitiator() {
        require(msg.sender == params.initial, 'adr is not initial');
        _;
    }
    ...
    */
    function deposit() external payable isBlacklisted {
        ...
        require(epoch != Epoch.Incoming, 'Sale has not started');
        require(epoch != Epoch.Finished, 'Sale is over');
        require(epoch != Epoch.Waiting, 'Waiting for start of the round');
    ...
    function getTimeParams() external view returns(
        uint256, 
        uint256,
        uint256,
        uint256
    ...
    */
    function _processPublic(address _sender, uint256 _amount) internal {
        ...
        if(totalPublicSold == 0){
            uint256 inValue = (params.publicBuyLimit * PCT_BASE) / (uint256(exchangeRate) * ORACLE_MUL);
        ...
        if(left < want){
            uint256 refund = _amount - ((left * params.publicTokenPrice) / PCT_BASE);
	...
	* @param _amount - investment amount in BNB
    */
    function _leftWant(address _sender, uint256 _amount ) internal view returns(uint256 want, uint256 left) {
        want = (_amount * PCT_BASE) / params.publicTokenPrice;
        uint256 forUser = (publicMaxValues - publicPurchased[_sender]);
        uint256 forContract = saleTokensAmountWithoutAirdrop() - (totalPrivateSold + totalPublicSold);
    ...
    */
    function _shift(uint256 _amount) internal view returns(uint256 value){
        if(tokenDecimals != 18){
            value = tokenDecimals < 18 ? (_amount / 10** (18 - tokenDecimals)) : (_amount * 10**(tokenDecimals - 18));
        }else {
            value = _amount;
        }
    }
    ...
    */
    function addToBlackList(address[] memory _blacklist) external override onlyInitiator {
        checkingEpoch();
        require(_blacklist.length <= 500, 'Too large array');
        require(epoch == Epoch.Incoming, 'Sale has already started');
    ...
    */
    function takeLeftovers() external override {
        ...
        bool soldOut = (totalPrivateSold + totalPublicSold) > saleTokensAmountWithoutAirdrop();
        ...
        if(saleTokensAmountWithoutAirdrop() > totalTokenSold()){
            returnAmount += (saleTokensAmountWithoutAirdrop() - totalTokenSold());
        }
        if(fee > 0){
        ...
        if(earned > 0){
            uint256 returnValue = earned <= address(this).balance ? earned : address(this).balance;
    ...
    function _earned(uint256 _saleAmount, bool _soldOut) internal view returns(uint256 earned){
        if(_soldOut){
            earned = (((_saleAmount - totalPublicSold) * params.privateTokenPrice) + (totalPublicSold * params.publicTokenPrice)) / PCT_BASE;
        }else{
            earned = ((totalPrivateSold * params.privateTokenPrice) + (totalPublicSold * params.publicTokenPrice)) / PCT_BASE;
        }
    }
    /**
    ...
    */
    ...
    function takeLocked() external override {
        require(block.timestamp >= (params.publicEnd + 172800), 'It is not time yet');
    ...
    */
    function _setMaxValueByPrice(uint256[4] memory _limits) internal {
        for(uint256 i = 0; i < _limits.length; i++) {
            unchecked {
                ...
                uint256 inValue = (_limits[i] * PCT_BASE) / (uint256(exchangeRate) * ORACLE_MUL);
                uint256 amount = (inValue * PCT_BASE) / (params.privateTokenPrice);
    ...
    function _returnEscrow() internal view returns (uint256 returnAmount) {
        ...
        uint256[2][] memory sortingMilestones = _sortMilestones(params.escrowReturnMilestones);
        for(uint256 i = 0; i < sortingMilestones.length; i++) {
            uint256 mustSold = (saleTokensAmountWithoutAirdrop() * sortingMilestones[i][0]) / 100;
            if(mustSold <= totalTokenSold()) {
                if(sortingMilestones[i][1] > 0) {
                    returnAmount = (blockedAmount * sortingMilestones[i][1]) / 100;
                }else {
    ...
    function _overcomeThreshold()internal view returns(bool overcome){
        if(saleTokensAmountWithoutAirdrop() > totalPrivateSold){
            overcome = ((saleTokensAmountWithoutAirdrop() - totalPrivateSold) >= _multiply(params.thresholdPublicAmount));
        }
    }
    /**
    ...
    */
    ...
    function _sortMilestones(uint256[2][] memory arr) internal pure returns (uint256[2][] memory) {
