...
contract Lottery is StarlinkComponent, ShopItemFulfilment, PancakeSwapHelper, IDepositable {
    ...
    mapping(address => uint256) public coalPerUser;
    ...
    uint256 public xpReasonId = uint256(uint160(address(this)));
    uint256 public drawTimespan = 6 minutes;
    uint256 public currentDrawId;
    uint256 public totalFunds;
    uint256 public totalClaimed;
    uint256 public maxTicketsPerUserInADraw = 10;
    uint256 public xpEarnPerTicket = 20 * 10**9;
    uint256 public xpPerXldPrize = 10**9 / 10000;
    uint256 public minFundsBeforeBurn = 1000000000 * 10 ** 9;
    ...
    constructor(IXLD xld, IStarlinkEngine engine, IShop shop, ILeveling _leveling, IRandomNumberGenerator _randomNumberGenerator, address routerAddress) StarlinkComponent(xld, engine) ShopItemFulfilment(shop) PancakeSwapHelper(routerAddress) {
        ...
        prizeAllocations[0] = 85000;
        prizeAllocations[1] = 9000;
        prizeAllocations[2] = 900;
        prizeAllocations[3] = 91;
        prizeAllocations[4] = 9;
    ...
    function fulfill(uint256, address from, uint256 quantity, uint256 param) external override onlyShop notPaused {
        ...
        require(ticketIdsPerUserAndDraw[currentDrawId][from].length + quantity <= maxTicketsPerUserInADraw, "Lottery: Reached limit of tickets in a draw");
    ...
    function increaseCoal(address user, uint256 amount) external onlyAdmins {
        coalPerUser[user] += amount;
    }

    function decreaseCoal(address user, uint256 amount) external onlyAdmins {
        coalPerUser[user] -= amount;
    }

    function coalBalanceOf(address user) external view returns(uint256) {
        return coalPerUser[user];
    }
	...
	function calculatePrize(uint256 drawId, uint24 ticketNumbers) public view returns(uint256) {
        ...
        return doCalculatePrize(drawId, ticketNumbers);
    }
    ...
    function claim(uint256[] calldata drawIds) external notUnauthorizedContract notPaused process {
        doClaim(msg.sender, drawIds);
    }

    function claim(address user, uint256[] calldata drawIds) external onlyAdmins {
        doClaim(user, drawIds);
    }

    function endDraw() external onlyAdmins {
        Draw storage draw = draws[currentDrawId];
        require(draw.endTime <= block.timestamp, "Lottery: Draw not finished");

        draw.numbers = uint24(randomNumberGenerator.random(0, 100000, nextTicketId));
        ...
        emit DrawEnded(currentDrawId);

        startDraw();

        if (unclaimed >= minFundsBeforeBurn) {
            burn(draw.amountBurn);
        } else {
            unclaimed += draw.amountBurn;
        }
    ...
    function canEnd() external view returns(bool) {
        return draws[currentDrawId].endTime <= block.timestamp;
    }
    ...
    function setAllocations(uint16[] calldata _allocations) public onlyAdmins {
        ...
        require(allocationSum <= 100000, "Lottery: Invalid allocations");
    ...
    function doClaim(address from, uint256[] memory drawIds) private {
        uint256 totalPrize;
        uint256 lostCount;
        uint256 ticketsLength;

        for(uint256 drawIndex = 0; drawIndex < drawIds.length; drawIndex++) {
            uint256 drawId = drawIds[drawIndex];
            ...
            for(uint256 i = 0; i < ticketIds.length; i++) {
                ...
                uint256 prize = doCalculatePrize(drawId, ticket.numbers);
            ...
            ticketsLength += ticketIds.length;
        }

        coalPerUser[from]+=lostCount;
        ...
        if (xpEarnPerTicket > 0 || xpPerXldPrize > 0) {
            leveling.grantXp(from, ticketsLength * xpEarnPerTicket + totalPrize * xpPerXldPrize, xpReasonId);
        }

        emit PrizeClaimed(from, drawIds);
    }

    function doCalculatePrize(uint256 drawId, uint24 ticketNumbers) internal view returns(uint256) {
        Draw storage draw = draws[drawId];

        uint256 prize;
        for(uint8 i = 0; i < 5; i++) {
            uint256 winningBracket = hashBracket(draw.numbers, i);
            if (winningBracket == hashBracket(ticketNumbers, i)) {
                prize += draw.prizePotPerBracket[i] / entriesPerBracket[drawId][winningBracket];
            }
        }
    ...
    function setMinFundsBeforeBurn(uint256 amount) external onlyOwner {
        minFundsBeforeBurn = amount;
    }

    function setXpPerXldPrize(uint256 amount) external onlyOwner { 
        xpPerXldPrize = amount;
    }

    function setMaxTicketsPerUserInADraw(uint256 amount) external onlyOwner {
        require(amount > 0, "Lottery: Invalid value");
        maxTicketsPerUserInADraw = amount;
    }
    ...
    function onDeposit(uint256 amount) private {
        ...
        for(uint i = 0; i < 5; i++) {
            uint256 allocation = prizeAllocations[i] * amount / 100000;
