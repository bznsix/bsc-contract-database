...
/*         if(!noF[recipient] && !isMarketPair(recipient)){
            _lastBuy[recipient] = block.timestamp;
        }
        uint256 intervalLastBuy = block.timestamp - _lastBuy[_msgSender()];
        require(intervalLastBuy > _minimumBuyTime, "Wait to sell"); */
