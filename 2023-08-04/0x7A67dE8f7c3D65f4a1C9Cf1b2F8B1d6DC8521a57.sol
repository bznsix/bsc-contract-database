...
interface IStaking {
    ...
    function setDirectComissionPercents(uint16 _percent) external;
...
contract Staking is IStaking, Ownable, ERC721Holder {
    ...
    uint8 public payWithBuyToken = 1; // 1: USDT, 2: TURBO, 3: token buy NFT
    uint256 public timeOpenStaking = 1689786000; // Wed Jul 19 2023 17:00:00 GMT+0000
    uint16 public directComissionPercent = 400;
    uint256 public tokenDecimal = 18;
    uint256 public stakingPeriod = 24;
    uint private unlocked = 1;

    // for network stats
    mapping(address => uint256) totalTeamInvesment;
    ...
    mapping(uint32 => uint16) public comissionPercents;
     ...
     */
    function setDirectComissionPercents(uint16 _percent) external override onlyOwner {
        require(_percent >= 0, "STAKING: INVALID DERECT COM PERCENT.");
        require(_percent <= 10000, "STAKING: INVALID DERECT COM PERCENT.");
        directComissionPercent = _percent;
    }
    ...
    function initComissionPercents() internal {
        comissionPercents[0] = 1500;
        comissionPercents[1] = 1000;
        comissionPercents[2] = 500;
        comissionPercents[3] = 500;
        comissionPercents[4] = 500;
        comissionPercents[5] = 500;
        comissionPercents[6] = 500;
        comissionPercents[7] = 300;
        comissionPercents[8] = 200;
        comissionPercents[9] = 100;
    }
    ...
    function getCommissionPercent(uint8 _level) public view override returns (uint16) {
        return comissionPercents[_level];
    }
    ...
    function forceUpdateTotalCrewInvesment(address _user, uint256 _value) external override onlyOwner {
        totalTeamInvesment[_user] = _value;
    }
    ...
    function stakeSingleNftExecute(
        ...
        uint256 effectDecimalCurrency = 10 ** tokenDecimal;
        // Get total balance in usd staked for user
        uint256 totalAmountStakeUsdDecimal = estimateValueUsdForStakeNft(_nftId) * effectDecimalCurrency;
        // Update struct data
        ...
        // Pay direct stake commission for currenfRef
        bool paidDirectSuccess = payDirectCommision(currentRef, totalAmountStakeUsdDecimal, rewardToken);
     ...
     */
    function payDirectCommision(address _firstRef, uint256 totalAmountStakeUsdDecimal, address rewardToken) internal returns (bool) {
        address currentRef = _firstRef;
        uint256 commissionPercent = getComissionPercentInRule(currentRef);
        uint256 _commissionInUsdDecimal = (totalAmountStakeUsdDecimal * commissionPercent) / 10000;
        require(_commissionInUsdDecimal > 0, "DERECT COMMISSION: INVALID TOKEN COMMISSION");
        // canculate max commission can earn
    ...
    function getComissionPercentInRule(address _user) internal view returns (uint256) {
        ...
        if (allF1s.length >= 5) {
            ...
            if (countF1Meaning >= 5 && countF1Meaning < 10) {
                _comissionPercent = 500;
            } else {
                if (countF1Meaning >= 10) {
                    _comissionPercent = 600;
                }
            }
        }
    ...
    function claim(uint256 _nftId) public override lock() {
        ...
        if (claimableRewardInUsdWithDecimal > 0) {
            ...
            if (earnableCommissionRewardUsdDecimal > 0) {
                ...
                bool paidDirectSuccess = payCommisionMultiLevels(currentRef, earnableCommissionRewardUsdDecimal);
                require(paidDirectSuccess == true, "STAKING: FAILD IN PAY COMMISSION FOR DERECT REF");
    ...
    function payCommisionMultiLevels(
        ...
        while (currentRef != address(0) && index < 10) {
            ...
            if (totalStakeAmount) {
                ...
                uint256 _commissionInUsdWithDecimal = (_totalAmountStakeUsdWithDecimal * commisionPercent) / 10000;
                // canculate max commission can earn
                uint256 _commissionCanEarnUsdWithDecimal = getUserCommissionCanEarnUsdWithDecimal(
                    currentRef,
    ...
    function getTotalStakeAmountUSD(address _staker) public view returns (uint256) {
        return amountStaked[_staker] / (10 ** tokenDecimal);
    }
    ...
    function updateCrewInvesmentData(uint256 _refCode, uint256 _totalAmountStakeUsdWithDecimal, uint256[] memory _nftIds) internal {
        ...
        while (currentRef != nextRef && nextRef != address(0) && index <= 100) {
            ...
            currentRef = nextRef;
            uint256 currentCrewInvesmentValue = totalTeamInvesment[currentRef];
            totalTeamInvesment[currentRef] = currentCrewInvesmentValue + _totalAmountStakeUsdWithDecimal;
     ...
     */
    function setRankingAddress(address _rankingddress) public override onlyOwner {
        require(_rankingddress != address(0), "STAKING: INVALID RANKING ADDRESS");
        rankingContract = _rankingddress;
    }
    ...
    */
    function getClaimableProfitF1(address _user, uint8 floor, uint8 maxFloor, uint256 totalClaimableUsdWithDecimal) internal view returns (uint256) {
        uint16 _comissionPercent = comissionPercents[floor];
