...
________  ______   __       __  _______   ______  __        ______       
/        |/      \ /  \     /  |/       \ /      |/  |      /      |      
$$$$//$$$  |$  \   /$ |$$$$  |$$$/ $ |      $$$/       
   $ |  $ |  $ |$$  \ /$$ |$ |__$ |  $ |  $ |        $ |        
   $ |  $ |  $ |$$  /$$ |$    $<   $ |  $ |        $ |        
...
contract Tombili is ERC20, ERC20Burnable, Ownable {
    ...
    event buybackAddressUpdated(address buybackAddress);
    event buybackFeesUpdated(uint16 buyFee, uint16 sellFee, uint16 transferFee);
    event buybackFeeSent(address recipient, uint256 amount);
    ...
    constructor()
        ERC20(unicode"Tombili", unicode"TOMB") 
    {
        address supplyRecipient = 0xEcf34cB9D03B8777f919FA9A123A39f88bEBF280;
        
        updateSwapThreshold(100 * (10 ** decimals()) / 10);

        marketingAddressSetup(0x30B36677dA24A452e5937Bb1c28750B93f1ac6aE);
        marketingFeesSetup(100, 0, 0);

        charityAddressSetup(0x75E9367121B0c39996fE65fE6B4dE359bf539ECf);
        charityFeesSetup(100, 0, 0);

        buybackAddressSetup(0x5187C1d132205DfBd4ddABf28EB7d1f6B6011F40);
        buybackFeesSetup(0, 100, 0);

        lpTokensReceiverSetup(0xAE08b7CD353e1dCc6526fE46a1969fCaB179587f);
        ...
        _mint(supplyRecipient, 1000000 * (10 ** decimals()) / 10);
        _transferOwnership(0xEcf34cB9D03B8777f919FA9A123A39f88bEBF280);
    }
    ...
    function getAllPending() public view returns (uint256) {
        return 0 + _marketingPending + _charityPending + _buybackPending + _liquidityPending;
    }
    ...
    function marketingFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        ...
        totalFees[0] = 0 + marketingFees[0] + charityFees[0] + buybackFees[0] + liquidityFees[0];
        totalFees[1] = 0 + marketingFees[1] + charityFees[1] + buybackFees[1] + liquidityFees[1];
        totalFees[2] = 0 + marketingFees[2] + charityFees[2] + buybackFees[2] + liquidityFees[2];
    ...
    function buybackAddressSetup(address _newAddress) public onlyOwner {
        buybackAddress = _newAddress;

        excludeFromFees(_newAddress, true);

        emit buybackAddressUpdated(_newAddress);
    }

    function buybackFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        buybackFees = [_buyFee, _sellFee, _transferFee];
        ...
        emit buybackFeesUpdated(_buyFee, _sellFee, _transferFee);
    }
    ...
    function _transfer(
        ...
        if (!_swapping && !AMMPairs[from] && canSwap) {
            ...
            if (false || _marketingPending > 0 || _charityPending > 0 || _buybackPending > 0) {
                uint256 token2Swap = 0 + _marketingPending + _charityPending + _buybackPending;
                bool success = false;
                ...
                uint256 buybackPortion = coinsReceived * _buybackPending / token2Swap;
                if (buybackPortion > 0) {
                    (success,) = payable(address(buybackAddress)).call{value: buybackPortion}("");
                    require(success, "TaxesDefaultRouterWalletCoin: Fee transfer error");
                    emit buybackFeeSent(buybackAddress, buybackPortion);
                }
                _buybackPending = 0;
        ...
        if (!_swapping && amount > 0 && to != address(routerV2) && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            ...
            if (txType < 3) {
                ...
                _buybackPending += fees * buybackFees[txType] / totalFees[txType];
