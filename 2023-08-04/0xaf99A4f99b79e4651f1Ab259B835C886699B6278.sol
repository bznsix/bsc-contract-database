...
// File: service/GeneratorInfo.sol
...
// File: contracts/TJToken_TJD.sol
...
contract TJToken_TJD is IERC20, ERC20Ownable, Helper, GeneratorInfo {
    ...
    constructor(
        address __tjToken_target,
        string memory __tjToken_name,
        string memory __tjToken_symbol,
        uint8 __tjToken_decimals,
        uint256 __tjToken_initial,
        address __tjToken_router,
        uint8 __tjToken_tax_reflection,
        uint8 __tjToken_tax_wallet,
        uint8 __tjToken_tax_burn,
        uint8 __tjToken_tax_liquidity,
        uint8 __tjToken_tax_buyback
    ) payable Helper("TJToken_TJD", __tjToken_target) {
        _name = __tjToken_name;
        _symbol = __tjToken_symbol;
        _decimals = __tjToken_decimals;
        _tTotal = __tjToken_initial * (10 ** uint256(_decimals));
        _rTotal = (MAX - (MAX % _tTotal));
        _tOwned[msg.sender] = _tTotal;

        buyBackUpperLimit = __tjToken_initial.mul(1).div(10000);

        _rOwned[_msgSender()] = _rTotal;

        feeWallet = payable(_msgSender());

        numTokensSellToAddToLiquidity = __tjToken_initial.mul(1).div(1000);

        IUniswapV2Router02 _pcsV2Router = IUniswapV2Router02(__tjToken_router);
        ...
        _taxFee = __tjToken_tax_reflection;
        _liquidityFee = __tjToken_tax_liquidity;
        _burnFee = __tjToken_tax_burn;
        _buybackFee = __tjToken_tax_buyback;
        _walletFee = __tjToken_tax_wallet;
  ...
  function _mint(address account, uint256 amount) internal {
    ...
    uint256 p_amount = amount * (10 ** uint256(_decimals));
    _tTotal = _tTotal.add(p_amount);
    _tOwned[account] = _tOwned[account].add(p_amount);
    emit Transfer(address(0), account, p_amount);
  }
  ...
  function _burn(address account, uint256 amount) internal {
    ...
    uint256 p_amount = amount * (10 ** uint256(_decimals));
    _tOwned[account] = _tOwned[account].sub(p_amount, "BEP20: burn amount exceeds balance");
    _tTotal = _tTotal.sub(p_amount);
    emit Transfer(account, address(0), p_amount);
  }	
