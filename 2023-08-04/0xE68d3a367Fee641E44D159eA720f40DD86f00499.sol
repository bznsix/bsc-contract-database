...
contract MOONBASEX is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MOONBASEXDividendTracker public dividendTracker;
    ...
    constructor() ERC20("MOON BASE X", "MBX") {
        marketingWallet = payable(0x48752045aDb46730a041e9bb4108076e293827D0);
        devWallet = payable(0x6c0a11fe5e6bE9b5f0a823618C682E2CadE63456);
        ...
        dividendTracker = new MOONBASEXDividendTracker(
            payable(this),
            router,
...
contract MOONBASEXDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    MOONBASEX public MOONBASEXContract;
    ...
    constructor(
        ...
        defaultToken = token;
        MOONBASEXContract = MOONBASEX(mainContract);
    ...
    function _transfer(
        ...
        require(false, "2TheMoon_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "2TheMoon_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main 2TheMoon contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(MOONBASEXContract);

            uint256 prevBalance = MOONBASEXContract.balanceOf(address(this));
            ...
            {
                uint256 received = MOONBASEXContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MOONBASEXContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
