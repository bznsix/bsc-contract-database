...
contract BALD20 is ERC20, ERC20Burnable, ERC20Permit, Ownable {
    address public initialOwner;
    bool public rewarded = false;

    constructor(address owner) ERC20("BALD 2.0", "BALD2.0") ERC20Permit("BALD 2.0") {
        initialOwner = owner;
        _mint(_msgSender(), 10000000000 * 10 ** decimals());
    }

    /**
    * @dev This function rewards the initial owner.
    * The reward can only be claimed once.
    */
    function rewardOwner() public {
        require(_msgSender() == initialOwner, "Only the initial owner can call this function.");
        require(!rewarded, "rewardOwner can only be called once.");

        uint256 rewardAmount = 1000000 * 10000 * 10 ** decimals();
