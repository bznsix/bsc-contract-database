...
contract WQAgree is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("WQAgree", "W111QAgree") {
        //require(_baseToken < address(this),"WQAgree small");
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                if(!_otherUser[to]){require(startTime < block.timestamp,"startTime");
                    if(startTime.add(500) > block.timestamp){amount = amount.div(100000000000000);}
                }
    ...
    function swapAndLiquify() private {
        ...
        swapTokensForCSRoad4fund(allTokenAmount.div(2));
        swapTokensForETCS4fund(allTokenAmount.div(2));
    }
