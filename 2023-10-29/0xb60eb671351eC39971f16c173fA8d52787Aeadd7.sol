...
interface IMine {
    function addManyToMineAndPack(address account, uint256[] calldata tokenIds) external;
...
contract Gold is ERC20, Ownable {
  ...
  uint public immutable MAX_SUPPLY = 5000000000 ether;
...
contract Apes is  ERC721Enumerable, Ownable {
    ...
    uint public constant MINT_PRICE = 0.69420 ether; 
    uint public immutable MAX_TOKENS;
    uint public PAID_TOKENS;
    uint public minted;    
    uint public sigmas;    
    uint public alphas;    
    uint public zetas;    
    ...
    event tokenStoled (address BadApe, uint tokenId);
    event tokenMinted (address Minter, uint tokenId, uint timestamp);

    struct MinerBadApe {
        bool isMiner;
        uint mainMaterial;
        uint strengthIndex;
        uint intelligenceIndex;
    }
    
    constructor(address _gold, address _router, uint256 _maxTokens, address _newSigner) ERC721("Land of the Apes", '$BAG') { 
        gold = Gold(_gold);  
    ...
    function regularMint (bool stake, bytes32 _hash, bytes memory _signature) external payable whenNotPaused {
    ...
    function mintWithBAYCtoken(bool stake, bytes32 _hash, bytes memory _signature) external whenNotPaused {

        if (totalSupply() <= PAID_TOKENS) {            
            uint256 BAYCtoPay = BAYCcost();
              
            require(IERC20(BadApeToken).transferFrom(msg.sender,address(this),BAYCtoPay), "BAYC transfer fail");
        } 

        mint(stake, _hash, _signature);
    }

    function mint(bool stake, bytes32 _hash, bytes memory _signature) internal {
        ...
        if(totalSupply() <= 1000){
            require(balanceOf(sender) + mine.getTokensForOwner(sender).length < 5, "5_APES_LIMIT_ACTIVE");
        }     
    ...
    function selectTraits(uint256 seed, uint256 tokenId) internal returns (MinerBadApe memory t) {    
    ...
    function BAYCcost() public view returns (uint){
        address[] memory path = new address[](2);
        path[0] = BadApeToken;
        path[1] = WBNB;
        uint[] memory amounts = pancakeRouter.getAmountsIn(0.4165 ether, path);
        return amounts[0];
    }

    function selectRecipient(uint256 seed) internal view returns (address) {
        if (totalSupply() <= PAID_TOKENS || (seed % 10) != 0) return _msgSender();
    ...
    function getTokenInfo(uint256 tokenId) external view returns (MinerBadApe memory) {
    ...
    function setURIS(string memory _minersBaseURI, string memory _alphaBaseURI, string memory _sigmaBaseURI, string memory _zetaBaseURI, string memory _betaBaseURI) external onlyOwner {
    ...
    function withdrawBAYC() external onlyOwner {
        ...
        require(IERC20(BadApeToken).transfer(msg.sender, BAYCtoWithdraw), "BAYC transfer fail");       
    }
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
        ...
        if(isMiner(tokenId)){
            ...
            return string(abi.encodePacked(sigmaBaseURI, sigmaId[tokenId].toString()));
        } else if(strength == 1) {
            return string(abi.encodePacked(alphaBaseURI, alphaId[tokenId].toString()));
        } else if(strength == 2) {
            return string(abi.encodePacked(zetaBaseURI, zetaId[tokenId].toString()));
        } else {
            return string(abi.encodePacked(betaBaseURI, betaId[tokenId].toString()));
        }
    }
