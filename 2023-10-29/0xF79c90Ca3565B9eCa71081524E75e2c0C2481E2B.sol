// �TELEGRAM : https://t.me/littleflokifi
/*
 ⚡Let's trust it. 
   ⚡Let's shill it.
     ⚡Let's spread it.
...
contract Littleflokifi is IERC20, Context, Ownable {
    ...
    uint256 private _tTotal = 100000000000000000000000  * 10 **_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    
    uint256 private genesis_block;
    
    string private constant _symbol = "LTLFI";
    string private constant _name = "Little flokiFI";

    uint256 public Max_Buy_Limit = 2000000000000000000000 * 10**9;
    uint256 public Max_Sell_Limit = 98000000000000000000000 * 10**9;

    address private Marketing_Wallet = 0x1Df1caEa296FB4217476635c426c08417115Fc90;
    address private Team_Wallet = 0x1Df1caEa296FB4217476635c426c08417115Fc90;

    uint256 public Max_Wallet_Holding = 2000000000000000000000 * 10**9;
    uint256 private swapTokensAtAmount = 60000000000000000 * 10**9;
    ...
    Taxes public sellTaxes = Taxes(0, 10, 0, 0);
    Taxes public taxes = Taxes(0, 20, 0, 0);
    ...
    constructor (address routerAddress) {
        ...
        allowedTransfer[Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Max_Buy_Limit, "You are exceeding Max_Buy_Limit");
            require(balanceOf(to) + amount <= Max_Wallet_Holding, "You are exceeding Max_Wallet_Holding");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Max_Sell_Limit, "You are exceeding Max_Sell_Limit");
            if(to != pair){
                require(balanceOf(to) + amount <= Max_Wallet_Holding, "You are exceeding Max_Wallet_Holding");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(teamAmt > 0){
            payable(Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Update_Max_Tx_Limit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Max_Buy_Limit = maxBuy * 10**decimals();
        Max_Sell_Limit = maxSell * 10**decimals();
    }
    
    function Update_Max_Wallet_Holding(uint256 amount) external onlyOwner{
        Max_Wallet_Holding = amount * 10**decimals();
    }
