/**
👑 ElonDune 👑 Best Elon Novel Meme Coin 👑
quote to the science fiction epic “Dune.”  Meme coin with degen play 
https://t.me/ElonDune
...
⚡️Ape Shill      ⚡️Hype 
⚡️Safe token  ⚡️Bullish

$Elon Tweet  Dune 2X in 2years mean Elon love quote to the science fiction epic “$Dune.” 

$Fanaticism is always a function of repressed doubt – Dune   🔔 2022
https://twitter.com/elonmusk/status/1594773242626965505
$Fanaticism is always a function of repressed doubt – Dune 🔔 2023
...
contract ElonDune  is ContextIERC20, IERC20, OwnableIERC20 {
    ...
    string private _name = 'ElonDune';
    string private _symbol = 'ElonDune';
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        return ElonDuneReflectionsCollectorService(_rOwned[account]);
    }
    ...
    function totalIERC20Reflections() public view returns (uint256) {
        return _tFeesTaxTotal;
    }

    function ElonDuneTokens(uint256 tAmount) public {
    ...
    function ElonDuneTokensFromTrading(uint256 tAmount, bool deductTransferFeesTax) public view returns(uint256) {
    ...
    function ElonDuneReflectionsCollectorService(uint256 rAmount) public view returns(uint256) {
        require(rAmount <= _rTotalSupply, "Amount must be less than total ValidatorElonDuneTokenss");
        uint256 currentRate =  _getElonDuneRewards();
    ...
    function excludeAccount(address account) external onlyOwner() {
        ...
        if(_rOwned[account] > 0) {
            _tOwned[account] = ElonDuneReflectionsCollectorService(_rOwned[account]);
        }
    ...
    */
    ...
    function _getWagnerFeeEarnFromTrading(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 currentRate =  _getElonDuneRewards();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFeesTax) = _getElonDuneTradingFees(tAmount, tFeesTax, currentRate);
    ...
    function _getTfeesregain(uint256 tAmount) private pure returns (uint256, uint256) {
        uint256 tFeesTax = tAmount.div(100).mul(3);
        uint256 tTransferAmount = tAmount.sub(tFeesTax);
        return (tTransferAmount, tFeesTax);
    }

    function _getElonDuneTradingFees(uint256 tAmount, uint256 tFeesTax, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function _getElonDuneRewards() private view returns(uint256) {
