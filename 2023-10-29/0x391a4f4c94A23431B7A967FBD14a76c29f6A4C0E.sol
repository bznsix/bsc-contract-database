...
contract SkyRocket is Context, IBEP20, Ownable {
    ...
    string private constant _name     = "SkyRocket Plus";
    string private constant _symbol   = "SR+";
    uint8  private constant _decimals =  9;

    address public BUSD     = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); // BUSD address for Swapping   
    ...
    uint256 public _devFee       = 100; // 1% of every transaction is sent to dev Wallet 
    uint256 public _totalFee     = _taxFee.add(_liquidityFee).add(_marketingFee).add(_devFee);
    ...
    constructor (address cOwner, address marketingWallet, address devWallet) Ownable(cOwner) {
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner {
        _taxFee = taxFee;
        _totalFee     = _taxFee.add(_liquidityFee).add(_marketingFee).add(_devFee);
    ...
    function _transfer(
        ...
        */
        ...
        if (
            ...
            uint256 tFees = _totalFee - _taxFee;
            uint256 liquidtyFunds        = contractTokenBalance.mul(_liquidityFee).div(tFees);
            swapAndLiquify(liquidtyFunds);
            uint256 marketingFunds       = contractTokenBalance.mul(_marketingFee).div(tFees);
            swapTokensForBUSD(_marketingWallet,marketingFunds);
            uint256 devFunds             = contractTokenBalance.mul(_devFee).div(tFees);
            swapTokensForBUSD(_devWallet,devFunds);
        }
    ...
    function swapTokensForBUSD(address walletAddress,uint256 tokenAmount) private {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        if (recipient == _uniswapV2Pair && amount >= _doubleFeeLimit)
        {
            doubleAllFee();
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        takeTransactionFee(address(this), (tLiquidity +tMarketing + tDev), currentRate);
