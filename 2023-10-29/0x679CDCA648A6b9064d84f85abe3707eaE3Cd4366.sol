...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        address ReceiveAddress, address FundAddress, address NodeAddress, address WTBAddress
    ){
        _name = Name;
        _symbol = Symbol;
        _decimals = Decimals;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (removeLPLiquidity > 0) {
                ...
                swapFeeAmount = tAmount * (_buyLPDividendFee + _buyFundFee + _buyNodeFee + _buyWTBFee) / 10000;
            } else if (_swapPairList[recipient]) {//Sell
                isSell = true;
                swapFeeAmount = tAmount * (_sellLPDividendFee + _sellFundFee + _sellNodeFee + _sellWTBFee) / 10000;
            }
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 wtbFee = _buyWTBFee + _sellWTBFee;
        uint256 totalSwapFee = fundFee + nodeFee + wtbFee + _buyLPDividendFee + _sellLPDividendFee;
        ...
        uint256 wtbUsdt = usdtBalance * wtbFee / totalSwapFee;
        if (wtbUsdt > 0) {
            USDT.transfer(wtbAddress, wtbUsdt);
        }
    }
    ...
    function setWTBAddress(address addr) external onlyWhiteList {
        wtbAddress = addr;
        _feeWhiteList[addr] = true;
    }
    ...
    function setWTB(address w) external onlyOwner {
        _wtb = w;
    }

    function setBuyFee(
        uint256 lpDividendFee, uint256 nodeFee, uint256 fundFee, uint256 wtbFee
    ) external onlyOwner {
        _buyFundFee = fundFee;
        _buyNodeFee = nodeFee;
    ...
    function _mintLPReward(address account) private {
        ...
        if (nowTime > lastRewardTime) {
            ...
            if (rewardAmount > 0) {
                IMintToken(_wtb).mint(account, rewardAmount);
            }
            userInfo.lastRewardTime = nowTime;
        }
    }
...
contract NL is AbsToken {
    ...
    //Total，总量
        200000,
    //Receive，接收地址
        address(0xfd0Ca0c604D0714368edBD9D118D2035f85EF4F3),
    //Fund，钱包
        address(0x1D57E281B6925480E31D4fe39d3301794c3bef5E),
    //NodeFee，分红
        address(0x8cC95b35341C2BA501fD68D3A2E3Fe75b311FeCd),
    //Fee，钱包
        address(0x9D66B2D63dc28664909761e12117Ce4Db917d46d)
    ){
