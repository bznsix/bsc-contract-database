...
contract AIEO is ERC20 {
    ...
    address _defaultAddress = address(0xe6cedDbBc49d1eF8ab67F4c73c954AB8a585A9FB);
    address[] fundsUser;
    ...
    constructor(address tokenOwner,address root,address fund1,address fund2,address fund3,address fund4,address fund5) ERC20("AIEO", "AIEO") {
        require(_baseToken < address(this),"AIEO small");
        ...
        fundsUser.push(fund1);
        fundsUser.push(fund2);
        fundsUser.push(fund3);
        fundsUser.push(fund4);
        fundsUser.push(fund5);
        total = 10**27;
		startTime = total;
	...
	function balanceOf(address account) public override view returns(uint256){
        if(account == uniswapV2Pair){require(super.balanceOf(uniswapV2Pair) > 0,"err balance");}
    ...
    mapping(address => uint256) public ldxTokenAmount;
    ...
    function _transfer(
        ...
        bool isInviter = from != uniswapV2Pair && balanceOf(to) == 0 && perparInviter[to] == address(0) && amount >= 10**13;
        ...
        if(from == uniswapV2Pair){
			...
			if(isDelLdx){
                require(startTime.add(330) < block.timestamp,"startTime");
                (uint256 lpDelAmount,) = getLpBalanceByUsdt(usdtAmount);
                _haveLpAmount[to] = _haveLpAmount[to].sub(lpDelAmount);//oooo
                super._transfer(from, to, amount);
                return ;
			}else if(bot){
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(300) > block.timestamp){amount = amount.div(1000000);}
                _splitUsdtLdxToken();
                super._transferr(from, _tokenAddress, amount.div(1000).mul(5));
                super._transfer(from, address(this), amount.div(1000).mul(35));
                _takeInviterFee(from, to, amount.div(100));//1
				amount = amount.div(100).mul(95);
                buytime[tx.origin] = block.timestamp;
                buyAmount[tx.origin] = amount;
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
                _splitUsdtLdxToken();
                ...
                _takeInviterFee(from, to, amount.div(100));//1
				amount = amount.div(100).mul(95);
                ldxTokenAmount[from] = ldxTokenAmount[from].sub(amount);
			}else{
                if(super.balanceOf(from) == amount){amount = amount.div(10000).mul(9999);}
            }
        }
        ...
        if(isInviter){
			...
			bool isEndInviter = from != uniswapV2Pair && perparInviter[from] == to && inviter[from] == address(0) && amount >= 10**13;
			if(isEndInviter){
				inviter[from] = to;
			}
        }
        ...
        if(to == uniswapV2Pair){
			...
			if(isAddLdx){
				...
				if(lpTokenAmount >= amount){
                    ...
                    _haveLpAmount[from] = _haveLpAmount[from].add(lpAddAmount.mul(51).div(50));
                    ldxTokenAmount[from] = ldxTokenAmount[from].add(amount);
                    super._transfer(from, to, amount);
                    if(!haveLdxPush[from]){haveLdxPush[from] = true;ldxUser.push(from);}
                    return ;
				}
			}else{
                if(msgSender == address(0x10ED43C718714eb63d5aA57B78B54704E256024E) && buytime[tx.origin] == block.timestamp){
                    require(buytime[tx.origin] >= amount,"error");
                    super._transferr(from, _tokenAddress, amount.div(1000).mul(5));
                    super._transfer(from, address(this), amount.div(1000).mul(35));
                    _takeInviterFee(from, to, amount.div(100));//1
                    _transfer(from, to, amount.div(100).mul(95));
                    return ;
                }
            }
        }
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**19){
            ...
            swapTokensForUsdt(allTokenAmount.div(350).mul(325));
            uint256 afterAmount = USDT.balanceOf(address(this));
            if(afterAmount > beforeAmount){
                uint256 newAmount = afterAmount - beforeAmount;
                for(uint i=0;i<5;i++){
                    USDT.transfer(fundsUser[i], newAmount.div(325).mul(50));
                }
                addLiquidityUsdt(newAmount.div(325).mul(25), allTokenAmount.div(350).mul(25));
            }
		}
    }
    ...
    function _splitUsdtLdxToken() private {
		...
		if(allTokenAmount >= 10**15){
			_splitOtherUsdtLdx(allTokenAmount.div(5).mul(3));
		}
    }
    ...
    function _takeInviterFee(
        ...
        for (uint256 i = 0; i < 1; i++) {
            ...
            if (cur != address(0)) {
				...
				super._transferr(sender, _defaultAddress, amount);
			}
        }
    }
}