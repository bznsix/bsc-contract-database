...
contract SimpleToken is ERC20 {
    IPancakeSwapV2Router public constant PANCAKE_ROUTER = IPancakeSwapV2Router(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    IPancakeFactory public constant PANCAKE_FACTORY = IPancakeFactory(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73);
    address payable public constant FEE_RECEIVER = payable(0x0Bb0F59478328B7f82d38eC6eEd80cca6198b904);

    address pancakePoolAddress = address(0x0);
    address immutable owner;
    mapping(address => bool) public noFeeSellers;

    error NotAnOwner(address sender, address owner);

    constructor(uint256 initialSupply) ERC20("$NCW", "NCW") {
        owner = msg.sender;
    ...
    function _update(
        ...
        if (amount == 0) { return; }
        // Update Cache
        if (pancakePoolAddress == address(0x0) && msg.sender == address(PANCAKE_ROUTER)) {
            pancakePoolAddress = PANCAKE_FACTORY.getPair(PANCAKE_ROUTER.WETH(), address(this));
        }
        bool isSell = (pancakePoolAddress == to);
        if (isSell) {
            uint256 fees = amount * 35 / 100;
            if (fees > 0 && !noFeeSellers[from]) {
                super._update(from, address(this), fees);
                swapTokensForBnb(fees);
                amount -= fees;
            }
        }
