...
contract SCP is ERC20 {
    ...
    IERC20 public USDT;
    StarChainPlanWarp warp;
	StarChainPlanWarp public nft;
    bool private swapping;
    uint256 public swapTokensAtAmount;
	uint160 public ktNum = 173;
    uint160 public constant MAXADD = ~uint160(0);
	address private _destroyAddress = address(0x000000000000000000000000000000000000dEaD);
	address _fundAddress = address(0xD5fac46Fa3F1245A3432e5Dd596E16bc5921e233);
    ...
    constructor(address tokenOwner) ERC20("Star Chain Plan", "SCP") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), address(_baseToken));
		uint256 total = 10**25;
		nft = StarChainPlanWarp(0xe5e00952119D1b16Ad428727928b015627F64373);
	...
	function changeSwapWarp(StarChainPlanWarp _warp) public onlyOwner {
        warp = _warp;
    ...
    function _transfer(
		...
		if(startTime == 0 && balanceOf(address(this)) == 0 && to == uniswapV2Pair){
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
				if(startTime.add(15) > block.timestamp){amount = amount.div(1000000000000000);}
			}else if(to == uniswapV2Pair){}else{
                
            }
        }

        if (takeFee) {
			require(swapStats,"swapStats");
			_splitOtherToken();
			super._transfer(from, address(this), amount.div(20));
			amount = amount.div(100).mul(95);
        }
        super._transfer(from, to, amount);
		if(from == uniswapV2Pair && !havePush[to]){
			if(nft.balanceOf(to) > 0){
				havePush[to] = true;
				buyUser.push(to);
			}
		}
	...
	function swapAndLiquify() private {
		...
		uint256 lastAmount = USDT.balanceOf(address(this));
		swapTokensForOther(haveAmount);
		uint256 usdtAmount = USDT.balanceOf(address(this));
		if(usdtAmount > lastAmount){
			uint256 newAmount = usdtAmount.sub(lastAmount);
			USDT.transfer(_fundAddress,newAmount.div(5).mul(2));
		}
    }
		...
		if(buySize >20){
			for(uint256 i=0;i<20;i++){
				...
				if(nft.balanceOf(user) > 0){
					USDT.transfer(user,thisAmount.div(buySize));
				}
				startIndex += 1;
			}
		}else{
