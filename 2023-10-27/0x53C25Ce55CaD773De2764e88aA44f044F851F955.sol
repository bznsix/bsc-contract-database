...
// XXHUBToken.
contract XXHUB is BEP20, DexRole {
    ...
    uint256 public liquiditySellFee = 20;
    uint256 public projectSellFee = 60;
    uint256 public burnSellFee = 5;
    address public projectFeeReceiver;
    uint256 public tBuyBack = 10; // buyback rate, calculated on the project rate
    
    address public tokenBuyBack = address(0x552E74B9B28A5b6c8aE476D8E91198f073Cf01fF); // token buyback
    uint256 public sellTaxRate = liquiditySellFee.add(projectSellFee).add(burnSellFee).add(tBuyBack);
    ...
    function setBuyBackRate(uint256 newRate) public onlyOwner {
        tBuyBack = newRate;
    }

    function setBuyBackToken(address newToken) public onlyOwner {
        tokenBuyBack = address(newToken);
    }
        ...
        if (tradingOpen || sender == owner()) {
            if (
                ...
                if (transferTaxRate == 0 || _excludedFromFees[sender] || _excludedFromFees[recipient]) {
                    ...
                    require(amount == taxAmount + sendAmount, "XXHUB:: transfer: Tax value invalid");
        ...
        if (contractTokenBalance >= minAmountToLiquify) {
            if (sellTaxRate > 0) {
                if (projectSellFee > 0) { 
                    ...
                    uint256 buyBackTokens = minAmountToLiquify.mul(tBuyBack).div(sellTaxRate);
                    payproject(projectTokens,buyBackTokens);
                }
                if (burnSellFee > 0) {
            ...
            { 
                    payproject(contractTokenBalance,0);               
                }
        }
    }

    function payproject(uint256 tokens, uint256 BuyBackTokens)  private {
        if(BuyBackTokens > 0){
            _buyBackAndBurn(BuyBackTokens); // executar a função de recompra
            }
...
// buybacks the governance token
function _buyBackAndBurn(uint256 amount) private {
        // Generate WETH token path pair for desired token
               address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = hashSwapRouter.WETH();
        path[2] = address(tokenBuyBack);
        _approve(address(this), address(hashSwapRouter), amount);
        // Convert entire contract balance (in native currency, i.e., BNB on Binance Smart Chain) to desired tokens
        // The 'amount' would be the amount in BNB you want to use for the buyback.
///        hashSwapRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{ value: amount }(
            hashSwapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                amount,
                0, // Accepts any number of tokens; this can be dangerous due to market volatility. Set a minimum as needed.
                path,
                projectFeeReceiver, // The address that will receive the tokens
                block.timestamp.add(300) // Deadline (here, for example, 6 minutes)
        );
    ...
    function updateMinAmountToLiquify(uint256 _minAmount) public onlyOwner {
        ...
        minAmountToLiquify = _minAmount * 10 ** 18;
    }

    function setExcludeFromFees(address _account, bool _excluded) public onlyOwner {
        require(_excludedFromFees[_account] != _excluded, "XXHUB: Account is already the value of 'excluded'");
    ...
    function setTxFees(uint256 _transactionFee) external onlyOwner{
        ...
        require(transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "XXHUB: Total fees can not be bigger than max value");
    }
    ...
    function setSellFees(uint256 _liquiditySellFee,  uint256 _projectSellFee, uint256 _burnSellFee) external onlyOwner{
        ...
        require(sellTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "XXHUB: Total fees can not be bigger than max value");
    }
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "XXHUB::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "XXHUB::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "XXHUB::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "XXHUB::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying XXHUBs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "XXHUB::_writeCheckpoint: block number exceeds 32 bits");
