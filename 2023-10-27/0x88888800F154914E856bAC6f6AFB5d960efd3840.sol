...
contract AGToken is Context, IERC20, Ownable {
    ...
    uint256 public startTime = 1664186400;
    ...
    address private  oldToNewAddr; 
    address private  _devWallet; 

    string private _name = "AG";
    ...
    function setStarrtTime(uint256 _startTime) public onlyOwner {
    ...
    function setOldToNewAddress(address  oldAddr) public onlyOwner() {
        oldToNewAddr = oldAddr;
    }    
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
	    ...
	    uint256 tT = tFee.add(tLiquidity);
        uint256 tTransferAmount = tAmount.sub(tT);

        return (tTransferAmount, TData(0, tFee, tLiquidity, 0));
    }
    ...
    function _getRValues( TData memory _data) private pure returns (uint256, uint256, uint256) {
		...
		uint256 rT = rFee.add(rLiquidity);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        swapTokensForTokens(contractTokenBalance,_devWallet);
    }

    function swapTokensForTokens(uint256 tokenAmount,address _userAddr) private {
        ...
        if(!takeFee)
          ...
          UserOLD storage user = userTokenOLD[sender];
            ...
            if ( block.timestamp < startTime) {
                ...
                countDay = (block.timestamp - startTime) / 604800; //1 week
             }
            if  (user.ISOLD && countDay <= 24 ){
    ...
    function setUser(address _user,uint256 _amount) public {
        require(msg.sender == oldToNewAddr, "ERC20: Address already attended"); 
         UserOLD storage user = userTokenOLD[_user];
         user.ISOLD= true;
         user.tokenAmount = _amount;
         user.freedAmount =  user.tokenAmount / 24;
    }
