...
contract AquawaUSDT is ReentrancyGuard {
    ...
    uint256 public minimumStake = 10 * 10**18; 
    uint256 public maximumStake = 10000 * 10**18;
    uint256 public dailyInterestRate = 2;
    uint256 public referralLevel1Percentage = 20;
    uint256 public referralLevel2Percentage = 10;
    uint256 public referralLevel3Percentage = 5;
    ...
    function stake(uint256 amount, address referrer) external nonReentrant {
        ...
        if(!isOldUser[msg.sender]){
            stakedAmounts[msg.sender].push(Stake(amount, block.timestamp,Status.PENDING,block.timestamp+uint256(180 days)));
        }
        else{
            stakedAmounts[msg.sender].push(Stake(amount, block.timestamp,Status.PENDING,block.timestamp+uint256(90 days)));
        }
    ...
    function updateMinimumAndMaximumStake(uint256 _minimumStake,uint256 _maximumStake) external onlyOwner nonReentrant{
