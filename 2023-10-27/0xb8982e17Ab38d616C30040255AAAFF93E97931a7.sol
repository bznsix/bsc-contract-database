...
interface IERC20 {
  ...
  event Approval(address indexed owner, address indexed spender, uint256 value);
    ...
    event RegisterActivation(address user, address receiver, uint256 types);
    event UnfreezeActivation(address user, address receiver, uint256 types);
   ...
   function checkteamconditions(address _user) private view returns(uint256, uint256, uint256 ) {
    ...
    function _deposit(address _user, uint256 _amount) private {
        ...
        require(user.maxDeposit == 0 || _amount >= user.maxDeposit, "less before");
         ...
         if( userInfo[_user].directnum>=5 &&  userInfo[_user].directnum<10)
            {
                if(_amount>100e18){
                   _amount = 1000e18;
                }
           ...
           if( userInfo[_user].directnum>=10)
            {
                if(_amount>2000e18){
                   _amount = 2000e18;
                }
         ...
         if(interest > 0 && user.isactive) 
          {
           ...
           uint256 addFreeze = (orderInfos[msg.sender].length.div(2)).mul(timeStep);
           if(addFreeze > maxAddFreeze){
             addFreeze = maxAddFreeze;
           }
 ...
 function _releaseReward(address _user, uint256 _amount, uint256 _initialamt) private {
    ...
    function registerActivation(address _user, uint256 _types) external {
        ...
        emit RegisterActivation(msg.sender, _user, _amount);
    }
  ...
  function unfreezeActivation(address _receiver, uint256 _types) external {
        ...
        emit UnfreezeActivation(msg.sender, _receiver, _types);
    }
