...
contract DRAGONINU is ERC20, Ownable {
    ...
    address public markettmWallet = 0x196FdEa1a94Ee162De6a9c12cBE99B9C2104F89a;
    ...
    function _transfer(
       ...
       if (!exemptFee[sender] && !exemptFee[recipient]) {
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipient])
            ...
            feeswap = sellTaxes.liquidity + sellTaxes.markettm;
            feesum = feeswap;
            currentTaxes = sellTaxes;
        } else if (!useLaunchFee) {
            feeswap = taxes.liquidity + taxes.markettm;
            feesum = feeswap;
            currentTaxes = taxes;
        } else if (useLaunchFee) {
            feeswap = launchtax;
    ...
    function Liquify(uint256 feeswap, Taxes memory swapTaxes) private lockTheSwap {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 markettmAmt = unitBalance * 2 * swapTaxes.markettm;
            if (markettmAmt > 0) {
                payable(markettmWallet).sendValue(markettmAmt);
            }
        }
    }
    ...
    function updatemarkettmWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0), "Fee Address cannot be zero address");
        markettmWallet = newWallet;
    }
