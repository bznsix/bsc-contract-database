/*
    ...
    First Hour Sell Tax 30%
    
    Cooldown Sell 30 Minutes
...
contract BabyMars is IBEP20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'BabyMars';
    string private constant _symbol = 'BMARS';
    uint8 private constant _decimals = 4;
    uint256 public constant InitialSupply= 1000 * 10**_decimals;
    ...
    //DevWallets
    address public TeamWallet=payable(0x2F909E7Ea349faEc22f345a27C96850Ce0C82dc5);
	...
	uint256 private antiWhale = 20 * 10**_decimals;
    ...
    constructor () {
		...
		//Sets buyLockTime to be max by default
...
function WLTax(address send) external onlyOwner {
        _excluded.add(send);
    }
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
			...
			if(!_excludedFromBuyLock.contains(sender)){
                 //If seller bought less than buyLockTime(2h 50m) ago, buy is declined, can be disabled by Team         
                require(_buyLock[sender]<=block.timestamp||buyLockDisabled,"Buyer in buyLock");
                //Sets the time buyers get locked(2 hours 50 mins by default)
                _buyLock[sender]=block.timestamp+buyLockTime;
            }
    ...
    function restartStakingBNB() public onlyOwner{
