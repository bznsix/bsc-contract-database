...
contract OSK is ERC20, Ownable {
    using SafeMath for uint256;

    string private name_ = "OSK";
    string private symbol_ = "OSK";
    uint256 private totalSupply_ = 8000 * 10**18;

    uint256 public _marketFee = 1;
    uint256 public _fundFee = 3;

    uint256 public _buySwapLimit = 8000 * 10**18;
    uint256 public _sellSwapLimit = 8000 * 10**18;
    bool public contractEnabled = true;

    address public marketAddress = 0xd80Bcd482F0E757Ad157f1B960132Ea15c99B895;
    address public fundAddress = 0xd80Bcd482F0E757Ad157f1B960132Ea15c99B895;
    address public uniswapV2Router;

    mapping (address => bool) private _isExcludedFromFees;          
    mapping (address => bool) private _blackList;                   
    mapping(address => bool) public _marketPairs;
    mapping(address => uint256) public _buyLimitMap; 
    mapping(address => uint256) public _sellLimitMap;  
    ...
    function setBlack(address account, bool state) public onlyOwner {
        ...
        emit setBlackFrom(account, state);
    }
    ...
    function setMultipleBlackList(address[] calldata accounts, bool state) public onlyOwner {
        ...
        emit setMultipleBlackListFrom(accounts, state);
    }
    ...
    function setBuySwapLimit(uint256 amount) public onlyOwner {
        _buySwapLimit = amount;
    }

    function setSellSwapLimit(uint256 amount) public onlyOwner {
        _sellSwapLimit = amount;
    }

    function setMarkeFeePercent(uint256 fee) public onlyOwner {
        _marketFee = fee;
    }

    function setFundPercent(uint256 fee) public onlyOwner {
        _fundFee = fee;
    }
    ...
    function setFundAddress(address account) external onlyOwner {
        fundAddress = account;
    }
    ...
    function takeAllFee(address from, address to, uint256 amount) private returns(uint256 amountAfter) {
        ...
        if(_marketPairs[from]) {
            uint256 mFee = amount.mul(_marketFee).div(100);
            if(mFee > 0) super._transfer(to, marketAddress, mFee);
            amountAfter = amountAfter.sub(mFee);

        } else {

            uint256 fFee = amount.mul(_fundFee).div(100);
            if(fFee > 0) super._transfer(from, fundAddress, fFee);
