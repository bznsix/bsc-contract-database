...
contract MultiPoolsStore is Ownable, IMultiPoolsStore {
    ...
    modifier check(uint16 poolId, uint256 strategyIndex) {
        if (router == address(0)) revert HasNoRouter();
        if (pools[poolId].chainId == 0) revert PoolDisabled();
        if (!pools[poolId].strategies[strategyIndex].enabled)
            revert PoolDisabled();
        _;
    }
    ...
    function setPool(uint16 poolId, uint256 chainId, Strategy[] calldata strategies) external onlyOwner {
        pools[poolId].chainId = chainId;
        uint256 i = 0;
        if (pools[poolId].strategies.length != 0) {
            for (i = 0; i < pools[poolId].strategies.length; i++) {
                pools[poolId].strategies.pop();
            }
        }
        for (i = 0; i < strategies.length; i++) {
            pools[poolId].strategies.push(strategies[i]);
        }
    }
    ...
    function checkPoolShares(uint16 poolId) external view override returns(bool) {
        uint16 result = 0;
        for (uint16 i = 0; i < pools[poolId].strategies.length; i++) {
            result += pools[poolId].strategies[i].poolShare;
        }
        if (result != 10000) revert BadPoolShares(result);
        return true;
    }

    function getStrategies(uint16 poolId) external view override returns (Strategy[] memory) {
        return pools[poolId].strategies;
    }

    function isCrosschain(uint16 poolId) external view override returns (bool) {
        if (pools[poolId].chainId == 0) revert PoolDisabled();
    ...
    function getStrategyAddress(
        ...
        if (pools[poolId].strategies[index].addr == address(0))
            revert PoolDisabled();

        return pools[poolId].strategies[index].addr;
    }
    ...
    function getBalance(
        ...
        return
            IBaseStrategy(pools[poolId].strategies[index].addr).allocationOf(
                poolId
            );
    }
    ...
    function getStrategyShare(
        ...
        return pools[poolId].strategies[index].poolShare;
    }

    function countStrategies(
        uint16 poolId
    ) external view override returns (uint256) {
        return pools[poolId].strategies.length;
    }
    ...
    function isAbleForWithdraw(
        ...
        return pools[poolId].user[user].allocation >= shares && pools[poolId].user[user].delayToBlock <= block.number;
    }
    ...
    function getWithdrawalFees(
        ...
        return pools[poolId].strategies[index].withdrawalFee;
    }
}
// SPDX-License-Identifier: GPL-3.0-or-later
pragma solidity 0.8.19;
