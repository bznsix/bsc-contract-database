...
contract TokenDeposit is ReentrancyGuard {
    ...
    // Minimum and maximum amount of CERT tokens that can be purchased
    uint256 public minPurchaseAmount = 1;
    uint public maxPurchaseAmount = 1000;
    ...
    // Allows the admin to update the maximum purchase amount
    function updateMaxPurchaseAmount(
        uint256 _newMaxPurchaseAmount
    ) external onlyAdmin {
    ...
    function buyCERT(
        ...
        require(
            certAmount <= maxPurchaseAmount,
            "Purchase amount exceeding maximum limit"
        );
