...
contract SinglePoolsStore is Ownable, ISinglePoolsStore {
    /// @dev SafeERC20 library from OpenZeppelin.
    using SafeERC20 for IERC20;

    /// @notice 20% withdrawal fee limitation.
    uint16 public constant FEE_MAX = 2000;
    ...
    modifier check(uint16 poolId) {
        if (router == address(0)) revert HasNoRouter();
        if (pools[poolId].chainId == 0) revert PoolDisabled();
        if (pools[poolId].strategy == address(0)) revert PoolDisabled();
        _;
    }

    function setRouter(
        address newRouter
    ) external onlyOwner checkAddress(newRouter) {
    ...
    function addPool(
        ...
        if (withdrawalFee > FEE_MAX) revert AmountTooBig();
        pools[poolId].strategy = strategy;
        pools[poolId].withdrawalFee = withdrawalFee;
    ...
    function increaseAllocation(
        ...
        pools[poolId].user[user].allocation += amount;
        pools[poolId].totalAllocation += amount;
        // Set withdraw delay
        pools[poolId].user[user].delayToBlock = uint96(
            block.number + withdrawDelay
        );
    }
    ...
    function isCrosschain(
        uint16 poolId
    ) external view override check(poolId) returns (bool) {
        return pools[poolId].chainId != block.chainid;
    }

    function getStrategyAddress(
        uint16 poolId
    ) external view override check(poolId) returns (address) {
        return pools[poolId].strategy;
    }

    function getTotalAllocation(
        uint16 poolId
    ) external view override check(poolId) returns (uint256) {
    ...
    function getBalance(
        ...
        return IBaseStrategy(pools[poolId].strategy).allocationOf(poolId);
    }

    function getChainId(
        uint16 poolId
    ) external view override check(poolId) returns (uint256) {
        return pools[poolId].chainId;
    }
    ...
    function isAbleForWithdraw(
        ...
        return pools[poolId].user[user].delayToBlock <= block.number;
    }
    ...
    function isEnoughtShares(
        ...
        return pools[poolId].user[user].allocation >= shares;
    }

    function getWithdrawalFees(
        uint16 poolId
    ) external view override check(poolId) returns (uint256) {
        return pools[poolId].withdrawalFee;
    }
}
// SPDX-License-Identifier: GPL-3.0-or-later
pragma solidity 0.8.19;
