...
contract XXHUB is BEP20, DexRole {
    ...
    IBEP20  public tokenBuyBack = IBEP20(0x552E74B9B28A5b6c8aE476D8E91198f073Cf01fF) ; // token buyback
    ...
    function payproject(uint256 tokens) private {
        uint256 buyBackAmount = tokens * tBuyBack / 100; // Cálculo da quantidade a ser usada para recompra
        tokens = buyBackAmount - tokens;
        _buyBackAndBurn(buyBackAmount); // executar a função de recompra
...
function _buyBackAndBurn(uint256 amount) private {
        ...
        address[] memory path = new address[](2);
        path[0] = hashSwapRouter.WETH();
        path[1] = address(tokenBuyBack);
        ...
        hashSwapRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{ value: amount }(
            ...
            block.timestamp + 360 // Deadline (here, for example, 6 minutes)
        );
