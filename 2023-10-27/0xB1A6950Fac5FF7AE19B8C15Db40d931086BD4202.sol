...
contract LiquidityVault is Ownable {
    ...
    /// @notice Accepted Slippage value when adding liquidity
    uint256 public slippage = 95;
    /// @notice Vesting contract address that vests LPs
    Releaser public vestingContract;

    /// @dev Uniswap V2 Factory and Router addresses
    ...
    /// @dev WETH and USDT addresses
    address private constant WETH = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address private constant USDT = 0x55d398326f99059fF775485246999027B3197955;

    /**
     * @dev Throws if called more than once.
     */
    modifier onlyOnce() {
        require(
            address(vestingContract) == address(0),
            "LiquidityVault: Liquidity already provided."
        );
        _;
    }

    /**
     * @dev Function to update slippage value
     * @param _slippage slippage in percentage
     */
    ...
    /**
     * @notice The function to add liquidity for the given tokens
     * and vest the LP tokens received according to schedule.
     * @dev Vesting is always Linear.
     *
     * @param _to The address to receive the LP tokens
     * @param _tokenA First token to provide liquidity
     * @param _tokenB Second token to provide liquidity
     * @param _amountA Amount of first token to provide liquidity
     * @param _amountB Amount of second token to provide liquidity
     * @param vestingStart The start of the vesting schedule
     * @param durationInSec The duration of vesting.
     */
    function createVestedLiquidity(
        address _to,
        address _tokenA,
        address _tokenB,
        uint256 _amountA,
        ...
        vestingContract = new Releaser(_to, pair, vestingStart, durationInSec, 1);
        vestingContract.transferOwnership(owner());

        _addLiquidity(_tokenA, _tokenB, _amountA, _amountB, address(vestingContract));
    }

    /**
     * @notice The function to claim vested LP tokens.
     */
    function claimLP() external {
        require(
            address(vestingContract) != address(0),
            "LiquidityVault: Liquidity not provided yet."
        );
        vestingContract.release();
    }

    /**
     * @notice Function to get the minimum amount from a swap
     *
     * @param _tokenIn The token to swap
     * @param _tokenOut The token to receive
     * @param _amountIn The amount of token to swap
     *
     * @return The minimum amount of token to receive
     */
    function getAmountOutMin(
        ...
        uint256[] memory amountOutMins = IUniswapV2Router01(ROUTER).getAmountsOut(_amountIn, path);
        return amountOutMins[path.length - 1];
    }

    /**
     * @notice Function to swap tokens
     * @dev This function should be called before adding liquidity.
     * The process is divided to increase safety.
     *
     * @param _tokenIn Token to give
     * @param _tokenOut Token to receive
     * @param _amountIn Amount of token to give
    ...
    /**
     * @dev The internal function to execute a liquidity provision.
     * This function is called by createVestedLiquidity.
     * Returns any leftover tokens to the caller.
     *
     * @param _tokenA First token to provide liquidity
     * @param _tokenB Second token to provide liquidity
     * @param _amountA Amount of first token to provide liquidity
     * @param _amountB Amount of second token to provide liquidity
     * @param _to The address to receive LP tokens.
     */
    function _addLiquidity(
        ...
        uint256 slippage1 = (_amountA * slippage) / 100;
        uint256 slippage2 = (_amountB * slippage) / 100;

        (uint amountA, uint amountB, uint liquidity) = IUniswapV2Router01(ROUTER).addLiquidity(
            _tokenA,
            _tokenB,
            _amountA,
            _amountB,
        ...
        //refund the sender any leftover tokens
        if (_amountA - amountA > 0) {
            IERC20(_tokenA).safeTransfer(msg.sender, _amountA - amountA);
        }
        if (_amountB - amountB > 0) {
...
contract Vesting is IVesting, Pausable, AccessControl, ReentrancyGuard {
    ...
    constructor(address _token, uint256 _cliff, uint256 _durationInSec, uint256 _periodInSeconds) {
    ...
    function getRefundees(uint _start, uint _end) external view returns (address[] memory) {
        ...
        require(_start < _refundees.length(), "Vesting: start is greater than refundees length");
    ...
    function refundUser(address _account) public onlyRole(DEFAULT_ADMIN_ROLE) {
        if (claimedOf(_account) > 0) {
            ...
            _removeShares(_account);
            releaser.emergencyWithdraw(msg.sender, notClaimed);
