/**
  ...
  X-Account : https://x.com/eXtremeBeansX2
  1% (Max) tax
  1% Liquidity on buy
  1% Burn on sales
...
contract eXtremeBeansBurn  is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    eXtremeBeansBurnDividendTracker public dividendTracker;

    address payable private marketingWallet;
    address payable private FREEWallet;
    ...
    uint256 private sellMarketingFees;
    uint256 private sellLiquidityFee;
    uint256 private buyDeadFees;
    uint256 private buyMarketingFees;
    uint256 public buyLiquidityFee;
    uint256 private buyRewardsFee;
    uint256 private buyFREEFee;
    uint256 private sellFREEFee;
    ...
    uint256 private cooldowntimer = 5; //default cooldown 5s
    ...
    constructor() ERC20("eXtreme Beans Burn", "eXtremeBurn") {
        marketingWallet = payable(0x123Bd506C379494bBC958ae2bDc245F91b1CE87b);
        FREEWallet = payable(0x30a091Cd9c32cF62a625574B71822837CE465DE4);
        ...
        dividendTracker = new eXtremeBeansBurnDividendTracker(
            payable(this),
            router,
            0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c,
            "eXtremeBeansBurnTRACKER",
            "eXtremeBeansBurnTRACKER"
        );
        ...
        _isExcludedFromFees[address(FREEWallet)] = true;
        _isExcludedFromFees[msg.sender] = true;

        uint256 totalTokenSupply = (750_000) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 1000; //
        swapTokensAtAmount = totalTokenSupply / 1000; //
    ...
    // set cooldown timer, can only be between 0 and 600 seconds (10 min max)
    function setcooldowntimer(uint256 value) external onlyOwner {
        require(value <= 600, "cooldown timer cannot exceed 5 minutes");
        cooldowntimer = value;
    }

    // set max wallet, can not be lower 
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 5000, "max wallet cannot be set lower");
        maxWallet = value;
    }
    ...
    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 80, "transfer fee cannot exceed 80%");
    ...
    function updateFees(
        ...
        require(totalSellFees <= 1 && totalBuyFees <= 1, "total fees cannot exceed 1% sell or 1% buy");
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            uint256 totalFees = RewardsFee
                .add(liquidityFee + marketingFees + ListFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buyMarketingFees + buyFREEFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellMarketingFees + sellFREEFee);
        }
        ...
        uint256 _completeFees = sellMarketingFees.add(sellFREEFee) +
            buyMarketingFees.add(buyFREEFee);
        ...
        uint256 FREEPayout = buyFREEFee.add(sellFREEFee) * feePortions;

        if (marketingPayout > 0) {
            (successOp1, ) = address(marketingWallet).call{value: marketingPayout}("");
        }
        if (FREEPayout > 0) {
            (successOp2, ) = address(FREEWallet).call{value: FREEPayout}("");
        }

        emit SendDividends(
            dividends,
            marketingPayout + FREEPayout,
...
contract eXtremeBeansBurnDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    bool public dividendsPaused = true;
    ...
    eXtremeBeansBurn public eXtremeBeansBurnContract;
    ...
    constructor(
        ...
        defaultToken = token;
        eXtremeBeansBurnContract = eXtremeBeansBurn(mainContract);
        minimumTokenBalanceForAutoDividends = 750000_000000000000000000; // 750,000 tokens
    ...
    function _transfer(
        ...
        require(false, "eXtremeBeansBurn_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "eXtremeBeansBurn_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(eXtremeBeansBurnContract);

            uint256 prevBalance = eXtremeBeansBurnContract.balanceOf(address(this));
            ...
            {
                uint256 received = eXtremeBeansBurnContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    eXtremeBeansBurnContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
