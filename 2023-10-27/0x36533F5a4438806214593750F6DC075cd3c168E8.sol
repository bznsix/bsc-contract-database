...
contract TheBullionGamePlayForGood is
    VRFConsumerBaseV2,
    ReentrancyGuard,
    Ownable,
    ...
    AccessControl
{
    ...
    PaidWinners private paidWinners;
    ...
    event RandomRequested(uint256 requestRandomId);
    ...
    event Deposited(address indexed depositor, uint256 totalDeposited);
    ...
    function updateEntryFee(
     ...
     function updateInterval(uint _newInterval) external onlyOwner {
        interval = _newInterval;
        emit IntervalUpdated(_newInterval); // Emitting an event to log the change
    }
    ...
    function getCharity1Address() public view returns (address payable) {
        return charity1;
    }

    function getCharity2Address() public view returns (address payable) {
        return charity2;
    }

    function updateCharity1(address payable _newCharity1) public onlyOwner {
        charity1 = _newCharity1;
    }

    function updateCharity2(address payable _newCharity2) public onlyOwner {
        charity2 = _newCharity2;
    }

    function startGame() public {
        require(game_state == GAME_STATE.CLOSED, "Game is not closed");
    ...
    function fulfillRandomWords(
        ...
        game_state = GAME_STATE.PAYING_CHARITIES;

        distributeFunds(_randomWords);
    }

    function distributeFunds(uint256[] memory _randomWords) private {
        require(
            game_state == GAME_STATE.PAYING_CHARITIES,
            "Game state not PAYING_CHARITY"
        );

        uint256 totalBalance = address(this).balance;

        uint256 charityAmount = (totalBalance * 70) / 100;

        uint256 numCharities = (charity1 != address(0) ? 1 : 0) +
            (charity2 != address(0) ? 1 : 0);

        uint256 charityShare = charityAmount / numCharities;

        game_state = GAME_STATE.CHARITIES_PAID;
        if (numCharities == 1) {
            require(charity1 != address(0), "Charity address required");
            charity1.transfer(charityShare);
        } else if (numCharities == 2) {
            require(
                charity1 != address(0) && charity2 != address(0),
                "Both charity addresses required"
            );
            charity1.transfer(charityShare);
            charity2.transfer(charityShare);
        }

        declareWinners(_randomWords);
    }

    function isWinnerPaid(address _address) private view returns (bool) {
        for (uint256 i = 0; i < paidWinners.addresses.length; i++) {
            if (paidWinners.addresses[i] == _address) {
                return true;
            }
        }
        return false;
    ...
    function getAllPaidWinners() external view returns (address[] memory) {
        return paidWinners.addresses;
    }

    function declareWinners(uint256[] memory _randomWords) private {
        require(
            game_state == GAME_STATE.CHARITIES_PAID,
            "Game must be in CHARITIES_PAID state"
        );

        game_state = GAME_STATE.PAYING_WINNERS;

        uint256 winnersShare = address(this).balance;
        uint256 numWinners = players.length;

        require(numWinners > 0, "No winners to select");

        if (numWinners <= 10) {
            uint256 winnersSharePerPlayer = winnersShare / numWinners;
            for (uint256 j = 0; j < numWinners; j++) {
                address payable winner = payable(players[j]);
                require(
                    winner != address(0) && winner != address(this),
                    "Invalid winner address"
                );
                winner.sendValue(winnersSharePerPlayer);
                emit WinnerDeclared(players[j], winnersSharePerPlayer);
                paidWinners.addresses.push(players[j]);
            }
        } else {
            uint256 iterations = 10;
            require(
                _randomWords.length >= iterations,
                "Not enough random words"
            );

            uint256[] memory winnerIndices = new uint256[](iterations);

            for (uint256 j = 0; j < iterations; j++) {
                uint256 randomIndex = _randomWords[j] % numWinners;
                while (isWinnerPaid(players[randomIndex])) {
                    randomIndex = (randomIndex + 1) % numWinners;
                }

                winnerIndices[j] = randomIndex;
                paidWinners.addresses.push(players[randomIndex]);
            }

            uint256 winnersSharePerPlayer = winnersShare / iterations;
            for (uint256 j = 0; j < iterations; j++) {
                address payable winner = payable(players[winnerIndices[j]]);
                require(
                    winner != address(0) && winner != address(this),
                    "Invalid winner address"
                );
                winner.sendValue(winnersSharePerPlayer);
                emit WinnerDeclared(
                    players[winnerIndices[j]],
                    winnersSharePerPlayer
                );
            }
        }
