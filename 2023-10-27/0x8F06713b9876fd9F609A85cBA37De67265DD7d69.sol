...
interface IFomo {
	...
	function transferbuymax(address top1user,address top2user,address top3user) external returns (bool);
...
contract CatKing is Context, IERC20, Ownable {
	...
	mapping (uint256 => mapping (address => uint256)) private buynum;
	uint256[] private datelist;
	address[3] public buytop;
    ...
    uint256 private _tTotal = 100000000 * 10**8 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    string private _name = "Cat King Coin"; 
    string private _symbol = "CatKing"; 
    ...
    uint256[3] public feeFomo = [1, 2, 3];
    uint256[3] public feeLiquidity = [2, 4, 6];
    uint256[3] public feeFee = [2, 4, 6];
    ...
    function setenterCount(uint256 _Count) public onlyOwner {
        enterCount = _Count;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tFomo, uint256 tLiquidity, uint256 tFee) {
        if (!feeIt) {
        ...
        tTransferAmount = tAmount.sub(tFomo).sub(tLiquidity).sub(tFee);
    }
    ...
    function _takeTax(uint256 tFomo, uint256 tLiquidity) private {
    ...
    function getHealthLevel() private view returns(uint256) {
        ...
        uint256 bal = wbnb.balanceOf(uniswapV2Pair);
        if (bal <= 300 * 10**18) {
            return 2;
    ...
    function _transfer(
        ...
        if (enterCount == 1) {
            ...
            if (!inSwapAndLiquify && from != uniswapV2Pair && from != fomoReceiver) {
				...
				IFomo(fomoReceiver).buyAndBurn();
            }
			if (true) {
			    bool res = IFomo(fomoReceiver).transferbuymax(buytop[0],buytop[1],buytop[2]);
			    if (res){
    				datelist.push(block.timestamp);
                    delete buytop;
			    }
	...
	function buylevel(address user,uint256 amount) private {
		buynum[datelist[datelist.length - 1]][user]=buynum[datelist[datelist.length - 1]][user].add(amount);
		if (buynum[datelist[datelist.length - 1]][user] > buynum[datelist[datelist.length - 1]][buytop[0]]){
			if (buynum[datelist[datelist.length - 1]][buytop[0]] > buynum[datelist[datelist.length - 1]][buytop[1]]){
				buytop[2] = buytop[1];
				buytop[1] = buytop[0];
				buytop[0] = user;
				return;
		...
		if (buynum[datelist[datelist.length - 1]][user] < buynum[datelist[datelist.length - 1]][buytop[0]]){
			if (buynum[datelist[datelist.length - 1]][user] > buynum[datelist[datelist.length - 1]][buytop[1]]){
				buytop[2] = buytop[1];
				buytop[1] = user;
				return;
			}
			...
			if (buynum[datelist[datelist.length - 1]][user] < buynum[datelist[datelist.length - 1]][buytop[1]]){
				if (buynum[datelist[datelist.length - 1]][user] > buynum[datelist[datelist.length - 1]][buytop[2]]){
					buytop[2] = user;
					return;
				}
			}
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (!launched() && recipient == address(uniswapV2Pair)) {
            launch();
        }
        if (takeFee) {
            uint256 time = block.timestamp;
            if (launchedAtTime + 3 minutes > time) {
                require(timeLimit[sender] + 1 minutes < time && timeLimit[recipient] + 1 minutes < time, "The cooling time has not reached, please try again later");
                require(amount <= _maxTxAmount, "Buy transfer amount exceeds the _maxTxAmount.");
                if (sender == address(uniswapV2Pair)) {
                    timeLimit[recipient] = time;
                } else if (recipient == address(uniswapV2Pair)) {
                    timeLimit[sender] = time;
                } else if (sender != address(uniswapV2Router) && recipient != address(uniswapV2Router)){
                    timeLimit[sender] = time;
                    timeLimit[recipient] = time;
                }
            }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tFomo,  uint256 tLiquidity, uint256 tFee) = _getTValues(tAmount);
