...
contract Longbitcoin is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LongbitcoinDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Long Bitcoin", "LongBTC") {
        marketingWallet = payable(0x3D07EC48586447E1321484A136FC4b6DcF53dc3A);
        devWallet = payable(0x1cd8BE631cBa24aCA702885562Dbc55720EB8384);
        ...
        dividendTracker = new LongbitcoinDividendTracker(
            payable(this),
            router,
...
contract LongbitcoinDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Longbitcoin public LongbitcoinContract;
    ...
    constructor(
        ...
        defaultToken = token;
        LongbitcoinContract = Longbitcoin(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Longbitcoin_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Longbitcoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Longbitcoin contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(LongbitcoinContract);

            uint256 prevBalance = LongbitcoinContract.balanceOf(address(this));
            ...
            {
                uint256 received = LongbitcoinContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    LongbitcoinContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
