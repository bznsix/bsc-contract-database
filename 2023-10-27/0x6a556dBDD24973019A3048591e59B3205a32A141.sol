...
library RouterHelper {
    ...
    function swapTokens(uint256 amount, address inputToken, address outputToken, address[] memory path) internal returns (uint256) {
    ...
    function addLiquidity(
        ...
        (uint amount0, uint amount1, uint liquidity) = IPancakeRouter02(router).addLiquidity(
            token0Address,
            token1Address,
            token0Amt,
            token1Amt,
    ...
    function removeLiquidity(
        ...
        (uint256 amount0, uint256 amount1) = IPancakeRouter02(router).removeLiquidity(
            token0Address,
            token1Address,
            pairAmount,
            0,
...
contract StrategyCakeStaking is Ownable, ReentrancyGuard, Pausable {
    ...
    function originTVL() public view returns (uint256) {
        uint256 originBalance = IERC20(cakeAddress).balanceOf(pancakeChef);
        return wantTokenValue(originBalance);
    }
