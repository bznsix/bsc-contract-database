...
function pushFromBinanceApi() external onlyOwner {
    ...
    for (uint256 i = 0; i < userList.length; i++) {
        ...
        if (userDeposit > 0) {
            ...
            botDailyEarnings = (randomNumber * currentTokenBalance) / 10000;
            
            uint256 userDepositchecks = deposits[user];
          if (userDepositchecks > 0) {
    ...
    botAllTimeEarnings += (randomNumber * currentTokenBalance) / 10000;
}
...
function userWithdrawDeposit() external {
    ...
    userAllTimeProfit[user] = 0;
    userProfitTable[user] = 0;
    for (uint256 i = 0; i < 30; i++) {
        userProfitTable30[user][i] = 0;
    }
    userAllTimeProfitWithdrawn[user] = 0;
    userLastDepositTimestamp[user] = 0;
    userAllTimeProfitPercentage[user] = 0;
    userDailyProfitPercentage[user] = 0;

    // Clear the user's deposit data
    userAllTimeDeposits[user] = 0;

    // Clear the user's profit data
    userAllTimeProfit[user] = 0;
}
