...
contract Vault is ERC20, IVault, Pausable, ReentrancyGuard {
    ...
    function harvest(
        ...
        (uint256 amount0, uint256 amount1)= IMCV3(masterchefV3).collect(collectParams);
       uint256 amountOut =  _swap(token0,
        token1,
        amount0,
        0);
        _arrangeAddliquidityObject(amount1+amountOut,
        token0,
        _token0,
        _token1,
        0);
    ...
    function _calculateshareAmount(
        ...
        if (supply == 0) {
            ...
            _mint(0x000000000000000000000000000000000000dEaD, MINIMUM_LIQUIDITY); // permanently lock the first MINIMUM_SHARE tokens
        } else {
            (uint256 res0, uint256 res1) = _getReservers();
            share = Math.min(
