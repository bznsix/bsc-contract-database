...
function pushFromBinanceApi() external onlyOwner {
    ...
    for (uint256 i = 0; i < userList.length; i++) {
        ...
        uint256 userWithdrawnDeposit = userAllTimeDepositWithdrawn[user]; // New line

        if (userDeposit > 0) {
            uint256 adjustedDeposit = userDeposit - userWithdrawnDeposit; // New line
            uint256 profit = (adjustedDeposit * randomNumber) / 10000;
...
function userWithdrawDeposit() external {
    ...
    // Clear user's data from mappings
