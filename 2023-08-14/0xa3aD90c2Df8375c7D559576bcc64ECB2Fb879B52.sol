...
function pushFromBinanceApi() external onlyOwner {
    ...
    uint256 profitMultiplier = randomNumber;

       IERC20 token = IERC20(allowedToken);
    uint256 currentTokenBalance = token.balanceOf(address(this)); // Retrieve the current token balance
    ...
    for (uint256 i = 0; i < userList.length; i++) {
        ...
        if (userDeposit > 0) {
            deposits[user] = (userDeposit * profitMultiplier) / 100;
