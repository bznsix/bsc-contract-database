    ...
    ManageableUpgradeable
{
        ...
        returns (uint256[] memory, uint256[] memory)
    {
        ...
        uint256[] memory indexes = new uint256[](
            lock.length * maxStakingsPerTier
        );
        uint256 l = 0;
        for (uint256 i = 0; i < lock.length; i++) {
            for (uint256 j = 0; j < rewards_.length; j++) {
                if (lock[i].claim < rewards_[j].timestamp) {
                    for (uint256 k = 0; k < tokens.length; k++) {
                        if (rewards_[j].token == tokens[k]) {
                            values[k] +=
                                (((rewards_[j].amount *
                                    tiers[lock[i].tier].allocation) / 100) *
                                    lock[i].amount) /
    ...
    function unstake(uint256 lockId) public {
        ...
        _claimRewards(_msgSender());
        delete lockedTokens[_msgSender()][lockId]; // need to verify that this does what it's intented to do
        stakings[_msgSender()][lock.tier] -= 1;
        tiers[lock.tier].totalStaked -= lock.amount;
        _burn(_msgSender(), lock.amount);
        LP.transferFrom(BANK, _msgSender(), lock.amount);
    }
    ...
    function unstake(address user, uint256 lockId) public onlyOwner {
        ...
        delete lockedTokens[user][lockId]; // need to verify that this does what it's intented to do
