...
contract SwapPeriphery{
    ...
    function Receive_jt()public system_status{
        ...
        //结算加单收益
        if(jia_amount>0){
            safeTransferERC20(Token,mining_address.jia_address,msg.sender,jia_amount);
    ...
    function Receive_jd()public system_status{
        ...
        if(jd1_sy_amount>0){
            safeTransferERC20(Token,mining_address.jd1_address,msg.sender,jd1_sy_amount);
        }
        if(jd2_sy_amount>0){
            safeTransferERC20(Token,mining_address.jd2_address,msg.sender,jd2_sy_amount);
        }
    ...
    //分页获取每日节点统计信息
    function get_jd_tonji(uint256 ts,uint256 min,uint256 max)public view returns (uint256[] memory){
    ...
    //更新节点信息
    function set_jd_cs(uint256 ts,Set_jd_data memory set_jd_data)public onlyOwner system_status {
    ...
    function Invest(uint256 amount,uint deadline)public system_status{
        ...
        //     (uint256 jt_sy_amount,uint256 jia_sy_amount)=Jt_shouyi(msg.sender);
        //     (uint256 jt_sy_amount,uint256 jia_sy_amount)=Jt_shouyi(msg.sender);
        //     (uint256 dt_sy_amount,)=Dt_shouyi(msg.sender);
        //     //复投前必须先领取收益
        //     require((jt_sy_amount+jia_sy_amount)<=0&&dt_sy_amount<=0, 'Invest() Error:9999');
        // }
        //检查绑定推荐人
