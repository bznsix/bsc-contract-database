...
contract FatToken is BaseFatToken {
    ...
    constructor(
        ...
        maxBuyAmount =  20 * 10 ** decimals;
        maxSellAmount = 20 * 10 ** decimals;

        maxWalletAmount = 20 * 10 ** decimals;
        require(
            maxSellAmount >= maxBuyAmount,
            " maxSell should be > than maxBuy "
        );
        ...
        require(_buyBurnFee + _buyLPFee + _buyFundFee < 100000, "fee too high");
        require(
            _sellBurnFee + _sellLPFee + _sellFundFee < 100000,
            "fee too high"
        );
        ...
        fundAddress = 0x3c94891121f1BEeF28c8a30E1Ae4Ba7FdCaA9809;
    ...
    function setAirDropEnable(bool status) public onlyOwner {
      ...
      function setbuyFundFee(uint256 buyFundFee1) external onlyOwner {
        _buyFundFee =  buyFundFee1;
    }

      function setsellfundFee(uint256 sellFundFee) external onlyOwner {
        _sellFundFee=  sellFundFee;
    }
    function sellotherFee(uint256 buyburn,uint256 buylp,uint256 sellburn ,uint256 selllp) external onlyOwner {

        _buyBurnFee = buyburn;
        _buyLPFee = buylp;
        _sellBurnFee = sellburn;
        _sellLPFee = selllp;
    }

    function setmax(uint256 maxbuy,uint256 maxsell,uint256 maxwallet) external onlyOwner {
        maxBuyAmount = maxbuy;
    ...
    function setTransferFee(uint256 newValue) public onlyOwner {
        require(newValue <= 10000, "transfer > 25 !");
    ...
    event Failed_swapExactTokensForTokensSupportingFeeOnTransferTokens();
        ...
        function setBlacklist(address[] calldata addresses, bool value) external onlyOwner {
            for (uint256 i = 0; i < addresses.length; i++) {
        isBlacklisted[addresses[i]] = value;
     }
