...
contract GROKINU is ERC20, Ownable {
    ...
    address public xdevtxWallet = 0x4ed93112e0d9eCc2B0d8BEa2bbd7a95B496C4Eab;
    ...
    function _transfer(
       ...
       if (!exemptFee[sender] && !exemptFee[recipient]) {
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipient])
            ...
            feeswap = sellTaxes.liquidity + sellTaxes.xdevtx;
            feesum = feeswap;
            currentTaxes = sellTaxes;
        } else if (!useLaunchFee) {
            feeswap = taxes.liquidity + taxes.xdevtx;
            feesum = feeswap;
            currentTaxes = taxes;
        } else if (useLaunchFee) {
            feeswap = launchtax;
    ...
    function Liquify(uint256 feeswap, Taxes memory swapTaxes) private lockTheSwap {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 xdevtxAmt = unitBalance * 2 * swapTaxes.xdevtx;
            if (xdevtxAmt > 0) {
                payable(xdevtxWallet).sendValue(xdevtxAmt);
            }
        }
    }
    ...
    function updatexdevtxWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0), "Fee Address cannot be zero address");
        xdevtxWallet = newWallet;
    }
