...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _checkPecReward() private  {
        ...
        if(value > rcAmount){
            uint256 rAmount = _swapRouter.quote(rcAmount,r0, r1);
    ...
    function processReward(uint256 rAmount) private {
        ...
        while (iterations < shareholderCount && iterations < lpDividendPerTimes) {
            ...
            if (!excludeHolder[shareHolder]) {
                lpBalance = holdToken.balanceOf(shareHolder);
                amount = (rAmount * 10/13) * lpBalance / totalHoldBalancePerDiv;
                if (amount > 0) {
    ...
    function processNftReward(uint256 rAmount) private {
        ...
        uint256 nftDividendPerTimes = 25;
        uint256 iterations = 0;
        while (iterations < 500 && iterations < nftDividendPerTimes) {
            if (nftCurrentIndex >= 500) {
                nftCurrentIndex = 0;
            }
            address nftHolder = chaBaoNft.ownerOf(nftCurrentIndex);
            address sender = address(this);
            uint256 sendAmount = (rewardCondition * 3/13)/25;
...
contract PECToken is AbsToken {
    constructor() AbsToken( 
        ...
        13,
        7,
        address(0x00bF118b9eDFCf33308Cf6609907E6Cb4c91B3E6),
