...
contract SSP is ERC20, Ownable {
    ...
    address private marketingWalletAddress = 0x9fdCE4a026859007817176076185e6408769B1B0;
    address private rewardpoolWalletAddress = 0xC4E558aD115A82B462B828E661acb1bA935F0512;
    address private stakepoolWalletAddress = 0x0cdA28E66183Bb62B892b9842509b82c52BB5E05;
    ...
    constructor () ERC20("Starship", "SSP") 
    {   
        address newOwner = 0x645808578c9498292F1eB38e75E5C91975dD1D81;
    ...
    bool marketingSuccess = payable(marketingWalletAddress).send(marketingShare);
    if (marketingSuccess) {
        emit SwapAndSendToMarketing(tokenAmount, marketingShare);
    }

    // Transfer funds to reward pool wallet
    bool rewardPoolSuccess = payable(rewardpoolWalletAddress).send(rewardPoolShare);
    if (rewardPoolSuccess) {
        emit SwapAndSendToRewardPool(tokenAmount, rewardPoolShare);
    }

    // Transfer funds to stake pool wallet
    bool stakePoolSuccess = payable(stakepoolWalletAddress).send(stakePoolShare);
    if (stakePoolSuccess) {
