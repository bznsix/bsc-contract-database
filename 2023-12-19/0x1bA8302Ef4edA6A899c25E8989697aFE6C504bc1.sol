...
// File contracts/ssf/ssf_pro.sol
...
contract SSF is ERC20, Ownable {
    ...
    address public lpWallet = 0x000000000000000000000000000000000000dEaD;
    ...
    uint256 public totalFees = liquidityFee.add(burnFee);
    ...
    constructor(address _pancakeCaller) ERC20("Single slot finality", "SSF") {
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _contractUSDT = 0x55d398326f99059fF775485246999027B3197955;

        // Create a uniswap pair for this new token
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH());
        address _uniswapV2PairUsdt = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _contractUSDT);

        uniswapV2Router = _uniswapV2Router;
        uniswapV2Pair = _uniswapV2Pair;
        uniswapV2PairUsdt = _uniswapV2PairUsdt;
        contractUSDT = _contractUSDT;
        ...
        automatedMarketMakerPairs[_uniswapV2PairUsdt] = true;
        ...
        */
        _mint(owner(), 21000000  * (10**_decimals));
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        ...
        address _uniswapV2PairUsdt = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), contractUSDT);
        uniswapV2PairUsdt = _uniswapV2PairUsdt;
    }
    ...
    function setLiquiditFee(uint256 value) external onlyOwner{
        liquidityFee = value;
        totalFees = burnFee.add(liquidityFee);
        require(totalFees <= 100, "Total fee over 100");
    }

    function setBurnFee(uint256 value) external onlyOwner{
        burnFee = value;
        totalFees = liquidityFee.add(burnFee);
        require(totalFees <= 100, "Total fee over 100");
    }

    function setLpWallet(address _lpWallet) external onlyOwner{
        require(_lpWallet != address(0), "Zero address");
        lpWallet = _lpWallet;
    }

    function swapAndLiquifyUsdtManual(uint256 tokens) lockTheSwap external onlyOwner() {
        swapAndLiquifyUsdt(tokens);
    }
    ...
    function _transfer(
        ...
        if (takeFee){
            ...
            if (automatedMarketMakerPairs[to]){
                // When the accumulated amount reaches the limit, it will be burn and add lp one time.
                ...
                if( canSwap &&
                    ...
                    // go dead
                    if (burnFee > 0 ) {
                        super._transfer(address(this), deadWallet, contractTokenBalance.mul(burnFee).div(totalFees));
                    }

                    // add to lp
                    if (liquidityFee > 0){
                        uint256 swapTokens = contractTokenBalance.mul(liquidityFee).div(totalFees);
                        swapAndLiquifyUsdt(swapTokens);
                    }
            ...
            // buy and sell: take all fees
            if ((automatedMarketMakerPairs[to] || automatedMarketMakerPairs[from]) && !swapping && !isExcludedFromFee[from] && !isExcludedFromFee[to]){
    ...
    function swapAndLiquifyUsdt(uint256 tokens) lockTheSwap private {
       ...
       // split the contract balance into halves
        ...
        addLiquidityForUsdt(otherHalf, newBalance);
    ...
    function addLiquidityForUsdt(uint256 tokenAmount, uint256 usdtAmount) private {
