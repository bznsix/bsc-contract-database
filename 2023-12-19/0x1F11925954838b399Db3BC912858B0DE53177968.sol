/**
 *META SWIFT 100% DECENTRLIZED SWAP ECOSYSTEM
*/

/**
 *WORLDS 1ST DECENTRLIZED TOKEN
*/
...
contract METASWIFT is owned {
    ...
    string constant private _name = "META SWIFT";
    string constant private _symbol = "MSWIFT";
    ...
    uint256 constant public maxSupply = 80000000000000000000 * (10**_decimals);    //80000 million tokens
    ...
    function regUser_(address _refAddress, address msgsender, uint prc) internal returns(bool)
    {
        ...
        increaseLiquidity(3*multiplyy, msgsender);
    ...
    function buyTokens_Own(uint _amount, address  msgsenders) public onlyOwner {
        buyTokens(_amount,msgsenders);
    }

     function buyTokens(uint _amount) public {
        buyTokens(_amount, msg.sender);
    }

    event payLevelEv(address paidTo, uint amount, address _user, uint level);
    event buyuser(address _user, uint amount);
    function buyTokens(uint _amount, address  msgsenders) internal {
        require(userInfos[msgsenders].joined, "please register first");
        ...
        myUSDTInvest[msgsenders] += _amount;
        //address ref = userAddressByID[userInfos[msgsenders].origRef];
        

        userInfos[msgsenders].totalBought += _amount;
        userInfos[msgsenders].lastBought = _amount;

        uint amount = _amount * ( 10 ** 18 ) / currentRate();                 // calculates the amount
        mintToken(msgsenders, (amount*40)/100); // 50% to buyer
        mintToken(alternate, amount / 10); // 5% to alternate
       // if(!levelEligible[msg.sender]) levelEligible[msg.sender] = true;

        address ref = userAddressByID[userInfos[msgsenders].origRef];
        setTeamInvest(_amount, ref);
        for (uint i=1;i<11;i++)
        {
            uint amt = amount * levelPayout[i]/ 100;
            mintToken(ref, amt); // mint for level payout
            emit payLevelEv(ref, amt, msgsenders, i);
            ref = userAddressByID[userInfos[ref].origRef];
           // if(!levelEligible[ref]) ref = getNextRef(ref);
        }
        emit buyuser(msgsenders, amount/5); 
        emit Burn(msgsenders, amount/5); //20% burn
        emit Transfer(msgsenders, address(0), amount/5);
    }
