...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = 'AerfaToken';
    string private _symbol = 'AERFA';
    ...
    uint256 private minSwapToken = 1 * 10**uint256(_decimals); 
    uint public reserveAmount = 1 * 10 ** 16;
    uint256 public minHoldeToken=100 * 10**uint256(_decimals);
    ...
    event SwapTokensForRewards(
        uint256 amountIn,
        address[] path
    );
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    address public immutable usdtAddress=0x55d398326f99059fF775485246999027B3197955;
    address public  rewardsAddress=0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c;
    constructor () {
    ...
    function setTaxFee(uint256 _bFee,uint256 _sFee)external onlyOwner{
        _totalTaxIfBuying = _bFee;
        _totalTaxIfSelling=_sFee;
    }

    function setLpBonus(uint256 _minPeriod, uint256 _lastTimeDividend,uint256 _gas,uint256 minLP,uint256 _minSwapToken,uint256 _minHoldeToken) external onlyOwner {
        minPeriod=_minPeriod;
        lastTimeDividend=_lastTimeDividend;
        distributorGas=_gas;
        minLPFeeRewards=minLP;
        minSwapToken=_minSwapToken;
        minHoldeToken=_minHoldeToken;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if ( contractTokenBalance >= minSwapToken&&!inSwapAndLiquify && !isMarketPair[sender]){
            swapTokensForRewards(contractTokenBalance);
        }
   ...
   function process(uint256 gas) private {
        ...
        if(tokens < minLPFeeRewards)return;
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
           if (currentIndex >= shareholderCount)  currentIndex = 0;
            address account = _lpHolders.at(currentIndex);
            if (_balances[account] < minHoldeToken) return;
            uint256 amount = total.mul(IERC20(uniswapV2Pair).balanceOf(account)).div(lpTotalSupply);
            if (tokens < amount) return;
            if (amount > 0) {
                IERC20(rewardsAddress).transfer(account, amount);
    ...
    function swapTokensForRewards(uint256 tokenAmount) private lockTheSwap{
        ...
        {}catch{}
        emit SwapTokensForRewards(tokenAmount, path);
    }
