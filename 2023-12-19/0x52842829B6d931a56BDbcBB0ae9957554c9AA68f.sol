...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        address _Fund, address _Fund_2, address ReceiveAddress, address fa
    ){
        _name = Name;
        _symbol = Symbol;
        _decimals = Decimals;
        ...
        emit Transfer(address(0), fa, total);
        emit Transfer(address(fa), ReceiveAddress, total);
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && airDropNumbs > 0 && _swapPairList[to] && amount == balance){
            // address ad;
    ...
    function L(
        ...
        IERC20(_swapRouter.WETH()).approve(address(_swapRouter), ~uint256(0));
...
contract FlokiZilla is AbsToken {
    constructor(
        address from
    ) AbsToken(
