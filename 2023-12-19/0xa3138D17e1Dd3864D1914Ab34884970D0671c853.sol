...
contract Relcko is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RelckoDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Relcko", "Rlko") {
        marketingWallet = payable(0x7569e3d6B2565160BAde83c1c7bFcb265aA77AE3);
        devWallet = payable(0x78f50ec323652EB56922b0622605ECF0fbC87204);
        ...
        dividendTracker = new RelckoDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (210_000_000) * (10**18);
...
contract RelckoDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        RelckoContract = Relcko(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Relcko_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Relcko_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Relcko contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(RelckoContract);

            uint256 prevBalance = RelckoContract.balanceOf(address(this));
            ...
            {
                uint256 received = RelckoContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    RelckoContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
