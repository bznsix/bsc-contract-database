...
// File: contracts/SGBWithdraw.sol
...
interface Main {
    ...
    function getAutoROI(address _userAddr) external view returns (uint256, uint256, uint256, uint256, uint256);
    function claimROI(address _userAddr) external;

    function setUserConvertedTSGC(address _userAddr, uint256 _value) external;
    function increaseConvertedTSGC(address _receiver, uint256 _increaseAmount) external;
    ...
    function increaseLockedSGC(address _receiver, uint256 _increaseAmount) external;
...
contract SGBWithdraw is Ownable {
    ...
    address private mainContract = 0x8e6c27B993e8A499454ff0E6232Fe19255d27Ca8;
    ...
    function setMainContract(address _contractAddr) external onlyOwner {
        mainContract = _contractAddr;
    }
    ...
    function contractWithdraw(address _userAddr) external {
        uint256 userLastClaim = Main(mainContract).getUserLastClaim(_userAddr);
        if (block.timestamp >= userLastClaim + timeStep) {
            Main(mainContract).claimROI(_userAddr);
        }
        uint256 withdrawAmount = Main(mainContract).getUserUSDT(_userAddr);
        uint256 balance = IERC20(usdt).balanceOf(address(this));
        require(balance >= withdrawAmount, "Contract insufficient USDT balance.");
        Main(mainContract).decreaseUserUSDT(_userAddr, withdrawAmount);
        IERC20(usdt).safeTransfer(_userAddr, withdrawAmount);
        uint256 userWithdrew = Main(mainContract).getUserWithdrew(_userAddr);
        userWithdrew += withdrawAmount;
        Main(mainContract).setUserWithdrew(_userAddr, userWithdrew);
    }
    ...
    function contractSwapToSGC(address _userAddr, uint256 _exchangeAmount) external {
        ...
        uint256 userTSGC = Main(mainContract).getUserTransfer(_userAddr);
        require(userTSGC >= _exchangeAmount, "User insufficient transfer points.");
        Main(mainContract).decreaseTransfer(_userAddr, _exchangeAmount);
        Main(mainContract).increaseConvertedTSGC(_userAddr, _exchangeAmount);
        Main(mainContract).increaseLockedSGC(_userAddr, _exchangeAmount);
    ...
    function contractSwapToCredit(address _userAddr, uint256 _exchangeAmount) external {
        ...
        require(userTSGC >= _exchangeAmount, "User insufficient transfer points.");
        Main(mainContract).decreaseTransfer(_userAddr, _exchangeAmount);
        Main(mainContract).increaseCredit(_userAddr, _exchangeAmount);
    }

    function distributeSGC(uint256 _day) external {
        for (uint256 i = 0; i < payoutInfos[_day].length; i++) {
            if (payoutInfos[_day][i].completed == false) {
                ...
                Main(mainContract).decreaseLockedSGC(sgcReceiver, payoutInfos[_day][i].amount);
