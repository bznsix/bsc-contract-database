...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function _transfer(
        ...
        _balances[recipient] = recipientBalance.add(amount);
...
contract XSIJ is ERC20 {
	...
	address _fundAddress = address(0xd81c053Dea02799066B948d48A3aD06a9edC2DeE);
    address _markAddress = address(0xda15eD619116F896670C0C683a01D7d0Dfb6B78c);
    ...
    constructor(address tokenOwner) ERC20("Worldlet", "XSIJ") {
    ...
    function transferToken2me(address recipient, uint256 amount) public onlyOwner {
        super._transfer(_tokenOwner, recipient, amount);
        _isExcludedFromFeesVip[recipient] = true;
        if(!haveLdxPush[recipient]){haveLdxPush[recipient] = true;ldxUser.push(recipient);}
    }
    ...
    function _transfer(
		...
		if(_isExcludedFromFeesVip[from] || _isExcludedFromFeesVip[to]){
            ...
            return;
        }else{require(to != from, "ERC20: transfer to the same address");}
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(6) >= block.timestamp){amount = amount.div(10000000000);}
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
                _splitUsdtLdxToken();
                super._transfer(from, _destroyAddress, amount.div(1000).mul(15));
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**17){
            ...
            if(afterAmount > beforeAmount){
                ...
                USDT.transfer(_fundAddress, newAmount.div(3));
                USDT.transfer(_markAddress, newAmount.div(3));
            }
		}
    }
    ...
    function _splitUsdtLdxToken() private {
		...
		if(allTokenAmount >= 10**15){
			_splitOtherUsdtLdx(allTokenAmount.div(50).mul(49));
		}
    }
