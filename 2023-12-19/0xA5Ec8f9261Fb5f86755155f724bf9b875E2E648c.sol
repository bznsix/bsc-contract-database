...
contract PinkChain is IERC20, Ownable {
    ...
    uint256 public _minPeriod = 60;
    uint256 public _lpDiv;
    address public _rewardToken;
    address public _marketRouter;

    bool public _startLimit;

    constructor() {
        address router;
        address admin = 0xC8aAae549ACa6227478E7469A0ada91d2D9B3e56;
        _name = "PinkChain";
        _symbol = "PKB";
        _decimals = uint8(18);
        _tTotal = 5000000000 * (10**uint256(_decimals));
        ...
        if (block.chainid == 56) {
            ...
            _nft = 0x407534E73D9e3635789A32f216C9FD626109EC7d;
        }
        ...
        _marketRouter = address(new URoter(_token, address(this)));
        _nftRouter = address(new URoter(_token, address(this)));
        (bool dd ,) = _token.call(abi.encodeWithSelector(0x095ea7b3, _uniswapV2Router, ~uint256(0)));
        require(dd);
    ...
    function openLiq() public onlyOwner {
        _startLimit = true;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            getSellFee() > 0 && to == _uniswapV2Pair
        ) {
            //sell
    ...
    function takeNFTDivi() public {
        ...
        for(uint i;i<ss;i++ ){
            ...
            IERC20(_token).transferFrom(_nftRouter, NFT(_nft).ownerOf(_nftIndex),5e18);
            _nftIndex++;
        }
    }


    uint public shareDiviIndex;
    function takeShareDivi() public {
        uint ddd  = IERC20(_token).balanceOf( _marketRouter);
        uint j = ddd/5e18;
        if(j==0||shareDivi.length==0 ) return;
        uint ss = j>10?10:j;
        for(uint i;i<ss;i++ ){
            if(shareDiviIndex>=shareDivi.length){
                shareDiviIndex=0;
            }
            IERC20(_token).transferFrom(_marketRouter, shareDivi[shareDiviIndex],5e18);
            shareDiviIndex++;
        }
    }
    ...
    function transferSwap(uint256 contractTokenBalance) private {
        ...
        if (_denominator > 0) {
            ...
            uint256 nftD = (tokenBal *
            (_buyLiquidityFee.add(_sellLiquidityFee))) / (_denominator);
            IERC20(_rewardToken).transfer(_nftRouter, nftD);
            IERC20(_rewardToken).transfer(
                _lpRouter,
                tokenBal.mul(_buyLpFee.add(_sellLpFee)).div(_denominator)
            );
            IERC20(_rewardToken).transfer(
                _marketRouter,
    ...
    address[] public shareDivi;

    function addShareDivi(address[] memory value  ) public onlyOwner {
        for(uint i;i<value.length;i++){
            shareDivi.push(value[i]);
        }
    }

    function deleteShareDivi() public onlyOwner {
        delete shareDivi;
    }
...
contract URoter {
    constructor(address token, address to) {
        (bool dd ,) = token.call(abi.encodeWithSelector(0x095ea7b3, to, ~uint256(0)));
        require(dd);
    }
}