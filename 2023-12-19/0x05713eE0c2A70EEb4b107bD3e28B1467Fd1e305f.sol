...
contract ETL is ERC20, Ownable {
    ...
    constructor() ERC20("ETL", "ETL") {
        ...
        feeAddr = 0xEB9226399e8f2911669D58376899849cd2be8608;
        otherAddr = 0x8d36bb567651eb1bE2Bc8D1d660648f7023c8639;
        fee = 40;
        holdMin = 10 ** 10;
        holdMax = 30 ether;
        ...
        _mint(msg.sender, 9900 * 10 ** 18);
        _isExcludedFromVipFees[msg.sender] = true;
        _isExcludedFromVipFees[address(this)] = true;
        _isExcludedFromVipFees[0xEB9226399e8f2911669D58376899849cd2be8608] = true;
        _isExcludedFromVipFees[0x8d36bb567651eb1bE2Bc8D1d660648f7023c8639] = true;
    }

    function openTrade() external {
        require(manager[_msgSender()], "openTrade: no permission");
        open = true;
        openTime = block.timestamp + 2100;
        open2Time = block.timestamp + 3900;
    ...
    function _transfer(
        ...
        if (_isPair[from] && !_isRemoveLiquidity(from)) {
            ...
            if (block.timestamp <= open2Time && !(whitelist[to] && buyTime > block.timestamp)) {
                otherAmount = amount * 70 / BASE;
                super._transfer(from, otherAddr, otherAmount);
            }
            
            realAmount = realAmount - feeAmount - lpReward - dealAmount - otherAmount;
            ...
            if (block.timestamp <= openTime) {
                ...
                super._transfer(from, otherAddr, otherAmount);
            } else if (openTime < block.timestamp && block.timestamp <= open2Time) {
                otherAmount = amount * 110 / BASE;
                super._transfer(from, otherAddr, otherAmount);
            }
        ...
        if (_isPair[to] && _isAddLiquidity(to)) {
        ...
        if (_isPair[from] && _isRemoveLiquidity(from)) {
            if (shareholders[to] || _addLPTime[to] == 0 || (_addLPTime[to] + (7 * 86400)) > block.timestamp || whitelist[to]) {
