...
contract TheWaterWizards is ERC721Enumerable, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  // Supply
  uint256 public mintReward = 20;
  uint256 public mintSplit = 5;
  ...
  // Lists 
  uint256[3777] public remainingIds;
  ...
  function handleMintReward(uint256 _amount) private {
      // Split payment
      uint256 mintShareM = _amount.div(mintReward); //5% Dev
      uint256 mintShareS = _amount.div(mintReward); //5% Dev
      uint256 mintShareR = _amount.div(mintSplit); //20% reflections
      uint256 OwnerShare = _amount.sub(mintShareM + mintShareR + mintShareS);
      // Save dividend for community
      reflectDividend(mintShareR); //20%
      // Send owner/dev shares
      payable(owner()).transfer(OwnerShare); 
      payable(0xEF667C250AeB276d7Ada9941fDa4f25EA0099D4A).transfer(mintShareS);
      payable(0x990a62424BE3F6bC5Fe0839480C38Aa36521FE63).transfer(mintShareM);
