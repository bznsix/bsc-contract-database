...
contract MerketNFT is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 private _maxMintSupply = 3000;
    string private constant _name = "Vitra Studios NFT";
    string private constant _symbol = "VTNFT";
    uint256 private constant _DECIMALFACTOR = 10 ** uint256(18);
    ...
    mapping(uint256 => storageConf) private _storageConfig;
    mapping(uint256 => uint256) private _nftToStorageId;
    ...
    modifier onlyTokenOrOwner() {
        require(
            msg.sender == _tokenContractAddress ||
            msg.sender == owner(),
     ...
     */
    function getActionFee() public view returns (uint256) {
        return _auction_fee;
    }
     ...
     */
    function getMinBidPercent() public view returns (uint256) {
     ...
     */
    function getMinBidValue() public view returns (uint256) {
        return _minBidValue;
    }
    ...
    {
        uint256 storId = _nftToStorageId[nftId];
        return _storageConfig[storId].isNode;
    }
    ...
    {
        uint256 _storId = _nftToStorageId[_nftId];
        return (
            _storageConfig[_storId].userAddress,
            _storId,
            _storageConfig[_storId].nftId,
            _storageConfig[_storId].isNode,
            _storageConfig[_storId].exist
        );
    }
    ...
    //Global Mint NFT
    function mint(address _to, uint256 storId, bool isNode, string memory _uriToken, bytes memory signature)
        public
        payable
        isPausable
    {
        ...
        require(!_storageConfig[storId].exist, "This item has already been minted.");
        if(newId > 1){
            bytes32 signatureHash = keccak256(signature);
            require(!usedSignatures[signatureHash], "Signature already used");
            bytes32 messageHash = keccak256(abi.encodePacked(_to, newId, _uriToken));
        ...
        _storageConfig[storId] = storageConf(_to, storId, newId, isNode, true);
        _nftToStorageId[newId] = storId;
    ...
    function addNftOnSale(uint256 tokenId, uint256 _price) public isPausable {
        ...
        // Clear approvals from the previous owner
        _approve(_OwnerAddr, tokenId, address(this));
    ...
    function deleteNftOnSale(uint256 tokenId) public isPausable {
        ...
        // Clear approvals from the previous owner
        _approve(_OwnerAddr, tokenId, address(0));
    ...
    function airDropSender(Receivers[] memory wallets) public onlyOwner {
        uint256 totalAmount;
    ...
    function notifyTransfer(uint256 amount) external onlyTokenOrOwner {
    ...
    function claimTokens(uint256 amount) public isPausable {
        ...
        if (balanceOfTokens(wallet) <= amount) amount = balanceOfTokens(wallet);
        require(
            amount > 0,
    ...
    function setTokenAddress(address payable TokenAddress)
        public onlyOwner
    {
    ...
    function setSignerAddress(address payable signerAddress)
        public onlyOwner
    {
        _signerAddress = signerAddress;
    }
    ...
    */
    function setBnbAdress(address bnb_addr) external virtual onlyOwner {
    ...
    */
    ...
    function setEstimateOrFixBid() public onlyOwner {
        if (estimateOrFixBid) {
            estimateOrFixBid = false;
        } else {
            estimateOrFixBid = true;
        }
    }
