...
contract WHEECOIN is Context, IERC20,Ownable{
    using SafeMath for uint256;
    using Address for address;
    string private _name = "WHEE-COIN";
    string private _symbol = "WHEE";
   ...
   function swapAndFeeForToken()private  lockTheFomoSwap {
    ...
    function _transfer(address from, address to, uint256 amount) private  {
       ...
       require(balance >= amount, "balanceNotEnough");
        ...
        if (from != address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp&&fUSDTBalance>0) {
