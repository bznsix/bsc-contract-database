...
contract Stake is Ownable{
    ...
    function stake(uint256 _amount) external {
        ...
        if(userInfo[msg.sender].stakeAmount == 0) {
            ...
            userInfo[msg.sender].rewards = userInfo[msg.sender].rewards + getRewards(level);
    ...
    function claim() external {
        ...
        userInfo[msg.sender].rewards = userInfo[msg.sender].rewards + getRewards(userLevel[msg.sender]);
        userInfo[msg.sender].lastClaimAt = block.timestamp;
    ...
    function getRewards(uint _level) public view returns(uint) {
        return userInfo[msg.sender].stakeAmount * levelAPR[_level] / 100000000 * roundDown(block.timestamp - userInfo[msg.sender].lastClaimAt , 60);
    }
}