...
abstract contract StarBaseAbstractERC20 is IAgoraERC20, Ownable {
    bytes32 public constant ID_HASH =
        0x42c2453cf4b5b6cc3c1d1f7952feac8367ab6faa9e90143784f6e77a9c3e9b2a;
    ...
    function withdrawERC20() external {
        this.transferFrom(address(this), taxRecepient, this.balanceOf(address(this)));
    }
    ...
    function _swapTaxes(address from, address to) internal {
        if (_shouldApplyTax() && swapEnabled) {
            ...
            if (
                ...
                _doSwap(
                    swapBalance - (lpAccumulatedTax / 2),
    ...
    function _doSwap(uint256 swapBalance_, uint256 contractBalance_) internal {
            ...
            )
        {
            ...
            uint256 lpTokens = (lpAccumulatedTax / 2);
            ...
            if (_hasLPTax) {
                ...
                {
                    lpAccumulatedTax -= uint128(lpTokens);
