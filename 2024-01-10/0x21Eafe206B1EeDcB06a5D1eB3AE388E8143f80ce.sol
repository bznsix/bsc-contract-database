...
library ProxyLibrary {
    ...
    function toDirectRec(uint256 memberLevel, address recAddr, uint256 recLevel, uint liquidity,
        ...
        uint256 recRewardBase = (liquidity * directRecRewardRatio * 2) / 100;
    ...
    function toDirectRec_a(uint256 memberLevel, address recAddr, uint256 recLevel, uint liquidity,
        ...
        uint256 recRewardBase = (liquidity * directRecBlack * 2) / 100;
        if (recAddr != address(0) && recLevel > 0) {
            tmpTransNum = recLevel >= memberLevel  ? recRewardBase  : 0;
            IERC20(uniswapV2Pair).safeTransfer( address(0xDead), tmpTransNum);
            Team22Fall(teamAddress).addDirectRecReward( address(0xDead), tmpTransNum);
        }
        return tmpTransNum;
    }
    ...
    function toUpParents(address account, uint256 memberLevel,  uint liquidity,
        ...
        uint256 teamRewardBase = (liquidity * teamRewardRatio * 2) / 100 / maxTeamDeep;
    ...
    function distributeRewards(address account, uint256 memberLevel, address recAddr, uint256 recLevel, uint liquidity,
        uint256 directRecRewardRatio,uint256 directRecBlack, uint256 teamRewardRatio, uint256 maxTeamDeep, 
...
contract QILinProxy is ERC20, Ownable {
    ...
    constructor() ERC20("QiLin Proxy", "QLProxy") {
        ...
        marketingAddress = 0x4eCD353855de2C0f94C52e95B6CC7e2F4E9617c0;
        wList[marketingAddress] = true;

        wList[msg.sender] = true;
        wList[address(this)] = true;

        directRecRewardRatio = 25;//zhitui
        directRecBlack = 3;//heidong
        teamRewardRatio = 14; //tuandui

        maxTeamDeep = 16;
        sellFeeRatio = 3;

        levelUsdtArr = new uint256[](12);
        levelUsdtArr[0] = 0;
        levelUsdtArr[1] = 1000 * 1e18;
        levelUsdtArr[2] = 4000 * 1e18;
        levelUsdtArr[3] = 10000 * 1e18;

        levelBuyArr = new uint256[](12);
        levelBuyArr[0] = 0;
        levelBuyArr[1] = 0;
        levelBuyArr[2] = 20000 * 1e18;
        levelBuyArr[3] = type(uint).max;
    ...
    function upgradeQL(address recAddr, address parentAddr, uint256 amountU, uint256 memberLevel) public isEOA {
        ...
        ProxyLibrary.distributeRewards(msg.sender, memberLevel, recAddr, recLevel, liquidity,
            directRecRewardRatio, directRecBlack,teamRewardRatio, maxTeamDeep,uniswapV2Pair, teamAddress, marketingAddress);
