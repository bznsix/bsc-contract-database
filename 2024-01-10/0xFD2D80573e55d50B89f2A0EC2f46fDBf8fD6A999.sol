...
contract Water is ERC20, Ownable {
    ...
    uint256 public deadFee = 3; //3% dead
    uint256 public lpRewardFee = 2; //2% lp
    uint256 public destoryZGLKingFee = 3; // 3% ZGLKing

    address public lpRewardToken = 0x55d398326f99059fF775485246999027B3197955; 
    address public deadWallet = 0x000000000000000000000000000000000000dEaD;
    uint256 public AmountDestoryZGLKing;  
   ...
   TokenDividendTracker public dividendTracker;
    ...
    function setDestoryZGLKingFee(uint256 val) public onlyOwner {
        destoryZGLKingFee = val;
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            if(AmountDestoryZGLKing > 0){
                destoryZGLKing(AmountDestoryZGLKing);
                AmountDestoryZGLKing = 0;
            }
    ...
    function takeAllFee(address from,uint256 amount) private returns(uint256 amountAfter) {
        ...
        uint256 ZGLKingFee = amount.mul(destoryZGLKingFee).div(100);
        AmountDestoryZGLKing += ZGLKingFee;
        amountAfter = amountAfter.sub(ZGLKingFee);

        super._transfer(from, address(this),  LPFee.add(ZGLKingFee));
    }
    ...
    function destoryZGLKing(uint256 tokenAmount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = 0x066717FEb54A7212C1DD6B3148267C6bC1282999;
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
