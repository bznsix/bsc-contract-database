...
interface IVLCakepie {
    function lockFor(uint256, address) external;
}

contract CakepieAirdrop is Ownable, Pausable, ReentrancyGuard {
    ...
    IVLCakepie public vlCKP;

    /* ============ Events ============ */

    event ClaimEvent(address account, uint256 amount, bool isLock);
    event VLCKPUpdated(address _newVlCKP, address _oldVlCKP);
    ...
    constructor(IERC20 _reward, address _vlCKP,  uint256 _startVestingTime, uint256 _vestingPeriodCount, uint256 _intervals, bytes32 _totalRewardMerkleRoot) {
        reward = _reward;
        totalRewardMerkleRoot = _totalRewardMerkleRoot;
        startVestingTime = _startVestingTime;
        vestingPeriodCount = _vestingPeriodCount;
        intervals = _intervals;
        vlCKP = IVLCakepie(_vlCKP);
    }
    ...
    function claim(uint256 totalAmount, bytes32[] calldata merkleProof, bool isLock) external whenNotPaused nonReentrant {
        ...
        if (isLock) {
            reward.safeApprove(address(vlCKP), claimable);
            vlCKP.lockFor(claimable, msg.sender);
        } else {
    ...
    {
        address oldVlCKP = address(vlCKP);
        vlCKP = IVLCakepie(_vlCKP);
        emit VLCKPUpdated(address(vlCKP), oldVlCKP);
    }
