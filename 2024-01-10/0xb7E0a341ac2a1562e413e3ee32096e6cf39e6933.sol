...
address[] public NFTs;
  ...
  address public RewardsFeeReceiver = 0x7eBFd7599be6d66dbecA187322dC8983f68C7a3D;
  ...
  constructor(address _stakingToken) {
    ...
    //Standard Pools => not locked, 30days, 12% APY, 20% fee for early unstake, 5, 000 8Bit minimum for staking
      Pools[0] = StakingPool(false, 15 days, 12, 200, 5000 * 10 ** decimals, 15 days);

    // NFT Pools => not locked, 30 days, 36% APY, 20% fee for early unstake, 25,000 8Bit minimum for staking, 15 days minUnstakeTime
    Pools[1] = StakingPool(false, 15 days, 36, 200, 25000 * 10 ** decimals, 15 days);
 ...
 for (uint256 i = 0; i < allStakers.length; i++) {
  ...
  function emergencyUnstake(uint256 _poolId, uint256 _toUnstake) public {
    ...
    uint256 amountToUnstakeAfterPenalty = _toUnstake - penaltyAmount;
