...
contract BAO is ERC20, Ownable {
    ...
    uint256 public DDFee;
    uint256 public sellFee;
    uint256 private taxAmount;
    uint256 public walletToWalletTransferFee;
    address private marketingWalletAddress = 0xe73d255F26bB83f4560FF03615C952A19692CEbF;
    address public immutable deadAddress =0x000000000000000000000000000000000000dEaD; // dead address      
    address DDcoin = address(0x30b63938B072E0fC341Ae48EBCB1cF7b8F8CaA34);
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || swapping) {
            ...
            _totalFees = buyFee + DDFee;
            taxAmount = deadFee;
        } else if (to == uniswapV2Pair) {
            _totalFees = sellFee + DDFee;
            taxAmount = deadFee;
        } else {
    ...
    function swap(uint256 tokenAmount) private {
        ...
        uint256 divFees = sellFee + DDFee;
        uint256 tokenAmountForDD = tokenAmount.mul(DDFee).div(divFees);
        uint256 tokenAmountForSwap = tokenAmount - tokenAmountForDD;
        ...
        if (success) {
            emit SwapAndSend(tokenAmountForSwap, newBalance);
        }

        if (tokenAmountForDD > 0) {
            swapTokensForDD(tokenAmountForDD);
        }
    }

    function swapTokensForDD(uint256 tokenAmount) private {
