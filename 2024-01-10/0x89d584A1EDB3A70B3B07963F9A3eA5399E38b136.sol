...
contract BatchTransferNFT is Ownable{
    ...
    modifier IsClaimed(uint256 index) {
        require(!isClaimed[index], "index");
        _;
    ...
    function batchTransfer(address[] memory _address, uint256 index) external onlyOwner() IsClaimed(index){
        require(claimEnabled, "CLAIM: WAITING_FOR_CLAIM_TO_BE_OPENED");
        for(uint256 i = 0; i < _address.length; i++){
            IAITNFT(AITNFT).mint(_address[i]);
        }      
    }
}