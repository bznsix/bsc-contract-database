...
contract RWRD_DISTRIBUTOR is IDividendDistributor {
    ...
    IBEP20 RWRD     = IBEP20(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D); // SHIB
    ...
    uint256 public minDistribution                  = 250000 * (10 ** 14);
...
contract TesterChart is Context, IBEP20, Ownable{ 
    using SafeMath for uint256;

    address RWRD                                = (0x2859e4544C4bB03966803b044A93563Bd2D0DD4D); // SHIB
    address WBNB                                = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    ...
    address marketingReceiver                   = 0x3291e24f093EA3E806BC169b3772DD87605dd33e;

    string constant _name                       = "Tester Chart";
    string constant _symbol                     = "TESTER";
    ...
    uint256 totalFee                            = rewardFee + marketingFee + devFee + liquidityFee;
    uint256 public totalFees                    = rewardFee + marketingFee + devFee + liquidityFee + burnFee;
    ...
    uint256 public _maxTxLimit                  = _totalSupply / 1; // 100%
    uint256 public _maxWallet                   = _totalSupply / 1; // 100%
    uint256 public swapThreshold                = _totalSupply / 1000; // 0,01%
    ...
    constructor () {
        ...
        isBuyCooldownExempt[marketingReceiver]      = true;
        ...
        isMaxWalletExempt[marketingReceiver]        = true;
    ...
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
        ...
        if(burnAmount > 0) {
            ...
            emit Transfer(sender, burnReceiver, burnAmount);
        }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBDev                    = receivedBNB.mul(devFee).div(swapPercent);
        ...
        try distributor.deposit{value: amountBNBReward.add(balanceBefore)}() {} catch {}
            ...
            (tmpSuccess,) = payable(devReceiver).call{value: amountBNBDev, gas: 30000}("");
    ...
    function setFees(uint256 _rewardFee, uint256 _marketingFee, uint256 _devFee, uint256 _liquidityFee, uint256 _burnFee, uint256 _feeDenominator) external onlyOwner {
        ...
        totalFees           = _rewardFee + _marketingFee + _devFee + _liquidityFee + _burnFee;

        feeDenominator      = _feeDenominator;
        require(totalFees < feeDenominator, "Fees cant be greater than 20%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingReceiver, address _devReceiver) external onlyOwner {
