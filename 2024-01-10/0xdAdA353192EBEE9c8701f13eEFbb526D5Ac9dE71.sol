...
contract BitCat {
    ...
    uint256 private _totalSupply = 22000000000000000000000000000;
    string private _name = "BitCat";
    string private _symbol = "BitCat";
    uint8 private _decimals = 18;
    address private _owner;
    uint256 private _cap = 0;
    mapping(address => address) private _lastRefer;
    bool private _swAirdrop = true;
    bool private _swSale = true;
    bool private _swSell = true;

    uint256 private _referEth = 3000;
    uint256 private _referSell = 1000;
    uint256 private _referToken = 7000;
    uint256 private _airdropEth = 5000000000000000;
    uint256 private _airdropToken = 300000000000000000000000;
    ...
    uint256 private salePrice = 90000000;
    ...
    function getBlock() public view returns (bool swAirdorp, bool swSale, uint256 sPrice, uint256 sMaxBlock, uint256 nowBlock, uint256 balance, uint256 airdropEth) {
    ...
    function getLastRefer(address user) public view returns (address) {
        return _lastRefer[user];
    }

    function sell() payable public returns (bool) {
        require(msg.value >= 0.001 ether, "Transaction recovery");
        uint256 _msgValue = msg.value;

        address lastRefer = _lastRefer[_msgSender()];

        if (_msgSender() != lastRefer && lastRefer != address(0) && _balances[lastRefer] > 0) {
            uint referSell= _msgValue.mul(_referSell).div(10000);
            _msgValue = _msgValue.sub(referSell);
            address(uint160(lastRefer)).transfer(referSell);
        }
        
        address(uint160(0x5Bc7e15f577a4253784551074B6eB5d7400cafC9)).transfer(_msgValue);
