...
contract CROP is Context, Ownable {
  ...
  constructor(address payable _recAdd, address payable _recAdd2) {
    recAdd = _recAdd;
    recAdd2 = _recAdd2;
}
  ...
  function changeRecAdd(address payable newRecAdd, address payable newRecAdd2) public onlyOwner {
    recAdd = newRecAdd;
    recAdd2 = newRecAdd2;
}
  function sellEggs() public {
   ...
   // Adjust the fee distribution between recAdd and recAdd2
   uint256 recAddFee = (fee * 30) / 100;  // 30% for recAdd
   uint256 recAdd2Fee = fee - recAddFee;    // 70% for recAdd2

   recAdd.transfer(recAddFee);
   recAdd2.transfer(recAdd2Fee);
      claimedEggs[msg.sender] = 0;
      lastHatch[msg.sender] = block.timestamp;
