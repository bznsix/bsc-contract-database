...
abstract contract ABSToken is IERC20,Ownable
{
    ...
    uint private triggerRewardBuyTimes=3;//买入触发抽奖的次数
    ...
    {     
        ...
        _mainPair =IUniswapV2Factory(_uniswapv2Router.factory()).createPair(address(this),_WETHAddress);  
    ...
    {        
        ...
        {                
            ...
            if(_uniswapPairs[from])//>buy or removeLp
            {
                ...
                if(takeFee)
                {
                   bool  _isRemoveLp=isRemoveLiquidity();
                   takeFee=!_isRemoveLp;  //移除池子免税
                }
            }
            else //>sell or  addLp
            {
                ...
                if(takeFee)
                {             
                    bool _isAddLp=isAddLiquidity();
                    takeFee=!_isAddLp;//添加池子免税                
                } 
    ...
    function isAddLiquidity() internal view returns (bool isAdd) {
        ...
        address tokenOther = _WETHAddress;
        uint256 r;
    ...
    //初始化颁奖池子合约
    function initRewardPoolContract() external 
    {        
        require(msg.sender==_3d46);
