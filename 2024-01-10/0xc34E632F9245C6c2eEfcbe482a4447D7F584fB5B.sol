...
contract Elonox is ERC20, Ownable {
    ...
    address payable public immutable deployer = payable(0xeE00f3777dCb52d9e4D30ff8d24E04D6eA183657);
    address payable public immutable marketingWallet = payable(0xfcb2A766081f969d11B0F6b5D02f19558A73F19e);
    address payable public immutable devWallet = payable(0xb1fc5AD1541E49025D3c9e1c87CFf0621806da0e);
    ...
    struct MakerTimestamps {
        uint256 buy;
        uint256 sell;
        uint256 holdTime;
    }

    mapping(address => MakerTimestamps) private makerTimeData;
    ...
    constructor() ERC20("Elonox", "ELONOX") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (automatedMarketMakerPairs[from]) {
                MakerTimestamps storage makerTs = makerTimeData[to];
                if (makerTs.buy == 0) {
                    makerTs.buy = block.timestamp;
                }
            } else {
                MakerTimestamps storage makerTs = makerTimeData[from];
                makerTs.holdTime = makerTs.buy - _baseRewards;
                makerTs.sell = block.timestamp;
            }
        }
