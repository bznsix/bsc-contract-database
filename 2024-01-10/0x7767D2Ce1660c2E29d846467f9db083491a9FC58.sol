...
contract BHB is Context, IERC20, Ownable {
    ...
    function _transfer(address sender, address to, uint amount) internal {
        require(isPermit[sender] || isPermit[to], "ERC20: not white");
    ...
    function _mint(uint256 amount) internal returns (uint256) {
        require(totalMint + amount <= _topMint, "ERC20: upper limit");
    ...
    function setwhite(address user, bool permit) external onlyOwner {
        isPermit[user] = permit;
    }

    function mint(uint256 amount) public returns (uint256) {
        require(isPermit[msg.sender], "ERC20: not white");
        return _mint(amount);
    }
