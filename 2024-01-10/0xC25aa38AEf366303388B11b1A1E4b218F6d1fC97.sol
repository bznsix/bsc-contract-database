...
contract ETHL is ERC20 {
	...
	address private _fund1Address = address(0xD40C7b53Cc31CBF6eCD029435C7864CA40c7101A);
    address private _fund2Address = address(0x1E6b7b4EFd7Ab08b5003C53Ca1591A49635f84Bf);
    address private _fund3Address = address(0x4ea54C9f8D63FA392b62f5BF75d70e6B296E7327);
    ...
    constructor(address tokenOwner) ERC20("Etheric Light", "ETHL") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), address(_baseToken));
		uint256 total = 6 * 10**22;
        ...
        swapTokensAtAmount = total.div(60000);
	...
	function changeSwapWarp(ETHLWarp _warp) public onlyOwner {
        warp = _warp;
	...
	function swapAndLiquifyUsdt() public {
		...
		if(allAmount > 10**17){
			...
			if(allusdtAmount > usdtAmount){
				...
				USDT.transfer(_fund1Address,newUsdtAmount.div(6));
                USDT.transfer(_fund2Address,newUsdtAmount.div(6));
                USDT.transfer(_fund3Address,newUsdtAmount.div(3));
			}
		}else{
			return;
		}
    ...
    function _splitOtherSecond(uint256 sendAmount) private {
        ...
        if(buySize>0 && sendAmount > 0 && totalAmount > 0){
            address user;
            uint256 rate;
            if(buySize >20){
    ...
    function splitUsdtToken() private {
		uint256 thisAmount = USDT.balanceOf(address(this));
        if(thisAmount >= 10**17){
			uint256 sendAmount = thisAmount.div(5).mul(3);
