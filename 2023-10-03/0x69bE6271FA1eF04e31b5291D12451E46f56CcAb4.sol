...
/**
 *
 * @title VTPPresale contract on Binance Smart Chain
 * @author InnoPlatforms - ventopal.com
 * @notice Presale contract for BillionAir $VTP VTP
 *
 */
contract VTPPresale is Ownable, ReentrancyGuard, IVTPPresale {
    using SafeERC20 for IERC20;

    using Address for address payable;

    IERC20 public immutable VTP; // $VTP BEP-20 VTP contract
    ...
    function _buyTokens(uint256 numberOfTokens, address referrer) internal {
        ...
        if (referrer != address(0)) {
            ...
            uint256 bonusTokens = (numberOfTokens * 10) / 100;
    ...
    function claimTokens() external nonReentrant {
        ...
        VTP.safeTransfer(msg.sender, tokensToClaim);
    ...
    /**
     * @param _vtp $VTP BEP-20 VTP contract
    ...
    /**
     * Calculate the cost of buying a number of tokens (VTP)
    ...
    function endSale() external onlyOwner {
        ...
        // Transfer unsold VTP to owner
        if (unsoldTokens > 0) {
            VTP.safeTransfer(treasury, unsoldTokens);
        }
    }
