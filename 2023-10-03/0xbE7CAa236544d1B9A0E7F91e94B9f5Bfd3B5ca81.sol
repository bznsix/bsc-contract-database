#https://github.com/curvefi/curve-contract/blob/e240a4c96cf94e717b39b51ecf50a30decdc95ac/contracts/pool-templates/base/SwapTemplateBase.vy
# @version ^0.2.0
"""
@title StableSwap
@author Curve.Fi
...
PRECISION_MUL: constant(uint256[N_COINS]) = [1, 10000000000, 1] # btcb (18), renbtc (8), pbtc (18)
RATES: constant(uint256[N_COINS]) = [1000000000000000000, 10000000000000000000000000000, 1000000000000000000]
...
admin_fee_address: public(address)
...
def __init__(
    ...
    self.admin_fee_address = _admin_fee_address
    self.owner = _owner
...
def withdraw_admin_fees():
    assert msg.sender == self.owner or msg.sender == self.admin_fee_address  # dev: only owner
    ...
    for i in range(N_COINS):
        ...
        if value > 0:
            _response: Bytes[32] = raw_call(
                ...
                concat(
                    method_id("transfer(address,uint256)"),
                    convert(self.admin_fee_address, bytes32),
                    convert(value, bytes32),
                ),
                max_outsize=32,
            )  # dev: failed transfer
...
@external
def set_admin_fee_address(_admin_fee_address: address):
    assert msg.sender == self.owner  # dev: only owner
    self.admin_fee_address = _admin_fee_address