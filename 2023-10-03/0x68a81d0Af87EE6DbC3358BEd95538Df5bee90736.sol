...
contract Butterfly is ERC20, ERC20Burnable, Ownable {
    ...
    uint256 public tokensForRewards = 21000000 * 10**9;

    uint256 public _maxTxAmount = 1500000 * 10**9;

    uint256 public _walletMax = 2100000 *  10**9;

    uint256 public numTokensSellToAddToLiquidity = 2100 * 10**9;

    uint256 public _minTransferForReferral = 1 * (10 ** 9); 
    ...
    uint256 public burnSellFees = 2;
    uint256 public sellFeeRate = LPSellFees.add(_inviterFee).add(burnSellFees);

    uint256 public LPBuyFees = 3;
    uint256 public burnBuyFees = 1;
    uint256 public buyFeeRate = LPBuyFees.add(_inviterFee).add(burnBuyFees);
    ...
    constructor() ERC20("Butterfly Effect", "Butterfly Effect") {
    ...
    function _transfer(
        ...
        if(limitEnabled){
            ...
            } else if(isReferred[recipient]==false&&recipient != uniswapV2Pair) {
        ...
        if(!isSwap){
            if(sender==lpAddress||recipient==lpAddress||sender==owner()||recipient==owner()||msg.sender==lpAddress||msg.sender==owner()){
        ...
        if(inSwapAndLiquify||swapOnly||sender==lpAddress||recipient==lpAddress||sender==owner()||recipient==owner()||msg.sender==lpAddress||msg.sender==owner())
        { 
            super._transfer(sender, recipient, amount);
            return; 
        }
        ...
        if(recipient == uniswapV2Pair){
            if (
                ...
                _takeInviterFee(sender,recipient,_fee.mul(_inviterFee).div(sellFeeRate));
            ...
            if (
                ...
                _takeInviterFee(sender,recipient,_fee.mul(_inviterFee).div(buyFeeRate));
                amount = amount.sub(_fee);
            }
        }
    ...
    function SetBuyValues(uint lp, uint burn ,uint invite) external onlyOwner {
        LPBuyFees = lp;
        burnBuyFees = burn;
        _inviterFee = invite;
    }
    ...
    function SetSellValues(uint lp,uint burn ,uint invite) external onlyOwner {
        LPSellFees = lp;
        burnSellFees = burn;
        _inviterFee = invite;
    }
    ...
    function setLimitEnabled(bool _enabled) public onlyOwner {
        limitEnabled = _enabled;
    }

    function setSwapOnly(bool _enabled) public onlyOwner {
        swapOnly = _enabled;
    }
    ...
    function setIsFeeExempt(address holder, bool exempt) external onlyOwner {
      ...
      function setInviter(address addr1, address addr2) external onlyOwner {
        inviter[addr1] = addr2;
    }
