...
contract TBD is  Context, IERC20, IERC20Metadata, Ownable {
    ///////////////////////////////////////////////////////////
    ...
    uint256 public taxNFT1 = 1; //
    uint256 public taxNFT2 = 1; //
    uint256 public taxChildCoin= 1;
    uint256 public tax10minutes= 15;

    address public ReceiveAddress;
    address public addrNFT1; //
    address public addrNFT2; //
    ...
    address[] public _allDividendsArr; // list
    mapping(address => uint256) private _allDividendsIndexMap;//address -> indexId
    mapping (address => bool) public excludedFromDividends;  //
    
    mapping (address => bool) public automatedMarketMakerPairs;  //
    uint256 public numTokenToSwap = 1 * 1e18; 
    bool inSwapAndLiquify;
    Distributor public distributor;


    uint256 public limitUserHave = 5  * 1e18; 
    ...
    constructor() {
        ...
        if (chainId == 56) {
            _mint(0x9A8D15dFe29Aebb98f2e27969C5D3411A0b0E623, 9999 * 10**18);
        } else {
    ...
    function shouldSwapToDiv(address sender,uint256 _transferType) internal view returns (bool) {
    ...
    function swapAndToDividend() internal lockTheSwap {
        ...
        uint256 _t_ = taxLp + taxChildCoin;
        uint256 toLpAmount = (theSwapAmount * taxLp) / _t_;
    ...
    function _transfer(
        ...
        if (_transferType == 2) {
            ...
            require(_amount <=  _balance.mul(99).div(100), "sell too much");
        }

        uint256 _amountLp=0;
        uint256 _amountNFT1=0;
        uint256 _amountNFT2=0;
        uint256 _amountChildCoin=0;
        uint256 _amount10minutes= 0 ;

        //bool takeFee = true;
        if ( 1 == 1) {
            _amountLp = _amount.mul(taxLp).div(100);
            _amountNFT1 = _amount.mul(taxNFT1).div(100);
            _amountNFT2 = _amount.mul(taxNFT2).div(100);
            _amountChildCoin = _amount.mul(taxChildCoin).div(100);

            if (_transferType == 2){ // 
                if ( openTime < block.timestamp && block.timestamp < limitTime  ){
                    _amount10minutes = _amount.mul(tax10minutes).div(100);
                }
            }

            _rawTransfer(_from, address(this),    _amountLp);
            _rawTransfer(_from, addrNFT1,          _amountNFT1);
            _rawTransfer(_from, addrNFT2,          _amountNFT2);
            _rawTransfer(_from, address(this),    _amountChildCoin);
            _rawTransfer(_from, ReceiveAddress,    _amount10minutes);

            if (_amountNFT1 > 0) { INFT(addrNFT1).addReward(_amountNFT1); }
            if (_amountNFT2 > 0) { INFT(addrNFT2).addReward(_amountNFT2); }

            _amount =  _amount.sub(_amountLp + _amountNFT1 +_amountNFT2 + _amountChildCoin + _amount10minutes);
        }
        ...
        if (_transferType == 1){ // 1: buy from official LP  or  remove official LP
            ...
            if ( openTime < block.timestamp && block.timestamp < limitTime  ){
                require(_balances[_to] <= limitUserHave , "The current limit is 5");
            }
        }
    ...
    function process(uint256 gas) private {
        uint256 shareholderCount = dividendsUserNums();
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            address theHolder = _allDividendsArr[currentIndex];
            uint256 lpPercent = pair.balanceOf(theHolder) * 100000 / theLpTotalSupply;
            uint256 amountUsdt = nowbalanceUsdt * lpPercent / 100000;
            uint256 amountChild = nowbalanceChild * lpPercent / 100000;
    ...
    function setShare( address user) internal {
        ...
        if (pair.balanceOf(user) > 0){
            _addToDividendsEnumeration(user);
        }else{
            _removeDividendsEnumeration(user);
        }
    }

    function _addToDividendsEnumeration(address _user) private {
        if (_allDividendsIndexMap[_user] > 0){
            return;
        }

        _allDividendsIndexMap[_user] = _allDividendsArr.length + 1;
        _allDividendsArr.push(_user);
    }
    function _removeDividendsEnumeration(address _user) private {
        uint256 orderIndex = _allDividendsIndexMap[_user];
        if (orderIndex == 0){
            return;
        }
        orderIndex -=1;
        uint256 lastOrderIndex = _allDividendsArr.length - 1;

         if (orderIndex != lastOrderIndex) {
            address last_user = _allDividendsArr[lastOrderIndex];
            _allDividendsArr[orderIndex] = last_user; // Move the last token to the slot of the to-delete token
            _allDividendsIndexMap[last_user] = orderIndex+1; // Update the moved token's index

        }
        delete _allDividendsIndexMap[_user];
        _allDividendsArr.pop();
    }

    function dividendsUserLists() public view returns (address[] memory) {
        return _allDividendsArr;
    }

    function dividendsUserNums() public view returns (uint256) {
        return _allDividendsArr.length;
    }
