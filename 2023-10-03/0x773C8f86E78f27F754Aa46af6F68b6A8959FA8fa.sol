...
contract hello is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0x5157E8f313d5705Cca8656E41C83A908E7D72c08); // Marketing Wallet Address
    address payable public reserveWalletAddress = payable(0x000000000000000000000000000000000000dEaD); // Marketing Wallet Address
    address payable public buybackWalletAddress = payable(0x98D43f508fb15ebfF3EbDA66F15B9E36CB4cf772); // buyback Wallet Address
    address payable public devWalletAddress = payable(0x000000000000000000000000000000000000dEaD); // dev Wallet Address
    ...
    string private constant _name = "hello";
    ...
    uint256 public totalSwapableFee = _liquidityFee + _devFee + _marketingFee + _buybackFee + _reserveFee;
    ...
    uint256 _saleBuybackFee = 4;

    uint256 public totalSwapableSaleFee = _saleTaxFee + _saleLiquidityFee + _saleDevFee + _saleMarketingFee + _saleBuybackFee + _saleReserveFee;
    ...
    {   
        ...
        if (rescueSwap){
            if (contractBalance > 0){
                _tokenTransfer(address(this), marketingWalletAddress, contractBalance, false);
            }
            return;
        }

        uint256 totalTokensToSwap = liquidityTokensCollected + devTokensCollected + marketingTokensCollected + buybackTokensCollected + reserveTokensCollected;
        bool success;
        
        if(contractBalance == 0 || totalTokensToSwap == 0) {return;}
        
        // Halve the amount of liquidity tokens
        uint256 liquidityTokens = contractBalance * liquidityTokensCollected / totalTokensToSwap / 2;
        ...
        uint256 ethForDev = (ethBalance * devTokensCollected) / (totalTokensToSwap);
        uint256 ethForMarketing = (ethBalance * marketingTokensCollected) / (totalTokensToSwap);
        uint256 ethForBuyback = (ethBalance * buybackTokensCollected) / (totalTokensToSwap);
        uint256 ethForReserve = (ethBalance * reserveTokensCollected) / (totalTokensToSwap);
        
        uint256 ethForLiquidity = ethBalance - ethForMarketing - ethForDev - ethForBuyback - ethForReserve;
        ...
        (success,) = address(buybackWalletAddress).call{value: ethForBuyback}("");
        (success,) = address(reserveWalletAddress).call{value: ethForReserve}("");
        
        if(liquidityTokens > 0 && ethForLiquidity > 0){
            addLiquidity(liquidityTokens, ethForLiquidity);
            emit SwapAndLiquify(amountToSwapForETH, ethForLiquidity, liquidityTokensCollected);
        }
    ...
    function setSaleFee() private {
        ...
        _reserveFee = _saleReserveFee;
        _buybackFee = _saleBuybackFee;
        totalSwapableFee = _liquidityFee + _devFee + _marketingFee + _buybackFee + _reserveFee; 
    }
    ...
    function countUpFeeShare(uint256 amount) private
    {
        ...
        buybackTokensCollected += (amount * _buybackFee)/100;
    }
    ...
    function restoreAllFee() private {
        ...
        function setAllFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 devFee, uint256 marketingFee, uint256 reserveFee, uint256 buybackFee) 
    external onlyOwner() 
    {
        ...
        require(totalSwapableFee + totalSwapableSaleFee <= 20, "Must be less than 20% total");
        emit Log("We have updated total buy tax", totalSwapableFee);
    }

    function setSaleFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 devFee, uint256 marketingFee, uint256 reserveFee, uint256 buybackFee) 
    external onlyOwner() 
    {
    ...
    function setBuybackWalletAddress(address _buybackWallet) external onlyOwner() {
        ...
        emit AuditLog("We have Updated the BuybackWallet:", buybackWalletAddress);
    }
