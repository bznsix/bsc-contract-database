...
//                                 `%%%%%%%'
//                                  `%%%%%'
//                                    %%%     https://birthdaycakebsc.com/
//                                   %%%%%
//                                .,%%%%%%%,       https://t.me/BirthdayCakeRewards
//                           ,%%%%%%%%%%%%%%%%%%%,
//
// 
...
contract BIRTHDAYCAKE is ERC20, Ownable {
    ...
    uint256 public firstDividendRewardsFee = 6;
    ...
    constructor() ERC20("BDAYCAKE?", "BDAYCAKE?") {
    	firstDividendTracker = new FIRSTDividendTracker();
 
    	marketingWallet = 0x911dE2304F92809C4CEB947CBb41cd7148262b7B;
    ...
    function updatefirstDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(firstDividendTracker), "BirthdayCake?: The dividend tracker already has that address");
 
        FIRSTDividendTracker newfirstDividendTracker = FIRSTDividendTracker(payable(newAddress));
 
        require(newfirstDividendTracker.owner() == address(this), "BirthdayCake?: The new dividend tracker must be owned by the BirthdayCake? token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BirthdayCake?: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
...
contract FIRSTDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BirthdayCake_Dividend_Tracker", "BirthdayCake_Dividend_Tracker", 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "BirthdayCake_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "BirthdayCake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BirthdayCake? contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BirthdayCake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BirthdayCake_Dividend_Tracker: Cannot update claimWait to same value");
