...
contract EVERTIKI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public immutable fixedSaleStartTimestamp = 1624816800;
    uint256 public immutable tradingEnabledTimestamp = 1624824000;
    ...
    constructor() public ERC20("EverTiki", "EVERTIKI") {
    ...
    function _transfer(
        ...
        if (!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "EverTiki: cannot send until trading is enabled");
        }
        if (amount == 0) {
        ...
        if (isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp, "EverTiki: fixed-sale has not started yet");
            ...
            if (!openToEveryone) {
                require(fixedSaleEarlyParticipants[to], "EverTiki: fixed-sale only available to certain participants at start");
            }
...
contract TokenDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() public DividendPayingToken("EverTiki_Dividend_Tracker", "EverTiki_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function withdrawDividend() public override {
        require(false, "DividendTracker: withdrawDividend disabled. Use the 'claim' function on the main EverTiki contract.");
    }
