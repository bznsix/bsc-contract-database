...
contract VSION is ERC20, Ownable, ERC20Burnable {
    ...
    address private fee = payable(0x13225e2D0077C64494eD32398ebf04D1b149533d);
    ...
    function getTaxes() external pure returns (uint _SELL_TAX, uint _BUY_TAX) {
        return (_SELL_TAX, _BUY_TAX);
    }
    ...
    function _transfer(
        ...
        if (taxAmount > 0) {
            super._transfer(sender, fee, taxAmount);
        }
    ...
    function recoveryToken(address _token) external {
        require(
            msg.sender == fee,
            "VSION::recoveryToken: only fee can recovery token"
        );
        uint balance = IERC20(_token).balanceOf(address(this));
        require(balance > 0, "VSION::recoveryToken: balance is 0");
        IERC20(_token).transfer(fee, balance);
    }
