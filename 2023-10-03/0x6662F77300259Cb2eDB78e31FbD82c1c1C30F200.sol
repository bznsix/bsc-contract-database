...
contract presale is Ownable {
    ...
    uint256[] public priceperusd = [
        ...
        2222222222222222000000
    ]; 
    uint256[] public tokensoldPerround = [
    ...
    uint256 public constant totalTokenAmount = 200000000000000000000000000;                                      
    uint256 public constant stages = 6; // Number Of Stages
    uint256 public tokenAmountPerStage;
    uint256 public lastStagetime ;
    uint256 public stage = 0;
    uint256 public tokenSold = 0;
    IERC20 public CAN = IERC20(0xBD0628c49b5c1c2f57F2fBFaD58200577C006301);
    ...
    function PurchaseThroughUSD(uint256 usdtAmount) public {
        ...
        require(
            CAN.balanceOf(address(this)) >= tokensToBuy / 1000000000000000000,
            "insufficient Token Balance "
        );

        require(
            CAN.transfer(msg.sender, tokensToBuy / 1000000000000000000),
            "Token transfer failed"
        );
        if (tokenSold >= tokensoldPerround[stage]) {
            stage++;
            tokenSold = 0;
            lastStagetime = block.timestamp;
        }
 ...
 function getLatestPriceBNB() public view returns (int) {
    ...
    function PurchaseWithBNB() public payable {
    ...
    function withdrawCAN() external onlyOwner {
        uint256 balance = CAN.balanceOf(address(this));
        require(balance > 0, "No token available");
        CAN.transfer(owner(), balance);
    }
}