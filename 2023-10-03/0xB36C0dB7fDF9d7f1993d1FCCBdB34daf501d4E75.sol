/*
telegram : https://t.me/opgeportal
website : https://opge.wtf/
*/
...
contract OPGE is Context, IERC20, Ownable {
    using SafeMath for uint256;
    
    string private _name = unicode"OPGE";
    string private _symbol = unicode"OPGE";
    uint8 private _decimals = 9;

    address payable public marketing = payable(0xc921b7AB6E93C2F0F8d4C50C53d19bDaEB474976);
    address payable public vault = payable(0xb39bd0924252607Af73C5c95f4b00b7aC24BBFE2);
    ...
    uint256 public _maxTxAmount = _totalSupply.mul(25).div(1000); //2.5% 
    uint256 public _walletMax =  _totalSupply.mul(25).div(1000);  //2.5%
    ...
    constructor () {
        ...
        isExcludedFromFee[vault] = true;
        isExcludedFromFee[address(this)] = true;

        isWalletLimitExempt[owner()] = true;
        isWalletLimitExempt[marketing] = true;
        isWalletLimitExempt[vault] = true;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(isExcludedFromFee[sender] || isExcludedFromFee[recipient]) { 
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !ammMarketPair[sender] && swapAndLiquifyEnabled) 
            {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        uint256 feeBalance = this.balanceOf(vault);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountETHMarketing > 0) {
            payable(marketing).transfer(amountETHMarketing);
        }

        if(amountETHDeveloper > 0) {
            payable(vault).transfer(amountETHDeveloper);
        }         
