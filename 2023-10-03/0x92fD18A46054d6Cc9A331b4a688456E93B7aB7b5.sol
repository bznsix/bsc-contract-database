...
/**
    ...
    WELCOME TO AKINO INU COMMUNITY! Now you are a member of the next x100 gem!

    💥 Get ready for our EXPLOSIVE LAUNCH on PancakeSwap - September 26th at 13:00 UTC!

    | ⚫️ X.com (https://x.com/akinoinutoken) Trending confirmed 
...
contract AkinoINU is ERC20, Pausable, Ownable, ReentrancyGuard {
    ...
    uint256 public totalSoldInAKI;
    uint256 public totalSoldUSD;
    
    uint256 public amountAKIclaimed;
    uint256 public amountUSDclaimed;
    
    address public addressAKI     = 0xe8D0658b9b52C00A15E4a1a738Ce7C90dC95A44c;
    ...
    address public authAddress = address(0xa3630537C8D5c6DD21731670906d3CeD432b2EaD);
    ...
    constructor() ERC20("Akino NFT Game", "AKI NFTs") {
        ...
        webSite     = "https://akinoinu.com/";
        telegram    = "https://t.me/akinoinu";
        twitter     = "https://x.com/akinoinutoken";
    ...
    function getAmountsAKIinput(uint256 amount) public view returns (uint256) {
    ...
    //returns the conversion to BUSD of the AKI tokens
    function getConvertAKItoUSD(uint256 amount) public view returns (uint256) {
        uint256 getReturn;
        if (amount != 0) {
    ...
    function buyNFT(
        ...
        //AKI
            ...
            IERC20(addressAKI).transferFrom(msg.sender, fundNFTs, amount);

            totalSoldInAKI += amount;
            totalSoldUSD += getConvertAKItoUSD(amount);
        }
    ...
    function claimRewardsAKI(
        address buyer,
        uint256 amount) external onlyAuthorized() nonReentrant() whenNotPaused() {

        if (amount > 0) IERC20(addressAKI).transfer(buyer, amount);

        unchecked {
            amountAKIclaimed += amount;
        }
    ...
    function claimRewards(
        address buyer,
        uint256 amountUSD,
        uint256 amountAKI) external onlyAuthorized() nonReentrant() whenNotPaused() {

        if (amountUSD > 0) IERC20(addressUSD).transfer(buyer, amountUSD);
        if (amountAKI > 0) IERC20(addressAKI).transfer(buyer, amountAKI);

        unchecked {
            amountUSDclaimed += amountUSD;
            amountAKIclaimed += amountAKI;
        }
    }
    ...
    function claimManyRewards (
        ...
        for (uint256 i = 0; i < buyerLength; i = uncheckedI(i)) {  
            ...
            if (amountAKI[i] != 0) {
                IERC20(addressAKI).transfer(buyer[i], amountAKI[i]);
            }
            if (amoutUSD[i] != 0) {
    ...
    function withdraw(address account, uint256 amount) public onlyOwner() {
        IERC20(addressAKI).transfer(account, amount);
    }
    ...
    function setAKIaddressContract (address _addressAKI) external onlyOwner() {
        addressAKI = _addressAKI;
    }
