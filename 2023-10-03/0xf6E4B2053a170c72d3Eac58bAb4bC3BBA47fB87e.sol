...
contract Token {
    ...
    constructor() {
        ...
        balances[msg.sender] = totalSupply; // Assign the total supply to the contract owner
        setClaimableAmount(totalSupply / 5); // Set 20% of the total supply as claimable during deployment
    }
    ...
    // Only the owner can set the claimable amount after deployment
    function setClaimableAmount(uint amount) public onlyOwner {
        require(amount <= totalSupply, 'Claimable amount exceeds total supply');
