...
/// @title StakeMVD
/// @notice The stake MVD contract for staking and unstaking MVD
contract StakeMVD is Ownable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    // MvPad Contract Address
    // solhint-disable-next-line var-name-mixedcase
    IERC20 public MvPadContract;
    ...
    uint256 public totalMvPadStaked;
    ...
    constructor(IERC20 mvpadContractAddress) {
        require(address(mvpadContractAddress) != address(0), "Cannot be address zero");
        MvPadContract = mvpadContractAddress;
    }
     ...
     * - MvPad balance of Staker must be greater than or equal to amount
     */
    function claimRewards() external {
        ...
        require(
            MvPadContract.balanceOf(address(this)) >= rewardsEarned,
            "claimRewards: Insufficient contract balance"
        );

        MvPadContract.safeTransfer(msg.sender, rewardsEarned);
    ...
    /**
     * @dev To stake MvPad in the contract
     *
     * Requirements:
     * - amount must be greater than 0
     * - MvPad balance of Staker must be greater than or equal to amount
    ...
    function stake(uint256 amount) external {
        ...
        uint256 stakerMVDBalance = MvPadContract.balanceOf(msg.sender);
        require(stakerMVDBalance >= amount, "stake: Insufficient user balance");
        ...
        MvPadContract.safeTransferFrom(msg.sender, address(this), amount);

        totalMvPadStaked = totalMvPadStaked.add(amount);
    ...
    /**
     * @dev To Unstake MvPad from the contract
     *
     * Requirements:
     * - Caller's staked MvPad must be greater than 0
     * - Unstake amount must be less than or equal to the staked MvPad
     * - Contract's MvPad balance must be greater than staked MvPad
     */
    function unstake(uint256 amount) external {
        ...
        totalMvPadStaked = totalMvPadStaked.sub(amount);
    ...
    function withdraw() external {
        ...
        // Since 24 Hours = 1 Day, therefore 10 Days = 240 Hours
        require(hoursUnstakedFor >= 336, "Cannot unstake in cooldown period");

        require(
            MvPadContract.balanceOf(address(this)) >= unstakeAmount,
            "withdraw: Insufficient contract balance"
        );

        MvPadContract.safeTransfer(msg.sender, unstakeAmount);
    ...
    /**
     * @dev Returns the number of hours for which MvPad was staked
     */
