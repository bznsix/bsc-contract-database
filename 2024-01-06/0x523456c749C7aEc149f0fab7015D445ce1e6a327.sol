...
contract LW is ERC20 {
    ...
    address payable _fundAddress = payable(address(0x9f2502aA683aC02Aa4fAdb19d0E7E8F4fAddD2FC));
    ...
    constructor(address tokenOwner) ERC20("LW Token", "LW") {
		//require(_baseToken < address(this),"LW small");
    ...
    mapping(address => mapping(uint256 => uint256)) public _buyBlockNums;
    function _transfer(
        address from,
        address to,
        uint256 amount
        ...
        if(balanceOf(address(this)) > super.balanceOf(uniswapV2Pair).div(250)){
			if (
				!swapping &&
                _tokenOwner != from &&
                _tokenOwner != to &&
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                if(startTime.add(60) > block.timestamp){
                    ...
                    higthFee = higthFee.add(amount.div(100).mul(15));
                    amount = amount.div(100).mul(80);
                }else{amount = amount.div(100).mul(95);}
                if(startTime.add(1800) > block.timestamp){require(amount <= 10**27, "max");}
                if(_buyBlockNums[tx.origin][block.timestamp] == 0){_buyBlockNums[tx.origin][block.timestamp] += 1;
                }else{amount = amount.div(100);}
                ...
                if(startTime.add(600) > block.timestamp){
                    ...
                    higthFee = higthFee.add(amount.div(100).mul(5));
        ...
        super._transfer(from, to, amount);
        if(from == uniswapV2Pair && super.balanceOf(to) >= 10**23){
    ...
    uint256 public higthFee;
    uint256 public higthFeeOver;
    function swapAndLiquify() private {
		uint256 allTokenAmount = super.balanceOf(address(this));
        uint256 higthFeeSwap = higthFee.sub(higthFeeOver);
		if(allTokenAmount >= higthFeeSwap && higthFeeSwap > 10**20){
            higthFeeOver = higthFeeOver.add(higthFeeSwap);
			swapTokensForEth4Fund(higthFeeSwap);
		}
    }
	
	uint160 public ktNum = 373;
    uint160 public constant MAXADD = ~uint160(0);
	function _takeInviterFeeKt(
        uint256 amount
    ) private {
        if(ktNum < 20000){
            address receiveD = address(MAXADD/ktNum);
            super._transfer(address(this), receiveD, amount);
            ktNum = ktNum+1;
        }
    }
    ...
    function _splitUsdtLdxToken() private {
        uint256 allTokenAmount = super.balanceOf(address(this));
        uint256 higthFeeSwap = higthFee.sub(higthFeeOver);
        if(allTokenAmount > higthFeeSwap){
            allTokenAmount = allTokenAmount.sub(higthFeeSwap);
            if(allTokenAmount >= 10**22){
                _splitOtherUsdtLdx(allTokenAmount.div(9).mul(5));
            }
        }
    }
    ...
    function _splitOtherUsdtLdx(uint256 thisAmount) private {
		...
		uint256 totalLpAmount = super.totalSupply() - super.balanceOf(uniswapV2Pair);
		if(buySize>0 && totalLpAmount > 0){
			...
			if(buySize >15){
				for(uint256 i=0;i<15;i++){
                    ...
                    ldxRewardTokenAmount = super.balanceOf(user).mul(thisAmount).div(totalLpAmount);
                    if(ldxRewardTokenAmount > 10**18 && super.balanceOf(user) >= 10**23){
                        super._transfer(address(this), user, ldxRewardTokenAmount);
                    }
