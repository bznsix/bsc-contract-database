...
abstract contract Ownable is Context {
    ...
    constructor() {
        _owner = address(0x6aeE64c5BA05bd9112B26a89d47E601b71e3C50C);
...
contract KekeszToken is Ownable, IBEP20 {
    ...
    string private _name = "TesztTokenFinal";
    string private _symbol = "TTF";

    address public immutable DeadWalletAddress =
        0xf9e6FD732Bc0E27e7bc1226925f87C466601F6d9;
    address payable public MarketingWalletAddress =
        payable(0xf9e6FD732Bc0E27e7bc1226925f87C466601F6d9);   
    address payable public DevelopmentWalletAddress =
        payable(0xf9e6FD732Bc0E27e7bc1226925f87C466601F6d9);

    uint256 internal _liquidityFee = 2;
    uint256 internal _DevelopmentFee = 2;
    uint256 internal _MarketingFee = 2;

    uint256 public TotalTaxFee =
        _liquidityFee + _DevelopmentFee + _MarketingFee;
    ...
    uint256 public numTokensSellToAddToLiquidity = 1 * 10**9 * 10**_decimals;
    ...
    function UpdateTaxFees(
        ...
        TotalTaxFee = _liquidityFee + _DevelopmentFee + _MarketingFee;
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) {
            ...
            totalTax =
                (amount * (_liquidityFee + _DevelopmentFee + _MarketingFee)) /
                (feeDenominator);
        }
        ...
        if (
            ...
            if (overMinTokenBalance) {
                ...
                if (_MarketingFee + _DevelopmentFee + _liquidityFee > 0) {
                    remainingLiquidityToken =
                        (contractTokenBalance *
                            (_MarketingFee + _DevelopmentFee)) /
                        (_MarketingFee + _DevelopmentFee + _liquidityFee);
                }
                ...
                if (_liquidityFee > 0) {
                    ...
                    if (_MarketingFee + _DevelopmentFee > 0) {
    ...
    function swapTokens(uint256 _contractTokenBalance) private lockTheSwap {
        uint256 combineFee = _MarketingFee + _DevelopmentFee;
        ...
        uint256 marketingBalance = (transferredBalance * (_MarketingFee)) /
            (combineFee);
        uint256 DevelopmentBalance = (transferredBalance * (_DevelopmentFee)) /
            (combineFee);
