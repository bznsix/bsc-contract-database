...
contract Amway is IERC20, Ownable {
    ...
    uint256 public _buyFoundFee = 350;
    uint256 public _buyFlowBackFee = 50;

    uint256 public _sellFoundFee = 350;
    uint256 public _sellFlowBackFee = 50;
    ...
    string Symbol = "Amway";
    uint8 Decimals = 18;
    uint256 Supply = 100_000;
    address ReceiveAddress = address(0xa1BB7B5AA2F3d8F03e99B5715cBdd4B812C9B4d1);
    address FoundAddress = address(0x7c8b0FF6585Ac9d2a3f6e706082a9F581E76C5D7);
    ...
    constructor (){
        ...
        address lpPair = PancakeLibrary.pairFor(address(swapFactory), address(this), _weth);
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (to == _mainPair) {
                    ...
                    isRemoveLP = _isRemoveLiquidityETH(amount);
                    if (isRemoveLP > 0) {
                        takeFee = false;
                    }
                }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (foundFeeAmount > 0) {
                ...
                _takeTransfer(sender, address(this), foundFeeAmount);
            }

            if (flowBackFeeAmount > 0) {
                feeAmount += flowBackFeeAmount;
                _takeTransfer(sender, address(this), flowBackFeeAmount);
            }

            if (!inSwap && isSell) {
                uint256 balanceExtra = _balances[address(this)];
                if (balanceExtra > 0) {
    ...
    function _swapAndAddLiquidity(uint stakeAmount) internal lockTheSwap returns (bool) {
        uint foundAmount = stakeAmount * 7 / 8;
        uint flowBackAmount = (stakeAmount - foundAmount) / 2;
        address weth = _weth;
        ...
        _swapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            foundAmount + flowBackAmount,
            0,
            path,
            address(this),
            MAX
        );
        uint256 balanceExtra = address(this).balance - balanceBefore;
        uint256 foundBnb = balanceExtra * foundAmount / (foundAmount + flowBackAmount);
        if (foundBnb > 0) {
            FoundAddress.call{value : foundBnb}("");
        }
        if (balanceExtra - foundBnb > 0) {
            _swapRouter.addLiquidityETH{value : balanceExtra - foundBnb}(
                address(this),
                flowBackAmount,
                0,
                0,
