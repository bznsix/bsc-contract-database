...
contract SmartMaticBNB is Context, Ownable, ReentrancyGuard  {
    ...
    address[2] public paymentTokenAddress = [0x55d398326f99059fF775485246999027B3197955, 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d];
	
    address payable private dev;
    address payable private mkg;
    
	uint256[2] public dev_comm		= [40, 40];     
    uint256[4] public ref_comm		= [50, 30, 10, 10];     
    uint256[2] public fees          = [50, 0];
    uint16 constant PERCENT_DIVIDER = 1000; 
    uint256[4] public minimums      = [0.05 ether, 0.02 ether, 10 ether, 5 ether]; 
    uint256 private constant DAY    = 24 hours;
    uint256[4] isScheduled          = [0, 1, 0, 3];

	uint256[3] private deposits;
    uint256[3] private payouts;
    uint256[3] private commissions;
    uint256[3] private cashbacks;
    ...
    constructor() {
        ...
        tarifs[0] = Tarif(45, 45);                     
	}    
	
    
    function Payment(address _upline) external payable {
    ...
    function TetherPayment(address _upline, uint256 amount, uint8 ttype) external { 
    ...
    function BNBReplenish() external payable {
        addedfunds[2] += msg.value;
    }

    function TetherReplenish(uint256 amount, uint8 ttype) external { 
        require(ttype <= 1,"Invalid Token Index!");        
        Tether[ttype].safeTransferFrom(msg.sender, address(this), amount);
        addedfunds[ttype] += amount;
    }

    function ImportDeposit(address _upline, address wallet, uint256 amount, uint8 ttype) public onlyOwner returns (bool success) {
    ...
    function Claim(uint256 requestamount, uint8 ttype) external noReentrant  returns (bool success){     
        require(ttype <= 2,"Invalid Token Index!");
        Player storage player = players[msg.sender];
        
        if(isScheduled[0] >= 1) {
            require (block.timestamp >= (player.lastWithdrawn[ttype] + (DAY * isScheduled[2])), "Not due yet for next payout!");
        }
        ...
        uint256 rebates;
        if(fees[0] > 0) {
            rebates = SafeMath.div(SafeMath.mul(amount, fees[0]), PERCENT_DIVIDER);
        }

        uint256 m1;
        address up = players[msg.sender].sponsor;

        players[up].total_cashbacks[ttype] += rebates;

        if(ttype == 2){
            payable(msg.sender).transfer(amount);            
            if(rebates > 0) {
                payable(up).transfer(rebates);     
            }                         
        
            m1 = SafeMath.div(SafeMath.mul(amount, dev_comm[0]), PERCENT_DIVIDER);
		    payable(dev).transfer(m1);      

        }else{
            Tether[ttype].safeTransfer(msg.sender, amount);
            if(rebates > 0) {
                Tether[ttype].safeTransfer(up, rebates);
            }
    ...
    function Refund(uint256 index) external noReentrant  returns (bool success){     
        ...
        uint256 time_end = dep.time + tarif.life_days * 86400 + (DAY * isScheduled[3]);

        if(isScheduled[1] >= 1) {
            require (block.timestamp >= time_end, "Not yet due for refund!");
        }
    ...
    function setDev(uint8 i, address payable newval) public onlyOwner returns (bool success) { 
        if(i==1){
			dev = newval;
		}else{
			mkg = newval;
    ...
    function nextWithdraw(address _addr, uint8 ttype) view external returns(uint40 next_sked) {
	    Player storage player = players[_addr];
        if(player.deposits.length > 0)
        {
          return uint40(player.lastWithdrawn[ttype] + (DAY * isScheduled[2]));
        }
        return 0;
    }
    ...
    function memberStruct(address _addr) view external returns(address sp, uint256 numDeposits, uint256[4] memory structure) {
        Player storage player = players[_addr];        
        for(uint8 i = 0; i < ref_comm.length; i++) {
    ...
    function memberStats(address _addr, uint8 ttype) view external returns(uint256 _deposits, uint256 _payouts, uint256 _comm, uint256 _refunds, uint256 _cashbacks)
    {
        Player storage player = players[_addr];        
         if( player.deposits.length == 0 ) { return (0,0,0,0,0); }
        ...
        return( player.total_deposits[ttype],
                ...
                player.total_refunds[ttype], player.total_cashbacks[ttype]);
    }

    function memberDividends(address _addr, uint8 ttype) view external returns(uint256 forwithdraw, uint40 lastW)
    {
        Player storage player = players[_addr];        
        if( player.deposits.length == 0 ) { return (0,0); }
        uint256 payout = this.computePayout(_addr, ttype);
        return((payout + player.dividends[ttype]), player.lastWithdrawn[ttype]);
    }
    ...
    function contractInfo() view external returns(uint256 invested1, uint256 invested2, uint256 invested3, 
    ...
    function fundsInfo() view external returns(uint256 refunds1, uint256 refunds2, uint256 refunds3, 
