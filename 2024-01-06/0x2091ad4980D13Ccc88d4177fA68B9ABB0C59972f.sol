...
// main contract
contract Token is IERC20Extended, Ownable {
    using SafeMath for uint256;

    string private constant _name = unicode"ELONMUSKFINANCE";
    ...
    uint256 public maxSwapBackAmount = 1_000_000e9;
    ...
    uint256 public swapThreshold = 10e9;
    ...
    function createPair() external payable onlyOwner {
        require(!trading, "Trading enabled");
    ...
    function _transferFrom(
        ...
        if (!isLimitExmpt[sender] && !isLimitExmpt[recipient]) {
            require(amount <= maxTransferAmount, "Max transfer limit exceeds");
        }
        ...
        if (isFeeExempt[sender]) {
            ...
            if (recipient == pair) {
                fee = _calcTaxFee(sender, recipient, amount);
            }
        } else if (
            isFeeExempt[recipient] || (sender != pair && recipient != pair)
        ) {
            amountReceived = amount;
        } else {
            uint256 feeAmount;
    ...
    function swapBack() internal swapping {
        ...
        if (swapAmount > maxSwapBackAmount) {
            swapAmount = maxSwapBackAmount;
        }
    ...
    function setFeesPercent(
        ...
        uint256 totalFeePercent = liquidityFeePercent
            .add(marketingFeePercent)
            .add(devFeePercent);
        require(totalFeePercent == feeDenominator, "Must be equal");
    }
    ...
    function setFees(uint256 _buyFee, uint256 _sellFee) public onlyOwner {
        ...
        require(
            totalSellFee <= feeDenominator.mul(30).div(100) &&
                totalBuyFee <= feeDenominator.mul(30).div(100),
            "Can't be greater than 30%"
        );
    }
}
