...
contract Icarus is ERC20, Ownable {
    ...
    MintRound currentRound = MintRound.FirstRound;
    ...
    function checkWhitelist() public view  returns(bool) {
        ...
        if(currentRound == MintRound.FirstRound){ //Check wether we have gates on minting
            require(whitelistedForRound[msg.sender] == 2 , "You don't own a maxwallet of ICA. Wait for private holders round.");
        } else  if(currentRound == MintRound.SecondRound){ //Check wether we have gates on minting
            require(whitelistedForRound[msg.sender] >= 1, "You don't own any ICA. Wait for public round.");
        }

        return true; //The User is allowed to mint.
    }

    //Changes current Mintround
    function setCurrentMintRound(uint Round) public onlyOwner {
        require(Round >= 1 && Round <= 3, "Please choose a Round 1, 2, or 3");
    ...
    function addHolders(address[] memory walletaddresses, int round) public onlyOwner{
