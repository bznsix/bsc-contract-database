...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    */
    ...
    function getAllToken(address owner,uint256 _pagesize) public view  returns (uint256[] memory) {
           uint256[] memory tokens = new uint256[](_holderTokens[owner].length());
          if(_holderTokens[owner].length() > 0){
        ...
        function getPageToken(address owner,uint256 _limit, uint256 _pageNumber) public view  returns (uint256[] memory) {
        uint256 pageEnd = _limit * (_pageNumber + 1);
           uint256 tokensSize = _holderTokens[owner].length() >= pageEnd ? _limit : _holderTokens[owner].length().sub(_limit * _pageNumber);  
             uint256[] memory tokens = new uint256[](tokensSize);
          if(_holderTokens[owner].length() > 0){
             uint256 counter = 0;
        uint8 tokenIterator = 0;
        for (uint256 i = 0; i < _holderTokens[owner].length() && counter < pageEnd; i++) {
     
                if(counter >= pageEnd - _limit) {
                    tokens[tokenIterator] = tokenOfOwnerByIndex(owner,i);
                    tokenIterator++;
                }
                counter++;
