...
contract TUUU is ERC20 {
    ...
    address payable _fundAddress = payable(address(0x920b31985F12919994a7fA6ce89B4F031CCa3549));
    ...
    //0x024d3D840AE57f53e59F3F4f16aC25dDdfa6cb57
    constructor(address tokenOwner) ERC20("TUUU Token", "TUUU") {
		require(_baseToken < address(this),"TUUU small");
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                if(sellRate > 0){
                    ...
                    amount = amount.div(100).mul(95-sellRate);
                }else{
                    amount = amount.div(100).mul(95);
                }
			}
    ...
    function swapAndLiquify(uint256 amount) private {
		...
		if(allTokenAmount >= 10**19){
			swapTokensForEth(allTokenAmount.div(10).mul(9));
			uint256 allBnbAmount = address(this).balance;
			if(allBnbAmount > 10**10){
				_fundAddress.transfer(allBnbAmount.div(9).mul(8));
				addLiquidity(allTokenAmount.div(10), allBnbAmount.div(9));
			}
		}
    }
