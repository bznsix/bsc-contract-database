...
contract Token is ERC20 {
	...
	address private _fundAddress = address(0x10954C796eB728f46C2437C2838d8DA39D738FFD);
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(20) > block.timestamp){
                   ...
                   if(startTime.add(22) > block.timestamp){require(balanceOf(to).add(amount) <= 300 * 10**22,"max amount");}
