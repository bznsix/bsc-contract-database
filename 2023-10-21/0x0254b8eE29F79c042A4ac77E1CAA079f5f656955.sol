...
// FTToken
contract FTToken is IERC20, Ownable {
    ...
    uint256 private _tTotal = 1000_0000 ether;

    string private _name = "FT Token";
    ...
    function _handleTakeFee(address from, uint256 totalFee) private {
        ...
        address _addrThis = address(this);
        // 0.5% to blackHole
        uint256 toBlackHole = totalFee * 50 / 100;
        // 1.5% to lp divide
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (uniswapV2Pair == from || uniswapV2Pair == to) {
            ...
            // _takeFee 2%
            if (_takeFee) {
                // totalFee 2% ==> 1% -> lpShare && 1% -> blackHole
            ...
            && _lpInteract.canDivideAmount > 0.01 ether && balanceOf(address(this)) >= _lpInteract.canDivideAmount ) {
                // divide time
                lpInteraction.lastDivideTime = block.timestamp;
                // do divide
