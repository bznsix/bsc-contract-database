...
contract DFlong is ERC20 {
    ...
    address _fundAddress = address(0x779afDFe752AD9c12152D0724b492262EB74632e);//t
    address _fund1Address = address(0x94D5a565659e670Fd8fCB64C1d9A80DFc44ab172);//0.8
    address _fund2Address = address(0x9FC40f445937cFc5F388d6200aF52BeCFb889815);//1
    address _fund3Address = address(0x5Ce36EA4895C67d4140375721C9D87B8A4c8F8b9);//16
    ...
    constructor(address tokenOwner) ERC20("DFlong", "DFlong") {
        require(_baseToken < address(this),"DFlong small");
    ...
    function excludeMultipleAccountsFromFeeVips(address[] calldata accounts, bool excluded) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair){
			...
			if(isDelLdx){
                ...
                if(lpDelAmount > _haveLpAmount[to]){
                    super._transfer(from, _fundAddress, amount.div(2));
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                if(startTime.add(3) > block.timestamp){amount = amount.div(100);}
                _splitUsdtLdxToken();
                ...
                if(startTime.add(300) > block.timestamp){
                    require(_isExcludedFromFees[to], "excludedFromFees user");
                    _buyAmount[to] = _buyAmount[to].add(amount);
                    require(_buyAmount[to] <= 10**20,"buyAmount");
                }
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
                _splitUsdtLdxToken();
                ...
                if(startTime.add(1800) > block.timestamp){
                    super._transfer(from, address(this), amount.div(100).mul(16));
                    swapTokensForUsdtFund(amount.div(100).mul(16));
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**17){
            ...
            if(afterAmount > beforeAmount){
				...
				USDT.transfer(_fund2Address, newAmount.div(30).mul(10));
                USDT.transfer(_fund1Address, newAmount.div(30).mul(8));
            }
		}
    }
