...
contract EBTT is Context, IBEP20, Ownable {
    ...
    uint256 public _remainingSuply;
    ...
    constructor() {
        _totalSupply = 0;
        _maxAccountHolding = 100000 * 10 ** 18;
        mintOnDeploy();
    ...
    function mintOnDeploy() internal {
        uint256 initialMint = 3150000 * 10 ** 18;
        _mint(owner(), initialMint);
        _remainingSuply = _maxSupply - initialMint;

        uint256 pancakeMint = 535500 * 10 ** 18;
        _mint(owner(), pancakeMint);
        _remainingSuply = _remainingSuply - pancakeMint;
    }
    ...
    function mintPeriodically() external onlyOwner {
        if (timeblock) {
            require(
                block.timestamp >= lastMintingTime + 180 days,
                "Can only mint every 6 months"
            );
        }

        uint256 amount = _remainingSuply * 5 / 100;
        _mint(msg.sender, amount);

        _remainingSuply = _remainingSuply - amount;

        lastMintingTime = lastMintingTime + 180 days;
    }
