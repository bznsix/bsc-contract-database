...
contract BusdWizardFarming is Ownable, ReentrancyGuard{
    ...
    function deposit(uint256 _amount, address _referrer) external nonReentrant{
        ...
        IERC20(Busdtoken).safeTransferFrom(msg.sender,address(this),_amount);
        ...
        IERC20(Busdtoken).safeTransfer(devWallet,depositFee);
        ...
        if(investors[msg.sender].referrer != address(0)){
            ...
            IERC20(Busdtoken).transfer(investors[msg.sender].referrer, referrerAmount);
    ...
    function stopAirdropRewards() public onlyOwner{
        ...
        IERC20(Busdtoken).safeTransfer(msg.sender, _bonus[msg.sender]);
    ...
    function claimRewards() public nonReentrant {
        ...
        investors[msg.sender].lastCalculationDate = block.timestamp;
        IERC20(Busdtoken).safeTransfer(msg.sender,amountToSend);
    ...
    function getBalance() public view returns(uint256) {
        return IERC20(Busdtoken).balanceOf(address(this));
    }
...
contract WizardAirdrop is Ownable{
  
    IERC20 private wizardToken;
    bool private reentrancyGuard;
    BusdWizardFarming private busdWizardFarming;
    uint256 _tokens = 1000000000000000;
    mapping(address => uint256) public claimsCount;

    constructor(address _wizardAddress, address wizardFarming) {
        wizardToken = IERC20(_wizardAddress);
        busdWizardFarming = BusdWizardFarming(wizardFarming);
        reentrancyGuard = false;
        wizardToken.approve(address(this), _tokens);
    }
    ...
    function depositTokens(uint256 _value) public onlyOwner {
        require(wizardToken.transferFrom(msg.sender, address(this), _value), "Transfer failed");
    }

    function stopAirdrop(uint256 _value) public onlyOwner {
        require(wizardToken.transferFrom(address(this), msg.sender, _value), "Transfer failed");
    }

    function setApprove(uint256 _value) public onlyOwner {
        require(wizardToken.approve(address(this), _value), "Approval failed");
    }


    function claimTokens() public nonReentrant {
        require(busdWizardFarming._points(msg.sender) >= 2, "Insufficient points.");
      
        uint256 previousClaims = claimsCount[msg.sender];
        uint256 cal = busdWizardFarming._points(msg.sender) - (previousClaims);
        uint256 fincal = cal*675_000000000;
        claimsCount[msg.sender] += cal;
        
        wizardToken.transferFrom(address(this), msg.sender, fincal);
    }
