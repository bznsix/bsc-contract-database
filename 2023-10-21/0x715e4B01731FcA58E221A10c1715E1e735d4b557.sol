...
contract bet24 is Ownable, ReentrancyGuard{
    ...
    function buyWithLAR(uint256 packageamount)
    public 
    nonReentrant
    {
        require(msg.sender == tx.origin," External Error ");
        (uint256 token0,) = getPrice(packageamount);
        require(LAR.balanceOf(msg.sender) >= token0,"Insufficient user LAR balance!");
        require(LAR.transferFrom(_msgSender(),address(this),token0)," Approve LAR First ");
    }
    ...
    external 
    onlyOwner
    {   require(LAR.transfer(owner(),(LAR.balanceOf(address(this)))),"transfer failed!");   }
