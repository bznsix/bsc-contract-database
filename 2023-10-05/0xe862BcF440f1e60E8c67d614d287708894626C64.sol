...
contract StrategyGalaxyHedge is StrategyBase, IERC721Receiver {
    ...
    address public supplyMarket;
    address public hedgeMarket;
    address public unitroller;
    address public masterchefV3;
    address public positionManager;
    address public factoryV3;
    
    uint256 public wantBorrowBalanceStored;
    uint256 public hedgeBorrowBalanceStored;
    uint256 internal balanceOfUnderlying;

    address constant public wrappedNative = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    ...
    uint24 public nativePoolFee = 500;
    uint24 public cakePoolFee = 2500;
    ...
    struct Order {
        address want;
        address hedge;
        uint256 wantAmountToSupply;
        uint256 hedgeAmountToSupply;
        uint24 poolFee;
        int24 tickLower;
        int24 tickUpper;
        bool farming;
    ...
    struct OrderIncrease {
        uint256 tokenId;
        uint256 wantAmountToAdd;
        uint256 hedgeAmountToAdd;
        bool farming;
    }
    ...
    constructor (
        ...
        address _manager,
        address _strategist
    ) StrategyBase(_manager, _vault, _strategist) {
        supplyMarket = _supplyMarket;
        hedgeMarket = _hedgeMarket;
        unitroller = _unitroller;
        positionManager = _positionManager;
        masterchefV3 = _masterchefV3;
        factoryV3 = INonfungiblePositionManager(positionManager).factory();
        want = ICompoundToken(supplyMarket).underlying();
        hedge = ICompoundToken(hedgeMarket).underlying();
        _giveAllowances();
    }
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            ICompoundToken(supplyMarket).mint(wantBal);
            balanceOfUnderlying = ICompoundToken(supplyMarket).balanceOfUnderlying(address(this));
            emit Deposit(tx.origin, wantBal);
        }
    }

    function withdraw(uint256 amount) external whenNotPaused {
        require(msg.sender == vault, "not vault");
        ICompoundToken(supplyMarket).redeemUnderlying(amount);
        uint256 wantBal = IERC20(want).balanceOf(address(this));
    ...
    function newOrder(Order calldata order)
        external
        onlyStrategist
        returns (
            uint256 tokenId, 
            ...
            uint256 amount0,
            uint256 amount1) {
        if (order.wantAmountToSupply > 0) {
            ICompoundToken(supplyMarket).borrow(order.wantAmountToSupply);
            wantBorrowBalanceStored = ICompoundToken(supplyMarket).borrowBalanceStored(address(this));
        }
        if (order.hedgeAmountToSupply > 0) {
            ICompoundToken(hedgeMarket).borrow(order.hedgeAmountToSupply);
            hedgeBorrowBalanceStored = ICompoundToken(hedgeMarket).borrowBalanceStored(address(this));
        }
        INonfungiblePositionManager.MintParams memory params =
            INonfungiblePositionManager.MintParams({
                ...
                amount0Desired: order.wantAmountToSupply,
                amount1Desired: order.hedgeAmountToSupply,
                amount0Min: 0,
                amount1Min: 0,
        ...
        positions[tokenId] = Position({
            ...
            wantAmount: amount0,
            hedgeAmount: amount1,
            farming: order.farming
        });
        tokenIds.push(tokenId);
        if (order.farming) {
            INonfungiblePositionManager(positionManager).safeTransferFrom(address(this), masterchefV3, tokenId);
        }
    }

    function increaseOrder(OrderIncrease calldata order) 
        external 
        onlyStrategist
        returns (
            uint128 liquidity,
        ...
        if (order.wantAmountToAdd > 0) {
            ICompoundToken(supplyMarket).borrow(order.wantAmountToAdd);
            _updateBorrowBalanceStored(supplyMarket);
        }
        if (order.hedgeAmountToAdd > 0) {
            ICompoundToken(hedgeMarket).borrow(order.hedgeAmountToAdd);
            _updateBorrowBalanceStored(hedgeMarket);
        }
        INonfungiblePositionManager.IncreaseLiquidityParams memory params =
            INonfungiblePositionManager.IncreaseLiquidityParams({
                tokenId: order.tokenId,
                amount0Desired: order.wantAmountToAdd,
                amount1Desired: order.hedgeAmountToAdd,
                amount0Min: 0,
                amount1Min: 0,
                deadline: block.timestamp
            });
        if (order.farming) {
            (liquidity, amount0, amount1) = IMasterChefV3(masterchefV3).increaseLiquidity(params);
        } else {
            (liquidity, amount0, amount1) = INonfungiblePositionManager(positionManager).increaseLiquidity(params);
        }
        positions[order.tokenId].liquidity = liquidity;
        positions[order.tokenId].wantAmount += amount0;
        positions[order.tokenId].hedgeAmount += amount1;
    }

    function closeOrder(OrderClose calldata order) 
        external 
        onlyStrategist 
        returns (uint256 amount0, uint256 amount1) {
            require(order.liquidityToRemove <= positions[order.tokenId].liquidity, "invalid amount");
            INonfungiblePositionManager.DecreaseLiquidityParams memory params =
                INonfungiblePositionManager.DecreaseLiquidityParams({
                    tokenId: order.tokenId,
                    liquidity: order.liquidityToRemove,
                    amount0Min: 0,
                    amount1Min: 0,
                    deadline: block.timestamp
                });
            if (order.farming) {
                (amount0, amount1) = IMasterChefV3(masterchefV3).decreaseLiquidity(params);
            } else {
                (amount0, amount1) = INonfungiblePositionManager(positionManager).decreaseLiquidity(params);
            }
            _collect(order.tokenId);
            positions[order.tokenId].liquidity = getLiquidity(order.tokenId);
            if (positions[order.tokenId].liquidity == 0) {
                delete positions[order.tokenId];
            } else {
                positions[order.tokenId].wantAmount -= amount0;
                positions[order.tokenId].hedgeAmount -= amount1;
            }
    ...
    function harvest() 
        external 
        whenNotPaused {
            Position[] memory p = getPositions();
            for (uint256 i = 0; i < p.length; i++) {
                if (p[i].liquidity > 0) {
                    _collect(p[i].tokenId);
                }
            }
            uint256 wantBalance = IERC20(want).balanceOf(address(this));
            uint256 hedgeBalance = IERC20(hedge).balanceOf(address(this));
            uint256 cakeBalance = IERC20(cake).balanceOf(address(this));
            if (cakeBalance > 0) {
                _swap(cake, want, cakePoolFee, cakeBalance);
            }
            if (hedgeBalance > 0) {
                uint256 strategistAmount = hedgeBalance.mul(strategistFee).div(DIVISOR);
                IERC20(hedge).safeTransfer(strategist, strategistAmount);
                hedgeBalance = IERC20(hedge).balanceOf(address(this));
                if (hedgeBorrowBalanceStored >= hedgeBalance) {
                    ICompoundToken(hedgeMarket).repayBorrow(hedgeBalance);
                    _updateBorrowBalanceStored(hedgeMarket);
                }
            }
            if (wantBalance > 0) {
                uint256 toNative = wantBalance.mul(keeperFee).div(DIVISOR);
                _swap(want, wrappedNative, nativePoolFee, toNative);
                uint256 nativeBal = IERC20(wrappedNative).balanceOf(address(this));
                IERC20(wrappedNative).safeTransfer(tx.origin, nativeBal);
                uint256 strategistAmount = wantBalance.mul(strategistFee).div(DIVISOR);
                IERC20(want).safeTransfer(strategist, strategistAmount);
                wantBalance = IERC20(want).balanceOf(address(this));
                ICompoundToken(supplyMarket).mint(wantBalance);
                balanceOfUnderlying = ICompoundToken(supplyMarket).balanceOfUnderlying(address(this));
            }
    }

    function positionTokenAmounts(uint256 tokenId)
        public 
        view 
        returns (uint256 amount0, uint256 amount1) {
        (
        ...
        address poolAddress = IPancakeV3Factory(factoryV3).getPool(token0, token1, fee);
        (uint160 sqrtPriceX96, int24 tick,,,,,) = IPancakeV3Pool(poolAddress).slot0(); 
        if (tick < tickLower) {
    ...
    function getPositions() public view returns (Position[] memory values) {
        values = new Position[](tokenIds.length);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            values[i] = positions[tokenIds[i]];
        }
    }

    function swap(address tokenIn, address tokenOut, uint24 fee, uint256 amountIn) external onlyManager
        returns (uint256 amountOut) {
            amountOut = _swap(tokenIn, tokenOut, fee, amountIn);
    }

    function changeSwapRouter(address newSwapRouter) external onlyManager {
        swapRouter = newSwapRouter;
    }

    function changeNativePoolFee(uint24 newFee) external onlyManager {
        nativePoolFee = newFee;
    }

    function changeCakePoolFee(uint24 newCakeFee) external onlyManager {
        cakePoolFee = newCakeFee;
    }

    function repayBorrow(address borrowMarket, uint256 amount) public onlyManager {
        ICompoundToken(borrowMarket).repayBorrow(amount);
        _updateBorrowBalanceStored(borrowMarket);
    }
    
    function borrow(address borrowMarket, uint256 amount) external onlyManager {
        ICompoundToken(borrowMarket).borrow(amount);
        _updateBorrowBalanceStored(borrowMarket);
    }

    function supply(uint256 amount) external onlyManager {
        ICompoundToken(supplyMarket).mint(amount);
        balanceOfUnderlying = ICompoundToken(supplyMarket).balanceOfUnderlying(address(this));
    }

    function redeem(uint256 shares) external onlyManager {
        ICompoundToken(supplyMarket).redeemUnderlying(shares);
        balanceOfUnderlying = ICompoundToken(supplyMarket).balanceOfUnderlying(address(this));
    }

    function redeemUnderlying(uint256 amount) external onlyManager {
        ICompoundToken(supplyMarket).redeemUnderlying(amount);
        balanceOfUnderlying = ICompoundToken(supplyMarket).balanceOfUnderlying(address(this));
    }

    function enterMarkets(address[] memory markets) external onlyManager {
        ICompoundUnitroller(unitroller).enterMarkets(markets);
    }

    function retriveLP(uint256 tokenId) external onlyManager {
        if (positions[tokenId].farming) {
            IMasterChefV3(masterchefV3).withdraw(tokenId, msg.sender);
        } else {
    ...
    function migrateToken(address token, address newContract) external onlyOwner {
    ...
    function _updateBorrowBalanceStored(address market) internal {
        if (market == supplyMarket) {
            wantBorrowBalanceStored = ICompoundToken(supplyMarket).borrowBalanceStored(address(this));
        } else if (market == hedgeMarket) {
            hedgeBorrowBalanceStored = ICompoundToken(hedgeMarket).borrowBalanceStored(address(this));
        }
    }
    ...
    function _collect(uint256 tokenId) internal
        returns (uint256 amount0, uint256 amount1) {
            ...
            if (positions[tokenId].farming) {
                (amount0, amount1) = IMasterChefV3(masterchefV3).collect(params);
    ...
    function _giveAllowances() internal {
        IERC20(want).safeApprove(supplyMarket, type(uint256).max);
        IERC20(hedge).safeApprove(hedgeMarket, type(uint256).max);
        IERC20(cake).safeApprove(swapRouter, type(uint256).max);
        IERC20(want).safeApprove(swapRouter, type(uint256).max);
        IERC20(hedge).safeApprove(swapRouter, type(uint256).max);
        IERC20(want).safeApprove(positionManager, type(uint256).max);
        IERC20(hedge).safeApprove(positionManager, type(uint256).max);
        IERC20(want).safeApprove(masterchefV3, type(uint256).max);
        IERC20(hedge).safeApprove(masterchefV3, type(uint256).max);
    }

    function _removeAllowances() internal {
        IERC20(want).safeApprove(supplyMarket, 0);
        IERC20(hedge).safeApprove(hedgeMarket, 0);
        IERC20(cake).safeApprove(swapRouter, 0);
        IERC20(want).safeApprove(swapRouter, 0);
        IERC20(hedge).safeApprove(swapRouter, 0);
        IERC20(want).safeApprove(positionManager, 0);
        IERC20(hedge).safeApprove(positionManager, 0);
        IERC20(want).safeApprove(masterchefV3, 0);
        IERC20(hedge).safeApprove(masterchefV3, 0);
    }
}
...
contract StrategyBase is Ownable, Pausable {
    ...
    uint256 public strategistFee = 50;
    ...
    // checks that caller is the stragist.
     ...
     * @param newStrategistFee new strategist fee.
     */
    function setStrategistFee(uint256 newStrategistFee) external onlyOwner {
     ...
     /**
     * @dev Updates Keeper Fee.
     * @param newKeeperFee new keeper fee.
     */
    function setKeeperFee(uint256 newKeeperFee) external onlyOwner {
        keeperFee = newKeeperFee;
    }
