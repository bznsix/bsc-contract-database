...
contract KABOOMM is Ownable {
    ...
    uint32 public minScore = 2000;

    uint256 public BOOMMEntryFee = 20000000000000; //AKA 200K
    uint256 public BNBEntryFee =  4000000000000000; //AKA 0.04bnb
    IBEP20 public BOOMMToken = IBEP20(0x2c6A4566003A9fDD1aD2eF0BAFfdc82665f23911);
    uint8 public winnerFeePercentage = 40; // 40% of the pool rewards

    address public utilFeeReceiver = address(0xD8772f761EA003cF3C2629Ddc8300c45b587bb22);
    ...
    function enterWithBOOMM(address referrer, string memory _playerName) external {
        ...
        uint256 initialBalance = BOOMMToken.balanceOf(address(this));
        BOOMMToken.safeTransferFrom(address(msg.sender), address(this), BOOMMEntryFee);
        amountTransferred = BOOMMToken.balanceOf(address(this)) - initialBalance;

        //Make Dam sure the amount transfered is right      
        if(amountTransferred >= BOOMMEntryFee){
            startKaboomm(true, _playerName, referrer);
        }
        
    }


    function enterWithBNB(address referrer, string memory _playerName) external payable {
        ...
        // Get the fee amount
        require(msg.value >= BNBEntryFee, "Not enough funds");
        
        // Make sure is right before start the game ;)        
        if(msg.value >= BNBEntryFee){
            startKaboomm(false, _playerName, referrer);
        }
    }


    function startKaboomm(bool _useBOOMM, string memory _playerName, address referrer) private {
        address _player = _msgSender();
        ...
        if (!registeredPlayer[_player]){
            ...
            players[_player].playerName = _playerName;
            players[_player]._playerAddress = _player;
    ...
    function sendScoreAndComplete(address _player, uint32 _newScore)  external onlyTeam {
        ...
        if (currentGameId != 0 ){
            ...
            Game memory currentGame = _games[currentGameId];
            ...
            if (currentGame.finished == false){
                ...
                bool isboomm = currentGame.useBOOMM;
                string memory playerName = players[_player].playerName;

                //Make sure ranking is good 
                highestScore = refreshHighestScore();
               
                if (_newScore > highestScore){

                    // Set New highestScore
                    highestScore = _newScore;
    ...
    function payoutBOOMM(address _player) private {

        address referrer = players[_player]._referrer;
        uint32 currentGameId = players[_player]._gameId;
        string memory playerName = players[_player].playerName;
        uint256 feeToWinner = 0;
        uint256 feeToUtil = 0;
        uint256 feeToReferrer = 0;

        uint256 BOOMMContractBalance =  BOOMMToken.balanceOf(address(this));
        if(BOOMMContractBalance > 0){
            feeToWinner = BOOMMContractBalance.mul(winnerFeePercentage).div(100);
            feeToUtil = BOOMMContractBalance.mul(utilFeePercentage).div(100);
            if (referrer != address(0x0)) {
                feeToReferrer = BOOMMContractBalance.mul(referrerFeePercentage).div(100);
                BOOMMToken.safeTransfer(referrer, feeToReferrer);
            }
            BOOMMToken.safeTransfer(_player, feeToWinner);
            BOOMMToken.safeTransfer(utilFeeReceiver, feeToUtil);

        }
        emit PayoutComplete(currentGameId ,_player, playerName, true , feeToWinner);

    }

    function payoutBNB(address _player) private {
        ...
        uint256 feeToUtil = 0;
        uint256 feeToReferrer = 0;
        uint256 BNBContractBalance = address(this).balance;

        if(BNBContractBalance > 0){
            feeToWinner = BNBContractBalance.mul(winnerFeePercentage).div(100);
            feeToUtil = BNBContractBalance.mul(utilFeePercentage).div(100);
            
            if (referrer != address(0x0)) {
                feeToReferrer = BNBContractBalance.mul(referrerFeePercentage).div(100);
                referrer.call{value: feeToReferrer}("");
            }

            _player.call{value: feeToWinner}("");
            utilFeeReceiver.call{value: feeToUtil}("");
        
        }
        emit PayoutComplete(currentGameId ,_player, playerName, false, feeToWinner);
        
    }

    function addScoreRankings(address _player) private {
        for (uint32 i = 0; i < rankings.length; i++) {
            if (_player == rankings[i]._playerAddress){
                rankings[i] = players[_player];
            }
        }
    }
    ...
    function getRanking() public view returns (Player[] memory) {
        Player[] memory rankingArray = new Player[](rankings.length);
        for (uint256 i = 0; i < rankings.length; i++) {
            rankingArray[i] = rankings[i];
        }
        return rankingArray;
    }

    function refreshHighestScore() public returns (uint32) {
        for (uint32 i = 0; i < rankings.length; i++) {
            if (highestScore < rankings[i].score){
                highestScore = rankings[i].score;
            }
        }
    ...
    function getHighestScore() public view returns (uint32) {
        return highestScore;
    }
 
    function BOOMMTokenRemaining() public view returns (uint256){
        return BOOMMToken.balanceOf(address(this));
    }

    function withdrawBOOMMToken(uint256 _amount) external onlyOwner {
        require(_amount <= BOOMMToken.balanceOf(address(this)), 'not enough tokens to take out');
        BOOMMToken.safeTransfer(address(msg.sender), _amount);
    }

    function BNBTokenRemaining() public view returns (uint256){
        return address(this).balance;
    }

    function withdrawBNBToken(uint256 _amount) external onlyOwner {
        require(_amount <= address(this).balance, 'not enough tokens to take out');
    ...
    function setUtilFeeReceiver(address newReceiver, uint8 newPercentage) external onlyOwner {
