...
contract Token is ERC20 {
	...
	address private _fundAddress = address(0x670C2C572CF46b8d4ea64F168ce340486313c360);
    ...
    constructor(address tokenOwner) ERC20("DOD", "1DOD") {
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(20) > block.timestamp){
                   ...
                   if(startTime.add(22) > block.timestamp){require(balanceOf(to).add(amount) <= 10000 * 10**26,"max amount");}
