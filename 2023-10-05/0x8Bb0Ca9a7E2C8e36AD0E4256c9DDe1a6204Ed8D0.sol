...
contract AMPERInstallment is AccessControlEnumerable, WithdrawAnyERC20Token {
    ...
    EnumerableSet.UintSet private _packages;
    ...
    mapping (uint => Installment[]) private _installments;

    bytes32 public constant CONFIG_ROLE = keccak256("CONFIG_ROLE");

    // todo buyInstallment price - promo, booster(not)
    // todo collect summary data
    // todo view installments
    // todo participant users set
    // todo price (distribution) impact. Every payment and when finish.
    // todo events
    // todo claimed = true when claim expired

    constructor (address _registry, address _admin) WithdrawAnyERC20Token(_admin, false) {
        registry = IRDNRegistry(_registry);
        
        _packages.add(100 ether);
        _packages.add(200 ether);
        _packages.add(300 ether);
        _packages.add(400 ether);
        _packages.add(500 ether);
    ...
    function createInstallment(uint _package) public {
        uint userId = registry.getUserIdByAddress(msg.sender);
        require(userId > 0, "Not registered RDN user");

        require(_packages.contains(_package), "Package unavailable");

        Installment memory _installment;
        
        _installment.amount = _package;

        (uint totalPrice, uint singlePrice) = estimatePricesForPackage(userId, _package);
        _installment.totalPrice = totalPrice;
        _installment.singlePaymentPrice = singlePrice;
        (_installment.paidAmount, ) = estimator.estimateAmountOut(userId, singlePrice);
        _installment.closedPaymentsCount = 1;
        _installment.nextPaymentTime = block.timestamp + PERIOD;

        _installments[userId].push(_installment);

        token.safeTransferFrom(msg.sender, address(this), singlePrice);
        uint toReward = (singlePrice * reward) / 10**4;
        if (toReward > 0) {
    ...
    function payInstallment(uint _id, uint _paymentsCount) public {
        // count remained payments
        // if closedPaymentsCount == 12 - claim amount
        uint userId = registry.getUserIdByAddress(msg.sender);
        require(userId > 0, "Not registered RDN user");
        require(_installments[userId].length >= (_id + 1), "Wrong installment id");

        Installment storage _installment = _installments[userId][_id];
        
        require(_installment.nextPaymentTime >= block.timestamp, "Installment expired");

        uint paymentsRemained = 12 - _installment.closedPaymentsCount;
        require(paymentsRemained >= _paymentsCount, "Installment paid");

        uint amountToPay = _installment.singlePaymentPrice * _paymentsCount;

        (uint incrementPaidAmount, ) = estimator.estimateAmountOut(userId, amountToPay);

        _installment.closedPaymentsCount += _paymentsCount;

        if (_installment.closedPaymentsCount < 12) {
    ...
    // only for expired
    function claimPaid(uint _id) public {
        uint userId = registry.getUserIdByAddress(msg.sender);
        require(userId > 0, "Not registered RDN user");
        require(_installments[userId].length >= (_id + 1), "Wrong installment id");
    }

    // estimate total price for package. 
    // based on amountOut for 1 token in.
    // ignores bonuses
    function estimatePricesForPackage(uint userId, uint _package) public view returns(uint, uint) {
        (uint amountOut, ) = estimator.estimateAmountOut(userId, 1 ether);
        uint totalPrice = (_package * 10**18) / amountOut;
        uint singlePrice = totalPrice / 12;
        return (totalPrice, singlePrice);
    }

    function viewAllUserInstallments(uint userId) public view returns(Installment[] memory) {
        return _installments[userId];
    }

    function viewInstallment(uint userId, uint id) public view returns(Installment memory) {
        return _installments[userId][id];
    }

    function viewPackages() public view returns(uint[] memory) {
        uint count = _packages.length();
        uint[] memory packagesArray = new uint[](count);
        for (uint i = 0; i < count; i++) {
            packagesArray[i] = _packages.at(i);
        }
        return packagesArray;
    }

    function configAddresses(address _estimator, address _staking, address _token) external onlyRole(CONFIG_ROLE) {
        estimator = IAMPEREstimator(_estimator);
        staking = IAMPERStaking(_staking);
        token = IERC20(_token);
    }

    function configReward(uint _reward) external onlyRole(CONFIG_ROLE) {
        reward = _reward;
    }

    function addPackage(uint _package) external onlyRole(CONFIG_ROLE) {
        _packages.add(_package);
    }

    function removePackage(uint _package) external onlyRole(CONFIG_ROLE) {
...
interface IAMPEREstimator {
    ...
    function onceBonusValues(uint _userId) external view returns(uint);
...
interface IAMPERStaking {
    ...
    function balanceSummary(uint _userId) external view returns(uint, uint, uint);
