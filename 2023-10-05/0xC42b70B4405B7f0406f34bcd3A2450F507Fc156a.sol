...
interface USDTWarp {
    function withdraw(uint256 oldAmount) external returns(bool);
    function pushUser(address user) external ;
    function splitOtherToken() external ;
}
...
contract PLB is ERC20 {
    ...
    address _fundAddress = address(0x1F0B29b43d9b278eedaC6aFdBA35dB371A56dded);
    ...
    constructor(address tokenOwner) ERC20("PLB", "PLB") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), address(0x55d398326f99059fF775485246999027B3197955));
        _approve(address(this), address(0x10ED43C718714eb63d5aA57B78B54704E256024E), 10**39);
        ...
        USDT.approve(address(0x10ED43C718714eb63d5aA57B78B54704E256024E),10**30);
        uint256 total = 13 * 10**21;
	...
	function changeSwapWarp(USDTWarp _warp) public onlyOwner {
        warp = _warp;
    }
    ...
    function _transfer(
        ...
        if((from == uniswapV2Pair || to == uniswapV2Pair) && distribution){
            warp.splitOtherToken();
        }
        ...
        super._transfer(from, to, amount);
        if(to == uniswapV2Pair){warp.pushUser(from);}
    }
    
    function swapTokensForOther(uint256 tokenAmount) public {
        uint256 uamount = USDT.balanceOf(address(warp));
        ...
        warp.withdraw(uamount);
    }

    function swapAndLiquify(uint256 tokenAmount) private {
        uint256 halftoken = tokenAmount.div(10).mul(9);
        uint256 otherToken = tokenAmount.sub(halftoken);
        uint256 usdtAmount = USDT.balanceOf(address(this));
        swapTokensForOther(halftoken);
        uint256 allAmount = USDT.balanceOf(address(this));
        uint256 newAmount;
        if(allAmount > usdtAmount){
            newAmount = allAmount.sub(usdtAmount);
        }else{
            newAmount = allAmount;
        }
        if(newAmount.mul(otherToken) > 10**30){
            addLiquidityUsdt(otherToken, newAmount);
        }
    }
