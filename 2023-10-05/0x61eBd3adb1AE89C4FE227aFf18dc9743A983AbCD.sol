...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _startMintLPHolderNum = 4;
    ...
    constructor (
        ...
        _bindCondition = 123 * tokenUnit / 100000;
        _poolInfo.totalMintReward = 200000000000 * tokenUnit;
        _stableMintPerDay = 20000000 * tokenUnit;

        uint256 usdtUnit = 10 ** IERC20(UsdtAddress).decimals();
        _minInviteLPUsdt = 10 * usdtUnit;
        _maxInviteLPUsdt = 50 * usdtUnit;

        _nodeSelfLPCondition = 50 * usdtUnit;
        _nodeInviteLPCondition = 100 * usdtUnit;
    }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            } else if (_swapPairList[sender]) {uint256 buyFeeAmount = tAmount * _buyFundFee / 10000;
                address rootInvitor = _rootInvitor[recipient];
                if (address(0) == rootInvitor) {
                    rootInvitor = fundAddress;
                }
                feeAmount += buyFeeAmount;
                _takeTransfer(sender, rootInvitor, buyFeeAmount);
                require(blockNum > _txTimes[recipient] + freezeBlockNum);
                _txTimes[recipient] = blockNum;
                if (_isContract(recipient)) {
                    _blackList[recipient] = true;
                }
            } else if (_swapPairList[recipient]) {require(blockNum > _txTimes[sender] + freezeBlockNum);
                _txTimes[sender] = blockNum;
                uint256 swapFeeAmount = tAmount * _sellFundFee / 10000;
    ...
    uint256 private constant _lpMintRate = 6000;
    uint256 private constant _nodeMintRate = 400;
    uint256 private constant _lpMintInviteRate = 6000;
...
contract TRAS is AbsToken {
    ...
    18,
    10000000000,
    address(0xc1C0f085941C9b7d8b0b32ef17dC2b4f65f0C367),
    address(0xbC23De6E20DD8aABE311D892503f76C6aD748A0B)
    ){
