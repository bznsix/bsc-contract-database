...
error AISHIBA__InvalidBuyFees();
error AISHIBA__InvalidSellFees();
error AISHIBA__UnableToTransfer();

contract AISHIBA is ERC20, Ownable {
    ...
    uint public totalBuyFee = 3;
    uint public totalSellFee = 3;

    uint public buyMarketing = 2;
    uint public buyLiquidity = 1;

    uint public sellMarketing = 1;
    uint public sellLiquidity = 1;
    uint public sellBBB = 1;

    uint public totalTokensMarketing;
    uint public totalTokensLiquidity;
    uint public totalTokensBBB;

    uint public threshold = 10_000 ether;

    uint public constant MAX_SUPPLY = 10_000_000 ether;
    ...
    ) ERC20("AISHIBA", "AISHIBA") {
        _transferOwnership(0x553070c020Fa03eA9A0100040e4806063118ae74);
        _mint(owner(), MAX_SUPPLY);

        marketingWallet = payable(0x63AF1BE0D01136D73A5dA55d0fB2354cf29Db452);
        lpWallet = payable(0xAa088aD2FA6B2552043B8AaC0A9A3a8c843ff9D0);
        bbbWallet = payable(0xD358AffA0d466FbB41fDE9856ac25933007356Aa);
    ...
    function distributeSell(uint amount) internal {
        ...
        totalTokensBBB += (amount * sellBBB) / totalSellFee;
    }

    function swapAndDistributeBNB() public onlyOwner {
        swapping = true;
        ...
        uint bbbAmount = (toSwap * totalTokensBBB) / totalTokens;
        ...
        (succ, ) = bbbWallet.call{value: bbbAmount}("");
        if (!succ) emit FailTransfer(bbbWallet, bbbAmount);
    ...
    function setBuyFees(uint lpFee, uint marketingFee) internal {
        uint fullBuyFee = lpFee + marketingFee;
        if (fullBuyFee > 4) revert AISHIBA__InvalidBuyFees();
    ...
    function setSellFees(
        ...
        uint fullSellFee = lpFee + marketingFee + bbbFee;
        if (fullSellFee > 8) revert AISHIBA__InvalidSellFees();
        ...
        emit SetSellFees(totalSellFee, lpFee, marketingFee, bbbFee);
    }

    function claimLeftoverBNB() external onlyOwner {
        uint balance = address(this).balance;
        (bool succ, ) = payable(msg.sender).call{value: balance}("");
        if (!succ) revert AISHIBA__UnableToTransfer();
    }

    function setRouterApprovalToMax() external {
        _approve(address(this), address(router), type(uint256).max);
    }
