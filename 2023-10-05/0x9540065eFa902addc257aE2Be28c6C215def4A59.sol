...
contract DOCSwap is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint public feePercent = 100; // 1%s

    uint256 public addLiquidtyFee = 0;
    uint256 public removeLiquidtyFee = 0;
    ...
    function customSwapTokens(
        ...
        if (_tokenA == address(0)) {
            require(
                weth == path[0] && _tokenB == path[path.length - 1],
                "Swap: Path must be specified"
            );
        } else if (_tokenB == address(0)) {
            require(
                _tokenA == path[0] && weth == path[path.length - 1],
                "Swap: Path must be specified"
            );
        } else {
            require(
    ...
    function _swap(
        ...
        if (_tokenA == address(0)) {
            ...
            _swapETHForTokens(amountToSwap, path);
        } else {
            ...
            if (_tokenB == address(0)) {
                _swapTokensForETH(amountToSwap, path);
            } else {
    ...
    function _swapETHForTokens(uint _amount, address[] memory path) internal {
    ...
    function _swapTokensForETH(uint _amount, address[] memory path) internal {
    ...
    function setLiquidtyFee(
        uint256 _addLiquidtyFee,
        uint256 _removeLiquidtyFee
    ) external onlyOwner {
        addLiquidtyFee = _addLiquidtyFee;
        removeLiquidtyFee = _removeLiquidtyFee;
    }
    ...
    function addLiquidity(
        ...
        require(
            ...
            "Swap: Tokens are not whitelisted"
        );
        require(
        ...
        payable(feeAccount).transfer(addLiquidtyFee);
    }
    ...
    function addLiquidityETH(
        ...
        require(whitelistedTokens[_token], "Swap: Token is not whitelisted");
        require(
            _amountToken > 0 && _amountETH > 0,
            "Swap: Amounts must be greater than 0"
        );
        require(
            msg.value >= addLiquidtyFee,
        ...
        IRouter(router).addLiquidityETH{value: _amountETH}(
            _token,
            _amountToken,
            _amountToken,
            _amountETH,
    ...
    function removeLiquidity(
        ...
        require(
            msg.value >= removeLiquidtyFee,
            "Swap: not enough balance to pay fee"
        );

        address pair = IFactory(factory).getPair(_tokenA, _tokenB);
        ...
        payable(feeAccount).transfer(removeLiquidtyFee);
    }
    ...
    function removeLiquidityETH(
        ...
        address pair = IFactory(factory).getPair(_token, weth);
