...
Gold Token, is a deflationary token which concentrates on the sole purpose of rewarding its holders, by letting them earn passive rewards,
and maintaning the growth and stability of the token. Invest with Gold Token Today.
...
5% fee to marketing wallet payment


CONTACT US @

Website : goldtokenofficial.com

Telegram: https: https://t.me/goldtokenofficial

TWITTER: https://twitter.com/GOTokenofficial?s=08


Note of thanks TO https://gentokens.com/
~DEV LABS.
...
contract GoldToken is Context, IERC20, Ownable {
    ...
    address payable private _marketingWalletAddress = payable(0x28Efa331b8832A652bc8Bb7fAe5d45ef6536B982);
    uint256 private constant MAX = ~uint256(0);
    uint8 private _decimals = 9;
    uint256 private _tTotal = 1000 * 10**12 * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "GoldToken";
    ...
    uint256 private _liquidityAndMarketingFee = _liquidityFee+_marketingFee;
    ...
    // Change it later, should trigger more often
    uint256 public _numTokensSellToAddToLiquidity =  _tTotal * 1 / 10000;
    ...
    function convertLiquidityBalance(uint256 tokensToConvert) public onlyOwner {
        ...
        } else if(tokensToConvert > contractTokenBalance){
            tokensToConvert = contractTokenBalance;
        }
    ...
    function setNumTokensToLiquidity(uint256 numTokensSellToAddToLiquidity) external onlyOwner() {
        _numTokensSellToAddToLiquidity = numTokensSellToAddToLiquidity;
    }
    
    function feeSetReflect(uint256 reflectionFee) external onlyOwner() {
        _reflectionFee = reflectionFee;
    }

   
    function feeSetLiquidity(uint256 liquidityFee) external onlyOwner() {
        _liquidityFee = liquidityFee;
    }
    
    function feeSetDev(uint256 marketingFee) external onlyOwner() {
    ...
    function setMaxHoldPercent(uint256 maxWallPercent) external onlyOwner() {
    ...
    function removeAllFee() private {
        if(_reflectionFee == 0 && _liquidityAndMarketingFee == 0) return;
    ...
    function _transfer(
        ...
        if (to != owner() && to != address(this)  && to != address(0x000000000000000000000000000000000000dEaD) && to != uniswapV2Pair && to != _marketingWalletAddress){
            uint256 heldTokens = balanceOf(to);
            require((heldTokens + amount) <= _maxWalletToken,"Max wallet holding reached");
        }
        
        require(from != address(0), "from 0 address");
        require(to != address(0), "to 0 address");
        require(amount > 0, "must be value");
        
        if(from != owner() && to != owner()){
            require(amount <= _maxTxAmount, "over max transaction limit");
        }
        ...
        // cooldown timer, so a bot doesnt do quick trades! 5min gap between 2 trades
     ...
     function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
       ...
       uint256 __splitMarketing = 0;
       uint256 __tokensToMarketing = 0;
       

      
        if (_marketingFee != 0){

            __splitMarketing = 100/_liquidityAndMarketingFee*_marketingFee;
            __tokensToMarketing = contractTokenBalance*__splitMarketing/100;

            uint256 balanceBeforeMarketingWallet = address(this).balance;
            swapTokensForEth(__tokensToMarketing);
            uint256 balanceToSendMarketing = address(this).balance - balanceBeforeMarketingWallet;
            sendToMarketingWallet(balanceToSendMarketing);
        }

        if (_liquidityFee != 0){
            uint256 __tokenstoLP = contractTokenBalance-__tokensToMarketing;
    ...
    function process_TokensFromContract(uint256 tokenAmount) public onlyOwner {
        ...
        sendToMarketingWallet(balanceToSend);
    }
    //safety flush 2
    function process_BNBFromContract(uint256 bnbAmount) public onlyOwner {
        ...
        sendToMarketingWallet(bnbAmount);
    }
    }
