...
contract LatteInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LatteInuDividendTracker public dividendTracker;
    ...
    constructor() ERC20("LatteInu", "LATTE") {
        marketingWallet = payable(0xEFD2D33bEE062D14A0500e7C173b77Ed047ff4C1);
        devWallet = payable(0xEFD2D33bEE062D14A0500e7C173b77Ed047ff4C1) ;
        ...
        dividendTracker = new LatteInuDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 500000; // 0.005%
        swapTokensAtAmount = totalTokenSupply / 5000000; // 0.05%;
...
contract LatteInuDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    LatteInu public LatteInuContract;
    ...
    constructor(
        ...
        defaultToken = token;
        LatteInuContract = LatteInu(mainContract);
    ...
    function _transfer(
        ...
        require(false, "LatteInu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "LatteInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LatteInu contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(LatteInuContract);

            uint256 prevBalance = LatteInuContract.balanceOf(address(this));
            ...
            {
                uint256 received = LatteInuContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    LatteInuContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
