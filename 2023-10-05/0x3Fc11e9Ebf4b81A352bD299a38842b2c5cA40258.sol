...
contract StakingRewards is Ownable {
    ...
    constructor() {  
        ...
        AMG=0x94fA283EFF5808b5E165854CA5fC02D6C271adf4;
    }
    ...
    function setBl(address[] memory addr)public {
    ...
    function stake(address token,address token1,address token2,address up)payable  external{
        address addr=kub(AMG).upaddress(msg.sender);
        ...
        //TokenOwner[_token]
      uint buyToken=_buy(token,amount * 46 / 100,address(this));
      require(buyToken > 0);
        _addL(token,buyToken,amount*46/100,address(this));       
        ...
        if(upaddress[msg.sender] == address(0) && up != msg.sender && addr==address(0)){
           upaddress[msg.sender]=up;
           usersAddr[up].arrs.push(msg.sender);
        }else {
            if(addr != address(0)){
                upaddress[msg.sender]=addr;
                usersAddr[addr].arrs.push(msg.sender);
            }
        }
        users[token][msg.sender].tz+=amount*2;
        _team(upaddress[msg.sender],msg.sender,amount*2);
    }
    ...
    function pingLevel(address token,address to1,address to2)private  {
        if(teamsw[to1][to2].pj >0){
           IERC20(token).transfer(msg.sender,teamsw[to1][to2].pj *10 /100);
           teamsw[to1][to2].pj=0;
    ...
    function sell(address token,address token1,uint amount)public {
        ...
        if(address(this).balance < _sellc+sumFomo()){
