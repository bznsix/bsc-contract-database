...
// File: contracts/pool/TestJackpot.sol


pragma solidity =0.8.19;

contract TestJackpot is Initializable, OwnableUpgradeable {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    function lottery() public onlyCaller {
        require(ticks.length > 0 && totalProbability > 0, "No tickets");
        ...
        uint256 randomValue = uint256(keccak256(abi.encodePacked(block.timestamp,blockhash(block.number-1), tickId))) % totalProbability;
    ...
    function _addTick(address user, uint256 amount) internal {
        ...
        uint256 prob = tickProbability[tick];
        if (prob > 0) {
            ticks.push(Tick({id:tickId, user: user, probability: prob}));
            totalProbability += prob;
        }
        emit NewTick(tickId, user, prob, amount, amountUSD, block.timestamp);
    }
    ...
    function addCaller(address val) public onlyOwner() {
        require(val != address(0), "BNBDogeJackpot: val is the zero address");
        _callers.add(val);
    }

    function delCaller(address caller) public onlyOwner returns (bool) {
        require(caller != address(0), "BNBDogeJackpot: caller is the zero address");
