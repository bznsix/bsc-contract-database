...
contract LaunchPadGamerse is Ownable {
    ...
    uint256 public fourthVestingUnlockTimestamp;
    ...
    function round2Allowance(address _address) public view returns (uint256) {
        ...
        if (_hasParticipated[_address]) {
            ...
            result = round1Allowance[_address].mul(2);
        }
        return result;
    }

    uint256 public tokenTarget = 5833100 * 1E18;
    uint256 public stableTarget = 70000 * 1E18;
    uint256 public multiplier = 8333; // div per 100
    ...
    function _claimableAmount(address user) internal view returns (uint256) {
        ...
        if (block.timestamp > fourthVestingUnlockTimestamp) {
            ...
            if (_thirdVestingAmount[user] > 0) {
                ...
                uint256 _toClaim = claimable[user].mul(multiplier).div(100);
                amount = _toClaim.mul(7750).div(10000);
            }
            ...
            if (_secondVestingAmount[user] > 0) {
                ...
                uint256 _toClaim = claimable[user].mul(multiplier).div(100);
                amount = _toClaim.mul(5500).div(10000);
            }
            ...
            if (_firstVestingAmount[user] > 0) {
                ...
                uint256 _toClaim = claimable[user].mul(multiplier).div(100);
                amount = _toClaim.mul(3250).div(10000);
            }
        }
        return amount;
    }
    ...
    function claim() external returns (bool) {
        ...
        if (!_initialClaimDone[msg.sender]) {
            ...
            _firstVestingAmount[msg.sender] = _toClaim.div(4); // 22,5% at first vesting (1month later)
            _secondVestingAmount[msg.sender] = _toClaim.div(4);
            _thirdVestingAmount[msg.sender] = _toClaim.div(4);
            _fourthVestingAmount[msg.sender] = _toClaim.div(4);
        }
        if (
            _fourthVestingAmount[msg.sender] > 0 &&
            block.timestamp >= fourthVestingUnlockTimestamp
    ...
    function buyRound1Stable(uint256 _amount) external {
        ...
        if (!_hasParticipated[msg.sender]) {
            ...
            _round2Allowance[msg.sender] = round1Allowance[msg.sender].mul(2);
        }
