...
contract ZQ is ERC20 ,Ownable{
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(isRemove){
            ...
            if(removeLiquidityBurnList[sender]){
        ...
        if(isTest || (block.timestamp > lastMintTimestamp + 24 hours)){
    ...
    bool public isTest;
    function setTestFlag( bool _flag) public{
        isTest = _flag;
    }
...
contract MinerPool {
    ...
    function removeShare(address shareholder, uint256 amount) external   onlyAdmins {
        ...
        if(shares[shareholder].amount > amount) shares[shareholder].amount -= amount;
        else shares[shareholder].amount = 0;
    ...
    function updateHistory(address shareholder) internal{
        ...
        for (uint256 i = 0; i < originalLength; i++) {
            if (shareHistory[shareholder][i].timestamp + 30 < block.timestamp) {
