...
import "contracts/staking/aggregator/IStakingAggregator.sol";
import "contracts/kyc/IKYCProvider.sol";
import "contracts/vendor/ISupraOraclePull.sol";
...
contract DutchBondSale is IBondSale, IPriceSink, Ownable, Pausable, ReentrancyGuard {
    ...
    IStakingAggregator public stakingAmounts;
    ...
    function setStaking(address _stakingAmounts, uint256 _minStake) external onlyOwner {
        stakingAmounts = IStakingAggregator(_stakingAmounts);
        minStake = _minStake;
    }
    ...
    /**
     * @dev Converts an amount from 18 decimals to an equivalent amount in a token with 'desiredDecimals'
     * @param amount The amount in 18 decimals
     * @param desiredDecimals The number of decimals of the target token
     * @return The amount adjusted to the desired decimals
     */
    function from18Decimals(uint256 amount, uint8 desiredDecimals) public pure returns (uint256) {
        if (desiredDecimals == 18) {
            return amount;
        } else if (desiredDecimals > 18) {
            return amount * (10 ** (desiredDecimals - 18));
        } else {
            return amount / (10 ** (18 - desiredDecimals));
        }
    }
    ...
    function currentVestingDurationFor(uint256 _amount) public view returns (uint256) {
        uint256 discount = currentDiscountFor(_amount);
        uint256 vestingTime = (discount * 1 days) / ONE_PERCENT;
        return vestingTime;
    }

    /**
     * Calculate the amount of bond tokens for a given amount of bond tokens
    ...
    function claimBond(uint256 index) internal {
        ...
        uint256 _convertedClaimable = from18Decimals(_claimable, tokenDecimals);
        bondToken.safeTransferFrom(bondReserve, msg.sender, _convertedClaimable);
