/**
 *Telegram: https://t.me/oliveai_official
 *Twitter: https://x.com/OliveaiOfficial
*/
...
contract OliveAI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public sellAmount = 5;
    uint256 public buyAmount = 5;

    uint256 private totalSellFees;
    uint256 private totalBuyFees;

    OliveAIDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Olive AI", "OLAI") {
        marketingWallet = payable(0x786af4D322DDCf408a838E9308f8d6a514878371);
        devWallet = payable(0x5AEd006F192c5E7F8D5FeC9F9E2a86e1CbA988c8);
        ...
        dividendTracker = new OliveAIDividendTracker(
            payable(this),
            router,
            0x06Dc293c250e2fB2416A4276d291803fc74fb9B5,
            "OliveAITRACKER",
            "OliveAITRACKER"
        );
...
contract OliveAIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    OliveAI public OliveAIContract;
    ...
    constructor(
        ...
        defaultToken = token;
        OliveAIContract = OliveAI(mainContract);
    ...
    function _transfer(
        ...
        require(false, "OliveAI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "OliveAI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(OliveAIContract);

            uint256 prevBalance = OliveAIContract.balanceOf(address(this));
            ...
            {
                uint256 received = OliveAIContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    OliveAIContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
