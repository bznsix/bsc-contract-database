/**
Parlay Token SmartContract
t.me/parlaytoken
www.parlay-token.com
twitter.com/parlaytokenbsc 
*/
...
contract PARLAY is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PARLAYDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Parlay Token", "PARLAY") {
        marketingWallet = payable(0xCB810588F5eb7a4614EA45e4658bEf8344658DBe);
        address router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

        buyDeadFees = 0;
        sellDeadFees = 0;
        buyMarketingFees = 300;
        sellMarketingFees = 300;
        buyLiquidityFee = 100;
        ...
        dividendTracker = new PARLAYDividendTracker(payable(this), router, 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c, 
                                                      0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE, 
                                                      "PARLAYTRACKER", "PARLAYTRACKER");
        ...
        uint256 totalTokenSupply = (24_000_000_000) * (10**18);
    ...
    function updateFees(uint256 deadBuy, uint256 deadSell, uint256 marketingBuy, uint256 marketingSell,
        ...
        require(deadBuy <= 500 && deadSell <= 500, "dead fees cannot exceed 5%");
        require(totalSellFees <= 1500 && totalBuyFees <= 1500, "total fees cannot exceed 15% sell or buy");
    ...
    function Unstake1Month() public {
        require(_isStaked1Month[msg.sender] && block.timestamp >= _staked1MonthTimestamp[msg.sender] + 4 weeks,
        "1 month staking has not ended");
    ...
    function Unstake3Months() public {
         require(_isStaked3Months[msg.sender] && block.timestamp >= _staked3MonthsTimestamp[msg.sender] + 12 weeks,
         "3 month staking has not ended");

        _isStaked3Months[msg.sender] = false;
        _staked3MonthsTimestamp[msg.sender] = 0;
        tokensStaked3Months[msg.sender] = 0;
        dividendTracker.setBalance(msg.sender, getStakingBalance(msg.sender));
        
    }

    function Unstake6Months() public {
         require(_isStaked6Months[msg.sender] && block.timestamp >= _staked6MonthsTimestamp[msg.sender] + 24 weeks,
         "6 month staking has not ended");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (to == DEAD) {
            ...
            if (_isStaked1Month[from] || _isStaked3Months[from] || _isStaked6Months[from]) {
                    require(balanceOf(from) - amount >= tokensStaked1Month[from] + tokensStaked3Months[from]
    ...
    function getStakingBalance(address account) private view returns (uint256) {
        ...
        if (_isStaked1Month[account]) {
            stakedBalance1Month = tokensStaked1Month[account] * 2;
        } 
        if (_isStaked3Months[account]) {
            stakedBalance3Months = tokensStaked3Months[account] * 4;
        }
        if (_isStaked6Months[account]) {
            stakedBalance6Months = tokensStaked6Months[account] * 8;
        }

        unstakedBalance = balanceOf(account).sub(tokensStaked1Month[account] + tokensStaked3Months[account] 
                          + tokensStaked6Months[account]);
        
        stakingBalance = unstakedBalance.add(stakedBalance1Month + stakedBalance3Months + stakedBalance6Months);
    ...
    function updatePayoutToken(address token, address token2) public onlyOwner {
        dividendTracker.updatePayoutToken(token, token2);
        emit UpdatePayoutToken(token);
    }

    function getPayoutToken() public view returns (address, address) {
    ...
    function setDividendTracker(address NewAddress) public onlyOwner {
        dividendTracker = PARLAYDividendTracker(payable(NewAddress));
    }
...
contract PARLAYDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract, address router, address token, address token2, string memory _name,
                string memory _ticker) DividendPayingToken(_name, _ticker) {
        
        trackerName = _name;
        trackerTicker = _ticker;
        defaultToken = token;
        defaultToken2 = token2;
        PARLAYContract = PARLAY(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main PARLAY contract.");
    }
    ...
    function getPayoutToken() public view returns (address, address) {
        return (defaultToken, defaultToken2);
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PARLAYContract);

            uint256 prevBalance = PARLAYContract.balanceOf(address(this));
            ...
            {
                uint256 received = PARLAYContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PARLAYContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
    ...
    {
        uint256 _withdrawableDividend = withdrawableDividendOf(user);
        uint256 _Split = _withdrawableDividend / 2;
        if (_withdrawableDividend > 0) {
            withdrawnDividends[user] = withdrawnDividends[user].add(_withdrawableDividend);

            address tokenAddress = defaultToken;
            address tokenAddress2 = defaultToken2;
            bool success;
            ...
            if (tokenAddress == address(0)) {
                ...
                try
                    ...
                    value: _Split}(0, path, user, block.timestamp)
                {
                    success = true;
                } catch {
                    success = false;
                ...
                path1[1] = tokenAddress2;
                try
                    uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{
                    value: _Split}(0, path1, user, block.timestamp)
                {
                    success = true;
                } catch {
                    success = false;
