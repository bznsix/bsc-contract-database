...
contract TokenStaking is Ownable, ReentrancyGuard, Initializable {
    ...
    function setApyRate(uint256 newApyRate) external onlyOwner {
    require(newApyRate > 0, "APY rate must be greater than zero");
    _apyRate = newApyRate;
}
   ...
   function unstake(uint256 _amount) external nonReentrant whenTreasuryHasBalance(_amount) {
    ...
    uint256 bonus = (_amount * 10) / 100;  // 10% bonus
    uint256 amountToUnstake = _amount - feeEarlyUnstake + bonus;
     ...
     * @param _user Address of the user
     * @return Estsimated rewards for the user
     */
