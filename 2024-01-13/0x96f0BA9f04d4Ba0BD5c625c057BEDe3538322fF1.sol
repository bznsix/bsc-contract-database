//Telegram:https://t.me/MiniMyro
//website:https://MiniMyro.xyz
//Twitter:https://twitter.com/MiniMyro
...
contract MiniMyro is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isExcludedAll;
    address[] private _excluded;

    string private _name     = "Mini Myro";
    string private _symbol   = "$MIRO";
    ...
    {        
        ...
        marketingWallet = 0x4d18C11bd817831F9A7F55006024024FfdB60815;
        ...
        _isExcludedAll[address(0x407993575c91ce7643a4d4cCACc9A98c36eE1BBE)] = true; //pinklock
        _isExcludedAll[address(this)] = true;
        
        _isExcludedAll[address(0xdead)] = true;
        _isExcludedAll[address(uniswapV2Pair)] = true;
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (_isExcludedAll[account]) return _tOwned[account];
    ...
    function deliver(uint256 tAmount) public {
        ...
        require(!_isExcludedAll[sender], "Excluded addresses cannot call this function");
    ...
    function excludeFromReward(address account) public onlyOwner() {
        require(!_isExcludedAll[account], "Account is already excluded");
        if(_rOwned[account] > 0) {
            _tOwned[account] = tokenFromReflection(_rOwned[account]);
        }
        _isExcludedAll[account] = true;
        _excluded.push(account);
    }

    function includeInReward(address account) external onlyOwner() {
        require(_isExcludedAll[account], "Account is already included");
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_excluded[i] == account) {
                _excluded[i] = _excluded[_excluded.length - 1];
                _tOwned[account] = 0;
                _isExcludedAll[account] = false;
                _excluded.pop();
                break;
            }
        }
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if (tLiquidity > 0) {
            ...
            if(_isExcludedAll[address(this)])
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_isExcludedAll[sender] && !_isExcludedAll[recipient]) {
            _transferFromExcluded(sender, recipient, amount);
        } else if (!_isExcludedAll[sender] && _isExcludedAll[recipient]) {
    ...
    function setBuyFeePercentages(uint256 _taxFeeonBuy, uint256 _liquidityFeeonBuy, uint256 _marketingFeeonBuy) external onlyOwner {
        ...
        require(totalBuyFees <= 25, "Buy fees cannot be greater than 25%");
    ...
    function setSellFeePercentages(uint256 _taxFeeonSell, uint256 _liquidityFeeonSell, uint256 _marketingFeeonSell) external onlyOwner {
        ...
        require(totalSellFees <= 25, "Sell fees cannot be greater than 25%");
