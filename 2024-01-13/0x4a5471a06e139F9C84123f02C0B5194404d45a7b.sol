/**
 Telegram : https://t.me/Bitcinerate
*/
...
contract Bitcinerate is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BitcinerateDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Bitcinerate", "Bitcinerate") {
        marketingWallet = payable(0x2dff67aA1d7be131D1CE6c3dAA1811ac798C7C56);
        ListWallet = payable(0x59420E43c2bEFEbF9e6fD3253DcE42D5f7f29aeF);
        ...
        dividendTracker = new BitcinerateDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (21_000_000) * (10**18);
    ...
    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 50, "transfer fee cannot exceed 50%");
    ...
    function updateFees(
        ...
        require(totalSellFees <= 3 && totalBuyFees <= 3, "total fees cannot exceed 3% sell or 3% buy");
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (!isBuying && !isSelling) {
                ...
                if (limitsInEffect) {
                require(block.number > launchblock + 4,"you shall not pass");
...
contract BitcinerateDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Bitcinerate public BitcinerateContract;
    ...
    constructor(
        ...
        defaultToken = token;
        BitcinerateContract = Bitcinerate(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Bitcinerate_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Bitcinerate_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BitcinerateContract);

            uint256 prevBalance = BitcinerateContract.balanceOf(address(this));
            ...
            {
                uint256 received = BitcinerateContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BitcinerateContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
