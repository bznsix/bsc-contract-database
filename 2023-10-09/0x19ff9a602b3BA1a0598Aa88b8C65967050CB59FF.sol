...
contract BEAREREUM is Context, IERC20, Ownable {

    using SafeMath for uint256;

    string private _name                    = "BEAREREUM";
    ...
    uint256 _buyRewardFee                   = 4;

    uint256 _sellMarketingFee               = 5;
    uint256 _sellRewardFee                  = 4;
    ...
    address public marketingWallet          = address(0x695824B167377C31950a375705a1233A49f23D0b);
    ...
    uint256 private _totalSupply            = 10_000 * 10**_decimals;
    ...
    function setBuyFee(uint _newMarketing, uint _newReward) external onlyOwner {
        ...
        totalBuyFee             = _buyMarketingFee.add(_buyRewardFee);
        require(totalBuyFee     <= 12,"Total buy fee can't be more than 12%");
    }
    ...
    function setSellFee(uint _newMarketing, uint _newReward) external onlyOwner {
        ...
        totalSellFee            = _sellMarketingFee.add(_sellRewardFee);
        require(totalSellFee    <= 12,"Total Sell fee can't be more than 12%");
    }
    ...
    function setMaxWalletLimit(uint256 newLimit) external onlyOwner() {
        require(newLimit >= _totalSupply.mul(1).div(100), "Wallet limit can't be less than 1% of total supply");
        _walletMax = newLimit;
    }

    function setTxLimit(uint256 newLimit) external onlyOwner() {
        require(newLimit >= _totalSupply.mul(1).div(1000), "TX limit can't be less than 0.1% of total supply");
