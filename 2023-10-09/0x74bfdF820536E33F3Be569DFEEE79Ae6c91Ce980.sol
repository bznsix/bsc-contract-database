...
contract XHY is ERC20{
    ...
    constructor(address _uniswapV2Router,address _project,address _pool)ERC20("XT","XT"){
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if(!interaction || sender == address(this)) super._transfer(sender, recipient, amount);
            ...
            if(sender == uniswapV2Pair){
                ...
                luidityFee = luidityFee + amount * 1 / 100;
                
            }

            if(recipient == uniswapV2Pair){
                super._transfer(sender, pool, amount * 1 / 100);
        ...
        if(!interaction){
            if (flowFee >= 10e18 && luidityFee < 50e18){
    ...
    function processReward(uint256 gas) private {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (tokenBalance > 0) {
                amount = (luidityFee * tokenBalance) / totalLP;
                if (amount > 0 && luidityFee > amount) {
