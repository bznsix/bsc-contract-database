...
contract mPendleOFT is OFTV2, Pausable {
    ...
    function mint(address _to, uint256 _amount) public _onlyMinter {
        _mint(_to, _amount);
    }
    ...
    function setMinter(address _newMinter) public onlyOwner {
        if (_newMinter == address(0)) revert AddressZero();

        // if (Address.isContract(_newMinter)) revert MustBeContract();
