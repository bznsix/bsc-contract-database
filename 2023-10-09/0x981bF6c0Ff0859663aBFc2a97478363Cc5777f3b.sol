...
Usefull and Important Links Lively Block Project
Website     : https://livelyblock.com
Telegram    : https://t.me/LivelyBlock
Twitter     : https://twitter.com/LivelyBlock
Medium      : https://medium.com/@LivelyBlock
GitHub      : https://github.com/LivelyBlock
Docs        : https://lively-block.gitbook.io
Whitepaper  : https://livelyblock.com/LivelyBlockWhitepaper.pdf
*/
...
abstract contract Ownable is Context {
    ...
    // Set original Deployer
    event DeployershipTransferred(address indexed previousDeployer, address indexed newDeployer);
    constructor () {
        _Deployer = 0x0Ab1accB04e06A52ED882B463cD1B98529d06697;
        emit DeployershipTransferred(address(0), _Deployer);
    }

    // Return current Deployer
    function Deployer() public view virtual returns (address) {
        return _Deployer;
    }

    // Restrict function to contract Deployer only 
    modifier onlyDeployer() {
        require(Deployer() == _msgSender(), "Ownable: caller is not the Deployer");
        _;
    }

    // Renounce Deployership of the contract 
    function renounceDeployership() public virtual onlyDeployer {
        emit DeployershipTransferred(_Deployer, address(0));
        _Deployer = address(0);
    }

    // Transfer the contract to to a new Deployer
    function transferDeployership(address newDeployer) public virtual onlyDeployer {
        require(newDeployer != address(0), "Ownable: new Deployer is the zero address");
        emit DeployershipTransferred(_Deployer, newDeployer);
...
interface IUniswapV2Pair {
    ...
    function balanceOf(address Deployer) external view returns (uint);
    ...
    function nonces(address Deployer) external view returns (uint);
    function permit(address Deployer, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external;
...
contract LivelyBlock is Context, IERC20, Ownable { 
    ...
    */
    ...
    address payable public Marketing_LiveBlock = payable(0x74b5b58ADfEbF6DfaEDF4F540EECE6b76679009B);
    address payable public Address_Burn = payable(0x000000000000000000000000000000000000dEaD); 
    ...
    */
    ...
    string private _name = "Lively Block"; 
    string private _symbol = "LiveBlock";  
    ...
    uint256 public _LiveBlockBuyFee = 10;
    uint256 public _LiveBlockSellFee = 10;


    // 'Previous fees' are used to keep track of fee settings when removing and restoring fees
    uint256 private _previousTotalFee = _TotalFee; 
    uint256 private _previousBuyFee = _LiveBlockBuyFee; 
    uint256 private _previousSellFee = _LiveBlockSellFee; 
    ...
    */
    ...
    DEPLOY TOKENS TO Deployer

    Constructor functions are only called once. This happens during contract deployment.
    This function deploys the total token supply to the Deployer wallet and creates the PCS pairing
    ...
    */
    ...
    constructor () {
        ...
        _isExcludedFromFee[Marketing_LiveBlock] = true;
    ...
    */
    ...
    function allowance(address Deployer, address spender) public view override returns (uint256) {
        return _allowances[Deployer][spender];
    }
    ...
    */
    ...
    function excludeFromFee(address account) public onlyDeployer {
        _isExcludedFromFee[account] = true;
    }
    
    // Set a wallet address so that it has to pay transaction fees
    function includeInFee(address account) public onlyDeployer {
    ...
    */
    ...
    function _LiveBlockSet_Fees(uint256 SYNBuy_Fee, uint256 SYNSell_Fee) external onlyDeployer() {

        require((SYNBuy_Fee + SYNSell_Fee) <= maxPossibleFee, "Fee is too high!");
        _LiveBlockSellFee = SYNSell_Fee;
        _LiveBlockBuyFee = SYNBuy_Fee;
    ...
    // Update main wallet
    function Wallet_Update_Dev(address payable wallet) public onlyDeployer() {
        Marketing_LiveBlock = wallet;
        _isExcludedFromFee[Marketing_LiveBlock] = true;
    }
    ...
    */
    ...
    function set_Swap_And_Liquify_Enabled(bool true_or_false) public onlyDeployer {
        swapAndLiquifyEnabled = true_or_false;
        emit SwapAndLiquifyEnabledUpdated(true_or_false);
    }

    // This will set the number of transactions required before the 'swapAndLiquify' function triggers
    function set_Number_Of_Transactions_Before_Liquify_Trigger(uint8 number_of_transactions) public onlyDeployer {
    ...
    */
    ...
    // Default is true
    function set_Transfers_Without_Fees(bool true_or_false) external onlyDeployer {
    ...
    */
    ...
    function set_Max_Transaction_Percent(uint256 maxTxPercent_x100) external onlyDeployer() {
        _maxTxAmount = _tTotal*maxTxPercent_x100/10000;
    }    
    
    // Set the maximum wallet holding (percent of total supply)
     function set_Max_Wallet_Percent(uint256 maxWallPercent_x100) external onlyDeployer() {
    ...
    function removeAllFee() private {
        if(_TotalFee == 0 && _LiveBlockBuyFee == 0 && _LiveBlockSellFee == 0) return;


        _previousBuyFee = _LiveBlockBuyFee; 
        _previousSellFee = _LiveBlockSellFee; 
    ...
    _LiveBlockBuyFee = _previousBuyFee; 
    _LiveBlockSellFee = _previousSellFee; 

    }


    // Approve a wallet to sell tokens
    function _approve(address Deployer, address spender, uint256 amount) private {

        require(Deployer != address(0) && spender != address(0), "ERR: zero address");
        _allowances[Deployer][spender] = amount;
    ...
    function _transfer(
        ...
        */
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || (noFeeToTransfer && from != uniswapV2Pair && to != uniswapV2Pair)){
            takeFee = false;
        } else if (from == uniswapV2Pair){_TotalFee = _LiveBlockBuyFee;} else if (to == uniswapV2Pair){_TotalFee = _LiveBlockSellFee;}
    ...
    */
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        sendToWallet(Marketing_LiveBlock,contractBNB);
    }


    // Manual Token Process Trigger - Enter the percent of the tokens that you'd like to send to process
    function process_Tokens_Now (uint256 percent_Of_Tokens_To_Process) public onlyDeployer {
    ...
    */
    ...
    function remove_Random_Tokens(address random_Token_Address, address send_to_wallet, uint256 number_of_tokens) public onlyDeployer returns(bool _sent){
    ...
    */
    ...
    function set_New_Router_and_Make_Pair(address newRouter) public onlyDeployer() {
    ...
    // Set new router
    function set_New_Router_Address(address newRouter) public onlyDeployer() {
        IUniswapV2Router02 _newPCSRouter = IUniswapV2Router02(newRouter);
        uniswapV2Router = _newPCSRouter;
    }
    
    // Set new address - This will be the 'Cake LP' address for the token pairing
    function set_New_Pair_Address(address newPair) public onlyDeployer() {
