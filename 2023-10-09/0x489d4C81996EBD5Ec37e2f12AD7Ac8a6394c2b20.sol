...
contract BEP20 is IBEP20, Context, Ownable {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "SPDX: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "SPDX: decreased allowance below zero"));
        return true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "SPDX: transfer from the zero address");
        require(recipient != address(0), "SPDX: transfer to the zero address");

        _balances[sender] = _balances[sender].sub(amount, "SPDX: transfer amount exceeds balance");
        _balances[recipient] = _balances[recipient].add(amount);
        emit Transfer(sender, recipient, amount);
    }

    function _mint(address account, uint256 amount) internal {
        require(account != address(0), "SPDX: mint to the zero address");        
    ...
    function _burnSupply(address account, uint256 amount) internal {
        require(account != address(0), "SPDX: burn from the zero address");
        _balances[account] = _balances[account].sub(amount, "SPDX: burn amount exceeds balance");
    ...
    function _approve( address owner, address spender, uint256 amount) internal {
        require(owner != address(0), "SPDX: approve from the zero address");
        require(spender != address(0), "SPDX: approve to the zero address");
...
contract SPDX is BEP20, DexRole {
    ...
    address public SPDXFeeReceiver;
    ...
    constructor(
            address _SPDXFeeReceiver,
            address _address_init1,
            uint256 _balance_init1,
            address _address_init2,
            uint256 _balance_init2
            ) 
        BEP20('SPDX', 'SPDX') {
        SPDXFeeReceiver = _SPDXFeeReceiver;
        ...
        _initBalance(_address_init1, _balance_init1 ); // 1000000000000000000000
        _initBalance(_address_init2, _balance_init2 );
    }


    receive() external payable {}

        function setSPDXFeeReceiver(address  _address) public onlyOwner {
        SPDXFeeReceiver = _address;
    ...
    function payproject(uint256 tokens) private {
        ...
        payable(SPDXFeeReceiver).sendValue(address(this).balance);
    }
