...
contract ProSwap is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint public feePercent = 10; // 0.25%
    ...
    uint256 maxUserIndex = 0;
    mapping(uint256 => address) airdropAddresses;
    mapping(address => uint256) airdropIndex;

    constructor(address _feeAccount, address _router, address _wavax) {
        feeAccount = _feeAccount;
        router = _router;
        wavax = _wavax;
    }

    function getListedAddress(uint256 index) public view returns (address) {
        return airdropAddresses[index];
    }
    ...
    function swapV2Tokens(
        ...
        if (airdropIndex[msg.sender] == 0) {
            maxUserIndex += 1;
            airdropAddresses[maxUserIndex] = msg.sender;
            airdropIndex[msg.sender] = maxUserIndex;
        }
