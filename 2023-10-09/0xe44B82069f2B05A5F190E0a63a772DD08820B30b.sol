/**
 *Submitted for verification at BscScan.com on 2023-10-10
*/
...
abstract contract LpDividendBNB is Ownable, DexBase, ERC20 {
    ...
    uint256 public minDistribution = 1 * 1e23 ether;
    uint256 public minHoldlp = 1 * 1e20 ether;
...
abstract contract FirstLaunch is Ownable {
...
contract SCToken is ExcludedFromFeeList, LpDividendBNB, BlackList, FirstLaunch, MaxHave {
    uint256 public numTokensSellToAddToLiquidity = 1 * 1e20 ether;
    address public lpReceiver = address(0xc6cA0A15578b5b7f7864467ec3D0fc61681dDCCf);

    address constant marketAddr = address(0x5093fC6f8e2612152D5C2636466A36A1AA236C1e); //1‰

    uint256 constant marketFee = 1;
    uint256 constant lpFee = 20;
    uint256 constant liqFee = 10;
    uint256 constant burnFee = 8;
    ...
    function swapTokensForBNB(uint256 tokenAmount) internal {
        ...
        uint256 amountBNB = address(this).balance;
        uint256 marketBNB = amountBNB * marketFee / (totalFee - (liqFee / 2));
        uint256 lpBNB = amountBNB * lpFee / (totalFee - (liqFee / 2));
    ...
    constructor() payable Ownable() ERC20("DPP", "DPP", 18, 1 * 1e27 ether) {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns(uint256) {
        ...
        if (launchedAt + 20 * 20 >= block.number) {
            if (sender == uniswapV2Pair) {
                ...
                require(balanceOf[recipient] <= 10 ether, "<=10");
            } else {
                fee = amount * 90 / 1000;
                super._transfer(sender, marketAddr, fee);
            }
