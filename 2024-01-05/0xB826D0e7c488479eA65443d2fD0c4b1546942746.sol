...
contract XDRAGON_BORROW is Ownable, BEPPausable {
    ...
    uint256 public lockConvertTime;
    ...
    mapping(uint256 => ConvertsInfo) public mapConvertsInfo;
    ...
    function setLockConvertTime(uint256 _time) external onlyOwner  whenNotPaused{ 
        lockConvertTime = _time;
    }
    ...
    function convert(uint256 amount) external whenNotPaused{
        require(IERC20(XDU_ADDRESS).allowance(msg.sender,address(this)) >= amount,"The XDU allowed is not enough. You need approve more XDU");
        require(IERC20(XDU_ADDRESS).balanceOf(msg.sender) >= amount,"not enough token");
        require(amount > 0, "amount < 0");
        require(IERC20(XDA_ADDRESS).balanceOf(address(this)) >= convertLockingAmount + amount, "Not enough XDU");

        Account storage acc = mapAcc[msg.sender];
        IERC20(XDU_ADDRESS).transferFrom(msg.sender,address(this),amount);
        mapConvertsInfo[convertId] = ConvertsInfo(convertId, msg.sender, amount, 0, block.timestamp, false);
        acc.lsConvertID.push(convertId);
        convertId++;
        convertLockingAmount += amount;
    }

    function claim_convert(uint256 id) external whenNotPaused{
        require(id > 0, "id <= 0");
        ConvertsInfo storage obj = mapConvertsInfo[id];
        require(obj.addr == msg.sender, "convert addr not correct");
        require(obj.claimed == false, "convert claimed");
        uint256 xdaAmount = calXDAClaimConvert(id);
        obj.xdaAmount = xdaAmount;
        obj.claimed = true;
        convertLockingAmount -= obj.amount;
        IERC20(XDA_ADDRESS).transfer(msg.sender, xdaAmount);
    }

    function calXDAClaimConvert(uint256 id) public view returns (uint256) {
        ConvertsInfo memory obj = mapConvertsInfo[id];
        uint256 deltaTime = block.timestamp - obj.time;
        uint256 tax = 70;
        if (deltaTime >= 86400 * 7){
            tax = 0;
        } else if (deltaTime >= 86400 * 6){
            tax = 10;
        } else if (deltaTime >= 86400 * 5){
            tax = 20;
        } else if (deltaTime >= 86400 * 4){
            tax = 30;
        } else if (deltaTime >= 86400 * 3){
            tax = 40;
        } else if (deltaTime >= 86400 * 2){
            tax = 50;
        } else if (deltaTime >= 86400 * 1){
            tax = 60;
        }
        tax = 100 - tax;
        uint256 res = obj.amount * tax / 100;
        return res;
    }
    ...
    function getLsPay(address addr) public view returns (PayInfo[] memory) {
     ...
     function getLsConvert(address addr) public view returns (uint256[] memory) {
        return mapAcc[addr].lsConvertID;
    }
