...
/**
 * @title LockedStakingAllocationProvider
 * @author Finceptor
 ...
 * There is also a boost added accroding to the locked time.
 */
contract LockedStakingAllocationProvider is StakingAllocationProvider {
    using SafeERC20 for IERC20;

    uint256 boostPercent; ///@notice boost percent for locked staking. 10000 = 100%
    ...
    constructor(
        ...
        uint256 theta_,
        uint256 boost_
    ) StakingAllocationProvider(staking_, alfa_, beta_, theta_) {
        require(boost_ > 0, "LockedStakingAllocationProvider: boost must be larger than zero");
        boostPercent = boost_;
    }

    function setBoost(uint256 boost_) public onlyOwner {
        require(boost_ > 0, "LockedStakingAllocationProvider: boost must be larger than zero");
        boostPercent = boost_;
    }
    ...
    function _calculateAllocation(address account) internal view override returns (uint256) {
        uint256 boostedStake = (_staking.stakedOf(account) * (boostPercent)) / 10000;
        uint256 param1 = (boostedStake * _alfa) / 1e18;
        ...
        uint256 total = param1 + param2 + param3;
        return total;
    }
}
// SPDX-License-Identifier: BSD-3-Clause
