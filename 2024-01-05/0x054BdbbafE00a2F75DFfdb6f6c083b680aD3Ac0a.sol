...
contract TEST20 {
    ...
    constructor() {
        (name, symbol, decimals) = ITEST20Factory(msg.sender).parameters();
...
contract TEST20Factory  {

    bytes32 private constant DOMAIN_NAME = keccak256("Test");
    bytes32 public constant DOMAIN_TYPEHASH = keccak256("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)");
    ...
    event TEST20Created(address indexed sender, address indexed test20);
    ...
    function createTEST20(string memory name, string memory symbol, uint8 decimals) external onlyOwner returns (address test20) {
        parameters = Parameters({name: name, symbol: symbol, decimals: decimals});
        test20 = address(new TEST20{salt: keccak256(abi.encode(name, symbol, decimals))}());
        delete parameters;
        emit TEST20Created(msg.sender, test20);
    }
    ...
    function mint(address token, address to, uint256 amount, string memory txid, uint8[] memory v, bytes32[] memory r, bytes32[] memory s) external nonReentrant {
        ...
        require(!used[digest], "SIG_REUSED");
        used[digest] = true;
    ...
    function burn(address token, uint256 amount, string memory receiver) external payable nonReentrant {
        ...
        TEST20(token).transferFrom(msg.sender, address(this), amount);
        TEST20(token).burn(amount);
