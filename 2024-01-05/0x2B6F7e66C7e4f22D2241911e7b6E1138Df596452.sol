...
contract AlitaMiner is Ownable {
    ...
    address public chAlita=0xFCa549C229A72a020A76593A5c6b89A1d142f622;
    ...
    function setLevelsw(address addr,uint au)public {
        require(msg.sender == auditor);
        teams[addr].A1=teams[addr].A1-au;
        setBl(addr);
    }
    function setLevAddr(address addr,address to,address _dev,uint fi)public {
        require(msg.sender == auditor);
        upaddress[addr].ups=to;
        upaddress[addr].dev=_dev;
        upaddress[addr].fiv=fi;
    }
    ...
    function sendTame(address to,uint amount)internal {
            IERC20(ALITA).transfer(auditor,amount * 3 / 100);
            if(to != address(0)){
               IERC20(ALITA).transfer(to,amount * 7 / 100);
            }else {
                IERC20(ALITA).transfer(auditor,amount * 7 / 100);
            }
    }
    function stake(address token,address token1,address token2,uint _amount,address up)payable  external{
        ...
        uint fiv;
        if(token1==WBNB){
            if(upaddress[msg.sender].fiv==1){
                fiv=msg.value*bl[100]/100;
                payable (upaddress[msg.sender].dev).transfer(msg.value - fiv);
              }else {
                fiv=msg.value*bl[101]/100;
            }
            payable (address(chAlita)).transfer(fiv);
            (uint _b1,)=getPrice(ALITA,msg.value);
            amount=_b1;
            inValue=pairs(chAlita).swapBNB(ALITA,fiv);
            if(PairToken[token2] && token2!=WBNB && token2!=USDT){
               bool isok=IERC20(token2).transferFrom(msg.sender, address(chAlita), _amount);
               require(isok);
               (uint _b,)=getPrice(token2,_amount);
               amount+=_b;
            }       
        }
        if(token1==USDT){
            bool isok=IERC20(USDT).transferFrom(msg.sender, address(chAlita), _amount);
            require(isok);
            if(upaddress[msg.sender].fiv==1){
                fiv=_amount*bl[100]/100;
                IERC20(USDT).transfer(upaddress[msg.sender].dev,_amount-fiv);
            }else {
                fiv=_amount*bl[101]/100;
            }
            (,uint _u1)=getPrice(ALITA,_amount);
            amount=_u1;
            inValue=pairs(chAlita).swap(ALITA,fiv);
           if(PairToken[token2] && token2!=WBNB && token2!=USDT){
               IERC20(token2).transferFrom(msg.sender, address(chAlita), _amount);
               (,uint _u)=getPrice(token2,_amount);
               amount+=_u;
            }
        }
        if(token1==ALITA){
            bool isok=IERC20(ALITA).transferFrom(msg.sender, address(this), _amount);
            require(isok);
            amount=_amount;
            inValue=amount;
            if(PairToken[token2] && token2!=WBNB && token2!=USDT){
        ...
        require(amount >= 0.01 ether,"amount can not be 0");
        if(stakedOfTime[token][msg.sender] ==0){
           stakedOfTime[token][msg.sender]=block.timestamp;
        }else {
            if(block.timestamp > stakedOfTime[token][msg.sender] + 3600){
              claim(token,ALITA);
            }
        }
        sendTame(upaddress[up].dev,inValue);
      ...
      require(buyToken > 0);
        ...
        if(upaddress[msg.sender].ups == address(0) && up != msg.sender){
           upaddress[msg.sender].ups=up;
           upaddress[msg.sender].dev=upaddress[up].dev;
           usersAddr[up].arrs.push(msg.sender);
        }
        users[token][msg.sender].tz+=amount;
        users[token][msg.sender].out+=amount*3;
        _team(upaddress[msg.sender].ups,msg.sender,amount);
    }   
    ...
    function pingLevel(address token,address to1,address to2)private  {
        if(teamsw[to1][to2].pj >0){
           ...
           uint mmnn=getTokenPriceSell(token,teamsw[to1][to2].pj /100);
    ...
    function claim(address token,address token1) public    {
        ...
        if(users[token][msg.sender].out <= outAlita){
            ...
            stakedOfTime[token][msg.sender]=0;
            return ;
        }else {
        require(users[token][msg.sender].out > outAlita);
        }
        users[token][msg.sender].out-=outAlita;
        bool isok=IERC20(token).transfer(msg.sender,coin*50/100);
        require(isok);
        stakedOfTime[token][msg.sender]=block.timestamp;
        updateU(token,msg.sender,coin*50/100);
        _teamvalue(upaddress[msg.sender].ups,msg.sender,coin*40/100);
    }
    //PairToken
    ...
    function getPrice(address token2,uint _value) view public  returns(uint b,uint u){
