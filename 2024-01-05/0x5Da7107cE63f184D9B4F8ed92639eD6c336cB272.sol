...
abstract contract ContractToken {
    struct Sell {
        uint256 fundFee;
        uint256 wardFee;
    }
    ...
    function getNowSellMap(uint256 page, uint256 size_)
        ...
        view
        onlyOwner
        returns (NowSell[] memory nowSellList)
    {
    ...
    function getNowSellTotal()
        public
        view
        onlyOwner
        returns (uint256 amount, uint256 size_)
    {
    ...
    function claimMarket(uint256 index) external virtual MarketLock {
        ...
        TransferMarket memory transferMarket = transferMarketList[
            userCard.transferIndex
        ];
    ...
    function userPuy(uint256 index) external virtual MarketLock {
        ...
        emit PuyLog(
            index,
            transferMarket.cardNumber,
            transferMarket.owner,
            transferMarket.ownerIndex
        );
