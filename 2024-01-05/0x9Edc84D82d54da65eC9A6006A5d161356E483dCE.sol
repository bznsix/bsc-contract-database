...
contract BFX is ERC20, Ownable {
    using SafeMath for uint256;
    IUniswapV2Router02 public uniswapRouter;
    address public uniswapPair;
    uint256 public burnFee = 100;
    uint256 public tradeFee = 0;
    address public burnHolder;
    mapping(address => bool) _excludedFees;
    uint256 public minBalanceSwapToken = 1000 * 10 ** 18;
    address private tokenOwner = 0xFcDBA08deFCB221359187bc16c4e53cB6dA92177;
    address private tokenSender = 0x572AD23c6711D378A93D13Cf76c00AA4b3161981;
    bool swapIng;
    // store addresses that a automatic market maker pairs. Any transfer *to* these addresses
    // could be subject to a maximum transfer amount
    mapping(address => bool) public automatedMarketMakerPairs;

    constructor() ERC20("BFX", "BFX") {
    ...
    function setLaunch(bool flag) external onlyOwner {
        launch = flag;
    }
    ...
    function _transfer(
        ...
        if (!_excludedFees[from] && !_excludedFees[to]) {
            ...
            uint256 fees;
            uint256 fee = tradeFee;
            if (automatedMarketMakerPairs[to] || automatedMarketMakerPairs[from]) {
                fee = burnFee;
            }
            if (fee > 0) {
                uint256 _marketingFee = amount.mul(fee).div(10000);
    ...
    //    function extract(uint256 balance) external {
