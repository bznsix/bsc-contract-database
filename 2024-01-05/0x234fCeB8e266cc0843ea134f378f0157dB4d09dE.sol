    ...
    contract LandshareAPIConsumer is ChainlinkClient, ConfirmedOwner {
        ...
        constructor(address _backendAddress, string memory _apiUrl, address _RWATokenAddress) ConfirmedOwner(msg.sender) {
        ...
        function requestPropertyValue() public  {
            Chainlink.Request memory req = buildOperatorRequest(jobId, this.fulfill.selector);
            req.add('url', apiUrl);
            req.add('path', 'totalValue');
        ...
        function changeApiUrl(string memory _url) public onlyAdmin {
            apiUrl = _url;
        }
        ...
        function updatePropertyValues(string memory id, uint _value) public onlyBackend {
            for(uint256 i = 0; i < properties.length; i ++) {
                if(keccak256(abi.encodePacked(id)) == keccak256(abi.encodePacked(properties[i].id))) {
