...
contract ReflectR is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ReflectRDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ReflectR", "RTR") {
        marketingWallet = payable(0xC0eDfA6B5F801201A12b93eB8aAB1d01A0c953f0);
        ...
        dividendTracker = new ReflectRDividendTracker(payable(this), router, 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c, 
                                                      0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE, 
                                                      "ReflectRTRACKER", "RTRTRACKER");
    ...
    function updateFees(uint256 deadBuy, uint256 deadSell, uint256 marketingBuy, uint256 marketingSell,
        ...
        require(deadBuy <= 200 && deadSell <= 200, "dead fees cannot exceed 2%");
        require(totalSellFees <= 900 && totalBuyFees <= 900, "total fees cannot exceed 9% sell or buy");
    ...
    function setDividendTracker(address NewAddress) public onlyOwner {
        dividendTracker = ReflectRDividendTracker(payable(NewAddress));
    }
...
contract ReflectRDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    ReflectR public ReflectRContract;
    ...
    constructor(address payable mainContract, address router, address token, address token2, string memory _name,
        ...
        ReflectRContract = ReflectR(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main ReflectR contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ReflectRContract);

            uint256 prevBalance = ReflectRContract.balanceOf(address(this));
            ...
            {
                uint256 received = ReflectRContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ReflectRContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
