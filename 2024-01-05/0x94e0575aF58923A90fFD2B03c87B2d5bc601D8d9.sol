...
contract AllocationProxy is IAllocationProxy, Ownable {
    ...
    IAllocationProvider[] private _relativeProviders;
    ...
    constructor(
        ...
        address directProvider,
        address[] memory relativeProviders,
        ...
        for (uint256 i = 0; i < relativeProviders.length; i++) {
            require(
                relativeProviders[i] != address(0),
                "AllocationProxy: relative provider cannot be the zero address"
            );
        }

        _creditProvider = IAllocationProvider(creditProvider);
        _directProvider = IAllocationProvider(directProvider);
        _relativeProviders = new IAllocationProvider[](relativeProviders.length);
        for (uint256 i = 0; i < relativeProviders.length; i++) {
            _relativeProviders[i] = IAllocationProvider(relativeProviders[i]);
        }
    ...
    function allocationOf(address account) external view override returns (uint256) {
        ...
        uint256 _relativeAllocation = relativeAllocationOf(account);

        if (_relativeAllocation == 0) {
            return _allocation;
        }
        _allocation += _relativeAllocation;
        return _allocation;
    }
    ...
    function relativeAllocationOf(address account) public view returns (uint256) {
        uint256 _totalRelativeAllocation;
        uint256 _totalUserRelativeAllocation;

        for (uint256 i = 0; i < _relativeProviders.length; i++) {
            _totalRelativeAllocation += _relativeProviders[i].totalAllocation();
        }
        if (_totalRelativeAllocation == 0) {
            return 0;
        }

        for (uint256 i = 0; i < _relativeProviders.length; i++) {
            _totalUserRelativeAllocation += _relativeProviders[i].allocationOf(account);
        }
        return (_totalUserRelativeAllocation * _remainingAllocation) / _totalRelativeAllocation;
    }
    ...
    function _setAllocation(uint256 newTotalAllocation) internal {
        ...
        require(
            ...
            "AllocationProxy: Snapshot balances exceeds allocation."
        );
