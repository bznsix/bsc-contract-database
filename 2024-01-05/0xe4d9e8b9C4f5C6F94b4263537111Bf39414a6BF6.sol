...
abstract contract DividendFee is Owned, DexBase, ERC20 {
    ...
    uint256 public buymarketingFee = 49;
    uint256 public sellmarketingFee = 49;
    uint256 public airDropFee = 1;
    address public marketingAddr =
        address(0xF6D8B5B83d4B24b498d565fBDb319F19C2c1d5Ce);
    ...
    function swapAndToDividend() internal lockTheSwap {
        ...
        uint256 _t_ = lpFee + lpFee + buymarketingFee + sellmarketingFee;
        uint256 toLpAmount = (theSwapAmount * (lpFee + lpFee)) / _t_;
        ...
        returns (uint256)
    {
        if (sender == uniswapV2Pair) {
            uint256 dividendAmount = (amount * (lpFee + buymarketingFee)) / 1000;
            super._transfer(sender, address(this), dividendAmount);
            return dividendAmount;
        } else {
            uint256 dividendAmount = (amount * (lpFee + sellmarketingFee)) /
                1000;
            super._transfer(sender, address(this), dividendAmount);
            return dividendAmount;
        }
...
contract TangYuan is ExcludedFromFeeList, BlackList, DividendFee {
    ...
    constructor() ERC20("TangYuan", "TangYuan", 18) DividendFee() {
        ...
        returns (uint256)
    {
        ...
        uint256 airDropFeeAmount = (amount * (airDropFee)) / (1000);
        uint256 airDropEve = airDropFeeAmount / 3;
        if (airDropEve > 0) {
            address ad;
            for (int256 i = 0; i < 3; i++) {
                ...
                super._transfer(sender, ad, airDropEve);
            }
        }

        return amount - divAmount - (airDropEve * 3);
    }
