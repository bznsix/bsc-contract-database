...
contract BTC3 is Ownable, ReentrancyGuard {
    ...
    event Upgrade(  uint256 amount);
     constructor(
        ...
        USDT = IERC20(0xe7bf4B443f6cC53CB009BA8Da5318F9c13b9A900);
        BTC = IERC20(0x780d5467F21Ba29ea54150e90A343b1F40A06597);
        router = ISwapRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        pair = IUniswapV2Pair(
            IPancakeFactory(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73).getPair(0xe7bf4B443f6cC53CB009BA8Da5318F9c13b9A900, 0x780d5467F21Ba29ea54150e90A343b1F40A06597)
        );
        publicReward = new PublicReward(address(USDT), address(BTC));
...
// invite邀请
    ...
    function MOLPlprice()public  onlyOperator{
       ...
       if(Nprice<Lprice*102/100){
        ...
        require(BTC.transfer(deadAddress, HDAmount), "dead error");
...
// LP 分红   矿池分红池
    address public LPfenhong = 0x33F247E0772Bc5D1d88a7447db562F874ac38bAB;
    // 平台
    address public PTAddress= 0x83d5f5D77dAC8Bdac1Ecf0CF311BC96731D9d4B9;
    ...
    function _getUSDTOut(uint256 HDAmount) private view returns (uint256) {
     ...
     function getUSDTOut( 
        ...
        uint256 USDTAmount = (LPAmount * USDTTotalBalance) /
            lpTotalSupply;
        return USDTAmount*9/10;
    }
    ...
    function _addLiquidity(uint256 USDTAmount) private {
        ...
        BTC.mint(address(this), HDAmount);
        ...
        if (b < HDAmount) BTC.destroy(HDAmount - b);
    }
    ...
    function _buy(uint256 amount) private {
        ...
        uint256 addLPAmount = (amount * 67) / 100;
        ...
        // uint256  addLP =  ADLPBalance - LPBalance;
         pair.transfer(msg.sender,   (addLP * 35)/ 67);


        pair.transfer(deadAddress,  (addLP * 24)/ 67);




        uint256 buyAmount = (amount * 26) / 100;
        uint256 HDAmount = _buyHD(buyAmount);
   
        require(
            BTC.transfer(address(deadAddress), HDAmount),
            "transfer error"
        );

        if(user.team.leader!=deadAddress){
        require(
        pair.transfer( user.team.leader, (addLP * 5) / 68),
            "transfer error"
        );
    }  

      require(
            pair.transfer( LPfenhong, (addLP * 3) / 68),
            "transfer error"
        ); 

        // 平台
        require(
            USDT.transfer(PTAddress, (amount * 4) / 100),
            "transfer error"
        );
   // 平台矿池分红
        require(
            USDT.transfer(LPfenhong, (amount * 3) / 100),
            "transfer error"
        );
 ...
 // 平台LP分红
        ...
        if(addLPB>0){
            pair.transfer(LPfenhong, addLPB);
        }
    ...
    function _sell(uint256 amount) private returns (uint256) {
        ...
        _buyHD(USDTToBuy);//进入分红池
        uint256 LPBalance = BTC.balanceOf(address(this));
        BTC.transfer(deadAddress,LPBalance);

        USDT.transfer(LPfenhong, (USDTAmount * 2) / 100) ;//进入分红池
        USDT.transfer(PTAddress, (USDTAmount * 3) / 100) ;//进入平台
...
// 30添加流动性
         uint256 addLPAmount = (price * 68) / 100;
...
// 30%买币
        uint256 HDAmount = _buyHD(price*25/100);//进入分红池
  require(
        BTC.transfer( address(1), HDAmount ),"transfer error");

// 直接上级20%

    if(user.team.leader!=deadAddress){
        require(
        pair.transfer( user.team.leader, (addLP * 25) / 68),
            "transfer error"
        );
    }   

        require(
            USDT.transfer(PTAddress, 4 * price/100),
            "transfer error"
        );
        ...
        while (user.team.top != address(0) && i < 10) {
            ...
            if (user.level < userLevel) {
                ...
                pair.transfer( user.team.top, (addLP * 8) / 680);
            }
            ...
            //     pair.transferFrom(LPfenhong, user.team.top, price/200);
            // }
            ++i;
        }

        pair.transfer( deadAddress, addLP*24/68);
        pair.transfer( LPfenhong, addLP*3/68);
        pair.transfer( PTAddress, addLP*8/68);



        uint256 LPBalance = pair.balanceOf(address(this));
        pair.transfer( LPfenhong, LPBalance);

        emit Upgrade( addLP*8/68);
