...
contract BTC3 is Ownable, ReentrancyGuard {
    ...
    function _buy(uint256 amount) private {
        ...
        uint256 addLPAmount = (amount * 63) / 100;
        ...
        pair.transfer(msg.sender,   (addLP * 40)/ 63);
        pair.transfer(LPfenhong,  (addLP * 15)/ 63);
        uint256 buyAmount = (amount * 30) / 100;
        ...
        require(
        pair.transfer( user.team.leader, (addLP * 5) / 63),
            "transfer error"
        );
    }  

      require(pair.transfer( LPfenhong, (addLP * 3) / 63),"transfer error"); 
    ...
    function upgrade() public   {
        ...
        uint256 addLPAmount = (price * 63) / 100;
        _addLiquidity(addLPAmount);
        uint256 addLP = pair.balanceOf(address(this));
        uint256 HDAmount = _buyHD(price*30/100);
        require( BTC.transfer( deadAddress, HDAmount ),"transfer error");
        if(user.team.leader!=deadAddress){
        require(
        pair.transfer( user.team.leader, (addLP * 27) / 63),
            "transfer error"
        );  
        }   
        require(
        ...
        while (user.team.top != address(0) && i < 10) {
            ...
            {
                pair.transfer( user.team.add, (addLP * 8) / 630);
            }
            ++i;
        }
        pair.transfer( PTAddress, addLP*8/63);
        pair.transfer( LPfenhong, addLP*17/63);
        pair.transfer( LPfenhong, addLP*3/63);
        uint256 LPBalance = pair.balanceOf(address(this));
        if (LPBalance>1e18){
            pair.transfer( LPfenhong, LPBalance-1e18);
            airpro(LPBalance);
        }
        emit Upgrade( addLP*8/63);
    }
