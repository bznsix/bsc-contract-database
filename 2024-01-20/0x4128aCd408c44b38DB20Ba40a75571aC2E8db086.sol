...
address constant rewardAddress = isLive ? 0x3bCF7029Bd30Ab88e8c24A8121aE90A4221ddb64 : 0xFa60D973F7642B748046464e165A65B7323b0DEE;
...
contract rewardsToken is IBEP20, Auth {
    ...
    string constant _name = "Baby Jelly";
    string constant _symbol = "BJELLY";
    ...
    uint256 public maxBuyPercent = 3;
    ...
    uint256 public reflectionSellFee   = 4;
    uint256 public liquiditySellFee    = 1;
    uint256 public devSellFee          = 1;
    uint256 public revenueSellFee      = 0;
    ...
    function swapBack() internal swapping {
        ...
        uint256 dynamicLiquidityFee = isOverLiquified(targetLiquidity, targetLiquidityDenominator) ? 0 : liquidityFee; // 10
        uint256 amountToLiquify = swapThreshold.mul(dynamicLiquidityFee).div(totalFee).div(2); // 250
        uint256 amountToSwap = swapThreshold.sub(amountToLiquify); // 750

        // DistributedliquidityFee = liquidityFee / 2
        // uint256 amountToLiquify = swapThreshold.mul(DistributedliquidityFee).div(totalFee);
        // uint256 totalFee = totalBuyFee + totalSellFee - DistributedliquidityFee;
        // uint256 amountBNBDev = amountBNB.mul(devBuyFee + devSellFee).div(totalFee); // 
        // uint256 amountBNBLiquidity = amountBNB.mul(DistributedliquidityFee).div(totalFee);

        // 1000 * 10 / 20 = 500 for liquidity
        // 500 / 2 = 250 as tokens and 250 as bnb
        // 1000 - 500 = 500 for dev
        // 500 + 250 = 750 tokens to be swapped to bnb
        // 750 tokens made 1 bnb
        // 250 worth of tokens should be taken out of the 1 bnb
        ...
        uint256 totalBNBFee = totalFee.sub(dynamicLiquidityFee.div(2)); // 15
