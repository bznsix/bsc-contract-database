...
error stake__Failed();
contract MoonStake is Context, Ownable, ReentrancyGuard {
    ...
    Percent.percent internal percentThreeMonth = Percent.percent(75, 1000);//7.5%
    Percent.percent internal percentSixMonth = Percent.percent(10, 100);//10%
    Percent.percent internal percentTwelveMonth = Percent.percent(15, 100);//15%

    event stakingBuyed(address indexed from, uint256 tokenAmount, uint256 bonusAmount, uint256 duration);
    event getWithdraw(address indexed beneficiary, uint256 indexed withdrawAmount, uint256 date);

    constructor(IERC20 token) {
      _token = token;
      _users = new UsersStorage();

      _durationStake = 31104000; //- 360days in seconds
      _periodStake = 86400; //- 1 days in seconds

    }

    function setStake(uint256 tokenAmount, uint256 period) public payable nonReentrant {
      address beneficiary = _msgSender();
      uint256 bonus;

      if (!_users.contains(beneficiary)) {
        _users.insertUser(beneficiary);
      }

      require(_token.balanceOf(beneficiary) >= tokenAmount, "Not enough tokens");
      require(_token.allowance(beneficiary,address(this)) >= tokenAmount, "Please allow fund first");
      bool success = _token.transferFrom(beneficiary, address(this), tokenAmount);
      if (!success) {
        revert stake__Failed();
      } else {
          uint256 duration = 0;

          if (period == 3) {
              duration = 3*2592000;
              bonus = percentThreeMonth.mul(tokenAmount)*3;
          } else if (period == 6) {
              duration = 6*2592000;
              bonus = percentSixMonth.mul(tokenAmount)*6;
          } else if (period == 12) {
              duration = 12*2592000;
              bonus = percentTwelveMonth.mul(tokenAmount)*12;
          } else {
            revert stake__Failed();
          }

          totalPendingStake += tokenAmount;
          totalPendingBonus += bonus;

          _users.insertStaking(beneficiary, tokenAmount, bonus, duration);

          emit stakingBuyed(beneficiary, tokenAmount, bonus, duration);
        ...
        for (uint256 i = 0; i < _users.getTotalStaking(beneficiary); i++) {
          ...
          if (stake.active) {
            if (currentTime >= stake.endDate) {
              stakingAmount = stake.value.sub(stake.released);
              stakingAmount += stake.body;
              totalPendingStake -= stake.body;
              ...
              stakingAmount = stake.value.mul(stakingSeconds).div(stake.endDate-stake.startDate);
    ...
    function changeStorageOwner(address newAddress) public onlyOwner {
