...
contract Astromusk_Sale is ReentrancyGuard, Ownable {
    ...
    uint256 public constant maxUsers = 100000;
    ...
    mapping(address => bool) public discountedUser;
    ...
    constructor(
        ...
        discountPercent = _discountPercent; //add two zeroes extra , recommended in case to work with floating values
        percentDivider = 100; //to zeroes extra to sort out floating point values , eg:2.5 or something
        registrationFee = _registrationFee * (10**USDTInterface.decimals());
    }
    ...
    function changeRegistrationFee(uint256 _feeWithoutDecimals)
        public
        onlyOwner
    {
        registrationFee = _feeWithoutDecimals * (10**USDTInterface.decimals());
    }
    ...
    function changeDiscountPercent(uint256 _discountPercent)
        external
        onlyOwner
    {
        ...
        returns (bool)
    {
        ...
        if (presaleId == 1 && registeredUser[msg.sender]) {
            //adding respective tokens according to desired discount percent or bonus
            tokens += (tokens * discountPercent) / percentDivider;
            if(discountedUser[msg.sender]){
            //returning token equivalent to registration fee of user which is 10 dollars (usdt)
            tokens += usdtToTokens(presaleId, registrationFee);
            //making discounted user false after paying to prevent the reclaim and reentrancy
            discountedUser[msg.sender] = false;
            }
        }
        ...
        IERC20(SaleToken).transfer(msg.sender, tokens);
    ...
    function register() external {
        require(!registeredUser[msg.sender], "You are already registered");
         require(registeredUsersCount < maxUsers, "Maximum number of users reached");
         require(presaleId == 0, "registertation time is over");
        (bool success, ) = address(USDTInterface).call(
            abi.encodeWithSignature(
                "transferFrom(address,address,uint256)",
                _msgSender(),
        ...
        discountedUser[msg.sender] = true;
