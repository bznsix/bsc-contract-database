...
contract GdsToken is IERC20, Ownable {
    ...
    constructor(
        ...
        uint256 total = 10000000000*1e18;
        _tTotal = total;

        fundAddress = payable(addressParams[0]);
        require(!isContract(fundAddress), "fundaddress is a contract ");
        fundAddress2 = payable(addressParams[5]);
        require(!isContract(fundAddress2), "fundaddress2 is a contract ");
    ...
    function swapTokenForFund(
        ...
        if (currencyIsEth) {
            ...
            if (fundAmount > 0 && fundAddress != address(0)) {
                fundAddress.transfer(fundAmount / 2);
                fundAddress2.transfer(fundAmount / 2);
            }
    ...
    function disableKillBatchBot(bool s) public onlyOwner {
        enableKillBatchBots = s;
    }

    function disableSwapLimit(bool s) public onlyOwner {
        enableSwapLimit = s;
    }

    function disableWalletLimit(bool s) public onlyOwner {
        enableWalletLimit = s;
    }

    function disableChangeTax(bool s) public onlyOwner {
        enableChangeTax = s;
    }
