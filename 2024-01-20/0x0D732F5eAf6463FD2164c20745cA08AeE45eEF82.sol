...
contract AngelMiner is Ownable, ReentrancyGuard {
    ...
    IERC20 public immutable angel =
        IERC20(0x7f3D83a96E62b18Fb5358F3e70113A18ce41329a);
    IERC20 public immutable usdt =
        IERC20(0x55d398326f99059fF775485246999027B3197955);
    IERC20 public immutable mainPair =
        IERC20(0x72177B36E5569D6FB7d6E616d8e253e4D89f0166);
    ...
    constructor(address luc, address sta) {
        luckyPool = luc;
        staticPool = sta;

        angel.safeApprove(address(uniswapV2Router), MAX);
    ...
    function deposit(uint256 uAmount) external nonReentrant returns (bool) {
        ...
        uint256 tokenBalanceBefore = angel.balanceOf(address(this));
        ...
        uint256 tokenBalanceExtra = angel.balanceOf(address(this)).sub(
            tokenBalanceBefore
        );
        if (tokenBalanceExtra > 0 && canBurn) {
            angel.safeTransfer(address(0xdead), tokenBalanceExtra);
        }
        if (angel.balanceOf(address(this)) > 0) {
            if (angel.balanceOf(msg.sender) == 0) {
                angel.safeTransfer(msg.sender, 10000);
            }
        }
        mainPair.safeTransfer(msg.sender, lpBalanceExtra.mul(60).div(100));
        mainPair.safeTransfer(staticPool, lpBalanceExtra.mul(10).div(100));
        mainPair.safeTransfer(luckyPool, lpBalanceExtra.mul(4).div(100));
    ...
    function withdraw(uint256 lpAmount) external nonReentrant returns (bool) {
        ...
        uint256 lpBalanceBefore = angel.balanceOf(address(this));
        uint256 usdtBalanceBefore = usdt.balanceOf(address(this));
        _removeLiquidity(lpAmount);
        uint lpBalanceExtra = angel.balanceOf(address(this)).sub(lpBalanceBefore);
        angel.safeTransfer(address(0xdead), lpBalanceExtra);
    ...
    function batchTransfer(address token, address[] memory addrs, uint256[] memory amounts) external onlyOwner returns (bool) {
        ...
        for (uint i = 0; i < len; i++) {
            IERC20(token).safeTransfer(addrs[i], amounts[i]);
        }
        return true;
    }
    ...
    function _swapAndAddLiquidity(uint stakeAmount) internal returns (bool) {
        ...
        uint balanceBefore = angel.balanceOf(address(this));
        _swapCakeForToken(address(this), stakingTokenAmount);
        uint swapedAmount = angel.balanceOf(address(this)).sub(balanceBefore);
    ...
    function setLuckyPool(address luc) external onlyOwner {
        luckyPool = luc;
    }

    function setStaticPool(address sta) external onlyOwner {
        staticPool = sta;
    }
