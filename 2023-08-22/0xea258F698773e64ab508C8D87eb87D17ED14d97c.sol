...
// Developed by WW4 TEAM - World War 4 Ⓒ
...
contract WORLDWAR4TOKEN is ERC20, Ownable {
    ...
    constructor(
        address marketingWallet_, address gameWallet_
    )
        payable
        ERC20("World War 4 Token", "WW4")
    {
        uint256 totalSupply_ = 1_000_000_000;
        buyFee = 3;
        sellFee = 3;

        marketingWallet = address(marketingWallet_);
        gameWallet = address(gameWallet_);

        autoSwapEnable = true;

        walletToWalletTransferWithoutFee = true;

        address router_ = 0x10ED43C718714eb63d5aA57B78B54704E256024E; // main-net
    ...
    function updateFees(uint256 _buyFee, uint256 _sellFee) external onlyOwner {
        require(_buyFee <= 6 && _sellFee <= 6, "Fees must be less than 6%");
        buyFee = _buyFee;
        sellFee = _sellFee;
    ...
    function changeWallets(address _marketingWallet, address _gameWallet)
        external
        onlyOwner
    {
        marketingWallet = address(_marketingWallet);
        gameWallet = address(_gameWallet);
    ...
    function _transfer(
        ...
        if (canSwap && !swapping && !automatedMarketMakerPairs[from] && autoSwapEnable) {
            swapping = true;
            uint256 amountT1 = contractTokenBalance;
            swapAndSendMarketing(amountT1);
            swapping = false;
        }
        ...
        if (takeFee) {
            ...
            uint256 gameReward = fees / 5;
            uint256 feesRemain = fees - gameReward;
            amount = amount - gameReward - feesRemain;
            super._transfer(from, address(this), feesRemain);
            super._transfer(from, gameWallet, gameReward);
        }
