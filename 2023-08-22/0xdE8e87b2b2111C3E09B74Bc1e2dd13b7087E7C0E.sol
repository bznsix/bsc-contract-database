 ...
 */
contract DMCToken is Context, IERC20, IERC20Metadata {
     ...
     */
    constructor() {
        _mint(_msgSender(), 520000000 * 10**18);
    }
...
contract RewardPool is Ownable {
    ...
    struct PoolInfo {
        IERC20 lpToken; // deposit lp token
        IERC20 token; // mining token
        uint256 tokenPerBlock; // token per block

        uint256 allocPoint; // token alloc point

        // running data
        uint256 lastRewardBlock;
        uint256 accTokenPerShare;
        uint256 lpBalance; // deposit lp token balance

        // reward token
        IERC20 rewardToken;
        uint256 maxReward;
        uint256 sendedReward;
        uint256 ratio; // ratio/100
    }
    ...
    constructor(uint256 _startBlock, IReferrer _referrer) {
    ...
    function add(PoolInfo memory _pools, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
        _pools.lastRewardBlock =
            block.number > startBlock ? block.number : startBlock;
        totalAllocPoint = totalAllocPoint.add(_pools.allocPoint);
        _pools.lpBalance = 0;
        poolInfo.push(_pools);
    }
    ...
    function set(
        ...
        poolInfo[_pid].maxReward = _maxReward;
        poolInfo[_pid].sendedReward = _sendedReward;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        require(block.timestamp > 1657886400, "Not withdraw");
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function emergencyWithdrawToken(uint256 _pid, uint256 _amount) public onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];

        pool.token.transfer(msg.sender, _amount);
    }

    function getTokenAmountWithLp(IERC20 _lp, IERC20 _token, uint256 _lpAmount) public view returns(uint256) {
    ...
    function lpReward(uint256 _pid, uint256 _amount) internal {
        ...
        if (pool.sendedReward >= pool.maxReward) {
            return;
        }
        if (pool.ratio == 0) {
            return;
        ...
        uint256 rewardByLp = getTokenAmountWithLp(pool.lpToken, pool.rewardToken, _amount);

        address _ref = referrer.getReferrer(msg.sender);
        uint256 refReward = rewardByLp.mul(pool.ratio).div(100);
        if (pool.sendedReward.add(refReward) > pool.maxReward) {
            refReward = pool.maxReward.sub(pool.sendedReward);
        }
        safeTokenTransfer(pool.rewardToken, _ref, refReward);
        pool.sendedReward = pool.sendedReward.add(refReward);
    }
 ...
 * allowances. See {IERC20-approve}.
 */
contract ODDToken is Context, IERC20, IERC20Metadata {
    mapping(address => uint256) private _balances;
    ...
    string private _name = "ODDToken";
    string private _symbol = "ODD";
     ...
     * construction.
     */
    constructor(address minerAddr, address mintAddr) {
        _mint(mintAddr, 2500 * 10**18);
        _mint(minerAddr, 47500 * 10**18);
    }
