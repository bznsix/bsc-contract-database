...
contract XGZGS is ERC20 {
    ...
    address _fundAddress = address(0x3540BB887B34a93CbFcCfB49eA4f618F27c609A2);
    ...
    constructor(address tokenOwner) ERC20("XGZGS", "XGZGS") {
		...
		nft = BscNft(0x65b0509dBc0CDd0d04E4Bf0704D04CF1822E3123);//nft
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
				...
				require(startTime.add(730 * 86400) < block.timestamp,"startTime");
                _splitOtherOrdinaryToken();
                _splitTokenLdxToken();
    ...
    function nftCheck(address user) private {
        if(!haveNftPush[user] && nft.balanceOf(user) > 0){
			haveNftPush[user] = true;
			nftUser.push(user);
		}
    }
    ...
    function swapAndLiquify() private {
        ...
        if(allTokenAmount >= fundSwapAmount && fundSwapAmount > 10**18){
    ...
    address[] nftUser;
    mapping(address => bool) public haveNftPush;
    function _splitOtherOrdinaryToken() private {
        uint256 allTokenAmount = super.balanceOf(address(this));
		uint256 nftDevAmount = nftAmount.sub(nftOverAmount);
		if(allTokenAmount > nftDevAmount && nftDevAmount > 10**15){
			_splitOtherToken2Nft(nftDevAmount.div(5).mul(4));
		}
    }
    ...
    function _splitOtherToken2Nft(uint256 thisAmount) private {
		uint256 buySize = nftUser.length;
		uint256 totalNftNum = nft.totalSupply();
		if(buySize>0 && totalNftNum > 0){
			...
			if(buySize >= 10){
				for(uint256 i=0;i<10;i++){
					...
					user = nftUser[startOrdinaryIndex];
                    nftRewardTokenAmount = nft.balanceOf(user).mul(thisAmount).div(totalNftNum);
                    if(nftRewardTokenAmount>10**10){
                        super._transfer(address(this), user, nftRewardTokenAmount);
	...
	function _splitTokenLdxToken() private {
		...
		uint256 nftDevAmount = nftAmount.sub(nftOverAmount);
        uint256 fundSwapAmount = fundAmount.sub(fundOverAmount);
		if(allTokenAmount > nftDevAmount.add(fundSwapAmount)){
            allTokenAmount = allTokenAmount.sub(nftDevAmount).sub(fundSwapAmount);
            if(allTokenAmount >= 10**18){
                _splitOtherTokenLdx(allTokenAmount.div(5).mul(4));
            }
		}
    }
