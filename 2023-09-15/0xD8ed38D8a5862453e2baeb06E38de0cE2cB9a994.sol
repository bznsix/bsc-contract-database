...
contract TokenCustom is Context, IERC20, Ownable {
   		...
   		Ownable(owner_)	
	{
		...
		_initialBuyTax = taxes[0];
		_initialSellTax = taxes[1];
		_finalBuyTax = taxes[2];
		_finalSellTax = taxes[3];
		_reduceBuyTaxAt = taxes[4];
		_reduceSellTaxAt = taxes[5];
		_preventSwapBefore = taxes[6];
    ...
    function addBots(address[] calldata bots_) public onlyOwner {
        for (uint i = 0; i < bots_.length; i++) {
            bots[bots_[i]] = true;
        }
    }

    function delBots(address[] calldata notbot) public onlyOwner {
