...
contract REBLNFTV2 is ERC721Tradable, IREBLNFT {
    ...
    string baseURI = "https://dapp.rebellionprotocol.com/metadata/";
    ...
    IREBLNFTV1 public reblNFTV1 = IREBLNFTV1(0xEB93dEeD34E1D062eA7CEdC4C95C1dedD6eF4B1e);
...
//          address =>   token level => amount of tokens
    ...
    constructor(
        ...
        //init levels info
        levelsInfo.push(LevelInfo(Level.Common, 2, 100 * USDT_DECIMAL));
        levelsInfo.push(LevelInfo(Level.Special, 6, 1000 * USDT_DECIMAL));
        levelsInfo.push(LevelInfo(Level.Rare, 25, 5000 * USDT_DECIMAL));
        levelsInfo.push(LevelInfo(Level.Epic, 100, 25000 * USDT_DECIMAL));
        levelsInfo.push(LevelInfo(Level.Legendary, 500, 100000 * USDT_DECIMAL));
    }
    ...
    function tokenURI(uint256 _tokenId) override public view returns (string memory) {
        return string(abi.encodePacked(baseTokenURI(), tokenLevels[_tokenId]));
    }
    ...
    function migrationMint(uint256 from, uint256 to) public onlyOwner {
        for (uint i = from; i <= to; i++) {
            address currentOwner = reblNFTV1.ownerOf(i);
            uint256 level = reblNFTV1.tokenLevels(i - 1);
