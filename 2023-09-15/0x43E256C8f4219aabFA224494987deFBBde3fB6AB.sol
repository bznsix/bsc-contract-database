...
contract BNOUPool is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 bnouAtLastUserAction; // keep track of bnou deposited at the last user action.
    ...
    /**
     * @notice Checks if the msg.sender is either the bnou owner address or the operator address.
     */
    modifier onlyOperatorOrBnouOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or bnou owner");
        _;
    }
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.bnouAtLastUserAction;
     ...
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrBnouOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    function depositOperation(
        ...
        user.bnouAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (user.shares > 0) {
            user.bnouAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
    ...
    function harvest() internal {
        uint256 pendingBnou = masterchefV2.pendingBnou(bnouPoolPID, address(this));
        if (pendingBnou > 0) {
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingBnouRewards();
     ...
     */
    function calculateTotalPendingBnouRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingBnou(bnouPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingBnouRewards()) * (1e18)) / totalShares);
    }
