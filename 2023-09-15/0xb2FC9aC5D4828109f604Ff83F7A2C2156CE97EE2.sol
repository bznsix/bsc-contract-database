...
contract Maintenance is Ownable, ERC721Holder {
    ...
    function repairNft(uint256 _nftId) external validId(_nftId) {
        ...
        if (feeNft != 0 && token != address(0)) {
            require(
                FitZenERC20(token).balanceOf(msg.sender) >=
                feeNft,
...
contract Staking is IStaking, Ownable, ERC721Holder {
    ...
    function getSaleAddresses() external override view returns (address[] memory) {
    ...
    function initCommissionConditionUsd() internal {
        ...
        amountConditions[9] = 7000;
        amountConditions[10] = 8000;
        amountConditions[11] = 8000;
        amountConditions[12] = 8000;
        amountConditions[13] = 8000;
        amountConditions[14] = 8000;
    }
    ...
    function stake(
        uint256 _nftId,
        bytes memory _data
    ) external override isTimeForStaking lock {
    ...
    function checkUserIsSaleAddress(address _user) public override view returns (bool) {
        bool isSale = false;
    ...
    function earnableForStakeInUsdWithDecimal(uint256 _nftId) public view override returns (uint256) {
        ...
        uint256 claimedTimeByRun = stakeInfo.startTime - claimableStartTime;
        uint256 timeLimit = stakingLimit * 30 * 24 * 3600;
        if (claimedTimeByRun >= timeLimit) { // 6 * 30 * 24 * 3600
            return 0;
        }
    ...
    function updateCrewInvestmentData(address nextRef, uint256 _totalAmountStakeUsdWithDecimal, uint256 _nftId) internal {
        address currentRef;
        uint8 index = 1;
    ...
    function updateStakeAprEmergency(
        ...
        require(_nftIds.length == _newAprs.length, "STAKING: ARRAYS MUST BE SAME LENGTH");
        ...
        for (uint i = 0; i < _nftIds.length; i++) {
            ...
            require(_newAprs[i] > 0, "STAKING: APR MUST BE A POSITIVE NUMBER");
            rewardUsdWithDecimal = calculateRewardInUsd(
                totalAmountStakeUsdWithDecimal,
                _newAprs[i]
            );
            stakedNFTs[_nftIds[i]].apr = _newAprs[i];
    ...
    function setMarketCommission(address _currentRef, uint256 _commissionUsdWithDecimal) external override {
        require(msg.sender == rankingContract || msg.sender == earnContract, "STAKING: SENDER NOT RANKING OR EARN CONTRACT");
        IMarketplace(marketplaceContract).updateCommissionStakeValueData(
            _currentRef,
