...
contract NFTCollection is ERC721Enumerable, Ownable {
  ...
  function checkForDiscount() public view returns (bool) {
    uint256 balance = NFT(discountNFT).getBalance();
  ...
  function setDiscount(uint256 newDiscount) external onlyOwner {
    ...
    require(newDiscount < 50,"Too high");
  }

 
  function ownerMint(address _to, uint256 _mintAmount) external onlyOwner returns (uint256[] memory ids) {
  ...
  function mint(address _to, uint256 _mintAmount) public payable returns (uint256[] memory ids) {
    ...
    uint256 actualTokenCost = costToken;
    uint256 actualBNBCost = costBNB;

    if(costToken > 0 ) {
        if(checkForDiscount()) actualTokenCost = actualTokenCost * discount / 100;
        uint256 fullCost = actualTokenCost * _mintAmount;
        require(PAY.transferFrom(msg.sender, address(this), fullCost),"Not Enough Tokens");
    }
    if(costBNB > 0) {
        if(checkForDiscount()) actualBNBCost = actualBNBCost * discount / 100;
        uint256 fullCost = actualBNBCost * _mintAmount;
        require(msg.value >= fullCost,"Not Enough BNB");
    }
    uint256 supply = totalSupply();
 ...
 function addFreeWhitelistUserOrAddMoreSpots(address _user, uint256 _howMany) public onlyOwner {
  ...
  function setCostToken(uint256 _newCost) public onlyOwner {
    costToken = _newCost;
  }
  function setPayToken(address _newToken) public onlyOwner {
    PayToken = _newToken;
  }

  
    // INFO View
    function pageInfo() external view returns (uint256 _MaxSupply, uint256 _CurrentSupply, uint256 _MaxMint, address _PayToken, uint256 _CostBNB, uint256 _CostToken, bool _goPublic, address _owner) {
        return (maxSupply,totalSupply(),maxMintAmount,PayToken, costBNB, costToken, goPublic, owner());
    }
