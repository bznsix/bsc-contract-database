...
interface IMOSTLottery {
    ...
    function viewUserInfoForLotteryId(uint256 _userId, uint256 _lotteryId) external view returns (
            uint256[] memory,
            uint32[] memory,
            bool[] memory
        );
...
contract MOSTLotteryClaimer is ReentrancyGuard, AccessControlEnumerable, WithdrawAnyERC20Token {
    ...
    constructor(
        address _mostLottery,
        address admin,
        address token0,
        address token1,
        address _registry
    ) WithdrawAnyERC20Token(admin, true) {
        lottery = IMOSTLottery(_mostLottery);
        rewardTokens.push(IERC20(token0));
        rewardTokens.push(IERC20(token1));
    ...
    function claimTickets(
        ...
        require(claimed[userId] == false, "User already claimed");

        _lotteryId = 1;

        (uint256[] memory lotteryTicketIds, , bool[] memory ticketStatuses) = lottery.viewUserInfoForLotteryId(userId, _lotteryId);

        for (uint256 i = 0; i < _ticketIds.length; i++) {
            require(_brackets[i] < 6, "Bracket out of range"); // Must be between 0 and 5
            require(ticketStatuses[i] == false, "Already claimed");

            uint256 thisTicketId = _ticketIds[i];

            bool isOwner = false;
            for (uint256 j = 0; j < lotteryTicketIds.length; j++) {
                if (thisTicketId == lotteryTicketIds[j]) {
                    isOwner = true;
                }
            }
            ...
            for (uint256 rewardTokenId = 0; rewardTokenId < rewardTokens.length; rewardTokenId++) {
                IERC20 token = rewardTokens[rewardTokenId];

                uint256 rewardForTicketId = lottery.viewRewardsForTicketId(_lotteryId, thisTicketId, _brackets[i], token);
