...
contract NFTCollection is ERC721Enumerable, Ownable {
  ...
  function checkForDiscount(address user) public view returns (bool) {
    uint256 balance = NFT(discountNFT).balanceOf(user, discountId);
    if(balance > 0) return true;
    return false;
  }
  function getPrice() public view returns (uint256 cost) {
    if(checkForDiscount(msg.sender)) {
        uint256 d =  (costBNB * discount) / 100;
        return costBNB - d;
    }
    return costBNB;
  }
  ...
  function mint(address _to, uint256 _mintAmount) public payable returns (uint256[] memory ids) {
    ...
    if(costBNB > 0) {
        uint256 fullCost = getPrice() * _mintAmount;
