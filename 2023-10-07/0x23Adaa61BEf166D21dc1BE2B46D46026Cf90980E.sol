...
contract MasterChef is Ownable {
    ...
    // The block number when YAMADA mining starts.
    ...
    // Update the given pool's YAMADA allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    // Deposit LP tokens to MasterChef for YAMADA allocation.
    ...
    // Update dev address ba the previous dev.
...
contract SunSwapToken is BEP20('SunSwapToken', 'Sun') {
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), 'SUN::delegateBySig: invalid signature');
        require(nonce == nonces[signatory]++, 'SUN::delegateBySig: invalid nonce');
        require(block.timestamp <= expiry, 'SUN::delegateBySig: signature expired');
    ...
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, 'SUN::getPriorVotes: not yet determined');
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying SUNs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(block.number, 'SUN::_writeCheckpoint: block number exceeds 32 bits');
