...
// MasterChef is the master of Pepe. He can make Pepe and he is a fair guy.
//
...
contract MasterChef is Ownable {
    ...
    // Update the given pool's PEPE allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingPepe(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 pepeReward = multiplier.mul(pepePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accPepePerShare = accPepePerShare.add(pepeReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 pepeReward = multiplier.mul(pepePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pepe.mint(devaddr, pepeReward.div(10));
        pepe.mint(address(this), pepeReward);
        pool.accPepePerShare = pool.accPepePerShare.add(pepeReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for PEPE allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accPepePerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
    ...
    function updateEmissionRate(uint256 _pepePerBlock) public onlyOwner {
        massUpdatePools();
        pepePerBlock = _pepePerBlock;
    }
}
...
contract PepeToken is BEP20('Boys Club', 'PEPE') {
    ...
    */
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "PEPE::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying PEPEs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "PEPE::_writeCheckpoint: block number exceeds 32 bits");
