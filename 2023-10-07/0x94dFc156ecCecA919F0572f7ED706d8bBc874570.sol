//
// 📱 Telegram: https://t.me/altnotifyapp
// 🌎 Website: https://altdapps.com
// 🌎 Playstore: https://play.google.com/store/apps/details?id=com.altnotify&hl=en&gl=US
// AltDapps - DAPP Token
// 
// Our token will empower our platform and tools. We are best known for our app AltNotify and custom telegram bots. DAPP Token is a user changing reward token. As a holder you are able to choose which token you are rewarded in from BNB right through to smaller capped gems! 
// 
// Using data from our apps such as AltNotify to reward users in top gems which are about to be listed to majour exchanges, plus high capped tokens. 
// Partnering with other tokens and gems to save you hours of research, let us reward you in the best tokens. Each buy or sell of DAPP has a % of tax which goes to buying holders rewards.
// 
// Visit our website to set your rewards, what you have been rewarded, the next AMAs, latest info and more!
// 
// We encrourage you to change rewards often building your portfolio, we have a never ending roadmap with a feature packed platform and apps which are already well established. 
//
// Shill to earn will be enabled via our website to reward those who promote our token to the masses. From youtube videos right through to tweets, we will reward you for helping us spread the word!
//
...
contract EarlyBird is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    // Saturday, 7 August 2021 12:00:00
    uint public immutable forceStartTradingAt = 1628337600;
    uint256 public sellAmount = 0;
    uint256 public buyAmount = 0;
    uint256 private totalSellFees;
    uint256 private totalBuyFees;

    EarlyBirdDividendTracker public dividendTracker;

    address public liquidityWallet;

    address payable public operations1Address =
        payable(0xBA48804474296DbDf5fC01e2708843Fb4d32A8fc);
    ...
    uint256 public BNBRewardsSellFee = 20;
    ...
    constructor() ERC20("AltDapps", "DAPP") {
        dividendTracker = new EarlyBirdDividendTracker(payable(this));
        ...
        totalSellFees = BNBRewardsSellFee.add(liquidityFee).add(devFees);
        totalBuyFees = BNBRewardsBuyFee.add(liquidityFee).add(devFees);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        EarlyBirdDividendTracker newDividendTracker = EarlyBirdDividendTracker(
            payable(newAddress)
        );
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (isSelling) {
                BNBRewardsFee = BNBRewardsSellFee;
            } else {
                BNBRewardsFee = BNBRewardsBuyFee;
            }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividendsFromBuy = address(this)
            ...
            .div(BNBRewardsBuyFee.add(devFees));
        ...
        uint256 dividendsFromSell = address(this)
            ...
            .div(BNBRewardsSellFee.add(devFees));
...
contract EarlyBirdDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "EarlyBird_Dividend_Tracker",
            "EarlyBird_Dividend_Tracker"
        )
    {
        earlyBird = EarlyBird(mainContract);
        minimumTokenBalanceForAutoDividends = 10000000000 * (10**9);
    ...
    function _transfer(
        ...
        require(false, "EarlyBird_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "EarlyBird_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EarlyBird contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(earlyBird);

            uint256 prevBalance = earlyBird.balanceOf(address(this));
            ...
            {
                uint256 received = earlyBird.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    earlyBird.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
