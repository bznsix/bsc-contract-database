...
contract extESdcls is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 888000000 * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public maxBuyAmount = _tTotal.mul(2).div(100);
    uint256 public maxSellAmount = _tTotal.mul(1).div(100);
    uint256 public swapTokensAtAmount = 250000 * 10**_decimals;
    uint256 public _maxWalletSize = 888000000 * 10**_decimals;
    ...
    address public dlcsAddress = 0x0Af8447Dd1758245f320b67F7Af2164f8B36B3ca;

    string private constant _name = "extESdlcs";
    string private constant _symbol = "$esdlcs";
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[dlcsAddress] = true;
    ...
    //@dev changed original RFI naming -> "reward" as in reflection to just RFI
   ...
   function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner() {
    ...
    function setFeeRates(uint256 _rfi, uint256 _treasury, uint256 _nft, uint256 _liquidity, uint256 _dlcs) external onlyOwner {
        feeRates.rfi = _rfi;
    ...
    function setSellFeeRates(uint256 _rfi, uint256 _treasury, uint256 _nft, uint256 _liquidity, uint256 _dlcs) external onlyOwner{
        sellFeeRates.rfi = _rfi;
        sellFeeRates.treasury = _treasury;
        sellFeeRates.nft = _nft;
        sellFeeRates.liquidity = _liquidity;
        sellFeeRates.dlcs = _dlcs;
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rTreasury, to_return.rNft, to_return.rLiquidity, to_return.rDlcs) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tTreasury = tAmount*sellFeeRates.treasury/1000;
            s.tNft = tAmount*sellFeeRates.nft/1000;
            s.tLiquidity = tAmount*sellFeeRates.liquidity/1000;
            s.tDlcs = tAmount*sellFeeRates.dlcs/1000;
            s.tTransferAmount = tAmount-s.tRfi-s.tTreasury-s.tNft-s.tLiquidity-s.tDlcs;
        }
        else{
            s.tRfi = tAmount*feeRates.rfi/1000;
            s.tTreasury = tAmount*feeRates.treasury/1000;
            s.tNft = tAmount*feeRates.nft/1000;
            s.tLiquidity = tAmount*feeRates.liquidity/1000;
            s.tDlcs = tAmount*feeRates.dlcs/1000;
            s.tTransferAmount = tAmount-s.tRfi-s.tTreasury-s.tNft-s.tLiquidity-s.tDlcs;
        }
        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rTreasury,uint256 rNft, uint256 rLiquidity, uint256 rDlcs) {
        ...
        rTransferAmount =  rAmount-rRfi-rTreasury-rNft-rLiquidity-rDlcs;
        return (rAmount, rTransferAmount, rRfi,rTreasury,rNft,rLiquidity, rDlcs);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        ...
        _takeDlcs(s.rDlcs, s.tDlcs);
        emit Transfer(sender, recipient, s.tTransferAmount);
        emit Transfer(sender, address(this), s.tLiquidity + s.tTreasury + s.tDlcs);
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        uint256 denominator= (feeRates.liquidity + feeRates.dlcs + feeRates.treasury) * 2;
        ...
        if(bnbToAddLiquidityWith > 0){
            // Add liquidity to Pancake or whatever swap you are using
            addLiquidity(tokensToAddLiquidityWith, bnbToAddLiquidityWith);
        }

        // Send BNB to treasuryWallet
        uint256 treasuryAmt = unitBalance * 2 * feeRates.treasury;
        if(treasuryAmt > 0){
          payable(treasuryAddress).transfer(treasuryAmt);
        }

        // Send BNB to dlcsWallet
        uint256 dlcsAmt = unitBalance * 2 * feeRates.dlcs;
        if(dlcsAmt > 0){
          payable(dlcsAddress).transfer(dlcsAmt);
        }
    }
