...
contract MetaBot is ERC20, Ownable {
    ...
    uint256 public  botDevFeeOnBuy;
    uint256 public  botDevFeeOnSell;
    ...
    event botDevWalletChanged(address botDevWallet);
    event UpdateBuyFees(uint256 marketingFeeOnBuy, uint256 botDevFeeOnBuy);
    event UpdateSellFees(uint256 marketingFeeOnSell, uint256 botDevFeeOnSell);
    event SwapAndLiquify(uint256 tokensSwapped,uint256 bnbReceived,uint256 tokensIntoLiqudity);
    event SwapAndSendMarketing(uint256 tokensSwapped, uint256 bnbSend);
    event SwapAndSendbotDev(uint256 tokensSwapped, uint256 bnbSend);
    event SwapTokensAtAmountUpdated(uint256 swapTokensAtAmount);

    constructor () ERC20("MetaBot", "MetaBot") 
    {   
        address router;
        if (block.chainid == 56) {
        ...
        marketingWallet = 0x14841ECb261967a70f1078f6F77668864115970E;
        botDevWallet = 0x26aC81159F376219eB13038C7D7A77181928D9b8;

        _isExcludedFromFees[owner()] = true;
        _isExcludedFromFees[address(0xdead)] = true;
        _isExcludedFromFees[address(this)] = true;
        _isExcludedFromFees[marketingWallet] = true;
        _isExcludedFromFees[botDevWallet] = true;
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (canSwap &&
            ...
            uint256 botDevShare = botDevFeeOnBuy + botDevFeeOnSell;
            ...
            if (botDevShare > 0) {
                uint256 botDevTokens = contractTokenBalance * botDevShare / totalFee;
                swapAndSendbotDev(botDevTokens);
            }                   
    ...
    function swapAndSendbotDev(uint256 tokenAmount) private {
        uint256 initialBalance = address(this).balance;
        ...
        payable(botDevWallet).sendValue(newBalance);

        emit SwapAndSendbotDev(tokenAmount, newBalance);
    }
}