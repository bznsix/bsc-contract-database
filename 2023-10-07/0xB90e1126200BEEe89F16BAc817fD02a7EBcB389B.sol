...
//  use SafeMath to acvoic buffer overflows/underflows
library SafeMath {
    /**
...
contract Lift is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 20 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Lift";
    string private _symbol = "LIFT";
    ...
    uint256 public _maxTxAmount = 1 * 10**5 * 10**9;
    ...
    constructor (address marketingWallet) public {
        require(marketingWallet.isContract() == false, "marketing cannot be a contract");
        _rOwned[_msgSender()] = _rTotal;
        _marketing = marketingWallet;
        ...
        //exclude owner and marketing wallet and this contract from fee
    ...
    function _transfer(
        ...
        if ((from != owner() && to != owner()) || (from != marketing() && to != marketing()))
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the ETH -> LIFT swap when swap+liquify is triggered
