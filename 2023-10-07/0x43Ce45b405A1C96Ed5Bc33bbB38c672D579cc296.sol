...
contract BitFine is Ownable {
    ...
    uint256 public minimumDeposit = 25 * 1e14;
    ...
    address tokenAddress = 0x7fB2d9c2a8635C44F38e437b857b6BeF42e08d6F;
    ...
    mapping(uint256 => address[]) top3Winners_account;
    mapping(uint256 => uint256[]) top3Winners_amount;
    ...
    function getTop3Winner(uint256 round) public view returns (address[] memory,uint256[] memory) {
        return (top3Winners_account[round],top3Winners_amount[round]);
    }
    ...
    function deposit(address referee,address referral) public payable returns (bool) {
        ...
        buyToken(getFeeAmount(msg.value,depositFeeToSwap,denominator));
    ...
    function updateTop3Amount(address account,uint256 amount) internal {
        ...
        top3[currentRound].reward += amount;
        if(!isTopAdsigned[account][currentRound]){
            top3[currentRound].account.push(account);
            top3[currentRound].amount.push(amount);
            isTopAdsigned[account][currentRound] = true;
            top3Key[account][currentRound] = top3[currentRound].account.length - 1;
        }else{
            uint256 key = top3Key[account][currentRound];
            top3[currentRound].amount[key] += amount;
        }
        updateTop3Winner(account,amount,currentRound);
    }

    function updateTop3Winner(address account,uint256 amount,uint256 round) internal {
        if(!top3[round].init){
            top3[round].init = true;
            address[] memory nullAccount = new address[](3);
            uint256[] memory nullAmount = new uint256[](3);
            top3Winners_account[round] = nullAccount;
            top3Winners_amount[round] = nullAmount;
        }
        for(uint256 i = 0; i < top3Winners_account[round].length; i++){
            if(amount>top3Winners_amount[round][i]){
                top3Winners_account[round][i] = account;
                top3Winners_amount[round][i] = amount;
            }
        }
    }
    ...
    function claimROI(address account) external returns (bool) {
    ...
    function claimCommission(address account) external returns (bool) {
    ...
    function claimTop3Reward(address account,uint256 round) external returns (bool) {
        ...
        bool isWinner;
        uint256 index;
        if(account==top3Winners_account[round][0]){ isWinner = true; index = 0; }
        if(account==top3Winners_account[round][1]){ isWinner = true; index = 1; }
        if(account==top3Winners_account[round][2]){ isWinner = true; index = 2; }
        require(isWinner,"BitFine Revert: This Account Does Not Win This Round");
        uint256 rewardToBeClaim = top3[round].reward;
