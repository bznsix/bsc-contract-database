...
contract PIXMonster is IERC20, Ownable {
    ...
    function process(
        ...
        if (token.extraFee > 0) {
            BNBToLiquifyEXTRAFee = BNBToLiquify.mul(token.extraFee).div(
                feeDenominator
            );

            // CHECKS IF BUYBACK IS OFF. IF IT IS, ADDS BUYBACK FEE TO PIXFEE
            if (!generalBuyBack) {
                BNBToLiquifyFeeAmount = BNBToLiquifyFeeAmount.add(
                    BNBToLiquifyEXTRAFee
                );
                BNBToLiquifyEXTRAFee = 0;
            }
        }
        if (
            ...
            BNBToLiquifyOVERFee = BNBToLiquify.mul(token.overFee).div(
                feeDenominator
            );
        }
        ...
        if (BNBToLiquifyFeeAmount > 0 && !isFeeExempt[_deliveryAddress]) {
            ...
            if (
                ...
                if (buybackAmount > 0) {
                    ...
                    buybackAmount = buybackAmount.div(feeDenominator).mul(
                        buybackPercentage
                    );
                    if (
        ...
        if (_token != WBNB) {
            ...
            if (balanceBefore < _mintokenAmount || _mintokenAmount == 0) {
                ...
                router.swapExactTokensForTokens{
                    value: BNBToLiquify
                }(BNBToLiquify, minAmount, path, deliveryAddress);
    ...
    function criptoNoPix(
        ...
        if (
            reserveSwap &&
            IERC20(RESERVE).balanceOf(address(this)) >= reserveSwapAmount &&
    ...
    function buy(
        ...
        if (_feeReceiver != _refferral) {
            ...
            }(_amountToLiquify, 0, path, _feeReceiver);

            router.swapExactTokensForTokens{
                value: _amountToLiquify
            }(_amountToLiquify, 0, path, _refferral);
        } else {
            router.swapExactTokensForTokens{
                value: _amountToLiquify
            }(_amountToLiquify, 0, path, _feeReceiver);
    ...
    function adjustBalance(
        ...
        if (IERC20(_tokenIn).allowance(address(this), ROUTER) < _amountIn) {
            require(
                IERC20(_tokenIn).approve(ROUTER, type(uint256).max),
