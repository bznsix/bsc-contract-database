...
contract Beri is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BeriDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Beri", "BERI") {
        ...
        dividendTracker = new BeriDividendTracker(payable(this), router, 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82,
                                                   "BeriTRACKER", "BERITRACKER");
...
contract BeriDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        BeriContract = Beri(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main Beri contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BeriContract);

            uint256 prevBalance = BeriContract.balanceOf(address(this));
            ...
            {
                uint256 received = BeriContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BeriContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
