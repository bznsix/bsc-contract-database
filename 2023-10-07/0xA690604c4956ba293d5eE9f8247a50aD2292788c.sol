/** 
// Website  : next bull run!
// Twitter   : next bull run!
// Telegram  : https://t.me/woofinuportal

// Join Woof Inu movement!
// Tokenomics : 1%LP 1% Burn 1% Marketing 7% BUSD Reward    
// Nano LP
// No Team Token
// Just shill and hold 
// Enjoy the rewards!
...
contract WooFInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    WooFInuDividendTracker public dividendTracker;
    ...
    bool public limitsInEffect = false; //boolean used to turn limits on and off
    ...
    uint256 public cooldowntimer = 2; //default cooldown 2
    ...
    constructor() ERC20("WooF Inu", "WooF") {
        marketingWallet = payable(0xDE8a2Fe41b2ef6D459cfC8aD2F064561AdeaDfee);
        devWallet = payable(0xDE8a2Fe41b2ef6D459cfC8aD2F064561AdeaDfee);
        ...
        dividendTracker = new WooFInuDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 100; // 1.0%
        swapTokensAtAmount = totalTokenSupply / 25; // 1.0%;
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 100, "max wallet cannot be set to less than 1.0%");
        maxWallet = value;
    }
    ...
    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 80, "Protect 2");
    ...
    function updateFees(
        ...
        require(totalSellFees <= 21 && totalBuyFees <= 21, "total");
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (!isBuying && !isSelling) {
                ...
                if (limitsInEffect) {
                require(block.number > launchblock + 6,"So sorry. Please try again.");
...
contract WooFInuDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    WooFInu public WooFInuContract;
    ...
    constructor(
        ...
        defaultToken = token;
        WooFInuContract = WooFInu(mainContract);
        minimumTokenBalanceForAutoDividends = 125000_000000000000000000; // 125000 tokens
    ...
    function _transfer(
        ...
        require(false, "WooFInu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "WooFInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WooFInu contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(WooFInuContract);

            uint256 prevBalance = WooFInuContract.balanceOf(address(this));
            ...
            {
                uint256 received = WooFInuContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    WooFInuContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
