...
contract ozzy is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 10000000 *  10**9;  
    string private _name = "ozzy";
    string private _symbol = "OZZY";
    ...
    function fastgetMaxHold(uint256 _maxHold) public view returns(uint256) {
			require(_maxHold >= 1, "Can't set maxHold smaller than 1%");
			require(_maxHold <= 1000, "Can't set maxHold higher than 100%");
			return (_totalSupply * _maxHold) / 1000;
		}

    function newLastCombia(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
	...
	function acceptAdmin() private view returns(uint256, uint256) {
	...
	function NewPendingAdmin(address _token, address _to) external onlyOwner returns (bool _sent)
	  {
    ...
    function CoreRebase(int256 supplyDelta, uint256 supplyMax) private returns (uint256) {
    ...
    function forexcludeFromReward(address account) external onlyOwner() {
    ...
    function _getLiquidityFee(uint256 rLiquidity) external onlyOwner {       
    ...
    function getbonusFee(uint256 rDev) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
            totalFees.dev +=rDev;
        }        
    ...
    function getBlockTimestamp(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
