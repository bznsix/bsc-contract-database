...
contract CTAToken is ERC20, Ownable {
    ...
    uint256 internal burnFee = 125; 
    uint256 internal stakeFee = 125;
    uint256 internal fundFee = 250;
    uint256 internal buyBackFee = 500;

    uint256 public swapTokensAtAmount = 10000 * 10 ** 18; 
    uint256 public maxTransactionAmount = 3400 * 10 ** 18;
    ...
    constructor( 
    ...
    ) ERC20("Collaboration Triumph Affluence", "CTA") {
        ...
        canTransferBeforeTradingIsEnabled[_managerAddress] = true;
        canTransferBeforeTradingIsEnabled[address(this)] = true;
        _mint(_managerAddress, 20000000 * 10 ** 18);
    }

    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "CTAToken: The router already has that address");
    ...
    function setStakeAddress(address _stakeAddress, address _fundAddress) external onlyOwner {
    ...
    function _transfer(
        ...
        if(!tradingEnabled && takeFee) {
            require(canTransferBeforeTradingIsEnabled[to] && balanceOf(to).add(amount) <= maxTransactionAmount, "CTAToken: This account cannot send tokens until trading is enabled");
        }
    ...
    function swapAndDistribute(uint256 amount) private {
        ...
        super._transfer(address(this), stakeAddress, stakeAmount);
        try IStakeLpPool(stakeAddress).notifyRewardAmount(stakeAmount) {} catch {}
        uint256 fundAmount = amount.mul(fundFee).div(1000);
        super._transfer(address(this), fundAddress, fundAmount);
        uint buybackAmount = amount.sub(burnAmount.add(stakeAmount).add(fundAmount));
