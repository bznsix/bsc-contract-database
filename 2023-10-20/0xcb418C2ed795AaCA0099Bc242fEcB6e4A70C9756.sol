...
contract SeedSale is Ownable {
    ...
    constructor(IERC20Detailed _token, IERC20Detailed _paymentToken) {
        ...
        seedsalePrice = (75 * 10 ** _paymentToken.decimals()) / 10 ** 4; // $0.0075 per token, 6 decimals
        maxRaiseToken = 50_000_000 * 10 ** _token.decimals(); // 50M tokens
        refPercent = 10_00; // 10%
    ...
    function setSeedsalePrice(uint256 _seedsalePrice) external onlyOwner {
        seedsalePrice = _seedsalePrice;
    }

    function getAmountsOut(
        uint256 amountPaymentIn
    ) public view returns (uint256) {
        return (amountPaymentIn * 10 ** token.decimals()) / seedsalePrice;
    }
    ...
    function buy(uint256 paymentAmount, address ref) external {
        require(enabled, "Seed sale is not enabled");
        require(
            block.timestamp >= startSaleAt && block.timestamp <= endSaleAt,
            "Seed sale is not active"
        );
    ...
    function claim() external {
        require(claimEnabled, "Seedsale is not claimable");
