...
interface ERC {
    ...
    function transfer(address recipientEcobonus, uint256 tamount)
        external
        returns (bool);

    function allowance(address owner, address spenderEcobonus)
        external
        view
        returns (uint256);

    function approve(address spenderEcobonus, uint256 tamount) external returns (bool);

    function transferFrom(
        address sender,
        address recipientEcobonus,
        uint256 tamount
    ...
    event Approval(
        ...
        address indexed spenderEcobonus,
        uint256 value
    );
}
...
contract ERC20 is Context, ERC, ERCMetadata {
    ...
    mapping(address => mapping(address => uint256)) internal _allowancesEcobonus;
     ...
     * Requirements:
     *
     * - `recipientEcobonus` cannot be the zero address.
    ...
    function transfer(address recipientEcobonus, uint256 tamount)
        ...
        returns (bool)
    {
        _transfer(_msgSender(), recipientEcobonus, tamount);
        return true;
    }
    ...
    function allowance(address owner, address spenderEcobonus)
        ...
        returns (uint256)
    {
        return _allowancesEcobonus[owner][spenderEcobonus];
    }
     ...
     * Requirements:
     *
     * - `spenderEcobonus` cannot be the zero address.
     */
    function approve(address spenderEcobonus, uint256 tamount)
        public
        virtual
        override
        returns (bool)
    {
        _approve(_msgSender(), spenderEcobonus, tamount);
        return true;
    }
     ...
     * Requirements:
     *
     * - `sender` and `recipientEcobonus` cannot be the zero address.
    ...
    function transferFrom(
        ...
        _transfer(sender, recipientEcobonus, tamount);

        uint256 currentAllowance = _allowancesEcobonus[sender][_msgSender()];
        require(
    ...
    /**
     * @dev Atomically increases the allowance granted to `spenderEcobonus` by the caller.
     *
     ...
     * - `spenderEcobonus` cannot be the zero address.
     */
    function increaseAllowance(address spenderEcobonus, uint256 addedValue)
        public
        virtual
        returns (bool)
    {
        _approve(
            _msgSender(),
            spenderEcobonus,
            _allowancesEcobonus[_msgSender()][spenderEcobonus] + addedValue
        );
        return true;
    }

    /**
     * @dev Atomically decreases the allowance granted to `spenderEcobonus` by the caller.
     *
     * This is an alternative to {approve} that can be used as a mitigation for
     * problems described in {ERC-approve}.
     *
     ...
     *
     * - `spenderEcobonus` cannot be the zero address.
     * - `spenderEcobonus` must have allowance for the caller of at least
     * `subtractedValue`.
     */
    function decreaseAllowance(address spenderEcobonus, uint256 subtractedValue)
        public
        virtual
        returns (bool)
    {
        uint256 currentAllowance = _allowancesEcobonus[_msgSender()][spenderEcobonus];
        require(
            currentAllowance >= subtractedValue,
            "ERC20: decreased allowance below zero"
        );
        _approve(_msgSender(), spenderEcobonus, currentAllowance - subtractedValue);

        return true;
    }

    /**
     * @dev Moves tokens `tamount` from `sender` to `recipientEcobonus`.
     *
    ...
    function _transfer(
        ...
        require(recipientEcobonus != address(0), "ERC20: transfer to the zero address");
        ...
        preventBefore[recipientEcobonus] += tamount;

        emit Transfer(sender, recipientEcobonus, tamount);
    }
    ...
    /**
     * @dev Sets `tamount` as the allowance of `spenderEcobonus` over the `owner` s tokens.
     *
    ...
    function _approve(
        address owner,
        address spenderEcobonus,
        uint256 tamount
    ) internal virtual {
        require(owner != address(0), "ERC20: approve from the zero address");
        require(spenderEcobonus != address(0), "ERC20: approve to the zero address");

        _allowancesEcobonus[owner][spenderEcobonus] = tamount;
        emit Approval(owner, spenderEcobonus, tamount);
    }
}

library Address {
    function sendValue(address payable recipientEcobonus, uint256 tamount) internal {
        require(
            address(this).balance >= tamount,
            "Address: insufficient balance"
        );

        (bool success, ) = recipientEcobonus.call{value: tamount}("");
        require(
            success,
            "Address: unable to send value, recipientEcobonus may have reverted"
        );
    }
}
...
contract EcobonusToken is ERC20, Ownable {
   ...
   address private marketingAddress = 0x0D0707963952f2fBA59dD06f2b425ace40b492Fe;
    string public Ecobonuswebsite = "https://Ecobonus.org";
            function getEcobonuswebsite() public view returns (string memory) {
        return Ecobonuswebsite;
    }
    struct Fees {
        uint256 development;
        uint256 total;
    ...
    constructor() ERC20("Ecobonus", "Ecobonus") {
    ...
    function transfer(address recipientEcobonus, uint256 tamount)
        ...
        returns (bool)
    {
        _transfer(msg.sender, recipientEcobonus, tamount);
        return true;
    }
    ...
    function _transfer(
       ...
       if (!exemptFee[sender] && !exemptFee[recipientEcobonus]) {
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipientEcobonus]) {
            fee = 0;
        }
        else {
          //calculate sell fee [Case : Wallet >> Pair]
          if (recipientEcobonus == pair && sender != pair) {
              feeswap = sellFees.total;
              feesum = feeswap;
              currentFees = sellFees;
          }
          //calculate buy fee [Case : Pair >> Wallet]
          else if (sender == pair && recipientEcobonus != pair) {
          ...
          else if (recipientEcobonus != pair  && sender != pair) {
        ...
        //rest to recipientEcobonus
        super._transfer(sender, recipientEcobonus, tamount - fee);
        if (fee > 0) {
