...
contract STMStake is Ownable {
    ...
    function getReward(uint256 level) public {
        ...
        if (reward > 0) {
            ...
            if (level == 1) {
                _totalSupply_V1 = _totalSupply_V1.sub(reward);
            } else if (level == 2) {
                _totalSupply_V2 = _totalSupply_V2.sub(reward);
            } else if (level == 3) {
                _totalSupply_V3 = _totalSupply_V3.sub(reward);
            }
    ...
    function withdrawLevel(uint256 level) public {
        ...
        if ((user.tokenAmount > 0 && block.timestamp >= user.endTime) || swapsEnabled) {
            user.lastUpdateTime = user.endTime;
            safeTransfer(msg.sender, user.tokenAmount);
        } else {
