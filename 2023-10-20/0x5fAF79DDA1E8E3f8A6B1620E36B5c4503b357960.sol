...
contract T0 is ERC20 ,Ownable{
    ...
    constructor() ERC20("CL", "CL") {
        ...
        walletLimit = 7200 * 10 ** decimals();
        _mint(msg.sender, 7200 * 10 ** decimals());
    ...
    function _applyTax(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (sender == mainPair) {
            ...
            if(launchBlock + 20 * 10 > block.number){
                _tax = 30;
            }else if(launchBlock + 20 * 15 > block.number){
                _tax = 10;
            }else{
                _tax = sellTax;
            }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(sender == mainPair){
            require(balanceOf(recipient) <= walletLimit,"wallet limmited");
        }

    }

    function swapAndAddLiquidity() public lockTheSwap{
        ...
        if (contractBalance >= swapAt) {
            ...
            path[0] = address(this);
            path[1] = usdt;
            try uniswapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(amountToSwap, 0, path, marketingWallet, block.timestamp) {}catch{}
        }
    }

    function setWhitelist(address[] memory userList, bool status) external onlyOwner {
        for(uint256 i = 0; i < userList.length; i++){
            isWhitelisted[userList[i]] = status;
        }
    }
    ...
    function setWalletLimit(uint256 _newValue) external onlyOwner{
        walletLimit = _newValue;
    }

    function setBuyTax(uint256 _newValue) external onlyOwner{
        buyTax = _newValue;
    }

    function setSellTax(uint256 _newValue) external onlyOwner{
        sellTax = _newValue;
    }
