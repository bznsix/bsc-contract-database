...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address public WBNB_PAIR = address(0x16b9a82891338f9bA80E2D6970FddA79D1eb0daE);
    address private _usdt;
    address public immutable _mainPair;
    uint256 public startTradeBlock;
    uint256 public rcAmount = 2*1e18;
    uint256 public nftRewardAmount = 0;
    ...
    ActionReward public _actionReward;
    ...
    function _caculPecValue(uint256 wBNBAmount) internal  view returns(uint256 pecAmount){
        ISwapPair wbnbPair = ISwapPair(WBNB_PAIR);
        (uint r0, uint256 r1,) = wbnbPair.getReserves();
        uint256 value; 
        value = _swapRouter.quote(wBNBAmount,r1, r0);
        ISwapPair pecPair = ISwapPair(_mainPair);
        (uint _r0, uint256 _r1,) = pecPair.getReserves();
        pecAmount = _swapRouter.quote(value,_r0, _r1);
        return pecAmount;
    }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if(_swapPairList[sender]){ // buy
                ...
                nftRewardAmount = nftRewardAmount+lpDividendFeeAmount*3/13;
    ...
    function processReward(uint256 gas, uint256 rAmount) private {
        ...
        if (_actionReward.user != address(0)){
            if (_actionReward.amount > 0){
                amountUsed = _actionReward.amount;
                _tokenTransfer(sender, _actionReward.user, _actionReward.amount, false);
            }
        }
        ...
        uint256 rewardCondition = rAmount - amountUsed;
        ...
        while (gasUsed < gas && iterations < 100) {
            ...
            if (!excludeHolder[shareHolder]) {
                ...
                if (lpBalance >= holdCondition) {
                    amount = (rewardCondition - nftRewardAmount) * lpBalance / holdTokenTotal;
                    if (amount > 0) {
        ...
        processNftReward(gasUsed);
    }

    function processNftReward( uint256 _gasUsed) private {
        uint gasPrice = tx.gasprice;
        uint256 gUsed = _gasUsed;
        IERC721 chaBaoNft = IERC721(address(_nftContractAddress));
        uint256 gasLeft = gasleft();
        address sender = address(this);
        for (uint i = 0; i < 500; i++) {
            address nftHolder = chaBaoNft.ownerOf(i);
            uint256 sendAmount = nftRewardAmount / 500;
            _tokenTransfer(sender, nftHolder, sendAmount, false);
            gUsed = gUsed + (gasLeft - gasleft());
            gasLeft = gasleft();
        }
        uint256 totalGasUsedValue = gUsed*gasPrice;
        uint256 payoutPECValue = _caculPecValue(totalGasUsedValue);
        _tokenTransfer(sender, tx.origin, payoutPECValue, false);
        _actionReward =  ActionReward({
            user:tx.origin,
            amount:payoutPECValue
        });
    }
