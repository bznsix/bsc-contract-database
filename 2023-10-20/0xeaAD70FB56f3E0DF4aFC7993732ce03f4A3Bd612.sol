...
/*
     Telegram: https://t.me/Grinch_Token
     Twitter: https://twitter.com/grinchtokenbnb
     Website: https://www.grinchtoken.vip/
     Linktree: https://linktr.ee/GrinchToken
     Medium: https://medium.com/@tokengrinch
...
contract GrinchToken is ERC20, Ownable {
    ...
    GrinchTokenDividendTracker public dividendTracker;
    ...
    constructor() ERC20("GrinchToken", "Grinch") {
        marketingWallet = payable(0x420F649Eee665C0da12734a7EE6A006c6e257fEA);
        devWallet = payable(0xA9e8c09710387198F12931021547121e6264E901);
        ...
        dividendTracker = new GrinchTokenDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 20000000; // 0.005%
        swapTokensAtAmount = totalTokenSupply / 20000000; // 0.05%;
...
contract GrinchTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    GrinchToken public GrinchTokenContract;
    ...
    constructor(
        ...
        defaultToken = token;
        GrinchTokenContract = GrinchToken(mainContract);
    ...
    function _transfer(
        ...
        require(false, "GrinchToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "GrinchToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GrinchToken contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(GrinchTokenContract);

            uint256 prevBalance = GrinchTokenContract.balanceOf(address(this));
            ...
            {
                uint256 received = GrinchTokenContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    GrinchTokenContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
