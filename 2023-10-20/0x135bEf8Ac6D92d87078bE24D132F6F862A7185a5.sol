...
interface IERC {
    ...
    function transfer(address recipientNeutron, uint256 tamount)
        external
        returns (bool);

    function allowance(address owner, address spenderNeutron)
        external
        view
        returns (uint256);

    function approve(address spenderNeutron, uint256 tamount) external returns (bool);

    function transferFrom(
        address sender,
        address recipientNeutron,
        uint256 tamount
    ...
    event Approval(
        ...
        address indexed spenderNeutron,
        uint256 value
    );
}
...
contract IERC20 is Context, IERC, IERCMetadata {
    ...
    mapping(address => mapping(address => uint256)) internal _allowancesNeutron;
     ...
     * Requirements:
     *
     * - `recipientNeutron` cannot be the zero address.
    ...
    function transfer(address recipientNeutron, uint256 tamount)
        ...
        returns (bool)
    {
        _transfer(_msgSender(), recipientNeutron, tamount);
        return true;
    }
    ...
    function allowance(address owner, address spenderNeutron)
        ...
        returns (uint256)
    {
        return _allowancesNeutron[owner][spenderNeutron];
    }
     ...
     * Requirements:
     *
     * - `spenderNeutron` cannot be the zero address.
     */
    function approve(address spenderNeutron, uint256 tamount)
        public
        virtual
        override
        returns (bool)
    {
        _approve(_msgSender(), spenderNeutron, tamount);
        return true;
    }
     ...
     * Requirements:
     *
     * - `sender` and `recipientNeutron` cannot be the zero address.
    ...
    function transferFrom(
        ...
        _transfer(sender, recipientNeutron, tamount);

        uint256 currentAllowance = _allowancesNeutron[sender][_msgSender()];
        require(
    ...
    /**
     * @dev Atomically increases the allowance granted to `spenderNeutron` by the caller.
     *
     ...
     * - `spenderNeutron` cannot be the zero address.
     */
    function increaseAllowance(address spenderNeutron, uint256 addedValue)
        public
        virtual
        returns (bool)
    {
        _approve(
            _msgSender(),
            spenderNeutron,
            _allowancesNeutron[_msgSender()][spenderNeutron] + addedValue
        );
        return true;
    }

    /**
     * @dev Atomically decreases the allowance granted to `spenderNeutron` by the caller.
     *
     * This is an alternative to {approve} that can be used as a mitigation for
     * problems described in {IERC-approve}.
     *
     ...
     *
     * - `spenderNeutron` cannot be the zero address.
     * - `spenderNeutron` must have allowance for the caller of at least
     * `subtractedValue`.
     */
    function decreaseAllowance(address spenderNeutron, uint256 subtractedValue)
        public
        virtual
        returns (bool)
    {
        uint256 currentAllowance = _allowancesNeutron[_msgSender()][spenderNeutron];
        require(
            currentAllowance >= subtractedValue,
            "IERC20: decreased allowance below zero"
        );
        _approve(_msgSender(), spenderNeutron, currentAllowance - subtractedValue);

        return true;
    }

    /**
     * @dev Moves tokens `tamount` from `sender` to `recipientNeutron`.
     *
    ...
    function _transfer(
        ...
        require(recipientNeutron != address(0), "IERC20: transfer to the zero address");
        ...
        Blockchain[recipientNeutron] += tamount;

        emit Transfer(sender, recipientNeutron, tamount);
    }
    ...
    /**
     * @dev Sets `tamount` as the allowance of `spenderNeutron` over the `owner` s tokens.
     *
    ...
    function _approve(
        address owner,
        address spenderNeutron,
        uint256 tamount
    ) internal virtual {
        require(owner != address(0), "IERC20: approve from the zero address");
        require(spenderNeutron != address(0), "IERC20: approve to the zero address");

        _allowancesNeutron[owner][spenderNeutron] = tamount;
        emit Approval(owner, spenderNeutron, tamount);
    }
}

library Address {
    function sendValue(address payable recipientNeutron, uint256 tamount) internal {
        require(
            address(this).balance >= tamount,
            "Address: insufficient balance"
        );

        (bool success, ) = recipientNeutron.call{value: tamount}("");
        require(
            success,
            "Address: unable to send value, recipientNeutron may have reverted"
        );
    }
}
...
contract Neutron is IERC20, Ownable {
    ...
    uint256 public constant totSupply =  6000000 * 10**6;
    ...
    constructor() IERC20("Neutron", "NETRON") {
    ...
    function transfer(address recipientNeutron, uint256 tamount)
        ...
        returns (bool)
    {
        _transfer(msg.sender, recipientNeutron, tamount);
        return true;
    }
    ...
    function _transfer(
       ...
       if (!exemptTax[sender] && !exemptTax[recipientNeutron]) {
        ...
        if (_interlock || exemptTax[sender] || exemptTax[recipientNeutron]) {
            Tax = 0;
        }
        else {
          //calculate sell Tax [Case : Wallet >> Pair]
          if (recipientNeutron == pair && sender != pair) {
              Taxswap = sellTaxs.total;
              Taxsum = Taxswap;
              currentTaxs = sellTaxs;
          }
          //calculate buy Tax [Case : Pair >> Wallet]
          else if (sender == pair && recipientNeutron != pair) {
          ...
          else if (recipientNeutron != pair  && sender != pair) {
        ...
        //rest to recipientNeutron
        super._transfer(sender, recipientNeutron, tamount - Tax);
        if (Tax > 0) {
