...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    IERC20 public usdtTokenAddress = IERC20(0x55d398326f99059fF775485246999027B3197955);
    ...
    function distributeDividends() public override payable {
        ...
        if (msg.value > 0) {
            uint256 beforeBalance = usdtTokenAddress.balanceOf(address(this));
            swapETHForTokens(msg.value);
            uint256 swapBalance = usdtTokenAddress.balanceOf(address(this)).sub(beforeBalance);

            magnifiedDividendPerShare = magnifiedDividendPerShare.add(
                (swapBalance).mul(magnitude) / totalSupply()
            );
            emit DividendsDistributed(msg.sender, swapBalance);

            totalDividendsDistributed = totalDividendsDistributed.add(swapBalance);
        }
    }
    ...
    function swapETHForTokens(uint256 amount) private {
        ...
        path[1] = address(usdtTokenAddress);
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            usdtTokenAddress.transfer(user, _withdrawableDividend);
...
contract ZillaKingDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ZillaKingDividendTracker", "ZillaKing_DividendTracker") {
        claimWait = 600;
        minimumTokenBalanceForDividends = 10000 * (10**decimals()); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "ZillaKingDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "ZillaKingDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main ZillaKing contract.");
    }
    ...
    function withdrawStuckTokens(address _token, address _receiver, uint256 _amount) public onlyOwner {
...
contract ZillaKing is ERC20, Ownable {
    using SafeMath for uint256;

    IPancakeRouter02 public pancakeRouter;
    ZillaKingDividendTracker public dividendTracker;
    address public pancakePair;
    address public marketingWallet;
    uint256 public swapTokensAtAmount = 10000 * (10**decimals());   
    uint256 public dividendFee = 7;   
    ...
    event SetFees(uint256 MarketingFee, uint256 DividendFee);
    event StartSwapTokensForEth(uint256 tokenAmountToBeSwapped);
    event FinishSwapTokensForEth(uint256 ethAmountSwapped);
    event CalculatedBNBForEachRecipient(uint256 forMarketing, uint256 forDividends);
    event ErrorInProcess(address msgSender);
    event SwapAndSendTo(uint256 amount, string to);
    event ProcessedDividendTracker(uint256 iterations, uint256 claims, uint256 lastProcessedIndex, bool indexed automatic, uint256 gas, address indexed processor);

    constructor() ERC20("ZillaKing Token", "ZillaKing") {
        dividendTracker = new ZillaKingDividendTracker();
        ...
        _isExcludedFromMaxHoldLimit[address(0x000000000000000000000000000000000000dEaD)] = true;
        ...
        */
        ...
        maxHoldingAmount = totalSupply().div(200);
    }

    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "ZillaKing: The router already has that address");
        ...
        _isExcludedFromMaxHoldLimit[newAddress] = true;
    ...
    function setFees(uint256 _DividendFee, uint256 _MarketingFee)  external onlyOwner {
        dividendFee = _DividendFee;
        marketingFee = _MarketingFee;
        emit SetFees(_MarketingFee, _DividendFee);
    }
    ...
    function setMarketWallet(address newMarketingWallet) external onlyOwner {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ZillaKing: The dividend tracker already has that address");

        ZillaKingDividendTracker newDividendTracker = ZillaKingDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ZillaKing: The new dividend tracker must be owned by the ZillaKing token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ZillaKing: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxHoldLimit(address account, bool excluded) public onlyOwner {
        _isExcludedFromMaxHoldLimit[account] = excluded;
    }
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ZillaKing: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ZillaKing: Cannot update gasForProcessing to same value");
    ...
    function withdrawInDividend(address _token, uint256 _amount) public onlyOwner {
        return dividendTracker.withdrawStuckTokens(_token, msg.sender, _amount);
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            uint256 fees = amount.mul(dividendFee.add(marketingFee)).div(100);
    ...
    function swapAndDistributeBNB(uint256 tokens) private {
        ...
        uint256 all = marketingFee.add(dividendFee);
        uint256 forMarketing = balance.mul(marketingFee).div(all);
        uint256 forDividends = balance.sub(forMarketing);

        emit CalculatedBNBForEachRecipient(forMarketing, forDividends);

        (bool success,) = address(marketingWallet).call{value: forMarketing}("");

        if(success) {
            emit SwapAndSendTo(forMarketing, "MARKETING");
        }
