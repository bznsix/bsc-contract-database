...
contract MOJO_INU is ERC20, ERC20Burnable, Ownable, Initializable {
    
    uint16 public swapThresholdRatio;
    
    uint256 private _tokenPending;
    uint256 private _liquidityPending;

    address public tokenAddress;
    uint16[3] public tokenFees;
    ...
    event tokenFeesUpdated(uint16 buyFee, uint16 sellFee, uint16 transferFee);
    event tokenFeeSent(address recipient, uint256 amount);
    ...
    constructor()
        ERC20(unicode"MOJO INU", unicode"MOJO") 
    {
        address supplyRecipient = 0x6AF4579c9207f11bE846497df294cF877c8aC633;
        
        updateSwapThreshold(50);

        tokenAddressSetup(0x6AF4579c9207f11bE846497df294cF877c8aC633);
        tokenFeesSetup(300, 299, 0);

        lpTokensReceiverSetup(0x6AF4579c9207f11bE846497df294cF877c8aC633);
        liquidityFeesSetup(200, 199, 0);

        excludeFromFees(supplyRecipient, true);
        excludeFromFees(address(this), true); 

        _mint(supplyRecipient, 1000000000 * (10 ** decimals()) / 10);
        _transferOwnership(0x6AF4579c9207f11bE846497df294cF877c8aC633);
    }
    ...
    function getAllPending() public view returns (uint256) {
        return 0 + _tokenPending + _liquidityPending;
    }

    function tokenAddressSetup(address _newAddress) public onlyOwner {
    ...
    function tokenFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        totalFees[0] = totalFees[0] - tokenFees[0] + _buyFee;
        totalFees[1] = totalFees[1] - tokenFees[1] + _sellFee;
        totalFees[2] = totalFees[2] - tokenFees[2] + _transferFee;
        require(totalFees[0] <= 2500 && totalFees[1] <= 2500 && totalFees[2] <= 2500, "TaxesDefaultRouter: Cannot exceed max total fee of 25%");

        tokenFees = [_buyFee, _sellFee, _transferFee];

        emit tokenFeesUpdated(_buyFee, _sellFee, _transferFee);
    }
    ...
    function _transfer(
        ...
        if (!_swapping && amount > 0 && to != address(routerV2) && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            ...
            if (txType < 3) {
                ...
                _tokenPending += fees * tokenFees[txType] / totalFees[txType];
        ...
        if (!_swapping && !AMMPairs[from] && from != address(routerV2) && canSwap) {
            ...
            if (false || _tokenPending > 0) {
                uint256 token2Swap = 0 + _tokenPending;
                bool success = false;

                _swapTokensForCoin(token2Swap);
                uint256 coinsReceived = address(this).balance;
                
                uint256 tokenPortion = coinsReceived * _tokenPending / token2Swap;
                if (tokenPortion > 0) {
                    success = payable(tokenAddress).send(tokenPortion);
                    if (success) {
                        emit tokenFeeSent(tokenAddress, tokenPortion);
                    }
                }
                _tokenPending = 0;
