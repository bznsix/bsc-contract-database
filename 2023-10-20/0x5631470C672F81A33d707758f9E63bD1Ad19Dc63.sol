...
contract CCA is ERC20 {
    ...
    uint256 public buy_fee = 400;
    uint256 public sell_fee = 400;
    uint256 fundAmount = 5*10**18;
    mapping(address => uint256)public IsFund;
    address[] public fundAddress;
	uint256 total;
    uint256 public limitAmount = 20*10**18;
    uint256 maxsupply = 21000000*10**18;
    address public FundAddress=0x48C57445EAf61e01E9f1a93370b489ef096e7cDa;
    address public MintAddress;
    constructor(address tokenOwner) ERC20("CCA", "CCA") {
    ...
    function mint(address _address,uint256 _amount)public {
         ...
         require(total + _amount <= maxsupply);
         total += _amount;
   ...
   function setLimitAmount(uint256 _limitAmount) public onlyOwner {
    ...
    function Addfund(address _address)public onlyOwner{
        require(_address != address(0x0));
        uint256 index = 0;
        IsFund[_address] = 1;
        if(fundAddress.length > 0){
            for(uint i;i<fundAddress.length;i++){
                  if(_address == fundAddress[i]){
                      index = 1;
                  }
            }
        }
        if(index == 0){
        fundAddress.push(_address);
        }    
    }
    function delFund(address _address)public onlyOwner{
        require(_address != address(0x0));
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                super._transfer(from, address(this), amount.div(10000).mul(buy_fee));
                 ldxAmount = ldxAmount.add(amount.div(10000).mul(300));
                amount = amount.div(10000).mul(9600);
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
                _splitUsdtLdxToken();
    ...
    function _splitOtherUsdtLdx() private {
		uint256 buySize = fundAddress.length;
		...
		if(buySize>0 ){
			...
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
					...
					user = fundAddress[startLdxIndex];
                    ldxRewardTokenAmount = fundAmount;
                    if(ldxRewardTokenAmount > 1**10**18 && IsFund[user] == 1){
