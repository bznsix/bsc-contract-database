...
interface IShareRewardPool {
    // Views
    function pendingGol(uint256 _pid, address _user)
        external
        view
        returns (uint256);
...
contract GolFinanceStrat is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public _strategistFee;
    address public lpPair;
    address public lpToken0;
    address public lpToken1;
    address public constant reward =
        address(0x52f6A03a53840C01470f33A0b016E4C3E3DFe796);
    address public constant wbnb =
        address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    address public constant unirouter =
        address(0x10C22079c1D0388056e54Be5bf6751D9320F54D1);
    ...
    address public strategist =
        address(0x89b49fAe3B6C276072BCf485E16EBBaf4A804Adc);
    address public admin;
    address public vault;

    address[] public rewardToWbnbRoute = [reward, wbnb];
    ...
    constructor(
        ...
        admin = address(0x3Ebd346253b6737626B9571694051502dEa1594C);
        vault = _vault;
        totalFee = 40;
        _treasurySup = 90;
        _strategistFee = 10;
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin || msg.sender == owner(), "Not autorized");
        _;
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IShareRewardPool(shareRewardPool).userInfo(
            poolId,
            address(this)
        );
        return _amount;
    ...
    function calculateTotalPendingRewards() external view returns (uint256) {
        uint256 amount = IShareRewardPool(shareRewardPool).pendingGol(
            poolId,
            address(this)
        );
        return amount;
    ...
    function depositFeeBP() public view returns (uint16) {
        (, , , , uint16 amount, ) = IShareRewardPool(shareRewardPool).poolInfo(
            poolId
        );
    ...
    function deposit() public whenNotPaused {
        require (depositFeeBP() == 0, "Deposit fee need update");
    ...
    function harvest() public whenNotPaused notContract {
        ...
        uint256 rewBal = IERC20(reward).balanceOf(address(this));
        if (rewBal > 0) {
            chargeCost();
            addLiquidity();
            deposit();
    ...
    function chargeCost() internal {
        uint256 toWbnb = IERC20(reward)
        ...
        uint256 strategistFee = wbnbBal.mul(_strategistFee).div(100);
    ...
    function setperfomFee(
        uint256 _totalFee,
        uint256 _newStrategistFee,
        uint256 _newTreasurySup
    ) public onlyAdmin {
        require(_totalFee <= 100, "max 10% fee");
        require(_newStrategistFee.add(_newTreasurySup) == 100, "!100%");
        require(_newStrategistFee > 0 && _newTreasurySup > 0, "must be > 0");
        totalFee = _totalFee;
        _strategistFee = _newStrategistFee;
        _treasurySup = _newTreasurySup;
    }

    function newFeeAddress(address _newTreasury, address _newStrategist)
        public
        onlyOwner
    {
    ...
    function newAdmin(address _newAdmin) public onlyOwner {
        admin = _newAdmin;
    }

    function rebalance(address _token) public onlyAdmin {
    ...
    function recoverStrangeTokens(address _token) external onlyOwner {
        require(
            _token != address(lpPair) && _token != address(reward),
            "!token"
        );
