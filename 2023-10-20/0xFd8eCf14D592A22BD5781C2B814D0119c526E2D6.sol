...
contract DefiOracle is DefiModifiers {
        ...
        onlyActiveProject(projectId)
    {
        ...
        uint256 toTake = _amount + feeAmount;
        IToken(usdt).transferFrom(msg.sender, address(this), toTake);
        ...
        uint perLpToken = (totalLpTokens * 1e18) / totalToRaise;
    ...
    function updatePoolToken(
        bytes32 projectId,
        address _poolToken
    ) external onlyHandler {
        project[projectId].poolToken = _poolToken;
        DefiProject(project[projectId].projectAddress).updatePoolToken(
            _poolToken
        );
    }
...
contract DefiProject is ProjectData {
    ...
    function invest(address _investor, uint256 _amount) external onlyOracle {
        ...
        if (totalInvested >= project.amount) {
    ...
    function claimTokens() external onlyProjectOwner onlyCompletedProject {
    ...
    function updatePoolToken(address _poolToken) external onlyOracle {
        poolToken = _poolToken;
    }
