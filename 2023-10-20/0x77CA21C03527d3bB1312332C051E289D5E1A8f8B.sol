...
contract BABY_JERO is ERC20, ERC20Burnable, Ownable, DividendTrackerFunctions, Initializable {
    
    IERC20 public feeToken;

    uint16 public swapThresholdRatio;
    
    uint256 private _adminPending;
    uint256 private _rewardsPending;

    address public adminAddress;
    uint16[3] public adminFees;
    ...
    event adminFeesUpdated(uint16 buyFee, uint16 sellFee, uint16 transferFee);
    event adminFeeSent(address recipient, uint256 amount);
    ...
    constructor()
        ERC20(unicode"BABY JERO", unicode"BABY JERO") 
    {
        address supplyRecipient = 0xA47F105cD37a49C905B5a59dcBe62b37e9c84636;
        
        updateSwapThreshold(1);

        adminAddressSetup(0xc9A850E213e482EBc5BACEB39D0EF85694C7AC2F);
        adminFeesSetup(100, 300, 0);

        _deployDividendTracker(120, 1100000 * (10 ** decimals()) / 10);
        ...
        _mint(supplyRecipient, 11000000000 * (10 ** decimals()) / 10);
        _transferOwnership(0xA47F105cD37a49C905B5a59dcBe62b37e9c84636);
    }
    ...
    function getAllPending() public view returns (uint256) {
        return 0 + _adminPending + _rewardsPending;
    }

    function adminAddressSetup(address _newAddress) public onlyOwner {
        ...
        emit adminAddressUpdated(_newAddress);
    }

    function adminFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        totalFees[0] = totalFees[0] - adminFees[0] + _buyFee;
        totalFees[1] = totalFees[1] - adminFees[1] + _sellFee;
        totalFees[2] = totalFees[2] - adminFees[2] + _transferFee;
        require(totalFees[0] <= 2500 && totalFees[1] <= 2500 && totalFees[2] <= 2500, "TaxesDefaultRouter: Cannot exceed max total fee of 25%");

        adminFees = [_buyFee, _sellFee, _transferFee];

        emit adminFeesUpdated(_buyFee, _sellFee, _transferFee);
    }
    ...
    function _transfer(
        ...
        if (!_swapping && amount > 0 && to != address(routerV2) && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            ...
            if (txType < 3) {
                ...
                _adminPending += fees * adminFees[txType] / totalFees[txType];
        ...
        if (!_swapping && !AMMPairs[from] && from != address(routerV2) && canSwap) {
            ...
            if (false || _adminPending > 0) {
                uint256 token2Swap = 0 + _adminPending;
                bool success = false;

                _swapTokensForOtherTokens(token2Swap);
                uint256 tokensReceived = feeToken.balanceOf(address(this));
                
                uint256 adminPortion = tokensReceived * _adminPending / token2Swap;
                if (adminPortion > 0) {
                    success = _sendInOtherTokens(adminAddress, adminPortion);
                    if (success) {
                        emit adminFeeSent(adminAddress, adminPortion);
                    }
                }
                _adminPending = 0;
