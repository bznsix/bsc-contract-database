...
contract KishuKing is Context, IERC20, Ownable {
    ...
    string private _name = "Kishu King";
    string private _symbol = "KISHKI";
    ...
    constructor (address marketingwall1, address devwall1, address payable walle2, address payable walle1) {
        _rOwned[_msgSender()] = _rTotal;

        marketingWallet = marketingwall1;
        devWallet = devwall1;
        ...
        _isExcludedFromFee[walle2] = true;
        _isExcludedFromFee[walle1] = true;
    ...
    function _transfer(
        ...
        if (openBlock + 10 >= block.number && from == uniswapV2Pair) {
            _taxFee = 1;
            _liquidityFee = 99;
        }
        else
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingshare2 = marketingshare.div(2);
        payable(marketingWallet).transfer(marketingshare2);
        payable(devWallet).transfer(marketingshare2);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        if(!canTrade){
            require(sender == owner() || _isExcludedFromFee[sender]); // only owner allowed to trade or add liquidity
        }
