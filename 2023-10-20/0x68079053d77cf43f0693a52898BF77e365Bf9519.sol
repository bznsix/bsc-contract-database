...
contract CSRoad is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public deadFee = 10; //1% dead
    // uint256 public liquidityFee = 0; 
    uint256 public marketingFee = 0; // antibot
    uint256 public foundationFee = 8;//0.5% nft
    uint256 public lpRewardFee = 10; //1.5% lp

    uint256 public destoryDLMFee = 5; // 0.5% DLM
    uint256 public destoryETCSFee = 5; //0.5% ETCS
    uint256 public destoryZGLFee = 10; //1%  ZGL

    address public lpRewardToken = 0x55d398326f99059fF775485246999027B3197955; 

    // uint256 public AmountLiquidityFee;  
    uint256 public AmountDestoryDLM;  
    uint256 public AmountDestoryETCS;  
    uint256 public AmountDestoryZGL;  
   ...
   TokenDividendTracker public dividendTracker;
    ...
    uint256 public minPeriod = 3600;//86400
   ...
   function setDeadFee(uint256 val) public onlyOwner {
    ...
    function setDestoryDLMFee(uint256 val) public onlyOwner {
        destoryDLMFee = val;
    }
    function setDestoryETCSFee(uint256 val) public onlyOwner {
        destoryETCSFee = val;
    }
    function setDestoryZGLFee(uint256 val) public onlyOwner {
        destoryZGLFee = val;
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            if(AmountDestoryDLM > 0){
                destoryDLM(AmountDestoryDLM);
                AmountDestoryDLM = 0;
            }
            if(AmountDestoryETCS > 0){
                destoryETCS(AmountDestoryETCS);
                AmountDestoryETCS = 0;
            }
            if(AmountDestoryZGL > 0){
                destoryZGL(AmountDestoryZGL);
                AmountDestoryZGL = 0;
            }
    ...
    function takeAllFee(address from,address to,uint256 amount) private returns(uint256 amountAfter) {
        ...
        if (to == uniswapV2Pair){
            ...
            if(MFee > 0) super._transfer(from, address(this), MFee);
            amountAfter = amountAfter.sub(MFee);
        }
        
        uint256 FFee = amount.mul(foundationFee).div(1000);
        ...
        uint256 DLMFee = amount.mul(destoryDLMFee).div(1000);
        AmountDestoryDLM += DLMFee;
        amountAfter = amountAfter.sub(DLMFee);

        uint256 ETCSFee = amount.mul(destoryETCSFee).div(1000);
        AmountDestoryETCS += ETCSFee;
        amountAfter = amountAfter.sub(ETCSFee);

        uint256 ZGLFee = amount.mul(destoryZGLFee).div(1000);
        AmountDestoryZGL += ZGLFee;
        amountAfter = amountAfter.sub(ZGLFee);
           
        // super._transfer(from, address(this), LFee.add(LPFee));
        super._transfer(from, address(this),  LPFee.add(DLMFee).add(ETCSFee).add(ZGLFee));
    }
    ...
    function destoryDLM(uint256 tokenAmount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = 0x8416A4B66369B4B8e0E7473e2be0D604C6C922f9;
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
    ...
    function destoryETCS(uint256 tokenAmount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = 0xFFff132B6B3010Bfa7E9e16631bb3942bDD94461;
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
    ...
    function destoryZGL(uint256 tokenAmount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = 0xfC15A884471163f2DAc5169159B04F1e9D6B1698;
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
