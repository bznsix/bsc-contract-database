...
contract GoldHero is ERC721Enumerable, Ownable {
    using Strings for uint256;
    using Address for address;
    struct Card {
        bool isValid;
    ...
    string private _uri = "https://nft.gpmmgusd.com/";
    mapping(uint256 => string) public cids;
    uint256 public carTotal;
    ...
    uint public perUSDT;
    uint private _lastBalanceUSDT;
    uint public rewardTotalUSDT;
    ...
    constructor() ERC721("GoldHero", "GoldHero") {
        manager = 0xdCfeb4294bf8c6fd3AEfE1E196ca227bd90585Eb;
    ...
    function mint(address to, uint256 level) public onlyBox returns (uint256) {
        ...
        carTotal++;
        while (_exists(carTotal)) {
            carTotal++;
        }
        levelCount[level]++;
        levelValid[level]++;
        _cards[carTotal] = Card({
            isValid: true,
            level: level,
            debt: perUSDT / 1e12,
            total: 0,
        ...
        super._safeMint(to, carTotal);
        emit Mint(to, carTotal, level, levelCount[level]);
        if (!users[to].isExist) {
    ...
    function mintBatch(address to, uint256 level, uint amount) public onlyBox {
        require(level > 0, "Level Error");
    ...
    function getUserInfo(
        ...
        user = users[account];
        uint256 _perUSDT = perUSDT;
        if (_TOKEN.balanceOf(address(this)) > _lastBalanceUSDT) {
    ...
    function claimReward() public {
        ...
        for (uint256 i = 0; i < cardNum; i++) {
            ...
            uint256 reward = perUSDT / 1e12 - card.debt;
            card.total += reward;
            card.debt = perUSDT / 1e12;
            total += reward;
        }
        user.balance += total;
        user.total += total;
        if (user.balance > 0) {
            _TOKEN.transfer(msg.sender, user.balance);
            _lastBalanceUSDT = _TOKEN.balanceOf(address(this));
            user.balance = 0;
        }
    }
    ...
    function sendReward(uint gas) public {
        ...
        for (uint i = 0; i < total; i++) {
            ...
            tokenId = tokenByIndex(currentIndex);
            Card storage card = _cards[tokenId];
            reward = perUSDT / 1e12 - card.debt;
            if (reward > 0) {
                card.total += reward;
                card.debt = perUSDT / 1e12;
