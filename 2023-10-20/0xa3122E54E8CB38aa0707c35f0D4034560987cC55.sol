...
contract TradingCompetitionManager is Ownable, ReentrancyGuard, TradingCompetitionManagerEvents {
    ...
    uint256 public constant MAX_TOKEN_PRIZE = 1;
    ...
    constructor() {
        ...
        _router = address(0x6352a56caadC4F1E25CD6c75970Fa768A3304e64);
        _idCounter = 0;
    ...
    function create(TC calldata _tradingCompetition) public nonReentrant returns(address,uint) {
        ...
        emit Create(_tradingCompetition, _tc, msg.sender, _idCounter-1,  block.timestamp);
    ...
    function _check(TC calldata tc) internal view {
        ...
        require(tc.prize.totalPrize > 0, "TCM: totalPrize too low");
        require(tc.prize.host_contribution == tc.prize.totalPrize, "TCM: HOST CONTRIBUTION");
        // check prize
        uint256 i = 0;
        require(isAllowedToken[tc.prize.token], "TCM: token prize not allowed");
    ...
    function _getPrize(TC calldata tc, address _tc) internal {
        address _token = tc.prize.token;
        uint256 _amount = tc.prize.host_contribution;
...
abstract contract TradingCompetitionManagerEvents is ITradingCompetitionManager {
    ...
    event Create(TC indexed tradingCompetition, address indexed competition, address indexed caller,uint idCounter, uint blocktimestamp);
