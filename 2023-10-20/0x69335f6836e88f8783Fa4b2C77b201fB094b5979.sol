...
contract BTCL is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("BTCL", "BTCL") {
        require(_baseToken < address(this),"BTCL small");
    ...
    function setMultipleAccountsForParter(address[] calldata accounts, bool excluded) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            if(!haveLdxPush[accounts[i]] && excluded){
    ...
    function getLpBalanceByUsdt(uint256 usdtAmount) public view returns(uint256,uint256){
		...
		uint256 probablyLpAmount = pairTotalAmount.mul(usdtAmount).div(pairUSDTAmount).div(1000).mul(1011);
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair){
			...
			if(isDelLdx){
                ...
                if(startTime.add(7 * 86400) > block.timestamp){
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(9) > block.timestamp){
				...
				require(startTime < block.timestamp,"startTime");
                _splitUsdtLdxToken();
                if(startTime.add(900) > block.timestamp){
                    super._transfer(from, address(this), amount.div(100).mul(20));
    ...
    function _splitUsdtLdxToken() private {
        uint256 allTokenAmount = BTC.balanceOf(address(this));
			...
			if(buySize >15){
				for(uint256 i=0;i<15;i++){
                    ...
                    if(ldxRewardTokenAmount > 10**10){
                        BTC.transfer(user, ldxRewardTokenAmount);
                    }
