...
contract XRPFIRE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    XRPFIREDividendTracker public dividendTracker;
    ...
    constructor() ERC20("XRPFIRE", "XRPFIRE") {
        marketingWallet = payable(0x4cfCC0d7EC658cc8c9a3EEd05AA8Ed40C5920d5C);
        devWallet = payable(
0x31B74F810863bf239B8756A7f0792Bc1AcCa1783);
        ...
        dividendTracker = new XRPFIREDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 1000000; // 2%
        swapTokensAtAmount = totalTokenSupply / 1000000; // 2%;
...
contract XRPFIREDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    XRPFIRE public XRPFIREContract;
    ...
    constructor(
        ...
        defaultToken = token;
        XRPFIREContract = XRPFIRE(mainContract);
    ...
    function _transfer(
        ...
        require(false, "XRPFIRE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "XRPFIRE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main XRPFIRE contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(XRPFIREContract);

            uint256 prevBalance = XRPFIREContract.balanceOf(address(this));
            ...
            {
                uint256 received = XRPFIREContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    XRPFIREContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
