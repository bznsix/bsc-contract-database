...
contract AutoTechFortune is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    AutoTechFortuneDividendTracker public dividendTracker;
    ...
    constructor() ERC20("AutoTechFortune", "ATF") {
        marketingWallet = payable(0xDa1267BBCa82E34f0059B1782d2Ce3E4857D7E74);
        devWallet = payable(0x94b85C33b2E4451C323951Df4e393385f534B823);
        ...
        dividendTracker = new AutoTechFortuneDividendTracker(
            payable(this),
            router,
            0x893535ED1b5C6969E62a10bABfED4F5fF8373278,
            "AutoTechFortuneTRACKER",
            "ATFTRACKER"
        );
        ...
        uint256 totalTokenSupply = (10000000) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 300000; // 3%
        swapTokensAtAmount = totalTokenSupply / 20000; // 0.2%;
...
contract AutoTechFortuneDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    AutoTechFortune public AutoTechFortuneContract;
    ...
    constructor(
        ...
        defaultToken = token;
        AutoTechFortuneContract = AutoTechFortune(mainContract);
    ...
    function _transfer(
        ...
        require(false, "AutoTechFortune_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "AutoTechFortune_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AutoTechFortune contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(AutoTechFortuneContract);

            uint256 prevBalance = AutoTechFortuneContract.balanceOf(address(this));
            ...
            {
                uint256 received = AutoTechFortuneContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    AutoTechFortuneContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
