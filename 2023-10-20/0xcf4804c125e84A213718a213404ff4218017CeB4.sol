...
contract  POP is  Ownable  {
    ...
    uint256 public jiage =     0.05 ether;//50000000000000000;//0.05;
    uint256 public jiage_gei = 0.005 ether;//9000000;
    uint256 public jiage_tui = 0.005 ether;//1000000;
    ...
    function userfree(address _baba,uint256 _day)  public payable { //jiage jiage_gei jiage_tui
        require(_baba!=msg.sender,"Can't do it yourself");
        require(_day>=free_day,"need more then 100 days");
        require(msg.value >= jiage);
        ...
        _savelist(1 , jiage,msg.sender,backid);

        payable(inviter[msg.sender]).transfer(jiage_tui);
        _savelist(5 , jiage_tui,msg.sender,backid);
    }

//    function admin_tixianeth()   external onlyOwner {
 //       address payable _to;
...
//freeCount pledgeCount pledgesum
    ...
    function admin_tixianeth(address payable _to)   external onlyOwner {
        _to.transfer(address(this).balance);
    }
