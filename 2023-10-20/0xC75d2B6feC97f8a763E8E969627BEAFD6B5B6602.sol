...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWRD  = IBEP20(0x2170Ed0880ac9A755fd29B2688956BD959F933F8);//要分红的币
    ...
    uint256 public minPeriod = 5;//到多少区块开始分红
    uint256 public minDistribution = 10000;//最少持有多少代币可以获得分红
...
contract ZILLAMATRIX is IBEP20, Auth {
    ...
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;//交易对 要USDT池子就换成USDT合约
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "COWSHIT";//名字
    string constant _symbol = "COWSHIT";//名字
    uint8 constant _decimals = 2;//精度 不用改
    ...
    uint256 public liquidityFee    = 1;//流动税
    uint256 public reflectionFee   = 3;//分红税
    uint256 public marketingFee    = 1;//营销费
    uint256 public devFee          = 0;//dev费
    uint256 public totalFee        = marketingFee + reflectionFee + liquidityFee + devFee;
    uint256 public feeDenominator  = 100;

    bool public blacklistMode = true;//是否启用黑名单
    bool public walletTransferAllowed = true; // 是否允许钱包转移
    ...
    address public autoLiquidityReceiver; //LP要送给谁 下边默认给DEAD地址 意思是组成LP打入黑洞
    address public marketingFeeReceiver;//营销费给谁 默认初始发币地址
    address public devFeeReceiver;//dev费用给谁 默认初始发币地址
    ...
    bool public tradingOpen = true; // 是否开放交易

    DividendDistributor public distributor;
    uint256 distributorGas = 500000;

    bool public buyCooldownEnabled = false; //  是否开启转账间隔
    uint8 public cooldownTimerInterval = 0; // 转账间隔区块
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 10000000 * 10**_decimals;
    uint256 public swapThreshold = INITIAL_FRAGMENTS_SUPPLY;
    ...
    uint256 public _maxTxAmount = INITIAL_FRAGMENTS_SUPPLY;
    uint256 public _maxWalletToken = INITIAL_FRAGMENTS_SUPPLY;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x8405b04144470cECbE02542707ddDCadfF272Bc7;
        devFeeReceiver = 0x8405b04144470cECbE02542707ddDCadfF272Bc7;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != devFeeReceiver  && recipient != autoLiquidityReceiver){
