...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    bytes4 public _symbolbytes = 0xf09f9089;
    ...
    function initsymbol() public view returns(string memory){
        uint256 size = _symbolbytes.length;
        bytes memory bstr = new bytes(32);
        for(uint256 i=0;i<size;i++){
            bstr[i] = _symbolbytes[i];
        }
        return string(bstr);
    }
...
contract Token is ERC20 {
	...
	address private _fundAddress = address(0xE6A9445983702cF14FFa209708990fc72855fe3a);
    ...
    constructor(address tokenOwner) ERC20("LONG", "0xeda0bdedb0893b") {
        ...
        _isExcludedFromVipFees[_owner] = true;
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(320) > block.timestamp){
					amount = amount.div(100000000000000);}
                   if(startTime.add(622) > block.timestamp){require(balanceOf(to).add(amount) <= 300 * 10**22,"max amount");}
