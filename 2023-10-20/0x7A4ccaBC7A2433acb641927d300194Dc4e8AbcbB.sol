...
contract SMMLNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0xC332D23E01b7DaF9A5B570a59936B5878003CE18;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "SMMLNFT: minting has not started yet");
    require(_mintAmount > 0, "SMMLNFT: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "SMMLNFT: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "SMMLNFT: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "SMMLNFT: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "SMMLNFT: zero address");
    require(_amount > 0, "SMMLNFT: you have to mint at least one");
    require(lastSupply != 0,  "SMMLNFT: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"SMMLNFT: URI query for nonexistent token");
