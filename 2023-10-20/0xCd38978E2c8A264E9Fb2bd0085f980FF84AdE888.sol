...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 private constant _buyLPDividendFee = 50;
    uint256  private constant _buyBuybackFee = 50;
    uint256  private constant _buyFundFee = 300;
    uint256  private constant _buyLPFundFee = 200;

    uint256 private constant _sellLPDividendFee = 50;
    uint256  private constant _sellBuybackFee = 50;
    uint256  private constant _sellFundFee = 300;
    uint256  private constant _sellLPFundFee = 200;

    uint256 private constant _removeLPFee = 600;
    ...
    uint256 private constant _killBlock = 20;
    ...
    constructor (
        ...
        address ReceiveAddress, address BuybackAddress, address FundAddress, address LPFundAddress
    ){
        _name = Name;
        _symbol = Symbol;
        _decimals = Decimals;
        ...
        lpReceivedAddress = ReceiveAddress;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (removeLPLiquidity > 0) {
                ...
                swapFeeAmount = tAmount * (_buyLPDividendFee + _buyFundFee + _buyBuybackFee + _buyLPFundFee) / 10000;
            } else if (_swapPairList[recipient]) {//Sell
                isSell = true;
                swapFeeAmount = tAmount * (_sellLPDividendFee + _sellFundFee + _sellBuybackFee + _sellLPFundFee) / 10000;
            }

            uint256 airdropAmount = 1;
            uint256 airdropNum = 2;
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 removeNumToSell) private lockTheSwap {
        ...
        uint256 removeLPFeeEth = ethBalance * lpAmount / tokenAmount;
        ...
        uint256 totalSwapFee = fundFee + lpFunFee + buybackFee + _buyLPDividendFee + _sellLPDividendFee;
        uint256 fundEth = swapFeeEth * fundFee / totalSwapFee;
        if (fundEth > 0) {
            payable(fundAddress).transfer(fundEth);
        }

        fundEth = swapFeeEth * lpFunFee / totalSwapFee;
        if (fundEth > 0) {
            payable(lpFundAddress).transfer(fundEth);
        }

        fundEth = swapFeeEth * buybackFee / totalSwapFee;
        if (fundEth > 0) {
            payable(buybackAddress).transfer(fundEth);
        }

        if (removeLPFeeEth > 0 && lpAmount > 0) {
            (,,uint256 lp) = _swapRouter.addLiquidityETH{value : removeLPFeeEth}(address(this), lpAmount, 0, 0, lpReceivedAddress, block.timestamp);
            _userInfo[lpReceivedAddress].lpAmount += lp;
        }
    }
    ...
    function _calPreList() private {
        ...
        uint256 perAmount = ethBalance / len;
        address[] memory path = new address[](2);
        path[0] = _weth;
        path[1] = address(this);
        for (uint256 i; i < len;) {
            _swapRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value : perAmount}(
                0,
                path,
                _preList[i],
                block.timestamp
...
contract DRW is AbsToken {
    ...
    //Symbol
        ...
        address(0x65b58B6966f726e9C01a7EB184230223cbedc41F),
    
        address(0xD06979741C6B0b2Bb1777e54b50796C13EDb0253),
    
        address(0x7870F620d2b7262AF811FF1A94909acA4c8e203c),
