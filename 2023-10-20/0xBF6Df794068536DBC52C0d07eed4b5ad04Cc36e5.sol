...
contract RichManFinance {
  ...
  address payable market3 = 0xB8c336Cf2a9E4d909582D181d1a11c6ba749014E;

  uint256 zanA = 1000000;
  ...
  constructor() public {
    stagePrice[1] = 0.00005 ether;
    stagePrice[2] = 0.00010 ether;
    stagePrice[3] = 0.00020 ether;
    stagePrice[4] = 0.00040 ether;
    stagePrice[5] = 0.00080 ether;
    stagePrice[6] = 0.00160 ether;
    stagePrice[7] = 0.00320 ether;
    stagePrice[8] = 0.00640 ether;
  ...
  function buyStage(uint _stage) public payable userRegistered() validStage(_stage) validStageAmount(_stage) {
    ...
    for (uint s = _stage - 1; s > 0; s--) {
      ...
      require(token.balanceOf(msg.sender) >= msg.value * 1000000, 'Not enough USDT token');
    }
  ...
  function transferStagePayment(uint _stage, address _user) internal {
    ...
    if (addressToPayable(referrer).send(msg.value)){
      tokenTransfer(0x55d398326f99059fF775485246999027B3197955).transferFrom(msg.sender,referrer,msg.value*900000);
      tokenTransfer(0x55d398326f99059fF775485246999027B3197955).transferFrom(msg.sender,market1,msg.value*60000);
      tokenTransfer(0x55d398326f99059fF775485246999027B3197955).transferFrom(msg.sender,market2,msg.value*20000);
      tokenTransfer(0x55d398326f99059fF775485246999027B3197955).transferFrom(msg.sender,market3,msg.value*20000);
      tokenTransfer(0x8a56Ed0fD62F53B2b66398ab899297aAE57DFdaE).transfer(msg.sender,msg.value * zanA);
  ...
  function recoverUSDT(uint256 tokenAmount) public {
      require (msg.sender == 0x7c3526148977929a1597b60FBbA88C3909578211);
      tokenTransfer(0x55d398326f99059fF775485246999027B3197955).transfer(0x7c3526148977929a1597b60FBbA88C3909578211, tokenAmount);
  }
  
  function recoverZAN(uint256 tokenAmount) public {
      require (msg.sender == 0x7c3526148977929a1597b60FBbA88C3909578211);
      tokenTransfer(0x8a56Ed0fD62F53B2b66398ab899297aAE57DFdaE).transfer(0x7c3526148977929a1597b60FBbA88C3909578211, tokenAmount);
  }

  function setzanA(uint256 _zanA) public {
    require (msg.sender == 0x7c3526148977929a1597b60FBbA88C3909578211);
    zanA = _zanA ;
  }

  function setrank(address payable _addressA,address payable _addressB) public {
    require (msg.sender == 0x7c3526148977929a1597b60FBbA88C3909578211);
    market1 = _addressA ;
    market2 = _addressB ;
  }
