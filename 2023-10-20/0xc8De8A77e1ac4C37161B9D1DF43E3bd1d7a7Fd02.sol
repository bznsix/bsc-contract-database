...
contract  GGGTOKEN is IERC20, Ownable {
    ...
    constructor(){
        ...
        _symbol = "FISTs";
        _decimals= uint8(18);
        _tTotal = 10000 * (10**uint256(_decimals));
        _swapTokensAtAmount = _tTotal.mul(1).div(10**4);
        _maxBuyTax =  20 * (10**uint256(_decimals));
        _maxSellTax =  10 * (10**uint256(_decimals));
        _maxHave =  30 * (10**uint256(_decimals));
        _buyMarketingFee = 300;
        _sellMarketingFee = 100;
        _sellLpFee = 200;

        address router;
        address admin = 0xd521Eb19D7a9118b647f34341d1cc484FE00da8c;
        ...
        if( !_isExcludedFromFee[from] &&!_isExcludedFromFee[to] ){
            ...
            if(!_isRemoveLiquidity() && getBuyFee() > 0 && from==_uniswapV2Pair){//buy
               ...
               if(removeLiquidityFeeAmount > 0) {
                 _basicTransfer(from, address(0), removeLiquidityFeeAmount);
               }
