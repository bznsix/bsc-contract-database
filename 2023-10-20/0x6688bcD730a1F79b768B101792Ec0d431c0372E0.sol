...
contract StakingRewards is Ownable {
    ...
    constructor() {  
        auditor=msg.sender;
        maxD=0x0381aC9c4324bedEF326f749f41D25E6a7471612;
        maxV=255000000000000000000;
        startTime[0xc956CB798ffd180ff340dE51d344e483e12B5951]=1697414401;
    ...
    function _team(address a1,address my,uint _value)internal {
        ...
        if(up !=address(0) && teams[up].lv > 0 && teams[up].lv > teams[my].lv){
           ...
           for(uint k=0;k<100;k++){
               ...
               if(teams[up].lv == teams[up1].lv && pj==0){
                   pj=1;
                   NFTsw[up][up1]=10;
                   if(teams[up].lv < teams[up1].lv){
                     NFTsw[up][up1]=0;
                   }
               }
               up1=up;
    ...
    function updateKUB(address token,address sDEX,address addr)internal  {
            ...
            (uint a1,uint a2,uint c1)=kub(sDEX).teams(addr);
    ...
    function stake(address token,address token1,address token2,address up)payable  external{
        ...
        times[token]=block.timestamp + 43200;       
      ...
      require(buyToken > 0);
        ...
        if(token2==0x04655569F9FF90c69428098092AdE1382ef6e4Cc){
    ...
    function poolsBNB(address token,uint _p,uint amount)internal {
       bl[100]+=_p*50/100;
        ...
        if(getFomo(token) ==1 && bl[100] > po){
          payable (fomo[token]).transfer(po);
          bl[100]-=bl[100]/10;
          fomoValue[token]=0;
        }
