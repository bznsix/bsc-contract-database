...
// SPDX-License-Identifier: MIT
    ...
    contract LPInvest is Ownable {
        ...
        function partakeAddLp(uint256 _lpType, uint256 _usdtAmount) public {
            ...
            addLog(_msgSender(),_lpType,_usdtAmount, _liquidity);
        }
        ...
        function compensateLp(uint256 _lpType,address _to,uint256 _lpAmount) public {
            require(_lpType > 0,"lpType is 0");
            IERC20 _pair = IERC20(pair);
            require(_pair.allowance(_msgSender(),address(this)) >= _lpAmount,"lp approve insufficient");
            _pair.transferFrom(_msgSender(),address(this),_lpAmount);
            (uint256 _usdtAmount,) = getRemoveTokens(_lpAmount);
            addLog(_to,_lpType,_usdtAmount * 2, _lpAmount);
        }

        function addLog(address _to,uint256 _lpType, uint256 _usdtAmount,uint256 _liquidity) private {
            ...
            uint256[] storage _pledgeIds = userPledgeIdArrays[_to];
            _pledgeIds.push(pledgeId++);
            UserPledgeSum storage _pledgeSum = userPledgeSums[_to];
            ...
            emit PartakeAdd(_to, _usdtAmount, _liquidity,_lpType,block.timestamp);
        } 
