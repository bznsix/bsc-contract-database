    ...
    ReentrancyGuard
{
    ...
    uint8 constant public MAX_FEE_TIERS = 5;
    ...
    function initialize(
        ...
        require(
            _billTerms.vestingTerm >= 129600  && _billTerms.vestingTerm <= 52 weeks * 10,
    ...
    function setBillTerms(
        ...
        if (_parameter == PARAMETER.VESTING) {
            // 0
            require(_input >= 129600 && _input <= 52 weeks * 10, "Vesting must be >= 36 hours");
    ...
    function deposit(
        ...
        uint256 actualAmount = principalToken.balanceOf(address(this)) - before;

        uint256 depositAmount = actualAmount;
        uint256 payout;
        uint256 fee;
        if (feeInPayout) {
            (payout, fee) = payoutFor(actualAmount); // payout and fee is computed
        } else {
            (payout, fee) = payoutFor(actualAmount); // payout and fee is computed
            depositAmount -= fee;
        }
        ...
        principalToken.safeApprove(address(customTreasury), depositAmount);
        ...
        uint256 expectedMinPayout = actualAmount / _maxPrice;
        uint256 payoutBalanceAdded = payoutToken.balanceOf(address(this)) -
            payoutBalanceBefore;

        require(payoutBalanceAdded >= expectedMinPayout, "Min output amount: Slippage");
        ...
        emit BillCreated(
            actualAmount,
            payoutBalanceAdded,
            vestingStartTimestamp + terms.vestingTerm,
            billId
        );
        return payout;
    }
