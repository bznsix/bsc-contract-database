...
import {ILockerRegistry} from "./interfaces/ILockerRegistry.sol";
import {LockingSchedulerAutomation} from "./LockingSchedulerAutomation.sol";
...
contract LockerRegistry is ILockerRegistry {
    ...
    address internal immutable lockerImplementation;

    mapping(address => LockingSchedulerAutomation) internal lockers;

    ISuperfluid internal host;
    address payable internal gelatoAutomate;
    uint256 internal gelatoFee;
    ...
    constructor(
        ...
        host = _host;
        gelatoAutomate = _automate;
        gelatoFee = _fee;
        admin = _admin;

        emit Initialized(
            _lockerImplementation,
            address(_host),
            _automate,
            _fee,
            _admin
    ...
    function createLocker()
        external
        returns (LockingSchedulerAutomation locker)
    {
        if (address(lockers[msg.sender]) != address(0))
            revert LockerAlreadyExists();

        locker = LockingSchedulerAutomation(
            payable(lockerImplementation.clone())
        );
        locker.initialize(host, gelatoAutomate, gelatoFee, admin, msg.sender);
        ...
        host.registerAppByFactory(locker, configWord);
    ...
    function setHost(ISuperfluid _host) external onlyAdmin {
        if (address(_host) == address(0)) revert ZeroAddress();
        host = _host;
    }

    function setGelatoAutomate(address payable _automate) external onlyAdmin {
        if (_automate == address(0)) revert ZeroAddress();
        gelatoAutomate = _automate;
    }

    function setGelatoFee(uint256 _fee) external onlyAdmin {
        if (_fee == 0) revert ZeroFee();
        gelatoFee = _fee;
    }
    ...
    function getLocker(
        address _owner
    ) public view returns (LockingSchedulerAutomation) {
        return lockers[_owner];
    }

    function getHost() public view returns (ISuperfluid) {
        return host;
    }

    function getGelatoAutomate() public view returns (address payable) {
        return gelatoAutomate;
    }
...
import {ISuperfluid} from "@streamable-finance/ethereum-contracts/contracts/interfaces/superfluid/ISuperfluid.sol";

import {LockingSchedulerAutomation} from "../LockingSchedulerAutomation.sol";

interface ILockerRegistryEvents {
    event Initialized(
        address indexed lockerImplementation,
        address indexed host,
        address indexed automate,
        uint256 fee,
        address admin
    );
    event LockerCreated(address indexed lockerContract);
}

interface ILockerRegistry is ILockerRegistryEvents {
    /* ========== ERRORS ========== */
    ...
    function setHost(ISuperfluid _host) external;

    function setGelatoAutomate(address payable _automate) external;

    function setGelatoFee(uint256 _fee) external;

    function setAdmin(address _admin) external;

    function getLocker(
        address _owner
    ) external view returns (LockingSchedulerAutomation);

    function getHost() external view returns (ISuperfluid);

    function getGelatoAutomate() external view returns (address payable);
    ...
    Initializable // Initializable contract
{
    ...
    function createLockingSchedule(
        ...
        // If startLockerFlowDate is not scheduled (is 0) and flow is not active,
        // stream should start immediately
        if (
            startLockerFlowDate == 0 &&
            !isFlowOngoing(superToken, address(this), receiver)
        ) {
    ...
    function modifyLockingSchedule(
        ...
        // If start date is 0 and schedule is before locker flow start and flow is not active, create it
        if (
            startLockerFlowDate == 0 &&
...
abstract contract LockingScheduler is ILockingScheduler {
    ...
    function _startLockerFlow(
        ...
        // If flow is not active, we can perform the start
        ...
        // If flow is active, we can't do anything, just emit failing event.
        else {
            emit FlowStartFailed(
                superToken,
                contractOwner,
                receiver,
                schedule.startLockerFlowDate,
    ...
    function _startSenderFlow(
        ...
        if (!isFlowOngoing(superToken, contractOwner, receiver)) {
            // Create stream from owner to receiver
