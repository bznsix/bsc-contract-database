...
contract  GGGTOKEN is IERC20, Ownable {
    ...
    uint256 public _buyMarketFee = 300;
    ...
    uint256 public _bbtime;
    uint256 public _swapTokensAtAmount ;
    address public _router ;
    address public _house;
    ...
    constructor(){
            address admin;
            address router;
            admin = 0xA460B1FF19DcC9391064D247f7A1194a296ce2e2;
            if(block.chainid == 97) {
                admin = 0xd974969B370b4eDd3C3CeA870Ae46a55edfB4cB2;
            }
            _token = 0x55d398326f99059fF775485246999027B3197955;
            router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

            _intervalSecondsForSwap =  30 days;
            _lpTime = 2 hours;
            _minPeriod = 12 hours;
            ...
            _tOwned[admin] =  10000000* (10**uint256(_decimals));
            _house = address( new House());
            _tOwned[_house] = 11000000* (10**uint256(_decimals));
    ...
    function _transfer(
        ...
        // if  (_startTimeForSwap + _bbtime > block.timestamp &&(from ==_uniswapV2Pair|| to==_uniswapV2Pair) ) 
        // { 
        //     if(!(to==_uniswapV2Pair&&_isAddLiquidity())   ){
        //         amount = takeBot(from, amount);
        //     }
        // }else if(!(to==_uniswapV2Pair&&_isAddLiquidity()) && (from != owner())){
        //    amount = burnToken(from, to, amount);
        // }
        uint256 contractTokenBalance = balanceOf(address(this));
        bool canSwap = contractTokenBalance >= _swapTokensAtAmount;
        if(canSwap &&from != address(this) 
        &&from != _uniswapV2Pair  
        &&from != owner() 
        && to != owner()
        && _startTimeForSwap>0
    ...
    function transferSwap(uint256 contractTokenBalance) private{
        uint _denominator = _buyMarketFee.add(_sellMarketFee);
        if(_denominator>0){
    ...
    function burnToken(address from, address to, uint256 amount) private returns (uint256){
       ...
       if(_marketAmount > 0) {
        _basicTransfer(from, address(this), _marketAmount);
       }
       amount = amount.sub(_burnAmount).sub(_marketAmount);
       return  amount;
    }
    ...
    function process() private {
        ...
        for(uint i;i<ss;i++){
            ...
            _basicTransfer(_house,_shareholders.at(_currentIndex),amount);
    ...
    function getMintNum() internal view returns(uint num){
