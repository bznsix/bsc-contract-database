...
library AlgebraUtils {
    ...
    function swap(
        ...
        IUniswapRouterV3WithDeadline.ExactInputParams
            memory params = IUniswapRouterV3WithDeadline.ExactInputParams({
                path: _path,
    ...
    function pathToRoute(bytes memory _path)
        internal
        pure
    ...
    function routeToPath(address[] memory _route)
        internal
        pure
...
contract ZapStrategyThenaGamma is OwnableUpgradeable {
    ...
    bytes public nativeToLp0Path; // 0x
    bytes public nativeToLp1Path; // 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095cCC42724C6683B7E57334c4E856f4c9965ED682bD
    ...
    function initialize(
        ...
        want = 0x831A0Db124E830c8fcE65Af2F3527F1AC5401E3F;

        vault = 0x242Eb69D4f379e7e0Cc1289b0CE959Da75d14168;
    ...
    function depositBNB() public payable {
        IWBNB(native).deposit{value: msg.value}();
        addLiquidity();
        depositFor();
        withdraw();
    }
    ...
    function deposit(uint256 _amount) public {
        IERC20(native).safeTransferFrom(msg.sender, address(this), _amount);
        addLiquidity();
        depositFor();
        withdraw();
    }

    // Puts the LPs to work
    ...
    function addLiquidity() internal {
        ...
        (uint256 amount1Start, uint256 amount1End) = gammaProxy
            .getDepositAmount(want, lpToken0, lp0Bal);
        ...
        returns (uint256 toLp0, uint256 toLp1)
    {
        ...
        if (isFastQuote) {
            ...
            uint256 price = ((sqrtPriceX96**2 *
                ...
                decimalsDenominator;
            (uint256 amountStart, uint256 amountEnd) = gammaProxy
    ...
    function depositFor() internal {
        IVault(vault).depositFor(
    ...
    function _giveAllowances() internal {
        ...
        IERC20(want).approve(vault, type(uint256).max);
    ...
    if (lpToken0Bal > 0) {
        IERC20(lpToken0).safeTransfer(msg.sender, lpToken0Bal);
    }
    if (lpToken1Bal > 0) {
        IERC20(lpToken1).safeTransfer(msg.sender, lpToken1Bal);
    }
    }
}
