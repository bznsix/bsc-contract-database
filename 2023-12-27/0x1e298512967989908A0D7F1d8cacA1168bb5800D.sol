...
contract Contract {
    ...
    uint256 public _operate_rate;

    constructor(address _receive,address _operate,address _miner) {
        manager = msg.sender;
        rAdmin = _receive;
        miner = _miner;
        operate = _operate;
        _asset_rate = 80;
        _miner_rate = 20;
        _operate_rate = 100-_asset_rate-_miner_rate;
    }
    ...
    function buy(IERC20 _token, uint256 _orderId, uint256 _amount) public {
        ...
        if(_operate_rate > 0){
            uint _operate = _amount*_operate_rate/100;
            IERC20(_token).transferFrom(msg.sender, operate, _operate);
        }
    ...
    // set operate
    function setOperate(address _addr) public onlyManager returns(address) {
        operate = _addr;
        return operate;
    }
    ...
    function setRate(uint256 asset_rate,uint256 miner_rate) public onlyManager returns(uint256, uint256) {
        ...
        _operate_rate = 100 - _asset_rate - _miner_rate;
        return (_asset_rate,_operate_rate);
    }

    // get rate
    function getRate() view public onlyManager returns(uint256, uint256, uint256) {
        return (_asset_rate,_miner_rate,_operate_rate);
    }
    ...
    function batchTransfer(IERC20 _token, address[] memory _tos, uint256[] memory _amounts, uint256[] memory _total) public {
        ...
        IERC20(_token).transferFrom(msg.sender, operate, _operate_amount);
    }
}