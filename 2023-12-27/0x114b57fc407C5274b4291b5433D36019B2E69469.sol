...
interface IStakingFounders {

    function stake(uint amount) external;

    function updatePosition(uint256 id, uint256 amount) external;

    function withdraw(uint id, uint amount) external;

    function stakeForSwap(address user, uint amount) external;
    
    function updatePositionForSwap(address user, uint256 id, uint256 amount) external;
...
import "./IStakingFounders.sol";
import "./IStakingControl.sol";
import "./IDPAToken.sol";
import "./ICoinToken.sol";

contract MigrateToken is AccessControl, ReentrancyGuard, Calculations {
    ...
    IDPAToken public immutable DPAToken;
    IERC20 public immutable oldDisrToken;

    IStakingFounders public staking;
    IStakingControl public stakeControl;

    bool public paused;

    uint256 private constant MAX_UINT = 2**255;
    uint256 private change = 10000000000000000000; //10 tokens
    uint256 private toStakeAmount = 1000000000000000000000; //1000 new Tokens
    uint256 public minMigrate = 10000000000000; //0.00001 Tokens
    uint256 public swapFee = 25000000000000000; //0.025 BNB

    address public feeAddress = 0xe9F355c451063ed9dD98A69B254fe55478F92579;

    event GetNewToken(address indexed user, uint256 indexed oldBalance, uint256 indexed newBalance, bool staking);

    constructor(address _DPAToken, address _oldDisrToken, address _staking, address _stakeControl){
        ...
        DPAToken = IDPAToken(_DPAToken);
        oldDisrToken = IERC20(_oldDisrToken);
        staking = IStakingFounders(_staking);
        approved(_staking);
    ...
    function getNewToken(bool stake) public payable nonReentrant {
        if (msg.value != swapFee){
            revert("fee is missing");
        }

        safeTransferNative(swapFee); 
        
        uint256 oldBalance = oldDisrToken.balanceOf(msg.sender);

        if(oldBalance < minMigrate) {
            revert("less than minimum");
        }

        uint256 newBalance = tokenQuantity(oldBalance);

        oldDisrToken.safeTransferFrom(msg.sender,address(this),oldBalance);

        ICoinToken(address(oldDisrToken)).burn(oldBalance);
        
        if(newBalance >= toStakeAmount || stake) {
            DPAToken.redeem(address(this), newBalance);
            openPosition(newBalance);
            emitEvent(msg.sender, oldBalance, newBalance, true);
        } else {
            DPAToken.redeem(msg.sender, newBalance);
            emitEvent(msg.sender, oldBalance, newBalance, false);
        }
        
    }

    function tokenQuantity(uint oldAmount) private view returns(uint256 quantity){
        quantity = (oldAmount * 1000000000000000000)/ change;
    }

    function openPosition(uint newBalance) private {
        uint id = stakeControl.getIdInfo(msg.sender);
        if(id == 0 ){
            staking.stakeForSwap(msg.sender, newBalance);
        }else {
            staking.updatePositionForSwap(msg.sender, id, newBalance);
        }
    }

    function safeTransferNative(uint256 value) private {
        (bool success, ) = feeAddress.call{value: value}(new bytes(0));
        require(success, 'Native transfer failed');
    }

    function approved(address _staking) public onlyDev {
        DPAToken.safeApprove(_staking,MAX_UINT);
    }

    function updateStaking(address newStaking) public onlyDev {
        staking = IStakingFounders(newStaking);
        approved(newStaking);
    }

     function updateStakingControl(address newStakeControl) public onlyDev {
        stakeControl = IStakingControl(newStakeControl);
    }

    function updateStakingAmount(uint256 newToStakeAmount) public onlyDev {
        toStakeAmount = newToStakeAmount;
    }

    function updateChangeRate(uint256 newChange) public onlyDev {
        change = newChange;
    }

    function updateMinMigrate(uint newMinimum) public onlyDev {
        minMigrate = newMinimum;
    }

    function updateSwapFee(uint256 newSwapFee) public onlyDev {
        swapFee = newSwapFee;
    }

    function updateFeeAddress(address newFeeAddress) public onlyDev {
        feeAddress = newFeeAddress;
    }

    function emitEvent(address user, uint oldBalance, uint newBalance, bool stake) private {
         assembly{
            //"GetNewToken(address,uint256,uint256,bool)";
            let signatureHash := 0xbce52efd46e2159641358af665f23828b31919e300c24d6f3802eb7137c03c31
            mstore(0, stake)
            log4(0, 0x20, signatureHash, user, oldBalance, newBalance)
        }
    }
}