...
contract blackholeAirdrop is Context{
    ...
    blackHoleAbi public _blackHole;
    IUniswapV2Router02 public uniswapRouter;
    bool inits = false;
    constructor(){}
    function init(address blackHoleAddr,address routeAddr)  public{
        require(!inits,'initsd');
        inits = true;
        _blackHole = blackHoleAbi(blackHoleAddr);
    ...
    function _drop() internal {
        ...
        if(msg.value == _value && !_blackHole.launch() && _msgSender != address(this)){
            ...
            if(_countBlock <10){
                ...
                IERC20 token = IERC20(address(_blackHole));
            ...
            if(totalDroped>= 150000 ){
                _blackHole.setLaunch(true);
                uint256 amount = payable(address(this)).balance;
                payable(address(_blackHole)).transfer(amount);
            }
        }
    }
...
contract BlackHole is ERC20, Ownable {
    ...
    bool swapIng;
    blackholeAirdrop public airdropAddr;
    // store addresses that a automatic market maker pairs. Any transfer *to* these addresses
    // could be subject to a maximum transfer amount
    mapping(address => bool) public automatedMarketMakerPairs;
    constructor() ERC20("BlackHole", "BHSC") {
        ...
        blackholeAirdrop _airdrop = new blackholeAirdrop();
        ...
        _mint(address(_airdrop),   18000000 * 10 ** 18);
        _mint(_msgSender(),        3000000 * 10 ** 18);
    }
    ...
    function _transfer(
        ...
        if (!_excludedFees[from] && !_excludedFees[to]) {
            ...
            uint256 fees;
            if (burnFee > 0) {
                uint256 _marketingFee = amount.mul(burnFee).div(300);
