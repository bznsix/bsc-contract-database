...
contract Emissary is ERC20, ERC20Burnable, Ownable, Mintable, Pausable, TokenRecover {
    
    mapping (address => bool) public blacklisted;

    address public emissaryAddress;
    uint16[3] public emissaryFees;
    ...
    event emissaryAddressUpdated(address emissaryAddress);
    event emissaryFeesUpdated(uint16 buyFee, uint16 sellFee, uint16 transferFee);
    event emissaryFeeSent(address recipient, uint256 amount);
    ...
    constructor()
        ERC20(unicode"Emissary", unicode"EMI") 
        Mintable(1000000000000)
    {
        address supplyRecipient = 0x1429D406Fba9d7B2D61AA15a914329636E17ad78;
        
        emissaryAddressSetup(0xa8Bf2DCb299d637d80556A05768354ab961B795F);
        emissaryFeesSetup(70, 70, 70);

        autoBurnFeesSetup(30, 30, 30);
        ...
        excludeFromLimits(emissaryAddress, true);

        updateTradeCooldownTime(30);

        _mint(supplyRecipient, 10000000000 * (10 ** decimals()) / 10);
        _transferOwnership(0x1429D406Fba9d7B2D61AA15a914329636E17ad78);
    }
    ...
    function emissaryAddressSetup(address _newAddress) public onlyOwner {
        emissaryAddress = _newAddress;

        excludeFromFees(_newAddress, true);

        emit emissaryAddressUpdated(_newAddress);
    }

    function emissaryFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        emissaryFees = [_buyFee, _sellFee, _transferFee];

        totalFees[0] = 0 + emissaryFees[0] + autoBurnFees[0];
        totalFees[1] = 0 + emissaryFees[1] + autoBurnFees[1];
        totalFees[2] = 0 + emissaryFees[2] + autoBurnFees[2];
        require(totalFees[0] <= 2500 && totalFees[1] <= 2500 && totalFees[2] <= 2500, "TaxesDefaultRouter: Cannot exceed max total fee of 25%");

        emit emissaryFeesUpdated(_buyFee, _sellFee, _transferFee);
    }
    ...
    function _transfer(
        ...
        if (!_swapping && amount > 0 && to != address(routerV2) && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            ...
            if (txType < 3) {
                ...
                if (emissaryFees[txType] > 0) {
                    emissaryPortion = fees * emissaryFees[txType] / totalFees[txType];
                    _sendInTokens(from, emissaryAddress, emissaryPortion);
                    emit emissaryFeeSent(emissaryAddress, emissaryPortion);
                }
                ...
                fees = fees - emissaryPortion - autoBurnPortion;
            }
