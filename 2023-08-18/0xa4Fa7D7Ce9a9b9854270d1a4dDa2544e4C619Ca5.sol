...
contract SmartContToken is Context, ERC20, Ownable {
    ...
    string private constant NAME = "SChain";
    string private constant SYMBOL = "SCN";
    uint8 private constant DECIMALS = 18;

    uint256 private constant DECIMALFACTOR = 10 ** uint256(18);
    uint256 private constant MAX = ~uint256(0);
    uint256 private constant _totalSupply = 590 * 10**6 * DECIMALFACTOR;
    uint256 private maxTransaction = 4 * DECIMALFACTOR;
    ...
    constructor(
        ...
        Swap_Route_ = _Swap_Route;
        IRouter02 _tokenSwapRoute = IRouter02(_Swap_Route);
        ...
        _approve(address(this), address(_Swap_Route), type(uint256).max);
    ...
    {
        return (
            buyItems[1].Development_Fee,
            buyItems[1].Marketing_Fee,
            buyItems[1].Listing_Fee
        );
    }
    ...
    {
        return (
            sellItems[1].Development_Fee,
            sellItems[1].Marketing_Fee,
            sellItems[1].Listing_Fee
        );
    }
     ...
     */
    function getTokenPrice(uint256 amountToken)
        public
        view
        virtual
        returns (uint256)
    {
        require(amountToken > 0, "Value is Invalid");
        IV2Pair _tokenPair = IV2Pair(tokenReserv);
        ...
        if (bnbAddress == _tokenPair.token0()) {
            ...
            return amountToken.div(pricebnb); //return amount of BNB needed to Transaction
        } else {
    ...
    function _transfer(
        ...
        if (
            ...
            developmentFee_ = buyItems[1].Development_Fee;
            marketingFee_ = buyItems[1].Marketing_Fee;
            listingFee_ = buyItems[1].Listing_Fee;
            is_buy = true;
        } else if (
            // is Sell transaction
            !automatedMarketMakerPairs[sender] &&
            ...
            if (executeMaxTransaction && !_isWhiteList[sender]) {
                require(_checkMaxTransaction(sender, amount), "ERC20: You have exceeded your daily transaction limit.");
            }
            
            developmentFee_ = sellItems[1].Development_Fee;
            marketingFee_ = sellItems[1].Marketing_Fee;
            listingFee_ = sellItems[1].Listing_Fee;
        }

        if (
            !automatedMarketMakerPairs[sender] && !automatedMarketMakerPairs[recipient] ||
            _isWhiteList[sender] || inSwapTransact
        ) {
            _removeAllFee();
        }

        uint256 tokenTransferAmount = _executeFeeTransfer(sender, amount, is_buy);
    ...
    function _checkMaxTransaction(address userAddr, uint256 amount) private returns(bool) {
        ...
        bool result = false;
        uint256 resetDaily = maxUserSell[userAddr].timestamp == 0 ? 0
        : (block.timestamp - maxUserSell[userAddr].timestamp) / 1 days;        
        if (!maxUserSell[userAddr].exist) {
            maxUserSell[userAddr].dailyAmount = 0;
        ...
        if (maxUserSell[userAddr].dailyAmount <= maxTransaction) {
           result = true;
        }
        maxUserSell[userAddr].timestamp = block.timestamp;
    ...
    function _executeFeeTransfer(address sender, uint256 tokenAmount, bool is_buy) private returns(uint256) {
        (uint256 transferAmount, uint256 tokeneDvelopment, uint256 tokenMarketing, uint256 tokenListing) = _getTransferAmount(tokenAmount);
        if (tokeneDvelopment > 0) {
            _tokenDevelopmentTotal += tokeneDvelopment;
            _sendToDevelopment(sender, tokeneDvelopment, is_buy);
        }
        if (tokenMarketing > 0) {
        ...
        if (tokenListing > 0){
            ...
            _sendToListing(sender, tokenListing, is_buy);
        }
        _tokenFeeTotal = _tokenFeeTotal.add(tokeneDvelopment).add(tokenMarketing).add(tokenListing);
        emit FeeTransaction(tokeneDvelopment, tokenMarketing, tokenListing);
        return transferAmount;
    }

    function _sendToDevelopment(address sender, uint256 tokeneDvelopment, bool is_buy) private {
        address DevelopmentAdd = address(this);
        if(is_buy){
            DevelopmentAdd = listingAddress;
        }
        
        _balance[DevelopmentAdd] += tokeneDvelopment;
        emit Transfer(sender, DevelopmentAdd, tokeneDvelopment);
    }
    ...
    function _sendToMarketing(address sender, uint256 tokenMarketing, bool is_buy) private {
        ...
        _balance[MarketingAdd] += tokenMarketing;
        emit Transfer(sender, MarketingAdd, tokenMarketing);
    }

    function _sendToListing(address sender, uint256 tokenListing, bool is_buy) private {
        address developmentAdd = address(this);
        if(is_buy){
            developmentAdd = developmentAddress;
        }
        _balance[developmentAdd] += tokenListing;
        emit Transfer(sender, developmentAdd, tokenListing);
    }
    ...
    {
        uint256 tokeneDvelopment = tokenAmount.mul(developmentFee_).div(100);
        uint256 tokenMarketing = tokenAmount.mul(marketingFee_).div(100);
        uint256 tokenListing = tokenAmount.mul(listingFee_).div(100);
        uint256 tokenTransferAmount = tokenAmount.sub(tokeneDvelopment).sub(tokenMarketing).sub(tokenListing);
        return (tokenTransferAmount, tokeneDvelopment, tokenMarketing, tokenListing);
    }
    ...
    function _removeAllFee() private {
        if (
            developmentFee_ == 0 && marketingFee_ == 0 && listingFee_ == 0
        ) return;
        developmentFee_ = 0; 
        marketingFee_ = 0;
        listingFee_ = 0;
    ...
    function createBuyFee(
        ...
        require((developmentFee.add(marketingFee).add(listingFee)) <= 10, "Total fees should not be more than 10%.");
        uint256 _buyId = 1;
    ...
    function updateBuyFee(
        ...
        require((developmentFee.add(marketingFee).add(listingFee)) <= 10, "Total fees should not be more than 10%.");
        buyItems[1].Development_Fee = developmentFee;
        buyItems[1].Marketing_Fee = marketingFee;
        buyItems[1].Listing_Fee = listingFee;
    }
    ...
    function updateSellFee(
        ...
        require((developmentFee.add(marketingFee).add(listingFee)) <= 10, "Total fees should not be more than 10%.");
        sellItems[1].Development_Fee = developmentFee;
        sellItems[1].Marketing_Fee = marketingFee;
        sellItems[1].Listing_Fee = listingFee;
    }
    ...
    function setDevelopmentAdress(address developmentAddress_) external onlyOwner {
        ...
        require(developmentAddress != developmentAddress_, "This is the same development address.");
    ...
    function setMarketingAdress(address marketingAddress_) external onlyOwner {
        ...
        require(marketAddress != marketingAddress_, "This is the same marketing address.");
    ...
    function setListingAdress(address listingAddress_) external onlyOwner {
        ...
        require(listingAddress != listingAddress_, "This is the same listing address.");
        listingAddress = listingAddress_;
        emit UpdateAddressFee(listingAddress_);
    }

    /*
     * @dev update Value of BNB for MaxTransaction.
     */
    function setMaxTransaction(uint256 amountBNB) external onlyOwner {
    ...
    /* Function     : Turns ON/OFF Max Transaction */
    /* Parameters   : Set 'true' to turn ON and 'false' to turn OFF */
    function setExecuteMaxTransaction() external onlyOwner {
        executeMaxTransaction ? executeMaxTransaction = false
        : executeMaxTransaction = true;

        emit ExecuteMaxTransaction(executeMaxTransaction);
    }
    ...
    event ExecuteMaxTransaction(bool enabled);
    event SwapEnabledUpdated(bool enabled);
    event UpdateAddressFee(address Address);
    event SetRouterAddressEvent(address Address);
    event FeeTransaction(uint256 tDevelopment,  uint256 tMarketing, uint256 tListing);
