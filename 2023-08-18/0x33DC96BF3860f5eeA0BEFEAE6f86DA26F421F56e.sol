...
contract SCToken is ExcludedFromFeeList, LpDividendBNB, BlackList, FirstLaunch, MaxHave {
    ...
    address public lpReceiver = address(0x4EbE3eED5323fac9D2cec4A5B2Ab5f859B7D8ee3);

    address constant marketAddr = address(0x4EbE3eED5323fac9D2cec4A5B2Ab5f859B7D8ee3); //1%

    uint256 constant marketFee = 1;
    uint256 constant lpFee = 3;
    uint256 constant liqFee = 1;
    ...
    constructor() payable Owned(msg.sender) ERC20("MOON", "MOON", 18, 10_000 ether) {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (launchedAt + 20 * 20 >= block.number) {
            if (sender == uniswapV2Pair) {
                ...
                require(balanceOf[recipient] <= 50 ether, "<=50");
            } else {
                fee = amount * 9 / 100;
                super._transfer(sender, marketAddr, fee);
            }
        ...
        uint256 thisfee = amount * (marketFee + lpFee + liqFee) / 100;
        uint256 burnfee = amount * (burnFee) / 100;
    ...
    function processLp(uint256 gas, uint256 amountBNB) private {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (lpBal >= minHoldlp) {
                ...
                if (percent > 0) {
                    (sent,) = payable(theHolder).call{value: (nowbanance * percent / 1000_000_000), gas: 24000}("");
                }
