...
DividendPayingTokenOptionalInterface
{
    ...
    returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            bool success;
            if (ETH_ADDRESS != rewardToken) {

                success = IERC20(rewardToken).transfer(
                    user,
                    _withdrawableDividend
                );
            } else {
...
contract TokenRewardAllReferrer is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    address ETH_ADDRESS = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;
    ...
    uint256 public referralFee1;
    uint256 public referralFee2;
    uint256 public totalFees;

    uint256 public holdLimitAmount = 520e18;
    ...
    constructor(
        ...
        uint256[5] memory feeSettings, // rewards, liquidity, marketing
        uint256 minimumTokenBalanceForDividends_,
        uint256 swapTokenAtAmount_
        ...
        if(feeSettings[3] != 0  && feeSettings[4] !=0 ){
            referralFee1 = feeSettings[3];
            referralFee2 = feeSettings[4];
            isOpenRef = true;
            totalFees = totalFees.add(referralFee1).add(referralFee2);
        }
        require(totalFees <= 250, "Total fee is over 25%");
        swapTokensAtAmount = swapTokenAtAmount_;
        // 0.002%

        // use by default 300,000 gas to process auto-claiming dividends
        gasForProcessing = 300000;

        dividendTracker = new BABYTOKENDividendTracker();
    ...
    function checkReferrer(address from, address to) internal {
        if (referrerMap[to] != address(0)) {
            return;
        }

        if (balanceOf(to) == 0 && !isContract(from) && !isContract(to) && msg.sender == tx.origin) {
            referrerMap[to] = from;
        }
    ...
    function _transfer(
        ...
        if (!automatedMarketMakerPairs[to] && !_isExcludedFromFees[to]) {
            ...
            uint tmpTotal = amount.sub(fees).add(balanceOf(to));
            require(tmpTotal <= holdLimitAmount, "maxLimit");
        }
        ...
        if (takeFee) {
            ...
            if(isOpenRef) {
                ...
                if (automatedMarketMakerPairs[from]) {
                    ...
                    ref1 = referrerMap[from];
                }
                if(ref1 != address(0)){
                    uint256 refFee1 = tmp.mul(referralFee1).div(1000);
                    super._transfer(from, ref1, refFee1);
                    fees = fees.sub(refFee1);
                    address ref2 = referrerMap[ref1];
                    if(ref2 != address(0)){
                        uint256 refFee2 = tmp.mul(referralFee2).div(1000);
                        super._transfer(from, ref2, refFee2);
                        fees = fees.sub(refFee2);
                    }
                }
            }
            //
    ...
    function getTokenBalance(address tokenAddress, address holder) internal view returns (uint256){
    ...
    function swapAndSendToFee(uint256 tokens) private {
        uint256 initialCAKEBalance = getTokenBalance(rewardToken, address(this));
        //IERC20(rewardToken).balanceOf(
        //address(this)
        //);

        swapTokensForCake(tokens);
        uint256 newBalance = getTokenBalance(rewardToken, address(this)).sub(initialCAKEBalance);
        if (ETH_ADDRESS != rewardToken) {
            IERC20(rewardToken).transfer(_marketingWalletAddress, newBalance);
        } else {
            _marketingWalletAddress.call{value : newBalance, gas : 3000}("");
        }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        swapTokensForCake(tokens);
        uint256 dividends = getTokenBalance(rewardToken, address(this));
        bool success;
        if (rewardToken != ETH_ADDRESS) {
            ...
            (success,) = address(dividendTracker).call{value : dividends, gas : 3000}("");
        }
