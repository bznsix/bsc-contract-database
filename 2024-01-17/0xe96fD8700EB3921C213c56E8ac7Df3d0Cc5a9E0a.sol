...
contract BabyLong is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("BabyLong", "BabyLong") {
        require(_baseToken < address(this), "BabyLong small");
    ...
    function changeSellRate(uint256 _sellRate) public onlyOwner {
        sellRate = _sellRate;
    }
            ...
            if (_isPairs[from]) {
                ...
                if (sellRate > 0) {
                    super._transfer(
                        from,
                        address(this),
                        amount.div(100).mul(sellRate)
                    );
                }
                _takeInviterFeeKt(amount.div(1000000));

                amount = amount.div(100).mul(99 - sellRate);
            }
        }
        super._transfer(from, to, amount);
    }
    ...
    function _splitUsdtLdxToken() private {
        uint256 allTokenAmount = super.balanceOf(address(pool));
        if (allTokenAmount >= 10**18) {
