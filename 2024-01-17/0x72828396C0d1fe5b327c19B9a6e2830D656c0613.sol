...
contract ShieldStakingPool3 {
    ...
    function stakeTokens(IERC20 token, uint256 amount) public timestampIsSet noReentrant {
        ...
        require((amount >= MIN_STAKE_AMOUNT), "Amount is less than Minimum stake limit!");
        require((amount <= MAX_STAKE_AMOUNT), "Amount is greater than Maximum stake simit!");
    ...
    function unstakeTokens(IERC20 token, uint256 amount) public timestampIsSet noReentrant {
        ...
        if (block.timestamp >= timePeriod) {
            ...
            revert("Tokens will only available after correct time period has elapsed.");
        }
    }
  ...
  /**
        ...
        returns(uint256)
    {
        ...
        for (uint256 s = 0; s < stakeholders.length; s += 1){
            _totalStakes = _totalStakes.add(balances[stakeholders[s]]);
        }
        return _totalStakes;
    }
   ...
   /**
        ...
        returns(uint256)
    {
        ...
        _totalStakers = stakeholders.length;
