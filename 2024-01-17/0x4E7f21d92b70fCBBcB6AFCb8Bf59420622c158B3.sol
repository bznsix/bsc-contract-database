 ...
 * @param order - The base socket order against which the user signed.
 * @param promisedAmount - amount promised by the solver on the destination.
 * @param toTokenId - token id of the token to be swapped into.
 * @param swapContractProvider - address of the swap contract provider.
 * @param minToTokenAmount - minimum amount of the token to be swapped into.
 * @param userSignature - signature of the user against the socket order.
 */
struct RFQOrderWithSwap {
    SocketOrder order;
    uint256 promisedAmount;
    uint8 toTokenId;
    uint16 swapProviderId;
...
/**
 * @notice Batch RFQ Order withSwap is the batch of rfq orders being submitted by an rfq solver.
 * @param settlementReceiver - address that will receive the user funds on the source side when order is settled.
 * @param orders - RFQ orders in the batch.
 * @param socketSignature - batch order signed by Socket so that the auction winner can only submit the orders won in auction.
 */
struct BatchRFQOrderWithSwap {
    address settlementReceiver;
    RFQOrderWithSwap[] orders;
    bytes socketSignature;
}
...
error InvalidOrder();
error SwapFailed();// SPDX-License-Identifier: GPL-3.0-only
pragma solidity ^0.8.17;

import {IPlug} from "../interfaces/IPlug.sol";
import {ISocket} from "../interfaces/ISocket.sol";
import {Ownable} from "../utils/Ownable.sol";
import {BatchGatewayOrder, BatchRFQOrderWithSwap, ExtractedAndSwapped, FulfillRFQOrder, BatchRFQOrder, GatewayOrder, RFQOrder, SocketOrder, BasicInfo, ExtractedRFQOrder, RFQOrderWithSwap} from "../common/SocketStructs.sol";
...
import {SocketOrder, RFQOrder, BatchRFQOrder, BatchRFQOrderWithSwap, RFQOrderWithSwap} from "../common/SocketStructs.sol";
import {SocketOrderLib} from "./SocketOrderLib.sol";
...
library RFQOrderLib {
    ...
    function hash(RFQOrder memory rfqOrder) internal pure returns (bytes32) {
        ...
        /// @notice hash a rfq order
    /// @param rfqOrder rfq order to be hashed
    function hash(RFQOrderWithSwap memory rfqOrder) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    ORDER_TYPE_HASH,
    ...
    /// @param batchOrder batch of rfq orders to be hashed
    function hashBatch(
        BatchRFQOrderWithSwap memory batchOrder
    ) internal pure returns (bytes32) {
        unchecked {
            bytes32 outputHash = keccak256(
                "RFQOrder(SocketOrder order,uint256 promisedAmount,bytes userSignature)"
...
contract Solver is Ownable {
    ...
    function performActionAndCallExtractor(
        address actionContract,
        address tokenForAction,
        ...
        if (tokenForAction != address(0)) {
            ERC20(tokenForAction).safeApprove(actionContract, amountOfToken);
        }
        
        (bool successAction, ) = actionContract.call(actionData);

        if (!successAction) revert ActionFailed();

        if (tokenToApprove != address(0)) {
            ERC20(tokenToApprove).safeApprove(
                SOCKET_EXTRACTOR,
                type(uint256).max
            );
        }
