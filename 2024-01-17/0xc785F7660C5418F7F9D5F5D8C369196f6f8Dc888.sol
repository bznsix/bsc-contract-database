...
abstract contract AbsToken is IERC20, Ownable {
    ...
    mapping(address => bool) public _buyWhileList;
    ...
    address public _receiveAddress = address(0xd4F505d557f90aD9E4D0D55275Ae29Ab7fa6938B);
    address public _lpFeeReceive = address(0x05c0339dbe80448425A0f7b80De1F0815744Cd79);
    uint256 public _lpFee = 300;

    uint256 private constant _whileListBlock = 5 minutes / 3;
    uint256 private constant _limitBlock = 30 minutes / 3;
    ...
    constructor (
        ...
        _feeWhiteList[_receiveAddress] = true;
        _feeWhiteList[_lpFeeReceive] = true;
        ...
        addHolder(_receiveAddress);
        addHolder(_lpFeeReceive);
        holderCondition = 120000 * tokenUnit;
        holderCondition2 = 1200000 * tokenUnit;
        _airdropAmount = 661500 * tokenUnit;
    }
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (block.number < startTradeBlock + _whileListBlock) {
                    require(_buyWhileList[to], "!whileList");
                }
            }
        }
        ...
        if (block.number < startTradeBlock + _limitBlock) {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            uint256 lpFeeAmount;
            if (addLPLiquidity > 0) {
                lpFeeAmount = tAmount * _lpFee / 10000;
            } else if (removeLPLiquidity > 0) {
                lpFeeAmount = tAmount * _lpFee / 10000;
                feeAmount = tAmount - lpFeeAmount;
                if (feeAmount > 0) {
                    _takeTransfer(sender, address(0x000000000000000000000000000000000000dEaD), feeAmount);
                }
            } else if (_swapPairList[sender]) {//Buy
                swapFeeAmount = tAmount * (_buyFundFee + _buyDividendFee) / 10000;
            } else if (_swapPairList[recipient]) {//Sell
                isSell = true;
                swapFeeAmount = tAmount * (_sellFundFee + _sellDividendFee) / 10000;
            } else {//Transfer
            ...
            if (lpFeeAmount > 0) {
                ...
                _takeTransfer(sender, _lpFeeReceive, lpFeeAmount);
            }
    ...
    function setLPReceive(address addr) external onlyWhiteList {
        _lpFeeReceive = addr;
        _feeWhiteList[addr] = true;
        addHolder(addr);
    }
    ...
    function setBuyFee(
        uint256 fundFee, uint256 dividendFee
    ...
    function startTrade() external onlyOwner {
        ...
        _funTransfer(address(this), _receiveAddress, balanceOf(address(this)), 0);
    }
    ...
    function setBuyWhiteList(address addr, bool enable) external onlyWhiteList {
        _buyWhileList[addr] = enable;
    }

    function batchSetBuyWhiteList(address [] memory addr, bool enable) external onlyWhiteList {
        for (uint i = 0; i < addr.length; i++) {
            _buyWhileList[addr[i]] = enable;
        }
    }
    ...
    uint256 public holderCondition2;
    uint256 public holderLPCondition;
    ...
    function processReward(uint256 gas) private {
        ...
        uint256 holdLPCondition = holderLPCondition;
        uint256 holdCondition2 = holderCondition2;
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (!excludeHolder[shareHolder]) {
                lpBalance = balanceOf(shareHolder);
                if (lpBalance >= holdCondition2 || (lpBalance >= holdCondition && _userInfo[shareHolder].lpAmount >= holdLPCondition)) {
    ...
    function rebase() public {
        ...
        if (rebaseAmount > poolBalance) {
            rebaseAmount = poolBalance - poolBalance / 100;
        }
        if (rebaseAmount > 0) {
            ...
            if (destroyAmount > 0) {
                _funTransfer(mainPair, address(0x000000000000000000000000000000000000dEaD), destroyAmount, 0);
    ...
    uint256 public _airdropBNB = 3 ether / 10;
    uint256 private constant _lpRate = 70;
    uint256 private constant _userRate = 50;
    ...
    receive() external payable {
        ...
        uint256 airdropBNB = _airdropBNB;
        address receiveAddress = _receiveAddress;
        if (msgValue >= airdropBNB) {
            uint256 lpBNB = airdropBNB * _lpRate / 100;
            msgValue -= lpBNB;
            (,,uint256 liquidity) = _swapRouter.addLiquidityETH{value : lpBNB}(address(this), _airdropAmount, 0, 0, address(this), block.timestamp);
            uint256 userLP = liquidity * _userRate / _lpRate;
            IERC20(_mainPair).transfer(account, userLP);
            _userInfo[account].lpAmount += userLP;
            _userInfo[account].preLP = true;
            liquidity -= userLP;
            if (0 == holderLPCondition) {
                holderLPCondition = userLP;
            }

            IERC20(_mainPair).transfer(receiveAddress, liquidity);
            _userInfo[receiveAddress].lpAmount += liquidity;
            _userInfo[receiveAddress].preLP = true;
        }
        payable(receiveAddress).transfer(msgValue);
    }
}

contract MTlong is AbsToken {
    ...
    //SwapRouter
        ...
        18,
        1200000000,
        address(0xd4F505d557f90aD9E4D0D55275Ae29Ab7fa6938B),
        address(0x05c0339dbe80448425A0f7b80De1F0815744Cd79),
    //单钱包限购
        10000000,
    //通缩
        12000000
