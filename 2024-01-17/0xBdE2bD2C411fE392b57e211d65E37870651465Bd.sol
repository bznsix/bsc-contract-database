...
contract SparkCitizensStaker is Ownable, IERC721Receiver{
    IERC721 public SparkcitizensNFT;
    ...
    uint128 private smallRewardTime = 1296000;
    uint128 private midRewardTime = 2592000;
    uint128 private largeRewardTime = 5184000;
    uint128 private intervalRewardTime = 1296000;

    uint128 private smallReward = 200;
    uint128 private midReward = 450;
    uint128 private largeReward = 2000;
    uint128 private intervalReward = 700;

    address private SparkTokenAddress;

    constructor(address SparkcitizensAddress){
        SparkcitizensNFT = IERC721(SparkcitizensAddress);
    }

    function stakeMultiple(uint256[] calldata tokenIds_) external{
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            require(SparkcitizensNFT.ownerOf(tokenIds_[i]) == msg.sender, "Not Your Sparkcitizen To Stake");
            storeSparkcitizen(msg.sender, tokenIds_[i]);
            SparkcitizensNFT.safeTransferFrom(
                msg.sender,
                address(this),
                tokenIds_[i]
            );
    ...
    function unstakeMultiple(uint256[] calldata tokenIds_) external {
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            require(removeSparkcitizen(msg.sender, tokenIds_[i]) == true, "Not your Sparkcitizen Token");
            SparkcitizensNFT.isApprovedForAll(address(this), msg.sender);
            SparkcitizensNFT.safeTransferFrom(
                address(this),
                msg.sender,
                tokenIds_[i]
    ...
    function stake(uint256 tokenIds_) external{
            require(SparkcitizensNFT.ownerOf(tokenIds_) == msg.sender, "Not Your Sparkcitizen To Stake");
            storeSparkcitizen(msg.sender, tokenIds_);
            SparkcitizensNFT.safeTransferFrom(
                msg.sender,
                address(this),
                tokenIds_
    ...
    function unstake(uint256 tokenIds_) external {
            require(removeSparkcitizen(msg.sender, tokenIds_) == true, "Not your Sparkcitizen Token");
            SparkcitizensNFT.isApprovedForAll(address(this), msg.sender);
            SparkcitizensNFT.safeTransferFrom(
                address(this),
                msg.sender,
    ...
    function storeSparkcitizen(address _owner, uint256 tokenId_) internal
    {       
    ...
    function removeSparkcitizen(address _owner, uint256 tokenId_) internal returns(bool)
    {
    ...
    function returnStakedSparkcitizens() external view returns(uint256 [] memory){
    ...
    function claimStakedRewards(address owner) external returns(uint256){
        require(msg.sender == SparkTokenAddress, "Function can only be called via token contract");
    ...
    function setSparkToken(address SparkTokenAddress_) external onlyOwner {
        SparkTokenAddress = SparkTokenAddress_;
    }
