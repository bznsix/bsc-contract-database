...
contract MstDeposit is Ownable {
    ...
    mapping(address => mapping(address => bool)) public firstRewardMap;
    ...
    struct User {
        ...
        bool isBlack;
        uint256 recommerCount;
        uint256 level;
        uint256 recommerRewardCount;
        bool ownLand;
    }
    ...
    constructor(address _coo,
        ...
        landPrice = 15500000000000000000;  
    ...
    function setHashrateByAdmin(address receiver, uint256 amount) external onlyOwner {
    ...
    function buyLand() external {
        ...
        if (!firstRewardMap[recommer][msg.sender]) {
            recommerLevelMap[0][recommer] += 1;
            firstRewardMap[recommer][msg.sender] = true;
        }
        ...
        if (recommerLevelMap[0][recommer] >= 3) { // 60%直推奖
            cloudWithdrawMap[recommer] += (landPrice * 60 / 100);
            emit DirectProfit(recommer, msg.sender, landPrice, landPrice * 60 / 100, recommerLevelMap[0][recommer]);
        } else if (recommerLevelMap[0][recommer] == 2) { // 50%直推奖
            cloudWithdrawMap[recommer] += (landPrice * 50 / 100);
            emit DirectProfit(recommer, msg.sender, landPrice, landPrice * 50 / 100, recommerLevelMap[0][recommer]);
        } else if (recommerLevelMap[0][recommer] == 1) { // 40%直推奖
            cloudWithdrawMap[recommer] += (landPrice * 40 / 100);
            emit DirectProfit(recommer, msg.sender, landPrice, landPrice * 40 / 100, recommerLevelMap[0][recommer]);
        }

        address[] memory fathers = Relation(relation).getForefathers(msg.sender, 50);
        ...
        for (uint256 i=0; i<fathers.length; i++) {
            ...
            if (father.level > rewardLevel && father.level <= 6) { // 可领取流水奖 3% + 星数加成2%
                uint level = father.level - rewardLevel;
                ...
                // 上级获得伯乐奖
                cloudWithdrawMap[father.recommer] += rolloverBonus;
                emit BoleProfit(father.recommer, fathers[i], landPrice, rolloverBonus);
                rewardLevel = father.level;
            }
        }

        // 分红奖
        // 总裁分红2%
        cloudWithdrawMap[president] += (landPrice * 2 / 100);
        emit PresidentProfit(president, msg.sender, landPrice, landPrice * presidentRadio / 10000);
        // 董事分红3%
        cloudWithdrawMap[director] += (landPrice * 3 / 100);
        emit DirectorProfit(director, msg.sender, landPrice, landPrice * directorRadio / 10000);
        // 老板分红5%
        cloudWithdrawMap[boss] += (landPrice * 5 / 100);
        emit DirectorProfit(boss, msg.sender, landPrice, landPrice * bossRadio / 10000);
    ...
    function withdrawUSDT() external {
        if (cloudWithdrawMap[msg.sender] > 0) {
            ...
            IERC20(USDT).transferFrom(cfo2, msg.sender, withdrawAmount);
    ...
    function levelUp(address sender) internal {
        address[] memory fathers = Relation(relation).getForefathers(sender, 50);
        for (uint256 i=0; i<fathers.length; i++) {
            User storage father = userInfo[fathers[i]];
            // 判断是否升级
