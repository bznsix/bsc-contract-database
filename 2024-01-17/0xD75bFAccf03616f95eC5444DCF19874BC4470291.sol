...
contract Token is IERC20, Ownable {
    ...
    uint256 public _buyHeightFee = 400; //买高税
    uint256 public _sellHeightFee = 3000; //卖高税
    uint256 public _heightFeeTime = 3600; //高税时间 过了这个时间就税率正常
    uint256 public _buyBaseFee = 400; //买正常税
    uint256 public _sellBaseFee = 2000; //卖正常税
    uint256 public _traFee = 9900; //转账税
    uint256 public limitBlock = 60; //设置限购时间300秒，到了时间自动解除限购，
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (block.timestamp < limitBlock + starBlock) {
                require(
                    (getETHout(tAmount - swapAmount) < 2 * 1e17), //2 * 1e17限制bnb数量，2就是限制0.2bnb，20就是限制2b，2 * 1e16这个就是0.02b
...
contract lkTOKEN is Token {
    constructor()
        Token(
            ...
            10000,
            1698841080, //开盘时间戳
            address(0xcE35b8a8fA6D4AD208bfA4b563B5A2070B5dD545), // 营销地址
            address(0xcE35b8a8fA6D4AD208bfA4b563B5A2070B5dD545) // 接受代币地址
        )
    {}
}