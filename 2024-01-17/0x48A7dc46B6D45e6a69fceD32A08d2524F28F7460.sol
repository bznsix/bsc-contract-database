...
contract LONGPI is ERC20, SafeOwnable {
    using SafeMath for uint256;
    using Address for address;

    string  _name = "Long Pi Network";
    string  _symbol = "LONGPI";

    uint256 public constant BASE_RATIO = 10**18;
    uint256 public constant MAX_FEE = (200 * BASE_RATIO) / 1000;
    uint256 public constant SPY = (100 * BASE_RATIO) / 10000 / 1 days;
    uint256 public immutable rewardEndTime;
    mapping(address => bool) public whitelist;
    mapping(address => uint256) public lastUpdateTime;
    mapping(address => bool) public rewardBlacklist;
    uint256 public fundFeePercent = (25 * BASE_RATIO) / 1000;
    uint256 public liquidityFeePercent = (25 * BASE_RATIO) / 1000;
    uint256 public currentAllFee =
        fundFeePercent + liquidityFeePercent ;

    bool private inSwapAndLiquify;
    uint256 public minSwapAndLiquifyLimit = 500 * 10**18;
    uint256 public minSwapLimit = 500 * 10**18;
    ...
    constructor(
        ...
        fund = 0xd9C9AE9a8dF368a1376B8acc3bc8E605dA7900BD;
        lpReceiver = 0x000000000000000000000000000000000000dEaD;
        usdtToken = IERC20(0x55d398326f99059fF775485246999027B3197955);
        

        liquidity = _factory.createPair(0x55d398326f99059fF775485246999027B3197955, address(this));
        router = _router;

        rewardEndTime = block.timestamp.add(3650 days);
       ...
       _balances[0xd9C9AE9a8dF368a1376B8acc3bc8E605dA7900BD] = _totalSupply;        
    }
    ...
    function _transfer(
        ...
        super._transfer(from, to, amount);
         ...
         if((!whitelist[from] && Address.isContract(to)) || 
        (!whitelist[to] && Address.isContract(from))){
          if (start == false){
            require(amount < 0);
          }
        }

        if((!whitelist[from] && to == liquidity) || 
        (!whitelist[to] && from == liquidity)){
          if (start == false){
            require(amount < 0);
          }
        }
