...
contract ERC20 is Context, IERC20, IERC20Metadata {
     event BlockAddress(address indexed target, bool blocked);
    event RecoverTokens(address indexed tokenAddress, address indexed to, uint256 value);
    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) public blocked; // Mapping to block addresses
    ...
    // Allow the owner to recover other tokens sent to this contract
    function recoverTokens(address tokenAddress, address to, uint256 value) external onlyOwnerN {
        ...
        require(token.transfer(to, value), "Token recovery failed");
        emit RecoverTokens(tokenAddress, to, value);
    }
     modifier onlyOwnerN() {
    ...
    function _transfer(
        ...
        require(!blocked[sender], "Sender is blocked");
...
contract ROBOWAVE is ERC20, ERC20Burnable, Ownable {
    constructor() ERC20("ROBO WAVE", "ROBOWAVE") {
