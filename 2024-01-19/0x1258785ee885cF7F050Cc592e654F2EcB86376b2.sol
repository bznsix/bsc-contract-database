/**
Https://t.me/TaylorSwiftErasPortal

Https://x.com/taylorswiftspl/

Https://taylorswifteras.live
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional, Ownable {
  ...
  address public DividendsToken = address(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c);
...
contract TaylorSwiftFanToken is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   ERASDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("Taylor Swift Fan Token", "ERAS") {
        _buyRewardsFee = 5;
        _buyLiquidityFee = 2;
		_buyMarketingFee = 2;
        _buyDevFee = 1;
    	...
    	dividendTracker = new ERASDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0xd095dC0B87C67e35B0454b7e40453660daF859E6);
        _devWallet = payable(0x587bB9591cc1E16d60d542fD2CDf073de2238F6b);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ERAS: The dividend tracker already has that address");

        ERASDividendTracker newDividendTracker = ERASDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ERAS: The new dividend tracker must be owned by the ERAS token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "ERAS: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ERAS: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ERAS: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "ERAS: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ERAS: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ERAS: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "ERAS: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "ERAS: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ERAS: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ERAS: Cannot update gasForProcessing to same value");
...
contract ERASDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ERAS_Dividend_Tracker", "ERAS_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "ERAS_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ERAS contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "ERAS_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ERAS_Dividend_Tracker: Cannot update claimWait to same value");
