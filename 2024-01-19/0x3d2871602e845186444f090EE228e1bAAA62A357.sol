...
contract HKDToken is ERC20, Ownable, Pausable {
    ...
    // Basis points rate and maximum fee for transfer
    ...
    event FeeChanged(uint256 newBasisPoints, uint256 newMaximumFee);
    ...
    // Change the basis points rate and maximum fee
    function setFee(
        uint256 newBasisPoints,
        uint256 newMaximumFee
    ) external onlyOwner {
        require(newBasisPoints < 20, "Basis points rate must be less than 20");
        require(newMaximumFee < 50, "Maximum fee must be less than 50");

        _basisPointsRate = newBasisPoints;
        _maximumFee = newMaximumFee.mul(10 ** decimals());

        emit FeeChanged(newBasisPoints, newMaximumFee);
    }

    // Override transfer function to check for paused, blacklist, and apply fee
    function transfer(
        address recipient,
        uint256 amount
    )
    ...
    // Override transferFrom function to check for paused, blacklist, and apply fee
