...
contract MineralCity {
    ...
    require(!blacklist[msg.sender], "You are not allowed to use this contract. Contact Our EXECUTIVE");
    _;
    }
	...
	constructor(address tokenAddr, address payable wallet, address payable wallet2, uint256 start) public {
    ...
    uint256 commission = rent.mul(10).div(100);  // Calculate 10% commission

    token.safeTransfer(commissionWallet, commission);  // Send 10% commission to commissionWallet
    token.safeTransfer(commissionWallet2, commission); // Send 10% commission to commissionWallet2
	...
	function withdraw() public {
        require(!blacklist[msg.sender], "You are not allowed to use this contract.Contact Our Support");
		User storage user = users[msg.sender];

		require(user.checkpoint.add(TIME_STEP) < block.timestamp, "Withdrawal only once a day");
		uint256 totalAmount = getUserDividends(msg.sender);
		require(totalAmount >= 0 ether, "Minimum 0");
