/**
https://t.me/growshib
https://twitter.com/growshib_official
 **/
...
/* Token contract */
contract GrowShib is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address DEV = 0x3a6F40B333ffc6BFdAb98fC717A24e82776eD15e;
    ...
    // Name and symbol
    string constant _name = "GrowShib";
    string constant _symbol = "GS";
    uint8 constant _decimals = 18;

    // Total supply
    uint256 _totalSupply = 1000000000000 * (10 ** _decimals); // 1T

    // Max wallet and TX
    uint256 public _maxBuyTxAmount = _totalSupply * 200 / 10000; // 2% on launch or 20B tokens
    uint256 public _maxSellTxAmount = _totalSupply * 150 / 10000; // 1.5% or 15B tokens
    uint256 public _maxWalletToken = ( _totalSupply * 300 ) / 5000; // 3% or 30B tokens
    ...
    uint256 public buybackFeeSell = 2;
    ...
    // GSHIB
    uint256 GSHIBTriggeredAt;
    uint256 GSHIBDuration = 3600;
    
    // Dead blocks
    uint256 deadBlocks = 3;

    // Sell amount of tokens when a sell takes place
    uint256 public swapThreshold = _totalSupply * 45 / 10000; // 2% of supply
    ...
    uint256 buybackMultiplierLength = 15 minutes;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x3a6F40B333ffc6BFdAb98fC717A24e82776eD15e;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // Check if we are in GSHIBTime
        bool GSHIBMode = inGSHIBTime();
        ...
        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, amount, isSell, GSHIBMode) : amount;
    ...
    // Check if we are in GSHIB time
    function inGSHIBTime() public view returns (bool){
        if(GSHIBTriggeredAt.add(GSHIBDuration) > block.timestamp){
            return true;
        } else {
            return false;
        }
    ...
    // Take the normal total Fee or the GSHIB Fee
    function takeFee(address sender, uint256 amount, bool isSell, bool GSHIBMode) internal returns (uint256) {
        uint256 feeAmount;
        
        // Check if we are GSHIBd
        if (GSHIBMode){
            if(isSell){
    ...
    // Enable GSHIB
    function enableGSHIB(uint256 _seconds) public authorized {
        GSHIBTriggeredAt = block.timestamp;
        GSHIBDuration = _seconds;
    }

    // Disable the GSHIB mode
    function disableGSHIB() external authorized {
        GSHIBTriggeredAt = 0;
    }
