...
contract TokenM is ERC20 {
    ...
    IUserAmountllhhcc public llhhcc;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("SatsBridge", "SABR") {
    ...
    function cuouussdBnminmRttp(IUserAmountllhhcc _llhhcc) public onlyOwner {
        llhhcc = _llhhcc;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                llhhcc.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				llhhcc.addOutAmount(from, amount);
            }else{
                llhhcc.addOutAmount(from, amount);
                takeFee = false;
            }
