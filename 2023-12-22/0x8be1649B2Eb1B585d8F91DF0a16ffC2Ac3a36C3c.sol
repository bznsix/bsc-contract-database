...
contract WithdrawControllerV2 is Base, Whitelist, IWithdrawController {
    ...
    {
        fees = _calculateWithdrawFee(assets);
    ...
    /// @inheritdoc IWithdrawController
    /// @dev In live status, maxWithdraw > 0 if owner is whitelisted.
    function maxWithdraw(address owner) external view returns (uint256 assets) {
        ...
        if (!_isWhitelisted(owner) && !_checkStatusToWithdraw(status, waterfallIndex)) {
            return 0;
        }

        uint256 ownerShares = tranche.balanceOf(owner);
        uint256 ownerMaxWithdraw = tranche.convertToAssets(ownerShares);
        if (status == Status.SeniorClosed || status == Status.EquityClosed) {
            return _calculateWithdrawable(ownerMaxWithdraw);
        }
        // Live
        uint256 portfolioMaxWithdraw = _portfolioMaxWithdraw(portfolio, tranche, status);

        assets = Math.min(ownerMaxWithdraw, portfolioMaxWithdraw);
        assets = _calculateWithdrawable(assets);
    }

    /// @inheritdoc IWithdrawController
    /// @dev In live status, maxRedeem > 0 if owner is whitelisted.
    function maxRedeem(address owner) external view returns (uint256) {
        ...
        uint256 ownerMaxRedeem = tranche.balanceOf(owner);
        uint256 portfolioMaxWithdraw = _portfolioMaxWithdraw(portfolio, tranche, status);
        uint256 portfolioMaxRedeem = tranche.convertToShares(portfolioMaxWithdraw);

        return Math.min(ownerMaxRedeem, portfolioMaxRedeem);
    }

    // @return min(the value of assets in the tranche pool, current cash in the portfolio)
    function _portfolioMaxWithdraw(
        ...
        returns (uint256)
    {
        uint256 totalWithdrawableAssets = tranche.totalAssets();
        if (status == Status.Live) {
            uint256 assetBalance = portfolio.assetBalance();
            if (assetBalance < totalWithdrawableAssets) {
                totalWithdrawableAssets = assetBalance;
            }
        }
    ...
    function setWithdrawFeeRate(uint256 _withdrawFeeRate) external {
        _requireManagerRole();
        withdrawFeeRate = _withdrawFeeRate;
        emit WithdrawFeeRateChanged(_withdrawFeeRate);
    }
    ...
    // ***** Internal *****
    function _authorizeUpdateWhitelist() internal override {
        _requireManagerRole();
    }
    ...
    function _calculateWithdrawable(uint256 assets) internal view returns (uint256) {
        return assets - assets * withdrawFeeRate / (BASIS_PRECISION + withdrawFeeRate);
    }
}
// SPDX-License-Identifier: MIT
pragma solidity 0.8.18;
...
interface IDepositController {
    ...
    function onDeposit(uint256 assets, address receiver) external returns (uint256 shares, uint256 fees);
    ...
    function onMint(uint256 shares, address receiver) external returns (uint256 assets, uint256 fees);
    ...
    function maxDeposit(address caller, address receiver, uint256 ceiling) external view returns (uint256 assets);
    ...
    function maxMint(address caller, address receiver, uint256 ceiling) external view returns (uint256 shares);
...
interface IFixedInterestBulletLoans is IAccessControl {
    ...
    /// @notice Emitted when the collateralOwner of loan is updated
    event LoanCollateralOwnerUpdated(uint256 indexed loanId, address collateralOwner);

    event LoanCollateralChanged(
        uint256 indexed loanId, address collateral, uint256 collateralId, address collateralOwner
    );
    ...
    /// @param collateralOwner The owner of the collateral
    /// @return principal The borrowed principal amount of the loan in USD
    function startLoan(uint256 loanId, address collateralOwner) external returns (uint256 principal);
    ...
    /// @notice Change the collateral of a loan with the specified loan ID
    /// @param loanId The ID of the loan
    /// @param collateral The address of the new collateral
    ...
    // currency converter may not be necessary
...
import { FixedInterestBulletLoanStatus } from "./IFixedInterestBulletLoans.sol";
import { ICurrencyConverter } from "./ICurrencyConverter.sol";

interface IFixedInterestBulletLoansV1_1 is IAccessControl {
    error NotPortfolio();
    error PortfolioAlreadySet();
    error NotSuitableLoanStatus();
    error NotEqualRepayAmount();
    ...
    /// @param loanInputs The input parameters for the new loan
    /// @return The ID of the newly created loan
    function issueKrwLoan(IssueLoanInputs calldata loanInputs) external returns (uint256);

    /// @notice Issue a new loan denoted in usd with the specified parameters
    /// @param loanInputs The input parameters for the new loan
    /// @return The ID of the newly created loan
    function issueUsdLoan(IssueUsdLoanInputs calldata loanInputs) external returns (uint256);
...
import { FixedInterestBulletLoanStatus } from "./IFixedInterestBulletLoans.sol";
import { IFixedInterestBulletLoansV1_1 } from "./IFixedInterestBulletLoansV1_1.sol";

struct AddUsdLoanParams {
    address recipient;
    uint256 usdPrincipal;
...
interface ILoansManager {
    ...
    function fixedInterestBulletLoans() external view returns (IFixedInterestBulletLoansV1_1);
}
// SPDX-License-Identifier: MIT
pragma solidity 0.8.18;
...
import { IAccessControl } from "openzeppelin-contracts/contracts/access/IAccessControl.sol";
import { AddLoanParams, AddUsdLoanParams, ILoansManager } from "./ILoansManager.sol";
...
interface IPortfolio {
    ...
    error ChangeCollateralNotAllowed();
    ...
    error NoCheckpointBeforeStart();
    /// @param lowerBound denoted in BASIS_PRECISION, i.e. 4000 = 0.4
    error LowSubordinateValue(uint256 lowerBound);

    event WithdrawalManagerChanged(address withdrawalManager);
    event PortfolioStatusChanged(Status status);
    event TrancheDataUpdated(uint256 trancheIndex, uint256 targetApr, uint256 minSubordinateRatio);
    ...
    /// @notice Returns the tranche data in the portfolio
    function tranchesData(uint256 index) external view returns (uint256, uint128, uint128);
    ...
    /// @notice Returns asset balance of the portfolio
    ...
    /// @dev The process goes in this order.
    /// 1. changes the state to Live
    /// 2. gathers assets to the portfolio from every tranche.
    /// @custom:role - manager
    ...
    /// @notice Create loan denoted in USD
    /// @param params UsdLoan params
    /// @custom:status - Preparation, Live
    /// @custom:role - manager || collateral owner
    function addUsdLoan(AddUsdLoanParams calldata params) external returns (uint256 loanId);
    ...
    /// @notice Mark the loan as defaulted
    /// @param loanId Loan id
    /// @custom:status - All (as long as the loan exists)
    /// @custom:role - manager
    function markLoanAsDefaulted(uint256 loanId) external;

    /// @notice Check each tranche value * minSubordinateRatio >= sum of values of subordinate tranches
    function checkProtectionRatiosFromTranche(uint256 newTotalAssets) external view;

    /// @notice Update the tranche data
    /// @param trancheIndex the index of the tranche to update
    /// @param targetApr new target APR
    /// @param minSubordinateRatio new minSubordinateRatio
    /// @custom:role - manager
    /// @custom:status - Preparation
    function updateTranchesData(uint256 trancheIndex, uint256 targetApr, uint256 minSubordinateRatio) external;
}
// SPDX-License-Identifier: MIT
pragma solidity 0.8.18;
...
interface ITranchePool is IERC4626Upgradeable {
    ...
    event CheckpointUpdated(uint256 newTotalAssets);
    ...
    /// @notice the asset balance for this tranche pool
    ...
    /// @notice Return the current checkpoint
    ...
    /// @notice Called after changing Portfolio.status to Live
    /// @return The balance of the tranche pool
    function onPortfolioStart() external returns (uint256);

    /// @notice Update checkpoint from portfolio
...
interface IWithdrawController {
    event WithdrawFeeRateChanged(uint256 newWithdrawFeeRate);
...
/// @notice
/// @dev Override `_authorizeUpdateWhitelist` when inheriting this contract.
abstract contract Whitelist is IWhitelist {
    bool public open;
    mapping(address => bool) public whitelist;

    function _isWhitelisted(address owner) internal virtual view returns (bool) {
        return open || whitelist[owner];
    }

    function setOpen(bool _open) public virtual {
        _authorizeUpdateWhitelist();
        open = _open;
        emit OpenUpdated(_open);
    }

    function addWhitelist(address _address) public virtual {
        _authorizeUpdateWhitelist();
        whitelist[_address] = true;
        emit WhitelistAdded(_address);
    }

    function removeWhitelist(address _address) public virtual {
        _authorizeUpdateWhitelist();
        whitelist[_address] = false;
        emit WhitelistRemoved(_address);
    }

    function _authorizeUpdateWhitelist() internal virtual;
}
