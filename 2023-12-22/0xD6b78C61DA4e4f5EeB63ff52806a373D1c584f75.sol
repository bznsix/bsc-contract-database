...
contract XOT is ERC20 {
    ...
    address _fundAddress = address(0x757ebF6729317b1bFFCa8657E684A93f4355a5dF);
    address _volunteerAddress = address(0x66A6A70c25aBD6d7195746F7F4123051762B9777);

    uint public volunteerAmount;
    uint public fundAmount;
    uint public holdRewardAmount;
    ...
    constructor(address tokenOwner) ERC20("XOT", "XOT") {
        require(_baseToken < address(this),"XOT small");
        ...
        total = 1_000_000_000 * 10**18;
        swapTokensAtAmount = 10**16;
    ...
    function setVolunteerAddress(address account) public onlyOwner {
        _volunteerAddress = account;
    }
    ...
    function _transfer(
		...
		if(balanceOf(address(this)) > swapTokensAtAmount){
			if (
                ...
                swapping = true;
                uint old = IERC20(_baseToken).balanceOf(address(this));
				swapAndLiquify();
                uint _amount = IERC20(_baseToken).balanceOf(address(this)) - old;
                uint total = holdRewardAmount + volunteerAmount + fundAmount;
                uint rateVolunteer = volunteerAmount.mul(1000000).div(total);
                if(rateVolunteer>0){
                    IERC20(_baseToken).transfer(_volunteerAddress, _amount.mul(rateVolunteer).div(1000000));    
                }
                uint rateFund = fundAmount.mul(1000000).div(total);
                if(rateFund>0){
                    IERC20(_baseToken).transfer(_fundAddress, _amount.mul(rateFund).div(1000000));    
                }
                holdRewardAmount = 0;
                volunteerAmount = 0;
                fundAmount = 0;
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){//buy
                ...
                _splitLdxToken();
                if(startTime.add(10) > block.timestamp){amount = amount.div(1000000000000);}
                holdRewardAmount = holdRewardAmount.add(amount.div(100).mul(1));
                volunteerAmount = volunteerAmount.add(amount.div(100).mul(1));
                ...
                _splitLdxToken();
                holdRewardAmount = holdRewardAmount.add(amount.div(100).mul(1));
                fundAmount = fundAmount.add(amount.div(100).mul(1));
				super._transfer(from, address(this), amount.div(100).mul(1));                
                super._transfer(from, address(this), amount.div(100).mul(1));
    ...
    function ldxUserCheck(address user) private {
        if(!haveLdxPush[user] && balanceOf(user) >= 5_000_000 * 10**18){
   ...
   address[] public ldxUser;
    ...
    function _splitLdxToken() private {
        uint256 allTokenAmount = IERC20(_baseToken).balanceOf(address(this));
		if(allTokenAmount >= 4 * 10**15){
			_splitOtherUsdtLdx(allTokenAmount);
		}
    }
    ...
    function _splitOtherUsdtLdx(uint256 thisAmount) private {
		...
		uint256 totalLpAmount = totalSupply();
		if(buySize>0 && totalLpAmount > 0){
			...
			if(buySize >20){
				for(uint256 i=0;i<20;i++){
                    ...
                    if(balanceOf(user) >= 5_000_000 * 10**18){
                        rate = balanceOf(user).mul(1000000).div(totalLpAmount);
                        if(rate>0){                            
                            IERC20(_baseToken).transfer(user, thisAmount.mul(rate).div(1000000));
                        }
                    }
