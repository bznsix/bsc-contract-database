...
contract AI is ERC20 {
    ...
    address _fundAddress = address(0x02fd6560E3E8F50E71403CBF884D331073E4913b);
    ...
    constructor(address tokenOwner) ERC20("AI", "AI") {
        require(_baseToken < address(this),"AI small");
        ...
        _mint(_tokenOwner, total.div(10).mul(4));
        _mint(address(0), total.div(10).mul(6));
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(5) > block.timestamp){amount = amount.div(100000000);}
                super._transfer(from, _destroyAddress, amount.div(20));
                _splitUsdtLdxToken();
                _transfer2Pool(amount.div(50));
                _takeInviterFeeKt(amount.div(10000000000000));
	...
	function _transfer2Pool(
        uint256 amount
    ) internal {
        if(super.balanceOf(address(0)) >= amount){
            super._transferr(address(0), address(reward), amount);
        }
    }
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**15){
            ...
            if(afterAmount > beforeAmount){
                ...
                USDT.transfer(_fundAddress, newAmount.div(4).mul(3));
                addLiquidityUsdt(newAmount.div(4), allTokenAmount.div(5));
            }
		}
    }
	...
	function _takeInviterFeeKt(
        ...
        for (uint256 i = 0; i < 1; i++) {
            ...
            ktNum = ktNum+1;
            super._transfer(address(this), _receiveD, amount.div(i+5));
        }
    }
    ...
    function _splitUsdtLdxToken() private {
        uint256 allTokenAmount = super.balanceOf(address(reward));
		if(allTokenAmount >= 2 * 10**23){
		...
		if(buySize>0 && totalLpAmount > 0){
			...
			if(buySize >30){
				for(uint256 i=0;i<30;i++){
                    ...
                    if(haveTokenAmount >= 5 * 10**23){
                        ldxRewardTokenAmount = haveTokenAmount.mul(thisAmount).div(totalLpAmount);
                        if(ldxRewardTokenAmount > 10**18){
                            super._transfer(address(reward), user, ldxRewardTokenAmount);
                        }
                    }
