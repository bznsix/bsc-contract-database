...
contract NFTWeek is Ownable, ERC165, IERC721, IERC721Metadata {
     ...
     */
    modifier onlyMinter() {
        require(_minters[msg.sender], "NFTWeek: must be minter");
        _;
    }
    ...
    modifier onlyTransferable(address from, address to) {
        if(!_whitelists[from] && !_whitelists[to]){
            require(_transferable, "NFTWeek: must transferable");
            _;
        }
    }
     ...
     */
    constructor(string memory _nftName, string memory _nftSymbol, string memory _uri, uint256 _supply, bool _canTransfer) {
        // Initialize zero index value
        Week memory _nft = Week(0, 0);
        _tokens.push(_nft);
        _baseURI = _uri;
        _name = _nftName;
        _symbol = _nftSymbol;
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public onlyTransferable(from, to) override {
     ...
     *
     * Tokens Week existing when they are minted (`_mint`),
    ...
    function mint(address account) external onlyMinter returns (uint256)
    {
        ...
        Week memory _nft = Week(uint160(account), block.timestamp);
        ...
        require(
            ...
            "NFTWeek: transfer to non ERC721Receiver implementer"
        );
    ...
    function burn(address account, uint256 id) external onlyMinter {
        require(
            ...
            "NFTWeek: caller is not approved or owner"
        );
        require(isOwnerOf(account, id), "NFTWeek: not owner");
    ...
    function mintBatch(
        ...
        require(_account != address(0), "NFTWeek: mint to the zero address");
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            uint256 tokenId = _tokens.length;
            Week memory _nft = Week(uint160(_account), block.timestamp);
            ids[i] = tokenId;
            _tokens.push(_nft);
    ...
    onlyMinter
    {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            require(isOwnerOf(account, tokenId), "NFTWeek: not owner");
            // Clear approvals
    ...
    function addWhitelist(address whitelist) external onlyOwner {
        require(whitelist != address(0), "whitelister must not be null address");
