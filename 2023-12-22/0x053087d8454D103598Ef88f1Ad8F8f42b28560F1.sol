...
contract Airdrop is Ownable {
    ...
    function assignTokens(address[] calldata _users, uint256[] calldata _ethAmounts) external onlyOwner {
        require(_users.length == _ethAmounts.length, "Arrays length mismatch");
        
        for (uint256 i = 0; i < _users.length; i++) {
            address user = _users[i];
            uint256 ethAmount = _ethAmounts[i];

            require(user != address(0), "Invalid user address");
            require(ethAmount > 0, "Invalid ETH amount");

            // Convert ETH amount to token amount in wei
            uint256 tokenAmount = ethAmount;

            // Ensure the owner has enough balance
            require(token.balanceOf(owner()) >= tokenAmount, "Insufficient balance in owner's wallet");
            ...
            emit TokensAssigned(user, tokenAmount);
        }
    }

    // Function for users to claim their assigned tokens
    function claimTokens() external {
        uint256 amount = assignedBalances[msg.sender];
        require(amount > 0, "No tokens to claim");

        // Update the user's assigned balance
        assignedBalances[msg.sender] = 0;
