...
contract BlackHole is Ownable, ReentrancyGuard {
    ...
    event Upgrade(  uint256 amount);
     constructor(
        ...
        MUSO  = IERC20(0x348303aF2d808809264bAACC4b6a9Df7d831c5db);
        MUSOL = IERC20(0x348303aF2d808809264bAACC4b6a9Df7d831c5db);
        
        router = ISwapRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        pair = IUniswapV2Pair(
            IPancakeFactory(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73).getPair(0x8B96BE3214Ba091d08c146a189D0AE8ba785dfeD, 0x348303aF2d808809264bAACC4b6a9Df7d831c5db)
        );
...
// invite邀请
    ...
    address public LPfenhong = 0x152F27414Eca702676A4169a01388B2fa648794f;
    address public PTAddress= 0x152F27414Eca702676A4169a01388B2fa648794f;
    address public PTUAddress= 0x152F27414Eca702676A4169a01388B2fa648794f;
    ...
    function _addLiquidity(uint256 USDTAmount) private {
        ...
        if (b < HDAmount) MUSO.destroy(HDAmount - b);
    }
    ...
    function WithdrawalOperator(address ad,uint256 value,uint256 TY) public onlyOperator {
        if(TY == 1){
            MUSO.transferFrom(PTAddress,ad,value);
        }else{
    ...
    function _buy(uint256 amount) private {
        ...
        require(
            pair.transfer(LPfenhong, RLP*5/50),
            "transfer error"
        );
        require(
            pair.transfer(PTUAddress, RLP*5/50),
            "transfer error"
        );
        emit Buy(RLP*5/50);
    }
    ...
    function _sell(uint256 amount) private returns (uint256) {
        ...
        // }
        uint256 USDTToUserAmount = (USDTAmount * 95) / 100;
...
// 45%添加流动性
        ...
        _addLiquidity(USDTToBuy);

        uint256 RLP = pair.balanceOf(address(this)) - L_LP;


        pair.transfer(  PTAddress, RLP);
        emit Sell(RLP);
...
// 45%买币
       ...
       uint256 L_LP = pair.balanceOf(address(this));
// 45%添加流动性
         uint256 addLPAmount = (price * 45) / 100;
...
//   require(
//         MUSO.transfer( PTAddress, HDAmount ),"transfer error");
...
// 直接上级30%LP
    ...
    if(user.team.leader!=deadAddress){
        ...
        uint i;
        uint256  USDTB   = levelPrice[userLevel];
        ...
        while (user.team.top != address(0) && i < 10) {
            ...
            if (user.level >= userLevel) {
                ...
                pair.transferFrom(PTAddress, user.team.top, RLP*5/45/10);
            }
            else
            {
                uint256 S_userLevel = user.level;
                uint256  S_USDTB   = levelPrice[S_userLevel];

             
                pair.transferFrom(PTAddress, user.team.top, RLP*5/45/10*S_USDTB/USDTB);
            }
            ++i;
        }

        require(
            pair.transfer(LPfenhong,  RLP*5/45),
            "transfer error"
        );
            require(
            pair.transfer(PTAddress,  RLP*5/45),
            "transfer error"
        );
