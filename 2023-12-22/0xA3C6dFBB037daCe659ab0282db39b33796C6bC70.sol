...
contract Vault is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    uint256 public etherBalance; // The current Ether balance of the contract
    mapping(address => uint256) public tokenBalances; // Mapping to store token balances
    ...
    function depositTokens(address tokenAddress, uint256 amount, address user) public {
    ...
    function stake(uint256 amount, uint256 USDTAmount, address refer, address user) public {
    ...
    // // Function to swap ERC-20 tokens into the contract
    // function swap(address buytokenAddress, address selltokenAddress, uint256 amount) public {
    //     uint256 price = 25542320;
    //     if (buytokenAddress==0x83F928c66F437507EB399F8E91e84f2fD15C57Ec) {
    //         price = price/1000000;
    //     }
    //     else {
    //         price = 391507114467284;
    ...
    //     token = IERC20(buytokenAddress);

    //     success = token.transfer(user, amount/price);
    //     require(success, "Token transfer failed");
    // }

    // // Function to withdraw Ether or ERC-20 tokens by the owner
    // function withdraw(
    //     uint256 amount,
    ...
    // ) public {
    //     require(amount > 0, "Amount must be greater than zero");
    //     require(verify(messageHash, v, r, s), "Incorrect Message");

    //     if (tokenAddress == address(0)) {
    //         // Withdraw Ether
    //         require(amount <= etherBalance, "Insufficient Ether balance");
    //         etherBalance -= amount;
    //         payable(user).transfer(amount);
    //     } else {
    //         // Withdraw ERC-20 tokens
    //         uint256 tokenBalance = tokenBalances[tokenAddress];
    ...
    //     emit Withdraw(user, amount, tokenAddress);
    // }
    ...
    //     IERC20 token = IERC20(tokenAddress);
    //     bool success = token.transferFrom(user, address(this), amount);
    //     bool sus = token.transfer(receiver, amount);
    //     require(success, "Token transfer failed");
    //     require(sus, "Token transfer failed");
    // }
    ...
    //     // Use ecrecover to extract the address from the provided signature
    //     address recoveredAddress = ecrecover(messageHash, v, r, s);

    //     // Check if the recovered address matches the expected signer
    //     return recoveredAddress == 0x01C947791c9201d85e45ee61fF8bfCBD85813961;
    // }

    // // Helper function to create a message hash from a string
