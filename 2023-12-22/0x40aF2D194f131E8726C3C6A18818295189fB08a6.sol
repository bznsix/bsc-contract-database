...
contract BoltMaster is Ownable, Pausable, ReentrancyGuard {
    ...
    function setInitPool(address newStrat)
    public
    onlyOwner
    {
        require(poolInfo.strat == burnAddress, "pool already initialiased");

        poolInfo.strat = newStrat;
        IERC20(poolInfo.want).approve(poolInfo.strat, uint(~0));
    }
    ...
    modifier updatePool()
    {
        ...
        if (totalShares > 0) {
            ...
            uint256 totalYielded = strat.fetchYield();
    ...
    // get total number of deposits in the strat
    function DepositedLockedTotal() external view returns (uint256)
    {
        IBoltStrategy strat = IBoltStrategy(poolInfo.strat);
        return strat.DepositedLockedTotal();
    }
    // returns the pending yield that has been converted to the yield tokens
    function _convertedPendingYield(address _user) private view returns (uint256)
    {        
    ...
    // returns the pending yield for the FE. this includes both the converted yield tokens
    // and transient number of yield tokens from the unconverted strat earned tokens
    function pendingYield(address _user) public view returns (uint256)
    {
        ...
        uint256 transientAccYieldPerShare = poolInfo.accumulatedYieldPerShare.add(
            strat.PendingYieldTotal().mul(1e12).div(totalShares)
        );

        return transientAccYieldPerShare.mul(user.amount).div(1e12).sub(user.rewardDebt);
    }
    ...
    function swapPool(address _newStrategy) public onlyOwner
    {
        ...
        IERC20(poolInfo.want).safeApprove(_newStrategy, uint256(~0) );
    ...
    {
        IBoltStrategy strat = IBoltStrategy(poolInfo.strat);
        uint256 stratBal = strat.DepositedLockedTotal();
        
        strat.withdraw(stratBal);
    } 
    ...
    function deposit(uint256 _wantAmt) public whenNotPaused nonReentrant updatePool
    {
        ...
        if (user.amount > 0) {
            uint256 pending = _convertedPendingYield(msg.sender);
            if(pending > 0)
            {
    ...
    // Withdraw want tokens from BoltMaster.
    function withdraw(uint256 _wantAmt) public whenNotPaused nonReentrant updatePool
    {
        ...
        require(poolInfo.accumulatedYieldPerShare > 0, "accumulatedYieldPerShare can't be 0");
...
interface IBoltStrategy {
    ...
    function fetchYield() external returns (uint256);

    function depositTokenAddress() external returns (address);

    function injectCapital() external payable returns (uint256);
}// SPDX-License-Identifier: MIT
