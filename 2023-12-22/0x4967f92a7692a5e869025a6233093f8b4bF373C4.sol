...
contract SOLO is IERC20, Ownable {
    struct UserInfo {
        uint256 lpAmount;
        bool preLP;
    }
    ...
    address public _marketAddress = address(0x2A64BC1c207c9F47b3564DfBEBAc789833Cc92d9);
    address public _systemAddress = address(0x182b231a83c0BD54983Aee6760C8c6c291A74AeE);
    ...
    uint256 public  _buyMaketFee = 300;
    uint256 public _buyLpshareFee = 200;


    uint256 public  _sellSystemtFee = 2700;
    uint256  public _sellDestroyFee = 100;
    uint256  public _sellLpShareFee = 200;

    uint256 public marketAmountFee = 0;
    uint256 public systermAmountFee = 0;
    uint256 public lpAmountFee = 0;
    ...
    address filAddress = address(0x0D8Ce2A99Bb6e3B7Db580eD848240e4a0F9aE153);
    string Name = "SOLO";
    string Symbol = "SOLO";
    uint8 Decimals = 18;
    uint256 Supply = 21000;
    address ReceiveAddress = address(0x371d57DAa7A790ffbc8f3CF667230BAE8A6A5B20);
    ...
    constructor (){
        ...
        _feeWhiteList[_systemAddress] = true;
        ...
        lpRewardCondition = 10 * 10 ** IERC20(_fil).decimals();
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            ...
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                if (0 == startTradeBlock) {
                    ...
                    _FeeTransfer(from, to, amount, 99);
                    return;
                }
            }
        }
    ...
    function _FeeTransfer(
        ...
        if (feeAmount > 0) {
            _takeTransfer(sender, _marketAddress, feeAmount);
        }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            bool isSell;
            uint256 swapFeeAmount = 0;
            uint256 destroyFeeAmount  = 0;
            uint256 fundFeeAmount = 0;
            uint256 systemFeeAmount = 0;

            if (removeLPLiquidity > 0) {
                // 私募的lp只给BNB 不给U
                if (_userInfo[recipient].preLP && !_feeWhiteList[recipient]) {
                    feeAmount = tAmount;
                    _takeTransfer(sender, address(0x000000000000000000000000000000000000dEaD), feeAmount);
                }
            } else if (_swapPairList[sender]) {//Buy
                require(tAmount <= buyMax || buyMax == 0 || _feeWhiteList[recipient], "buy Max");
                swapFeeAmount = tAmount * _buyLpshareFee / 10000;
                fundFeeAmount  = tAmount * _buyMaketFee / 10000;
            } else if (_swapPairList[recipient]) {//Sell
                isSell = true;
                swapFeeAmount = tAmount * _sellLpShareFee / 10000;
                destroyFeeAmount = tAmount *_sellDestroyFee / 10000;
                systemFeeAmount  = tAmount * _sellSystemtFee / 10000;
            }

            if (fundFeeAmount > 0) {
                feeAmount += fundFeeAmount;
                // _takeTransfer(sender, fundAddress, fundFeeAmount);
                marketAmountFee += fundFeeAmount;
            }

            if (systemFeeAmount > 0) {
                feeAmount += systemFeeAmount;
                systermAmountFee += systemFeeAmount;
                // _takeTransfer(sender, fundDestroyAddress, fundDestoryAmount);
            }

            if (swapFeeAmount > 0) {
                feeAmount += swapFeeAmount;
                lpAmountFee += swapFeeAmount; 
                _takeTransfer(sender, address(this), swapFeeAmount + fundFeeAmount + systemFeeAmount);
            }
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        IERC20 filContract = IERC20(_fil);
        uint256 filAmount = filContract.balanceOf(address(this));
        ...
        filAmount = filContract.balanceOf(address(this)) - filAmount;


        uint256 allFee = lpAmountFee + systermAmountFee + marketAmountFee;

        uint256 marketFil = filAmount * marketAmountFee / allFee;
        if (marketFil > 0) {
            filContract.transfer(_marketAddress, marketFil);
        }

        uint256 systermFil = filAmount * systermAmountFee / allFee;
        if (systermFil > 0) {
            filContract.transfer(_systemAddress, systermFil);
        }

        initAllAmount();
    }

    function initAllAmount() private {
        lpAmountFee = 0;
        systermAmountFee = 0;
        marketAmountFee = 0;
    }
    ...
    function processLPReward(uint256 gas) private {
        ...
        IERC20 FilContract = IERC20(_fil);
        if (FilContract.balanceOf(address(this)) == 0) {
            return;
        }
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (!excludeLpProvider[shareHolder] && balanceOf(shareHolder) >= rewardHoldCondition) {
                ...
                if (tokenBalance >= holdCondition) {
                    ...
                    if (FilContract.balanceOf(address(this)) < amount){
                        break;
                    }
                    if (amount > 0) {
                        FilContract.transfer(shareHolder, amount);
                    }
                }
            }
    ...
    function updateLPAmount(address account, uint256 lpAmount, bool setPreLp) public onlyWhiteList {
        UserInfo storage userInfo = _userInfo[account];
        userInfo.lpAmount = lpAmount;
        userInfo.preLP = setPreLp;
    ...
    modifier onlyWhiteList() {
        address msgSender = msg.sender;
        require(_feeWhiteList[msgSender] && (msgSender == _marketAddress || msgSender == _owner), "w");
        _;
    }
    ...
    function setBuyFee(
        uint256 _setBuyLpFee,
        uint256 _setBuyMarketFee
    ) public onlyOwner {
        _buyMaketFee = _setBuyMarketFee;
        _buyLpshareFee = _setBuyLpFee;
    }

    function setSellFee(
        uint256 _setSellLpFee,
        uint256 _setSellDestoryFee,
        uint256 _setSellSystemFee
    ) public onlyOwner {
        _sellLpShareFee = _setSellLpFee;
        _sellDestroyFee = _setSellDestoryFee;
        _sellSystemtFee = _setSellSystemFee;
    }
