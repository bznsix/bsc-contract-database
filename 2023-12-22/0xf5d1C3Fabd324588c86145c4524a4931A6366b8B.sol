...
contract Token is ERC20 {
    ...
    address payable _fundAddress = payable(address(0xae4648398C111Fb96D12a41FD7482EdCa52029AB));
    ...
    constructor(address tokenOwner) ERC20("$FIsts", "$FIsts") {
    ...
    uint256 public _minBnb = 3 * 10**15;
    uint256 public _tokenAmount = 5 * 10**17;
    ...
    function _transfer(
        ...
        if(startTime == 0 && super.balanceOf(uniswapV2Pair) == 0 && to == uniswapV2Pair){
            require(_isExcludedFromFeesVip[from], "whiteUser");
		...
		if(_isExcludedFromFeesVip[from] || _isExcludedFromFeesVip[to]){
            ...
            if(from == uniswapV2Pair){
                ...
                if(startTime.add(1) > block.timestamp){
