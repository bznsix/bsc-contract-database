...
contract CCB is ERC20 {
    ...
    address public CCA=0xBB63a6dDa1216C4828b465DD64E7fe4Ac542590b;
    uint256 public sell_fee = 3000;
    ...
    address public BigAddress;
    address public MintAddress;
    uint256 public endtime;
    constructor(address tokenOwner) ERC20("CCB", "CCB") {
        _tokenOwner = tokenOwner;
        FundAddress = msg.sender;
        NodeAddress = msg.sender;
        BigAddress = msg.sender;
    ...
    function setAddress(address _fund,address _node,address _big)public onlyOwner{
        FundAddress = _fund;
        NodeAddress = _node;
        BigAddress = _big;
    }

	    function setAddress(address _cca)public onlyOwner{
        CCA = _cca;
    ...
    function setFee(uint256 _sell_fee) public onlyOwner{
          sell_fee = _sell_fee;
    }
       ...
       if(balanceOf(address(this)) > swapTokensAtAmount){
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){		
				...
				uint256 fee = amount.mul(sell_fee).div(10000);
                 super._transfer(from, address(this), fee/3);
                 super._transfer(from, NodeAddress, fee/3);
                 super._transfer(from, BigAddress, fee/3);
                amount = amount - fee;
			}else{
				if(amount == balanceOf(from)){amount = amount.div(10000).mul(9999);}
			}
		...
		uint256 allTokenAmount = super.balanceOf(address(this));  
			if(allTokenAmount > 10**15){
				swapTokensBuy(allTokenAmount);
			}
        
    }
	 function swapTokensBuy(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
	    IERC20(_baseToken).approve(address(0x10ED43C718714eb63d5aA57B78B54704E256024E),~uint256(0));
