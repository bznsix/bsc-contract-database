...
contract DCT is ERC20, Ownable {
    ...
    DCTDividendTracker public dividendTracker;
    ...
    constructor() ERC20("DUTCH CAT", "DCT") {
        marketingWallet = payable(0x6045793d235782DfA20eC63F03213Ae1592207Dd);
        devWallet = payable(0xFb3E9DD2c5FF76BE17099ceCCB808bC01c5Ed202);
        ...
        dividendTracker = new DCTDividendTracker(
            payable(this),
            router,
            0xB397Fd3D14d7464F043A242e776B4f52871D28B6,
            "DCTTRACKER",
            "DCTTRACKER"
        );
...
contract DCTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "DCT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DCT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DCT contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(DCTContract);

            uint256 prevBalance = DCTContract.balanceOf(address(this));
            ...
            {
                uint256 received = DCTContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    DCTContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
