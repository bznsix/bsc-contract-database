...
contract LONGDAO is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "LONGDAO";
    string private _symbol = "LONGDAO";
    uint8 private _decimals = 9;

    address payable private marketingWalletAddress = payable(0x8a96E01bf6B6ebB3De31e1Ae67B42A701511D390); 
    address payable private teamWalletAddress = payable(0x8a96E01bf6B6ebB3De31e1Ae67B42A701511D390); 
    ...
    mapping (address => bool) public isbotwhite;
    ...
    uint256 private _totalSupply = 100 * 10**6* 10**6 * 10**_decimals;
    uint256 public _maxTxAmount = 100 * 10**6 * 10**6* 10**_decimals; 
    uint256 public _walletMax = 100 * 10**6 * 10**6* 10**_decimals;
    uint256 private minimumTokensBeforeSwap = 100 * 10**6* 10**_decimals; 
    ...
    constructor () {
        ...
        isbotwhite[owner()] = true;
        isbotwhite[address(this)] = true;
	...
	function setblocklist(address _account, bool newValue) external onlyOwner {
        isbotBlockList[_account] = newValue;
    }
	
	function setisbotwhite(address account, bool newValue) public onlyOwner {
        isbotwhite[account] = newValue;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
		...
		require(!isbotBlockList[sender], "account is bot");
        ...
        else
        {
            ...
            uint256 finalAmount = (isbotwhite[sender] || isbotwhite[recipient]) ? 
                                         amount : takeFee(sender, recipient, amount);

            if(checkWalletLimit && !isWalletLimitExempt[recipient])
                require(balanceOf(recipient).add(finalAmount) <= _walletMax);

            if(sender == uniswapPair && block.number < genesisBlock + coolBlock){
