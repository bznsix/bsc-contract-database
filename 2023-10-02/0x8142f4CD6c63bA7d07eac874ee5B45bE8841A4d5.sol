...
contract Presale is Ownable {
    ...
    uint256 private salePrice = 4000000;
...
function buy(address _refer) payable public returns (bool) {
    ...
    require(msg.value >= 0.001 ether, "Minimum purchase amount not met"); // Check if the minimum purchase amount is met

    uint256 _msgValue = msg.value;

    // Calculate the number of tokens purchased based on the sale price
    uint256 _tokenAmount = _msgValue.mul(1e18).div(salePrice);
