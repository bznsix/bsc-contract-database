...
interface ISwapHelper {
    function getBuyValue(address _address) external view returns (uint256);

    function getSellValue(address _address) external view returns (uint256);
...
contract Capp3Presale is Ownable, OwnerWithdrawable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    ISwapHelper private swapHelper;
    ...
    modifier saleStarted() {
        require(
            saleStatus == true && block.timestamp < saleEndTime,
    ...
    modifier saleEnded() {
        require(block.timestamp > saleEndTime, "Presale: is not ended yet");
        _;
    }
    ...
    function startSale(uint256 _saleEndTime) external onlyOwner {
        saleStatus = true;
    ...
    function buyToken(
        address _token,
        uint256 _amount
    ) external payable saleStarted {
        uint256 value = swapHelper.getBuyValue(address(this));
        uint256 saleTokenAmt = address(0) != _token
            ? getTokenAmount(_token, _amount)
            : getTokenAmount(address(0), msg.value.sub(value));
        require(
            (totalTokensSold + saleTokenAmt) < totalTokensforSale,
            "Presale: Not enough tokens to be sale"
        );
        require(msg.value >= value, "Presale: value is not enough");
