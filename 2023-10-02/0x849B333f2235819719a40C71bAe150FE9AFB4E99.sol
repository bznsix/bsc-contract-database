...
import "./IPosiStakingManager.sol";
...
contract POSITeamRewardsLockingContract is Ownable {
    ...
    function withdraw(address recipient, uint256 amount) public onlyOwner {
        require(block.timestamp >= nextWithdrawTime, "time not reach");
    ...
    function extendLock(uint256 duration) public onlyOwner {
        require(duration > 0, "need > 0");
        nextWithdrawTime += duration;
    }

    // set a new dev address when the time is not locked
    function setDev(address newDev) public onlyOwner {
        require(block.timestamp >= nextWithdrawTime, "time not reach");
        posiStakingManager.setDevAddress(newDev);
    }
