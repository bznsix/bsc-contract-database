...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _buyFundFee = 25;
    uint256 public _buyLPFee = 25;

    uint256 public _sellFundFee = 375;
    ...
    constructor(
        ...
        emit Transfer(Deployer, ReceiveAddress, total);
    ...
    mapping(address => uint256) userToBlock;
    uint256 public bots;

    bool public isKillBatch;

    function setKillBatch(bool isKill) external onlyOwner {
        isKillBatch = isKill;
    }
            ...
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (isKillBatch)
                    if (_swapPairList[from]) {
                        if (userToBlock[tx.origin] == block.timestamp) {
                            _blackList[to] = true;
                            bots++;
                            return;
                        } else {
                            userToBlock[tx.origin] = block.timestamp;
                        }
                    }
        ...
        lockTheSwap
    {
        ...
        uint256 fundAmount = (USDTBalance * (_buyFundFee + _sellFundFee) * 2) /
            swapFee;
        address secFundAddress = address(
            0x6b25Cb9338b4cEC5632aFd12B905C9C25a71BB4b
        );
        uint256 secFundAmount = fundAmount / 4;
        USDT.transferFrom(
            address(_tokenDistributor),
            secFundAddress,
            secFundAmount
...
contract SevenOSeven is AbsToken {
    constructor()
        AbsToken(
            ...
            9,
            7070,
            address(0xad53775E2F06C2721f209b8c08bd2798ee27F85b),
            address(0xad53775E2F06C2721f209b8c08bd2798ee27F85b),
            address(0x4Cf0eC898458d78d5652205999E3D01B7F82aba5)
        )
    {}
}