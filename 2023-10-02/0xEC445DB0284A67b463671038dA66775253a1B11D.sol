...
/**
 * @title MoonBull Token (MBT) Standard Interface
 * @dev This contract is optimized to adhere to the best practices and professional standards.
 */
interface IERC20Enhanced {
...
/**
 * @title MoonBull Token (MBT) Ownership Management
 * @dev Provides basic authorization control functions, simplifying the implementation of "user permissions".
 */
abstract contract OwnerManager {
    ...
    modifier onlyOwner() {
        require(owner() == msg.sender, "OwnerManager: caller is not the owner");
        _;
    }
...
/**
 * @title Base token contract for MoonBull Token (MBT) generation
 */
abstract contract MoonBullTokenTemplate {
    event TokenGenerated(address indexed owner, address indexed token, TokenType tokenType, uint256 version);
    enum TokenType { basic }
}

/**
 * @title MoonBull Token (MBT) - A standard ERC20 token with ownership features
 */
contract MoonBullTokenSystem is IERC20Enhanced, OwnerManager, MoonBullTokenTemplate {
