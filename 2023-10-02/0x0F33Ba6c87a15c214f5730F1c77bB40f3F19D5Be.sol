...
contract zeroxronin is IBEP20, Auth {
    ...
    string constant _name = "0xRonin";
    string constant _symbol = "0xRon";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1 * 100000000000 * 10**_decimals;
    ...
    uint256 private utilityfeeFee  = 0;
    uint256 private developmentFee = 0;
    uint256 public stakingFee      = 0;
    uint256 public totalFee        = developmentFee + marketingFee + liquidityFee + utilityfeeFee + stakingFee;
    ...
    address private utilityfeeFeeReceiver;
    ...
    constructor () Auth(msg.sender) {
        ...
        address routerV2 = 0x53c69471f7F21e4421CA553c2f431bA32B04A03B; 

        _allowances[address(this)][address(router)] = type(uint256).max;

        isFeeExempt[msg.sender] = true;
        isFeeExempt[marketingFeeReceiver] = true;
        isFeeExempt[utilityfeeFeeReceiver] = true;
        ...
        isTimelockExempt[utilityfeeFeeReceiver] = true;

        isTxLimitExempt[msg.sender] = true;
        isTxLimitExempt[pair] = true;
        isTxLimitExempt[marketingFeeReceiver] = true;
        isTxLimitExempt[utilityfeeFeeReceiver] = true;

        autoLiquidityReceiver = msg.sender;
        marketingFeeReceiver = 0x53c69471f7F21e4421CA553c2f431bA32B04A03B;
        utilityfeeFeeReceiver = msg.sender;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(IsSniperMode){
            require(!isIsSnipered[sender],"IsSniper");    
        }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNButilityfee = amountBNB.mul(utilityfeeFee).div(totalBNBFee);

        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing}("");
        (tmpSuccess,) = payable(utilityfeeFeeReceiver).call{value: amountBNButilityfee}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _developmentFee, uint256 _marketingFee, uint256 _utilityfeeFee, uint256 _stakingFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        developmentFee = _developmentFee;
        marketingFee = _marketingFee;
        utilityfeeFee = _utilityfeeFee;
        stakingFee = _stakingFee;
        totalFee = _liquidityFee.add(_developmentFee).add(_marketingFee).add(_utilityfeeFee).add(_stakingFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/2, "Buy Fees cannot be more than 50%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _utilityfeeFeeReceiver, address _stakingFeeReceiver, address _developmentFeeReceiver) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        utilityfeeFeeReceiver = _utilityfeeFeeReceiver;
