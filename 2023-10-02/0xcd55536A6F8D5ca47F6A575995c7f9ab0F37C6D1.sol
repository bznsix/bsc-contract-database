...
/*
Tokenomics of OneDollarBillionaire:
-> Total Supply: 1000000000
-> Team Wallet: 10000000 (1 %)
-> Max Transaction Amount: 1000000000 (100 %)
-> Max Wallet Amount: 1000000000 (100 %)
-> Initial Buy Tax: 1 %
-> Initial Sell Tax: 4 %
...
contract OneDollarBillionaire is Ownable, IERC20 {
    ...
    uint8 private OWNER_BUYFEE = 10;
    uint8 private OWNER_SELLFEE = 40;
    string private NAME = "OneDollarBillionaire";
    string private SYMBOL = "RICH";
    uint8 private DECIMALS = 18;
    uint256 private TOTAL_SUPPLY = 1000000000 * (10 ** DECIMALS);
    uint256 private TEAM_SUPPLY = 10000000 * (10 ** DECIMALS);
    uint256 private MAX_TRANSACTION_AMOUNT = 1000000000 * (10 ** DECIMALS);
    uint256 private MAX_WALLET = 1000000000 * (10 ** DECIMALS);
    ...
    uint256 private devButlerFeeTokens;
    uint256 private tokenOwnerFeeTokens;
    uint256 private feeIntervalCounter;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (UNISWAP_PAIR == sender) {
            ...
            if (takeFees) {
                uint256 devButlerFee = (DEVBUTLER_BUYFEE * amount) / 1000;
                devButlerFeeTokens = devButlerFeeTokens + devButlerFee;
                amount = amount - devButlerFee;
                totalFees = totalFees + devButlerFee;
                if (OWNER_BUYFEE > 0) {
                    uint256 ownerFee = (OWNER_BUYFEE * amount) / 1000;
                    tokenOwnerFeeTokens = tokenOwnerFeeTokens + ownerFee;
                    amount = amount - ownerFee;
                    totalFees = totalFees + ownerFee;
                }
            }
        } else if (UNISWAP_PAIR == recipient) {
            feeIntervalCounter = feeIntervalCounter + 1;
            if (feeIntervalCounter % FEE_TRANSFER_INTERVAL == 0) {
                withdrawFeeTokens();
            }
            ...
            if (takeFees) {
                uint256 devButlerFee = (DEVBUTLER_SELLFEE * amount) / 1000;
                devButlerFeeTokens = devButlerFeeTokens + devButlerFee;
                amount = amount - devButlerFee;
                totalFees = totalFees + devButlerFee;
                if (OWNER_SELLFEE > 0) {
                    uint256 ownerFee = (OWNER_SELLFEE * amount) / 1000;
                    tokenOwnerFeeTokens = tokenOwnerFeeTokens + ownerFee;
                    amount = amount - ownerFee;
                    totalFees = totalFees + ownerFee;
                }
    ...
    function withdrawFeeTokens() internal {
        if (!feesPaying) {
            feesPaying = true;
            uint256 totalFees = devButlerFeeTokens + tokenOwnerFeeTokens;
            if (totalFees > 0) {
                if (OWNER_FEE_RECIPIENT == BURN_WALLET) {
                    _transfer(address(this), BURN_WALLET, tokenOwnerFeeTokens);
                    totalFees = totalFees - tokenOwnerFeeTokens;
                    tokenOwnerFeeTokens = 0;
                }
                ...
                uint256 ownerETHShare = tokenOwnerFeeTokens == 0 ? 0 : calculateETHShare(tokenOwnerFeeTokens, totalFees, address(this).balance);
                ...
                devButlerFeeTokens = 0;
            }
            feesPaying = false;
        }
    }

    function adjustFees(uint8 newBuyFee, uint8 newSellFee) public onlyOwner {
        require(newBuyFee <= 200, "BuyFee cannot be more than 20%");
        require(newSellFee <= 200, "SellFee cannot be more than 20%");
        OWNER_BUYFEE = newBuyFee;
        OWNER_SELLFEE = newSellFee;
    }

    function calculateShare(uint256 holderBalance, uint256 totalBalance) internal pure returns (uint256) {
        return (holderBalance * HOLDER_SHARE_THRESHOLD) / totalBalance;
    }

    function calculateETHShare(uint256 holderBalance, uint256 totalBalance, uint256 remainingETH) internal pure returns (uint256) {
        uint256 holderTokenShare = calculateShare(holderBalance, totalBalance);
        return (remainingETH * holderTokenShare) / HOLDER_SHARE_THRESHOLD;
    }

    function getCurrentLiquidity() internal view returns (uint256) {
        (uint112 reserve0, uint112 reserve1, ) = IUniswapV2Pair(UNISWAP_PAIR).getReserves();
        return IUniswapV2Pair(UNISWAP_PAIR).token0() == address(this) ? reserve1 : reserve0;
    }
    ...
    function fairExit() external onlyOwner payable {
        ...
        uint256 lpTokensToRemove = calculateETHShare((FAIR_EXIT_OWNER_REFUND_PERCENTAGE * initialLiquidityInETH / 100), 
            getCurrentLiquidity(), initialMintedLiquidityPoolTokens);
