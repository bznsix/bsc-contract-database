...
contract PZPCrossChain is IERC20, ReentrancyGuard {
    ...
    event LogSwapIn(address from, address to, uint256 amount, uint date);
    event LogSwapOut(address from, address to, uint256 amount, uint date);
    event Deposit(address from, address to, uint256 amount, uint date);
    event DepositFrom(address from, address to, uint256 amount, uint date);
    ...
    function setLimit(uint256 amount) external OnlyAdmin(){
        _limit = amount;
    }
    ...
    function SwapIn(uint256 amount)nonReentrant external HasRole returns (bool) {
        ...
        require(token.balanceOf(address(this)) <= _limit, "Token balance is more than the limit");

        // Transfer tokens from the 'caller' address to the 'to' address
        token.safeTransferFrom(msg.sender, address(this), amount);
        emit LogSwapIn(msg.sender, address(this), amount, block.timestamp);
        return true;
    }
    ...
    function SwapOut(address from,address to,uint256 amount, address commissionAddress, uint256 fee)nonReentrant external HasRole{
        ...
        emit LogSwapOut(from, to, amount, block.timestamp);
    }

    function deposit(uint256 amount)nonReentrant external returns (bool) {
        require(token.balanceOf(msg.sender) > 0, "PZPV1: Not enough pzp");
        require(token.balanceOf(address(this)) <= _limit, "Token balance is more than the limit");

        // Transfer tokens from the 'caller' address to the 'to' address
        token.safeTransferFrom(msg.sender, address(this), amount);
        emit Deposit(msg.sender, address(this), amount, block.timestamp);
        return true;
    }
    ...
    function grantRole(address _address) external OnlyAdmin {
