...
contract FarmageddonLottoKeeper is KeeperCompatibleInterface, Ownable {
    ...
    address  public treasury = 0x9ac1A7751335656C44723B5E9Ed62571fc763c11;
    uint256  public bnbToCL = 10000000000000000;
    ...
    function fundCLAdmin() external payable onlyOwner {
        address[] memory path1 = new address[](2);
            path1[0] = bnb;
            path1[1] = linkPeg;
    ...
    function UnpauseAndStartNextLottery(address lottery, uint256 _NextLotteryEndTime, uint256 addHowManyDraws) external onlyOwner {
        ...
        lotteries[lottery].Pause = false;
            lotteryContract.startLottery(_NextLotteryEndTime, lotteries[lottery].fticketPrice, lotteries[lottery].fdiscount, lotteries[lottery].frewardsBreakdown, lotteries[lottery].ftreasuryFee);
            ...
            lotteries[lottery].upKeepTime = block.timestamp + 300;
                ...
                if (lotteries[lottery].Pause) lotteries[lottery].step = 0;    
                    ...
                    lotteries[lottery].upKeepTime = lotteries[lottery].intervalSeconds + lotteries[lottery].drawTime;
                        lotteryContract.startLottery(lotteries[lottery].upKeepTime, lotteries[lottery].fticketPrice, lotteries[lottery].fdiscount, lotteries[lottery].frewardsBreakdown, lotteries[lottery].ftreasuryFee);
                    lotteries[lottery].step = 1;
                        lotteries[lottery].fcurrentLotteryId = lotteryContract.viewCurrentLotteryId();
