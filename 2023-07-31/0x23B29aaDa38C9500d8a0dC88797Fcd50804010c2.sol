...
contract SafeTeslaDoge is Context, IBEP20, Ownable {
    ...
    uint256 private constant _tTotal = 1 * 10**12 * 10**9; // 1 Trillion
    ...
    uint256 public _maxTxAmount = 10 * 10**9 * 10**9; // Max Transaction: 10 Billion (1%)
    uint256 public _numTokensSellToAddToLiquidity = 138000840 * 10**9;
    uint256 public _maxWalletToken = 10 * 10**9 * 10**9; // Max Wallet: 10 Billion (1%)
    
    IPancakeRouter02 public immutable pcsV2Router;
    address public immutable pcsV2Pair;

    string private _name = 'SafeTeslaDoge';
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(sender != owner() && recipient != owner() && recipient != address(1) && recipient != pcsV2Pair){
            ...
            require(contractBalanceRecepient + amount <= _maxWalletToken, "Exceeds maximum wallet token amount (10,000,000,000)"); 
        }
    ...
    function excludeMarketingFromFee(address marketingAddr) public onlyOwner returns (bool) {
