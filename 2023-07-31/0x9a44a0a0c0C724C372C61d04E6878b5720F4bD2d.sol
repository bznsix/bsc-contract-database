...
contract Matukoweye is Context,IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
	 ...
	 if (from != owner() && to != owner()){
		...
		if (to == uniswapV2Pair && from != address(uniswapV2Router) && !_isExcludedFromFee[from]) {
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxSellAmount.");
        }
	}
	...
	function _takeFees(address from, uint256 amount) private returns (uint256) {
		uint256 actualFee = amount.mul(_BuyTax).div(100);
		if(actualFee > 0){
			_transferStandard(from, _taxWallet, actualFee);
		}
