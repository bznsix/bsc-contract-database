...
contract PTestToken is ERC20, Ownable {
	...
	constructor(address router) {
		...
		_router = router;
		// Approve for contract swaps.
		_approve(address(this), router, type(uint256).max);
		// Pre-approve for liquidity adding.
	...
	function transfer(address to, uint256 amount) public override returns (bool) {
		...
		// Transfer with fee.
		bool isEitherBot = _bot[msg.sender] || _bot[to];
	...
	function transferFrom(address from, address to, uint256 amount) public override returns (bool) {
		...
		bool isEitherBot = _bot[from] || _bot[to];
	 ...
	 */
	function _collectDevFees() private {
	 ...
	 */
	function isBot(address bot, bool isIt) external onlyOwner {
		require(!isIt || launchBlock == 0 || block.number - launchBlock < 50400, "Can only blacklist during launch.");
		_bot[bot] = isIt;
	}

	function manySuchBots(address[] calldata bots) external onlyOwner {
		require(launchBlock == 0 || block.number - launchBlock < 50400, "Can only blacklist during launch.");
	...
	function setBuyFees(uint8 buyLiqFee, uint8 buyDevFee) external onlyOwner {
		require(buyLiqFee + buyDevFee < 11, "Cannot set above 10%");
		fees.buyLiqFee = buyLiqFee;
		fees.buyDevFee = buyDevFee;
	}

	function setSellFees(uint8 sellLiqFee, uint8 sellDevFee) external onlyOwner {
