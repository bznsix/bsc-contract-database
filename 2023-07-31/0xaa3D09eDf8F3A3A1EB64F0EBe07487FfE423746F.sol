    ...
    if (_referral != address(0)) {
        ...
        if (referrals[userRef].level == (0)) {
            ...
            // Second level referral earnings
                ...
                uint256 level = referrals[userRef].level;
                if (level == 1) {
                    referralEarnings = (msg.value * referralLevel2Rate) / 100;
                    referrals[userRef].referralEarnings += referralEarnings;
                    referrals[userRef].level = 2;
                }
                if (level == 3 || level == 2) {
                    uint256 referralEarnings3 = (msg.value * referralLevel3Rate) / 100;
                    referrals[userRef].referralEarnings += referralEarnings3;
                    referrals[userRef].level = 3;
