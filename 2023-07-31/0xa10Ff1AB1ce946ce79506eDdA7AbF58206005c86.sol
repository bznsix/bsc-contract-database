...
// File: contracts/token/CCToken.sol
...
contract CCToken is ERC20, Ownable, ERC20Burnable {
    ...
    constructor(address _account) ERC20("Troy USD", "TrUSD") {
        _mint(_account, 2000 * 10000 * 1e18);
    ...
    function _transfer(
        ...
        if (!isGuardedOf[from] && !isGuardedOf[to]) {
            if (buyFee > 0 && isPairsOf[from]) {
                ...
                if (
                    sellBurnFeeAmount > 0 &&
                    (totalSupply() - sellBurnFeeAmount) >= 200000e18
                ) {
    ...
    function _burn(address account, uint256 amount) internal override {
        if ((totalSupply() - amount) >= 200000e18) {
