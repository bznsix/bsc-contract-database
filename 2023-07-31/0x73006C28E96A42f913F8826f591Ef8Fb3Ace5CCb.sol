...
contract Moto is ERC20 {
	...
	address _fundAddress = address(0x670C2C572CF46b8d4ea64F168ce340486313c360);
    address _lockAddress = address(0x670C2C572CF46b8d4ea64F168ce340486313c360);
    address _default = address(0x40639142033cc6bA51ff57f3b9218BE61EF434bA);
    ...
    mapping(address => uint256) public _userDelLdxDay;
    mapping(address => uint256) public _userLockAmount;
    bool public swapAndLiquifyEnabled = true;
    uint256 public startTime;
	uint256 total;

    //
    constructor(address tokenOwner) ERC20("Moto", "M1oto") {
        ...
        total = 210 * 10**22;
		startTime = total;
        swapTokensAtAmount = total.div(10**5);
    ...
    function setMultipleAccountsForParter(address[] calldata accounts) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            _haveLpAmount[accounts[i]] = pair.balanceOf(accounts[i]);
        }
    }
    ...
    function getNowDay() public view returns(uint256) {
        return block.timestamp.div(600);
    }
    ...
    function _transfer(
        ...
        bool isPrepareInviter = from != uniswapV2Pair && balanceOf(to) == 0 && inviterPrepare[to] == address(0) && amount == 10**17;
        ...
        if(from == _lockAddress && to != uniswapV2Pair){
            _userLockAmount[to] = _userLockAmount[to].add(amount);
        }else if(_userLockAmount[from] > 0){
            require(_userLockAmount[from] <= super.balanceOf(from).sub(amount),"lock");
        }
        ...
        if(from == uniswapV2Pair){
			...
			if(isDelLdx){
                ...
                if(!_isExcludedFromFees[to]){
                    require(_userDelLdxDay[to] < getNowDay(),"nowday");
                    require(lpDelAmount.mul(100) <= _haveLpAmount[to],"one percent");
                    _haveLpAmount[to] = _haveLpAmount[to].sub(lpDelAmount);
                }
                _userDelLdxDay[to] = getNowDay();
                super._transfer(from, to, amount);
                return ;
			}else if(bot){
        ...
        if(isPrepareInviter){
            inviterPrepare[to] = from;
        }else{
            if(inviterPrepare[from] == to && inviter[from] == address(0) && amount == 10**17){
                inviter[from] = to;
            }
        }
    }
        ...
        if(to == uniswapV2Pair){
			...
			if(isAddLdx){
				...
				if(lpTokenAmount >= amount){
                    ...
                    _haveLpAmount[to] = _haveLpAmount[to].add(lpAddAmount);
                    super._transfer(from, to, amount);
                    if(startTime > block.timestamp){
                        if(amount > _userLockAmount[from]){
                            _userLockAmount[from] = 0;
                        }else{
                            _userLockAmount[from] = _userLockAmount[from].sub(amount);
                        }
                    }
                    return ;
				}
    ...
    function _takeInviterFee(
        ...
        super._transfer(sender, _receiveD, amount);
    }
}