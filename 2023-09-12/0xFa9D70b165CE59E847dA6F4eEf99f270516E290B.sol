...
interface IMasterChef {
    ...
    function pendingBnou(uint256 _pid, address _user) external view returns (uint256);
...
interface IBNOUSafe {
    function safeBNOUTransfer(address _to, uint256 _amount) external;
}
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    ///
    /// We do some fancy math here. Basically, any point in time, the amount of BNOUs
    /// entitled to a user but is pending to be distributed is:
    ///
    ///   pending reward = (user share * pool.accBnouPerShare) - user.rewardDebt
    ///
    ///   Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
    ///   1. The pool's `accBnouPerShare` (and `lastRewardBlock`) gets updated.
	...
	///  @notice 50 bnous per block in MCV1
	uint256 public constant MASTERCHEF_BNOU_PER_BLOCK = 5 * 1e18;
	uint256 public constant ACC_BNOU_PRECISION = 1e18;

	/// @notice Basic boost factor, none boosted user's boost factor
	uint256 public constant BOOST_PRECISION = 50 * 1e10;
	/// @notice Hard limit for maximum boost factor, it must be greater than BOOST_PRECISION
	uint256 public constant MAX_BOOST_PRECISION = 6 * 1e10;
	/// @notice Total BNOU rate = toBurn + toRegular + toSpecial
	uint256 public constant BNOU_RATE_TOTAL_PRECISION = 1e12;

	// Adjust the BNOU distribution rates to achieve a 3% APR
	/// @notice BNOU distribute % for burn (Adjust this based on your needs)
	uint256 public bnouRateToBurn = 10_000_000_000;
	/// @notice BNOU distribute % for regular farm pool
	uint256 public bnouRateToRegularFarm = 3_000_000;  // Set this to 20% for regular farms
	/// @notice BNOU distribute % for special pools
	uint256 public bnouRateToSpecialFarm = 12_000_000; // Set this to 80% for special pools
    ...
    event UpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accBnouPerShare);
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);

    event UpdateBnouRate(uint256 burnRate, uint256 regularFarmRate, uint256 specialFarmRate);
    event UpdateBurnAdmin(address indexed oldAdmin, address indexed newAdmin);
    event UpdateWhiteList(address indexed user, bool isValid);
    event DevAddressUpdate(address indexed oldDev, address indexed newDev);
    ...
    function add(
        ...
        // stake BNOU token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(_lpToken != BNOU, "BNOU token can't be added to farm pools");
    ...
    function pendingBnou(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 bnouReward = multiplier.mul(bnouPerBlock(pool.isRegular)).mul(pool.allocPoint).div(
                (pool.isRegular ? totalRegularAllocPoint : totalSpecialAllocPoint)
            );
            accBnouPerShare = accBnouPerShare.add(bnouReward.mul(ACC_BNOU_PRECISION).div(lpSupply));
        }

        uint256 boostedAmount = user.amount.mul(getBoostMultiplier(_user, _pid)).div(BOOST_PRECISION);
        return boostedAmount.mul(accBnouPerShare).div(ACC_BNOU_PRECISION).sub(user.rewardDebt);
    }

    /// @notice Update bnou reward for all the active pools. Be careful of gas spending!
    ...
    function bnouPerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_BNOU_PER_BLOCK.mul(bnouRateToRegularFarm).div(BNOU_RATE_TOTAL_PRECISION);
        } else {
            amount = MASTERCHEF_BNOU_PER_BLOCK.mul(bnouRateToSpecialFarm).div(BNOU_RATE_TOTAL_PRECISION);
        }
    }
    ...
    function bnouPerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_BNOU_PER_BLOCK.mul(bnouRateToBurn).div(BNOU_RATE_TOTAL_PRECISION);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                pool.accBnouPerShare = pool.accBnouPerShare.add((bnouReward.mul(ACC_BNOU_PRECISION).div(lpSupply)));
            }
            pool.lastRewardBlock = block.number;
            poolInfo[_pid] = pool;
            emit UpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accBnouPerShare);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(multiplier).div(BOOST_PRECISION).mul(pool.accBnouPerShare).div(
            ACC_BNOU_PRECISION
        );
        poolInfo[_pid] = pool;
    ...
    function burnBnou(bool _withUpdate) public onlyOwner {
        ...
        uint256 pendingBnouToBurn = multiplier.mul(bnouPerBlockToBurn());
    ...
    /// @notice Update the % of BNOU distributions for burn, regular pools and special pools.
    /// @param _burnRate The % of BNOU to burn each block.
    /// @param _regularFarmRate The % of BNOU to regular pools each block.
    /// @param _specialFarmRate The % of BNOU to special pools each block.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function updateBnouRate(
        ...
        require(
            _burnRate.add(_regularFarmRate).add(_specialFarmRate) == BNOU_RATE_TOTAL_PRECISION,
        ...
        emit UpdateBnouRate(_burnRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    function updateDevAddress(address _devAddr) external onlyOwner{
    ...
    function updateBoostMultiplier(
        ...
        user.rewardDebt = user.amount.mul(_newMultiplier).div(BOOST_PRECISION).mul(pool.accBnouPerShare).div(
            ACC_BNOU_PRECISION
        );
    ...
    /// @notice Settles, distribute the pending BNOU rewards for given user.
    ...
    function settlePendingBnou(
        ...
        uint256 accBnou = boostedAmount.mul(poolInfo[_pid].accBnouPerShare).div(ACC_BNOU_PRECISION);
