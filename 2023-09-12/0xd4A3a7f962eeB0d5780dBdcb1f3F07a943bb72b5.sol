...
interface ICollectionNftWhitelist is IERC721Enumerable {
...
contract NftWhitelist is
    Ownable,
    ReentrancyGuard,
    ERC721Enumerable,
    ICollectionNftWhitelist
{
    ...
    /*
     * tokenId: nftId mint,
     * to: nft receiving address
     */
    event Mint(uint256 indexed tokenId, address indexed to);

    /*
     * tokenIds: list nftId mint,
     * to: nft receiving address
     */
    event MintBatch(uint256[] tokenIds, address indexed to);

    constructor(string memory _baseUrl, string memory _name, string memory _symbol)
    ...
    function mint(address _to) external override returns (uint256) {
        require(whitelist[_to], "You are not on the whitelist");
        require(!userMinted[_to], "You have already minted this NFT");
        require(totalClaimed < totalAllocation, "The number of NFTs has reached its limit");
    ...
    function updateWhitelist(address[] memory _walletAddress, bool _isAdd)
        external
        nonReentrant
        onlyOwner
    {
        for (uint256 i = 0; i < _walletAddress.length; i++) {
            whitelist[_walletAddress[i]] = _isAdd;
        }
    }

    function checkUserWhitelist(address _walletAddress) public view returns (bool) {
        if (whitelist[_walletAddress]) {
            return true;
        }
        return false;
    }
