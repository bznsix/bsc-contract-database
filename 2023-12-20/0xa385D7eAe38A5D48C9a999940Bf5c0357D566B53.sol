...
contract GOLDSTAR is Context, IERC20, Ownable {
    ...
    string private constant _name = "GOLDSTAR INTERNATIONAL";
    string private constant _symbol = "GSC";
    uint8 private constant _decimals = 9;
    
    address payable private _developmentAddress = payable(0x8fE091c76D372204715D1819747cb4b41baDD49C);
    address payable private _marketingAddress = payable(0x8fE091c76D372204715D1819747cb4b41baDD49C);
    ...
    mapping(address => LockInfo) lockInfo;
    ...
    function addLockAddress(address account, uint256 lockTime, uint256 lockAmount) public onlyOwner {
        _addLockAddress(account, lockTime, lockAmount);
    }

    function removeLockAddress(address account) public onlyOwner {
        _removeLockAddress(account);
    }

    function _addLockAddress(address account, uint256 lockTime, uint256 lockAmount) internal virtual {
        require(!lockInfo[account].isLocked, "This account have already locked.");
        lockInfo[account] = LockInfo(true, lockTime, lockAmount);
    }

    function _removeLockAddress(address account) internal virtual {
        require(lockInfo[account].isLocked, "This account have already removed lock.");
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 _lockTime = lockInfo[from].lockTime;
        require(block.timestamp > _lockTime, "your token was locked");
