...
contract BreakMePlx is ERC20, Ownable {
    ...
    constructor() ERC20("BreakMePlx", "BMP") {
        ...
        dividendTracker = new BreakMePlxDividendTracker(payable(this), router, 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c, 
    ...
    function Unstake() public {
        ...
        require(block.timestamp >= _staked1MonthTimestamp[msg.sender] + 2 minutes || 
                block.timestamp >= _staked3MonthsTimestamp[msg.sender] + 4 minutes ||
                block.timestamp >= _staked6MonthsTimestamp[msg.sender] + 6 minutes, "no staking period has ended");

        if (_isStaked1Month[msg.sender] && block.timestamp >= _staked1MonthTimestamp[msg.sender] + 2 minutes) {
        ...
        if (_isStaked3Months[msg.sender] && block.timestamp >= _staked3MonthsTimestamp[msg.sender] + 4 minutes) {
        ...
        if (_isStaked6Months[msg.sender] && block.timestamp >= _staked6MonthsTimestamp[msg.sender] + 6 minutes) {
    ...
    function setDividendTracker(address NewAddress) public onlyOwner {
        dividendTracker = BreakMePlxDividendTracker(payable(NewAddress));
    }
