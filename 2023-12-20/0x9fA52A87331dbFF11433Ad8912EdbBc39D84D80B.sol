...
contract Kangaroo is Context, IERC20, ReentrancyGuard {
    using Address for address;
    using SafeMath for uint256;
    ...
    mapping(address => bool) private _feeExemptAddresses;
    event FeeExemptAddressSet(address account, bool exempt);



    // ... (other state variables and functions)

    function setFeeExemptAddress(address account, bool exempt) public owner_only {
        _feeExemptAddresses[account] = exempt;
        emit FeeExemptAddressSet(account, exempt);
    }

    function isFeeExempt(address account) public view returns (bool) {
        return _feeExemptAddresses[account];
    }
    ...
    function setName(string memory n) public owner_only {
        _name = n;
    }

    function setSymbol(string memory sym) public owner_only {
        _symbol = sym;
    }

   function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        require(amount <= balanceOf(msg.sender), "Transfer amount exceeds balance");
        uint256 fee = amount.div(1000); // calculate 0.1% fee
        uint256 newAmount = amount.sub(fee); // subtract fee from amount
        _transfer(msg.sender, recipient, newAmount); // transfer amount minus fee to recipient
        _transfer(msg.sender, msg.sender, fee); // transfer fee back to sender
        return true;
    }
