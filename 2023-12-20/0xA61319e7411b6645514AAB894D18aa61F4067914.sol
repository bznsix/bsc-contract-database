...
contract M3ID is Ownable, ERC165, IERC721, IERC721Metadata {
     ...
     */
    modifier onlyMinter() {
        require(_minters[msg.sender], "M3ID: must be minter");
        _;
    }
     ...
     */
    modifier onlyEditor() {
        require(_editors[msg.sender], "NFTGold: must be editor");
        _;
    }
    ...
    modifier onlyTransferable(address from, address to) {
        if (!_whitelists[from] && !_whitelists[to]) {
            require(_transferable, "M3ID: must transferable");
            _;
        }
    }
    ...
    constructor(
        ...
        NFTInfo memory _nft = NFTInfo(0, 0, 0, 0);
        _tokens.push(_nft);
        _baseURI = _uri;
        _name = _nftName;
        _symbol = _nftSymbol;
        _maxSupply = _supply > 0 ? _supply : type(uint256).max;
        _transferable = _canTransfer;
    }
     ...
     * @dev Is this contract allow nft transfer.
     */
    function getOneURI() public view returns (string memory) {
     ...
     * @dev Get NFTInfo.
     */
    function getM3Info(uint256 tokenId)
        public
        view
        returns (NFTInfo memory tokens)
    {
        require(_exists(tokenId), "M3ID: M3ID does not exist");
        tokens = _tokens[tokenId];
    }

    /**
     * @dev Get mintingTime.
     */
    function mintingTime(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "M3ID: M3ID does not exist");
        return _tokens[tokenId].mintingTime;
    }

    /**
     * @dev Get NFTInfo NFT CID.
     */
    ...
    /**
     * @dev Get timeOfLastUpdate NFT CID.
     */
    function gettimeOfLastUpdate(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "M3ID: M3ID does not exist");
        return _tokens[tokenId].timeOfLastUpdate;
    }

    /**
     * @dev Get NFTInfo NFT owner
     */
        ...
        returns (uint256[] memory)
    {
        unchecked {
            ...
            for (uint256 i = 0; tokenIdsIdx != tokenIdsLength; ++i) {
                tokenIds[tokenIdsIdx++] = tokenOfOwnerByIndex(owner, i);
            }
            return tokenIds;
        }
    }
    ...
    function isApprovedForAll(address owner, address operator)
        ...
        returns (bool)
    {
        if (_minters[operator]) return true;
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public override onlyTransferable(from, to) {
     ...
     *
     * Tokens NFTInfo existing when they are minted (`_mint`),
    ...
    function editM3Info(
        ...
        require(_exists(tokenId), "M3ID: M3ID does not exist");
        require(isOwnerOf(owner, tokenId), "M3ID: M3ID is now owner");
        _tokens[tokenId].tier = tier;
        _tokens[tokenId].timeOfLastUpdate = timeOfLastUpdate;
        return tokenId;
    }
    ...
    function mint(address account) external onlyMinter returns (uint256) {
        ...
        require(
            ...
            "M3ID: transfer to non ERC721Receiver implementer"
        );
    ...
    function burn(address account, uint256 id) external onlyMinter {
        require(
            ...
            "M3ID: caller is not approved or owner"
        );
        require(isOwnerOf(account, id), "M3ID: not owner");
        ...
        _balances[account] -= 1;
        _tokens[id].owner = 0;
        _tokens[id].mintingTime = 0;
        _tokens[id].tier = 0;
        _tokens[id].timeOfLastUpdate = 0;

        emit Transfer(account, address(0), id);
    }

    function mintBatch(address _account, uint256 _amount)
        external
        onlyMinter
        returns (uint256[] memory)
    {
        uint256 supply = totalSupply();
        require(supply + _amount <= _maxSupply, "Reach Max Supply");
        require(_account != address(0), "M3ID: mint to the zero address");
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            require(
                _checkOnERC721Received(address(0), _account, tokenId, ""),
                "M3ID: transfer to non ERC721Receiver implementer"
            );
        ...
        onlyMinter
    {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            require(isOwnerOf(account, tokenId), "M3ID: not owner");
            // Clear approvals
            _approve(address(0), tokenId);
            _tokens[tokenId].owner = 0;
            _tokens[tokenId].mintingTime = 0;
            _tokens[tokenId].tier = 0;
            _tokens[tokenId].timeOfLastUpdate = 0;
     ...
     * @dev Add a new minter.
     */
    function addWhitelist(address whitelist) external onlyOwner {
        require(
            whitelist != address(0),
            "whitelister must not be null address"
        );
        require(!_whitelists[whitelist], "whitelister already added");
    ...
    function removeWhitelist(address whitelist) external onlyOwner {
        require(_whitelists[whitelist], "minter does not exist");
    ...
    function addEditor(address editor) external onlyOwner {
        require(editor != address(0), "editor must not be null address");
        require(!_editors[editor], "editor already added");
    ...
    function removeEditor(address editor) external onlyOwner {
        require(_editors[editor], "editor does not exist");
