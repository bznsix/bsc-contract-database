...
contract Market is IERC721Receiver,Ownable {
    ...
    constructor(IERC721 _mercenary) {
        mercenary = _mercenary;
    }

    function configsTax(uint256 _tax)  public onlyOwner {
        tax = _tax;
    }
    ...
    function getStakedAllNfts() view public returns (StakeDetail [] memory) {
        uint256 length = getStakingAmount();
        StakeDetail[] memory myNft = new StakeDetail[](length);
        uint count = 0;
    ...
    function buyNft(uint256 _tokenId) external payable {
        ...
        payable(stakeDetail[_tokenId].author).transfer(_price * (100 - tax)/100);
