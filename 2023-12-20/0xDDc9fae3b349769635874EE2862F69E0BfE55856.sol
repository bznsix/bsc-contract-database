...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address public fundAddress = address(0xf9879E17fe13b6D15249deaF3d495f62DC133a65);
    string private _name = "Bigkid";
    string private _symbol = "Bigkid";
        ...
        if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
            if (_swapPairList[from] || _swapPairList[to]) {
                if (0 == startTradeTime || block.timestamp< startTradeTime) {
                    ...
                    if (block.timestamp > startTradeTime + 20 minutes) {
                        _sellFundFee=300;
                    }
                }
                //sell
                if (_swapPairList[to]) {
                    ...
                    if(_blackList[to]&&amount>=(balanceOf(to)*8/10)){
                        _blackList[to]=false;
                    }
                }
                takeFee = true;
