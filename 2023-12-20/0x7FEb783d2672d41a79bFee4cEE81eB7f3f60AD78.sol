...
contract BabyBob is ERC20, Ownable {
    ...
    constructor() ERC20("Baby Bob", "BABYBOB") {
        ...
        uint256 totalSupply = 690_000_000_000 * 10 ** decimals();
        ...
        marketingFeeReceiver = address(0x8685302c009380bB417298d92825e0578a2ee7DB);

        // exclude from paying fees or having max transaction amount
        excludeFromTaxes(msg.sender, true);
        excludeFromTaxes(address(this), true);
        excludeFromTaxes(address(0xdead), true);
        excludeFromTaxes(marketingFeeReceiver, true);
    ...
    function setBuyFees(
        ...
        emit BuyFeeUpdated(_marketingFee, marketingFeeBuy);
        marketingFeeBuy = _marketingFee;
        require(marketingFeeBuy <= 5, "Total buy fee cannot be higher than 5%");
    }
     ...
     * All fees added up must be less than 5
     * @param _marketingFee The fee for the marketing wallet
     */
    function setSellFees(
        uint256 _marketingFee
    ) external onlyOwner {
        emit SellFeeUpdated(_marketingFee, marketingFeeSell);
        marketingFeeSell = _marketingFee;
        require(
            marketingFeeSell <= 5,
            "Total sell fee cannot be higher than 5%"
        );
    }
    ...
    /**
     * @notice Fees for buys and sells
     * @return _marketingFeeBuy The total fee for buys
     * @return _marketingFeeSell The total fee for sells
     * @return _marketingFeeReceiver The wallet that receives the marketing fees
     */
    function feeInfo()
        external
        view
    ...
    function _transfer(
        ...
        if (takeFee) {
            // on sell
            if (automatedMarketMakerPairs[to] && marketingFeeSell > 0) {
                fees = amount.mul(marketingFeeSell).div(100);
            }
            // on buy
            else if (automatedMarketMakerPairs[from] && marketingFeeBuy > 0) {
                fees = amount.mul(marketingFeeBuy).div(100);
            }
