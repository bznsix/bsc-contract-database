...
contract CDN is ERC20 {
	...
	uint256 total;
     ...
     constructor(address tokenOwner) ERC20("CDN", "CDN") {
        require(_baseToken < address(this),"CDN small");
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
            if(_isPairs[from]){//buy
                ...
                super._transfer(from, _destroyAddress, feeAmount);
                amount = amount.sub(feeAmount);
            }else if(_isPairs[to]){//sell
                uint256 feeAmount = amount.mul(3).div(100);// 3%
                super._transfer(from, _destroyAddress, feeAmount);
                amount = amount.sub(feeAmount);
			}
        }
    ...
    function _isAddLiquidityV2(address mssender) internal view returns(bool ldxAdd, uint256 otherAmount){
        if(mssender != _uniswapRouterV2Address){return (false, 0);}
