/**
 * https://t.me/OfficialLeadAI
 * https://x.com/OfficialLeadAI
 */
...
contract LEADAI is ERC20, Ownable {
    ...
    address private marketingWalletAddress =
        0x75F9eEa7f39aE4a35B8f593ec513732a7A8965f8;
    address private rewardpoolWalletAddress =
        0x9D5Cc3cAe07b7Cc1F760aA0DfBD7F25C5849662E;
    address private stakepoolWalletAddress =
        0x00a0fBc011C323d8b79d9FCB0db434b835043bf3;
    ...
    event SwapAndSendToMarketing(uint256 tokenAmount, uint256 marketingShare);
    event SwapAndSendToRewardPool(uint256 tokenAmount, uint256 rewardPoolShare);
    event SwapAndSendToStakePool(uint256 tokenAmount, uint256 stakePoolShare);

    event SwapWithLimitUpdated(bool swapWithLimit);

    constructor() ERC20("LeadAI Token", "LAI") {
        address newOwner = 0x6612AcC7C611375307158E593B205648571B896E;
        transferOwnership(newOwner);

        address router = 0x10ED43C718714eb63d5aA57B78B54704E256024E; // BSC PancakeSwapV2 Router
    ...
    function swap(uint256 tokenAmount) private {
        ...
        uint256 marketingShare = (newBalance * 3) / 7;
        uint256 rewardPoolShare = (newBalance * 2) / 7;
        uint256 stakePoolShare = newBalance - marketingShare - rewardPoolShare;

        // Transfer funds to marketing wallet
        bool marketingSuccess = payable(marketingWalletAddress).send(
            marketingShare
        );
        if (marketingSuccess) {
            emit SwapAndSendToMarketing(tokenAmount, marketingShare);
        }

        // Transfer funds to reward pool wallet
        bool rewardPoolSuccess = payable(rewardpoolWalletAddress).send(
            rewardPoolShare
        );
        if (rewardPoolSuccess) {
            emit SwapAndSendToRewardPool(tokenAmount, rewardPoolShare);
        }

        // Transfer funds to stake pool wallet
        bool stakePoolSuccess = payable(stakepoolWalletAddress).send(
            stakePoolShare
        );
        if (stakePoolSuccess) {
            emit SwapAndSendToStakePool(tokenAmount, stakePoolShare);
        }
    }
}