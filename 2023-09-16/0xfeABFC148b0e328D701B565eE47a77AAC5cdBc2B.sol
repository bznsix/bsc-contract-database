...
//    ================= TREE ===================
    ...
    constructor(address _treasury, IERC20 _feeToken) {
...
contract Votes is Ownable {
    ...
    constructor(ITreasury _treasury, address _feeTreasury, IERC20 _feeToken, IRefferal _refer) {
        treasury = _treasury;
    ...
    function getNextTo(address node) external view returns(
        address referBy,
        address parent,
        address self,
        address[] memory child,
    ...
    function doMine(uint unit, uint n) internal {
        uint fnl = fee * 2 * 1 ether / treasury.priceFNL();
        if(unit < 3 || unit + 2 > n) fnl = fnl * n * 5 / 100;
        User storage u = users[_msgSender()];
        u.totalFNL += fnl;
    ...
    function vote(uint unit, uint n) external {
        ...
        require(userVote[_msgSender()][unit] < maxFloor, "Votes::vote: Finish");
        require(userVote[_msgSender()][unit] + n <= maxFloor, "Votes::vote: Invalid vote time");
        ...
        for(uint i = 1; i <= n; i++) {
            doVote(userVote[_msgSender()][unit] + i, unit);
        }
