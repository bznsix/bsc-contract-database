...
// Part: IPantherMasterChef

interface IPantherMasterChef {
...
// Part: OpenZeppelin/openzeppelin-contracts@3.4.0/Pausable
...
// File: PantherStrategy.sol

contract PantherStrategy is Ownable, ReentrancyGuard, Pausable {
    using SafeMath for uint256;
    ...
    IPANTHER20 public panther = IPANTHER20(0x1f546aD641B56b86fD9dCEAc473d1C7a357276B7);
    ...
    uint256 public constant basisPoint = 10_000;
    ...
    constructor(
        ...
        IBEP20(earnedAddress).approve(routerAddress, uint256(-1));
        IBEP20(token0Address).approve(routerAddress, uint256(-1));
        IBEP20(token1Address).approve(routerAddress, uint256(-1));

        IBEP20(wantAddress).approve(masterChefAddress, uint256(-1));
    ...
    function deposit(address _userAddress, uint256 _wantAmt) public onlyOwner whenNotPaused returns (uint256) {
        ...
        if (wantLockedTotal > 0) {
            sharesAdded = _wantAmt.mul(sharesTotal).mul(entranceFeeFactor).div(wantLockedTotal).div(basisPoint);
        }
        ...
        if (IPantherMasterChef(masterChefAddress).canHarvest(pid, address(this))) {
            earn();
        }
        return sharesAdded;
    }
    ...
    function _farm() internal {
        ...
        IPantherMasterChef(masterChefAddress).deposit(pid, wantAmt, feeAddress);
    }

    function withdraw(address _userAddress, uint256 _wantAmt) public onlyOwner nonReentrant returns (uint256) {
        require(_wantAmt > 0, "_wantAmt <= 0");

        IPantherMasterChef(masterChefAddress).withdraw(pid, _wantAmt);
    ...
    function earn() public whenNotPaused {
        ...
        IPantherMasterChef(masterChefAddress).withdraw(pid, 0);

        // Converts farm tokens into want tokens
        uint256 earnedAmt = IBEP20(earnedAddress).balanceOf(address(this));
        distributeFees(earnedAmt);
        earnedAmt = IBEP20(earnedAddress).balanceOf(address(this));
        ...
        if (earnedAddress != token0Address && earnedAmt > 0) {
        ...
        if (earnedAddress != token1Address && earnedAmt > 0) {
    ...
    function distributeFees(uint256 _earnedAmt) internal returns (uint256) {
        if (_earnedAmt > 0) {
            // Performance fee
            if (controllerFee > 0) {
                uint256 fee = _earnedAmt.mul(controllerFee).div(basisPoint);
    ...
    function setEntranceFeeFactor(uint256 _entranceFeeFactor) external {
        ...
        require(_entranceFeeFactor <= basisPoint, "!safe - too high");
