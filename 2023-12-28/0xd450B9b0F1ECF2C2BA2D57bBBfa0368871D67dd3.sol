...
contract psykoLottery is ReentrancyGuard{
    ...
    address private collabPsyko = 0x4630F0ddA44e3C1aF4d6706b545BdFA73e9aAfEd;
    address private dev = 0x2CE95071Bc3139f92F73D6a4d3b92a672F277BCB;
...
uint256 private maxParticipantNumbers1;
    ...
    IERC20 public PsykoInterface;



    constructor()  {  
        owner =  msg.sender;
        maxParticipantNumbers = 16;
        ticketPrice = 4000 ether;

         maxParticipantNumbers1 = 8;
        ticketPrice1 = 8000 ether;

         maxParticipantNumbers2 = 4;
        ticketPrice2 = 16000 ether;

        tokenAdress = 0xaaD978c2155FAccF64b93A8D2B8A5D8f2317D308; 
        PsykoInterface = IERC20(tokenAdress);
    }
...
event logshh(uint256 _id, uint256 _value);
    ...
    function lotteryBalance() public view returns(uint256) {
        return PsykoInterface.balanceOf(address(this));
    }
...
function setMaximmNumbers1(uint256 _maxNumbers) public onlyOwner{
   ...
   function announceLottery() public onlyOwner(){
    ...
    function joinLottery(uint256 _amount) external notOwner() noReentrant{
        ...
        //PsykoInterface.transferFrom(msg.sender,address(this),_amount);
          bool chk= PsykoInterface.transferFrom(msg.sender,address(this),_amount);
       if(chk){
...
for (uint i=0; i<arrayLength2; i++) {
  ...
  if (participants[i]==ad){
    ...
    function pickwinner() internal {
        ...
        uint256 cllbPsykoFee = SafeMath.div(SafeMath.mul(contractBalance,3),100);
        uint256 devFee = SafeMath.div(SafeMath.mul(contractBalance,87),100);
      
        PsykoInterface.transfer(collabPsyko,cllbPsykoFee);
        uint256 winnerAmount = SafeMath.sub(contractBalance,cllbPsykoFee);
        winnerAmount -= devFee;
  
       PsykoInterface.transfer(dev,devFee);
        PsykoInterface.transfer(participants[win],winnerAmount);
    ...
    function pickwinner1() internal {
     ...
     PsykoInterface.transfer(participants1[win],winnerAmount);
    ...
    function pickwinner2() internal {
      ...
      PsykoInterface.transfer(participants2[win],winnerAmount);
