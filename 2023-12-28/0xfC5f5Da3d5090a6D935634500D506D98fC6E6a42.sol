...
contract wassiebscDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("wassiebsc_Dividend_Tracker", "wassiebsc_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18);
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "wassiebsc_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "wassiebsc_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main wassiebsc contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "wassiebsc_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"); 
        require(newClaimWait != claimWait, "wassiebsc_Dividend_Tracker: Cannot update claimWait to same value");
...
contract wassiebsc is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    wassiebscDividendTracker public dividendTracker;
    ...
    constructor() ERC20("wassiebsc", "wassiebsc") 
    {
        BNBRewardsFee = 2;
        liquidityFee = 1;
        marketingFee = 5;
        ...
        marketingWallet = payable(0xcB0634F904eFada33B736AD530ed3772f0e7D08f);
        devWallet = payable(0x6d1FC2d002098Eb58Ca8BD9c88717B43Dc39d230);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(devFee); // total fee transfer and buy

        dividendTracker = new wassiebscDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "wassiebsc: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "wassiebsc: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "wassiebsc: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "wassiebsc: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "wassiebsc: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "wassiebsc: Cannot update gasForProcessing to same value");
