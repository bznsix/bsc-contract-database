...
// File contracts/MutliSender.sol
...
contract MultiSender is Ownable {
    ...
    constructor(uint256 _fee, uint256 _arrayLimit) {
        fee = _fee;
...
checkArrayLength(_contributors.length, _balances.length)
    ...
    checkArrayLength(_contributors.length, _balances.length)
    {
        ...
        for (i; i < _contributors.length; i++) {
            require(total >= _balances[i], "Your balance is low");
            total = total - _balances[i];
    ...
    function withdrawCurrencyAll() external onlyOwner {
