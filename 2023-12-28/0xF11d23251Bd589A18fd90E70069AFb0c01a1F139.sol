...
contract PEACE is ERC20, Ownable {
    ...
    PEACEDividendTracker public dividendTracker;
    ...
    constructor() ERC20(unicode"✌️", "PEACE") {
        marketingWallet = payable(0x48403b126b3cb455d83E1188aE9cde6b8aE1240F);
        devWallet = payable(0xe999a61759390082F2E7E59d60529BD49B5c0346);
        ...
        dividendTracker = new PEACEDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (2_000_000) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 60000; // 3%
        swapTokensAtAmount = totalTokenSupply / 25000; //
...
contract PEACEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        PEACEContract = PEACE(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Peace_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Peace_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Peace contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PEACEContract);

            uint256 prevBalance = PEACEContract.balanceOf(address(this));
            ...
            {
                uint256 received = PEACEContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PEACEContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
