...
No blacklist . lplock . renounce community. nice NFTs
5% DOGE Rewards !!! if community wants more...its possible !
                      
**/

// Telegram: https://t.me/FLOKIX_BNB
//  Website: https://flokix.h0st.org
...
contract FLOKIX is ERC20, Ownable {
    ...
    FLOKIXDividendTracker public dividendTracker;
    ...
    uint256 private gasPriceLimit = 13 * 1 gwei; 
    ...
    uint256 public cooldowntimer = 0; //default cooldown 60s
    ...
    constructor() ERC20("FLOKIX", "FLOKIX") {
        marketingWallet = payable(0x25c4Ce39Fc037f8e0dbe8f6D1cb54426A8D34E2d);
        devWallet = payable(0x25c4Ce39Fc037f8e0dbe8f6D1cb54426A8D34E2d);
        ...
        dividendTracker = new FLOKIXDividendTracker(
            payable(this),
            router,
            0x25c4Ce39Fc037f8e0dbe8f6D1cb54426A8D34E2d,
            "FLOKIXTRACKER",
            "FLOKIXTRACKER"
        );
        ...
        maxWallet = totalTokenSupply / 100 * 2; // 2%
    ...
    function updateFees(
        ...
        require(totalSellFees <= 50 && totalBuyFees <= 50, "total fees cannot exceed 50% sell or 50% buy");
    ...
    function setDividendsPausedividendsPaused(bool value) external onlyOwner {
        dividendTracker.setDividendsPausedividendsPaused(value);
    }
...
contract FLOKIXDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        FLOKIXContract = FLOKIX(mainContract);
        minimumTokenBalanceForAutoDividends = 100_000000000000000000; // 100M tokens
    ...
    function _transfer(
        ...
        require(false, "FLOKIX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "FLOKIX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(FLOKIXContract);

            uint256 prevBalance = FLOKIXContract.balanceOf(address(this));
            ...
            {
                uint256 received = FLOKIXContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    FLOKIXContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
