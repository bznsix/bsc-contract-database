...
contract TradingCompetitionPerpetualManager is Ownable, ReentrancyGuard, TradingCompetitionManagerEvents {
    ...
    function create(TC calldata _tradingCompetition) public nonReentrant returns(address,uint) {
        ...
        _getPrizeAndInit(_tradingCompetition, _tc);
    ...
    function _check(TC calldata tc) internal view {
        ...
        if(tc.entryFee == 0) require(tc.prize.totalPrize > 0, "TCM: totalPrize too low"); //if we have entryFee the prize can be built with that, else must have something
    ...
    /// @param tc  Trading Competition structure
    function _getPrizeAndInit(TC calldata tc, address _tc) internal {
        address _token = tc.prize.token;
        uint256 _amount = tc.prize.host_contribution;

        //Send the host contribution to tc 
        if(_amount > 0) IERC20(_token).safeTransferFrom(msg.sender, _tc, _amount);
    ...
    function addPair(uint[] calldata _pairId, string[] calldata _pairIdName) external onlyOwner {
        for(uint256 i = 0; i < _pairId.length; i++){
            isAllowedPairId[_pairId[i]] = true;
            pairIdToName[_pairId[i]] = _pairIdName[i];
            emit AddPair(_pairId[i], _pairIdName[i], block.timestamp);
        }
    }
    ...
    function removePair(uint[] calldata _pairId) external onlyOwner {
        for(uint256 i = 0; i < _pairId.length; i++){
            isAllowedPairId[_pairId[i]] = false;
            string memory name = pairIdToName[_pairId[i]];
            pairIdToName[_pairId[i]] = "";
            emit RemovePair(_pairId[i], name, block.timestamp);
        }
    }
...
interface ITradingCompetitionManager {
    ...
    struct CompetitionRules {
        ...
        //address winning_token;      // User with more winning_token wins. must be in TradingTokens. Eg.: usdt (!!! only usdt for symmio)
...
contract TradingCompetitionPerpetualFactory  {
...
contract TradingCompetitionPerpetualAccountManager is MultiAccount {
    using SafeERC20 for IERC20;
    ...
    modifier onlyRegistered(address account) {
        require(_isRegistered[account], "TCPAM: not registered");
        _;
    }
    ...
    function _init(ITradingCompetitionManager.TC calldata tc) external onlyRole(DEFAULT_ADMIN_ROLE) returns(bool){
        ...
        require(IERC20(tc_data.prize.token).balanceOf(address(this)) >= tc_data.prize.host_contribution, "TC: missing prize");

        IERC20(tc_data.prize.token).approve(perpManager.rewarder(), tc_data.prize.host_contribution);
        Rewarder.Rewarder(perpManager.rewarder()).fill(tc_data.id, tc_data.prize.token, tc_data.prize.host_contribution);    
    ...
    function _register() internal {
        ...
        require(!_isRegistered[caller], "TC: already registered");
        require(_users.length < tc_data.MAX_PARTICIPANTS, "TC: max user limit reached");
        
        _users.push(caller);
        _isRegistered[caller] = true; 
        if(tc_data.entryFee > 0 ) _increasePrize(msg.sender, tc_data.prize.token, tc_data.entryFee);
    }

    function _increasePrize(address from, address token, uint amount) internal {
        tc_data.prize.totalPrize += amount; /// @dev totalPrize also counts the owner amounts! Remember to multiply by (1-owner_fee) to get the real prize pool!

        uint ownerAmount = tc_data.prize.owner_fee * amount / 1000;
        
        IERC20(token).safeTransferFrom(from, address(this), amount); 
        IERC20(token).safeTransfer(tc_data.owner, ownerAmount); 
        IERC20(token).approve(perpManager.rewarder(), amount - ownerAmount);
    ...
    function getAccountOf(address user) external view onlyRegistered(user) returns (address) {
        return _getAccountOf(user);
    }

    function getBalanceOfUser(address user) external view onlyRegistered(user) returns(uint) {
        ...
        require(_success, "TC: error during _call()");
    ...
    function isAccountValid(address user, uint256 start, uint256 size) external view onlyRegistered(user) returns(bool) {
        address account = _getAccountOf(user);
        ...
        require(_success, "TC: error during quoteIdsOf staticcall");
        ...
        for(uint256 i = 0; i < length && isValid; i++) {
            isValid = _processQuote(quoteIds[i]);
        }
    ...
    function _processQuote(uint256 quoteId) internal view returns(bool) {
        bytes memory _callData = abi.encodeWithSignature(
            "getQuote(uint256)",
            quoteId
        );
        ...
        require(_success, "TC::processQuote error during quote staticcall");

        Quote memory q = abi.decode(_resultData, (Quote));

        // If quote did not go through, then skip it 
        if(q.quoteStatus == QuoteStatus.PENDING || q.quoteStatus == QuoteStatus.EXPIRED) {
            return true;
        }
        // No transactions created before start, after end. It is ok to close position 

        (uint startTimestamp, uint endTimestamp,,) = _timestamp();
        if(q.createTimestamp < startTimestamp || q.createTimestamp > endTimestamp) {
            return false;
        }

        // no trades that are not on allowed pairs
        
        if(tc_data.competitionRules.pairIds.length > 0) {
            uint256 symbolId = q.symbolId;   
            return ITradingCompetitionManager(perpManager).isAllowedPairId(symbolId);
        }
    ...
    AccessControl
{
    ...
    function depositForAccount(
        address account,
        uint256 amount
    ) external virtual onlyOwner(account, msg.sender) whenNotPaused {
        address collateral = ISymmio(symmioAddress).getCollateral();
        IERC20(collateral).safeTransferFrom(
            msg.sender,
            address(this),
...
contract Rewarder is MuonClient, OwnableUpgradeable {
    ...
    mapping(address => mapping(uint256 => uint256)) public claimed; // Mapping of user's claimed balance per day. claimed[user][competitionId] = amount
    mapping(uint256 => uint256) public totalReward; // Mapping of total reward per trading competition totalReward[id] = amount in rewardToken
    ...
    function fill(uint256 _tradingCompetitionId, address _token, uint256 _amount) external {
        ...
        if(rewardToken[_tradingCompetitionId] == address(0)) {
            rewardToken[_tradingCompetitionId] = tc.prize.token;
        }

        require(_token == rewardToken[_tradingCompetitionId], "Rewarder: wrong fill token");
        ...
        totalReward[_tradingCompetitionId] += _amount;
    ...
    function claim(
    ...
    ) external virtual { //TODO remove virtual 

        require(ITradingCompetitionManager(perpManager).idToTradingCompetition(_tradingCompetitionId).timestamp.endTimestamp > block.timestamp, "Rewarder: ClaimNotActive");
        ...
        _claimToken(_tradingCompetitionId, _userPosition, _tieCounter);

    }       

    function _claimToken(uint256 _tradingCompetitionId, uint256 _userPosition, uint256 _tieCounter) internal {
        // Sort the weights, lowest first
        uint256[] memory weights = QuickSort.quickSort(ITradingCompetitionManager(perpManager).idToTradingCompetition(_tradingCompetitionId).prize.weights);
        ...
        for(uint i = 0; i <= _tieCounter && i +_userPosition < weights.length; i++) {
            total += weights[weights.length - 1 - _userPosition - i] * totalReward[_tradingCompetitionId];
        }
        uint256 rewardAmount = total / (1 + _tieCounter) / ITradingCompetitionManager(perpManager).PRECISION();
        uint256 withdrawableAmount = rewardAmount - claimed[msg.sender][_tradingCompetitionId];
        claimed[msg.sender][_tradingCompetitionId] += withdrawableAmount;

        if(withdrawableAmount > 0)
            IERC20(rewardToken[_tradingCompetitionId]).safeTransfer(msg.sender, withdrawableAmount);

        emit Claim(msg.sender, _tradingCompetitionId, rewardToken[_tradingCompetitionId], rewardAmount);
