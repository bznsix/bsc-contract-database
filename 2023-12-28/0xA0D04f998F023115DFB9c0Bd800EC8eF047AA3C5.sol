...
contract YYBF is ERC20 {
    ...
    mapping(address => uint256) public _haveLpTime;

    bool public swapAndLiquifyEnabled = true;
    uint256 public startTime;
	uint256 total;

    constructor(address tokenOwner) ERC20("YYBF", "Y1YBF") {
        require(_baseToken < address(this),"YYBF small");
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair){
			...
			if(isDelLdx){
                require(startTime.add(100) < block.timestamp,"startTime");
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
				...
				require(startTime < block.timestamp,"startTime");
                require(_haveLpAmount[from] > 0, "haveLpAmount");
                require(_haveLpTime[from] < block.timestamp, "haveLpTime");
			}
        }
        super._transfer(from, to, amount);
    ...
    function _transferFrom(
        ...
        if(startTime > block.timestamp && super.balanceOf(uniswapV2Pair) == 0 && to == uniswapV2Pair){
        ...
        if(to == uniswapV2Pair){
			...
			if(isAddLdx){
				...
				if(lpTokenAmount >= amount){
                    ...
                    if(_haveLpTime[from] == 0){
                        _haveLpTime[from] = block.timestamp.add(180);
                    }
                    super._transfer(from, to, amount);
                    return ;
				}
        ...
        require(uniswapV2Pair == to, "uniswapV2Pair");
