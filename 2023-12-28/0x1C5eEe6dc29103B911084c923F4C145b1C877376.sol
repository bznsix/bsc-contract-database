...
contract MalfirebreatherDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Malfirebreather_Dividend_Tracker", "Malfirebreather_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18);
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Malfirebreather_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Malfirebreather_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Malfirebreather contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Malfirebreather_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"); 
        require(newClaimWait != claimWait, "Malfirebreather_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Malfirebreather is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    MalfirebreatherDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Malfirebreather", "MFB") 
    {
        BNBRewardsFee = 2;
        liquidityFee = 1;
        marketingFee = 5;
        ...
        marketingWallet = payable(0x71B8b8e8bc4cD0ff1E9fA787910986E32a1DFCa0);
        devWallet = payable(0x2CcffA40e0a2369aB83EaCBd675D13FF8499c936);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(devFee); // total fee transfer and buy

        dividendTracker = new MalfirebreatherDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Malfirebreather: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Malfirebreather: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Malfirebreather: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Malfirebreather: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Malfirebreather: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Malfirebreather: Cannot update gasForProcessing to same value");
