...
contract FUCKYOUPAULY is Context, IERC20, Ownable { 
    ...
    address payable public Wallet_Market = payable(0x41571885443B5ac0860377F837514F5e5467955b);

    string public _name = "FUCK YOU PAULY";
    string public _symbol = "FUCKPAULY";
    uint8 private _decimals = 9;
    uint256 public _tTotal = 690000000 * 10 **_decimals;

    uint8 private txCount = 0;
    uint8 private swapTrigger = 2;
     
    uint256 private fee_total = 6;
    uint256 public buying_fee = 3;
    uint256 public selling_fee = 3;

    uint256 private _previousTotalFee = fee_total; 
    ...
    function disableTaxes() private {
        if(fee_total == 0 && buying_fee == 0 && selling_fee == 0) return;

        _previousBuyFee = buying_fee; 
        _previousSellTax = selling_fee; 
        _previousTotalFee = fee_total;
        buying_fee = 0;
        selling_fee = 0;
        fee_total = 0;
    ...
    function _transfer(
        ...
        if( _No_Fee_Wallet[from] || _No_Fee_Wallet[to] ){
            ...
            fee_total = buying_fee;
            } else if (to == uniswapV2Pair){
                fee_total = selling_fee;
                }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256) {
        uint256 tmarketing = tAmount*fee_total/100;
