...
// File contracts/sbt/ITabiNFT.sol
...
interface ITabiNFT {
    ...
    function tokenStruct(uint256 tokenId)
        external
        view
        returns (TokenStruct memory);
...
// File contracts/sbt/TabiProxy.sol
...
contract TabiProxy is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize() public initializer {
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                0x15de4c450696d3698613e8c5c9bf481f69fbd451f7847da3fe86a2d9699fa6f8, //keccak256("TabiProxy"),
                0xc89efdaa54c0f20c7adf612882df0950f5a951637e0307cdcb4c672f298b8bc6, //keccak256("1"),
                block.chainid,
                address(this)
            )
        );
        signatureAddress = 0xf33214055f9C54E257D1b4973f1E8Bf57C03f393;
    }
    ...
    function updateSignature(address _signatureAddress) public onlyOwner {
        signatureAddress = _signatureAddress;
    }

    function hashStruct(MintParam memory mintParam)
        public
        pure
        returns (bytes32)
    {
        return
            keccak256(
                abi.encode(
                    0x68d8512c77ec1c01d0d80a31da7f39ff6046f81ec55b8f27654ab14120e36ef4, //keccak256("MintParam(address nftAddress,address account,uint256 id,uint256 level,uint256 amount)"),
    ...
    function _validateMintParam(MintParam calldata mintParam) internal view {
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
                DOMAIN_SEPARATOR,
                hashStruct(mintParam)
            )
        );

        require(
            digest.recover(mintParam.signature) == signatureAddress,
            "Signature: Invalid"
        );
    }

    function mint(MintParam calldata mintParam) external payable {
        _validateMintParam(mintParam);
        ITabiNFT nft = ITabiNFT(mintParam.nftAddress);
        nft.proxyMint(
            mintParam.account,
            mintParam.id,
            mintParam.level,
    ...
    function ownerOf(address nft, uint256 tokenId)
        ...
        returns (address)
    {
        ITabiNFT nft2 = ITabiNFT(nft);
        return nft2.ownerOf(tokenId);
    }

    function exchange(MintParam calldata mintParam) public payable {
        // ITabiNFT nft = ITabiNFT(mintParam.nftAddress);
        // nft.proxyBurn(mintParam.account, mintParam.id, mintParam.amount);
    }
}