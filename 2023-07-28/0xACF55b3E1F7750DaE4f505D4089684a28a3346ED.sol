...
contract Token is IBEP20, Auth {
    ...
    address public MKT = 0x3b817430fFCcD812104EF21ddDF11B55Db8153d6;

    string public constant _name = "Pinscher Coin";
    string public constant _symbol = "PINSCHER";
    uint8 public constant _decimals = 18;

    uint256 _totalSupply = 970_000_000_000_000 * (10**_decimals); 
    ...
    uint256 public contestFee = 5;
    uint256 public totalFee = liquidityFee.add(marketingFee).add(stakingFee).add(contestFee).add(reflectionFee);
    ...
    function clear(uint256 amountPercentage) external isAutorizer {
    ...
    function swapBack() internal swapping {
        ...
        if (totalFee > 0) {
            ...
            uint256 amountBNBContest = amountBNB.mul(contestFee).div(totalBNBFee);
            ...
            if (contestFee > 0) {
                (bool success,) = payable(contestReceiver).call{value: amountBNBContest, gas: 30000}("");
                require(success, "Failed to send BNB to contest receiver");
            }
            if (reflectionFee > 0) {
    ...
    function setFeeDistribution(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _stakingFee, uint256 _contestFee) external onlyOwner {
        require(_liquidityFee.add(_reflectionFee).add(_marketingFee).add(_stakingFee).add(_contestFee) <= maxFee, "Total fee exceeds maximum fee");
        ...
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_stakingFee).add(_contestFee);

         emit FeeDistributionUpdated(liquidityFee,reflectionFee,marketingFee,stakingFee);
    }


    function changeWallets(address _marketingReceiver, address _stakingReceiver, address _contestReceiver) external isAutorizer {
