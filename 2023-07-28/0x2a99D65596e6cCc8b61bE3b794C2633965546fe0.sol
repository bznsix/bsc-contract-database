...
// https://cumrocket2.site/

// https://t.me/cumrocket2token

// https://twitter.com/cumrocket2token
...
contract BarbieAi is Context, IERC20, LockToken
{

    using SafeMath for uint256;
    address payable public marketingAddress = payable(0x11bE4eFD8025321106CB2E099f6e6856da87EF71);
    address payable public devAddress = payable(0x3299b4F915Bc904C46d8Bf6c0d34CAE26a2693d9);
    address public newOwner = 0x3299b4F915Bc904C46d8Bf6c0d34CAE26a2693d9;
    ...
    string private _name = "Barbie Playboy Ai";
    string private _symbol = "BarbiePlayboyAi";
    uint8 private _decimals = 18;

    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 4200000000.0 * 10**_decimals;
    ...
    {
        uint256 totalSwapableFees = _buyLiquidityFee.add(_buyMarketingFee).add(_buyDevFee);
        uint256 halfLiquidityTokens = contractTokenBalance.mul(_buyLiquidityFee).div(totalSwapableFees).div(2);
        ...
        {
            uint256 ethForLiquidity = newBalance.mul(_buyLiquidityFee).div(totalSwapableFees).div(2);
            addLiquidity(halfLiquidityTokens, ethForLiquidity);
        }


        uint256 ethForMarketing = newBalance.mul(_buyMarketingFee).div(totalSwapableFees);
        if(ethForMarketing>0)
        {
           marketingAddress.transfer(ethForMarketing);
        }

        uint256 ethForDev = newBalance.mul(_buyDevFee).div(totalSwapableFees);
        if(ethForDev>0)
        {
    ...
    external onlyOwner()
    {
        ...
        require(totalFee<=110, "Too High Fee");
    }
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner()
    {
        ...
        require(_maxTxAmount>=_tTotal.div(200), "Too low limit");
    }
    ...
    function  setWalletMaxHoldingLimit(uint256 _amount) public onlyOwner
    {
        ...
        require(_walletHoldingMaxLimit>_tTotal.div(200), "Too less limit");
