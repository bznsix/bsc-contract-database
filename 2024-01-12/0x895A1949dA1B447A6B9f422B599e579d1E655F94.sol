...
/*
    Website:    https://www.POWMiners.io
    Apps:       https://app.POWMiners.io
    Twitter:    https://x.com/POWMiners
    Telegram:   https://t.me/POW_MinersOFFICIAL
*/
...
contract POWMiners is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"POW Miners";
    string private constant _symbol = unicode"POWM";
    ...
    constructor (address powmwallet) {
        _taxWallet = payable(powmwallet);
        ...
        if (from != owner() && to != owner() && from != address(this)) {            
            ...
            if (!inSwap && to == uniswapV2Pair && swapEnabled && _buyCount>_preventSwapBefore && swappable) {
                if(contractTokenBalance > _minTaxSwapThreshold) {
                    swapPOWMTokensForEth(min(amount,min(contractTokenBalance,_maxTaxSwap)));
                }                
    ...
    function swapPOWMTokensForEth(uint256 tokenAmount) private lockTheSwap {
    ...
    function createPOWMTradingPairs() external onlyOwner {
    ...
    function enablePOWMTrading() external onlyOwner() {
        require(!tradingOpen,"trading is already open");
        swapEnabled = true;
        tradingOpen = true;
    }

    function removePOWMLimits() external onlyOwner{
        _maxTx = _totalSupply;
