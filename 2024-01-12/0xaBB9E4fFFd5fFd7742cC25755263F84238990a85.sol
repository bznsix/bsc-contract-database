...
interface INimbusPair is IERC20 {
    ...
    function usersLiquidities(address user, uint id) external view returns (ChangeLiquidity memory);
    function idCount(address user) external view returns (uint256 lastId);
...
contract StakingLPRewardFixedAPY is IStakingRewards, ReentrancyGuard, Ownable, Pausable {
    ...
    function earned(address account) public view override returns (uint256 totalEarned) {
        ...
        if (stakeNonceInfos[account][i].rewardsTokenAmount == 0) {
            continue;
        }
    ...
    function stake(uint256 id) external override nonReentrant {
        require(id > 0, "StakingLPRewardFixedAPY: Cannot stake 0");
        _stake(id, msg.sender,msg.sender);
    }
    

    function stakeFor(uint256 id, address user) external override nonReentrant {
        require(id > 0, "StakingLPRewardFixedAPY: Cannot stake 0");
        require(user != address(0), "StakingLPRewardFixedAPY: Cannot stake for zero address");
        _stake(id, user,msg.sender);
    }

    function _stake(uint256 id, address user,address liquidityProvider) private {

        INimbusPair.ChangeLiquidity memory liquidityDetails = INimbusPair(address(stakingLPToken)).usersLiquidities(liquidityProvider, id);
        uint256 liquidity = liquidityDetails.liquidity;

        INimbusPair(address(stakingLPToken)).transferFromWithId(msg.sender, address(this), id);

        uint[] memory lastId = stakingLPToken.getUserLiquidityIds(address(this));
        uint256 lastIdThis = lastId[lastId.length - 1];

        require(liquidity > 0, "StakingLPRewardFixedAPY: No liquidity found for the given id");

        uint amountRewardEquivalent = getCurrentLPPrice() * liquidity / 1e18;

        _totalSupply += liquidity;
        _totalSupplyRewardEquivalent += amountRewardEquivalent;
        _balances[user] += liquidity;

        uint stakeNonce = stakeNonces[user]++;
        stakeNonceInfos[user][stakeNonce].stakingLPTokenAmount = liquidity;
        ...
        stakeNonceInfos[user][stakeNonce].lastId = lastIdThis;
        _balancesRewardEquivalent[user] += amountRewardEquivalent;
        emit Staked(user, liquidity);
    }
    ...
    function withdraw(uint256 nonce) public override nonReentrant {
        ...
        uint256 id = stakeNonceInfos[msg.sender][nonce].lastId;

        stakeNonceInfos[msg.sender][nonce].stakingLPTokenAmount = 0;
        stakeNonceInfos[msg.sender][nonce].rewardsTokenAmount = 0;
        stakingLPToken.transferWithId(msg.sender,id);
        stakeNonceInfos[msg.sender][nonce].lastId = 0;
