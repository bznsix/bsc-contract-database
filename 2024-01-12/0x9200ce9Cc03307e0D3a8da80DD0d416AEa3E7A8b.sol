...
// BitMinerX: https://www.bitminerx.net
...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    address private ownerToChangeMin = address(0);
    ...
    constructor(address __owner, uint256 minBalance, address _rewardToken) DividendPayingToken("Reward Tracker", "DividendTracker", _rewardToken) {
        claimWait = 3600;
        ownerToChangeMin = __owner;
    ...
    function updateMinimumTokenBalanceForDividends(uint256 _newMinimumBalance) external {
        require(ownerToChangeMin == msg.sender,"Caller not the owner");
...
contract BitMinerX is ERC20, Ownable {
    ...
    uint256 private btcRewardsAllocation = 400;
    uint256 private marketingAllocation = 600;
    uint256 private denominator_allocation = 1000;
    address public lpPair;
    bool private isTradingEnabled = false;

    address public marketingAddress = address(0x1C8Faecf1b470E414F466a43999219161e06195B);
    ...
    constructor() payable ERC20("BitMinerX", "BMX") {
        _isExcludedFromFees[owner()] = true;
        _isExcludedFromFees[DEAD] = true;
        _isExcludedFromFees[address(this)] = true;

        rewardToken = 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c;

        dividendTracker = new DividendTracker(address(0x7Ef842fDb339189D616B837C9d23D6a1A1d24abC), 20_000, rewardToken);
    ...
    function _transfer(
            ...
            if(contractTokenBalance > 0) {
                ...
                try swapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
                    ...
                    block.timestamp) {}
                        ...
                        uint256 BTCRewards = address(this).balance * btcRewardsAllocation / denominator_allocation;
                        bool success;
                        if(mkt > 0) {(success,) = marketingAddress.call{value: mkt, gas: 35000}("");}
                        if(BTCRewards > 0) {swapAndSendDividends(BTCRewards);}
            }
        ...
        if(!swapping && btcRewardsAllocation > 0) {
    ...
    function changeAllocation(uint256 marketing, uint256 btcRewards) external onlyOwner {
        marketingAllocation = marketing;
        btcRewardsAllocation = btcRewards;
        require(marketingAllocation + btcRewardsAllocation == denominator_allocation,"Freddy wrong with contract");
    }
    ...
    function changeTaxes(uint256 buy, uint256 sell, uint256 transfer) external onlyOwner {
        require(buy <= 10,"Freddy: No more than 10%");
        require(sell <= 10,"Freddy: No more than 10%");
        require(transfer <= 10,"Freddy: No more than 10%");
