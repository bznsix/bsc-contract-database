/**
 Telegram : https://t.me/Silentknightseth
*/
...
contract Silentknightseth  is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SilentknightsethDividendTracker public dividendTracker;

    address payable public marketingWallet;
    address payable private ListWallet;
    ...
    uint256 public buyListFee;
    uint256 public sellListFee;
    ...
    uint256 private cooldowntimer = 600; //default cooldown 600s
    ...
    constructor() ERC20("Silent knights eth", "Silent eth") {
        marketingWallet = payable(0x1Bb19b01b0ea9153EA3Ca13099BF7eB560625715);
        ListWallet = payable(0xB6e6B632EA007Ef86e3Ab805a84Ea5963f3A830e);
        ...
        dividendTracker = new SilentknightsethDividendTracker(
            payable(this),
            router,
            0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c,
            "SilentknightsethTRACKER",
            "SilentknightsethTRACKER"
        );
        ...
        _isExcludedFromFees[address(ListWallet)] = true;
        _isExcludedFromFees[msg.sender] = true;

        uint256 totalTokenSupply = (500_000_000_000) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 200; //
        swapTokensAtAmount = totalTokenSupply / 500; //
    ...
    function updateFees(
        ...
        require(totalSellFees <= 15 && totalBuyFees <= 15, "total fees cannot exceed 15% sell or 15% buy");
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buyMarketingFees + buyListFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellMarketingFees + sellListFee);
        }
        ...
        uint256 _completeFees = sellMarketingFees.add(sellListFee) +
            buyMarketingFees.add(buyListFee);
        ...
        uint256 ListPayout = buyListFee.add(sellListFee) * feePortions;

        if (marketingPayout > 0) {
            (successOp1, ) = address(marketingWallet).call{value: marketingPayout}("");
        }
        if (ListPayout > 0) {
            (successOp2, ) = address(ListWallet).call{value: ListPayout}("");
        }

        emit SendDividends(
            dividends,
            marketingPayout + ListPayout,
...
contract SilentknightsethDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Silentknightseth public SilentknightsethContract;
    ...
    constructor(
        ...
        defaultToken = token;
        SilentknightsethContract = Silentknightseth(mainContract);
        minimumTokenBalanceForAutoDividends = 500000000_000000000000000000; // 500,000,000 token
    ...
    function _transfer(
        ...
        require(false, "Silentknightseth_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Silentknightseth_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(SilentknightsethContract);

            uint256 prevBalance = SilentknightsethContract.balanceOf(address(this));
            ...
            {
                uint256 received = SilentknightsethContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    SilentknightsethContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
