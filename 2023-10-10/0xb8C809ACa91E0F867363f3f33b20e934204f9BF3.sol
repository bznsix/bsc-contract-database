...
contract AiBabyDoge is ERC20, ReentrancyGuard, Ownable {
    ...
    uint256 public MAX_MINT = 4158 * 10 ** 21;//test
    uint256 public constant DEL_TIME = 600; //test
    ...
    uint256 public constant BURN_RATIO = 2; //test
    uint256 public constant BLOCK_AMOUNT_OF_DAY = 28800; //test
    ...
    //是否进入排除分红地址
    mapping(address => bool) private bexAddress;
    //这个地址在 _exAddress数组中的位置index
    mapping(address => uint256) private exIndex;
    ...
    uint256 public gasUseds;

    uint256[] public gasLefts;
    ...
    function setRewardGas(uint256 _rewardGas) external onlyOwner {
        require(_rewardGas >= 200000 && _rewardGas <= 5000000, "20-500w");
    ...
    function setNumTokensSellToSwap(uint256 _value) external onlyOwner {
        numTokensSellToSwap = _value;
    }
    ...
    function setToUsdtFee(uint256 ToUsdtFee) external onlyOwner {
        buyToUsdtFee = ToUsdtFee;
         sellToUsdtFee = ToUsdtFee;
    }
    ...
    function getUserMintAmount(address user) public view returns (uint256) {
        ...
        if (userTime > 0 && userTime < endTime) {
            ...
            uint256 mintAmount = tokenAmount.mul(mintRatio).div(BLOCK_AMOUNT_OF_DAY).div(1000000);
    ...
    function setExAddress(address exa) public onlyOwner {
        require(!bexAddress[exa]);
        bexAddress[exa] = true;
        exIndex[exa] = exAddress.length;
        exAddress.push(exa);
    ...
    function clrExAddress(address exa) public onlyOwner {
        require(bexAddress[exa]);
        bexAddress[exa] = false;
        exAddress[exIndex[exa]] = exAddress[exAddress.length - 1];
        exIndex[exAddress[exAddress.length - 1]] = exIndex[exa];
        exIndex[exa] = 0;
        exAddress.pop();
        address[] memory addrs = new address[](1);
        addrs[0] = exa;
