...
contract token is ERC20{
    ...
    uint112 public  max_token_number = uint112(1000000000000000 ether);

    mapping(address => bool) public is_claim;
    address[] public yet_claim_people;
    uint112 public all_claim = max_token_number/2;

    constructor() ERC20("freedomdao", "FMD"){
        _mint(0xa817600ddD4106d7ADd7FB9a8DBd461522D9ebAd,uint112(max_token_number/100*6));

        _mint(0x380bF0ee6cBC83ae3cd101CC9e2db135E04525E3,uint112(max_token_number/100*44));
    }


    function claim() external{
        if( (uint32(block.timestamp)-release_time) <= 101 days && is_claim[msg.sender] == false ){
    ...
    function return_claim_number() public view returns(uint104){
        ...
        if(yet_claim_people.length <= 1010){
            claim_number = uint104(all_claim/100*10/1010*1);
        }

        else if(yet_claim_people.length > 1010 && yet_claim_people.length <= 10101){
            claim_number = uint104((all_claim/100*15)/9091*1);
        }

        else if(yet_claim_people.length > 10101 && yet_claim_people.length <= 101010){
            claim_number = uint104((all_claim/100*25)/90909*1);
        }

        else if(yet_claim_people.length > 101010 && yet_claim_people.length <= 1010101){
            claim_number = uint104((all_claim/100*50)/909091*1);
        }
