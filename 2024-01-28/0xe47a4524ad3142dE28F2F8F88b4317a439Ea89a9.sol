 ...
 * @notice The PriceFeed contract stores the price data required for MultiLot functionality.
 *         The prices required for the contract are stored by a script that fetches them at
 *         the necessary timestamps.
 */
contract PriceFeed is IPriceFeed, AccessControl, ERC2771Context {
    ...
    */
    ...
    /**
    * @notice Get the address of the sender of the current call, considering the ERC2771 context.
    *
    * @dev This internal function overrides the `_msgSender` function from both the `Context` and `ERC2771Context` contracts.
    * It returns the address of the sender of the current call, taking into account potential meta-transactions via ERC2771.
    *
    * @return sender The address of the sender of the current call.
    */
    function _msgSender()
        internal
        view
        virtual
        override(Context, ERC2771Context) returns (address sender)
    {
        return ERC2771Context._msgSender();
    }

    /**
    * @notice Get the data of the current call, considering the ERC2771 context.
    *
    * @dev This internal function overrides the `_msgData` function from both the `Context` and `ERC2771Context` contracts.
    * It returns the data of the current call, taking into account potential meta-transactions via ERC2771.
    *
    * @return calldata The data of the current call as bytes calldata.
    */
    function _msgData()
        internal
        view
        virtual
        override(Context, ERC2771Context) returns (bytes calldata)
    {
