...
contract BuyNft is Ownable {
    ...
    address public receiveFee = 0xbDCbB21e74F89885dADA30F58318Ebf2915f30aC;

    bool public stt = false;
    uint256 public totalMint = 200;
    uint256 public timeOpen = 1706454000;
    uint256 public priceNft = 1.5 * 1e18;
    mapping(address => bool) public hasPurchased;
    mapping(address => bool) public whitelist;
    address public devAddress;
    address[] public whitelistAddress;
    uint256 public totalWhitelist = 0;

    modifier onlyDev() {
        require(
            msg.sender == devAddress,
            "Only approved dev can call this function"
        );
        _;
    }

    constructor(address nft, address devAddress_) {
    ...
    function configs(
        ...
        receiveFee = receiveFee_;
        priceNft = priceNft_;
        stt = _stt;
    }

    function addToWhitelist(address[] memory addressToAdd)
        external
        onlyDev
    {
        for (uint256 i = 0; i < addressToAdd.length; i++) {
            address addr = addressToAdd[i];
    ...
    function removeFromWhitelist(address[] memory addressToRemove)
        external
        onlyDev
    {
        for (uint256 i = 0; i < addressToRemove.length; i++) {
            address addr = addressToRemove[i];
            if (whitelist[addr] && addr != address(0)) {
    ...
    function _removeAddressFromWhitelist(address addr) private {
        for (uint256 i = 0; i < whitelistAddress.length; i++) {
            if (whitelistAddress[i] == addr) {
                whitelistAddress[i] = whitelistAddress[
                    whitelistAddress.length - 1
                ];
    ...
    function _buyNfts() private {
        ...
        totalMint -= 1;
        payable(receiveFee).transfer(priceNft);
        _nft.mintNft(msg.sender, 0);
    }
