...
contract WolfpunksDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Wolfpunks_Dividend_Tracker", "Wolfpunks_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18);
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Wolfpunks_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Wolfpunks_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Wolfpunks contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Wolfpunks_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"); 
        require(newClaimWait != claimWait, "Wolfpunks_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Wolfpunks is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    WolfpunksDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Wolfpunks", "punks") 
    {
        BNBRewardsFee = 1;
        liquidityFee = 1;
        marketingFee = 3;
        ...
        marketingWallet = payable(0x4f798F69bF0c0c521ddEd37fA1ec4518b2Dc98b5);
        devWallet = payable(0x752509d7F3A94c01a3Fe23caDA1a2C0CB1368b5c);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(devFee); // total fee transfer and buy

        dividendTracker = new WolfpunksDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Wolfpunks: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Wolfpunks: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Wolfpunks: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Wolfpunks: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Wolfpunks: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Wolfpunks: Cannot update gasForProcessing to same value");
