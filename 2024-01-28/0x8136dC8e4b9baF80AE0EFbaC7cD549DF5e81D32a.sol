...
interface IERC721 is IERC165 {
    ...
    function ONFTMint(address recAddr, uint256 tokenId) external;
...
contract ONFTBridge is NonblockingLzApp, EIP712("Voucher-Domain", "1") {
    address public commissionWallet = 0x93F3a3A80817BFD3e3A47312cCFF5D76D84F510c;
    ...
    function setCommissionWallet(address _a) public onlyOwner {
        commissionWallet = _a;
    }
    ...
    function traverse(uint16 _dstChainId, address _srcToken, address _dstToken, uint[] memory tokenIds, bytes calldata adminSignature) public payable {
        ...
        payable(commissionWallet).transfer(msg.value / 100 * feePercentage);

        bytes memory payload = abi.encode(msg.sender, tokenIds, _dstToken);
        _lzSend(_dstChainId, payload, payable(msg.sender), address(0x0), bytes(""), msg.value - (msg.value/100*feePercentage));
    }
    ...
    function traverseV2(address _from, uint16 _dstChainId, bytes memory _toAddress, uint _tokenId, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams, address _srcToken) public payable {
        payable(commissionWallet).transfer(msg.value / 100);
        IERC721(_srcToken).sendFrom{ value: msg.value - msg.value / 100 }(_from, _dstChainId, _toAddress, _tokenId, _refundAddress, _zroPaymentAddress, _adapterParams);
    }
    ...
    function traverseV2Batch(address _from, uint16 _dstChainId, bytes memory _toAddress, uint[] memory _tokenIds, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams, address _srcToken) public payable {
        payable(commissionWallet).transfer(msg.value / 100);
        IERC721(_srcToken).sendBatchFrom{ value: msg.value - msg.value / 100 }(_from, _dstChainId, _toAddress, _tokenIds, _refundAddress, _zroPaymentAddress, _adapterParams);
    }
