...
contract HodlBuild is Context, IERC20, Ownable {
    ...
    string private _name = "Hodl Build";
    string private _symbol = "Hodl Build";
    ...
    constructor(address hold_token_address) {
        ...
        _hodlToken = HodlToken( hold_token_address);//主币地址
    ...
    function burnToHolder(uint256 amount,address _invitation) external {
        ...
        path[0] = address(_hodlToken);
        path[1] = uniswapRouter.WETH();
        uint256 deserved = 0;
        deserved = uniswapRouter.getAmountsOut(amount, path)[path.length - 1];
        require(payable(address(_hodlToken)).balance>=deserved,'not enough balance');
        _hodlToken.burnToholder(sender, amount, deserved);
        _hodlTokenToDead(sender,amount);
    ...
    function _hodlTokenToDead(address to,uint256 amountIn) private {
        address _addr = address(_hodlToken);
