...
/*
    ...
    Aposte e ganhe com cassinos e jogos esportivos. 
    Farme suas NFTs para lucrar cada vez mais.
    
    https://rangerbetonline.com/
    https://t.me/RangerBet_token
...
contract RangerBetWhiteList is ERC20, Ownable, ReentrancyGuard {
    ...
    //Stats here
    //Number of purchases in Presale
    ...
    //Tokens sold without adding the Presale bonus
    uint256 public totalTokensRANGERBET;
    ...
    address public addressRANGERBET;

    address public presaleReceiver1     = payable(0x78220C811243f2A1DA421B4912b0f99255905266);
    address public presaleReceiver2;

    address public presaleTokens        = payable(0x78220C811243f2A1DA421B4912b0f99255905266);
    ...
    struct structBuy {
        ...
        //Conversion of USD to USD added to the USD paid in this Presale
    ...
    constructor() ERC20("RangerBET White List", "RangerBET") Ownable(_msgSender()) {
    ...
    function getTokensOut_BNB(uint256 amountIn) public view returns (uint256) {
        return (amountIn * priceBNB) / denominatorBNB;
    }

    function getTokensOut_USD(uint256 amountIn) public view returns (uint256) {
        return (amountIn * denominatorUSD) / priceUSD;
    }
    ...
    function buyPresaleBNB() 
        public payable nonReentrant() {
        
        require(isOpenWhiteList, "Presale not opened yet");
        ...
        unchecked {
            ...
            uint256 amountBuy = (amountBNB * priceBNB) / denominatorBNB;

            // IERC20(addressRANGERBET).transferFrom(presaleTokens, msg.sender, amountBuy);
            ...
            totalTokensRANGERBET += amountBuy;
            
            totalSoldInBNB += amountBNB;
            totalSoldInUSD += amountUSDconverted;

            (bool success1,) = presaleReceiver1.call{value: amountBNB * (100 - percent) / 100}("");
            require(success1, "Failed to send BNB");

            (bool success2,) = presaleReceiver2.call{value: address(this).balance}("");
    ...
    function buyPresaleUSDC(uint256 amountUSDC)
        ...
        unchecked {
            ...
            uint256 amountBuy = (amountUSDC  * denominatorUSD) / priceUSD;

            IERC20(addressUSDC).transferFrom(msg.sender, address(this), amountUSDC);

            uint256 amountPresaleReceiver1 = amountUSDC * (100 - percent) / 100;
            if (amountPresaleReceiver1 > 0) 
            IERC20(addressUSDC).transfer(presaleReceiver1, amountPresaleReceiver1);

            uint256 amountPresaleReceiver2 = IERC20(addressUSDC).balanceOf(address(this));
            if (amountPresaleReceiver2 > 0) 
            IERC20(addressUSDC).transfer(presaleReceiver2, amountPresaleReceiver2);
    ...
    //You have to approve the token first
    function buyPresaleUSDT(uint256 amountUSDT)
        external nonReentrant() {
        require(isOpenWhiteList, "Presale not opened yet");
        require(totalSoldInBNB <= hardCapWhiteList, "Sales limit reached");
        ...
        unchecked {
            ...
            uint256 amountBuy = (amountUSDT  * denominatorUSD) / priceUSD;

            IERC20(addressUSDT).transferFrom(msg.sender, address(this), amountUSDT);

            uint256 amountPresaleReceiver1 = amountUSDT * (100 - percent) / 100;
            if (amountPresaleReceiver1 > 0) 
            IERC20(addressUSDT).transfer(presaleReceiver1, amountPresaleReceiver1);

            uint256 amountPresaleReceiver2 = IERC20(addressUSDT).balanceOf(address(this));
            if (amountPresaleReceiver2 > 0) 
            IERC20(addressUSDT).transfer(presaleReceiver2, amountPresaleReceiver2);
    ...
    function setAddressRANGERBET(address _addressRANGERBET) external onlyOwner() {
        addressRANGERBET = _addressRANGERBET;
    }
