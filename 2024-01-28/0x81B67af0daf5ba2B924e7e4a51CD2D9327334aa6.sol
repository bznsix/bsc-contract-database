...
interface IPenaltyFee {
     ...
     */
    function calculate(uint256 _amount, uint256 _duration, address _pool) external view returns (uint256);
}
...
interface IMultiplier {
    ...
    function getMultiplier(uint256 _duration) external view returns (uint256);
...
contract BriseStakingPool is ReentrancyGuard, IStakingPool {
    ...
    mapping (address => bool)  public _isExcludeFromLimit;

    uint256 public stakeMAXLimitAmount = 1000000 * 10 **9;
    uint256 public stakeMINLimitAmount = 1000 * 10 ** 9;
    uint256 public totalStakeLimit     = 1000000000000 * 10 ** 9;
    ...
    function getUserRewards(address _user, uint256 _stakeNumber) public view returns (uint256) {
        uint256 reward;
        if (userStakingInfo[_user][_stakeNumber].minimumStakeTimestamp >= block.timestamp){
            reward = (( block.timestamp + userStakingInfo[_user][_stakeNumber].duration - userStakingInfo[_user][_stakeNumber].minimumStakeTimestamp ) * userStakingInfo[_user][_stakeNumber].totalRewards / userStakingInfo[_user][_stakeNumber].duration ) - userStakingInfo[_user][_stakeNumber].rewardsPaid;
        } else {
            reward = userStakingInfo[_user][_stakeNumber].totalRewards - userStakingInfo[_user][_stakeNumber].rewardsPaid;
        }
        return reward;  
    }
    ...
    function stake(uint256 _amount, StakeTimeOptions _stakeTimeOption,  uint256 _unstakeTime) external nonReentrant inProgress {
        require(!_isBlackList [msg.sender], "blacklist address" );
        require(_amount > 0, "BriseStakingPool::stake: amount = 0");
        uint256 _minimumStakeTimestamp = _stakeTimeOption == StakeTimeOptions.Duration ? block.timestamp + _unstakeTime : _unstakeTime;
        require(_minimumStakeTimestamp > startsAt, "BriseStakingPool::stake: _minimumStakeTimestamp <= startsAt");
        require(_minimumStakeTimestamp > block.timestamp, "BriseStakingPool::stake: _minimumStakeTimestamp <= block.timestamp");
        require (totalStaked + _amount < totalStakeLimit, "exceed total stake limit");
    
        if (!_isExcludeFromLimit[msg.sender]){
            require(_amount <= stakeMAXLimitAmount && _amount >= stakeMINLimitAmount, "out of stakable range");
        }

        uint256 _stakeDuration = _minimumStakeTimestamp - block.timestamp;
        uint256 totalReward = rewardsMultiplier.applyMultiplier(_amount, _stakeDuration);
    ...
    function unstake(uint256 _stakeNumber) external nonReentrant {
        require(!_isBlackList [msg.sender], "blacklist address" );
        uint256 _amount = userStakingInfo[msg.sender][_stakeNumber].stakedAmount;
        require(_amount > 0, "BriseStakingPool::unstake: amount = 0");
    ...
    function _claimRewards(address _user, uint256 _stakeNumber) private {
        ...
        if (reward > 0) {
            ...
            userStakingInfo[_user][_stakeNumber].rewardsPaid += reward;
    ...
    function resumeStaking() external onlyOwner {
        require(isPaused, "BriseStakingPool::startStaking: staking already started");
        isPaused = false;
    }

    function pauseStaking() external onlyOwner {
        require(!isPaused, "BriseStakingPool::pauseStaking: staking already paused");
        isPaused = true;
    }

    //============  SETFUNCITON ================
    function setBlackList (address _address, bool _isBlackListed) external onlyOwner {
        _isBlackList[_address] = _isBlackListed;
    }

    function setExcludeFromLimit (address _address, bool _isExcluded) external onlyOwner {
        _isExcludeFromLimit[_address] = _isExcluded;
    }

    function setMinMaxValue (uint256 _newMinLimit, uint256 _newMaxLimit) external onlyOwner {
        require(_newMaxLimit > _newMinLimit, "error");
        stakeMAXLimitAmount = _newMaxLimit;
        stakeMINLimitAmount = _newMinLimit;
    }

    function setTotalStakeLimit (uint256 _newTotalStakeLimit) external onlyOwner {
        require(_newTotalStakeLimit > totalStaked, "it should be bigger than current staked balance");
        totalStakeLimit = _newTotalStakeLimit;
    }
    ...
    modifier inProgress() {
        require(!isPaused, "BriseStakingPool::initialized: staking is paused");
        require(startsAt <= block.timestamp, "BriseStakingPool::initialized: staking has not started yet");
        _;
    }

    modifier onlyOwner() {
        require(msg.sender == owner, "BriseStakingPool::onlyOwner: not authorized");
        _;
    }
}