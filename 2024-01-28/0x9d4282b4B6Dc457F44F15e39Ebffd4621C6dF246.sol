...
contract Eloin is ERC20, ERC20Burnable, Ownable {
    ...
    uint16 immutable DIVISOR = 1000;

    uint256 public buyLimit;
    uint256 public sellLimit;

    address public taxWallet = 0x4d7C456DbC1a79fD6242FEfA119a9514caA8D991;
    ...
    ) ERC20("Eloin", "ELOIN") Ownable(initialOwner) {
        require(initialOwner != address(0), "Eloin : Invalid Address");
    ...
    function _update(
        ...
        if (takeFee) {
            ...
            if (timeLimit) {
                _validateTime(from, to);
            }

            uint256 burnAmount = (value * burnFee) / DIVISOR;

            uint256 tax = 0;

            if (_isSell(from, to)) {
                tax = (value * sellFee) / DIVISOR;
            }

            if (_isBuy(from)) {
                tax = (value * buyFee) / DIVISOR;
            }
    ...
    function _validateTime(address sender, address recipient) private {
        if (_isBuy(sender)) {
            require(
                _buyTime[recipient] + 4 minutes <= block.timestamp,
                "Eloin : wait 4 minutes to buy again"
            );
            _buyTime[recipient] = block.timestamp;
        } else if (_isSell(sender, recipient)) {
            require(
                _sellTime[sender] + 5 minutes <= block.timestamp,
                "Eloin : wait 5 minutes to sell again"
            );
    ...
    function _validateTransfer(
        ...
        if (_isBuy(sender)) {
            require(amount <= buyLimit, "Eloin : Buy amount exceeds limit");
        } else if (_isSell(sender, recipient)) {
            require(amount <= sellLimit, "Eloin : Sell amount exceeds limit");
        }
    }
    ...
    function excludeFromFee(address account) public onlyOwner {
        require(account != address(0), "Eloin : Invalid Address");
    ...
    function setBurnFeePercent(uint256 fee) external onlyOwner {
        require(fee >= 0 && fee <= 1000, "Eloin : Invalid Value");
        burnFee = fee;
    }
    ...
    function updateBuyLimit(uint256 limit) external onlyOwner {
        require(limit > 0, "Eloin : Invalid Value");
        buyLimit = limit;
    }
    ...
    function updateRouter(address router) external onlyOwner {
        require(router != address(0), "Eloin : Invalid Address");
        uniswapV2Router = IUniswapV2Router02(router);
    }

    function updatePair(address pair) external onlyOwner {
        require(pair != address(0), "Eloin : Invalid Address");
        uniswapV2Pair = pair;
    }

    function updateTimelimit(bool status) external onlyOwner {
        timeLimit = status;
    }
