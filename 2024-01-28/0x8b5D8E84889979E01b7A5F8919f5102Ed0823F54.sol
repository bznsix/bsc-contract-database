...
/**
Telegram: https://t.me/DXTBSC
Website: https://dragonxtokenbsc.com/
twitter: https://twitter.com/DragonXtokenBSC
*/
...
contract DragonXToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DragonXTokenDividendTracker public dividendTracker;
    ...
    constructor() ERC20("DRAGONXTOKEN", "DXT") {
        marketingWallet = payable(0xac24F5268063DDf394D142250a6382bBa9fE0faC);
        devWallet = payable(0x368962574127221f96D1088d4DA4DCf177A03521);
        ...
        dividendTracker = new DragonXTokenDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (80_000_000_000_000) * (10**18);
...
contract DragonXTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    DragonXToken public DragonXTokenContract;
    ...
    constructor(
        ...
        defaultToken = token;
        DragonXTokenContract = DragonXToken(mainContract);
    ...
    function _transfer(
        ...
        require(false, "DragonXToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DragonXToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Fish contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(DragonXTokenContract);

            uint256 prevBalance = DragonXTokenContract.balanceOf(address(this));
            ...
            {
                uint256 received = DragonXTokenContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    DragonXTokenContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
