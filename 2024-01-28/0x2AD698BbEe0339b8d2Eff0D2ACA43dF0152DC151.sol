...
contract Icarus is ERC20, Ownable {
    ...
    function _axoTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(launched()){
            require(checkWhitelist(), "You are not eligible for this Round. Wait for the next Round.");
        }
    ...
    function checkWhitelist() public view  returns(bool) {
        ...
        if(currentRound == MintRound.FirstRound){
            return whitelistedForRound[msg.sender] == 2;
        } else if (currentRound == MintRound.SecondRound){
            return whitelistedForRound[msg.sender] == 1 ||  whitelistedForRound[msg.sender] == 2;
