...
contract Burn2Point is ERC20, Ownable {
    using SafeMath for uint256;
    IUniswapV2Router02 public uniswapRouter;
    address public projector = 0x11221E5012AC561999B101B2d269712C0AF12562;
    ...
    constructor() ERC20("Burn2.0", "Burn2.0") {
        ...
        _mint(projector, 21000000 * 10 ** 18);
    }
        ...
        if (!_excludedFees[from] && !_excludedFees[to]) {
            ...
            if (to == uniswapPair) { //to sell
                ...
                if (!swapIng && contractBalance > minBalanceSwapToken) {
                    ...
                    uint256 amountToDead = contractBalance.div(3); // 1/3 to dead;
                    super._transfer(address(this), address(0xdead), amountToDead);
                    contractBalance = contractBalance - amountToDead;
                    swapTokensForEth(contractBalance);

                    uint256 amountAllEth = payable(address(this)).balance;
                    payable(address(projector)).transfer(amountAllEth);
