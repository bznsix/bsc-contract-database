...
// File @openzeppelin/contracts/token/ERC721/extensions/ERC721Royalty.sol@v4.9.2
...
// File contracts/Tokens/FitBurnNFTMigratable.sol
...
contract FitBurnNFTMigratable is
    ERC721,
    ERC721Enumerable,
    ERC721Burnable,
    ERC721Royalty,
    ...
    Ownable
{
    ...
    uint256 price;
    ERC721Burnable public oldCollection;
    mapping(address => uint256) public nftPriceInToken;
    ...
    constructor(
        ...
        oldCollection = ERC721Burnable(_oldCollection);
    ...
    function mint() external payable whenNotPaused {
        require(!bannedAddresses[msg.sender], "Your address is banned");
    ...
    function mintTo(address to) external onlyOwner {
        require(!bannedAddresses[to], "To address is banned");

        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        require(!_exists(tokenId), "NFT with the specified ID already exists");
    ...
    function migrateMint(
        ...
        require(
            oldCollection.ownerOf(tokenId) == msg.sender,
            "You don't own the existing NFT"
        );
        require(!_exists(tokenId), "NFT with the specified ID already exists");

        ERC721Burnable(address(oldCollection)).burn(tokenId);
    ...
    function migrateTo(
        ...
        require(
            oldCollection.ownerOf(tokenId) == to,
            "You don't own the existing NFT"
        );
        require(!_exists(tokenId), "NFT with the specified ID already exists");
    ...
    function buyNFTWithToken(address token) external whenNotPaused {
        IERC20 ERC20Token = IERC20(token);

        uint256 requiredERC20Amount = nftPriceInToken[token];
        require(requiredERC20Amount > 0, "Wrong token");
        require(
            ERC20Token.balanceOf(msg.sender) >= requiredERC20Amount,
            "Insufficient USDT balance"
        );

        ERC20Token.transferFrom(msg.sender, address(this), requiredERC20Amount);
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
    ...
    function _beforeTokenTransfer(
        ...
        require(
            !bannedAddresses[from],
            "Transfer not allowed from this address"
        );
        require(!bannedAddresses[to], "Transfer not allowed to this address");
