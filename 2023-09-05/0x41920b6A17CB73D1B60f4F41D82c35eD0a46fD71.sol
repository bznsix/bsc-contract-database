...
contract TradingFeeTopTraders is Ownable, Pausable {
    ...
    function canClaim(string calldata _campaignId, address _sender, uint256 _selfTradingFee) public view returns (uint256 amount) {
        if (ITradingFeeClaimedRecord(tradingFeeClaimedRecord).hasClaimedRebate(_sender)) {
            amount = 0;
        } else {
    ...
    function claimReward(string calldata _campaignId, bytes32[] calldata _merkleProof, uint256 _selfTradingFee) public whenNotPaused {
        ...
        // update hasClaimed
        ITradingFeeClaimedRecord(tradingFeeClaimedRecord).updateHasClaimedTopTraders(msg.sender);
    ...
    /// @dev Update the switch of startTimeSwitch
    function updateStartTimeSwitch(bool _newValue) external onlyOwner {
        startTimeSwitch = _newValue;
    }
