...
import "./interfaces/IVTheX.sol";

contract TheXPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 theXAtLastUserAction; // keep track of x deposited at the last user action.
    ...
    IERC20 public immutable token; // x token.
    ...
    event NewVTheXContract(address TheX);
    ...
    /**
     * @notice Constructor
     * @param _token: X token contract
     * @param _masterchefV2: MasterChefV2 contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     * @param _operator: address of operator
     * @param _pid: x pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(theXPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the x owner address or the operator address.
     */
    modifier onlyOperatorOrTheXOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or x owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onTheXPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.theXAtLastUserAction;
    ...
    /**
     * @notice Unlock user x funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrTheXOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the x Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in X)
    ...
    function depositOperation(
        ...
        if (TheX != address(0)) {
            IVTheX(TheX).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.theXAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the x Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (TheX != address(0)) {
            IVTheX(TheX).withdraw(msg.sender);
        }
        ...
        if (user.shares > 0) {
            user.theXAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.theXAtLastUserAction = 0;
        }
    ...
    /**
     * @notice Harvest pending x tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingTheX = masterchefV2.pendingTheX(theXPoolPID, address(this));
        if (pendingTheX > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(theXPoolPID, 0);
    ...
    /**
     * @notice Set TheX Contract address
     * @dev Callable by the contract admin.
     */
    function setTheXContract(address _TheX) external onlyAdmin {
        require(_TheX != address(0), "Cannot be zero address");
        TheX = _TheX;
        emit NewVTheXContract(TheX);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the x Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingTheXRewards();
     ...
     * @return Returns total pending x rewards
     */
    function calculateTotalPendingTheXRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingTheX(theXPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingTheXRewards()) * (1e18)) / totalShares);
    }
...
interface IMasterChefV2 {
    ...
    function pendingTheX(uint256 _pid, address _user) external view returns (uint256);
