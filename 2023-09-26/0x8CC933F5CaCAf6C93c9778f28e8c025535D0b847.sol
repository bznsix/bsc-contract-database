...
import "./interfaces/ITheXPool.sol";

contract TheXFlexiblePool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user
        uint256 lastDepositedTime; // keeps track of deposited time for potential penalty
        uint256 theXAtLastUserAction; // keeps track of x deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // x token
    ITheXPool public immutable theXPool; //x pool
    ...
    //When call theXpool.withdrawByAmount function,there will be a loss of precision, so need to withdraw more.
    uint256 public withdrawAmountBooster = 10001; // 1.0001
    uint256 public performanceFee = 200; // 2%
    uint256 public withdrawFee = 10; // 0.1%
    uint256 public withdrawFeePeriod = 72 hours; // 3 days

    event DepositX(address indexed sender, uint256 amount, uint256 shares, uint256 lastDepositedTime);
     ...
     * @param _theXPool: theXPool contract
    ...
    constructor(
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_theXPool), type(uint256).max);
    }
    ...
    /**
     * @notice Deposits funds into the X Flexible Pool.
    ...
    function deposit(uint256 _amount) external whenNotPaused {
        ...
        if (user.shares > 0) {
            ...
            if (currentPerformanceFee > 0) {
                ...
                if (_amount >= currentPerformanceFee) {
                    ...
                    theXPool.withdrawByAmount(withdrawAmount);
        ...
        emit DepositX(msg.sender, _amount, currentShares, block.timestamp);
    }

    /**
     * @notice Withdraws funds from the X Flexible Pool
    ...
    /**
     * @notice Withdraws from X Pool without caring about rewards.
     * @dev EMERGENCY ONLY. Only callable by the contract admin.
     */
    function emergencyWithdraw() external onlyAdmin {
        require(staking, "No staking X");
        staking = false;
        theXPool.withdrawAll();
    }

    /**
     * @notice Withdraw unexpected tokens sent to the X Flexible Pool
     */
     ...
     * @dev It includes tokens held by the contract and held in theXPool
     */
    function balanceOf() public view returns (uint256) {
        (uint256 shares, , , , , , , , ) = theXPool.userInfo(address(this));
        uint256 pricePerFullShare = theXPool.getPricePerFullShare();

        return token.balanceOf(address(this)) + (shares * pricePerFullShare) / 1e18;
    }

    /**
     * @notice Deposits tokens into theXPool to earn staking rewards
     */
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            theXPool.deposit(bal, 0);
        }
    }
}// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
