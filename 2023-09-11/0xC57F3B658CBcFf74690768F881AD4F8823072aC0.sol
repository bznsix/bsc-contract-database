...
contract OIdoooooFULL is Ownable, ReentrancyGuard {
    ...
    address public Tuna = 0x0000000000000000000000000000000000000000; // Token Contract
    address public Pair = 0x0000000000000000000000000000000000000000; // Pair Address
    ...
    function Deposit(uint amount, address partner) external nonReentrant 
    {
        ...
        uint256 needSend = (amount * 10 / 100) * tokenPrice / 10**18;
    ...
    {
        uint256 RewardTime = (block.timestamp - user[account].timestamp) / 3600; // TIMING 86400
        //RewardTime = RewardTime; // (RewardTime > 1) ? 1 : 
    ...
    function _updatePercentage(address account) internal 
    {
        ...
        if(user[account].percentage < updPercentage)
            user[account].percentage = updPercentage * 10;
    }
    ...
    function addToken(address smartTuna) external onlyOwner {
        Tuna = smartTuna;
    }

    function addPair(address smartPair) external onlyOwner {
        Pair = smartPair;
    }
