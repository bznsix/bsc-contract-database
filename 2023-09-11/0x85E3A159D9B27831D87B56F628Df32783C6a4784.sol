...
contract UniswapV3Router is BaseCore {
    ...
    function _verifyCallback(uint pool, bytes32 poolSalt, address caller) internal {
        ...
        emit _verifyCallbackEvent(poolDigit, v3Pool.factory, poolSalt, v3Pool.initCodeHash, calcPool, caller);
        if (!WPoolList[caller]) {
