...
contract BABYTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TOKEN contract."
        );
    }
...
contract DogeCoin is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    address public _marketingAddress1;
    address public _marketingAddress2;
    address public _marketingAddress3;
    address public _marketingAddress4;

    uint256 public gasForProcessing;
    
    uint256 public minRemain = 1;
    ...
    constructor(
        address _router,
        address _rewardToken,
        address dividendTracker_, // reward, router, marketing wallet, dividendTracker
        uint256 minimumTokenBalanceForDividends_
    ) payable ERC20("DogeCoin", "DogeCoin") {

        rewardToken = _rewardToken;
        _marketingAddress1 = 0x155fec724B118A8E8aA8c2FD43C4d46fEA1f69E2;
        _marketingAddress2 = 0x57d7FE2df7A2d72667e8A5D6b1f576CeC5e8e805;
        _marketingAddress3 = 0xA97CFE2aFC1813b335adB71A5a38718F4527e750;
        _marketingAddress4 = 0xcd51B89Fd61BF21aE0B545832736044a7aFe0D62;

        uint256 totalSupply_ = 1000000000000000 * 1e18;

        tokenRewardsFee = 5;
        burnFee = 1;
        marketingFee = 1;

        totalFees = tokenRewardsFee.add(marketingFee.mul(4));
        ...
        dividendTracker = BABYTOKENDividendTracker(
            payable(Clones.clone(dividendTracker_))
        );
        ...
        excludeFromFees(_marketingAddress1, true);
        excludeFromFees(_marketingAddress2, true);
        excludeFromFees(_marketingAddress3, true);
        excludeFromFees(_marketingAddress4, true);
    ...
    function setMinRemain(uint256 _minRemain) external onlyOwner {
        minRemain = _minRemain;
    }
    ...
    function _transfer(
        ...
        uint256 fbalance = balanceOf(from);

        if(from != uniswapV2Pair && from != address(uniswapV2Router)) {
            if(!_isExcludedFromFees[from]) {
                require(fbalance.sub(amount) >= fbalance.mul(minRemain).div(10000), "must remain 0.01%");
            }
        }
    ...
    function swapAndSendToFee(uint256 tokens) private {
        ...
        newBalance = newBalance.div(4);

        IERC20(rewardToken).transfer(_marketingAddress1, newBalance);
        IERC20(rewardToken).transfer(_marketingAddress2, newBalance);
        IERC20(rewardToken).transfer(_marketingAddress3, newBalance);
        IERC20(rewardToken).transfer(_marketingAddress4, newBalance);
    }
