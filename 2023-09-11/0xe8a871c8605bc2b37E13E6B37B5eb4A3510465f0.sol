// SPDX-License-Identifier: MIT
	...
	contract LIFE is ERC20, Ownable{
		...
		address public kuo_addr=0x000000000000000000000000000000000000dEaD;
		...
		function _beforeTokenTransfer(
			...
			uint256 sell_amount_tmp=0;
			uint256 price_hole=0;
			uint8 txType;
			uint256 left_amount=_balances[_from]-_amount;
			
			if(ido_addr!=0x000000000000000000000000000000000000dEaD){
				sell_amount=ido(ido_addr).sell_amount(_from);	
			}
			
			if(kuo_addr!=0x000000000000000000000000000000000000dEaD){
				sell_amount_tmp=ido(kuo_addr).sell_amount(_from);
				sell_amount=sell_amount.add(sell_amount_tmp);
			}
			require(left_amount>=sell_amount,"MORE error");
			...
			if (_from==uniswapV2Pair && isRoute[_to]){
				...
				if (_to==uniswapV2Pair){
							...
							uint256 hole_rate_tmp=hole_rate;
							...
							hole_rate_tmp=price_hole+hole_rate;
							
							
							_trueAmount = _amount * (10000 - ( share_rate+holder_rate+lp_rate+hole_rate_tmp  )) / 10000;
					...
					_balances[hole_addr] = _balances[hole_addr] + (_amount * hole_rate_tmp / 10000 );
					...
					emit Transfer(_from, hole_addr, (_amount * hole_rate_tmp / 10000 ));
