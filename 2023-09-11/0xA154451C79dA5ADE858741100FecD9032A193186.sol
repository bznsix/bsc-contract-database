...
contract BaseCore is Ownable, Pausable, ReentrancyGuard {
    ...
    uint public _uniswapV3_factory_allowed_num;
    ...
    //event _verifyCallbackEvent(uint _poolDigit, address _factory, bytes32 _poolSalt, bytes initCodeHash, address calcPool, address caller);
    ...
    function setV3FactoryList(uint[] calldata poolIndex, address[] calldata factories, bytes[] calldata initCodeHash) public onlyExecutor {
        ...
        _uniswapV3_factory_allowed_num = len;
        for (uint i; i < len; i++) {
    ...
    function setV3FactoryListNum(uint _num) external onlyExecutor {
        _uniswapV3_factory_allowed_num = _num;
    }
...
contract UniswapV3Router is BaseCore {
    ...
    function _verifyCallback(uint pool, bytes32 poolSalt, address caller) internal view {
        ...
        if (!WPoolList[caller]) {
            for (uint i = 0; i < _uniswapV3_factory_allowed_num; i++) {
                uint poolDigit = i;
            ...
            require(_verifyOK, "Callback bad pool");
        }
    }
