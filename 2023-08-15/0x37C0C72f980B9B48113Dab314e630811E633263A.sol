...
contract BtcBot is Ownable {
    ...
    mapping(address => uint256) public userCompoundedValues;
    ...
    function deposit(uint256 amount) external {
        ...
        userLastDepositTimestamp[msg.sender] = currentTimestamp; // Store the timestamp for the user's last deposit
        botDailyDeposits += amount;

        deposits[msg.sender] += amount; // Add the deposited amount to the existing balance
    }
    ...
    function pushFromBinanceApi() external onlyOwner {
        ...
        for (uint256 i = 0; i < userList.length; i++) {
            ...
            if (userDeposit > 0) { 
                ...
                if (userDeposit > 0) {
                    ...
                    userDailyProfitPercentage[user] = 0;
        ...
        function userWithdrawDeposit() external {
            ...
            require(deposits[user] > 0, "No deposit for the user");
                ...
                uint256 elapsedTime = block.timestamp - userLastDepositTimestamp[user];
                uint256 withdrawAmount;
            
               if (elapsedTime >= secondsUntilPenaltyDone) {
                // User is entitled to full deposit if penalty time has passed
               // withdrawAmount = deposits[user];
               withdrawAmount = deposits[user] + userProfitTable[user]; // maybe change to the above line
            } else {
                // Apply penalty
                uint256 penaltyAmount = (deposits[user] * penaltyPercentage) / 100;
                withdrawAmount = deposits[user] - penaltyAmount;
                // Penalized amount goes to the contract (botAllTimeDeposits)
            ...
            deposits[user] = 0; // Set user's deposit balance to zero
            userProfitTable[user] = 0; // Reset user's profit table mayeb remove this also
            botDailyDepositWithdrawals += withdrawAmount;
            
            botAllTimeDepositWithdrawals[address(this)] += withdrawAmount;
            userAllTimeDepositWithdrawn[user] += withdrawAmount;
            ...
            for (uint256 i = 0; i < userList.length; i++) {
                if (userList[i] == user) {
                    // Swap with the last user in the list and then pop
            ...
            // Optionally, clear other user-specific mappings if needed
            delete userProfitTable[user];
            delete userProfitTable30[user];
            delete userLastDepositTimestamp[user];
            delete userAllTimeProfitWithdrawn[user];
            delete userAllTimeDepositWithdrawn[user];
            delete userAllTimeProfitPercentage[user];
            delete userDailyProfitPercentage[user];
            delete userAllTimeDeposits[user];
            delete userAllTimeProfit[user];
            delete userCompoundedValues[user];
        }
        ...
        userCompoundedValues[user] += depositIncrease;        
        userProfitTable[user] = 0;
        userLastDepositTimestamp[user] = block.timestamp;
        

         // Not sure if we should addd to all time earnings here I think not
         // provided currently deposited is updated
        }
    ...
    function getUserSecondsUntilPenaltyFinished(address user) external view returns (uint256) {
    uint256 currentTimestamp = block.timestamp;
    uint256 userLastDepositTime = userLastDepositTimestamp[user];

    if (userLastDepositTime == 0) {
        // User has not made a deposit yet, so no penalty applies
        return 0;
    }

    uint256 elapsedTime = currentTimestamp - userLastDepositTime;
    
    if (elapsedTime >= secondsUntilPenaltyDone) {
        // Penalty time has passed, user is not penalized
        return 0;
    } else {
        // Return the remaining time until penalty is finished
        return secondsUntilPenaltyDone - elapsedTime;
    }
    }



    function setSecondsUntilPenaltyDone(uint256 newSecondsUntilPenaltyDone) external onlyOwner {
    secondsUntilPenaltyDone = newSecondsUntilPenaltyDone;
    }

    function setPenaltyPercentage(uint256 newPenaltyPercentage) external onlyOwner {
    require(newPenaltyPercentage <= 100, "Penalty percentage must be between 0 and 100");
    ...
    function getUsersCompoundedValue(address user) external view returns (uint256) {
        return userCompoundedValues[user];
    }
