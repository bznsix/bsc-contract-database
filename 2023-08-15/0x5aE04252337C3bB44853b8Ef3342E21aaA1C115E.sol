...
interface IBTSExchangeWrapperV2 {
    function buyTokens(uint, address, address, address, uint, uint) external payable;
...
contract BTSRouterV2 {
    ...
    function Snipe(address _routerAddress, address _baseToken, address _snipeToken, uint _buyAmount, uint _amountOutMin, uint _txDeadline) external payable {
    ...
    function Sell(uint _snipeID, uint _sellPercentage, bytes32[] memory _userInfo) external {
        ...
        ITokenStore(snipeData.tokenStore).sendTokens(snipeData.snipeToken, exchangeWrappers[snipeData.routerAddress], _sellPercentage);
        uint tokensReceived = IERC20(snipeData.snipeToken).balanceOf(exchangeWrappers[snipeData.routerAddress]);
