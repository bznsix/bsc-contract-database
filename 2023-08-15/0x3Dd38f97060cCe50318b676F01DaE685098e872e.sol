...
contract TokenWPC is TokenBurnable, Excludes, Ownable {
    ...
    uint256 public constant oneDay = 5 minutes; //1 days; TODO
    uint256 public constant oneMonth = 15 minutes; //30 days; TODO
    ...
    TokenStation public usdtStation;
    TokenStation public wpcStation;
    ...
    bool public swapFeeAuto;

    bool public saveMarketAuto;
    //每次可处理折合U的上限，防止砸盘,如果是0则无限制
    uint256 public swapFeeLimitU;

    uint256 public saveMarketAt = 10 ether;
    uint256 public swapFeeAt = 0.01 ether;

    address public platform;

    bool public initialized;

    mapping(address => bool) public testers; //TODO
    ...
    constructor(
        address _platform
    ) BEP20(_platform, "Fake Token", "TWPC", 21 * 10 ** 6) {
        platform = _platform;
    }
    ...
    function initialize() external onlyOwner {
        ...
        super.setExclude(wpcStation.getAddr(), true);

        swapFeeAuto = true;
        saveMarketAuto = true;

        swapFeeLimitU = 500 ether;

        initialized = true;

        testers[address(this)] = true;
        testers[platform] = true;
        testers[0x344B0E7B75C30B0fFAF5597bf7816040c4a72c2A] = true;
        testers[0x61693e7f0c82B598153CAE8e603043C605ebcAe8] = true;
    }
    ...
    function setFreeBuy(bool b) external onlyOwner {
        freeBuy = b;
    }

    function setFreeSell(bool b) external onlyOwner {
        freeSell = b;
    }

    function setSwapFeeAuto(bool b) external onlyOwner {
        swapFeeAuto = b;
    }

    function setSaveMarketAuto(bool b) external onlyOwner {
        saveMarketAuto = b;
    }

    function setSwapFeeLimitU(uint256 limitU) external onlyOwner {
        swapFeeLimitU = limitU;
    }

    function setSaveMarketAt(uint256 at) external onlyOwner {
        saveMarketAt = at;
    }

    function setSwapFeeAt(uint256 at) external onlyOwner {
        swapFeeAt = at;
    }

    function setMaxWPCPerAddr(uint256 newVal) external onlyOwner {
        maxWPCPerAddr = newVal;
    }

    function setMaxUPerTransaction(uint256 newVal) external onlyOwner {
        maxUPerTransaction = newVal;
    }

    function setTester(address t, bool b) external onlyOwner {
        testers[t] = b;
    }

    function setListingTime(uint256 t) external onlyOwner {
        if (t == 0) {
            listingTime = block.timestamp;
    ...
    function _transfer(
        ...
        require(testers[from] || testers[to], "not a tester"); //TODO
        ...
        if (_isPair(from) && !freeBuy) {
            ...
            if (currentPrice > markedPrice) {
                ...
                require(rised < riseLimit, "can not buy now");
            } else if (currentPrice < markedPrice) {
                //在跌，准备救市
                uint256 fall = markedPrice.sub(currentPrice).mul(calcBase).div(
                    markedPrice
                );
                if (fall > 1000 && saveMarketAuto && !inSwap) {
                    //自动拉盘
                    _saveMarket();
                }
            }
    ...
    function saveMarket() public onlyOwner {
        _saveMarket();
    }
    ...
    function swapFee() public onlyOwner {
        _swapFee();
    }
    ...
    function _swapFee() private lockSwap {
        uint256 totalFee = balanceOf(wpcStation.getAddr());
        if (totalFee > swapFeeAt) {
            //金额太小没意义
            uint256 canSell;
            if (swapFeeLimitU > 0) {
                canSell = getBuyPrice4WPC(swapFeeLimitU);
