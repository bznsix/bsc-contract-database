...
contract VaultsManager is Ownable, Pausable {
    ...
    // A record of vaults that are offered for sale at a specific minimum value, and perhaps to a specific person
    ...
    constructor(uint8 _rewardPerVault) {
    ...
    function _isNameAvailable(
        ...
        for (uint256 i = 0; i < vaults.length; i++) {
            if (
                keccak256(bytes(vaults[i].name)) == keccak256(bytes(vaultName))
            ) {
                return false;
            }
        }
    ...
    function _calculateVaultRewards(
        uint _lastClaimTime,
        uint256 amount_
        ...
        return ((rewardPerDay.mul(10000).div(1440) *
            (elapsedTime_ / 1 minutes)) / 10000);
    }

    function _getVaultReward(
        address account,
        uint256 id
    ...
    function _vaultsAvailable() private view returns (bool) {
    ...
    function _vaultsValueAvailable() private view returns (bool) {
    ...
    function createVault(
        ...
        require(_vaultsAvailable(), "Maximum Vaults reached");
        require(_isNameAvailable(account, vaultName), "Name not available");
        require(_vaultsValueAvailable(), "Maximum Vaults Value reached");
    ...
    function getVaultsRewards(address account) public view returns (uint256) {
        VaultsEntity[] storage vaults = _vaultsOfUser[account];
        ...
        for (uint256 i = 0; i < vaultsCount; i++) {
            _vault = vaults[i];
            rewardsTotal += _calculateVaultRewards(
                _vault.lastClaimTime,
                _vault.amount
            );
        }
        return rewardsTotal;
    ...
    function getCurrentVaultOwnerOffer(
        uint id
    ...
    function compoundVaultReward(
        address account
    ) external onlyGuard onlyVaultsOwner(account) whenNotPaused {
        require(_vaultsValueAvailable(), "Maximum Vaults Value reached");
        VaultsEntity[] storage vaults = _vaultsOfUser[account];
        require(
            vaults.length > 0,
    ...
    function getVaultsForSale(
        address account
    ) public view onlyVaultsOwner(account) returns (string memory) {
        VaultsEntity[] memory vaults = _vaultsOfUser[account];
        uint256 vaultsCount = vaults.length;
        Offer memory _vault;
        string memory _isforSale;

        if (vaultsOfferedForSale[vaults[0].id].isForSale == false) {
            _isforSale = "false";
        } else {
            _isforSale = "true";
        }
        ...
        for (uint256 i = 1; i < vaultsCount; i++) {
            _vault = vaultsOfferedForSale[vaults[i].id];
            ...
            _isforSale = string(
                abi.encodePacked(_isforSale, separator, _result)
            );
        }
        return _isforSale;
    }

    function getVaultsForSalePrice(
        address account
    ) public view onlyVaultsOwner(account) returns (string memory) {
        VaultsEntity[] memory vaults = _vaultsOfUser[account];
        uint256 vaultsCount = vaults.length;
        Offer memory _vault;
        string memory _minValue = _uint2str(
            vaultsOfferedForSale[vaults[0].id].minValue
        );
        string memory separator = "#";
        for (uint256 i = 1; i < vaultsCount; i++) {
            _vault = vaultsOfferedForSale[vaults[i].id];
    ...
    function getVaultsId(
        address account
    ) public view onlyVaultsOwner(account) returns (string memory) {
        VaultsEntity[] memory vaults = _vaultsOfUser[account];
        uint256 vaultsCount = vaults.length;
        VaultsEntity memory _vault;
        string memory ids = _uint2str(vaults[0].id);
        string memory separator = "#";
        for (uint256 i = 1; i < vaultsCount; i++) {
            _vault = vaults[i];
            ids = string(abi.encodePacked(
                ids, 
                separator, 
                _uint2str(_vault.id)
                )
    ...
    function getVaultsAmounts(
        address account
    ) public view onlyVaultsOwner(account) returns (string memory) {
        VaultsEntity[] memory vaults = _vaultsOfUser[account];
        uint256 vaultsCount = vaults.length;
        VaultsEntity memory _vault;
        string memory amounts = _uint2str(vaults[0].amount);
        string memory separator = "#";
        for (uint256 i = 1; i < vaultsCount; i++) {
            _vault = vaults[i];
            amounts = string(
    ...
    function getAllVaultsNames() public view returns (string memory) {
    ...
    function getVaultsCreationTime(
        address account
    ) public view onlyVaultsOwner(account) returns (string memory) {
        VaultsEntity[] memory vaults = _vaultsOfUser[account];
        uint256 vaultsCount = vaults.length;
    ...
    function getAllVaultsCreationTime() public view returns (string memory) {
        uint256 vaultsCount = allVaults.length;
        VaultsEntity memory _vault;
    ...
    function getAllAmount() public view returns (string memory) {
        uint256 vaultsCount = allVaults.length;
        VaultsEntity memory _vault;
        ...
        for (uint256 i = 1; i < vaultsCount; i++) {
            ...
            _amount = string(
                abi.encodePacked(_amount, separator, _uint2str(_vault.amount))
            );
        }
        return _amount;
    }

    function getAllforSale() public view returns (string memory) {
        uint256 vaultsCount = allVaults.length;
        Offer memory _vault;
        string memory _isforSale;
    ...
    function getAllforSalePrice() public view returns (string memory) {
        uint256 vaultsCount = allVaults.length;
        Offer memory _vault;
    ...
    function getVaultsLastClaimTime(
        address account
    ) public view onlyVaultsOwner(account) returns (string memory) {
        VaultsEntity[] memory vaults = _vaultsOfUser[account];
        uint256 vaultsCount = vaults.length;
    ...
    function getVaults(
        address account
    ) external view returns (VaultsEntity[] memory) {
        return _vaultsOfUser[account];
    }

    function getIndexOfKey(
        address account
    ) external view onlyGuard returns (int256) {
        require(account != address(0));
        return vaultsOwners.getIndexOfKey(account);
    }
    ...
    function offerVaultToSaleToAddress(
    ...
    function buyVault(
        ...
        require(
            offer.onlySellTo == address(0) || offer.onlySellTo == account,
            "Offer is not available for this account"
        ); // vault not supposed to be sold to this user
