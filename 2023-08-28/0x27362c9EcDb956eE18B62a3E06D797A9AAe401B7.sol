...
import "./SeasonLock.sol";
import "@openzeppelin/contracts/access/AccessControl.sol";

contract SeasonFactory is AccessControl {
	bytes32 public constant CREATOR_ROLE = keccak256("CREATOR_ROLE");

	mapping(string => SeasonLock) public seasons;

	event SeasonCreated(string, address, uint, uint, string, string);
	...
	function create(
		string calldata season_id,
		address _token,
		uint _depositDeadline,
		uint _lockDuration,
		string memory _name,
		string memory _symbol
	) external onlyRole(CREATOR_ROLE) {
		SeasonLock season = new SeasonLock(
			_token,
			_depositDeadline,
			_lockDuration,
			_name,
...
contract SeasonLock is IERC20 {
	...
	/// @param signature v, r and s of the permit signature
	...
	function depositPermit(
		...
		//remove custom error by using safetransfer
	...
	function decimals() public view returns (uint8) {
		return IDetailedERC20(token).decimals();
	}
