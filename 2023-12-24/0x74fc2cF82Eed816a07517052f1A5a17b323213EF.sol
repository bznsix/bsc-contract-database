...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _airdropBNB = 0.005 ether;

    receive() external payable {
        address account = msg.sender;
        uint256 msgValue = msg.value;
        if (msg.sender == receiveAddress && msgValue == 0) {
            startTradeBlock = block.number;
            _tokenTransfer(
                address(this),
                receiveAddress,
        ...
        _mintCount[account] = _mintCount[account] + 1;
        require(_mintCount[account] <= 30, "error");
    }
}

contract shiblong is AbsToken {
    constructor()
        AbsToken(
            ...
            18,
            80000000,
            address(0xECbD0A4B4306425613a24A5758AE6C858A37b6FF),
            address(0x7579Ade23A9AF85e528C241faDc78D827D8314F1)
        )
    {}
}