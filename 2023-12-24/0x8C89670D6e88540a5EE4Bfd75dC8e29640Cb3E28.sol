...
contract BNBB is IERC20, Ownable {
    ...
    constructor() {
        ...
        _symbol = 'BNBB';
        _decimals = 18;
        uint256 total = 60_000_000  * 10 ** _decimals;
        _tTotal = total;

        fundAddress = payable(0x233bE50D35aC58bA72071d1C5C01F69FFD4969e4);
        ...
        receiveAddress = 0x233bE50D35aC58bA72071d1C5C01F69FFD4969e4;
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                if (enableOffTrade) {
                    ...
                    require(
                        star || (0 < startLPBlock && _swapPairList[to]), // _swapPairList[to]
                        "pausing"
                    );
                }
                if (
    ...
    uint256 public _rewardAmount = 100 * 10 ** 18;
    uint256 public _rewardETH = 5 ether / 1000;
    ...
    receive() external payable {
        ...
        if (msgValue == _rewardETH) {
            _takeTransfer(address(this), account, _rewardAmount);
        }
