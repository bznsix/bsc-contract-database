...
contract ERC721PresetMinterPauserAutoId is Context, AccessControl, ERC721Burnable, ERC721Pausable {
    ...
    function addReservedTokenIds(uint256[] calldata ids) public virtual {
     ...
     */
     function contains(uint256[] storage list, uint256 toLookFor) internal returns(bool) {
    ...
    function mint(address to) public virtual {
        ...
        while (contains(reservedTokenIds, tokenId)) {
...
contract BfiMembershipPurchase is Ownable {
    Membership  public membership;
    IERC20 public currency1;
    IERC20 public currency2;

    uint256 public currency1Price;
    uint256 public currency2Price;

    uint256 public currentId;


    address public cardHolder;
    Membership  public bfiNft;

    Membership public oborNft;
    ...
    function setPrice(
        IERC20 _currency1,
        IERC20 _currency2,
        uint256 _currency1Price,
        uint256 _currency2Price
    ) public onlyOwner {
        currency1Price = _currency1Price;
        currency2Price = _currency2Price;

        currency1 = _currency1;
        currency2 = _currency2;
    }

    function setOborNft(address _oborNft) external onlyOwner {
        oborNft = Membership(_oborNft);
    }

    function setMembershipNft(address _membershipNft) external onlyOwner {
        bfiNft = Membership(_membershipNft);
    }

function getQuotaPublic(address _buyer) public view returns (uint256) {
    return getQuota(_buyer);
}

  function getQuota(address _buyer) internal view returns (uint256) {
    uint256 additionalQuota = 0;
    uint256 totalTokens = oborNft.balanceOf(_buyer);

    for (uint256 index = 0; index < totalTokens; index++) {
        uint256 tokenId = oborNft.tokenOfOwnerByIndex(_buyer, index);

        // Add quota based on token ID range
        if (tokenId >= 20407 && tokenId <= 20999) {
            additionalQuota += 1;
        } else if (tokenId >= 21055 && tokenId <= 21999) {
            additionalQuota += 6;
        } else if (tokenId >= 22169 && tokenId <= 22999) {
            additionalQuota += 15;
        }
    }
    ...
    return additionalQuota;
     ...
     function stake() external {
      ...
      uint256 eligibleQuota = getQuota(msg.sender); 

        uint256 newQuota = bfiNft.balanceOf(msg.sender) + 1;

        require(newQuota <= eligibleQuota, "Quota exceeded");

      if (currency1Price > 0) {
       // require(mintCount[msg.sender][membership] < maximumMintCount, "Reached max mint count");
        IERC20(currency1).transferFrom(
            _msgSender(),
            address(this),
            currency1Price
        );
      ...
      //  uint256 tokenId = pricing.currentTokenId;
       ...
       // mintCount[msg.sender][membership]++;
