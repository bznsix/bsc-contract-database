...
contract FatToken is BaseFatToken {
    ...
    constructor(
        ...
        symbol = symbol_;
        decimals = 18;
        totalSupply = numberParams[0]* 10**18;
        currency = addressParams[0];

        _buyFundFee = numberParams[1];
        _buyBurnFee = numberParams[2];
        _buyLPFee = numberParams[3];
        _sellFundFee = numberParams[4];
        _sellBurnFee = numberParams[5];
        _sellLPFee = numberParams[6];
        kb = 0;
        maxBuyAmount = totalSupply * 10**18;
        maxSellAmount = totalSupply* 10**18;
        maxWalletAmount = totalSupply* 10**18;
        airdropNumbs = 0;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
           ...
           if (_swapPairList[to]) {
            ...
            uint256 minBalanceToSell = totalSupply / 10000; 

            if (contractBalance >= minBalanceToSell) {
               if (!inSwap) {

                uint256 swapFee = _buyFundFee + _buyLPFee + _sellFundFee + _sellLPFee;
                swapTokenForFund(contractBalance, swapFee);
            }
            }
    ...
    function swapTokenForFund(
        ...
        uint256 DDAAmount = lpAmount / 2;
        lpAmount -= DDAAmount;  

        if (DDAAmount > 0) {
            swapTokensForDDA(DDAAmount);
        }
    ...
    function swapTokensForDDA(uint256 tokenAmount) private {
    address[] memory path = new address[](3);
    path[0] = address(this);
    path[1] = currency;
    path[2] = 0xB99a061eD71a637bdC129AB3f8c4FA6C80000001;
