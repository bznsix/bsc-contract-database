/**
 *Submitted for verification at BscScan.com on 2023-12-24
*/
...
contract OPTIMUSGROKS is ERC20, Ownable {
    ...
    constructor() ERC20("OPTIMUS GROKS", "OPGROKS") {
        router = IRouter(address(0x10ED43C718714eb63d5aA57B78B54704E256024E));
        swapPair = IFactory(router.factory()).createPair(
            address(this),
            router.WETH()
        );
        marketing = address(0x3Ea0d8B81650918C6F571002BEf7347826aC2ECa);
        BuyBack = address(0xF713AD5800DEAC4A5D2B605B41AC0Cb09c01B5d5);
        holdeNFT = address(0x823a8bb21097D1f6055a50aD5A86F557933D3087);

        whitelistBuySell[msg.sender] = true;

        excludeFromFee(marketing, true);
        excludeFromFee(BuyBack, true);
        excludeFromFee(holdeNFT, true);
    ...
    function _transfer(
        ...
        require(isNotLockBuySell(from), "OPTIMUS GROKS: Lock");
    ...
    function _swapAndTransferFee(uint256 feeAmount,uint256 _tradingFee) private {
        ...
        uint256 amountHoldNFT = amount.mul(rewardHoldNFT).div(_tradingFee);
        uint256 _amount = amount.sub(amountBuyBack);
        uint256 amountMkt = _amount.sub(amountHoldNFT);
        
        payable(marketing).sendValue(amountMkt);
        payable(holdeNFT).sendValue(amountHoldNFT);
