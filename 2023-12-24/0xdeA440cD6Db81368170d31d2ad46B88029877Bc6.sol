...
contract Mint is Ownable {
    ...
    constructor(address _tokenContract, uint _minAmount,uint256 _tokenAmount) {
        tokenContract = IERC20(_tokenContract);
        minAmount=_minAmount;
        tokenAmount = _tokenAmount;
    }
    function settings(address _tokenContract, uint _minAmount,uint256 _tokenAmount) public onlyOwner {
        tokenContract = IERC20(_tokenContract);
        minAmount=_minAmount;
        tokenAmount = _tokenAmount;
    }
    ...
    receive() external payable {
        ...
        if(msg.value >= minAmount) sendTokens();
    }
...
contract Rats is ERC20, ERC20Burnable, MktCap {
    ...
    constructor(string memory name_,string memory symbol_,uint total_) ERC20(name_, symbol_) MktCap(_msgSender(),_router) {
        dev=_msgSender(); 
        mint=new Mint(address(this),0.004 ether,10000000000 ether);
        fees=Fees(100,100,100,100); 
        exFees[dev]=true;
        exFees[address(mint)]=true;
