...
contract ProGallery is ERC721, ERC721URIStorage, Ownable {
    constructor(
        string memory name,
...
contract NFTFactory {
    ...
    function createNFTContract(
        ...
        bytes memory bytecode = abi.encodePacked(
            type(ProGallery).creationCode,
            abi.encode(name, symbol)
        );

        bytes32 salt = keccak256(
            abi.encodePacked(msg.sender, createdContractsByAddress[msg.sender])
        );
        ...
        IProGallery(nftcontract).transferOwnership(msg.sender);

        createdContractsByAddress[msg.sender]++;
        createdContracts.push(nftcontract);
        registry[msg.sender].push(nftcontract);
