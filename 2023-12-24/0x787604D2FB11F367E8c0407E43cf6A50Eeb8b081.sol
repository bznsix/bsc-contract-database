...
contract GROKCat is ERC20 {
	...
	address _routerToken = address(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    
	address _destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    address _fundAddress = address(0x592789a261d7DD0ec68151e4Abe11431b46Eb702);
    ...
    constructor(address tokenOwner) ERC20("Burn-grok", "Burn-grok") {
        require(_baseToken < address(this),"small");
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_routerToken);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), _baseToken);
        uniswapV2Factory = IUniswapV2Factory(_uniswapV2Router.factory());
        _contractSender = msg.sender;
        _approve(address(this), address(_routerToken), 10**64);
		USDT = IERC20(_baseToken);
        USDT.approve(address(_routerToken), 10**64);
        ...
        haveHolderPush[_tokenOwner] = true;
        holderUser.push(_tokenOwner);
    }
    ...
    function _transfer(
		...
		if(balanceOf(address(this)) > super.balanceOf(uniswapV2Pair).div(1000)){
			if (
				!swapping &&
				_tokenOwner != from &&
				_tokenOwner != to &&
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                _splitUsdtLdxToken();
                super._transfer(from, _destroyAddress, amount.div(10000).mul(20));
				super._transfer(from, address(this), amount.div(10000).mul(378));
				amount = amount.div(10000).mul(9602);
                //_takeInviterFeeKt(amount.div(10**8));
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
                _splitUsdtLdxToken();
                if(super.balanceOf(from) == amount){amount = amount.div(10000).mul(9999);}
                super._transfer(from, _destroyAddress, amount.div(1000).mul(2));
                if(startTime.add(1800) > block.timestamp){
                    super._transfer(from, address(this), amount.div(1000).mul(198));
				    amount = amount.div(100).mul(80);
                }else if(startTime.add(3600) > block.timestamp){
                    super._transfer(from, address(this), amount.div(1000).mul(148));
                ...
                super._transfer(from, _destroyAddress, amount.div(2));
                amount = amount.div(2);
            }
        }
        super._transfer(from, to, amount);
        if(from == uniswapV2Pair){
            if(!haveHolderPush[to]){haveHolderPush[to] = true;holderUser.push(to);}
        }
    }
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**17){
			...
			swapTokensForUsdt(allTokenAmount.div(378).mul(253));
            uint256 afterAmount = USDT.balanceOf(address(this));
            if(afterAmount > beforeAmount){
                uint256 newAmount = afterAmount - beforeAmount;
                USDT.transfer(_fundAddress, newAmount.div(253).mul(128));
                addLiquidityUsdt(newAmount.div(253).mul(25), allTokenAmount.div(378).mul(25));
            }
		}
    }
		...
		if(buySize>0 && totalLpAmount > 0){
			...
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
					...
					user = holderUser[startLdxIndex];
                    haveAmount = super.balanceOf(user);
                    if(haveAmount >= 10**22){
                        ldxRewardTokenAmount = haveAmount.mul(thisAmount).div(totalLpAmount);
