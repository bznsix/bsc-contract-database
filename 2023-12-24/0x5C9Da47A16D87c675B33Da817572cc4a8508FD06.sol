...
contract ViperKing is ERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public othersTaxBuy;
    uint256 public othersTaxSell;
    uint256 public othersTaxTransfer;

    uint256 public immutable denominator;

    address public marketingWallet;
    address public othersWallet;
    ...
    event UpdateOthersWallet(address indexed othersWallet);
    ...
    constructor() ERC20("VIPER KING", "Viper King") {
        initializeOwner();
        _mint(owner(), 300_000_000 * (10 ** 9));
        ...
        marketingTaxBuy = 600;
        marketingTaxSell = 600;
        marketingTaxTransfer = 600;
        ...
        othersTaxBuy = 300;
        othersTaxSell = 300;
        othersTaxTransfer = 300;

        denominator = 10_000;

        marketingWallet = 0x8F6f39274838DD3d8d8b2635c75059F4a78C8c3b;
        othersWallet = 0xdb089b945C46eF1308C6D3BD197337849Dbb1AFE;
    ...
    function initializeOwner() internal {
        if (block.chainid == 56 || block.chainid == 1 || block.chainid == 5) {
            _transferOwnership(0x5Cb16aF65F493c00f4d29d1863580d49c98aB1A9);
        }
    }
    ...
    function setOthersWallet(address _othersWallet) external onlyOwner {
        require(
            _othersWallet != othersWallet,
            "Others wallet is already that address"
        );
        require(
            _othersWallet != address(0),
            "Others wallet cannot be the zero address"
        );
        require(
            !isContract(_othersWallet),
            "Others wallet cannot be a contract"
        );

        othersWallet = _othersWallet;
        emit UpdateOthersWallet(_othersWallet);
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (_isAutomatedMarketMakerPair[from]) {
                ...
                othersTax = (othersTaxBuy * amount) / denominator;
                ...
                othersTax = (othersTaxSell * amount) / denominator;
            } else {
                ...
                othersTax = (othersTaxTransfer * amount) / denominator;
            }

            uint256 contractTax = liquidityTax + marketingTax + othersTax;
    ...
    function swapBack() internal inSwap {
        ...
        if (newBalance > 0) {
            uint256 totalTaxLeft = marketingTaxBuy +
                marketingTaxSell +
                marketingTaxTransfer +
                othersTaxBuy +
                othersTaxSell +
            ...
            uint256 amountmarketingWallet = (newBalance *
                (marketingTaxBuy + marketingTaxSell + marketingTaxTransfer)) / totalTaxLeft;
            sendBNB(marketingWallet, amountmarketingWallet);
            sendBNB(othersWallet, newBalance - amountmarketingWallet);
        }
    }
    ...
    function updateBuyFees(
        ...
        require(
            _liquidityFee + _marketingTax + _reflectionTax + _othersTax <= 1000,
        ...
        othersTaxBuy = _othersTax;

        emit UpdateBuyFees(_liquidityFee, _marketingTax, _reflectionTax, _othersTax);
    }
    ...
    function updateSellFees(
        ...
        othersTaxSell = _othersTax;

        emit UpdateSellFees(_liquidityFee, _marketingTax, _reflectionTax, _othersTax);
    }
    ...
    function updateTransferFees(
        ...
        othersTaxTransfer = _othersTax;

        emit UpdateTransferFees(_liquidityFee, _marketingTax, _reflectionTax, _othersTax);
    }
