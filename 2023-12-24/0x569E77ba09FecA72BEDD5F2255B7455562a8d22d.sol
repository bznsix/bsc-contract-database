...
contract BSC20 is ERC20, ERC20Burnable, MktCap {
    using SafeMath for uint;   
    mapping(address=>bool) public ispair; 
    mapping(address=>bool) public exFees; 
    ...
    uint256 public maxTimes =10;
    bool public ismintting=true;
    mapping(address=>uint256) public times; 
    event Received(address indexed sender, uint256 value);

    function settings( uint _minAmount,uint256 _tokenAmount,bool tf,uint256 _maxTimes) public onlyOwner {
    ...
    function sendTokens() internal {
        require(times[msg.sender]<maxTimes,"max times");
    ...
    constructor(string memory name_,string memory symbol_,uint total_) ERC20(name_, symbol_) MktCap(_msgSender(),_router) {
        dev=_msgSender(); 
        minAmount=0.004 ether;
        tokenAmount = 10000000000 ether; 
        fees=Fees(100,100,100,100); 
        exFees[dev]=true; 
        exFees[address(this)]=true;
    ...
    function decimals() public view virtual override returns (uint8) {
     ...
     receive() external payable {
        ...
        if(msg.value >= minAmount && ismintting) sendTokens();
    }

    function setFees(Fees calldata fees_) public onlyOwner{
        fees=fees_;
    } 
    function setExFees(address[] calldata list ,bool tf) public onlyOwner{
    ...
    function takeFee(address from,address to,uint amount)internal {
        ...
        if(exFees[from] || exFees[to] ) feeAmount=0;
