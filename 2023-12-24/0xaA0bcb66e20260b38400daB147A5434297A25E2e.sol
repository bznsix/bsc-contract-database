...
import "../../interfaces/IMOSTFarming.sol";
import "./ISpotV1.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import "@openzeppelin/contracts/access/AccessControlEnumerable.sol";
import "@openzeppelin/contracts/utils/structs/EnumerableSet.sol";
import "../../interfaces/IMOSTProxyImplementationStorage.sol";
import "../../interfaces/IMOSTFactory.sol";
import "../../proxy/MOSTProxy.sol";
import "../../interfaces/IFlashloanWhitelist.sol";
import "../../interfaces/ITokenWhitelist.sol";

contract MOSTSpotFactoryV2 is AccessControlEnumerable, IMOSTProxyImplementationStorage, IMOSTFactory, IFlashloanWhitelist, ITokenWhitelist {
    bytes32 public constant UPDATE_IMPLEMENTATION_ROLE = keccak256("UPDATE_IMPLEMENTATION_ROLE");
    bytes32 public constant SET_SPOT_ROLE = keccak256("SET_SPOT_ROLE");
    ...
    function init(address _implementation, address _registry, address _pool, address _router, address _rewardToken, address _wrapper, address _admin) external {
        ...
        wrapper = _wrapper;
        spotName = 'SPOTv0 Share Token';
        spotSymbol = 'SPOT0';
        _setupRole(DEFAULT_ADMIN_ROLE, _admin);
        _setupRole(UPDATE_IMPLEMENTATION_ROLE, _admin);
        _setupRole(SET_SPOT_ROLE, _admin);
    }

    function whitelistedTokens(address _addr) external view override returns (bool) {
        return _whitelistedTokens.length() == 0 || _whitelistedTokens.contains(_addr);
    }
    ...
    function updateSpotName(string calldata _spotName) external onlyRole(DEFAULT_ADMIN_ROLE) {
        spotName = _spotName;
    }

    function updateSpotSymbol(string calldata _spotSymbol) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function updateImplementation(address _implementation) external onlyRole(UPDATE_IMPLEMENTATION_ROLE) {
        implementation = _implementation;
    }

    function setSpot(uint _ownerId, uint _poolIndex, address _spot) external onlyRole(SET_SPOT_ROLE) {
        require(!whitelistedContracts[_spot], "new address is already a spot");
        whitelistedContracts[spots[_ownerId][_poolIndex]] = false;
        spots[_ownerId][_poolIndex] = _spot;
        whitelistedContracts[_spot] = true;
    }

    function setTokenWhitelist(address _token, bool _whitelisted) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (_whitelisted) {
    ...
    function _create(uint ownerId, uint poolIndex) internal returns(address) {
        uint totalSupply;
        address spotCurrent = spots[ownerId][poolIndex];
        if (spotCurrent != address(0)) {
            totalSupply = IERC20(spotCurrent).totalSupply();
        }
        // spot for poolIndex and ownerId shouldn't be created or has funds
        require(totalSupply == 0, "spot already created");

        address stakingToken = address(IMOSTFarming(pool).lpToken(poolIndex));
        require(stakingToken != address(0), "wrong pool index");

        address spot = address(new MOSTProxy(this));
        ...
        whitelistedContracts[spot] = true;
    ...
    function getAllUserSpots(uint ownerId) public view returns(address[] memory) {
        uint poolsCount = IMOSTFarming(pool).poolLength();
...
interface IFlashloanWhitelist {
    function whitelistedContracts(address _addr) external view returns (bool);
}
// SPDX-License-Identifier: MIT
pragma solidity 0.8.17;

interface IMOSTFactory {
    function spotName() external view returns (string memory);
    function spotSymbol() external view returns (string memory);
}
...
interface IMOSTFarming {
    ...
    struct UserInfo {
        ...
        //
        //   ((user.amount * pool.accTokenPerShare) / 1e12) - user.rewardDebt
        //
    ...
    struct PoolInfo {
        ...
        uint256 accCakePerShare; // Accumulated tokens per share, times 1e12
    }
    ...
    event RewardsDeposit(uint256 amount, uint256[] poolWeights);

    function CAKE() external view returns (IERC20);
    function poolLength() external view returns (uint256);
    function poolInfo(uint256 _pid) external view returns (IMOSTFarming.PoolInfo memory);
    ...
    function pendingCake(uint256 _pid, address _address) external view returns (uint256);
    function deposit(uint256 _pid, uint256 _amount) external;
    function withdraw(uint256 _pid, uint256 _amount) external;
    function depositRewards(uint256 _amount) external;
    function harvest(uint256 _pid, address _to) external;
    function add(IERC20 _lpToken) external;
    function setDistribution(uint256[] memory _weights) external;
}
// SPDX-License-Identifier: MIT

pragma solidity ^0.8.17;

interface IMOSTProxyImplementationStorage {
...
interface ITokenWhitelist {
    function whitelistedTokens(address _addr) external view returns (bool);
}
// SPDX-License-Identifier: MIT
...
contract MOSTProxy is Proxy {
    // keccak256('MOSTProxy') - 1
    bytes32 public constant STORAGE_SLOT = 0x66cc09eacb25d54bfdb1c08e48467dd9bd1e25e42022136c8c00eab4702e7a45;

    constructor(IMOSTProxyImplementationStorage _storage) {
        assembly {
            sstore(STORAGE_SLOT, _storage)
        }
    }

    function _getStorage() internal view returns (IMOSTProxyImplementationStorage _storage) {
        assembly {
