...
//--- Contract v3 ---//
contract BabyGrok is Context, Ownable, IERC20 {
    ...
    address payable private marketingAddress = payable(0x2aeE9fe1b4b74493b72077FfdfeDA0FBEad2a221); 
    address payable private devWallet = payable(0xC73405652597a4C780Bc8cC095525B1ffB63221C); 
    ...
    string constant private _name = "Bad Grok";
    string constant private _symbol = "BADGROK";
    string constant public copyright = "badgrok.com";
    ...
    constructor () {
        ...
        require(buyAllocation + sellAllocation + liquidityAllocation == 100,"BADGROK");
    ...
    if (is_sell(from, to) && !inSwap && canSwap(from, to)) {
        ...
        if (contractTokenBalance >= swapThreshold) {
            if (buyAllocation > 0 || sellAllocation > 0) {
                internalSwap((contractTokenBalance * (buyAllocation + sellAllocation)) / 100);
            }
    ...
    function changeWallets(address newBuy, address newDev) external onlyOwner {
        require(newBuy != address(0),"BADGROK");
        require(newDev != address(0),"BADGROK");
