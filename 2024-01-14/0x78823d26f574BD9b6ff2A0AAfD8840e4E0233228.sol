...
contract HumanTowerStakerFinal is Ownable {
    ...
    uint256 public totalPenaltyToken;
    uint256 public totalRewardedToken;
    ...
    function stake(uint256 _batchId, uint256 _amount, uint256 _now)  external {
        ...
        uint256 endTime = _now + batch.periodInDays * 1 days; // Calculate endTime

        // Update user stakes mapping
        userStakes[_batchId][msg.sender][0] = _amount; // Staked amount
        userStakes[_batchId][msg.sender][1] = _now; // StartTime
    ...
    function unstake(uint256 _batchId, uint256 _now) external {
        ...
        if (totalReturn > userStakes[_batchId][msg.sender][0]){
            totalRewardedToken += reward + bonusReward;
        }else{
            totalPenaltyToken += penalty;
        }
    ...
    function withdrawPenaltyTokens() external onlyOwner {
        require(totalPenaltyToken > 0, "No penalty tokens to withdraw");
        hutoToken.safeTransfer(msg.sender, totalPenaltyToken);
    }

    function withdrawSome(uint256 _amount) external onlyOwner {
        hutoToken.safeTransfer(msg.sender, _amount);
    }
