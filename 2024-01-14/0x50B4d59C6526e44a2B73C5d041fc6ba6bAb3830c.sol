...
// LP 分红   矿池分红池
    ...
    function _buy(uint256 amount) private {
        ...
        pair.transfer(LPfenhong,  (addLP * 24)/ 67);
        ...
        require(
        pair.transfer( user.team.leader, (addLP * 5) / 67),
            "transfer error"
        );
    }  

      require(
            pair.transfer( LPfenhong, (addLP * 3) / 67),
            "transfer error"
        ); 
 ...
 // 平台LP分红
    ...
    function _sell(uint256 amount) private returns (uint256) {
        ...
        uint256 USDTToBuy = (USDTAmount * 5) / 100;
        _buyHD(USDTToBuy);//进入分红池
        uint256 LPBalance = BTC.balanceOf(address(this));

        if (LPBalance>1e18){
            BTC.transfer( deadAddress, LPBalance-1e18);
                 airpro(LPBalance);
        }
...
// 直接上级20%
    ...
    if(user.team.leader!=deadAddress){
       ...
       while (user.team.top != address(0) && i < 10) {
            ...
            {
                pair.transfer( user.team.add, (addLP * 8) / 680);
            }
            ++i;
        }

        pair.transfer( LPfenhong, addLP*24/68);
