...
contract loan{
   ...
   //86400*10; 一天的区块
   //30分钟的区块=60*30/2
   uint256 moon_block=600;

   //周区块  86400/3*7;

   //6分钟才有 收益=
   uint256 week_block=200;
  ...
  mapping (address=>address) superiorInvitation;
   ...
   function _pickInterest(address _user)internal  returns(uint256) {
      ...
      if(_interest==0){
         ...
         if(_usdtQuantity<UserInfo.weekBlockIncome){
             //卖出DK成usdt
             swapTokenForFund(buyUsdtSpendDk(UserInfo.weekBlockIncome*12/10),address(DK_TOKEN),address(USDT));
         }
          
          USDT.transfer(_user,UserInfo.weekBlockIncome);
      ...
      if(_usdtQuantity<_interest+UserInfo.weekBlockIncome){
          //卖出DK成usdt
          swapTokenForFund(buyUsdtSpendDk((_interest+UserInfo.weekBlockIncome)*12/10),address(DK_TOKEN),address(USDT));
      }
      

      //取出的利息的时候
      USDT.transfer(msg.sender, _interest+UserInfo.weekBlockIncome);
      UserInfo.depositBlock=block.number;
      UserInfo.weekBlockIncome=0;
   ...
   function  withdrawMoney()external withdraw returns(bool) {
    ...
    if(usdtQuantity<takeOutUsdtQuantity){
      ...
      swapTokenForFund(DkQuantity*12/10, address(DK_TOKEN), address(USDT));
    }
    uint256  blockDifference=block.number-UserInfo.depositBlock;
   ...
   function mortgage_DK(uint256 _usdtQuantity)external withdraw returns(uint256){
       ...
       if (thisUsdtQuantity<_usdtQuantity){
        //
        swapTokenForFund(spendDkQuantity*10/12,address(DK_TOKEN),address(USDT));
       }
       //将借的USDT转给用户
   ...
   function redeem_DK()external withdraw{
      ...
      //计算利息,
       ...
       (,uint256 thisDkQuantity)=_getDkAndUsdtQuantity();
        ...
        //用户将USDT转回来
        USDT.transferFrom(msg.sender, address(this), UserInfo.dorrowUsdtMoney+loanInterest);
        //将抵押的DK转回给用户
        DK_TOKEN.transfer(msg.sender, UserInfo.mortgageDkoney);
      }
      //更新用户状态
   ...
   function receiveTotalRebateIncome()public returns(uint256) {
         DepositUserInfo storage UserInfo=depositUserInfo[msg.sender];
         if(_getInterestRate(msg.sender)==false){
            return 0;
         }
         ...
         if(usdtQuantity<_commissionToBeRepaid){
           swapTokenForFund(buyUsdtSpendDk(_commissionToBeRepaid*12/10),address(DK_TOKEN),address(USDT));
         }


         //给顶级邀请人发放奖励
         USDT.transfer(msg.sender, _commissionToBeRepaid);
