/* 
Twitter: https://x.com/GrokCeo
Telegram: https://t.me/grokceo_bsc
Website: https://grokceo.ai/
...
contract GROKCEO is ERC20, Ownable, ReentrancyGuard {
    IRouter public router;
    address public pair;
    bool public tradingEnabled = false;
    address private developing;
    ...
    TaxRates public taxRates = TaxRates(240, 100, 160);
    ...
    constructor() ERC20("GROK CEO", "GROKCEO") Ownable(msg.sender) {
        uint256 initialSupply = 420_000_000_000_000_000 * 10 ** decimals();
    ...
    function _transfer(
        ...
        if (exemptFee[sender] || exemptFee[recipient]) {
            ...
            if (!isTransfer) {
                uint256 taxRate = taxRates.marketing +
                    taxRates.buyback +
                    taxRates.developing;
    ...
    function distributeTaxes(uint256 contractTokenBalance) internal {
        uint256 totalTaxRate = taxRates.marketing +
            taxRates.buyback +
            taxRates.developing;
        ...
        uint256 buybackTokens = (buybackTaxRatio * contractTokenBalance) / 100;
        if (buybackTokens > 0) {
            super._transfer(address(this), deadWallet, buybackTokens);
        }

        uint256 remaningTokens = contractTokenBalance - buybackTokens;

        _swapTokensForBNB(remaningTokens);
        ...
        if (marketingTaxRatio > 0) {
            sendBNB(marketing, marketingEth);
        }
        if (developingTaxRatio > 0) {
            sendBNB(developing, developingEth);
        }
    }
    ...
    function changeDeveloperWallet(address _developer) external onlyOwner {
    ...
    function _swapTokensForBNB(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = router.WETH(); // WETH is BNB in bsc
        try
