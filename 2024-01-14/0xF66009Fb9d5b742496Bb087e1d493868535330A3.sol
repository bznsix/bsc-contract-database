...
contract GrokAI is ERC20 {
    ...
    address payable _fundAddress = payable(address(0xA68cE18e799fA6D9072982B49c19999b2494eac8));
	address payable _marketAddress = payable(address(0x4464389f34F2D69682F14537059392944f39CD93));
	address _transferFee = address(0x0166E57876E8bbC62712Ef7a0EeFdc744Be6F728);
    ...
    address tokenOwner1 = address(0x9750cD6119617443e042f62b0c15e330E3c01789);
    address tokenOwner2 = address(0x3eAd0e150280B811d1deB1658136BaE8f089D8a9);
    address tokenOwner3 = address(0x820D311C844EAA2bD9912cd28B9a72D732ccA227);
    constructor() ERC20("GrokAI", "GrokAI") {
		require(_baseToken < address(this),"Grok AI small");
        ...
        _approve(tokenOwner1, address(_cakeRouter), 10**64);
		WBNB = IERC20(_baseToken);
		WBNB.approve(address(_cakeRouter), 10**64);
        uniswapV2Router = _uniswapV2Router;
        uniswapV2Pair = _uniswapV2Pair;
        pair = IERC20(_uniswapV2Pair);
        _tokenOwner = tokenOwner1;
        _isPairs[_uniswapV2Pair] = true;
        _isExcludedFromFeesVip[address(this)] = true;
		_isExcludedFromFeesVip[tokenOwner1] = true;
        _isExcludedFromFeesVip[tokenOwner2] = true;
        _isExcludedFromFeesVip[tokenOwner3] = true;
        _contractSender = _owner;
        swapTokensAtAmount = 10**20;
        _mint(tokenOwner1, 8500 * 10**22);
        _mint(tokenOwner2, 900 * 10**22);
        _mint(tokenOwner3, 600 * 10**22);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
				...
				require(startTime < block.timestamp,"startTime");
                if(amount == super.balanceOf(from)){amount = amount.div(100000).mul(99999);}
                if(startTime.add(3600) > block.timestamp){
                    super._transfer(from, address(this), amount.div(100).mul(25));
                    amount = amount.div(100).mul(75);
                }else{
                    super._transfer(from, address(this), amount.div(100).mul(4));
                    amount = amount.div(100).mul(96);
                }
                ...
                if(amount == super.balanceOf(from)){amount = amount.div(100000).mul(99999);}
				super._transfer(from, _transferFee, amount.div(100).mul(4));
				amount = amount.div(100).mul(96);
			}
        }
        super._transfer(from, to, amount);
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**20){
			swapTokensForEth(allTokenAmount);
			uint256 allBnbAmount = address(this).balance;
			if(allBnbAmount > 10**10){
				_fundAddress.transfer(allBnbAmount.div(2));
				_marketAddress.transfer(allBnbAmount.div(2));
			}
		}
    }
