...
contract Token is ERC20,Ownable,AccessControl {
    ...
    constructor()ERC20("ECG", "ECG") {
        admin=0x3337de3D9B4428333253B91802C47814D586a2aA;
        //admin=msg.sender;
        fundAddr=0x41cf7CA5889E4cF3439da5487c8886cb953E6286;
    ...
    function initPair(address _token,address _swap)external onlyOwner{
        usdt=_token;//0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;//bnb test
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair) {
            ...
            super._transfer(from, address(this), amount.mul(fee).div(100));
            fundCount+=amount.mul(fee).div(100);
            super._transfer(from, to, amount.mul(ofee).div(100));
            return;
        }
        if(to == uniswapV2Pair) {
            if(whiteList[from]){
  ...
  function setFee(uint f,uint _ofee) external onlyOwner {
        fee = f;
        ofee = _ofee;
    }
