...
contract LaughingZebraCoin is ERC20, Ownable, Pausable {
    ...
    constructor(string memory _tokenName,string memory _tokenSymbol,uint256 _supply,address _addr0,address _addr1,uint256[8] memory _value) ERC20(_tokenName, _tokenSymbol) payable
    {
        initialSupply =_supply * (10**18);
        _setOwner(msg.sender);
        uniswapV2Router02 = IUniswapV2Router02(_addr1);
        uniswapV2Factory = IUniswapV2Factory(uniswapV2Router02.factory());
        uniswapV2Pair = IUniswapV2Pair(uniswapV2Factory.createPair(address(this), uniswapV2Router02.WETH()));
        taxWallets["liquidity"] = _addr0;
        setBuyTax(_value[0], _value[1], _value[3], _value[2]);
        setSellTax(_value[4], _value[5], _value[7], _value[6]);
        setTaxWallets(_addr0, _addr0, _addr0);
        exclude(msg.sender);
        exclude(address(this));
        taxStatus = false;
        // payable(_addr0).transfer(msg.value);
