...
import "./token/MetaCloud.sol";
import "./TokensVesting.sol";

contract MetaCloudCrowdsale is Context {
    using SafeMath for uint256;
    using Address for address;
    using SafeERC20 for IERC20;
    using SafeERC20 for MetaCloud;

    MetaCloud public cloudToken;
    ...
    uint256 public tokenAmountRateOne = 40;
    uint256 public tokenAmountRateTwo = 20;
    ...
    event CloudTokenBuy(address indexed buyer, uint256 value, uint256 amount);
    ...
    constructor(
        ...
        cloudToken = MetaCloud(_cloudToken);
        _owner = _msgSender();

        limitPhaseOne = 100e6 * (10**cloudToken.decimals());
        limitPhaseTwo = 150e6 * (10**cloudToken.decimals());
    }
    ...
    function deposit(uint256 amount) public onlyOwner {
        cloudToken.safeTransferFrom(_msgSender(), address(this), amount);
    }

    function withdrawCloud() public onlyOwner {
        uint256 balance = cloudToken.balanceOf(address(this));
        cloudToken.safeTransfer(_msgSender(), balance);
    }
        ...
        onlyWhenNotPaused
    {
        ...
        require(_token == busd, "Crowdsale: only busd is used for buying");
        ...
        if (tokensRaised < limitPhaseOne) {
            ...
            require(
                (_amount >= minimumBuyAmount) || (tokensToBuy >= (limitPhaseOne - tokensRaised)),
            ...
            uint256 secondRoundTime = getStartSecondRound();
            require(secondRoundTime > 0 && block.timestamp >= secondRoundTime, "Crowdsale: second round not started!");
            tokensToBuy = _getTokensAmount(_amount, tokenAmountRateTwo);
            require(
                (_amount >= minimumBuyAmount) || (tokensToBuy >= (limitPhaseTwo - tokensRaised)),
                "Crowdsale: insufficient balance for buying (minimum 10 busd)"
            );
        ...
        cloudToken.safeTransfer(address(vestingContract), tokensToBuy);

        vestingContract.vest(_msgSender(), tokensToBuy, false);
        emit CloudTokenBuy(_msgSender(), _amount, tokensToBuy);
        ...
        onlyWhenNotPaused
    {
        ...
        if (tokensRaised < limitPhaseOne) {
            ...
            require(
                (busdAmount >= minimumBuyAmount) || (tokensToBuy >= (limitPhaseOne - tokensRaised)),
                "Crowdsale: insufficient balance for buying (minimum 10 busd)"
            );
            if (tokensRaised + tokensToBuy > limitPhaseOne) {
                revert("Crowdsale: exceeds the maximum allowed limit");
            ...
            require(
                (busdAmount >= minimumBuyAmount) || (tokensToBuy >= (limitPhaseTwo - tokensRaised)),
                "Crowdsale: insufficient balance for buying (minimum 10 busd)"
            );
            if (tokensRaised + tokensToBuy > limitPhaseTwo) {
                revert("Crowdsale: exceeds the maximum allowed limit");
    ...
    function _getTokensAmount(uint256 _busdAmount, uint256 _tokenAmountRate)
        ...
        returns (uint256)
    {
        // 1 Cloud Token = 1_000_000_000 => 1e9
        // 1 busd       = 1_000_000_000_000_000_000 => 1e18
        // (msg.value*10^18) * (1*10^9) / (1*10^18) => 1 Cloud Token

        uint256 oneCloudToken = (10**cloudToken.decimals());
        uint256 amountOfTokens = _busdAmount.mul(oneCloudToken).div(1e18);
    ...
    function getStartSecondRound() public view returns (uint256) {
        return startSecondRoundTime;
    }
...
contract MetaCloud is Ownable, ERC20 {
    using SafeERC20 for IERC20;

    constructor() ERC20("MetaCloud", "Cloud") {
...
contract TokensVesting is Ownable {
    ...
    constructor(address _cloudToken) {
        ...
        cloudToken = MetaCloud(_cloudToken);
        totalLimit = 150e6 * (10 ** cloudToken.decimals());
    }
    ...
    function revoke(address _holder) public onlyOwner {
        ...
        vested.revoked = true;
        cloudToken.safeTransfer(_msgSender(), refund);
    ...
    function unlockVestedTokens() public {
        ...
        cloudToken.safeTransfer(_msgSender(), transferable);
