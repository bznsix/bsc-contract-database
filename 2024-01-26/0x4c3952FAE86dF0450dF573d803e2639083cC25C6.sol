...
contract BTXJ is IERC20, Ownable {
    ...
    address public dappAddress = 0x9A3E64ED7f5aBEfd093367289e325A8D57A94902;

    constructor() public {
        uint256 total = 2100000 * 10**18;
        _totalSupply = total;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(pairs[sender] || pairs[recipient]) {
            uint256 feeAmount = amount * 3 / 100; // 3%手续费
            uint256 dappAmount = amount * 2 / 100; // 2% DApp费用
            _transferNormal(sender, feeAddress, feeAmount); // 转账3%手续费到 feeAddress
            _transferNormal(sender, dappAddress, dappAmount); // 转账2% DApp费用到 dappAddress
            receiveAmount -= feeAmount + dappAmount; // 更新接收方应接收的金额
        }
