...
contract CrosschainYieldIndex is Ownable2Step, IStargateReceiver {
    using SafeERC20 for IERC20;

    uint256 constant public DENOMINATOR = 100_00;

    uint8 constant public OPERATION_DEPOSIT_REMOTE = 1;

    IStargateComposer public stargateComposer;

    uint256 public withdrawalCommissionNumerator;

    uint256 public feeMultiplierNumerator;

    IStargateRouter.lzTxObj public lzTxObjForDeposit;

    mapping(address token => uint16) public poolId;

    mapping(uint16 poolId => uint16[]) public chainIds;

    mapping(uint16 poolId => mapping(uint16 chainId => uint256)) public allocation;

    mapping(uint16 chainId => address) public remoteCrosschainYieldIndex;

    mapping(uint16 poolId => uint256) public totalPoolAllocation;

    /// @notice total fees
    mapping(address token => uint256) public totalFee;

    /// @notice user`s stargateLP amount by providing token
    mapping(address user => mapping(address token => uint256)) public stargateLP;

    /// @notice total amount of stargate Liquidity Pool token
    mapping(address token => uint256) public totalStargateLP;

    modifier onlyListedToken(address token) {
        require(poolId[token] > 0, "CrosschainYieldIndex: token not listed");
        _;
    }

    constructor(address _stargateComposer)
        //OFT("One Click - Crosschain Yield Index", "1CC-CYI", _lzEndpoint, msg.sender)
    {
        stargateComposer = IStargateComposer(_stargateComposer);
        withdrawalCommissionNumerator = 100; // 1% 
        feeMultiplierNumerator = 150_00;
        lzTxObjForDeposit = IStargateRouter.lzTxObj({
    ...
    function setTokenInfo(address _token, uint16 _poolId, uint16[] memory _chainIds, uint256[] memory _allocation) public onlyOwner {
        require(_chainIds.length == _allocation.length, "CrosschainYieldIndex: invalid length of _chainIds and _allocation");
        poolId[_token] = _poolId;
        chainIds[_poolId] = _chainIds;
        uint256 _totalAllocation = 0;
        for(uint256 i = 0; i < _chainIds.length; i++) {
            _totalAllocation += _allocation[i];
            allocation[_poolId][_chainIds[i]] = _allocation[i];
        }
        totalPoolAllocation[_poolId] = _totalAllocation; 
    }

    function setRemoteCrosschainYieldIndex(uint16 _chainId, address remoteCYI) public onlyOwner {
        remoteCrosschainYieldIndex[_chainId] = remoteCYI;
    }

    function setWithdrawalCommissionNumerator(uint256 _withdrawalCommissionNumerator) public onlyOwner {
        require(_withdrawalCommissionNumerator < DENOMINATOR, "CrosschainYieldIndex");
        withdrawalCommissionNumerator = _withdrawalCommissionNumerator;
    }

    function setFeeMultiplierNumerator(uint256 _feeMultiplierNumerator) public onlyOwner {
        feeMultiplierNumerator = _feeMultiplierNumerator;
    }

    function collectFee(address token) public onlyOwner {
        IERC20(token).safeTransfer(msg.sender, totalFee[token]);
    }

    //---------------------------------------------------------------------------
    // USER FUNCTIONS

    function deposit(address token, uint256 amountLD) public payable {
        depositTo(msg.sender, token, amountLD);
    }

    function depositTo(address to, address token, uint256 amountLD) public payable {
        IERC20(token).safeTransferFrom(msg.sender, address(this), amountLD);
        uint16 _poolId = poolId[token];       
        uint16 _currentChainId = chainIds[_poolId][0];
        uint256 _amountLD = amountLD * allocation[_poolId][_currentChainId] / totalPoolAllocation[_poolId];
        _depositTo(to, token, _amountLD);
        _distributeDeposit(to, token, amountLD);
    }

    function _depositTo(address to, address token, uint256 amountLD) internal onlyListedToken(token) {
        IERC20(token).forceApprove(address(stargateComposer), amountLD);
        uint256 balanceLPbefore = getPoolLPBalance(token);
        stargateComposer.addLiquidity(poolId[token], amountLD, address(this));
        uint256 balanceLPafter = getPoolLPBalance(token);
        require(balanceLPafter > balanceLPbefore, "CrosschainYieldIndex: no minted balance");
        uint256 mintedStargateLP = balanceLPafter - balanceLPbefore;
        
        stargateLP[to][token] += mintedStargateLP;
        totalStargateLP[token] += mintedStargateLP;
    }

    function _distributeDeposit(address depositRecipient, address token, uint256 amountLD) internal {
        uint16 _poolId = poolId[token];
        uint16[] memory _chainIds = chainIds[_poolId];
        uint16 _chainId;
        uint256 _totalAllocation = totalPoolAllocation[_poolId];
        uint256 _amountLD;
        uint256 value;
        bytes memory _payload = encodePayloadForDepisitRemote(depositRecipient);
        for (uint256 i = 1; i < _chainIds.length; i++) {
            _chainId = _chainIds[i];
            bytes memory _to = abi.encodePacked(remoteCrosschainYieldIndex[_chainId]);
            _amountLD = amountLD * allocation[_poolId][_chainId] / _totalAllocation;
            IERC20(token).forceApprove(address(stargateComposer), _amountLD);
            (value, ) = quoteDepositFee(depositRecipient, _chainId);
            stargateComposer.swap{value: value}(
                _chainId, // _dstChainId,
                _poolId, // _srcPoolId,
                _poolId , // _dstPoolId,
                payable(msg.sender), // _refundAddress,
                _amountLD, // _amountLD,
                0,         // _minAmountLD,       
                lzTxObjForDeposit,// _lzTxParams,
                _to, // _to,
                _payload  // _payload
            );
        }
    ...
    function sgReceive(
        ...
        require(remoteCrosschainYieldIndex[_chainId] == address(bytes20(_srcAddress)), "CrosschainYieldIndex: remoteCYI != _srcAddress");
        (uint8 operation, bytes memory nestedPayload) = abi.decode(_payload, (uint8, bytes));
        if (operation == OPERATION_DEPOSIT_REMOTE) {
            address depositReceiver = abi.decode(nestedPayload, (address));
            _depositTo(depositReceiver, _token, _amountLD);
        }
    }

    function withdraw(address token, uint256 amountLP) public {
        withdrawFrom(msg.sender, token, amountLP);
    }

    function withdrawFrom(address from, address token, uint256 amountLP) internal onlyListedToken(token) {
        require(amountLP <= stargateLP[from][token], "CrosschainYieldIndex: more that exist in position");
        stargateLP[from][token] -= amountLP;
        uint256 amountLDbefore = IERC20(token).balanceOf(address(this));
        IERC20(getPoolLP(token)).forceApprove(address(stargateComposer), amountLP);
        stargateComposer.instantRedeemLocal(
            poolId[token],
            amountLP,
            address(this)
        );
        uint256 amountLDafter = IERC20(token).balanceOf(address(this));
        require(amountLDafter > amountLDbefore, "CrosschainYieldIndex: no redeem");
        uint256 redeemAmount = amountLDafter - amountLDbefore;
        uint256 withdrawalFee = redeemAmount * withdrawalCommissionNumerator / DENOMINATOR;
        totalFee[token] += withdrawalFee;
        uint256 withdrawal = redeemAmount - withdrawalFee;
        IERC20(token).safeTransfer(from, withdrawal);
    }

    function encodePayloadForDepisitRemote(address receiver) public pure returns (bytes memory) {
        bytes memory nestedPayload = abi.encode(receiver);
        return abi.encode(OPERATION_DEPOSIT_REMOTE, nestedPayload);
    }

    function quoteDepositFees(address depositRecipient, address token, uint256 amountLD) public view returns (uint256, uint256) {
        uint256 totalNativeTokenForDepositOnLayerZeroFee = 0;
        uint256 totalLayerZeroTokenForDepositOnLayerZeroFee = 0;
        uint16 _poolId = poolId[token];
        
        uint16[] memory _chainIds = chainIds[_poolId];
        uint256 nativeTokenFee;
        uint256 layerZeroTokenFee;
        for (uint256 i = 1; i < _chainIds.length; i++) {
            (nativeTokenFee, layerZeroTokenFee) = quoteDepositFee(depositRecipient, _chainIds[i]);
            totalNativeTokenForDepositOnLayerZeroFee += nativeTokenFee;
            totalLayerZeroTokenForDepositOnLayerZeroFee += layerZeroTokenFee;
        }
        return (totalNativeTokenForDepositOnLayerZeroFee, totalLayerZeroTokenForDepositOnLayerZeroFee);
    }

    function quoteDepositFee(address depositRecipient, uint16 _chainId) public view returns (uint256, uint256) {
        uint256 nativeTokenFee = 0;
        uint256 layerZeroTokenFee = 0;
        uint8 STARGATE_TYPE_SWAP_REMOTE = 1;
        bytes memory _to = abi.encodePacked(remoteCrosschainYieldIndex[_chainId]);
        bytes memory _payload = encodePayloadForDepisitRemote(depositRecipient);
        (nativeTokenFee, layerZeroTokenFee) = stargateComposer.quoteLayerZeroFee(
            _chainId,               // _dstChainId,
            STARGATE_TYPE_SWAP_REMOTE,  // _functionType,
            _to,                    // _toAddress,
            _payload,               // _transferAndCallPayload,
            lzTxObjForDeposit       // _lzTxParams
        );
        return (nativeTokenFee * feeMultiplierNumerator / DENOMINATOR, layerZeroTokenFee * feeMultiplierNumerator / DENOMINATOR);
    }

    function getPoolLPBalance(address token) public view returns (uint256) {
        IStargateFactory factory = IStargateFactory(stargateComposer.factory());
        IStargatePool pool = IStargatePool(factory.getPool(poolId[token]));
        return pool.balanceOf(address(this));
    }

    function getPoolLP(address token) public view returns (address pool) {
        IStargateFactory factory = IStargateFactory(stargateComposer.factory());
        pool = factory.getPool(poolId[token]);
    }
