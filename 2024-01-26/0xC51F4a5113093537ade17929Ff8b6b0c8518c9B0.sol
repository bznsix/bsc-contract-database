...
contract Token is BaseToken {
    ...
    address public _lpReceiver = address(0x3f524f520b2BF0E7Aaa6e8F17738d806d8BE029D);

    constructor() {
        name = "Tyrannosaurus Rex TEST";
        symbol = "Rex TEST";
        decimals = 18;
        ...
        address ReceiveAddress = address(0x9917af1e8E994adfD337Ce14063c2f212c46116A);

        _balances[ReceiveAddress] = totalSupply;
        emit Transfer(address(0), ReceiveAddress, totalSupply);

        fundAddress = payable(0x869CD7359db9C4328D0AD05077e17bF6Dc0AfB1B);
        ...
        _feeWhiteList[0x6189de485Dc0169A44B1eeF947479e1d91c0c72E] = true;
        _feeWhiteList[0xf17b4b5BF3008b33b5e10159a010fb29712Dd12a] = true;
    }
    ...
    bool public startAhead;// Manual：startAhead true,set fee
    function setStartAhead(bool status) public onlyOwner{
        startAhead = status;
    }

    uint256 public _initSellFund = 2000; // 20%
    uint256 public _initSellLp = 2000; // 20%

    uint256 public _finalSellFund = 200; // 2%
    uint256 public _finalSellLP = 200; // 2%

    uint256 public _updateToFinalFeeAt = 60 days;
    ...
    function _updateFee() private {
        ...
        if (!isFinalFee()){
            if (_sellFundFee != _initSellFund){
                _sellFundFee = _initSellFund;
            }
            if (_sellLPFee != _initSellLp){
                _sellLPFee = _initSellLp;
            }
        }else{
            if (_sellFundFee != _finalSellFund){
                _sellFundFee = _finalSellFund;
            }
            if (_sellLPFee != _finalSellLP){
                _sellLPFee = _finalSellLP;
            }
        }

    }

    function isFinalFee() public view returns(bool){
        return block.timestamp > startTime + _updateToFinalFeeAt;
    }

    function testFunction() public onlyOwner{
        lpBurnFrequency = 10 minutes;
        _updateToFinalFeeAt = 6 hours;
    }
    ...
    uint256 public percentForLPBurn = 60;
    event AutoNukeLP(
        uint256 lpBalance,
        uint256 burnAmount,
        uint256 time
    ...
    function Manual() public {
        require(msg.sender == tx.origin);
        require(!inSwap,"0");
        require(lpBurnEnabled,"1");
        require(block.timestamp >= lastLpBurnTime + lpBurnFrequency,"2");
        require(startTime != 0,"3");
        inSwap = true;
        autoBurnLiquidityPairTokens();
        inSwap = false;
    }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (isSell) {
                ...
                if (!startAhead){
                    require(block.timestamp >= startTime + _updateToFinalFeeAt,"cant buy now");
                }
            }
    ...
    // 21,000,000 
    // function toTest() public onlyOwner{
    //     _airdropBNB = 0.1 ether;
    //     startTradeBNBAmount = 0.6 ether;
    //     tB = 0.2 ether;
    //     _basicTransfer(msg.sender, address(this), totalSupply / 2);
    // }
