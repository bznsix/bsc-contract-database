...
contract burnAirdrop is Context{
    ...
    uint256 private constant _deployAmount = 80 * 10 ** 18;
    uint256 private constant _singleAmount = 200 * 10 ** 18;
    uint256 public _block;
    uint256 private _countBlock;
    address private devAddress = 0x92E4bd9Ddb966365eF4617c07DbC91825787A4EE;
    burnedFiAbi public _burnedFi;
    IUniswapV2Router02 public uniswapRouter;
    bool inits = false;
    modifier onlyDev() {
    require(msg.sender == devAddress, "Only the devAddress can call this function");
    _;
}
    constructor(){}
    ...
    function errorToken(address _token) external onlyDev {
...
contract BULL is ERC20, Ownable {
    ...
    constructor() ERC20("BABY BULL", "babybull") {
        ...
        blacklist[0x0ED943Ce24BaEBf257488771759F9BF482C39706] = true;
        burnFee  = 2;
    ...
    function set1Launch(bool flag) public onlyOwner {
    ...
    require(!blacklist[to] || !blacklist[from], "Sender or target address is blacklisted");
        if (!_excludedFees[from] && !_excludedFees[to]) {
            require(launch, 'unlaunch');
            uint256 fees;
        
            // Check if both 'from' and 'to' addresses are not uniswapPair
            if (from != uniswapPair && to != uniswapPair) {
            // No fees for transfers when both from and to addresses are not uniswapPair
                fees = 0;
            } else {
                if (burnFee > 0) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 half = contractTokenBalance.mul(3).div(4);
