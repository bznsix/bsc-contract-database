...
contract CrosschainYieldIndex is Ownable2Step, IStargateReceiver {
    ...
    uint256 constant public COMMISSION_DENOMINATOR = 10000;

    uint8 constant public OPERATION_REMOTE_DEPOSIT = 1;

    IStargateComposer public stargateComposer;

    uint256 public withdrawalCommission;

    mapping(address token => uint16) public poolId;

    mapping(uint16 poolId => uint16[]) public chainIds;

    mapping(uint16 poolId => mapping(uint256 chainId => uint256)) public allocation;
    ...
    function setWithdrawalCommission(uint256 _withdrawalCommission) public onlyOwner {
        require(_withdrawalCommission < COMMISSION_DENOMINATOR, "CrosschainYieldIndex");
        withdrawalCommission = _withdrawalCommission;
    }
    ...
    function depositTo(address to, address token, uint256 amountLD) public payable {
        ...
        uint256 value = msg.value / _chainIds.length;
        uint256 totalValue = msg.value;
        address depositReceiver = to;
        uint256 _amountLD = amountLD * allocation[_poolId][_chainIds[0]] / _totalAllocation;
        _depositTo(depositReceiver, token, _amountLD);
        for (uint256 i = 1; i < _chainIds.length; i++) {
            uint16 _chainId = _chainIds[i];
            ...
            stargateComposer.swap{value: totalValue >= value ? value : totalValue }(
                ...
                _amountLD, // _amountLD,
                0,         // _minAmountLD,       
                IStargateRouter.lzTxObj(500000, 0, "0x"),// _lzTxParams,
                abi.encodePacked(remoteCrosschainYieldIndex[_chainId]), // _to,
                abi.encode(OPERATION_REMOTE_DEPOSIT, abi.encode(depositReceiver)) // _payload
            );
            totalValue -= value;
        }
    }
    ...
    function sgReceive(
        ...
        (uint8 operation, bytes memory nestedPayload) = abi.decode(payload, (uint8, bytes));
        if (operation == OPERATION_REMOTE_DEPOSIT) {
            address depositReceiver = abi.decode(nestedPayload, (address));
            _depositTo(depositReceiver, _token, amountLD);
        }
    }
    ...
    function withdrawFrom(address from, address token, uint256 amountLP) internal onlyListedToken(token) {
        ...
        uint256 withdrawalFee = redeemAmount * withdrawalCommission / COMMISSION_DENOMINATOR;
