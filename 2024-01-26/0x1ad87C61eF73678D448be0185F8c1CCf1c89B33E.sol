...
contract CrowdsaleRAXToken is ICrowdsale, Ownable, Pausable, ReentrancyGuard {
     ...
     */
    constructor(address BUSD_, address USDT_, address USDC_, address RAX_, address pancakeRouter_, address walletFactory_) {
        BUSD = BUSD_;
        USDT = USDT_;
        USDC = USDC_;
        RAX = RAX_;
    ...
    function _buy(address token, uint256 amountIn, uint256 minAmountOut, address referrer) internal {
        ...
        for (uint256 i = 0; i < _vestingManagers.length; ++i) {
            ...
            amountRaxOuts[0] = amountOut;
            amountRaxOuts[1] = (amountOut * 500) / PERCENTAGE_DENOM;
            amountRaxOuts[2] = (amountOut * 300) / PERCENTAGE_DENOM;
            amountRaxOuts[3] = (amountOut * 200) / PERCENTAGE_DENOM;
