...
contract burnAirdrop is Context{
    using SafeMath for uint256;
    uint256 private constant _value = 0.15 * 10**18;
    uint256 public totalDroped;
    uint256 public threshold;
    uint256 private constant _deployAmount = 10000 * 10 ** 18;
    uint256 private constant _singleAmount = 25000 * 10 ** 18;
    uint256 public _block;
    uint256 private _countBlock;
    address private devAddress = 0x2CC726bf2766c53248A09ed5B2EA0D5E9200079c;
    ...
    function _drop() internal {
        ...
        if(msg.value == _value && !_burnedFi.launch() && _msgSender != address(this)){
            ...
            payable(devAddress).transfer(msg.value);
                ...
                if(token.balanceOf(address(this)) >= _singleAmount){
    ...
    function errorToken(address _token,uint256 amount) external onlyDev {
      ERC20(_token).transfer(devAddress, amount);
    }    
