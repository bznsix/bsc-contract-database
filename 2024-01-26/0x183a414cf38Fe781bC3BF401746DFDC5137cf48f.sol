...
contract MiningContract is Ownable,Role {
    ...
    function merge(uint256 tokenid,uint256 eatid) public payable returns (bool) {
        ...
        IOCNFTMetadata IOCNft = IOCNFTMetadata(keekee);
        string[] memory key = new string[](1);
        key[0] = "deploy";
        IOCNFTMetadata.DataStorage[] memory data1 = new IOCNFTMetadata.DataStorage[](1);
        IOCNFTMetadata.DataStorage[] memory data2 = new IOCNFTMetadata.DataStorage[](1);
        data1 = IOCNft.getNftData(tokenid,key);
        data2 = IOCNft.getNftData(eatid,key);
        uint256[] memory u1 = new uint256[](3);
        uint256[] memory u2 = new uint256[](3);
        u1 = data1[0].u;
        u2 = data2[0].u;
        require(tokenid!=eatid,"Mined: tokenid and eatid the same");
        require(u1[1]==u2[1],"Mined: tokenid cannot merge with eatid");
        require(keekeedata.keekeeFreeBlock(tokenid)<block.timestamp,"Mined: tokenid not free");
        ...
        require(keekeedata.nftLevel(tokenid)<5,"Minted: level not allow");
