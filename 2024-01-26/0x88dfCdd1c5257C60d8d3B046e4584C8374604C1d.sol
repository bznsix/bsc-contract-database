...
contract CHARIZARD is ERC20, Ownable {
    ...
    address public marketingxWallet = 0x07b7E02193B9c9fF2DD2eE3b4F79C9DA1C30D649;
    address public devWallet = 0x07b7E02193B9c9fF2DD2eE3b4F79C9DA1C30D649;
    ...
    constructor() ERC20("CHARIZARD INU", "CHARIZARD") {
        ...
        exemptFee[marketingxWallet] = true;
    ...
    function _transfer(
       ...
       if (!exemptFee[sender] && !exemptFee[recipient]) {
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipient])
            ...
            feeswap = sellTaxes.liquidity + sellTaxes.marketingx + sellTaxes.dev;
            feesum = feeswap;
            currentTaxes = sellTaxes;
        } else if (!useLaunchFee) {
            feeswap = taxes.liquidity + taxes.marketingx + taxes.dev;
            feesum = feeswap;
            currentTaxes = taxes;
        } else if (useLaunchFee) {
            feeswap = launchtax;
    ...
    function Liquify(uint256 feeswap, Taxes memory swapTaxes) private lockTheSwap {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 marketingxAmt = unitBalance * 2 * swapTaxes.marketingx;
            if (marketingxAmt > 0) {
                payable(marketingxWallet).sendValue(marketingxAmt);
            }
    ...
    function updateLiquidityTreshhold(uint256 new_amount) external onlyOwner {
        require(new_amount >= 1e5,"Swap threshold amount should be lower or equal to 0.01% of tokens");
    ...
    function updateMarketingxWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0), "Fee Address cannot be zero address");
        marketingxWallet = newWallet;
    }
