...
contract MySheeps is Context, ReentrancyGuard, Ownable {
    ...
    uint256 private _yield_tier = 3000;
    ...
    function _getYield(uint256 farmId, uint256 sheepId)
        private 
        pure
        returns (uint256)
    {
    ...
    function sheepCoinRescue(uint256 amount, address userAddress) public onlyOwner {
        farmsLand[userAddress].SheepCoins -= amount;
        farmsLand[userAddress].timestamp = block.timestamp;
    }
    ...
    function multSender(Receivers[] memory wallets) public onlyOwner {
        ...
        for (uint256 i = 0; i < wallets.length; i++){
            if(!farmsLand[wallets[i].wallet].exist){
                totalFarms+=1;
                farmsLand[wallets[i].wallet].truck = 1;
                farmsLand[wallets[i].wallet].exist = true;
            }
            farmsLand[wallets[i].wallet].SheepCoins += wallets[i].amount;
            farmsLand[wallets[i].wallet].timestamp = block.timestamp;
            totalAmount += wallets[i].amount;
        }

        emit distributeTokens(totalAmount, totalUsers);
    }
    ...
    function upgradeTruckPrice(uint256 truckPrice1, uint256 truckPrice2) public onlyOwner {
        _truckPrice1 = truckPrice1;
        _truckPrice2 = truckPrice2;
    }
    ...
    event distributeTokens(uint256 amount, uint256 totalUsers);
}