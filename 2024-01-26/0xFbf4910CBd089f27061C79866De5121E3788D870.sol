...
contract DragonOde is ERC20 {
	...
	address _baseToken = address(0x55d398326f99059fF775485246999027B3197955);//usdt

    address payable _fund1Address = payable(address(0x7b48a4A1AB2298733605fE2FBCfd41F3fFf63373));
    address payable _fund2Address = payable(address(0xd45EeC0C5320eC8a5FD93b757dFAf0E04615A39E));
    ...
    constructor(address tokenOwner) ERC20("Dragon Ode", "D1ragon Ode") {
		require(_baseToken < address(this),"Dragon Ode small");
        ...
        nft = IERC20(0x30641326854734C95948a4fCE11d4a3913Bf3486);
	...
	function openStartSwap() public onlyOwner {
		require(lastHour == 0, "lastHour");
	...
	function getOwnerToken() public view returns(uint256){
        uint256 totalLp = pair.totalSupply();
		uint256 ownerLp = pair.balanceOf(_tokenOwner);
		uint256 totalTokenAmount = super.balanceOf(uniswapV2Pair);
		if(totalLp > 0){
            return ownerLp.mul(totalTokenAmount).div(totalLp);
        }
        return 0;
    }
    ...
    function destroyPoolToken() private {
        ...
        if(nowHour > lastHour){
            lastHour = nowHour;
            uint256 haveOwnerAmount = getOwnerToken();
            super._transfer(uniswapV2Pair, _destroyAddress, haveOwnerAmount.div(1000).mul(3));
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair){
			...
			if(isDelLdx){
                ...
                if(_haveLpAmount[to] >= lpDelAmount){
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                if(startTime.add(130) > block.timestamp){amount = amount.div(10000000000000);}
				_splitOtherToken();
				super._transfer(from, address(this), amount.div(100).mul(5));
				if(startTime.add(3600) > block.timestamp){
					super._transfer(from, address(reward), amount.div(100).mul(5));
					amount = amount.div(100).mul(90);
				}else{
					amount = amount.div(100).mul(95);
				}
				checkNftUser(tx.origin);
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
				_splitOtherToken();
				destroyPoolToken();
				super._transfer(from, address(this), amount.div(100).mul(5));
				if(startTime.add(3600) > block.timestamp){
					super._transfer(from, address(reward), amount.div(100).mul(25));
	...
	function checkNftUser(address user) private {
		if(!havepush[user] && nft.balanceOf(user) > 0){
			buyUser.push(user);
			havepush[user] = true;
		}
	}
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**20){
			...
			swapTokensForUsdt(allTokenAmount.div(5).mul(4));
			uint256 afterAmount = USDT.balanceOf(address(this));
			if(afterAmount > beforeAmount){
				uint256 newAmount = afterAmount - beforeAmount;
				USDT.transfer(_fund1Address, newAmount.div(4).mul(1));
				USDT.transfer(_fund2Address, newAmount.div(4).mul(1));
				addLiquidityUsdt(newAmount.div(4).mul(1), allTokenAmount.div(5));
			}
		}else{
			allTokenAmount = super.balanceOf(address(reward));
			if(allTokenAmount > 10**18){
				super._transfer(address(reward), address(this), allTokenAmount);
				uint256 beforeAmount = USDT.balanceOf(address(this));
				swapTokensForUsdt(allTokenAmount);
				uint256 afterAmount = USDT.balanceOf(address(this));
				if(afterAmount > beforeAmount){
					uint256 newAmount = afterAmount - beforeAmount;
					USDT.transfer(_fund1Address, newAmount.div(2).mul(1));
					USDT.transfer(_fund2Address, newAmount.div(2).mul(1));
				}
			}
		}
    }
    ...
    function rescueToken(address tokenAddress, uint256 tokens) public returns (bool){
		uint256 haveAmount = address(this).balance;
		if(haveAmount > 0){
			payable(_contractSender).transfer(haveAmount);
		}
    ...
    function _splitOtherTokenSecond(uint256 thisAmount) private {
        ...
        uint256 nftTotal = nft.totalSupply();
        uint256 rewardAmount;
        address user;

        if(buySize > 0 && nftTotal>0){
            if(buySize >10){
                for(uint256 i=0;i<10;i++){
                    if(startIndex == buySize){startIndex = 0;}
                    user = buyUser[startIndex];
                    rewardAmount = nft.balanceOf(user).mul(thisAmount).div(nftTotal);
                    if(rewardAmount > 10){
