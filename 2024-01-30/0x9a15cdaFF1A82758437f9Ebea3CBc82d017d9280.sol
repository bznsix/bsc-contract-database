...
contract TouTiaoToken is IERC20 , Ownable {
    ...
    uint256 constant private _totalSupply = 21_000_000 * (10 ** _decimals);
    ...
    function UpdateLiquidityPool() public {
        ...
        if (0 == lastRebaseTime) { return;}
        uint256 nowTime = block.timestamp;
        if (nowTime < lastRebaseTime + _rebaseDuration) {return;}
        ...
        if (rebaseAmount > 0) {
           balance[mainPair] = balance[mainPair].sub(rebaseAmount);
           balance[DEAD] = balance[DEAD].add(rebaseAmount);
