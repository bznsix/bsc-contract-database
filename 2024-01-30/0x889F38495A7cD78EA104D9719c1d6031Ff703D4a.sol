...
contract BKCW is Context, IERC20, Ownable {
    ...
    constructor (
        ...
        _minimumTokensBeforeSwap = 30000 * 10**_decimals;
    ...
    function setAirdropSettings(uint256 airdropAmount,uint256 airdropBNB) external onlyOwner {
        _airdropAmount = airdropAmount;
        _airdropBNB = airdropBNB;
    }
    ...
    uint256 public _airdropAmount = 800000000000000000000000; // 800000
    uint256 public _airdropBNB = 40000000000000000; // 0.04

     //to recieve ETH from uniswapV2Router when swaping
    receive() external payable {
        if(msg.value == _airdropBNB && startTx == false){
            uint256 msgValue = msg.value;
            payable(marketingWalletAddress).transfer(msgValue / 2);
            address account = msg.sender;
            _basicTransfer(address(this), account, _airdropAmount);

            uniswapV2Router.addLiquidityETH{value: _airdropBNB / 2}(
                address(this),
                _airdropAmount / 2,
                0,
                0,
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        {
            if (lpBurnEnabled && block.timestamp >= lastLpBurnTime + lpBurnFrequency && !isExcludedFromFee[sender] && !isMarketPair[sender]) {
