...
contract BMW is ERC20, Ownable {
    ...
    address public constant treasury = 0x40701eeB022ae17d19852696C131E13160f1DD0B;
    address public marketingAddr0 = 0x3E728FAA5cC2baA020587Fe82E6BfB2C17253994;
    address public marketingAddr1 = 0x6Fa6F5a56bf8Ff278d968FE05258E000b7A07DC6;

    mapping(address => bool) public exemptFee;
    mapping(address => bool) public blclist;

    uint public mintFee = 500 * 1e15;

    uint public lastDeflationTime;
    uint public deflationFee = 3;

    uint public buyBurnFee = 7;
    uint public buyLpDividendFee = 20;
    uint public buyMarketingFee = 15;
    uint public buyLpFee = 8;

    uint public sellBurnFee = 7;
    uint public sellLpDividendFee = 20;
    uint public sellMarketingFee = 210;
    uint public sellLpFee = 8;

    uint256 public numTokensSellToSwap = 200 * 1e18;
    ...
    constructor() ERC20("BMW", "BMW") {
        ...
        tokenReceiver = new TokenReceiver(WETH);
    ...
    receive() external payable {
        ...
        super._transfer(address(this), msg.sender, 1 * 1e18);
    }
    ...
    function setBuyFees(uint _buyBurnFee, uint _buyLpDividendFee, uint _buyMarketingFee, uint _buyLpFee) external onlyOwner {
    ...
    function setSellFees(uint _sellBurnFee, uint _sellLpDividendFee, uint _sellMarketingFee, uint _sellLpFee) external onlyOwner {
    ...
    function _transfer(
        ...
        if(lastPotentialLPHolder != address(0) && !_isLPHolderExist[lastPotentialLPHolder]) {
            uint256 lpAmount = IERC20(pair).balanceOf(lastPotentialLPHolder);
            if(lpAmount > 0) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        if(takeFee) {
            ...
            if(isPair[sender]) { //buy
                ...
                feeToThis = buyLpFee + buyMarketingFee + buyLpDividendFee;
            } else if (isPair[recipient]) {
                feeToBurn = sellBurnFee;
                feeToThis = sellLpFee + sellMarketingFee + sellLpDividendFee;
            } 
    ...
    function swapAndDividend(uint256 tokenAmount) private lockTheSwap {
        uint totalBuyShare = buyLpFee + buyMarketingFee + buyLpDividendFee;
        uint totalSellShare = sellLpFee + sellMarketingFee + sellLpDividendFee;
        ...
        uint256 initialBalance = IERC20(WETH).balanceOf(address(tokenReceiver));
        // make the swap
        ...
        uint256 newBalance = IERC20(WETH).balanceOf(address(tokenReceiver)) - initialBalance;
        uint leftShare = buyMarketingFee + buyLpDividendFee + sellMarketingFee + sellLpDividendFee;
        uint bToMarketing = newBalance * (buyMarketingFee + sellMarketingFee) / leftShare;
        uint half = bToMarketing / 2;
        IERC20(WETH).transferFrom(address(tokenReceiver), marketingAddr0, half);
        IERC20(WETH).transferFrom(address(tokenReceiver), marketingAddr1, half);
        IERC20(WETH).transferFrom(address(tokenReceiver), address(this), newBalance - half * 2);
    }
    ...
    function dividendToLPHolders(uint256 gas) private {
        ...
        uint256 totalRewards = IERC20(WETH).balanceOf(address(this));
        if (totalRewards < 1e15) {
            return;
        }
        ...
        while (gasUsed < gas && iterations < numberOfTokenHolders) {
            ...
            if (LPAmount >= minAmountForLPDividend) {
                ...
                if (dividendAmount > 0) {
                    uint256 balanceOfThis = IERC20(WETH).balanceOf(address(this));
                    if (balanceOfThis < dividendAmount)
                        return;
                    IERC20(WETH).transfer(cur, dividendAmount);
                }
