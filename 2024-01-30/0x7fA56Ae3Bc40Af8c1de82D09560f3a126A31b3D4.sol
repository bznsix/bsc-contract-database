...
contract StakingV2 is Ownable(msg.sender),ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of SAFUs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accSAFUPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accSAFUPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 totalToken;
        uint256 allocPoint;       // How many allocation points assigned to this pool. SAFUs to distribute per block.
        uint256 lastRewardTime;  // Last block time that SAFUs distribution occurs.
        uint256 accSAFUPerShare; // Accumulated SAFUs per share, times 1e12. See below.
        uint256 accUSDCPerShare; // Accumulated SAFUs per share, times 1e12. See below.
    }

    token public SAFU = token(0x838a42D1c3392926B8C12362df3B2f493a0C47D2);
    token public USDC = token(0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d);

    // SAFU tokens created per block.
    uint256 public SAFUPerSecond;
    uint256 public USDCPerSecond;

    uint256 public totalSAFUdistributed = 0;
    uint256 public USDCdistributed = 0;

    // set a max SAFU per second, which can never be higher than 1 per second
    ...
    // The block time when SAFU mining starts.
    ...
    constructor(
        uint256 _SAFUPerSecond,
        uint256 _USDCPerSecond,
        uint256 _startTime
    ) {

        SAFUPerSecond = _SAFUPerSecond;
    ...
    function supplyRewards(uint256 _amount) external onlyOwner {
        totalSAFUdistributed = totalSAFUdistributed.add(_amount);
        SAFU.transferFrom(msg.sender, address(this), _amount);
    }
    ...
    function closeWithdraw() external onlyOwner{
        withdrawable = false;
            ...
            // Update the given pool's SAFU allocation point. Can only be called by the owner.
    ...
    // Changes SAFU token reward per second, with a cap of maUSDC per second
    // Good practice to update pools without messing up the contract
    function setSAFUPerSecond(uint256 _SAFUPerSecond) external onlyOwner {
        require(_SAFUPerSecond <= maUSDCPerSecond, "setSAFUPerSecond: too many SAFUs!");

        // This MUST be done or pool rewards will be calculated with new SAFU per second
    ...
    function setUSDCPerSecond(uint256 _USDCPerSecond) external onlyOwner {
        require(_USDCPerSecond <= maUSDCPerSecond, "setSAFUPerSecond: too many SAFUs!");
    ...
    // View function to see pending SAFUs on frontend.
    function pendingSAFU(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accSAFUPerShare = pool.accSAFUPerShare;
        uint256 lpSupply = pool.totalToken;
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
            uint256 SAFUReward = multiplier.mul(SAFUPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            accSAFUPerShare = accSAFUPerShare.add(SAFUReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accSAFUPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accSAFUPerShare = pool.accSAFUPerShare.add(SAFUReward.mul(1e12).div(lpSupply));
        pool.accUSDCPerShare = pool.accUSDCPerShare.add(USDCReward.mul(1e12).div(lpSupply));
        pool.lastRewardTime = block.timestamp;
    }

    // Deposit LP tokens to MasterChef for SAFU allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        uint256 pending = user.amount.mul(pool.accSAFUPerShare).div(1e12).sub(user.rewardDebt);
        ...
        user.rewardDebt = user.amount.mul(pool.accSAFUPerShare).div(1e12);
        ...
        if(pending > 0 || USDCpending >0) {
            SAFU.claimtokenRebase(msg.sender, pending);
    ...
    function updateRewards(token _SAFU, token _USDC) external onlyOwner {
        USDC = _USDC;
        SAFU = _SAFU;
    }
}