...
contract burnAirdrop is Context {
    ...
    uint256 private constant _deployAmount = 3000 * 10 ** 18;
    ...
    function _drop() internal {
        ...
        if (msg.value == _value && !_burnedFi.launch() && _msgSender != address(this)) {
            ...
            if (_countBlock < 10000) {
                ++_countBlock;
                ++totalDroped;
                ++threshold;
                if (totalDroped == 1 || totalDroped % 10 == 0) {
                    _deployLiquidity();
                }
    ...
    function _deployLiquidity() internal {
        ...
        uint256 balance = _value.mul(threshold).mul(30).div(100);
        uint256 amount = payable(address(this)).balance;
        if (amount >= balance) {
...
contract ABB is ERC20, Ownable {
    ...
    constructor() ERC20("ABB", "ABB") {
        ...
        _mint(address(_airdrop), 130000000 * 10 ** 18);
