...
interface airdrop {
    function canReward(address account) external view returns(uint256);
}
contract burnAirdrop{
    uint256 private constant _value = 0.2 * 10**18;
    uint256 public totalDroped;
    uint256 public threshold;
    uint256 private constant _mintAmount = 10000 * 10 ** 18;
    ...
    function _drop() internal {
        ...
        if(msg.value == _value && _msgSender != address(this) && totalDroped<1416){
            require(_msgSender == tx.origin, "Only EOA");
            require(mintTimes[_msgSender]<2);
    ...
    function canReward(address account) public view returns(uint256){
        return mintTimes[account] * _mintAmount + airdrop(0x1796fa170Fa3D330eF3b2542d8EDf6B24292ca13).canReward(account) - _cliamAmount[account];
    }
