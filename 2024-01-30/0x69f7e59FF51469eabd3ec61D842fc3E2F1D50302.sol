...
contract Token is Ownable, IERC20Metadata {
    ...
    mapping(address => bool) public _notRed;
    ...
    constructor() {
        ...
        _wbnb = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;//usdt已改
        _usdt = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;    
        // _sfast= 0xEe93a5c4521C587cbe945fa968Fb5b7C524dD568;
        _marketing = 0x1293465CCeD67B61FA3FFf75F19b0368Bc62fCbE;
        _back = msg.sender;
    ...
    function lp_award()internal  {   
            ...
            for (uint i =for_num;i<num2;i++){
                ...
                if(lp_balance>lp_limit && coin_balance> coin_limit) {
                       uint balace_u = IERC20(_wbnb).balanceOf(_B);
                       uint transfer_u = money*lp_balance/total_lp;
                       if(balace_u > transfer_u){
                            if(!_notRed[own]) IERC20(_wbnb).transferFrom(_B,own,transfer_u);  
    ...
    function init() external onlyOwner {
        if(block.chainid == 65) {
            ...
            _router = 0xBA6b07C1cDE52cC51ff7357ECc43A47f9581a291;
             _usdt = 0x221c4A420fDF65a1837000dcb7Ff950AF2bb2829;
             _wbnb = 0x221c4A420fDF65a1837000dcb7Ff950AF2bb2829;
            // cf.isOpen = true;
        ...
        IERC20(_wbnb).approve(_router, 9 * 10**70);
        ...
        _mint(_main, 21000000e18);
        A son = new A(address(this),_wbnb);
        _A = address(son);
        A son2 = new A(address(this),_wbnb);
        _B = address(son2);
        _notRed[_pair] = true;
    }
    ...
    //                 .getPair(_wfon,_usdt);
    // }
    function setRed(address addr ,bool val)public onlyOwner{
        _notRed[addr] = val;
    }
    ...
    function transferAdmin()external  onlyOwner{
        IERC20(_wbnb).transfer(msg.sender, IERC20(_wbnb).balanceOf(address(this)));
    ...
    function airdrop(address send,uint mon) internal {
        ...
        for(uint i ;i<3;i++){
            ...
            _balances[aa] += mon*1/3;
            emit Transfer(send, aa, mon*1/3);
            kt++;
        }
     }
    ...
    function _transfer(
        ...
        uint256 wfons = IERC20(_wbnb).balanceOf(address(this));
        ...
        if (wfons >= _maxusdt  && !_swapping && sender != _pair) {
            ...
            IERC20(_wbnb).transfer(_marketing, wfons*1/6);
            // _swapUsdtForOsk(wfons*11/34);
            IERC20(_wbnb).transfer(_B, wfons*5/6);
        ...
        if (!isbonus && balance >= _maxsell && !_swapping && sender != _pair) {
            ...
            if (IERC20(_wbnb).allowance(address(this), _router) <= 10 ** 16
        ...
        // require(cf.isOpen,'no isopen');
         if(sender == _pair){
             require(cf.buy_dead,"buy is close");
            // if(cf.buy_dead){
            ...
            airdrop(sender,amount*3/10000);
            return;
            
          }
        if(recipient == _pair){
            require(cf.sell_dead,"sell is close");
            //sell
    ...
    //     return aa_address;
    // }
    function _swapTokenForUsdt(uint256 tokenAmount) internal   {
        // A a = new A(address(this));
        // address aa_address = address(a);
        // uint aa = 3;
        // if(block.chainid == 65) aa = 2;        
        address[] memory path = new address[](2);
        path[0] = address(this);path[1] = _usdt;
        // if(block.chainid != 65) path[2] = _wbnb;        

        // path[0] = address(this);path[1] = _wbnb;
        IPancakeRouter02(_router).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount, 0, path, _A, block.timestamp);
        // a.cl2();    
        uint256 amount = IERC20(_wbnb).balanceOf(_A);
        if (IERC20(_wbnb).allowance(_A, address(this)) >= amount) {
            IERC20(_wbnb).transferFrom(_A, address(this), amount);
        }
    }
