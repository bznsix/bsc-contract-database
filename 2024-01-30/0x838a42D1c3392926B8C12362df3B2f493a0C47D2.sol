...
Defi Hedge Fund with disruptive debase mechanism $SAFU

Telegram: https://t.me/SafuOnBSC
Twitter: https://twitter.com/SafuOnBSC
Website: https://safuonbsc.com/
Docs: https://safuonbsc.gitbook.io/
Dapp: https://dapp.safuonbsc.com/
...
// File: contracts/safu.sol

pragma solidity ^0.8.0;

// Storage for a safu token
contract safu {
...
// File: contracts/Isafu.sol

pragma solidity ^0.8.0;

abstract contract Isafu {
    /**
     * @notice Event emitted when tokens are rebased
     */
    event Rebase(
        uint256 epoch,
        uint256 prevsafusScalingFactor,
        uint256 newsafusScalingFactor
    );
...
contract SAFU is ERC20PresetMinterRebaser, Ownable, Isafu {
    using SafeMath for uint256;
     ...
     * @notice Scaling factor that adjusts everyone's balances
     */
    uint256 public safusScalingFactor;
 
    mapping(address => uint256) internal _safuBalances;
    ...
    constructor() ERC20PresetMinterRebaser("Safu", "SAFU") {
        safusScalingFactor = BASE;
        initSupply = _fragmentTosafu(INIT_SUPPLY);
        _totalSupply = INIT_SUPPLY;
        _safuBalances[owner()] = initSupply;
    ...
    function _maxScalingFactor() internal view returns (uint256) {
        // scaling factor can only go up to 2**256-1 = initSupply * safusScalingFactor
        // this is used to check if safusScalingFactor will be too high to compute balances when rebasing.
    ...
    function _mint(address to, uint256 amount) internal override {
        ...
        uint256 safuValue = _fragmentTosafu(amount);

        // increase initSupply
        initSupply = initSupply.add(safuValue);

        // make sure the mint didnt push maxScalingFactor too low
        require(
            safusScalingFactor <= _maxScalingFactor(),
            "max scaling factor too low"
        );

        // add balance
        _safuBalances[to] = _safuBalances[to].add(safuValue);
    ...
    function _burn(uint256 amount) internal {
        ...
        // decrease initSupply
        initSupply = initSupply.sub(safuValue);

        // decrease balance
        _safuBalances[msg.sender] = _safuBalances[msg.sender].sub(safuValue);
    ...
    function _mintUnderlying(address to, uint256 amount) internal {
        ...
        // get external value
        uint256 scaledAmount = _safuToFragment(amount);
        ...
        // add balance
        _safuBalances[to] = _safuBalances[to].add(amount);
        ...
        returns (bool)
    {
        ...
        _safuBalances[msg.sender] = _safuBalances[msg.sender].sub(value);

        // add to balance of receiver
        _safuBalances[to] = _safuBalances[to].add(value);
        emit Transfer(msg.sender, to, _safuToFragment(value));
        return true;
    }
    ...
    function transfer(address to, uint256 value)
        ...
        returns (bool)
    {
        // underlying balance is stored in safus, so divide by current scaling factor

        // note, this means as scaling factor grows, dust will be untransferrable.
        // minimum transfer value == safusScalingFactor / 1e24;

        // get amount in underlying
        uint256 safuValue = _fragmentTosafu(value);
    ...
    function transferFrom(
        ...
        // sub from from
        _safuBalances[from] = _safuBalances[from].sub(safuValue);
        _safuBalances[to] = _safuBalances[to].add(safuValue);
    ...
    function balanceOf(address who) public view override returns (uint256) {
        return _safuToFragment(_safuBalances[who]);
    }
    ...
    function permit(
        ...
        require(block.timestamp <= deadline, "safu/permit-expired");
        ...
        require(owner != address(0), "safu/invalid-address-0");
        require(owner == ecrecover(digest, v, r, s), "safu/invalid-permit");
    ...
    function rebase(
        ...
        // no change
        if (indexDelta == 0) {
            emit Rebase(epoch, safusScalingFactor, safusScalingFactor);
            return _totalSupply;
        }

        // for events
        uint256 prevsafusScalingFactor = safusScalingFactor;

        if (!positive) {
            // negative rebase, decrease scaling factor
            safusScalingFactor = safusScalingFactor
                .mul(BASE.sub(indexDelta))
                .div(BASE);
        } else {
            // positive rebase, increase scaling factor
            uint256 newScalingFactor = safusScalingFactor
                .mul(BASE.add(indexDelta))
                .div(BASE);
            if (newScalingFactor < _maxScalingFactor()) {
                safusScalingFactor = newScalingFactor;
            } else {
                safusScalingFactor = _maxScalingFactor();
            }
        }
        ...
        _totalSupply = _safuToFragment(initSupply);
        ...
        emit Rebase(epoch, prevsafusScalingFactor, safusScalingFactor);
        return _totalSupply;
    }
    ...
    function safuToFragment(uint256 _safu) public view returns (uint256) {
        return _safuToFragment(_safu);
    }

    function fragmentTosafu(uint256 value) public view returns (uint256) {
        return _fragmentTosafu(value);
    }

    function _safuToFragment(uint256 _safu) internal view returns (uint256) {
        return _safu.mul(safusScalingFactor).div(internalDecimals);
    }

    function _fragmentTosafu(uint256 value) internal view returns (uint256) {
        return value.mul(internalDecimals).div(safusScalingFactor);
    }
