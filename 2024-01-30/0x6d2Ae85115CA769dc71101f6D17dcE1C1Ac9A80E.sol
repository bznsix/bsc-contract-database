...
/**
TELEGRAM : https://t.me/BabyDragonGrokV2
...
contract BabyDragonGrokV2 is Context, IERC20, Ownable {
    ...
    string private constant _name = "BabyDragonGrokV2";
    string private constant _symbol = "BDG";
    uint256 private _tTotal = 1_000_000_000 * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    
    uint256 public swapTokensAtAmount = 3_000_000 * 10**_decimals; // Should be between 0.03% to 0.2%  of the total supply
    // Max Transaction Amount
    uint256 public maxTxAmount = 30_000_000 * 10**_decimals; 
    // Max Wallet Amount
    uint256 public maxWalletBalance = 30_000_000 * 10**_decimals; 

    address public marketingAddress = 0xF0d6bF71Ffe6fb091d56EDAC7df4a89d5F6B1493; // Marketing Fee Receiver
    ...
    Taxes public Buytaxes = Taxes(0,15,5);
    Taxes public sellTaxes = Taxes(0,15,5);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && !swapping){
            if(from == pair){
                require(amount <= maxTxAmount, "You are exceeding maxBuyAmount");
            }
            if(to == pair){
                require(amount <= maxTxAmount, "You are exceeding maxSellAmount");
            }
            if(to != pair){
