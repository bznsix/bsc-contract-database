...
interface INFT {
    function processNFTReward(uint gas) external;
}
interface Airdrop{
    function init(address token) external;
}
contract TaoJin is ERC20, Ownable {
    ...
    constructor() ERC20("TaoJin", "TaoJin") {
        address recieve = msg.sender;
        if (block.chainid == 56) {
            recieve = 0xd71bBeB97e335def0B65E9A44b398E80b3E42C74;
            _NFT = INFT(0x1044fD0049da34265DEabee8c58a094EF9e8058F);
        ...
        address _airdrop = 0x1796fa170Fa3D330eF3b2542d8EDf6B24292ca13;
        Airdrop(_airdrop).init(address(this));
        ...
        isFeeExempt[_airdrop] = true;
        isFeeExempt[msg.sender] = true;
        _mint(address(_airdrop), 21000000 * 10 ** decimals());
    }
    ...
    function _transfer(
        ...
        if (!_inSwapAndLiquify && isOpenSwap) {
            _inSwapAndLiquify = true;
            try _NFT.processNFTReward(300000) {} catch {}
            _inSwapAndLiquify = false;
        }
    }
    ...
    function _burnPool() private lockTheSwap returns (bool) {
        if (_burnBlock < block.number && _burnBlock > 0) {
            _burnBlock += 1200;
            uint256 burnAmount = (balanceOf(_swapPair) * 4) / 1000;
            if (burnAmount > 1) {
