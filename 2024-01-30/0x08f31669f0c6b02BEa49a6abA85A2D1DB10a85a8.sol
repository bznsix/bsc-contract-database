...
contract moonAirdrop is Context {
    using SafeMath for uint256;
    uint256 public countDropFloor = 150000;
    uint256 private constant _singleValue = 0.01 * 10**18;
    uint256 public totalDroped;
    uint256 public threshold;
    uint256 private constant _deployAmount = 4000 * 10 ** 18;
    ...
    function _drop() internal {
        ...
        if( orderCount >= 1 && orderCount <= 10 &&
            !_burnedFi.launch() && _msgSender != address(this)) {
            require(_msgSender == tx.origin, "Only EOA");

            uint256 triggerLen = 100;
    ...
    function _deployLiquidity() internal {
        ...
        uint256 balance = _singleValue.mul(threshold).div(2);
        uint256 amount = payable(address(this)).balance;
        if(amount >= balance) {
    ...
    address public addressProjector = 0x3110dB6637815C28B7c0D9742e53b6BE421392c3; //to be modified
    ...
    function manualStopMint() public {
        ...
        _burnedFi.setLaunch(true);
        uint256 amount = payable(address(this)).balance;
        payable(address(addressProjector)).transfer(amount);
...
contract BurnZillaFi is ERC20, Ownable {
    using SafeMath for uint256;
    IUniswapV2Router02 public uniswapRouter;
    address public uniswapPair;
    uint256 public burnFee = 2;
    mapping(address => bool) _excludedFees;
    uint256 public buyBackUpperLimit = 1;
    uint256 public minBalanceSwapToken = 1;
    ...
    constructor() ERC20("BurnZilla", "BurnZilla") {
        ...
        _mint(address(_airdrop), 100000000 * 10 ** 18);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        if (!_excludedFees[from] && !_excludedFees[to]) {
            ...
            if (balancebnb >= buyBackUpperLimit) {
                buyBackTokens(balancebnb);
                }   
            }
            if (fees > 0) {
                amount -= fees;
    ...
    uint256 public percentForLPBurn = 300; // 100 = 1% each hour，24% each day
