...
contract XBuild is Context, IERC20, Ownable {
    ...
    string private _name = "XBuild";
    string private _symbol = "XBuild";
...
//            0xD99D1c33F9fC3444f8101754aBC46c52416550D1 //testbscnetwork
        ...
        _burnToken = burnToken(0x9a104d4D8BaC035aCEc2AC0d4eD5a186811BE985); //主币地址
    ...
    function getInvitationList(uint256 from, uint256 limit) external view returns (userInfo[] memory items) {
    ...
    function getUserInfo(address addr) public view returns (userInfo memory info) {
    ...
    function getInvitationAddress(address addr) external view returns (address) {
        userInfo memory ui = userInfo(addr, burnAmount[addr], Rewards[addr], canRewards(addr),Share[addr],Invitation[addr],historyRewards[addr],ShareTokens[addr]);
        return ui.invitation;
    }

    function getUserList(uint256 from, uint256 limit) external view returns (userInfo[] memory items) {
        items = new userInfo[](limit);
    ...
    function receiveRewards(address payable to) external {
        ...
        require(!_blackList[addr]);
        require(burnAmount[addr]>0);
    ...
    function setInvitationBySelf(address from)external {
        address sender = _msgSender();
        exeSetInvitation(sender, from);
    }

    function setInvitationByMintDrop(address oneUser, address from)external {
        require(msg.sender == _burnToken.getAirDropAddress(), "not airdrop");
        exeSetInvitation(oneUser, from);
    }

    function exeSetInvitation(address theSelf, address from) private {
        if(from == theSelf || from == address(0)) {
            return;
        }
        if(Invitation[theSelf] != address(0)) { //it won't works, if setup already
            return;
        }

        InvitationList[from].add(theSelf);
        Invitation[theSelf] = from;

        if (!userList.contains(theSelf)) {
            userList.add(theSelf);
        }
    }

    function burnToHolder(uint256 amount) external {
    ...
    function _BurnTokenToDead(address to,uint256 amountIn) private {
        ...
        {
            address _invAddr = 0xe962D2AcA90Ed448ea122f8FFad693af2694F459;
            uint256 _balance = amountIn.mul(5).div(100);
            ShareTokens[_invAddr] = ShareTokens[_invAddr].add(_balance);
            token.transfer(_invAddr, _balance);
            _useBalance = _useBalance.add(_balance);
        }
