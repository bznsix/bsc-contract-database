...
contract  GHT is IERC20, Ownable {
    ...
    constructor(){
        ...
        _symbol = "GHT";
        _decimals= uint8(18);
        _tTotal = 100000 * (10**uint256(_decimals));
        ...
        _marketAddr =  0x0775F6E0028B035D18f3364d27C4c0Ce6cb32A09;
        address admin = 0xD1de534ebaB3982Fe35d45c9501b0dA945EFf6a6;
        address router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
        _token = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
        root = 0x065C82D14B76342C543b15d7dc89CD1f0ED7E5aA;
        _rewardToken = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
        _nft = 0x550FAf3fb67812aA8c369d1Ab6e5be695EdaeCAd;
        if(block.chainid == 97) {
...
bool public _enableMint = true;
    ...
    receive() external payable {
        require(_enableMint);
        uint num = msg.value/price;
        _basicTransfer(root,msg.sender,tokenNums*num );
        payable(_marketAddr).transfer(msg.value);
    }



    uint public tokenNums =30e18 ;
    function setTokenNums(uint value) external onlyOwner {
        tokenNums = value;
    }
   ...
   function getExcludedFromFee(address account) public view returns (bool) {
    ...
    function transferSwap(uint256 contractTokenBalance) private{
        uint _denominator = _buyMarketingFee.add(_sellMarketingFee).add(_buyLpFee).add(_sellLpFee).add(_buyNFTFee).add(_sellNFTFee);
        if(_denominator>0){
            ...
            if(_token==_rewardToken){
                ...
                try IERC20(_rewardToken).transfer(_nftRouter,  tokenBal.mul(_buyNFTFee.add(_sellNFTFee)).div(_denominator)) {} catch {}
                ...
                try IERC20(_rewardToken).transfer(_nftRouter,  lpDivBal.mul(_buyNFTFee.add(_sellNFTFee)).div(_denominator.sub(_buyLiquidityFee).sub(_sellLiquidityFee)  )) {} catch {}
                try IERC20(_rewardToken).transfer(_lpRouter,  IERC20(_rewardToken).balanceOf(address(this))) {} catch {}
            }
        }
    }
