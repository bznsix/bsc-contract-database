...
contract FixedInterestBulletLoans is Base, ERC721, IFixedInterestBulletLoans {
    ...
    function currentUsdValue(uint256 loanId) public view override returns (uint256) {
        ...
        return _convertToUSD(loan.krwPrincipal + interestInKRW, loan.asset.decimals());
    }
    ...
    function expectedUsdRepayAmount(uint256 loanId) public view override returns (uint256) {
        ...
        return _convertToUSD(loan.krwPrincipal + fullInterestInKRW, loan.asset.decimals());
    }
    ...
    function setCurrencyConverter(ICurrencyConverter _currencyConverter) external {
        _requireManagerRole();
        currencyConverter = _currencyConverter;
        emit CurrencyConverterChanged(address(_currencyConverter));
    }
        ...
        returns (uint256 principal)
    {
        ...
        LoanMetadata storage loan = loans[loanId];
        principal = _convertToUSD(loan.krwPrincipal, loan.asset.decimals());
    ...
    function _convertToUSD(uint256 wonAmount, uint8 decimals) internal view returns (uint256) {
        return currencyConverter.convertToUSD(wonAmount, decimals);
    }
...
library MathUtils {
    uint256 public constant BASIS_PRECISON = 10_000;
