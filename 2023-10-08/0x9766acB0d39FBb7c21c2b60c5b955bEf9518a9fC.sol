...
contract NewVsageNFT is ERC1155Upgradeable {
    ...
    constructor() {
        ...
        _mint(topAddress, 0, 1, "");
        _mint(topAddress, 1, 1, "");
        _mint(topAddress, 2, 1, "");

        tokenList[0] = NFT({mintRate: 80, sellRate: 80, amount: 3 * 1e18, profitMultiple: 6});
        tokenList[1] = NFT({mintRate: 80, sellRate: 80, amount: 6 * 1e18, profitMultiple: 6});
        tokenList[2] = NFT({mintRate: 80, sellRate: 80, amount: 12 * 1e18, profitMultiple: 6});
    ...
    function open(uint256 tokenId) external {
        ...
        //当前账户金币>0 ， 全部售出
        if (userList[tokenId][msg.sender].coin > 0) {
            redeem(tokenId, 0, msg.sender);
        }

        uint256 amount = tokenList[tokenId].amount;
        //从付款人的账户中转移代币
        IERC20Upgradeable(payToken).safeTransferFrom(msg.sender, address(this), amount);
    ...
    function redeem(uint256 _tokenId, uint _retentionRatio, address userAddress) private {
        ...
        //保留比例分母
        require(_retentionRatio < 10000, "Retention ratio must be less than 100");

        NFT memory nft = tokenList[_tokenId];
        require(nft.amount > 0, "Amount not set");

        //计算实际扣除金币数量 和 最终获得Mc
        (uint256 actualSaleCoin, uint256 finallySellAmount) = getRedeemAmount(_tokenId, _retentionRatio, userAddress);

        require(user.coin >= actualSaleCoin, "The actual quantity sold is wrong");
        //扣除账户金币
        user.coin -= actualSaleCoin;

        if (userAddress != topAddress) {
            //最大可获利金额 = 开启权益金额 * 倍数
            uint256 maxProfit = nft.amount.mul(nft.profitMultiple);
            //获利金额 + 本次获利 > 总金额
            uint256 allProfit = user.profit.add(finallySellAmount);
            if (allProfit >= maxProfit) {
                finallySellAmount = maxProfit.sub(user.profit);
                //设置可重新激活
        ...
        //发送代币
        IERC20Upgradeable(payToken).safeApprove(address(this), finallySellAmount);
        IERC20Upgradeable(payToken).safeTransferFrom(address(this), userAddress, finallySellAmount);
        //扣除金额池子
        tokenPool -= finallySellAmount;
        //扣除币池子
        coinPool -= actualSaleCoin;
    ...
    function getRedeemAmount(
        uint256 _tokenId,
        uint _retentionRatio,
        address userAddress
    ) public view returns (uint256 actualSaleCoin, uint256 finallySellAmount) {
        User memory user = userList[_tokenId][userAddress];
        require(user.coin > 0, "The number of gold coins is 0");
        ...
        if (userCoinValue <= allAmount) {
            (actualSaleCoin, finallySellAmount) = calculateSellAmount(user.coin, _retentionRatio, tokenPrice, nft.sellRate);
        } else {
    ...
    function calculateSellAmount(
        ...
        //保留金币数量
        uint256 reserveCoin = userCoin.mul(_retentionRatio).div(denominator);
        //实际扣除金币数量
        uint256 actualSaleCoin = userCoin.sub(reserveCoin);
        //最终输出金币价值 = 实际扣除金币数量 * 单价 * 手续费
        uint256 finallySellAmount = actualSaleCoin.mul(tokenPrice).div(1e18).mul(sellRate).div(100);
        return (actualSaleCoin, finallySellAmount);
    }
    ...
    //设置已获利金额
    function setUserProfit(address userAddress, uint256 _tokenId, uint256 profit) external onlyOwner {
        User storage user = userList[_tokenId][userAddress];
        user.profit = profit;
    }
...
contract NewVsage is AccessControlUpgradeable, UUPSUpgradeable {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    // 初始化顶级账户
    function setTopUser(address _topAddress) public onlyOwner {
        topAddress = _topAddress;
        User storage topUser = users[topAddress];
        topUser.id = 1;
        topUser.referrer = address(0);
        topUser.partnersCount = 0;
        for (uint8 j = 0; j < matrixList.length; j++) {
            topUser.activeLevelsFlag[j] = true;
        }
        idToAddress[1] = topAddress;
    }
    ...
    function buyNewLevel() external {
        ...
        //从付款人的账户中转移代币
        IERC20Upgradeable(payToken).safeTransferFrom(msg.sender, address(this), matrixInfo.price);
    ...
    function sendETHDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        //向上级发送代币
        IERC20Upgradeable(payToken).safeApprove(address(this), dividendAmount);
        IERC20Upgradeable(payToken).safeTransferFrom(address(this), receiver, dividendAmount);

        //向NFT发送代币
        IERC20Upgradeable(payToken).safeApprove(address(this), precipitationAmount);
        IERC20Upgradeable(payToken).safeTransferFrom(address(this), nft, precipitationAmount);
        //增加币池
