...
contract TunaLayerV2 is Ownable, ReentrancyGuard 
{
    ...
    uint256[4] internal PERCENTS = [104, 120, 140, 0];
    uint256[4] internal PERCENTS_WITH_TUNA = [107, 130, 170, 270];
    uint256[4] internal AMOUNT_TUNA = [200, 300, 400, 700];

    uint256[4] internal DEPOSIT_PERIODS = [1, 3, 5, 10];
    uint256[4] internal DEPOSIT_LIMITS_MIN = [20, 50, 100, 200];
    uint256[4] internal DEPOSIT_LIMITS_MAX = [300, 500, 1000, 2000];

    uint256[2] internal AFFILIATE_PERCENTS_FIRST = [20, 10]; // 2%-1%
    uint256[2] internal AFFILIATE_PERCENTS_OTHER = [50, 20]; // 5%-2%
    ...
    function Deposit(uint256 amount, address partner, uint8 plan, uint8 mode) external nonReentrant 
    {
        ...
        require(amount >= (DEPOSIT_LIMITS_MIN[plan] * 10**18), "Min deposit limit");
        require(amount <= (DEPOSIT_LIMITS_MAX[plan] * 10**18), "Max deposit limit");
        ...
        uint256 percentage = (amountTuna == 0) ? PERCENTS[plan] : PERCENTS_WITH_TUNA[plan];
        ...
        user[_msgSender()].invested += amount;
    ...
    function TransferTuna(uint256 amount, address tokenAddress) external contractEnded
    {
        tokenUse = IERC20(tokenAddress);
    ...
    function _updateprePayment(address account) internal 
    {
        ...
        if(count_deposits > 0)
        {
            for(uint8 i; i < count_deposits; i++)
            {
                ...
                {
                    if(deposits[account][i].status == false)
                    {
