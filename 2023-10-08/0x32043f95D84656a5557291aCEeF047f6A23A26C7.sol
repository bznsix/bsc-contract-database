...
contract DMSTest is IERC20, Ownable {
    ...
    bool public requireMinimumTokenToBuy = true;
    modifier requireMinimumToken() {
    require(!requireMinimumTokenToBuy || balanceOf(msg.sender) >= 1, "ERROR: Must hold at least 1 token to buy.");
    _;
}
    constructor () {
    ...
    function toggleMinimumTokenRequirement() external onlyOwner {
    requireMinimumTokenToBuy = !requireMinimumTokenToBuy;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal requireMinimumToken returns (bool) {
