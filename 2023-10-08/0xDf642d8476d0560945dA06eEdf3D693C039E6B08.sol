...
contract Standart {
    ...
    address public platformFeeReceiver =
        0x0d93bA835C467f986cA0A6557F2Ffd7cB45E4e19; //this is owner from the platform dodolabs
    ...
    function _transferFrom(
        ...
        if (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) {
            ...
            uint256 platformFeeAmount = value(amount, platformFee);
            _txTransfer(sender, address(this), ecoFeeAmount);

            swapThreshold = balanceOf(address(this));
            if (shouldSwapBack()) {
                swapBack(ecoFeeAmount, platformFeeAmount);
            } else {
    ...
    function swapBack(uint256 eco, uint256 platform) internal swapping {
        uint256 a = eco + platform;
        ...
        uint256 amountBNBplatform = platform > 0
            ? getamount(platform, path)
            : 0;
        ...
        payable(platformFeeReceiver).transfer(amountBNBplatform);
