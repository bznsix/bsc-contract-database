...
contract StrategyGalaxyHedge is StrategyBase, IERC721Receiver {
    ...
    function newOrder(Order calldata order)
        ...
        returns (
            ...
            uint256 amount0,
            uint256 amount1) {
        if (order.token0Amount > 0) {
            ICompoundToken(order.cToken0).borrow(order.token0Amount);
            _updateBorrowBalanceStored(order.cToken0);
        }
        if (order.token1Amount > 0) {
            ICompoundToken(order.cToken1).borrow(order.token1Amount);
            _updateBorrowBalanceStored(order.cToken1);
        }
        INonfungiblePositionManager.MintParams memory params =
            INonfungiblePositionManager.MintParams({
                ...
                amount0Desired: order.token0Amount,
                amount1Desired: order.token1Amount,
                amount0Min: 0,
                amount1Min: 0,
    ...
    function increaseOrder(OrderIncrease calldata order) 
        ...
        if (order.token0ToAdd > 0) {
            ICompoundToken(order.cToken0).borrow(order.token0ToAdd);
            _updateBorrowBalanceStored(order.cToken0);
        }
        if (order.token1ToAdd > 0) {
            ICompoundToken(order.cToken1).borrow(order.token1ToAdd);
            _updateBorrowBalanceStored(order.cToken1);
        }
        INonfungiblePositionManager.IncreaseLiquidityParams memory params =
            INonfungiblePositionManager.IncreaseLiquidityParams({
                tokenId: order.tokenId,
                amount0Desired: order.token0ToAdd,
                amount1Desired: order.token1ToAdd,
                amount0Min: 0,
                amount1Min: 0,
                deadline: block.timestamp
            });
        ...
        positions[order.tokenId].token0Amount += amount0;
        positions[order.tokenId].token1Amount += amount1;
    }
    ...
    function redeem(uint256 shares) external onlyManager {
        ICompoundToken(supplyMarket).redeem(shares);
        _updateBalance();
    }
...
contract StrategyBase is Ownable, Pausable {
    ...
    uint256 public strategistFee = 5;
    uint256 public keeperFee = 1;
