...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional, Ownable {
  ...
  address public DividendsToken = address(0xcb45bfFfbf0033f9BD3D726a21D8675aA9fb412B);
...
contract MetaWars is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   MetaWarsDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("MetaWars", "MetaWars") {
        _buyRewardsFee = 0;
        _buyLiquidityFee = 5;
        _buyMarketingFee = 5;
        _buyDevFee = 5;
        ...
        dividendTracker = new MetaWarsDividendTracker();

        liquidityWallet = owner();
        
        marketingWallet = payable(0x8F655db12AbAe9Eff19a129520e9BA8e141b1B21);
        _devWallet = payable(0x20a7056784D5d6e31be5966B5C9169a51b8f3B64);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MetaWars: The dividend tracker already has that address");

        MetaWarsDividendTracker newDividendTracker = MetaWarsDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MetaWars: The new dividend tracker must be owned by the MetaWars token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "MetaWars: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MetaWars: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MetaWars: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "MetaWars: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MetaWars: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MetaWars: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "MetaWars: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "MetaWars: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MetaWars: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MetaWars: Cannot update gasForProcessing to same value");
    ...
    function DisableBotsForLaunch() external onlyOwner() {
        updateMaxWallet(1);
    }
...
contract MetaWarsDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MetaWars_Dividend_Tracker", "MetaWars_Dividend_Tracker") {
        claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "MetaWars_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MetaWars contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "MetaWars_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MetaWars_Dividend_Tracker: Cannot update claimWait to same value");
