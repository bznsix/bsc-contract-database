...
contract TOKEN is ERC20, Ownable {
    ...
    {   
        address newOwner = 0xEB9D2aAcf236CF7a297fC5e5839B686533929fc5;
    ...
    function updateUniswapV2Pair(address newUniswapV2Pair) external onlyOwner {
        ...
        emit UniswapV2PairUpdated(newUniswapV2Pair);  
    }
    ...
    function setSellFee(uint256 _sellFee) external onlyOwner {
        require(_sellFee <= 30, "Sell Fee cannot be more than 10%");
        sellFee = _sellFee;
    ...
    if(liquidityPool[from] || liquidityPool[to] || _isExcludedFromFees[from] || _isExcludedFromFees[to]) {
        taxAmount = 0;
    } else if(liquidityPool[from] == true) {
        taxAmount = (amount * 4) / 100;
    } else if(liquidityPool[to] == true) {
        taxAmount = (amount * 4) / 100;
    }
