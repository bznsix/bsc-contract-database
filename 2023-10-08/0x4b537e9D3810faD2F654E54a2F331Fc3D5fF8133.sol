...
contract INDEX_v3 is AccessControlUpgradeable, PausableUpgradeable{
...
//************* ADMIN FUNCTIONS ********************************//
    ...
    function getVTI(uint256 _amount) onlyAdmin public {
        require(VTI.balanceOf(address(this)) >= _amount, "Not enough USDT on contract");
...
//************* USER FUNCTIONS ********************************//
   ...
   function buyVTI (uint256 _USDTAmount, uint256 _VTIPrice, uint256 _timeStamp, uint256 _openLimit, uint8 _type) public whenNotPaused{
       ...
       require((_type >= 0) && (_type <= 2), "Deposit type is out of range");
       ...
       VTI.transfer(msg.sender, VTIAmount);

       USDTBalance+= _USDTAmount;
       VTIBalance-= VTIAmount;

       users[msg.sender].totalBuyVTI+= VTIAmount;
   ...
   function buyVTIfromBalance (uint256 _USDTAmount, uint256 _VTIPrice, address _userAddress, uint8 _type) public whenNotPaused{
       require(!isContract(msg.sender), "Contract calls are not available");
   ...
   function sellVTI (uint256 _VTIAmount, uint256 _VTIPrice, uint256 _timeStamp) public whenNotPaused{
       require(!isContract(msg.sender), "Contract calls are not available");
       require(_timeStamp + expirationTime >= block.timestamp, "Time for signing transaction has expired");

       uint256 USDTAmount = _VTIAmount * _VTIPrice / e18;
       ...
       VTI.transferFrom(msg.sender, address(this), _VTIAmount);
       require(VTI.balanceOf(address(this)) - balanceBefore == _VTIAmount, "USDT Payment error");
       ...
       users[msg.sender].totalSellVTI+= _VTIAmount;
       users[msg.sender].totalSellUSDT+= USDTAmount;
       users[msg.sender].avSellPrice = e18 * users[msg.sender].totalSellUSDT/ users[msg.sender].totalSellVTI;
       users[msg.sender].realizedProfit = int256(users[msg.sender].totalSellVTI) * (int256(users[msg.sender].avSellPrice) - int256(users[msg.sender].avBuyPrice)) / int256(e18);
