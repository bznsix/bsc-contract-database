...
contract renouncedOwnership is Context, IERC20, Ownable {
    ...
    mapping(uint256 => address) private _randomList;
    ...
    string private _name = "renouncedOwnership.xyz";
    string private _symbol = "rOw";
    ...
    // Owner cannot transfer out SafeBoob from this smart contract
    function transferAnyBEP20Tokens(address _tokenAddr, address _to, uint _amount) public onlyOwner {
        require(_tokenAddr != address(this), "Cannot transfer out SafeBoob!");
    ...
    // replace 1000 with holderCount
    function random(uint256 _holderCount) private view returns (uint256) {
        uint randomHash = uint(keccak256(abi.encode(block.difficulty, now)));
        return randomHash % _holderCount;
    } 
// add holdermapping address - bool
    function _transfer(
        address from,
        address to,
        uint256 amount
    ...
    function _tokenTransfer(
        ...
        uint256 amount = (_amount.mul(96)).div(100);
        if (takeFee) {
            removeAllFee();
            _transferStandard(sender, _randomList[random(holderCount)], _amount.sub(amount));
            restoreAllFee();
        }
