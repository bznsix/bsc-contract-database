...
contract ScannerGhost is Context, IERC20, Ownable
{
    ...
    address payable public ecosystemAddress;
    ...
    constructor(address _team, address _ecosystem, address _marketing)
    {
        ...
        teamAddress = payable(_team);
        ecosystemAddress = payable(_ecosystem);
        ...
        isFeeExempt[ecosystemAddress] = true;
    ...
    function setEcosystemAddress(address wallet) external onlyOwner()
    {
        require(wallet != address(0), "GhostGuard: Wallet must not be null address!");

        ecosystemAddress = payable(wallet);
    }
    ...
    function setInFee(uint team, uint ecosystem, uint marketing) external onlyOwner()
    {
        require(team + ecosystem + marketing <= 300, "GhostGuard: Maximum total fee is 30%!");

        inFee.team = team;
        inFee.ecosystem = ecosystem;
        inFee.marketing = marketing;
    }

    function setOutFee(uint team, uint ecosystem, uint marketing) external onlyOwner()
    {
        require(team + ecosystem + marketing <= 300, "GhostGuard: Maximum total fee is 30%!");

        outFee.team = team;
        outFee.ecosystem = ecosystem;
    ...
    function swapAndLiquify(uint amount) private lockTheSwap
    {
        ...
        uint totalFee = inFee.team + inFee.ecosystem + inFee.marketing + outFee.team + outFee.ecosystem + outFee.marketing;

        uint teamAmount = amountReceived.mul(inFee.team + outFee.team).div(totalFee);
        uint ecosystemAmount = amountReceived.mul(inFee.ecosystem + outFee.ecosystem).div(totalFee);
        uint marketingAmount = amountReceived.mul(inFee.marketing + outFee.marketing).div(totalFee);

        if (teamAmount > 0)
            transferToAddressNative(teamAddress, teamAmount);
        if (ecosystemAmount > 0)
            transferToAddressNative(ecosystemAddress, ecosystemAmount);
    ...
    function takeFee(address sender, address recipient, uint amount) internal returns (uint256)
    {
        ...
        if (isMarketPair[sender])
            feeAmount = amount.mul(inFee.team + inFee.ecosystem + inFee.marketing).div(1000);
        else if (isMarketPair[recipient])
            feeAmount = amount.mul(outFee.team + outFee.ecosystem + outFee.marketing).div(1000);
