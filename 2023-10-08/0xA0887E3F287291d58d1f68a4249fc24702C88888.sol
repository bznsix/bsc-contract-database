...
contract LpDividend is Context, IERC20, Ownable {
    ...
    address public burnReceiver = address(0xfEBD06Be1dc30Dc0E29005E5aa7b9b4cB6DE8A88);
    address public JD = address(0xf097D091F98b1CDBa362E14112E0bCbB7f6D2097);
    ...
    uint256 public inoutLiquidityFee;
    uint256 public inoutMarketingFee;
    uint256 public inoutLpRewardFee;
    uint256 public inoutBurnFee;

    uint256 public swapStartTime = 1696853880;
    ...
    constructor (
        ...
        inoutLiquidityFee = baseFees[0];
        inoutMarketingFee = baseFees[1];
        inoutLpRewardFee = baseFees[2];
    ...
    function setBurnReceiver(address account) external onlyOwner {
        burnReceiver = payable(account);
        _isFeeExempt[burnReceiver] = true;
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(automatedMarketMakerPairs[sender] || automatedMarketMakerPairs[recipient]) {
            ...
            lpRewardAmount = amount.mul(inoutLpRewardFee).div(FeeDeno);
            LpFeeAmounts += lpRewardAmount;

            burnAmount = amount.mul(inoutBurnFee).div(FeeDeno);
            burnFeeAmounts += burnAmount;

            feeAmount = liquidityAmount.add(lpRewardAmount).add(marketingAmount).add(burnAmount);
        }
    ...
    function processLP(uint256 gas) external onlyOwner{
     ...
     function _processLp(uint256 gas) private {
        ...
        if(currencyFlag){
            total = total.sub(LiquidityFeeAmounts).sub(LpFeeAmounts).sub(MarketingFeeAmounts).sub(burnFeeAmounts);
        }
    ...
    function setInOutTaxes(uint256 liquidityFee, uint256 marketingFee, uint256 lpRewardFee, uint256 burnFee) external onlyOwner {
        inoutLiquidityFee = liquidityFee;
        inoutMarketingFee = marketingFee;
        inoutLpRewardFee = lpRewardFee;
        inoutBurnFee = burnFee; 
    }

    function setSwapStartTime(uint256 time) external onlyOwner {
        require(swapStartTime == 0 || balanceOf(address(uniswapPair)) == 0, "Transaction has started");
