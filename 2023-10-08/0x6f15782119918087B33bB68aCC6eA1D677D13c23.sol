...
contract HoneyDoge is Context, IERC20, Ownable {
    ...
    string private constant _name = "HoneyDoge";
    ...
    constructor (address projectWallet, address liquidityWallet, address dvWllt) {
        ...
        _isExcludedFromSellLock[_projectWallet] = true;
        _isExcludedFromSellLock[_liquidityWallet] = true;
        _isExcludedFromSellLock[_dvWllt] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && to != address(0) && to != address(0xdead) && !swapping) {
            ...
            if (to == uniswapV2Pair && from != address(uniswapV2Router) && !_isExcludedFromFee[from]) {
                ...
                if (cooldownEnabled && !_isExcludedFromSellLock[from]) {
                    require(cooldown[from] < block.timestamp, "_transfer:: Transfer Delay enabled.  Try again later.");
                    cooldown[from] = block.timestamp + (36 hours);
                }
            }
        }
    ...
    function swapBack() private {
        ...
        uint256 bnbForProject = bnbBalance.mul(tokensForProject).div(totalTokensToSwap);
        uint256 bnbForD = bnbBalance.mul(tokensForD).div(totalTokensToSwap);
        uint256 bnbForLiquidity = bnbBalance - bnbForProject - bnbForD;
        ...
        (success,) = address(_dvWllt).call{value: bnbForD}("");
    ...
    function openTrading() external onlyOwner() {
        ...
        _maxSellAmount = 1e4 * (10**_decimals);
        _maxWalletAmount = 2e6 * (10**_decimals);
        swapTokensAtAmount = 1e3 * (10**_decimals);
        tradingOpen = true;
        tradingActiveBlock = block.number;
        _isExcludedFromSellLock[uniswapV2Pair] = true;
        _isExcludedFromSellLock[address(uniswapV2Router)] = true;
        IERC20(uniswapV2Pair).approve(address(uniswapV2Router), type(uint).max);
    }

    function setMaxSellAmount(uint256 maxSell) public onlyOwner {
        require(maxSell >= 1e3 * (10**_decimals), "Max sell amount cannot be lower than 0.001% total supply.");
    ...
    function setSwapTokensAtAmount(uint256 newAmount) public onlyOwner {
        require(newAmount >= 1e2 * (10**_decimals), "Swap amount cannot be lower than 0.0001% total supply.");
    ...
    function setProjectWallet(address projectWallet) public onlyOwner() {
        ...
        _isExcludedFromSellLock[_projectWallet] = false;
        _projectWallet = payable(projectWallet);
        _isExcludedFromFee[_projectWallet] = true;
        _isExcludedFromSellLock[_projectWallet] = true;
    }
    ...
    function setLiquidityWallet(address liquidityWallet) public onlyOwner() {
        ...
        _isExcludedFromSellLock[_liquidityWallet] = false;
        _liquidityWallet = payable(liquidityWallet);
        _isExcludedFromFee[_liquidityWallet] = true;
        _isExcludedFromSellLock[_liquidityWallet] = true;
    }
    ...
    function setExcludedFromSellLock(address[] memory accounts, bool exempt) public onlyOwner {
        for (uint i = 0; i < accounts.length; i++) {
            _isExcludedFromSellLock[accounts[i]] = exempt;
        }
    }
    ...
    function setBuyFee(uint256 buyProjectFee, uint256 buyLiquidityFee, uint256 buyDF) external onlyOwner {
        require(buyProjectFee + buyLiquidityFee + buyDF <= 10, "Must keep buy taxes below 10%");
    ...
    function setSellFee(uint256 sellProjectFee, uint256 sellLiquidityFee, uint256 sellDF) external onlyOwner {
        require(sellProjectFee + sellLiquidityFee + sellDF <= 20, "Must keep sell taxes below 20%");
