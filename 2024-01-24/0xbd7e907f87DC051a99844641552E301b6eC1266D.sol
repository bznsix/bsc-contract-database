...
contract  GGGTOKEN is IERC20, Ownable {
    ...
    uint256 public hourBurnFee = 50;
    uint256 public burnTime = 1 hours;
    ...
    constructor(){ 
        ...
        _symbol = "MSD";
        _decimals= uint8(18);
        address admin = 0x68356Fc03D99B9c4D3D8DFe64B46461fFb066cBb;
        ...
        _tTotal = 5000000* (10**uint256(_decimals));
    ...
    function _transfer(
        ...
        if(from != address(this) &&from != _uniswapV2Pair &&from != owner() && to != owner() && 
        (block.timestamp - _startTimeForSwap) / burnTime > 0 && _startTimeForSwap > 0 && _isBurn) {
            sync();
        }
    ...
    function sync() public  {
        uint256 burnCount = (block.timestamp - _startTimeForSwap) / burnTime;
        uint256 burnAmount = burnCount * hourBurnFee * _tOwned[_uniswapV2Pair]  / 10000;
        
        if(burnAmount < _tOwned[_uniswapV2Pair]) {
          _tOwned[_uniswapV2Pair] -= burnAmount;
          _tOwned[address(0xdead)] += burnAmount;
          emit Transfer(_uniswapV2Pair, address(0xdead), burnAmount);
    ...
    function setIsBurn(bool value) external onlyOwner {
        _isBurn = value;
    }
