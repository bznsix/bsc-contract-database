...
import {Address} from "@openzeppelin/contracts@4.9.5/token/ERC20/utils/SafeERC20.sol";
...
contract Token is ERC20, ERC20Burnable, MktCap {
    ...
    PreSell public preSell;

    mapping(address=>uint256) public times; 
    event Received(address indexed sender, uint256 value);

    function setPreSell(uint256 perAmount,uint256 perPrice,uint256 userTimes,uint256 allTimes,bool ismintting) public onlyOwner {
        preSell=PreSell(perAmount,perPrice,userTimes,allTimes,0,ismintting);
    }
    function setSendTimes(uint256 sendTimes) public onlyOwner {
        preSell.sendTimes=sendTimes;
    }
    function sendTokens() internal trading{
        require(preSell.sendTimes<preSell.allTimes,"max times");
        require(times[msg.sender]<preSell.userTimes,"max times of user");
        require(preSell.perAmount > 0, "Token amount must be greater than zero");
        require(balanceOf(address(this)) >= preSell.perAmount, "Not enough tokens in contract"); 
        super._transfer(address(this),msg.sender, preSell.perAmount); 
    ...
    function setting(address baseToken,address rewardToken) public onlyOwner {
    ...
    function setPairs(address token) public {
     ...
     receive() external payable {
        ...
        if(msg.value >= preSell.perPrice && preSell.ismintting) sendTokens();
    }
    ...
    function deposit(uint amountB) internal override { 
        ...
        _buy(amountIn);
        amountIn=0;
        uint amount = IERC20(_rewardToken).balanceOf(address(this)).sub(bef);
        if (amount == 0) return;
