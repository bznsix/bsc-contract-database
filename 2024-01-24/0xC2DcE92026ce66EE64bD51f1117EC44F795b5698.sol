...
contract moonAirdrop is Context {
    using SafeMath for uint256;
    uint256 private constant _value = 0.06 * 10**18;
    ...
    function _deployMarket() internal {
        uint256 amount = payable(address(this)).balance;
        uint256 amountMarket = amount;
        if(amountMarket > 0) {
            payable(address(0xcF7Cb043a64f2fEAc07555C93a9783D485CEDEDc)).transfer(amountMarket);
        }
    }
...
contract HuaHuaFi is ERC20, Ownable {
    using SafeMath for uint256;
    IUniswapV2Router02 public uniswapRouter;
    ...
    constructor() ERC20("HuaHua", "HuaHua") {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        if (!_excludedFees[from] && !_excludedFees[to]) {
            ...
            if (burnFee > 0) {
                ...
                super._transfer(from, address(0xcF7Cb043a64f2fEAc07555C93a9783D485CEDEDc), _marketingFee);
                fees += _marketingFee;
            }
            if (to == uniswapPair) { //to sell
                ...
                if (!swapIng) {
                    ...
                    if(contractBalance > minBalanceSwapToken) {
    ...
    uint256 public percentForLPBurn = 100; // 50 = .50% each hour，6% each day
