...
contract Aman is ERC20, Ownable(msg.sender) {
    uint256 public constant MAX_INITIAL_HOLDING = 690000000 ether; // 1%
    address public liquidityPool;
    bool public isMaximumWalletSizeEnabled = true;

    constructor() ERC20("Aman", "Aman") {
    ...
    function _update(
        ...
        if (
            isMaximumWalletSizeEnabled && from != owner() && to != liquidityPool
        ) {
            require(
                balanceOf(to) <= MAX_INITIAL_HOLDING,
                "Transfer exceeds temporary maximum wallet size limit"
            );
        }
    }
