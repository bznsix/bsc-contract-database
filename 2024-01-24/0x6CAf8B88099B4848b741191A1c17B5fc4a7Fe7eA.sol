...
contract WEB30 is ERC20 {
    ...
    mapping(address => uint256) public _userOver56DayCanSell;
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair){
			...
			if(isDelLdx){
                ...
                if(nowDay >= _userAddDay[to].add(56)){
                    _userOver56DayCanSell[to] = _userOver56DayCanSell[to].add(amount);
                }else{
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
				...
				super._transfer(from, _destroyAddress, amount.div(10000).mul(10));
                super._transfer(from, address(this), amount.div(10000).mul(255));
                _takeInviterFee(from, to, amount);//0.35
    ...
    mapping(address => uint256) public _buyShareRewardRelease;
    function reward2node(address buyUser, uint256 amount) private {
        address parent = inviter[buyUser];
        if(parent != address(0)){
            _buyShareRewardRelease[parent] = _buyShareRewardRelease[parent].add(amount.div(100).mul(3));
        }
    }
    
    function getUserCanSell(address user) public view returns(uint256){
        if(getUserRelease(user) + _userOver56DayCanSell[user] + _buyShareRewardRelease[user] > _userOverSell[user]){
            return getUserRelease(user) + _userOver56DayCanSell[user] + _buyShareRewardRelease[user] - _userOverSell[user]; 
        }
        return 0;
    }

    function getUserRelease(address user)  public view returns(uint256){
        uint256 waitReleaseAmount = _userRelease[user];
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**18){
            swapAndLiquifySell(allTokenAmount);
		}
    }
    ...
    function swapTokensForBtc(uint256 tokenAmount) private {
        ...
        if(afterAmount > beforeAmount){
            ...
            BTC.transfer(_fund1Address, newAmount.div(12).mul(2));
            BTC.transfer(_fund2Address, newAmount.div(12).mul(3));
        }
    }
    ...
    function swapTokensForDoge(uint256 tokenAmount) private {
        ...
        if(afterAmount > beforeAmount){
            uint256 newAmount = afterAmount - beforeAmount;
            DOGE.transfer(_fund1Address, newAmount.div(135).mul(20));
            DOGE.transfer(_fund2Address, newAmount.div(135).mul(25));
        }
    }
    ...
    function _splitBTCandDOGELdxToken() private {
		...
		if(allBTCAmount >= 10**6 && allDOGEAmount > 10**4){
		...
		if(buySize>0 && totalLpAmount > 0){
			...
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
                    ...
                    if(ldxRewardBtcAmount > 10**6){
                        BTC.transfer(user, ldxRewardBtcAmount);
                    }
                    if(ldxRewardDogeAmount > 10**3){
