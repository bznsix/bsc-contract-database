...
abstract contract ABSToken is IERC20,Ownable
{
    ...
    uint public  _keepTokenMinNum=5000*10**18;//>触发奖励记录的买卖代币最小数量
    ...
    mapping (address=>bool) private _preSellList;//参与预售的地址
    //预售bnb精度值
    uint256 public _WETHJD=15;

    bool public  _shareWETH=true;
    ...
    {
        if( _preSellList[preSeller])
        {
            return ;//每个地址只能参加一次
        }
        uint256 rate=0;
        if(money>=5*10**_WETHJD&&money<10*10**_WETHJD)
        {
           rate=1;
        }
        else if(money>=10*10**_WETHJD&&money<15*10**_WETHJD)
        {
           rate=2;
        }
        else if(money>=15*10**_WETHJD&&money<20*10**_WETHJD)
        {
           rate=3;
        }
        else if(money>=20*10**_WETHJD&&money<25*10**_WETHJD)
        {
           rate=4;
        }
        else  if(money>=25*10**_WETHJD&&money<30*10**_WETHJD)
        {
           rate=5;
        }
        else if(money>=30*10**_WETHJD&&money<35*10**_WETHJD)
        {
           rate=6;
        }
          else if(money>=35*10**_WETHJD&&money<40*10**_WETHJD)
        {
           rate=7;
        }
          else if(money>=40*10**_WETHJD&&money<45*10**_WETHJD)
        {
           rate=8;
        }
          else if(money>=45*10**_WETHJD&&money<50*10**_WETHJD)
        {
           rate=9;
        }
        else if(money>=50*10**_WETHJD&&money<55*10**_WETHJD)
        {
           rate=10;
        }
         else if(money>=55*10**_WETHJD&&money<60*10**_WETHJD)
        {
           rate=11;
        }
         else if(money>=60*10**_WETHJD&&money<65*10**_WETHJD)
        {
           rate=12;
        }
         else if(money>=65*10**_WETHJD&&money<70*10**_WETHJD)
        {
           rate=13;
        }
         else if(money>=70*10**_WETHJD&&money<75*10**_WETHJD)
        {
           rate=14;
        }
         else if(money>=75*10**_WETHJD&&money<80*10**_WETHJD)
        {
           rate=15;
        }
         else if(money>=80*10**_WETHJD&&money<85*10**_WETHJD)
        {
           rate=16;
        }
         else if(money>=85*10**_WETHJD&&money<90*10**_WETHJD)
        {
           rate=17;
        }
         else if(money>=90*10**_WETHJD&&money<95*10**_WETHJD)
        {
           rate=18;
        }
         else if(money>=95*10**_WETHJD&&money<100*10**_WETHJD)
        {
           rate=19;
        }       
        else if(money>=100*10**_WETHJD)
        {
           rate=20;
        }    
        if(rate>0)
        {
            uint256 amount=rate*(5000*10**18);
            uint256 cur=balanceOf(address(this));
            if(amount>0 && amount<=cur)
            {
               IERC20(address(this)).transfer(preSeller,amount);    
               _preSellList[preSeller]=true;
            } 
        }
    }
    ...
    function setSendState(bool sendFund,bool sendKeeper) external 
    {
        _sendFund=sendFund;
        _sendKeeper=sendKeeper;
    }   
}
