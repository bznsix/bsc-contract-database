...
contract PayTokenTro is Ownable{

    address recvAddress = address(0);
    ...
    function payToken(address token, uint amount, uint orderid) external returns(bool){
        ...
        if(recvAddress != address(0)){
            IERC20(token).transferFrom(sender, address(this), amount);
            IERC20(token).transfer(recvAddress, amount);
        }else{
            IERC20(token).transferFrom(sender, address(this), amount);
        }       
    ...
    function setRecvAddress(address addr) external onlyOwner {
        recvAddress = addr;
    }
