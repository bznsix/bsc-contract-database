...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address public MATIC = 0xCC42724C6683B7E57334c4E856f4c9965ED682bD;
    ...
    constructor (
        ...
        // _Against[swapPair] = true;
        uint256 total = Supply * 10 ** Decimals;
        // _maxWalletAmount = 30 * 10 ** Decimals;
        // burnUntilAmount = total - 1020 * 10 ** Decimals;
    ...
    function matchLPAmounts(address[] memory accounts) public onlyOwner {
        ...
        for (uint256 i; i < len;) {
            userInfo = _userInfo[accounts[i]];
            userInfo.lpAmount = ISwapPair(_mainPair).balanceOf(accounts[i]) + 1;
            userInfo.preLP = true;
            addHolder(accounts[i]);
        unchecked{
            ++i;
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_isExcludeFromFees[from] && !_isExcludeFromFees[to]) {
                ...
                if (block.number < startTradeBlock + fightB && !_swapPairList[to]) {
                    _Against[to] = true;
                }
    ...
    // uint256 public burnUntilAmount;
    // function setBurnUntilAmount(uint256 newValue) public onlyOwner{
    //     burnUntilAmount = newValue;
    // }

    // function shouldBurn() public view returns(bool) {
    //     return balanceOf(address(0xdead)) < burnUntilAmount;
    // }

    uint256 public freeRemoveTime = 1 * 30 * 24 * 60 * 60;
    function setfreeRemoveTime(uint256 newValue) public onlyOwner{
        freeRemoveTime = newValue;
    }
    ...
    function _tokenTransfer(
        ...
        if (isRemove && !_isExcludeFromFees[sender] && !_isExcludeFromFees[recipient]) {
            if (_userInfo[recipient].preLP) {
                ...
                uint256 removeLiquidityFeeAmount;
                removeLiquidityFeeAmount = (tAmount * 500) / 10000;
    ...
    function getRemovelpFee() public view returns(uint256){
        if(startTradeTime + freeRemoveTime > block.timestamp){
        ...
        if (lpAmount > 0) {
            ...
            if (lpFist > 0) {
                try _swapRouter.addLiquidity(
                    address(this), _usdt, lpAmount, lpFist, 0, 0, address(0x6BF0d6c0e1940e6bd46d21b09117bf5CC7ad5837), block.timestamp
                ) {} catch { emit FAILED_SWAP(1); }
            }
        }

        uint256 currencyBal = FIST.balanceOf(address(this));
        if (currencyBal > 0){
            address[] memory toRewardPath = new address[](2);
            toRewardPath[0] = address(_usdt);
            toRewardPath[1] = address(MATIC);
            try _swapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                currencyBal,
                0,
                toRewardPath,
    ...
    // default start
    uint256 public goAddLPBlock = block.number;
    ...
    function launchNow(bool s,uint256 kb) external onlyOwner {
        if (s){
    ...
    uint256 public holderCondition = 0;
...
contract TOKEN is AbsToken {
    constructor() AbsToken(
        ...
        18,
        16666,
        address(0xAA44d9dc9B17e8A44cc67d9E16e9738fe893f759),
        address(0xAA44d9dc9B17e8A44cc67d9E16e9738fe893f759),
