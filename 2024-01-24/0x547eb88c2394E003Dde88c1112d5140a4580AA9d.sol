/*
** PAPA SMURF $PAPA
** X: https://twitter.com/papasmurfcoin
** Telegram: https://t.me/PapaSmurfVillage
** Website: https://papasmurf.io/
*/

// SPDX-License-Identifier: MIT

// File: pancake/PancakeSwap.sol
...
contract Papa is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 21000000000000000000000000;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Papa Smurf";
    string private _symbol = "PAPA";
    uint8 private _decimals = 18;

    address payable public _devwallet = 0xB64727F1F85225458697eed4461d5424CB783720;
    address payable public _prodwallet = 0x4f1B3AA2a6424a52E1EC9136fe2232C79bcD6ea3;
    address payable public _marketingwallet = 0x0893972059a95089E6F9cC3C270D5A63402D45De;

    uint256 public _currentTax = 10;   
    uint256 public _totalTax = 10;   
    uint256 private _previousRefTax = _totalTax;
    
    uint256 public _refPer = 0; 

    uint256 public _devShare = 2000; 
    uint256 public _prodShare = 5000;  
    uint256 public _marketingShare = 3000;  
                                     
    uint256 public _maxHoldAmount =  420000000000000000000000;                              
    
    bool public _nomicsActive = false;
    ...
    constructor () {
        ...
        excludeAccount(_prodwallet);
        excludeAccount(_devwallet);
        excludeAccount(_marketingwallet);
        excludeAccount(0xf209114271249bf6d71740af0683E4ad7e4Cb0f1);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(_nomicsActive && sender != uniswapV2Pair) {
            _doTokenomics();
        }
    ...
    function setMaxHold(uint256 maxHoldAmount) external onlyOwner() {
    ...
    function _setNomicsActive(bool nomicsActive) external onlyOwner() {

        _nomicsActive = nomicsActive;
    }

    function _doTokenomics() public {
        
        uint256 amount = balanceOf(address(this));
        
        if(amount > 0) {

            uint256 prod = _findPercent(amount, _prodShare);
            uint256 dev = _findPercent(amount, _devShare);
            uint256 marketing = amount.sub(prod).sub(dev);
    
            _sendFunds(prod, _prodwallet);
            _sendFunds(dev, _devwallet);
            _sendFunds(marketing, _marketingwallet);
        }
    }

    function _sendFunds(uint256 amount, address payable wallet) private {
    ...
    function sProdWallet(address payable wallet) public onlyOwner() {

        _prodwallet = wallet;
    }
    
    function sDevWallet(address payable wallet) public onlyOwner() {

        _devwallet = wallet;
    }

    function sMarketingWallet(address payable wallet) public onlyOwner() {
