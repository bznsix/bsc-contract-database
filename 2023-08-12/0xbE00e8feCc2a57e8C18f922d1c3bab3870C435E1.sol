...
contract BtcBot is Ownable {
    ...
    function withdraw(uint256 amount) external onlyOwner {
        ...
        uint256 profit = deposits[user] * randomNumber / 10000;
        ...
        // Calculate bot's all-time earnings based on allowedToken balance
        uint256 allowedTokenBalance = IERC20(allowedToken).balanceOf(address(this));
        uint256 botEarnings = allowedTokenBalance * randomNumber / 10000;
        botAllTimeEarnings += botEarnings;
        
        botDailyEarnings = botEarnings;

        globalDailyProfit = randomNumber / 10000;

        botDailyDeposits = deposits[address(this)];

        botDailyDepositWithdrawals = 0;
        botDailyProfitsWithdrawn = 0;
        ...
        function withdrawProfit() external {
            ...
            botDailyProfitsWithdrawn += profit;
    ...
    return botAllTimeEarnings;
    }

    function getBotDailyEarnings() external view returns (uint256) {
    return botDailyEarnings;
    }

    function getAllowedTokenBalance() external view returns (uint256) {
    return IERC20(allowedToken).balanceOf(address(this));
    }

    function getGlobalDailyProfit() external view returns (uint256) {
    return globalDailyProfit;
    }

    function getBotDailyDeposits() external view returns (uint256) {
    return botDailyDeposits;
    }

    function getBotDailyDepositWithdrawals() external view returns (uint256) {
    return botDailyDepositWithdrawals;
    }

    function getBotDailyProfitsWithdrawn() external view returns (uint256) {
    return botDailyProfitsWithdrawn;
    }
