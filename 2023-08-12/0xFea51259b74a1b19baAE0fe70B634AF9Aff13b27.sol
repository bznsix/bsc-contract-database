...
contract RCRewardTokenDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RewardToken contract."
        );
    }
...
contract RCRewardToken is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    RCRewardTokenDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        require(
            _owner != _marketingWalletAddress,
        ...
        dividendTracker = RCRewardTokenDividendTracker(
        ...
        */
        ...
        emit TokenCreated(_owner, address(this), TokenType.baby, VERSION);
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        RCRewardTokenDividendTracker newDividendTracker = RCRewardTokenDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "RewardToken: The new dividend tracker must be owned by the RewardToken token contract"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "RewardToken: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "RewardToken: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
...
contract RCRewardTokenFactory is Ownable{

    address[] public contractAddresses;
    string[] public contractNames;
    address[] public projectOwners;

    uint256 public launchPadDeployFee = 150000000000000000;      // in wei 18 decimals
    uint256 public launchPadServiceFeePercentage = 1;
    ...
    function deploy(
        ...
        if(launchPadDeployFee > 0){
            require(msg.value >= launchPadDeployFee, "Insufficient Fee: Not Enough Fee Paid To Deploy Token!");
        }

        RCRewardToken token = new RCRewardToken(
            name_,
            symbol_,
            totalSupply_,
        ...
        contractAddresses.push(address(token));
        contractNames.push(name_);
