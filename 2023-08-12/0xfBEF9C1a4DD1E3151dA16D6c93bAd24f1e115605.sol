...
contract FlokiCity is Ownable, ERC20 {
    ...
    address public _lotteryAddr;
    address public _marketingAddr = 0x1840273107EBFe4030D8eCc85905C31E94d41492;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override virtual {
        ...
        if(!_inSwapAndLiquify && swapAndLiquifyEnabled && recipient == uniswapV2Pair){
            ...
            if(overMinimumTokenBalance && buyBackEnabled) {
                ...
                swapETHForTokens(contractTokenBalance, deadAddress);
            }
            ...
            if(overMinimumTokenBalance && _fMarketing > 0 && _fLottery > 0) {
                ...
                uint amountMarketing = balanceToken.mul(_fMarketing).div(_fMarketing.add(_fLottery));
                uint amountLottery = balanceToken - amountMarketing;
                swapTokenFor(_marketingAddr, amountMarketing);
                swapTokenFor(_lotteryAddr, amountLottery);
            }
        }
        bool takeFee = !(_addressExcludedFromFee[sender] || _addressExcludedFromFee[recipient]);
    ...
    function getFeeMarketing() external view returns(uint8) {
        return _fMarketing;
    }

    function getFeeLottery() external view returns(uint8) {
        return _fLottery;
    }
    ...
    function setMarketingAddr(address addr) public onlyOwner {
        _addressExcludedFromFee[_marketingAddr] = false;
        _addressExcludedFromFee[addr] = true;
        _marketingAddr = addr;
    }

    function setLotteryAddr(address addr) public onlyOwner {
        _addressExcludedFromFee[_lotteryAddr] = false;
        _addressExcludedFromFee[addr] = true;
        _lotteryAddr = addr;
    }
    ...
    function numTokensSell() public view returns(uint){
        return _numTokensSell;
    }

    function setNumMaxEthBuyBack(uint amountWithoutDecimal) public onlyOwner {
        _numMaxEthBuyBack = amountWithoutDecimal * 10 ** decimals();
    }

    function getNumMaxEthBuyBack() external view returns(uint) {
        return _numMaxEthBuyBack;
    }
    ...
    function setBuyBackEnabled(bool enable) public onlyOwner {
