...
contract CommunityRound is Ownable {
    ...
    constructor(
        ...
        feeRecivingWallet = 0x3D6a0181aFdc43644C288B40E4fc693EA7259483;
    ...
    function buyWithCode(
        ...
        require(
            isPoolActive && startTime <= block.timestamp,
            "Pool not started"
        );
        bytes32 label = keccak256(bytes(_code));
        Pools storage _currentPool = poolDetails[label];
    ...
    function changePoolStartTime(uint256 _time) external onlyOwner {
        startTime = _time;
    }

    function stopPool(bool _status) external onlyOwner {
        isPoolActive = _status;
    }
