...
contract NRW is Context, IBEP20, Ownable {
  ...
  uint256 private _buyFeeRatio = 90;
  uint256 private _sellFeeRatio = 90;

  uint256 private _feeBurnRatio = 1;

  address private _feeNodeReceiver;
  ...
  constructor() {
    ...
    _min = 10_000_000 * (10 ** uint256(_decimals));
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if (sender == _pairAddress && !_isExcluded[recipient]) {
      ...
      if (fee > 0) {
        ...
        if (subFeeAmount > 0) {
          _balances[_feeNodeReceiver] = _balances[_feeNodeReceiver].add(subFeeAmount);
  ...
  function _isLiquidity(address from, address to) internal view returns (bool isAdd, bool isDel) {
    ...
    (uint256 r0, uint256 r1, ) = IUniswapV2Pair(_pairAddress).getReserves();
    if (token0 != address(this)) {
      uint256 bal0 = IBEP20(token0).balanceOf(_pairAddress);
      if (to == _pairAddress) {
        if (bal0 > r0) {
          isAdd = bal0 - r0 > addPriceTokenAmount;
        }
      }
      if (from == _pairAddress) {
        if (bal0 < r0) {
          isDel = r0 - bal0 > 0;
        }
      }
      ...
      uint256 bal1 = IBEP20(token1).balanceOf(_pairAddress);
      if (to == _pairAddress) {
        if (bal1 > r1) {
  ...
  function burn(uint256 amount) public {
    ...
    if (total <= _min) return;
    if (total < _min.add(amount)) {
      amount = total.sub(_min);
    }
  ...
  function setExcluded(address[] calldata accounts, bool flag) external onlyOwner {
  ...
  function setDeadAddress(address addr) external onlyOwner {
    deadAddress = addr;
  }
  ...
  function setFeeNodeReceiver(address addr) public onlyOwner {
    _feeNodeReceiver = addr;
  }

  function setTxAmount(uint256 apta) external onlyOwner {
