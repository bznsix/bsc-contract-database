...
contract IRONFISHStaking is ReentrancyGuard {
    ...
    function updatePerformance(address _user, uint256 _performance) private {
        ...
        for (uint256 i = 0; i <= MAX_REFERRAL_LEVEL && currentUser != address(0); i++) {
            users[currentUser].totalPerformance += _performance;

            if (i > 0 && users[currentUser].referringAddress != address(0)) {
                users[users[currentUser].referringAddress].referralsPerformance[_user] += _performance;
            }

            currentUser = users[currentUser].referringAddress;
        }
    }
