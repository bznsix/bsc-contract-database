...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address public fundAddress = address(0x88C55D46CC8DCe46F0b82E2B2feaecc1e08A4fc5);
    address public ownerAddress = address(0x6aE147cA0399052349662C900b26b7D88561639d);
    string private _name = "Niao B";
    string private _symbol = "Niao B";
    ...
    uint256 private _tTotal = 2000 * 10 ** _decimals;
        ...
        if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
            if (_swapPairList[from] || _swapPairList[to]) {
                ...
                if (_swapPairList[to]) {
                    ...
                    if(isRemoveLP && startTradeTime>0 && block.timestamp> startTradeTime && block.timestamp<startTradeTime+2 days){
                        if(block.timestamp<startTradeTime+1 days){
                            _lpBurnTransfer(from,to,amount,40);
                        }else{
                            _lpBurnTransfer(from,to,amount,20);
                        }
                        return;
                    }
                }
    ...
    function setSellFee(uint256 fundFee,uint256 lpFee,uint256 lpDivFee) external onlyOwner {
