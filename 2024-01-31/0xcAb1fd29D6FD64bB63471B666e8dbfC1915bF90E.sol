...
contract MEGALAND is ERC20Burnable, Ownable {
    ...
    ) ERC20("METAGALAXY LAND", "MEGALAND") Ownable(msg.sender) {
    ...
    function setMarketingWallet(address marketingWallet_) external onlyOwner {
        require(
            ...
            "MEGALAND: Wrong Address"
        );
        marketingWallet = marketingWallet_;
    }

    function setBuyFee(uint256 buyFee_) external onlyOwner {
        require(buyFee_ <= 100, "MEGALAND: Wrong Buy Fee");
        buyFee = buyFee_;
    }

    function setSellFee(uint256 sellFee_) external onlyOwner {
        require(sellFee_ <= 100, "MEGALAND: Wrong Sell Fee");
        sellFee = sellFee_;
    }
