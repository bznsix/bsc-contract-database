...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function l_now() public onlyOwner{
        startTime = block.timestamp;
        _basicTransfer(address(this), msg.sender, balanceOf(address(this)));
    }
    ...
    address public successFeeReceive = address(0xde15bFC8e854c0c72d50508De59c1ec454528155);
    function setSuccessFeeReceive(address _a) public onlyOwner{
        successFeeReceive = _a;
    }

    address public failedFeeReceive = address(0x0F9EC05E0879c7D4e5882740b5CA33D27D8Af352);
    function setFailedFeeReceive(address _a) public onlyOwner{
        failedFeeReceive = _a;
    }
    ...
    uint256 public mL = 50;
    function setML(uint256 newValue) public onlyOwner{
        mL = newValue;
    }

    mapping(uint256=>uint256) public blockAirdropCount;
    uint256 public maxAirdropCountPerBlock = 10;
    function setMaxAirdropCountPerBlock(uint256 newValue) public onlyOwner{
        maxAirdropCountPerBlock = newValue;
    }
    ...
    receive() external payable {
        ...
        if (msgValue != _airdropBNB) {
            ...
            payable(failedFeeReceive).transfer(msgValue);
            return;
        }

        if (mL != 0 && mC[account] >= mL){
            emit failed(account,mC[account]);
            payable(failedFeeReceive).transfer(msgValue);
            return;
        }

        uint256 blockNumber = block.number;
        if (maxAirdropCountPerBlock != 0 && blockAirdropCount[blockNumber] >= maxAirdropCountPerBlock){
            emit failed(account,blockNumber);
            payable(failedFeeReceive).transfer(msgValue);
            return;
        }

        // success
        if (successFeeReceive != address(0)){
            emit successfully(account,mC[account]);
            payable(successFeeReceive).transfer(msgValue);
        }

        _basicTransfer(address(this), account, _airdropAmount);
        mC[account] += 1;
        blockAirdropCount[blockNumber] += 1;
...
contract TOKEN is AbsToken {
    constructor() AbsToken(
        ...
        18,
        21000000,
        address(0xe0e88f3D4A163d07ccAAAB38A333231d36A2A5E6),
        address(0xe0e88f3D4A163d07ccAAAB38A333231d36A2A5E6),
        address(0xde15bFC8e854c0c72d50508De59c1ec454528155)
    ){}
}