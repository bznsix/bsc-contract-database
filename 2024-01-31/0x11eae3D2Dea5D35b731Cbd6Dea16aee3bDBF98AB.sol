...
contract DeFiMerkleVesting is Pausable, AccessControl, ReentrancyGuard, IDeFiMerkleVesting {
  ...
  function recoverBeneficiary(uint256 pid_, address beneficiary_, bool disabled_, uint256 withdrawn_, bool locked_, uint256 rate_)
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    if (!_pools[pid_].defined) revert PoolUndefinedErr(pid_);
    ...
    whenNotPaused()
  {
    ...
    _beneficiaries[pid_][sender].rate = pool.baseRate + pool.boostRate;
    _beneficiaries[pid_][sender].locked = true;
    _beneficiaries[pid_][sender].lockedAt = block.timestamp;

    emit Locked(pid_, sender, _beneficiaries[pid_][sender].rate);
  }
...
interface IDeFiMerkleVestingActions {
   ...
   */
  function recoverBeneficiary(uint256 pid_, address beneficiary_, bool disabled_, uint256 withdrawn_, bool locked_, uint256 rate_) external;
