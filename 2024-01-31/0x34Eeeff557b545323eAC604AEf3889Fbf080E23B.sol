...
contract LongCEOGold is Ownable, ERC721Enumerable {
    // total sales
    uint256 public TOTAL_PRESALE_QTY = 300;
    uint256 public PRE_SALES_PRICE = 1.5 ether;
    // Whitelist to mint in presale
    bool public whitelistOpen = true;
    mapping(address => bool) public isWhiteList;
    mapping(address => bool) public mktWallet;
    // pre minter
    mapping(address => uint256) public preSalesMinterToTokenQty;

    // max number of NFTs every wallet can buy
    uint256 public max_qty_per_minter_in_pre_sales = 1;
    ...
    constructor() ERC721("LongCEO Gold", "LONGCEOGOLD") {
        mktWallet[msg.sender] = true;
    }
    ...
    function mint(uint256 _mintQty)
        ...
        callerIsUser
    {
        require(whitelistOpen, "Not Allow");
        require(isWhiteList[msg.sender], "Not Whitelist");
        require(msg.value == PRE_SALES_PRICE, "Invalid Balance");
        require(preSalesMintedQty + _mintQty <= TOTAL_PRESALE_QTY,"Exceed sales max limit!");
        require(preSalesMinterToTokenQty[msg.sender] + _mintQty <= max_qty_per_minter_in_pre_sales,"Exceed max mint per minter!");
    ...
    function setQtyPerMinterPreSales(uint256 _preQty) external onlyOwner {
        max_qty_per_minter_in_pre_sales = _preQty;
    }

    // set total supply NFT
    function setTotalSupply(uint256 _pre) external onlyOwner {
        TOTAL_PRESALE_QTY = _pre;
    }
    ...
    // set price mint
    function setPriceMint(uint256 _amountPre) external onlyOwner {
        PRE_SALES_PRICE = _amountPre;
    }

    // set whitelist for presale
    function setWhitelist(address[] memory _add, bool _status) public onlyOwner {
        for (uint i = 0; i < _add.length; i++) {
            isWhiteList[_add[i]] = _status;
        }
    }
    // set whitelist for presale
    function setMktWallet(address _add) public onlyOwner {
        mktWallet[_add] = true;
    }
