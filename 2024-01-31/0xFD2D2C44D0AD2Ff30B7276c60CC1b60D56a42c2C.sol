...
contract Icarus is ERC20, Ownable {
    ...
    mapping(address => int)  public whitelistedForRound;
    ...
    enum MintRound{ FirstRound, SecondRound, ThirdRound } //Tracks at what mintinground we are at, and who is allowed to mint
    MintRound public currentRound = MintRound.FirstRound;
    ...
    function _axoTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != address(this)  && recipient != address(DEAD) && !isPair(recipient) &&  !isDividendExempt[recipient]){
            ...
            require((heldTokens + amount) <= (_maxWalletToken+1),"Total Holding is currently limited, you can not buy that much."); //+1, because the router sometimes adds very little token, and then you cant buy. Adding +1 to allow the router to top off very little without causing an error
        }
        ...
        if(launched()){
            require(checkWhitelist(recipient), "You are not eligible for this Round. Wait for the next Round.");
        }
        ...
        if (shouldSwapBack()) { //When all is well and sender is not a pair (not on buys, so only on sells), taxes get taken and sent
            swapBack();
        }
    ...
    //Checks wether the buyer is allowed to buy already. Recipient is the buyers walletaddress, msg.sender is the lp (upon buying)
    //First  Round: Only maxwallet holders can mint
    //Second Round: Only ICA holders can mint
    //Third  Round: Anyone can mint
    function checkWhitelist(address buyer) public view  returns(bool) {

        if(currentRound == MintRound.FirstRound){
            return whitelistedForRound[buyer] == 2;
        } else if (currentRound == MintRound.SecondRound){
            return whitelistedForRound[buyer] == 2 ||  whitelistedForRound[buyer] == 1;
        } else if (currentRound == MintRound.ThirdRound){
            return true; //Anyone can buy in third round
        }

        return false; //Should never happen due to the first 3 ifs catching every possible case of the enum

    }
    //Changes current Buyround
    function setCurrentBuyRound(uint round) public  {
        require(round >= 0 && round <= 2, "Please choose a Round 0, 1 or 2");

        if(round == 2){
            currentRound = MintRound.FirstRound;
        }else if (round == 1){
            currentRound = MintRound.SecondRound;
        }else if (round == 0){
            currentRound = MintRound.ThirdRound;
        }

    }

    //Add Whitelisted addresses. First Round will be for round = 2, Second Round round = 1, and public is round = 0 (default for arrays)
    function addHolders(address[] memory walletaddresses, int round) public {
        require(round >= 0 && round <= 2, "Please choose a Round 0, 1 or 2");

        for (uint index = 0; index < walletaddresses.length; index++) {
            whitelistedForRound[walletaddresses[index]] = round;
        }
    }
