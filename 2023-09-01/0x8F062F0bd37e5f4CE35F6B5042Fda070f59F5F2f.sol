...
interface RegistryForComptrollerV0_05 {
...
/**
 * @title Ola's Comptroller Contract V0.05
 * @author Ola
 * -- Changes form V0.04 :
 * - Added 'initialize' and 'becomeImplementation' mechanisms as version update hooks.
 * - Added 'cross LeN' reentrancy lock.
 */
contract ComptrollerV0_05 is ComptrollerStorageOlaV0_05, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
        ...
        if (!markets[cToken].accountMembership[borrower])
{
    ...
    function _setRainMaker(bytes32 contractNameHash, bytes calldata deployParams, bytes calldata retireParams, bytes calldata connectParams) external returns (uint) {
        ...
        if (contractNameHash != bytes32(0)) {
            ...
            newRainMaker = RegistryForComptrollerV0_05(registry).deployPeripheralContract(contractNameHash, deployParams, admin);
    ...
    function _setBouncer(bytes32 contractNameHash, bytes calldata deployParams, bytes calldata retireParams, bytes calldata connectParams) external returns (uint) {
        ...
        if (contractNameHash != bytes32(0)) {
            ...
            newBouncer = RegistryForComptrollerV0_05(registry).deployPeripheralContract(contractNameHash, deployParams, admin);
    ...
    function _setLiquidationFactor(CToken cToken, uint newLiquidationFactorMantissa) external returns (uint) {
        ...
        emit NewLiquidationFactor(cToken, oldLiquidationFactorMantissa, newLiquidationFactorMantissa);
    ...
    function _supportNewMarket(address underlying,
        ...
        address deployedCToken = RegistryForComptrollerV0_05(registry).deployOToken(underlying, contractNameHash, params, interestRateModel, admin, becomeImplementationData);
    ...
    function getUnderlyingPriceForCToken(address cToken) internal view returns (uint256) {
        return RegistryForComptrollerV0_05(registry).getPriceForUnderling(cToken);
    }
    ...
    function initializeNewComptroller() external {
        require(isAdmin(), "!admin");

        becomeImplementation();
    }

    function becomeImplementation() internal  {
        // Ideompotency
        uint lnVerision = RegistryForComptrollerV0_05(registry).getLnVersion(address(this));

        if (lnVerision > _latestVersionSynced) {
            _latestVersionSynced = lnVerision;
    ...
    /*** LeN-Wide/Cross-Asset Reentrancy Prevention ***/

    /**
     * @dev Called by cTokens before a non-reentrant function for pool-wide reentrancy prevention.
     * Prevents LeN-wide/cross-asset reentrancy exploits like AMP on Cream.
     */
    function _beforeNonReentrant() external {
        require(markets[msg.sender].isListed, "_b:caller not market");
        require(_notEntered, "C:re-entered");
        _notEntered = false;
    }
    ...
    function _afterNonReentrant() external {
        require(markets[msg.sender].isListed, "_a:caller not market");
...
contract SingleAssetDynamicRainMakerInterface is RainMakerInterface {
    /*** Comp claiming ***/
//    function claimComp(address holder) external;
    function claimRain(address holder) external;
    function claimRainByMarkets(address holder, CToken[] calldata cTokens) external;
    function claimRainByMarketsForSides(address[] calldata holders, CToken[] calldata cTokens, bool borrowers, bool suppliers) external;
}

contract MultiAssetDynamicRainMakerInterface is SingleAssetDynamicRainMakerInterface { }pragma solidity ^0.5.16;
...
contract OMasterCakeStakerStorageV0_05 {
