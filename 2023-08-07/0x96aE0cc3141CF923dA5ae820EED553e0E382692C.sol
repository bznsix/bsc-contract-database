...
contract MetastrikeBeginnerBox is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    event UnBox(address user, uint256 tokenId);
    ...
    function unbox(uint256 _tokenId) public nonReentrant {
        require(!isUnboxed[_tokenId], "already unboxed");
        require(ownerOf(_tokenId) == msg.sender, "not owner");

        _burn(_tokenId);
        isUnboxed[_tokenId] = true;
        emit UnBox(msg.sender, _tokenId);
    }
}
