...
contract MetastrikeGenesisBox is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function adminSetBattlePass(address _battlePass) public onlyOwner {
        require(_battlePass != address(0), "invalid battlepass");
    ...
    function buyBoxFromBattlePass(address _user) public nonReentrant {
        require(battlePass != address(0), "invalid battle pass");
        require(msg.sender == battlePass);
        require(price != 0, "price must be set");
        require(block.timestamp >= startTime, "sale not open yet");
        uint256 tokenId = _tokenIdCounter.current();
        require(tokenId < MAX_SUPPLY, "Exceeds MAX SUPPLY");
