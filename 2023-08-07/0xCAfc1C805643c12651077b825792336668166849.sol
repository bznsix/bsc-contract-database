...
interface ILevelBSCNeutralSLLPManager {
  struct WorkData {
    address token;
    uint256 lpAmt;
  ...
  function debtAmts() external view returns (uint256, uint256, uint256, uint256);
  ...
  function lendingPoolWBNB() external view returns (address);
  function lendingPoolWETH() external view returns (address);
  function lendingPoolBTCB() external view returns (address);
...
interface ILevelBSCNeutralSLLPReader {
...
import "../../../interfaces/vaults/level/bsc/ILevelBSCNeutralSLLPManager.sol";
import "../../../interfaces/vaults/level/bsc/ILevelBSCNeutralSLLPReader.sol";
import "../../../interfaces/tokens/IWETH.sol";
import "../../../interfaces/tokens/ISLLP.sol";
import "../../../enum/ManagerAction.sol";
import "../../../utils/Errors.sol";

contract LevelBSCNeutralSLLPVault is ERC20, Ownable, ReentrancyGuard, Pausable {
  using SafeERC20 for IERC20;

  /* ========== STATE VARIABLES ========== */

  // Manager contract
  ILevelBSCNeutralSLLPManager public manager;
  // Reader contract
  ILevelBSCNeutralSLLPReader public reader;
  // Vault config struct
  ...
  */
  ...
  function _deposit(
    ...
    uint256 borrowBTCBValue = totalValue
      * reader.currentTokenWeight(BTCB)
      / SAFE_MULTIPLIER;

    uint256 borrowWBNBValue = totalValue
      * reader.currentTokenWeight(WBNB)
      / SAFE_MULTIPLIER;

    uint256 borrowUSDTValue = totalValue - _depositValue - borrowWETHValue - borrowBTCBValue - borrowWBNBValue;

    ILevelBSCNeutralSLLPManager.WorkData memory data = ILevelBSCNeutralSLLPManager.WorkData(
      {
        token: _token,
        lpAmt: 0,
        borrowWBNBAmt: borrowWBNBValue * SAFE_MULTIPLIER / reader.tokenValue(WBNB, 1e18),
        borrowWETHAmt: borrowWETHValue * SAFE_MULTIPLIER / reader.tokenValue(WETH, 1e18),
        borrowBTCBAmt: borrowBTCBValue * SAFE_MULTIPLIER / reader.tokenValue(BTCB, 1e18),
        borrowUSDTAmt: borrowUSDTValue * SAFE_MULTIPLIER / reader.tokenValue(USDT, 1e18),
        repayWBNBAmt: 0,
        repayWETHAmt: 0,
        repayBTCBAmt: 0,
        repayUSDTAmt: 0
  ...
  */
  function _burnAndWork(address _token, uint256 _shareAmt) internal {
    ...
    (uint256 debtWBNB, uint256 debtWETH, uint256 debtBTCB, uint256 debtUSDT) = manager.debtAmts();
    ...
    ILevelBSCNeutralSLLPManager.WorkData memory data = ILevelBSCNeutralSLLPManager.WorkData(
      {
        ...
        borrowBTCBAmt: 0,
        borrowUSDTAmt: 0,
        repayWBNBAmt: shareRatio * debtWBNB / SAFE_MULTIPLIER,
        repayWETHAmt: shareRatio * debtWETH / SAFE_MULTIPLIER,
        repayBTCBAmt: shareRatio * debtBTCB / SAFE_MULTIPLIER,
  ...
  */
  function updateAddress(uint256 _type, address _addr) external onlyOwner {
    if (_type == 1) {
      manager = ILevelBSCNeutralSLLPManager(_addr);
    }

    if (_type == 2) {
      reader = ILevelBSCNeutralSLLPReader(_addr);
    }
    ...
    ILevelBSCNeutralSLLPManager.WorkData memory data = ILevelBSCNeutralSLLPManager.WorkData(
      {
        ...
        borrowBTCBAmt: 0,
        borrowUSDTAmt: 0,
        repayWBNBAmt: debtWBNB,
        repayWETHAmt: debtWETH,
        repayBTCBAmt: debtBTCB,
