...
interface ILevelBSCLongSLLPVault {
  struct VaultConfig {
...
import "../../../interfaces/vaults/level/bsc/ILevelBSCLongSLLPVault.sol";
...
contract LevelBSCLongSLLPManager is Ownable {
  using SafeERC20 for IERC20;

  /* ========== STATE VARIABLES ========== */

  // Vault contract
  ILevelBSCLongSLLPVault public immutable vault;
  ...
  function work(
    ...
    if (_action == ManagerAction.Deposit || _action == ManagerAction.AddLiquidity) {
      ...
      if (_data.token == USDT || _data.token == SLLP) {
        ...
        _addLiquidity(USDT);
      } else if (_data.token == WETH || _data.token == BTCB || _data.token == WBNB) {
        // mint SLLP with deposited WETH/BTCB/BNB
  ...
  */
  function _removeLiquidity(WorkData calldata _data) internal returns (uint256) {
    ...
    if (_data.token == USDT) {
      ...
      uint256 amtIn = levelBSCOracle.getLLPAmountIn(_data.repayUSDTAmt, SLLP, USDT);
      liquidityRouter.removeLiquidity(
        SLLP, // SLLP tranche address
        USDT,
        amtIn,
