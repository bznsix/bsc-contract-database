...
abstract contract FeeManager {
    ...
    uint256 public constant WITHDRAWAL_MAX = 100000;
    ...
    uint256 public constant STRATEGIST_FEE = 5;
    uint256 public privacyFee = 900;
...
contract StratManager is Ownable, Pausable {
    ...
    address public privacyFeeRecipient;
    ...
    constructor(
        ...
        address _unirouter,
        address _vault,
        address _privacyFeeRecipient
    ) public {
        keeper = _keeper;
        strategist = _strategist;
        unirouter = _unirouter;
        vault = _vault;
        privacyFeeRecipient = _privacyFeeRecipient;
    ...
    function setPrivacyFeeRecipient(address _privacyFeeRecipient) external onlyOwner {
        privacyFeeRecipient = _privacyFeeRecipient;
    }
...
contract StrategyMasterChefLP is StratManager, FeeManager {
    ...
    constructor(
        ...
        address _wrapped,
        address _output
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _privacyFeeRecipient) public {
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (tx.origin == owner() || paused()) {
            ...
            uint256 withdrawalFeeAmount = wantBal.mul(WITHDRAWAL_FEE).div(WITHDRAWAL_MAX);
            IERC20(want).safeTransfer(vault, wantBal.sub(withdrawalFeeAmount));
            IERC20(want).safeTransfer(privacyFeeRecipient, withdrawalFeeAmount);

        }
    }

    // compounds earnings and charges performance fee
    function harvest() external whenNotPaused onlyEOA  onlyWhitelisted{
    ...
    function chargeFees() internal {
       uint256 toWrapped =
            IERC20(output).balanceOf(address(this)).mul(100).div(MAX_FEE);
        ...
        uint256 privacyFeeAmount = wrappedBal.mul(privacyFee).div(MAX_FEE);
        IERC20(wrapped).safeTransfer(privacyFeeRecipient, privacyFeeAmount);
