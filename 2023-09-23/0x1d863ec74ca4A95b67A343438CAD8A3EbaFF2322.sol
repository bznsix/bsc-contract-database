...
contract RoyaltiesV2Impl is AbstractRoyalties, RoyaltiesV2 {
    ...
    function getSTRMV2Royalties(uint256 id) override external view returns (LibPart.Part[] memory) {
        return royalties[id];
    }
...
library LibRoyaltiesV2 {
    /*
     * bytes4(keccak256('getSTRMV2Royalties(uint256)')) == 0xcad96cca
     */
...
contract ERC721STRMMinimal is ERC721BaseMinimal {
    /// @dev true if collection is private, false if public
    bool isPrivate;

    event CreateERC721STRM(address owner, string name, string symbol);
    event CreateERC721STRMUser(address owner, string name, string symbol);

    function __ERC721STRMUser_init(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators, address transferProxy, address lazyTransferProxy) external initializer {
        __ERC721STRM_init_unchained(_name, _symbol, baseURI, contractURI, transferProxy, lazyTransferProxy);
        ...
        isPrivate = true;
        emit CreateERC721STRMUser(_msgSender(), _name, _symbol);
    }

    function __ERC721STRM_init(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address transferProxy, address lazyTransferProxy) external initializer {
        __ERC721STRM_init_unchained(_name, _symbol, baseURI, contractURI, transferProxy, lazyTransferProxy);

        isPrivate = false;
        emit CreateERC721STRM(_msgSender(), _name, _symbol);
    }

    function __ERC721STRM_init_unchained(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address transferProxy, address lazyTransferProxy) internal {
        _setBaseURI(baseURI);
