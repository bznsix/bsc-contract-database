...
contract FarmBoosterProxyFactory {
    ...
    address public immutable theXToken;
     ...
     * @param _theXToken: the address of the x token
     */
    constructor(
    ...
    function createFarmBoosterProxy() external {
        ...
        FarmBoosterProxy(farmBoosterProxyAddress).initialize(msg.sender, Farm_Booster, masterchefV2, theXToken);
...
contract FarmBoosterProxy is ReentrancyGuard {
    ...
    function harvestTheX() internal {
        uint256 theXBalance = theXToken.balanceOf(address(this));
        if (theXBalance > 0) {
            theXToken.safeTransfer(msg.sender, theXBalance);
        }
    }
}
// SPDX-License-Identifier: MIT
