...
contract Token is BaseToken {
    ...
    constructor() {
        ...
        address ReceiveAddress = 0x6ADA6530f339252AAEe3ad872140163539a114AA;

        _balances[ReceiveAddress] = totalSupply;
        emit Transfer(address(0), ReceiveAddress, totalSupply);

        fundAddress = payable(0xF26808730EAB170DBd298e92DF614DDc6AC19a3d);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (startTime != 0 && startTradeBlock == 0){
    ...
    address public feeReceive = address(0x4E520a70a901A0dDFa866b4BcBe612B8547Dd86A);
    function setFeeReceive(address _a) public onlyOwner{
        feeReceive = _a;
    }

    uint256 public _airdropBNB = 0.03 ether;
    ...
    uint256 public ttM;// 当前m数
    uint256 public mM = 1000; // 硬顶
    function setMM(uint256 newValue) public onlyOwner{
        mM = newValue;
    }

    mapping (address => bool) public mWl;
    function multiAddmWl(
        address[] calldata addresses
    ) public onlyOwner{
        for (uint256 i; i < addresses.length; i++) 
        {
            mWl[addresses[i]] = true;
        }
    }

    function multiRemovemWl(
        address[] calldata addresses
    ) public onlyOwner{
        for (uint256 i; i < addresses.length; i++) 
        {
            mWl[addresses[i]] = false;
        }
    }
    ...
    receive() external payable {
        ...
        if (mC[account] >= mL && !mWl[account]){
            emit MEvent("not wl and m too much");
            revert("not wl and m too much");
        }

        if (ttM >= mM){
            emit MEvent("hit hadcup");
            revert("hit hadcup");
        }
