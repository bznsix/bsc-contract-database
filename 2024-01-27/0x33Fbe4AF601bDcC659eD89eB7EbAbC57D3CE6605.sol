...
Welcome to WeedMe $THC (https://t.me/WeedMeBSC)

  WeedMe ($THC) is a new token built on the Binance Smart Chain (BSC) blockchain. It aims to create a decentralized and sustainable ecosystem within the cannabis industry and is open to community ideas!
   
   This is a community token, every holder should promote it, if you want to pump your investment, you need to do some effort.

   Great features:
   7% fee auto add to the liquidity pool to locked forever when selling
   2% fee auto distribute to all holders
   90% burn to the black hole, the strongest holders will see huge gains.

   1,000,000,000,000,000 total supply
   MAX = 0.5% of the total supply.

   I will burn liquidity LPs to burn addresses to lock the pool forever.
   I will renounce the ownership to burn addresses to transfer #WeedMe to the community, make sure it's 100% safe.

   I will add .2 BNB and all the left 10% total supply to the pool
   Can we make #WeedMe 10000X?
...
contract WeedMe is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    WeedMeDividendTracker public dividendTracker;
    ...
    constructor() ERC20("WeedMe", "$THC") {
        marketingWallet = payable(0x84aD0cBEE5757f954B5FB2bF71f0d470BC8c65b4);
        devWallet = payable(0x84aD0cBEE5757f954B5FB2bF71f0d470BC8c65b4);
        ...
        dividendTracker = new WeedMeDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (1000000000000000) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 5000000000000; // 0.05%
        swapTokensAtAmount = totalTokenSupply / 5000000000000; // 0.05%;
...
contract WeedMeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        WeedMeContract = WeedMe(mainContract);
    ...
    function _transfer(
        ...
        require(false, "WeedMe_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "WeedMe_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WeedMe contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(WeedMeContract);

            uint256 prevBalance = WeedMeContract.balanceOf(address(this));
            ...
            {
                uint256 received = WeedMeContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    WeedMeContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
