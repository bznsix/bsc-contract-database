...
contract SClix is Ownable, ERC1155 {
	...
	function lockedBalanceOfBatch(address userAddress) public view returns(uint[] memory balance, uint[] memory locked) {
		balance = new uint[](9);
		locked = new uint[](9);

		for(uint i = 1; i <= 9; i++) {
			balance[i-1] = balanceOf(userAddress, i);
			locked[i-1] = lockedNfts[userAddress][i];
		}
