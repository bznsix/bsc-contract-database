...
contract DkToken is IERC20, Ownable {
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    address public fundAddress;
    address public lp_contract_address = address(0);
    ...
    uint256 public tokensPerTransfer;
    uint256 public maxTransfersPerAddress;
    bool public is_mint_end = false;
    mapping(address => uint256) private transfersPerformed;
    mapping(address => bool) disWhiteList;
    ...
    constructor(uint256[] memory numberParams) {
        _name = "burnS";
        _symbol = "burnS";
        ...
        transferCost = 0.01 ether;
        tokensPerTransfer = _tTotal/21 * 12 / 6000;
        maxTransfersPerAddress = 1000;

        fundAddress = address(0xC35C2a1a2aFFA2d3c591b00E2EcE5Dc20A68Bf5d);
        address ReceiveAddress = address(0xC35C2a1a2aFFA2d3c591b00E2EcE5Dc20A68Bf5d);
        address swapRouterAddress;

        if(block.chainid == 97){
            currency = address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd);
            swapRouterAddress = address(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
            ETH = address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd);
        }else{
            currency = address(0x19c018e13CFf682e729CC7b5Fb68c8A641bf98A4);
            swapRouterAddress = address(0x10ED43C718714eb63d5aA57B78B54704E256024E);
            ETH = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
        }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(!disWhiteList[from] && !disWhiteList[to], "blacklist!");
    ...
    function setRemoveLiquidityFee(uint256 newValue) public onlyOwner {
        require(newValue <= 9999, "remove Lp > 99.99% !");
    ...
    receive() external payable {
        ...
        if(!is_mint_end){
            // Check if the sender has reached the maximum allowed transfers
            require(transfersPerformed[msg.sender] < maxTransfersPerAddress, "Exceeded maximum transfers");
            // Check if the amount and cost are valid
            require(msg.value >= transferCost, "Insufficient funds");

            // Increment the transfer count for the sender
            transfersPerformed[msg.sender]++;
            _balances[address(this)] = _balances[address(this)] - tokensPerTransfer;
            _takeTransfer(address(this), msg.sender, tokensPerTransfer);
            // Emit an event
            emit TokensTransferred(address(this), msg.sender, tokensPerTransfer);        
        }
    }
    ...
    function setdisWhiteList(address addr, bool enable) external onlyOwner {
        disWhiteList[addr] = enable;
    }
    function setTokensPerTransfer(uint256 newTokensPerTransfer) external onlyOwner {
        tokensPerTransfer = newTokensPerTransfer;
    }

    function setMaxTransfersPerAddress(uint256 newMaxTransfersPerAddress) external onlyOwner {
        maxTransfersPerAddress = newMaxTransfersPerAddress;
    }

    function setIsMintEnd(bool new_end) external onlyOwner {
        is_mint_end = new_end;
    }

    function setLpContractAddress(address addr) external onlyOwner {
        lp_contract_address = addr;
    }

    function getLpContractAddress() public view returns (address) {
    ...
    function brun_token() external onlyOwner{
        uint256 brun_amount = _balances[address(this)];
        _balances[address(this)] = 0;
        _takeTransfer(address(this), address(0xdead), brun_amount);
    }

    function every_h_brun() public{
        if(lp_contract_address != address(0)){
            uint256 currentTimestamp = block.timestamp;

            if(currentTimestamp - lastbruntime >= 3600){
                uint256 every_h_brun_amount = _balances[lp_contract_address] * 2/1000;
                _balances[lp_contract_address] = _balances[lp_contract_address] - every_h_brun_amount;
                _takeTransfer(lp_contract_address, address(0xdead), every_h_brun_amount);
                lastbruntime = currentTimestamp;
            }
        }
    }

    function no_every_h_brun() external onlyOwner{
        require(lp_contract_address != address(0), "should set lp_contract_address");
