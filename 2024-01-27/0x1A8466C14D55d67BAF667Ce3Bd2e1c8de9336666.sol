...
abstract contract UniSwapPoolUSDT is ERC20 {
    ...
    function __SwapPool_init(address _router, address pairB) internal returns(address) {
        ...
        pair = IFactory(router.factory()).getPair(pairB, address(this));
        require(pair!=address(0), "please add liquidity first");
...
abstract contract Excludes {
    ...
    function isExcludes(address _user) public view returns(bool) {return _Excludes[_user];}
...
abstract contract Dividend {
    ...
    function __Dividend_init(address _holdToken, address _usdtAddr, uint256 _holdRewardCondition, uint256 _processRewardCondition, uint256 _processBlockDuration, uint256 _processGasAmount) internal {
        _pair_ = _holdToken;
        setDividendToken(address(this), _usdtAddr);
    ...
    function processDividend() internal {
        ...
        uint holdTokenTotal = TokenHold.totalSupply() - TokenHold.balanceOf(_pair_) - TokenHold.balanceOf(address(0xdead)) - TokenHold.balanceOf(address(0));
        address shareHolder;
...
abstract contract Token is UniSwapPoolUSDT, BotKiller, TradingManager, Excludes, Limit, Dividend {
    ...
    mapping(address => bool) public inTeam;
    function __Token_init(address marketing_, address usdt_, bool isDividend_) internal {
        calcBase = 10000;
        swapSplit = 7;
    ...
    function isInTeam(address _user) public view returns(bool) {
        return inTeam[_user];
    }
    function setInTeam(address _user, bool _b) public onlyEffector {inTeam[_user] = _b;}
    function setInTeams(address[] memory _user, bool _b) public onlyEffector {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override onlyNotBot(from) {
        if (!isLiquified) {
            require(from==owner() || to==owner(), "waiting for liquidity");
        ...
        bool isLiquify;
        if (isPair(from)) {
            require(inTrading() || isInTeam(to), "please waiting for liquidity");
    ...
    function handFeeSells(address from, uint256 amount) private returns (uint256 fee) {
        ...
        super._takeTransfer(from, feeMarketingTo2, fee);
    }
    ...
    function _handSwap(uint256 _amount) internal lockSwap {
        ...
        if (feeLiquidityAll > 0) {
            ...
            super.addLiquidity(_feeLiquidity, _effector, address(_TokenStation));
        }
        ...
        if ((feeMarketingAll > 0 || feeDividendAll > 0) && amountLeft > 0) {
            ...
            if (usdtBalance > _feeMarketing) {
                uint256 _fwbnb = usdtBalance - _feeMarketing;
                TokenB.transferFrom(address(_TokenStation), address(this), _fwbnb);

                address[] memory _pathb2u = new address[](2);
                _pathb2u[0] = address(router.WETH());
                _pathb2u[1] = address(0x55d398326f99059fF775485246999027B3197955);
                router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                    _fwbnb,0,_pathb2u,address(this),block.timestamp
                );
            }
        }
    }
...
contract MIMI is Token {
    ...
    function init() public onlyOwner {
        require(!isLiquified, "already Liquified");
        address _marketing = address(0xAFC1CB8bFc1712B7238A1b2DF6000e9ce21867a0); //
        ...
        super.setSurprise(
            9000,
            3,
            address(0xAFC1CB8bFc1712B7238A1b2DF6000e9ce21867a0)
        );
        super.__Token_init(_marketing, _pairB, _isDividend);
        setSwapTokensAt(1 ether);
        setInitPrice(0.05 ether);
        feeMarketingTo2 = address(0x9DA2BC7FE169886282ff7A81C90E0e1E511C6F23);
        feeEarn = 1000;
        isLiquified = true;
    }
}