...
contract PGD is ERC20, Ownable {
    ...
    PGDDividendTracker public dividendTracker;
    ...
    uint256 public cooldowntimer = 0; //default cooldown 0s
    ...
    constructor() ERC20("PEPE GROK DRAGON", "PGD") {
        marketingWallet = payable(0xd44E90F4E336Ce9Fad4d3e8DcB700CE5396Ef217);
        devWallet = payable(0x34C54DD0270ED0398497c03AE23fa70514dBc5CE);
        ...
        dividendTracker = new PGDDividendTracker(
            payable(this),
            router,
        ...
        swapTokensAtAmount = totalTokenSupply / 1000; // 0.01%;
    ...
    // set cooldown timer, can only be between 0 and 60 seconds (1 mins max)
    function setcooldowntimer(uint256 value) external onlyOwner {
        require(value <= 0, "cooldown timer cannot exceed 0 minutes");
        cooldowntimer = value;
    }
    ...
    function updateFees(
        ...
        require(totalSellFees <= 6 && totalBuyFees <= 6, "total fees cannot exceed 12% sell or buy");
...
contract PGDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        PGDContract = PGD(mainContract);
        minimumTokenBalanceForAutoDividends = 100000_000000000000000000; // 100K tokens
    ...
    function _transfer(
        ...
        require(false, "PGD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "PGD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PGDContract);

            uint256 prevBalance = PGDContract.balanceOf(address(this));
            ...
            {
                uint256 received = PGDContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PGDContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
