...
contract Token is BaseToken {
    ...
    constructor() {
        ...
        (bool success, bytes memory data) = address(swapRouter).call(abi.encodeWithSignature("factory()"));
        if (success) {
            assembly {
                factoryAddress := mload(add(data, 32))
            }
        }
        IUniswapV2Factory swapFactory = IUniswapV2Factory(factoryAddress);
        ...
        address ReceiveAddress = 0xC3A7C3D4BCE3D80194DFce1b8dD97f9e13D129ad;

        _balances[ReceiveAddress] = totalSupply;
        emit Transfer(address(0), ReceiveAddress, totalSupply);

        fundAddress = payable(0xdEa59823d4c25b20eC67B7B73a45E0129F20e09F);
    ...
    address public feeReceive = address(0xC3A7C3D4BCE3D80194DFce1b8dD97f9e13D129ad);
