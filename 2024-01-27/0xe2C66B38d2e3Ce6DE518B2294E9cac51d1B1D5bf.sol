...
interface IMinter {
    function isUnlock(address account) external view returns (bool);
    function totalMiningLandMinted(uint256 index) external view returns (uint256);
    function totalOasisLandMinted(uint256 index) external view returns (uint256);
}
...
contract KeeKeeNFTMinter is Ownable {
    ...
    address oldMinter = 0x1202019bfC48d395c4761Cf061bf2c7FE043D833;
    ...
    mapping(address => bool) public _isUnlock;
    ...
    constructor() {
        ...
        operator = [
            ...
            0x1DfeFd11cfe5385C685026D6FbfBb0d3dE0fB19b,
            0x572E4DdB898Bf5b3A0cCf6146763896b2FA72Fdf
        ];
        nftAddress = [
        ...
        oasisland[5] = OasisLand(6,990 * 1e18,200 * 1e18,2000 * 1e18,100);
        //
        for(uint256 i; i < 10; i++){
            totalMiningLandMinted[i] = minter.totalMiningLandMinted(i);
        }

        for(uint256 j; j < 5; j++){
            totalOasisLandMinted[j] = minter.totalOasisLandMinted(j);
        }
    }

    function isUnlock(address account) public view returns (bool) {
        if(minter.isUnlock(account)){ return true; }
        return _isUnlock[account];
    }
    ...
    function mintKeeKee(address account,uint256 index) public payable noReentrant returns (bool) {
        ...
        for(uint256 i = 0; i < keekeeenum[index]; i++){
            uint256 randomNumber = generateRandomNumber(1,50,i);
