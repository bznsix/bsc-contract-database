...
contract DemoOApp is OApp {
    // srcEid => nonce => message
    mapping(uint32 => mapping(uint64 => string)) public data; // Our data, in this case a string.
    ...
    event MessageReceived(string message, uint64 nonce, uint32 senderEid, bytes32 sender, bytes32 _guid);
    ...
    function _lzReceive(
        ...
        data[_origin.srcEid][_origin.nonce] = abi.decode(payload, (string));
        ...
        emit MessageReceived(data[_origin.srcEid][_origin.nonce], _origin.nonce, senderEid, sender, _guid);
    }
}
// SPDX-License-Identifier: MIT
