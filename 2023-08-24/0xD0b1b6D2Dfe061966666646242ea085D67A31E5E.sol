...
contract TWAR is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public marketingAddress = 0x88A472C09384038D664462016952D22153268E92;
    address public penguinToken = 0xe2c1d433b2Bb58354eD4A5B5ad13127D03666888;
    ...
    constructor() ERC20("TWAR", "TWAR") {
        uint256 totalSupply = 10000000 * (10**18);
        swapTokensAtAmount = totalSupply.div(10**3);
    ...
    function _transfer(
        ...
        bool canSwap = (SellFeeAmount + LpRewardAmount + MarketingFeeAmount) >= swapTokensAtAmount;
        if( canSwap &&
            !swapping &&
            from != uniswapV2Pair &&
            from != owner() &&
            ...
            swapping = true;
            if(SellFeeAmount > 0){
                _buyAndBurn(SellFeeAmount);
                SellFeeAmount = 0;
            }
            if(LpRewardAmount > 0){
                _swapToken(LpRewardAmount, address(tokenHelper));
                LpRewardAmount = 0;
            }
            if (MarketingFeeAmount > 0) {
                swapAndLiquify(MarketingFeeAmount);
    ...
    function takeBuyFee(address from, uint256 amount) private returns (uint256 amountAfter) {
        ...
        uint256 LFee = amount.mul(buyLpFee).div(10000);
        uint256 MFee = amount.mul(buyMarketingFee).div(10000);
        uint256 feeSum = MFee.add(LFee);
        if (feeSum > 0) {
            super._transfer(from, address(this), feeSum);
            amountAfter = amountAfter.sub(feeSum);
            LpRewardAmount = LpRewardAmount.add(LFee);
    ...
    function takeSellFee(address from, uint256 amount) private returns (uint256 amountAfter) {
        ...
        if (SFee > 0) {
            ...
            SellFeeAmount = SellFeeAmount.add(SFee);
        }
    }

    function _swapToken(uint256 tokenAmount, address receiver) internal {
    ...
    function _buyAndBurn(uint256 tokenAmount) internal {
    ...
    function swapAndLiquify(uint256 tokens) private {
       // split the contract balance into halves
        ...
        uint256 newBalance = IERC20(bep20usdt).balanceOf(address(tokenReceiver)) - initialBalance;
        IERC20(bep20usdt).transferFrom(address(tokenReceiver), address(this), newBalance);
