...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address public _tokenDistributor1 = 0xcc8Fab1FF65CBc2Ea037e105D5A41376436f5FAd;

    uint256 public _buyLPDividendFee = 200;
    uint256 public _buyBlackFee = 100;
    uint256 public _buyCardFee = 0;
    uint256 public _sellLPDividendFee = 200;
    uint256 public _sellBlackFee = 0;
    uint256 public _sellCardFee = 100;
    
    uint256 public numTokensSellToFund = 100;
    ...
    constructor (
        ...
        holderRewardCondition = 20 * 10 ** IERC20(LPAddress).decimals();
    ...
    function _transfer(
        ...
        bool isRemove;
        if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
            if (0 < startTradeBlock && block.number < startTradeBlock + step) {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _sellLPDividendFee + _sellCardFee;
                if(_sellBlackFee > 0){
                    blackAmount = tAmount * _sellBlackFee / 10000;
                    tAmount -= blackAmount;
                }
            } else {
                swapFee = _buyLPDividendFee + _buyCardFee;
                if(_buyBlackFee > 0){
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        uint256 swapFee = _buyLPDividendFee + _sellLPDividendFee + _buyCardFee + _sellCardFee;
        ...
        uint256 lpBalance = uBalance * (_buyLPDividendFee + _sellLPDividendFee) / swapFee;
        USDT.transferFrom(address(_tokenDistributor), address(this), lpBalance);
        USDT.transferFrom(address(_tokenDistributor), address(_tokenDistributor1), uBalance - lpBalance);
    }
