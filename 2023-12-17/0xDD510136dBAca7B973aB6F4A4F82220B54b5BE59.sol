...
abstract contract AbsLPPool is Ownable {
    ...
    uint256 public v1Amount = 2500 ether;
    uint256 public v2Amount = 20000 ether;
    uint256 public v3Amount = 50000 ether;
    uint256 public v4Amount = 150000 ether;
    uint256 public v5Amount = 500000 ether;

    address public nftAddress;

    event nftTokenReward(
        address indexed account,
        uint256 indexed level,
        uint256 indexed tokenAmount
    );

    event nftPowerReward(
        address indexed account,
        uint256 indexed level,
        uint256 indexed powerAmount
    );
    ...
    uint256 public _lpReleaseDuration = 90 days;
    //
    uint256 private _speedUpCost;
    uint256 public _speedUpDuration = 10 days;
    ...
    function distributeMainReward(address invitor, uint256 rewardAmount)
        private
    {
        // Add power
    ...
    function distributeSuperRewards(
        address invitor,
        uint256 invLevel,
        uint256 amount
    ) private {
    ...
    function distributeSameLevelReward(address invitor, uint256 sAmount)
        private
    {
    ...
    function calculateNFTReward(uint256 userRewardLevel, uint256 amount)
        public
        pure
        returns (uint256)
    {
        if (userRewardLevel == 1) {
            return (amount * 1) / 100; // 1% reward for level 1
        } else if (userRewardLevel == 2) {
            return (amount * 2) / 100; // 2% reward for level 2
        } else if (userRewardLevel == 3) {
            return (amount * 3) / 100; // 3% reward for level 3
        } else if (userRewardLevel == 4) {
            return (amount * 4) / 100; // 4% reward for level 4
        } else if (userRewardLevel == 5) {
            return (amount * 5) / 100; // 5% reward for level 5
        } else {
            return 0; // No reward for other levels
        }
    }
    ...
    function checkForLevelUp(address invitor) public {
        if (referrals[invitor].length > 1) {
            uint256 totalReferralAmount = referralAmount[invitor] +
                depositAmount[invitor];
            uint256 currentLevel = userLevel[invitor];
            address[] memory referralArr = referrals[invitor];
            if (currentLevel == 0 && totalReferralAmount >= v1Amount * 2) {
                // v1
                uint256 v0 = 0;
                for (uint256 i; i < referrals[invitor].length; ++i) {
                    if (
                        referralAmount[referralArr[i]] +
                            depositAmount[referralArr[i]] >=
                        v1Amount
                    ) {
                        ++v0;
                    }
            ...
            } else if (currentLevel == 1 && totalReferralAmount >= v2Amount) {
                // v2
                uint256 v1 = 0;
                for (uint256 i; i < referrals[invitor].length; ++i) {
            ...
            } else if (currentLevel == 2 && totalReferralAmount >= v3Amount) {
                // v3
                uint256 v2 = 0;
                for (uint256 i; i < referrals[invitor].length; ++i) {
                    if (userLevel[referralArr[i]] >= 2) {
                        ++v2;
                    }
                }
            ...
            } else if (currentLevel == 3 && totalReferralAmount >= v4Amount) {
                // v4
                uint256 v3 = 0;
                for (uint256 i; i < referrals[invitor].length; ++i) {
                    if (userLevel[referralArr[i]] >= 3) {
                        ++v3;
                    }
                }
            ...
            } else if (currentLevel == 4 && totalReferralAmount >= v5Amount) {
                // v5
                uint256 v4 = 0;
                for (uint256 i; i < referrals[invitor].length; ++i) {
                    if (userLevel[referralArr[i]] >= 4) {
                        ++v4;
                    }
                }
    ...
    function getDirectList(address account)
        external
        view
    ...
    modifier onlyWhiteList() {
        address msgSender = msg.sender;
        require(
            msgSender == _fundAddress ||
                msgSender == _owner ||
                msgSender == nftAddress,
            "nw"
        );
        _;
    }
...
contract MintPool is AbsLPPool {
    constructor()
        AbsLPPool(
            ...
            //CUSD
            address(0x4145553a65276093CffB8522FBC5d0002a605b5F),
            //BOS
            address(0x66707B994a8E0Bd67d4E1621b91c3C5c43914B93),
            //NFT
            address(0x4a6189CC5E257561Ec173F06bf4E7658Ab8F4956),
            //DefaultInvitor
            address(0x3226750136a5D53c93dBFd5d652042f14B82b1f4),
            //Fund
            address(0x3226750136a5D53c93dBFd5d652042f14B82b1f4)
        )
    {}
