...
contract Earn is AccessControl {
     ...
     */
    function updateAllocationWithUpdate(uint8 stakeType, uint256 newAllocPoint) public onlyRole(GOVERNOR_ROLE) {
        uint256 allocPoint = poolInfo[stakeType].allocPoint;

        require(stakeType < 3, "Earn: invalid stake type");
        require(newAllocPoint != allocPoint, "Earn: AllocPoint is already this value");
        massUpdatePools();
     ...
     * @param stakeType The unique id of the pool.
     */
    function updatePool(uint8 stakeType) public returns (uint256) {
        require(stakeType < 3, "Earn: invalid stake type");
        PoolInfo storage pool = poolInfo[stakeType];

        if (block.timestamp <= pool.lastRewardTime) {
            return _allPoolInfo[stakeType].length - 1;
        }
    ...
    function stake(uint256 amount, uint8 stakeType) external {
        ...
        PoolInfo storage pool = poolInfo[stakeType];
        uint256 allPoolIndex = updatePool(stakeType);
        pool.stakeSupply += amountWithFee;
        _allPoolInfo[stakeType][allPoolIndex].stakeSupply += amountWithFee;
    ...
    function claimPayout(uint256 index) external {
        ...
        uint256 allPoolIndex = updatePool(staking.stakeType);
        ...
        if (block.timestamp < staking.endTime) {
            ...
            _allPoolInfo[stakingType][allPoolIndex].stakeSupply -= stakingAmount;
            uint256 activeDeposits = sHYDT.balanceOf(address(this));
