...
contract FAToken is ERC20Capped, Orchestrated {
	using SafeMath for uint256;

	uint public marketFee = 50;
	uint public burnFee = 50;
	uint public lpFee = 100;
	uint public levelFee = 50;

	address public marketFeeWallet;
	address public levelFeeWallet;
	...
	uint public liquidityDividendFeeTotal;

	constructor() ERC20Capped(1000000 * 1e18) ERC20("FA", "FA") {
		ERC20._mint(msg.sender, 1000000 * 1e18);
		marketFeeWallet = address(0x87c07A8fABB0B3eACed974BfDA88CDd78bD5D7fd);
		levelFeeWallet = address(0xe03d57e30fb8ABb5B8cA834A4683925b453A044c);
	...
	function setMarketFeeWallet(address _marketFeeWallet) public onlyOwner(){
		marketFeeWallet = _marketFeeWallet;
	}

	function setLevelFeeWallet(address _levelFeeWallet) public onlyOwner(){
		levelFeeWallet = _levelFeeWallet;
	}
	...
	function setLpDividendTracker(TokenDividendTracker _tokenDividendTracker) public onlyOwner() {
		lpDividendTracker = _tokenDividendTracker;
	}

	function updateLpMinimumTokenBalanceForDividends(uint256 val) public onlyOwner() {
		lpDividendTracker.updateMinimumTokenBalanceForDividends(val);
	}

	function lpExcludeFromDividends(address account) external onlyOwner() {
	...
	function _transfer(
		...
		if(!isContract(sender)) {
			_updateDividendTrackerLpBalance(sender);
		}
		if(!isContract(recipient)) {
			_updateDividendTrackerLpBalance(recipient);
		}
		...
		if (sender == address(lpDividendTracker) || recipient == address(lpDividendTracker)) {
			takeFee = false;
		}

		uint256 tokens = amount;
		if(takeFee) {
			if(_isSwapInclude[sender] || _isSwapInclude[recipient]) {
				if(marketFee > 0) {
				...
				if(burnFee > 0) {
					...
					super._transfer(sender, address(0x000000000000000000000000000000000000dEaD), burnFees);
					tokens = tokens.sub(burnFees);
				}
				if(lpFee > 0) {
					uint256 lpFees = amount.mul(lpFee).div(10000);
					super._transfer(sender, address(lpDividendTracker), lpFees);
					tokens = tokens.sub(lpFees);
					lpDividendTracker.distributeCAKEDividends(lpFees);
					liquidityDividendFeeTotal = liquidityDividendFeeTotal.add(lpFees);
				}
				if(levelFee > 0) {
					uint256 levelFees = amount.mul(levelFee).div(10000);
					super._transfer(sender, address(levelFeeWallet), levelFees);
					tokens = tokens.sub(levelFees);
				}
