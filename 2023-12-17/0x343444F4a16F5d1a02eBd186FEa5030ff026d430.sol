...
contract MoonAirV2 is Context, Ownable, ReentrancyGuard {
    ...
    event matchingBonusPaid(address indexed from, address indexed to, uint256 value, uint256 date);
    ...
    event insuranceBuyed(address indexed beneficiary, uint256 usdAmount, uint256 insurance, uint256 package);
    ...
    modifier minAmountNFT(uint256 package) {
        ...
        require(addedCanVoteWalletsCount < 201, "No more nft can be added.");
        _;
    }
    ...
    constructor(IERC20 token, IERC20 usdt, address payable feeaddr, address payable owneraddr, UsersStorage userstorage) {
      _token = token;
      _usdt = usdt;
      _users = userstorage;
      ...
      career.push(careerInfo(200, 10000000, 19999999, 150000, 5)); //20%
      career.push(careerInfo(205, 20000000, 29999999, 200000, 5)); //20.5%
      career.push(careerInfo(210, 30000000, 49999999, 250000, 5)); //21%
      career.push(careerInfo(215, 50000000, 99999999, 300000, 5)); //21.5%
      career.push(careerInfo(220, 10000000, 100000000, 350000, 5)); //22%
    }

    function buyProduct(uint256 usdAmount, uint256 typeAir, uint256 insurance) public payable minAmount(usdAmount) activeUser nonReentrant {
      address beneficiary = _msgSender();

      require(_usdt.balanceOf(beneficiary) >= usdAmount+insurance, "Not enough tokens");
      require(_usdt.allowance(beneficiary,address(this)) >= usdAmount+insurance, "Please allow fund first");
      bool success = _usdt.transferFrom(beneficiary, address(this), usdAmount+insurance);
      if (!success) {
        revert packageBuy__Failed();
          ...
          if (insurance > 0) {
            ...
            emit insuranceBuyed(beneficiary, usdAmount, insurance, typeAir);
          }

          address payable mySponsor = payable(_users.referral_tree(beneficiary));
          //if (_users.haveValue(mySponsor)) {
            uint256 reward = _addReferralBonus(true, beneficiary, mySponsor, usdAmount);
          //}	
          _compressionBonus(true, usdAmount, mySponsor, 0, reward, 1);
    ...
    function withdrawNFTProfit(uint256 from, uint256 to, bool clean) public payable onlyOwner nonReentrant {
      uint256 share = totalProfitNFT/200; 
      for(uint256 i = 0; i < voteWallets.length; i++) {
        if (voteWallets[i].notZero() && share > 0 && from <= i && to >= i) {
    ...
    function _compressionBonus(bool needTransfer, uint256 usdAmount, address payable user, uint256 prevPercent, uint256 prevBonus, uint256 line) internal {
      ...
      if (_users.contains(user)) {
        ...
        //if (_users.haveValue(user)) {
          ...
          if (line >= 2) {
            ...
            } else if (prevPercent == careerPercent) {
                uint256 bonus = prevBonus*10/100;
                assert(_users.addRefBonus(user, bonus, line));
                if (needTransfer && bonus > 0) {
                  _usdt.transfer(user, bonus);
                }
                emit matchingBonusPaid(_msgSender(), user, bonus, block.timestamp);
                prevBonus = bonus;
            }
          }
        //}
      ...
      if (_notZeroNotSender(mySponsor)) {
        line = line + 1;
        if (line < _lines) {
          _compressionBonus(needTransfer, usdAmount, mySponsor, prevPercent, prevBonus, line);
        }
      }
    }

    function _addReferralBonus(bool needTransfer, address user, address payable sponsor, uint256 usdAmount) internal returns(uint256) {
      uint256 reward;
    ...
    function setStakeM(address beneficiary, address sponsor, uint tokenAmount, uint bonus, uint256 duration, bool earnings) public payable onlyOwner {
      ...
      address payable mySponsor = payable(_users.referral_tree(beneficiary));
      //if (_users.haveValue(mySponsor)) {
        uint256 reward = _addReferralBonus(earnings, beneficiary, mySponsor, package*10**18);
      //}	
      _compressionBonus(earnings, package*10**18, mySponsor, 0, reward, 1);
    }
    ...
    function setLines(uint256 lines) public onlyOwner {
      _lines = lines;
    } 
    ...
    function addVoteWallet(address wallet, uint256 weight, uint256 usdAmount, bool needNft) public onlyOwner {
      require(addedCanVoteWalletsCount < 201, "No more wallets can be added.");
      require(weight < 10, "Weight can be only between 1 and 10");
    ...
    function pendingStake(address wallet, uint256 i) public onlyOwner {
        _users.setNotActiveStaking(wallet, i);
    }

    function setPendingStake(uint256 _totalPendingStake) public checkTransfered onlyOwner {
        totalPendingStake = _totalPendingStake;
    }
