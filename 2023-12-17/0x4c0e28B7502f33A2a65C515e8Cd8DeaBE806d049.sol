...
contract ZANDEFI is Ownable {
    using SafeMath for uint256;
    
    IBEP20 public ZANTOKEN;
    ...
    uint256 public constant MIN_INVESTMENT = 1e18; // 0.1 bnb
    ...
    function _stake(address payable _addr, uint256 _planId, uint256 _amount) private returns(bool){
        require(ZANTOKEN.balanceOf(_addr)>=_amount, "insufficient amount of soda token");
        ...
        ZANTOKEN.transferFrom(_addr, address(this), _amount);
    ...
    function invest(uint256 _referrerCode) public payable {
        require(tokenA.balanceOf(msg.sender) >= HCTamount, 'Not enough HCT token');
    ...
    function claimReward() public {
        ...
        ZANTOKEN.transfer(msg.sender,payout);
    ...
    function unstake() public {
        ...
        uint256 withdrawalAmount = 0;
         ...
         ZANTOKEN.transfer(msg.sender, body);
         ZANTOKEN.transfer(msg.sender, payout);
    ...
    function swap(uint256 _sodaAmount) public{
        require(ZANTOKEN.balanceOf(msg.sender)>=_sodaAmount, "insufficient amount of soda token");

        uint256 price = getSodaPrice();
        uint256 payout = _sodaAmount.mul(price).div(1e18);

        ZANTOKEN.transferFrom(msg.sender, address(this), _sodaAmount);
    ...
    function recoverERC20(uint256 tokenAmount) public onlyOwner{
        ZANTOKEN.transfer(developerAccount_, tokenAmount);
     }
    ...
    function claimRef() public {
         ...
         ZANTOKEN.transfer(msg.sender,amount);
    ...
    function ChangeHCTamount(uint256 _newHCTamount) public onlyOwner {
         HCTamount = _newHCTamount;
    }
