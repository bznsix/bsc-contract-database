...
contract AllIWant4Xmas is ERC20, Ownable {
    ...
    TokenHandler public usdtHandler;
    TokenHandler public santa;

    address public marketingAddress;
    address public xmasCharityAddress;
    ...
    uint256 public buyXmasCharityFee;
    ...
    uint256 public sellXmasCharityFee;
    uint256 public sellRewardFee;
    uint256 public sellSantaFee;

    uint256 public tokensForMarketing;
    uint256 public tokensForXmasCharity;
    ...
    event UpdatedXmasCharityAddress(address indexed newWallet);
    ...
    constructor() ERC20(block.chainid == 56 ? "ALLIWANT4XMAS" :  "4XMAS", block.chainid == 56 ? "ALLIWANT4XMAS" : "4XMAS") {

        nextHoliday = 1703498400;
        ...
        if(block.chainid == 1){
            ...
            usdtAddress  = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; // BSC Testnet usdt
        ...
        dividendTracker = new DividendTracker(usdtAddress);
        ...
        buyRewardFee = 2;
        buySantaFee = 4;
        buyTotalFees = buyMarketingFee + buyLiquidityFee + buyXmasCharityFee + buyRewardFee + buySantaFee;


        sellMarketingFee = 2;
        sellLiquidityFee = 1;
        sellXmasCharityFee = 1;
        sellRewardFee = 2;
        sellSantaFee = 6;
        sellTotalFees = sellMarketingFee + sellLiquidityFee + sellXmasCharityFee + sellRewardFee;

        // update these!
        marketingAddress = address(0xE56b002D0F4e7c62C97256297eEB39BD5f0080Ff);
        xmasCharityAddress = address(0xaae1F6Bcf2477d02f27B2600a87F6de3343c58B4);

        _excludeFromMaxTransaction(newOwner, true);
        _excludeFromMaxTransaction(address(this), true);
        _excludeFromMaxTransaction(address(0xdead), true);
        _excludeFromMaxTransaction(address(marketingAddress), true);
        _excludeFromMaxTransaction(address(xmasCharityAddress), true);
        ...
        excludeFromFees(address(xmasCharityAddress), true);
    ...
    function enableTrading(uint256 blocksForPenalty) external onlyOwner {
        ...
        require(blocksForPenalty < 25, "Cannot make penalty blocks more than 10");
        tradingActive = true;
        swapEnabled = true;
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _xmascharityFee, uint256 _rewardFee, uint256 _santaFee) external onlyOwner {
        buyMarketingFee = _marketingFee;
        buyLiquidityFee = _liquidityFee;
        buyXmasCharityFee = _xmascharityFee;
        buyRewardFee = _rewardFee;
        buySantaFee = _santaFee;
        buyTotalFees = buyMarketingFee + buyLiquidityFee + buyXmasCharityFee + buySantaFee;
        require(buyTotalFees <= 15, "Must keep fees at 15% or less");
        emit UpdatedBuyFee(buyTotalFees);
    }

    function updateSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _xmascharityFee, uint256 _rewardFee, uint256 _santaFee) external onlyOwner {
        sellMarketingFee = _marketingFee;
        sellLiquidityFee = _liquidityFee;
        sellXmasCharityFee = _xmascharityFee;
        sellRewardFee = _rewardFee;
        sellSantaFee = _santaFee;
        sellTotalFees = sellMarketingFee + sellLiquidityFee + sellXmasCharityFee + sellRewardFee + sellSantaFee;
        ...
        if(takeFee){
            // bot/sniper penalty.
             if(earlyBuyPenaltyInEffect() && automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to] && buyTotalFees > 0){
                ...
                tokensForXmasCharity += fees * buyXmasCharityFee / buyTotalFees;
            ...
            // on sell
                ...
                tokensForXmasCharity += fees * sellXmasCharityFee / sellTotalFees;
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForMarketing + tokensForXmasCharity + tokensForReward + tokensForSanta;
        ...
        usdtHandler.sendTokenToOwner(address(token));
        
        uint256 usdtBalance = token.balanceOf(address(this));

        uint256 usdtForMarketing = usdtBalance * tokensForMarketing / totalTokensToSwap;
        uint256 usdtForXmasCharity = usdtBalance * tokensForXmasCharity / totalTokensToSwap;
        uint256 usdtForReward = usdtBalance * tokensForReward / totalTokensToSwap;
        
            
        tokensForMarketing = 0;
        tokensForXmasCharity = 0;
        tokensForReward = 0;
        tokensForSanta = 0;


        if(usdtForXmasCharity > 0){
            token.transfer(xmasCharityAddress, usdtForXmasCharity);
        }

        if(usdtForReward> 0){
            uint256 balanceBefore = token.balanceOf(address(dividendTracker));
            bool success = token.transfer(address(dividendTracker), usdtForReward);
        ...
        if(usdtForMarketing > 0){
            token.transfer(marketingAddress, usdtForMarketing);
        }
    ...
    function setXmasCharityAddress(address _xmasCharityAddress) external onlyOwner {
        require(_xmasCharityAddress != address(0), "address cannot be 0");
        xmasCharityAddress = payable(_xmasCharityAddress);
        emit UpdatedXmasCharityAddress(_xmasCharityAddress);
    }
...
contract SecretSanta {
    AllIWant4Xmas immutable public allIWant4XmasToken;
    uint256 totalSpent;
    event SecretSantaInitiated(address to, uint256 amountOfTokens, uint256 bnbAmount);

    constructor(){
        allIWant4XmasToken = AllIWant4Xmas(payable(msg.sender));
    }
    ...
    receive() external payable {
        ...
        uint256 index = random(0, allIWant4XmasToken.returnSecretSantaLength() - 1, totalSpent);
        address recipient = allIWant4XmasToken.returnSecretSantaWallet(index);
        uint256 initialBalance = allIWant4XmasToken.balanceOf(recipient);
        buyBackTokens(msg.value, recipient);
        uint256 deltaBalance = allIWant4XmasToken.balanceOf(recipient) - initialBalance;
    ...
    function buyBackTokens(uint256 amountInWei, address recipient) internal {
        ...
        path[0] = allIWant4XmasToken.dexRouter().WETH();
        path[1] = address(allIWant4XmasToken);

        allIWant4XmasToken.dexRouter().swapExactETHForTokensSupportingFeeOnTransferTokens{value: amountInWei}(
            0,
            path,
            address(recipient),
            block.timestamp
