    ...
    Blacklist
{
    ...
    function _transfer(
        ...
        require(
            !Blacklist.isAddressBlacklisted(from) &&
                !Blacklist.isAddressBlacklisted(to),
            "You are blacklisted by Owner"
        );
        // require(!Blacklist._blacklist(from), "Error: Sender is blacklisted");
        // require(!Blacklist._blacklist(to), "Error: Receiver is blacklisted");
...
contract WFTOKEN is BEP20 {
    uint256 private initialSupply;
    uint256 private max_supply = 10000000 * (10 ** 18);
    using SafeMath for uint256;
    uint256 private totalMint;

    constructor() BEP20("WFTOKEN", "WF", 18) {
    ...
    function mint(uint256 amount) public onlyOwner {
        require(
            totalMint.add(amount) <= max_supply,
            "Error: Minting would exceed the maximum supply"
        );
        _mint(owner(), amount);
