...
contract GrokCommunity is ERC20, Ownable {
    ...
    event MKTWalletChanged(address MKTWallet);
    ...
    constructor() payable ERC20("Grok Community", "GROK CM") {

        rewardToken = 0x2170Ed0880ac9A755fd29B2688956BD959F933F8; // ETH

        liquidityFeeOnBuy   = 0;
        MKTFeeOnBuy    = 4;
        rewardsFeeOnBuy     = 4;

        totalBuyFee         = liquidityFeeOnBuy + MKTFeeOnBuy + rewardsFeeOnBuy;

        liquidityFeeOnSell  = 0;
        MKTFeeOnSell   = 4;
        rewardsFeeOnSell    = 4;

        totalSellFee        = liquidityFeeOnSell + MKTFeeOnSell + rewardsFeeOnSell;

        MKTWallet = 0xa6a3Bc17aF5081d44cded20A25a5B92508aa3d3f;

        dividendTracker = new DividendTracker(10_000_000, rewardToken);
        ...
        _mint(owner(), 6_900_000_000 * (10 ** 9));
    ...
    function changeMKTWallet(address _MKTWallet) external onlyOwner {
        require(_MKTWallet != MKTWallet, "Marketing wallet is already that address");
        require(!isContract(_MKTWallet), "Marketing wallet cannot be a contract");
        require(_MKTWallet != address(0), "Marketing wallet cannot be the zero address");
        MKTWallet = _MKTWallet;
        emit MKTWalletChanged(MKTWallet);
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 bnbShare = (MKTFeeOnBuy + MKTFeeOnSell) + (rewardsFeeOnBuy + rewardsFeeOnSell);
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                if((MKTFeeOnBuy + MKTFeeOnSell) > 0) {
                    uint256 MKTBNB = newBalance * (MKTFeeOnBuy + MKTFeeOnSell) / bnbShare;
                    uint256 half = MKTBNB / 2;

                    sendBNB(payable(MKTWallet), half);

                    emit SendMarketing(MKTBNB);
                }
