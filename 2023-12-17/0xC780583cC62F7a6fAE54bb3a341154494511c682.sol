...
contract DepositeContract is OwnableUpgradeable {
    ...
    function changeTotalDeposite(address _token, uint256 _amount) public onlyOwner {
        _totalDeposite[_token] = _amount;
    }
    ...
    function depositBoth(uint256 amountBBC,address _supportBBCToken, uint256 amountBBTC, address _supportBBTCToken, uint256 _orderid) external {
 
        require(_supportTokens[_supportBBCToken], "BBC token not supported");
        require(_supportTokens[_supportBBTCToken], "BBTC token not supported");

        IERC20(_supportBBCToken).transferFrom(msg.sender, address(this), amountBBC);
        IERC20(_supportBBTCToken).transferFrom(msg.sender, address(this), amountBBTC);

        _tokenDepositeRecord[_supportBBCToken][msg.sender] += amountBBC;
        _tokenDepositeRecord[_supportBBTCToken][msg.sender] += amountBBTC;

        _totalDeposite[_supportBBCToken] += amountBBC;
        _totalDeposite[_supportBBTCToken] += amountBBTC;

        _userByOrderIDs[_supportBBCToken][_orderid] = msg.sender;
        _userByOrderIDs[_supportBBTCToken][_orderid] = msg.sender;
        
        emit userDeposite(msg.sender, _supportBBCToken, amountBBC, _orderid);
        emit userDeposite(msg.sender, _supportBBTCToken, amountBBTC, _orderid);
    }
    ...
    function withdraw(address _withdrawToken, uint256 _withdrawAmount) external {
        ...
        uint256 contractBalance = IERC20(_withdrawToken).balanceOf(address(this));  
        _totalDeposite[_withdrawToken] = contractBalance;
    ...
    function transferFunds(address _transferToken, uint256 _transferAmount) external {
        ...
        emit fundsTransfered(_fundReceiver, _transferToken, _transferAmount);
         uint256 contractBalance = IERC20(_transferToken).balanceOf(address(this));  
        _totalDeposite[_transferToken] = contractBalance;
