...
contract SAT is ERC20, Ownable {
    ...
    uint256 public tradingBlockNumber;
    uint256 public deadFee = 10; //1% dead
    uint256 public foundationFee = 10;//1% fd
    uint256 public lpRewardFee = 15; //1.5% lp
   ...
   TokenDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        excludeFromFees(receiverAddress_, true);
        ...
        _cast(receiverAddress_, totalSupply);
    }
    ...
    function setBlacklist(address account, bool include) public onlyOwner {
        if(_blacklist[account] != include){
            _blacklist[account] = include;
        }
    }
   ...
   function setDeadFee(uint256 val) public onlyOwner {
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            require(!_blacklist[from] && !_blacklist[to], "Account blacklisted");
            if (tradingBlockNumber == block.number){
    ...
    function takeAllFee(address from,uint256 amount) private returns(uint256 amountAfter) {
        ...
        super._transfer(from, address(this),  LPFee.add(DLMFee));
    }
