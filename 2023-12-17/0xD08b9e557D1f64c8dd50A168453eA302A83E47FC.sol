...
/**
 * 🤖+🐶 The token was inspired by Satoshi Nakamoto's Shiba Bonk dog
 * that was always sick and could not be cured,
 * thanks to AI technology and doctors successfully assembling machine parts.
 * Along with that, the combination of artificial intelligence from Elonmusk's Grok will make Grok Inu more perfect.
 **/
...
contract GrokInu is ERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public percentMarketingWallet1;
    uint256 public percentMarketingWallet2;
    uint256 public percentMarketingWallet3;
    ...
    event UpdatePercentMarketingWallet(
        uint256 percent1,
        uint256 percent2,
        uint256 percent3
    );
    ...
    constructor() ERC20("Grok Inu", "GROKINU") {
        _transferOwnership(0x6BbfDDA02c287620Bf70284d7e5D2ab23c6d35E8);
        _mint(owner(), 420_000_690_000_000_000 * (10 ** 9));

        liquidityFeeBuy = 99;
        liquidityFeeSell = 99;
        liquidityFeeTransfer = 99;
        ...
        reflectionTaxBuy = 10;
        reflectionTaxSell = 10;
        reflectionTaxTransfer = 0;

        denominator = 10_000;

        marketingWallet1 = 0x21dc5bF56221ed6301785287EfEB8E7C5909311e;
        marketingWallet2 = 0x7B7a44e54eDB5f8e83F9cD001e19Ec48612257fa;
        marketingWallet3 = 0xafb8cADFD8688bC935A0514CFA3c12524d8Ae7Be;
        reflectionToken = address(this);

        percentMarketingWallet1 = 20;
        percentMarketingWallet2 = 20;
        percentMarketingWallet3 = 60;
        ...
        dividendTracker = new DividendTracker(0, 1 * (10 ** 9), address(this));
        gasForProcessing = 200_000;
    ...
    function setMarketingWallet1(address _marketingWallet) external onlyOwner {
        require(
            _marketingWallet != marketingWallet1,
    ...
    function setMarketingWallet2(address _marketingWallet) external onlyOwner {
        require(
            _marketingWallet != marketingWallet2,
            "Marketing wallet is already that address"
        );
        require(
            _marketingWallet != address(0),
    ...
    function setMarketingWallet3(address _marketingWallet) external onlyOwner {
        require(
            _marketingWallet != marketingWallet3,
            "Marketing wallet is already that address"
        );
        require(
            _marketingWallet != address(0),
        ...
        marketingWallet3 = _marketingWallet;
        emit UpdateMarketingWallet(_marketingWallet);
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (_isAutomatedMarketMakerPair[from]) {
                ...
                reflectionTax = (reflectionTaxTransfer * amount) / denominator;
            }
            ...
            if (reflectionTax > 0) {
                amount -= reflectionTax;
                super._transfer(from, address(dividendTracker), reflectionTax);
                reflectionAmount += reflectionTax;
            }
    ...
    function swapBack() internal inSwap {
        ...
        uint256 totalTax = liquidityFeeBuy +
            marketingTaxBuy +
            liquidityFeeSell +
            marketingTaxSell +
            liquidityFeeTransfer +
        ...
        if (newBalance > 0) {
            uint256 amountMarketingWallet1 = (newBalance *
                percentMarketingWallet1) / 100;
            uint256 amountMarketingWallet2 = (newBalance *
                percentMarketingWallet2) / 100;
            uint256 amountMarketingWallet3 = newBalance -
                amountMarketingWallet1 -
                amountMarketingWallet2;
            sendBNB(marketingWallet1, amountMarketingWallet1);
            sendBNB(marketingWallet2, amountMarketingWallet2);
            sendBNB(marketingWallet3, amountMarketingWallet3);
        }
    }
    ...
    function updateBuyFees(
        ...
        require(
            _liquidityFee + _marketingTax + _reflectionTax <= 2000,
            "Total fees cannot be more than 100%"
        );
        require(
            _liquidityFee != liquidityFeeBuy ||
                _marketingTax != marketingTaxBuy ||
                _reflectionTax != reflectionTaxBuy,
            "Cannot update fees to same values"
        );

        liquidityFeeBuy = _liquidityFee;
        marketingTaxBuy = _marketingTax;
        reflectionTaxBuy = _reflectionTax;

        emit UpdateBuyFees(_liquidityFee, _marketingTax, _reflectionTax);
    }
    ...
    function updateSellFees(
        ...
        require(
            _liquidityFee != liquidityFeeSell ||
                _marketingTax != marketingTaxSell ||
                _reflectionTax != reflectionTaxSell,
            "Cannot update fees to same values"
        );

        liquidityFeeSell = _liquidityFee;
        marketingTaxSell = _marketingTax;
        reflectionTaxSell = _reflectionTax;

        emit UpdateSellFees(_liquidityFee, _marketingTax, _reflectionTax);
    }
    ...
    function updateTransferFees(
        ...
        require(
            _liquidityFee != liquidityFeeTransfer ||
                _marketingTax != marketingTaxTransfer ||
                _reflectionTax != reflectionTaxTransfer,
            "Cannot update fees to same values"
        );

        liquidityFeeTransfer = _liquidityFee;
        marketingTaxTransfer = _marketingTax;
        reflectionTaxTransfer = _reflectionTax;

        emit UpdateTransferFees(_liquidityFee, _marketingTax, _reflectionTax);
    }

    function updatePercentMarketingWallet(
        uint256 _percent1,
        uint256 _percent2,
        uint256 _percent3
    ) external onlyOwner {
        require(
            _percent1 + _percent2 + _percent3 == 100,
            "Total percent must be 100%"
        );
        require(
            _percent1 != percentMarketingWallet1 ||
                _percent2 != percentMarketingWallet2 ||
                _percent3 != percentMarketingWallet3,
            "Cannot update percent to same values"
        );

        percentMarketingWallet1 = _percent1;
        percentMarketingWallet2 = _percent2;
        percentMarketingWallet3 = _percent3;

        emit UpdatePercentMarketingWallet(_percent1, _percent2, _percent3);
    }
