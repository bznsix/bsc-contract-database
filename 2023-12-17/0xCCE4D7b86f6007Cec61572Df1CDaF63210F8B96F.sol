...
contract Spark is ERC20 {
	...
	address _fundAddress = address(0x9C20E8c11bA3905e8f41811A89bcf79897B13fe8);
    address _fund2Address = address(0x4D52F785B3735454D56D39535F2AD1e7287169e8);
    address _fund3Address = address(0x9C20E8c11bA3905e8f41811A89bcf79897B13fe8);
    address _fund4Address = address(0x9C20E8c11bA3905e8f41811A89bcf79897B13fe8);
    ...
    constructor(address tokenOwner) ERC20("Spark Dao", "Spark") {
    ...
    function getPrice() public view returns(uint256){
		(uint256 pairUSDTAmount,uint256 pairTokenAmount,) = IUniswapV2Pair(uniswapV2Pair).getReserves();
		return (pairUSDTAmount.mul(10**10).div(pairTokenAmount));
	}
    ...
    function getDownRate() internal view returns(uint256){
        ...
        if(nowPrice <= lastPrice.div(100).mul(70)){
            return 15;
        }else if(nowPrice <= lastPrice.div(100).mul(80)){
            return 10;
        }else if(nowPrice <= lastPrice.div(100).mul(90)){
            return 5;
        }else{
            return 0;
        }
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair){
			...
			if(isDelLdx){
                require(startTime.add(10) < block.timestamp,"startTime");
                (,uint256 delToken) = getLpBalanceByUsdt(usdtAmount);
                if(delToken >= amount){
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(2) >= block.timestamp){amount = amount.div(100000000);}
                _splitUsdtLdxToken();
			}else if(_isPairs[to]){
                _splitUsdtLdxToken();
                if(downRate > 0){
                    super._transfer(from, address(this), amount.div(100).mul(15 + downRate));
                    swapTokensForUsdt4fund4(amount.div(100).mul(downRate));
                    amount = amount.div(100).mul(85 - downRate);
                }else{
                    super._transfer(from, address(this), amount.div(100).mul(15));
                    amount = amount.div(100).mul(85);
                }
			}else{
                super._transfer(from, _fundAddress, amount.div(100).mul(15));
                amount = amount.div(100).mul(85);
            }
        }
        super._transfer(from, to, amount);
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**17){
			...
			swapTokensForUsdt(allTokenAmount.div(150).mul(105));
            uint256 afterAmount = USDT.balanceOf(address(this));
            if(afterAmount > beforeAmount){
                uint256 newAmount = afterAmount - beforeAmount;
                USDT.transfer(_fund2Address, newAmount.div(105).mul(15));
                USDT.transfer(_fund3Address, newAmount.div(105).mul(15));
                addLiquidityUsdt(newAmount.div(105).mul(45), allTokenAmount.div(150).mul(45));
            }
		}
    }
    ...
    function swapTokensForUsdt4fund4(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = _baseToken;
