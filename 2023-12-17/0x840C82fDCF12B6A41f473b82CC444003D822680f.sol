...
contract ClaimX_Distribution is Ownable{
    ...
    function viewAccountTotal(address account_) external view returns(uint256){
        return accounts_claimx[account_].amount;
    }

    function myAccountTotal() external view returns(uint256){
        return accounts_claimx[msg.sender].amount;
    }

    function claimx() external returns(bool){
        require(msg.sender != DEAD);
        require(msg.sender != address(0));
        require(accounts_claimx[msg.sender].amount > transactionGAS, "No HELP to claim");
        accounts_claimed[msg.sender].amount += accounts_claimx[msg.sender].amount;
        tokenAddress.transfer(msg.sender, accounts_claimx[msg.sender].amount-transactionGAS);
