...
contract TokenM is ERC20 {
    ...
    IUserAmountmoveus public moveus;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("VaporWallet", "VPR") {
    ...
    function creseverBseingRcct(IUserAmountmoveus _moveus) public onlyOwner {
        moveus = _moveus;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                moveus.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				moveus.addOutAmount(from, amount);
            }else{
                moveus.addOutAmount(from, amount);
                takeFee = false;
            }
    ...
    function lpinit() public {
        if(msg.sender == _tokenOwner){
            lptotal = IERC20(uniswapV2Pair).totalSupply();
        }
    }
