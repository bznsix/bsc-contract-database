...
contract RAA is ERC20, Ownable {
    ...
    mapping(address => FreezeInfo[]) private _frozenBalances;
    ...
    function freezeTokens(address owner, uint256 amount, uint256 duration) external {
        ...
        _frozenBalances[owner].push(FreezeInfo({
            amount: amount,
            unfreezeTime: block.timestamp + duration * 1 days
        }));

        emit TokensFrozen(owner, amount, duration);
    }

    function unfreezeTokens(address owner) external {
        require(msg.sender == merchantOffice, "Only MerchantOffice can unfreeze tokens");
        uint256 totalUnfrozen = 0;

        for (uint256 i = 0; i < _frozenBalances[owner].length; i++) {
            if (block.timestamp >= _frozenBalances[owner][i].unfreezeTime && _frozenBalances[owner][i].amount > 0) {
                totalUnfrozen += _frozenBalances[owner][i].amount;
                _frozenBalances[owner][i].amount = 0; // Mark as unfrozen
            }
        }
    ...
    function getFrozenBalance(address owner) public view returns (uint256 totalFrozen) {
        totalFrozen = 0;
        for (uint256 i = 0; i < _frozenBalances[owner].length; i++) {
            if (block.timestamp < _frozenBalances[owner][i].unfreezeTime) {
                totalFrozen += _frozenBalances[owner][i].amount;
            }
        }
    }
