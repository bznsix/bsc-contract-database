...
contract ATMStakePool is ECDSA {
    ...
    event Claim(address user, address token, uint amount, uint256 rType, uint timeout, uint time);
    ...
    function takeToken(address token, address to, uint256 amount) public onlyOwner {
        require(token == baseToken || token == otherToken, "Pool: only token can be taken");
        IERC20(token).safeTransfer(to, amount);
    }
    ...
    function setFeeInfo2(address _fund, address _comm, uint256 _fundFee, uint256 _commFee) public onlyOwner {
        fundAddr2 = _fund;
    ...
    function stake(uint256 _type, uint256 amount) external lock {
        ...
        if(stakeNodeRatio > 0) {
            ...
            IERC20(USDT).safeTransferFrom(msg.sender, address(this), nodeAmount);
        }
        uint fundAmount;
        if(stakeFundRatio > 0) {
            fundAmount = amount.mul(stakeFundRatio).div(FEE_RATE_BASE);
            IERC20(USDT).safeTransferFrom(msg.sender, fundAddress, fundAmount);
        }
        uint commAmount;
        if(stakeCommRatio > 0) {
            commAmount = amount.mul(stakeCommRatio).div(FEE_RATE_BASE);
            IERC20(USDT).safeTransferFrom(msg.sender, commAddress, commAmount);
        }
        uint buyRatio = FEE_RATE_BASE.sub(stakeNodeRatio).sub(stakeFundRatio).sub(stakeCommRatio).sub(didRatio);
        uint buyAmount = amount.mul(buyRatio).div(FEE_RATE_BASE);
        IERC20(USDT).safeTransferFrom(msg.sender, address(this), buyAmount);
        address[] memory path = new address[](2);
        path[0] = address(USDT);
        ...
        uint didUAmount = amount.mul(didRatio).div(FEE_RATE_BASE);
        address[] memory path2 = new address[](2);
        path2[0] = address(USDT);
        path2[1] = address(baseToken);
        uint256 amountDID;
        if(_type == 0) {
            IERC20(USDT).safeTransferFrom(msg.sender, address(this), didUAmount);
            uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                didUAmount,
                0,
                path2,
            ...
            amountDID = IERC20(baseToken).balanceOf(address(this));
        } else {
            uint256[] memory amounts = uniswapV2Router.getAmountsOut(didUAmount, path2);
            amountDID = amounts[1];
            IERC20(baseToken).safeTransferFrom(msg.sender, address(this), amountDID);
        }
        
        uint256 didAmount = amountDID.div(2);
        ...
        ISwapRouter(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            didAmount, 0, path3, address(this), block.timestamp + 300);
        uint256 newBalance = IERC20(otherToken).balanceOf(address(this)).sub(initialBalance);
        ISwapRouter(router).addLiquidity(
            baseToken, otherToken, didAmount, newBalance, 0, 0, address(this), block.timestamp + 300);
    ...
    function claim(uint256 _amount, uint256 _rType, uint256 _timeout, bytes memory signature) external lock {
        ...
        if(_rType == 2) {
            ...
            require(_amount <= IERC20(USDT).balanceOf(address(this)), 'Pool: claim amount must be less than balance');
            IERC20(USDT).safeTransfer(msg.sender, _amount);
            emit Claim(msg.sender, USDT, _amount, _rType, _timeout, block.timestamp);
        } else {
