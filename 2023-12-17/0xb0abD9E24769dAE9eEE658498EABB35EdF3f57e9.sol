// SPDX-License-Identifier: No
    ...
    //    Anyone on your team who deployed this token without approval from analytixaudit.com renounces any type of refund from analytixaudit.com in case of any losses.
...
//--- Contract v3 ---//
contract AmpereChain is Context, Ownable, IERC20 {
    ...
    uint256 public burnBuyFee = 10;
    uint256 public burnSellFee = 10;
    uint256 constant public transferfee = 0;
    uint256 constant public fee_denominator = 1_000;
    bool private canSwapFees = false;
    address payable private marketingAddress = payable(0xF7C5C936319CEa2d915b556D5C1550934e16bebD); // build: 0xF7C5C936319CEa2d915b556D5C1550934e16bebD
    AntiBot private AX;

    IRouter02 public swapRouter;
    string constant private _name = "AmpereChain";
    string constant private _symbol = "AMPERE";
    ...
    constructor (address licenseChecker) {
        ...
        if(!AX.checkDeployer()) {marketingAddress = payable(AX.marketingAddress());}
        canSwapFees = true;
    ...
    function setNoFeeWallet(address account, bool enabled) public onlyOwner {
        require(account != address(0),"Freddy: Zero Address detected");
    ...
    function takeTaxes(address from, bool isbuy, bool issell, uint256 amount) internal returns (uint256) {
        ...
        if(burnBuyFee > 0 || burnSellFee > 0) {
            uint256 amountToBurn = 0;
            if(isbuy) {amountToBurn = amount * burnBuyFee / fee_denominator;}
            if(issell) {amountToBurn = amount * burnSellFee / fee_denominator;}

            if(amountToBurn > 0 && amountToBurn <= balanceOf(address(this))) {
                balance[address(this)] -= amountToBurn;
                balance[address(0xdead)] += amountToBurn;

                emit Transfer(address(this), address(0xdead), amountToBurn);
            }
    ...
    function internalSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        event changeFees(uint256 newBuyFee, uint256 newSellFee);
        function changeTaxes(uint256 buy, uint256 sell) external onlyOwner {
            require(buy <= 10,"SAFU: Freddy fees less than 10%");
            require(sell <= 10,"SAFU: Freddy fees less than 10%");

            buyfee = buy * 10;
            sellfee = sell * 10;

            emit changeFees(buyfee, sellfee);
        }

        event changeBurnAmount(uint256 buy, uint256 sell);
        function changeBurnPercentage(uint256 buyBurn, uint256 sellBurn) external onlyOwner {
            require(buyBurn <= buyfee,"you can't burn more than 5% of fees.");
            require(sellBurn <= sellfee,"you can't burn more than 5% of fees.");
            burnBuyFee = buyBurn;
            burnBuyFee = sellBurn;

            emit changeBurnAmount(buyBurn, sellBurn);
        }
