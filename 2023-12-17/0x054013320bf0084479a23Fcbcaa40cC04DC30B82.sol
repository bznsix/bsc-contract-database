...
abstract contract ERC20UserBlocklist is ERC20, UserBlocklist {
    string internal constant ERC20_LOCK_MESSAGE = "ERC20UserBlocklist: Transfer is blocked for";
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(UserBlocklist) returns (bool) {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        ...
        address[] memory usersToCheck;
        // In this case sender and operator are the same, so we need to check only sender and receiver
        if (from == _msgSender()) {
            usersToCheck = new address[](2);
        } else {
            // In this case operation is performed via transferFrom and we need to check sender, receiver and operator
            usersToCheck = new address[](3);
            usersToCheck[2] = _msgSender();
        }
        usersToCheck[0] = from;
        usersToCheck[1] = to;
        _batchCheckIfActionIsAllowed(blocklistInfo.blocklist, usersToCheck, ERC20_LOCK_MESSAGE);
    }
}
// SPDX-License-Identifier: MIT
...
import { ERC20UserBlocklist } from "../blocklist/extensions/ERC20UserBlocklist.sol";

contract KaleToken is ERC20, ERC20Permit, ERC20UserBlocklist, Ownable {
    constructor(
        address owner_,
        address receiver,
        address blocklist_
    ) Ownable() ERC20("Kale Currency", "KALE") ERC20Permit("Kale Currency") ERC20UserBlocklist(blocklist_) {
        _transferOwnership(owner_);
        _mint(receiver, 10_000_000_000 * 1E18);
    }

    /**
     * @dev Change the blocklist address
     * @param blocklist_ The new blocklist address
     */
    ...
    /**
     * @dev Change the blocklist state
     * @param state The new blocklist state
     */
    function setBlocklistState(bool state) external onlyOwner {
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 amount
    ) internal override(ERC20UserBlocklist, ERC20) {
        super._beforeTokenTransfer(from, to, amount);
    }
}
