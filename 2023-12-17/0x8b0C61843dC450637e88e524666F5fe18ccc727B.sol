...
/// @title Callback for ISquadV3PoolActions#swap
/// @notice Any contract that calls ISquadV3PoolActions#swap must implement this interface
interface ISquadV3SwapCallback {
    /// @notice Called to `msg.sender` after executing a swap via ISquadV3Pool#swap.
    ...
    /// @param data Any data passed through by the caller via the ISquadV3PoolActions#swap call
...
library OracleLibrary {
    /// @notice Calculates time-weighted means of tick and liquidity for a given SquadSwap V3 pool
        ...
        returns (int24 arithmeticMeanTick, uint128 harmonicMeanLiquidity)
    {
        ...
        (int56[] memory tickCumulatives, uint160[] memory secondsPerLiquidityCumulativeX128s) =
            ISquadV3Pool(pool).observe(secondsAgos);
    ...
    /// @param pool Address of SquadSwap V3 pool that we want to observe
    /// @return secondsAgo The number of seconds ago of the oldest observation stored for the pool
    function getOldestObservationSecondsAgo(address pool) internal view returns (uint32 secondsAgo) {
        (, , uint16 observationIndex, uint16 observationCardinality, , , ) = ISquadV3Pool(pool).slot0();
        require(observationCardinality > 0, 'NI');

        (uint32 observationTimestamp, , , bool initialized) =
            ISquadV3Pool(pool).observations((observationIndex + 1) % observationCardinality);

        // The next index might not be initialized if the cardinality is in the process of increasing
        // In this case the oldest observation is always in index 0
        if (!initialized) {
            (observationTimestamp, , , ) = ISquadV3Pool(pool).observations(0);
        }
    ...
    /// @param pool Address of SquadSwap V3 pool
    /// @return The tick that the pool was in at the start of the current block
    function getBlockStartingTickAndLiquidity(address pool) internal view returns (int24, uint128) {
        (, int24 tick, uint16 observationIndex, uint16 observationCardinality, , , ) = ISquadV3Pool(pool).slot0();
        ...
        (uint32 observationTimestamp, int56 tickCumulative, uint160 secondsPerLiquidityCumulativeX128, ) =
            ISquadV3Pool(pool).observations(observationIndex);
        if (observationTimestamp != uint32(block.timestamp)) {
            return (tick, ISquadV3Pool(pool).liquidity());
        }
        ...
        ) = ISquadV3Pool(pool).observations(prevIndex);
...
/// @notice Allows callers to approve the SquadSwap V3 position manager from this contract,
...
import '@squadswap/v3-periphery/contracts/base/Multicall.sol';
...
import '@squadswap/v3-periphery/contracts/base/PeripheryImmutableState.sol';
import '@squadswap/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@squadswap/v3-periphery/contracts/libraries/Path.sol';
import '@squadswap/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@squadswap/v3-core/contracts/interfaces/ISquadV3Pool.sol';
import '@squadswap/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(ISquadV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (ISquadV3Pool pool) {
        pool = ISquadV3Pool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            ISquadV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = ISquadV3Pool(pool).slot0();
            }
...
import '@squadswap/v3-periphery/contracts/base/PeripheryPayments.sol';
import '@squadswap/v3-periphery/contracts/libraries/TransferHelper.sol';
...
import '@squadswap/v3-periphery/contracts/base/PeripheryPaymentsWithFee.sol';
...
import '@squadswap/v3-periphery/contracts/base/PeripheryValidation.sol';
...
interface IImmutableState {
    /// @return Returns the address of the Squad V2 factory
    function factoryV2() external view returns (address);

    /// @return Returns the address of Squad V3 NFT position manager
...
import '@squadswap/v3-periphery/contracts/interfaces/IMulticall.sol';
...
import '@squadswap/v3-periphery/contracts/interfaces/IPeripheryPayments.sol';
...
import '@squadswap/v3-periphery/contracts/interfaces/IPeripheryPaymentsWithFee.sol';
...
import '@squadswap/v3-periphery/contracts/interfaces/ISelfPermit.sol';
...
/// @title Router token swapping functionality
/// @notice Functions for swapping tokens via Squad Stable Swap
...
/// @title Router token swapping functionality
/// @notice Functions for swapping tokens via Squad V2
...
/// @title Router token swapping functionality
/// @notice Functions for swapping tokens via Squad V3
interface IV3SwapRouter is ISquadV3SwapCallback {
...
library SmartRouterHelper {
    ...
    bytes32 internal constant V2_INIT_CODE_HASH = 0xd424455c1204e4f46a4a380651928652376a351698d3d97e2da05d3041c15fbe;
    ...
    bytes32 internal constant V3_INIT_CODE_HASH = 0xf08a35894b6b71b07d95a23022375630f6cee63a27d724c703617c17c4fc387d;
...
import '@squadswap/v3-periphery/contracts/base/SelfPermit.sol';
import '@squadswap/v3-periphery/contracts/base/PeripheryImmutableState.sol';
...
/// @title Squad Stable Swap Router
abstract contract StableSwapRouter is IStableSwapRouter, PeripheryPaymentsWithFeeExtended, Ownable, ReentrancyGuard {
    ...
    /**
     * @notice Set Squad Stable Swap Factory and Info
