...
abstract contract ABSToken is IERC20,Ownable
{
    ...
    uint256 public  _minLPValue=100;//lp最小值，低于这个值则没有分红
    ...
    {        
        ...
        {                
            ...
            else //>sell
            {
                ...
                if(takeFee)
                {
                    takeFee=!isAddLiquidity();//添加池子免滑点
                }      
            }           
            ...
            _takeTranstion(from,to,amount,takeFee,isRemoveLp);                
        }       
        else  //>transfer
        {
            require(!_breakerList[from]); 
    ...
    function correntPPMTranstionNum(address addr,uint256 amount) private  returns (uint256)
    {
        ...
        if(lockedNum>0)
        {
            ...
            {  
                ...
                {
                    timeOffset=block.timestamp-_privatePlaceMentLockedTime;
    ...
    function isLpKeeper(address addr) private view   returns(bool)
    {
        if(!poolAddress[addr])
        {
    ...
    {       
        ...
        require(addr!=address(0));
        require(addr!=DEAD);
        require(num<=2000);//私募最多每个地址只有2000个币
    ...
    function getFreePPMTokenNum(address addr) external   returns(uint256)
    {
        uint256 amount=balanceOf(addr);
        amount=correntPPMTranstionNum(addr,amount);   
        return  amount;
    }
    //获取指定地址持有的lp值
    function getLPValue(address addr) external view  returns(uint256)
    {
        return  IERC20(_usdtPair).balanceOf(addr);
    }
    function isLpKeeperAndLPV(address addr) external view   returns (bool,uint256)
    {
        bool isLpkeeper=isLpKeeper(addr);
        uint256 lpV=IERC20(_usdtPair).balanceOf(addr);
        return (isLpkeeper,lpV);
    }
    function getTotalLPValue() external view   returns (uint256)
    {
        return getAllLPValue();        
    }
    function getMainPairs() external view returns (uint,uint256)
    {
        IUniswapV2Pair mainPair = IUniswapV2Pair(_usdtPair);
        (uint256 r0, uint256 r1, ) = mainPair.getReserves();
        return  (r0,r1);
