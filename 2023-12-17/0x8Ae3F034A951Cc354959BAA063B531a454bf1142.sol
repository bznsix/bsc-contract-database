...
// File: contracts/MinPitPhoenixB3.sol
...
contract MinPitPhoenixB3 is Ownable,AccessControl{
    ...
    //全网总单数
    uint256 public _totalNumber;

    address _main = 0x143Ba3484FE85A75F3Bca65871d7B717ac9d44bb;
    ...
    constructor() Ownable(){
        _bis = Bis(
            ...
            0xE4130e3f83F00Dd3Fb7e5B6E8E8fA3dC3c90eF2b,
            10000,
        ...
        _config = Config(
            ...
            Mechanism(10000000000000000000000,10000000000000000000),
            Mechanism(100000000000000000000000,1),
            Mechanism(300000000000000000000000,2),
            Mechanism(800000000000000000000000,3)
        );
    ...
    **/
            ...
            if(i % 2 == 0){
                ...
                if(_userDirect[daiPrAddr][1] >= _config.kpiLimit){
                    ...
                    if(_performance[daiPrAddr][1] < _config.kpi2.condition){
                        ...
                        checkKpi = checkKpi + _config.kpi1.award;
    ...
    function purchaseMinerals(
        ...
        if(_bis.exchange_rate > 0){
            ...
            IERC20(_bis.usdt).transferFrom(user,_bis.collect,plan.usdtPrice);
        }
    ...
    function checkinV2(
        ...
        for(uint i = 0;i<userPurs.length;i++){
            ...
            if(pur.remainAmount > checkTotalTest){
                pur.remainAmount = trySub(pur.remainAmount,checkTotalTest);
                _purchaseHistorys[userAddr][i] = pur;
                checkTotalTest = 0;
            }else {
                checkTotalTest = trySub(checkTotalTest,pur.remainAmount);
                pur.remainAmount = 0;
                _purchaseHistorys[userAddr][i] = pur;
                uint256 checkSalary = pur.totalAmount * pur.dailyRate / 10000;
                _check[userAddr][1] = trySub(_check[userAddr][1],checkSalary);
                _check[userAddr][0] = trySub(_check[userAddr][0],checkSalary);
                _userDirect[userAddr][0] = trySub(_userDirect[userAddr][0],1);
                //上级直推减少
                address prAddr = _info[userAddr];
                _performance[prAddr][0] = trySub(_performance[prAddr][0],pur.usdtPrice);
                uint256 checkAward = pur.usdtPrice * _config.invite / 10000;
                _check[prAddr][2] = trySub(_check[prAddr][2],checkAward);
                _check[prAddr][0] = trySub(_check[prAddr][0],checkAward);
                uint256 checkSubsidy;
                if(_performance[prAddr][0] >= _config.directPush3.condition){
                    checkSubsidy = _config.directPush3.award;
                }else if(_performance[prAddr][0] >= _config.directPush2.condition){
                ...
                if(_check[prAddr][3] > checkSubsidy){
                    checkSubsidy = trySub(_check[prAddr][3],checkSubsidy);
                    _check[prAddr][0] = trySub(_check[prAddr][0],checkSubsidy);
                    _check[prAddr][3] = trySub(_check[prAddr][3],checkSubsidy);
                }
                ...
                for(uint j = 1;j<=10;j++){
                    if(j % 2 == 0){
                        _performance[daiPrAddr][1] = trySub(_performance[daiPrAddr][1],pur.usdtPrice);
                        uint256 checkKpiAward;
                        if(_performance[daiPrAddr][1] >= _config.kpi4.condition){
                            checkKpiAward = _config.kpi4.award;
                        }else if(_performance[daiPrAddr][1] >= _config.kpi3.condition){
                        ...
                        _userDirect[daiPrAddr][1] = trySub(_userDirect[daiPrAddr][1],1);
                        if(_userDirect[daiPrAddr][1] >= _config.kpiLimit){
                            if(_performance[daiPrAddr][1] < _config.kpi1.condition){
                                if(_check[daiPrAddr][4] > 0){
                                    _check[daiPrAddr][0] = trySub(_check[daiPrAddr][0],_check[daiPrAddr][4]);
                                ...
                                uint256 checkKpi = _performance[daiPrAddr][1] * checkKpiAward / 10000;
                                if(_performance[daiPrAddr][1] < _config.kpi2.condition){
                                    checkKpi = checkKpiAward;
                                }else {
                                    checkKpi = checkKpi + _config.kpi1.award;
                                }
                                checkKpi = trySub(_check[daiPrAddr][4],checkKpi);
                                _check[daiPrAddr][0] = trySub(_check[daiPrAddr][0],checkKpi);
                                _check[daiPrAddr][4] = trySub(_check[daiPrAddr][4],checkKpi);
                            }
                        }else {
                            _check[daiPrAddr][0] =  trySub(_check[daiPrAddr][0],_check[daiPrAddr][4]);
                            _check[daiPrAddr][4] = 0;
                ...
                _totalNumber = trySub(_totalNumber,1);
    ...
    function dataMoveV2(
        ...
        uint256 checkKpi,
        uint256 userDirect,
        uint256 userDirectTD,
        uint256 performanceZT,
        uint256 performanceTD
    ) internal {
        _info[userAddr] = prAddr;
        _canQuota[userAddr][0] = canQuota;
        _canQuota[userAddr][1] = totalAmount;
        _performance[userAddr][0] = performanceZT;
        _performance[userAddr][1] = performanceTD;
        _check[userAddr][0] = checkKpi + checkSubsidy + checkAward +checkSalary;
        _check[userAddr][1] = checkSalary;
        _check[userAddr][2] = checkAward;
        _check[userAddr][3] = checkSubsidy;
        _check[userAddr][4] = checkKpi;
        _userDirect[userAddr][0] = userDirect;
        _userDirect[userAddr][1] = userDirectTD;
    }
    ...
    function dataMove(
        ...
        dataMoveV2(userAddr,prAddr,totalAmount,canQuota,checkSalary,checkAward,checkSubsidy,checkKpi,userDirect,userDirectTD,performanceZT,performanceTD);
    }
    ...
    function dataMove2(
        ...
        uint256 totalAmountNew = plan.usdtPrice * plan.quotaMultiple / 10000;
        uint256 remainAmount = totalAmountNew;
        uint256 purchaseTime = block.timestamp;
        if(userDirect > 1){

            remainAmount = canQuota - totalAmountNew;
            PurchaseHistory memory val1 = PurchaseHistory(plan.usdtPrice,plan.tokenPrice,plan.quotaMultiple,plan.dailyRate,totalAmountNew,remainAmount,purchaseTime);
            _purchaseHistorys[userAddr].push(val1);


            PurchaseHistory memory val = PurchaseHistory(plan.usdtPrice,plan.tokenPrice,plan.quotaMultiple,plan.dailyRate,totalAmountNew,totalAmountNew,purchaseTime);
            _purchaseHistorys[userAddr].push(val);
            
        }else {
            remainAmount = canQuota;
            PurchaseHistory memory val = PurchaseHistory(plan.usdtPrice,plan.tokenPrice,plan.quotaMultiple,plan.dailyRate,totalAmountNew,remainAmount,purchaseTime);
            _purchaseHistorys[userAddr].push(val);
        }
    }
    ...
    function initPlan() public onlyOwner{
        Plan memory plan1 = Plan(1000000000000000000000,100000000000000000000,20000,125);
        _plans.push(plan1);
    }
