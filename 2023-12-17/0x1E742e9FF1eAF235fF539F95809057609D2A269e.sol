...
contract KoreaBtg {
    ...
    IPancakeV3PoolState public poolBtg;
    IERC20 public usdt;
    IERC20 public amc;
    IERC20 public btg;
    struct OBJInviter {
    ...
    // mapping(address => uint256) stcBalance; 
    // mapping(address => uint256) dynBalance; 
    ...
    constructor(){
        ...
        btg = IERC20(0x0Bd74A50F0309Bc11158c4af11c20E5Fda5E43da);
        pool = IPancakeV3PoolState(0xC1fEb3e2E411f82BC5892328C17B36B45a1A64c1);
        poolBtg = IPancakeV3PoolState(0x93C8fC4304174934b45557C6c70cd323f333E67E);
        _zero = address(0x601d4d229Ebc978850845E9080Cbf7761d8318F4);
        _deadAddr = address(0x1);
        _oneday = 86400;
    }
    ...
    function getAmount1(uint256 _u) view private returns(uint256){
        (, int24 tick,,,,,) = poolBtg.slot0();
        return OracleLibrary.getQuoteAtTick(tick, uint128(_u), address(usdt), address(btg));
    }
    ...
    event joinLog(address indexed _u,uint256 _total, 
                uint256 _btg, uint256 _amc,uint _time);
    function join(uint256 _index) external {
        ...
        uint256 toBtg = getAmount1(uint128(_joinAmt[_index].mul(70).div(100)));
        require(amc.transferFrom(_user, address(this), toAmc),"transfer amc error");
        require(btg.transferFrom(_user, address(this), toBtg),"transfer btg error");
        orders.push(OBJorder(_user, _joinAmt[_index], toBtg, toAmc, _time));
        emit joinLog(_user, _joinAmt[_index], toBtg, toAmc, _time);
    }
    ...
    function atmUserRate(uint256 _num, bool _is) external {
        address _user = msg.sender;
        require(!_isCt(_user) && (tx.origin == _user), "atm address error");
        uint256 _atmAmt = atmBalance[_user];
        require(_atmAmt > 0 && _num > 0,"atm balance zero");
        require(_num <= _atmAmt, "atm out of range");
        if(_is){
            atmBalance[_user] = _atmAmt.sub(_num);
            uint256 to_btg = getAmount1(uint128(_num));
            require(btg.transfer(_user, to_btg.mul(97).div(100)),"atm btg error0");
            require(btg.transfer(_deadAddr, to_btg.mul(3).div(100)),"atm btg error1");
        }else{
            atmBalance[_user] = _atmAmt.sub(_num);
            uint256 to_amc = getAmount(uint128(_num));
            require(amc.transfer(_user, to_amc.mul(97).div(100)),"atm amc error0");
            require(amc.transfer(_deadAddr, to_amc.mul(3).div(100)),"atm amc error1");
        }
    }
