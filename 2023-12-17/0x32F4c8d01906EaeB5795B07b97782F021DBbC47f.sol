...
contract fzStake is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public FZAddress = address(0x4641724b1Ec5376719473FE0f334D3Bdc4CEa2ce);
    address public MKAddress = address(0xD33a88C1cd585B29651d24A038FB8634d31B7dde);
    uint256 public minAmount = 1 * 10 ** 18;
    uint256 public towAmount = 50 * 10 ** 18;
    uint256 public threeAmount = 100 * 10 ** 18;
    uint256 public foveAmount = 200 * 10 ** 18;
    uint256 public fiveAmount = 500 * 10 ** 18;
    ...
    function earned(address account) public view returns (uint256) {
        ...
        uint256 userTokenAmount = _balances[account];
        uint256 userTimestamp = _userTime[account];
        uint256 rewardRate = userTokenAmount.div(100).div(DURATION_1);

        if (userTokenAmount > 0) {
            returnAmount = block.timestamp.sub(userTimestamp).mul(rewardRate);
        }
    ...
    function deposit(uint256 amount) public nonReentrant {
        ...
        IERC20(FZAddress).safeTransferFrom(msg.sender, address(this), amount);
    ...
    function withdrawLevel() public nonReentrant {

        uint256 backAmount = _balances[msg.sender];

        require(backAmount > 0, 'ERROR balances');
        ...
        _userTime[msg.sender] = block.timestamp;
        _totalSupply = _totalSupply.sub(backAmount);
    ...
    function _takeInviterFee(address _address, uint256 amount) private {
        ...
        for (int256 i = 0; i < 13; i++) {
            ...
            if (cur == address(0)) {
                ...
                uint256 currentAmount = _balances[msg.sender];
                if (i == 0 && minAmount < currentAmount) {
                    ...
                    safeTransfer(cur, curTAmount);
                } else if (i  < 9 && foveAmount < currentAmount) {
                    safeTransfer(cur, curTAmount);
                } else if (i  < 13 && fiveAmount < currentAmount) {
                    safeTransfer(cur, curTAmount);
                } else {
                    mkAmount = mkAmount.add(curTAmount);
                }
