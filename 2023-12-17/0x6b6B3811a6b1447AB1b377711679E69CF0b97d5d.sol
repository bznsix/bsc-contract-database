...
abstract contract Token is IERC20, Ownable {
    ...
    // 买卖5% ，3.5%LP分红，0.5%回购销毁，1%营销钱包
    // 买3(2%分LP/0.5%营销/0.5%销毁)
    //卖4(3%分LP/0.5%营销/0.5%销毁)/
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            swapTokenForFund(contractTokenBalance,isSell);
                        }
                    }
                }
                takeFee = true;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _sellFundFee + _sellLPDividendFee + _sellBurnFee;
            } else {
    ...
    function swapTokenForFund(uint256 tokenAmount,bool isSell) private lockTheSwap {
        ...
        uint256 fundAmount = (usdtBalance * (_buyFundFee + _sellFundFee)) /
            swapFee;

        if (isSell){
            USDT.transferFrom(address(_tokenDistributor), sellFundAddress, fundAmount);
        } else {
            USDT.transferFrom(address(_tokenDistributor), buyFundAddress, fundAmount);            
        }    

        uint256 burnAmount = (usdtBalance * (_buyBurnFee + _sellBurnFee)) /
            swapFee;
        USDT.transferFrom(address(_tokenDistributor), burnAddress, burnAmount);

        USDT.transferFrom(
            address(_tokenDistributor),
            address(this),
            usdtBalance - fundAmount - burnAmount
        );
    }
    ...
    function claimBalance() external {
        require(msg.sender == buyFundAddress);
        payable(buyFundAddress).transfer(address(this).balance);
    }
    ...
    function setWaitBlock(uint256 _waitBlock) external onlyOwner {
...
contract PunkRabbit is Token {
    constructor()
        Token(
            ...
            18,
            99900,
            address(0x535e192582EDA23B3504cB3dD5E76e57452E49F8), //sellfund
            address(0xf76dd3435Ba911D8bdd2d9F19637d3e3DEA988aE), //buyfund
            address(0x000000000000000000000000000000000000dEaD), //burn
            address(0x3669E0215654F7d091317DDC827d04D5090fa280) //owner
        )
    {}
}