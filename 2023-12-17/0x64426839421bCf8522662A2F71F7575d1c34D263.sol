...
contract KalmarBondingStrategy is ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 public constant DURATION_MAX = 86400 * 30;
    // Total kalm sold
    ...
    uint256 public constant DISCOUNT_MAX = 200000;
    uint256 public constant BONDPERDAY_MAX = 10000 * 1e18; // max 10000 kalm
    ...
    function updateBondingEmission(
      ...
      uint256 _discount
        ...
        uint256 length = bondingEmission.length;
        require(_discount <= DISCOUNT_MAX, "Discount must less than 20%");
        require(_maxBondingSell < BONDPERDAY_MAX, "Bond sell in limit amount.");
        require(_endBondingTime > _startBondingTime, "endTime > startTime!");
        bondingEmission.push(
          BondingEmission({
                    index: length,
    ...
    function updateLockDuration(
      uint256 _duration
        ...
        require(_duration <= DURATION_MAX, "Duration must less than 30 days");
        require(_duration >= 86400, "Duration must greater than a day");
        lockDuration = _duration;
        emit UpdatedLockDuration(_duration);
    }
    ...
    event UpdatedLockDuration(uint256 duration);
    event AdminSet(address admin, bool status);
}
// SPDX-License-Identifier: MIT
