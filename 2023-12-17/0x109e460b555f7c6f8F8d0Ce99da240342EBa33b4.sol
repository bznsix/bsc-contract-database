...
contract LinkDaoOracle is Ownable {
    mapping(uint256 => uint256) private coinPrices;
    uint256 private isWithdrawalAllowed = 1;
    uint256 private isDepositAllowed = 1;
    ...
    function setWithdrawalAllowed(
        uint256 _isWithdrawalAllowed
    ) public onlyOwner {
        isWithdrawalAllowed = _isWithdrawalAllowed;
    }

    function getWithdrawalAllowed() public view returns (uint256) {
        return isWithdrawalAllowed;
    }

    function setDepositAllowed(uint256 _isDepositAllowed) public onlyOwner {
        isDepositAllowed = _isDepositAllowed;
    }

    function getDepositAllowed() public view returns (uint256) {
