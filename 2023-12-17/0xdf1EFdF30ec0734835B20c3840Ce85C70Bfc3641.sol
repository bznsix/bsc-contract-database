...
contract GrokPlus is Context, IBEP20, Ownable {
    ...
    uint256 constant private _tTotal = 1_000_000_000_000 * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public swapTokensAtAmount = _tTotal / 5_000;

    address constant private DEAD = 0x000000000000000000000000000000000000dEaD;
    address public marketingWallet = address(0x0FC1e93ED38904a6F93575a163aF2a91Ff4604da);
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        uint256 amountToSwap = contractBalance * 8 / 9;
        if(amountToSwap > 0) {
        ...
        uint256 amountToSendMarketing = address(this).balance * 5 / 8;

        if(amountToSendMarketing > 0) {(success,) = marketingWallet.call{value: amountToSendMarketing, gas: 35000}("");}

        if(address(this).balance > 0) {
            buyBackAndBurn(address(this).balance);
        }

        uint256 restToLp = balanceOf(address(this));
        swapAndLiquify(restToLp);
    ...
    function viewTaxes() external pure returns(uint256 reflections, uint256 _buyBackAndBurn, uint256 marketing, uint256 liquidity) {
        return(1,3,5,1);
    }
