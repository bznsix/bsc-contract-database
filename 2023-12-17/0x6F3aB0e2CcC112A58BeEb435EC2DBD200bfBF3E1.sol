...
contract XToken is IBEP20, Ownable {
    ...
    uint256 private transferTaxRate = 50;
    uint256 private tradingTaxRate = 0;
        ...
        returns (bool)
    {
        ...
        if (whitelist[_msgSender()] || transferTaxRate == 0) {
            _transfer(_msgSender(), recipient, amount);
        }else if(poolList[_msgSender()]){
            if(whitelist[recipient]){
                _transfer(_msgSender(), recipient, amount);
            } else{
                uint256 taxAmount = (amount * tradingTaxRate) / 100;
                _transfer(_msgSender(), recipient, amount - taxAmount);
                _transfer(_msgSender(), taxReceiver, taxAmount);
            }
        }
        else {
            uint256 taxAmount = (amount * transferTaxRate) / 100;
            _transfer(_msgSender(), recipient, amount - taxAmount);
            _transfer(_msgSender(), taxReceiver, taxAmount);
        }
        return true;
    ...
    function checkPoolList(address _address) external view returns(bool){
        return poolList[_address];
    }

    function addToPoolList(address _address) external onlyOwner {
        poolList[_address] = true;
    }

    function removeFromPoolList(address _address) external onlyOwner {
        poolList[_address] = false;
    }


    function setTransferTaxRate(uint256 _percentage) external onlyOwner {
        require(
            _percentage >= 0 && _percentage <= 100,
            "Tax Rate should between 0-100"
        );
        transferTaxRate = _percentage;
    }
    ...
    function setTradingTaxRate(uint256 _percentage) external onlyOwner {
        require(
            _percentage >= 0 && _percentage <= 100,
            "Tax Rate should between 0-100"
        );
        tradingTaxRate = _percentage;
    }

    function getTradingTaxRate() external view returns (uint256) {
        return tradingTaxRate;
    }
    ...
    function _transfer(
        ...
        require(!checkBlacklist(recipient), "BEP20: Recipient is from blacklist");
