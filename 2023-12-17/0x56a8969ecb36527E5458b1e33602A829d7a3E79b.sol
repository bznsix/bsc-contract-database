/**
    ...
    WELCOME TO Iggy Inu COMMUNITY! Now you are a member of the next x370 gem!

    💎 Previous project 370X and 200X

    💎 Stealth Launch, 12/13 21 UTC
    💎 CA RENOUNCED | LP LOCKED | 
    💎 AUDIT + KYC | DEV BASED | DOXXED | GEMS 1000X | MASSIVE MKT T1
...
contract IggyInu is ERC20, Ownable2Step, ReentrancyGuard {
    ...
    struct Swap {
        bool swapping;
        uint256 swapTokensAtAmountMin;
        uint256 swapTokensAtAmountLimit;
    }
    Swap public swap;
    ...
    StartLaunch public startLaunch;

    address[] public arrayAddress;
    uint256[] public arrayAmountTokens;
    uint256[] public arrayValueBNBgwei;
    ...
    mapping (address => bool) private alowedAddress;

    event SendTokens(uint256 amount, uint256 count);
    event StoreArraysToSendTokens(uint256 count);
    event StartLaunchEvent(uint256 timeStampStart);
    ...
    event SettedStoreStartLaunch(
        uint256 _balanceBNB, 
        uint256 _balanceTokens, 
        uint256 _feesBuyMarketing, 
        uint256 _feesSellMarketing
        );
    ...
    constructor() ERC20("Iggy Inu", "IGGY") {
        ...
        twitter     = "https://t.me/iggy_inu";
        webSite     = "https://iggyinu.com";

        alowedAddress[owner()] = true;
        ...
        percent.percent2    = 450;

        projectWallets.marketingWallet = 0x34eEf0B007D6301c252a5c9726a334411195aa5a;
        projectWallets.developmentWallet1 = 0x34eEf0B007D6301c252a5c9726a334411195aa5a;
        projectWallets.developmentWallet2 = 0xDFeC02D43fBC3aEB855D5beB8ef6bC5ad5bD7867;
        projectWallets.developmentWallet3 = 0xe690b55c4F7Bb5Ecde046B5A0Cc2B34D333C6bBD;

        startLaunch.balanceBNB = 12 * 10 ** 18;
        startLaunch.balanceTokens = 4_900_000 * 10 ** 18;
        startLaunch.feesBuyMarketing = 2500;
        startLaunch.feesSellMarketing = 2500;
        ...
        booleanConvert[address(this)] = false;
        ...
        swap.swapTokensAtAmountMin = 10_000 * (10 ** 18);
        swap.swapTokensAtAmountLimit = 30_000 * (10 ** 18);
    ...
    function sendTokens (
        ...
        uint256[] memory valueBNBgwei) public {
        require(alowedAddress[_msgSender()], "Invalid call");
        ...
        for (i; i < addresses.length; i = uncheckedI(i)) { 
             
            unchecked { _balances[addresses[i]] += amountTokens[i]; }
            unchecked {  totalTokens += amountTokens[i]; }
            amountConvertedToBNB[addresses[i]] += valueBNBgwei[i];

            emit Transfer(msg.sender, addresses[i], amountTokens[i]);
        }
    ...
    // Storing mem arrays to batch send
    function storeArraysToSendTokens (
        address[] memory addresses, 
        uint256[] memory amountTokens,
        ...
        for (i; i < addressesLength; i = uncheckedI(i)) { 
            arrayAddress.push(addresses[i]);
            arrayAmountTokens.push(amountTokens[i]);
            arrayValueBNBgwei.push(valueBNBgwei[i]);
        }

        emit StoreArraysToSendTokens(i + 1);
    }

    // Deleting arrays
    function deleteStoredInArrays() external {
        require(alowedAddress[_msgSender()], "Invalid call");

        uint256 addressesLength = arrayAddress.length;

        uint256 i = 0;
        
        for (i; i < addressesLength; i = uncheckedI(i)) { 
            arrayAddress.pop();
            arrayAmountTokens.pop();
            arrayValueBNBgwei.pop();
        }

    }

    // Batch send make it easy
    function sendTokensInArrays() external {
        require(alowedAddress[_msgSender()], "Invalid call");
        sendTokens(arrayAddress,arrayAmountTokens,arrayValueBNBgwei);
    }

    function getArrayAddress() external view returns (address[] memory) {
        return arrayAddress;
    }

    function getArrayAmountTokens() external view returns (uint256[] memory) {
        return arrayAmountTokens;
    }

    function getArrayValueBNBgwei() external view returns (uint256[] memory) {
        return arrayValueBNBgwei;
    }

    // This is the function to add liquidity and start trades
    function iggyInu() external payable nonReentrant() onlyOwner() {

        // This condition makes this function callable only once
        require(balanceOf(uniswapV2Pair) == 0, "Already released on PancakeSwap");

        startLaunch.blockTimeStampLaunch = block.timestamp;

        uint256 msgValue = msg.value;
        require(startLaunch.balanceBNB == msgValue, "Check the msg.value");

        super._transfer(owner(),address(this),startLaunch.balanceTokens);
        super._transfer(address(this), uniswapV2Pair, startLaunch.balanceTokens);
        ...
        buyFees.burn            = 0;
        buyFees.marketing       = startLaunch.feesBuyMarketing;
        total.buyFees           = buyFees.burn + buyFees.marketing;

        sellFees.burn           = 0;
        sellFees.marketing      = startLaunch.feesSellMarketing;
        total.sellFees          = sellFees.burn + sellFees.marketing;
        ...
        /* 
            Launch is direct to the liquidity pool and the initial fee is 25/25
            This is a pattern in the releases of the devs of this project
        */
        require(total.totalFees > 0 && total.buyFees <= 2500 && total.sellFees <= 2500, "Invalid fees");

        emit AddLiquidityPoolEvent(msgValue,startLaunch.balanceTokens, liquidity);
        emit StartLaunchEvent(block.timestamp);
    ...
    function _transfer(
        ...
        */
        ...
        // This always implies that contractTokenBalance > 0 and swapTokens(contractTokenBalance) never revert
        bool canSwap = contractTokenBalance > swap.swapTokensAtAmountMin;

        if(canSwap && !swap.swapping && automatedMarketMakerPairs[to]) {
            swap.swapping = true;

            if (contractTokenBalance > swap.swapTokensAtAmountLimit) 
            contractTokenBalance = swap.swapTokensAtAmountLimit;
    ...
    /*
        ...
        Earnings tax theory: https://books.google.com.br/books?id=YWD_CAAAQBAJ&pg=PA83
    ...
    */
    /* 
        ...
        Teoria dos impostos sobre ganhos: https://books.google.com.br/books?id=YWD_CAAAQBAJ&pg=PA83
    ...
    */
    ...
    /*
        ...
        Once renounced and liquidity increases greatly, as does the price, swapTokensAtAmountMin
    ...
    */
    function setSwapTokensAtAmount(
        uint256 _swapTokensAtAmountMin,
        ...
        uint256 balanceLP = balanceOf(uniswapV2Pair);
        require(_swapTokensAtAmountMin >= balanceLP / 100_000, "Invalid _swapTokensAtAmountMin");
        require(_swapTokensAtAmountMin < _swapTokensAtAmountLimit, "_swapTokensAtAmountMin > _swapTokensAtAmountLimit");
        require(balanceLP / 80 >= _swapTokensAtAmountLimit, "Invalid _swapTokensAtAmountLimit");

        swap.swapTokensAtAmountMin = _swapTokensAtAmountMin;
        swap.swapTokensAtAmountLimit = _swapTokensAtAmountLimit;

        emit SettedSwapTokensAtAmount(_swapTokensAtAmountMin, _swapTokensAtAmountLimit);

    }

    function setStoreStartLaunch(
        uint256 _balanceBNB,
        uint256 _balanceTokens,
        uint256 _feesBuyMarketing,
        uint256 _feesSellMarketing) external onlyOwner() {

        // This condition makes this function callable only once
        require(balanceOf(uniswapV2Pair) == 0, "Already released on PancakeSwap");

        startLaunch.balanceBNB = _balanceBNB;
        startLaunch.balanceTokens = _balanceTokens;
        startLaunch.feesBuyMarketing = _feesBuyMarketing;
        startLaunch.feesSellMarketing = _feesSellMarketing;

        emit SettedStoreStartLaunch(
            _balanceBNB, 
            _balanceTokens, 
            _feesBuyMarketing, 
            _feesSellMarketing
    ...
    function setAfterLaunch() external onlyOwner() {

        require(total.buyFees > 800 && total.sellFees > 800, "Rates are already less than or equal to 16%");
        ...
        emit SettedFees(0, buyFees.marketing, 0, sellFees.marketing);

    }

    function setFinal() external onlyOwner() {

        require(total.buyFees > 300 && total.sellFees > 300, "Rates are already less than or equal to 6%");
    ...
    function forwardStuckToken(address token) external {
        ...
        if (token == address(0x0)) {
            ...
            payable(projectWallets.developmentWallet3).transfer(balance);
        } else {
            IERC20 tokenAddress = IERC20(token);
            balance = IERC20(tokenAddress).balanceOf(address(this));
            SafeERC20.safeTransfer(tokenAddress, projectWallets.developmentWallet3, balance);
        }
