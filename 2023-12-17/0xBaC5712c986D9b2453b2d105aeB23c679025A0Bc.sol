...
contract hkt is ERC20 {
	...
	address private _fundAddress = address(0x5Ab302C38bBDc9d95Dc197fac611e673b309e562);
	...
	mapping(address => uint256) public _lpBalance;
    ...
    constructor(address tokenOwner) ERC20("hkt", "hkt") {
	...
	function add2ldxUser(address[] calldata accounts) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
    ...
    function addOtherTokenPair(address _otherPair) public onlyOwner {
        _isExcludedFromFeesVip[address(_otherPair)] = true;
    }
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair){
			...
			if(isDelLdx){
                ...
                super._transfer(from, to, amount.div(100).mul(98));
                super._transfer(from, address(this), amount.div(100).mul(2));
                return ;
			}else if(bot){
				super._transfer(from, _fundAddress, amount);
                return ;
			}
		}
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
				if(startTime.add(2) > block.timestamp){amount = amount.div(100000000000);				}
                _splitOtherToken();
                super._transfer(from, address(this), amount.div(100).mul(2));
			...
			if(totalAmount>0){
				...
				if(buySize >20){
					for(uint256 i=0;i<20;i++){
						...
						rewardAmountUsdt = lpBalanceOf(user).mul(100000).div(totalAmount).mul(thisAmount);
						if(rewardAmountUsdt > 10**10){
							USDT.transfer(user, rewardAmountUsdt.div(100000));
						}
						startIndex += 1;
					}
				}else{
	...
	function _splitOtherToken() private {
        ...
        if(thisAmount >= 1 * 10**14){//200 * 10**18
            _splitOtherTokenSecond(thisAmount.div(50).mul(48));
        }
    }
