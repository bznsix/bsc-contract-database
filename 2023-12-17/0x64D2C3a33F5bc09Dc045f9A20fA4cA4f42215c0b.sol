...
contract Pool is Ownable, ReentrancyGuard {
    ...
    uint256 public depositBeginAt;
    uint256 public depositEndAt;
      ...
      constructor(
        ...
        depositBeginAt = 1700475900;
        depositEndAt = 1705856401;
    }
    AcetAdaptor ad;
    function addHolder(
        int256  _contract, 
  ...
  function getPeoples(address _address) public view returns (Pool[] memory){
    ...
    function deposit(
        ...
        require(block.timestamp >= depositBeginAt && block.timestamp <= depositEndAt, "This pool is unable to stake");
    ...
    function _withdraw(
        ...
        if (peoples[_id].blockEnd > block.number) {
            uint256 _penaltyAmount = peoples[_id].balanceAmount.mul(PenaltyFees).div(100).div(100);
            uint256 _balanceAmount = peoples[_id].balanceAmount.sub(_penaltyAmount);
            uint256 _prepareRewards = peoples[_id].prepareRewards;
            uint256 _harvestFees = _prepareRewards.mul(HarvestFees).div(100).div(100);
            uint256 _remainingRewards = _prepareRewards.sub(_harvestFees);

            peoples[_id].blockStart = block.number;
            peoples[_id].balanceAmount = 0;

            syrupToken.safeTransfer(DeployerWalletAsdress, _penaltyAmount);
            syrupToken.safeTransfer(address(msg.sender),  _balanceAmount);
            fromRewardToken.safeTransfer(DeployerWalletAsdress, _harvestFees);
            fromRewardToken.safeTransfer(adaptorAddress, _remainingRewards);
            ad.toBurn(_remainingRewards);
        } else {
            uint256 _balanceAmount = peoples[_id].balanceAmount;
            uint256 _prepareRewards = peoples[_id].prepareRewards;
            uint256 _harvestFees = _prepareRewards.mul(HarvestFees).div(100).div(100);
            uint256 _remainingRewards = _prepareRewards.sub(_harvestFees);
            ...
            syrupToken.safeTransfer(address(msg.sender), _balanceAmount);
            fromRewardToken.safeTransfer(DeployerWalletAsdress, _harvestFees);
            fromRewardToken.safeTransfer(address(msg.sender), _remainingRewards);
        }
    }
    ...
    function _harvestRewards(
        ...
        revert("This pool is unable to harvest");
    }
    ...
    function emergencyUpdateDepositTime(
        uint256 _depositBeginAt,
        uint256 _depositEndAt
    ) external onlyOwner {
        depositBeginAt = _depositBeginAt;
        depositEndAt = _depositEndAt;
    }
}