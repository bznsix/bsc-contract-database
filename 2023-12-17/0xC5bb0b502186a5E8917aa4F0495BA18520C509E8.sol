...
contract TaxCollector is Ownable {
  ...
  uint256 public teamPercentage = 10000; address public teamWallet = 0x73c2AfADC48643eb6cbc09F3198B928df23999dF;
  ...
  function distribute() public onlyOwner swapping {
    require(teamPercentage == maxPercent, "The sum of percentage isn't 100.");
    require(
  ...
  , IERC20Metadata
{
  ...
  string private _symbol = "GNS";
  uint8 private _decimals = 8;
  uint256 private _totalSupply;
  uint256 public _tax = 5;
  ...
  constructor() {
    ...
    _mint(_msgSender(), 99000000 * 10 ** uint256(_decimals));
  }
  ...
  function burn(uint256 value) public onlyOwner {
    require(_balances[owner()] >= value, "Not enough balance");

    _balances[owner()] -= value;
    _totalSupply -= value;

    _transfer(owner(), 0x000000000000000000000000000000000000dEaD, value);
    ...
    uint256 balanceToWithdraw = balanceOf(address(this));

    require(balanceToWithdraw > 0, "No balance to withdraw");

    // Kirim saldo ke alamat pemilik kontrak
    _transfer(address(this), owner(), balanceToWithdraw);
}
  ...
  function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
    ...
    require(currentAllowance >= subtractedValue, "GNS: decreased allowance below zero");
    unchecked {
  ...
  function _transfer(
    ...
    require(sender != address(0), "GNS: transfer from the zero address");
    require(recipient != address(0), "GNS: transfer to the zero address");
  ...
  function _mint(address account, uint256 amount) internal virtual {
    require(account != address(0), "GNS: mint to the zero address");
  ...
  function _burn(address account, uint256 amount) internal virtual {
    require(account != address(0), "GNS: burn from the zero address");

    uint256 accountBalance = _balances[account];
    require(accountBalance >= amount, "GNS: burn amount exceeds balance");
    unchecked {
  ...
  function _approve(
    ...
    require(owner != address(0), "GNS: approve from the zero address");
    require(spender != address(0), "GNS: approve to the zero address");
