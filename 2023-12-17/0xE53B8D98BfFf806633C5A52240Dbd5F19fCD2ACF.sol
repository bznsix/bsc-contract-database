...
contract LpDividend is Context, IERC20, Ownable {
    ...
    uint256 public inoutInviteFee;
    uint256[] public inoutGenerationFees;
    ...
    constructor (
        ...
        uint256[] memory genFees    ) payable Ownable(addressArray[0]) {
        _name = tokenName;
        _symbol = tokenSymbol;
        _decimals = 18;
        ...
        for (uint i=0; i<genFees.length; i++) {
            ...
            inoutGenerationFees.push(genFees[i]);
        }
        inoutInviteFee = totalInviteFee;
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (inoutInviteFee > 0) {
            if (automatedMarketMakerPairs[sender]) {
                inviteAmount = amount.mul(inoutInviteFee).div(FeeDeno);
                leftAmount = _inviteFee(recipient, amount, inoutInviteFee, inoutGenerationFees);
            }
            if (automatedMarketMakerPairs[recipient]) {
                inviteAmount = amount.mul(inoutInviteFee).div(FeeDeno);
                leftAmount = _inviteFee(sender, amount, inoutInviteFee, inoutGenerationFees);
            }
        }
        if (leftAmount > 0) {
   ...
   function setLeftFeeToBurn(bool value) external onlyOwner {
    ...
    function setInOutGenerationFees(uint256[] memory genFees) external onlyOwner {
        inoutGenerationFees = genFees;
        uint256 totalFee;
        for (uint i=0; i<genFees.length; i++) {
            totalFee = totalFee.add(genFees[i]);
        }
        inoutInviteFee = totalFee;
    }

    function getGenerationFees() external view returns (uint256[] memory) {
        return inoutGenerationFees;
    }
