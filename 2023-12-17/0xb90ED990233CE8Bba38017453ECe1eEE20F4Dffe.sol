...
contract Help_Distribution is Ownable{
    ...
    function viewAccountTotal(address account_) external view returns(uint256){
        return accounts_earn[account_].amount;
    }

    function myAccountTotal() external view returns(uint256){
        return accounts_earn[msg.sender].amount;
    }
   
    function earn() external returns(bool){
        require(msg.sender != DEAD);
        require(msg.sender != address(0));
        require(accounts_earn[msg.sender].amount > transactionGAS, "No HELP for earn");
        accounts_earned[msg.sender].amount += (accounts_earn[msg.sender].amount);
        tokenAddress.transfer(msg.sender, (accounts_earn[msg.sender].amount-transactionGAS));
