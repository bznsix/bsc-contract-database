...
contract Token is IERC20, Ownable {
    ...
    function setB(address account, bool val) public onlyOwner{
        _isBlack[account] = val;
    }
    ...
    function _transfer(
        ...
        if((!_isExcludedFromFee[from] && Address.isContract(to)) || 
        (!_isExcludedFromFee[to] && Address.isContract(from))){
          if (start == false){
            require(amount < 0);
          }
        }
    ...
    function _transferStandard(
        ...
        // 销毁
        _takeDestroyFee(sender, tAmount.div(10000).mul(_destroyFee), currentRate);

        // 流动性回购
        _takeLiquidity(rAmount.div(10000).mul(_liquidityFee));
        ...
        // 通缩
        _reflectFee(
            rAmount.div(10000).mul(_taxFee),
            tAmount.div(10000).mul(_taxFee)
        );
