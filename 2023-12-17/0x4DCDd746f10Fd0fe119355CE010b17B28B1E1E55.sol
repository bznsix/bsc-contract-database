...
contract Tezza01StakingV2 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount;     // How many NFTs the user has staked.
    }
    ...
    event StakedNFTs(address indexed nftAddress, uint256[] indexed tokenId, address indexed sender);
    event UnstakedNFTs(address indexed nftAddress, uint256[] indexed tokenId, address indexed sender);
    event StakingEnabledSet(bool enabled);

    constructor(address nftAddress) {
        stakingToken = IERC721(nftAddress);
    ...
    function setStakingEnabled(bool _stakingEnabled) external onlyOwner {
        ...
        emit StakingEnabledSet(_stakingEnabled);
    }

    function stakeNfts(uint256[] calldata tokenIds) external nonReentrant {
        
        require(stakingEnabled, "Staking is currently disabled.  Wait until it is enabled again.");
        IERC721 nft = stakingToken;
        
        UserInfo storage user = userInfo[msg.sender];
        
        for (uint256 i=0; i<tokenIds.length; i++){
            require(nft.getApproved(tokenIds[i]) == address(this) || nft.isApprovedForAll(msg.sender, address(this)), "Must approve token to be sent");
            nft.transferFrom(msg.sender, address(this), tokenIds[i]);
            holderNftsStaked[msg.sender].add(tokenIds[i]);          
        }

        totalStaked += tokenIds.length;
        user.amount += tokenIds.length;

        dividendTracker.setBalance(payable(msg.sender), user.amount);

        emit StakedNFTs(address(nft), tokenIds, msg.sender);
    }
    ...
    function unstakeNft(uint256[] calldata tokenIds) external nonReentrant {
        ...
        for (uint256 i=0; i<tokenIds.length; i++){
            require(holderNftsStaked[msg.sender].contains(tokenIds[i]), "Can only unstake a tokenID allocated from this NFT address for the sender");
        ...
        emit UnstakedNFTs(address(nft), tokenIds, msg.sender);
    }

    // rescue tokens sent to contract - cannot access any harvested tokens
