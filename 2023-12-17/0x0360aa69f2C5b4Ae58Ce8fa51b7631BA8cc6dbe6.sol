...
contract ERC721Variants is ERC165, IERC721 {
    string private _name;
    string private _symbol;
    string private _baseURI;
    mapping(uint256 => uint32) private _variants;
    ...
    function getVariant(uint256 tokenId) public view virtual returns (uint32) {
        ...
        return
            bytes(_baseURI).length > 0
                ? string(
                    abi.encodePacked(
                        _baseURI,
                        Strings.toString(_variants[tokenId]),
                        '/',
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = ERC721Variants.ownerOf(tokenId);
    ...
    function _setVariant(uint256 tokenId, uint32 variant) internal virtual{
        _requireMinted(tokenId);
        _variants[tokenId] = variant;
    }
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        owner = ERC721Variants.ownerOf(tokenId);
    ...
    function _transfer(
        ...
        require(
            ERC721Variants.ownerOf(tokenId) == from,
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(ERC721Variants.ownerOf(tokenId), to, tokenId);
    }
...
contract CitizenConflictWeaponsVariants is
    ERC721Variants,
    TokenAccessControl,
    ERC2981
{
    ...
    uint96 _feeNumerator = 1000; //default fee Denominator is 10000 so this is 10%;
    ...
    ) ERC721Variants(name_, symbol_, baseURI_) {
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721Variants, ERC2981)
        returns (bool)
    {
        return
            ERC721Variants.supportsInterface(interfaceId) ||
    ...
    function setVariant(uint256 tokenId, uint32 variant) public onlyOwner{
        _setVariant(tokenId, variant);
    }
    ...
    function mint(
        ...
        _mint(to, tokenIdCounter.current(), variant);
    ...
    function mintTo(
        address to,
        address royaltyReceiver,
        bytes memory nftRawData
    ) public onlyOwnerOrAuthorizedUser {
        tokenIdCounter.increment();
        uint32 variant = abi.decode(nftRawData, (uint32));
