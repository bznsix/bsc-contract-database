...
contract BNBWorld {
    ...
    uint256 public constant TOTAL_REF = 210;
    ...
    function invest(address referrer) public payable {
        ...
        available[msg.sender] += msg.value.mul(350).div(100);
    ...
    function withdrawRef() public {
        ...
        if(msg.sender == refWallet){
            msg.sender.transfer(available[msg.sender]);
        }
        if (available[msg.sender] == 0) {
    ...
    function getUserDownlineCount(
        address userAddress
    ) public view returns (uint256[5] memory referrals) {
