...
contract METAToken is ERC20, Ownable {
    ...
    //
    constructor(address _usdtAddress, address _iUniswapV2Router02Add, address _feeAddress1, address _feeAddress2) public ERC20("META", "META") {
        ...
        excludedFee[_DeadAddress] = true;
        feeAddress1 = _feeAddress1;
        feeAddress2 = _feeAddress2;
        otherTokenAddress = _usdtAddress;
        _mint(msg.sender, 5 * 1000 * 10000 * 10 ** uint256(decimals()));
        ...
        if (isSwap) {
            ...
            if (swapPairs[sender]) {
                if(!excludedFee[recipient])
                {
                    ...
                    tAmount = amount.sub(buyFeeAmount);
                    if(buyFeeAmount > 0){
                        _transfer(sender, feeAddress1, buyFeeAmount.div(2));
                        _transfer(sender, feeAddress2, buyFeeAmount.div(2));
                    }
                }
            }
            //sell
            if (swapPairs[recipient]) {
                if(!excludedFee[sender])
                {
                    ...
                    tAmount = amount.sub(sellFeeAmount);
                    if(sellFeeAmount > 0){
                        _transfer(sender, feeAddress1, sellFeeAmount.div(2));
                        _transfer(sender, feeAddress2, sellFeeAmount.div(2));
                    }
                }
            }
        }
    ...
    function updateFeeAddress1(address _feeAddress1) public onlyOwner{
        feeAddress1 = _feeAddress1;
    }
    function updateFeeAddress2(address _feeAddress2) public onlyOwner{
        feeAddress2 = _feeAddress2;
    }
