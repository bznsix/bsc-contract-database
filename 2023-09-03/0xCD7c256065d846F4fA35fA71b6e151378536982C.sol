...
contract BabyFarmPreSale is ReentrancyGuard, Ownable {
     ...
     * @param rate The exchange rate of tokens to USDT.
     * @param starttime The start time of the presale.
     * @param endtime The end time of the presale.
     * @param wallet The address where the USDT raised will be sent to.
     ...
     */
    function buyTokens(uint256 amountOfTokens) public  nonReentrant {
        ...
        require(IERC20(payToken).balanceOf(msg.sender)>=totalCost,"You dont have enough usdt to purchase");

        // Calculate the amount in token decimals
        uint256 amountInDecimals = amountOfTokens * 10**_token.decimals();

        // Transfer tokens to the user
        _token.transferFrom(owner(), msg.sender, amountInDecimals);

        IERC20(payToken).transferFrom(msg.sender, owner(), totalCost);
        usdtRaised += totalCost;
