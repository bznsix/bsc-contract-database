...
contract TCHPresale is Ownable {
    ...
    IERC20 public immutable token = IERC20(0x9c6c2617D408F50fEF599A3e03c4c464293fdAD3);
    ...
    address public wallet = payable(0x1D5f93fBf6CBB34b6651868AD089B349791aCb89);

    // How many token units a buyer gets per BNB & USDT
    uint256 public tokenPriceUSD = 9;
    uint256 public bnbDivider = 100000;
    uint256 private usdtRate = 11111;
    ...
    //referral bonus
    uint256 private referralBonus = 3;
    ...
    function buyTokens() public payable {
        ...
        require(msg.value >= 0.02 ether, "cant buy less with than 0.02 BNB");
        uint256 weiAmount = msg.value;
        //uint256 tokens = weiAmount * ethRate;

        // Get the ETH price from the ChainlinkPriceOracle
        int256 bnbPrice = priceOracle.getLatestPrice();
        require(bnbPrice > 0, "Invalid ETH price");

        uint256 tokens = weiAmount.mul(uint256(bnbPrice)).mul(bnbDivider).div(tokenPriceUSD);
    ...
    function buyTokensReferral(address _friend) public payable {
      ...
      //  uint256 tokens = weiAmount * bnbRate;
        ...
        uint256 bonus = tokens.mul(referralBonus).div(100);

        require(claimableTokens >= tokens.add(bonus), "Not enought tokens availible");
        ...
        Customer[msg.sender].totalTokens += tokens;
        Customer[msg.sender].totalAmount += tokens;
        Customer[_friend].referrerBonus += bonus;
        Customer[_friend].totalTokens += bonus;
        Customer[_friend].totalAmount += bonus;
        totalBonus +=bonus;
        tokensSold += tokens;
        claimableTokens -= tokens.add(bonus);
    ...
    function buyTokensWithUSDT(uint256 amount) public {
        require(hasPresaleStarted,"Presale not started");
        require(!hasPresaleEnded, "Presale has ended");
        require(amount >= 10 * 10 ** 18, "cant buy less with than 10 USDT");
   ...
   function claimTokens() public {
    ...
    if (BuyerPhase[msg.sender].phaseOne == false) {
        ...
        valueToBeClaimed += Customer[msg.sender].totalAmount.mul(10).div(100);
    }
    ...
    if (block.timestamp >= endTime + 120 days) {
    ...
    if (block.timestamp >= endTime + 150 days) {
        if (BuyerPhase[msg.sender].phaseSix == false) {
            BuyerPhase[msg.sender].phaseSix = true;
            valueToBeClaimed += Customer[msg.sender].totalAmount.mul(10).div(100);
        }
    }

    if (block.timestamp >= endTime + 180 days) {
        if (BuyerPhase[msg.sender].phaseSeven == false) {
            BuyerPhase[msg.sender].phaseSeven = true;
            valueToBeClaimed += Customer[msg.sender].totalAmount.mul(10).div(100);
        }
    }

    if (block.timestamp >= endTime + 210 days) {
        if (BuyerPhase[msg.sender].phaseEight == false) {
            BuyerPhase[msg.sender].phaseEight = true;
            valueToBeClaimed += Customer[msg.sender].totalAmount.mul(10).div(100);
        }
    }

    if (block.timestamp >= endTime + 240 days) {
        if (BuyerPhase[msg.sender].phaseNine == false) {
            BuyerPhase[msg.sender].phaseNine = true;
            valueToBeClaimed += Customer[msg.sender].totalAmount.mul(10).div(100);
        }
    }

    if (block.timestamp >= endTime + 270 days) {
        if (BuyerPhase[msg.sender].phaseTen == false) {
            BuyerPhase[msg.sender].phaseTen = true;
            valueToBeClaimed += Customer[msg.sender].totalAmount.mul(10).div(100);
        }
    }
    ...
    //TO UPDATE RATES
    function changeRate(uint256 _usdtRate, uint256 _bnbToUsd, uint256 divider) public onlyOwner {
        require(_usdtRate > 0, "Rate cannot be 0");
        require(_bnbToUsd > 0, "Rate cannot be 0");
        tokenPriceUSD = _bnbToUsd;
        usdtRate = _usdtRate;
        bnbDivider = divider;
    }
   ...
   function getBNBPriceInUSD() public view returns (int256) {
