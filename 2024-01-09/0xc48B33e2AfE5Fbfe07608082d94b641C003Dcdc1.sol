...
contract TokenBridgeWhitelisted is Ownable, FeeCollector {
    ...
    function addEligibleAddresses(address[] memory addresses, uint256[] memory balances) public onlyOwner {
        require(addresses.length == balances.length, "TokenVesting: Addresses and amounts must have the same length");

        for (uint256 i = 0; i < addresses.length; ++i)
            currentBalances[addresses[i]] = balances[i];
    }
    ...
    function exchangeTokens(uint256 amount) public payable {
    ...
    function exchangeTokens(uint256 amount, uint256 minAmountBack) public payable collectFee('exchangeTokens') {
        ...
        require(currentBalances[msg.sender] >= srcAmount, "TokenBridge: insufficient balance");
        currentBalances[msg.sender] = currentBalances[msg.sender].sub(srcAmount);
