...
contract YF33 is Ownable, ReentrancyGuard {
    ...
    event Upgrade(  uint256 amount);
     constructor(
        ...
        YF3  = IERC20(0x4bbA0a644B665A51E3E944eEf7B08315C4B23F46);
        YFDT  = IERC20(0x66c70e028b8af5A3113972dDB8e668F4b24BA9D1);
         
        router = ISwapRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        pair = IUniswapV2Pair(
            IPancakeFactory(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73).getPair(0x02B2e0C9BB05b6378Ae5dA675116c5e4fCD5C931, 0x4bbA0a644B665A51E3E944eEf7B08315C4B23F46)
        );
    ...
    function YFDTToUSDT(uint256 value ) public   {
        ...
        YF3.transfer(msg.sender ,value/50/getTokenPrice());
    }
...
// 价值10U的YF  5个ID
     function sellCard(uint256 ID1,uint256 ID2,uint256 ID3,uint256 ID4,uint256 ID5) public   {
        YF3.transfer(msg.sender ,10*(1e18)/getTokenPrice());
    }
    ...
    function addLiquidity(uint256 amount,address sender) public   {
        require(
            USDT.transferFrom(msg.sender, address(this), amount),
            "transfer error"
        );
        ...
        uint256 _M50Amount =   _buyHD(_50Amount);
        _addLiquidity(_M50Amount*2);
        YF3.transferFrom(msg.sender, deadAddress, amount);
    ...
    function sell(uint256 amount) public  returns (uint256) {
        ...
        (uint256 LPAmount, ) =  _calculateLPAmount(amount);
