...
contract BITCOIN_ETF {
    DividendDistributor public distributor;
    IDEXRouter public router =
        IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);

    BEP20 MTK = BEP20(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c);
    string constant _name = "BITCOIN ETF 2024";
    string constant _symbol = "btcETF2024";
    ...
    //SELL FEES
    uint256 public sellLiquidityFee = 400; 
    uint256 public sellMarketingFee = 200; 
    uint256 public sellPoolFee = 0; 
    uint256 public sellReflectionFee = 1400; 
  ...
  function swapBack(
        ...
        uint256 totalFees = marketing + liquidity + stakePool + reflection;

        if (totalFees > swapThreshold) {
            totalFees = swapThreshold;
        }
        ...
        uint256 amountBNBLiquidity = liquidity > 0 ? getamount(liquidity / 2, path) : 0;
        uint256 amountBNBMarketing = marketing > 0 ? getamount(marketing, path) : 0;
        uint256 amountBNBStakePool = stakePool > 0 ? getamount(stakePool, path) : 0;
        uint256 amountBNBReflection = reflection > 0 ? getamount(reflection, path) : 0;

        uint256 amountToLiquidify = liquidity > 0 ? (liquidity / 2) : 0;

        uint256 amountToSwap = amountToLiquidify > 0 ? totalFees - amountToLiquidify : totalFees;
        ...
        if (amountBNBMarketing > 0) {
            bool success;
            (success, ) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: txbnbGas}("");
            require(success, "Transfer failed");
        }

        if (amountBNBStakePool > 0) {
            bool success;
            (success, ) = payable(StakePoolReceiver).call{value: amountBNBStakePool, gas: txbnbGas}("");
            require(success, "Transfer failed");
        }

        if (amountBNBReflection > 0) {
            try distributor.deposit{value: amountBNBReflection, gas: distributorBuyGas}() {} catch {}
        }

        if (amountToLiquidify > 0) {
            uint256 amountBNBLiquidityToUse = amountToLiquidify <= address(this).balance ? amountBNBLiquidity : address(this).balance;

            router.addLiquidityETH{value: amountBNBLiquidityToUse, gas: LiquidifyGas}(
                address(this),
                amountToLiquidify,
                0,
                0,
