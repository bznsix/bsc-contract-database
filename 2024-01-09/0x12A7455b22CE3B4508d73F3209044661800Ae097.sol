...
contract chicken is IERC20, tokenStaking, Ownable {
    ...
    mapping (address => uint256) public _holdings;
    mapping (address => uint256) public walletReflections;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        setBalanceAmounts(sender, recipient);
    }
    ...
    function setReflectionAmounts(address sender, address recipient, uint256 balanceSender, uint256 balanceRecipient) internal {
        ...
        if(balanceOfSender.sub(balanceSender) >= 0){reflectionsSender = balanceOfSender.sub(balanceSender);}
        walletReflections[sender] = walletReflections[sender].add(reflectionsSender);
        uint256 balanceOfRecipient = balanceOf(recipient); uint256 reflectionsRecipient;
        if(balanceOfRecipient.sub(balanceRecipient) >= 0){reflectionsRecipient = balanceOfRecipient.sub(balanceRecipient);}
        walletReflections[recipient] = walletReflections[recipient].add(reflectionsRecipient);
    }

    function setBalanceAmounts(address sender, address recipient) internal {
    ...
    function setLaunch(address _pair) external onlyOwner {
        ...
        _isExcluded[address(pair)] = true; _isExcluded[address(router)] = true;
    }
    ...
    function myTotalReflections(address wallet) public view returns (uint256) {
        ...
        uint256 totalReflectionAmount = walletReflections[wallet];
    ...
    function setminBuybackToken(uint256 amount) external onlyOwner {
    ...
    function setBuybackParameters(uint256 _ETHAdd, bool enable) external onlyOwner {
