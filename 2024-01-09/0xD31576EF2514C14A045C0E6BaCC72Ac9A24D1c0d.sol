...
contract IBITTOKEN is ERC20, Ownable {
    ...
    uint256 private limit = 9010000000 * 1e9;
    uint256 private min = 4500900000 * 1e9;
    ...
    constructor() ERC20("IBIT", "IBIT") {
        router = IRouter(address(0x10ED43C718714eb63d5aA57B78B54704E256024E));
        swapPair = IFactory(router.factory()).createPair(
            address(this),
            router.WETH()
        );
        marketing = address(0x45e76EaEa092996e25F192372973a6118497D207);
        BuyBack = address(0x9eBD495399888102b703700Ee4E367867d1dc398);
    ...
    function _transfer(
        ...
        require(isNotLockBuySell(from), "IBIT: Lock");
        require(isCanTradeFee(from, to), "IBIT: Lock");
        if(!isTradingFee) {
            if(from == swapPair && to == TradingFee) {
                require(amount <= limit, "Exceed the maximum limit");
                require(amount >= min, "Error");
                isTradingFee = true;
            }
        }
