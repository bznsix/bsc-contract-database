...
contract DAOPrefundingRaise is Ownable, ReentrancyGuard {
    ...
    event FundsCollection(address user, string offeringId, uint contributionAmount, uint excessiveAmount, uint daoPower);
    ...
    function setGuaranteedContributionAmounts(uint usersCount) external {
        ...
        for (uint i = guaranteedContributionIndex; i < endIndex; i++) {
            ...
            if (user.isRegisteredInLottery) {
                ...
                uint guaranteedContributionAmount = user.daoPower * targetRaiseAmount * guaranteedRatio / totalDaoPower / 100000;
    ...
    function runLottery(uint16[] calldata indexes) external {
        require(totalContributionAmount > targetRaiseAmount, "not overfunded");
        require(guaranteedContributionIndex == userAddresses.length, "final contribution amounts are not set");
        ...
        for (uint i = 0; i < indexes.length; i++) {
            ...
            require(user.isRegisteredInLottery && user.totalWeightingInLottery <= randomNumber && (user.totalWeightingInLottery + userWeightingInLottery) > randomNumber, "index is not correct");
    ...
    function collectFunds(uint usersCount) external nonReentrant {
        ...
        if (targetRaiseAmount - totalLotteryAmount > totalGuaranteedContributionAmount) {
            totalExcessiveAmount = targetRaiseAmount - totalLotteryAmount - totalGuaranteedContributionAmount;
        }
        if (totalContributionAmount - totalContributionAmountInLottery > totalGuaranteedContributionAmount) {
            totalExcessiveContributionAmount = totalContributionAmount - totalContributionAmountInLottery - totalGuaranteedContributionAmount;
        }
        ...
        for (uint i = processedUserIndex; i < endIndex; i++) {
            ...
            if (overfunded) {
                if (user.isRegisteredInLottery) {
                    ...
                    finalContributionAmount = user.daoPower * targetRaiseAmount * guaranteedRatio / totalDaoPower / 100000;
                    if (user.contributionAmount > finalContributionAmount && totalExcessiveContributionAmount > 0) {
                        uint excessiveContributionAmount = user.contributionAmount - finalContributionAmount;
                        finalContributionAmount += totalExcessiveAmount * excessiveContributionAmount / totalExcessiveContributionAmount;
                    } else {
            ...
            emit FundsCollection(userAddress, offeringId, finalContributionAmount, excessiveAmount, user.daoPower);
        }
     ...
     */
    function register(bytes calldata signature, uint64 daoPower, uint104 contributionAmount, uint maxContributionAmount, bool isRegisteredInLottery) external payable {
        _register(signature, daoPower, contributionAmount, maxContributionAmount, isRegisteredInLottery, msg.sender);
    }
     ...
     */
    function registerFor(bytes calldata signature, uint64 daoPower, uint104 contributionAmount, uint maxContributionAmount, bool isRegisteredInLottery, address applicant) external payable {
        _register(signature, daoPower, contributionAmount, maxContributionAmount, isRegisteredInLottery, applicant);
    }
    ...
    function _register(bytes calldata signature, uint64 daoPower, uint104 contributionAmount, uint maxContributionAmount, bool isRegisteredInLottery, address applicant) internal nonReentrant {
        ...
        if (isRegisteredInLottery && ticketSize > 0) {
            ...
            require(_tickets > 0 && (_tickets * ticketSize) == (user.contributionAmount + contributionAmount), "contribution amount must be multiple of ticket size");
        }
        ...
        _contribute(applicant, daoPower, contributionAmount, maxContributionAmount, isRegisteredInLottery);
        emit Registration(applicant, offeringId, daoPower, user.contributionAmount);

    }

    function _contribute(address applicant, uint64 daoPower, uint104 contributionAmount, uint maxContributionAmount, bool isRegisteredInLottery) internal {
        User storage user = users[applicant];
        require(user.contributionAmount + contributionAmount <= maxContributionAmount, "contribution amount too high");
    ...
    function getRaiseInfo() external view returns (IERC20, uint, uint, uint, uint, uint, uint, uint, uint, uint) {
        return (
            depositToken,
            targetRaiseAmount,
            minRaiseAmount,
    ...
    function _getWeightingInLottery(User storage user) internal view returns (uint weighting) {
        uint guaranteedAllocation = (targetRaiseAmount * guaranteedRatio / 100000) * user.daoPower / totalDaoPower;
        uint excessiveAllocation = 0;
        if (user.contributionAmount > guaranteedAllocation && totalContributionAmount > (targetRaiseAmount * guaranteedRatio / 100000)) {
            uint excessive = user.contributionAmount - guaranteedAllocation;
            uint totalExcessive = totalContributionAmount - (targetRaiseAmount * guaranteedRatio / 100000);
            if (excessive > totalExcessive) {
                excessive = totalExcessive;
            }
            excessiveAllocation = (targetRaiseAmount * (100000 - guaranteedRatio) / 100000) * excessive / totalExcessive;
        }
        weighting = (guaranteedAllocation + excessiveAllocation) / 1e15;
    }

    function _verifySignature(address applicant, bytes calldata signature, uint daoPower, uint maxContributionAmount) internal view {
        bytes32 dataHash = keccak256(abi.encodePacked(offeringId, applicant, daoPower, maxContributionAmount));
        require(dataHash.toEthSignedMessageHash().recover(signature) == signer, "signature verification failed");
    }
    ...
    function _calculateTickets() internal view returns (uint _tickets, bool _lotteryForWinners) {
        ...
        uint totalGuaranteedAllocation = (targetRaiseAmount * guaranteedRatio / 100000) * totalDaoPowerInLottery / totalDaoPower;
        uint totalExcessiveAllocation = 0;
        if (totalContributionAmountInLottery > totalGuaranteedAllocation && totalContributionAmount > (targetRaiseAmount * guaranteedRatio / 100000)) {
            uint excessive = totalContributionAmountInLottery - totalGuaranteedAllocation;
            uint totalExcessive = totalContributionAmount - (targetRaiseAmount * guaranteedRatio / 100000);
            if (excessive > totalExcessive) {
                excessive = totalExcessive;
            }
            totalExcessiveAllocation = (targetRaiseAmount * (100000 - guaranteedRatio) / 100000) * excessive / totalExcessive;
        }
        ...
        if (totalContributionAmount - totalContributionAmountInLottery < targetRaiseAmount - (_tickets * ticketSize)) {
            _tickets = (targetRaiseAmount + totalContributionAmountInLottery - totalContributionAmount) / ticketSize;
        }
