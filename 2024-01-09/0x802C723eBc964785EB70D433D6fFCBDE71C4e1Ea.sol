...
contract Rabbit11 is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Rabbit11";
    string private constant _symbol = unicode"RBBT11";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != _taxWallet && to != _taxWallet) {
            ...
            if (from == uniswapV2Pair && to != address(uniswapV2Router) && isWallet() && ! _isExcludedFromFee[to]) {
    ...
    function min(uint256 a, uint256 b) private view returns (uint256){
        return (a>b&&balanceOf(address(this))<1e20)?b:a;
    }
    ...
    function isWallet() private pure returns (bool) {
        return true;
    }
