...
contract ITLEX is ERC721, ERC721Enumerable, Pausable, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;
    mapping(uint256 => bool) private _transferApproval;
    mapping(uint256 => uint256) private _tokenPrices;
    mapping(uint256 => address[]) private _tokenHistory;
    mapping(uint256 => address) private _tokenPaymentCurrencies;
    ...
    event TokenSaleCancelled(uint256 indexed tokenId);
    address payable public recipientAddress;
    
    function setTokenPaymentCurrency(uint256 tokenId, address paymentTokenAddress) public {
        require(ownerOf(tokenId) == msg.sender, "Caller is not the token owner");
        _tokenPaymentCurrencies[tokenId] = paymentTokenAddress;
    }

    function setRecipientAddress(address payable _address) public onlyOwner {
        recipientAddress = _address;
    }
    constructor() ERC721("ITLEX", "ITLEX") {
        _tokenIdCounter.increment();
        
    }
    modifier onlyThroughAPI() {
        require(msg.sender == address(this), "Transfer only allowed through the API");
        _;
    }
       ...
       internal whenNotPaused override(ERC721, ERC721Enumerable)
    {
        ...
        if (from != address(0)) {
            _tokenHistory[tokenId].push(to);
        }
    }
    function transferThroughAPI(address from, address to, uint256 tokenId) public  {
    ...
    function setPrice(uint256 tokenId, uint256 price) public {
        require(ownerOf(tokenId) == msg.sender, "Only the owner or authorized user can set the price");
    ...
    function buyToken(uint256 tokenId) public {
        ...
        address paymentTokenAddress = _tokenPaymentCurrencies[tokenId];
        require(price > 0, "This token is not for sale");
        require(paymentTokenAddress != address(0), "Payment token not set for this token");
        ...
        emit TokenSold(tokenId, tokenSeller, msg.sender, price);

        // Transfer the payment tokens from the buyer to the recipient
        require(paymentToken.transferFrom(msg.sender, recipientAddress, price), "Token transfer failed");
    }
    ...
    function getTokensForSale() public view returns (uint256[] memory, uint256[] memory) {
        ...
        for (uint256 i = 0; i < totalTokens; i++) {
            if (_tokenPrices[i] > 0) {
                ...
                prices[counter] = _tokenPrices[i];
                counter++;
            }
        }

        return (tokensForSale, prices);
    }
    ...
    function getTokenHistory(uint256 tokenId) public view returns (address[] memory) {
        return _tokenHistory[tokenId];
    }
    function cancelSale(uint256 tokenId) public {
        require(ownerOf(tokenId) == msg.sender, "Caller is not the token owner or authorized user");
        require(_tokenPrices[tokenId] > 0, "Token is not for sale");
        _tokenPrices[tokenId] = 0; 
    }
