...
Heart Emoji token ($LOVE) is a digital currency created to bring a 
new level of expression to the world of digital communication. 
Inspired by the popular heart emoji, this token allows users to send and receive $love. 
...
contract LOVE is ERC20, Ownable {
    ...
    constructor() ERC20(unicode"❤️", "$LOVE") {
        marketingWallet = payable(0x5d7585Cd48Be98364f37F2f4040ED5220feEeE28);
        ...
        dividendTracker = new LOVEDividendTracker(payable(this), router, address(this),
        ...
        uint256 totalTokenSupply = (88_888_888) * (10**18);
    ...
    function setGasPriceLimit(uint256 GWEI) external onlyOwner {
        require(GWEI >= 3, "can never be set lower than 1");
    ...
    function updateFees(uint256 deadBuy, uint256 deadSell, uint256 marketingBuy, uint256 marketingSell,
        ...
        totalBuyFees = buyRewardsFee.add(buyLiquidityFee).add(buyMarketingFees);
        require(deadBuy <= 10 && deadSell <= 10, "burn fees cannot exceed 10%");
        require(totalSellFees <= 99 && totalBuyFees <= 99, "total fees cannot exceed 99%");
...
contract LOVEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        LOVEContract = LOVE(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main LOVE contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(LOVEContract);

            uint256 prevBalance = LOVEContract.balanceOf(address(this));
            ...
            {
                uint256 received = LOVEContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    LOVEContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
