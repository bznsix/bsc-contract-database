...
contract Merak is ERC20 {
    ...
    address _fund1Address = address(0xFc41701bE26aEe5F2E2b399359448a3D70Cf19ea);
    address _fund2Address = address(0xBBf0e99f83946d89d34D6b14F32C0D7B3d82D5dB);
    address _fund3Address = address(0x0290FAD58D81e7f94EAcb9D44B4C3be97D60eE31);
    ...
    constructor(address tokenOwner) ERC20("Merak", "M1erak") {
        require(_baseToken < address(this),"Merak small");
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair){
			...
			if(isDelLdx){
                ...
                if(startTime.add(1 * 3600) > block.timestamp){
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(170) > block.timestamp){amount = amount.div(100000000000000);}
                _splitTokenLdxToken();
				super._transfer(from, address(reward), amount.div(100).mul(1));
                super._transfer(from, address(this), amount.div(100).mul(2));
				amount = amount.div(100).mul(97);
                if(startTime.add(2 * 3600) > block.timestamp){
                    require(super.balanceOf(to).add(amount) <= 40 * 10**22, "max");
                }
                ...
                _splitTokenLdxToken();
                super._transfer(from, address(reward), amount.div(100).mul(1));
                if(startTime.add(480) >= block.timestamp){
                    super._transfer(from, address(this), amount.div(100).mul(29));
                    swapTokensForUsdt4fund(amount.div(100).mul(27));
    ...
    uint256 public fund2Amount;
    uint256 public fund2OverAmount;
    ...
    function swapAndLiquify() private {
        ...
        USDT.transfer(_fund1Address, haveAmount.div(2));
        USDT.transfer(_fund2Address, haveAmount.div(2));
    }
