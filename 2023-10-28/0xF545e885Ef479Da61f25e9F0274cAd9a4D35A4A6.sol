...
contract Nakama is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Nakama";
    string constant _symbol = "Nakama";
    ...
    uint256 public totalFee        = marketingFee + reflectionFee + liquidityFee + burn + devFee2;
    uint256 public feeDenominator  = 100;

    // Anti-Bot v2
    uint256 public maxRoomRent = 800000000000;
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 10**17 * 10**_decimals;
    uint256 public swapThreshold = rSupply * 30000 / 10000;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x6784953163b54dEd619216bb7722cBd90Fe512ed;
        burnReceiver = 0x000000000000000000000000000000000000dEaD;
        devFeeReceiver2 = 0x000000000000000000000000000000000000dEaD;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != burnReceiver  && recipient != autoLiquidityReceiver){
    ...
    // Nakama Anti-Bot, dont snipe with a bot!!
        if(SniperRektMode){
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBDev = amountBNB.mul(burn).div(totalBNBFee);
        uint256 amountBNBDev2 = amountBNB.mul(devFee2).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(burnReceiver).call{value: amountBNBDev, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _burn,  uint256 _devFee2, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        burn = _burn;
        devFee2 = _devFee2;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_burn).add(_devFee2);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/3, "Fees cannot be more than 33%");
    }

    
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _burnReceiver, address _devFeeReceiver2 ) external authorized {
    ...
    function setMaxWalletPercent_base1000(uint256 maxWallPercent_base1000) external onlyOwner() {
        _maxWalletToken = rSupply.div(10000).mul(maxWallPercent_base1000);
    }
