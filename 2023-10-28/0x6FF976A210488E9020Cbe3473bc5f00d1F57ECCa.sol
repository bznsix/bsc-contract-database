...
contract TokenSwap is Ownable {
    ...
    function swapTokens(
        ...
        address[] memory pathsIn,
        address[] memory pathsOut
    ) external noReentrancy {
        // Checks if user has enough tokens to swap
        require(
            deposits[msg.sender][tokenIn] >= swapAmount,
        ...
        if (
            IERC20(tokenIn).allowance(address(this), router1Address) <
            swapAmount
        ) {
            IERC20(tokenIn).approve(router1Address, swapAmount);
        }

        // Interactions: Perform the first token swap
        uint256[] memory amounts = IUniswapV2Router(router1Address)
        ...
        if (
            IERC20(tokenOut).allowance(address(this), router2Address) <
            amountOut
        ) {
            IERC20(tokenOut).approve(router2Address, amountOut);
        }

        // Interactions: Perform the second token swap
        uint256[] memory amounts2 = IUniswapV2Router(router2Address)
            .swapExactTokensForTokens(
                amountOut,
                amountOutFinal,
                pathsOut,
        ...
        deposits[msg.sender][tokenIn] += amountOut2;
    }
