...
abstract contract AbsToken is IERC20, Ownable {
 ...
 function isContract(address account) internal view returns (bool) {
    ...
    function _transfer(
        ...
        bool shouldInvite = (inviter[to] == address(0) && !isContract(from) && !isContract(to) && amount >= 1 * 10 ** (_decimals - 3));
        //>=0.001
        if (shouldInvite) {
            inviter[to] = from;
        }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if(inviteAmount>0 && inviter[sender]!=address(0)){
                feeAmount+=inviteAmount;
                 _takeTransfer(sender, inviter[sender], inviteAmount);
            }
            if(destroyAmount>0){
