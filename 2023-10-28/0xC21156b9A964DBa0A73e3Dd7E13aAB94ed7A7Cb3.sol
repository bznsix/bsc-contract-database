...
contract ERC20 is Context,Ownable, IERC20, IERC20Metadata {
    ...
    constructor(string memory name_, string memory symbol_) {
        ...
        defaultInviter = address(0x66Bc013C52852a0A6fd4D8875C8404673f8dCad3);
        foundation = address(0xb3749CB8E730E3D9B5200CFB817e5C1ab26090Fd);
        defaultNode = address(0x74CE70047aD1A1b33b1EA3e98DeBE6Aff8aCCb22);
        MIN_AMOUNT = 100000 * 10 ** 6;
    ...
    function signUp(address inviter) public {
    ...
    function _transfer(
        ...
        if (sendExcludeFee[sender] || receiveExcludeFee[recipient]) {
            //白名单用户正常转账，不进行10%分配，一般只有ower为白名单
            unchecked {
    ...
    function _transferIncludeFee(address sender, address recipient, uint256 amount) internal {
        ...
        uint256 temp = amount.mul(3).div(100);
        ...
        //Dao的2%
        temp = amount.mul(2).div(100);
        _balances[foundation] = _balances[foundation].add(temp);
        emit Transfer(sender, foundation, amount.mul(2).div(100));
        //节点的1%
        temp = amount.mul(1).div(100);
        _balances[defaultNode] = _balances[defaultNode].add(temp);
        emit Transfer(sender, defaultNode, amount.mul(1).div(100));
    }
...
contract TestSHEToken is ERC20 {

    mapping(address => bool) public blacklist;
    
    constructor() ERC20("Test GAMEFI WORD SHARE", "TestSHE") {
