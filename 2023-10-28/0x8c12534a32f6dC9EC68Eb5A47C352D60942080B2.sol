...
contract StrategyGalaxyHedge is StrategyBase, IERC721Receiver {
    ...
    constructor (
        ...
        want = ICompoundToken(wantMarket).underlying();
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            ICompoundToken(wantMarket).mint(wantBal);
            _updateBalance();
    ...
    function withdraw(uint256 amount) external whenNotPaused {
        ...
        ICompoundToken(wantMarket).redeemUnderlying(amount);
        uint256 wantBal = IERC20(want).balanceOf(address(this));
    ...
    function increaseOrder(OrderIncrease calldata order) 
        ...
        if (order.farming) {
            (, amount0, amount1) = IMasterChefV3(masterchefV3).increaseLiquidity(params);
        } else {
            (, amount0, amount1) = INonfungiblePositionManager(positionManager).increaseLiquidity(params);
        }
        liquidity = getLiquidity(order.tokenId);
    ...
    function harvest() 
        ...
        onlyStrategist {
            ...
            if (hedgeBalance > 0) {
                ...
                uint256 repayAmount = hedgeBorrowBalanceStored >= hedgeBalance ? hedgeBalance : hedgeBorrowBalanceStored;
                ICompoundToken(hedgeMarket).repayBorrow(repayAmount);
            ...
            if (wantBalance > 0) {
                ...
                uint256 repayAmount = wantBorrowBalanceStored >= wantBalance ? wantBalance : wantBorrowBalanceStored;
                ICompoundToken(wantMarket).repayBorrow(repayAmount);
                _updateBorrowBalanceStored(wantMarket);
                wantBalance = IERC20(want).balanceOf(address(this));
                if (wantBalance > 0) {
                    ICompoundToken(wantMarket).mint(wantBalance);
                    _updateBalance();
                }
            }
    }
    ...
    function supply(uint256 amount) external onlyManager {
        ICompoundToken(wantMarket).mint(amount);
        _updateBalance();
    }

    function redeem(uint256 shares) external onlyManager {
        ICompoundToken(wantMarket).redeem(shares);
        _updateBalance();
    }
    ...
    function _updateBalance() internal {
        balanceOfUnderlying = ICompoundToken(wantMarket).balanceOfUnderlying(address(this));
    }

    function _updateBorrowBalanceStored(address market) internal {
        if (market == wantMarket) {
            wantBorrowBalanceStored = ICompoundToken(wantMarket).borrowBalanceStored(address(this));
    ...
    function _giveAllowances() internal {
        IERC20(want).safeApprove(wantMarket, type(uint256).max);
    ...
    function _removeAllowances() internal {
        IERC20(want).safeApprove(wantMarket, 0);
