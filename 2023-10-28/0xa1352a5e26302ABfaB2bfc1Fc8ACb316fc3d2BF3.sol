...
contract LuckyMiningContract is Ownable{
    ...
    function createMiner(uint256 _releaseSeconds, uint256 _totalReleaseAmount,address _recipient) external onlyOperator{
        require(lucky.balanceOf(address(this)) >= _totalReleaseAmount, "Insufficient tokens");
        ...
        emit MinerCreated(miners[_recipient].length, _recipient, _totalReleaseAmount);
    }
    ...
    function claimMiner(uint256 minerIndex) external payable returns (uint256) {
        ...
        if (timeElapsed > 0) {
            uint256 amountPerSecond = miner.totalReleaseAmount / miner.releaseSeconds;
            uint256 claimAmount = timeElapsed * amountPerSecond;
            
            uint256 maxClaimAmount = miner.totalReleaseAmount - miner.claimedAmount;
            claimAmount = (claimAmount > maxClaimAmount) ? maxClaimAmount : claimAmount;
            require(claimAmount > 0, "Claim completed");
            require(lucky.balanceOf(address(this)) >= claimAmount, "Insufficient tokens");
    ...
    function getMinerInfo(uint256 _startIndex,address _recipient) public view returns(Miner[] memory){
