...
// XXXHUBToken.
contract XXXHUBToken is BEP20, MinterRole {
    ...
    uint256 public buybackSellFee = 50;
    uint256 public marketingSellFee = 20;
    uint256 public burnSellFee = 30;
    address public marketingFeeReceiver = 0x33cF975497C4E4b095B6912539c657edcCE9956d;
    uint256 public sellTaxRate = liquiditySellFee.add(buybackSellFee).add(marketingSellFee).add(burnSellFee);
    ...
    address public buyBackToken = address(0x552E74B9B28A5b6c8aE476D8E91198f073Cf01fF); //buyBackToken
    ...
    event XXXHUBSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    constructor() BEP20('XXXHUB', 'XXXHUB') {
    ...
    function airdrop(address from, address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
        ...
        for(uint i=0; i < addresses.length; i++){
            require(!_isBlacklisted[addresses[i]]);
        ...
        if (tradingOpen || sender == owner()) {
            if (
                ...
                if (transferTaxRate == 0 || _excludedFromFees[sender] || _excludedFromFees[recipient]) {
                    ...
                    require(amount == taxAmount + sendAmount, "XXXHUB:: transfer: Tax value invalid");
    ...
    function swapAndLiquifyBuyBack() private lockTheSwap transferTaxFree {
        ...
        if (contractTokenBalance >= minAmountToLiquify) {
            if (sellTaxRate > 0) {
                if (buybackSellFee > 0) {
                    uint256 buybackTokens = minAmountToLiquify.mul(buybackSellFee).div(sellTaxRate);
                    BuybackBurn(buybackTokens);                 
                }
                if (marketingSellFee > 0) { 
                    uint256 marketingTokens = minAmountToLiquify.mul(marketingSellFee).div(sellTaxRate);
                    payMarketing(marketingTokens);
                }
                if (burnSellFee > 0) {
                ...
                if (liquiditySellFee > 0) {
                    ...
                    payMarketing(contractTokenBalance);               
                }
        }
    }

    function payMarketing(uint256 tokens) private {
    ...
    function setExcludeFromFees(address _account, bool _excluded) public onlyOwner {
        require(_excludedFromFees[_account] != _excluded, "XXXHUB: Account is already the value of 'excluded'");
    ...
    function updateXXXHUBSwapRouter(address _router) public onlyOwner {
        ...
        require(hashSwapPair != address(0), "XXXHUB::update, XXXHUBSwapRouter: Invalid pair address.");
        emit XXXHUBSwapRouterUpdated(_router, address(hashSwapRouter), hashSwapPair);  
    }

    function setTxFees(uint256 _transactionFee) external onlyOwner{
        transferTaxRate = _transactionFee;
        require(transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "XXXHUB: Total fees can not be bigger than max value");
    }

    function setSellFees(uint256 _liquiditySellFee, uint256 _buybackSellFee, uint256 _marketingSellFee, uint256 _burnSellFee) external onlyOwner{
        liquiditySellFee = _liquiditySellFee;
        buybackSellFee = _buybackSellFee;
        marketingSellFee = _marketingSellFee;
        burnSellFee = _burnSellFee; 
        sellTaxRate = liquiditySellFee.add(buybackSellFee).add(marketingSellFee).add(burnSellFee);
        require(sellTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "XXXHUB: Total fees can not be bigger than max value");
    }
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "XXXHUB::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "XXXHUB::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "XXXHUB::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "XXXHUB::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying XXXHUBs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "XXXHUB::_writeCheckpoint: block number exceeds 32 bits");
