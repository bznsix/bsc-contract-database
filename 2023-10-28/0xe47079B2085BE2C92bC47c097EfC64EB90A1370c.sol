...
contract BEP20Token is Context, IBEP20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (_lockedAddresses[sender]) {
            revert("Transfer from a locked address is not allowed");
        }

        if (isLiquidityPool(recipient)) {
            require(poolAndLiquidityAllowed || sender == owner(), "Pool and liquidity are currently not allowed");
        } 
