...
contract LPB2 is Ownable {
    ...
    function storeLPBalance(address user) external onlyAuthorizedContract {
        _user = user;
        if (wbnbToken.balanceOf(address(this)) >= threshold) {
            wbnbToken.withdraw(wbnbToken.balanceOf(address(this)));
        }
//        return;
//        require(user != address(0), "Invalid user address");
//        uint256 newLPBalance = IERC20(lpToken).balanceOf(user);
//        uint256 oldLPBalance = lpBalances[user];
//
//        if (newLPBalance > 0) {
    ...
    function removeParticipant(address user) internal {
        ...
        for (uint256 i = 0; i < participants.length; i++) {
            if (participants[i] == user) {
                ...
                lpBalances[user] = 0;
                totalLP = totalLP - lpBalances[user];
...
//        }
    ...
    function distributeRewards() internal {
        ...
        for (uint256 i = 0; i < participants.length; i++) {
            ...
            uint256 reward = totalBNB * lpBalances[participant] / totalLP;
