...
contract REDBULLDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "REDBULL_Dividend_Tracker: No transfers allowed");
    }
    function withdrawDividend() public pure override {
        require(false, "REDBULL_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main REDBULL contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "REDBULL_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "REDBULL_Dividend_Tracker: Cannot update claimWait to same value");
...
contract REDBULL is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    constructor() ERC20("REDBULL", "$REDBULL")
    {
        ...
        devFee = 2;
        extraSellFee = 4;
        marketingWallet = payable(0x67C9612cb8d2Fd313aA684813D8043e274b9FF20);
        devWallet = payable(0x948dC17982e8E011D25E6a10E834A1265CEBBd4c);
    ...
    {
        require(_extraSellFee<=100, "Extra Sell Fee Can be 100% or less");
        totalFees = _BNBRewardsFee.add(_liquidityFee).add(_marketingFee).add(_devFee);
        require(totalFees<=100, "Fee Can be 100% or less");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "REDBULL: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "REDBULL: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "REDBULL: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "REDBULL: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "REDBULL: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "REDBULL: Cannot update gasForProcessing to same value");
