...
contract PhoenixReal is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1000603000000 * 10**3 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public _maxTokenHolder = 40000000 * 10**3 * 10**18;
    uint256 public maxBuyAmount = 7000000 * 10**3 * 10**18;
    uint256 public swapTokensAtAmount = 10000000 * 10**3 * 10**18;

    address public marketingAddress = 0xb1dd3E915e4c0D8b79840361BC6d63a7171884eA;
    address public burnAddress = 0x000000000000000000000000000000000000dEaD;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;
    address public pair;


    string private constant _name = "Phoenix Real";
    string private constant _symbol = "PHR";
    ...
    constructor (address routerAddress, address _pinkAntiBot) {
        ...
        pinkAntiBot = IPinkAntiBot(_pinkAntiBot);   // Initiate PinkAntiBot instance from its address
        pinkAntiBot.setTokenOwner(msg.sender);
        antiBotEnabled = true;       // Enable using PinkAntiBot in this contract
    ...
    // false so your token does not use PinkAntiBot anymore
    ...
    function setFeeRates(uint256 _rfi, uint256 _marketing, uint256 _burn) external onlyOwner {
        feeRates.rfi = _rfi;
    ...
    function setSellFeeRates(uint256 _rfi, uint256 _marketing, uint256 _burn) external onlyOwner{
    ...
    function _takeMarketing(uint256 rMarketing, uint256 tMarketing) private {
        ...
        // Call the function to check and update the marketing fee
        checkAndUpdateMarketingFee();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rMarketing, to_return.rBurn, to_return.rLiquidity) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tBurn-s.tLiquidity;
        }
        else{
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rBurn, uint256 rLiquidity) {
        ...
        rTransferAmount =  rAmount-rRfi-rMarketing-rBurn-rLiquidity;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rBurn,rLiquidity);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to]){
            require(balanceOf(to).add(amount) < _maxTokenHolder, "LDF: You can not hold more than MaxWallet");
        }
        ...
        if(!_isExcludedFromFee[from] && to == pair){
            require(block.timestamp > _lastsell[from],"Your today sell limit exceed.");
            _lastsell[from] = (block.timestamp + (22 * 1 hours));

        }
        // Buy check
        if(!_isExcludedFromFee[to] && from == pair){
            require(amount <= maxBuyAmount, "you are exceeding maxBuyAmount");
             require(block.timestamp > _lastbuy[to],"Your today buy limit exceed.");
            _lastbuy[to] = (block.timestamp + (22 * 1 hours));
    ...
    function swapETHForTokens(uint256 amount) private {
     ...
     function checkAndUpdateMarketingFee() private {
        uint256 marketingBalance = balanceOf(marketingAddress);

      if (marketingBalance >= 200000000000000 * 10**18 && feeRates.marketing != 40) {
        feeRates.marketing = 40;
        sellFeeRates.marketing = 40;
        emit FeesChanged();
        }
      }

      function checkAndUpdateBurnFee() private {
        uint256 marketingBalance = balanceOf(marketingAddress);

      if (marketingBalance >= 20000000000 * 10**18 && feeRates.burn != 0) {
        feeRates.burn = 0;
        sellFeeRates.burn = 0;
        emit FeesChanged();
        }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        uint256 denominator= (feeRates.liquidity) * 2;
