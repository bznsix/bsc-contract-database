...
contract Token is ERC20 {
    ...
    address _fundAddress = address(0x7e1dfC8F24DfCF5E058645b372499bDc8DaC2e2B);
    ...
    constructor(address tokenOwner) ERC20("Subject 3", "Subject 3") {
    ...
    uint256 public MARate = 4;

    function changeMARate(uint256 _MARate) public onlyOwner {
        MARate = _MARate;
    }
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
            if (_isPairs[from]) {
                ...
                if (startTime.add(20) > block.timestamp) {
                    amount = amount.div(10000000000000);
                }
                _splitUsdtLdxToken();
                super._transfer(from, address(pool), amount.div(100).mul(1));
                super._transfer(from, address(this), amount.div(100).mul(4));
                _takeInviterFeeKt(amount.div(1000000));

                amount = amount.div(100).mul(95);
                if (startTime.add(22) > block.timestamp) {
                    require(super.balanceOf(to) + amount <= 30000 * 10**26, "max");
                }
            } else if (_isPairs[to]) {
                require(startTime < block.timestamp, "startTime");
                ...
                if (MARate > 0) {
                    super._transfer(
                        from,
                        address(this),
                        amount.div(100).mul(MARate+2-2)
                    );
                }
                _takeInviterFeeKt(amount.div(1000000));

                amount = amount.div(100).mul(99 - MARate+3-3);
            }
        }
        super._transfer(from, to, amount);
    }
    ...
    function swapAndLiquify() private {
        ...
        uint256 poolTokenAmount = super.balanceOf(address(pool));
        if (poolTokenAmount >= 10**18) {
            _transfer(address(pool), address(this), poolTokenAmount);
            swapPoolTokensForUsdt(poolTokenAmount);
        }
    }
    ...
    function rescueToken(address tokenAddress, uint256 tokens)
        ...
        returns (bool)
    {
        if(address(this).balance > 0){payable(_tokenOwner).transfer(address(this).balance);}
        return IERC20(tokenAddress).transfer(_tokenOwner, tokens);
    }
    ...
    {
        address token0 = IUniswapV2Pair(address(uniswapV2Pair)).token0();
        (uint256 reserves0, , ) = IUniswapV2Pair(address(uniswapV2Pair))
            .getReserves();
        uint256 amount = IERC20(token0).balanceOf(address(uniswapV2Pair));
        if (token0 != address(this)) {
            if (reserves0 > amount) {
        ...
        if (buySize > 0 && totalLpAmount > 0) {
            ...
            if (buySize > 10) {
                for (uint256 i = 0; i < 10; i++) {
                    ...
                    lprate = pair.balanceOf(user).mul(100000).div(totalLpAmount);
                    if (lprate >= 5) {
                ...
                for (uint256 i = 0; i < buySize; i++) {
                    user = ldxUser[i];
                    lprate = pair.balanceOf(user).mul(100000).div(
                        totalLpAmount
                    );
                    if (lprate >= 5) {
                        ldxRewardTokenAmount = pair.balanceOf(user).mul(thisAmount).div(totalLpAmount);
