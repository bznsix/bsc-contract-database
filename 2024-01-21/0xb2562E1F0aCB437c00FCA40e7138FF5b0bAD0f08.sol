...
contract SnailStake1 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    IBEP20 public immutable mainToken;
    ...
    constructor(
        ...
        mainToken = IBEP20(0xE226d9879a1dda12030c3C25dd6f4Cf3dc9c2691);
        stakingToken = IBEP20(0xE226d9879a1dda12030c3C25dd6f4Cf3dc9c2691);
        rewardToken = stakingToken;

        apy = 122; // 122% rewards 122 days 
        lockDuration = 122 days;
        exitPenaltyPerc = 10;
    ...
    function updateApy(uint256 newApy) external onlyOwner {
        require(newApy <= 20000, "APY must be below 20000%");
        updatePool(0);
        apy = newApy;
    }

    function updatelockduration(uint256 newlockDuration) external onlyOwner {
        require(newlockDuration <= 21799615, "Duration must be below 256 days");
    ...
    function mainTokenRemaining() public view returns (uint256){
        return mainToken.balanceOf(address(this));
    }

    function withdrawMainToken(uint256 _amount) external onlyOwner {
        require(_amount <= rewardToken.balanceOf(address(this)), 'not enough tokens to take out');
        mainToken.safeTransfer(address(msg.sender), _amount);
    }

    function burnMainToken(uint256 _amount) external onlyOwner {
        require(_amount <= rewardToken.balanceOf(address(this)), 'not enough tokens to burn');
        mainToken.safeTransfer(address(DEAD), _amount);
    }
