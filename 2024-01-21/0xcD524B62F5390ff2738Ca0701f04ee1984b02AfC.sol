...
contract PayTokenVestingFactory is VestingFactory {
    /**
...
import "./AtlantisVestingProxy.sol";
import "./AtlantisVestingLogic.sol";
import "../test/AtlantisToken.sol";
...
contract VestingFactory is Context, Ownable, Operator {
    ...
    event vestingLogicUpdated(address indexed newVestingLogic);
    ...
    constructor(
        ...
        require(
            _vestingLogic != address(0),
            "StakingFactory: logic address is zero"  
        );

        require(
            _vestingToken != address(0),
            "StakingFactory: staking token is zero"  
        );

        require(
            _proxyAdmin != address(0),
            "StakingFactory: proxy admin address is zero"  
        );
        ...
        emit vestingLogicUpdated(_vestingLogic);
        emit ProxyAdminUpdated(_proxyAdmin);
    }

    /**
     * @dev create token sale contract
     */
    function createVesting( 
        ...
        AtlantisVestingProxy theVestingProxy = new AtlantisVestingProxy(
            vestingLogic,
            proxyAdmin,
            nullData
        );

        address theVestingAddr = address(theVestingProxy);

        AtlantisVestingLogic(theVestingAddr).initialize(
            _beneficiary,  
            vestingToken,  
            _totalVested,
            _startingDate,
        ...
        AtlantisVestingLogic(theVestingAddr).transferOwnership(_msgSender());

        AtlantisToken(vestingToken).configureMinter(
            theVestingAddr, 
            // 2X because everything will be vested to the user will be minted for sale
            2 * _totalVested
        );

        emit NewStakingCreated(
            theVestingAddr,
            _beneficiary,  
            vestingToken,  
            _totalVested,
    ...
    function updateVestingLogic(address  newVestingLogic) external onlyOwner {
        require(
            newVestingLogic != address(0),
            "StakingFactory: logic address is zero"  
        );

        vestingLogic = newVestingLogic;

        emit vestingLogicUpdated(newVestingLogic);
    }

    function updateProxyAdmin(address  newProxyAdmin) external onlyOwner {
        require(
            newProxyAdmin != address(0),
            "SaleFactory: proxy admin address is zero"  
        );
...
contract AtlantisVestingProxy is TransparentUpgradeableProxy {
...
import "vesting-contracts/contracts/tokenSale/Vesting.sol";
import "../test/AtlantisToken.sol";

contract AtlantisVestingLogic is Vesting {

    bool public isLocked;

    event VestingLocked(address indexed doer);
    event VestingUnLocked(address indexed doer);
    ...
    function lockVesting() external onlyOwner {
        ...
        emit VestingLocked(_msgSender());
    }

    function unlockVesting() external onlyOwner {
        isLocked = false;

        emit VestingUnLocked(_msgSender());
    }
    ...
    function release() external {
        ...
        uint256 releasingVesting = _release();

        AtlantisToken(vestingToken).mint(_msgSender(), releasingVesting);
        uint256[3] memory theDistribution = [uint256(10000),uint256(0),uint256(0)];
        AtlantisToken(vestingToken).mintAndDistribute(releasingVesting, theDistribution);
    }
}// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.0;
...
contract AtlantisToken is Context, ERC20, Ownable {
        ...
        returns (bool)
    {
        ...
        emit Mint(_msgSender(), _msgSender(), _amount);
    ...
    modifier onlyMasterMinter() {
        require(
            ...
            "TokenToken: caller is not masterMinter"
        );
        _;
    }
     ...
     */
    function theMasterMinter(address account) external view returns (bool) {
...
contract Vesting is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function __Vesting_init(
        ...
        require(
            ...
            "Vesting: starting date is in past"
        );
    ...
    function _release() internal returns (uint256) {
        ...
        require(
            vestedForNow >= 0, 
            "Vesting: no vested token (not started, finished, or not released yet)"
        );
