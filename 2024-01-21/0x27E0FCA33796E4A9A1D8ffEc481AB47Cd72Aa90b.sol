...
abstract contract yanToken is IERC20, Ownable { 
    ...
    constructor(
        ...
        IERC20(YANZOAddress).approve(address(swapRouter), MAX);
        ...
        address swapPair = swapFactory.createPair(address(this), YANZOAddress);
        _mainPair = swapPair;
        ...
        holderRewardCondition = 10 ** IERC20(YANZOAddress).decimals();

        _tokenDistributor = new TokenDistributor(YANZOAddress);
    }
    ...
    function setMaxAmount(uint256 _walletLimit) public onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (_swapPairList[from] || _swapPairList[to]) { 
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (block.number < startTradeBlock + 16) { 
                    _blackList[to] = true; 
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (isSell) { 
                ...
                swapFee =_buyFundFee.add(_buyLPDividendFee);
            }
 ...
 function swapTokenForFund(
        ...
        IERC20 YANZO = IERC20(_yanzo);
        uint256 yanzoBalance = YANZO.balanceOf(address(_tokenDistributor)); 
        uint256 fundAmount = (yanzoBalance * (_buyFundFee + _sellFundFee) * 2) /
            swapFee; 
        uint fundamount = fundAmount / 10; 
        uint firstfundamount = fundamount * 2; 
        uint secfundamount = fundamount * 4; 
        uint threefundamout = fundamount * 3;
        address secFundAddress = 0xF2d6369C73EA2041C2149C7390861687F64DfbAF;
        address threeFunAddress = 0x06079E1912c49e5E08158E881f78FC770b2276f4;
        // nft 1%
        YANZO.transferFrom(
            address(_tokenDistributor),
            fundAddress,
        ...
        if (lpAmount > 0) {
            uint256 lpYanzo = (yanzoBalance * lpFee) / swapFee;
            if (lpYanzo > 0) {
    ...
    function processReward(uint256 gas) private {
        ...
        uint256 balance = YANZO.balanceOf(address(this));
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (tokenBalance > 0 && !excludeHolder[shareHolder]) {
                ...
                if (amount > 0) {
                    YANZO.transfer(shareHolder, amount);
                }
            }
...
contract YQSTOKEN is yanToken {
    constructor()
        yanToken(
            ...
            18,
            1000000000,
            address(0x8971dd9777Dd62F5d0157C122D9edB148b43702b), 
            msg.sender  
        )
    {}
}