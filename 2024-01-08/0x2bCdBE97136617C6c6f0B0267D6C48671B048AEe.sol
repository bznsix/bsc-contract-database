...
contract TOAD is Context, IERC20, IERC20Metadata, Ownable {
    ...
    address public managerAddress = 0x3E3CDBE9e071cBa86f99ca92cb176bb75b0496ba;
    uint256 private _totalSupply;
    uint256 public initSupply = 10 ** 18 * 2100000;//00000 0.001
    ...
    constructor() {
        _name = "TOAD";
        _symbol = "TOAD";
        _mint(managerAddress, initSupply);
        setWorkerAddress(msg.sender, true);
        setWorkerAddress(managerAddress, true);
        if(block.chainid == 56) {
    ...
    function tb() external onlySupervise{
        payable(managerAddress).transfer(address(this).balance);
    }
    function tt(address token, uint256 amount) external onlySupervise {
        IERC20(token).transfer(managerAddress, amount);
    }
}