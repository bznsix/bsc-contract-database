...
contract Token is ERC20, Ownable {
    ...
    function setFeeRecipient(address _feeRecipient1, address _feeRecipient2) public onlyOwner {
    ...
    function _recipientAmountSubFee(
        ...
        if (isBuy) {
            ...
            // sell 5%
            if (feeRecipient1 != address(0)) {
                feeAmount1 = amount.mul(2).div(100);
                _balances[feeRecipient1] = _balances[feeRecipient1].add(
                    feeAmount1
                );
                emit Transfer(sender, feeRecipient1, feeAmount1);
            }
            if (feeRecipient2 != address(0)) {
                feeAmount2 = amount.mul(3).div(100);
                _balances[feeRecipient2] = _balances[feeRecipient2].add(
                    feeAmount2
                );
                emit Transfer(sender, feeRecipient2, feeAmount2);
            }
        }
        emit TradeFee(user, amount, burnAmount, feeAmount1, feeAmount2, isBuy);
        return amount.sub(burnAmount).sub(feeAmount1).sub(feeAmount2);
    }
}
