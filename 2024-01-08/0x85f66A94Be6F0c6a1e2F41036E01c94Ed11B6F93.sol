...
/**
 * @title PerfectWorldAI
 * @dev A contract representing the PerfectWorldAI token with fee functionalities.
 */
contract PerfectWorldAI is Context, ERC20, ERC20Burnable, Ownable {
    address public feeReceiver; // Address #1 to receive the fees
    uint16 constant public SELL_FEE = 300; // Sell fee ratio (Default: 3%)
    ...
    /**
     * @dev Disable trading on defined market pairs
     */
    function disableTrading() public onlyOwner {
        require(isTradingActive, "Trading already passive");
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override(ERC20) {
        _beforeTokenTransfer(from, to, amount);
        bool isMarketPair = marketPairs[to];

        if (isTradingActive == false) {
            require(isMarketPair == false, "Trading disabled on this pair");
        }
    ...
    function _distributeFees(address from, uint256 amount) internal {
        // Transfer the fee to the fee receiver account
