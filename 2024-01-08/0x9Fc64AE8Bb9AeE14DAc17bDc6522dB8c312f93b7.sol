...
contract PEPEMEVDividends is IPEPEMEVDividends {
    using SafeMath for uint256;
    address _token;

    address public PEPEMEV;
    ...
    constructor() {
        ...
        PEPEMEV = 0x3ba1dE83ec9b39C10A14c9ADc0AEDcA63CD29a75;
    }
    ...
    function removeStuckDividends() external onlyToken {
        uint256 balance = IERC20(PEPEMEV).balanceOf(address(this));

        IERC20(PEPEMEV).transfer(
    ...
    function deposit() public payable override {
        uint256 balanceBefore = IERC20(PEPEMEV).balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = address(PEPEMEV);

        router.swapExactETHForTokensSupportingFeeOnTransferTokens{
            value: msg.value
        }(0, path, address(this), block.timestamp);

        uint256 amount = IERC20(PEPEMEV).balanceOf(address(this)).sub(
            balanceBefore
        );
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if (amount > 0) {
            ...
            IERC20(PEPEMEV).transfer(shareholder, amount);
...
contract LibertyBTC is IERC20, Auth {
    using SafeMath for uint256;

    address public PEPEMEV = 0x3ba1dE83ec9b39C10A14c9ADc0AEDcA63CD29a75; // PEPEMEV

    string private constant _name = "Crypto is Liberty";
    string private constant _symbol = "LibertyBTC";
    ...
    PEPEMEVDividends public PEPEMEVDividend;
    uint256 public PEPEMEVDividendsGas = 0;
    ...
    constructor(address _owner) Auth(_owner) {
        ...
        PEPEMEVDividend = new PEPEMEVDividends();
    ...
    function setDistributionCriteria(
        ...
        PEPEMEVDividend.setDistributionCriteria(newMinPeriod, newMinDistribution);
    }
    ...
    function _setIsDividendExempt(address holder, bool exempt) internal {
        ...
        if (exempt) {
            PEPEMEVDividend.setShare(holder, 0);
        } else {
            PEPEMEVDividend.setShare(holder, _balances[holder]);
        }
    }
    ...
    function changePEPEMEVGas(uint256 newGas) external onlyOwner {
        PEPEMEVDividendsGas = newGas;
    }
    ...
    function _transfer(
        ...
        if (!isDividendExempt[sender]) {
            try PEPEMEVDividend.setShare(sender, _balances[sender]) {} catch {}
        }

        if (!isDividendExempt[recipient]) {
            try PEPEMEVDividend.setShare(recipient, _balances[recipient]) {} catch {}
        }
    ...
    function burnPEPEMEV(uint256 amount) private {
        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = address(PEPEMEV);
    ...
    function swapBack() internal lockTheSwap {
        ...
        try PEPEMEVDividend.deposit{value: ethForReflections}() {} catch {}
    ...
    function manualProcessGas(uint256 manualGas) external onlyOwner {
        PEPEMEVDividend.process(manualGas);
    }
    ...
    function checkPendingReflections(address shareholder)
        ...
        returns (uint256)
    {
        return PEPEMEVDividend.getUnpaidEarnings(shareholder);
    }

    function withdraw() external {
        PEPEMEVDividend.withdraw(msg.sender);
    }

    function removeStuckDividends() external onlyOwner {
        PEPEMEVDividend.removeStuckDividends();
    }
}