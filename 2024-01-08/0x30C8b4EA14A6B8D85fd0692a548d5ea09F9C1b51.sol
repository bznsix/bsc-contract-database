...
/**
 * @title SellSlqqBSC
 * @notice This contract is for the selling Slqq token in BSC.
 */

contract SellSlqqBSC is Ownable, ReentrancyGuard {
    /// ------------------------------------- LIBRARYS ------------------------------------- \\\
    ...
    event Withdrawn(address admin, uint256 usdtAmount, uint256 bnbAmount);

    /// --------------------------------- MODIFIERS ---------------------------------- \\\

    modifier isNotClosed() {
        require(!isClosed, "SellSlqqBSC: Presale is closed.");
        _;
    }
     ...
     * @dev This function is a fallback function that allows the contract to receive BNB without triggering any specific logic.
     *      It is commonly used to receive BNB sent directly to the contract address.
     */
    fallback() external {}

    /**
     * @notice Withdraws USDT and BNB from the contract.
     * @dev This function allows the owner to withdraw both USDT and BNB from the contract. It can only be called by the owner.
     */
    function withdraw() external onlyOwner {
        ...
        emit Withdrawn(msg.sender, usdtBalance, bnbBalance);
    }
    ...
    /**
     * @notice Buys SLQQ tokens with BNB.
     * @dev This function allows users to buy SLQQ tokens using BNB. It also handles referral rewards.
     * @param _referrer Referrer's address for potential referral rewards.
     * @param _axelarFee Fee to be paid to axelar for delivering message to the base chain.
     */
    function buySlqqWithBNB(
        address _referrer,
        uint256 _axelarFee
    ) external payable {
    ...
    function _buySlqq(
        ...
        require(
            ...
            "SellSlqqBSC: Can't refer yourself."
        );
        ...
        if (_withUSDT) {
            require(
                ...
                "SellSlqqBSC: Insufficiant fee."
            );
            tokenCost = _amount / 1e12;
            tokenCostDollarValue = _amount;
        } else {
            require(
                msg.value > _axelarFee,
                "SellSlqqBSC: Insufficiant native token."
            );
