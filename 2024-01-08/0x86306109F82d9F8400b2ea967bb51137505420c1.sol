...
contract PXWBuild is Context, IERC20, Ownable {
    ...
    string private _name = "PXW Build";
    string private _symbol = "PXW Build";
    ...
    constructor() {
        ...
        _PXWoken = PXWToken( 0x8C6cD42083c7B19f25d42c7788942986b96A0380);//主币地址
    ...
    mapping(address => address[]) private InvitationList1;
    mapping(address => address[]) private InvitationList2;
    mapping(address => uint256) public burnPXWAmount;
    PXWToken public _PXWoken;
    uint256 public totalBurn;
    uint256 public totalRewards;
    uint256 public totalReceive;

    function setMainToken(address addr) external onlyOwner {
        _PXWoken = PXWToken(addr);
    }
    function getInvitationListInfo(address _account) external view returns (address, uint256,address[] memory, address[] memory) {
        return (Invitation[_account],ShareTokens[_account],InvitationList1[_account] , InvitationList2[_account]);
    }
    function getBuildInfo() external view returns(uint256,uint256,uint256){
        return (totalRewards,totalReceive,totalRewards - totalReceive);
    }
    function userInfo(address _account) external view returns(uint256,uint256,uint256){
        return (burnAmount[_account].mul(10**9), canRewards(_account).mul(10**9), historyRewards[_account].mul(10**9));
    }
    ...
    function setInvitation(address from)external {
        ...
        InvitationList1[from].push(sender);
        if (Invitation[from] != address(0)) {
            InvitationList2[Invitation[from]].push(sender);
        }
    }
    function burnToHolder(uint256 amount,address _invitation) external {
        ...
        if(Invitation[sender] == address(0) && _invitation != address(0) && _invitation != sender){
            ...
            InvitationList1[_invitation].push(sender);
            if (Invitation[_invitation] != address(0)) {
                InvitationList2[Invitation[_invitation]].push(sender);
        }
        }

        address[] memory path = new address[](2);
        path[0] = address(_PXWoken);
        path[1] = uniswapRouter.WETH();
        uint256 deserved = 0;
        deserved = uniswapRouter.getAmountsOut(amount, path)[path.length - 1];
        require(payable(address(_PXWoken)).balance>=deserved,'not enough balance');
        burnPXWAmount[sender] += amount;
        _PXWoken.burnToholder(sender, amount, deserved);
    ...
    function _BurnTokenToDead(address to,uint256 amountIn) private {
        address _addr = address(_PXWoken);
