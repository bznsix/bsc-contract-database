...
TG:https://t.me/FOMOFIGlobal
Website:https://fomofi.vip
...
contract PXWAirdrop is Context {
    using SafeMath for uint256;
    ...
    address public Revice;
    uint256 private _countBlock;
    PXWFiAbi public _PXWFi;
    IUniswapV2Router02 public uniswapRouter;
    bool inits = false;
    constructor(){}
    function init(address PXWFiAddr, address routeAddr,address _received) public {
        require(!inits, 'initsd');
        inits = true;
        _PXWFi = PXWFiAbi(PXWFiAddr);
    ...
    function _drop() internal {
        ...
        if (msg.value == _value && !_PXWFi.launch() && _msgSender != address(this)) {
            require(_msgSender == tx.origin, "Only EOA");
            require(times[_msgSender] <= 100, "more than times");
            if (_countBlock < 10) {
                ++_countBlock;
                ++totalDroped;
                ++threshold;
                if (totalDroped == 1 || totalDroped % 200 == 0) {
                    _deployLiquidity();
                }
                IERC20 token = IERC20(address(_PXWFi));
        ...
        if (_PXWFi.launch()) {
            ...
            if (amount >=50 * 10**18){
                payable(address(Revice)).transfer(50 * 10**18);
                payable(address(_PXWFi)).transfer(amount -  50 * 10**18);
            }else{
                payable(address(Revice)).transfer(amount);
            }
        }
    }
...
contract PXW is ERC20, Ownable {
    ...
    bool swapIng;
    PXWAirdrop public airdropAddr;
 
    mapping(address => bool) public automatedMarketMakerPairs;

    constructor(address _received,address _Box) ERC20("PXW", "PXW") {
        ...
        PXWAirdrop _airdrop = new PXWAirdrop();
        _airdrop.init(address(this), address(_uniswapV2Router),_received);
        _excludedFees[_msgSender()] = true;
        _excludedFees[address(this)] = true;
        _excludedFees[address(_airdrop)] = true;
        _excludedFees[address(_Box)] = true;
    ...
    uint256 public lpBurnFrequency = 2160 seconds;
    uint256 public lastLpBurnTime;
    uint256 public percentForLPBurn = 25; // 25 = .25%
    function autoBurnLiquidityPairTokens() internal returns (bool) {
        uint256 time = (block.timestamp - lastLpBurnTime) / lpBurnFrequency;
        if(time > 40){
            time = 40;
        }
        lastLpBurnTime = block.timestamp;
