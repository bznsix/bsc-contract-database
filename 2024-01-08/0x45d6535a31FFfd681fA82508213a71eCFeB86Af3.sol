...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    function _tokensOfOwner(address owner,uint256 start, uint256 end) internal view returns (uint256[] memory) {
        ...
        require(start <= end && end < tokenCount,"Start, end is not ok");
        
        mapping(uint256 => uint256) storage all = _ownedTokens[owner];
    ...
    function _getAllToken(uint256 start, uint256 end) internal view returns (uint256[] memory _tokenID, address[] memory _Address) {
        ...
        require(start <= end && end < tokenCount,"Start, end is not ok");
        uint256[] memory TokenList = new uint256[](end - start + 1);
        address[] memory AddressRes = new address[](end - start + 1);
        for (uint256 i = start; i <= end; i++) {
            TokenList[i-start] = _allTokens[i];
            AddressRes[i-start] = ownerOf(_allTokens[i]);
        }
        return (TokenList,AddressRes);
    }
...
contract XDRAGON_NFT is ERC721Enumerable {
    
    address private admin;

    event changeadmin(address admin);
    event mintNew(address to,uint256 tokenId);
    string public baseURL = "";

    constructor() ERC721("XDRAGON_NFT", "XDRAGON_NFT") {
