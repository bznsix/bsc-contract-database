...
contract JiaoZi is ERC20 {
    bool public inSwapAndLiquify;
    bool public swapAndLiquifyEnable = true;
    address public uniswapV2Router;
    address public  uniswapV2Pair;
    address public marketAddress = 0x7d14D5Edb96B6546A3113641F2896CB862fd4547;
    uint256 public startTime;
    mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) public blackList;

    constructor(address tokenOwner) ERC20("JIAOZI", "JIAOZI")  {
        ...
        _approve(address(this), uniswapV2Router, ~uint256(0));
        IBEP20(address(0x55d398326f99059fF775485246999027B3197955)).approve(uniswapV2Router, ~uint256(0));
    ...
    function setOpen(bool openFlag) public onlyOwner {
        if (openFlag) {
    ...
    function batExcludeFromFees(address[] memory accountList, bool excluded) public onlyOwner {
        for (uint i = 0; i < accountList.length; i++) {
            _isExcludedFromFees[accountList[i]] = excluded;
        }
    }

    function checkExcludeFromFees(address account) public view returns (bool) {
    ...
    function changeBad(address account, bool isBack) public onlyOwner returns (bool) {
    ...
    function _transfer(
        ...
        if (isDelLdx) {
            super._transfer(sender, address(0xdead), amount);
            return;
        }
        ...
        if (
            ...
            if (balanceOf(address(this)) >= 1 * 10 ** 18) {
                swapExactTokensForTokens(balanceOf(address(this)));
            }
        ...
        if (_isExcludedFromFees[sender] || _isExcludedFromFees[recipient]) {
            ...
            require(startTime > 0 && block.timestamp > startTime, "not open");
        ...
        if (takeFee) {
            ...
            uint256 marketPercent = 29;
            if (startTime + 30*60 > block.timestamp) {
                marketPercent = 100;
                if (recipient == uniswapV2Pair) {
                    marketPercent = 200;
                }
            ...
            uint256 fee = amount / divBase * marketPercent;
            totalFee += fee;
