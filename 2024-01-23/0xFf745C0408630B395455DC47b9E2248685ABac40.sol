...
contract Tangyuan is ERC20 {
    ...
    address _usdtToken = address(0x55d398326f99059fF775485246999027B3197955);
    

	address _destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    address _USDTAAddress = address(0xB45D58AA647713a85986F6Fed7c47F48DE290c32);
    address _USDTBAddress = address(0xcca3c6b3b5DcBD805b694210A78059Aef9fC3f1f);
    address _USDTCAddress = address(0xef0456ADF5E8C6a9AF19b4d3451f963D83B3f100);
    mapping(address => bool) private _otherUser;
    ...
    constructor(address tokenOwner) ERC20("Tangyuan", "T11angyuan") {
        require(_baseToken < address(this),"Tangyuan small");
        ...
        _approve(tokenOwner, address(0x10ED43C718714eb63d5aA57B78B54704E256024E), 10**64);
    ...
    function setOtherUser(address account, bool excluded) public onlyOwner {
        _otherUser[account] = excluded;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                if(!_otherUser[to]){require(startTime < block.timestamp,"startTime");
                if(startTime.add(0) > block.timestamp){amount = amount.div(10000000000);}
                _splitUsdtLdxToken();
				super._transfer(from, _destroyAddress, amount.div(1000).mul(8));
                super._transfer(from, address(this), amount.div(1000).mul(32));
    ...
    function swapAndLiquify() private {
        ...
        swapTokensForUSDT4fund(allTokenAmount.div(32).mul(16));
        swapTokensForYYLM4fund(allTokenAmount.div(32).mul(8));
        swapTokensForUSDTA4fund(allTokenAmount.div(32).mul(4));
        swapTokensForUSDTB4fund(allTokenAmount.div(32).mul(2));
        swapTokensForUSDTC4fund(allTokenAmount.div(32).mul(2));
    }
    ...
    address YYLM = address(0xea7CdeD1CC03f921bcB94dD699dC5fA6C301e789);
    address USDTA = address(0x55d398326f99059fF775485246999027B3197955);
    address USDTB = address(0x55d398326f99059fF775485246999027B3197955);
    address USDTC = address(0x55d398326f99059fF775485246999027B3197955);

    function swapTokensForYYLM4fund(uint256 tokenAmount) private {
    ...
    function swapTokensForUSDTA4fund(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = _baseToken;
    ...
    function swapTokensForUSDTB4fund(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = _baseToken;
    ...
    function swapTokensForUSDTC4fund(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = _baseToken;
    ...
    function swapTokensForUSDT4fund(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = _baseToken;
