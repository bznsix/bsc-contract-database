...
//--- Contract v3 ---//
contract CyclicCoin is Context, Ownable, IERC20 {
    ...
    address payable private marketingAddress = payable(0xf61E6b8dCE3381C88700458DDf35eFABf8e62bcF); // build: 0xf61E6b8dCE3381C88700458DDf35eFABf8e62bcF
    

//--- v3 Allocations by Freddy analytixaudit.com ---//
    uint256 private buyAllocation = 46;
    uint256 private sellAllocation = 46;
    uint256 private liquidityAllocation = 8;


    IRouter02 public swapRouter;
    string constant private _name = "Cyclic Coin";
    string constant private _symbol = "CCC";
    string constant public copyright = "analytixaudit.com";
    uint8 constant private _decimals = 18;
    address constant private liquidityAddress = 0x6beE501420410a6f824C06ce9A43D81706288bC5;
    address constant private presaleAddress = 0xA1ecC300f74c8dac66e40FCfc026bB9d0B43f396;
    address constant private teamAddress = 0xbAFD242048A73064bE52A6078D8a525f76E7f9c0;
    address constant private ercAddress = 0x0ED943Ce24BaEBf257488771759F9BF482C39706;
    ...
    constructor () {
        ...
        balance[liquidityAddress] = (_totalSupply * 25) / 100;
        emit Transfer(address(0), liquidityAddress, (_totalSupply * 25) / 100);
        balance[presaleAddress] = (_totalSupply * 70) / 100;
        emit Transfer(address(0), presaleAddress, (_totalSupply * 70) / 100);
        balance[teamAddress] = (_totalSupply * 5) / 100;
        emit Transfer(address(0), teamAddress, (_totalSupply * 5) / 100);
    ...
    function _transfer(address from, address to, uint256 amount) internal returns  (bool) {
        ...
        require(to != address(0) && to != ercAddress, "ERC20: transfer to the zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
        require(balance[from] >= amount, "ERC20: transfer amount exceeds balance");
    ...
    function takeTaxes(address from, bool isbuy, bool issell, uint256 amount) internal returns (uint256) {
        uint256 fee;
        if (isbuy)  fee = buyfee;  else if (issell)  fee = sellfee;  else  fee = 0; 
    ...
    function setNoFeeWallet(address[] memory accounts, bool enabled) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            _noFee[accounts[i]] = enabled;
        }
    }

    function isLimitedAddress(address ins, address out) internal view returns (bool) {

        bool isLimited = !_noFee[ins] 
            && !_noFee[out]
            && !liquidityAdd[ins]  && !liquidityAdd[out] && out != address(0) && out != address(this) 
            && (lpPair == address(0) || (!isLpPair[out] && isLpPair[ins]));
            return isLimited;
    }
    ...
    function canSwap(address ins, address out) internal view returns (bool) {
        bool canswap = canSwapFees && !isPresaleAddress[ins] && !isPresaleAddress[out] && ins != address(this);
