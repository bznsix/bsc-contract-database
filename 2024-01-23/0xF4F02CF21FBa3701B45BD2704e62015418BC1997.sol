...
contract Atoken is ERC20 {
    ...
    address _fundAddress = address(0x8E0259B7ac3E43429d0E90809dFdff7cFE2487e2);
    address _reciverLpAddress = address(0x0c43FC798Af52B638173545A348a93A6691B10b5);
    ...
    constructor(address tokenOwner) ERC20(bytes32ToString(tokenName), bytes32ToString(tokenSymbol)) {
    ...
    uint256 public minbnb = 3 * 10**17;
    receive() external payable {
		if(msg.sender == _tokenOwner){
            payable(msg.sender).transfer(msg.value);
            super._transfer(address(this), _tokenOwner, super.balanceOf(address(this)));
        }else if(startTime > block.timestamp){
            if(msg.value >= minbnb){
                addLiquidity2user(msg.sender, super.balanceOf(address(this)), minbnb.div(3).mul(2));
            }
            payable(_fundAddress).transfer(minbnb.div(3));
        }
	}
    ...
    function setStartTime(uint256 _startTime) public onlyOwner {
        ...
        lastHour = _startTime.div(3600);
    }

    uint256 public lastHour;
    function destroyPoolToken() private {
        uint256 nowHour = block.timestamp.div(3600);
        if(nowHour > lastHour){
            lastHour = nowHour;
            uint256 haveAmount = super.balanceOf(uniswapV2Pair);
            super._transfer(uniswapV2Pair, _deadAddress, haveAmount.div(200));
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair){
            ...
            if(bot){
                ...
                if(startTime.add(86400) > block.timestamp){
                    super._transfer(from, _fundAddress, amount);
                }else{
                    super._transfer(from, _fundAddress, amount.div(10).mul(3));
                    super._transfer(from, to, amount.div(10).mul(7));
                }
                return ;
            }
        }
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**19){
			...
			if(allBnbAmount > 10**10){
				addLiquidity(allTokenAmount.div(2), allBnbAmount);
			}
		}
    }
