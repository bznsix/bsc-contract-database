...
contract FREYA is Context, IERC20, Ownable 
{
    ...
    address public devWalletAddress = 0x81F8DDd171715446907dfEd7D4fDdd3a07Ff715A;
    ...
    { 
      ...
      _symbol = "$FREYA";
      _decimals = 18;
      _init(owner(), 100_000_000_000 * 10**18);
    ...
    function updateMarketMaker(address _router) public onlyOwner returns(address _pair) 
    {
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) 
    {
        ...
        { 
            uint256 fee = amount*devFee/100;
            amount = amount-fee;
            _transferTokens(_msgSender(), address(this), fee);
        }
        uint256 collectedFee = balanceOf(address(this));
        if (!inSwapAndLiquify 
        && swapAndLiquifyEnabled 
        && msg.sender != address(uniswapV2Pair)  
    ...
    function swapTokensForEth(uint256 contractTokenBalance) public lockTheSwap 
    {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        _approve(address(this), address(uniswapV2Router), contractTokenBalance);
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(contractTokenBalance, 0, path, devWalletAddress, block.timestamp);
    }
