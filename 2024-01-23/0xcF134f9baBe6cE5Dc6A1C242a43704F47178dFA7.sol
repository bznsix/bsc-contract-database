...
// File: contracts/BSCTRLKTaxable.sol
...
contract TRLKToken is ERC20, Ownable {
    ...
    constructor() Ownable(msg.sender) ERC20("Trust Linker", "TRLK") {
    ...
    function setPoolAddress(address _newPoolAddress) external onlyOwner {
        ...
        emit PoolAddressChanged(_newPoolAddress);
    }

    function setTaxPercentage(uint256 _newTaxPercentage) external onlyOwner {
        require(_newTaxPercentage <= 5, "Invalid tax percentage");
        taxPercentage = _newTaxPercentage;
        emit TaxPercentageChanged(_newTaxPercentage);
    }
    ...
    function addToWhitelist(address _address) external onlyOwner {
        taxWhitelist[_address] = true;
        emit AddressAddedToWhitelist(_address);
    }

    function removeFromWhitelist(address _address) external onlyOwner {
        taxWhitelist[_address] = false;
    ...
    function _transfer(
        ...
        bool isTaxableTrade = (sender == poolAddress ||
            recipient == poolAddress) &&
            !taxWhitelist[sender] &&
            !taxWhitelist[recipient];
        require(tradeEnabled || taxWhitelist[sender] || taxWhitelist[recipient], "Trading is not enabled yet");

        if (isTaxableTrade && taxPercentage > 0) {
            // Apply taxes only on trades to the pool address for non-whitelisted addresses
