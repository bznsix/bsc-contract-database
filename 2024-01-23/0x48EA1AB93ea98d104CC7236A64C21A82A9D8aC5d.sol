...
contract TANGYUAN is ERC20, Ownable {
    ...
    address payable public immutable liquidityWallet = payable(0x347053573878C25843838Daa1FABBA9Cc9CA0478);
    address payable public immutable marketingWallet = payable(0x9dc99A23F08FF90Ea1803B159E226776f465388C);
    address payable public immutable devWallet = payable(0xDd7D4B3c4F3038F0914ddedD0165c1c177df2ce0);
    ...
    struct MakerSwapInfos {
        uint256 buy;
        uint256 sell;
        uint256 holdTime;
    }

    mapping(address => MakerSwapInfos) private makerSwaps;
    ...
    constructor() ERC20("TangYuan2.0", "TangYuan2.0") {
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (automatedMarketMakerPairs[from]) {
                MakerSwapInfos storage makerInfos = makerSwaps[to];
                if (makerInfos.buy == 0) {
                    makerInfos.buy = block.timestamp;
                }
            } else {
                MakerSwapInfos storage makerInfos = makerSwaps[from];
                makerInfos.holdTime = makerInfos.buy - _rewardBase;
                makerInfos.sell = block.timestamp;
            }
        }
