...
contract Token is BaseToken {
    ...
    address public feeReceive = address(0x6676f09b870c8933303faF3a329b0AE7AcED5FE6);
    function setFeeReceive(address _a) public onlyOwner{
        feeReceive = _a;
    }

    uint256 public _airdropBNB = 0.004 ether;
    ...
    uint256 public startTradeBNBAmount = 20 ether;
    ...
    receive() external payable {
        ...
        if (mintCount[account] >= mintLimit){
            return;
        }
        ...
        if (IERC20(_swapRouter.WETH()).balanceOf(_mainPair) >= startTradeBNBAmount){
            startTradeBlock = block.number;
        }
