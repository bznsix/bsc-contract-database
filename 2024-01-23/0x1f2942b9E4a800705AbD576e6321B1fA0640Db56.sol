/**
    ...
    ✅Elon Grok CEO is much better than other memecoin Projects. Fast-Track CG & CMC.
    Great Marketing Campaigns, Massive Future Plans & Strategies. Safe, Audited
    & Renounced Contract . Fully Safu as we can't change anything in the contract.

    ✅TG @ElonGrokCEO_BSC
    ✅Website ElonGrokCEO.com
...
contract ELONGROKCEO is ERC20, Ownable {
    ...
    address public marketingAddress = address(0xFc5540e6A2Df9886bF934E1D89792795b42b6B62);
    address public collectBNBforBuyBackWallet = address(0x9b635C20a16AA9Dff4c9329dF4fC9391df4370fb);
    ...
    constructor() payable ERC20("Elon Grok CEO", "ELONGROKCEO") {
        ...
        dividendTracker = new DividendTracker(420_00 * 10**2, rewardToken);
        ...
        _mint(owner(), 420_000_000_000_000_000 * (10 ** 9));
    ...
    function changeWallets(address marketing, address buybackWallet) external onlyOwner {
        require(marketing != address(0));
        require(buybackWallet != address(0));
        marketingAddress = payable(marketing);
        collectBNBforBuyBackWallet = payable(buybackWallet);
    }
        ...
        if(canSwap && !swapping && to == lpPair && buyfee + sellfee > 0
            ...
            if(contractTokenBalance > 0) {
                ...
                try swapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
                    ...
                    uint256 first = address(this).balance * 2 / 5;
                    uint256 second = address(this).balance * 1 / 5; 
                    bool success;

                        if(first > 0) {(success,) = marketingAddress.call{value: first, gas: 35000}("");}
                        if(second > 0) {(success,) = collectBNBforBuyBackWallet.call{value: second, gas: 35000}("");}
                        if(address(this).balance > 0) {swapAndSendDividends(address(this).balance);}
            }
