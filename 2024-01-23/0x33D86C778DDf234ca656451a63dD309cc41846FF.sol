...
contract GTTStakingLockup is Ownable, ReentrancyGuard {
    ...
    uint256 public duration = 36500; // 365 days
     ...
     */
    function initialize(IERC20 _stakingToken, IERC20 _earnedToken) external {
        ...
        lockups.push(Lockup(0, 7, 0, 0, 4756468800000, 0, 0, 0)); // 1% liquidity
        lockups.push(Lockup(1, 500, 0, 0, 9512937600000, 0, 0, 0)); // 1% liquidity
        lockups.push(Lockup(2, 800, 0, 0, 14269406400000, 0, 0, 0)); // 1% liquidity
        lockups.push(Lockup(3, 1200, 0, 0, 19025875200000, 0, 0, 0)); // 1.5% liquidity
    ...
    function removeFromWhitelist(address[] calldata addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            require(addresses[i] != address(0), "Cannot add null address");
            require(whitelisted[addresses[i]], "User not whitelisted");
    ...
    function deposit(uint256 _amount, uint8 _stakeType) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            lockup.totalStaked = lockup.totalStaked.add(_amount);
            totalStaked = totalStaked.add(_amount);
        }

        if (lockup.depositFee > 0) {
            uint256 fee = _amount.mul(lockup.depositFee).div(10000);
            if (fee > 0) {
    ...
    function withdraw(uint256 _amount, uint8 _stakeType) external nonReentrant {
        ...
        if (realAmount > 0) {
            ...
            lockup.totalStaked = lockup.totalStaked.sub(realAmount);
            totalStaked = totalStaked.sub(realAmount);
        }

        emit Withdraw(msg.sender, _stakeType, _amount);
    }
    ...
    function pendingReward(address _account, uint8 _stakeType) external view returns (uint256) {
        ...
        if (block.number > lockup.lastRewardBlock && lockup.totalStaked != 0) {
            ...
            adjustedTokenPerShare = lockup.accTokenPerShare.add(reward.mul(PRECISION_FACTOR).div(lockup.totalStaked));
        }
        ...
        for (uint256 i = 0; i < stakes.length; i++) {
            ...
            pending = pending.add(stake.amount.mul(adjustedTokenPerShare).div(PRECISION_FACTOR).sub(stake.rewardDebt));
        }
        return pending;
    }
    ...
    function updateRewardToken(IERC20 _earnedToken) external onlyOwner {
        earnedToken = _earnedToken;
    }
    ...
    function _updatePool(uint8 _stakeType) internal {
        ...
        lockup.accTokenPerShare = lockup.accTokenPerShare.add(_reward.mul(PRECISION_FACTOR).div(lockup.totalStaked));
        lockup.lastRewardBlock = block.number;
    }
