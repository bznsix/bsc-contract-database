...
contract Maintenance is Ownable, ERC721Holder {
    ...
    mapping(uint256 => uint256) totalTimeRepair;
    mapping(uint256 => StructData.ListMaintenance) listTimeNftMaintenance;
    mapping(uint16 => uint256) priceRepair;

    event NftRepaired(uint256 nft, uint256 _time);

    constructor(address _nft, address _token, address _oracle) {
        nft = _nft;
        token = _token;
    ...
    function initPriceRepairNft() internal {
        priceRepair[1] = 3000;
        priceRepair[2] = 917;
        priceRepair[3] = 417;
        priceRepair[4] = 225;
    ...
    function setTierPriceUsdRepair(uint16 _tier, uint256 _price) external onlyOwner {
        priceRepair[_tier] = _price;
    }

    function getTierUsdPercent(uint16 _tier) external view returns (uint256) {
        return priceRepair[_tier];
    }

    function setSaleWalletAddress(address _saleAddress) external onlyOwner {
        require(_saleAddress != address(0), "MAINTENANCE: INVALID SALE ADDRESS");
    ...
    function getFeeMaintainNft(uint256 _nftId) public view validId(_nftId) returns (uint256) {
        ...
        uint16 _tierNft = NovaXNFT(nft).getNftTier(_nftId);
        uint256 _fee =  priceRepair[_tierNft];
        uint256 feeToMaintain = (_fee * (10 ** NovaXERC20(token).decimals())) / 100;
    ...
    function getTotalTimeRepair(uint256 _nftId) external view validId(_nftId) returns (uint256) {
        ...
        totalTime = totalTime + totalTimeRepair[_nftId];
        return totalTime;
    }
    ...
    function updateDataByAdmin(uint256 _nftId, uint256 _lastTimeRepair, uint256 _totalTimeRepair) external onlyOwner {
        lastRepair[_nftId] = _lastTimeRepair;
        totalTimeRepair[_nftId] = _totalTimeRepair;
    }
...
contract Staking is IStaking, Ownable, ERC721Holder {
    ...
    // for network stats
    mapping(address => uint256) private totalTeamInvestment;
    mapping(address => uint256) private stakingCommissionEarned;
    mapping(address => uint256) private refStakingValue;
    mapping(address => uint256) private teamStakingValue;
    mapping(address => uint256) public amountStaked;
    mapping(address => uint256[]) private userStakingNfts; // nft user staking
     ...
     */
    function getTimeClaimEarn() external view override returns (uint256) {
    ...
    function earnableForStakeWithDecimal(uint256 _nftId) public view override returns (uint256) {
        ...
        if (startBuyTime > 0) {
            endEarnableTime = startBuyTime + 2592000 * timeClaimEarn;
        } else {
            endEarnableTime = stakeInfo.unlockTime - 2592000 * (stakingPeriod - timeClaimEarn);
        }
    ...
    function setMarketCommission(address _currentRef, uint256 _commissionUsdWithDecimal) external override {
        require(msg.sender == earnContract || msg.sender == stakeTokenContract, "STAKING: SENDER NOT EARN CONTRACT");
...
contract NovaXToken is NovaXERC20, NovaXERC20Burnable {
    constructor(address taxAddress_, uint16 taxBuyFee_) NovaXERC20("NovaX", "NOVAX", taxAddress_, taxBuyFee_) {
