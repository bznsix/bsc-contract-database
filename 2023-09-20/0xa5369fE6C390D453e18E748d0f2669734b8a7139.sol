...
contract Staking is IStaking, Ownable, ERC721Holder {
    ...
    uint256 public stakingLimit = 5;
    uint private unlocked = 1;
    uint256 private timeClaimEarn = 5;
    uint256 private usdToToken = 3000;
    ...
    modifier isOwnerNft(uint256 _nftId) {
        require(NovaXNFT(nft).ownerOf(_nftId) == msg.sender, "STAKING: NOT OWNER THIS NFT ID");
        _;
    }
    ...
    /**
     * @dev set time claim token
     */
    function setTimeClaimEarn(uint256 _timeClaimEarn) external override onlyOwner {
        timeClaimEarn = _timeClaimEarn;
    }

    /**
     * @dev get time claim token
     */
    function getTimeClaimEarn() external override view returns (uint256) {
        return timeClaimEarn;
    }
    ...
    function setUsdToToken(uint256 _usdToToken) external override onlyOwner {
        usdToToken = _usdToToken;
    }
    ...
    function getUsdToToken() external view override returns (uint256) {
        return usdToToken;
    }
    ...
    function stake(
        ...
        require(NovaXNFT(nft).isApprovedForAll(msg.sender, address(this)), "STAKING: MUST APPROVE FIRST");
        ...
        require(NovaXNFT(nft).ownerOf(_nftId) == msg.sender, "STAKING: NOT OWNER THIS NFT ID");
        try NovaXNFT(nft).safeTransferFrom(msg.sender, address(this), _nftId, _data) {
            // Emit event
        ...
        updateCrewInvestmentData(currentRef, totalAmountStakeUsdDecimal);
        //update stat
    ...
    function claim(uint256 _nftId) public override lock() {
        ...
        if (claimableRewardInTokenWithDecimal > 0) {
            require(
                IERC20(novaxToken).balanceOf(address(this)) >= claimableRewardInTokenWithDecimal,
                "STAKING: NOT ENOUGH TOKEN BALANCE TO PAY REWARD"
            );
            require(
                IERC20(novaxToken).transfer(msg.sender, claimableRewardInTokenWithDecimal),
    ...
    function payCommissionMultiLevels(
        ...
        while (currentRef != address(0) && index < 8) {
            ...
            if (totalStakeAmount) {
                ...
                uint256 _commissionInUsdWithDecimal = totalCommissionInTokenDecimal * tokenDecimal / Oracle(oracleContract).convertUsdBalanceDecimalToTokenDecimal(
                    tokenDecimal
                );
                ...
                if (_commissionCanEarnUsdWithDecimal > 0) {
                    totalCommissionInTokenDecimal = Oracle(oracleContract)
                    ...
                    stakingCommissionEarned[currentRef] = stakingCommissionEarned[currentRef] + totalCommissionInTokenDecimal;
                }
            }
            index++;
    ...
    function earnableForStakeWithDecimal(uint256 _nftId) public view override returns (uint256) {
        StructData.StakedNFT memory stakeInfo = stakedNFTs[_nftId];
        uint256 startBuyTime = NovaXNFT(nft).getBuyTime(_nftId);
        uint256 endEarnableTime = startBuyTime + 2592000 * timeClaimEarn;
        if (stakeInfo.lastClaimedTime >= endEarnableTime) {// 6 * 30 * 24 * 3600
            return 0;
        }
        ...
        uint32 tier = NovaXNFT(nft).getNftTier(_nftId);
        ...
        for (uint i = 0; i < index; i++) {
            ...
            if (startTime <= stakeInfo.lastClaimedTime && (endTime > stakeInfo.lastClaimedTime || endTime == 0)) {
                ...
                totalTokenCanEarnDecimal += (endTime - stakeInfo.lastClaimedTime) * tokenEarn * tokenDecimal / 86400 / 10000;
            }
        }
    ...
    function estimateValueUsdForStakeNft(uint256 _nftId) public view returns (uint256) {
        return NovaXNFT(nft).getNftPriceUsd(_nftId);
    }
    ...
    function claimableForStakeInTokenWithDecimal(uint256 _nftId) public view override returns (uint256) {
        ...
        uint256 timeDuration = block.timestamp < stakeInfo.unlockTime ? block.timestamp : stakeInfo.unlockTime;
        uint256 timeBroken = Maintenance(maintenanceContract).getTotalTimeStakeBroken(_nftId, stakeInfo.lastClaimedTime);
        uint32 tier = NovaXNFT(nft).getNftTier(_nftId);
        uint256 index = ITier(tierContract).getMaxIndex(tier);
        uint256 tokenEarn = 0;
        for (uint i = 0; i < index; i++) {
            ...
            if (startTime <= stakeInfo.lastClaimedTime && (endTime > stakeInfo.lastClaimedTime || endTime == 0)) {
                endTime = endTime > 0 ? endTime : timeDuration;
                totalTokenClaimDecimal += (endTime - stakeInfo.lastClaimedTime) * tokenEarn * tokenDecimal / 86400 / 10000;
    ...
    function unstake(uint256 _nftId, bytes memory _data) external override lock() {
        ...
        try NovaXNFT(nft).safeTransferFrom(address(this), staker, _nftId, _data) {
            // Emit event
        ...
        require(
            IERC20(novaxToken).transfer(staker, claimableRewardInTokenWithDecimal),
            "STAKING: UNABLE TO TRANSFER COMMISSION PAYMENT TO RECIPIENT"
        );
        uint256 earnableCommissionRewardTokenDecimal = earnableForStakeWithDecimal(_nftId);
        if (earnableCommissionRewardTokenDecimal > claimableRewardInTokenWithDecimal) {
    ...
    function transferNftEmergency(address _receiver, uint256 _nftId) public override onlyOwner {
        require(NovaXNFT(nft).ownerOf(_nftId) == address(this), "STAKING: NOT OWNER OF THIS NFT");
    ...
    function setMarketCommission(address _currentRef, uint256 _commissionUsdWithDecimal) external override {
        require(msg.sender == earnContract, "STAKING: SENDER NOT EARN CONTRACT");
        IMarketplace(marketplaceContract).updateCommissionStakeValueData(
            _currentRef,
    ...
    function getClaimableProfitF1(address _user, uint8 floor, uint8 maxFloor, uint256 totalClaimableUsdWithDecimal) internal view returns (uint256) {
        ...
        for (uint index = 0; index < f1s.length; index++) {
            ...
            for (uint i = 0; i < nfts.length; i++) {
                uint256 earnableCommissionRewardUsdDecimal = claimableForStakeInTokenWithDecimal(nfts[i]);
...
interface ITier {
    ...
    function setNftTierEarnPerDay(uint32 _nftTier, uint16 _nftTierEarnPerDay) external;
