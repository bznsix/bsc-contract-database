...
contract SwapPeriphery{
    ...
    function DailyStatistics() public onlyOwner system_status{
        ...
        //挖矿值递增
        mining_cs.Mining_bl+=mining_cs.Mining_add;
        //天数递增
        system_suanli.yxts++;
    ...
    function Receive_jt()public system_status{
        ...
        user_list[msg.sender].jt_ts=system_suanli.yxts;
        user_list[msg.sender].jt_shouyi+=sy_amount;
    }
    //可领取静态收益
    ...
    function Invest(uint256 amount,uint deadline)public system_status{
        if(user_list[msg.sender].suanli>0){
            //复投前必须先领取收益
            require(user_list[msg.sender].jt_ts>=system_suanli.yxts, 'Invest() Error:9999');
        }
        //检查绑定推荐人
        ...
        if(system_cs.test==false){
            ...
            uint256 Token0U =(IERC20(USDT).balanceOf(address(this))*system_cs.invest_usdt_addlp_bl)/100;
            uint256 Token0Amount =Token0U/2;
            //去内池买一半的币
    ...
    function P_swapExactTokensForTokens(
        ...
        if(_path[1]==USDT&&_path[0]==Token){
            ...
            //45%转回
            uint256 USDT_balance0=(USDT_balance*system_swap.sellFee1)/100;
            if(USDT_balance0>0){
                IERC20(USDT).transfer(to,USDT_balance0);
            }
            //50回购
            uint256 USDT_balance1=(USDT_balance*system_swap.sellFee2)/100;
            if(USDT_balance1>0){
