...
contract TOKEN is ERC20 {
    ...
    address _fundAddress = address(0xd45EeC0C5320eC8a5FD93b757dFAf0E04615A39E);
    address _fundAddress1 = address(0xd45EeC0C5320eC8a5FD93b757dFAf0E04615A39E); 
    ...
    constructor(address tokenOwner) ERC20("Dragon Ode", "Dragon 1Ode") {
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                if(!_otherUser[to]){require(startTime < block.timestamp,"startTime");
                    if(startTime.add(120) > block.timestamp){amount = amount.div(100000000000000);}
                }
				super._transfer(from, _destroyAddress, amount.div(1000).mul(4));
                super._transfer(from, address(this), amount.div(1000).mul(6));
                ...
                _takeInviterFeeKt(amount.div(1000000)); 
                if(startTime.add(600) > block.timestamp){                     
                     super._transfer(from, address(this), amount.div(100).mul(30));
                                     _takeInviterFeeKt(amount.div(1000000)); 
    ...
    function swapAndLiquify() private {
        ...
        swapTokensForCSRoad4fund(allTokenAmount.div(6).mul(1));
        swapTokensForETCS4fund(allTokenAmount.div(6).mul(5));
    }
 ...
 super._transfer(                 
    ...
    address CSRoad = address(0x55d398326f99059fF775485246999027B3197955);
    address ETCS = address(0x55d398326f99059fF775485246999027B3197955);
    function swapTokensForCSRoad4fund(uint256 tokenAmount) private {
    ...
    function swapTokensForETCS4fund(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = _baseToken;
