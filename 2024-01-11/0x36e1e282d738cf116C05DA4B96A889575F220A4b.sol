...
contract Spot2WinNFT is ERC721, ERC721Enumerable, ERC2981, AccessControl {
    using Strings for uint256;
    ...
    function _findTokenId(uint256 _type) private view returns (uint256) {
        return (_type * 1e8) + runningNumberByType[_type];
    }
    ...
    function tokenURI(
        ...
        return
            bytes(baseURI).length > 0
                ? string(abi.encodePacked(baseURI, _type))
                : "";
    }
    ...
    function mintByTokenId(
         ...
         */
        require(
            findTokenIndexByTokenId(_tokenId) <= supplyByType,
    ...
    function findTypeStrByTokenId(
        ...
        return
            _substring(
                _tokenId.toString(),
                0,
                bytes(_tokenId.toString()).length - 8
            );
    }
     ...
     */
    function findTokenIndexByTokenId(
        uint256 _tokenId
