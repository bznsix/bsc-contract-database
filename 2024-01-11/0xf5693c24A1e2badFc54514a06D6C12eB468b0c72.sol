...
contract SPARKCITIZENS is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0x1B2f3c0F251297Bb22eA1d1E7983574bb1C8179e;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "SPARKCITIZENS: minting has not started yet");
    require(_mintAmount > 0, "SPARKCITIZENS: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "SPARKCITIZENS: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "SPARKCITIZENS: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "SPARKCITIZENS: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "SPARKCITIZENS: zero address");
    require(_amount > 0, "SPARKCITIZENS: you have to mint at least one");
    require(lastSupply != 0,  "SPARKCITIZENS: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"SPARKCITIZENS: URI query for nonexistent token");
