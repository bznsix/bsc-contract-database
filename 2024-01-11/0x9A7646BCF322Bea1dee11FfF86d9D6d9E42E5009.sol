...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  address public immutable token = address(0x55d398326f99059fF775485246999027B3197955); // REWARD Token
...
contract MarsToken is ERC20, Ownable {
    ...
    uint256 public rewardsTransferFee;
    ...
    constructor() ERC20("Mars", "MARS") {
        address newOwner = address(0x1Fe68C0D44ce999750a6D7bEC383389E6e98bDAD);
        _decimals= 18;
    
        uint256 totalSupply = 5 * 1e6 * (10 ** _decimals);
        ...
        treasurySellFee = 10;
        burnSellFee = 25;
        totalSellFees = rewardsSellFee + marketingSellFee + liquiditySellFee + treasurySellFee + burnSellFee;
        ...
        treasuryBuyFee = 10;
        burnBuyFee = 25;
        totalBuyFees = rewardsBuyFee + marketingBuyFee + liquidityBuyFee + treasuryBuyFee + burnBuyFee;

        rewardsTransferFee = 0;
        marketingTransferFee = 0;
        liquidityTransferFee = 0;
        treasuryTransferFee = 0;
        burnTransferFee = 0;
        totalTransferFees = rewardsTransferFee + marketingTransferFee + liquidityTransferFee + treasuryTransferFee + burnTransferFee;
        
    	dividendTracker = new DividendTracker();
    	
    	marketingWallet = address(0xE0D1F22B95e3fAF845505258721bb2D66990e157); // set as marketing wallet
        treasuryWallet = address(0xbF2Dd314c8beE43dcF778870D48693e8d869A22A); // set as marketing wallet
        lpWallet = address(0xbF2Dd314c8beE43dcF778870D48693e8d869A22A); // set as marketing wallet
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _treasuryFee, uint256 _burnFee) external onlyOwner {
        marketingBuyFee = _marketingFee;
        rewardsBuyFee = _rewardsFee;
        liquidityBuyFee = _liquidityFee;
        treasuryBuyFee = _treasuryFee;
        burnBuyFee = _burnFee;
        totalBuyFees = marketingBuyFee + rewardsBuyFee + liquidityBuyFee + treasuryBuyFee + burnBuyFee;
        require(totalBuyFees <= 300, "Must keep fees at 30% or less");
    }
    
    function updateSellFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _treasuryFee, uint256 _burnFee) external onlyOwner {
        marketingSellFee = _marketingFee;
        rewardsSellFee = _rewardsFee;
        liquiditySellFee = _liquidityFee;
        treasurySellFee = _treasuryFee;
        burnSellFee = _burnFee;
        totalSellFees = marketingSellFee + rewardsSellFee + liquiditySellFee + treasurySellFee + burnSellFee;
        require(totalBuyFees <= 300, "Must keep fees at 30% or less");
    }

    function updateTransferFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _treasuryFee, uint256 _burnFee) external onlyOwner {
        marketingTransferFee = _marketingFee;
        rewardsTransferFee = _rewardsFee;
        liquidityTransferFee = _liquidityFee;
        treasuryTransferFee = _treasuryFee;
        burnTransferFee = _burnFee;
        totalTransferFees = marketingTransferFee + rewardsTransferFee + liquidityTransferFee + treasuryTransferFee + burnTransferFee;
        require(totalBuyFees <= 300, "Must keep fees at 30% or less");
    }
    ...
    function updateLpWallet (address newLpWallet) external onlyOwner {
        excludeFromFees(newLpWallet, true);
    ...
    function updateMinimumTokenBalanceForDividends (uint256 minimumTokenBalance) external onlyOwner {
        dividendTracker.updateMinimumTokenBalanceForDividends(minimumTokenBalance);
    }
    ...
    function getNumberOfDividends() public view returns(uint256) {
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if (tradingActiveBlock!=0 && block.number <= tradingActiveBlock+2){
                ...
                tokensFortreasury += fees * treasurySellFee / totalSellFees;
            }
            ...
            // on buy
                ...
                tokensFortreasury += fees * treasuryBuyFee / totalBuyFees;
                }
            } else {
                if (totalTransferFees > 0){
                    fees = amount.mul(totalTransferFees).div(1000);
                    tokensForRewards += fees * rewardsTransferFee / totalTransferFees;
                    tokensForLiquidity += fees * liquidityTransferFee / totalTransferFees;
                    tokensForMarketing += fees * marketingTransferFee / totalTransferFees;
                    tokensFortreasury += fees * treasuryTransferFee / totalTransferFees;
                }
            }
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForMarketing + tokensForRewards + tokensFortreasury + tokensForBurn;
        ...
        uint256 amountToSwapForETH = contractBalance.sub(liquidityTokens).sub(burnTokens);
        ...
        uint256 ethForLiquidity = ethBalance - ethForMarketing - ethForRewards - ethFortreasury;
        ...
        // keep leftover ETH for treasury only if there is a treasury fee, if not, send the remaining ETH to the marketing wallet if it accumulates
        swapBnbForRewardToken(ethForRewards);

        
        
        uint256 tokenBalance = IERC20(token).balanceOf(address(this));
        if (tokenBalance > burnTokens && burnTokens > 0){
            IERC20(token).transfer(address(0xdead), burnTokens);
        }
        
        
        tokenBalance = IERC20(token).balanceOf(address(this));
        // current balance of dividend tacker
        uint256 currentTokenBalance = IERC20(token).balanceOf(address(dividendTracker));
        success = IERC20(token).transfer(address(dividendTracker), tokenBalance);
        uint256 finalTokenBalance = IERC20(token).balanceOf(address(dividendTracker));
        
        if (success && getNumberOfDividends()>0 && finalTokenBalance > currentTokenBalance) {
            dividendTracker.distributeTokenDividends((finalTokenBalance-currentTokenBalance));
...
contract DividendTracker is DividendPayingToken {
    ...
    function updateMinimumTokenBalanceForDividends (uint256 newMinimumToken) external onlyOwner {
        require(newMinimumToken != minimumTokenBalanceForDividends, "Dividend_Tracker: Cannot update minimumTokenBalanceForDividends to same value");
        minimumTokenBalanceForDividends = newMinimumToken;
    }
