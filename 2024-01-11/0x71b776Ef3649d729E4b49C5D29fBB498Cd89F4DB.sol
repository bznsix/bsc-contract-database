...
contract DragonKingNFT is ERC721Enumerable, Ownable {
    mapping(uint256 => string) public _uris;
    mapping(uint256 => uint256) public _properties;
    mapping(address => bool) public _minter;

    uint256 public constant _baseId = 1;
    mapping(uint256 => uint256) public _propertySupply;
    uint256 public _maxTotal = 100;

    function mint(address to, uint256 num) external {
        require(_minter[msg.sender], "no minter");
        uint256 property = 1;
        uint256 tokenId = totalSupply() + _baseId;
        for (uint256 i; i < num;) {
            _mint(to, tokenId);
    ...
    constructor() ERC721("Dragon King NFT", "Dragon King NFT"){
        _uris[1] = "https://bnb.bscs.cc/DragonKingNFT.json";
    }
}