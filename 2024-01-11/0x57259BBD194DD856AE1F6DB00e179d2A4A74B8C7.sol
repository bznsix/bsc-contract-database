...
contract SOUND is ERC20, Ownable {
    ...
    {   
        ...
        marketingWallet = 0xaCF4022A52dB9a0C34Ca48A1de6F7Db5b3452B0C;
        developmentWallet = 0xC22EF476D7d55A0Cea843223Dc8337C8bDF5AbC5;
        ...
        _mint(owner(), 67232 * (10 ** decimals()));
        swapTokensAtAmount = totalSupply() / 400; // 0.25%
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (canSwap &&
            ...
            if (marketingShare > 0) {
                ...
                swapAndSendTokens(marketingTokens, marketingWallet);
            }

            if (developmentShare > 0) {
                uint256 developmentTokens = contractTokenBalance * developmentShare / totalFee;
                swapAndSendTokens(developmentTokens, developmentWallet);
            }
    ...
    function setSwapEnabled(bool _enabled) external onlyOwner{
        require(swapEnabled != _enabled, "swapEnabled already enabled");
    ...
    function swapAndSendTokens(uint256 tokenAmount, address recipient) private {
        require(tokenAmount > 0, "Amount to swap must be greater than 0");
        ...
        payable(recipient).sendValue(newBalance);

        if (recipient == marketingWallet) {
            emit SwapAndSendMarketing(tokenAmount, newBalance);
        } else if (recipient == developmentWallet) {
            emit SwapAndSendDevelopment(tokenAmount, newBalance);
        }
    }

    function setEnableMaxWalletLimit(bool enable) external onlyOwner {
        require(enable != maxWalletLimitEnabled,"Max wallet limit is already enabled");
    ...
    function excludeFromMaxWallet(address account, bool exclude) external onlyOwner {
        require( _isExcludedFromMaxWalletLimit[account] != exclude,"Account already excluded");
