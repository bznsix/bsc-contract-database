...
contract CentBit is ERC20, Ownable {
    ...
    constructor() ERC20("CentBit", "CBIT") {
        ...
        _mint(_msgSender(), 2_000_000 * 10**18);
        excludeFromFees[_msgSender()] = true; // Exclude deployer/owner from fees
    }
    ...
    function _transfer(
        ...
        if (excludeFromFees[sender] || excludeFromFees[recipient]) {
    ...
    function removeFromWhitelist(address _address) external onlyOwner {
        excludeFromFees[_address] = false;
    }

    function isExcludedFromFees(address _address) external view returns (bool) {
        return excludeFromFees[_address];
    }
}