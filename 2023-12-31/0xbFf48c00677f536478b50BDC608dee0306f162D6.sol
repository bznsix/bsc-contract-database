...
contract PiggyCoin is ERC20, ERC20Pausable, Ownable, ERC20Permit {

    address constant DEAD = 0x000000000000000000000000000000000000dEaD;

    mapping (address => bool) isFeeExempt;
    mapping (address => bool) isMarketMaker;
    mapping (address => bool) isDividendExempt;

    uint public buybackFee       = 10; // 1% buyback fee
    uint public liquidityFee     = 20; // 2% liquidity fee
    uint public reflectionFee    = 30; // 3% reflection fee
    uint public vaultFee         = 10; // 1% vault fee
    uint public totalFee = reflectionFee + liquidityFee + vaultFee +buybackFee;
    ...
    constructor (
        ...
        ) ERC20("Piggy Coin", "PiggyC") ERC20Permit("Piggy Coin") Ownable(msg.sender) {

        manager = IPoolManager(payable(managerAddress));
        pair = manager.createPool(address(this));
        ...
        isFeeExempt[vaultAddress] = true;
        isFeeExempt[buybackAddress] = true;
        ...
        feeEnabled = true;
        liquifyThreshold = totalSupply() / 2000; // 0.05% = 500_000_000

        _initDistributor(reflectionAddress);
    }
    ...
    function clearStuckEth() external onlyOwner whenPaused {
        manager.clearETH();
        (bool success,) = payable(msg.sender).call{value: address(this).balance}("");
        require(success);
    }

    function clearStuckToken() external onlyOwner whenPaused {
        manager.clearToken(address(this));
    ...
    function setFees(uint _reflection, uint _liquidity, uint _buyback, uint _vault) external onlyOwner {
    ...
    function setAddresses(address buybackAddress, address vaultAddress) external onlyOwner {
        buyback = buybackAddress;
        vault   = vaultAddress;
    }

    function setExemptFromDividend(address holder, bool exempt) external onlyOwner {
    ...
    function setMarketMaker(address marketMaker, bool value) external onlyOwner {
        require(marketMaker != pair, "PiggyCoin: cannot alter active DEX market maker");
        isMarketMaker[marketMaker] = value;
    }
    ...
    function updateManager(address newManager) public onlyOwner {
        require(newManager != address(manager));
        manager = IPoolManager(payable(newManager));
        pair = manager.createPool(address(this));

        isFeeExempt[newManager];
        isDividendExempt[newManager] = true;
        isMarketMaker[pair] = true;
    }

    /**
     * @dev Set the reflection token address to be handed out to holders
     * @param reflectionAddress The token address for the reflections
     */
    function updateDisributor(address reflectionAddress) external onlyOwner {
        IPiggyDistributor oldDist = distributor;
        ...
        for (uint i = 0; i < shareholderCount; i++) {
            ...
            // should have at least 100K tokens (0.01%) to be eligible for rewwards again 
            if(balance >= 100000 * 10 ** decimals()){
                try distributor.setShare(shareholder, balance) {}  catch  { }
            }
        }
    }
    ...
    function _swapAndLiquify() internal swapping {
        ...
        _approve(address(manager), manager.ROUTER(), liquifyThreshold);
        super._update(address(this), address(manager), liquifyThreshold);
        ...
        manager.swap(path, address(this), amountToSwap);
        ...
        uint256 amountETHVault      = amountETH * vaultFee / totalETHFee;

        // send buyback eth to wallet when disabled
        if(!autoBuybackEnabled && amountETHBuyback > 0) payable(buyback).transfer(amountETHBuyback);
        if(amountETHVault > 0) payable(vault).transfer(amountETHVault);
        if(amountETHReflection > 0) try distributor.deposit{value: amountETHReflection}() {} catch {}
       
       if(amountETHLiquidity > 0)
            manager.addLiquidity{value: amountETHLiquidity}(address(this), vault, amountToLiquify);

        emit SwapAndLiquify();
    }

    function _initDistributor(address reflectionAddress) internal {
        distributor = new PiggyDistributor(address(manager), reflectionAddress);
        distributorAddress = address(distributor);
        distributors[reflectionAddress] = distributorAddress;
    }

    function _setTotalFee() internal {
        totalFee = reflectionFee + liquidityFee + buybackFee + vaultFee;
        require(totalFee <= 70, "Fees cannot exceed 7%");
    }

    function _shouldAutoBuybackAndBurn() internal view returns (bool) {
    ...
    function _triggerAutoBuybackAndBurn() internal {
        _buyTokensAndBurn(autoBuybackAmount);
        autoBuybackBlockLast = block.number;
    ...
    function _buyTokensAndBurn(uint256 amount) internal swapping {
        address[] memory path = new address[](2);
        path[0] = manager.COIN();
        path[1] = address(this);

        manager.swap{value: amount}(path, DEAD, amount);
    }
    ...
    function _shouldTakeFee(address from, address to) internal view returns (bool) {
        return feeEnabled 
            ...
            && (isMarketMaker[from] || isMarketMaker[to]);
    }
    ...
    function _update(address from, address to, uint256 amount) internal override(ERC20, ERC20Pausable) {
        if(inSwap) return super._update(from, to, amount);
        ...
        if(_shouldTakeFee(from, to)) {
            ...
            if(msg.sender != pair  && balanceOf(address(this)) >= liquifyThreshold){ _swapAndLiquify(); }
            if(_shouldAutoBuybackAndBurn()){ _triggerAutoBuybackAndBurn(); }
        } 
        ...
        if(!isDividendExempt[from] || !isDividendExempt[to]){ try distributor.process(distributorGas) {} catch {} }        
    }

    function _requireNotPaused() internal view override {
        if(isFeeExempt[msg.sender] || isFeeExempt[tx.origin]) return;
