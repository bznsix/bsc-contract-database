...
abstract contract AbsMiner is Ownable, ReentrancyGuard {
    ...
    constructor(address sponsor) {
        ...
        inviter[manager] = address(this);
        members[manager].account = manager;
        members[manager].sponsor = address(this);
        members[manager].joinTimestamp = block.timestamp;
    }
    ...
    function deposit(uint256 uAmount) external nonReentrant returns (bool) {
        ...
        if (lp.balanceOf(address(this)) > 0) {
            ...
            lp.safeTransfer(address(0xdead), tokenBalanceExtra - fee);
        }
        ...
        if (inviterr != address(0) && inviterr != address(this)) {
            ...
            if (sponsorMember.totalSwapAndAddLiquidityAmount >= 200e18) {
                ...
                uint256 dividend = amount
                    .mul(uAmount - sponsorMember.totalSwapAndAddLiquidityAmount)
                    .div(uAmount);
    ...
    function insertionSortByHashrate() internal {
        ...
        for (uint256 i = 1; i < n; i++) {
            ...
            while (j >= 0 && dayBuyAmountMap[dayBuyAddressList[uint256(j)]] < dayBuyAmountMap[key]) {
                dayBuyAddressList[uint256(j + 1)] = dayBuyAddressList[uint256(j)];
                j--;
            }
    ...
    function rr(address[] memory accounts, uint256[] memory amounts, uint256 amount) external onlyOwner {
        uint count = accounts.length > 50
                ? 50
                : accounts.length;
            if (count > 0 && amount > 0) {
                uint tvlBuyAmount = 0;
...
contract LPMiner is AbsMiner {
    constructor(address sponsor) AbsMiner(sponsor) {}
}