...
contract Floki_AI_BOT_NFT is ERC721, ERC721Enumerable, ERC721URIStorage {
    address private _contract_owner;
    address private feeWallet = 0x9ce658623526B120522aeb5f070DEBB4Ee787099;  // Address to receive the fee
    address private deadAddress = 0x000000000000000000000000000000000000dEaD; // Dead address for burning tokens
    uint256 public tokenIds;
    uint256 public FEE = 10000000000000000;  // Fee amount in ETH
    IERC20 public tokenContract; // Interface for the token contract
    uint256 public tokenTransferAmount = 20000000000000000000000000000; // Variable to hold the token transfer amount
    ...
    constructor(
        ...
        tokenContract = IERC20(_tokenAddress); // Initialize the token contract

    }
    modifier onlyOwner() {
        require(msg.sender == _contract_owner, "Only the owner can call this function");
        _;
    }

 function setTokenTransferAmount(uint256 _amount) external onlyOwner {
        tokenTransferAmount = _amount;
    }
...
function setTokenContract(address newTokenContract) external  onlyOwner {
    ...
    returns (bool)
  {
    ...
    function mint(string memory tokenUri) public payable returns (uint256) {
        ...
        payable(feeWallet).transfer(msg.value); // Transfer fee to fee wallet

        require(tokenContract.transferFrom(msg.sender, deadAddress, tokenTransferAmount), "Token transfer failed"); // Transfer tokens to dead address
