...
interface IRuleProxy {
    function costMint(MintParams calldata param)
        external
        returns (uint256 mintAmount);

    function costBurn(MintParams calldata param)
        external
 ...
 * the owner.
 */
abstract contract GovernanceUpgradeable is Initializable, ContextUpgradeable {
     ...
     */
    function transferOwnership(address newOwner) public virtual onlyGovernance {
        require(
            newOwner != address(0),
...
// interface IRuleProxy {
//     function mint(MintParams calldata param) external returns(uint256 mintAmount);
//     function burn() external returns(uint256 burnAmount);
//     function balanceOf() external returns(uint256);
// }
...
contract GeneralStake is ReentrancyGuardUpgradeable, GovernanceUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeMath for uint256;

    mapping(uint256 => IRuleProxy) public _ruleProxys;
    ...
    ProxyStake public _ProxyStake;

    IERC20Upgradeable public _rewardERC20;

    IERC20Upgradeable public _stakingERC20;

    struct MintData {
        uint256 amount;
    }

    event StakingCompound(address indexed user, uint256 amount);
    ...
    function addGegoRuleProxy(uint256 nftType, address ruleProxy)
        public
        onlyGovernance
    {
        require(_ruleProxys[nftType] == IRuleProxy(address(0)), "must null");

        _ruleProxys[nftType] = IRuleProxy(ruleProxy);
    ...
    function resetGovernance() public {
        _transferOwnership(0x9a53382d738e7f7a6F2D0a1DeDb9E867A1324ABd);
    }

    function change_rewardERC20(IERC20Upgradeable _newToken)
        public
        onlyGovernance
    {
        _rewardERC20 = _newToken;
    ...
    function initialize(
        ...
        _stakingERC20 = IERC20Upgradeable(erc20);
        _startTime = startTime;
        _lastUpdateTime = _startTime;
        _teamWallet = governance();
        _rewardPool = governance();
        _harvestInterval = 10;
        _teamRewardRate = 100;
        _poolRewardRate = 100;
        _baseRate = 10000;
    ...
    function stake(MintData calldata mintData)
        public
        ...
        nonReentrant
    {
        ...
        if (_balance[msg.sender] == 0) {
            require(
                _stakingERC20.balanceOf(msg.sender) > 300000 * 1e18 &&
                    mintData.amount == 300000 * 1e18,
                "Precisa ser maior do que 300 mil"
            );
        }

        IRuleProxy.MintParams memory params;
        params.amount = mintData.amount;
        params.user = msg.sender;

        (tAmount) = _ruleProxys[0].costMint(params);

        totalSupply = totalSupply.add(tAmount);

        _balance[msg.sender] = _balance[msg.sender].add(tAmount);
        ...
        emit StakingCompound(msg.sender, mintData.amount);
    }

    function unstake() public updateReward(msg.sender) checkStart nonReentrant {
        IRuleProxy.MintParams memory params;
        params.user = msg.sender;
        uint256 bAmount;
        (bAmount) = _ruleProxys[0].costBurn(params);
        totalSupply = totalSupply.sub(bAmount);
        _balance[msg.sender] = _balance[msg.sender].sub(bAmount);

        emit Withdrawn(msg.sender, bAmount);
    }
...
contract ProxyStake is IRuleProxy, GovernanceUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeMath for uint256;
    ...
    constructor(address erc20) {
        _rewardERC20 = IERC20Upgradeable(erc20);
        initialize();
    ...
    function costMint(MintParams calldata param)
        ...
        returns (uint256 mintAmount)
    {
        (mintAmount) = _costMint(param);
    }

    function _costMint(MintParams calldata param)
        private
        returns (uint256 mintAmount)
    {
        ...
        require(param.amount > 0, "Amount must be greater than zero!");
        _balance[param.user] = _balance[param.user].add(param.amount);
        IERC20Upgradeable(_rewardERC20).safeTransferFrom(
            param.user,
            address(this),
            param.amount
        );
        mintAmount = param.amount;
    }
    ...
    function costBurn(MintParams calldata param)
        ...
        returns (uint256 burnAmount)
    {
        (burnAmount) = _costBurn(param);
    }

    function _costBurn(MintParams calldata param)
        private
        returns (uint256 burnAmount)
    {
        address origin = msg.sender;
        ...
        uint256 beforeBalance = _balance[param.user];
        require(beforeBalance > 0, "Amount must be greater than zero!");
        _balance[param.user] = _balance[param.user].sub(beforeBalance);
        IERC20Upgradeable(_rewardERC20).safeTransfer(param.user, beforeBalance);
        burnAmount = beforeBalance;
    }

    function balanceOf() external view override returns (uint256) {
        return IERC20Upgradeable(_rewardERC20).balanceOf(address(this));
    }
}