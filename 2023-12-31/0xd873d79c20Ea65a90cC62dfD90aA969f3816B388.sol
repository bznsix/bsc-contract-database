...
interface IUniswapV2Router01 {
    ...
    function swMEMEokensForExactTokens(
        uint256 amountOut,
        uint256 amountInMax,
        address[] calldata path,
        address to,
    ...
    function swMEMEokensForExactETH(
        uint256 amountOut,
        uint256 amountInMax,
        address[] calldata path,
        address to,
...
contract MEME is ERC20, Ownable {
    ...
    uint256 public sellAmount = 10;
    uint256 public buyAmount = 10;

    uint256 private totalSellFees;
    uint256 private totalBuyFees;

    MEMEDividendTracker public dividendTracker;
    ...
    uint256 public swMEMEokensAtAmount;
    ...
    constructor() ERC20("MEME 2.0", "MEME") {
        marketingWallet = payable(0x6E93752CA92F0A85F39116e0C3E74Ea0BB2a96b6);
        devWallet = payable(0xf3F3d969F6a58dC5e52029D45D303c8DbaA50d1d);
        ...
        dividendTracker = new MEMEDividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (1_000) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 50; // 10%
        swMEMEokensAtAmount = totalTokenSupply / 10; // 0.01%;
    ...
    // rewards threshold
    function setSwMEMEriggerAmount(uint256 amount) public onlyOwner {
        swMEMEokensAtAmount = amount * (10**18);
    }
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 300000);
    ...
    function updateFees(
        ...
        require(totalSellFees <= 10 && totalBuyFees <= 5, "total fees cannot exceed 15% sell or buy");
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            bool canSwap = contractTokenBalance >= swMEMEokensAtAmount;
            ...
            if (canSwap && !automatedMarketMakerPairs[from]) {
                ...
                if (swapAndLiquifyEnabled && liquidityFee > 0 && totalBuyFees > 0) {
                    ...
                    uint256 swMEMEokens = swapSellTokens.add(swapBuyTokens);

                    swapAndLiquify(swMEMEokens);
                }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        swMEMEokensForEth(half); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        uint256 newBalance = address(this).balance.sub(initialBalance);
        addLiquidity(otherHalf, newBalance);
        emit SwapAndLiquify(half, newBalance, otherHalf);
    }

    function swMEMEokensForEth(uint256 tokenAmount) private {
...
contract MEMEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        MEMEContract = MEME(mainContract);
        minimumTokenBalanceForAutoDividends = 10_000000000000000000; // 10 tokens
    ...
    function _transfer(
        ...
        require(false, "MEME_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "MEME_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(MEMEContract);

            uint256 prevBalance = MEMEContract.balanceOf(address(this));
            ...
            {
                uint256 received = MEMEContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MEMEContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
