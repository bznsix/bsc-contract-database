...
contract DPexPair is IDPexPair, Initializable, DPexBEP20 {
    ...
    modifier lock() {
        require(unlocked == 1, 'DPEX: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'DPEX: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IDPexFactory(factory).feeTo();
    ...
    function mint(address to) external override lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'DPEX: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external override lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'DPEX: INSUFFICIENT_LIQUIDITY_BURNED');
...
import "./IDPexBEP20.sol";

interface IDPexPair is IDPexBEP20 {
...
abstract contract DPexBEP20 is IDPexBEP20, Ownable {
    ...
    string public constant override symbol = "DPEX-LP";
