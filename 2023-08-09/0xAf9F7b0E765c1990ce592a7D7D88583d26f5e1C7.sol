...
contract DAOPrefundingRaise is Ownable, ReentrancyGuard {
    ...
    uint public ticketSize;

    /**
        @dev Upon registration, users are required to send a certain amount of native gas token (eth, bnb...) for post-processing their funds.
     */
    ...
    function setRegistrationEndTime(uint newRegistrationEndTime) external onlyOwner {
        ...
        require(newRegistrationEndTime < block.timestamp + EXPIRES_AFTER);
        require(guaranteedContributionIndex == 0);
    ...
    function setGuaranteedContributionAmounts(uint usersCount) external {
        ...
        for (uint i = guaranteedContributionIndex; i < endIndex; i++) {
            ...
            if (user.isRegisteredInLottery) {
                user.totalDaoPowerInLottery = uint64(_totalDaoPowerInLottery);
                _totalDaoPowerInLottery += user.daoPower;
                _totalContributionAmountInLottery += user.contributionAmount;
            } else {
                uint guaranteedContributionAmount = user.daoPower * targetRaiseAmount / totalDaoPower;
    ...
    function runLottery(uint16[] calldata indexes) external {
        ...
        for (uint i = 0; i < indexes.length; i++) {
            ...
            uint randomNumber = uint(_randomHash) % totalDaoPowerInLottery;

            require(user.isRegisteredInLottery && user.totalDaoPowerInLottery <= randomNumber && (user.totalDaoPowerInLottery + user.daoPower) > randomNumber, "index is not correct");
        ...
        for (uint i = processedUserIndex; i < endIndex; i++) {
            ...
            if (overfunded) {
                if (user.isRegisteredInLottery) {
                    ...
                    finalContributionAmount = user.daoPower * targetRaiseAmount / totalDaoPower;
     ...
     */
    function register(bytes calldata signature, uint64 daoPower, uint104 contributionAmount, uint maxContributionAmount, bool isRegisteredInLottery) external payable nonReentrant {
        User storage user = users[msg.sender];
        require(daoPower > 0, "0 dao power");
        ...
        _verifySignature(signature, daoPower, maxContributionAmount);
        _contribute(daoPower, contributionAmount, maxContributionAmount, isRegisteredInLottery);
        emit Registration(msg.sender, offeringId, daoPower, user.contributionAmount);
    }
    ...
    function _contribute(uint64 daoPower, uint104 contributionAmount, uint maxContributionAmount, bool isRegisteredInLottery) internal {
        User storage user = users[msg.sender];
    ...
    function _verifySignature(bytes calldata signature, uint daoPower, uint maxContributionAmount) internal view {
        bytes32 dataHash = keccak256(abi.encodePacked(offeringId, msg.sender, daoPower, maxContributionAmount));
    ...
    function _calculateTickets() internal view returns (uint _tickets, bool _lotteryForWinners) {
        ...
        _tickets = totalDaoPowerInLottery * targetRaiseAmount / totalDaoPower / ticketSize;

        if (totalDaoPowerInLottery * 2 < totalDaoPower && _tickets * ticketSize < totalDaoPowerInLottery * targetRaiseAmount / totalDaoPower) {
            _tickets += 1;
        }
