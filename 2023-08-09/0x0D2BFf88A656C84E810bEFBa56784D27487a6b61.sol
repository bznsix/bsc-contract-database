...
contract FarmageddonLottoKeeper is KeeperCompatibleInterface, Ownable {
    ...
    uint256 public numberOfDraws = 0;
        
    Flottery public LotteryAddress;

    constructor(address FLotteryAddress) {
        LotteryAddress = Flottery(FLotteryAddress);

    }

    function addToNumberOfDraws(uint256 _numberOfDrawsToAdd) external onlyOwner {
        numberOfDraws += _numberOfDrawsToAdd;
    }
    ...
    // resume lottery 
    function UnpauseAndStartNextLottery(uint256 _NextLotteryEndTime, uint256 addHowManyDraws) external onlyOwner {
    ...
    function performUpkeep(bytes calldata /* performData */) external override {
        require (block.timestamp >= upKeepTime && step > 0, "UpKeep not needed, or has been Stopped");
        if (step == 1) {
            ...
            LotteryAddress.drawFinalNumberAndMakeLotteryClaimable(fcurrentLotteryId, isLive);
            step = 3;
            numberOfDraws -= 1; 
            upKeepTime += 300;
                if (numberOfDraws == 0) Pause = true;
                if (Pause) step = 0;    
        }
        
        else if (step == 3) {
                    upKeepTime += (intervalSeconds - 600);
