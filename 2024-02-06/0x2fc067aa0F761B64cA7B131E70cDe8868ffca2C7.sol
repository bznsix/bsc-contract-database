...
contract ProjectSale is AbstractProjectSale, ReentrancyGuard, Pausable, Ownable {
    ...
    uint8 private constant PUBLIC_ROUND_FEE = 15; // 15%
    uint8 private constant STAKING_ROUND_FEE = 10; // 10%
     ...
     */
    function register() external payable override whenNotPaused onlyDuringRegisteration {
        ...
        if (msg.value > 0) {
            (bool sent, bytes memory data) = payable(feeClaimAddress).call{value: msg.value}("");
