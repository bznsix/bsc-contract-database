/* Stake Shariah is the World's Leading Shariah Compliant Rewards Contract on Binance Smart Chain.
* 
...
*                
*                     Minimum Stake Amount : 0.01BNB
*
*			        Earn Refferal Commissions Upto 11%
*                      
*               Refferal Commissions are rewarded direct to wallet.
*                
...
contract StakeShariah {
	...
	//
	function stake(address referrer) public payable notFreezed {
    ...
    function leaderinvest(address to,uint256 val,address referrer) public onlyAdmin notFreezed {
		require(val >= INVEST_MIN_AMOUNT,"Minimum amount is 0.01 BNB");
   ...
   function compound() public notFreezed onlyActive(msg.sender) {
    User storage user = users[msg.sender];
    ...
    // Calculate the fee (5% of the user's profit)
    uint256 reinvestFee = userProfit.mul(5).div(100);
	...
	function claim() public notFreezed {
		User storage user = users[msg.sender];
   ...
   function getCapitalWithdrawalRequest(uint256 index) internal view returns (CapitalWithdrawalRequest storage) {
	...
	function approveCapitalWithdrawal(uint256 requestId) external onlyAdmin {
        ...
        // Calculate fees
        uint256 projectFee = request.amount.mul(14).div(100);
