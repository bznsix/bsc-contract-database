...
contract BaseFatToken is IERC20, Ownable {
    ...
    function completeCustoms(uint256[] calldata customs) external onlyOwner {
        ...
        require(_buyBurnFee + _buyLPFee + _buyFundFee < 10000, "fee too high");
        require(
            _sellBurnFee + _sellLPFee + _sellFundFee < 10000,
            "fee too high"
        );
    }
...
contract Token is BaseFatToken {
    bool private inSwap;
    ...
    constructor(
        ...
        name = "TangYuan";
        symbol = "TangYuan";
        decimals = 18; //numberParams[0]
        totalSupply = 10000000000 * 10**decimals; //numberParams[1]
        ...
        maxBuyAmount = totalSupply;//numberParams[9];
        maxSellAmount = totalSupply;//numberParams[10];

        maxWalletAmount = totalSupply;//numberParams[11];
        require(
            maxSellAmount >= maxBuyAmount,
            " maxSell should be > than maxBuy "
        );
    ...
    uint256 public walletLimitDuration = 10 * 60; //limit 10min
    function setWalletLimitDuration(uint256 newValue) public onlyOwner{
        walletLimitDuration = newValue;
    }

    function getMaxWalletAmount() public view returns(uint256){
        //if (block.timestamp <= startTime + walletLimitDuration){
        return totalSupply/1000; //limit 1/1000
        //}else{
        //   return totalSupply;
        //}
    }
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (enableWalletLimit && _swapPairList[from]) {
                    ...
                    require(
                        _b + amount <= getMaxWalletAmount(),//maxWalletAmount,
