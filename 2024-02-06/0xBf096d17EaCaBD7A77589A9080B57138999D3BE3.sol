...
contract ClaimStakeAmount{
    address owner;
    address public stakeToken;
    mapping(uint256 => bool) usedNonces;
    mapping(address =>mapping(uint256=>bool)) public claimedUsers;
...
//////////////////////////////////////Verification Functions////////////////////////////////////////////////
    ...
    function _generateKey(uint256 amount,uint256 nonce,uint256 poolId,address receiver)internal pure returns(bytes32 key){
        key = keccak256(abi.encode(amount,nonce,poolId,receiver));
    }

    function generateKey(uint256 amount,uint256 nonce,uint256 poolId,address receiver )external pure returns(bytes32 key){
        key = _generateKey(amount,nonce,poolId,receiver);
    }

    function genMessage(uint256 amount,uint256 nonce,uint256 poolId,address receiver) external  pure returns(string memory _message){
        _message = _generateKey(amount,nonce,poolId,receiver).toString();
    }
...
////////////////////////////////////////////////AirDrop Functions////////////////////////////////////////////////
    ...
    function claimStakedAmount(uint256 amount, uint256 nonce,uint256 poolId, address receiver, bytes memory signature) external {
        require(!usedNonces[nonce], "Already used nonce");
        require(!claimedUsers[msg.sender][poolId], "Already claimed");
        require(poolId==4 || poolId==5,"Incorrect Pid");
        usedNonces[nonce] = true;
        claimedUsers[msg.sender][poolId] = true;
        bytes32 message = _generateKey(amount, nonce, poolId,receiver);
        (uint8 v, bytes32 r, bytes32 s) = splitSignature(signature);
        address confirmed = message.toString().recover(v,r,s);
        require(confirmed == owner, "Signer is not owner");
        require(IERC20(stakeToken).transfer(receiver, amount), "Transfer of tokens fails");
    }
 ...
 function shutdown(uint256 _amount) external {
  ...
  function setStakeToken(address _token) external {
