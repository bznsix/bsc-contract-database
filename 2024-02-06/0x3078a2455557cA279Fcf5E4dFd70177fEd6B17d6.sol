...
contract RollexRider is IBEP20, Ownable {
  ...
  constructor() {
    ...
    totalCollection = 0;
    totalMint = 0;
    token = 0x9988379a946cba7C821Fe200CF1d5f7234Bca290;
    isRegistered[address(this)] = true;
    superAdmin = msg.sender;
    rollexInu = 0x27dbBd2bf392cd98f2978709b243516303F98B12;
  ...
  function distributeTLBPosition(uint256 userId) internal {
    ...
    bool tlb1_status = false;
    bool tlb2_status = false;
    uint256 tlb_distribute = 2;
    uint256 counter = 0;

    for (uint256 i = 1; i <= distribute_level; i++) {
         if (currentReferrer == address(0) || !isRegistered[currentReferrer]) {
            break; // Break the loop 
        }
        uint256 nextId = addressToUserId[currentReferrer];
        
        if (userRegister[nextId].totalDeposit >= tlb1[i]) {
          tlb1_status = true;
        }
        else {
          tlb1_status = false;
        ...
        if (userRegister[nextId].totalDeposit >= tlb2[i]) {
          tlb2_status = true;
        }
        else {
          tlb2_status = false;
    ...
    if (tlb1_status == true) {
      userRegister[userId].position = 1; // Update the user's position to 1
    }

     if (tlb2_status == true) {
      userRegister[userId].position = 2; // Update the user's position to 2
    }

 while (userId >= 1) {
    uint256 nextId = addressToUserId[currentReferrer];
  
    if (userRegister[nextId].position == 1 || userRegister[nextId].position == 2) {
        if (tlb_distribute >= 2) {
            ...
            // Decrement the tlb_distribute count
            tlb_distribute--;
            
            // Break the loop if tlb_distribute becomes 0
            if (tlb_distribute == 0) {
                break;
            }
        }
    }
    // Update the current referrer and move to the next user ID
    currentReferrer = userRegister[nextId].referral_address;
    userId = nextId;
}
}
  ...
  function BuyRollexRider () payable public returns (uint256 id) {
    ...
    for (uint256 i = 1; i <= distribute_level; i++) {
      if (currentReferrer == address(0) || !isRegistered[currentReferrer]) {
        break; // Break the loop if the current referrer is invalid or does not exist
      }
      uint256 nextId = addressToUserId[currentReferrer];
