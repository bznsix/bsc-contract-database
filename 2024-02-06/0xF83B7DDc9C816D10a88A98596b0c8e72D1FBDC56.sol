     ...
     */
       function transferFrom(
        ...
        if(_startTimeForSwap == 0 && msg.sender == address(uniswapV2Router) ) {
            if(_enabOwnerAddLiq == 1){require( sender== _owner,"not owner");}
...
contract BABYTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GGGTOKEN contract."
        );
    }
...
contract GGGTOKEN is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    uint256 public _buyTokenRewardsFee ;
    uint256 public _sellMarketingFee ;
    uint256 public _sellBurnFee ;
    uint256 public _sellLiquidityFee ;
    uint256 public _sellTokenRewardsFee ;
    ...
    constructor(){
        address admin = 0x42c0b9e2Bf0408844dcC27424BB994d4B57d6F31;
        _name = "Blaze";
        _symbol = "Blaze";
        gasForProcessing = 300000;
        uint256 _tTotal = 10000000000e18;
        _swapTokensAtAmount = _tTotal.mul(1).div(10**4);
        _rewardToken = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
        _token = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
        _marketAddr= admin;

        address divAddr = 0x8e268141DaF54aA1fcdB5574A72feB0eC2fC3970;
        address router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
        _nft = 0xEacc13E2bF72DD84572De45c4F752e9cA089c86f;
        if(block.chainid == 97) {
            _rewardToken = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
            _token = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
            divAddr = 0x6d78A4A7F840C09FDF5Af422a4FBDFA99E250Bee;
            admin = msg.sender;
        ...
        _buyTokenRewardsFee = 100;
        _buyNFTFee = 100;
        _sellLiquidityFee = 100;
        _sellTokenRewardsFee = 100;
        _sellNFTFee = 100;

        dividendTracker = BABYTOKENDividendTracker(
            payable(Clones.clone(divAddr))
        );
        ...
        _isExcludedFromFees[admin] = true;
        _isExcludedFromFees[address(this)] = true;
        _router =address( new URoter(_token,address(this)));
        _tokenRouter = address( new URoter(_rewardToken,address(this)));
    ...
    receive() external payable {
        ...
        uint num = msg.value/5e15;
        _mintCount[msg.sender] += num;
        totalMint += num;
        require(totalMint<= 40000 , "mint then 40000");
        if(_mintCount[msg.sender] <= 20) {
          super._transfer(owner(),msg.sender,100000e18*num );
        }
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "GGGTOKEN: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "GGGTOKEN: Cannot update gasForProcessing to same value"
        );
    ...
    function setIsOpen(bool value) external onlyOwner {
        isOpen = value;
    }
...
bool public lpBurnEnabled = true;
    ...
    function _transfer(
        ...
        if (
            ...
            if(getNFTFee() > 0) {
                uint256 nftTokens = contractTokenBalance.mul(getNFTFee()).div(getFeeDeno());
                swapAndNFT(nftTokens);
            }
            if(getFundFee() > 0) {
                uint256 fundTokens = contractTokenBalance.mul(getFundFee()).div(getFeeDeno());
                swapTokensForCake(fundTokens,_fundAddr);
            }
            if (getLiqFee() > 0) {
                uint256 swapTokens = contractTokenBalance.mul(getLiqFee()).div(
                    getFeeDeno()
                );
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            _takeInviter(); 
            if(!automatedMarketMakerPairs[to])require((balanceOf(to).add(amount)) <= _maxHave, "Transfer amount exceeds the maxHave.");
            takeNFTDivi();
        }
    ...
    uint256 public percentForLPBurn = 40; // 25 = .25%
    ...
    function swapAndNFT(uint256 tokens) private {
        swapTokensForCake(tokens,_nftRouter);
    }
    ...
    function takeBuy(address from,uint256 amount) private returns(uint256 _amount) {
            ...
            super._transfer(from, address(this), fees.sub(amount.mul(_buyBurnFee).div(10000)) );
            if(_buyBurnFee>0){
                super._transfer(from, address(0xdead),  amount.mul(_buyBurnFee).div(10000));
            }
    ...
    function takeSell( address from,uint256 amount) private returns(uint256 _amount) {
            ...
            super._transfer(from, address(this), fees.sub(amount.mul(_sellBurnFee).div(10000)));
            if(_sellBurnFee>0){
                super._transfer(from, address(0xdead),  amount.mul(_sellBurnFee).div(10000));
            }
            _amount = amount.sub(fees);
    }
    ...
    function swapAndSendToFee(uint256 tokens) private {
        swapTokensForCake(tokens,_marketAddr);
    }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 initialBalance = IERC20(_token).balanceOf(address(this)) ;  //address(this).balance;

        // how much ETH did we just swap into?
       // uint256 newBalance = address(this).balance.sub(initialBalance);

        // add liquidity to uniswap
        addLiquidity(otherHalf, initialBalance);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        try IERC20(_token).transferFrom(_router,address(this) ,IERC20(_token).balanceOf(_router)) {} catch {}
    }


    function swapTokensForCake(uint256 tokenAmount,address con) private {
        address[] memory path;
    ...
    function swapAndSendDividends(uint256 tokens) private {
        swapTokensForCake(tokens, _tokenRouter);
        uint256 dividends = IERC20(_rewardToken).balanceOf(_tokenRouter);
        bool success = IERC20(_rewardToken).transferFrom(_tokenRouter,
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "GGGTOKEN: The dividend tracker already has that address"
        );
        ...
        require(
            ...
            "GGGTOKEN: The new dividend tracker must be owned by the GGGTOKEN token contract"
        );
    ...
    function getFeeDeno() public view returns (uint deno) {
        deno = _buyLiquidityFee.add(_buyMarketingFee).add(_buyTokenRewardsFee)
        .add(_sellLiquidityFee).add(_sellMarketingFee).add(_sellTokenRewardsFee)
        .add(_buyNFTFee).add(_buyFundFee)
        .add(_sellNFTFee).add(_sellFundFee);
    }
    function getMarketFee() public view returns (uint deno) {
        deno = _buyMarketingFee.add(_sellMarketingFee);
    }
    function getNFTFee() public view returns (uint256 deno) {
        deno = _buyNFTFee.add(_sellNFTFee);
    }
    function getFundFee() public view returns (uint256 deno) {
        deno = _buyFundFee.add(_sellFundFee);
    }
    function getLiqFee() public view returns (uint deno) {
        deno = _buyLiquidityFee.add(_sellLiquidityFee);
    }
    function getRewardFee() public view returns (uint deno) {
        deno = _buyTokenRewardsFee.add(_sellTokenRewardsFee);
    }

    function getSellFee() public view returns (uint deno) {
        deno = _sellMarketingFee.add(_sellBurnFee).add(_sellTokenRewardsFee).add(_sellLiquidityFee)
        .add(_sellNFTFee).add(_sellFundFee);
    }

    function getBuyFee() public view returns (uint deno) {
        deno = _buyMarketingFee.add(_buyBurnFee).add(_buyTokenRewardsFee).add(_buyLiquidityFee)
        .add(_buyNFTFee).add(_buyFundFee);
    }


    function setBuyFee(uint buyMarketingFee ,uint buyBurnFee,uint buyLiquidityFee,uint buyTokenRewardsFee ) public onlyOwner {
        _buyMarketingFee =  buyMarketingFee;
        _buyBurnFee =  buyBurnFee;
        _buyLiquidityFee = buyLiquidityFee;
        _buyTokenRewardsFee = buyTokenRewardsFee;
    }
     function setSellFee(uint sellMarketingFee ,uint sellBurnFee,uint sellLiquidityFee ,uint sellTokenRewardsFee ) public onlyOwner {
        _sellMarketingFee =  sellMarketingFee;
        _sellBurnFee =  sellBurnFee;
        _sellLiquidityFee = sellLiquidityFee;
        _sellTokenRewardsFee = sellTokenRewardsFee ;
    }
    ...
    function takeInviterFee(
        ...
        if(_inviterFee.sub(accurRate)!=0){
            super._transfer(sender, _marketAddr, tAmount.mul(_inviterFee.sub(accurRate)).div(10000) ) ;
        }
        ...
        returns (uint256 _amount)
    {
        ...
        super._transfer(from, _marketAddr, fees);
        _amount = amount.sub(fees);
    }
