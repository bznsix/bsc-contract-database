...
contract MetaVrStaking is Ownable, ReentrancyGuard{
    ...
    function adjustROI(uint256 _roi) public onlyOwner {
        apr = _roi;
    }
    ...
    function deposit(uint256 _amount, address _referrer) external {
        ...
        if (investors[msg.sender].investor == address(0)) {
            // New investor, initialize their data
            ...
            investors[msg.sender].claimedAmount = 0;
            investors[msg.sender].referAmount = 0;
            investors[msg.sender].finishedAmount = 0;
        } else {
            // Existing investor, accumulate their rewards
            uint256 existingRewards = getAllClaimableReward(msg.sender);
            investors[msg.sender].claimableAmount = investors[msg.sender].claimableAmount.add(existingRewards);
        }
    ...
    function getAllClaimableReward(address _investor) public view returns (uint256) {
        uint256 toRemove;
        uint256 _finishedAmount;
        uint256 __finishedAmount;
        uint256 __claimed;
        for(uint256 i = 1; i < ownedDeposits[_investor].length+1; i++) {
            if(depositState[i].state == true) {
                if(block.timestamp - depositState[i].depositAt > withdrawPeriod) {
                    toRemove += depositState[i].depositAmount;
                }
                if(block.timestamp - depositState[i].depositAt > withdrawPeriod && depositState[i].claimed == false) {
                    __finishedAmount += depositState[i].depositAmount;
                    __claimed += depositState[i].claimedAmount;
                }
            }
        }
        ...
        uint256 allClaimableAmount = (lastRoiTime *
            investors[_investor].totalLocked.sub(toRemove) *
            apr).div(percentRate * rewardPeriod);

        if (toRemove > 0) {
            _finishedAmount = (withdrawPeriod *
                __finishedAmount *
                apr).div(percentRate * rewardPeriod);
        }

        return investors[_investor].claimableAmount.add(allClaimableAmount).add(_finishedAmount.sub(__claimed));
    }
    ...
    function withdrawFunds(uint256 _amount) public onlyOwner {
        require(IERC20(BUSDContract).balanceOf(address(this)) >= _amount, "Amount exceeds contract balance.");
        IERC20(BUSDContract).safeTransfer(msg.sender, _amount);
    }

    function injectProfits(uint256 _amount) public onlyOwner {
        IERC20(BUSDContract).safeTransferFrom(msg.sender, address(this), _amount);
    }
