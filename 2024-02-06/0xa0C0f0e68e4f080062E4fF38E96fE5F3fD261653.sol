...
contract SNOWGROK is ERC20, Ownable {
    ...
    constructor(
        ...
        address owner,
        address mintToAddr
    ) ERC20('SNOW GROK', 'SGROK') {
        require(
            feeAccount != address(0),
            'SNOW GROK: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'SNOW GROK: incorrect fee percent');
        require(owner != address(0), 'SNOW GROK: incorrect owner address');
        require(mintToAddr != address(0), 'SNOW GROK: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'SNOW GROK: transfer from the zero address');
        require(recipient != address(0), 'SNOW GROK: transfer to the zero address');
