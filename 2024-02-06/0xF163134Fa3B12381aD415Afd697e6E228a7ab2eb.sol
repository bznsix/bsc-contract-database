...
contract GEC is ERC20 {
    ...
    address _fundAddress = address(0xCD1D412a84d14d1Bac43678A69233B52cf50D101);
    ...
    constructor(address tokenOwner) ERC20("DogeKing", "DogeKing") {
        require(_baseToken < address(this),"GEC small");
    ...
    function _transfer(
		...
		if(super.balanceOf(address(this)) > super.balanceOf(uniswapV2Pair).div(400)){
			if (
				!swapping &&
				_tokenOwner != from &&
				_tokenOwner != to &&
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                //if(startTime.add(1800) > block.timestamp){require(amount <= 10**22, "max");                }
                super._transfer(from, _destroyAddress, amount.div(10000).mul(10));
                super._transfer(from, address(this), amount.div(10000).mul(388));
                    amount = amount.div(10000).mul(9602);
                // if(startTime.add(7200) > block.timestamp){
                //     super._transfer(from, address(this), amount.div(10000).mul(990));
                //     amount = amount.div(10000).mul(9000);
                // }else{
                //     super._transfer(from, address(this), amount.div(10000).mul(388));
                //     amount = amount.div(10000).mul(9602);
                // }
                ...
                //     super._transfer(from, address(this), amount.div(10000).mul(2490));
                //     amount = amount.div(10000).mul(7500);
                // }else if(startTime.add(3600) > block.timestamp){
                //     super._transfer(from, address(this), amount.div(10000).mul(1990));
                //     amount = amount.div(10000).mul(8000);
                // }else if(startTime.add(7200) > block.timestamp){
                //     super._transfer(from, address(this), amount.div(10000).mul(990));
                //     amount = amount.div(10000).mul(9000);
                // }else{
                //     super._transfer(from, address(this), amount.div(10000).mul(388));
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**19){
            swapTokensForUsdt(allTokenAmount.div(388).mul(387));
            uint256 allUsdtAmount = USDT.balanceOf(address(this));
            if(allUsdtAmount > 10**10){
                USDT.transfer(_fundAddress, allUsdtAmount.div(387).mul(386));
                addLiquidityUsdt(allUsdtAmount.div(387), allTokenAmount.div(388));
            }
		}
    }
