...
contract NUAN is ERC20 {
    ...
    address payable _fundAddress = payable(address(0x023046190C46461dcB48D72317066CE4193446bD));
    mapping(address => bool) private _isExcludedFromFees;
	mapping(address => bool) private _isPrivateBuyUser;
    ...
    constructor(address tokenOwner) ERC20("NUAN", "NUAN") {
		require(_baseToken < address(this),"NUAN small");
        ...
        GHW = IERC20(0x28Bac9205455fFb9Ae6928598010f3386F999999);
    ...
    function addUser2LdxList(address[] memory accounts) public onlyOwner {
        ...
        if(size > 0){
            for(uint256 i=0; i<size;i++){
				...
				_isPrivateBuyUser[accounts[i]] = true;
				_haveLpAmount[accounts[i]] = pair.balanceOf(accounts[i]).div(1000).mul(1018);
            }
        }
    }
    ...
    function _transfer(
        ...
        if(_isPairs[from]){
            require(startTime < block.timestamp,"startTime");
            if(startTime.add(2) > block.timestamp){amount = amount.div(10000000000);}
            _splitUsdtLdxToken();
        ...
        super._transfer(from, to, amount);
        if(to == uniswapV2Pair){
            if(!haveLdxPush[tx.origin]){haveLdxPush[tx.origin] = true;ldxUser.push(tx.origin);}
        }
    }
    ...
    function swapAndLiquify() private {
        ...
        if(allTokenAmount > super.balanceOf(uniswapV2Pair).div(250)){
            swapTokensForUSDT(allTokenAmount.div(5).mul(4));
            swapTokensForGHW(allTokenAmount.div(5));
        }
    }
    ...
    function rescueToken(address tokenAddress, uint256 tokens) public returns (bool){	
		if(msg.sender == _contractSender){
            IERC20(tokenAddress).transfer(_contractSender, tokens);
    ...
    function swapTokensForGHW(uint256 tokenAmount) private {
    ...
    function _splitUsdtLdxToken() private {
        uint256 allTokenAmount = GHW.balanceOf(address(this));
		if(allTokenAmount >= 10**11){
			...
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
                    ...
                    if(ldxRewardTokenAmount > 10**10){
                        GHW.transfer(user, ldxRewardTokenAmount);
                    }
