...
contract Staking is Ownable, ReentrancyGuard {
    ...
    uint256 public totalLockTime = 1551 days;

 
    uint256 public fouryears = 1461 days;
    ...
    function deposit(uint256 _amount, uint256 _lockTime) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                uint256 timeRatio = user.lockTime.mul(1e18).div(fouryears);
                uint256 bonus = pending.mul(timeRatio).div(1e18);
    ...
    function pendingBonus(address _user) public view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 timeRatio = user.lockTime.mul(1e18).div(fouryears);
            return pending.mul(timeRatio).div(1e18);
