...
contract PFLOKI is Context, IERC20, Ownable {
    ...
    uint256 public transferDelay = 300;
    ...
    uint256 public numTokensSellToAddToLiquidity = 100000000000000000000000000000;
    uint256 private buyBackLimit = 1000000000000000000;
    uint256 private buyBackUpperLimit = 10000000000000000000;
    ...
    function _transfer(
        ...
        require(block.timestamp >= (lastTrade[_msgSender()]).add(transferDelay) || from == owner() || to == owner() || _isExcludedFromFee[from] || _isExcludedFromFee[to], "Wait for transfer delay");
        ...
        lastTrade[_msgSender()] = block.timestamp; //bot pot
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for BNB
        swapTokensForEth(half); // <- this swaps BNB -> SLTVN when swap+liquify is triggered
    ...
    function upgradeBurnStrat() external {
        ...
        stratBurnTax.tax = 200; // reset to defaults
    ...
    function upgradeHoldersStrat() external {
        ...
        stratHoldersTax.tax = 200;  // reset to defaults
    ...
    function upgradeLiquidityStrat() external {
        ...
        stratLiquidityTax.tax = 500; // reset to defaults
    ...
    function setBuybackLimit(uint256 _buyBackLimit) external onlyOwner() {
        buyBackLimit = _buyBackLimit;
    }
    
    function setBuybackUpperLimit(uint256 _buyBackUpperLimit) external onlyOwner() {
    ...
    function afterPreSale() external onlyOwner {
        ...
        _burnFee = 200;
        _liquidityFee = 500;
        _maxTxAmount = 2400000000000 * 10**18;
        transferDelay = 300;
    ...
    function transferBEP20Tokens(address _tokenAddr, address _to, uint256 _amount) external onlyOwner {
        require(_tokenAddr != address(this), "Transfer failed");
