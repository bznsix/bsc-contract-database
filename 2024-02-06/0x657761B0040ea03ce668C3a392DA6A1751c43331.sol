...
contract DefiEdgeTwapStrategyFactory is ITwapStrategyFactory {
    ...
    uint256 public override maximumManagerPerformanceFeeRate = 20e6; // default 20% - 1e8 means 100%

    uint256 public constant MAX_PROTOCOL_PERFORMANCE_FEES_RATE = 100e6; // maximum 100%
    ...
    IAlgebraFactory public override algebraFactory; // Uniswap V3 pool factory
    ...
    /**
     * @notice Changes maximum cap for manager performance fees
     * @param _fee New fee in 1e8 format
     */
    function changeMaxManagerPerformanceFeeRate(uint256 _fee) external onlyGovernance {
        require(_fee <= 1e8, "IA"); // should be less than 100%
        maximumManagerPerformanceFeeRate = _fee;
        emit ChangeDefaultMaxManagerPerformanceFee(_fee);
    }
    ...
    function changeProtocolPerformanceFeeRate(address pool, address strategy, uint256 _feeRate) external onlyGovernance {
        require(_feeRate <= MAX_PROTOCOL_PERFORMANCE_FEES_RATE, "IA"); // should be less than 100%
...
contract DefiEdgeTwapStrategy is UniswapV3TwapLiquidityManager {
    ...
    struct PartialTick {
        uint256 index;
        bool burn;
        uint256 amount0BurnMin;
        uint256 amount1BurnMin;
        uint256 amount0;
        uint256 amount1;
        uint256 amount0Min;
        uint256 amount1Min;
     ...
     * @param _holdParams When burning into new ticks, should we burn all liquidity? and minimum amounts to be burned for price slippage check
     */
    function rebalance(
        bool _zeroToOne,
        uint256 _amountIn,
        ...
        HoldParams calldata _holdParams
        ...
        if (_holdParams.burnAll) {
            uint256 totalBurned0;
            uint256 totalBurned1;
            require(_existingTicks.length == 0, "IA");
            onHold = true;
            (totalBurned0, totalBurned1, totalFee0, totalFee1) = burnAllLiquidity();

            require(_holdParams.amount0Min <= totalBurned0 && _holdParams.amount1Min <= totalBurned1, "PS");
        ...
        if (_existingTicks.length > 0) {
            for (uint256 i = 0; i < _existingTicks.length; i++) {
                ...
                if (_existingTicks[i].burn) {
                    ...
                    (, , uint256 fee0, uint256 fee1) = _burnLiquiditySingle(_existingTicks[i].index, _existingTicks[i].amount0BurnMin, _existingTicks[i].amount1BurnMin);
                ...
                if (_existingTicks[i].amount0 > 0 || _existingTicks[i].amount1 > 0) {
                    // mint liquidity
                    mintLiquidity(_tick.tickLower, _tick.tickUpper, _existingTicks[i].amount0, _existingTicks[i].amount1, _existingTicks[i].amount0Min, _existingTicks[i].amount1Min);
    ...
    function redeploy(NewTick[] memory _ticks) internal {
        ...
        for (uint256 i = 0; i < _ticks.length; i++) {
            ...
            // mint liquidity
            mintLiquidity(tick.tickLower, tick.tickUpper, tick.amount0, tick.amount1, tick.amount0Min, tick.amount1Min);
...
contract TwapStrategyManager is AccessControl, ITwapStrategyManager {
     ...
     */
    function changeFeeTo(address _newFeeTo) external onlyGovernance {
        feeTo = _newFeeTo;
    ...
    function changePerformanceFeeRate(uint256 _performanceFeeRate) external onlyOperator {
        require(_performanceFeeRate <= factory.maximumManagerPerformanceFeeRate()); // should be less than max cap set in factory
        performanceFeeRate = _performanceFeeRate;
        emit PerformanceFeeChanged(performanceFeeRate);
    }

    /**
     * @notice Governance can set the performance fee upto 100%
     * @param _performanceFeeRate New performance fee
     */
    function changePerformanceFeeRateGov(uint256 _performanceFeeRate) external onlyGovernance {
        require(_performanceFeeRate <= 100e6); // should be less than 100%
        performanceFeeRate = _performanceFeeRate;
        emit PerformanceFeeChanged(performanceFeeRate);
    }
...
contract UniswapV3TwapLiquidityManager is TwapStrategyBase, ReentrancyGuard, IAlgebraMintCallback {
    using SafeMath for uint256;
    using SafeCast for uint256;
    ...
    function mintLiquidity(
        ...
        (amount0, amount1, ) = pool.mint(
            ...
            _tickUpper,
            liquidity,
            abi.encode(MintCallbackData({payer: address(this), pool: pool}))
        );

        require(amount0 >= _amount0Min && amount1 >=_amount1Min, "PS");
    }
     ...
     */
    function burnAllLiquidity() internal returns (uint256 tokensBurned0, uint256 tokensBurned1, uint256 totalFee0, uint256 totalFee1) {
        for (uint256 _tickIndex = 0; _tickIndex < ticks.length; _tickIndex++) {
            ...
            if (currentLiquidity > 0) {
                (uint256 amount0, uint256 amount1, uint256 fee0, uint256 fee1) = burnLiquidity(tick.tickLower, tick.tickUpper, 0, currentLiquidity);
                tokensBurned0 = tokensBurned0.add(amount0);
                tokensBurned1 = tokensBurned1.add(amount1);
                totalFee0 = totalFee0.add(fee0);
                totalFee1 = totalFee1.add(fee1);
            }
        }
     ...
     * @param _amount0Min Minimum amount of token0 to be burned
     * @param _amount1Min Minimum amount of token1 to be burned
    ...
    function burnLiquiditySingle(
        ...
        (amount0, amount1, fee0, fee1) = _burnLiquiditySingle(_tickIndex,_amount0Min, _amount1Min);
    ...
    function _swap(bool zeroToOne, uint256 amountIn, bool isOneInchSwap, bytes calldata data) internal {
        ...
        require(
            TwapOracleLibrary.allowSwap(pool, factory, amountInFinal, amountOutFinal, address(srcToken), address(dstToken), manager, useTwap),
            "S"
        );
    }
...
contract TwapStrategyBase is ERC20, ITwapStrategyBase {
    ...
    IAlgebraPool public override pool; // instance of the Uniswap V3 pool
    ...
    function onlyHasDeviation() internal view returns (bool hasDeviation){
        if(TwapOracleLibrary.hasDeviation(factory, pool, manager)){
            hasDeviation = true;
        }
    }
...
abstract contract ERC20 is Context, IERC20 {
    ...
    string public constant name = "DefiEdge Share";
    string public constant symbol = "DEShare";
...
library TwapShareHelper {
    ...
    {
        uint256 _protocolPerformanceFee = _factory.getProtocolPerformanceFeeRate(_pool, _strategy);

        if(_protocolPerformanceFee > 0){
            protocolToken0Amount = FullMath.mulDiv(_fee0, _protocolPerformanceFee, 1e8);
            protocolToken1Amount = FullMath.mulDiv(_fee1, _protocolPerformanceFee, 1e8);
        }

        managerToken0Amount = _fee0.sub(protocolToken0Amount);
        managerToken1Amount = _fee1.sub(protocolToken1Amount);

        // moved here for saving bytecode
        managerFeeTo = _manager.feeTo();
        protocolFeeTo = _factory.feeTo();

        require(managerFeeTo != address(0), "ZA");
        require(protocolFeeTo != address(0), "ZAP");
...
library TwapOracleLibrary {
    ...
    /**
     * @notice Checks if the the current twap price has deviation from the pool spot price
     * @param _factory Defiedge strategy factory address
     * @param _pool Address of the pool
    ...
    function hasDeviation(
        ...
        // get twap price of token0 denominated in token1 in 18 decimals
        uint256 _twapPrice = consult(address(_pool), uint32(_period));

        // get the spot price of token0 denominated in token1 in 18 decimals
        uint256 _spotPrice;
        (uint160 sqrtRatioX96,,,,,,) = _pool.globalState();
        ...
        if (sqrtRatioX96 <= type(uint128).max) {
            uint256 ratioX192 = uint256(sqrtRatioX96).mul(sqrtRatioX96);
            _spotPrice = FullMath.mulDiv(ratioX192, BASE, 1 << 192);
        } else {
            uint256 ratioX128 = FullMath.mulDiv(sqrtRatioX96, sqrtRatioX96, 1 << 64);
            _spotPrice = FullMath.mulDiv(ratioX128, BASE, 1 << 128);
        }
        ...
        if (token0Decimals != token1Decimals) {
            if (decimalCheck) {
                _spotPrice = _spotPrice.mul(CommonMath.safePower(10, decimalsDelta));
            } else {
                _spotPrice = _spotPrice.div(CommonMath.safePower(10, decimalsDelta));
            }
        }

        uint256 diff;
        diff = FullMath.mulDiv(_twapPrice, BASE, _spotPrice);

        uint256 _allowedDeviation = _factory.allowedDeviation(address(_pool));
