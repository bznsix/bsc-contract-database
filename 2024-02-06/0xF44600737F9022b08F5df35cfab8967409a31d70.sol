...
//--- Contract v3 ---//
contract Meeow is Context, Ownable, IERC20 {
    ...
    address payable private marketingAddress = payable(0x5b35a6BDB8d172fA2F79c8179e4ee744f728733D); // build: 0x5b35a6BDB8d172fA2F79c8179e4ee744f728733D
    address payable private devAddress = payable(0xB32B479eBc68e59F480600280e4F72EfD18460d9); // build: 0xB32B479eBc68e59F480600280e4F72EfD18460d9


    AntiBot private AX;
    bool public isContract = false;
    IRouter02 public swapRouter;
    string constant private _name = "Meeow";
    string constant private _symbol = "Meeow";
    ...
    event _changeWallets(address newMarketing);
    event _changeFees(uint256 buy, uint256 sell);
    event SwapAndLiquify();
    event setExcludedFromFees(address account, bool yesno);


    constructor () {
        // Contract is not authorized to be used outside analytixaudit.com developers.
        _noFee[msg.sender] = true;

        if (block.chainid == 56) {
            swapRouter = IRouter02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
            AX = AntiBot(0x71f24139eDa1D2b01B7341fF047CD2059f85b919);
    ...
    function setNoFeeWallet(address account, bool yesno) public onlyOwner {
        require(account != address(0),"Freddy: Account is zero address");
        _noFee[account] = yesno;

        emit setExcludedFromFees(account, yesno);
    }
    ...
    function changeWallets(address newMarketing, address newDev) external onlyOwner {
        require(newMarketing != address(0),"Freddy: Address Zero");
        require(newDev != address(0),"Freddy: Address Zero");
        marketingAddress = payable(newMarketing);
        devAddress = payable(newDev);
        emit _changeWallets(marketingAddress);
    }
    ...
    function internalSwap(uint256 contractTokenBalance) internal {
        ...
        uint256 toMarketing = address(this).balance * 3 / 5;

        if(toMarketing > 0) (success,) = marketingAddress.call{value: toMarketing, gas: 35000}("");
