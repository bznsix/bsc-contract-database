...
contract Zilong is ERC20, Ownable, ReentrancyGuard {
    ...
    event UpdateWhitelist(address account, bool isWhitelisted);
    event UpdateMaxWalletAmount(uint256 newAmount);
    event ExcludeFromMaxWalletAmount(address account, bool isExcluded);
    ...
    constructor() ERC20("ZILONG", "ZL") {
        ...
        marketingWallet = 0xc6F15871efdf06D126caAC7a641D28443431A9da;
        treasuryWallet = 0x40225aF6eFec24A4f16b0Dfa04842BD29a4991B1;

        marketingBuyFee = 10;
        marketingSellFee = 10;
        marketingTransferFee = 10;

        treasuryBuyFee = 10;
        treasurySellFee = 10;
        treasuryTransferFee = 10;
        ...
        reflectionBuyFee = 10;
        reflectionSellFee = 10;
        reflectionTransferFee = 15;
        ...
        _isExcludedFromMaxWalletAmount[uniswapV2Pair] = true;

        _isExcludedFromMaxWalletAmount[owner()] = true;
        _isExcludedFromMaxWalletAmount[DEAD] = true;
        _isExcludedFromMaxWalletAmount[address(this)] = true;
        _isExcludedFromMaxWalletAmount[marketingWallet] = true;
        _isExcludedFromMaxWalletAmount[treasuryWallet] = true;
        _isExcludedFromMaxWalletAmount[_getRouterAddress()] = true;
        _isExcludedFromMaxWalletAmount[uniswapV2Pair] = true;

        _whitelist[owner()] = true;
        _whitelist[DEAD] = true;
        _whitelist[address(this)] = true;
        _whitelist[marketingWallet] = true;
        _whitelist[treasuryWallet] = true;
        _whitelist[_getRouterAddress()] = true;

        reflectionToken = _getUSDTAddress();
    ...
    function excludeFromWhitelist(
        ...
        _excludeFromWhitelist(account, excluded);
    }

    function excludeFromWhitelistBatch(
        address[] calldata accounts,
        bool excluded
    ) external onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            _excludeFromWhitelist(accounts[i], excluded);
        }
    }


    function isExcludeFromWhitelist(address account) public view returns (bool) {
    ...
    function excludeFromMaxWalletAmount(address account, bool excluded) external onlyOwner {
    ...
    function isExcludedFromMaxWalletAmount(address account) public view returns (bool) {
    ...
    function updateMaxTxAmount(uint256 newAmount) external onlyOwner {
        require(newAmount >= totalSupply() * 10 / 100000, "MaxTxAmount must be greater than 0.01% of total supply");
    ...
    function updateMaxWalletAmount(uint256 newAmount) external onlyOwner {
        require(newAmount >= totalSupply() * 10 / 1000, "MaxWalletAmount must be greater than 0.01% of total supply");
        maxWalletAmount = newAmount;
        emit UpdateMaxWalletAmount(newAmount);
    }
    ...
    function _initializeMainnet() internal {
        ...
        if(id == 56 || id == 1){
            transferOwnership(0x79d719D62D35a4E3437714F557Fb7DEbEf31d915);
        }
    }
    ...
    function _getUSDTAddress() internal view returns (address) {
        uint256 id;
        assembly {
            id := chainid()
        }
        if (id == 97) return 0xaB1a4d4f1D656d2450692D237fdD6C7f9146e814;
        else if (id == 56) return 0x55d398326f99059fF775485246999027B3197955;
        else if (id == 1) return 0xdAC17F958D2ee523a2206206994597C13D831ec7;
        else return 0xdAC17F958D2ee523a2206206994597C13D831ec7;
    }

    function _excludeFromWhitelist(address account, bool excluded) internal {
        require(_whitelist[account] != excluded, "Account is already the value of 'excluded'");
        require(account != address(uniswapV2Router), "Cannot whitelist router address");
        require(account != uniswapV2Pair, "Cannot whitelist pair address");
        require(account != address(this), "Cannot whitelist contract address");
        require(account != owner(), "Cannot whitelist owner address");
        require(account != DEAD, "Cannot whitelist dead address");
        require(account != ZERO, "Cannot whitelist zero address");
        _whitelist[account] = excluded;

        emit UpdateWhitelist(account, excluded);
    }
    ...
    function _swapFeeToUSDT(uint256 tokenAmount, address receiver) internal {
        if(allowance(address(this),address(uniswapV2Router)) < tokenAmount){
            _approve(address(this), address(uniswapV2Router), type(uint256).max);
        }
    ...
    function _swapback(uint256 tokenAmount) internal nonReentrant {
        ...
        totalPercentDistribution -= (liquidityBuyFee+liquiditySellFee+liquidityTransferFee);
        if(totalPercentDistribution == 0 || tokenAmount == 0) return ;

        uint256 amountForMarketing = (tokenAmount * (marketingBuyFee+marketingSellFee+marketingTransferFee)) / totalPercentDistribution;
        if(amountForMarketing > 0){
            _swapFeeToUSDT(amountForMarketing, marketingWallet);
            tokenAmount -= amountForMarketing;
        }

        uint256 amountForTreasury = (tokenAmount * (treasuryBuyFee+treasurySellFee+treasuryTransferFee)) / totalPercentDistribution;
        if(amountForTreasury > 0){
            _swapFeeToUSDT(amountForTreasury, treasuryWallet);
            tokenAmount -= amountForTreasury;
        }

        // distribution reflection
        if(tokenAmount == 0) return ;
        uint256 amountBeforeReflection = ERC20(_getUSDTAddress()).balanceOf(address(this));
        _swapFeeToUSDT(tokenAmount, address(this));
        uint256 amountForReflection = ERC20(_getUSDTAddress()).balanceOf(address(this)) - amountBeforeReflection;
        if(amountForReflection > 0 && getNumberOfDividendTokenHolders() > 0){
            bool success = SafeERC20NoRevert.safeTransfer(
            ...
            if (success) {
                try dividendTracker.distributeDividends(amountForReflection) {} catch {}
                emit SendDividends(amountForReflection);
            }

        }
    }

    function _sendRewardForMarketing(uint256 amount, address target) internal {
        if(amount > 0){
            _sendETH(payable(target), amount);
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(!_isExcludedFromMaxWalletAmount[to] && balanceOf(to) > maxWalletAmount){
            revert("Wallet amount is exceeding maxWalletAmount");
        }
