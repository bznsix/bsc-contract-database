 ...
 * @param stakedMicros Amount of micro tokens staked by the user.
 */
struct UserInfo {
    uint256 stakedNfts;
    uint256 rewardDebt;
    uint256 unpaidRewards;
    uint256 stakedMicros;
}
...
struct AppStorage {
    ...
    /// @dev Total amount of micro tokens staked in the contract.
    uint256 totalStakedMicros;
    ...
    /// @dev ERC20 token contract representing the micro tokens.
    IERC20 microToken;
...
library LibDiamond {
    ...
    function enforceIsNft() internal view {
        require(
            msg.sender == address(appStorage().nft),
            "LibDiamond: Must be nft"
        );
    }
...
abstract contract RewardDistributorDiamond {
    ...
    fallback() external payable {
        ...
        require(
            facet != address(0),
            "RewardDistributorDiamond: Function does not exist"
        );
...
contract DigitalGoldRewardDistributorDiamond is RewardDistributorDiamond {
    constructor(
