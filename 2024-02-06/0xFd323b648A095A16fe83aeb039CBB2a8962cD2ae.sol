...
contract SmartContToken is Context, ERC20, Ownable {
    ...
    // @dev Token Informations and Supply
    string private constant NAME = "Miner Gold";
    string private constant SYMBOL = "MGOLD";
    uint8 private constant DECIMALS = 18;

    uint256 private constant DECIMALFACTOR = 10 ** uint256(18);
    uint256 private constant MAX = ~uint256(0);
    uint256 private constant _totalSupply = 120 * 10**6 * DECIMALFACTOR;

    // @dev Totals of acumulation of fee
    uint256 private _tokenFeeTotal;
    uint256 private _tokenDevelopmentTotal;
    uint256 private _tokenMarketingTotal;
    uint256 private _tokenGamePoolTotal;

    // @dev Prevent multiple calls
    ...
    constructor(
        ...
        address _GamePoolAddr,
        address Swap_Route_
    ) ERC20(NAME, SYMBOL, DECIMALS) Ownable(msg.sender) {
        marketAddress = _MarketingAddr;
        gamePoolAddress = _DevelopmentAddr;
        developmentAddress = _GamePoolAddr;
        ...
        _isExcludedList[msg.sender] = true;
    ...
    function totalGamePool() public view returns (uint256) {
        return _tokenGamePoolTotal;
    }

    function isIncludedToFeeList(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        if (
            ...
            gamepool_ = buyItems[0].GamePool_Fee;
        } else if (
            // is Sell transaction
            !automatedMarketMakerPairs[sender] &&
            automatedMarketMakerPairs[recipient]
            ...
            gamepool_ = sellItems[0].GamePool_Fee;
        }

        if (
            !automatedMarketMakerPairs[sender] && !automatedMarketMakerPairs[recipient] ||
            _isExcludedList[sender]
        ) {
            _removeAllFee();
        }

        uint256 tokenTransferAmount = _executeFeeTransfer(amount);
    ...
    function _executeFeeTransfer(uint256 tokenAmount) private returns(uint256) {
        (uint256 tokenDevelopment, uint256 tokenMarketing, uint256 tokenGamePool) = _getTransferFee(tokenAmount);
        if (tokenDevelopment > 0) {
            _tokenDevelopmentTotal += tokenDevelopment;
            _balance[developmentAddress] += tokenDevelopment;
        }
        if (tokenMarketing > 0) {
            _tokenMarketingTotal += tokenMarketing;
            _balance[marketAddress] += tokenMarketing;
        }
        if (tokenGamePool > 0){
            _tokenGamePoolTotal += tokenGamePool;
            _balance[gamePoolAddress] += tokenGamePool;
        }
        _tokenFeeTotal = _tokenFeeTotal.add(tokenDevelopment).add(tokenMarketing).add(tokenGamePool);
        emit FeeTransaction(tokenDevelopment, tokenMarketing, tokenGamePool);
        return tokenAmount.sub(tokenDevelopment).sub(tokenMarketing).sub(tokenGamePool);
    }
        ...
        )
    {
        ...
        uint256 tokenGamePool = tokenAmount.mul(gamepool_).div(100);
        return (tokenDevelopment, tokenMarketing, tokenGamePool);
    }

    function _removeAllFee() private {
        if (
            developmentFee_ == 0 && marketingFee_ == 0 && gamepool_ == 0
        ) return;
        developmentFee_ = 0;
        marketingFee_ = 0;
        gamepool_ = 0;
    ...
    function createBuyFee(
        ...
        require((developmentFee.add(marketingFee).add(gamepool)) <= 15, "Total fees should not be more than 15%.");
        uint256 _buyId = 0;
        buyItems[_buyId].Development_Fee = developmentFee;
        buyItems[_buyId].Marketing_Fee = marketingFee;
        buyItems[_buyId].GamePool_Fee = gamepool;
    ...
    function updateBuyFee(
        ...
        buyItems[0].GamePool_Fee = gamepool;
    }
    ...
    function createSellFee(
        ...
        sellItems[_sellId].GamePool_Fee = gamepool;
    ...
    function updateSellFee(
        ...
        sellItems[0].GamePool_Fee = gamepool;
    }
    ...
    /* Function     : Set a new Fee address for Update */
    /* Parameters   : New Address */
    function setGamePoolAdress(address gamePoolAddress_) external onlyOwner {
        require(gamePoolAddress_ != address(0), "ERC20: It is not a valid address");
        require(gamePoolAddress_ != gamePoolAddress, "This is the same GamePool address.");
        gamePoolAddress = gamePoolAddress_;
        emit UpdateAddressFee(gamePoolAddress_);
    }

    /* Function     : Turns ON/OFF User for ExcludedList */
    /* Parameters   : Set 'true' to turn ON and 'false' to turn OFF */
    function setExcludedList(address account) external onlyOwner {
    ...
    event FeeTransaction(uint256 tDevelopment,  uint256 tMarketing, uint256 tGamePool);
