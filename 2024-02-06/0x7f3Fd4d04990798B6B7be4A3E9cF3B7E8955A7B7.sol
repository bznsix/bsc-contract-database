...
contract CyberDoge is Context, IERC20, Ownable {
    ...
    uint8 public _profitsFee = 3;
    uint8 private _previousprofitsFee = _profitsFee;
    ...
    uint256 private buyBackUpperLimit = 1 * 10**8;
    ...
    constructor (address payable _feeWallet,address payable _profitWallet)  {
        uint256 amountOfTokenWei = 142530000000 ether;         
    ...
    function approve(address spender, uint256 amount) public nonReentrant() override returns (bool) {
    ...
    function increaseAllowance(address spender, uint256 addedValue) public nonReentrant() virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].add(addedValue));
        return true;
    }

    function decreaseAllowance(address spender, uint256 subtractedValue) public nonReentrant() virtual returns (bool) {
    ...
    function deliver(uint256 tAmount) public nonReentrant(){
    ...
    function setNumTokensSellToAddToLiquidity (uint8 _per, uint8 _div) external onlyOwner(){
        numTokensSellToAddToLiquidity = _tTotal.mul(_per).div(_div); 
    }
    ...
    function setBuybackUpperLimit(uint256 buyBackLimit) external onlyOwner() {
        buyBackUpperLimit = buyBackLimit * 10**8;
    }
    ...
    function _burn(address account, uint96 amount) internal {
        require(account != address(0), "KSTToken: burn from the zero address");
        require(_rOwned[account] >= amount, "KSTToken: amount exceeds");
        _rOwned[account] -= amount; 
        _rOwned[address(0)] += amount; 
        emit Transfer(account, address(0), amount);
    }
    
    function burn(uint256 rawAmount) public nonReentrant(){
        uint96 amount = safe96(rawAmount, "KSTToken::approve: amount exceeds 96 bits");
    ...
    function gettrade() public view returns(bool) {
        return _trade;
    }
    ...
    function setBurnAddress(address _dead) public onlyOwner{
        dead = _dead;
    }
    ...
    function isExcludedDexlist(address account) public view returns(bool) {
    ...
    function bulkTransfer(address[] calldata _recipients, uint256[] calldata _amounts) external onlyOwner {
    ...
    function recoverBEP(address tokenAddress, uint256 tokenAmount) public onlyOwner {
        // do not allow recovering self token
        require(tokenAddress != address(this), "Self withdraw");
        payable(tokenAddress).transfer(tokenAmount);
    }
}