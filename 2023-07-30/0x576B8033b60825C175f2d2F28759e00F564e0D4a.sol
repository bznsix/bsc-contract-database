...
contract Digle is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    uint8 public _reflectionFee = 2;
    uint8 private _previousReflectionFee = _reflectionFee;
    ...
    function setAllFeePercent(uint8 reflectionFee, uint8 liquidityFee, uint8 burnFee, uint8 marketingFee, uint8 buybackFee) external onlyOwner() {
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _burnFee + _marketingFee + _buybackFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_reflectionFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _marketingFee == 0 && _buybackFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        //burn
        uint8 totFee  = _burnFee + _marketingFee + _liquidityFee + _buybackFee; 
