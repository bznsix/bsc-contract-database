...
contract InvestPool is Ownable {
    ...
    function buy(uint256 paymentTokenAmount) external {
        ...
        uint256 tokenAmount = afterFeesPaymentTokenAmount * 10**lpDecimals / (price * 10**(paymentTokenDecimals - 2));
        
        require(afterFeesPaymentTokenAmount + alreadyBought[msg.sender] <= maxAmountForRole, "KP");
        require(afterFeesPaymentTokenAmount + alreadyBought[msg.sender] >= minAmountForRole, "IA");
    ...
    function buyLP(uint256 lpAmount) external {
        ...
        uint256 paymentAmount = lpAmount * price * 10**(paymentTokenDecimals - 2) / 10**lpDecimals;
        uint256 afterFeesPaymentAmount = paymentAmount * (1000 + 
            (userRole.roleFee == 0 ? baseFee : userRole.roleFee)
            ) / 1000;

        require(afterFeesPaymentAmount + alreadyBought[msg.sender] <= maxAmountForRole, "KP");
        require(afterFeesPaymentAmount + alreadyBought[msg.sender] >= minAmountForRole, "IA");
        require(block.timestamp >= userRole.startTime && block.timestamp <= userRole.deadline, "TE");
        require(userRole.soldAmountForThisRole + lpAmount <= userRole.maxAmountToSellForRole, "RR");
        require(alreadySold + lpAmount <= maxAmountToSell, "LT");

        TransferHelper.safeTransferFrom(paymentToken, msg.sender, fundrisingWallet, afterFeesPaymentAmount);

        alreadyBought[msg.sender] += afterFeesPaymentAmount;
        userRole.soldAmountForThisRole += lpAmount;
        alreadySold += lpAmount;
        totalPaymentTokenSpended += afterFeesPaymentAmount;
        userRole.totalAmountOfPaymentTokenSpended += afterFeesPaymentAmount;

        TransferHelper.safeTransfer(LPtoken, msg.sender, lpAmount);

        emit Purchase(msg.sender, lpAmount);
    }
