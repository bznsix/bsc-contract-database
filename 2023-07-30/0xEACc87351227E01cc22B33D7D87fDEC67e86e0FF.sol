...
contract Terea is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    TereaDividendTracker public dividendTracker;
    ...
    uint256 public cooldowntimer = 0; //default cooldown 60
    ...
    constructor() ERC20("Terea", "Terea") {
        marketingWallet = payable(0xAC44b87e5d65CDCb7457B0052A37f2c46D388888);
        devWallet = payable(0xAC44b87e5d65CDCb7457B0052A37f2c46D388888);
        ...
        dividendTracker = new TereaDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 800; // 0.10%
    ...
    function updateFees(
        ...
        require(totalSellFees <= 35 && totalBuyFees <= 35, "total");
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (!isBuying && !isSelling) {
                ...
                if (limitsInEffect) {
                require(block.number > launchblock + 4,"So sorry. Please try again.");
            ...
            uint256 totalFees = RewardsFee
                .add(liquidityFee + marketingFees + devFees + deadFees);
...
contract TereaDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        TereaContract = Terea(mainContract);
        minimumTokenBalanceForAutoDividends = 500000000000_000000000000000000; // 500.000.000.000 tokens
    ...
    function _transfer(
        ...
        require(false, "TereaDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Terea_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main QuadrillionEth contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(TereaContract);

            uint256 prevBalance = TereaContract.balanceOf(address(this));
            ...
            {
                uint256 received = TereaContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    TereaContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
