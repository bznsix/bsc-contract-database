...
contract YX is ERC20, Ownable {
    ...
    YXDividendTracker public dividendTracker;
    ...
    uint256 private gasPriceLimit = 9 * 1 gwei; 
    ...
    uint256 public cooldowntimer = 30; //default cooldown 60s
    ...
    constructor() ERC20("YXITH", "YX") {
        marketingWallet = payable(0x4D8DBa82e6d05b32e634c4d5de3209eC4bc44f78);
        devWallet = payable(0x43fb5d9F24D222Fc67D80f48A31D150a959e3bAb);
        ...
        dividendTracker = new YXDividendTracker(
            payable(this),
            router,
            0x9426657BDDBfC5085121cbA01C1CDb6E29D332aC,
            "YXTRACKER",
            "YXTRACKER"
        );
        ...
        _isExcludedFromFees[msg.sender] = true;
         ...
         uint256 totalTokenSupply = (420_666_777_286) * (10**18);
    ...
    function updateFees(
        ...
        require(totalSellFees <= 20 && totalBuyFees <= 20, "total fees cannot exceed 20% sell or 20% buy");
...
contract YXDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "YX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "YX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(YXContract);

            uint256 prevBalance = YXContract.balanceOf(address(this));
            ...
            {
                uint256 received = YXContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    YXContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
