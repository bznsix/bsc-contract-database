...
contract FANTOMCLUBGROUP is Context, IERC20, Ownable {
    ...
    uint256 public  liquidityPercent = 1;
    uint256 public  adminFeePercent = 2;
    uint256 public  backupFeePercent = 2;
    
    string constant _name = "FANTOM CLUB GROUP";
    string constant _symbol = "FCG";
    uint8 constant _decimals = 18;

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;
    address public backUpAddress = 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4;
    ...
    function setBackupAddress(address _newBackupAddress) external onlyOwner returns(bool){
        backUpAddress = _newBackupAddress;
        return true;
    }
    
    function updateTokensSellToAddToLiquidity(uint256 _tokensSellToAddToLiquidity) external onlyOwner {
        numTokensSellToAddToLiquidity = _tokensSellToAddToLiquidity;
    }
    function name() external pure returns (string memory) {
        return _name;
    }
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        uint256 tLiquidityAdmin = onePercentRate * adminFeePercent;
        uint256 tLiquidityLiquidity = onePercentRate * liquidityPercent;
        uint256 tLiquidityBackup = onePercentRate * backupFeePercent;
        
        uint256 rLiquidityAdmin = tLiquidityAdmin.mul(currentRate);
        uint256 rLiquidityLiquidity = tLiquidityLiquidity.mul(currentRate);
        uint256 rLiquidityBackup = tLiquidityBackup.mul(currentRate);
        ...
        _tOwned[address(this)] = _tOwned[address(this)].add(
         ...
         _rOwned[owner()] = _rOwned[owner()].add(
            rLiquidityAdmin
        );

        _tOwned[owner()] = _tOwned[owner()].add(
            tLiquidityAdmin
        );

         _rOwned[backUpAddress] = _rOwned[backUpAddress].add(
            rLiquidityBackup
        );

        _tOwned[backUpAddress] = _tOwned[backUpAddress].add(
            tLiquidityBackup
        );
