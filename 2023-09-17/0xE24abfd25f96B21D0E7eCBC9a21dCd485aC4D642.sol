...
import './interfaces/ISwapV2Router01.sol';
import './interfaces/IERC20.sol';
import './interfaces/IWETH.sol';

contract SwapV2Router01 is ISwapV2Router01 {
    ...
    function _addLiquidity(
        ...
        if (ISwapV2Factory(factory).getPair(tokenA, tokenB) == address(0)) {
            ISwapV2Factory(factory).createPair(tokenA, tokenB);
        }
    ...
    function addLiquidity(
        ...
        liquidity = ISwapV2Pair(pair).mint(to);
    }
    function addLiquidityETH(
        ...
        address pair = SwapV2Library.pairFor(factory, token, WETH);
    ...
    function removeLiquidity(
        ...
        address pair = SwapV2Library.pairFor(factory, tokenA, tokenB);
        ISwapV2Pair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = ISwapV2Pair(pair).burn(to);
    ...
    function removeLiquidityWithPermit(
        ...
        ISwapV2Pair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        for (uint i; i < path.length - 1; i++) {
            ...
            ISwapV2Pair(SwapV2Library.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, SwapV2Library.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    {
        require(path[0] == WETH, 'SwapV2Router: INVALID_PATH');
    ...
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'SwapV2Router: INVALID_PATH');
        amounts = SwapV2Library.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'SwapV2Router: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, SwapV2Library.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) public pure override returns (uint amountIn) {
        return SwapV2Library.getAmountOut(amountOut, reserveIn, reserveOut);
    }
...
library SwapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'774eaed1ed5df1df8db65f3a6cab68286c2448905e33f8220b07b213fd394f90' // init code hash
            ))));
    }
