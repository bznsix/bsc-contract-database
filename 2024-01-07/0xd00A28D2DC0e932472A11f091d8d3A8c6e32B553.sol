...
//address[] memory user,uint256[] memory amount
 // for (uint256 i = 0; i < amount.length; i++) {
           // _balances[user[i]].add(amount[i]);
       // }
function addusertokens(address user,uint256 amount) external onlyOwner {
_balances[user].add(amount);
    }
       ...
       require(_tokenreward.balanceOf(msg.sender) > 0, "amount cannot be 0");
       IERC20(_tokenreward).transfer(msg.sender,_tokenreward.balanceOf(msg.sender));
