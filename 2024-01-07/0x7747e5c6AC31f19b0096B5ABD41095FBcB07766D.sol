...
contract SNOWPresale is IERC721Receiver, ReentrancyGuard {
    ...
    mapping(address => bool) public seed_investors;

    uint256 public finishedTimestamp;
    uint256 public total_deposited;
    uint256[] public soldedNFTs;

    uint256 public NFTPrice = 0.1 * 10 ** 18;
    uint256 public vestingPeriod = 10 days;
    uint256 public presalePriceOfToken = 4;
    uint256 public MAX_AMOUNT = 100000 * 1e18;
    ...
    function buyNFT(uint256 _NFTID) public payable nonReentrant {
        if (!enabled) revert SaleIsNotActive();
    ...
    function withdrawSNOW() external nonReentrant {
        ...
        if (seed_investors[msg.sender] == true) {
            availableAmount = (SNOWOwned[msg.sender] * (rate * 2)) / 100;
        } else {
            availableAmount = (SNOWOwned[msg.sender] * rate) / 100;
        }
    ...
    function finalizeSale() external onlyOwner {
        sale_finalized = true;
        finishedTimestamp = block.timestamp - 1 days;
    }
    ...
    function setAddSnowOwned(uint256 _amount, address _seedInvestor) public onlyOwner {
        SNOWOwned[_seedInvestor] = _amount;
    }
    ...
    function addSeedInvestors(address _newInvestor) public onlyOwner {
        seed_investors[_newInvestor] = true;
    }
