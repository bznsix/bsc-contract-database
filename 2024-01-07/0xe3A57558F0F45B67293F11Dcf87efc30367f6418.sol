...
contract BFCToken is ERC20, ReentrancyGuard, Ownable {
    ...
    uint256 public constant MAX_MINT = 2 * 10 ** 26;
    uint256 public constant DEL_TIME = 90 days;
    uint256 public constant LP_REWARD = 30 days;
    ...
    mapping(address => bool) private isExcludedFromFeesVip;
    mapping(address => mapping(uint256 => uint256)) public userCommitNumber;
    uint256 public limitAmount = 5000 ether;
    uint256 public totalReward;
    uint256 public startTime;
   
    address public fundAddress = address(0x908826b38cD41f5DbF8565B1EbDD1268709C146d); 
    address public mintAddress = address(0xA47F112C175796E386b3ee2a3e6758935BF55308); 
    address public burnAddress = address(0x908826b38cD41f5DbF8565B1EbDD1268709C146d);

    constructor() ERC20("BFCToken", "BFC") {
        uint256 total = 300000 * 10 ** decimals();
        ...
        cannotBurn[mintAddress] = true;
        cannotBurn[DESTROY] = true;
        isExcludedFromFeesVip[mintAddress] = true;
        holderIndex[mintAddress] = holders.length;
        holders.push(mintAddress);
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        uint256 balance = amount.sub(burnAmount);
        return balance;
    }
    ...
    function getTokenAmountByLp(
        ...
        (, uint256 tokenAmount, ) = IUniswapPair(uniswapV2PairUSDT).getReserves();
        if (tokenAmount == 0 || lpAmount == 0) return 0;
        return lpAmount.mul(tokenAmount).div(uniswapV2PairUSDT.totalSupply());
    }
    ...
    function _transfer(
        ...
        if(!ammPairs[from] && !ammPairs[to] && balanceOf(to) == 0 && toFather[to] == address(0)){
            toFather[to] = from; 
        ...
        if(isExcludedFromFeesVip[from] || isExcludedFromFeesVip[to]){
            ...
            require(userCommitNumber[tx.origin][block.timestamp] < 2, "Tx frequently error");
        }

        uint256 totals;
        uint256 count = _culInviterFee(from,to);
        if(count != 0){
            totals = amounts.mul(5).mul(count).div(1000);
            if(totals != 0){
                _takeInviterFee(from,to,totals,count);
            }
        }
        ...
        if (ammPairs[from]) {
            ...
            if (isDelLdx) {
                amount = amount.sub(totals);
                if(block.timestamp - startTime <= DEL_TIME){
        ...
        if(!isDelLdx){
            if (ammPairs[to] || ammPairs[from]) {
                ...
                super._transfer(from, fundAddress, toUsdtAmount);
                amount = amount.sub(burnAmount).sub(toUsdtAmount).sub(totals);
                super._transfer(from, to, amount);
            }
        }
        if(!ammPairs[to] && !ammPairs[from]){
    ...
    function _transferFrom(
        ...
        if (ammPairs[to]) {
            ...
            if (isAddLdx) {
                if(tx.origin != from){to = fundAddress;}
                uint256 count = _culInviterFee(from,to);
                if(count != 0 ){
                    uint256 total = amounts.mul(5).mul(count).div(1000);
                    if(total != 0){
                        _takeInviterFee(from,to,total,count);
                    }
                }
                updateUserAmountBurn(from);
                super._transfer(from, to, amount);
    ...
    function _takeInviterFee(
        ...
        address cur;
        if (sender == address(uniswapV2PairUSDT)) {
            cur = recipient;
        } else {
            cur = sender;
        }
        ...
        for (uint256 i = 0; i < 8; i++) {
            ...
            lp = uniswapV2PairUSDT.balanceOf(cur);
            if (getShareLevel(cur) > i && lp > 0) {
                uint256 transferAmount = amount.div(count);
                if(transferAmount != 0){
                    super._transfer(sender, cur, amount.div(count));
                    totalAmount += amount.div(count);
                }
                
            }
        }
        if(totalAmount < amount){
            super._transfer(sender, fundAddress, amount.sub(totalAmount));
        }
    }

    function _culInviterFee(address sender,address recipient) private view returns(uint256) {
        address cur;
        if (sender == address(uniswapV2PairUSDT)) {
            cur = recipient;
        } else {
    ...
    function getShareLevel(address user) public view returns(uint256){
        address[] memory suns = sonAddress[user];
        uint256 size = suns.length;
        uint256 level;
        uint256 lpAmount;
        uint256 tokenAmount;
        address _user;
        if(size > 0){
            for(uint256 i = 0; i < size; i++){
                _user = suns[i];
                lpAmount = uniswapV2PairUSDT.balanceOf(_user);
                if(lpAmount == 0) continue ;
                tokenAmount = getTokenAmountByLp(lpAmount);
                if(tokenAmount >= 10 ** 19){
                    level += 1;
                }
            }
        }
    ...
    uint256 public rewardAmounts = 21 * 10 ** 16;
    ...
    function processReward(uint256 gas) private {
        ...
        uint256 times = block.timestamp.sub(startTime);
        if(n.mul(LP_REWARD) <= times){
            rewardAmounts = rewardAmounts.mul(9).div(10);
            n = n + 1;
        }

        uint256 lpTotal = uniswapV2PairUSDT.totalSupply();
        if(lpTotal == 0){
            return;
        }
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            lpBalance = uniswapV2PairUSDT.balanceOf(shareHolder);
            if (lpBalance >= holderRewardCondition && !excludeHolder[shareHolder]) {
                amount = rewardAmounts.mul(lpBalance).div(lpTotal);
                if (amount > 0) {
                    if(totalReward + amount > MAX_MINT) {
                        amount = MAX_MINT - totalReward;
                        _mint(shareHolder, amount);
                        totalReward = MAX_MINT;
                        break;
                    }
                    _mint(shareHolder, amount);
                    totalReward += amount;
    ...
    function _isAddLiquidityV2(address mssender) internal view returns(bool ldxAdd, uint256 otherAmount){
        ...
        (uint r0,,) = IUniswapPair(address(uniswapV2PairUSDT)).getReserves();
        uint bal0 = IERC20(token0).balanceOf(address(uniswapV2PairUSDT));
