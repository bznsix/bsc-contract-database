...
contract MRT is IERC20, Ownable {
    string constant _name = "Marbles Rush";
    ...
    uint256 public maxBurn = 4 * 10 ** 6 * (10 ** _decimals);
    ...
    constructor() {
        ...
        developmentWallet = 0x88bE694A21132B7886191f7DED2323e769559232;
    ...
    function takeFee(
        ...
        if (recipient == pair) {
            ...
            if (sellBurnFee > 0 && totalBurn < maxBurn)
                burnTokens = (feeToken * sellBurnFee) / sellTotalFee;
        } else if (sender == pair) {
            feeToken = (amount * buyTotalFee) / 100;
            if (buyBurnFee > 0 && totalBurn < maxBurn)
                burnTokens = (feeToken * buyBurnFee) / buyTotalFee;
        }
        if (feeToken > 0) {
            if (burnTokens > 0) {
                if ((totalBurn + burnTokens) > maxBurn) {
                    burnTokens = maxBurn - totalBurn;
                }
    ...
    function doContractSwap() internal swapping {
        ...
        if (contractTokenBalance > 0) {
            ...
            if (swappedTokens > 0) {
                (bool success, ) = developmentWallet.call{value: swappedTokens}(
                    ""
                );

                if (!success)
                    emit ETHTransferFailed(developmentWallet, swappedTokens);
            }
        }
    }
