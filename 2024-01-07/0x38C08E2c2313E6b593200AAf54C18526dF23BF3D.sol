...
contract MasterChef is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (pool.isNFTPool) {
            ...
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
            uint256 SnowReward = multiplier.mul(pool.allocPoint).div(totalAllocPoint);
            SnowToken(Snow).mint(devaddr, SnowReward.div(20));
            SnowToken(Snow).mint(address(this), SnowReward);
            totalDevAlloc += SnowReward.div(20);
    ...
    function depositFor(uint256 _pid, uint256 _amount, address _recipient) external nonReentrant {
        ...
        if (_amount > 0) {
            if (pool.isNFTPool) {
                uint256 id = _amount;
                require(IERC721(pool.lpToken).ownerOf(id) == _sender, "Invalid owner");
                IERC721(pool.lpToken).safeTransferFrom(_sender, address(this), id);
                user.amount = user.amount.add(1);
                user.tokenIds.push(id);
            } else {
                uint256 before = IERC20(pool.lpToken).balanceOf(address(this));
                IERC20(pool.lpToken).safeTransferFrom(address(_sender), address(this), _amount);
                uint256 _after = IERC20(pool.lpToken).balanceOf(address(this));
