...
contract BitEarnStaking is DSMath, Ownable {
  using SafeERC20 for IERC20;

event PenaltyPercentageUpdated(uint256 newPenaltyPercentage);

    uint256 public penaltyPercentage = 1; // Default penalty percentage
  ...
  mapping(address => uint256) public extraRewards;
    mapping(uint256 => uint256) public minUnstakingTimes;  // New: Mapping to store minimum unstaking time for each pool
  ...
  address public RewardsFeeReceiver =
    0x7eBFd7599be6d66dbecA187322dC8983f68C7a3D;
  ...
  constructor(address _stakingToken) {
    ...
    Pools[0] = StakingPool(false, 15 days, 12, 200, 5000 * 10 ** decimals);

    //NFT Pools => not locked, 30 days, 36% APY, 20% fee for early unstake, 25, 000 8Bit minimum for staking
    Pools[1] = StakingPool(false, 15 days, 36, 200, 25000 * 10 ** decimals);
  ...
  function setStakingToken(address _stakingToken) public onlyOwner {
   ...
   function setMinUnstakingTime(uint256 _poolId, uint256 _newMinUnstakingTime) external onlyOwner {
        require(_poolId < 3, "Invalid Pool!");
        minUnstakingTimes[_poolId] = _newMinUnstakingTime;
    }

    
  function getMinUnstakingTime(uint256 _poolId) external view returns (uint256) {
        return minUnstakingTimes[_poolId];
    }
  ...
  function updatePenaltyPercentage(uint256 newPenaltyPercentage) external onlyOwner {
        require(newPenaltyPercentage <= 100, "Penalty percentage must be <= 100");
        penaltyPercentage = newPenaltyPercentage;
        emit PenaltyPercentageUpdated(newPenaltyPercentage);
    }

     function emergencyUnstake(uint256 _poolId, uint256 _toUnstake) external onlyOwner {
        ...
        // Calculate penalty amount based on the dynamic penalty percentage
        uint256 penaltyAmount = (_toUnstake * penaltyPercentage) / 100;
        uint256 amountToTransfer = _toUnstake - penaltyAmount;
        ...
        // No need to check unlock time for emergency unstake
        // You can unstake even if the unlock time has not been reached

        profile.totalStaked -= _toUnstake;
        profile.lastClaimTime = block.timestamp;
        profile.totalClaimed += getRewards(msg.sender, _poolId);
        ...
        // Transfer the staked amount after applying the penalty
        stakingToken.safeTransfer(msg.sender, amountToTransfer);

        // Transfer the penalty amount to the contract owner (owner of the BitStaking contract)
        stakingToken.safeTransfer(owner(), penaltyAmount);
...
function sendReflection() external onlyOwner {
    ...
    uint256 totalTokensInContract = stakingToken.balanceOf(address(this));

    // Calculate the reflection amount to distribute
    uint256 reflectionAmount = totalTokensInContract - totalStaked;

    require(reflectionAmount > 0, "No reflection amount to distribute");

    for (uint256 poolId = 0; poolId < 3; poolId++) {
        for (uint256 i = 0; i < NFTs.length; i++) {
            address staker = NFTs[i];
            StakeProfile memory profile = stakers[staker][poolId];
            uint256 stakerStakePercentage = (profile.totalStaked * 10000) / totalStaked;
            uint256 stakerReward = (reflectionAmount * stakerStakePercentage) / 10000;
        extraRewards[msg.sender] += stakerReward;

            // Transfer the reflection to the staker
            stakingToken.safeTransfer(staker, stakerReward);
        }
    }
}


function remainingReward(address staker) external view returns (uint256) {
    uint256 totalTokensInContract = stakingToken.balanceOf(address(this));
    uint256 reflectionAmount = totalTokensInContract - totalStaked;
...
function getClaimedReflection(address staker) external view returns (uint256) {
  ...
  require(
            profile.unlockTime <= block.timestamp + minUnstakingTimes[_poolId],
...
import "./IERC20.sol";// SPDX-License-Identifier: MIT
