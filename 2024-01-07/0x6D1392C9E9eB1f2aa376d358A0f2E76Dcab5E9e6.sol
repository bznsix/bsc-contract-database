 ...
 * {ERC721Enumerable}.
 */
abstract contract ERC721Upgradeable is Initializable, ContextUpgradeable, ERC165Upgradeable, IERC721, IERC721Metadata {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual {
        // In this contract, an address is not allowed to proccess multiple tokens
        require(balanceOf(to) == 0, "Profile NFT: Single address is not allowed to have multiple tokens"); 
    }
 ...
 * there is an account (an owner)/Manager/Minter that can be granted exclusive access to
 * specific functions.
 *
 ...
 * the owner.
 */
abstract contract OwnableExtUpgradeable is Initializable, ContextUpgradeable {
 ...
 */
abstract contract PausableUpgradeable is Initializable, ContextUpgradeable, OwnableExtUpgradeable {
    /**
...
abstract contract ERC721EnumerableUpgradeable is ERC721PausableUpgradeable, IERC721Enumerable {
    ...
    function tokenOfOwnerTotal(address owner) public view returns (uint256[] memory _ids) {
        uint256 number = ERC721Upgradeable.balanceOf(owner);
...
contract ProfileNFT is ERC721EnumerableUpgradeable {
    ...
    uint256 public _currentNextId;
    mapping(uint256 => mapping (bytes32 => uint256)) internal NFTConfig;

    bytes32[] public _properties;

    function __ERC721Enumerable_init(string memory name_, string memory symbol_, string memory baseUri_) internal initializer {
        __Pausable_init_unchained();
        _baseUri = baseUri_;
    ...
    function initialize(string memory name_, string memory symbol_, string memory baseUri_) public {
        __ERC721Enumerable_init(name_, symbol_, baseUri_);
    }
    ...
    function setProperties(bytes32[] calldata _propertyList) public onlyOwner {
    ...
    function tokenURI(uint256 _id) external view override returns (string memory) {
        ...
        bytes memory temp = abi.encodePacked(_baseUri, "?id=", uint2str(_id), "&");
        for (uint256 i = 0; i < _properties.length; ++i) {
            temp = abi.encodePacked(temp, "a", uint2str(i), "=", uint2str(getSingleProperty(_properties[i], _id)), "&");
        }
        return string(temp);
    }
    ...
    function _mint(address _to) internal returns (uint256){
        ...
        uint256 _id = _currentNextId;
        require(!_exists(_id), "ERC721: token already minted; NextId requires reset");

        // For this contract, a single account is not allowed to process multiple NFT
        _beforeTokenTransfer(address(0), _to, _id);

        _balances[_to] += 1;
        _owners[_id] = _to;

        _currentNextId = _currentNextId.add(1);
        emit Transfer(address(0), _to, _id);

        return _id;
    }

    function setCurrentNextId (uint256 _newNextId) external onlyOwner {
        _currentNextId = _newNextId;
    }
    ...
    function safeBatchMint(address[] calldata _to) external {
        //require(msg.sender == creators[_id]);
