...
contract TokenVesting is Ownable {
    ...
    event DistributionChanged(uint256 initPercent, uint256 stopPercent);
    event TokenAddressChanged(address indexed token);
    event LockChanged(bool locked);
     ...
     */
    constructor(address initialOwner, uint256 initTimestamp, uint256 stopTimestamp, uint8 initPercentValue) Ownable(initialOwner) {
        ...
        emit StartBlockChanged(initTime);
        emit CloseBlockChanged(stopTime);
        emit DistributionChanged(initPercent, uint256(100).sub(initPercent));
    }
