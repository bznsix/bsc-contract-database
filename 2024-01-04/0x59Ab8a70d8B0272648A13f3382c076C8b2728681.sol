...
contract PancakeIFOHelper is Ownable, ReentrancyGuard, Pausable {
    ...
    //IFO related
    IIFO public immutable pancakeIFO;
    ...
    {
        _depositToken = IIFO(pancakeIFO).lpToken();
        _offeredToken = IIFO(pancakeIFO).offeringToken();
    }

    function isSaleStarted() external view returns (bool) {
        return block.timestamp > IIFO(pancakeIFO).startTimestamp();
    }

    function isSaleEnded() external view returns (bool) {
        return block.timestamp > IIFO(pancakeIFO).endTimestamp();
    }
    ...
    function participateInIFO(uint256 _amount) external nonReentrant whenNotPaused {
        ...
        address iCakeAddress = IIFO(pancakeIFO).iCakeAddress();
        uint256 totalLimit = IICake(iCakeAddress).getUserCreditWithIfoAddr(
    ...
    function _calReward(address _account) internal view returns (uint256) {
        ...
        bytes32 _vestingScheduleId = IIFO(pancakeIFO).computeVestingScheduleIdForAddressAndPid(
            address(pancakeStaking),
            pid
        );

        uint256 vestedAmount = IIFO(pancakeIFO).computeReleasableAmount(_vestingScheduleId);
