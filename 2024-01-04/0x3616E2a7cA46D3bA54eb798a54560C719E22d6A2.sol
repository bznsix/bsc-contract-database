...
contract BlackHole is Ownable, ReentrancyGuard {
    ...
    event Upgrade(  uint256 amount);
     constructor(
        ...
        MUSO = IERC20(0xf85F09B93Cb2688cB2664Ad56660A799553Bce7F);
        MUSOL = IERC20(0xf85F09B93Cb2688cB2664Ad56660A799553Bce7F);
        
        router = ISwapRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        pair = IUniswapV2Pair(
            IPancakeFactory(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73).getPair(0xA99f47Eacf07833147ee2ccbFc7b7c54184956fC, 0xf85F09B93Cb2688cB2664Ad56660A799553Bce7F)
        );
    ...
    {
        if (MUSO.balanceOf(msg.sender) < amount) {
            amount = MUSO.balanceOf(msg.sender);
        }
            _sell(amount);
...
// invite邀请
    ...
    function _buy(uint256 amount) private {
        ...
        require(MUSO.transfer(msg.sender, HDAmount*30/32), "transfer error");
        require(
            MUSO.transfer(LPfenhong, (HDAmount * 2) / 32),
            "transfer error"
        );
           ...
           require(
            ...
            emit Buy((HDAmount * 2) / 32);
    }

    function _sell(uint256 amount) private returns (uint256) {
        uint256 HDBalance = MUSO.balanceOf(msg.sender);
        require(HDBalance >= amount, "balance error");

        require(
            MUSO.transferFrom(msg.sender, address(this), amount),
            "transfer error"
        );
        MUSO.destroy(amount);
        ...
        MUSO.transfer(LPfenhong, HDAmount);//进入分红池
...
// 直接上级20%
    ...
    if(user.team.leader!=deadAddress){
        require(
        MUSO.transferFrom(PTAddress, user.team.leader, HDAmount*2/3),
            "transfer error"
        );
    }   
        ...
        while (user.team.top != address(0) && i < 10) {
            ...
            if (user.level >= userLevel) {
                ...
                MUSO.transferFrom(PTAddress, user.team.top, HDAmoun/200);
            }else
            {
