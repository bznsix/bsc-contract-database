...
contract HedgeCoin is ERC20, ERC20Pausable, Ownable {
    ...
    mapping (address => int256) public fee;
    ...
    uint256 public defaultMaxInWallet;
    ...
    constructor(address _ownerToken) ERC20("HedgeCoin", "HDGC") Ownable(_ownerToken) { 
        ...
        q= 1 * 10 ** decimals();
        q_init = q;
        defaultMaxTransaction = 10000 * 10 ** decimals();
        defaultMaxInWallet = 100000 * 10 ** decimals();
    }
    ...
    function leerFee(address _addr) public view returns(uint256 _fee){
    ...
    function readMaxTransaction(address _addr) public view returns(uint256 _value) {
        _value = maxTransaction[_addr];
        if(_value!=0) {
            return(_value);
        } else {
            return(defaultMaxTransaction);
        }
    }

    function readMaxInWallet(address _addr) public view returns(uint256 _value) {
        _value = maxInWallet[_addr];
        if(_value!=0) {
            return(_value);
        } else {
            return(defaultMaxInWallet);
        }
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        uint256 rAmount = tAmount * q/q_init;
        int256 _fee = fee[sender];
        if(_fee == 0) {
            _fee = int256(_tFeeTotal); 
        } else if(_fee < 0){
            _fee = 0;
        }
        uint256 rfee = uint256(_fee) * rAmount /10000;
        uint256 rDev = readDevFee(sender) * rAmount / 10000;
        _rOwned[sender] -= rAmount;
        rAmount -= (rfee + rDev);
        _rOwned[recipient] += rAmount;
        _rTotal -= rfee;
        _rOwned[devWallet] += rDev;
    ...
    function _transferToExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        _tOwned[recipient] +=  tAmount - tfee - tDev; 
        _tn -= tAmount - tfee;
        _te += tAmount - tfee - tDev;
        _rTotal -= rAmount - rDev;
        _rOwned[sender] -= rAmount - rDev;
        _rOwned[devWallet] += rDev;
        q = q_init * _rTotal / _tn; 
        emit Transfer(sender, recipient, tAmount);
    }
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        uint256 tFee = _fee * tAmount /10000;
        uint256 rFee = tFee * q/q_init;
        uint256 rAmount = tAmount * q/q_init;
        uint256 rDev = readDevFee(sender) * rAmount / 10000;
        rAmount -= rFee;
        _te -= tAmount;
        ...
        _rOwned[recipient] += rAmount - rDev;
        _rOwned[devWallet] += rDev;
        q = q_init * _rTotal / _tn;
        emit Transfer(sender, recipient, tAmount);
    }
    ...
    function _transferR(address sender, address recipient, uint256 amount) internal {
        ...
        require(readMaxTransaction(sender)>=amount, "You can't send so many tokens");
        require(readMaxInWallet(recipient)>=balanceOf(recipient), "You can't send so many tokens");
    ...
    function set_tFeeTotal(uint256 _value) external onlyOwner() {
        require(_value<10000,"El fee no puede ser mas del 100%");
        _tFeeTotal = _value;
    }

    function setMaxTransaction(address _addr, uint256 _value) external onlyOwner() {
        maxTransaction[_addr] = _value;
    }

    function setdefaultMaxTransaction(uint256 _value) external onlyOwner() {
        defaultMaxTransaction = _value;
    }

    function setMaxInWallet(address _addr, uint256 _value) external onlyOwner() {
        maxInWallet[_addr] = _value;
    }

    function setdefaultMaxInWallet(uint256 _value) external onlyOwner() {
        defaultMaxInWallet = _value;
    }
    ...
    function setExclutionFees(address addr, bool isExcluded, int256 _fee) external onlyOwner {
        require(_fee<10000,"El fee no puede superar 100%");
        if(_isExcluded[addr]!=isExcluded) {
            ...
            if(isExcluded) {
                _tn -= balance;
                _te += balance;
                _tOwned[addr] = balance;
                _rTotal -= balance * q / q_init;
                ingreso = balance;
            }else {
                _tn += balance;
                _te -= balance;
                uint256 aux_rOwned = balance * q / q_init;
