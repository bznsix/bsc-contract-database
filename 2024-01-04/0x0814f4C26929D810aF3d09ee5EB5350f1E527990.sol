...
contract Calculations is AccessControl {
    ...
    function calcInterest(uint128 lastCalcTime, uint256 payPerSecond) internal view returns(uint256 interesToPay, uint256 timeOfCalc){
    ...
    /// @param _interes interest payable on ten thousand basis.
    function interestForSecond(uint256 _amountDeposit, uint256 _interes) internal pure returns(uint256 payPerSecond){
        if(_amountDeposit == 0){
           return payPerSecond = 0;
        }
        PRBMath.UD60x18 memory amount = PRBMath.UD60x18({value: _amountDeposit});
    ...
    function _getPortion(uint256 amountDeposited, uint256 percentage)internal pure returns (uint256 portion){
        PRBMath.UD60x18 memory amount = PRBMath.UD60x18({value: amountDeposited});
        PRBMath.UD60x18 memory AmountxPercentage = amount.mul(PRBMath.UD60x18({value: percentage}));
        PRBMath.UD60x18 memory finalAmount = AmountxPercentage.div(PRBMath.UD60x18({value: 10000}));
        portion = finalAmount.value; // return number in factor 18
    }
...
interface IStakingControl {
    ...
    function addDeposit(uint256 id, address user, uint amount, uint _payPerSecond) external;

    function addInfo(uint256 id, address wallet) external;

    function updateInteresAccumulated(address user, uint id, uint payPersecond) external;

    function updateRegistry(address user, uint id, uint amount, uint payPersecond) external;

    function updateStaking(uint256 id, address user, uint256 amount, uint payPersecond) external;

    function updateLastTimeClaim(address user, uint256 id, uint256 _timeClaim) external;

    function updateRewards(address user, uint256 id, uint256 amount,  uint256 _timeClaim) external;

    function validateId(address wallet, uint256 id) external view returns (bool _valid);

    function getStakeInfo(address user, uint256 id) external view returns (StakeInfo memory info);

    function getIdInfo(address wallet) external view returns(uint256 _id);

    function getStakedAmount(address user, uint256 id) external view returns(uint256 amount);

    function stakeInterest() external view returns(uint256);

    function getStakinId() external view returns(uint256 _id);

    function addStakingId() external;
...
interface IVaultStaking {
    ...
    function getTotalStaking() external view returns(uint total);
...
import "./IStakingControl.sol";
import "./IDPAToken.sol";

contract StakingFounders is AccessControl, ReentrancyGuard, Calculations {

    bytes32 public constant DEV_ROLE = 0x51b355059847d158e68950419dbcd54fad00bdfd0634c2515a5c533288c7f0a2;

    using SafeERC20 for IERC20;
    using SafeERC20 for IDPAToken;

    IDPAToken public immutable stakingToken;

    bool public paused;

    uint256 private constant MAX_UINT = 2**255;
    uint256 private withdrawTime = 2_628_000; //one month
    uint256 public minimumCalculationAmount = 1000000000000000000; // 1 token
    uint256 public withdrawPercentage = 100; // 1% en base 10 mil

    IStakingControl public stakeControl;
    IVaultStaking public vaultStaking;

    uint256 public rewardsWaitTime;

    mapping(address => bool) public migrateContract;

    event Deposit(uint256 stakingId, address indexed _user, uint256 _amount);
    event UpdatePosition(uint256 stakingId, address indexed _user, uint256 _amount);
    event Withdraw(uint256 stakingId, address indexed _user, uint256 _amount);
    event Rewards(uint256 stakingId, address indexed _user, uint256 _rewards);

    constructor(address _stakingToken, address _stakeControl, address _vaultStaking) {
        _grantRole(DEFAULT_ADMIN_ROLE, 0x4376E0C41B68D63E69863EA5E34fa0bC6602b4Bc);
        _grantRole(DEV_ROLE, 0x4376E0C41B68D63E69863EA5E34fa0bC6602b4Bc);

         _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(DEV_ROLE, msg.sender);
        
        stakingToken = IDPAToken(_stakingToken);
        stakeControl = IStakingControl(_stakeControl);
        vaultStaking = IVaultStaking(_vaultStaking);
        approveVault(_vaultStaking);
    }
    ...
    modifier onlyMigrateContract() {
        if(!migrateContract[msg.sender]){
            revert ("not auth");
        }
        _;
    }
    ...
    function check(uint256 _controlDate) public view returns (bool expired){
        uint256 controlTime;
        block.timestamp > _controlDate ? controlTime = (block.timestamp - _controlDate) : controlTime = (_controlDate - block.timestamp); 
        uint256 newTime = _controlDate + withdrawTime;
        expired = block.timestamp >= newTime ? true : false; 
    }

    function stake(uint amount) public NotPaused nonReentrant  {
        if(amount == 0){
            revert ("amount = 0");
        }

        uint256 _id = stakeControl.getIdInfo(msg.sender);
        ...
        vaultStaking.deposit(amount);

        stakeControl.addStakingId();

        uint256 id = stakeControl.getStakinId();
        uint256 payPersecond = interestForSecond(amount, stakeControl.stakeInterest());
        stakeControl.addInfo(id ,msg.sender);
        stakeControl.addDeposit(id,msg.sender,amount,payPersecond);

        emit Deposit(id, msg.sender,amount);
    }

    function updatePosition(uint256 id, uint256 amount) public NotPaused nonReentrant{    
        uint256 payPersecond;
        ...
        bool state = stakeControl.validateId(msg.sender, id);
        if(!state){
            revert("don't exists");
        }
        ...
        StakeInfo memory info  = stakeControl.getStakeInfo(msg.sender,id);

        stakeControl.updateInteresAccumulated(msg.sender, id, info.payPerSecond);//update rewards before update deposit

        uint256 totalAmount = (info.amount + amount);
        payPersecond = interestForSecond(totalAmount, stakeControl.stakeInterest());
        stakeControl.updateRegistry(msg.sender, id, amount,payPersecond); 

        emit UpdatePosition(id, msg.sender, amount);
    }
    ...
    function withdraw(uint id) public nonReentrant  {
        ...
        bool status = check(info.withdrawTime);

        if(!status){
            revert ("Once a month");
        }

        if(info.amount <= minimumCalculationAmount){
            
            stakeControl.updateInteresAccumulated(msg.sender, id, info.payPerSecond); //update rewards before update withdraw

            stakeControl.updateStaking(id, msg.sender, info.amount, 0);

            vaultStaking.withdraw(info.amount); 
            stakingToken.safeTransfer(msg.sender, info.amount);

            emit Withdraw(id, msg.sender, info.amount);

        }else {

            uint256 amount = getPercentage(info.amount);

            stakeControl.updateInteresAccumulated(msg.sender, id, info.payPerSecond); //update rewards before update withdraw

            uint256 totalAmount = (info.amount - amount);
            uint256 payPersecond = interestForSecond(totalAmount, stakeControl.stakeInterest());
        
            stakeControl.updateStaking(id, msg.sender, amount, payPersecond);
        
            vaultStaking.withdraw(amount); 
    ...
    function getPercentage(uint amount) private view returns(uint256 percentage) {
        percentage = _getPortion(amount, withdrawPercentage);
    }

    function claimReward(uint256 id) external nonReentrant {
        StakeInfo memory info  = stakeControl.getStakeInfo(msg.sender,id);

        if(info.amount == 0 && info.unclaimedRewards == 0){
            revert("no rewards available");
        }

        (uint256 amountRewards, uint256 timeOfCalc) = calcInterest(info.lastCalcTime, info.payPerSecond);
        uint256 totalRewards = amountRewards + info.unclaimedRewards;

        if(totalRewards == 0){  
            revert("not have rewards");
        }

        if((block.timestamp - uint256(info.claimTime)) < rewardsWaitTime){
            revert("You must wait to claim");
        } 

        stakeControl.updateLastTimeClaim(msg.sender, id, timeOfCalc);
        stakeControl.updateRewards(msg.sender, id, totalRewards, timeOfCalc);

        stakingToken.redeem(msg.sender, totalRewards);

        emit Rewards(id, msg.sender, totalRewards);
    }

    function getRewards(uint256 id, address wallet) public view returns(uint256 rewards){
        StakeInfo memory info  = stakeControl.getStakeInfo(wallet,id);

        if (info.amount == 0 && info.unclaimedRewards == 0) {
            rewards = 0;
        } else if (info.amount == 0 && info.unclaimedRewards != 0) {
            rewards = info.unclaimedRewards;
        } else {
            (uint256 amountRewards,) = calcInterest(info.lastCalcTime, info.payPerSecond);
            rewards = amountRewards + info.unclaimedRewards;
        }
    }

    function stakeForSwap(address user, uint amount) public NotPaused nonReentrant onlyMigrateContract {
        if(amount == 0){
            revert ("amount = 0");
        }

        uint256 _id = stakeControl.getIdInfo(user);
        ...
        uint256 id = stakeControl.getStakinId();
        uint256 payPersecond = interestForSecond(amount, stakeControl.stakeInterest());
        stakeControl.addInfo(id ,user);
        stakeControl.addDeposit(id,user,amount,payPersecond);

        emit Deposit(id, user,amount);
    }

    function updatePositionForSwap(address user, uint256 id, uint256 amount) public NotPaused nonReentrant onlyMigrateContract {    
        uint256 payPersecond;
        ...
        bool state = stakeControl.validateId(user, id);
        if(!state){
            revert("don't exists");
        }
        ...
        StakeInfo memory info  = stakeControl.getStakeInfo(user,id);

        stakeControl.updateInteresAccumulated(user, id, info.payPerSecond);//update rewards before update deposit

        uint256 totalAmount = (info.amount + amount);
        payPersecond = interestForSecond(totalAmount, stakeControl.stakeInterest());
        stakeControl.updateRegistry(user, id, amount,payPersecond); 

        emit UpdatePosition(id, user, amount);
    }

    function updateRewardsWaitTime(uint256 _newRewardsWaitTime) public onlyDev {
        if(_newRewardsWaitTime > withdrawTime) {
            revert ("long time");
        }
        rewardsWaitTime = _newRewardsWaitTime;
    }

    function approveVault(address vault) public onlyDev {
       stakingToken.safeApprove(vault, MAX_UINT);
    }

    function updateStakeControl(address newStakeControl) public onlyDev {
        stakeControl = IStakingControl(newStakeControl);
    }

    function updateStakeVault(address newVaultStaking) public onlyDev {
        vaultStaking = IVaultStaking(newVaultStaking);
    }

    function updateWithdrawPercentage(uint newWithdrawPercentage) public onlyDev {
        withdrawPercentage = newWithdrawPercentage;
    }

    function setMigrateContract(address migrateAddress, bool status) public onlyDev {
        migrateContract[migrateAddress] = status;
    }

    function updateWithdrawTime(uint newTime) public onlyDev {
        withdrawTime = newTime;
    }

     function updateMinCalcAmount(uint newMinCalcAmount) public onlyDev {
        minimumCalculationAmount = newMinCalcAmount;
    }

    function totalStaking() public view returns(uint total){
        total = vaultStaking.getTotalStaking();
    }
}
