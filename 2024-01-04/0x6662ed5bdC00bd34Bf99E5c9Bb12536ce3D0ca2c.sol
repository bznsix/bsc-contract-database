...
contract FRENNA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FRENNADividendTracker public dividendTracker;
    ...
    constructor() ERC20("Marketing Wallet Token", "MARKETINGW") {
        marketingWallet = payable(0xc14F9E6AEE0204eA802a9F2CaB99d7c9faE9202e);
        devWallet = payable(0xfee24F75A42AFeBCDEF7bf84E1132dB3D909aEE6);
        address router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
        payoutToken = 0x4c1692141212938E7e262fbF386ef59C7314F827;
        ...
        dividendTracker = new FRENNADividendTracker(
            payable(this),
            router,
            0x4c1692141212938E7e262fbF386ef59C7314F827,
            "RWRDTRACKER",
            "RTRACKER"
        );
        ...
        maxWallet = totalTokenSupply / 50; // 4%
    ...
    function enableStaking(bool enable) private onlyOwner {
        require(stakingEnabled != enable);
        stakingEnabled = enable;
        emit EnableStaking(enable);
    }

    function stake(uint256 duration) private {
...
contract FRENNADividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        FRENNAContract = FRENNA(mainContract);
    ...
    function _transfer(
        ...
        require(false, "FRENNA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "FRENNA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FRENNA contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(FRENNAContract);

            uint256 prevBalance = FRENNAContract.balanceOf(address(this));
            ...
            {
                uint256 received = FRENNAContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    FRENNAContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
