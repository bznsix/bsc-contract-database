    ...
    IMarketplaceSetting
{
    ...
    bool private typePayCom = false; // false is pay com by token, true is pay com by usdt
    ...
    function buyByTokenAndCurrency(uint256 _nftId, uint256 _refCode) external override isAcceptBuyByFlex lock {
        ...
        uint256 valueToken = IOracle(oracleContract).convertUsdBalanceDecimalToTokenDecimal(valueUsdToken);
        ...
        pay(token, payValueTokenWithDecimal);
        // Transfer nft from marketplace to buyer
        IAICrewNFT(nft).safeTransferFrom(address(this), msg.sender, nftId, "");
