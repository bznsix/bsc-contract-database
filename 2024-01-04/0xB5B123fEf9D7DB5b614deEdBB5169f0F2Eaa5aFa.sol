...
contract StakingControl is AccessControl, ReentrancyGuard, Calculations {

    bytes32 public constant DEV_ROLE = 0x51b355059847d158e68950419dbcd54fad00bdfd0634c2515a5c533288c7f0a2;

    mapping(address => bool) stakingAuth;

    mapping(uint256 => address) private walletsRegister; //Keep the staking wallets

    mapping(address => uint256) private idInfo;

    uint256 private totalStakes;

    uint256 public stakeInterest = 1500; // 15% en base 10 mil
    
    uint256 private stakingId = 0; //Keeps track of the number of times staking has been performed
    constructor() {
    ...
    mapping(address =>mapping(uint => StakeInfo)) private stakingBalance;


    function addDeposit(uint256 id, address user, uint amount, uint _payPerSecond) public onlyStaking nonReentrant{
        stakingBalance[user][id].amount += amount;
        stakingBalance[user][id].depositDate = safeCastToUint128(block.timestamp);
        stakingBalance[user][id].lastCalcTime = safeCastToUint128(block.timestamp);
        stakingBalance[user][id].payPerSecond = _payPerSecond;
        stakingBalance[user][id].withdrawTime = safeCastToUint128(block.timestamp);
        ++totalStakes;
        walletsRegister[totalStakes] = user;
    }

    function addInfo(uint256 id, address wallet) public onlyStaking nonReentrant {
        idInfo[wallet]= id;
    }

    function updateInteresAccumulated(address user, uint id, uint payPersecond) public onlyStaking nonReentrant{
        uint256 interesToPay;
        uint256 timeOfCalc;
        uint128 lastCalc = stakingBalance[user][id].lastCalcTime;
            (interesToPay, timeOfCalc) = Calculations.calcInterest(lastCalc, payPersecond);
            stakingBalance[user][id].unclaimedRewards += interesToPay;
            stakingBalance[user][id].lastCalcTime = safeCastToUint128(timeOfCalc);
    }

    function updateRegistry(address user, uint id, uint amount, uint payPersecond) public onlyStaking nonReentrant{
        stakingBalance[user][id].amount += amount;
        stakingBalance[user][id].depositDate = safeCastToUint128(block.timestamp);
        stakingBalance[user][id].payPerSecond = payPersecond;
    }

    function updateStaking(uint256 id, address user, uint256 amount, uint payPersecond) public onlyStaking nonReentrant{
        if(stakingBalance[user][id].amount == amount){
            stakingBalance[user][id].amount = 0;
            stakingBalance[user][id].payPerSecond = 0;
            stakingBalance[user][id].withdrawTime = safeCastToUint128(block.timestamp);
        }else{
            stakingBalance[user][id].amount -= amount;
            stakingBalance[user][id].payPerSecond = payPersecond;
    ...
    function updateLastTimeClaim(address user, uint256 id, uint256 _timeClaim) public onlyStaking nonReentrant {
        stakingBalance[user][id].lastCalcTime = safeCastToUint128(_timeClaim);
    }

    function updateRewards(address user, uint256 id, uint256 amount,  uint256 _timeClaim) public onlyStaking nonReentrant {
        stakingBalance[user][id].unclaimedRewards = 0;
        stakingBalance[user][id].claimed += amount;
        stakingBalance[user][id].claimTime = safeCastToUint128(_timeClaim);
    }
    ...
    function getStakeInfo(address user, uint256 id) public view returns (StakeInfo memory info){
        info = stakingBalance[user][id];
    }
    ...
    function getStakedAmount(address user, uint256 id) public view returns(uint256 amount){
        amount = stakingBalance[user][id].amount;
    }

    function addStakingId() public onlyStaking {
        ++stakingId;
    }

    function getStakinId() public view returns(uint256 _id) {
            _id = stakingId;
    }

    function authorizedStaking(address stakeContract, bool status) public onlyDev {
        stakingAuth[stakeContract] = status;
    }

    ///@dev function to avoid overflow
    function safeCastToUint128(uint256 value) private pure returns(uint128){
        require(value <= type(uint128).max, "overflow 128 bits");
        return uint128(value);
    }

    function updateStakeInterest(uint newInterest) public onlyDev {
        stakeInterest = newInterest;
    }

    function SaveInteresAccumulated(uint start, uint length) public onlyDev {
        uint256 interesToPay;
        uint256 timeOfCalc;

        for (uint256 i = start; i <= length;) {
            (interesToPay, timeOfCalc) = Calculations.calcInterest(stakingBalance[walletsRegister[i]][i].lastCalcTime, stakingBalance[walletsRegister[i]][i].payPerSecond);
            stakingBalance[walletsRegister[i]][i].unclaimedRewards += interesToPay;
            stakingBalance[walletsRegister[i]][i].lastCalcTime = safeCastToUint128(timeOfCalc);
            stakingBalance[walletsRegister[i]][i].payPerSecond = Calculations.interestForSecond(stakingBalance[walletsRegister[i]][i].amount, stakeInterest);
            unchecked {
                ++i;
            }
        }
