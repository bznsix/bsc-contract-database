...
contract GluTechV2 is ReentrancyGuard {
    struct StakingPlan {
    ...
    modifier validPlanId(uint256 planId) {
        require(planId >= 1 && planId <= 4, "Invalid plan ID");
        _;
    }
    ...
    function getAllUserPlansEarnings(
        ...
        uint256 planFourthReward = getRewards(userAddress, 4);

        return planOneReward + planTwoReward + planThreeReward + planFourthReward;
    }
