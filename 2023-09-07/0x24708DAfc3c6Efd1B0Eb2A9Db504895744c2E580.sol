...
contract EarthERC20 is Context, Ownable, IERC20 {
    ...
    uint256 public minsell;  //token

    constructor (address feeAddress_, address lpAddress_, address dropAddress_, address idoAddress_, 
                uint256 idoUsdtAmount_, uint256 lpUsdtAmount_) public {
        _name = "EARTH";
        _symbol = "EARTH";
        _decimals = 18;
        minsell = 10**9 * 10**18;
        ...
        idoAddress = idoAddress_;
        
        _mint(idoAddress_, idoUsdtAmount_ * 10**8);
        _mint(lpAddress_, lpUsdtAmount_ * 10**8);
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (sellAmount >= minsell && !swapping && from != pair) {
            swapping = true;
            _swapTokenForUsdt(sellAmount, feeAddress);
            swapping = false;
        }
        
        if (from == pair || to == pair) {
            uint256 tokenAmount = amount.div(100).mul(1);
            _takeToken(from, tokenAmount);
            amount -= tokenAmount;
        } 
    ...
    function _takeToken(address from, uint256 tokenAmount) private {
        address to = dropAddress;
        uint256 token = tokenAmount.div(10).mul(2);
