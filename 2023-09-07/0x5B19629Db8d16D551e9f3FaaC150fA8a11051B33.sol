...
contract HonorERC20 is IHonorERC20 {
    using SafeMath for uint;

    string public constant name = 'Honor LPs';
    string public constant symbol = 'HNR-LP';
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'HonorSwap: EXPIRED');
...
contract HonorPair is IHonorPair, HonorERC20 {
    ...
    modifier lock() {
        require(unlocked == 1, 'HonorSwap: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
