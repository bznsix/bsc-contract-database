...
contract TGLP is Ownable {
    ...
    mapping(address => mapping(uint256 => uint256)) public subscriptionExtendedAt;

    event SubscriptionExtended(address indexed owner, uint256 id, uint256 price, uint256 timestamp);

    modifier onlyWhitelisted(uint256 id, bytes32[] calldata proof) {
        bytes32 leaf = keccak256(abi.encode(_msgSender(), id));
        require(MerkleProof.verify(proof, rootHash, leaf), "Sender is not whitelisted");
        _;
    }

    constructor(address token_, bytes32 rootHash_) {
        token = IERC20(token_);
        rootHash = rootHash_;
    }
     ...
     */
    function scheduleExtensionOffer(uint256 startTimestamp, uint256 endTimestamp, uint256 price, uint256 seatsAvailable) external onlyOwner {
        require(endTimestamp != 0 || seatsAvailable != 0, "Unlimited extension offer");

        extensionOffer = Offer({ startTimestamp: startTimestamp, endTimestamp: endTimestamp, price: price, seatsAvailable: seatsAvailable, seatsSold: 0 });
    }
    ...
    function extend(uint256 id, bytes32[] calldata proof) external onlyWhitelisted(id, proof) {
        require(extensionOffer.startTimestamp != 0, "Offer not started");
        
        require(subscriptionExtendedAt[_msgSender()][id] == 0, "Subscription already extended");

        bool isInTimeframe = block.timestamp >= extensionOffer.startTimestamp && (extensionOffer.endTimestamp == 0 || block.timestamp <= extensionOffer.endTimestamp);
        ...
        subscriptionExtendedAt[_msgSender()][id] = block.timestamp;

        emit SubscriptionExtended(_msgSender(), id, extensionOffer.price, block.timestamp);
    }
