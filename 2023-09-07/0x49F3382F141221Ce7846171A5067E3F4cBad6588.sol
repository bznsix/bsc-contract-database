...
contract LibriumSwapper is ReentrancyGuard {
    bool public swapEnabled;
    address private owner;
    
    IUniswapV2Router02 router;
    address constant WETH = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address constant LIB = 0x5F2172b64a33f423bC7F0c48c23F7D3008f7e4fe;
    ...
    //To enable the use of the swapper!
    function enableSwap(bool enabled) public {
        require(msg.sender==owner);
        swapEnabled=enabled;
    }
    
    //To withdraw a lost token sent to swapper by mistake!
    function WithdrawlostToken(address lostToken) public {
        require(msg.sender==owner);
        IBEP20 token=IBEP20(lostToken);
    ...
    function buyWithToken(uint amt, IBEP20 token) external {
        require(token.allowance(msg.sender, address(router)) >= amt);
        try
            router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                amt,
                0,
                getPath(address(token), LIB),
                msg.sender,
                block.timestamp
            ) {
