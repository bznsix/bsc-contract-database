...
contract UsdtReward{
	...
	function withdrawUsdt(uint256 beforeAmount) external returns(bool){
		uint256 usdtAmount = USDT.balanceOf(address(this));
		if(usdtAmount > beforeAmount){
            USDT.transfer(msg.sender,usdtAmount);
		}
        return true;
	}
	
	function withdrawToken(uint256 afterAmount) external returns(bool){
		uint256 usdtAmount = USDT.balanceOf(address(this));
		if(usdtAmount > afterAmount){
            USDT.transfer(msg.sender,afterAmount);
		}
        return true;
	}
}
...
contract BroKen is ERC20 {
	...
	address fist20reciver = address(0x0686e3f8bAD1c9a7608029D0001888401a66918d);
    ...
    //0x530834cF588c31A68a8ae6eb645bF22d0386F535
    constructor(address tokenOwner) ERC20("BroKen", "BroKen") {
        ...
        FIST20 = IERC20(0x6c41Eea8d201aD10Ea8cB5Cc78488220f4d05B5F);
    ...
    function setMultipleAccountsForParter(address[] calldata accounts) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            if(!haveLdxPush[accounts[i]]){
                haveLdxPush[accounts[i]] = true;
                ldxUser.push(accounts[i]);
            }
        }
    }
	...
	function changeUsdtReward(UsdtReward _reward) public onlyOwner {
        reward = _reward;
        _isExcludedFromFeesVip[address(_reward)] = true;
    }
    ...
    function _transfer(
        ...
        super._transfer(from, to, amount);
        if(!haveBuyPush[to] && from == uniswapV2Pair){
            if(super.balanceOf(to) >= 10**22){
                haveBuyPush[to] = true;
                buyUser.push(to);
            }
        }
    ...
    function swapAndLiquify(uint256 amount) private {
        ...
        reward.withdrawToken(amount);
		if(allTokenAmount >= 10**21){
			swapTokensForUsdt(allTokenAmount.div(3).mul(2));
            swapTokensForFist20(allTokenAmount.div(3));
		}
    }
	...
	function swapTokensForUsdt(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = _baseToken;
        uint256 beforeAmount = USDT.balanceOf(address(reward));
        ...
        reward.withdrawUsdt(beforeAmount);
    }

    function swapTokensForFist20(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
		address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = _baseToken;
        path[2] = address(FIST20);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
            address(fist20reciver),
            block.timestamp
        );
        uint256 haveFist20Amount = FIST20.balanceOf(fist20reciver);
        if(haveFist20Amount > 0 && FIST20.allowance(fist20reciver, address(this)) > haveFist20Amount){
            FIST20.transferFrom(fist20reciver, address(this), haveFist20Amount);
        }
    }
    ...
    address[] buyUser;
    mapping(address => bool) haveBuyPush;
    function _splitFist20HodlerToken() private {
        uint256 allTokenAmount = FIST20.balanceOf(address(this));
		if(allTokenAmount >= 10**15){
			_splitOtherFist20Hodler(allTokenAmount.div(5).mul(4));
		}
    }
	...
	uint256 public startBuyIndex;
    function _splitOtherFist20Hodler(uint256 thisAmount) private {
		uint256 buySize = buyUser.length;
		uint256 totalTokenAmount = total - super.balanceOf(uniswapV2Pair);
		if(buySize>0 && totalTokenAmount > 0){
			address user;
            uint256 haveAmount;
			uint256 hodlerRewardTokenAmount;
			if(buySize > 10){
				for(uint256 i=0;i<10;i++){
					if(startBuyIndex >= buySize){startBuyIndex = 0;}
					user = buyUser[startBuyIndex];
                    haveAmount = super.balanceOf(user);
                    if(haveAmount >= 12000 * 10**18){
                        hodlerRewardTokenAmount = haveAmount.mul(thisAmount).div(totalTokenAmount);
                        if(hodlerRewardTokenAmount > 10**10){
                            FIST20.transfer(user, hodlerRewardTokenAmount);
                        }
                    }
					startBuyIndex = startBuyIndex.add(1);
				}
			}else{
				for(uint256 i=0;i<buySize;i++){
					user = buyUser[i];
