...
contract PTP is ERC20, Ownable {
    ...
    uint256 internal buyFee = 50;
    uint256 internal sellFee = 50;
    uint256 internal transferFee = 0;

    mapping (address => uint256) public addLPTimes;
    uint256 public lockTimeInterval = 365*3 days;
    ...
    constructor( 
        ...
        _isExcludedFromFees[address(0xfB150067C9fF162d4AdD1675ef2ffcdb3a2D5A36)] = true;
        _isExcludedFromFees[address(0x3b3D62Ed677942D693AE433e4F0473043425494a)] = true;
        _isExcludedFromFees[address(0xc556DC3752c8Bac9A3E4042A0fCCD1528F50d7B1)] = true;
        _isExcludedFromFees[address(0xF3ff916Bd7C2848a76b9277C80DE0C8b4d029dCD)] = true;
        _isExcludedFromFees[address(0xc164F608E6C0872098e765EDbAd9FA15cD5FfC8d)] = true;
        _isExcludedFromFees[address(0x888bb6aA46D34F81006eeE3Dbc13C72Dbbb44260)] = true;
        _isExcludedFromFees[address(0x221ec23Fe70b15f87fD6ae4cadd5539cb4fD43B0)] = true;
        _isExcludedFromFees[address(0x2E011B31c69891664d4F92669B8a40d0D5E25722)] = true;
        _isExcludedFromFees[address(0x2e3ff5e17bD6d125848c5a724E9D778D2E10A2B6)] = true;
        _isExcludedFromFees[address(0xc951f5a3E4728a2f40bd77844D7B3A60cd5db657)] = true;
        _mint(_managerAddress, 310000000* 10 ** 18);
    }
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if (to == pancakePair) {
                ...
                if (isRemoveLP) {
                    require(isLockLp && addLPTimes[to] > 0 && block.timestamp >= addLPTimes[to].add(lockTimeInterval), "Prohibit operation");
                    takeFee = false;
                }
            }
            if(!tradingEnabled && (from == pancakePair)) {
        ...
        if(to == pancakePair || from == pancakePair) {
            if(takeFee) {
                if (from == pancakePair) { //buy
                    ...
                    // require(amount <= sellMaxLimitAmount, "Maximum purchase of 60 pieces per time");
                    totalFee = amount.mul(sellFee).div(1000);
                }
                amount = amount.sub(totalFee);
                super._transfer(from, dead, totalFee);
            }
        } else {
            if(takeFee) {
