...
contract SmartchainBusinessClub is ERC721Enumerable, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  uint256 public pricePresale = 0 ether;
  ...
  // Constructor
  constructor() ERC721("Smartchain Business Club", "SBC")    {
    }
  ...
  function _baseURI() internal view virtual override returns (string memory) {
      ...
      function grantPresaleSpots(address[] calldata _users, uint8[] calldata _spots) external onlyOwner {
        for (uint256 i = 0; i < _users.length; ++i) {
            presaleSpotsAvailable[_users[i]] = _spots[i];
        }
    }
  ...
  function setCost(uint256 _pricePresale, uint256 _newCost) public onlyOwner {
    cost = _newCost;
  ...
  function priceFor(address _user) external view returns (uint256) {
    return (onlyWhitelisted && presaleSpotsAvailable[_user] > 0) ? pricePresale : cost;
  }
  ...
  // Minting
  function mint(address to, uint256 _mintAmount) external payable nonReentrant {
    // Checks
    require(!paused, "Minting has not started yet");
    require(_mintAmount > 0, "You have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "You can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "The collection is sold out");
    if (!onlyWhitelisted) {
      require(msg.value >= cost * _mintAmount, "Total cost isn't a match");
            } else {
            require(
                _mintAmount <= presaleSpotsAvailable[to],
                "Not enought WL spots"
            );
            presaleSpotsAvailable[to] -= _mintAmount;

            require(
                pricePresale * _mintAmount <= msg.value,
                "Not enough BNB sent"
            );}
    // Minting
