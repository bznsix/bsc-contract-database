...
import "./ozeppelin/SafeERC20.sol";
import "./ozeppelin/Pausable.sol";
import "./interfaces/IMasterChefV2.sol";
import "./interfaces/IBoostContract.sol";
import "./interfaces/IVHonor.sol";

contract HonorPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 honorAtLastUserAction; // keep track of honor deposited at the last user action.
    ...
    event NewVHonorContract(address VHonor);
    ...
    /**
     * @notice Constructor
     * @param _token: Honor token contract
     * @param _masterchefV2: MasterChefV2 contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     * @param _operator: address of operator
     * @param _pid: honor pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(honorPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the honor owner address or the operator address.
     */
    modifier onlyOperatorOrHonorOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or honor owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onHonorPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.honorAtLastUserAction;
    ...
    /**
     * @notice Unlock user honor funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrHonorOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Honor Pool.
     * @dev Only possible when contract not paused.
    ...
    function depositOperation(
        ...
        if (VHonor != address(0)) {
            IVHonor(VHonor).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.honorAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Honor Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VHonor != address(0)) {
            IVHonor(VHonor).withdraw(msg.sender);
        }
        ...
        if (user.shares > 0) {
            user.honorAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.honorAtLastUserAction = 0;
        }
    ...
    function harvest() internal {
        uint256 pendingHONOR = masterchefV2.pendingHONOR(honorPoolPID, address(this));
        if (pendingHONOR > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(honorPoolPID, 0);
    ...
    /**
     * @notice Set VHonor Contract address
     * @dev Callable by the contract admin.
     */
    function setVHonorContract(address _VHonor) external onlyAdmin {
        require(_VHonor != address(0), "Cannot be zero address");
        VHonor = _VHonor;
        emit NewVHonorContract(VHonor);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Honor Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingHonorRewards();
     ...
     * @return Returns total pending honor rewards
     */
    function calculateTotalPendingHonorRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingHONOR(honorPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingHonorRewards()) * (1e18)) / totalShares);
    }
