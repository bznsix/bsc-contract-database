...
contract Marketplace is IMarketplace, Ownable, ERC721Holder {
    ...
    modifier notExpireForSelling() {
        require(block.timestamp <= (activeSystemTrading + 3600 * 24 * 365), "MARKETPLACE: EXPIRED TO SELL NFT");
        _;
    }
     ...
     * @dev allow buy NFT by token
     */
    function allowSellNft(bool _activeSellNft) public override onlyOwner {
    ...
    function buyByCurrency(
        ...
        payReferralCommissions(msg.sender, refAddress, commissionByUsd, false);
    ...
    function buyByToken(
        ...
        // Transfer referral commissions & update data
        payReferralCommissions(msg.sender, refAddress, commissionByUsd, true);
        // Fixed the ref data of buyer
        if (possibleChangeReferralData(msg.sender)) {
            updateReferralData(msg.sender, _refCode);
        }
    ...
    function sell(uint256[] memory _nftIds) public override notExpireForSelling {
        ...
        for (uint index = 0; index < _nftIds.length; index++) {
            require(TurboDexNFT(nft).ownerOf(_nftIds[index]) == msg.sender, "MARKETPLACE: ONLY NFT'S OWNER CAN SELL");
            uint256 priceNftUsd = TurboDexNFT(nft).getNftPriceUsd(_nftIds[index]);
            require(priceNftUsd > 0, "MARKETPLACE: WRONG NFT ID TO SELL");
            totalValueUsd += priceNftUsd;
        }
        require(TurboDexNFT(nft).isApprovedForAll(msg.sender, address(this)), "MARKETPLACE: MUST APPROVE FIRST");
        uint256 totalValueUsdWithDecimal = totalValueUsd * (10 ** ERC20(currency).decimals());
        ...
        for (uint index = 0; index < _nftIds.length; index++) {
            try TurboDexNFT(nft).safeTransferFrom(msg.sender, address(this), _nftIds[index], data) {
    ...
    function updateReferralDataByAdmin(address _wallet, uint256 _refCode) external checkOwner {
        userReferralCode[_wallet] = _refCode;
        referralCodeUser[_refCode] = _wallet;
    }
