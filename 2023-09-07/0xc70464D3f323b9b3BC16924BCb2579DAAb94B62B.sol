...
contract MemeSeasonX is ERC20, Ownable {
    ...
    // Addresses exempted from fees and transaction limits
    mapping(address => bool) private isExemptedFromFee;
    ...
    constructor(
        ...
        address _lpWallet
    ) ERC20("Meme Season X", "MemeX") {
        bonusWallet = _bonusWallet;
        marketingWallet = _marketingWallet;
        lpWallet = _lpWallet;
        _mint(_lpWallet, 42000000000 * 10**decimals());
        transferOwnership(_lpWallet);
        _maxTxAmount = 1000 * 10**decimals();
    }

    // Transfers tokens with fee processing
    ...
    // Helper function for token transfer with fee processing
    function _memeTransfer(
        ...
        bool shouldTakeFee = !isExemptedFromFee[sender];
    ...
    // Takes transfer fees and distributes them to respective wallets
        ...
        returns (uint256)
    {
        ...
        _transfer(sender, lpWallet, liquidityFee);
    ...
    // Set fee exemption for a specific address
    function setIsFeeExempt(address holder, bool exempt) external onlyOwner {
        isExemptedFromFee[holder] = exempt;
    }

    // Set transaction limit exemption for a specific address
    ...
    // Check if a transaction exceeds the maximum limit
