...
contract Token is ERC20, ERC20Burnable, StatusList, MktCap {
    ...
    function start(Fees calldata fees_) external onlyOwner trading { 
        fees = fees_;
        //开始时间
    ...
    function airDrop( address[] calldata list, uint amount ) public onlyOwner trading {
    ...
    function takeFee(address from, address to, uint amount) internal {
        ...
        if (t==4 &&  isStatus[to]!=4 && startTime.add(1 days) > block.timestamp) {
    ...
    receive() external payable {}
        function _getReserves() public view returns (uint rOther, uint rThis, uint balanceOther){
    ...
    function _isAddLiquidity(uint amount) internal view returns (uint liquidity) {
    ...
    function _isRemoveLiquidity(uint amount) internal view returns (uint liquidity) {
