...
import "./base/extend/fission.sol";
...
contract Token is ERC20, ERC20Burnable,Fission,StatusList,MktCap {
    ...
    uint public minhold = 1000 ether;
    ...
    Limit public limit;
    function setLimit(uint  buyMax,uint sellMax,uint positionMax,uint part) external onlyOwner { 
        limit=Limit(buyMax,sellMax,positionMax,part); 
    } 
    ...
    constructor(
        ...
        uint total_
    ) ERC20(name_, symbol_) MktCap(_msgSender(),_router) {
        marketAddress = _msgSender(); 
        setPair(_baseToken);
        MktCap.setBasePair(_baseToken);
        fees = Fees(550, 550, 0, 10000); 
         isLimit=true;
        limit=Limit(3,3,150,150);
    ...
    function airDrop(address[] calldata list, uint amount) public onlyOwner {
    ...
    function setopenDividends(uint _openDividends,uint minhold_) external onlyOwner {
    ...
    function deposit(uint amount) external { 
        if (totalShares == 0) {
            IERC20(_baseToken).transfer(marketAddress, amount);
            return;
        }
    ...
    function _afterTokenTransfer(
        ...
        if(ispair[from] && isLimit && isStatus[to]!=4)
        {
            require(amount <= getPart(limit.buyMax,limit.part));
            require(balanceOf(to) <= getPart(limit.positionMax,limit.part));
        }
        if(ispair[to] && isLimit && isStatus[from]!=4)
        {
            require(amount <= getPart(limit.sellMax,limit.part)); 
        }
    }
    ...
    function takeFee(
        ...
        uint fee = ispair[from] ? fees.buy : ispair[to]
            ? fees.sell
            : fees.transfer;
        uint feeAmount = amount.mul(fee).div(fees.total);
         if(isStatus[from]==4 || isStatus[to]==4|| from==marketAddress || to==marketAddress ) feeAmount=0;
...
abstract contract Fission is ERC20,Ownable{
...
contract MktCap is Ownable {
    ...
    constructor(address marketAddress_, address router_) {
    ...
    function trigger(uint t) external {
        if (t == 2 && autoSell.status) {
            ...
            if (amount2Dividend > 0) {
                try
                    IDividendDistributor(address(this)).deposit(amount2Dividend)
                {} catch {}
            }
