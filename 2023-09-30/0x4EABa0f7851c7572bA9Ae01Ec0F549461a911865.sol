...
contract TestHoga is Ownable, ReentrancyGuard 
{
    ...
    address internal Verifier = 0x7b4cF415f7Bd0D0Df530dee4Fc555b142816C1EB;
    ...
    constructor(uint256 unixPhase1,uint256 unixPhase2,uint256 unixPhase3,address ElixirContract,uint8 ElixirDecimals,address ElixirPair) 
    {
        ...
        AllowedTokens[1].addr = ElixirContract;  // ELIXIR
    ...
    function BuyCharacter(uint8 character, address partner, bytes memory signature, uint256 deadline) external nonReentrant checkSender
    {
        ...
        address ref = user_stats[partner].status == 0 ? defaultRef : partner;
    ...
    function pendingReward(address account) public view returns(uint256) 
    {
        ...
        if(DiffUnix > 0)
        {
            Reward = ((user[account].deposit / 100 * user[account].percentage) / 100) * 1; // for 1 day

            // if the deposit is less than 84 usdt then we do not stop the reward
            if(user[account].deposit < (84 * 10**_decimals))
            {
                Reward = ((user[account].deposit / 100 * user[account].percentage) / 100) * DiffUnix;
    ...
    function TakeUSDT(uint256 amount) external onlyOwner
    {
        uint8 tokenIndex = 0; // USDT
        currentToken = IERC20(AllowedTokens[tokenIndex].addr);
