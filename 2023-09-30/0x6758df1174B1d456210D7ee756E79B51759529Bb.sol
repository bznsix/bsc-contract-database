...
contract GreenBlockConversor is Ownable, ReentrancyGuard, Pausable {
    ...
    //---------- Events -----------//
    event Distributed(uint256 pool);

    //---------- Constructor ----------//
    constructor(IGreenBlockPool pool, IERC20 weth, IRouterV2 swapRouter) {
        WBNB = swapRouter.WETH();
        POOL = pool;
        WETH = weth;
        SWAP_ROUTER = swapRouter;
        minDistribution = 0.01 ether;
    }
    ...
    function distribute() external nonReentrant {
        ...
        require(ethAmount >= 1000, "Eth too low");
        _performDistribution(ethAmount);
    }
    ...
    function withdrawnBnb(uint256 _amount) external onlyOwner {
