...
contract TunaContractV2 is Ownable, ReentrancyGuard { 
    ...
    TunaV1Smart internal TunaInterface;
    ...
    uint256[3] internal AMOUNT_TUNA = [0, 100, 200];
    ...
    {
        TunaInterface = TunaV1Smart(TunaV1);
    ...
    function Deposit(uint amount, address partner, uint plan) external nonReentrant 
    {
        ...
        if (user[_msgSender()].earned == 0) 
        {
            ...
            if(totalInvestors <= 1000)
            {
                ...
                uint256 needSend = 10 * 10**_decimals;
    ...
    {
        uint256 Completed = (block.timestamp - user[account].timestamp) / (86400 * 3);
    ...
    function _updateLimits() internal
    {
        ...
        {
            daysWork++;
            MAX_DEPOSIT = MAX_DEPOSIT * ((daysWork / 3) * 1500) / 1000;
