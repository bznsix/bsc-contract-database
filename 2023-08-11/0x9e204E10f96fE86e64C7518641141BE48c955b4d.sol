...
// contracts
import "../interfaces/pancake-v3/IPancakeV3Pool.sol";
import "../interfaces/pancake-v3/IPancakeV3Factory.sol";
...
contract DefiEdgeTwapStrategy is UniswapV3TwapLiquidityManager {
     ...
     * @param _oneInchRouter Address of the swap router
...
// libraries
import "../../interfaces/pancake-v3/callback/IPancakeV3MintCallback.sol";
import "@openzeppelin/contracts/token/ERC20/SafeERC20.sol";
import "@openzeppelin/contracts/utils/ReentrancyGuard.sol";

contract UniswapV3TwapLiquidityManager is TwapStrategyBase, ReentrancyGuard, IPancakeV3MintCallback {
    using SafeMath for uint256;
    ...
    /**
     * @dev Callback for Pancake V3 pool.
     */
    function pancakeV3MintCallback(
        ...
        // check if the callback is received from Pancake V3 Pool
        ...
        )
    {
        ...
        for (uint256 i = 0; i < ticks.length; i++) {
            ...
            if (_includeFee && currentLiquidity > 0) {
                // update fees earned in Pancake pool
                // Pancake Pool recalculates the fees and updates the variables when amount is passed as 0
...
contract TwapStrategyBase is ERC20, ITwapStrategyBase {
    ...
    IPancakeV3Pool public override pool; // instance of the Pancake V3 pool

    IERC20 internal token0;
    IERC20 internal token1;

    IOneInchRouter public override oneInchRouter; // instance of the Swap Router
...
library OneInchHelper {
        ...
        )
    {
        ...
        if (selector == 0x7c025200) {
            ...
            address tokenIn = zeroForOne ? IPancakeV3Pool(_pool).token0() : IPancakeV3Pool(_pool).token1();
            srcToken = IERC20(tokenIn);
            dstToken = srcToken == token0 ? token1 : token0;
        } else {
            revert("IM");
...
library TwapOracleLibrary {
     ...
     */
    function getUniswapPrice(IPancakeV3Pool _pool) internal view returns (uint256 price) {
    ...
    function getTick(address _pool, uint32 _period) internal view returns (int24 timeWeightedAverageTick) {
        ...
        (int56[] memory tickCumulatives, ) = IPancakeV3Pool(_pool).observe(secondAgos);
    ...
    function consult(address _pool, uint32 _period) internal view returns (uint256 price) {
        ...
        uint256 token0Decimals = IERC20Minimal(IPancakeV3Pool(_pool).token0()).decimals();
        uint256 token1Decimals = IERC20Minimal(IPancakeV3Pool(_pool).token1()).decimals();
