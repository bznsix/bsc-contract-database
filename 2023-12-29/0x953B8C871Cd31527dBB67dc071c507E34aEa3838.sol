...
contract ViVe is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ViVeDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ViVe", "ViV") {
        marketingWallet = payable(0x71a118340A56DDeb0d83f680435a578cD0D11863);
        devWallet = payable(0x068181A722cEB830AB12149560912e879d9F9ff6);
        ...
        dividendTracker = new ViVeDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 4000000; // 0.05%
        swapTokensAtAmount = totalTokenSupply / 4000000; // 0.05%;
...
contract ViVeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        ViVeContract = ViVe(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ViVe_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ViVe_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ViVeToken contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ViVeContract);

            uint256 prevBalance = ViVeContract.balanceOf(address(this));
            ...
            {
                uint256 received = ViVeContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ViVeContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
