...
contract Token is Context, IBEP20, Ownable {
    ...
    address private _charityWalletAddress = 0xEA61c1ba47CdfEA4daadd16c667CF538C0aA14ab;
	...
	uint256 private numTokensBurn = 500000000000000 * 10**18;
    uint256 public _maxHold = 500000000000000 * 10**18;
	...
	function _takeBurn(uint256 tBurn) private {
        ...
        emit Transfer(address(0), _burnWalletAddress, rBurn);   
    ...
    function _transfer(
        ...
        if(from == pancakeSwapV2Pair && !_isExcludedFromFee[to]){
            require(balanceOf(to)+amount <= _maxHold);
        }
    ...
    function swapForMarketing(uint256 tokens) private {
        ...
        payable(address(_charityWalletAddress)).transfer(WalletAddressBalance);
    }
    ...
    function _swap() private {
        uint256 swapFees = _charityFee;
        uint256 contractTokenBalance = balanceOf(address(this));

        uint256 swapTokens = contractTokenBalance.mul(swapFees).div(_charityFee + _liquidityFee);
        ...
        if (swapAndLiquifyEnabled && _liquidityFee > 0) {
            uint256 liquidityTokens = contractTokenBalance.mul(_liquidityFee).div(_charityFee + _liquidityFee);
