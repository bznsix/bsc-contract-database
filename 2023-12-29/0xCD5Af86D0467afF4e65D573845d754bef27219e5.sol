...
contract BULLPEPE is ERC20, Ownable {
    ...
    address public xmrktdvWallet = 0x3896509Bc45C19FDB6d23bAAecE9E01BeB59427D;
    ...
    constructor() ERC20("BULLPEPE", "BULLPEPE") {
    ...
    function _transfer(
       ...
       if (!exemptFee[sender] && !exemptFee[recipient]) {
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipient])
            ...
            feeswap = sellTaxes.liquidity + sellTaxes.xmrktdv;
            feesum = feeswap;
            currentTaxes = sellTaxes;
        } else if (!useLaunchFee) {
            feeswap = taxes.liquidity + taxes.xmrktdv;
            feesum = feeswap;
            currentTaxes = taxes;
        } else if (useLaunchFee) {
            feeswap = launchtax;
    ...
    function Liquify(uint256 feeswap, Taxes memory swapTaxes) private lockTheSwap {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 xmrktdvAmt = unitBalance * 2 * swapTaxes.xmrktdv;
            if (xmrktdvAmt > 0) {
                payable(xmrktdvWallet).sendValue(xmrktdvAmt);
            }
        }
    }
    ...
    function updatexmrktdvWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0), "Fee Address cannot be zero address");
        xmrktdvWallet = newWallet;
    }
