/*
   ...
   ___   _         _                       _     _            ___         _        
 / __| (_)  _ _  (_)  __ _   _  _   ___  (_)   (_)  ___     / __|  ___  (_)  _ _  
 \__ \ | | | '_| | | / _` | | || | / -_) | |   | | / _ \   | (__  / _ \ | | | ' \ 
 |___/ |_| |_|   |_| \__, |  \_,_| \___| |_|  _/ | \___/    \___| \___/ |_| |_||_|
                     |___/                   |__/                                 
...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    function setRewardTokenAddress(address newToken) external virtual onlyOwner{
...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(uint256 minBalance, address _rewardToken) DividendPayingToken("Reward Tracker", "DividendTracker", _rewardToken) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = minBalance * 1 ** 9;
    }
    ...
    function withdrawDividend() public pure override {
      ...
      function setRewardTokenAddress(address newToken) external override onlyOwner {
      rewardToken = newToken;
    }
...
contract SirigueijoCoin is ERC20, Ownable {


    uint256 private liquidityFeeOnBuy;
    uint256 public SirigueijoCoinFeeOnBuy;
    uint256 public ReawardsFeeOnBuy;

    uint256 private totalBuyFee;

    uint256 private liquidityFeeOnSell;
    uint256 public SirigueijoCoinFeeOnSell;
    uint256 public ReawardsFeeOnSell;

    uint256 private totalSellFee;

    address public SirigueijoCoinWallet;
    ...
    event SirigueijoCoinWalletChanged(address SirigueijoCoinWallet);
    ...
    constructor() payable ERC20("Sirigueijo Coin", "$SirigueijoCoin") {

        rewardToken = 0xD73Cb308AA2E896cFc7836869913FD6EEb10fA15; // Bob-Dollar

        liquidityFeeOnBuy   = 0;
        SirigueijoCoinFeeOnBuy    = 6;
        ReawardsFeeOnBuy     = 4;

        totalBuyFee         = liquidityFeeOnBuy + SirigueijoCoinFeeOnBuy + ReawardsFeeOnBuy;

        liquidityFeeOnSell  = 1;
        SirigueijoCoinFeeOnSell   = 5;
        ReawardsFeeOnSell    = 4;

        totalSellFee        = liquidityFeeOnSell + SirigueijoCoinFeeOnSell + ReawardsFeeOnSell;

        SirigueijoCoinWallet = 0x52370a337Ec7B7EC3fF7527CBebD2e96EF896B03;

         webSite     = "https://sirigueijo-coin.xyz";
         telegram    = "https://t.me/Sirigueijocoin_official";
         twitter     = "https://twitter.com/sirigueijocoin";

        Company       = "https://crypto-solutions.live";
        FullStackDev  = "https://t.me/Italo_Dev_Bsc";

        dividendTracker = new DividendTracker(1e8, rewardToken);
        ...
        _isExcludedFromFees[SirigueijoCoinWallet] = true;
         _isExcludedFromFees[address(0xB018D868988bc5fb77A1B3721D5AC0395C54B1Cb)] = true;

      

    
        _mint(SirigueijoCoinWallet, 2e9 * (10 ** 9));
    ...
    function claimStuckTokens(address token) external {
        ...
        if (token == address(0x0)) {
            sendBNB(payable(SirigueijoCoinWallet), address(this).balance);
            return;
        }
        IERC20 ERC20token = IERC20(token);
        uint256 balance = ERC20token.balanceOf(address(this));
        ERC20token.transfer(SirigueijoCoinWallet, balance);
    }
    ...
    function changeSirigueijoCoinWallet(address _SirigueijoCoinWallet) external onlyOwner {
        require(_SirigueijoCoinWallet != SirigueijoCoinWallet, "Marketing wallet is already that address");
        require(!isContract(_SirigueijoCoinWallet), "Marketing wallet cannot be a contract");
        require(_SirigueijoCoinWallet != DEAD, "Marketing wallet cannot be the zero address");
        SirigueijoCoinWallet = _SirigueijoCoinWallet;
        emit SirigueijoCoinWalletChanged(SirigueijoCoinWallet);
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 bnbShare = (SirigueijoCoinFeeOnBuy + SirigueijoCoinFeeOnSell) + (ReawardsFeeOnBuy + ReawardsFeeOnSell);
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                if((SirigueijoCoinFeeOnBuy + SirigueijoCoinFeeOnSell) > 0) {
                    uint256 SirigueijoCoinBNB = newBalance * (SirigueijoCoinFeeOnBuy + SirigueijoCoinFeeOnSell) / bnbShare;
                    sendBNB(payable(SirigueijoCoinWallet), SirigueijoCoinBNB);
                    emit SendMarketing(SirigueijoCoinBNB);
                }

                if((ReawardsFeeOnBuy + ReawardsFeeOnSell) > 0) {
                    uint256 rewardBNB = newBalance * (ReawardsFeeOnBuy + ReawardsFeeOnSell) / bnbShare;
    ...
    function totalReawardsEarned(address account) public view returns (uint256) {
