...
contract Juicy is ERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 private sellAmount   = 0;
    uint256 private buyAmount    = 0;

    uint256 public totalSellFees;
    uint256 public totalBuyFees;

    JuicyDividendTracker public dividendTracker;
    ...
    uint256 private cooldowntimer   = 15; // Default cooldown 15 seconds
    ...
    event SetWhitelistedWallet(address wallet);
    ...
    constructor() ERC20("Stay Juicy", "$JUICY") {
        ...
        // The transferfee remains always 0% and can't be changed.
        transferFee         = 0;
        ...
        dividendTracker = new JuicyDividendTracker(
            payable(this),
            router,
            0x2170Ed0880ac9A755fd29B2688956BD959F933F8, // Binance-Peg Ethereum Token (ETH) rewards
            "Juicy Rewards Tracker",
            "JuicyTracker"
        );
        ...
        uint256 totalTokenSupply                            = (10_000) * (10**18);
        _mint(owner(), totalTokenSupply); 
        maxWallet                                           = totalTokenSupply / 200; // 0.5% = 50 tokens
        swapTokensAtAmount                                  = totalTokenSupply / 500; // 0.2%; = 20 tokens
    ...
    function setWhitelistedWallet(address wallet) external onlyOwner {
        canTransferBeforeTradingIsEnabled[wallet]   = true;
        _isExcludedFromFees[wallet]                 = true;
        dividendTracker.excludeFromDividends(wallet);
        emit SetWhitelistedWallet(wallet);
    }
    ...
    function setGasPriceLimit(uint256 GWEI) external onlyOwner {
        require(GWEI >= 5, "Can't be set below 5 gwei.");
        gasPriceLimit = GWEI * 1 gwei;
    }

    function setcooldowntimer(uint256 value) external onlyOwner {
        require(value <= 60, "Cooldown timer can't exceed 1 minute.");
        cooldowntimer = value;
    }

    // Max wallet can't be set below 0.25% of the total supply (25 tokens) to prevent a honeypot
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 400, "Max wallet can't be set below 0.25%.");
        maxWallet = value;
    }
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 750000);
    ...
    // The buy and sellfee are limited in range: the buyfee range is between 0-8%, and the sellfee range 0-20.%
        require(totalSellFees <= 20 && totalBuyFees <= 8, "The max buyfee is 8%, and the max selfee 20%. It can't exceed the max percentage.");
    ...
    function _transfer(
        ...
        if (!canTransferBeforeTradingIsEnabled[from]) {
            require(tradingEnabled, "Trading has not yet been enabled yet.");
        }
        if (amount == 0) {
            ...
            if (!isBuying && !isSelling) {
                ...
                require(_holderLastTransferBlock[tx.origin] != block.number, "Too many TX in 1 block.");
                _holderLastTransferBlock[tx.origin] = block.number;
            }
            
            uint256 contractBalanceRecipient = balanceOf(to);
            require(contractBalanceRecipient + amount <= maxWallet,
                    "Exceeds the maximum wallet token amount." );
            }
    ...
    function multiSend(
        ...
        // Max 100 sends in bulk, uint8 in loop limited to 255
        require(
            _contributors.length <= 100,
            "Contributor list length must be <= 100."
        );
...
contract JuicyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    address public defaultToken; // Binance-Peg Ethereum Token (ETH) rewards
    ...
    constructor(
        ...
        defaultToken                        = token;
        JuicyContract                       = Juicy(mainContract);
        minimumTokenBalanceForAutoDividends = 20_000000000000000000; // You must hold 20 $Juicy tokens to be eligible to receive rewards
    ...
    function _transfer(
        ...
        require(false, "Juicy_Dividend_Tracker: No transfers allowed.");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Juicy_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Juicy contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(JuicyContract);

            uint256 prevBalance = JuicyContract.balanceOf(address(this));
            ...
            {
                uint256 received = JuicyContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    JuicyContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
