...
contract LevelBSCLongSLLPManager is Ownable {
  ...
  /**
    ...
    * @param _levelOracle Level Oracle
  */
  constructor(
  ...
  */
  function work(
    ...
    if (_action == ManagerAction.Withdraw) {
      ...
      // restake any remaining token balance
      _addLiquidity(USDT);
      _stake(SLLP);
    }
  ...
  */
  function _removeLiquidity(WorkData calldata _data) internal returns (uint256) {
    ...
    if (_data.token == USDT) {
      ...
      uint256 amtIn = levelOracle.getLLPAmountIn(_data.repayUSDTAmt, SLLP, USDT);
      liquidityRouter.removeLiquidity(
        SLLP, // SLLP tranche address
        USDT,
        amtIn,
  ...
  */
  function unstakeAndTransferLVL() external onlyVault {
