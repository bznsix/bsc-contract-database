...
interface ILevelBSCLongSLLPManager {
  struct WorkData {
    address token;
    uint256 lpAmt;
...
interface ILevelBSCLongSLLPReader {
...
import "../../../interfaces/vaults/level/bsc/ILevelBSCLongSLLPManager.sol";
import "../../../interfaces/vaults/level/bsc/ILevelBSCLongSLLPReader.sol";
// import "../../../interfaces/oracles/IChainlinkOracle.sol";
import "../../../interfaces/tokens/IWETH.sol";
import "../../../interfaces/tokens/ISLLP.sol";
import "../../../enum/ManagerAction.sol";

contract LevelBSCLongSLLPVault is ERC20, Ownable, ReentrancyGuard, Pausable {
  using SafeERC20 for IERC20;

  /* ========== STATE VARIABLES ========== */

  // Manager contract
  ILevelBSCLongSLLPManager public manager;
  // Reader contract
  ILevelBSCLongSLLPReader public reader;
  // Vault config struct
  ...
  */
  ...
  function deposit(address _token, uint256 _amt, uint256 _minSharesAmt) external nonReentrant whenNotPaused {
    ...
    if (_token != SLLP) {
      ...
      // Don't use reader.equityValue() here to avoid computing sllpPrice twice
      // equityBefore = reader.assetValueWithPrice(sllpPrice) - reader.debtValue();
      IERC20(_token).safeTransferFrom(msg.sender, address(manager), _amt);
    } else {
      depositValue = _amt * sllpPrice / SAFE_MULTIPLIER;
      // equityBefore must be checked before transfer of SLLP so as to not include SLLP deposit value
      // equityBefore = reader.assetValueWithPrice(sllpPrice) - reader.debtValue();
      (bool success) = ISLLP(SLLP).transferFrom(msg.sender, address(manager), _amt);
      require(success, "Transfer SLLP failed");
    }

    _deposit(_token, _amt, _minSharesAmt, sllpPrice, depositValue);
  }
  ...
  function depositNative(uint256 _amt, uint256 _minSharesAmt) payable external nonReentrant whenNotPaused {
    ...
    uint256 sllpPrice = reader.sllpPrice(false);
    uint256 depositValue = reader.tokenValue(WBNB, _amt);

    IWETH(WBNB).deposit{ value: msg.value }();
    IERC20(WBNB).safeTransfer(address(manager), _amt);

    _deposit(WBNB, _amt, _minSharesAmt, sllpPrice, depositValue);
  }
  ...
  */
  function withdrawNative(uint256 _shareAmt, uint256 _minWithdrawAmt) external nonReentrant whenNotPaused() {
    // require(block.number != lastDepositBlock, "Withdrawal not allowed in same block as deposit");
    _burnAndWork(WBNB, _shareAmt);

    uint256 withdrawAmt = IERC20(WBNB).balanceOf(address(this));
    require(withdrawAmt >= _minWithdrawAmt, "Assets received less than minimum");

    IWETH(WBNB).withdraw(withdrawAmt);
  ...
  */
  ...
  function _deposit(
    ...
    require(equityBefore + _depositValue <= vaultConfig.maxCapacity, "Exceeded capacity");
    ...
    uint256 borrowTokenAmt = _depositValue
      * (vaultConfig.targetLeverage - 1e18)
      / reader.tokenValue(USDT, 1e18); // note that USDT in BSC has 18 decimals

    ILevelBSCLongSLLPManager.WorkData memory data = ILevelBSCLongSLLPManager.WorkData(
      {
        token: _token,
        lpAmt: 0,
    ...
    uint256 _equityChange = (reader.assetValueWithPrice(_sllpPrice) - reader.debtValue()) - equityBefore;
  ...
  */
  function updateAddress(uint256 _type, address _addr) external onlyOwner {
    if (_type == 1) {
      manager = ILevelBSCLongSLLPManager(_addr);
    }

    if (_type == 2) {
      reader = ILevelBSCLongSLLPReader(_addr);
    }
  ...
  */
  receive() external payable {
    require(msg.sender == WBNB, "fallback function error");
  }
}
