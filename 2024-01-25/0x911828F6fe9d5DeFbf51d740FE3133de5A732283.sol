/**
 *Submitted for verification at BscScan.com on 2024-01-22
*/
...
interface ANGE {
    ...
    function teamsw(address,address)view external   returns(uint,uint,uint);
    function NFTsw(address,address)view external   returns(uint);
}
interface pairs{
...
contract  MktCap{
    ...
    mapping (address=>bool)public Vlist;
    mapping (address=>bool)public Vlist7;
    mapping (address=>uint)public bounsAll;
    uint public boumse;
    address[] public V6;
    address[] public V7;
    address[] public V8;
    ...
    function setVE(address lab,uint vv)public{
    ...
    function senBon(address token,address to,uint amount,uint level)public{
        require(msg.sender == ceo);
        if(level ==3 && !list[to]){
           setMier(token,to,bl[5],amount);
        }
        if(level ==4 && !list[to]){
           setMier(token,to,bl[4],amount);
        }
        if(level ==5 && !list[to]){
           setMier(token,to,bl[3],amount);
           if(users[to].out < bl[101] && !list[ANGEL]){
              if(level==5 && token==WBNB){
                payable (to).transfer(amount*bl[100]/100);
                uint amos=getLv(token,amount,bl[100],5);
                boumse+=amos;
                users[to].out+=amos;
              }else {
        ...
        if(level ==6 && !list[to]){
           setMier(token,to,bl[2],amount);
        }
        if(level ==7 && !list[to]){
           setMier(token,to,bl[1],amount);
        }
    }
    function setMier(address token,address to,uint _bl,uint value)internal {
            if(token==WBNB){
             bounsAll[WBNB]+=value*_bl/100;
             users[to].bnb+=value*_bl/100;
             payable (to).transfer(value*_bl/100);
            }
            if(token==USDT){
              bounsAll[USDT]+=value*_bl/100;
              users[to].usdt+=value*_bl/100;
              IERC20(token).transfer(to,value*_bl/100);
            }
            if(token==ALITA){
                bounsAll[ALITA]+=value*_bl/100;
              users[to].alita+=value*_bl/100;
              IERC20(token).transfer(to,value*_bl/100);
            }
            
    }
    function getLv(address token,uint amount,uint _bl,uint lv)public view returns (uint){
        uint amo;
          if(lv==5 && token==WBNB){
             amo=getPrice(amount*_bl/100);
            }
          if(lv==5 && token==USDT){
             amo=amount*_bl/100;
            }
          if(lv==5 && token==ALITA){
             amo=getPriceToken(ALITA,WBNB,amount*_bl/100);
            }
          if(lv==5 && token==ANGEL){
             amo=getPriceToken(ANGEL,WBNB,amount*_bl/100);
            }
            return amo;
    }
    ...
    function getPriceToken(address token1,address token2,uint bnb) view private  returns(uint){
           address[] memory routePath = new address[](3);
           routePath[0] = token1;
           routePath[1] = token2;
           routePath[2] = USDT;
           return IRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E).getAmountsOut(bnb,routePath)[2];    
    }
    function getAll()public view returns (uint,uint,uint){
        return (bounsAll[WBNB],bounsAll[USDT],bounsAll[ALITA]);
    }
}
...
contract AlitaMiner is Ownable {
    ...
    mapping (address=>address)public updat;
    MktCap public mkt;
    struct UP{
        address dev;
        address ups;
    ...
    function update(address add,address dexx)internal  {
        (,,uint c,uint d)=ANGE(dexx).users(ANGEL,add);
        (uint a1,,uint bbb)=ANGE(dexx).teams(add);
        (address dev,address ups)=ANGE(dexx).upaddress(add);
        users[ANGEL][add].mnu=1;
        users[ANGEL][add].tz=c;
        users[ANGEL][add].out=d;
        stakedOfTime[ANGEL][add]=ANGE(dexx).stakedOfTime(ANGEL,add);
        stakedOf[ANGEL][add][1] = c;
        if(upaddress[add].ups == address(0) && ups != add){
            if(upaddress[0x8767663d5f3382587438Ec3789FEcba415C6324c].ups== address(0)){
               upaddress[0x8767663d5f3382587438Ec3789FEcba415C6324c].ups=0x4C5b24047a038B0B89Dff59731dDC358D76B73A3;
            }
           upaddress[add].ups=ups;
           upaddress[add].dev=dev;
           usersAddr[ups].arrs.push(add);
        }
        (,,uint mn)=ANGE(dexx).teamsw(ups,add);
        teamsw[ups][add].value=mn;
        NFTsw[ups][add]=ANGE(dexx).NFTsw(ups,add);
        teams[add].A1=a1;
        teams[add].value=bbb;
    }
    function skyTema(address[] memory addr,address dexx)public {
        require(msg.sender == auditor);
    ...
    function _team(address a1,address my,uint _value)internal {
        ...
        for(uint k=0;k<200;k++){
               ...
               if(teams[up].lv==teams[up1].lv && mmm==0){
                updat[up1]=up;
                mmm=1;
               }else {
                updat[up1]=address(0);
    ...
    function inTo(address token,address to,uint _amount)internal {
            address up1=upaddress[to].ups;
                uint levels=2;
            for(uint k=0;k<100;k++){  
                if(up1 !=address(0) && teams[up1].lv > levels){
                   mkt.senBon(token, up1, _amount,teams[up1].lv);
                   levels=teams[up1].lv;
                }
                up1=upaddress[up1].ups;
                if(up1 == address(0)){
                  break ;
                }
            }
    ...
    function updateTeam()public {     
        ...
        if(teams[msg.sender].lv>2){
           ...
           mkt.setVE(msg.sender,teams[msg.sender].lv);
        }
        setBl(msg.sender);
              ...
              for(uint i=0;i<addr.length;i++){
                  if(teams[msg.sender].lv == teams[addr[i]].lv){
                     ...
                     if(NFTsw[msg.sender][addr[i]] >0 && NFTsw[msg.sender][addr[i]] <81){
                        ...
                        if(amount >0){
                            if(updat[msg.sender]!=address(0)){
                              uint amou=teamsw[msg.sender][addr[i]].value * 10 /100;
                              IERC20(token).transfer(updat[msg.sender],amou);
                            }
