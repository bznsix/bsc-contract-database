...
contract BTXJStaking {
    ...
    uint256 public dailyInterestRate1 = uint256(9000);
    ...
    function addFundsToReferralRewardPool(uint256 amount) public onlyOwner {
        require(ERC20(address(0x75Eee84ab02E6FECD8e94ba07dc8757289fc6241)).balanceOf(address(this)) >= amount, "Insufficient balance in contract");
    ...
    // 用户购买代币
    function buyTokens(uint256 amountOutMin) public payable {
    ...
    function _stake4(uint256 _stakeAmt) private {
        ...
        // 检查推荐人并更新有效推荐人数
        address referrer = referralRecordMap[msg.sender].referringAddress;
        if (record.stakeAmt == 0 && referrer != address(0) && !referralRecordMap[msg.sender].hasMadeValidReferral) {
            referralRecordMap[referrer].validDirectReferrals += 1;
            referralRecordMap[msg.sender].hasMadeValidReferral = true; // 标记为已产生有效推荐
        }
