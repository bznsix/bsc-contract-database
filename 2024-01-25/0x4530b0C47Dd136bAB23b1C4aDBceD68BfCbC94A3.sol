...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _safeMint(address to) internal virtual {
        _maxtokenId = _maxtokenId + 1;
        _safeMint(to, _maxtokenId, "");
    }
    ...
    function _safeMint(
        ...
        require(_totalSupply <= _maxSupply, "maxSupply");
        _mint(to, tokenId);
        require(
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        require(_totalSupply + 1 <= _maxSupply, "maxSupply");

        _totalSupply = _totalSupply + 1;
        _maxtokenId = tokenId > _maxtokenId ? tokenId : _maxtokenId;
...
contract DADA_NFT is ERC721, Ownable {
    uint256 public mintPrice = 1 ether;
    uint256 public mintStart;
    uint256 public mintEnd;
    address public mintFeeTo = 0x03E285e4958A319dfF38dEa694b1530F914730f2;

    // tokenURIs
    uint256 public _tokenURIsLength;
    mapping (uint256 => string) public _tokenURIs;

    mapping (address => bool) public wl; // whitelist
    mapping (address => bool) public wlMinted; // per wl can mint only once

    constructor() ERC721("DADA_NFT", "DADA_NFT", 400) { }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        uint256 idx = tokenId % _tokenURIsLength;
        return _tokenURIs[idx];
    }

    modifier check() { 
        require(msg.value == mintPrice, "invalid value");
        require(block.timestamp >= mintStart, "Not start");
        require(block.timestamp <= mintEnd, "End");
        require(wl[msg.sender], "just for whitelist"); 
        require(!wlMinted[msg.sender], "per wl can mint only once");
        _;
        wlMinted[msg.sender] = true;
    }

    function mint() public payable check {
        _safeMint(msg.sender);
        TransferHelper.safeTransferETH(mintFeeTo, msg.value);
    }

    function setWL(address[] calldata _wl, bool iswl) public onlyOwner {
        for (uint256 i = 0; i < _wl.length; i++) {
            wl[_wl[i]] = iswl;
        }
    }

    function setMintFeeTo(address _mintFeeTo) public onlyOwner {
    ...
    function setMintTime(uint256 _mintStart, uint256 _mintEnd) public onlyOwner {
        mintStart = _mintStart;
        mintEnd = _mintEnd;
    }

    function setTokenURI(string calldata tokenURI_, uint256 idx) public onlyOwner {
        if (bytes(_tokenURIs[idx]).length == 0) _tokenURIsLength++;
        _tokenURIs[idx] = tokenURI_;
    }
}