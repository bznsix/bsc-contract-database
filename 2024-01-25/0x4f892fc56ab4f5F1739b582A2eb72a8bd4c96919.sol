...
// https://coinmarketcap.com/currencies/bonus-floki/
library SafeMath {
...
contract BonusFlokiZilla is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    mapping (address => uint256) private CREATOR;
    mapping (address => uint256) private CHEFDEV;
    ...
    string private _name = "BonusFlokiZilla";
    string private _symbol = "FloZiB";
    ...
    address public charityWallet = 0x8c05c728Ca714D30BBFF30Af4fD98e062e651336;
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (_isExcluded[account]) return CHEFDEV[account];
        return tokenFromReflection(CREATOR[account]);
    }
    ...
    function deliver(uint256 tAmount) public {
        ...
        CREATOR[sender] = CREATOR[sender].sub(rAmount);
    ...
    function excludeFromReward(address account) public onlyOwner() {
        ...
        if(CREATOR[account] > 0) {
            CHEFDEV[account] = tokenFromReflection(CREATOR[account]);
        }
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        CHEFDEV[sender] = CHEFDEV[sender].sub(tAmount);
        CREATOR[sender] = CREATOR[sender].sub(rAmount);
        CHEFDEV[recipient] = CHEFDEV[recipient].add(tTransferAmount);
        CREATOR[recipient] = CREATOR[recipient].add(rTransferAmount);        
    ...
    function _getCurrentSupply() private view returns(uint256, uint256) {
        ...
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (CREATOR[_excluded[i]] > rSupply || CHEFDEV[_excluded[i]] > tSupply) return (_rTotal, _tTotal);
            rSupply = rSupply.sub(CREATOR[_excluded[i]]);
            tSupply = tSupply.sub(CHEFDEV[_excluded[i]]);
        }
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        uint256 currentRate =  _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        CREATOR[address(this)] = CREATOR[address(this)].add(rLiquidity);
        if(_isExcluded[address(this)])
            CHEFDEV[address(this)] = CHEFDEV[address(this)].add(tLiquidity);
    }
