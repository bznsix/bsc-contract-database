...
contract Token is ERC20 {
    ...
    address payable _fundAddress = payable(address(0x6d724a8D09a42e7A5908099bF9B8439a72dDD564));
    address payable _bnbReciverAddress = payable(address(0x1E433817FBDBDea8a95f433303F0115e2Ea60525));
    ...
    constructor(address tokenOwner) ERC20("$ LXDD", "$ LXDD") {
        ...
        nft = IERC20(0xaD2513e48A93f4344b6397b2d60CAe66c500bc2C);
        ...
        lastDay = block.timestamp.div(daySecond);
    }

    uint256 public lastDay;
    uint256 public oneAmount = 20 * 10**22;
    receive() external payable {
		if(msg.value >= 10**17 && startTime == 0){
            uint256 nowDay = block.timestamp.div(daySecond);
            if(nowDay > lastDay){
                lastDay = nowDay;
                oneAmount = oneAmount.div(10).mul(9);
            }
            super._transfer(_tokenOwner, msg.sender, oneAmount);
            _lockAmount[msg.sender] = _lockAmount[msg.sender].add(oneAmount);
    ...
    function getRemainingAmount(address user) public view returns(uint256){
        uint256 overDay = (block.timestamp.sub(_lockTime[user])).div(daySecond) + 1;
        if(overDay < 100){
            return _lockAmount[user].div(100).mul(100 - overDay);
        }
        return 0;
    }
    ...
    function _transfer(
        ...
        bool isInviter = from != uniswapV2Pair && balanceOf(to) == 0 && perparInviter[to] == address(0) && amount >= 10**10;
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                amount = amount.div(100).mul(96);
				checkNftUser(tx.origin);
                uint256 remainAmount = getRemainingAmount(from);
                require(super.balanceOf(from).sub(amount) >= remainAmount, "remainAmount");
			}else{
                uint256 remainAmount = getRemainingAmount(from);
                require(super.balanceOf(from).sub(amount) >= remainAmount, "remainAmount");
            }
        ...
        if(isInviter){
			...
			bool isEndInviter = from != uniswapV2Pair && perparInviter[from] == to && inviter[from] == address(0) && amount >= 10**10;
			if(isEndInviter){
				inviter[from] = to;
			}
        }
	...
	function _transferFrom(
        ...
        if(startTime == 0 && super.balanceOf(uniswapV2Pair)==0){
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**20){
			swapTokensForEth(allTokenAmount.div(30).mul(25));
			uint256 allBnbAmount = address(this).balance;
			if(allBnbAmount > 10**10){
				_fundAddress.transfer(allBnbAmount.div(25).mul(10));
				addLiquidity(allTokenAmount.div(30).mul(5), allBnbAmount.div(25).mul(5));
			}
		}
    }
    ...
    address defaultInviterUser = address(0x1E433817FBDBDea8a95f433303F0115e2Ea60525);
    uint256[] InviterFee = [6,5,4,3,2];
