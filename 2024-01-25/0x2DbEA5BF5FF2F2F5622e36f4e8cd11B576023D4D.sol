    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (_swapPairList[sender]) {//Buy
                uint256 swapAmount = tAmount * (_buyFundFee + _buyLPFee) / 10000;
                if (swapAmount > 0) {
                    feeAmount += swapAmount;
                    _takeTransfer(sender, address(this), swapAmount);
                }
            } else if (_swapPairList[recipient]) {//Sell
                uint256 swapAmount = tAmount * (_sellFundFee + _sellLPFee) / 10000;
                if (swapAmount > 0) {
                    feeAmount += swapAmount;
                    _takeTransfer(sender, address(this), swapAmount);
                    if (!inSwap) {
...
contract Dragon is AbsToken {
    constructor() AbsToken(
        ...
        18,
        21000000,
        address(0xC6D1614d68Ea05ebfC2c77bfeFA1Be640CF45Cb4),
        address(0xFd0de36832A3680E56873E8F544e0B840d2463e1)
    ){
