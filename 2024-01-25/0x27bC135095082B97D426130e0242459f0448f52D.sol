...
contract Token is ERC20, Ownable {
    ...
    uint256 public pnpFee = 20;
    uint256 public swapTime;
    uint256 public swapBlockNumber;
    mapping(address => bool) _excludedFees;
    uint256 public minBalanceSwapToken = 20 * 10 ** 18;
    uint256 public perMintPrice = 2*10**16;// 0.02 per mint
    bool swapIng;
    ...
    function getBackToken(address _token) external onlyOwner {
    ...
    function _transfer(
        ...
        if (!_excludedFees[from] && !_excludedFees[to]) {
            ...
            if (to == uniswapPair || from == uniswapPair) {
                uint256 _marketingFee = calculateFee(from, to, amount);
                super._transfer(from, fundsPool, _marketingFee);
                fees += _marketingFee;
            }
    ...
    function calculateFee(
        ...
        if (block.timestamp <= swapTime + 15 minutes) {
            //
            if (from == uniswapPair) {
                //buy 5%
        ...
        if (
            block.timestamp > swapTime + 15 minutes &&
            block.timestamp <= swapTime + 1 hours
        ) {
            //
            if (from == uniswapPair) {
                //buy 3%
        ...
        return amount.mul(pnpFee).div(1000); //2%
    }
    ...
    uint256 public percentForLPBurn = 62; // 25 = .25%
    event AutoNukeLP();
    ...
    function _mintToken() internal {
        ...
        require(minter[msg.sender]+msg.value <= 1 ether, "Mint amount limit");
        require(msg.value > 0 && msg.value % perMintPrice == 0, "Mint err amount");

        uint256 amount = msg.value.mul(100 ether).div(perMintPrice);
        _transfer(address(this), msg.sender, amount);
        payable(address(fundsPool)).transfer(msg.value);
        minter[msg.sender] += msg.value;
    }
