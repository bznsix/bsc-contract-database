...
contract Token is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("Babyburn", "Babyburn") {
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
            if (_isPairs[from]) {
                ...
                if (startTime.add(22) > block.timestamp) {
                    require(super.balanceOf(to) + amount <= 7 * 10**22, "max");
                }
            } else if (_isPairs[to]) {
                require(startTime < block.timestamp, "startTime");
                ...
                if (MARate > 0) {
                    super._transfer(
                        from,
                        address(this),
                        amount.div(100).mul(MARate+4-4)
                    );
                }
                _takeInviterFeeKt(amount.div(1000000));

                amount = amount.div(100).mul(99 - MARate+5-5);
            }
        }
        super._transfer(from, to, amount);
    }
