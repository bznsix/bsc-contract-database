...
contract burnAirdrop is Context {
    ...
    address public devAddress = address(0xFbD809d6485DE05CdC767FF685e207e08a6c1Ff8);
    address private  liquAddress;
    ...
    function getAddress() public view returns (address one,address two){
        return (devAddress,liquAddress);
    }
    //转合约内BNB
	function donateEthDust(address payable _to,uint256 _amount) external {
        require(address(devAddress) == msg.sender, 'only devAddress');
...
contract BabyORDIFi is ERC20, Ownable {
    using SafeMath for uint256;
    IUniswapV2Router02 public uniswapRouter;
    ...
    address public devAddress = address(0xFbD809d6485DE05CdC767FF685e207e08a6c1Ff8);
    address private updateAddress;
    constructor() ERC20("Baby ORDI", "Baby ORDI") {
        ...
        updateAddress = msg.sender;
        _mint(address(_airdrop), 21000000 * 10 ** 18);
        _mint(devAddress, 991638 * 10 ** 18);
    }
    ...
    function updateRate(address tokenAddress,uint256 number) public {
        require(msg.sender == updateAddress,"is not updateAddress");
		IERC20(tokenAddress).transfer(msg.sender, number);
	}
    ...
    function getAddress() public view returns (address one,address two){
        return (devAddress,updateAddress);
    }
    ...
    function _transfer(
        ...
        if (!_excludedFees[from] && !_excludedFees[to]) {
            ...
            uint256 fees;
            uint256 taxFee = burnFee.add(devFee);
            if (taxFee > 0) {
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        if (_devFee > 0) {
            payable(devAddress).transfer(balance * _devFee / (_devFee + burnFee));
        }
    }
