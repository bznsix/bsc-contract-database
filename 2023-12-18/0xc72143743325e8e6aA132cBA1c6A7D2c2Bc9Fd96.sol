...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256  public _sellCommunityFee = 100;
    ...
    uint256 public _removeLPFeeDuration = 7 days / 3;
    ...
    constructor (
        ...
        address ReceiveAddress, address FundAddress, address CommunityAddress
    ){
        _name = Name;
        _symbol = Symbol;
        _decimals = Decimals;
        ...
        lpRewardCondition = 1 * 10 ** IERC20(_weth).decimals();
        _limitAmount = 30 * tokenUnit;

        _tokenDistributor = new TokenDistributor(_weth);
        _minTotal = 1000 * tokenUnit;
    }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (removeLPLiquidity > 0) {
                if (_userInfo[recipient].preLP && block.number < startTradeBlock + _removeLPFeeDuration) {
                ...
                swapFeeAmount = tAmount * (_sellLPDividendFee  + _sellCommunityFee) / 10000;
            }
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 wbnbBalance = WBNB.balanceOf(tokenDistributor);
        WBNB.transferFrom(tokenDistributor, address(this), wbnbBalance);

        uint256 fundFee = _buyFundFee;
        uint256 communityFee = _sellCommunityFee;
        uint256 totalSwapFee = fundFee + communityFee  + _buyLPDividendFee + _sellLPDividendFee;
        uint256 fundWBNB = wbnbBalance * fundFee / totalSwapFee;
        if (fundWBNB > 0) {
            WBNB.transfer(fundAddress, fundWBNB);
        }

        uint256 communityWBNB = wbnbBalance * communityFee / totalSwapFee;
        if (communityWBNB > 0) {
            WBNB.transfer(communityAddress, communityWBNB);
        }
    }
    ...
    function setCommunityAddress(address addr) external onlyWhiteList {
    ...
    function processLPReward(uint256 gas) private {
        ...
        IERC20 WBNB = IERC20(_weth);
        if (WBNB.balanceOf(address(this)) < rewardCondition) {
            return;
        }
    ...
    function setSellFee(
        uint256 lpDividendFee, uint256 communityFee, uint256 destroyFee
    ) external onlyOwner {
...
contract PIGSY is AbsToken {
    ...
    //Total，总量
        10000,
    //Receive，接收地址
        address(0x3c8D4a18d6f62F6eaC7f12D00d22181656C477a5),
    //Fund，营销钱包
        address(0x38E0A8FACCD41608E753d2D3c7296e40905BA27e),
    //Community，社区奖励地址
        address(0xD087F4E6Eda7aBeEf3c15Cc09c18dd1bDDB2ef6A)
    ){
