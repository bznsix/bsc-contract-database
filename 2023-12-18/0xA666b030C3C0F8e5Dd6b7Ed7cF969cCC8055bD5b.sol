...
contract FMB is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("FMB", "FMB") {
        require(_baseToken < address(this),"FMB small");
		...
		if(buySize>0 && totalLpAmount > 0){
			...
			if(buySize >20){
				for(uint256 i=0;i<20;i++){
                    ...
                    rate = pair.balanceOf(user).mul(1000000).div(totalLpAmount);
                    if(rate>0){                            
                        USDT.transfer(user, thisAmount.mul(rate).div(1000000));
                    }
