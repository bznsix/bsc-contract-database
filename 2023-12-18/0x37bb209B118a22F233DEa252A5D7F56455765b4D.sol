    ...
    AxelarExecutable
{
    ...
    /**
     * @notice Immutable team address used for setting XEN address.
     */
    address public team;
     ...
     * @param _ratio The ratio between vXEN and XEN used for minting and burning.
     */
    function setXENAndRatio(address _XEN, uint256 _ratio)
        external
        override
    {
        if (msg.sender != team) {
        ...
        if (address(XEN) != address(0)) {
            revert XENIsAlreadySet();
        }
        XEN = IBurnableToken(_XEN);
        RATIO = _ratio;
    }
