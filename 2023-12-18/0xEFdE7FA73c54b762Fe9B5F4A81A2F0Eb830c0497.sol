/** 
 *  SourceUnit: c:\Users\HP\OneDrive\Desktop\self-bridge-contracts\contracts\SelfBridge.sol
*/
...
/**
    ...
    - Add var to keep track of deposited tokens
    - Replace two fee variables with a single variable to simplify the process of keeping track of fees
...
contract SelfBridge is ReentrancyGuard, Ownable, Pausable {
    using SafeERC20 for IERC20;

    /// @dev constants
    string public constant title = "SelfBridge v0.2.1";

    /// @dev Tokens
    IERC20 public immutable selfToken;

    /// @dev Decimals
    uint256 public immutable selfDecimals;
    ...
    /// @dev toal locked tokens
    uint256 public depositedTokens;

    address public admin;

    /// Events
    event TokensOutBound(
    ...
    /// Errors
    // Throw this error when provided token address is Zero i.e address(0)
    error ZeroAddressError();
    //Throw this error when the amount is less than the minimum required for bridging
    error MinAmountError();
    //Throw this error when the amount is greater than the maximum allowed for bridging
    error MaxAmountError();
    //Throw this error when the owner attempts to withdraw a fee, but there is no collected fee available
    error ZeroCollectedFee();
    //Throw this error when the owner attempts to withdraw more tokens than are currently available for withdrawal
    error NotEnoughTokensDeposited();
    ...
    function outBound(uint256 _amount) external whenNotPaused nonReentrant {
        if (_amount < minBridge) revert MinAmountError();
        if (_amount > maxBridge) revert MaxAmountError();
        ...
        /// @dev sum locked tokens
        depositedTokens += (_amount - fee);

        /// @dev sum total fee
        collectedFee += fee;

        // send out event
        emit TokensOutBound(msg.sender, (_amount - fee), fee);
    }
    ...
    function inBound(
        ...
        if (_amount > depositedTokens) revert NotEnoughTokensDeposited();
    ...
    /// @notice this function should only be used to deposit token on ETH for bridging
    ...
    /// @notice this function should only be used in case of emergency
    ...
    function withdraw(uint256 _amount) external onlyOwner {
        ...
        selfToken.transfer(msg.sender, _amount);
    ...
    function transferFee() external onlyOwner {
        uint _collectedFee = collectedFee;
        if (_collectedFee == 0) revert ZeroCollectedFee();

        /// @dev update new saldo for fee transferred
        collectedFee = 0;

        /// @dev direct transfer by contract
        selfToken.transfer(msg.sender, _collectedFee);

        // send out event
        emit FeeTransferred(_collectedFee);
    }
    ...
    function setBridgeFee(uint256 _newBridgeFee) external onlyOwner {
