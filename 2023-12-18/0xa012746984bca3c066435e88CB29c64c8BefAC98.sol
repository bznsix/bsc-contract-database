...
contract MPT2 is ERC20Burnable, OperationWithSigner {
    using ECDSA for bytes32;

    uint8 constant _DECIMALS = 9;
    uint256 constant _GWEI = 10**_DECIMALS;
    uint256 constant _TOTAL_SUPPLY = 100000 * _GWEI;
    ...
    uint256[] private _lpIncomePercent = [30, 3, 3];
    uint256[] private _platformIncomePercent = [39, 4, 4];
    uint256[] private _nodeIncomePercent = [0, 0, 0];
    uint256[] private _burnPercent = [30, 3, 3];
        ...
        ERC20("Meta Mine Token", "MPT")
    {
        ...
        _mint(address(this), _TOTAL_SUPPLY - _LP_ALLOCATION);
    ...
    function setNodeIncomePercent(uint8 txnType, uint256 percent)
        external
        onlyOperators
    {
        require(txnType < _nodeIncomePercent.length, "INVALID_TXN_TYPE");
        _nodeIncomePercent[txnType] = percent;
    }

    function setLpIncomePercent(uint8 txnType, uint256 percent)
        external
        onlyOperators
    {
        require(txnType < _lpIncomePercent.length, "INVALID_TXN_TYPE");
        _lpIncomePercent[txnType] = percent;
    }

    function setPlatformIncomePercent(uint8 txnType, uint256 percent)
        external
        onlyOperators
    {
        require(txnType < _platformIncomePercent.length, "INVALID_TXN_TYPE");
        _platformIncomePercent[txnType] = percent;
    }

    function setBurnPercent(uint8 txnType, uint256 burnPercent)
        external
        onlyOperators
    {
        require(txnType < _burnPercent.length, "INVALID_TXN_TYPE");
        _burnPercent[txnType] = burnPercent;
    }

    function getNodeIncomePercent(uint8 txnType)
        external
        view
        returns (uint256)
    {
        require(txnType < _nodeIncomePercent.length, "INVALID_TXN_TYPE");
        return _nodeIncomePercent[txnType];
    }

    function getLpIncomePercent(uint8 txnType) external view returns (uint256) {
        require(txnType < _lpIncomePercent.length, "INVALID_TXN_TYPE");
        return _lpIncomePercent[txnType];
    }

    function getPlatformIncomePercent(uint256 txnType)
        external
        view
        returns (uint256)
    {
        require(txnType < _platformIncomePercent.length, "INVALID_TXN_TYPE");
        return _platformIncomePercent[txnType];
    }

    function getBurnPercent(uint8 txnType) external view returns (uint256) {
        require(txnType < _burnPercent.length, "INVALID_TXN_TYPE");
        return _burnPercent[txnType];
    }
    ...
    function withdraw(
        ...
        require(
            this.balanceOf(address(this)) >= amount,
    ...
    function _transfer(
        ...
        if (!_feeEnabled || _feeWhitelist[from] || _feeWhitelist[to]) {
            //without fee
            super._transfer(from, to, amount);
        } else {
            uint8 txnType = TRANSFER;
            //purchase with fee
            ...
            uint256 toBurn = (_burnPercent[txnType] * amount) / 100;
            uint256 lpIncome = (_lpIncomePercent[txnType] * amount) / 100;
            uint256 nodeIncome = (_nodeIncomePercent[txnType] * amount) / 100;
            uint256 platformIncome = (_platformIncomePercent[txnType] *
                amount) / 100;
    ...
    function addLiquidity(uint256 usdtAmount, uint256 mptAmount)
        external
        returns (
            uint256 usdt,
            uint256 mpt,
    ...
    {
        require(balanceOf(msg.sender) >= mptAmount, "INSUFFICIENT_BALANCE");
        USDT.transferFrom(msg.sender, address(this), usdtAmount);
        super._transfer(msg.sender, address(this), mptAmount);
        USDT.approve(pancakeRouterAddress, usdtAmount);
        this.approve(pancakeRouterAddress, mptAmount);
        (mpt, usdt, liquidity) = IPancakeRouter01(pancakeRouterAddress)
            .addLiquidity(
                address(this),
                address(USDT),
                mptAmount,
        ...
        if (mpt < mptAmount) {
            super._transfer(address(this), msg.sender, mptAmount - mpt);
            approve(pancakeRouterAddress, 0);
        }
        return (usdt, mpt, liquidity);
    }
