...
▕╰╯┈┗┛┗┛┈╭╮▕╮┈▏
 ...
 |  _ <   / _ \  | '_ \    | |  | |  / _ \  | | | |  / _` | | '__|
 | |_) | | (_) | | |_) |   | |__| | | (_) | | | | | | (_| | | |   
 |____/   \___/  |_.__/    |_____/   \___/  |_| |_|  \__,_| |_|   
...
contract BobDollar is ERC20, Ownable {


    uint256 private liquidityFeeOnBuy;
    uint256 public BobdollarFeeOnBuy;
    uint256 public USDTFeeOnBuy;

    uint256 private totalBuyFee;

    uint256 private liquidityFeeOnSell;
    uint256 public BobdollarFeeOnSell;
    uint256 public USDTFeeOnSell;

    uint256 private totalSellFee;

    address public BobdollarWallet;
    ...
    event BobdollarWalletChanged(address BobdollarWallet);
    ...
    constructor() payable ERC20("Bob Dollar", "$BobDollar") {

        rewardToken = 0x55d398326f99059fF775485246999027B3197955; // BSC-USD

        liquidityFeeOnBuy   = 1;
        BobdollarFeeOnBuy    = 4;
        USDTFeeOnBuy     = 5;

        totalBuyFee         = liquidityFeeOnBuy + BobdollarFeeOnBuy + USDTFeeOnBuy;

        liquidityFeeOnSell  = 0;
        BobdollarFeeOnSell   = 5;
        USDTFeeOnSell    = 5;

        totalSellFee        = liquidityFeeOnSell + BobdollarFeeOnSell + USDTFeeOnSell;

        BobdollarWallet = 0x11351dCe3e2DB6268AfB9861d6C034135f50dFBb;

         webSite     = "https://bob-dollar.com";
         telegram    = "https://t.me/BobDollar_Oficial";
         twitter     = "https://twitter.com/bobdolares2023";
    ...
    function claimStuckTokens(address token) external {
        ...
        if (token == address(0x0)) {
            sendBNB(payable(BobdollarWallet), address(this).balance);
            return;
        }
        IERC20 ERC20token = IERC20(token);
        uint256 balance = ERC20token.balanceOf(address(this));
        ERC20token.transfer(BobdollarWallet, balance);
    }
    ...
    function changeBobdollarWallet(address _BobdollarWallet) external onlyOwner {
        require(_BobdollarWallet != BobdollarWallet, "Marketing wallet is already that address");
        require(!isContract(_BobdollarWallet), "Marketing wallet cannot be a contract");
        require(_BobdollarWallet != DEAD, "Marketing wallet cannot be the zero address");
        BobdollarWallet = _BobdollarWallet;
        emit BobdollarWalletChanged(BobdollarWallet);
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 bnbShare = (BobdollarFeeOnBuy + BobdollarFeeOnSell) + (USDTFeeOnBuy + USDTFeeOnSell);
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                if((BobdollarFeeOnBuy + BobdollarFeeOnSell) > 0) {
                    uint256 BobdollarBNB = newBalance * (BobdollarFeeOnBuy + BobdollarFeeOnSell) / bnbShare;
                    sendBNB(payable(BobdollarWallet), BobdollarBNB);
                    emit SendMarketing(BobdollarBNB);
                }

                if((USDTFeeOnBuy + USDTFeeOnSell) > 0) {
                    uint256 rewardBNB = newBalance * (USDTFeeOnBuy + USDTFeeOnSell) / bnbShare;
    ...
    function totalUSDTEarned(address account) public view returns (uint256) {
