...
contract Marketplace is IMarketplace, Ownable, ERC721Holder {
    ...
    function buyByCurrency(uint256[] memory _nftIds, address _refAddress) public override isAcceptBuyByCurrency {
        ...
        require(
            OceanFiERC20(currency).balanceOf(msg.sender) >= (totalValueUsdWithDecimal - saleValueUsdWithDecimal),
            "MARKETPLACE: NOT ENOUGH BALANCE CURRENCY TO BUY NFTs"
        );
        require(
            OceanFiERC20(currency).allowance(msg.sender, address(this)) >=
        ...
        require(
            OceanFiERC20(currency).transferFrom(
                msg.sender,
                saleWallet,
                (totalValueUsdWithDecimal - saleValueUsdWithDecimal)
            ),
        ...
        for (index = 0; index < _nftIds.length; index++) {
            try OceanFiNFT(nft).safeTransferFrom(address(this), msg.sender, _nftIds[index]) {
    ...
    function buyByToken(uint256[] memory _nftIds, address _refAddress) public override isAcceptBuyByToken {
        ...
        require(
            OceanFiERC20(token).balanceOf(msg.sender) >= (totalValueInTokenWithDecimal - saleValueInTokenWithDecimal),
            "MARKETPLACE: NOT ENOUGH BALANCE CURRENCY TO BUY NFTs"
        );
        require(
            OceanFiERC20(token).allowance(msg.sender, address(this)) >=
                (totalValueInTokenWithDecimal - saleValueInTokenWithDecimal),
            "MARKETPLACE: MUST APPROVE FIRST"
        );
        // Transfer token from buyer to sale wallet
        require(
            OceanFiERC20(token).transferFrom(
                msg.sender,
                saleWallet,
                (totalValueInTokenWithDecimal - saleValueInTokenWithDecimal)
            ),
...
contract Network is Ownable {
    ...
    function claimAward() external {
        if (typePayCom == 0) {
            require(oracleContract != address(0), "MARKETPLACE: INVALID ORACLE ADDRESS");
        }
