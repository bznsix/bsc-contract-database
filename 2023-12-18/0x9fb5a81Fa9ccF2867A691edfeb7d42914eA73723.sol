...
contract CDL is ERC20 {
	...
	address _baseToken = address(0xf9fB3b928F8f3f797dffea378f8f941714424b29);
    address _usdtToken = address(0x55d398326f99059fF775485246999027B3197955);

    address _cdReciver = address(0x26749F3b143883a9d5Aa2Cff4f71AAb8A256ae08);
    ...
    constructor(address tokenOwner) ERC20("CDL", "CDL") {
		...
		CD = IERC20(_baseToken);
		CD.approve(address(_cakeRouter), 10**64);
        ...
        _isPairs[uniswapV2PairUsdt] = true;
        ...
        startTime = total;
		reward = new UsdtReward(_usdtToken);
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                require(!_isPairs[to],"pair");
                super._transfer(from, address(this), amount.div(50));
                amount = amount.div(100).mul(98);
                if(startTime.add(10800) > block.timestamp){
                    require(super.balanceOf(to).add(amount) <= 2 * 10**18,"max");
                }
			}else if(_isPairs[to]){
                require(!_isPairs[from],"pair");
				require(startTime < block.timestamp,"startTime");
				super._transfer(from, address(this), amount.div(50));
                if(startTime.add(1800) > block.timestamp){
                    super._transfer(from, address(this), amount.div(100).mul(28));
    ...
    function swapAndLiquify() private {
        ...
        if(allTokenAmount >= 10**16){
            swapTokensForUsdt(allTokenAmount.div(4));
            uint256 usdtAmount = USDT.balanceOf(address(this));
            addLiquidityUsdt(usdtAmount, allTokenAmount.div(4));
            swapTokensForUsdtCd(allTokenAmount.div(2));
        }
    }
	...
	function swapTokensForCd(uint256 tokenAmount) private {
    ...
    function swapTokensForUsdtCd(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
    ...
    function addLiquidityCd(uint256 cdAmount, uint256 tokenAmount) private {
