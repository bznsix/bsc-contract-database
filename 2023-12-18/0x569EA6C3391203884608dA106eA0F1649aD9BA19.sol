...
contract SaintGrok is ERC20, Ownable {
    ...
    address private marketing = 0x73c651610242210a6265821C3a58e6FceCDe4735;

    uint8 private constant _decimals = 18;
    uint256 private supply = 1 * (10**9) * (10**18);

    uint256 public sell = 5;
    uint256 public buy = 3;
    ...
    constructor() ERC20("SaintGrok", "SAINTGROK") {
    ...
    function _firstBlocksProcess(address to) private {
        if (startBlock == 0 && isSwapPair(to)) {
            starting = true;
            startBlock = block.timestamp;
            maxTx = supply;
            return;
        } else if (starting == true) {
            if(maxTx < supply && block.timestamp > (startBlock + 3)) {
                maxTx = supply;
                starting = false;
            }
        }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (takeFee) {
            ...
            if(isSwapPair(from)) {
                ...
                feeAmount = amount.mul(buy).div(100);
            } else if (isSwapPair(to)) {
                require(amount <= maxTx, "can not transfer");
                feeAmount = amount.mul(sell).div(100);
            }
