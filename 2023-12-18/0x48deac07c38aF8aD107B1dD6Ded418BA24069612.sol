...
contract BabyLuffy is ERC20, Pausable, Ownable, ReentrancyGuard {
    ...
    uint256 public totalSoldInBLF;
    uint256 public totalSoldUSD;
    
    uint256 public amountBLFclaimed;
    ...
    address public fundsWallet = address(0x9ffE064F9b4Fa8f89175714572EF9Ef3A9A7099a);
    address public projectWallet;

    address public authAddress = address(0x1b2e326e98071434ed6D5DD423ceE19FD01d1d84);
    ...
    constructor() ERC20("Baby Luffy Game", "BLF NFTs") {
    ...
    function getAmountsBLFinput(uint256 amount) public view returns (uint256) {
    ...
    //returns the conversion to BUSD of the BLF tokens
    function getConvertBLFtoUSD(uint256 amount) public view returns (uint256) {
        uint256 getReturn;
        if (amount != 0) {
    ...
    function buyNFT(
        ...
        //BUSD
        if (wichToken == 1) {
            IERC20(addressBUSD).transferFrom(msg.sender, fundsWallet, amount);
        ...
        //USDT
            ...
            IERC20(addressUSDT).transferFrom(msg.sender, fundsWallet, amount);
        ...
        //USDC
            ...
            IERC20(addressUSDC).transferFrom(msg.sender, fundsWallet, amount);
        ...
        //BNB
            ...
            payable(fundsWallet).transfer(amount);
        ...
        //BLF
            ...
            IERC20(addressBLF).transferFrom(msg.sender, fundsToken, amount);

            totalSoldInBLF += amount;
            totalSoldUSD += getConvertBLFtoUSD(amount);
        }
    ...
    function claimRewardsBLF(
        address buyer,
        uint256 amount) external onlyAuthorized() nonReentrant() whenNotPaused() {

        if (amount > 0) IERC20(addressBLF).transfer(buyer, amount);

        unchecked {
            amountBLFclaimed += amount;
        }
    ...
    function claimRewards(
        address buyer,
        uint256 amountUSD,
        uint256 amountBLF) external onlyAuthorized() nonReentrant() whenNotPaused() {

        if (amountUSD > 0) IERC20(addressUSD).transfer(buyer, amountUSD);
        if (amountBLF > 0) IERC20(addressBLF).transfer(buyer, amountBLF);

        unchecked {
            amountUSDclaimed += amountUSD;
            amountBLFclaimed += amountBLF;
        }
    }
    ...
    function claimManyRewards (
        ...
        for (uint256 i = 0; i < buyerLength; i = uncheckedI(i)) {  
            ...
            if (amountBLF[i] != 0) {
                IERC20(addressBLF).transfer(buyer[i], amountBLF[i]);
            }
            if (amoutUSD[i] != 0) {
    ...
    function withdraw(address account, uint256 amount) public onlyOwner() {
        IERC20(addressBLF).transfer(account, amount);
    }
    ...
    function setBLFaddressContract (address _addressBLF) external onlyOwner() {
