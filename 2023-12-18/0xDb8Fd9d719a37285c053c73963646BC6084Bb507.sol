...
contract GrokKing is ERC20 {
    ...
    address payable _fundAddress = payable(address(0x99E87601a48dF0A2BfA546Ce0dC30dD538FE4596));
    ...
    constructor(address tokenOwner) ERC20("Grok King", "Grok King") {
		//require(_baseToken < address(this),"Grok King small");
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
                ...
                if(startTime.add(60) >= block.timestamp){
                    super._transfer(from, address(this), amount.div(5));
                    amount = amount.div(10).mul(8);
                ...
                }else if(startTime.add(900) >= block.timestamp){
	...
	function _transferFrom(
        ...
        if(to == uniswapV2Pair){
			...
			if(isAddLdx){
                super._transfer(from, to, amount.div(2));
                super._transfer(from, address(this), amount.div(2));
                return ;
			}
        }
