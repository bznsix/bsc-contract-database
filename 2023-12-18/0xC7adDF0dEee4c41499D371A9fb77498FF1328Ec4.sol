...
contract BLOCK is Context, IERC20, Withdrawable, Ownable {
    ...
    mapping(address => bool) private _ispancakePair;
    ...
    modifier isAuthorized() {
        require(
            ...
            "BLOCK Token: not authorized user"
        );
        _;
    }
    ...
    function _transfer(
        ...
        // buy
        if (
            (from == pancakePair || _ispancakePair[from]) &&
        ...
        if (
            ...
            if (
                ...
                (reserve0, reserve1) = IUniswapV2Pair(pancakePair)
                    .token0() == address(this)
                    ? (reserve0, reserve1)
                    : (reserve1, reserve0);

                uint256 estimatedEquivalentETH = pancakeRouter.getAmountOut(
                    amount,
                    reserve0,
                    reserve1
                );
    ...
    function distribute() external isAuthorized {
        ...
        if (_contractTokenBalance > 0) {
            uint256 maxSwapAmount = balanceOf(pancakePair)
                .mul(feeRate)
                .mul(PRECISION)
                .div(100)
                .div(PRECISION);
    ...
    function _addLiquidity(uint256 _tokenAmount, uint256 _ETHAmount) internal {
        ...
        (bool success, ) = payable(address(pancakeRouter)).call{
            value: _ETHAmount
        }(
    ...
    function ispancakePair(address _pair) external view returns (bool) {
        if (_pair == pancakePair) return true;
        return _ispancakePair[_pair];
    }
    ...
    function setpancakeRouter(address _pancakeRouterAddress)
        external
        onlyOwner
    {
        pancakeRouter = IUniswapV2Router02(_pancakeRouterAddress);
    }
    ...
    function addpancakePair(address _pair) external onlyOwner {
        _ispancakePair[_pair] = true;
    }

    function removepancakePair(address _pair) external onlyOwner {
        _ispancakePair[_pair] = false;
    }
