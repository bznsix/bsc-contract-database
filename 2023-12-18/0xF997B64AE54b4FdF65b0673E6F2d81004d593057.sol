...
contract DJPD is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    // Addresses
    address public FeeReceiver = 0xF0C2bBc210b6420Acb6DF623792f1CFF02502593;
    address public FeeReceiver2 = 0xC25ca8aDaD40e0a329C1cD34139ea3529E5D898b;

    // Constructor
    constructor() ERC721("D.J.P.D Xmas", "D.J.P.D") {
        _baseTokenURI = "ipfs://bafybeigi6aclfsehuxqv26izndfsckf2h4iqefpocrsbqf4q7dqloqa4me/";
    }
    ...
    function setFeeReceivers(address _newFeeReceiver, address _newFeeReceiver2) public onlyOwner {
        FeeReceiver = _newFeeReceiver;
        FeeReceiver2 = _newFeeReceiver2;
    }

    // Minting
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "DJPD: minting has not started yet");
        require(_mintAmount > 0, "DJPD: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "DJPD: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "DJPD: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "DJPD: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "DJPD: zero address");
        require(_amount > 0, "DJPD: you have to mint at least one");
        require(lastSupply != 0, "DJPD: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "DJPD: URI query for nonexistent token");
    ...
    function handleMintReward(uint256 _amount) private {
        uint256 _amount1 = _amount.mul(50).div(100);
        uint256 _amount2 = _amount.mul(50).div(100);
        payable(FeeReceiver).transfer(_amount1);
        payable(FeeReceiver2).transfer(_amount2);
    }
