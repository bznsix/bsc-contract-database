...
contract ASXTokenStorage {
    address private immutable asx;
    ...
    function transfer(IERC20 token) external {
        require(asx == msg.sender, 'No ASX');
        token.transfer(asx, token.balanceOf(address(this)));
    }
}
...
contract ASX is ERC20, Ownable2Step {
    ...
    ASXTokenStorage public tokenStorage;
    ...
    constructor(address payable _marketingWallet, address _liquidityWallet, address _rewardToken, address _router) ERC20("ASX", "ASX") {
        ...
        tokenStorage = new ASXTokenStorage();
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "ASX: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ASX: The new dividend tracker must be owned by the ASX token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "ASX: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ASX: Account is already the value of 'excluded'");
    ...
    function setRewardTokenFee(uint256 value) external onlyOwner{
        require(value.add(liquidityFee).add(marketingFee)<=4,"ASX: Maximum tax is 4%");
    ...
    function setLiquidityFee(uint256 value) external onlyOwner{
        require(RewardTokenFee.add(value).add(marketingFee)<=4,"ASX: Maximum tax is 4%");
        liquidityFee = value;
        totalFees = RewardTokenFee.add(liquidityFee).add(marketingFee);
    ...
    function setMarketingFee(uint256 value) external onlyOwner{
        require(RewardTokenFee.add(liquidityFee).add(value)<=4,"ASX: Maximum tax is 4%");
        marketingFee = value;
        totalFees = RewardTokenFee.add(liquidityFee).add(marketingFee);
    ...
    function setRewardToken(address _rewardToken) external onlyOwner{
        require(_rewardToken != address(0), "ASX: rewardToken is 0 address");
        RewardToken = _rewardToken;
    ...
    function setWallet(
        ...
        require(_marketingWallet != address(0), "ASX: marketingWallet is 0 address");
        require(_liquidityWallet != address(0), "ASX: liquidityWallet is 0 address");
        marketingWallet = _marketingWallet;
        liquidityWallet = _liquidityWallet;
    }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ASX: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200_000 && newValue <= 500_000, "ASX: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ASX: Cannot update gasForProcessing to same value");
    ...
    function setBabt(address babtAddr) external onlyOwner{
        require(babtAddr != address(0), "ASX: babt is 0 address");
    ...
    function _transfer(
        ...
        require(checkIfWhitelisted(from, to), "ASX: Not whitelisted");
    ...
    function swapAndLiquify(uint256 tokens, address tokenB) private {
       // split the contract balance into halves
        ...
        swapTokensForToken(half, tokenB); // <- this breaks the ETH -> ASX swap when swap+liquify is triggered
    ...
    function swapAndLiquifyETH(uint256 tokens) private {
       // split the contract balance into halves
        ...
        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the ETH -> ASX swap when swap+liquify is triggered
