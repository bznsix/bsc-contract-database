...
contract ALPHA_SWAP is Ownable, ReentrancyGuard{
    ...
    string private _name = "ALPHA SWAP v2";
    ...
    address public dasAddress= 0xF69dc1586145d1654A93B013449c3889c15FEdb8;//Decentralize Account System Wallet
    ...
    constructor(){
        ...
        rewardPoolAddress = 0x303eB2Fa35E1d1FdFCe8d7CCA6d671516f73f555;//0x605afd99625289232E92f7a1aB552C325C594682 //0x303eB2Fa35E1d1FdFCe8d7CCA6d671516f73f555
        vaultAddress = 0xF5c27FaD680Ea584dc9973F80920D74aCc1290af;//0xc66a1642BdC9489C853081772452ACbfdcB31305 //0xF5c27FaD680Ea584dc9973F80920D74aCc1290af
    }
    ...
    function setBlacklist(address[] memory _botAddress, bool _flag) external onlyOwner {
    ...
    function getBUSDValue(uint _busdAmount) public view returns (uint){
        uint _busdBal = BUSD.balanceOf(address(this));
        uint _tokenBal = token.balanceOf(address(this));
        //require(_busdAmount > 0, "no value to calculate");
        //return (_busdAmount / tokenPriceInBusd) * 100;
        return (_tokenBal * _busdAmount) / _busdBal;
    ...
    function getTokenValue(uint _tokenAmount) public view returns (uint){
        ...
        uint _busdBal = BUSD.balanceOf(address(this));
        uint _tokenBal = token.balanceOf(address(this));
        return (_busdBal * _tokenAmount) / _tokenBal;
    }
    ...
    function updateTokenAddress(address _token, address _nft, address _vaultAddress, address _busd) external onlyOwner{
        token = IERC20(_token);
    ...
    function updateFeePercent(uint256 buyFee, uint sellFee) external onlyOwner{
        totalBuyFee = buyFee;
    ...
    function buyBUSDForVaultDeposit(uint256 busdAmount) external nonReentrant{
            ...
            uint256 taxes = (totalBuyFee * _tokenVal) / 100;
            ...
            uint amountTosend = _tokenVal-taxes;
            token.transfer(investor,amountTosend);
            distributeBuyDepositFee(busdAmount);
            
            // if(useVault==true){
            //     IAlphaVault(vaultAddress).proxyDepositVault(investor,_tokenVal,referer);
    ...
    function sellAlphaForBUSD(uint256 tokenAmount) external  nonReentrant{
            ...
            require(blacklist[msg.sender] != true, "You are not allow to sell!");
    ...
    function distributeBuyDepositFee(uint256 amount) private {
        ...
        uint256 liq = (totalBuyFee * balance) / 100;
