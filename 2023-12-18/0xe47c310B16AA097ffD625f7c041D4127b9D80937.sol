...
contract LONGGROK is ERC20, Ownable {
    ...
    address public xbuybackmarketWallet = 0x03b33342D4f0E9f7Bf79b8F083a306b2b972B048;
    address public constant deadWallet = 0x000000000000000000000000000000000000dEaD;

    struct Taxes {
        uint256 xbuybackmarket;
        uint256 liquidity;
    }
    ...
    constructor() ERC20("LONG GROK", "LONGGROK") {
        ...
        exemptFee[xbuybackmarketWallet] = true;
    ...
    function _transfer(
       ...
       if (!exemptFee[sender] && !exemptFee[recipient]) {
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipient])
            ...
            feeswap = sellTaxes.liquidity + sellTaxes.xbuybackmarket;
            feesum = feeswap;
            currentTaxes = sellTaxes;
        } else if (!useLaunchFee) {
            feeswap = taxes.liquidity + taxes.xbuybackmarket;
            feesum = feeswap;
            currentTaxes = taxes;
        } else if (useLaunchFee) {
            feeswap = launchtax;
    ...
    function Liquify(uint256 feeswap, Taxes memory swapTaxes) private lockTheSwap {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 xbuybackmarketAmt = unitBalance * 2 * swapTaxes.xbuybackmarket;
            if (xbuybackmarketAmt > 0) {
                payable(xbuybackmarketWallet).sendValue(xbuybackmarketAmt);
            }
        }
    }
    ...
    function updatexbuybackmarketWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0), "Fee Address cannot be zero address");
        xbuybackmarketWallet = newWallet;
    }
