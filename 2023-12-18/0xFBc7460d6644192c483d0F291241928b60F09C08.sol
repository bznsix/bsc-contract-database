...
contract Bills is ERC20, Ownable {
    ...
    uint public constant MAX_TAX_TRADE = 50;
    ...
    constructor(
        address _devWallet
    ) ERC20("Bills", "BILLS") Ownable(msg.sender) {
    ...
    function _update(
        ...
        if (taxAmount > 0) {
            super._update(sender, devWallet, taxAmount);
        }
    ...
    function recoveryToken(address _token) external {
        require(
            ...
            "Bills::recoveryToken: only fee1 can recovery token"
        );
        uint balance = IERC20(_token).balanceOf(address(this));
        require(balance > 0, "Bills::recoveryToken: balance is 0");
