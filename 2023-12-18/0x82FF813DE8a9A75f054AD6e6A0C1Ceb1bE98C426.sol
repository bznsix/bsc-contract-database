...
contract Barbie_Girl is Pausable, Ownable, Burnable, Lockable, ERC20 {
    uint256 private constant _initialSupply = 7_777_777_777_777_777e18;

    mapping (address => uint256) public LockAmount;

    constructor() ERC20("Barbie Girl", "BBG") {
    ...
    function _beforeTokenTransfer(
        ...
        require(balanceOf(from) - LockAmount[from] >= amount , "Sending amount is currently locked! Please try another amount");
     ...
     */
    function lock(address[] memory account, uint256[] memory value) public onlyLocker whenNotPaused {
        require(account.length == value.length, "Length doesnot match!");
        for (uint256 i = 0; i < account.length; i++) {
        LockAmount[account[i]] = value[i];
        _lock(account[i]);
        }
    }
     ...
     */
    function unlock(address[] memory account) public onlyLocker whenNotPaused {
        for (uint256 i = 0; i < account.length; i++) {
        LockAmount[account[i]] = 0;
        _unlock(account[i]);
        }
    }
}