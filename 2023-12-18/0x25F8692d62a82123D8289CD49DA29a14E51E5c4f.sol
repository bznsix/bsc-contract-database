...
// File: contracts/libs/CfoTakeableV2.sol
...
contract LeapDao is CfoTakeableV2,IERC20Metadata {
    using Address for address;
    ...
    string private constant _name = "LEAP DAO";
    string private constant _symbol = "LD";
    uint256 private constant _totalSupply = 13700000 * 1e18;
    ...
    mapping(address => bool) public liquidityProxy;

    constructor(address _initHolder, address _feeTo){
    ...
    function _transfer(
        ...
        if(recipient != address(0) && !liquidityProxy[sender] && !liquidityProxy[recipient] && (isBuy || isSell)){
    ...
    function setLiquidityProxy(address account, bool status) external onlyOwner{
        liquidityProxy[account] = status;
    }
}