...
interface IPancakeRouter02 is IPancakeRouter01 {
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        // Protocol by { https://www.fiverr.com/forexshark }

contract CHCToken is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 1 * 1e12  ether; // 1 trillion total supply
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Community Helper Coin"; // token name
    string private _symbol = "CHCToken"; // token ticker
    uint8 private _decimals = 18; // token decimals

    IPancakeRouter02 public pancakeRouter;
    address public pancakePair;
    address payable public VaultWallet = payable(0xC30d5CA7bf908186c0418cCB4D0872DA19488d2C);
    address payable public buybackAndBurnWallet = payable(0xfadc01C9Fb2688B0C8D397c2E55afCED650AC875);
    uint256 minTokenNumberToSell = 100000000 ether; // 100000000 max tx amount will trigger swap and add liquidity
    uint256 public maxFee = 50; // 50% max fees limit per transaction
    ...
    // buy tax fee
    uint256 public redistributionFeeOnBuying = 10; // 10% will be distributed among holder as token divideneds
    uint256 public liquidityFeeOnBuying = 3; // 3% will be added to the liquidity pool
    uint256 public vaultWalletFeeOnBuying = 3; // 3% will go to the marketingwallet address
    uint256 public buybackAndBurnFeeOnBuying = 4; // 4% will go to the roadmap wallet address

    // sell tax fee
    uint256 public redistributionFeeOnSelling = 10; // 10% will be distributed among holder as token divideneds
    uint256 public liquidityFeeOnSelling = 3; // 3% will be added to the liquidity pool
    uint256 public vaultWalletFeeOnSelling = 3; // 3% will go to the market address
    uint256 public buybackAndBurnFeeOnSelling = 4; // 4% will go to the roadmap wallet address
    ...
    uint256 public vaultWalletFee = 0; // 0% will go to the market address
    uint256 public buybackAndBurnFee = 0; // 0% will go to the roadmap wallet address

    // for smart contract use
    uint256 private _currentRedistributionFee;
    uint256 private _currentLiquidityFee;
    uint256 private _currentvaultWalletFee;
    uint256 private _currentbuybackAndBurnFee;
    ...
    function setWallets(address payable _VaultWallet, address payable _buybackAndBurnWallet) external onlyOwner {
        require(_VaultWallet!=address(0),"Market wallet cannot be address zero");
        require(_buybackAndBurnWallet!=address(0),"Market wallet cannot be address zero");
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount*(_currentRedistributionFee+(_currentLiquidityFee)+(_currentvaultWalletFee)+(_currentbuybackAndBurnFee))/(1e2);
        return percentage;
    }
    ...
    function _takeVaultFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tMarketingFee = tAmount*(_currentvaultWalletFee)/(1e2);
        uint256 rMarketingFee = tMarketingFee*(currentRate);
        _rOwned[VaultWallet] = _rOwned[VaultWallet]+(rMarketingFee);
        if (_isExcluded[VaultWallet])
            _tOwned[VaultWallet] = _tOwned[VaultWallet]+(tMarketingFee);
        emit Transfer(_msgSender(), VaultWallet, tMarketingFee);
    }
    
    function _takebuybackAndBurnFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 buybackAndBurnFees = tAmount*(_currentbuybackAndBurnFee)/(1e2);
        uint256 rbuybackAndBurnFee = buybackAndBurnFees*(currentRate);
        _rOwned[buybackAndBurnWallet] = _rOwned[buybackAndBurnWallet]+(rbuybackAndBurnFee);
        if (_isExcluded[buybackAndBurnWallet])
            _tOwned[buybackAndBurnWallet] = _tOwned[buybackAndBurnWallet]+(buybackAndBurnFees);
        emit Transfer(_msgSender(), buybackAndBurnWallet, buybackAndBurnFees);
    }

    function removeAllFee() private {
        _currentRedistributionFee = 0;
        _currentLiquidityFee = 0;
        _currentvaultWalletFee = 0;
        _currentbuybackAndBurnFee = 0;
    }

    function setBuyFee() private {
        _currentRedistributionFee = redistributionFeeOnBuying;
        _currentLiquidityFee = liquidityFeeOnBuying;
        _currentvaultWalletFee = vaultWalletFeeOnBuying;
        _currentbuybackAndBurnFee = buybackAndBurnFeeOnBuying;
    }

    function setSellFee() private {
        _currentRedistributionFee = redistributionFeeOnSelling;
        _currentLiquidityFee = liquidityFeeOnSelling;
        _currentvaultWalletFee = vaultWalletFeeOnSelling;
        _currentbuybackAndBurnFee = buybackAndBurnFeeOnSelling;
    }

    function setNormalFee() private {
        _currentRedistributionFee = redistributionFee;
        _currentLiquidityFee = liquidityFee;
        _currentvaultWalletFee = vaultWalletFee;
        _currentbuybackAndBurnFee = buybackAndBurnFee;
    }
    ...
    function setBuyFeePercent(
        ...
        uint256 _vaultWalletFee,
        uint256 _buybackAndBurnFee
    ) external onlyOwner {
        redistributionFeeOnBuying = _redistributionFee;
        liquidityFeeOnBuying = _liquidityFee;
        vaultWalletFeeOnBuying = _vaultWalletFee;
        buybackAndBurnFeeOnBuying = _buybackAndBurnFee;
        require(
            redistributionFeeOnBuying
                +(liquidityFeeOnBuying)
                +(vaultWalletFeeOnBuying)
                +(buybackAndBurnFeeOnBuying) <= maxFee,
    ...
    function setSellFeePercent(
        ...
        vaultWalletFeeOnSelling = _vaultWalletFee;
        buybackAndBurnFeeOnSelling = _buybackAndBurnFee;
        require(
            redistributionFeeOnSelling
                +(liquidityFeeOnSelling)
                +(vaultWalletFeeOnSelling)
                +(buybackAndBurnFeeOnSelling) <= maxFee,
            "BEP20: Can not be greater than max fee"
        );
    }
    ...
    function setNormalFeePercent(
        ...
        vaultWalletFee = _vaultWalletFee;
        buybackAndBurnFee = _buybackAndBurnFee;
        require(
            redistributionFee+(liquidityFee)+(vaultWalletFee)+(
                buybackAndBurnFee
            ) <= maxFee,
            "BEP20: Can not be greater than max fee"
        );
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _takebuybackAndBurnFee(tAmount, currentRate);
        _reflectFee(tAmount);
