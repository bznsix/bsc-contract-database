 ...
 function fillOrder(uint128 _tokenId) external payable {
    ...
    function removeAllInvalidOrders() external onlyOwner {
        ...
        while (i < totalOrders) {
            ...
            IterableMapping.Order memory order = tokenIdToOrder.get(tokenId);

            try collection.ownerOf(order.tokenId) returns (address owner) {
    ...
    function cancelOldOrders() external onlyOwner {
        uint32 totalOrders = tokenIdToOrder.size();
        uint32 i = 0;
        ...
        while (i < totalOrders) {
            ...
            if (order.startTime < sixMonthsAgo) {
                tokenIdToOrder.remove(tokenId);
                totalOrders--;
            } else {
                i++;
    ...
    function cancelAllOrders() external onlyOwner {
        while (tokenIdToOrder.size() > 0) {
            uint128 tokenId = tokenIdToOrder.getKeyAtIndex(0);
            tokenIdToOrder.remove(tokenId);
        }
    }
    ...
    function getSellerOrders(address _seller) external view returns (IterableMapping.Order[] memory) {
        ...
        for (uint32 i = 0; i < totalOrders; i++) {
            ...
            try collection.ownerOf(order.tokenId) returns (address owner) {
                if (order.seller == _seller && owner == _seller) {
