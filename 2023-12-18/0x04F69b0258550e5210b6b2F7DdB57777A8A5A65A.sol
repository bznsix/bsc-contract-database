...
contract FiFiToken is Ownable, ERC20 {
    ...
    event WhitelistUpdated(address indexed user, uint256 timestamp);

    constructor(uint256 _totalSupply) ERC20("FIfi", "FIFI") Ownable() {
        _mint(msg.sender, _totalSupply);
        launchTime = block.timestamp;
    }

    modifier onlyPancakeSwapV2Pair() {
        require(msg.sender == pancakeSwapV2Pair, "Only the PancakeSwap V2 pair can call this function");
        _;
        
    }
        
    function updateWhitelist(address user) external onlyOwner {
        whitelist[user] = block.timestamp + 2 hours; 
        emit WhitelistUpdated(user, whitelist[user]);}
    ...
    function _transfer(
        ...
        require(amount == sendAmount + burnAmount, "ERC20: Burn value invalid");
