...
contract TokenDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("Token_Dividend_Tracker", "Token_Dividend_Tracker") {
        claimWait = 18000; //18000 Seconds = 5 Hour
...
contract ChadXMas is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    uint256 public maxTransactionLimit = 20_000_000 * (10**18); // 3% Max Transaction
    uint256 public MaxWalletLimit = 20_000_000 * (10**18); // 3% Max Waallet limit
    uint256 public swapTokensAtAmount = 2_000_000 * (10**18); // Should be Between 0.05 to 0.5% of the total supply
    ...
    function setMaxtxLimit(uint256 _maxTxAmount) public onlyOwner {
        maxTransactionLimit = _maxTxAmount * (10 ** 18);
    }
    ...
    function setMaxWalletLimit(uint256 _maxToken) external onlyOwner {
  	    MaxWalletLimit = _maxToken * (10**18);
  	}

    constructor() ERC20("ChadXMas", "CHAD") { // Name and Symbol
        BNBRewardsFee = 4;
        liquidityFee = 1;
        extraFeeOnSell = 5; // extra fee on sell
        marketingFee = 10;
        marketingWallet = payable(0x7F0933a26917585a3b16831670d857552FA6888A); // Marketing Wallet
        ...
        // IUniswapV2Router02 ETH Mainnet/Testnet  0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
        ...
        dividendTracker.excludeFromDividends(0x407993575c91ce7643a4d4cCACc9A98c36eE1BBE);
        ...
        excludeFromFees(0x407993575c91ce7643a4d4cCACc9A98c36eE1BBE, true);
        excludeFromFees(0x5f60f506a0064582e6bf8334F285E691D59620bb, true);
        excludeFromFees(0xd54936073D36B161CbBAa561e0572f56b36Dd916, true);
        excludeFromFees(0x750AF187A6D585dBe8B09CD8a586DFC1D7aB006b, true);
    ...
    function setExcludeFromMaxTx(address _address, bool value) public onlyOwner { 
        _isExcludedFromLimits[_address] = value;
    }
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function _transfer(
        ...
        if(automatedMarketMakerPairs[to] && (!_isExcludedFromLimits[from]) && (!_isExcludedFromLimits[to])){
            require(amount <= maxTransactionLimit, "Sell transfer amount exceeds the maxTransactionLimit.");
        }

        if(automatedMarketMakerPairs[from] && (!_isExcludedFromLimits[from]) && (!_isExcludedFromLimits[to])){
            require(amount <= maxTransactionLimit, "Sell transfer amount exceeds the maxTransactionLimit.");
            require(super.balanceOf(to) + amount <= MaxWalletLimit, "Forbid");
        }
