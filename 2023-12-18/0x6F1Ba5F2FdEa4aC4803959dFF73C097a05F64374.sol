...
contract Airdrop is Ownable {
    uint256 public releasePercent = 50_00;
    IERC20 public token;
    bytes32 public root;
    ...
    function setReleasePercent(uint256 _releasePercent) public onlyOwner {
        require(_releasePercent <= 100_00, "Invalid release percent");
        require(_releasePercent >= releasePercent, "Can't decrease release percent");
    ...
    function getClaimableAmount(address _addr, uint256 _fullAmount) public view returns (uint256) {
        return (_fullAmount * releasePercent) / 100_00 - claimedAmount[_addr];
    }
    ...
    function claim(bytes32[] memory proof, uint256 amount) public {
        ...
        uint256 amountToTransfer = getClaimableAmount(msg.sender, amount);
