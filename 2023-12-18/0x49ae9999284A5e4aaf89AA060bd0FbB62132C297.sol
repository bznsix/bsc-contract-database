...
interface IERC20 {
  ...
  event Approval(address indexed owner, address indexed spender, uint256 value);
    ...
    contract UNIVERSALBLOCKCHAINCONTRACT  {
    ...
    uint256 private constant timeStep = 2 days;
    ...
    uint256 public totalUser; 
     ...
     uint256 public directorPool;
      ...
      uint256 public GDPool;
       uint256 public totaldepositcompany;
    ...
    struct RewardInfo{
        ...
        uint256 statics;
        uint256 directs;
        uint256 directsclaim;
        uint256 directorclaim;
        uint256 crown;
        uint256 diamond;
        uint256 global;          
        uint256 split;
    ...
    constructor(address _usdtaddr)   {
        USDT = IERC20(_usdtaddr);
         ...
         feeReceivers[0] = address(0x572855f912fbF48AfBAe371D4a13247c0686D47c);   //// DEvelopment Fund
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 250e18){
            ...
            if(total >= 2000e18 && totaldirectnum>=10    && user.teamNum >= 250 &&   otherTeam  + othermaxTeam + maxTeam >=180000e18       ){
                levelNow = 5;
            }else if(total >= 1000e18 && totaldirectnum>=8  && user.teamNum >= 100 &&  otherTeam  + othermaxTeam + maxTeam >=54000e18    ){
                levelNow = 4;
            }else if(total >= 500e18  && totaldirectnum>=7  && user.teamNum >= 50 &&  otherTeam  + othermaxTeam + maxTeam >=18000e18  ){
   ...
   function checkteamconditions(address _user) private view returns(uint256, uint256, uint256 ) {
    ...
    function deposit(uint256 _amount) external {
                   ...
                   totaldepositcompany = totaldepositcompany + _amount;
 ...
 function _releaseReward(address _user, uint256 _amount, uint256 _initialamt) private {
       ...
       if(userInfo[upline].totalRevenue < userInfo[upline].totalDeposit.mul(_rewarding).div(baseDivider) && idstatus==true)
        {
                    ...
                    if(i==0 && idstatus==true){
                       ...
                       upRewards.directsclaim = upRewards.directsclaim.add(reward);                       
                     ...
                     reward = newAmount.mul(directorPercents[i - 1]).div(baseDivider);
                       if(userInfo[upline].totalRevenue.add(reward) > userInfo[upline].totalDeposit.mul(_rewarding).div(baseDivider)) {
                        ...
                        upRewards.directorclaim = upRewards.directorclaim.add(reward);
                        userInfo[upline].totalRevenue = userInfo[upline].totalRevenue.add(reward);
                       ...
                       reward = newAmount.mul(crowndirectorPercents[i - 2]).div(baseDivider);
                       if(userInfo[upline].totalRevenue.add(reward) > userInfo[upline].totalDeposit.mul(_rewarding).div(baseDivider)) {
                         reward = (userInfo[upline].totalDeposit.mul(_rewarding).div(baseDivider)).sub(userInfo[upline].totalRevenue);
                        } 
    ...
    function transferByActivation(address _receiver, uint256 _amount) external {
        require(_amount >= minDepositActivation , "amount err");
        uint256 splitLeft = getCurSplit(msg.sender);
        require(splitLeft >= _amount, "insufficient income");
            uint256 deductioncharge = 0; 
            
             if(userInfo[msg.sender].level==2){
  ...
  function unfreezeActivation(address _receiver, uint256 _types) external {
    ...
    function withdraw() external {
       if(!isFreezeReward){
        ...
        userRewards.directsclaim = 0;
        userRewards.directorclaim = 0;
        userRewards.crown = 0;
    ...
    function _calCurDynamicRewards(address _user) private view returns(uint256, uint256) {
        ...
        uint256 totalRewards = userRewards.directs.add(userRewards.directsclaim).add(userRewards.directorclaim).add(userRewards.crown).add(userRewards.diamond).add(userRewards.global);     
