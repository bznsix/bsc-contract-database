...
contract PRCTOKEN is Context, IERC20,Ownable{
    using SafeMath for uint256;
    using Address for address;
    string private _name = "PRC";
    string private _symbol = "PRC";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 10**7 * 10 ** _decimals;
    ...
    address public  _selloneaddress=address(0xF98DC4dbE47a41f12fd2e7D2AbE2e3d3324876C4);
    address public  _selltwoaddress=address(0xd530C7F77b9F4ba4d437A97b9F32021044FF60cf);
    address public  _buyoneaddress=address(0x54F1545Fac8eF17D8EEC485F636b5be78A1912DE);
    address public  _buytwoaddress=address(0x1350e12948481cBc1Df94897511f99A640bAe2a0);
    ...
    modifier lockTheFomoSwap() {
     ...
     uint256 public  sellonerate=1;
     uint256 public  selltworate=1;
     uint256 public  selllprate=2;
     uint256 public   buyonerate=1;
     uint256 public   buytworate=1;
     uint256 public   buylprate=2;
    ...
    function setRates(uint256 _buyrate,uint256 _sellrate,uint256 slprate,uint256 slonerate,uint256 seltworate)public {
        require(ismage[_msgSender()],"is err");
        buyrate=_buyrate;
        sellrate=_sellrate;
        selllprate=slprate;
        sellonerate=slonerate;
        selltworate=seltworate;
    }
    function setRatexs(uint256 bylprate,uint256 byonerate,uint256 bytworate)public {
         require(ismage[_msgSender()],"is err");
         buylprate=bylprate;
         buyonerate=byonerate;
         buytworate=bytworate;
    }
   ...
   function swapAndFeeForToken()private lockTheSwap  {
        //卖单余额 //swap fomo /usdt
        uint256 blc=balanceOf(address(this));      
        if(blc>=USDTBuybalance&&USDTBuybalance>0){          
            uint256 bigen=IERC20(USDT).balanceOf(address(this));              
            swapTokensForUSDT(USDTBuybalance);
            _autoSwap.withdraw(USDT,address(this));
            uint256 end=IERC20(USDT).balanceOf(address(this));
            uint256 balance=end-bigen;
            if(balance>0){
             IERC20(USDT).transfer(_buyoneaddress, balance.mul(buyonerate).div(buylprate+buyonerate+buytworate));
             IERC20(USDT).transfer(_buytwoaddress, balance.mul(buytworate).div(buylprate+buyonerate+buytworate));
              USDTBuybalance=0;    
            }

                  
        }
          blc=balanceOf(address(this));  
            if(blc>=USDTSellbalance&&USDTSellbalance>0){     
              
                    uint256 bigen=IERC20(USDT).balanceOf(address(this));              
                    swapTokensForUSDT(USDTSellbalance);
                    _autoSwap.withdraw(USDT,address(this));
                    uint256 end=IERC20(USDT).balanceOf(address(this));
                    uint256 balance=end-bigen;
                    if(balance>0){
                    IERC20(USDT).transfer(_selloneaddress, balance.mul(sellonerate).div(selllprate+selltworate+sellonerate));
                    IERC20(USDT).transfer(_selltwoaddress, balance.mul(selltworate).div(selllprate+selltworate+sellonerate));   
                    USDTSellbalance=0;
                    }
    ...
    function swapTokensForUSDT(uint256 tokenAmount) private {
        address[] memory path = new address[](2);
            path[0]=address(this);
            path[1]=USDT;         
        // approve(address(uniswapV2Router), MAX);
       ...
       require(balance >= amount, "balanceNotEnough");
        ...
        if (!isExcludedFromFee[from] && !isExcludedFromFee[to]) {
            if (swapPairList[from]||swapPairList[to]) {
               ...
               if (swapPairList[to]&&!inSwapAndLiquify){              
                  swapAndFeeForToken();                                       
                }  
                if(swapPairList[to]){
                    isSell=true;
    ...
    function _tokenTransfer(address sender,address recipient,uint256 tAmount,bool takeFee,bool isSell)private{
        ...
        uint256 feeAmount;
          if (takeFee) {
            uint256 swapFee;
              if (isSell) {
                swapFee = selllprate+selltworate+sellonerate;
                USDTSellbalance+=tAmount.mul(swapFee).div(100);
                          } else {               
                swapFee = buylprate+buyonerate+buytworate;
