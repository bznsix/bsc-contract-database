...
Wakandan Dollar - The Cryptocurrency to Unite the world.
...
Wakandan dollar is the native currency of the Wakandan kingdom which is deployed on the Binance Smart Chain.
...
contract WAKANDANDOLLAR is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    WAKANDANDOLLARDividendTracker public dividendTracker;
    ...
    constructor() ERC20("WAKANDAN DOLLAR", "$WAKANDA") {
        marketingWallet = payable(0xF636fc9313c029888606b7ae3483fe6055916b1B);
        devWallet = payable(0xf4464DEb5c6CEfb18636922785F34e1fB658fa31);
        ...
        dividendTracker = new WAKANDANDOLLARDividendTracker(
            payable(this),
            router,
            0x87374baef80590f5274bAE9C2533e10b6A3A79b8,
            "WAKANDANDOLLARTRACKER",
            "WAKANDANDOLLARTRACKER"
        );
        ...
        maxWallet = totalTokenSupply / 300000; // 0.005%
        swapTokensAtAmount = totalTokenSupply / 3000000; // 0.05%;
...
contract WAKANDANDOLLARDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    WAKANDANDOLLAR public WAKANDANDOLLARContract;
    ...
    constructor(
        ...
        defaultToken = token;
        WAKANDANDOLLARContract = WAKANDANDOLLAR(mainContract);
        minimumTokenBalanceForAutoDividends = 1_000000000000000000; // 1 tokens
    ...
    function _transfer(
        ...
        require(false, "WAKANDANDOLLAR_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "WAKANDANDOLLAR_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WAKANDANDOLLAR contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(WAKANDANDOLLARContract);

            uint256 prevBalance = WAKANDANDOLLARContract.balanceOf(address(this));
            ...
            {
                uint256 received = WAKANDANDOLLARContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    WAKANDANDOLLARContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
