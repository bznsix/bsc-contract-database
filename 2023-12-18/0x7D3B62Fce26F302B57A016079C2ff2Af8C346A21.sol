...
contract DutchBondSale is IBondSale, Ownable, Pausable, ReentrancyGuard {
    ...
    modifier onlyWhenStaked(address account) {
        if (address(staking) != address(0)) {
            require(staking.stakedOf(account) >= minStake, "Staking balance below minimum");
        }
        _;
    }
    ...
    constructor(
        ...
        require(_discountStart > _discountEnd, "Discount start must be greater than discount end");
        require(
    ...
    function setStaking(address _staking, uint256 _minStake) external onlyOwner {
        staking = IStaking(_staking);
        minStake = _minStake;
    }

    function setPrice(uint256 _currentPrice) external onlyOwner {
    ...
    function bond(
        ...
        uint256 _bondToken,
        uint16 _slippage
    ) external whenNotPaused nonReentrant onlyWhenStaked(msg.sender) {
        ...
        bonds[msg.sender].push(Bond(_bondToken, 0, block.timestamp, vestingDuration, discountRate));

        totalBondedTokens += _bondToken;
        peggedToken.safeTransferFrom(msg.sender, owner(), peggedValue);
    ...
    function currentPrice() public view returns (uint256) {
        require(block.timestamp <= latestPriceTimestamp + 300, "Price is outdated");
        return latestPrice;
    }
