...
// File: contracts/4_usdtYieldFarm.sol
...
contract USDTYieldFarm is ReentrancyGuard, Ownable {
    ...
    function claimUSDT() external nonReentrant {
        ...
        require(user.claimedUSDT <= user.depositedUSDT * TOTAL_CLAIMABLE_PERCENT / PERCENT_DENOMINATOR, "Claim limit reached.");
    ...
    function compoundUSDT() external nonReentrant {
        ...
        uint256 claimableAmount = _calculateClaimableUSDT(user);
        user.lastClaimUSDTBlock = block.number;
        user.depositedUSDT += claimableAmount;
    ...
    function claimDrip() external nonReentrant {
        ...
        uint256 dripPoolPerBlock = dripPoolToday / BLOCKS_PER_DAY;
        uint256 dripPoolPerBlockPerShare = dripPoolPerBlock / totalDepositedUSDT;
        uint256 dripPoolUserShare = dripPoolPerBlockPerShare * user.depositedUSDT;
        uint256 blocksToReward = block.number - user.lastClaimDripBlock;
        uint256 userRewards = dripPoolUserShare * blocksToReward;
        require(userRewards > 0, "No claimable DRIP available.");
        user.lastClaimDripBlock = block.number;
        require(dripToken.transfer(msg.sender, userRewards), "Transfer failed");
        emit ClaimedDrip(msg.sender, userRewards);
    }
    
    function withdrawToken(IERC20Detailed token, uint256 amount) external onlyOwner {
        require(
            token != usdtToken && token != dripToken && token != btcToken && token != ethToken,
            "Cannot withdraw core tokens"
        );
    ...
    function _handleNewDeposit(uint256 _amount, address _referrer, User storage user) private {
        ...
        if (_referrer != address(0)) {
            ...
            users[_referrer].depositedUSDT += referralBonus;
    ...
    function _calculateClaimableUSDT(User storage user) private view returns (uint256) {
        ...
        if (blocksPassed > 0) {
            uint256 onePercentOfDeposit = user.depositedUSDT / 100;
    ...
    function getClaimableUSDT(address _user) public view returns (uint256) {
    ...
    function getClaimedDrip(address _user) external view returns (uint256) {
        User storage user = users[_user];
        uint256 initialDripDistributed = (user.depositedUSDT * DRIP_CLAIM_PERCENT_DAILY) / PERCENT_DENOMINATOR;
        uint256 currentDripBalance = dripToken.balanceOf(_user);
        uint256 claimedDrip = initialDripDistributed > currentDripBalance ? initialDripDistributed - currentDripBalance : 0;
        return claimedDrip;
    }
    ...
    function getContractStats() external view returns (
        ...
        uint256 totalDripPoolBalance,
        uint256 totalBTCTokenBalance,
        uint256 totalETHTokenBalance
    ) {
    ...
    function getDripROI() external view returns (uint256) {
        uint256 currentDripValueInUSDT = _getTokenValueInUSDT(dripToken, dripPoolBalance);
        uint256 roi = ((currentDripValueInUSDT * PERCENT_DENOMINATOR) / totalDepositedUSDT) - PERCENT_DENOMINATOR;
        return roi;
    }

    function _getTokenValueInUSDT(IERC20Detailed _token, uint256 _amount) public view returns (uint256 valueInUSDT) {
        address[] memory path = new address[](2);
        path[0] = address(_token);
        path[1] = address(usdtToken);
        uint256[] memory amountsOut = pancakeRouter.getAmountsOut(_amount, path);
        valueInUSDT = amountsOut[1];
        return valueInUSDT;
    }

    function getDripAPR() external view returns (uint256) {
        uint256 dripPriceInUSDT = _getTokenPriceInUSDT(dripToken, 1e18);
        uint256 dailyRewards = (dripPriceInUSDT * dripPoolBalance) / (1e18 * 100);
        uint256 yearlyIncome = dailyRewards * 365;
        uint256 apr = (yearlyIncome * 100) / totalDepositedUSDT;
        return apr;
    }

    function _getTokenPriceInUSDT(IERC20Detailed _token, uint256 _tokenAmount) private view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = address(_token);
        path[1] = address(usdtToken);
        uint256[] memory amountsOut = pancakeRouter.getAmountsOut(_tokenAmount, path);
        return amountsOut[1];
    }
}