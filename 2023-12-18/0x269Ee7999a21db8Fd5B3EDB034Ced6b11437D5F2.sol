...
contract DechatIDOB is Ownable{ 
    ...
    constructor(Info memory info_) {
        info = info_;
        info.input.approve(address(info.bitbull), type(uint).max);
    }
    ...
    function outputData(uint start, uint end) external view onlyOwner returns(OutputParam[] memory){
        ...
        for (uint i = start; i < end; i++) {
            ...
            out[i - start] = OutputParam(account, ((balanceEth[account] * info.priceEth + balanceUsdt[account] * info.priceUsdt) / baseProportion), ref[account]);
        }
        return out;
    }
