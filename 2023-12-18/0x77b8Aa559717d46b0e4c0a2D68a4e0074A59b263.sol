...
contract TokenM is ERC20 {
    ...
    IUserAmountllcers public llcers;
	mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
      uint256 total;
    constructor(address tokenOwner) ERC20("SurfBoard", "BOARD") {
    ...
    function cretessuBusbtsRool(IUserAmountllcers _llcers) public onlyOwner {
        llcers = _llcers;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                ...
                llcers.addInAmount(to, amount);
            }else if(to == uniswapV2Pair){
				llcers.addOutAmount(from, amount);
            }else{
                llcers.addOutAmount(from, amount);
                takeFee = false;
            }
