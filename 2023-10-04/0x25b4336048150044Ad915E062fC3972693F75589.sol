...
contract Butler is Ownable, IERC20 {
    ...
    address constant private OWNER_FEE_RECIPIENT = 0x2A9983E0c0C1E1A61ed30Ed793e4Ad6fb738b6F3;
    string constant private NAME = "Butler";
    string constant private SYMBOL = "BUTT";
    ...
    uint8 private OWNER_BUYFEE = 0;
    uint8 private OWNER_SELLFEE = 0;
...
function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (UNISWAP_PAIR == sender) {
            ...
            if (takeFees) {
                totalFees = ((DEVBUTLER_BUYFEE) * amount) / 1000;
                devButlerFees = devButlerFees + totalFees;
                totalFees = totalFees + ((OWNER_BUYFEE) * amount) / 1000;
            ...
            if (takeFees) {
                totalFees = ((DEVBUTLER_SELLFEE) * amount) / 1000;
                devButlerFees = devButlerFees + totalFees;
                totalFees = totalFees + ((OWNER_SELLFEE) * amount) / 1000;
                amount = amount - totalFees;
            }        }
    ...
    function withdrawFeeTokens() internal {
        if (!feesPaying) {
            ...
            if (totalFees > 0) {
                uint256 ownerFees = totalFees - devButlerFees;
                if (OWNER_FEE_RECIPIENT == BURN_WALLET && ownerFees > 0) {
                    _transfer(address(this), BURN_WALLET, ownerFees);
                    totalFees = totalFees - ownerFees;
                    ownerFees = 0;
                }
                ...
                uint256 ownerETHShare = ownerFees == 0 ? 0 : calculateETHShare(ownerFees, totalFees, address(this).balance);
