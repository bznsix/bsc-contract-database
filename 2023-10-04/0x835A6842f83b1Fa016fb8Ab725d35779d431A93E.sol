...
contract WeBuildIt is ERC20, Ownable {
    ...
    uint256 public buyBuybacksFee;

    uint256 public totalFeesSell;
    uint256 public sellMarketingFee;
    uint256 public sellLiquidityFee;
    uint256 public sellBuybacksFee;

    uint256 public totalFeesTransfer;
    uint256 public transferMarketingFee;
    uint256 public transferLiquidityFee;
    uint256 public transferBuybacksFee;

    uint256 public tokensForMarketing;
    uint256 public tokensForLiquidity;
    uint256 public tokensForBuybacks;
    ...
    constructor() ERC20("We Build It", "BUILD") {
        ...
        uint256 _buyBuybacksFee = 0;

        uint256 _sellMarketingFee = 0;
        uint256 _sellLiquidityFee = 10;
        uint256 _sellBuybacksFee = 10;

        uint256 _transferMarketingFee = 0;
        uint256 _transferLiquidityFee = 0;
        uint256 _transferBuybacksFee = 0;

        uint256 totalSupply = 1000000 * 1e18;

        maxTransactionAmount = (totalSupply * 20) / 1000; // 1.5% of total supply
        maxWallet = (totalSupply * 20) / 1000; // 2.5% of total supply

        swapTokensAtAmount = (totalSupply * 5) / 10000; // 0.05% swapback trigger

        buyMarketingFee = _buyMarketingFee;
        buyLiquidityFee = _buyLiquidityFee;
        buyBuybacksFee = _buyBuybacksFee;
        totalFeesBuy = buyMarketingFee + buyLiquidityFee + buyBuybacksFee;

        sellMarketingFee = _sellMarketingFee;
        sellLiquidityFee = _sellLiquidityFee;
        sellBuybacksFee = _sellBuybacksFee;
        totalFeesSell = sellMarketingFee + sellLiquidityFee + sellBuybacksFee;

        transferMarketingFee = _transferMarketingFee;
        transferLiquidityFee = _transferLiquidityFee;
        transferBuybacksFee = _transferBuybacksFee;
        totalFeesTransfer = transferMarketingFee + transferLiquidityFee + transferBuybacksFee;

        marketingWallet = address(0x63Ed65ba80999449E0b38742E54BAA033842A3Db); 
        buybacksWallet = address(0x41560a4D24Bf2a7A0566B75077943C5FA62D8F0c);
        lpWallet = msg.sender;
    ...
    /// @param _buybacksFee The fee for the buybacks wallet
    function updateBuyFees(
        uint256 _marketingFee,
        uint256 _liquidityFee,
        uint256 _buybacksFee
    ) external onlyOwner {
        buyMarketingFee = _marketingFee;
        buyLiquidityFee = _liquidityFee;
        buyBuybacksFee = _buybacksFee;
        totalFeesBuy = buyMarketingFee + buyLiquidityFee + buyBuybacksFee;
    }
    ...
    function updateTransferFees(
        ...
        transferBuybacksFee = _buybacksFee;
        totalFeesTransfer = transferMarketingFee + transferLiquidityFee + transferBuybacksFee;
    }
    ...
    function updateBuybacksWallet(address newWallet) external onlyOwner {
        emit buybacksWalletUpdated(newWallet, buybacksWallet);
        buybacksWallet = newWallet;
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to] && totalFeesSell > 0) {
                ...
                tokensForBuybacks += (fees * sellBuybacksFee) / totalFeesSell;
            ...
            // on buy
                ...
                tokensForBuybacks += (fees * buyBuybacksFee) / totalFeesBuy;
                tokensForMarketing += (fees * buyMarketingFee) / totalFeesBuy;
            }

            // on transfer
            else if (totalFeesTransfer > 0) {
                fees = amount.mul(totalFeesTransfer).div(100);
                tokensForLiquidity += (fees * transferLiquidityFee) / totalFeesTransfer;
                tokensForBuybacks += (fees * transferBuybacksFee) / totalFeesTransfer;
                tokensForMarketing += (fees * transferMarketingFee) / totalFeesTransfer;
            }
    ...
    function swapBack() private {
        ...
        uint256 ethForBuybacks = ethBalance.mul(tokensForBuybacks).div(totalTokensToSwap);

        uint256 ethForLiquidity = ethBalance - ethForMarketing - ethForBuybacks;

        tokensForLiquidity = 0;
        tokensForMarketing = 0;
        tokensForBuybacks = 0;

        (success, ) = address(buybacksWallet).call{value: ethForBuybacks}("");
        ...
        onlyOwner
    {
        require(
            ...
            "Cannot withdraw this token from contract."
        );
