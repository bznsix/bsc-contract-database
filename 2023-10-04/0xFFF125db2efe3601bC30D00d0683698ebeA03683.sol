...
contract JNReward{
	...
	constructor(address _jnAddress) {
		JN = IERC20(_jnAddress);
	}
	
	function withdraw() external returns(bool){
		uint256 jnAmount = JN.balanceOf(address(this));
		if(jnAmount > 0){
            JN.transfer(msg.sender,jnAmount);
		}
        return true;
	}
}

contract XN is ERC20 {
    using SafeMath for uint256;
	...
	address _baseToken = address(0xFFDb88c2cd07598C622f344487a41bfdB99959cD);
    ...
    constructor(address tokenOwner) ERC20("XN", "X1N") {
        require(_baseToken < address(this),"XN small");
		...
		JN = IERC20(_baseToken);
        JN.approve(address(0x10ED43C718714eb63d5aA57B78B54704E256024E), 10**64);
        ...
        reward = new JNReward(_baseToken);
    ...
    mapping(address => bool) isParter;
    function setMultipleAccountsForParter(address[] calldata accounts, bool excluded) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            address acc = accounts[i];
            isParter[acc] = excluded;
        }
    }
	...
	function openSwapStatas() public onlyOwner {
	...
	mapping(address => uint256) public _parterBuyAmount;
    function _transfer(
        address from,
        address to,
        uint256 amount
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                if(startTime.add(600) > block.timestamp){
                    require(isParter[to], "isParter");
					_parterBuyAmount[to] = _parterBuyAmount[to].add(amount);
                    require(_parterBuyAmount[to] <= 50 * 10**18, "one address max");
                }else{
                    if(startTime.add(610) > block.timestamp){amount = amount.div(100);}
                }
			}else if(_isPairs[to]){
				require(startTime < block.timestamp,"startTime");
                _splitUsdtLdxToken();
                ...
                if(startTime.add(600) > block.timestamp){
                    require(super.balanceOf(to).add(amount) <= 50 * 10**18, "one address max");
                }
            }
        }
        super._transfer(from, to, amount);
        ...
        if(to == uniswapV2Pair){
			...
			if(isAddLdx){
				...
				if(lpTokenAmount >= amount){
                    ...
                    _haveLpAmount[from] = _haveLpAmount[from].add(lpAddAmount.mul(101).div(100));
                    super._transfer(from, to, amount);
    ...
    function swapAndLiquify() private {
		...
		if(allTokenAmount >= 10**17){
            uint256 beforeAmount = JN.balanceOf(address(this));
            swapTokensForJN(allTokenAmount);
            uint256 afterAmount = JN.balanceOf(address(this));
            if(afterAmount > beforeAmount){
                uint256 newAmount = afterAmount - beforeAmount;
                JN.transfer(_destroyAddress, newAmount.div(2));
            }
		}
    }
	...
	function swapTokensForJN(uint256 tokenAmount) private {
    ...
    function _splitUsdtLdxToken() private {
        uint256 allTokenAmount = JN.balanceOf(address(this));
    ...
    function _splitOtherUsdtLdx(uint256 thisAmount) private {
		uint256 ldxSize = ldxUser.length;
		uint256 totalLpAmount = pair.totalSupply();
		if(ldxSize>0 && totalLpAmount > 0){
			address user;
            uint256 lpRate;
			uint256 ldxRewardTokenAmount;
			if(ldxSize >20){
				for(uint256 i=0;i<20;i++){
					if(startLdxIndex >= ldxSize){startLdxIndex = 0;}
					user = ldxUser[startLdxIndex];
                    lpRate = pair.balanceOf(user).mul(1000).div(totalLpAmount);
                    if(lpRate > 0){
                        ldxRewardTokenAmount = lpRate.mul(thisAmount).div(1000);
                        JN.transfer(user, ldxRewardTokenAmount);
                    }
					startLdxIndex = startLdxIndex.add(1);
				}
			}else{
				for(uint256 i=0;i<ldxSize;i++){
					user = ldxUser[i];
					lpRate = pair.balanceOf(user).mul(1000).div(totalLpAmount);
                    if(lpRate > 0){
                        ldxRewardTokenAmount = lpRate.mul(thisAmount).div(1000);
