...
telegram: https://t.me/EchibaOfficial
...
contract Echiba is Context, IERC20 {
    ...
    address payable public constant _Burn =
        payable(0x000000000000000000000000000000000000dEaD);
    address payable public _Marketing =
        payable(0x2cf465138DF43aeCF43f867D041c8Eb5448d332d);
    address payable public _Dev =
        payable(0x2cf465138DF43aeCF43f867D041c8Eb5448d332d);

    string private constant _name = "Echiba";
    string private constant _symbol = unicode"ECHIBA";
    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 10**9 * 10**_decimals;

    uint256 public _Buy__Fee = 6;
    uint256 public _Sell__Fee = 6;

    uint8 private txCount = 0;
    uint8 private swapTrigger = 10;

    uint256 public _Auto_Burn = 0;
    uint256 public _Auto_LP = 20;
    uint256 public _Auto_Marketing = 70;
    uint256 public _Auto_Dev = 10;
    ...
    constructor() {
        _owner = 0x2cf465138DF43aeCF43f867D041c8Eb5448d332d;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 tokens_to_Burn = (contractTokenBalance * _Auto_Burn) /
            100;
        _tTotal = _tTotal - tokens_to_Burn;
        _tOwned[_Burn] = _tOwned[_Burn] + tokens_to_Burn;
        _tOwned[address(this)] = _tOwned[address(this)] - tokens_to_Burn;

        uint256 tokens_to_M = (contractTokenBalance * _Auto_Marketing) /
            100;
        uint256 tokens_to_D = (contractTokenBalance * _Auto_Dev) / 100;
        uint256 tokens_to_LP_Half = (contractTokenBalance * _Auto_LP) /
            200;
        ...
        uint256 split_M = (_Auto_Marketing * 100) /
            (_Auto_LP + _Auto_Marketing + _Auto_Dev);
        uint256 BNB_M = (BNB_Total * split_M) / 100;

        uint256 split_D = (_Auto_Dev * 100) /
            (_Auto_LP + _Auto_Marketing + _Auto_Dev);
        ...
        sendToWallet(_Marketing, BNB_M);

        BNB_Total = address(this).balance;
        sendToWallet(_Dev, BNB_Total);
    }
    ...
    function _tokenTransfer(
        ...
        if (!takeFee) {
            ...
            if (recipient == _Burn) _tTotal = _tTotal - tAmount;
        } else if (isBuy) {
            ...
            if (recipient == _Burn) _tTotal = _tTotal - tTransferAmount;
        } else {
