...
// aaaaaaaaaaa  ANIMALIA GENESIS ARCANA  aaaaaaaaaaa
// aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
    ...
    ReentrancyGuard
{
    ...
    uint8 public whitelistMintMaxCount;
    uint8 public constant RESERVED = 66;
    uint8 public constant MAX_MINT_PER_BLOCK = 150;
    uint256 private immutable _cap;

    MintFee public whitelistMintFee;
    MintFee public publicMintFee;
    ...
    modifier canMint(uint8 amount) {
        ...
        require(
            _tokenIdCounter.current() > RESERVED,
            "Reserved not minted yet"
        );
        require(amount > 0, "Amount must be greater than 0");
        require(amount <= MAX_MINT_PER_BLOCK, "Amount cannot exceed 150");
        require(totalSupply() + amount <= cap(), "Max supply exceeded");
        _;
    }
    ...
    constructor(
        ...
        uint256 cap_,
        address reservedReceiver,
        address defaultRoyaltyReceiver,
        uint96 defaultRoyaltyFeeNumerator,
        MintFee memory _whitelistMintFee,
        MintFee memory _publicMintFee,
        uint8 _whitelistMintMaxCount
        ...
        whitelistMintMaxCount = _whitelistMintMaxCount;

        // Skip tokenId 0
        _tokenIdCounter.increment();

        // Reserve tokenIds
        while (_tokenIdCounter.current() <= RESERVED) {
            _mintToken(reservedReceiver);
        }
    }
    ...
    function setWhitelistMintFee(MintFee memory _whitelistMintFee)
        external
        onlyOwner
    {
        whitelistMintFee = _whitelistMintFee;
    ...
    function setPublicMintFee(MintFee memory _publicMintFee)
        external
        onlyOwner
    {
        publicMintFee = _publicMintFee;
    ...
    function mintRemaining(address to, uint8 amount)
        external
        nonReentrant
        onlyOwner
        canMint(amount)
    ...
    function whitelistMint(
        ...
        require(
            balanceOf(_msgSender()) + amount <= whitelistMintMaxCount,
            "Reached whitelist mint limit"
        );

        uint256 fee = whitelistMintFee.amount * amount;

        _transferIn(whitelistMintFee.currency, fee);
        _transferOut(whitelistMintFee.receiver, whitelistMintFee.currency, fee);
        ...
        canMint(amount)
    {
        ...
        uint256 fee = publicMintFee.amount * amount;

        _transferIn(publicMintFee.currency, fee);
        _transferOut(publicMintFee.receiver, publicMintFee.currency, fee);
    ...
    function setWhitelistMintMaxCount(uint8 _whitelistMintMaxCount)
        external
        onlyOwner
    {
        whitelistMintMaxCount = _whitelistMintMaxCount;
    ...
    function recover(uint256 amount) external nonReentrant onlyOwner {
        require(amount > 0, "Amount must be greater than 0");
        uint256 balance = address(this).balance;
        require(amount <= balance, "Amount greater than balance");
