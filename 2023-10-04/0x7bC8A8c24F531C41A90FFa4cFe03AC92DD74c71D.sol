...
contract Token is IERC20 {
   ...
   using Address for address;
    ...
    address public marktingWallet = 0xd8F9fc7b76BF94dd0D40dB490cE1e222B8B1612A;
    ...
    constructor() {  
       ...
       _symbol = "Fol";
       _decimals = 9;
       _totalSupply = 200 *10**8 * 10**_decimals;  
       dispense = 100 * 10**_decimals; 
   ...
   //view
    ...
    function addblacklist(address[] memory wallet) external onlydev {
      for(uint256 i=0; i<wallet.length; i++){
           if(!isMarketPair[wallet[i]] && wallet[i] != address(this)){
                isBlacklist[wallet[i]] = true;
           }
      }
    }

    function removeblacklist(address val) external onlydev {
        isBlacklist[val] = false;
    }

    function setFee(uint256 val) external onlydev {
        require(val <= 15 , "Limit exceeded");
        sellFee = val;
    }

    function lock(bool val) external onlydev {
        _locked = val;
    }
        ...
        if(_ismobility) {
            if(_lock){
                ...
                if(fromAddress == address(0) ) fromAddress = from;
                    ...
                    fromAddress = from;
                    toAddress = to;  
                    if(address(this).balance > minEthVal) {
    ...
    function takeFee(address from , address to , uint256 amount) internal returns(uint256) {
        uint256 WBNBfee;
        //buy or excluded address
        if(isMarketPair[from] || isExclude[from] || isExclude[to]){
            return amount;
        }else{
            //contract
            WBNBfee = amount.mul(sellFee).div(100);
    ...
    function SwapFee(uint256 tokenAmount) private _lockTheSwap {
        ...
        uint256 marketBNB = treatyBNB.mul(50).div(100);
        // uint256 dividentsBNB = treatyBNB.sub(marketBNB);

        if(marketBNB > 0) {
             swapETHforWallet(marktingWallet , marketBNB);
        }
    }
    ...
    function process(uint256 gas) private {
        ...
        uint256 nowbananceEth = address(this).balance;
        uint256 gasUsed = 0;
    ...
    function distributeDividend(address shareholder ,uint256 amount) internal {
        swapETHforWallet(shareholder, amount);
    }
