...
contract RainiLpStakingPoolv3 is AccessControl {
    ...
    IRainiLpStakingPoolv3.GeneralRewardVars public generalRewardVars;

    // account specific variables

    mapping(address => IRainiLpStakingPoolv3.AccountRewardVars) public accountRewardVars;
    mapping(address => IRainiLpStakingPoolv3.AccountVars) public accountVars;
    mapping(address => uint256) public staked;

    constructor(address _rainiLpToken, address _xphotonToken) {
    ...
    function setPhotonToken(address _photonTokenAddress) external onlyOwner {
        require (_photonTokenAddress != address(rainiLpToken), "bad addr");
        photonToken = IERC20(_photonTokenAddress);
    }

    function setGeneralRewardVars(IRainiLpStakingPoolv3.GeneralRewardVars memory _generalRewardVars) external onlyEditor {
        generalRewardVars = _generalRewardVars;
    }

    function setAccountRewardVars(address _user, IRainiLpStakingPoolv3.AccountRewardVars memory _accountRewardVars) external onlyEditor {
        accountRewardVars[_user] = _accountRewardVars;
    }

    function setAccountVars(address _user, IRainiLpStakingPoolv3.AccountVars memory _accountVars) external onlyEditor {
        accountVars[_user] = _accountVars;
    }

    function stakeLpTokens(address _user, uint256 _amount) external onlyEditor {
        rainiLpToken.safeTransferFrom(_user, address(this), _amount);
        staked[_user] = staked[_user] + _amount;
        totalSupply = totalSupply + _amount;
    }

    function withdrawLpTokens(address _user, uint256 _amount) external onlyEditor {
        staked[_user] = staked[_user] - _amount;
        totalSupply = totalSupply - _amount;
        rainiLpToken.safeTransfer(_user, _amount);
    }
