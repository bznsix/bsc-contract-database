...
import "./libs/IZeusReferral.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/utils/ReentrancyGuard.sol";

import "./ZeusToken.sol";

// MasterChef is the master of Zeus. He can make Zeus and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Initial emission rate: 1 ZEUS per block.
    uint256 public constant INITIAL_EMISSION_RATE = 1 ether;
    // Minimum emission rate: 0.1 ZEUS per block.
    ...
    // Zeus referral contract address.
    IZeusReferral public zeusReferral;
    ...
    constructor(
        ...
        zeusPerBlock = INITIAL_EMISSION_RATE;
    }
    ...
    // Update the given pool's ZEUS allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingZeus(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accZeusPerShare = accZeusPerShare.add(zeusReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 zeusReward = multiplier.mul(zeusPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        zeus.mint(devAddress, zeusReward.div(10));
        zeus.mint(address(this), zeusReward);
        pool.accZeusPerShare = pool.accZeusPerShare.add(zeusReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for ZEUS allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (_amount > 0 && address(zeusReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            zeusReferral.recordReferral(msg.sender, _referrer);
        }
        if (user.amount > 0) {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(zeus)) {
    ...
    function updateEmissionRate() public {
        ...
        require(zeusPerBlock > MINIMUM_EMISSION_RATE, "updateEmissionRate: Emission rate has reached the minimum threshold");
        ...
        uint256 newEmissionRate = zeusPerBlock;
        ...
        if (newEmissionRate >= zeusPerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = zeusPerBlock;
        zeusPerBlock = newEmissionRate;
        emit EmissionRateUpdated(msg.sender, previousEmissionRate, newEmissionRate);
    }

    // Update the zeus referral contract address by the owner
    function setZeusReferral(IZeusReferral _zeusReferral) public onlyOwner {
        zeusReferral = _zeusReferral;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(zeusReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = zeusReferral.getReferrer(_user);
            uint256 commissionAmount = _pending.mul(referralCommissionRate).div(10000);

            if (referrer != address(0) && commissionAmount > 0) {
                zeus.mint(referrer, commissionAmount);
...
contract ZeusToken is BEP20("ZeusFarms Token", "ZEUS") {
    // Burn address
    ...
    /// @dev overrides transfer function to meet tokenomics of ZEUS
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        if (recipient == BURN_ADDRESS) {
            ...
            require(amount == sendAmount + burnAmount, "ZEUS::transfer: Burn value invalid");
