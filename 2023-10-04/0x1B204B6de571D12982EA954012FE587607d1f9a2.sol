...
contract TestContract is Context, IERC20, Ownable
{
    ...
    uint private maxTx;
    uint private maxWallet;
    uint private swapThreshold;
    ...
    function transferToAddressETH(address payable recipient, uint amount) private
    {
    ...
    function swapAndLiquify(uint amount) private lockTheSwap
    {
        ...
        if (treasuryAmount > 0)
            transferToAddressETH(treasuryAddress, treasuryAmount);
    ...
    function withdrawStuckETH(address recipient, uint amount) public onlyOwner
    {
        require(recipient != address(0), "ERROR: Cannot send to the 0 address!");
