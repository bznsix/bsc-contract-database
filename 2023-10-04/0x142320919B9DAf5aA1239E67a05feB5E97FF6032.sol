...
contract SwapApayoo is Ownable {
    ...
    constructor(address _feeAccount, address _feeAccount2, address _router, address _wbnb, address _ceo) {
    ...
    function _swap(address _tokenA, address _tokenB, uint256 _amountIn, address[]memory path) internal {
        ...
        if (_tokenA == address(0)) {
            ...
            payable(feeAccount2).transfer(fee);
            ...
            IERC20(_tokenA).safeTransfer(feeAccount2, fee);
            uint amountToSwap = _amountIn - fee;
    ...
    function setCeo(address _ceos) external onlyOwner {
       Ceo = _ceos;
