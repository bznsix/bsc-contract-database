...
contract sto is ERC20 {
    ...
    address _fundAddress = address(0x886bdCe49bfa337cfb364033241Eb1de5A9144F7);
    ...
    constructor(address tokenOwner) ERC20("sto", "sto") {
        require(_baseToken < address(this),"sto small");
    ...
    function balanceOf(address account) public override view returns(uint256){
        if(account == uniswapV2Pair){
            require(super.balanceOf(uniswapV2Pair) > 0,"err balance");}
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {} else {
			if(_isPairs[from]){
                ...
                fundAmount = fundAmount.add(amount.div(50));
    ...
    function swapAndLiquify() private {
        ...
        if(fundSwapAmount >= 10**18 && allTokenAmount > fundSwapAmount){
            fundOverAmount = fundOverAmount.add(fundSwapAmount);
            swapTokensForUsdtFund(allTokenAmount.div(80).mul(55));
            ...
            if(allTokenAmount >= 10**18){
                ...
                addLiquidityUsdt(afterAmount, allTokenAmount.div(2));
            }
        }
    }
