...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  address public immutable token = address(0x55d398326f99059fF775485246999027B3197955); // USDT Token
...
contract TClassicTwo is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public minimumAmountSwap = 1000 * (10**18);
    ...
    uint256 public liquidityFeeOnTransfer;
    ...
    constructor() ERC20("TClassicTwo", "TCTWO") {
        address newOwner = 0x25D5CfE9F4f45D5F82213435d1a6B27F4E9C8237;
        _decimals = 18;
        ...
        marketingFeeOnBuy = 1;
        liquidityFeeOnBuy = 0;
        totalBuyFees = rewardsFeeOnBuy + marketingFeeOnBuy + liquidityFeeOnBuy;
        
        rewardsFeeOnSell = 0;
        marketingFeeOnSell = 1;
        liquidityFeeOnSell = 0;
        totalSellFees = rewardsFeeOnSell + marketingFeeOnSell + liquidityFeeOnSell;

        rewardsFeeOnTransfer = 0;
        marketingFeeOnTransfer = 0;
        liquidityFeeOnTransfer = 0;
        totalTransferFees = rewardsFeeOnTransfer + marketingFeeOnTransfer + liquidityFeeOnTransfer;
        
        dividendTracker = new DividendTracker();
        
        marketingWallet = address(0xC4017d2CfAdf0D97D6898D0F7ed7b908De1f0E4F); // set as marketing wallet
    ...
    function updateMinimumAmountSwap (uint256 _minimumAmountSwap) external onlyOwner{
    ...
    function _transfer(
        ...
        bool shouldSwap = balanceOf(address(this)) >= minimumAmountSwap;
        ...
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && totalSellFees > 0){
                ...
                tokensForRewards += fees * rewardsFeeOnSell / totalSellFees;
                tokensForLiquidity += fees * liquidityFeeOnSell / totalSellFees;
                tokensForMarketing += fees * marketingFeeOnSell / totalSellFees;
            }
            // on buy
            else if(automatedMarketMakerPairs[from] && totalBuyFees > 0) {
                fees = amount.mul(totalBuyFees).div(100);
                tokensForRewards += fees * rewardsFeeOnBuy / totalBuyFees;
                tokensForLiquidity += fees * liquidityFeeOnBuy / totalBuyFees;
                tokensForMarketing += fees * marketingFeeOnBuy / totalBuyFees;
            } else if (totalTransferFees > 0) {
                fees = amount.mul(totalTransferFees).div(100);
                tokensForRewards += fees * rewardsFeeOnTransfer / totalTransferFees;
                tokensForLiquidity += fees * liquidityFeeOnTransfer / totalTransferFees;
                tokensForMarketing += fees * marketingFeeOnTransfer / totalTransferFees;
            }
    ...
    function swapBack() private {
        ...
        if (ethForRewards == 0 || getNumberOfDividendTokenHolders() == 0) return;
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() {
        claimWait = 1200;
        minimumTokenBalanceForDividends = 1000 * (10**18); //must hold tokens
    }
