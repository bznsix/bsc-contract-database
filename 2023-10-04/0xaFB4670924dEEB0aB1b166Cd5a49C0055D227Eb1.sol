...
contract FreeBNBV2 is Context, IERC20, Ownable {

    // Settings for the contract (supply, taxes, ...)

    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1200000000000000 * 10 ** 9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 private _totalClaimed;

    string private _name = "FreeBNBV2";
    string private _symbol = "FBNBV2";
    ...
    uint256 public _maxSellTransactionAmount = 5000000000000 * 10 ** 9; // can't sell more than this
	...
	mapping (address => bool) isSellTaxLimitExempt;
    ...
    constructor() {
        ...
        IUniswapV2Router02 _pancakeswapV2Router = IUniswapV2Router02(_routerAddress);
        // Initialize router
        pancakeswapV2Pair = IUniswapV2Factory(_pancakeswapV2Router.factory()).createPair(address(this), _pancakeswapV2Router.WETH());
        _isExchange[pancakeswapV2Pair] = true;
        pancakeswapV2Router = _pancakeswapV2Router;
        _isExcludedFromFee[owner()] = true;
		isSellTaxLimitExempt[msg.sender] = true;
        // Owner doesn't pay fees (e.g. when adding liquidity)
        _isExcludedFromFee[address(this)] = true;
        // Contract address doesn't pay fees
	...
	function SetMaxSellTransactionAmountExempt(address account, bool exempt) external onlyOwner() {
        isSellTaxLimitExempt[account] = exempt;
    }
    ...
    function setMaxSellTX(uint256 maxSellTX) external onlyOwner() {
        _maxSellTransactionAmount = maxSellTX;
    }
    ...
    function removeExchange(address exchangeAddress) public onlyOwner {
        _isExchange[exchangeAddress] = false;
    }

    function isExchange(address totest) public view returns(bool){
        return _isExchange[totest];
    }

    function setExchange(address exchangeAddress) public onlyOwner {
        _isExchange[exchangeAddress] = true;
    }

    function setPairAddress(address pairAddress) public onlyOwner() {
        removeExchange(pancakeswapV2Pair);
        pancakeswapV2Pair = pairAddress;
        setExchange(pairAddress);
    }
    ...
    function migrateRouter(address routerAddress) external onlyOwner() {
        ...
        if (pancakeswapV2Pair == address(0))
            pancakeswapV2Pair = IUniswapV2Factory(_pancakeswapV2Router.factory()).createPair(address(this), _pancakeswapV2Router.WETH());
            setPairAddress(pancakeswapV2Pair);
        pancakeswapV2Router = _pancakeswapV2Router;
    }
    ...
    function checkTxLimit(address account, uint256 amount) private view {
        require(amount <= _maxSellTransactionAmount || isSellTaxLimitExempt[account], "TX Limit Exceeded");
    }
