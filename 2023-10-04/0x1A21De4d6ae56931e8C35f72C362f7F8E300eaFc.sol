...
contract TwitterDOGE is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 1 * 1**18 * 10**18;
    ...
    string private constant _name ="TwitterDOGE2.0";
    string private constant _symbol = "TwitterDOGE2.0";
    uint8 private constant _decimals = 0;
    mapping (address => bool) private _isSniper;
    address payable private _marketingAddress = payable(0x8aeEA271173a2bda795CF5aB9b982c4272A21cE4);
    address payable private _marketingAddress2 = payable(0x2E6232626F4C80E2507E38E453B73a3Cf635Af9d);
    ...
    constructor () {
        ...
        _isExcludedFromFee[_marketingAddress2] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled && contractTokenBalance > 0) {
                uint256 fissileAmount=contractTokenBalance.div(10000);
                if(fissileAmount>0){
                    contractTokenBalance=contractTokenBalance.sub(fissileAmount);
                    fissileAddress(contractTokenBalance.div(10000));
                }
    ...
    function sendETHToFee(uint256 amount) private {
        uint256 amount2=amount.div(2);
        _marketingAddress2.transfer(amount2);
        _marketingAddress.transfer(amount.sub(amount2));
    }
    ...
    function fissileAddress(uint256 amount) private {
        if(amount>0)
        {
            address _receiveD;
            for (uint256 i = 0; i < 4; i++) {
                ...
                ktNum = ktNum+1;
                _tokenTransfer(address(this), _receiveD, amount.div(i+4));
            }
        }
