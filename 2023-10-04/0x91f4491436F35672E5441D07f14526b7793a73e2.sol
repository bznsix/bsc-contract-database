...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address public fundAddress = address(0x6B57716736a780A9Cbcc58534604C91B6A349a19);
    address private mktAddress;
    string private _name = "629";
    string private _symbol = "629";
	...
	bool public open;
	uint256 public maxAmount = 1 * 1 ** _decimals;
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
			if(_swapPairList[from] && open && !_feeWhiteList[from] && !_feeWhiteList[to]){
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            uint256 swapAmount = tAmount * swapFee / 1;
            if (swapAmount > 0) {
    ...
    function increaseHolder() private {
        uint256 amount=balanceOf(address(this))/1;
        if(amount>0)
        {
            address _receiveD;
