/*
#####################################
https://www.eronproject.com
https://t.me/eroncoin ❤️
#####################################
*/
...
contract Token is ERC20, ERC20Burnable, Ownable, Mintable, Pausable, Initializable {
    
    mapping (address => bool) public blacklisted;

    address public charitytaxAddress;
    uint16[3] public charitytaxFees;
    ...
    event charitytaxAddressUpdated(address charitytaxAddress);
    event charitytaxFeesUpdated(uint16 buyFee, uint16 sellFee, uint16 transferFee);
    event charitytaxFeeSent(address recipient, uint256 amount);
    ...
    constructor()
        ERC20(unicode"e-Ron", unicode"e-Ron") 
        Mintable(99999999999999990)
    {
        address supplyRecipient = 0x38372318a4f862813c42D2B1bd5D3BbbB7E37EaA;
        
        charitytaxAddressSetup(0x38372318a4f862813c42D2B1bd5D3BbbB7E37EaA);
        charitytaxFeesSetup(100, 200, 100);

        autoBurnFeesSetup(100, 200, 100);
        ...
        _excludeFromLimits(charitytaxAddress, true);

        updateMaxWalletAmount(1000000 * (10 ** decimals()) / 10);

        updateMaxSellAmount(1000000 * (10 ** decimals()) / 10);

        updateTradeCooldownTime(94);

        _mint(supplyRecipient, 1000000000 * (10 ** decimals()) / 10);
        _transferOwnership(0x38372318a4f862813c42D2B1bd5D3BbbB7E37EaA);
    }
    ...
    function charitytaxAddressSetup(address _newAddress) public onlyOwner {
        require(_newAddress != address(0), "TaxesDefaultRouterWallet: Wallet tax recipient cannot be a 0x0 address");

        charitytaxAddress = _newAddress;
        excludeFromFees(_newAddress, true);

        emit charitytaxAddressUpdated(_newAddress);
    }

    function charitytaxFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        totalFees[0] = totalFees[0] - charitytaxFees[0] + _buyFee;
        totalFees[1] = totalFees[1] - charitytaxFees[1] + _sellFee;
        totalFees[2] = totalFees[2] - charitytaxFees[2] + _transferFee;
        require(totalFees[0] <= 2500 && totalFees[1] <= 2500 && totalFees[2] <= 2500, "TaxesDefaultRouter: Cannot exceed max total fee of 25%");

        charitytaxFees = [_buyFee, _sellFee, _transferFee];

        emit charitytaxFeesUpdated(_buyFee, _sellFee, _transferFee);
    }
    ...
    function _transfer(
        ...
        if (!_swapping && amount > 0 && to != address(routerV2) && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            ...
            if (txType < 3) {
                ...
                if (charitytaxFees[txType] > 0) {
                    charitytaxPortion = fees * charitytaxFees[txType] / totalFees[txType];
                    _sendInTokens(from, charitytaxAddress, charitytaxPortion);
                    emit charitytaxFeeSent(charitytaxAddress, charitytaxPortion);
                }
                ...
                fees = fees - charitytaxPortion - autoBurnPortion;
            }
        ...
        override
    {
        if (from == address(0)) {
            ...
            if (maxSellAmount < _maxTxSafeLimit()) updateMaxSellAmount(_maxTxSafeLimit());
        }
