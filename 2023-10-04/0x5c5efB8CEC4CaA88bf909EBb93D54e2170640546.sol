...
contract RefundInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RefundInuDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Refund Inu", "REFUND") {
        marketingWallet = payable(0x1AB6b2Ee898A0af5D02b430325d2B038129a14eD);
        devWallet = payable(0x3aeBB09F4fd87Bb09B8c16BBb3F25Db22F7C5C2B);
        ...
        dividendTracker = new RefundInuDividendTracker(
            payable(this),
            router,
            0x3962eaE59Ba15910eA2ACeD462b05696B9A5435A,
            "RefundInuTRACKER",
            "RefundInuTRACKER"
        );
        ...
        uint256 totalTokenSupply = (1_000_00) * (10**18);
        _mint(owner(), totalTokenSupply); // only time internal mint function is ever called is to create supply
        maxWallet = totalTokenSupply / 1000; // 0.005%
        swapTokensAtAmount = totalTokenSupply / 100; // 0.05%;
...
contract RefundInuDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    RefundInu public RefundInuContract;
    ...
    constructor(
        ...
        defaultToken = token;
        RefundInuContract = RefundInu(mainContract);
    ...
    function _transfer(
        ...
        require(false, "RefundInu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "RefundInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RefundInu contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(RefundInuContract);

            uint256 prevBalance = RefundInuContract.balanceOf(address(this));
            ...
            {
                uint256 received = RefundInuContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    RefundInuContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
