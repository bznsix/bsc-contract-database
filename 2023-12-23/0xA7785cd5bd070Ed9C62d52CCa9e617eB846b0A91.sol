...
contract BONKGROK is ERC20, Ownable {
    ...
    address public xbuybcktxWallet = 0x6A63c726b283Ea587A41044DeeBde7790f06bE3b;
    ...
    constructor() ERC20("BONKGROK", "BONKGROK") {
        ...
        exemptFee[xbuybcktxWallet] = true;
    ...
    function _transfer(
       ...
       if (!exemptFee[sender] && !exemptFee[recipient]) {
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipient])
            ...
            feeswap = sellTaxes.liquidity + sellTaxes.xbuybcktx;
            feesum = feeswap;
            currentTaxes = sellTaxes;
        } else if (!useLaunchFee) {
            feeswap = taxes.liquidity + taxes.xbuybcktx;
            feesum = feeswap;
            currentTaxes = taxes;
        } else if (useLaunchFee) {
            feeswap = launchtax;
    ...
    function Liquify(uint256 feeswap, Taxes memory swapTaxes) private lockTheSwap {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 xbuybcktxAmt = unitBalance * 2 * swapTaxes.xbuybcktx;
            if (xbuybcktxAmt > 0) {
                payable(xbuybcktxWallet).sendValue(xbuybcktxAmt);
            }
        }
    }
    ...
    function updatexbuybcktxWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0), "Fee Address cannot be zero address");
        xbuybcktxWallet = newWallet;
    }
