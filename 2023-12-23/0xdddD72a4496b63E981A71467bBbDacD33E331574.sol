...
contract GalaxiusV2 is Ownable {
    ...
    mapping (address => AccountGame[]) public accountGameHistory;
    ...
    mapping (address => mapping (uint256 => bool)) blockGameing;
    struct Gameplay {
        uint256 _winner;
        uint256 _fail;
    }
    mapping (GAMELEVEL=> Gameplay) public gamePlayStatics;
    mapping (GAMELEVEL => uint256[]) public gameLevelRequiredAmount;
    uint256 public magnification = 10;
    ...
    constructor(
        ...
        gameLevelRequiredAmount[GAMELEVEL.LV1] = [minRequiredGaxAmount, maxRequiredGaxAmount];
        gameLevelRequiredAmount[GAMELEVEL.LV2] = [minRequiredGaxAmount, maxRequiredGaxAmount];
        gameLevelRequiredAmount[GAMELEVEL.LV3] = [minRequiredGaxAmount, maxRequiredGaxAmount];
        gameLevelRequiredAmount[GAMELEVEL.LV4] = [minRequiredGaxAmount, maxRequiredGaxAmount];
        gameLevelRequiredAmount[GAMELEVEL.LV5] = [minRequiredGaxAmount, 45000 * 10 ** 18];
        gameLevelRequiredAmount[GAMELEVEL.LV6] = [minRequiredGaxAmount, 40000 * 10 ** 18];
        gameLevelRequiredAmount[GAMELEVEL.LV7] = [minRequiredGaxAmount, 35000 * 10 ** 18];
        gameLevelRequiredAmount[GAMELEVEL.LV8] = [minRequiredGaxAmount, 30000 * 10 ** 18];
        gameLevelRequiredAmount[GAMELEVEL.LV9] = [minRequiredGaxAmount, 25000 * 10 ** 18];
        gameLevelRequiredAmount[GAMELEVEL.LV10] = [minRequiredGaxAmount, 20000 * 10 ** 18];
        gameLevelRequiredAmount[GAMELEVEL.LV11] = [minRequiredGaxAmount, 15000 * 10 ** 18];
    ...
    function start(uint256 amount, GAMELEVEL _level) public {
        require(gameLevelRequiredAmount[_level][0] <= amount && amount <= gameLevelRequiredAmount[_level][1] , "Amount error");
        require(accountGameInfo[msg.sender].length <= 0, "Treasure Chest has not been opened");
        require(!blockGameing[msg.sender][block.number], "Block is limit");
        ...
        blockGameing[msg.sender][block.number] = true;
    ...
    function openTheTreasureChest() public {
        ...
        require(accountGameItem._blockNumber > 0, "Error blockNumber");
        bytes32 blockHash = IOracle(oracleAddress).getBlockhash(accountGameItem._blockNumber.add(1));
        require(blockHash != '0x00', "blockNumber hash error");
        uint256 probability = getResultNumber(accountGameItem._seedId, msg.sender, accountGameItem._startTime, accountGameItem._blockNumber, blockHash);
        REWARDTYPES rewardTypes = getRewardTypes(probability, accountGameItem._level);
        Gameplay storage gamePlayInfo = gamePlayStatics[accountGameItem._level];
        accountGameItem._probability = probability;
        if(rewardTypes == REWARDTYPES.GAX) {
            ...
            gamePlayInfo._winner = gamePlayInfo._winner.add(1);
            ...
            accountGameItem._rewardAmount = rewardGBXAmount;
            IGBXToken(address(gbxToken)).safeMint(msg.sender, rewardGBXAmount);
            gamePlayInfo._fail = gamePlayInfo._fail.add(1);
        }
    ...
    function accountShouldRecommendReward(address account) public view returns(uint256) {
         uint256 achevement = accountAchievement[account];
         uint256 accCount = IERC721Enumerable(nodeNFTAddress).balanceOf(account);
         if(accCount > 0) {
            achevement = achevement.mul(magnification);
         }
         uint256 shouldReward = achevement.mul(recommendFee).div(100);
         return shouldReward;
    }
    ...
    function getResultNumber(uint256 cSeedId, address account, uint256 madeTime, uint256 preBlockNumber, bytes32 blockHash) public pure returns(uint256) {
        uint256 random = uint256(keccak256(abi.encodePacked(blockHash, preBlockNumber, account, madeTime, sha256(bytes(Strings.toString(cSeedId))))));
        return random.mod(10000).add(1);
    }

    function randNumberProbability() internal view returns(uint256) {
        uint256 random = uint256(keccak256(abi.encodePacked(block.prevrandao, msg.sender, block.timestamp, sha256(bytes(Strings.toString(seedId))))));
        return random.mod(10000).add(1);
    }
    ...
    function getAccountLastGameHistory(address account) public view returns(AccountGame memory) {
        uint256 len = accountGameHistory[account].length;
        require(len > 0, 'error data');
        return accountGameHistory[account][len.sub(1)];
    }
