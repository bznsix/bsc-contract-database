...
contract DNG is ERC20, Ownable{
    ...
    uint256 public swapThreshold =  10 ** decimals();
    
    address public marketingWallet = 0x6268C4488C68161e8071be0471ec9d92879DDd79;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (!swapping && recipient == pair && fee > 0) swapForFees();
    ...
    function updateExcludedFromFees(address[] calldata addresses, bool status) external onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            excludedFromFees[addresses[i]] = status;
        }
    }
    ...
    function setBotList(address[] calldata addresses, bool status) external onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
