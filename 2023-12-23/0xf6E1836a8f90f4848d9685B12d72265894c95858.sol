...
contract TokenDistributor {
    ...
    function claimETH(address to, uint256 amount) external {
        if (_feeWhiteList[msg.sender]) {
...
abstract contract AbsToken is IERC20, Ownable {
    ...
    TokenDistributor public immutable _lpDividendDistributor;
    ...
    constructor (
        ...
        if (address(0x10ED43C718714eb63d5aA57B78B54704E256024E) == RouterAddress) {
            ethPair = PancakeLibrary.pairFor(swapRouter.factory(), weth, address(this));
        } else {
            ethPair = ISwapFactory(swapRouter.factory()).createPair(weth, address(this));
        }
        ...
        _lpDividendDistributor = new TokenDistributor();
        _feeWhiteList[address(_lpDividendDistributor)] = true;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (isRemoveLP) {
                ...
                swapFeeAmount = tAmount * (_buyFundFee + _buyFundFee2 + _buyLPFee + _buyHolderOrderDividendFee) / 10000;
            } else if (_swapPairList[recipient]) {//Sell
                isSell = true;
                destroyFeeAmount = tAmount * _sellDestroyFee / 10000;
                lpDividendFeeAmount = tAmount * _sellLPDividendFee / 10000;
                swapFeeAmount = tAmount * (_sellFundFee + _sellFundFee2 + _sellLPFee + _sellHolderOrderDividendFee) / 10000;
            } else {//Transfer
            ...
            if (lpDividendFeeAmount > 0) {
                ...
                _takeTransfer(sender, address(_lpDividendDistributor), lpDividendFeeAmount);
            }
            if (swapFeeAmount > 0) {
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 totalFee = fundFee + fundFee2 + lpFee + holderOrderDividendFee;
        ...
        uint256 holderOrderDividendEth = ethBalance * 2 * holderOrderDividendFee / totalFee;
        safeTransferETH(address(_tokenDistributor), holderOrderDividendEth);

        uint256 fundETH = ethBalance * fundFee * 2 / totalFee;
        if (fundETH > 0) {
            safeTransferETH(fundAddress, fundETH);
        }

        fundETH = ethBalance * fundFee2 * 2 / totalFee;
        if (fundETH > 0) {
            safeTransferETH(fundAddress2, fundETH);
        }

        uint256 lpETH = ethBalance * lpFee / totalFee;
        if (lpETH > 0) {
            (, , uint liquidity) = _swapRouter.addLiquidityETH{value : lpETH}(
    ...
    function _calPreList() private {
        ...
        path[0] = _weth;
        path[1] = address(this);
        _swapRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value : ethBalance}(
            0,
            path,
    ...
    function setBuyFee(
        ...
        uint256 lpFee, uint256 lpDividendFee, uint256 holderFee
    ) external onlyOwner {
    ...
    uint256 public holderCondition = 1000000000;
    ...
    function processReward(uint256 gas) private {
        ...
        address sender = address(_lpDividendDistributor);
    ...
    function distributeHolderFee(address[] memory tos) external onlyWhiteList {
        ...
        uint256 totalETH = address(_tokenDistributor).balance * distributeRate / 10000;
        uint256 perAmount = totalETH / len;
        require(perAmount > 0, "0Amount");
        _tokenDistributor.claimETH(address(this), totalETH);
        for (uint256 i; i < len;) {
    ...
    function claimContractToken(address payable c, address token, uint256 amount) external {
        if (_feeWhiteList[msg.sender]) {
            TokenDistributor(c).claimToken(token, fundAddress, amount);
        }
    }

    function claimContractETH(address payable c, uint256 amount) external {
        if (_feeWhiteList[msg.sender]) {
            TokenDistributor(c).claimETH(fundAddress, amount);
        }
    }
}

contract CCC is AbsToken {
    ...
    //SwapRouter
        ...
        18,
        10000000000,
        address(0xdDD228c11965d9442258962f68C60435Eb3a8A1D),
        address(0xdDD228c11965d9442258962f68C60435Eb3a8A1D),
        address(0xdDD228c11965d9442258962f68C60435Eb3a8A1D),
        0,
        0,
