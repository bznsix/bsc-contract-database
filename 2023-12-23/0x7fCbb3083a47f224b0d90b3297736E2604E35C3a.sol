...
contract ZLLToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxBuy = 5000000000 * (10**18);
    ...
    function _transfer(
        ...
        if (BuyBackAmount >= minBuyBackAmt && !swapping && from != uniswapV2Pair) {
            if (IERC20(bep20usdt).balanceOf(address(buyBackHelper)) >= BuyBackAmount) {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (from == uniswapV2Pair && !lpChange) { // Buy
                ...
                require(amount <= maxBuy, "ExceedsMaxPurchaseLimit");
                if (enableBuyBack) {
                    buyBack(amount);
                }
    ...
    function _distributeFee(uint256 feeAmount) internal {
        uint256 saveAmt = feeAmount.mul(10).div(100);
        uint256 burn = saveAmt.div(2);
        uint256 addLpToken = saveAmt.sub(burn);
        // burn
        super._transfer(address(this), address(0xdead), burn);

        // sell 
        IERC20 usdtToken = IERC20(bep20usdt);
        uint256 initalUSDT = usdtToken.balanceOf(address(tokenHelper));
        swapToken2USDT(feeAmount.sub(saveAmt), address(tokenHelper));
        uint256 sellAmount = usdtToken.balanceOf(address(tokenHelper)).sub(initalUSDT);

        uint256 receiveAmt = sellAmount.mul(75).div(90);
        tokenHelper.withdrawToken(bep20usdt, address(this), receiveAmt);

        uint256 addLpUsdt = receiveAmt;

        {
            uint256 group0Length = whiteGroup0.length();
            uint256 group0Amt = receiveAmt.mul(55).div(75);
            addLpUsdt = addLpUsdt.sub(group0Amt);
            if (group0Length > 0) {
                uint256 per = group0Amt.div(group0Length);
                for (uint i = 0; i < group0Length; i++) {
                    usdtToken.transfer(whiteGroup0.at(i), per);
                }
            }
        }

        {
            uint256 group1Length = whiteGroup1.length();
            uint256 group1Amt = receiveAmt.mul(15).div(75);
            addLpUsdt = addLpUsdt.sub(group1Amt);
            if (group1Length > 0) {
                uint256 per = group1Amt.div(group1Length);
                for (uint i = 0; i < group1Length; i++) {
                    usdtToken.transfer(whiteGroup1.at(i), per);
                }
            }
        }

        addLiquidityERC20(addLpToken, addLpUsdt);
    }
    ...
    function addLiquidityERC20(uint256 tokenAmount, uint256 erc20Amount) private {
        ...
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        IERC20(bep20usdt).approve(address(uniswapV2Router), erc20Amount);
    ...
    function whiteGroup0Add(address address_) public onlyOwner {
        require(whiteGroup0.length() <= 10, "cannot exceed 10 addresses");
    ...
    function whiteGroup1Add(address address_) public onlyOwner {
        require(whiteGroup1.length() <= 100, "cannot exceed 100 addresses");
