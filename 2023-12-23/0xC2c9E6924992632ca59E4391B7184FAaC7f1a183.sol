...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _buyBurnFee = 300;
    ...
    function _transfer(
        ...
        if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
            uint256 maxSellAmount = balance * 20999999 / 21000000;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (_swapPairList[sender]) {//Buy
                uint256 swapAmount = tAmount * _buyBurnFee / 10000;
                if (swapAmount > 0) {
                    feeAmount += swapAmount;
                    _takeTransfer(sender, address(this), swapAmount);
                }
            } else if (_swapPairList[recipient]) {//Sell
                uint256 swapAmount = tAmount * _sellBurnFee / 10000;
                if (swapAmount > 0) {
                    feeAmount += swapAmount;
                    _takeTransfer(sender, address(this), swapAmount);
                    if (!inSwap) {
    ...
    function setBuyFee(uint256 BurnFee) external onlyOwner {
        _buyBurnFee = BurnFee;
    }
...
contract BOOMS is AbsToken {
    ...
    //SwapRouter
        ...
        21000000,
    //Receive，接收
        address(0xF9C249129CE4f6f2035AE16384fc56A053E07db7),
    //Fund，营销
        address(0x6692Bf951215f973100fBaa7Abb9d2107696d391)
    ){
