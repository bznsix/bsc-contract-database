...
abstract contract LORDBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	// MAIN TOKEN PROPERTIES
	string private constant NAME = "Lord Of Lands";
	string private constant SYMBOL = "LORD";
	...
	uint8 private _poolFee; //The total fee to be taken and added to the pool on PCS TX, this includes all fees
    uint8 public _transferFee; //The total fee to be taken and added to the pool on non-PCS TX
	uint8 private _highBuyFee;

	uint256 private constant _totalTokens = 1 * 10**8 * 10**DECIMALS;	//total supply
    ...
    // Additional transfer restriction logic
    // Used to allow/deny trades prior to trading
    bool private _tradingOpen;

    // Whitelist can trade prior to open (bulksendering/airdrops)    
    mapping(address => bool) public _whiteList;

    // Blacklist wallets from trading
    // To be used only by external security calls through multi-signature wallets to prevent abuse
    mapping (address => bool) public _blacklist;

    // marketing and dev address
    // Multi-signature wallets only
    address private _marketingWallet = 0x37657A8F650ab5fc4229Ab67d4eE627ac42f5084;
    address private _devAddress = 0xEE624d87cA66f09db6ebEa2Aa2237D3c261a1921;
	...
	constructor (address routerAddress) {
		...
		// Initialize PancakeSwap V2 router and LORD <-> BNB pair.
		setPancakeSwapRouter(routerAddress);

		_maxHoldAmount = 1200000 * 10**DECIMALS;

		// 1% liquidity fee, 0% reward fee, 2% marketing fee, 2% dev fee
		setFees(1, 0, 2, 2);
	...
	function calculateFeeRate(address sender, address recipient) private view returns(uint256) {
		...
		if (applyFees) {
		    bool antiBotFlag = onBeforeCalculateFeeRate();
		    if (isPancakeSwapPair(sender) && antiBotFlag) {
		        return _highBuyFee;
		    }		    
			else if (isPancakeSwapPair(recipient) || isPancakeSwapPair(sender)) {
				return _poolFee;
			}
            else
            {
	...
	// This function swaps a {tokenAmount} of LORD tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for LORD -> WBNB
	...
	// Returns how many more $LORD tokens are needed in the contract before triggering a swap
	...
	function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
     ...
     function addToWhiteList(address _add, bool _val) public onlyOwner {
       
            _whiteList[_add] = _val;
        
    }


    // Only to be called at contract initialization or if necessary due to PCS issues, in which case 
    // the swap must be properly prepared for.
	function setPancakeSwapRouter(address routerAddress) public onlyOwner {
        ...
        address get_pair = IPancakeFactory(_pancakeswapV2Router.factory()).getPair(address(this),_pancakeswapV2Router.WETH());

        if (get_pair == address(0))
        {
            pancakePair = IPancakeFactory(_pancakeswapV2Router.factory()).createPair(address(this), _pancakeswapV2Router.WETH());
        }
        else
        {
	...
	function setFees(uint8 liquidityFee, uint8 rewardFee, uint8 marketingFee, uint8 devFee) public onlyOwner {
		require(liquidityFee <= 5, "Liquidity fee must be between 0% and 5%");
		require(rewardFee <= 10, "Reward fee must be between 0% and 10%");
        require(marketingFee <= 5, "Marketing fee must be between 0% and 5%");
        require(devFee <= 5, "Dev fee must be between 0% and 5%");
    ...
    // Transfer fee is the total percent taken from transfers not going to or from the PCS pool
    function setTransferFee(uint8 transferFee) public onlyOwner {
        require(transferFee <= 25, "Transfer fee must be between 0% and 25%");
    ...
    // Trading allows or denies all trades except owner
    function setTrading(bool open_) public onlyOwner {
        _tradingOpen = open_;
    }

    // Blacklist denies malicious actors from trading
    function setBlacklist(address add_, bool blacklist_) public onlyOwner {
        _blacklist[add_] = blacklist_;
    }
	...
	function setTokenSwapThreshold(uint256 threshold) public onlyOwner {
		require(threshold >  _totalTokens / 10000, "Threshold must be greater than 0");
	...
	function setAutoLiquidityWallet(address liquidityWallet) public onlyOwner {
         require(address(0) != liquidityWallet, "Address cannot be zero address");
	...
	function setMarketingWallet(address marketingWalletAddress) public onlyOwner {
        require(address(0) != marketingWalletAddress, "Address cannot be zero address");
	...
	function setDevWallet(address devWalletAddress) public onlyOwner {
        require(address(0) != devWalletAddress, "Address cannot be zero address");
...
contract LORD is LORDBase {
	...
	uint256 private _totalBNBAsLORDClaimed; // The total number of BNB that was converted to LORD and claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsLORDClaimed; // The amount of BNB converted to LORD and claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy LORD automatically
	...
	bool private _reimburseAfterLORDClaimFailure; // If true, and LORD reward claim portion fails, the portion will be given as BNB instead
	...
	constructor (address routerAddress) LORDBase(routerAddress) {
	...
	function onActivated() internal override {
		...
		setReimburseAfterLORDClaimFailure(true);
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough LORD balance");
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        // Claim LORD tokens
		if (!claimLORD(user, claimBnbAsTokens)) {
			// If token claim fails for any reason, award whole portion as BNB
			if (_reimburseAfterLORDClaimFailure) {
	...
	function claimLORD(address user, uint256 bnbAmount) private returns (bool) {
		if (bnbAmount == 0) {
			return true;
		}
		...
		_bnbAsLORDClaimed[user] += bnbAmount;
		_totalBNBAsLORDClaimed += bnbAmount;
		return true;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $LORD on a zero balance address:
		...
		// Depending on the % of $LORD tokens transferred, relative to the balance, we might need to extend the period
    ...
    function bnbRewardClaimedAsLORD(address byAddress) public view returns (uint256) {
		return _bnbAsLORDClaimed[byAddress];
	}
    ...
    function totalBNBClaimedAsLORD() public view returns (uint256) {
		return _totalBNBAsLORDClaimed;
	}
	...
	function reimburseAfterLORDClaimFailure() public view returns(bool) {
		return _reimburseAfterLORDClaimFailure;
	}


	function setReimburseAfterLORDClaimFailure(bool value) public onlyOwner {
		_reimburseAfterLORDClaimFailure = value;
	}
