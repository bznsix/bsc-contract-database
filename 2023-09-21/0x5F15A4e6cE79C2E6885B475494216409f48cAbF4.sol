...
contract WombatPlugin is IExchangePlugin, Ownable {
    ...
    function swap(
        ...
        amountReceivedTokenB = wombatRouter.swapExactTokensForTokens(
            tokenPath,
            poolPath,
            amountA,
            minAmountOut,
    ...
    function getPathsForTokenPair(
        address tokenA,
        address tokenB
    ) public view returns (address[] memory tokenPath, address[] memory poolPath) {
        (address token0, address token1) = sortTokens(tokenA, tokenB);
