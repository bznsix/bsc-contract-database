...
contract TheLastHeroi is Ownable, ReentrancyGuard 
{
    ...
    constructor() { currentToken = IERC20(0x55d398326f99059fF775485246999027B3197955); }
    ...
    function GameLastHero(uint256 amount, uint8 round) external nonReentrant
    {
        ...
        require(amount >= 1*10**18, "Min bid limit");
        require(round == LastHeroId, "Round the end");

        if(LastHero[LastHeroId].timestamp == 0)
        {
            // start ROUND
            Heroes.push(_msgSender());
            LastHero[LastHeroId].timestamp = block.timestamp + 120;
        ...
        else
        {
            ...
            LastHero[LastHeroId].timestamp += 120;
            LastHero[LastHeroId].bid = amount;
            LastHero[LastHeroId].bank += amount;
            Heroes.push(_msgSender());
            Bids++;
    ...
    function StartNewRound() external nonReentrant
    {
        ...
        require(LastHero[LastHeroId].timestamp > 0, "Dont active rounds"); // NEW
        ...
        currentToken.transfer(owner(), adminFee);
        LastHeroId++;
        delete Heroes;
        Bids = 0;
    }

    function _checkLastHero() public view returns(bool)
    {
