...
contract AnkrBNBPoolHelper is IHarvesttablePoolHelper, Ownable {
    ...
    function batchDepositLPFor(
        ...
        IERC20(stakingToken).safeApprove(masterMagpie, _lpAmount);

        for (uint256 i = 0; i < _users.length; i++) {
            if (_amounts[i] > 0) {
                lockedAmount[_users[i]] = _amounts[i];
                IMasterMagpie(masterMagpie).depositFor(stakingToken, _amounts[i], _users[i]);
                emit NewBatchDeposit(_users[i], _amounts[i]);
            }
        }
        
        IERC20(stakingToken).safeApprove(masterMagpie, 0);
    }
