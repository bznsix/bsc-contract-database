...
import { IStargateChef } from "../interfaces/stargate/IStargateChef.sol";
import { IStargateRouter } from "../interfaces/stargate/IStargateRouter.sol";
import "../common/BaseStrategy.sol";

// @dev Only works if the reward is STG token.
contract StrategyStargateUSDT is BaseStrategy {
  using SafeERC20 for IERC20;

  address public unirouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E; // default to pancake router
  // default constants
  address public constant STG = 0xB0D502E938ed5f4df2E681fE6E419ff29631d62b; // STG
  address public constant NATIVE = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
  IStargateRouter public constant STARGATE_ROUTER =
  ...
  constructor(
    ...
    // default route
    rewardTokenRoute = [STG, stargateAsset, NATIVE];
  ...
  function name() external view override returns (string memory) {
    return
      string(
        abi.encodePacked("StrategyStargateUSDT", IERC20Metadata(want).symbol())
      );
  }
  ...
  function _giveAllowanceRouter() internal {
    ...
    IERC20(STG).safeApprove(unirouter, type(uint256).max);
  ...
  function _removeAllowanceRouter() internal {
    IERC20(stargateAsset).safeApprove(address(STARGATE_ROUTER), 0);
    IERC20(STG).safeApprove(unirouter, 0);
  ...
  function setRewardRoute(address[] memory _rewardTokenRoute)
    ...
    onlyManager
  {
    _checkSwapPath(_rewardTokenRoute, STG, NATIVE);
  ...
  function _checkForWant(
    ...
    IStargateChef.PoolInfo memory pool = IStargateChef(_chef).poolInfo(_chefId);
  ...
  function _harvest() internal override returns (uint256, uint256) {
    _claimRewards();
    uint256 outputBal = IERC20(STG).balanceOf(address(this));
    if (outputBal > 0) {
      uint256 fee;
      if (publicHarvest) {
        fee = _chargeHarvestFee(outputBal, STG, msg.sender);
    ...
    returns (uint256)
  {
    ...
    uint256 govFeeAmount = (nativeBal * govFee) / totalFee;
...
abstract contract BaseStrategy is Ownable, Pausable {
  ...
  modifier onlyManager() {
    require(msg.sender == owner() || msg.sender == strategist, "Only managers");
    _;
  }
  ...
  function _onlyKeepers(address caller) internal view {
    if (publicHarvest) return;
    require(caller == strategist || caller == keeper, "Only keepers");
  }

  function setKeeper(address _keeper) external onlyManager {
    _keeper.validateAddress();
    keeper = _keeper;
  ...
  function setStrategist(address _strategist) external onlyOwner {
    _strategist.validateAddress();
    strategist = _strategist;
    emit StrategistChanged(strategist);
  }

  // also updates gov fee !!
  function setGovStrategistFee(uint256 _fee) external onlyOwner {
  ...
  function setTotalFee(uint256 _totalFee) external onlyOwner {
    require(_totalFee <= MAX_FEE, "MAX_FEE_CAP");
    totalFee = _totalFee;
    strategistFee = (_totalFee * 4_000) / 10_000; // back to default
    govFee = totalFee - strategistFee; // back to default
    emit FeesUpdated(_totalFee, strategistFee, govFee);
  }
  ...
  function setGovFeeRecipient(address _govFeeRecipient) external onlyOwner {
    _govFeeRecipient.validateAddress();
    govFeeRecipient = _govFeeRecipient;
  ...
  function withdraw(uint256 _amount) external onlyVault returns (uint256 wantWithdrawn) {
  ...
  function _harvest() internal virtual returns (uint256 wantHarvested, uint256 harvesterFee);

  function harvest() external {
    _onlyKeepers(msg.sender);
    (uint256 _wantHarvested, uint256 _harvesterFee) = _harvest();

    emit StrategyHarvested(msg.sender, _wantHarvested, _harvesterFee, block.timestamp);
  }

  // return the fee taken from the harvested rewards in terms of 'wnative' token (wbnb, wavax, weth etc)
  // Ideally, swap the entire reward tokens to wnative here and charge the fees
  // fee can be taken in terms of the reward token too, it's flexible.
  function _chargeFees(uint256 _tokensForFee) internal virtual returns (uint256 feeTaken);

  // Default harvester fee is the reward token * harvesterFee
  // override it for specific cases
  function _chargeHarvestFee(
    uint256 _rewards,
    address _token,
    address _recipient
  ) internal virtual returns (uint256) {
    uint256 _harvesterFee = (_rewards * harvesterFee) / 10_000;

    if (_harvesterFee > 0) {
      IERC20(_token).safeTransfer(_recipient, _harvesterFee);
    }

    return _harvesterFee;
  }
  ...
  function sweep(address _token) external onlyManager {
    require(_token != want, "can't sweep want");
